...
/** @file
 * Nemu Qt GUI - UIMessageCenter class implementation.
 */
...
void UIMessageCenter::cannotLoadGlobalConfig(const CVirtualBox &nemu, const QString &strError) const
{
    error(0, MessageType_Critical,
          tr("<p>Failed to load the global GUI configuration from <b><nobr>%1</nobr></b>.</p>"
             "<p>The application will now terminate.</p>")
             .arg(CVirtualBox(nemu).GetSettingsFilePath()),
          !nemu.isOk() ? formatErrorInfo(nemu) : QString("<!--EOM--><p>%1</p>").arg(nemuGlobal().emphasize(strError)));
}

void UIMessageCenter::cannotSaveGlobalConfig(const CVirtualBox &nemu) const
{
    error(0, MessageType_Critical,
          tr("<p>Failed to save the global GUI configuration to <b><nobr>%1</nobr></b>.</p>"
             "<p>The application will now terminate.</p>")
             .arg(CVirtualBox(nemu).GetSettingsFilePath()),
          formatErrorInfo(nemu));
}

void UIMessageCenter::cannotFindMachineByName(const CVirtualBox &nemu, const QString &strName) const
{
...
void UIMessageCenter::cannotFindMachineById(const CVirtualBox &nemu, const QString &strId) const
{
    error(0, MessageType_Error,
...
void UIMessageCenter::cannotSetExtraData(const CVirtualBox &nemu, const QString &strKey, const QString &strValue)
{
    error(0, MessageType_Error,
...
void UIMessageCenter::cannotOpenMachine(const CVirtualBox &nemu, const QString &strMachinePath) const
{
    error(0, MessageType_Error,
...
int UIMessageCenter::confirmMachineRemoval(const QList<CMachine> &machines) const
{
    ...
    foreach (const CMachine &machine, machines)
    {
        ...
        else
        {
            ...
            strMachineName = NemuGlobal::hasAllowedExtension(fi.completeSuffix(), NemuFileExts) ? fi.completeBaseName() : fi.fileName();
...
void UIMessageCenter::cannotCreateNATNetwork(const CVirtualBox &nemu, QWidget *pParent /* = 0*/)
{
    error(pParent, MessageType_Error,
...
void UIMessageCenter::cannotRemoveNATNetwork(const CVirtualBox &nemu, const QString &strNetworkName, QWidget *pParent /* = 0*/)
{
    error(pParent, MessageType_Error,
...
void UIMessageCenter::cannotCreateDHCPServer(const CVirtualBox &nemu, QWidget *pParent /* = 0*/)
{
    error(pParent, MessageType_Error,
...
void UIMessageCenter::cannotRemoveDHCPServer(const CVirtualBox &nemu, const QString &strInterfaceName, QWidget *pParent /* = 0*/)
{
    error(pParent, MessageType_Error,
...
bool UIMessageCenter::confirmMediumRelease(const UIMedium &medium, QWidget *pParent /* = 0*/) const
{
    ...
    {
        CMachine machine = nemu.FindMachine(strMachineID);
        if (!nemu.isOk() || machine.isNull())
            continue;
...
void UIMessageCenter::cannotOpenMedium(const CVirtualBox &nemu, UIMediumType /* type */, const QString &strLocation, QWidget *pParent /* = 0*/) const
{
    /* Show the error: */
    error(pParent, MessageType_Error,
          tr("Failed to open the disk image file <nobr><b>%1</b></nobr>.").arg(strLocation), formatErrorInfo(nemu));
}
...
void UIMessageCenter::cannotCreateMachine(const CVirtualBox &nemu, QWidget *pParent /* = 0*/) const
{
    error(pParent, MessageType_Error,
...
void UIMessageCenter::cannotRegisterMachine(const CVirtualBox &nemu, const QString &strMachineName, QWidget *pParent /* = 0*/) const
{
    error(pParent, MessageType_Error,
...
void UIMessageCenter::cannotCreateHardDiskStorage(const CVirtualBox &nemu, const QString &strLocation, QWidget *pParent /* = 0*/) const
{
    error(pParent, MessageType_Error,
...
void UIMessageCenter::showRuntimeError(const CConsole &console, bool fFatal, const QString &strErrorId, const QString &strErrorMsg) const
{
    ...
    if (!strErrorMsg.isEmpty())
        formatted.prepend(QString("<p>%1.</p>").arg(nemuGlobal().emphasize(strErrorMsg)));
...
bool UIMessageCenter::remindAboutGuruMeditation(const QString &strLogFolder)
{
    return questionBinary(0, MessageType_GuruMeditation,
                          tr("<p>A critical error has occurred while running the virtual "
                             ...
                             "log file <tt>Nemu.log</tt> and the image file <tt>Nemu.png</tt>, "
...
void UIMessageCenter::warnAboutNemuSVCUnavailable() const
{
    alert(0, MessageType_Critical,
          tr("<p>A critical error has occurred while running the virtual "
...
bool UIMessageCenter::confirmInputCapture(bool &fAutoConfirmed) const
{
    int rc = question(0, MessageType_Info,
                      ...
                      tr("<p>The host key is currently defined as <b>%1</b>.</p>", "additional message box paragraph")
                         .arg(UIHostCombo::toReadableString(nemuGlobal().settings().hostCombo())),
                      "confirmInputCapture",
                      AlertButton_Ok | AlertButtonOption_Default,
                      AlertButton_Cancel | AlertButtonOption_Escape,
                      0,
...
bool UIMessageCenter::confirmGoingFullscreen(const QString &strHotKey) const
{
    return questionBinary(0, MessageType_Info,
                          tr("<p>The virtual machine window will be now switched to <b>full-screen</b> mode. "
                             ...
                             .arg(strHotKey, UIHostCombo::toReadableString(nemuGlobal().settings().hostCombo())),
...
bool UIMessageCenter::cannotEnterFullscreenMode(ULONG /* uWidth */, ULONG /* uHeight */, ULONG /* uBpp */, ULONG64 uMinVRAM) const
{
    return questionBinary(0, MessageType_Warning,
                          tr("<p>Could not switch the guest display to full-screen mode due to insufficient guest video memory.</p>"
                             ...
                             .arg(NemuGlobal::formatSize(uMinVRAM)),
                          0 /* auto-confirm id */,
                          tr("Ignore"));
}
...
void UIMessageCenter::askUserToDownloadExtensionPack(const QString &strExtPackName, const QString &strExtPackVersion, const QString &strNemuVersion) const
{
    alert(windowManager().networkManagerOrMainWindowShown(), MessageType_Info,
          tr("<p>You have version %1 of the <b><nobr>%2</nobr></b> installed.</p>"
             ...
             .arg(strExtPackVersion).arg(strExtPackName).arg(strNemuVersion));
}
...
bool UIMessageCenter::confirmDownloadGuestAdditions(const QString &strUrl, qulonglong uSize) const
{
    return questionBinary(windowManager().networkManagerOrMainWindowShown(), MessageType_Question,
                          tr("<p>Are you sure you want to download the <b>VirtualBox Guest Additions</b> disk image file "
                             ...
                             .arg(strUrl, QLocale(NemuGlobal::languageId()).toString(uSize)),
                          0 /* auto-confirm id */,
                          tr("Download"));
}
...
bool UIMessageCenter::confirmDownloadExtensionPack(const QString &strExtPackName, const QString &strURL, qulonglong uSize) const
{
    return questionBinary(windowManager().networkManagerOrMainWindowShown(), MessageType_Question,
                          tr("<p>Are you sure you want to download the <b><nobr>%1</nobr></b> "
                             ...
                             .arg(strExtPackName, strURL, QLocale(NemuGlobal::languageId()).toString(uSize)),
                          0 /* auto-confirm id */,
                          tr("Download"));
}
...
{
    nemuGlobal().openURL("https://www.virtualbox.org");
}
...
void UIMessageCenter::sltShowHelpAboutDialog()
{
    ...
    if (nemuGlobal().brandingIsActive())
    {
        strFullVersion = QString("%1 r%2 - %3").arg(nemu.GetVersion())
                                               .arg(nemu.GetRevision())
                                               .arg(nemuGlobal().brandingGetKey("Name"));
    }
    else
    {
        strFullVersion = QString("%1 r%2").arg(nemu.GetVersion())
                                          .arg(nemu.GetRevision());
    }
    AssertWrapperOk(nemu);

    (new NemuAboutDlg(windowManager().mainWindowShown(), strFullVersion))->show();
}
...
void UIMessageCenter::sltShowHelpHelpDialog()
{
    ...
    sltShowUserManual(nemuGlobal().helpFile());
#else /* #ifndef NEMU_OSE */
    /* For OSE version we have to check if it present first: */
    QString strUserManualFileName1 = nemuGlobal().helpFile();
    QString strShortFileName = QFileInfo(strUserManualFileName1).fileName();
    QString strUserManualFileName2 = QDir(nemuGlobal().homeFolder()).absoluteFilePath(strShortFileName);
...
# else /* #ifndef NEMU_OSE */
    nemuGlobal().openURL("file://" + strLocation);
# endif /* #ifdef NEMU_OSE */
...
void UIMessageCenter::prepare()
{
    ...
    tr("VirtualBox is not currently allowed to access USB devices.  You can change this by adding your user to the 'nemuusers' group.  Please see the user manual for a more detailed explanation");
                                           ...
                                           HRESULT wrapperRC /* = S_OK */)
{
    ...
    if (!strDetailsInfo.isEmpty())
    {
        ...
        if (strDetailsInfo == QString::fromLatin1(strDetailsInfo.toLatin1()) &&
            ...
            formatted += QString("<p>%1.</p>").arg(nemuGlobal().emphasize(tr(strDetailsInfo.toLatin1().constData())));
        else
            formatted += QString("<p>%1.</p>").arg(nemuGlobal().emphasize(strDetailsInfo));
    }
