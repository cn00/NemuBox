/* $Id: NemuInternalManage.cpp $ */
/** @file
 * NemuManage - The 'internalcommands' command.
 *
 * NemuInternalManage used to be a second CLI for doing special tricks,
 * not intended for general usage, only for assisting Nemu developers.
 * It is now integrated into NemuManage.
 */
...
{
    RTStrmPrintf(pStrm,
        "Usage: NemuManage internalcommands <command> [command arguments]\n"
        "\n"
        "Commands:\n"
        "\n"
        ...
        ? "  createrawvmdk -filename <filename> -rawdisk <diskname>\n"
          ...
          "         NemuManage internalcommands listpartitions\n"
          "\n"
        : "",
...
static RTEXITCODE CmdLoadSyms(int argc, char **argv, ComPtr<IVirtualBox> aVirtualBox, ComPtr<ISession> aSession)
{
     ...
     */
    Utf8Str KeyStr;
    HRESULT hrc = NewUniqueKey(machine, "NemuInternal/DBGF/loadsyms", KeyStr);
    if (SUCCEEDED(hrc))
        hrc = SetString(machine, "NemuInternal/DBGF/loadsyms", KeyStr.c_str(), "Filename", pszFilename);
    if (SUCCEEDED(hrc) && argc >= 3)
        hrc = SetInt64(machine, "NemuInternal/DBGF/loadsyms", KeyStr.c_str(), "Delta", offDelta);
    if (SUCCEEDED(hrc) && argc >= 4)
        hrc = SetString(machine, "NemuInternal/DBGF/loadsyms", KeyStr.c_str(), "Module", pszModule);
    if (SUCCEEDED(hrc) && argc >= 5)
        hrc = SetUInt64(machine, "NemuInternal/DBGF/loadsyms", KeyStr.c_str(), "ModuleAddress", ModuleAddress);
    if (SUCCEEDED(hrc) && argc >= 6)
        hrc = SetUInt64(machine, "NemuInternal/DBGF/loadsyms", KeyStr.c_str(), "ModuleSize", ModuleSize);
...
static RTEXITCODE CmdLoadMap(int argc, char **argv, ComPtr<IVirtualBox> aVirtualBox, ComPtr<ISession> aSession)
{
     ...
     */
    Utf8Str KeyStr;
    HRESULT hrc = NewUniqueKey(machine, "NemuInternal/DBGF/loadmap", KeyStr);
    if (SUCCEEDED(hrc))
        hrc = SetString(machine, "NemuInternal/DBGF/loadmap", KeyStr.c_str(), "Filename", pszFilename);
    if (SUCCEEDED(hrc))
        hrc = SetUInt64(machine, "NemuInternal/DBGF/loadmap", KeyStr.c_str(), "Address", ModuleAddress);
    if (SUCCEEDED(hrc) && pszModule != NULL)
        hrc = SetString(machine, "NemuInternal/DBGF/loadmap", KeyStr.c_str(), "Name", pszModule);
    if (SUCCEEDED(hrc) && offSubtrahend != 0)
        hrc = SetUInt64(machine, "NemuInternal/DBGF/loadmap", KeyStr.c_str(), "Subtrahend", offSubtrahend);
    if (SUCCEEDED(hrc) && iSeg != UINT32_MAX)
        hrc = SetUInt64(machine, "NemuInternal/DBGF/loadmap", KeyStr.c_str(), "Segment", iSeg);
...
static RTEXITCODE CmdSetHDUUID(int argc, char **argv, ComPtr<IVirtualBox> aVirtualBox, ComPtr<ISession> aSession)
{
    ...
    rc = VDInterfaceAdd(&vdInterfaceError.Core, "NemuManage_IError", VDINTERFACETYPE_ERROR,
...
static int partRead(RTFILE File, PHOSTPARTITIONS pPart)
{
    ...
    NEMUHDDPARTTYPE partitioningType;
...
static PNEMUHDDRAWPARTDESC appendPartDesc(uint32_t *pcPartDescs, PNEMUHDDRAWPARTDESC *ppPartDescs)
{
    (*pcPartDescs)++;
    PNEMUHDDRAWPARTDESC p;
    p = (PNEMUHDDRAWPARTDESC)RTMemRealloc(*ppPartDescs,
                                          *pcPartDescs * sizeof(NEMUHDDRAWPARTDESC));
    *ppPartDescs = p;
    if (p)
    {
        p = p + *pcPartDescs - 1;
        memset(p, '\0', sizeof(NEMUHDDRAWPARTDESC));
    }
...
static RTEXITCODE CmdCreateRawVMDK(int argc, char **argv, ComPtr<IVirtualBox> aVirtualBox, ComPtr<ISession> aSession)
{
    ...
    if (!pszPartitions)
    {
        RawDescriptor.uFlags = NEMUHDDRAW_DISK;
        RawDescriptor.pszRawDisk = rawdisk.c_str();
    }
    else
    {
        RawDescriptor.uFlags = NEMUHDDRAW_NORMAL;
        ...
        for (unsigned i = 0; i < partitions.cPartitions; i++)
        {
            PNEMUHDDRAWPARTDESC pPartDesc = NULL;
            ...
            if (uPartitions & RT_BIT(partitions.aPartitions[i].uIndex))
            {
                if (uPartitionsRO & RT_BIT(partitions.aPartitions[i].uIndex))
                    pPartDesc->uFlags |= NEMUHDDRAW_READONLY;
...
#ifdef DEBUG_klaus
    if (!(RawDescriptor.uFlags & NEMUHDDRAW_DISK))
    {
...
static RTEXITCODE CmdRenameVMDK(int argc, char **argv, ComPtr<IVirtualBox> aVirtualBox, ComPtr<ISession> aSession)
{
    ...
    int vrc = VDInterfaceAdd(&vdInterfaceError.Core, "NemuManage_IError", VDINTERFACETYPE_ERROR,
                             NULL, sizeof(VDINTERFACEERROR), &pVDIfs);
    AssertRC(vrc);
...
static RTEXITCODE CmdConvertHardDisk(int argc, char **argv, ComPtr<IVirtualBox> aVirtualBox, ComPtr<ISession> aSession)
{
    ...
    Utf8Str dst;
    int vrc;
    PNEMUHDD pSrcDisk = NULL;
    PNEMUHDD pDstDisk = NULL;
    ...
    vrc = VDInterfaceAdd(&vdInterfaceError.Core, "NemuManage_IError", VDINTERFACETYPE_ERROR,
                         NULL, sizeof(VDINTERFACEERROR), &pVDIfs);
    AssertRC(vrc);
