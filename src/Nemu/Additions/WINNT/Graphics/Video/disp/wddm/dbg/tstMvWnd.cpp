...
#define nemuVDbgPrint(_m) do {} while (0)
...
//    {
//        case WM_CLOSE:
//            nemuVDbgPrint((__FUNCTION__": got WM_CLOSE for hwnd(0x%x)", hwnd));
//            return 0;
//        case WM_DESTROY:
//            nemuVDbgPrint((__FUNCTION__": got WM_DESTROY for hwnd(0x%x)", hwnd));
//            return 0;
//        case WM_NCHITTEST:
//            nemuVDbgPrint((__FUNCTION__": got WM_NCHITTEST for hwnd(0x%x)\n", hwnd));
//            return HTNOWHERE;
//    }
...
#define NEMUDISPWND_NAME L"tstMvWnd"
...
HRESULT tstMvWndCreate(DWORD w, DWORD h, HWND *phWnd)
{
    ...
    WNDCLASS wc;
    if (!GetClassInfo(hInstance, NEMUDISPWND_NAME, &wc))
    {
        wc.style = CS_OWNDC;
        ...
        wc.lpszClassName = NEMUDISPWND_NAME;
        if (!RegisterClass(&wc))
        {
            DWORD winErr = GetLastError();
            nemuVDbgPrint((__FUNCTION__": RegisterClass failed, winErr(%d)\n", winErr));
            hr = E_FAIL;
        }
    }
    ...
    if (hr == S_OK)
    {
        HWND hWnd = CreateWindowEx (0 /*WS_EX_CLIENTEDGE*/,
                                        NEMUDISPWND_NAME, NEMUDISPWND_NAME,
                                        WS_OVERLAPPEDWINDOW,
                                        0, 0,
                                        w, h,
        ...
        {
            DWORD winErr = GetLastError();
            nemuVDbgPrint((__FUNCTION__": CreateWindowEx failed, winErr(%d)\n", winErr));
            hr = E_FAIL;
        }
    }
