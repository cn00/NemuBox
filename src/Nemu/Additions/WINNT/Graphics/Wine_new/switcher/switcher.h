...
#define SW_FILLPROC(dispatch, hdll, name) \
    dispatch.p##name = ((hdll) != NULL) ? (name##Proc) GetProcAddress((hdll), #name) : nemu##name##Stub;

#define SW_DISPINIT(dispatch)                                   \
    {                                                           \
        if (!dispatch.initialized)                              \
        {                                                       \
            InitD3DExports(dispatch.nemuName, dispatch.msName); \
...
extern BOOL IsNemu3DEnabled(void);
extern BOOL CheckOptions(void);
extern void FillD3DExports(HANDLE hDLL);
extern void InitD3DExports(const char *nemuName, const char *msName);
