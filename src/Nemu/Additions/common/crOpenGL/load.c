...
#include <d3d9types.h>
#include <D3dumddi.h>
#include "../../WINNT/Graphics/Video/common/wddm/NemuMPIf.h"
#endif
...
#if defined(WINDOWS) && defined(NEMU_WITH_WDDM)
...
static HRGN stubMakeRegionFromRects(PNEMUVIDEOCM_CMD_RECTS pRegions, uint32_t start)
{
    HRGN hRgn, hTmpRgn;
    uint32_t i;
...
static void stubSyncTrCheckWindowsCB(unsigned long key, void *data1, void *data2)
{
    ...
    stub.spu->dispatch_table.NemuPackSetInjectID(pWindow->u32ClientID);
...
# ifdef NEMU_WITH_WDDM
    hNemuD3D = NULL;
    if (!GetModuleHandleEx(0, NEMU_MODNAME_DISPD3D, &hNemuD3D))
    {
    ...
    if (hNemuD3D)
    {
                    crDebug("running with " NEMU_MODNAME_DISPD3D);
                    stub.trackWindowVisibleRgn = 0;
...
# endif /* NEMU_WITH_WDDM */
    ...
    spuConnection =
#endif
            stub.spu->dispatch_table.NemuPackSetInjectThread(NULL);
#if defined(WINDOWS) && defined(NEMU_WITH_WDDM)
    if (stub.bRunningUnderWDDM && !spuConnection)
    {
        crError("NemuPackSetInjectThread failed!");
    }
#endif
    crUnlockMutex(&stub.mutex);
...
# endif
    ...
    if (spuConnection)
    {
        stub.spu->dispatch_table.NemuConDestroy(spuConnection);
    }
    if (hNemuD3D)
    {
        FreeLibrary(hNemuD3D);
    }
#endif
...
stubInitLocked(void)
{
    ...
    {
        disable_sync = 1;
        crDebug("running with %s", NEMU_MODNAME_DISPD3D);
        stub.trackWindowVisibleRgn = 0;
...
# include <dbghelp.h>
# include <cr_string.h>
static PVOID g_NemuVehHandler = NULL;
static DWORD g_NemuVehEnable = 0;

/* generate a crash dump on exception */
#define NEMUVEH_F_DUMP 0x00000001
/* generate a debugger breakpoint exception */
#define NEMUVEH_F_BREAK 0x00000002
/* exit on exception */
#define NEMUVEH_F_EXIT  0x00000004

static DWORD g_NemuVehFlags = 0;

typedef BOOL WINAPI FNNEMUDBG_MINIDUMPWRITEDUMP(HANDLE hProcess,
            DWORD ProcessId,
            HANDLE hFile,
...
typedef FNNEMUDBG_MINIDUMPWRITEDUMP *PFNNEMUDBG_MINIDUMPWRITEDUMP;

static HMODULE g_hNemuMdDbgHelp = NULL;
static PFNNEMUDBG_MINIDUMPWRITEDUMP g_pfnNemuMdMiniDumpWriteDump = NULL;
static size_t g_cNemuMdFilePrefixLen = 0;
static WCHAR g_aszwNemuMdFilePrefix[MAX_PATH];
static WCHAR g_aszwNemuMdDumpCount = 0;
static MINIDUMP_TYPE g_enmNemuMdDumpType = MiniDumpNormal
...
#define NEMUMD_DUMP_DIR_DEFAULT "C:\\dumps"
#define NEMUMD_DUMP_NAME_PREFIX_W L"NemuDmp_"
...
static DWORD nemuMdMinidumpCreate(struct _EXCEPTION_POINTERS *pExceptionInfo)
{
    ...
    if (!g_pfnNemuMdMiniDumpWriteDump)
    {
        if (!g_hNemuMdDbgHelp)
        {
            g_hNemuMdDbgHelp = loadSystemDll("DbgHelp.dll");
            if (!g_hNemuMdDbgHelp)
                return GetLastError();
        }

        g_pfnNemuMdMiniDumpWriteDump = (PFNNEMUDBG_MINIDUMPWRITEDUMP)GetProcAddress(g_hNemuMdDbgHelp, "MiniDumpWriteDump");
        if (!g_pfnNemuMdMiniDumpWriteDump)
            return GetLastError();
    }

    ++g_aszwNemuMdDumpCount;

    memcpy(aszwMdFileName, g_aszwNemuMdFilePrefix, g_cNemuMdFilePrefixLen * sizeof (g_aszwNemuMdFilePrefix[0]));
    swprintf(aszwMdFileName + g_cNemuMdFilePrefixLen, RT_ELEMENTS(aszwMdFileName) - g_cNemuMdFilePrefixLen, L"%d_%d.dmp", ProcessId, g_aszwNemuMdDumpCount);
    ...
    if (!g_pfnNemuMdMiniDumpWriteDump(hProcess, ProcessId, hFile, g_enmNemuMdDumpType, &ExceptionInfo, NULL, NULL))
        winErr = GetLastError();
...
LONG WINAPI nemuVDbgVectoredHandler(struct _EXCEPTION_POINTERS *pExceptionInfo)
{
    ...
    switch (pExceptionRecord->ExceptionCode)
    {
        ...
        case EXCEPTION_ILLEGAL_INSTRUCTION:
            if (g_NemuVehFlags & NEMUVEH_F_BREAK)
            {
                BOOL fBreak = TRUE;
#ifndef DEBUG_misha
            ...
            if (g_NemuVehFlags & NEMUVEH_F_DUMP)
                nemuMdMinidumpCreate(pExceptionInfo);

            if (g_NemuVehFlags & NEMUVEH_F_EXIT)
                exit(1);
            break;
        default:
            break;
...
{
    CRASSERT(!g_NemuVehHandler);
    g_NemuVehHandler = AddVectoredExceptionHandler(1,nemuVDbgVectoredHandler);
    CRASSERT(g_NemuVehHandler);
}
...
void nemuVDbgVEHandlerUnregister()
{
    ...
    if (g_NemuVehHandler)
    {
        uResult = RemoveVectoredExceptionHandler(g_NemuVehHandler);
        CRASSERT(uResult);
        g_NemuVehHandler = NULL;
    }
}
#endif
...
BOOL WINAPI DllMain(HINSTANCE hDLLInst, DWORD fdwReason, LPVOID lpvReserved)
{
    ...
    case DLL_PROCESS_ATTACH:
    {
        ...
        hCrUtil = GetModuleHandleA("NemuOGLcrutil.dll");
        Assert(hCrUtil);
        crDbgCmdSymLoadPrint("NemuOGLcrutil.dll", hCrUtil);
#endif
#ifdef CHROMIUM_THREADSAFE
        ...
        g_NemuVehEnable = crStrParseI32(env,
# ifdef DEBUG_misha
                1
# else
                0
# endif
        ...
        if (g_NemuVehEnable)
        {
            ...
            g_NemuVehFlags = crStrParseI32(env,
                    0
# ifdef DEBUG_misha
                    | NEMUVEH_F_BREAK
# else
...
# endif
            ...
            if (!env)
                env = NEMUMD_DUMP_DIR_DEFAULT;

            g_cNemuMdFilePrefixLen = strlen(env);

            if (RT_ELEMENTS(g_aszwNemuMdFilePrefix) <= g_cNemuMdFilePrefixLen + 26 + (sizeof (NEMUMD_DUMP_NAME_PREFIX_W) - sizeof (WCHAR)) / sizeof (WCHAR))
            {
                g_cNemuMdFilePrefixLen = 0;
                env = "";
            }

            mbstowcs_s(&cChars, g_aszwNemuMdFilePrefix, g_cNemuMdFilePrefixLen + 1, env, _TRUNCATE);

            Assert(cChars == g_cNemuMdFilePrefixLen + 1);

            g_cNemuMdFilePrefixLen = cChars - 1;

            if (g_cNemuMdFilePrefixLen && g_aszwNemuMdFilePrefix[g_cNemuMdFilePrefixLen - 1] != L'\\')
                g_aszwNemuMdFilePrefix[g_cNemuMdFilePrefixLen++] = L'\\';

            memcpy(g_aszwNemuMdFilePrefix + g_cNemuMdFilePrefixLen, NEMUMD_DUMP_NAME_PREFIX_W, sizeof (NEMUMD_DUMP_NAME_PREFIX_W) - sizeof (WCHAR));
            g_cNemuMdFilePrefixLen += (sizeof (NEMUMD_DUMP_NAME_PREFIX_W) - sizeof (WCHAR)) / sizeof (WCHAR);

            crGetProcName(procName, RT_ELEMENTS(procName));
            cProcName = strlen(procName);

            if (RT_ELEMENTS(g_aszwNemuMdFilePrefix) > g_cNemuMdFilePrefixLen + cProcName + 1 + 26)
            {
                mbstowcs_s(&cChars, g_aszwNemuMdFilePrefix + g_cNemuMdFilePrefixLen, cProcName + 1, procName, _TRUNCATE);
                Assert(cChars == cProcName + 1);
                g_cNemuMdFilePrefixLen += cChars - 1;
                g_aszwNemuMdFilePrefix[g_cNemuMdFilePrefixLen++] = L'_';
            }

            /* sanity */
            g_aszwNemuMdFilePrefix[g_cNemuMdFilePrefixLen] = L'\0';

            env = crGetenv("CR_DBG_VEH_DUMP_TYPE");

            g_enmNemuMdDumpType = crStrParseI32(env,
                    MiniDumpNormal
                    | MiniDumpWithDataSegs
                    | MiniDumpWithFullMemory
                    | MiniDumpWithHandleData
    ...
    case DLL_PROCESS_DETACH:
    {
        ...
        {
            CRASSERT(stub.spu);
            stub.spu->dispatch_table.NemuDetachThread();
        }
    ...
    case DLL_THREAD_ATTACH:
    {
        ...
        {
            CRASSERT(stub.spu);
            stub.spu->dispatch_table.NemuAttachThread();
        }
        break;
    }
