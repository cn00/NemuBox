/* $Id: VD.cpp $ */
/** @file
 * NemuHDD - Nemu HDD Container implementation.
 */
 ...
 */
#if !defined(NEMU_HDD_NO_DYNAMIC_BACKENDS) && !defined(RT_ARCH_X86) && !defined(RT_ARCH_AMD64)
# define NEMU_HDD_NO_DYNAMIC_BACKENDS
#endif

#define NEMUHDDDISK_SIGNATURE 0x6f0e2a7d
...
/**
 * Nemu HDD Container image descriptor.
 */
...
/**
 * Nemu HDD Cache image descriptor.
 */
...
/**
 * Nemu HDD Container main structure, private part.
 */
struct NEMUHDD
{
    /** Structure signature (NEMUHDDDISK_SIGNATURE). */
...
/**
 * Nemu parent read descriptor, used internally for compaction.
 */
...
static void vdDiskProcessBlockedIoCtx(PNEMUHDD pDisk);
static int vdDiskUnlock(PNEMUHDD pDisk, PVDIOCTX pIoCtxRc);
static void vdIoCtxSyncComplete(void *pvUser1, void *pvUser2, int rcReq);

/**
 * internal: add several backends.
 */
static int vdAddBackends(RTLDRMOD hPlugin, PCNEMUHDDBACKEND *ppBackends, unsigned cBackends)
{
    PCNEMUHDDBACKEND *pTmp = (PCNEMUHDDBACKEND*)RTMemRealloc(g_apBackends,
           (g_cBackends + cBackends) * sizeof(PCNEMUHDDBACKEND));
    if (RT_UNLIKELY(!pTmp))
    ...
    memcpy(&g_apBackends[g_cBackends], ppBackends, cBackends * sizeof(PCNEMUHDDBACKEND));
 ...
 */
DECLINLINE(int) vdAddBackend(RTLDRMOD hPlugin, PCNEMUHDDBACKEND pBackend)
{
 ...
 */
static int vdError(PNEMUHDD pDisk, int rc, RT_SRC_POS_DECL,
 ...
 */
DECLINLINE(int) vdThreadStartRead(PNEMUHDD pDisk)
{
    int rc = VINF_SUCCESS;
 ...
 */
DECLINLINE(int) vdThreadFinishRead(PNEMUHDD pDisk)
{
    int rc = VINF_SUCCESS;
    if (RT_UNLIKELY(pDisk->pInterfaceThreadSync))
 ...
 */
DECLINLINE(int) vdThreadStartWrite(PNEMUHDD pDisk)
{
    int rc = VINF_SUCCESS;
    if (RT_UNLIKELY(pDisk->pInterfaceThreadSync))
 ...
 */
DECLINLINE(int) vdThreadFinishWrite(PNEMUHDD pDisk)
{
    int rc = VINF_SUCCESS;
    if (RT_UNLIKELY(pDisk->pInterfaceThreadSync))
 ...
 */
static int vdFindBackend(const char *pszBackend, PCNEMUHDDBACKEND *ppBackend)
{
    int rc = VINF_SUCCESS;
    PCNEMUHDDBACKEND pBackend = NULL;
 ...
 */
static void vdAddImageToList(PNEMUHDD pDisk, PVDIMAGE pImage)
{
    pImage->pPrev = NULL;
    pImage->pNext = NULL;
 ...
 */
static void vdRemoveImageFromList(PNEMUHDD pDisk, PVDIMAGE pImage)
{
    Assert(pDisk->cImages > 0);
 ...
 */
static PVDIMAGE vdGetImageByNumber(PNEMUHDD pDisk, unsigned nImage)
{
 ...
 */
static int vdFilterChainApplyWrite(PNEMUHDD pDisk, uint64_t uOffset, size_t cbWrite,
                                   PVDIOCTX pIoCtx)
{
    int rc = VINF_SUCCESS;
 ...
 * @param   pIoCtx   The I/O context associated with the request.
 */
static int vdFilterChainApplyRead(PNEMUHDD pDisk, uint64_t uOffset, size_t cbRead,
                                  PVDIOCTX pIoCtx)
{
    int rc = VINF_SUCCESS;
...
DECLINLINE(void) vdIoCtxRootComplete(PNEMUHDD pDisk, PVDIOCTX pIoCtx)
{
 ...
 */
DECLINLINE(void) vdIoCtxInit(PVDIOCTX pIoCtx, PNEMUHDD pDisk, VDIOCTXTXDIR enmTxDir,
 ...
 * @param   pIoCtx     The I/O context to rite from.
 ...
 */
static int vdDiscardRemoveBlocks(PNEMUHDD pDisk, PVDDISCARDSTATE pDiscard, size_t cbDiscardingNew)
{
    int rc = VINF_SUCCESS;
 ...
 * @param   pDisk    VD disk container.
 */
static int vdDiscardStateDestroy(PNEMUHDD pDisk)
{
    int rc = VINF_SUCCESS;
 ...
 */
static int vdDiscardSetRangeAllocated(PNEMUHDD pDisk, uint64_t uOffset, size_t cbRange)
{
...
DECLINLINE(PVDIOCTX) vdIoCtxAlloc(PNEMUHDD pDisk, VDIOCTXTXDIR enmTxDir,
...
DECLINLINE(PVDIOCTX) vdIoCtxRootAlloc(PNEMUHDD pDisk, VDIOCTXTXDIR enmTxDir,
                                      uint64_t uOffset, size_t cbTransfer,
                                      PVDIMAGE pImageStart, PCRTSGBUF pcSgBuf,
                                      PFNVDASYNCTRANSFERCOMPLETE pfnComplete,
...
DECLINLINE(void) vdIoCtxDiscardInit(PVDIOCTX pIoCtx, PNEMUHDD pDisk, PCRTRANGE paRanges,
...
DECLINLINE(PVDIOCTX) vdIoCtxDiscardAlloc(PNEMUHDD pDisk, PCRTRANGE paRanges,
                                         unsigned cRanges,
                                         PFNVDASYNCTRANSFERCOMPLETE pfnComplete,
                                         void *pvUser1, void *pvUser2,
                                         void *pvAllocation,
...
DECLINLINE(PVDIOCTX) vdIoCtxChildAlloc(PNEMUHDD pDisk, VDIOCTXTXDIR enmTxDir,
                                       uint64_t uOffset, size_t cbTransfer,
                                       PVDIMAGE pImageStart, PCRTSGBUF pcSgBuf,
...
DECLINLINE(void) vdIoCtxFree(PNEMUHDD pDisk, PVDIOCTX pIoCtx)
{
...
DECLINLINE(void) vdIoTaskFree(PNEMUHDD pDisk, PVDIOTASK pIoTask)
{
#ifdef DEBUG
...
DECLINLINE(void) vdIoCtxDefer(PNEMUHDD pDisk, PVDIOCTX pIoCtx)
{
 ...
 *
 * @returns Nemu status code, only valid if pIoCtxRc is not NULL, treat as void
 *          function otherwise.
 ...
 */
static int vdDiskProcessWaitingIoCtx(PNEMUHDD pDisk, PVDIOCTX pIoCtxRc)
{
 ...
 * for it to complete.
 *
 * @returns Nemu status code of the completed request.
...
DECLINLINE(bool) vdIoCtxIsDiskLockOwner(PNEMUHDD pDisk, PVDIOCTX pIoCtx)
{
    return pDisk->pIoCtxLockOwner == pIoCtx;
}

static int vdIoCtxLockDisk(PNEMUHDD pDisk, PVDIOCTX pIoCtx)
{
    int rc = VINF_SUCCESS;
...
static void vdIoCtxUnlockDisk(PNEMUHDD pDisk, PVDIOCTX pIoCtx, bool fProcessBlockedReqs)
{
 ...
 **/
static int vdDiskReadHelper(PNEMUHDD pDisk, PVDIMAGE pImage, PVDIMAGE pImageParentOverride,
 ...
 */
static int vdReadHelperEx(PNEMUHDD pDisk, PVDIMAGE pImage, PVDIMAGE pImageParentOverride,
 ...
 * will give us.
 */
static int vdReadHelper(PNEMUHDD pDisk, PVDIMAGE pImage, uint64_t uOffset,
 ...
 */
static void vdResetModifiedFlag(PNEMUHDD pDisk)
{
 ...
 */
static void vdSetModifiedFlag(PNEMUHDD pDisk)
{
    pDisk->uModified |= VD_IMAGE_MODIFIED_FLAG;
 ...
 * write optimizations.
 */
static int vdWriteHelperEx(PNEMUHDD pDisk, PVDIMAGE pImage,
 ...
 * write optimizations.
 */
static int vdWriteHelper(PNEMUHDD pDisk, PVDIMAGE pImage, uint64_t uOffset,
 ...
 */
static int vdCopyHelper(PNEMUHDD pDiskFrom, PVDIMAGE pImageFrom, PNEMUHDD pDiskTo,
 ...
 */
static int vdSetModifiedFlagAsync(PNEMUHDD pDisk, PVDIOCTX pIoCtx)
{
    int rc = VINF_SUCCESS;
 ...
 *                             The number of bytes waiting can be less.
 */
static int vdDiscardRemoveBlocksAsync(PNEMUHDD pDisk, PVDIOCTX pIoCtx, size_t cbDiscardingNew)
{
    int rc = VINF_SUCCESS;
    PVDDISCARDSTATE pDiscard = pDisk->pDiscard;
...
#ifndef NEMU_HDD_NO_DYNAMIC_BACKENDS

/**
 * @interface_method_impl{VDBACKENDREGISTER,pfnRegisterImage}
 */
static DECLCALLBACK(int) vdPluginRegisterImage(void *pvUser, PCNEMUHDDBACKEND pBackend)
{
    int rc = VINF_SUCCESS;

    if (pBackend->cbSize == sizeof(NEMUHDDBACKEND))
...
static int vdRemovePlugin(const char *pszFilename)
{
    ...
    for (unsigned i = 0; i < g_cBackends; i++)
    {
        ...
        {
            memcpy(&g_apBackends[i], &g_apBackends[i + 1], (g_cBackends - i - 1) * sizeof(PCNEMUHDDBACKEND));
    ...
    for (unsigned i = 0; i < g_cCacheBackends; i++)
    {
        ...
        {
            memcpy(&g_apCacheBackends[i], &g_apCacheBackends[i + 1], (g_cCacheBackends - i - 1) * sizeof(PCNEMUHDDBACKEND));
    ...
    for (unsigned i = 0; i < g_cFilterBackends; i++)
    {
        ...
        {
            memcpy(&g_apFilterBackends[i], &g_apFilterBackends[i + 1], (g_cFilterBackends - i - 1) * sizeof(PCNEMUHDDBACKEND));
...
#endif /* !NEMU_HDD_NO_DYNAMIC_BACKENDS */
...
{
#ifndef NEMU_HDD_NO_DYNAMIC_BACKENDS
    /* To get all entries with NemuHDD as prefix. */
...
{
    PNEMUHDD pDisk = pIoStorage->pVDIo->pDisk;
    RTLISTNODE ListIoCtxWaiting;
    bool fFlush;
 ...
 */
static void vdIoTaskProcessWaitingList(PNEMUHDD pDisk)
{
    LogFlowFunc(("pDisk=%#p\n", pDisk));
 ...
 */
static void vdIoCtxProcessHaltedList(PNEMUHDD pDisk)
{
    LogFlowFunc(("pDisk=%#p\n", pDisk));
...
DECLINLINE(int) vdMessageWrapper(PNEMUHDD pDisk, const char *pszFormat, ...)
{
    va_list va;
    va_start(va, pszFormat);
...
static void vdIoCtxSyncComplete(void *pvUser1, void *pvUser2, int rcReq)
{
    PNEMUHDD pDisk = (PNEMUHDD)pvUser1;
 ...
 * @returns Nemu status code.
 */
NEMUDDU_DECL(int) VDShutdown(void)
{
    PCNEMUHDDBACKEND *pBackends = g_apBackends;
 ...
 * @param   pszFilename     The plugin filename to load.
 */
NEMUDDU_DECL(int) VDPluginLoadFromFilename(const char *pszFilename)
{
    if (!g_apBackends)
    {
        int rc = VDInit();
 ...
 *
 * @returns Nemu statuse code.
 * @param   pszPath         The path to load plugins from.
 */
NEMUDDU_DECL(int) VDPluginLoadFromPath(const char *pszPath)
{
    if (!g_apBackends)
    {
        int rc = VDInit();
 ...
 * @param   pszFilename     The plugin filename to unload.
 */
NEMUDDU_DECL(int) VDPluginUnloadFromFilename(const char *pszFilename)
{
    if (!g_apBackends)
    {
        int rc = VDInit();
 ...
 * @param   pszPath         The path to unload plugins from.
 */
NEMUDDU_DECL(int) VDPluginUnloadFromPath(const char *pszPath)
{
    if (!g_apBackends)
    {
        int rc = VDInit();
 ...
 */
NEMUDDU_DECL(int) VDBackendInfo(unsigned cEntriesAlloc, PVDBACKENDINFO pEntries,
 ...
 */
NEMUDDU_DECL(int) VDBackendInfoOne(const char *pszBackend, PVDBACKENDINFO pEntry)
{
 ...
 * @param   pcEntriesUsed   Number of entries returned.
 */
NEMUDDU_DECL(int) VDFilterInfo(unsigned cEntriesAlloc, PVDFILTERINFO pEntries,
                               unsigned *pcEntriesUsed)
{
    int rc = VINF_SUCCESS;
    unsigned cEntries = 0;
 ...
 * @param   pEntries        Pointer to an entry.
 */
NEMUDDU_DECL(int) VDFilterInfoOne(const char *pszFilter, PVDFILTERINFO pEntry)
{
 ...
 */
NEMUDDU_DECL(int) VDCreate(PVDINTERFACE pVDIfsDisk, VDTYPE enmType, PNEMUHDD *ppDisk)
{
    int rc = VINF_SUCCESS;
    PNEMUHDD pDisk = NULL;
    ...
    do
    {
        ...
        pDisk = (PNEMUHDD)RTMemAllocZ(sizeof(NEMUHDD));
        if (pDisk)
        {
            pDisk->u32Signature            = NEMUHDDDISK_SIGNATURE;
 ...
 */
NEMUDDU_DECL(int) VDDestroy(PNEMUHDD pDisk)
{
    int rc = VINF_SUCCESS;
    LogFlowFunc(("pDisk=%#p\n", pDisk));
    do
    {
        ...
        AssertMsg(pDisk->u32Signature == NEMUHDDDISK_SIGNATURE, ("u32Signature=%08x\n", pDisk->u32Signature));
 ...
 */
NEMUDDU_DECL(int) VDGetFormat(PVDINTERFACE pVDIfsDisk, PVDINTERFACE pVDIfsImage,
 ...
 * @param   pVDIfsImage     Pointer to the per-image VD interface list.
 */
NEMUDDU_DECL(int) VDOpen(PNEMUHDD pDisk, const char *pszBackend,
 ...
 */
NEMUDDU_DECL(int) VDCacheOpen(PNEMUHDD pDisk, const char *pszBackend,
                              const char *pszFilename, unsigned uOpenFlags,
                              PVDINTERFACE pVDIfsCache)
{
    int rc = VINF_SUCCESS;
...
NEMUDDU_DECL(int) VDFilterAdd(PNEMUHDD pDisk, const char *pszFilter, uint32_t fFlags,
                              PVDINTERFACE pVDIfsFilter)
{
    int rc = VINF_SUCCESS;
    int rc2;
 ...
 */
NEMUDDU_DECL(int) VDCreateBase(PNEMUHDD pDisk, const char *pszBackend,
 ...
 * @param   pVDIfsOperation Pointer to the per-operation VD interface list.
 */
NEMUDDU_DECL(int) VDCreateDiff(PNEMUHDD pDisk, const char *pszBackend,
 ...
 * @param   pVDIfsOperation Pointer to the per-operation VD interface list.
 */
NEMUDDU_DECL(int) VDCreateCache(PNEMUHDD pDisk, const char *pszBackend,
                                const char *pszFilename, uint64_t cbSize,
                                unsigned uImageFlags, const char *pszComment,
                                PCRTUUID pUuid, unsigned uOpenFlags,
 ...
 * @param   pVDIfsOperation Pointer to the per-operation VD interface list.
 */
NEMUDDU_DECL(int) VDMerge(PNEMUHDD pDisk, unsigned nImageFrom,
 ...
 */
NEMUDDU_DECL(int) VDCopyEx(PNEMUHDD pDiskFrom, unsigned nImage, PNEMUHDD pDiskTo,
                           ...
                           PVDINTERFACE pDstVDIfsOperation)
{
    ...
    do {
        ...
        AssertMsgBreakStmt(VALID_PTR(pDiskFrom), ("pDiskFrom=%#p\n", pDiskFrom),
                           rc = VERR_INVALID_PARAMETER);
        AssertMsg(pDiskFrom->u32Signature == NEMUHDDDISK_SIGNATURE,
        ...
        AssertMsgBreakStmt(VALID_PTR(pDiskTo), ("pDiskTo=%#p\n", pDiskTo),
                           rc = VERR_INVALID_PARAMETER);
        AssertMsg(pDiskTo->u32Signature == NEMUHDDDISK_SIGNATURE,
 ...
 */
NEMUDDU_DECL(int) VDCopy(PNEMUHDD pDiskFrom, unsigned nImage, PNEMUHDD pDiskTo,
                         const char *pszBackend, const char *pszFilename,
                         bool fMoveByRename, uint64_t cbSize,
                         unsigned uImageFlags, PCRTUUID pDstUuid,
                         unsigned uOpenFlags, PVDINTERFACE pVDIfsOperation,
 ...
 * @param   pVDIfsOperation Pointer to the per-operation VD interface list.
 */
NEMUDDU_DECL(int) VDCompact(PNEMUHDD pDisk, unsigned nImage,
                            PVDINTERFACE pVDIfsOperation)
{
    int rc = VINF_SUCCESS;
    int rc2;
    ...
    do {
        ...
        AssertMsgBreakStmt(VALID_PTR(pDisk), ("pDisk=%#p\n", pDisk),
                           rc = VERR_INVALID_PARAMETER);
        AssertMsg(pDisk->u32Signature == NEMUHDDDISK_SIGNATURE,
                  ("u32Signature=%08x\n", pDisk->u32Signature));
 ...
 * @param   pVDIfsOperation Pointer to the per-operation VD interface list.
 */
NEMUDDU_DECL(int) VDResize(PNEMUHDD pDisk, uint64_t cbSize,
                           PCVDGEOMETRY pPCHSGeometry,
                           PCVDGEOMETRY pLCHSGeometry,
                           PVDINTERFACE pVDIfsOperation)
{
...
NEMUDDU_DECL(int) VDPrepareWithFilters(PNEMUHDD pDisk, PVDINTERFACE pVDIfsOperation)
{
    int rc = VINF_SUCCESS;
    int rc2;
    bool fLockRead = false, fLockWrite = false;
 ...
 */
NEMUDDU_DECL(int) VDClose(PNEMUHDD pDisk, bool fDelete)
{
    int rc = VINF_SUCCESS;
    int rc2;
    bool fLockWrite = false;
 ...
 * @param   fDelete         If true, delete the image from the host disk.
 */
NEMUDDU_DECL(int) VDCacheClose(PNEMUHDD pDisk, bool fDelete)
{
    int rc = VINF_SUCCESS;
    int rc2;
    bool fLockWrite = false;
...
NEMUDDU_DECL(int) VDFilterRemove(PNEMUHDD pDisk, uint32_t fFlags)
{
    int rc = VINF_SUCCESS;
    int rc2;
    bool fLockWrite = false;
 ...
 * @param   pDisk           Pointer to HDD container.
 */
NEMUDDU_DECL(int) VDCloseAll(PNEMUHDD pDisk)
{
    int rc = VINF_SUCCESS;
    int rc2;
    bool fLockWrite = false;
 ...
 * @param   pDisk           Pointer to HDD container.
 */
NEMUDDU_DECL(int) VDFilterRemoveAll(PNEMUHDD pDisk)
{
    int rc = VINF_SUCCESS;
    int rc2;
    bool fLockWrite = false;
 ...
 */
NEMUDDU_DECL(int) VDRead(PNEMUHDD pDisk, uint64_t uOffset, void *pvBuf,
                         size_t cbRead)
{
    int rc = VINF_SUCCESS;
    int rc2;
 ...
 * @param   cbWrite         Number of bytes to write.
 */
NEMUDDU_DECL(int) VDWrite(PNEMUHDD pDisk, uint64_t uOffset, const void *pvBuf,
                          size_t cbWrite)
{
    int rc = VINF_SUCCESS;
    int rc2;
 ...
 * @param   pDisk           Pointer to HDD container.
 */
NEMUDDU_DECL(int) VDFlush(PNEMUHDD pDisk)
{
    int rc = VINF_SUCCESS;
    int rc2;
    bool fLockWrite = false;
 ...
 * @param   pDisk           Pointer to HDD container.
 */
NEMUDDU_DECL(unsigned) VDGetCount(PNEMUHDD pDisk)
{
    unsigned cImages;
    int rc2;
    bool fLockRead = false;
 ...
 * @param   pDisk           Pointer to HDD container.
 */
NEMUDDU_DECL(bool) VDIsReadOnly(PNEMUHDD pDisk)
{
    bool fReadOnly;
    int rc2;
    bool fLockRead = false;
 ...
 * @param   nImage          Image number, counts from 0. 0 is always base image of container.
 */
NEMUDDU_DECL(uint32_t) VDGetSectorSize(PNEMUHDD pDisk, unsigned nImage)
{
    uint64_t cbSector;
    int rc2;
    bool fLockRead = false;
 ...
 * @param   nImage          Image number, counts from 0. 0 is always base image of container.
 */
NEMUDDU_DECL(uint64_t) VDGetSize(PNEMUHDD pDisk, unsigned nImage)
{
    uint64_t cbSize;
    int rc2;
    bool fLockRead = false;
 ...
 * @param   nImage          Image number, counts from 0. 0 is always base image of container.
 */
NEMUDDU_DECL(uint64_t) VDGetFileSize(PNEMUHDD pDisk, unsigned nImage)
{
    uint64_t cbSize;
    int rc2;
    bool fLockRead = false;
 ...
 */
NEMUDDU_DECL(int) VDGetPCHSGeometry(PNEMUHDD pDisk, unsigned nImage,
 ...
 */
NEMUDDU_DECL(int) VDSetPCHSGeometry(PNEMUHDD pDisk, unsigned nImage,
                                    PCVDGEOMETRY pPCHSGeometry)
{
    int rc = VINF_SUCCESS;
    int rc2;
 ...
 */
NEMUDDU_DECL(int) VDGetLCHSGeometry(PNEMUHDD pDisk, unsigned nImage,
 ...
 */
NEMUDDU_DECL(int) VDSetLCHSGeometry(PNEMUHDD pDisk, unsigned nImage,
                                    PCVDGEOMETRY pLCHSGeometry)
{
    int rc = VINF_SUCCESS;
    int rc2;
 ...
 */
NEMUDDU_DECL(int) VDGetVersion(PNEMUHDD pDisk, unsigned nImage,
                               unsigned *puVersion)
{
    int rc = VINF_SUCCESS;
    int rc2;
 ...
 */
NEMUDDU_DECL(int) VDBackendInfoSingle(PNEMUHDD pDisk, unsigned nImage,
 ...
 */
NEMUDDU_DECL(int) VDGetImageFlags(PNEMUHDD pDisk, unsigned nImage,
 ...
 */
NEMUDDU_DECL(int) VDGetOpenFlags(PNEMUHDD pDisk, unsigned nImage,
 ...
 * @param   uOpenFlags      Image file open mode, see VD_OPEN_FLAGS_* constants.
 */
NEMUDDU_DECL(int) VDSetOpenFlags(PNEMUHDD pDisk, unsigned nImage,
                                 unsigned uOpenFlags)
{
    int rc;
    int rc2;
 ...
 */
NEMUDDU_DECL(int) VDGetFilename(PNEMUHDD pDisk, unsigned nImage,
 ...
 */
NEMUDDU_DECL(int) VDGetComment(PNEMUHDD pDisk, unsigned nImage,
 ...
 */
NEMUDDU_DECL(int) VDSetComment(PNEMUHDD pDisk, unsigned nImage,
 ...
 */
NEMUDDU_DECL(int) VDGetUuid(PNEMUHDD pDisk, unsigned nImage, PRTUUID pUuid)
{
    int rc;
    int rc2;
    bool fLockRead = false;
 ...
 * @param   pUuid           New UUID of the image. If NULL, a new UUID is created.
 */
NEMUDDU_DECL(int) VDSetUuid(PNEMUHDD pDisk, unsigned nImage, PCRTUUID pUuid)
{
    int rc;
    int rc2;
    bool fLockWrite = false;
 ...
 */
NEMUDDU_DECL(int) VDGetModificationUuid(PNEMUHDD pDisk, unsigned nImage, PRTUUID pUuid)
{
    int rc = VINF_SUCCESS;
    int rc2;
    bool fLockRead = false;
 ...
 */
NEMUDDU_DECL(int) VDSetModificationUuid(PNEMUHDD pDisk, unsigned nImage, PCRTUUID pUuid)
{
    int rc;
    int rc2;
    bool fLockWrite = false;
 ...
 */
NEMUDDU_DECL(int) VDGetParentUuid(PNEMUHDD pDisk, unsigned nImage,
                                  PRTUUID pUuid)
{
    int rc = VINF_SUCCESS;
    int rc2;
 ...
 */
NEMUDDU_DECL(int) VDSetParentUuid(PNEMUHDD pDisk, unsigned nImage,
                                  PCRTUUID pUuid)
{
    int rc;
    int rc2;
 ...
 * @param   pDisk           Pointer to HDD container.
 */
NEMUDDU_DECL(void) VDDumpImages(PNEMUHDD pDisk)
{
    int rc2;
    bool fLockRead = false;
...
NEMUDDU_DECL(int) VDDiscardRanges(PNEMUHDD pDisk, PCRTRANGE paRanges, unsigned cRanges)
{
    int rc;
    int rc2;
    bool fLockWrite = false;
...
NEMUDDU_DECL(int) VDAsyncRead(PNEMUHDD pDisk, uint64_t uOffset, size_t cbRead,
                              PCRTSGBUF pcSgBuf,
                              PFNVDASYNCTRANSFERCOMPLETE pfnComplete,
                              void *pvUser1, void *pvUser2)
{
...
NEMUDDU_DECL(int) VDAsyncWrite(PNEMUHDD pDisk, uint64_t uOffset, size_t cbWrite,
                               PCRTSGBUF pcSgBuf,
                               PFNVDASYNCTRANSFERCOMPLETE pfnComplete,
                               void *pvUser1, void *pvUser2)
{
...
NEMUDDU_DECL(int) VDAsyncFlush(PNEMUHDD pDisk, PFNVDASYNCTRANSFERCOMPLETE pfnComplete,
                               void *pvUser1, void *pvUser2)
{
    int rc;
    int rc2;
...
NEMUDDU_DECL(int) VDAsyncDiscardRanges(PNEMUHDD pDisk, PCRTRANGE paRanges, unsigned cRanges,
                                       PFNVDASYNCTRANSFERCOMPLETE pfnComplete,
                                       void *pvUser1, void *pvUser2)
{
    int rc;
...
NEMUDDU_DECL(int) VDRepair(PVDINTERFACE pVDIfsDisk, PVDINTERFACE pVDIfsImage,
...
/**
 * @interface_method_impl{NEMUHDDBACKEND,pfnComposeLocation}
 */
...
/**
 * @interface_method_impl{NEMUHDDBACKEND,pfnComposeName}
 */
