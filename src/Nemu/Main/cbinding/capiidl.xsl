 ...
 * Useful for comparing the interface version in NEMUCAPI::uVersion. */
#define NEMU_CAPI_MAJOR(x) (((x) &amp; 0xffff0000U) &gt;&gt; 16)

/** Extract the C API style major version.
 * Useful for comparing the interface version in NEMUCAPI::uVersion. */
#define NEMU_CAPI_MINOR(x) ((x) &amp; 0x0000ffffU)

/** The current interface version.
 * For use with NemuGetCAPIFunctions and to be found in NEMUCAPI::uVersion. */
#define NEMU_CAPI_VERSION 0x00040001U
...
<xsl:template match="interface">
  <xsl:if test="not(@internal='yes')">
    ...
    <xsl:text>NEMU_EXTERN_CONST(nsIID, IID_</xsl:text>
    <xsl:value-of select="$name"/>
    <xsl:text>);&#x0A;</xsl:text>
    <xsl:text>#ifndef NEMU_WITH_GLUE&#x0A;</xsl:text>
...
<xsl:template match="module/class">
  ...
  <xsl:text>/* COM compatibility */&#x0A;</xsl:text>
  <xsl:text>NEMU_EXTERN_CONST(nsCID, CLSID_</xsl:text>
  <xsl:value-of select="@name"/>
  <xsl:text>);&#x0A;</xsl:text>
  <xsl:text>&#x0A;&#x0A;</xsl:text>
</xsl:template>
