...
typedef struct NEMUCRFPS
{
    ...
    uint64_t *mpaTimes;
} NEMUCRFPS, *PNEMUCRFPS;

void nemuCrFpsInit(PNEMUCRFPS pFps, uint32_t cPeriods)
{
...
void nemuCrFpsTerm(PNEMUCRFPS pFps)
{
...
void nemuCrFpsReportFrame(PNEMUCRFPS pFps)
{
...
uint64_t nemuCrFpsGetEveragePeriod(PNEMUCRFPS pFps)
{
    return pFps->mPeriodSum / pFps->mcPeriods;
}

double nemuCrFpsGetFps(PNEMUCRFPS pFps)
{
    return ((double)1000000000.0) / nemuCrFpsGetEveragePeriod(pFps);
}

double nemuCrFpsGetBps(PNEMUCRFPS pFps)
{
    return nemuCrFpsGetFps(pFps) * pFps->mBytesSum / pFps->mcPeriods;
}

double nemuCrFpsGetBpsSent(PNEMUCRFPS pFps)
{
    return nemuCrFpsGetFps(pFps) * pFps->mBytesSentSum / pFps->mcPeriods;
}

double nemuCrFpsGetCps(PNEMUCRFPS pFps)
{
    return nemuCrFpsGetFps(pFps) * pFps->mCallsSum / pFps->mcPeriods;
}

double nemuCrFpsGetOps(PNEMUCRFPS pFps)
{
    return nemuCrFpsGetFps(pFps) * pFps->mOpsSum / pFps->mcPeriods;
}

double nemuCrFpsGetTimeProcPercent(PNEMUCRFPS pFps)
{
    return 100.0*pFps->mTimeUsedSum/pFps->mPeriodSum;
}

uint64_t nemuCrFpsGetNumFrames(PNEMUCRFPS pFps)
{
...
#ifdef NEMUCR_LOGFPS
  static NEMUCRFPS Fps;
  static bool bFpsInited = false;

  if (!bFpsInited)
  {
      nemuCrFpsInit(&Fps, 64 /* cPeriods */);
      bFpsInited = true;
  }
  nemuCrFpsReportFrame(&Fps);
  if(!(nemuCrFpsGetNumFrames(&Fps) % 31))
  {
      double fps = nemuCrFpsGetFps(&Fps);
      double bps = nemuCrFpsGetBps(&Fps);
      double bpsSent = nemuCrFpsGetBpsSent(&Fps);
      double cps = nemuCrFpsGetCps(&Fps);
      double ops = nemuCrFpsGetOps(&Fps);
      double tup = nemuCrFpsGetTimeProcPercent(&Fps);
