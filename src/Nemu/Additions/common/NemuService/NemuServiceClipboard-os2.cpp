/** $Id: NemuServiceClipboard-os2.cpp $ */
/** @file
 * NemuService - Guest Additions Clipboard Service, OS/2.
 */
...
/** @page pg_vgsvc_clipboard NemuService - Clipboard (OS/2)
 *
 * The Clipboard subservice provides clipboard sharing for OS/2 guests only.
 *
 * This was the second subservice that was added to NemuService.  OS/2 is a
 * single user system and we don't provide any NemuTray or NemuClient like
 * processes. Because it's kind of simple system, it became natural to put the
 * clipboard sharing here in NemuService for OS/2.
 *
    ...
    if (WinOpenClipbrd(g_habWorker))
    {
        ...
        if (WinEmptyClipbrd(g_habWorker))
        {
            ...
            if (WinSetClipbrdOwner(g_habWorker, g_hwndWorker))
            {
                 ...
                 */
                if (fFormats & (NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT/* | NEMU_SHARED_CLIPBOARD_FMT_HTML ?? */))
                {
                ...
                if (fFormats & NEMU_SHARED_CLIPBOARD_FMT_BITMAP)
                {
                    /** @todo bitmaps */
                }
            }
...
static void *vgsvcClipboardOs2ConvertToPM(uint32_t fFormat, USHORT usFmt, void *pv, uint32_t cb)
{
    ...
    else if (usFmt == CF_TEXT)
    {
         ...
         * -lazybird
         */
        Assert(fFormat & NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT);
        char *pszUtf8;
...
static void vgsvcClipboardOs2RenderFormat(USHORT usFmt)
{
    ...
    if (    usFmt == CF_TEXT
        ||  usFmt == g_atomOdin32UnicodeText)
        fFormat = NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT;
    else /** @todo bitmaps */
        fFormat = 0;
    if (fFormat)
    {
...
static void vgsvcClipboardOs2SendDataToHost(uint32_t fFormat)
{
    if (WinOpenClipbrd(g_habWorker))
    {
        ...
        if (fFormat & NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT)
        {
...
static void vgsvcClipboardOs2ReportFormats(void)
{
    ...
    while ((ulFormat = WinEnumClipbrdFmts(g_habWorker, ulFormat)) != 0)
    {
        if (    ulFormat == CF_TEXT
            ...
            fFormats |= NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT;
...
static MRESULT EXPENTRY vgsvcClipboardOs2WinProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
    ...
    switch (msg)
    {
         ...
         */
        case WM_USER + NEMU_SHARED_CLIPBOARD_HOST_MSG_FORMATS:
         ...
         */
        case WM_USER + NEMU_SHARED_CLIPBOARD_HOST_MSG_READ_DATA:
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    switch (Msg)
                    {
                        ...
                        case NEMU_SHARED_CLIPBOARD_HOST_MSG_FORMATS:
                            if (!WinPostMsg(g_hwndWorker, WM_USER + NEMU_SHARED_CLIPBOARD_HOST_MSG_FORMATS,
                        ...
                        case NEMU_SHARED_CLIPBOARD_HOST_MSG_READ_DATA:
                            if (!WinPostMsg(g_hwndWorker, WM_USER + NEMU_SHARED_CLIPBOARD_HOST_MSG_READ_DATA,
                                            MPFROMLONG(fFormats), 0))
...
static DECLCALLBACK(int) vgsvcClipboardOs2Worker(bool volatile *pfShutdown)
{
    ...
    if (g_habWorker != NULLHANDLE)
    {
        ...
        if (g_hmqWorker != NULLHANDLE)
        {
             ...
             */
            if (WinRegisterClass(g_habWorker, (PCSZ)"NemuServiceClipboardClass", vgsvcClipboardOs2WinProc, 0, 0))
            {
                g_hwndWorker = WinCreateWindow(HWND_OBJECT,                             /* hwndParent */
                                               (PCSZ)"NemuServiceClipboardClass",       /* pszClass */
