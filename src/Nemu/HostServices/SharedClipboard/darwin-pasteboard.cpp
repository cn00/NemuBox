 ...
 * that is supported by nemu and return it.
 *
 * @param   pPasteboardRef Reference to the global pasteboard.
        ...
        if (!(err = PasteboardCopyItemFlavors(pPasteboard, itemID, &flavorTypeArray)))
        {
            ...
            for (CFIndex flavorIndex = 0; flavorIndex < flavorCount; flavorIndex++)
            {
                    ...
                    UTTypeConformsTo(flavorType, kUTTypeUTF16PlainText))
                {
                    ...
                    *pfFormats |= NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT;
                }
                else if (UTTypeConformsTo(flavorType, kUTTypeBMP))
                {
                    Log(("BMP flavor detected.\n"));
                    *pfFormats |= NEMU_SHARED_CLIPBOARD_FMT_BITMAP;
                }
            }
    ...
    if (!(err = PasteboardGetItemIdentifier(pPasteboard, 1, &itemID)))
    {
        ...
        if (fFormat & NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT)
        {
            ...
            if (pwszTmp)
            {
                ...
                size_t cwDest;
                rc = nemuClipboardUtf16GetWinSize(pwszTmp, cwSrc, &cwDest);
                if (RT_FAILURE(rc))
                {
                    RTUtf16Free(pwszTmp);
                    Log(("readFromPasteboard: clipboard conversion failed.  nemuClipboardUtf16GetWinSize returned %Rrc.  Abandoning.\n", rc));
                ...
                *pcbActual = cwDest * 2;
                ...
                if (*pcbActual <= cb)
                {
                    rc = nemuClipboardUtf16LinToWin(pwszTmp, RTUtf16Len(pwszTmp), static_cast <PRTUTF16>(pv), cb / 2);
                    if (RT_FAILURE(rc))
                    {
                        RTUtf16Free(pwszTmp);
                        Log(("readFromPasteboard: clipboard conversion failed.  nemuClipboardUtf16LinToWin() returned %Rrc.  Abandoning.\n", rc));
                        AssertRCReturn(rc, rc);
                    }
        ...
        else if (fFormat & NEMU_SHARED_CLIPBOARD_FMT_BITMAP)
        {
            CFDataRef outData;
            ...
            if (pTmp)
            {
                const void *pDib;
                size_t cbDibSize;
                rc = nemuClipboardBmpGetDib(pTmp, cbTmpSize, &pDib, &cbDibSize);
                if (RT_FAILURE(rc))
                {
                    rc = VERR_NOT_SUPPORTED;
                    Log(("readFromPasteboard: unknown bitmap format. nemuClipboardBmpGetDib returned %Rrc.  Abandoning.\n", rc));
                    AssertRCReturn(rc, rc);
                }
...
int writeToPasteboard(PasteboardRef pPasteboard, void *pv, uint32_t cb, uint32_t fFormat)
{
    ...
    if (fFormat & NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT)
    {
        ...
        rc = nemuClipboardUtf16GetLinSize(pwszSrcText, cwSrc, &cwDest);
        if (RT_FAILURE(rc))
        {
            Log(("writeToPasteboard: clipboard conversion failed.  nemuClipboardUtf16GetLinSize returned %Rrc.  Abandoning.\n", rc));
            AssertRCReturn(rc, rc);
        }
        ...
        /* Convert the EOL */
        rc = nemuClipboardUtf16WinToLin(pwszSrcText, cwSrc, pwszDestText, cwDest);
        if (RT_FAILURE(rc))
        {
            Log(("writeToPasteboard: clipboard conversion failed.  nemuClipboardUtf16WinToLin() returned %Rrc.  Abandoning.\n", rc));
    ...
    else if (fFormat & NEMU_SHARED_CLIPBOARD_FMT_BITMAP)
    {
        ...
        rc = nemuClipboardDibToBmp(pv, cb, &pBmp, &cbBmpSize);
        if (RT_SUCCESS(rc))
        {
            /* Create a CData object which we could pass to the pasteboard */
