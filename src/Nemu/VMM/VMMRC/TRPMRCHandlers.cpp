 ...
 * @param   rc          The Nemu status code to return.
 ...
 * @param   pRegFrame   Pointer to the register frame for the trap.
 * @internal
 * @remark  This is not hooked up unless you're building with NEMU_WITH_NMI defined.
 */
...
DECLASM(int) TRPMGCTrap06Handler(PTRPMCPU pTrpmCpu, PCPUMCTXCORE pRegFrame)
{
    ...
    if (CPUMGetGuestCPL(pVCpu) <= (EMIsRawRing1Enabled(pVM) ? 1U : 0U))
    {
        ...
        else if (Cpu.pCurInstr->uOpcode == OP_MONITOR)
        {
            ...
            rc = NEMUSTRICTRC_TODO(EMInterpretInstructionDisasState(pVCpu, &Cpu, pRegFrame, PC, EMCODETYPE_SUPERVISOR));
        }
...
static int trpmGCTrap0dHandlerRing0(PVM pVM, PVMCPU pVCpu, PCPUMCTXCORE pRegFrame, PDISCPUSTATE pCpu, RTGCPTR PC)
{
    ...
    switch (pCpu->pCurInstr->uOpcode)
    {
        ...
        case OP_WRMSR:
        {
            rc = NEMUSTRICTRC_TODO(EMInterpretInstructionDisasState(pVCpu, pCpu, pRegFrame, PC, EMCODETYPE_SUPERVISOR));
...
static int trpmGCTrap0dHandler(PVM pVM, PTRPMCPU pTrpmCpu, PCPUMCTXCORE pRegFrame)
{
    ...
    {
        NEMUSTRICTRC rcStrict = IOMRCIOPortHandler(pVM, pVCpu, pRegFrame, &Cpu);
        if (IOM_SUCCESS(rcStrict))
        {
                            ...
                            || DBGFBpIsHwIoArmed(pVM)))
            {
                ...
                NEMUSTRICTRC rcStrict2 = DBGFBpCheckIo(pVM, pVCpu, CPUMCTX_FROM_CORE(pRegFrame), uPort, cbValue);
 ...
 *
 * @returns Nemu status code.  (Anything but VINF_SUCCESS will cause guru.)
 * @param   pVM         The cross context VM structure.
