...
#endif /* #if defined(RT_OS_WINDOWS) && defined(NEMU_WITH_NETFLT) */
...
# ifdef NEMU_USE_LIBHAL
#  include "nemu-libhal.h"
...
# endif
...
#elif defined(RT_OS_LINUX) && defined(NEMU_WITH_DBUS)
    ...
    for (size_t i = 0; i < aGlobalExtraDataKeys.size(); ++i)
    {
        ...
        if (!strKey.startsWith("HostOnly/nemunet"))
            continue;

        size_t pos = strKey.find("/", sizeof("HostOnly/nemunet"));
        if (pos != Utf8Str::npos)
...
#if defined(RT_OS_WINDOWS) && defined(NEMU_WITH_NETFLT)
# define NEMU_APP_NAME L"Nemu"

static int nemuNetWinAddComponent(std::list< ComObjPtr<HostNetworkInterface> > *pPist,
...
#endif /* defined(RT_OS_WINDOWS) && defined(NEMU_WITH_NETFLT) */
...
{
#if defined(RT_OS_WINDOWS) || defined(NEMU_WITH_NETFLT) /*|| defined(RT_OS_OS2)*/
# ifdef NEMU_WITH_HOSTNETIF_API
...
#   ifndef NEMU_WITH_NETFLT
    hr = E_NOTIMPL;
#   else /* #  if defined NEMU_WITH_NETFLT */
    INetCfg              *pNc;
    ...
    hr = NemuNetCfgWinQueryINetCfg(FALSE,
                                   NEMU_APP_NAME,
                                   &pNc,
                                   &lpszApp);
    Assert(hr == S_OK);
    if (hr == S_OK)
    {
#    ifdef NEMU_NETFLT_ONDEMAND_BIND
...
#    else
        /* for the filter-based approach we get all miniports our filter (oracle_NemuNetLwf)is bound to */
        hr = pNc->FindComponent(L"oracle_NemuNetLwf", &pTcpIpNcc);
        if (hr != S_OK)
        {
            /* fall back to NDIS5 miniport lookup (sun_NemuNetFlt) */
            hr = pNc->FindComponent(L"sun_NemuNetFlt", &pTcpIpNcc);
        }
...
#    endif
        ...
        if (hr == S_OK)
        {
            hr = NemuNetCfgWinGetBindingPathEnum(pTcpIpNcc, EBP_BELOW, &pEnumBp);
            Assert(hr == S_OK);
            if (hr == S_OK)
            {
                hr = NemuNetCfgWinGetFirstBindingPath(pEnumBp, &pBp);
                ...
                while (hr == S_OK)
                {
                    ...
                    {
                        hr = NemuNetCfgWinGetBindingInterfaceEnum(pBp, &pEnumBi);
                        Assert(hr == S_OK);
                        if (hr == S_OK)
                        {
                            hr = NemuNetCfgWinGetFirstBindingInterface(pEnumBi, &pBi);
                            Assert(hr == S_OK);
                            while (hr == S_OK)
                            {
                                ...
                                if (hr == S_OK)
                                {
                                    ...
                                    if (hr == S_OK)
                                    {
                                        ...
                                        {
                                            nemuNetWinAddComponent(&list, pMpNcc);
                                        }
                                    }
                                    NemuNetCfgWinReleaseRef(pMpNcc);
                                }
                                NemuNetCfgWinReleaseRef(pBi);

                                hr = NemuNetCfgWinGetNextBindingInterface(pEnumBi, &pBi);
                            }
                            NemuNetCfgWinReleaseRef(pEnumBi);
                        }
                    }
                    NemuNetCfgWinReleaseRef(pBp);

                    hr = NemuNetCfgWinGetNextBindingPath(pEnumBp, &pBp);
                }
                NemuNetCfgWinReleaseRef(pEnumBp);
            }
            NemuNetCfgWinReleaseRef(pTcpIpNcc);
        }
        else
        {
            LogRel(("failed to get the oracle_NemuNetLwf(sun_NemuNetFlt) component, error (0x%x)\n", hr));
        }

        NemuNetCfgWinReleaseINetCfg(pNc, FALSE);
    }
#   endif /* #  if defined NEMU_WITH_NETFLT */
...

{
    ...
    if (RT_SUCCESS(r))
    {
         ...
         * so the interface gets re-created after nemunetadp.ko reload.
                                    ...
                                    const ComPtr<IHostUSBDeviceFilter> &aFilter)
{
    ...
    alock.release();
    AutoWriteLock nemuLock(m->pParent COMMA_LOCKVAL_SRC_POS);
...
{
#ifndef NEMU_WITH_HOSTNETIF_API
    return E_NOTIMPL;
#else
    if (!aName.length())
        return E_INVALIDARG;
 ...
 * to that drive; otherwise returns NEMU_E_OBJECT_NOT_FOUND.
 *
 ...
 * @return NEMU_E_OBJECT_NOT_FOUND if not found, or S_OK if found, or errors from getDrives().
 */
 ...
 * it sets pMedium to that drive; otherwise returns NEMU_E_OBJECT_NOT_FOUND.
 *
 * @param mediumType  Must be DeviceType_DVD or DeviceType_Floppy.
...
#if defined(RT_OS_SOLARIS) && defined(NEMU_USE_LIBHAL)
...
#endif  /* RT_OS_SOLARIS and NEMU_USE_HAL */
 ...
 *  Returns S_OK when the Proxy is present and OK, NEMU_E_HOST_ERROR (as a
