/* $Id: UIDnDHandler.cpp $ */
/** @file
 * Nemu Qt GUI - UIDnDHandler class implementation.
 */
                                       ...
                                       const QMimeData *pMimeData)
{
    LogFlowFunc(("enmMode=%RU32, screenID=%RU32, x=%d, y=%d, action=%ld\n",
                 m_enmMode, screenID, x, y, toNemuDnDAction(proposedAction)));
    ...
    KDnDAction result = m_dndTarget.Enter(screenID,
                                          x,
                                          y,
                                          toNemuDnDAction(proposedAction),
                                          toNemuDnDActions(possibleActions),
...
#  ifdef RT_OS_DARWIN
#    ifdef NEMU_WITH_DRAG_AND_DROP_PROMISES
        dropAction = pDrag->exec(actions, defAction, true /* fUsePromises */);
#    else
        /* Without having NEMU_WITH_DRAG_AND_DROP_PROMISES enabled drag and drop
         * will not work on OS X! It also requires some handcrafted patches within Qt
         * (which also needs NEMU_WITH_DRAG_AND_DROP_PROMISES set there). */
...
#  endif /* RT_OS_DARWIN */
    LogRel3(("DnD: Ended with dropAction=%ld\n", UIDnDHandler::toNemuDnDAction(dropAction)));
...
#else /* !NEMU_WITH_DRAG_AND_DROP_GH */
...
/* static */
KDnDAction UIDnDHandler::toNemuDnDAction(Qt::DropAction action)
{
...
/* static */
QVector<KDnDAction> UIDnDHandler::toNemuDnDActions(Qt::DropActions actions)
{
