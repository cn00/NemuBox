...
/** Selector flag shift between qemu and Nemu.
 * Nemu shifts the qemu bits to the right. */
#define SEL_FLAGS_SHIFT     (8)
/** Mask applied to the shifted qemu selector flags to get the attributes Nemu
 * (VT-x) needs. */
...
REMR3DECL(int) REMR3Init(PVM pVM)
{
     ...
     * Init the recompiler.
     */
    if (!cpu_x86_init(&pVM->rem.s.Env, "nemu"))
    {
...
#if !defined(NEMU_ALLOW_IF0) && !defined(NEMU_RUN_INTERRUPT_GATE_HANDLERS)
        if (!(env->eflags & IF_MASK))
        {
            STAM_COUNTER_INC(&gStatRefuseIF0);
...
{
#ifdef NEMU_REM_PROTECT_PAGES_FROM_SMC
...
REMR3DECL(int)  REMR3State(PVM pVM, PVMCPU pVCpu)
{
        ...
        )
    {
        ...
        if (fFlags & CPUM_CHANGED_LDTR)
        {
            if (pCtx->ldtr.fFlags & CPUMSELREG_FLAGS_VALID)
            {
                ...
                pVM->rem.s.Env.ldt.fNemuFlags  = pCtx->ldtr.fFlags;
    ...
    pVM->rem.s.Env.tr.fNemuFlags  = pCtx->tr.fFlags;
...
#define SYNC_IN_SREG(a_pEnv, a_SReg, a_pRemSReg, a_pNemuSReg) \
        do \
        { \
            if (CPUMSELREG_ARE_HIDDEN_PARTS_VALID(pVCpu, a_pNemuSReg)) \
            { \
                cpu_x86_load_seg_cache(a_pEnv, R_##a_SReg, \
                                       (a_pNemuSReg)->Sel, \
                                       (a_pNemuSReg)->u64Base, \
                                       (a_pNemuSReg)->u32Limit, \
                                       ((a_pNemuSReg)->Attr.u & SEL_FLAGS_SMASK) << SEL_FLAGS_SHIFT); \
                (a_pRemSReg)->fNemuFlags = (a_pNemuSReg)->fFlags; \
            } \
            /* This only-reload-if-changed stuff is the old approach, we should ditch it. */ \
            else if ((a_pRemSReg)->selector != (a_pNemuSReg)->Sel) \
            { \
                Log2(("REMR3State: " #a_SReg " changed from %04x to %04x!\n", \
                      (a_pRemSReg)->selector, (a_pNemuSReg)->Sel)); \
                sync_seg(a_pEnv, R_##a_SReg, (a_pNemuSReg)->Sel); \
 ...
 *
 * @remark  This function will only work correctly in NEMU_STRICT builds!
 */
...
void remR3PhysRead(RTGCPHYS SrcGCPhys, void *pvDst, unsigned cb)
{
    ...
    NEMU_CHECK_ADDR(SrcGCPhys);
    NEMUSTRICTRC rcStrict = PGMPhysRead(cpu_single_env->pVM, SrcGCPhys, pvDst, cb, PGMACCESSORIGIN_REM);
    AssertMsg(rcStrict == VINF_SUCCESS, ("%Rrc\n", NEMUSTRICTRC_VAL(rcStrict))); NOREF(rcStrict);
#ifdef NEMU_DEBUG_PHYS
...
void remR3PhysWrite(RTGCPHYS DstGCPhys, const void *pvSrc, unsigned cb)
{
    ...
    NEMU_CHECK_ADDR(DstGCPhys);
    NEMUSTRICTRC rcStrict = PGMPhysWrite(cpu_single_env->pVM, DstGCPhys, pvSrc, cb, PGMACCESSORIGIN_REM);
    AssertMsg(rcStrict == VINF_SUCCESS, ("%Rrc\n", NEMUSTRICTRC_VAL(rcStrict))); NOREF(rcStrict);
...
static uint32_t remR3HandlerReadU8(void *pvVM, target_phys_addr_t GCPhys)
{
    ...
    NEMUSTRICTRC rcStrict = PGMPhysRead((PVM)pvVM, GCPhys, &u8, sizeof(u8), PGMACCESSORIGIN_REM);
    AssertMsg(rcStrict == VINF_SUCCESS, ("%Rrc\n", NEMUSTRICTRC_VAL(rcStrict))); NOREF(rcStrict);
    return u8;
}
...
static uint32_t remR3HandlerReadU16(void *pvVM, target_phys_addr_t GCPhys)
{
    ...
    NEMUSTRICTRC rcStrict = PGMPhysRead((PVM)pvVM, GCPhys, &u16, sizeof(u16), PGMACCESSORIGIN_REM);
    AssertMsg(rcStrict == VINF_SUCCESS, ("%Rrc\n", NEMUSTRICTRC_VAL(rcStrict))); NOREF(rcStrict);
    return u16;
}
...
static uint32_t remR3HandlerReadU32(void *pvVM, target_phys_addr_t GCPhys)
{
    ...
    NEMUSTRICTRC rcStrict = PGMPhysRead((PVM)pvVM, GCPhys, &u32, sizeof(u32), PGMACCESSORIGIN_REM);
    AssertMsg(rcStrict == VINF_SUCCESS, ("%Rrc\n", NEMUSTRICTRC_VAL(rcStrict))); NOREF(rcStrict);
    return u32;
}
...
static void     remR3HandlerWriteU8(void *pvVM, target_phys_addr_t GCPhys, uint32_t u32)
{
    ...
    NEMUSTRICTRC rcStrict = PGMPhysWrite((PVM)pvVM, GCPhys, &u32, sizeof(uint8_t), PGMACCESSORIGIN_REM);
    AssertMsg(rcStrict == VINF_SUCCESS, ("%Rrc\n", NEMUSTRICTRC_VAL(rcStrict))); NOREF(rcStrict);
}
...
static void     remR3HandlerWriteU16(void *pvVM, target_phys_addr_t GCPhys, uint32_t u32)
{
    ...
    NEMUSTRICTRC rcStrict = PGMPhysWrite((PVM)pvVM, GCPhys, &u32, sizeof(uint16_t), PGMACCESSORIGIN_REM);
    AssertMsg(rcStrict == VINF_SUCCESS, ("%Rrc\n", NEMUSTRICTRC_VAL(rcStrict))); NOREF(rcStrict);
}
...
static void     remR3HandlerWriteU32(void *pvVM, target_phys_addr_t GCPhys, uint32_t u32)
{
    ...
    NEMUSTRICTRC rcStrict = PGMPhysWrite((PVM)pvVM, GCPhys, &u32, sizeof(uint32_t), PGMACCESSORIGIN_REM);
    AssertMsg(rcStrict == VINF_SUCCESS, ("%Rrc\n", NEMUSTRICTRC_VAL(rcStrict))); NOREF(rcStrict);
}
...
{
    uint64_t u64;
    NEMUSTRICTRC rcStrict = CPUMQueryGuestMsr(env->pVCpu, MSR_IA32_APICBASE, &u64);
