...
/** @file
 * Nemu Qt GUI - UIWizardNewVD class implementation.
 */
...
bool UIWizardNewVD::createVirtualDisk()
{
    ...
    CMedium virtualDisk = nemu.CreateMedium(mediumFormat.GetName(), strMediumPath, KAccessMode_ReadWrite, KDeviceType_HardDisk);
    if (!nemu.isOk())
    {
        msgCenter().cannotCreateHardDiskStorage(nemu, strMediumPath, this);
        return false;
    }
    ...
    nemuGlobal().createMedium(UIMedium(m_virtualDisk, UIMediumType_HardDisk, KMediumState_Created));
