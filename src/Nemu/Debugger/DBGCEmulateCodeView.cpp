 ...
 *
 * @returns Nemu status code. Any failure will stop the enumeration.
...
static DECLCALLBACK(int) dbgcCmdListSource(PCDBGCCMD pCmd, PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR paArgs, unsigned cArgs)
{
    ...
    for (;;)
    {
        ...
        if (RT_FAILURE(rc))
            return pCmdHlp->pfnNemuError(pCmdHlp, rc, "Expression: (%Dv) + %x\n", &pDbgc->SourcePos, 1);
        if (iRangeLeft <= 0)
            break;
    }
                                          ...
                                          const char *pszPrefix)
{
    ...
    if (RT_FAILURE(rc))
    {
        if (rc == VERR_DBGF_REGISTER_NOT_FOUND)
            return DBGCCmdHlpNemuError(pCmdHlp, VERR_INVALID_PARAMETER, "Unknown register: '%s%s'.\n",
                                       pszActualPrefix,  pszReg);
        return DBGCCmdHlpNemuError(pCmdHlp, rc, "DBGFR3RegNmQuery failed querying '%s%s': %Rrc.\n",
    ...
    if (cArgs == 1)
    {
        ...
        if (RT_SUCCESS(rc))
            ...
            rc = DBGCCmdHlpNemuError(pCmdHlp, rc, "DBGFR3RegFormatValue failed: %Rrc.\n", rc);
    }
    else
    {
        DBGCVAR   NewValueTmp;
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (RT_FAILURE(rc))
                rc = DBGCCmdHlpNemuError(pCmdHlp, rc, "DBGFR3RegNmSet failed settings '%s%s': %Rrc\n",
                                         pszActualPrefix, pszReg, rc);
            if (rc != VINF_SUCCESS)
...
static DECLCALLBACK(int) dbgcCmdRegGuest(PCDBGCCMD pCmd, PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR paArgs, unsigned cArgs)
{
    ...
    if (cArgs == 0)
    {
        ...
        if (RT_FAILURE(rc))
            return DBGCCmdHlpNemuError(pCmdHlp, rc, "DBGFR3RegPrintf failed");
...
static DECLCALLBACK(int) dbgcCmdTrace(PCDBGCCMD pCmd, PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR paArgs, unsigned cArgs)
{
    ...
    if (RT_SUCCESS(rc))
        ...
        rc = pDbgc->CmdHlp.pfnNemuError(&pDbgc->CmdHlp, rc, "When trying to single step VM %p\n", pDbgc->pVM);
    ...
    for (unsigned i = 0; i < cArgs; i++)
    {
        ...
        if (paArgs[i].u.u64Number < 256)
        {
                   ...
                   &&   iInt < 256)
            {
                ...
                if (RT_FAILURE(rc))
                    return pCmdHlp->pfnNemuError(pCmdHlp, rc, "Reading IDT entry %#04x.\n", (unsigned)iInt);
...
static DECLCALLBACK(int) dbgcCmdDumpMem(PCDBGCCMD pCmd, PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR paArgs, unsigned cArgs)
{
    ...
    for (;;)
    {
        ...
        if (RT_FAILURE(rc))
        {
            ...
            return pCmdHlp->pfnNemuError(pCmdHlp, rc, "Reading memory at %DV.\n", &pDbgc->DumpPos);
        }
...
static DECLCALLBACK(int) dbgcCmdDumpPageDir(PCDBGCCMD pCmd, PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR paArgs, unsigned cArgs)
{
    ...
    else
    {
        ...
        if (RT_FAILURE(rc))
            return DBGCCmdHlpNemuError(pCmdHlp, rc, "%%(%Dv)", &paArgs[0]);
        ...
        if (fLME)
        {
            ...
            if (RT_FAILURE(rc))
                return DBGCCmdHlpNemuError(pCmdHlp, rc, "Reading PML4E memory at %DV.\n", &VarCur);
        ...
        if (fPAE)
        {
            ...
            if (RT_FAILURE(rc))
                return DBGCCmdHlpNemuError(pCmdHlp, rc, "Reading PDPE memory at %DV.\n", &VarCur);
            if (!Pdpe.n.u1Present)
    ...
    do
    {
        ...
        if (RT_FAILURE(rc))
            return pCmdHlp->pfnNemuError(pCmdHlp, rc, "Reading PDE memory at %DV.\n", &VarPDEAddr);
...
static DECLCALLBACK(int) dbgcCmdDumpPageTable(PCDBGCCMD pCmd, PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR paArgs, unsigned cArgs)
{
    ...
    else
    {
        ...
        if (fPAE)
        {
            ...
            if (RT_FAILURE(rc))
                return DBGCCmdHlpNemuError(pCmdHlp, rc, "Reading PDE memory at %DV.\n", &VarCur);
            if (!Pde.n.u1Present)
    ...
    do
    {
        ...
        if (RT_FAILURE(rc))
            return DBGCCmdHlpNemuError(pCmdHlp, rc, "Reading PTE memory at %DV.\n", &VarPTEAddr);
        ...
        switch (paVars[i].enmType)
        {
            ...
            case DBGCVAR_TYPE_SYMBOL:
            {
                ...
                if (cbUnit & RT_BIT_32(31))
                {
                    ...
                    {
                        pCmdHlp->pfnNemuError(pCmdHlp, VERR_TOO_MUCH_DATA, "Max %d bytes.\n", uEnd.pu8 - (uint8_t *)pvBuf);
            ...
            default:
                *pcbBuf = u.pu8 - (uint8_t *)pvBuf;
                pCmdHlp->pfnNemuError(pCmdHlp, VERR_INTERNAL_ERROR,
                                      "i=%d enmType=%d\n", i, paVars[i].enmType);
...
static DECLCALLBACK(int) dbgcCmdEditMem(PCDBGCCMD pCmd, PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR paArgs, unsigned cArgs)
{
    ...
    for (iArg = 1;;)
    {
        ...
        if (RT_FAILURE(rc))
            return pCmdHlp->pfnNemuError(pCmdHlp, rc, "Writing memory at %DV.\n", &Addr);
    ...
    for (;;)
    {
        ...
        if (RT_FAILURE(rc))
        {
            if (rc != VERR_DBGF_MEM_NOT_FOUND)
                return pCmdHlp->pfnNemuError(pCmdHlp, rc, "DBGFR3MemScan\n");
        ...
        if (RT_FAILURE(rc))
            return DBGCCmdHlpNemuError(pCmdHlp, rc, "DBGCCmdHlpVarFromDbgfAddr\n");
        if (!pResult)
                                  ...
                                  PCDBGCVAR paPatArgs, unsigned cPatArgs, PDBGCVAR pResult)
{
    ...
    switch (chType)
    {
        ...
        default:
            return pCmdHlp->pfnNemuError(pCmdHlp, VERR_INVALID_PARAMETER, "chType=%c\n", chType);
    }
    ...
    if (RT_FAILURE(rc))
        return pCmdHlp->pfnNemuError(pCmdHlp, rc, "VarToDbgfAddr(,%Dv,)\n", pAddress);
...
static int dbgcDoListNear(PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR pArg)
{
    ...
    if (pArg->enmType == DBGCVAR_TYPE_SYMBOL)
    {
        ...
        if (RT_FAILURE(rc))
            return pCmdHlp->pfnNemuError(pCmdHlp, rc, "DBGFR3AsSymbolByName(,,%s,)\n", pArg->u.pszString);
    ...
    else
    {
        ...
        if (RT_FAILURE(rc))
            return pCmdHlp->pfnNemuError(pCmdHlp, rc, "%%(%DV)\n", pArg);
        ...
        if (RT_FAILURE(rc))
            return pCmdHlp->pfnNemuError(pCmdHlp, rc, "DBGFR3ASymbolByAddr(,,%RGv,,)\n", AddrVar.u.GCFlat);
...
static DECLCALLBACK(int) dbgcCmdListNear(PCDBGCCMD pCmd, PDBGCCMDHLP pCmdHlp, PUVM pUVM, PCDBGCVAR paArgs, unsigned cArgs)
{
    ...
    if (!cArgs)
    {
        ...
        if (RT_FAILURE(rc))
            return pCmdHlp->pfnNemuError(pCmdHlp, rc, "%s\n", pszFmtExpr + 1);
