...
#ifdef NEMU_WITH_WDDM
//#pragma comment(linker, "/export:NemuWineExD3DDev9Flush=_NemuWineExD3DDev9Flush@4")

NEMUWINEEX_DECL(HRESULT) NemuWineExD3DDev9Flush(IDirect3DDevice9Ex *iface)
{
	struct d3d9_device *device = impl_from_IDirect3DDevice9Ex(iface);
    HRESULT hr;
...
NEMUWINEEX_DECL(HRESULT) NemuWineExD3DDev9FlushToHost(IDirect3DDevice9Ex *iface)
{
	struct d3d9_device *device = impl_from_IDirect3DDevice9Ex(iface);
    HRESULT hr;
...
NEMUWINEEX_DECL(HRESULT) NemuWineExD3DDev9Finish(IDirect3DDevice9Ex *iface)
{
	struct d3d9_device *device = impl_from_IDirect3DDevice9Ex(iface);
    HRESULT hr;
...
//#pragma comment(linker, "/export:NemuWineExD3DDev9CreateTexture=_NemuWineExD3DDev9CreateTexture@40")
...
{
#ifdef NEMU_WITH_WDDM
	return NemuWineExD3DDev9CreateTexture(iface, width, height, levels, usage, format,
...
{
#ifdef NEMU_WITH_WDDM
    return NemuWineExD3DDev9CreateVolumeTexture(iface, width, height, depth, levels, usage, format,
...
{
#ifdef NEMU_WITH_WDDM
    return NemuWineExD3DDev9CreateCubeTexture(iface, edge_length, levels, usage, format,
            pool, texture, shared_handle, NULL);
#else
    struct d3d9_device *device = impl_from_IDirect3DDevice9Ex(iface);
    struct d3d9_texture *object;
...
#if defined(NEMU) && defined(RT_ARCH_AMD64)
DECLASM(uint16_t) NemuAsmFpuFCWGet();
DECLASM(void) NemuAsmFpuFCWSet(uint16_t u16FCW);
#endif
...
static void setup_fpu(void)
{
    ...
    cw = (cw & ~0xf3f) | 0x3f;
    __asm fldcw cw;
#elif defined(NEMU) && defined(RT_ARCH_AMD64)
    uint16_t cw = NemuAsmFpuFCWGet();
    cw = (cw & ~0xf3f) | 0x3f;
    NemuAsmFpuFCWSet(cw);
#else
...
#ifdef NEMU_WITH_WDDM
        , NEMUWINEEX_D3DPRESENT_PARAMETERS *parameters
#else
...
#ifdef NEMU_WITH_WDDM
AssertCompile(sizeof (struct wined3d_box) == sizeof (NEMUBOX3D));
AssertCompile(RT_SIZEOFMEMB(struct wined3d_box, left) == RT_SIZEOFMEMB(NEMUBOX3D, Left));
AssertCompile(RT_SIZEOFMEMB(struct wined3d_box, top) == RT_SIZEOFMEMB(NEMUBOX3D, Top));
AssertCompile(RT_SIZEOFMEMB(struct wined3d_box, right) == RT_SIZEOFMEMB(NEMUBOX3D, Right));
AssertCompile(RT_SIZEOFMEMB(struct wined3d_box, bottom) == RT_SIZEOFMEMB(NEMUBOX3D, Bottom));
AssertCompile(RT_SIZEOFMEMB(struct wined3d_box, front) == RT_SIZEOFMEMB(NEMUBOX3D, Front));
AssertCompile(RT_SIZEOFMEMB(struct wined3d_box, back) == RT_SIZEOFMEMB(NEMUBOX3D, Back));

AssertCompile(RT_OFFSETOF(struct wined3d_box, left) == RT_OFFSETOF(NEMUBOX3D, Left));
AssertCompile(RT_OFFSETOF(struct wined3d_box, top) == RT_OFFSETOF(NEMUBOX3D, Top));
AssertCompile(RT_OFFSETOF(struct wined3d_box, right) == RT_OFFSETOF(NEMUBOX3D, Right));
AssertCompile(RT_OFFSETOF(struct wined3d_box, bottom) == RT_OFFSETOF(NEMUBOX3D, Bottom));
AssertCompile(RT_OFFSETOF(struct wined3d_box, front) == RT_OFFSETOF(NEMUBOX3D, Front));
AssertCompile(RT_OFFSETOF(struct wined3d_box, back) == RT_OFFSETOF(NEMUBOX3D, Back));
...
NEMUWINEEX_DECL(HRESULT) NemuWineExD3DDev9VolBlt(IDirect3DDevice9Ex *iface,
                                                    ...
                                                    const NEMUBOX3D *pSrcBoxArg,
                                                    const NEMUPOINT3D *pDstPoin3D)
{
    struct d3d9_device *device = impl_from_IDirect3DDevice9Ex(iface);
