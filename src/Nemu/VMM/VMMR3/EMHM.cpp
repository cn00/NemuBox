 ...
 * @thread  EMT.
 */
VMMR3_INT_DECL(NEMUSTRICTRC) EMR3HmSingleInstruction(PVM pVM, PVMCPU pVCpu, uint32_t fFlags)
{
    ...
    for (;;)
    {
        ...
        {
            NEMUSTRICTRC rcStrict = emR3HmForcedActions(pVM, pVCpu, pCtx);
        ...
        NEMUSTRICTRC rcStrict = VMMR3HmRunGC(pVM, pVCpu);
        ...
        {
            rcStrict = emR3HighPriorityPostForcedActions(pVM, pVCpu, NEMUSTRICTRC_TODO(rcStrict));
            LogFlow(("EMR3HmSingleInstruction: FFs after -> %Rrc\n", NEMUSTRICTRC_VAL(rcStrict)));
        }

        if (rcStrict != VINF_SUCCESS && (rcStrict < VINF_EM_FIRST || rcStrict > VINF_EM_LAST))
        {
            rcStrict = emR3HmHandleRC(pVM, pVCpu, pCtx, NEMUSTRICTRC_TODO(rcStrict));
 ...
 *
 * @returns Nemu status code suitable for EM.
 *
 * @param   pVM         The cross context VM structure.
...
#endif
{
    ...
    rc = NEMUSTRICTRC_TODO(IEMExecOne(pVCpu));
...
static int emR3HmExecuteIOInstruction(PVM pVM, PVMCPU pVCpu)
{
     ...
     */
    NEMUSTRICTRC rcStrict = HMR3RestartPendingIOInstr(pVM, pVCpu, pCtx);
    if (IOM_SUCCESS(rcStrict))
    {
        STAM_COUNTER_INC(&pVCpu->em.s.CTX_SUFF(pStats)->StatIoRestarted);
        STAM_PROFILE_STOP(&pVCpu->em.s.StatIOEmu, a);
        return NEMUSTRICTRC_TODO(rcStrict);     /* rip already updated. */
    }
    AssertMsgReturn(rcStrict == VERR_NOT_FOUND, ("%Rrc\n", NEMUSTRICTRC_VAL(rcStrict)),
                    RT_SUCCESS_NP(rcStrict) ? VERR_IPE_UNEXPECTED_INFO_STATUS : NEMUSTRICTRC_TODO(rcStrict));
 ...
 *
 * @returns Nemu status code. May return VINF_EM_NO_MEMORY but none of the other
 *          EM statuses.
 * @param   pVM         The cross context VM structure.
 * @param   pVCpu       The cross context virtual CPU structure.
 ...
 *
 * @returns Nemu status code. The most important ones are: VINF_EM_RESCHEDULE, VINF_EM_RESCHEDULE_RAW,
