...
EFI_DRIVER_BINDING_PROTOCOL gNemuVgaDriverBinding = {
...
///
NEMU_VGA_VIDEO_MODES  NemuVgaVideoModes[] =
{
  ...
  )
{
  ...
  //
  if (Pci.Hdr.VendorId == NEMU_VENDOR_ID) {
    if (Pci.Hdr.DeviceId == NEMU_VGA_DEVICE_ID) {
  ...
  )
{
  ...
  //
  Private = AllocateZeroPool (sizeof (NEMU_VGA_PRIVATE_DATA));
  if (Private == NULL) {
  ...
  //
  Private->Signature  = NEMU_VGA_PRIVATE_DATA_SIGNATURE;
  Private->Handle     = NULL;
  ...
  //
  Status = NemuVgaVideoModeSetup (Private);
  if (EFI_ERROR (Status)) {
    goto Error;
  }
  ...
  if (FeaturePcdGet (PcdSupportUga)) {
    ...
    //
    Status = NemuVgaUgaDrawConstructor (Private);
    ...
    if (Private->GopDevicePath == NULL) {
      ...
      //
      Status = NemuVgaGraphicsOutputConstructor (Private);
      ASSERT_EFI_ERROR (Status);
  ...
  )
{
  ...
  if (FeaturePcdGet (PcdSupportUga)) {
    ...
    //
    Private = NEMU_VGA_PRIVATE_DATA_FROM_UGA_DRAW_THIS (UgaDraw);
    NemuVgaUgaDrawDestructor (Private);

    if (FeaturePcdGet (PcdSupportGop)) {
      NemuVgaGraphicsOutputDestructor (Private);
      //
    ...
    // Get our private context information
    //
    Private = NEMU_VGA_PRIVATE_DATA_FROM_GRAPHICS_OUTPUT_THIS (GraphicsOutput);
                   ...
                   OUT UINT32                           *Depth)
{
    ...
    UINT32 W, H, BPP;
    NEMU_VGA_PRIVATE_DATA  *Private = This->Private;
    UINTN CurrentModeNumber = Private->CurrentMode;
    NEMU_VGA_MODE_DATA const *pCurrentMode = &Private->ModeData[CurrentModeNumber];
  ...
  )
{
  ...
  Status = EfiLibInstallDriverBindingComponentName2 (
             ImageHandle,
             SystemTable,
             &gNemuVgaDriverBinding,
             ImageHandle,
             &gNemuVgaComponentName,
             &gNemuVgaComponentName2
             );
  ASSERT_EFI_ERROR (Status);
  ...
  //
  gNemuVgaDriverSupportedEfiVersion.FirmwareVersion = PcdGet32 (PcdDriverSupportedEfiVersion);
  Status = gBS->InstallMultipleProtocolInterfaces (
                  &ImageHandle,
                  &gEfiDriverSupportedEfiVersionProtocolGuid,
                  &gNemuVgaDriverSupportedEfiVersion,
