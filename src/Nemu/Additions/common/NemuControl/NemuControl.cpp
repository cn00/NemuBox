/* $Id: NemuControl.cpp $ */
/** @file
 * NemuControl - Guest Additions Command Line Management Interface.
 */
...
# include "../NemuGuestLib/VBGLR3Internal.h" /* HACK ALERT! Using vbglR3DoIOCtl directly!! */
#endif
...
{
    /* Allow for up to 15 characters command name length (NemuControl.exe) with
...
static RTEXITCODE usage(enum NemuControlUsage eWhich = USAGE_ALL)
{
    RTPrintf("Usage:\n\n");
...
#if defined(RT_OS_WINDOWS) && !defined(NEMU_CONTROL_TEST)
 ...
 */
static RTEXITCODE NemuControlError(const char *pszFormat, ...)
{
 ...
 */
static RTEXITCODE NemuCtrlGetOptError(int ch, PCRTGETOPTUNION pValueUnion)
{
    /** @todo prefix with current command. */
 ...
 * @param   ...         Format arguments.
 */
static RTEXITCODE NemuControlSyntaxError(const char *pszFormat, ...)
{
    /** @todo prefix with current command. */
    va_list va;
    va_start(va, pszFormat);
...
static BOOL ResizeDisplayDevice(ULONG Id, DWORD Width, DWORD Height, DWORD BitsPerPixel)
{
    ...
    {
        Log(("NemuDisplayThread : already at desired resolution.\n"));
        return FALSE;
    }
...
static DECLCALLBACK(RTEXITCODE) handleSetVideoMode(int argc, char *argv[])
{
    ...
    if (hUser)
    {
        *(uintptr_t *)&gpfnChangeDisplaySettingsEx = (uintptr_t)GetProcAddress(hUser, "ChangeDisplaySettingsExA");
        Log(("NemuService: pChangeDisplaySettingsEx = %p\n", gpfnChangeDisplaySettingsEx));
        ...
        if (gpfnChangeDisplaySettingsEx)
        {
            ...
            NemuControlError("Error retrieving API for display change!");
    }
    else
        NemuControlError("Error retrieving handle to user32.dll!");

    return RTEXITCODE_SUCCESS;
}

static int checkNemuVideoKey(HKEY hkeyVideo)
{
    char szValue[128];
...
static HKEY getVideoKey(bool writable)
{
    ...
    {
        NemuControlError("Error opening video device map registry key!\n");
        return 0;
    }
    ...
    /*
     * Scan all '\Device\VideoX' REG_SZ keys to find Nemu video driver entry.
    ...
    for (iDevice = 0; iDevice < numDevices; iDevice++)
    {
            ...
            && _strnicmp(szVideoLocation, "\\REGISTRY\\Machine", 17) == 0)
        {
            ...
            if (status == ERROR_SUCCESS)
            {
                int rc = checkNemuVideoKey(hkeyVideo);
                if (RT_SUCCESS(rc))
                {
    ...
    if (hkeyVideo == 0)
    {
        NemuControlError("Error opening video registry key!\n");
    }
...
static DECLCALLBACK(RTEXITCODE) handleSetVideoAcceleration(int argc, char *argv[])
{
    ...
    if (hkeyVideo)
    {
        ...
        if (status != ERROR_SUCCESS)
        {
            NemuControlError("Error %d writing video acceleration status!\n", status);
        }
        RegCloseKey(hkeyVideo);
    }
    return RTEXITCODE_SUCCESS;
...
static DECLCALLBACK(RTEXITCODE) videoFlagsGet(void)
{
    ...
    if (hkeyVideo)
    {
        ...
        DWORD dwKeyType;
        ULONG status = RegQueryValueExA(hkeyVideo, "NemuVideoFlags", NULL, &dwKeyType, (LPBYTE)&dwFlags, &len);
        if (status != ERROR_SUCCESS)
...
static DECLCALLBACK(RTEXITCODE) videoFlagsDelete(void)
{
    ...
    if (hkeyVideo)
    {
        ULONG status = RegDeleteValueA(hkeyVideo, "NemuVideoFlags");
        if (status != ERROR_SUCCESS)
            NemuControlError("Error %d deleting video flags.\n", status);
        RegCloseKey(hkeyVideo);
        return RTEXITCODE_SUCCESS;
    }
...
static DECLCALLBACK(RTEXITCODE) videoFlagsModify(bool fSet, int argc, char *argv[])
{
    if (argc != 1)
    {
        NemuControlError("Mask required.\n");
        return RTEXITCODE_FAILURE;
    }
    ...
    if (RT_FAILURE(rc))
    {
        NemuControlError("Invalid video flags mask.\n");
        return RTEXITCODE_FAILURE;
    }
    ...
    if (hkeyVideo)
    {
        ...
        status = RegSetValueExA(hkeyVideo, "NemuVideoFlags", 0, REG_DWORD, (LPBYTE)&dwFlags, sizeof(dwFlags));
        if (status != ERROR_SUCCESS)
        {
            NemuControlError("Error %d writing video flags.\n", status);
...
static DECLCALLBACK(RTEXITCODE) handleVideoFlags(int argc, char *argv[])
{
    ...
    if (argc != 1 && argc != 2)
    {
        NemuControlError("Invalid number of arguments.\n");
        usage(VIDEO_FLAGS);
        return RTEXITCODE_FAILURE;
    }
    ...
    {
        NemuControlError("Invalid command.\n");
        exitCode = RTEXITCODE_FAILURE;
    }
...
static DECLCALLBACK(RTEXITCODE) handleAddCustomMode(int argc, char *argv[])
{
    ...
    if (   (xres > (1 << 16))
        ...
        || (   (bpp != 16)
            ...
            && (bpp != 32)))
    {
        NemuControlError("invalid mode specified!\n");
        return RTEXITCODE_FAILURE;
    }
 ...
 * This is accessed through the "NemuGuestPropSvc" HGCM service.
 *
...
static RTEXITCODE getGuestProperty(int argc, char **argv)
{
    ...
    if (RT_FAILURE(rc))
        NemuControlError("Failed to connect to the guest property service, error %Rrc\n", rc);
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        for (unsigned i = 0; (i < 10) && !finish; ++i)
        {
            ...
            {
                rc = VERR_NO_MEMORY;
                NemuControlError("Out of memory\n");
            }
            else
            {
                pvBuf = pvTmpBuf;
        ...
        if (VERR_TOO_MUCH_DATA == rc)
            NemuControlError("Temporarily unable to retrieve the property\n");
        else if (RT_FAILURE(rc) && rc != VERR_NOT_FOUND)
            NemuControlError("Failed to retrieve the property value, error %Rrc\n", rc);
    }
...
static RTEXITCODE setGuestProperty(int argc, char *argv[])
{
    ...
    else
    {
        ...
        if (RT_FAILURE(rc))
            NemuControlError("Failed to store the property value, error %Rrc\n", rc);
    }
...
static RTEXITCODE deleteGuestProperty(int argc, char *argv[])
{
    ...
    else
    {
        ...
        if (RT_FAILURE(rc))
            NemuControlError("Failed to delete the property value, error %Rrc\n", rc);
    }
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            while (RT_SUCCESS(rc) && pszName)
            {
                ...
                if (RT_FAILURE(rc))
                    NemuControlError("Error while enumerating guest properties: %Rrc\n", rc);
            }
            ...
            NemuControlError("Failed to enumerate the guest properties! Error: %Rrc\n", rc);
        VbglR3GuestPropDisconnect(u32ClientId);
    }
    else
        NemuControlError("Failed to connect to the guest property service! Error: %Rrc\n", rc);
...
static RTEXITCODE waitGuestProperty(int argc, char **argv)
{
         ...
         ++i)
    {
        ...
        if (rc == VERR_TOO_MUCH_DATA)
            NemuControlError("Temporarily unable to get a notification\n");
        else if (rc == VERR_INTERRUPTED)
            NemuControlError("The request timed out or was interrupted\n");
#ifndef RT_OS_WINDOWS  /* Windows guests do not do this right */
        else if (RT_FAILURE(rc) && rc != VERR_NOT_FOUND)
            NemuControlError("Failed to get a notification, error %Rrc\n", rc);
#endif
    }
...
/**
 * Access the guest property store through the "NemuGuestPropSvc" HGCM
 * service.
 *
...
static RTEXITCODE listSharedFolders(int argc, char **argv)
{
    ...
    if (RT_FAILURE(rc))
        NemuControlError("Failed to connect to the shared folder service, error %Rrc\n", rc);
    else
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            for (uint32_t i = 0; i < cMappings; i++)
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    NemuControlError("Error while getting the shared folder name for root node = %u, rc = %Rrc\n",
            ...
            NemuControlError("Error while getting the shared folder mappings, rc = %Rrc\n", rc);
...
#if !defined(NEMU_CONTROL_TEST)
/**
 * @callback_method_impl{FNNEMUCTRLCMDHANDLER, Command: writecoredump}
 */
static DECLCALLBACK(RTEXITCODE) handleWriteCoreDump(int argc, char *argv[])
{
    ...
    {
        NemuControlError("Error while taking guest core dump. rc=%Rrc\n", rc);
        return RTEXITCODE_FAILURE;
    }
}
#endif
...
#ifdef NEMU_WITH_DPC_LATENCY_CHECKER
/**
 * @callback_method_impl{FNNEMUCTRLCMDHANDLER, Command: help}
 */
static DECLCALLBACK(RTEXITCODE) handleDpc(int argc, char *argv[])
{
# ifndef NEMU_CONTROL_TEST
    int rc;
    for (int i = 0; i < 30; i++)
    {
        rc = vbglR3DoIOCtl(NEMUGUEST_IOCTL_DPC_LATENCY_CHECKER, NULL, 0);
        if (RT_FAILURE(rc))
            break;
        RTPrintf("%d\n", i);
    }
...
# endif
    if (RT_FAILURE(rc))
        return NemuControlError("Error. rc=%Rrc\n", rc);
    RTPrintf("Samples collection completed.\n");
    return RTEXITCODE_SUCCESS;
}
#endif /* NEMU_WITH_DPC_LATENCY_CHECKER */


/**
 * @callback_method_impl{FNNEMUCTRLCMDHANDLER, Command: writelog}
 */
        ...
        while ((ch = RTGetOpt(&GetOptState, &ValueUnion)) != 0)
        {
            switch (ch)
            {
                case VINF_GETOPT_NOT_OPTION:
                {
                    ...
                    if (RT_FAILURE(rc))
                        return NemuControlError("VbglR3WriteLog: %Rrc", rc);
                    break;
                }
                ...
                default:
                    return NemuCtrlGetOptError(ch, &ValueUnion);
            }
        }
    }
    else
        return NemuControlError("RTGetOptInit: %Rrc", rc);
    return RTEXITCODE_SUCCESS;
}


/**
 * @callback_method_impl{FNNEMUCTRLCMDHANDLER, Command: takesnapshot}
 */
static DECLCALLBACK(RTEXITCODE) handleTakeSnapshot(int argc, char *argv[])
{
    //VbglR3VmTakeSnapshot(argv[0], argv[1]);
    return NemuControlError("not implemented");
}

/**
 * @callback_method_impl{FNNEMUCTRLCMDHANDLER, Command: savestate}
 */
...
/**
 * @callback_method_impl{FNNEMUCTRLCMDHANDLER, Command: suspend|pause}
 */
...
/**
 * @callback_method_impl{FNNEMUCTRLCMDHANDLER, Command: poweroff|powerdown}
 */
...
/**
 * @callback_method_impl{FNNEMUCTRLCMDHANDLER, Command: version}
 */
static DECLCALLBACK(RTEXITCODE) handleVersion(int argc, char *argv[])
{
    if (argc)
        return NemuControlSyntaxError("getversion does not take any arguments");
    return printVersion();
}
...
typedef DECLCALLBACK(RTEXITCODE) FNNEMUCTRLCMDHANDLER(int argc, char *argv[]);
typedef FNNEMUCTRLCMDHANDLER *PFNNEMUCTRLCMDHANDLER;

/** The table of all registered command handlers. */
struct COMMANDHANDLER
{
    const char *pszCommand;
    PFNNEMUCTRLCMDHANDLER pfnHandler;
} g_aCommandHandlers[] =
{
#if defined(RT_OS_WINDOWS) && !defined(NEMU_CONTROL_TEST)
...
int main(int argc, char **argv)
{
    ...
    if (fShowLogo)
        RTPrintf(NEMU_PRODUCT " Guest Additions Command Line Management Interface Version "
                 NEMU_VERSION_STRING "\n"
                 "(C) 2008-" NEMU_C_YEAR " " NEMU_VENDOR "\n"
                 "All rights reserved.\n\n");
    if (fDoHelp)
        usage();
    ...
    if (!fOnlyInfo)
    {
        ...
        if (RT_FAILURE(rrc))
        {
            NemuControlError("Could not contact the host system.  Make sure that you are running this\n"
