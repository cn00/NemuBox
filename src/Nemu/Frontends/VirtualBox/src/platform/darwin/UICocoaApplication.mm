...
/** @file
 * Nemu Qt GUI - UICocoaApplication - C++ interface to NSApplication for handling -sendEvent.
 */
...
@public
    ...
    uint32_t            fMask;
    /** The callback. */
    PFNNEMUCACALLBACK   pfnCallback;
    /** The user argument. */
    void               *pvUser;
}
- (id) initWithMask:(uint32)mask callback:(PFNNEMUCACALLBACK)callback user:(void*)user;
...
- (void)setCallback:(uint32_t)fMask :(PFNNEMUCACALLBACK)pfnCallback :(void *)pvUser;
- (void)unsetCallback:(uint32_t)fMask :(PFNNEMUCACALLBACK)pfnCallback :(void *)pvUser;
...
void UICocoaApplication::registerForNativeEvents(uint32_t fMask, PFNNEMUCACALLBACK pfnCallback, void *pvUser)
{
    [m_pNative setCallback:fMask :pfnCallback :pvUser];
}

void UICocoaApplication::unregisterForNativeEvents(uint32_t fMask, PFNNEMUCACALLBACK pfnCallback, void *pvUser)
{
