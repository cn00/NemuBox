                                 ...
                                 const char *pszDesc, PRTGCPTR pGCPtr)
{
    ...
    AssertReturn(cPages <= NEMU_MAX_ALLOC_PAGE_COUNT, VERR_PAGE_COUNT_OUT_OF_RANGE);
 ...
 * @remark  We assume the threading structure of Nemu imposes natural
...
static int mmR3HyperHeapCreate(PVM pVM, const size_t cb, PMMHYPERHEAP *ppHeap, PRTR0PTR pR0PtrHeap)
{
    ...
    int rc = SUPR3PageAllocEx(cPages,
                              0 /*fFlags*/,
                              &pv,
#if defined(NEMU_WITH_2X_4GB_ADDR_SPACE) || defined(NEMU_WITH_MORE_RING0_MEM_MAPPINGS)
                              &pvR0,
#else
                              NULL,
#endif
...
VMMR3DECL(int) MMR3HyperAllocOnceNoRelEx(PVM pVM, size_t cb, unsigned uAlignment, MMTAG enmTag, uint32_t fFlags, void **ppv)
{
    ...
    int rc = SUPR3PageAllocEx(cPages,
                              0 /*fFlags*/,
                              &pvPages,
#ifdef NEMU_WITH_MORE_RING0_MEM_MAPPINGS
                              &pvR0,
#else
 ...
 *
 * @returns Nemu status code (first failure).
 * @param   pVM                 The cross context VM structure.
