...
public class TestNemuNATEngine
{
    void testEnumeration(VirtualBoxManager mgr, IVirtualBox nemu, IMachine vm)
    {
        String name;
        ...
        INetworkAdapter adapters[] = 
            new INetworkAdapter[
                    nemu.getSystemProperties().getMaxNetworkAdapters(chipsetType).intValue()];
    ...
    static void testStart(VirtualBoxManager mgr, IVirtualBox nemu, IMachine vm)
    {
    ...
    public TestNemuNATEngine(String[] args)
    {
        ...
        try
            {
                ...
                if (nemu != null)
                {
                    if (vmname != null)
                    {
                        ...
                        vm = nemu.getMachines().get(0);
                    System.out.println("VirtualBox version: " + nemu.getVersion() + "\n");
                    if (vm != null)
                    {
                        testEnumeration(mgr, nemu, vm);
                        testStart(mgr, nemu, vm);
                    }
            ...
            {
                System.out.println("Nemu error: "+e.getMessage()+" original="+e.getWrapped());
                e.printStackTrace();
            }
