...
/** @file
 * Nemu Qt GUI - UIMachineSettingsNetwork class implementation.
 */
...
bool UIMachineSettingsNetwork::validate(QList<UIValidationMessage> &messages)
{
    ...
    message.first = nemuGlobal().removeAccelMark(tabTitle());
...
{
    return NemuGlobal::tr("Adapter %1").arg(QString("&%1").arg(m_iSlot + 1));
}
...
{
    m_pMACEditor->setText(nemuGlobal().host().GenerateMACAddress());
}
    ...
    : m_pTwAdapters(0)
{
    ...
    ulong uCount = qMin((ULONG)4, nemuGlobal().virtualBox().GetSystemProperties().GetMaxNetworkAdapters(KChipsetType_PIIX3));
...
void UIMachineSettingsNetworkPage::refreshBridgedAdapterList()
{
    ...
    const CHostNetworkInterfaceVector &ifaces = nemuGlobal().host().GetNetworkInterfaces();
...
void UIMachineSettingsNetworkPage::refreshNATNetworkList()
{
    ...
    const CNATNetworkVector &nws = nemuGlobal().virtualBox().GetNATNetworks();
...
QStringList UIMachineSettingsNetworkPage::otherInternalNetworkList()
{
    ...
    QStringList otherInternalNetworks(QList<QString>::fromVector(nemu.GetInternalNetworks()));
...
QStringList UIMachineSettingsNetworkPage::otherGenericDriverList()
{
    ...
    CVirtualBox nemu = nemuGlobal().virtualBox();
    QStringList otherGenericDrivers(QList<QString>::fromVector(nemu.GetGenericNetworkDrivers()));
