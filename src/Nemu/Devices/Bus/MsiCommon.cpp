...
{
    return PCIDevGetWord(pDev, pDev->Int.s.u8MsiCapOffset + NEMU_MSI_CAP_MESSAGE_CONTROL);
}
...
{
    uint8_t iOff = msiIs64Bit(pDev) ? NEMU_MSI_CAP_MASK_BITS_64 : NEMU_MSI_CAP_MASK_BITS_32;
...
{
    uint8_t iOff = msiIs64Bit(pDev) ? NEMU_MSI_CAP_PENDING_BITS_64 : NEMU_MSI_CAP_PENDING_BITS_32;
    iOff += pDev->Int.s.u8MsiCapOffset;
    return (uint32_t*)(pDev->config + iOff);
}
...
{
    return (msiGetMessageControl(pDev) & NEMU_PCI_MSI_FLAGS_ENABLE) != 0;
}

DECLINLINE(uint8_t) msiGetMme(PPCIDEVICE pDev)
{
    return (msiGetMessageControl(pDev) & NEMU_PCI_MSI_FLAGS_QSIZE) >> 4;
}
...
DECLINLINE(RTGCPHYS) msiGetMsiAddress(PPCIDEVICE pDev)
{
    if (msiIs64Bit(pDev))
    {
        uint32_t lo = PCIDevGetDWord(pDev, pDev->Int.s.u8MsiCapOffset + NEMU_MSI_CAP_MESSAGE_ADDRESS_LO);
        uint32_t hi = PCIDevGetDWord(pDev, pDev->Int.s.u8MsiCapOffset + NEMU_MSI_CAP_MESSAGE_ADDRESS_HI);
        return RT_MAKE_U64(lo, hi);
    }
    else
    {
        return PCIDevGetDWord(pDev, pDev->Int.s.u8MsiCapOffset + NEMU_MSI_CAP_MESSAGE_ADDRESS_32);
    }
}

DECLINLINE(uint32_t) msiGetMsiData(PPCIDEVICE pDev, int32_t iVector)
{
    int16_t  iOff = msiIs64Bit(pDev) ? NEMU_MSI_CAP_MESSAGE_DATA_64 : NEMU_MSI_CAP_MESSAGE_DATA_32;
        ...
        switch (reg)
        {
            ...
            default:
                if (pDev->config[uAddr] != u8Val)
                {
                    ...
                    if (    !f64Bit
                         && (reg >= NEMU_MSI_CAP_MASK_BITS_32)
                         && (reg < NEMU_MSI_CAP_MASK_BITS_32 + 4)
                        )
                    {
                        maskUpdated = reg - NEMU_MSI_CAP_MASK_BITS_32;
                    }
                    if (    f64Bit
                         && (reg >= NEMU_MSI_CAP_MASK_BITS_64)
                         && (reg < NEMU_MSI_CAP_MASK_BITS_64 + 4)
                       )
                    {
                        maskUpdated = reg - NEMU_MSI_CAP_MASK_BITS_64;
                    }
...
int MsiInit(PPCIDEVICE pDev, PPDMMSIREG pMsiReg)
{
    ...
    if ((cVectors > NEMU_MSI_MAX_ENTRIES) || (1 << iMmc) < cVectors)
        return VERR_TOO_MUCH_DATA;

    Assert(iCapOffset != 0 && iCapOffset < 0xff && iNextOffset < 0xff);

    /* We always support per-vector masking */
    iFlags |= NEMU_PCI_MSI_FLAGS_MASKBIT | iMmc;
    if (f64bit)
        iFlags |= NEMU_PCI_MSI_FLAGS_64BIT;
    /* How many vectors we're capable of */
    iFlags |= iMmc;

    pDev->Int.s.u8MsiCapOffset = iCapOffset;
    pDev->Int.s.u8MsiCapSize   = f64bit ? NEMU_MSI_CAP_SIZE_64 : NEMU_MSI_CAP_SIZE_32;

    PCIDevSetByte(pDev,  iCapOffset + 0, NEMU_PCI_CAP_ID_MSI);
    PCIDevSetByte(pDev,  iCapOffset + 1, iNextOffset); /* next */
    PCIDevSetWord(pDev,  iCapOffset + NEMU_MSI_CAP_MESSAGE_CONTROL, iFlags);
