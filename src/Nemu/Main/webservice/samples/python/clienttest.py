...
# - If not already done, set the environment variable "NEMU_INSTALL_PATH"
...
#   "[python] nemuapisetup.py install"
...
def main(argv):
    ...
    nemuConstants = wrapper.constants

    # Enumerate all defined machines
    for mach in wrapper.getArray(nemu, 'machines'):
        ...
        try:
            ...
            print "    State:           %s" %(enumToString(nemuConstants, "MachineState", mach.state))
            print "    Session state:   %s" %(enumToString(nemuConstants, "SessionState", mach.sessionState))

            # Do some stuff which requires a running VM
            if mach.state == nemuConstants.MachineState_Running:

                # Get the session object
                session = mgr.getSessionObject(nemu)

                 # Lock the current machine (shared mode, since we won't modify the machine)
                mach.lockMachine(session, nemuConstants.LockType_Shared)
                ...
                print "    Additions State: %s" %(enumToString(nemuConstants, "AdditionsRunLevelType", addRunLevel))
                if addRunLevel != nemuConstants.AdditionsRunLevelType_None:
