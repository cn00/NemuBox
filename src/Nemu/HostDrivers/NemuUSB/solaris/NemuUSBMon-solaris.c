...
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static int NemuUSBMonSolarisOpen(dev_t *pDev, int fFlag, int fType, cred_t *pCred);
static int NemuUSBMonSolarisClose(dev_t Dev, int fFlag, int fType, cred_t *pCred);
static int NemuUSBMonSolarisRead(dev_t Dev, struct uio *pUio, cred_t *pCred);
static int NemuUSBMonSolarisWrite(dev_t Dev, struct uio *pUio, cred_t *pCred);
static int NemuUSBMonSolarisIOCtl(dev_t Dev, int Cmd, intptr_t pArg, int Mode, cred_t *pCred, int *pVal);
static int NemuUSBMonSolarisGetInfo(dev_info_t *pDip, ddi_info_cmd_t enmCmd, void *pArg, void **ppResult);
static int NemuUSBMonSolarisAttach(dev_info_t *pDip, ddi_attach_cmd_t enmCmd);
static int NemuUSBMonSolarisDetach(dev_info_t *pDip, ddi_detach_cmd_t enmCmd);
 ...
 */
static struct cb_ops g_NemuUSBMonSolarisCbOps =
{
    NemuUSBMonSolarisOpen,
    NemuUSBMonSolarisClose,
    nodev,                      /* b strategy */
    nodev,                      /* b dump */
    nodev,                      /* b print */
    NemuUSBMonSolarisRead,
    NemuUSBMonSolarisWrite,
    NemuUSBMonSolarisIOCtl,
    nodev,                      /* c devmap */
    nodev,                      /* c mmap */
    nodev,                      /* c segmap */
    nochpoll,                   /* c poll */
 ...
 */
static struct dev_ops g_NemuUSBMonSolarisDevOps =
{
    DEVO_REV,                   /* driver build revision */
    0,                          /* ref count */
    NemuUSBMonSolarisGetInfo,
    nulldev,                    /* identify */
    nulldev,                    /* probe */
    NemuUSBMonSolarisAttach,
    NemuUSBMonSolarisDetach,
    nodev,                      /* reset */
    &g_NemuUSBMonSolarisCbOps,
    (struct bus_ops *)0,
    nodev,                      /* power */
 ...
 */
static struct modldrv g_NemuUSBMonSolarisModule =
{
    &mod_driverops,             /* extern from kernel */
    DEVICE_DESC_DRV " " NEMU_VERSION_STRING "r" RT_XSTR(NEMU_SVN_REV),
    &g_NemuUSBMonSolarisDevOps
};

/**
 * modlinkage: export install/remove/info to the kernel
 */
static struct modlinkage g_NemuUSBMonSolarisModLinkage =
{
    MODREV_1,
    &g_NemuUSBMonSolarisModule,
    NULL,
};
 ...
 * Client driver info.
 */
typedef struct nemuusbmon_client_t
{
    dev_info_t                 *pDip;                       /* Client device info. pointer */
    NEMUUSB_CLIENT_INFO         Info;                       /* Client registration data. */
    struct nemuusbmon_client_t *pNext;                      /* Pointer to next client */
} nemuusbmon_client_t;
...
/** Global Mutex. */
static kmutex_t g_NemuUSBMonSolarisMtx;
/** Number of userland clients that have kept us open. */
static uint64_t g_cNemuUSBMonSolarisClient = 0;
/** Global list of client drivers registered with us. */
nemuusbmon_client_t *g_pNemuUSBMonSolarisClients = NULL;
/** Opaque pointer to list of soft states. */
static void *g_pNemuUSBMonSolarisState;


/*********************************************************************************************************************************
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static int nemuUSBMonSolarisProcessIOCtl(int iFunction, void *pvState, void *pvData, size_t cbData, size_t *pcbReturnedData);
static int nemuUSBMonSolarisResetDevice(char *pszDevicePath, bool fReattach);


/*********************************************************************************************************************************
*   Monitor Global Hooks                                                                                                         *
*********************************************************************************************************************************/
static int nemuUSBMonSolarisClientInfo(nemuusbmon_state_t *pState, PNEMUUSB_CLIENT_INFO pClientInfo);
...
int NemuUSBMonSolarisElectDriver(usb_dev_descr_t *pDevDesc, usb_dev_str_t *pDevStrings, char *pszDevicePath, int Bus, int Port,
...
int _init(void)
{
     ...
     */
    modctl_t *pModCtl = mod_getctl(&g_NemuUSBMonSolarisModLinkage);
    if (pModCtl)
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        mutex_init(&g_NemuUSBMonSolarisMtx, NULL, MUTEX_DRIVER, NULL);
        rc = NemuUSBFilterInit();
        if (RT_SUCCESS(rc))
        {
            rc = ddi_soft_state_init(&g_pNemuUSBMonSolarisState, sizeof(nemuusbmon_state_t), 1);
            if (!rc)
            {
                rc = mod_install(&g_NemuUSBMonSolarisModLinkage);
                if (!rc)
                    return rc;

                LogRel((DEVICE_NAME ":mod_install failed! rc=%d\n", rc));
                ddi_soft_state_fini(&g_pNemuUSBMonSolarisState);
            }
            else
                LogRel((DEVICE_NAME ":ddi_soft_state_init failed! rc=%d\n", rc));
        }
        else
            LogRel((DEVICE_NAME ":NemuUSBFilterInit failed! rc=%d\n", rc));

        mutex_destroy(&g_NemuUSBMonSolarisMtx);
        RTR0Term();
    }
    else
...
int _fini(void)
{
    ...
    rc = mod_remove(&g_NemuUSBMonSolarisModLinkage);
    if (!rc)
    {
        ddi_soft_state_fini(&g_pNemuUSBMonSolarisState);
        NemuUSBFilterTerm();
...
int _info(struct modinfo *pModInfo)
{
    ...
    return mod_info(&g_NemuUSBMonSolarisModLinkage, pModInfo);
}
...
static int NemuUSBMonSolarisAttach(dev_info_t *pDip, ddi_attach_cmd_t enmCmd)
{
    LogFunc((DEVICE_NAME ":NemuUSBMonSolarisAttach pDip=%p enmCmd=%d\n", pDip, enmCmd));
    switch (enmCmd)
    {
        case DDI_ATTACH:
        {
            ...
            {
                LogRel((DEVICE_NAME ":NemuUSBMonSolarisAttach global instance already initialized.\n"));
                return DDI_FAILURE;
            }
            ...
            if (rc == DDI_SUCCESS)
            {
                ...
                LogRel((DEVICE_NAME ":NemuUSBMonSolarisAttach ddi_create_minor_node failed! rc=%d\n", rc));
            return DDI_FAILURE;
        }
...
static int NemuUSBMonSolarisDetach(dev_info_t *pDip, ddi_detach_cmd_t enmCmd)
{
    LogFunc((DEVICE_NAME ":NemuUSBMonSolarisDetach\n"));
    ...
    switch (enmCmd)
    {
        case DDI_DETACH:
        {
             ...
             */
            mutex_enter(&g_NemuUSBMonSolarisMtx);
            nemuusbmon_client_t *pCur = g_pNemuUSBMonSolarisClients;
            while (pCur)
            {
                nemuusbmon_client_t *pNext = pCur->pNext;
                RTMemFree(pCur);
                pCur = pNext;
            }
            mutex_exit(&g_NemuUSBMonSolarisMtx);
 ...
 * @returns corresponding solaris error code.
 */
static int NemuUSBMonSolarisGetInfo(dev_info_t *pDip, ddi_info_cmd_t enmCmd, void *pvArg, void **ppvResult)
{
    int rc = DDI_SUCCESS;

    LogFunc((DEVICE_NAME ":NemuUSBMonSolarisGetInfo\n"));
...
static int NemuUSBMonSolarisOpen(dev_t *pDev, int fFlag, int fType, cred_t *pCred)
{
    nemuusbmon_state_t *pState = NULL;
    unsigned iOpenInstance;

    LogFunc((DEVICE_NAME ":NemuUSBMonSolarisOpen\n"));
    ...
    if (!g_pDip)
    {
        LogRel((DEVICE_NAME ":NemuUSBMonSolarisOpen invalid state for opening.\n"));
        return ENXIO;
    }

    mutex_enter(&g_NemuUSBMonSolarisMtx);
    if (!g_cNemuUSBMonSolarisClient)
    {
        mutex_exit(&g_NemuUSBMonSolarisMtx);
        int rc = usb_register_dev_driver(g_pDip, NemuUSBMonSolarisElectDriver);
    ...
    g_cNemuUSBMonSolarisClient++;
    mutex_exit(&g_NemuUSBMonSolarisMtx);

    for (iOpenInstance = 0; iOpenInstance < 4096; iOpenInstance++)
    {
        if (    !ddi_get_soft_state(g_pNemuUSBMonSolarisState, iOpenInstance) /* faster */
            &&  ddi_soft_state_zalloc(g_pNemuUSBMonSolarisState, iOpenInstance) == DDI_SUCCESS)
        {
            pState = ddi_get_soft_state(g_pNemuUSBMonSolarisState, iOpenInstance);
            break;
        }
    }
    if (!pState)
    {
        LogRel((DEVICE_NAME ":NemuUSBMonSolarisOpen: too many open instances."));
        mutex_enter(&g_NemuUSBMonSolarisMtx);
        g_cNemuUSBMonSolarisClient--;
        mutex_exit(&g_NemuUSBMonSolarisMtx);
        return ENXIO;
...
static int NemuUSBMonSolarisClose(dev_t Dev, int fFlag, int fType, cred_t *pCred)
{
    ...
    LogFunc((DEVICE_NAME ":NemuUSBMonSolarisClose\n"));

    pState = ddi_get_soft_state(g_pNemuUSBMonSolarisState, getminor(Dev));
    if (!pState)
    {
        LogRel((DEVICE_NAME ":NemuUSBMonSolarisClose: failed to get pState.\n"));
        return EFAULT;
    }
     ...
     */
    NemuUSBFilterRemoveOwner(pState->Process);

    ddi_soft_state_free(g_pNemuUSBMonSolarisState, getminor(Dev));
    pState = NULL;
...
static int NemuUSBMonSolarisRead(dev_t Dev, struct uio *pUio, cred_t *pCred)
{
    LogFunc((DEVICE_NAME ":NemuUSBMonSolarisRead\n"));
    return 0;
}
...
static int NemuUSBMonSolarisWrite(dev_t Dev, struct uio *pUio, cred_t *pCred)
{
    LogFunc((DEVICE_NAME ":NemuUSBMonSolarisWrite\n"));
    return 0;
}
...
static int NemuUSBMonSolarisIOCtl(dev_t Dev, int Cmd, intptr_t pArg, int Mode, cred_t *pCred, int *pVal)
{
    LogFunc((DEVICE_NAME ":NemuUSBMonSolarisIOCtl Dev=%d Cmd=%d pArg=%p Mode=%d\n", Dev, Cmd, pArg));

    /*
     * Get the session from the soft state item.
     */
    nemuusbmon_state_t *pState = ddi_get_soft_state(g_pNemuUSBMonSolarisState, getminor(Dev));
    if (!pState)
    {
        LogRel((DEVICE_NAME ":NemuUSBMonSolarisIOCtl: no state data for %d\n", getminor(Dev)));
        return EINVAL;
    }
    ...
    {
        LogRel((DEVICE_NAME ": NemuUSBMonSolarisIOCtl: bad request %#x size=%d expected=%d\n", Cmd, IOCPARM_LEN(Cmd), sizeof(ReqWrap)));
        return ENOTTY;
    }
    ...
    if (RT_UNLIKELY(rc))
    {
        LogRel((DEVICE_NAME ": NemuUSBMonSolarisIOCtl: ddi_copyin failed to read header pArg=%p Cmd=%d. rc=%d.\n", pArg, Cmd, rc));
        return EINVAL;
    }

    if (ReqWrap.u32Magic != NEMUUSBMON_MAGIC)
    {
        LogRel((DEVICE_NAME ": NemuUSBMonSolarisIOCtl: bad magic %#x; pArg=%p Cmd=%d.\n", ReqWrap.u32Magic, pArg, Cmd));
        return EINVAL;
    }
    if (RT_UNLIKELY(   ReqWrap.cbData == 0
                    || ReqWrap.cbData > _1M*16))
    {
        LogRel((DEVICE_NAME ": NemuUSBMonSolarisIOCtl: bad size %#x; pArg=%p Cmd=%d.\n", ReqWrap.cbData, pArg, Cmd));
        return EINVAL;
    }
    ...
    {
        LogRel((DEVICE_NAME ":NemuUSBMonSolarisIOCtl: RTMemTmpAlloc failed to alloc %d bytes.\n", ReqWrap.cbData));
        return ENOMEM;
    }
    ...
    {
        RTMemTmpFree(pvBuf);
        LogRel((DEVICE_NAME ":NemuUSBMonSolarisIOCtl: ddi_copyin failed; pvBuf=%p pArg=%p Cmd=%d. rc=%d\n", pvBuf, pArg, Cmd, rc));
        return EFAULT;
    }
    if (RT_UNLIKELY(   ReqWrap.cbData != 0
                    && !VALID_PTR(pvBuf)))
    {
        RTMemTmpFree(pvBuf);
        LogRel((DEVICE_NAME ":NemuUSBMonSolarisIOCtl: pvBuf invalid pointer %p\n", pvBuf));
        return EINVAL;
    }
    Log((DEVICE_NAME ":NemuUSBMonSolarisIOCtl: pid=%d.\n", (int)RTProcSelf()));

    /*
     * Process the IOCtl.
     */
    size_t cbDataReturned = 0;
    rc = nemuUSBMonSolarisProcessIOCtl(Cmd, pState, pvBuf, ReqWrap.cbData, &cbDataReturned);
    ReqWrap.rc = rc;
    rc = 0;

    if (RT_UNLIKELY(cbDataReturned > ReqWrap.cbData))
    {
        LogRel((DEVICE_NAME ":NemuUSBMonSolarisIOCtl: too much output data %d expected %d\n", cbDataReturned, ReqWrap.cbData));
        cbDataReturned = ReqWrap.cbData;
    }
    ...
    if (RT_LIKELY(!rc))
    {
        ...
        if (cbDataReturned > 0)
        {
            ...
            if (RT_UNLIKELY(rc))
            {
                LogRel((DEVICE_NAME ":NemuUSBMonSolarisIOCtl: ddi_copyout failed; pvBuf=%p pArg=%p Cmd=%d. rc=%d\n", pvBuf, pArg, Cmd, rc));
                rc = EFAULT;
            }
        }
    }
    ...
    {
        LogRel((DEVICE_NAME ":NemuUSBMonSolarisIOCtl: ddi_copyout(1) failed pArg=%p Cmd=%d\n", pArg, Cmd));
        rc = EFAULT;
    }
...
static int nemuUSBMonSolarisProcessIOCtl(int iFunction, void *pvState, void *pvData, size_t cbData, size_t *pcbReturnedData)
{
    ...
    nemuusbmon_state_t *pState = (nemuusbmon_state_t *)pvState;
    int rc;
...
#define CHECKRET_MIN_SIZE(mnemonic, cbMin) \
    do { \
        if (cbData < (cbMin)) \
        { \
            LogRel(("nemuUSBSolarisProcessIOCtl: " mnemonic ": cbData=%#zx (%zu) min is %#zx (%zu)\n", \
                 cbData, cbData, (size_t)(cbMin), (size_t)(cbMin))); \
            return VERR_BUFFER_OVERFLOW; \
        } \
        if ((cbMin) != 0 && !VALID_PTR(pvData)) \
        { \
            LogRel(("nemuUSBSolarisProcessIOCtl: " mnemonic ": Invalid pointer %p\n", pvData)); \
    ...
    switch (iFunction)
    {
        case NEMUUSBMON_IOCTL_ADD_FILTER:
        {
            CHECKRET_MIN_SIZE("ADD_FILTER", sizeof(NEMUUSBREQ_ADD_FILTER));

            NEMUUSBREQ_ADD_FILTER *pReq = (NEMUUSBREQ_ADD_FILTER *)pvData;
            PUSBFILTER pFilter = (PUSBFILTER)&pReq->Filter;

            Log(("nemuUSBMonSolarisProcessIOCtl: idVendor=%#x idProduct=%#x bcdDevice=%#x bDeviceClass=%#x bDeviceSubClass=%#x bDeviceProtocol=%#x bBus=%#x bPort=%#x\n",
            ...
            Log(("nemuUSBMonSolarisProcessIOCtl: Manufacturer=%s Product=%s Serial=%s\n",
            ...
            rc = NemuUSBFilterAdd(pFilter, pState->Process, &pReq->uId);
            *pcbReturnedData = cbData;
            Log((DEVICE_NAME ":nemuUSBMonSolarisProcessIOCtl: ADD_FILTER (Process:%d) returned %d\n", pState->Process, rc));
            break;
        }

        case NEMUUSBMON_IOCTL_REMOVE_FILTER:
        {
            CHECKRET_MIN_SIZE("REMOVE_FILTER", sizeof(NEMUUSBREQ_REMOVE_FILTER));

            NEMUUSBREQ_REMOVE_FILTER *pReq = (NEMUUSBREQ_REMOVE_FILTER *)pvData;
            rc = NemuUSBFilterRemove(pState->Process, (uintptr_t)pReq->uId);
            *pcbReturnedData = 0;
            Log((DEVICE_NAME ":nemuUSBMonSolarisProcessIOCtl: REMOVE_FILTER (Process:%d) returned %d\n", pState->Process, rc));
            break;
        }

        case NEMUUSBMON_IOCTL_RESET_DEVICE:
        {
            CHECKRET_MIN_SIZE("RESET_DEVICE", sizeof(NEMUUSBREQ_RESET_DEVICE));

            NEMUUSBREQ_RESET_DEVICE *pReq = (NEMUUSBREQ_RESET_DEVICE *)pvData;
            rc = nemuUSBMonSolarisResetDevice(pReq->szDevicePath, pReq->fReattach);
            *pcbReturnedData = 0;
            Log((DEVICE_NAME ":nemuUSBMonSolarisProcessIOCtl: RESET_DEVICE (Process:%d) returned %d\n", pState->Process, rc));
            break;
        }

        case NEMUUSBMON_IOCTL_CLIENT_INFO:
        {
            CHECKRET_MIN_SIZE("CLIENT_INFO", sizeof(NEMUUSBREQ_CLIENT_INFO));

            NEMUUSBREQ_CLIENT_INFO *pReq = (NEMUUSBREQ_CLIENT_INFO *)pvData;
            rc = nemuUSBMonSolarisClientInfo(pState, pReq);
            *pcbReturnedData = cbData;
            Log((DEVICE_NAME ":nemuUSBMonSolarisProcessIOCtl: CLIENT_INFO (Process:%d) returned %d\n", pState->Process, rc));
            break;
        }

        case NEMUUSBMON_IOCTL_GET_VERSION:
        {
            CHECKRET_MIN_SIZE("GET_VERSION", sizeof(NEMUUSBREQ_GET_VERSION));

            PNEMUUSBREQ_GET_VERSION pGetVersionReq = (PNEMUUSBREQ_GET_VERSION)pvData;
            pGetVersionReq->u32Major = NEMUUSBMON_VERSION_MAJOR;
            pGetVersionReq->u32Minor = NEMUUSBMON_VERSION_MINOR;
            *pcbReturnedData = sizeof(NEMUUSBREQ_GET_VERSION);
            rc = VINF_SUCCESS;
            Log((DEVICE_NAME ":nemuUSBMonSolarisProcessIOCtl: GET_VERSION returned %d\n", rc));
            break;
        }

        default:
        {
            LogRel((DEVICE_NAME ":nemuUSBMonSolarisProcessIOCtl: Unknown request (Process:%d) %#x\n", pState->Process, iFunction));
            *pcbReturnedData = 0;
...
static int nemuUSBMonSolarisResetDevice(char *pszDevicePath, bool fReattach)
{
    ...
    LogFunc((DEVICE_NAME ":nemuUSBMonSolarisResetDevice pszDevicePath=%s fReattach=%d\n", pszDevicePath, fReattach));
    ...
    if (pDeviceInfo)
    {
        ...
        for (;;)
        {
            ...
            if (!pTmpDeviceInfo)
            {
                LogRel((DEVICE_NAME ":nemuUSBMonSolarisResetDevice failed to get parent device info for %s\n", pszDevicePath));
    ...
    else
    {
        ...
        LogRel((DEVICE_NAME ":nemuUSBMonSolarisResetDevice Cannot obtain device info for %s\n", pszDevicePath));
    }
...
static int nemuUSBMonSolarisClientInfo(nemuusbmon_state_t *pState, PNEMUUSB_CLIENT_INFO pClientInfo)
{
    LogFunc((DEVICE_NAME ":nemuUSBMonSolarisClientInfo pState=%p pClientInfo=%p\n", pState, pClientInfo));
    ...
    mutex_enter(&g_NemuUSBMonSolarisMtx);
    nemuusbmon_client_t *pCur = g_pNemuUSBMonSolarisClients;
    nemuusbmon_client_t *pPrev = NULL;
    while (pCur)
    {
        if (strncmp(pClientInfo->szDeviceIdent, pCur->Info.szDeviceIdent, sizeof(pCur->Info.szDeviceIdent) - 1) == 0)
        {
            ...
            if (pCur->Info.pfnSetConsumerCredentials)
            {
                ...
                if (RT_FAILURE(rc))
                    LogRel((DEVICE_NAME ":nemuUSBMonSolarisClientInfo pfnSetConsumerCredentials failed. rc=%d\n", rc));
            }
            else
            ...
            Log((DEVICE_NAME ":nemuUSBMonSolarisClientInfo found. %s rc=%d\n", pClientInfo->szDeviceIdent, rc));
            return rc;
        }
        pPrev = pCur;
        pCur = pCur->pNext;
    ...
    LogRel((DEVICE_NAME ":nemuUSBMonSolarisClientInfo Failed to find client %s\n", pClientInfo->szDeviceIdent));
...
int NemuUSBMonSolarisRegisterClient(dev_info_t *pClientDip, PNEMUUSB_CLIENT_INFO pClientInfo)
{
    LogFunc((DEVICE_NAME ":NemuUSBMonSolarisRegisterClient pClientDip=%p pClientInfo=%p\n", pClientDip, pClientInfo));
    AssertPtrReturn(pClientInfo, VERR_INVALID_PARAMETER);

    if (RT_LIKELY(g_pDip))
    {
        nemuusbmon_client_t *pClient = RTMemAllocZ(sizeof(nemuusbmon_client_t));
        if (RT_LIKELY(pClient))
        {
            ...
            mutex_enter(&g_NemuUSBMonSolarisMtx);
            pClient->pNext = g_pNemuUSBMonSolarisClients;
            g_pNemuUSBMonSolarisClients = pClient;
            mutex_exit(&g_NemuUSBMonSolarisMtx);

            Log((DEVICE_NAME ":NemuUSBMonSolarisRegisterClient registered. %d %s %s\n",
...
int NemuUSBMonSolarisUnregisterClient(dev_info_t *pClientDip)
{
    LogFunc((DEVICE_NAME ":NemuUSBMonSolarisUnregisterClient pClientDip=%p\n", pClientDip));
    ...
    if (RT_LIKELY(g_pDip))
    {
        ...
        while (pCur)
        {
            if (pCur->pDip == pClientDip)
            {
                if (pPrev)
                    ...
                    g_pNemuUSBMonSolarisClients = pCur->pNext;

                mutex_exit(&g_NemuUSBMonSolarisMtx);

                Log((DEVICE_NAME ":NemuUSBMonSolarisUnregisterClient unregistered. %d %s %s\n",
        ...
        LogRel((DEVICE_NAME ":NemuUSBMonSolarisUnregisterClient Failed to find registered client %p\n", pClientDip));
        return VERR_NOT_FOUND;
    }
    return VERR_INVALID_STATE;
}
...
int NemuUSBMonSolarisElectDriver(usb_dev_descr_t *pDevDesc, usb_dev_str_t *pDevStrings, char *pszDevicePath, int Bus, int Port,
                                char **ppszDrv, void *pvReserved)
{
    LogFunc((DEVICE_NAME ":NemuUSBMonSolarisElectDriver pDevDesc=%p pDevStrings=%p pszDevicePath=%s Bus=%d Port=%d\n", pDevDesc,
    ...
    Log((DEVICE_NAME ":NemuUSBMonSolarisElectDriver: idVendor=%#x idProduct=%#x bcdDevice=%#x bDeviceClass=%#x bDeviceSubClass=%#x bDeviceProtocol=%#x bBus=%#x bPort=%#x\n",
    ...
    Log((DEVICE_NAME ":NemuUSBMonSolarisElectDriver: Manufacturer=%s Product=%s Serial=%s\n",
     ...
     */
    uintptr_t uId = 0;
    RTPROCESS Owner = NemuUSBFilterMatch(&Filter, &uId);
    ...
    *ppszDrv = ddi_strdup(NEMUUSB_DRIVER_NAME, KM_SLEEP);
