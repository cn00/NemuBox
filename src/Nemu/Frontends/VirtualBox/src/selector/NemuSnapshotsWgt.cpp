/* $Id: NemuSnapshotsWgt.cpp $ */
/** @file
 * Nemu Qt GUI - NemuSnapshotsWgt class implementation.
 */
...
# include "NemuSnapshotDetailsDlg.h"
# include "NemuSnapshotsWgt.h"
# include "NemuTakeSnapshotDlg.h"
...
public:
    ...
    SnapshotWgtItem (NemuSnapshotsWgt *pSnapshotWidget, QTreeWidget *aTreeWidget, const CSnapshot &aSnapshot)
    ...
    SnapshotWgtItem (NemuSnapshotsWgt *pSnapshotWidget, QTreeWidgetItem *aRootItem, const CSnapshot &aSnapshot)
    ...
    SnapshotWgtItem (NemuSnapshotsWgt *pSnapshotWidget, QTreeWidget *aTreeWidget, const CMachine &aMachine)
        : QTreeWidgetItem (aTreeWidget, ItemType)
        , m_pSnapshotWidget(pSnapshotWidget)
    ...
    SnapshotWgtItem (NemuSnapshotsWgt *pSnapshotWidget, QTreeWidgetItem *aRootItem, const CMachine &aMachine)
        : QTreeWidgetItem (aRootItem, ItemType)
        , m_pSnapshotWidget(pSnapshotWidget)
        , mIsCurrentState (true)
        , mMachine (aMachine)
    ...
    void recache()
    {
        if (mIsCurrentState)
        {
            ...
            setText (0, mCurStateModified ?
                        NemuSnapshotsWgt::tr ("Current State (changed)", "Current State (Modified)") :
                        NemuSnapshotsWgt::tr ("Current State", "Current State (Unmodified)"));
            mDesc = mCurStateModified ?
                    NemuSnapshotsWgt::tr ("The current state differs from the state stored in the current snapshot") :
                    parent() != 0 ?
                    NemuSnapshotsWgt::tr ("The current state is identical to the state stored in the current snapshot") :
                    QString::null;
        }
        else
        {
    ...
    SnapshotAgeFormat updateAge()
    {
        ...
        {
            age = NemuSnapshotsWgt::tr (" (%1)").arg (then.toString (Qt::LocalDate));
            ageFormat = AgeMax;
        }
        else if (then.secsTo (now) > 60 * 60 * 24)
        {
            age = NemuSnapshotsWgt::tr (" (%1 ago)").arg(NemuGlobal::daysToString(then.secsTo (now) / 60 / 60 / 24));
            ageFormat = AgeInDays;
        }
        else if (then.secsTo (now) > 60 * 60)
        {
            age = NemuSnapshotsWgt::tr (" (%1 ago)").arg(NemuGlobal::hoursToString(then.secsTo (now) / 60 / 60));
            ageFormat = AgeInHours;
        }
        else if (then.secsTo (now) > 60)
        {
            age = NemuSnapshotsWgt::tr (" (%1 ago)").arg(NemuGlobal::minutesToString(then.secsTo (now) / 60));
            ageFormat = AgeInMinutes;
        }
        else
        {
            age = NemuSnapshotsWgt::tr (" (%1 ago)").arg(NemuGlobal::secondsToString(then.secsTo (now)));
...
private:
    ...
    void recacheToolTip()
    {
        ...
        if (!mSnapshot.isNull())
        {
            ...
            if (bold())
                details = NemuSnapshotsWgt::tr (" (current, ", "Snapshot details");
            else
                details = " (";
            details += mOnline ? NemuSnapshotsWgt::tr ("online)", "Snapshot details")
                               : NemuSnapshotsWgt::tr ("offline)", "Snapshot details");

            if (dateTimeToday)
                dateTime = NemuSnapshotsWgt::tr ("Taken at %1", "Snapshot (time)").arg (dateTime);
            else
                dateTime = NemuSnapshotsWgt::tr ("Taken on %1", "Snapshot (date + time)").arg (dateTime);
        }
        else
        {
            dateTime = NemuSnapshotsWgt::tr ("%1 since %2", "Current State (time or date + time)")
    ...
    QPointer<NemuSnapshotsWgt> m_pSnapshotWidget;
...
NemuSnapshotsWgt::NemuSnapshotsWgt (QWidget *aParent)
    ...
    , m_fShapshotOperationsAllowed(false)
{
    ...
    Ui::NemuSnapshotsWgt::setupUi (this);
...
// #endif
    ...
    connect (gNemuEvents, SIGNAL(sigMachineDataChange(QString)),
             this, SLOT(machineDataChanged(QString)));
    connect (gNemuEvents, SIGNAL(sigMachineStateChange(QString, KMachineState)),
             this, SLOT(machineStateChanged(QString, KMachineState)));
    connect (gNemuEvents, SIGNAL(sigSessionStateChange(QString, KSessionState)),
...
void NemuSnapshotsWgt::setMachine (const CMachine &aMachine)
{
    mMachine = aMachine;
...
void NemuSnapshotsWgt::retranslateUi()
{
    /* Translate uic generated strings: */
    Ui::NemuSnapshotsWgt::retranslateUi(this);
...
void NemuSnapshotsWgt::onCurrentChanged (QTreeWidgetItem *aItem)
{
...
void NemuSnapshotsWgt::onContextMenuRequested (const QPoint &aPoint)
{
...
void NemuSnapshotsWgt::onItemChanged (QTreeWidgetItem *aItem)
{
    if (mEditProtector)
        return;
...
void NemuSnapshotsWgt::sltItemDoubleClicked(QTreeWidgetItem *pItem)
{
...
void NemuSnapshotsWgt::sltTakeSnapshot()
{
    takeSnapshot();
}

void NemuSnapshotsWgt::sltRestoreSnapshot(bool fSuppressNonCriticalWarnings /* = false*/)
{
    ...
    CSession session = nemuGlobal().openSession(mMachineId);
    if (session.isNull())
        return;
...
void NemuSnapshotsWgt::sltDeleteSnapshot()
{
    ...
    CSession session;
    if (busy)
        session = nemuGlobal().openExistingSession(mMachineId);
    else
        session = nemuGlobal().openSession(mMachineId);
    if (session.isNull())
        return;
...
void NemuSnapshotsWgt::sltShowSnapshotDetails()
{
    SnapshotWgtItem *item = !mTreeWidget->currentItem() ? 0 :
        static_cast <SnapshotWgtItem*> (mTreeWidget->currentItem());
    AssertReturn (item, (void) 0);
    ...
    NemuSnapshotDetailsDlg dlg (this);
...
void NemuSnapshotsWgt::sltCloneSnapshot()
{
    SnapshotWgtItem *item = !mTreeWidget->currentItem() ? 0 :
        static_cast <SnapshotWgtItem*> (mTreeWidget->currentItem());
    AssertReturn (item, (void) 0);
...
void NemuSnapshotsWgt::machineDataChanged(QString strId)
{
...
void NemuSnapshotsWgt::machineStateChanged(QString strId, KMachineState state)
{
    SnapshotEditBlocker guardBlock (mEditProtector);
...
void NemuSnapshotsWgt::sessionStateChanged(QString strId, KSessionState state)
{
    SnapshotEditBlocker guardBlock (mEditProtector);
...
void NemuSnapshotsWgt::updateSnapshotsAge()
{
...
bool NemuSnapshotsWgt::takeSnapshot()
{
    /* Prepare result: */
    bool fIsValid = true;
    ...
    if (fIsValid)
    {
        ...
        QPointer<NemuTakeSnapshotDlg> pDlg = new NemuTakeSnapshotDlg(pDlgParent, mMachine);
        windowManager().registerNewParent(pDlg, pDlgParent);

        /* Assign corresponding icon: */
        pDlg->mLbIcon->setPixmap(nemuGlobal().vmGuestOSTypeIcon(mMachine.GetOSTypeId()));
...
void NemuSnapshotsWgt::refreshAll()
{
    SnapshotEditBlocker guardBlock (mEditProtector);
...
SnapshotWgtItem* NemuSnapshotsWgt::findItem (const QString &aSnapshotId)
{
...
SnapshotWgtItem *NemuSnapshotsWgt::curStateItem()
{
...
void NemuSnapshotsWgt::populateSnapshots (const CSnapshot &aSnapshot, QTreeWidgetItem *aItem)
{
...
SnapshotAgeFormat NemuSnapshotsWgt::traverseSnapshotAge (QTreeWidgetItem *aParentItem)
{
