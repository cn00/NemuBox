/* $Id: NemuLA.cpp $ */
/** @file
 * NemuLA - Nemu Location Awareness notifications.
 */
...
static void laUpdateClientName(NEMULACONTEXT *pCtx)
{
...
static void laDoAttach(NEMULACONTEXT *pCtx)
{
...
static void laDoDetach(NEMULACONTEXT *pCtx)
{
...
static int laGetActiveClient(NEMULACONTEXT *pCtx, uint64_t *pu64Timestamp, uint32_t *pu32Value)
{
...
static int laUpdateCurrentState(NEMULACONTEXT *pCtx, uint32_t u32ActiveClientId, uint64_t u64ActiveClientTS)
{
...
static int laWait(NEMULACONTEXT *pCtx, uint64_t *pu64Timestamp, uint32_t u32Timeout)
{
...
static void laProcessClientInfo(NEMULACONTEXT *pCtx)
{
...
static void laProcessAttach(NEMULACONTEXT *pCtx)
{
...
static void laDoActions(NEMULACONTEXT *pCtx)
{
    /* Check if the attach was changed.
     *
...
int NemuLAInit(const NEMUSERVICEENV *pEnv, void **ppInstance, bool *pfStartThread)
{
    ...
    DWORD dwValue = 0;
    if (   laGetRegistryDWORD(L"SOFTWARE\\Oracle\\VirtualBox Guest Additions", L"NemuTrayLog", &dwValue)
        && (dwValue & 0x10) != 0)
    {
    ...
    dwValue = 0x02;
    if (   laGetRegistryDWORD(L"SOFTWARE\\Oracle\\VirtualBox Guest Additions", L"NemuTrayLA", &dwValue)
        && (dwValue & 0x02) == 0)
    {
...
void NemuLADestroy(const NEMUSERVICEENV *pEnv, void *pInstance)
{
    ...
    NEMULACONTEXT *pCtx = (NEMULACONTEXT *)pInstance;
