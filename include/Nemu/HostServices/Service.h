...
#ifndef ___Nemu_HostService_Service_h
#define ___Nemu_HostService_Service_h
...
public:
    Message(uint32_t uMsg, uint32_t cParms, NEMUHGCMSVCPARM aParms[])
        : m_uMsg(0)
        , m_cParms(0)
        , m_paParms(0)
    {
    ...
    int getData(uint32_t uMsg, uint32_t cParms, NEMUHGCMSVCPARM aParms[]) const
    {
        if (m_uMsg != uMsg)
        {
...
//            cleanup(aParms);
    ...
    int getParmU32Info(uint32_t iParm, uint32_t *pu32Info) const
    {
        ...
        AssertReturn(m_paParms[iParm].type == NEMU_HGCM_SVC_PARM_32BIT, VERR_INVALID_PARAMETER);
    ...
    int getParmU64Info(uint32_t iParm, uint64_t *pu64Info) const
    {
        ...
        AssertReturn(iParm < m_cParms, VERR_INVALID_PARAMETER);
        AssertReturn(m_paParms[iParm].type == NEMU_HGCM_SVC_PARM_64BIT, VERR_INVALID_PARAMETER);
    ...
    int getParmPtrInfo(uint32_t iParm, void **ppvAddr, uint32_t *pcSize) const
    {
        ...
        AssertReturn(iParm < m_cParms, VERR_INVALID_PARAMETER);
        AssertReturn(m_paParms[iParm].type == NEMU_HGCM_SVC_PARM_PTR, VERR_INVALID_PARAMETER);
    ...
    static int copyParms(uint32_t cParms, PNEMUHGCMSVCPARM paParmsSrc, PNEMUHGCMSVCPARM paParmsDst)
    {
...
private:
    ...
    uint32_t m_uMsg;
    uint32_t m_cParms;
    PNEMUHGCMSVCPARM m_paParms;

    int initData(uint32_t uMsg, uint32_t cParms, NEMUHGCMSVCPARM aParms[])
    {
        ...
        if (cParms > 0)
        {
            m_paParms = (NEMUHGCMSVCPARM*)RTMemAllocZ(sizeof(NEMUHGCMSVCPARM) * m_cParms);
            if (!m_paParms)
    ...
    static int copyParmsInternal(uint32_t cParms, PNEMUHGCMSVCPARM paParmsSrc, PNEMUHGCMSVCPARM paParmsDst, bool fCreatePtrs)
    {
        int rc = VINF_SUCCESS;
...
{
public:
    Client(uint32_t uClientId, NEMUHGCMCALLHANDLE hHandle = NULL,
           uint32_t uMsg = 0, uint32_t cParms = 0, NEMUHGCMSVCPARM aParms[] = NULL)
...
public:
    ...
    NEMUHGCMCALLHANDLE handle(void) const { return m_hHandle; }
...
public:
     ...
     */
    static DECLCALLBACK(int) svcLoad(NEMUHGCMSVCFNTABLE *pTable)
    {
...
protected:
    explicit AbstractService(PNEMUHGCMSVCHELPERS pHelpers)
        : m_pHelpers(pHelpers)
        , m_pfnHostCallback(NULL)
        , m_pvHostData(NULL)
    {}
    virtual int  init(NEMUHGCMSVCFNTABLE *ptable) { return VINF_SUCCESS; }
    ...
    virtual void guestCall(NEMUHGCMCALLHANDLE callHandle, uint32_t u32ClientID, void *pvClient, uint32_t eFunction, uint32_t cParms, NEMUHGCMSVCPARM paParms[]) = 0;
    virtual int  hostCall(uint32_t eFunction, uint32_t cParms, NEMUHGCMSVCPARM paParms[]) { return VINF_SUCCESS; }

    /** Type definition for use in callback functions. */
    typedef AbstractService SELF;
    /** HGCM helper functions. */
    PNEMUHGCMSVCHELPERS m_pHelpers;
    /*
    ...
    /**
     * @copydoc NEMUHGCMSVCFNTABLE::pfnUnload
    ...
    /**
     * @copydoc NEMUHGCMSVCFNTABLE::pfnConnect
    ...
    /**
     * @copydoc NEMUHGCMSVCFNTABLE::pfnCall
    ...
    /**
     * @copydoc NEMUHGCMSVCFNTABLE::pfnHostCall
    ...
    /**
     * @copydoc NEMUHGCMSVCFNTABLE::pfnRegisterExtension
...
#endif /* !___Nemu_HostService_Service_h */
