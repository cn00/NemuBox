/* $Id: NemuHlp.cpp $ */
/** @file
 * Nemu Qt GUI - Implementation of OS/2-specific helpers that require to reside in a DLL
 */
...
/* Parameters for the keyboard hook (NemuHlpInstallKbdHook()) */
 ...
 */
static
BOOL EXPENTRY nemuInputHook (HAB aHab, PQMSG aMsg, ULONG aFS)
{
    if (aMsg->msg == WM_CHAR)
    {
        /* For foreign processes that didn't call NemuHlpInstallKbdHook(),
         ...
         * called NemuHlpInstallKbdHook(), we ignore the message. */
...
/**
 *  Uninstalls the keyboard hook installed by NemuHlpInstallKbdHook().
 *  All arguments must match arguments passed to NemuHlpInstallKbdHook(),
