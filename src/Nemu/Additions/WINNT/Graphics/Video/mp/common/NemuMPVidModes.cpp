/* $Id: NemuMPVidModes.cpp $ */

/** @file
 * Nemu Miniport video modes related functions
 */
...
# define NEMU_WITHOUT_24BPP_MODES
#endif

/* Custom video modes which are being read from registry at driver startup. */
static VIDEO_MODE_INFORMATION g_CustomVideoModes[NEMU_VIDEO_MAX_SCREENS] = { 0 };

static BOOLEAN
NemuMPValidateVideoModeParamsGuest(PNEMUMP_DEVEXT pExt, uint32_t iDisplay, uint32_t xres, uint32_t yres, uint32_t bpp)
{
    switch (bpp)
    {
        case 32:
            break;
        case 24:
#ifdef NEMU_WITHOUT_24BPP_MODES
            return FALSE;
#else
            break;
#endif
...
static void
NemuFillVidModeBPP(VIDEO_MODE_INFORMATION *pMode, ULONG bitsR, ULONG bitsG, ULONG bitsB,
...
static void
NemuFillVidModeInfo(VIDEO_MODE_INFORMATION *pMode, ULONG xres, ULONG yres, ULONG bpp, ULONG index, ULONG yoffset)
{
    ...
    switch (bpp)
    {
#ifdef NEMU_WITH_8BPP_MODES
        case 8:
            NemuFillVidModeBPP(pMode, 6, 6, 6, 0, 0, 0);

            pMode->AttributeFlags |= VIDEO_MODE_PALETTE_DRIVEN | VIDEO_MODE_MANAGED_PALETTE;
            break;
#endif
        case 16:
            NemuFillVidModeBPP(pMode, 5, 6, 5, 0xF800, 0x7E0, 0x1F);
            break;
        case 24:
        case 32:
            NemuFillVidModeBPP(pMode, 8, 8, 8, 0xFF0000, 0xFF00, 0xFF);
            break;
        default:
            Assert(0);
            break;
...
void NemuMPCmnInitCustomVideoModes(PNEMUMP_DEVEXT pExt)
{
    ...
    NemuFillVidModeInfo(&g_CustomVideoModes[0], 800, 600, 32, 0, 0);
    ...
    for (iMode=0; iMode<NemuCommonFromDeviceExt(pExt)->cDisplays; ++iMode)
    {
        ...
        if (iMode==0)
        {
            ...
            rc = NemuMPCmnRegQueryDword(Registry, L"CustomXRes", &CustomXRes);
            NEMUMP_WARN_VPS_NOBP(rc);
            rc = NemuMPCmnRegQueryDword(Registry, L"CustomYRes", &CustomYRes);
            NEMUMP_WARN_VPS_NOBP(rc);
            rc = NemuMPCmnRegQueryDword(Registry, L"CustomBPP", &CustomBPP);
            NEMUMP_WARN_VPS_NOBP(rc);
        }
        else
        {
            ...
            rc = NemuMPCmnRegQueryDword(Registry, keyname, &CustomXRes);
            NEMUMP_WARN_VPS_NOBP(rc);
            swprintf(keyname, L"CustomYRes%d", iMode);
            rc = NemuMPCmnRegQueryDword(Registry, keyname, &CustomYRes);
            NEMUMP_WARN_VPS_NOBP(rc);
            swprintf(keyname, L"CustomBPP%d", iMode);
            rc = NemuMPCmnRegQueryDword(Registry, keyname, &CustomBPP);
            NEMUMP_WARN_VPS_NOBP(rc);
        }
        ...
        if (CustomXRes || CustomYRes || CustomBPP)
        {
            ...
            if (NemuMPValidateVideoModeParamsGuest(pExt, iMode, CustomXRes, CustomYRes, CustomBPP))
            {
                NemuFillVidModeInfo(&g_CustomVideoModes[iMode], CustomXRes, CustomYRes, CustomBPP, 0, 0);
            }
        }
    }
...
static bool NemuMPVideoModesMatch(const PVIDEO_MODE_INFORMATION pMode1, const PVIDEO_MODE_INFORMATION pMode2)
{
...
static int
NemuMPFindVideoMode(const PVIDEO_MODE_INFORMATION pModesTable, int cModes, const PVIDEO_MODE_INFORMATION pMode)
{
    for (int i = 0; i < cModes; ++i)
    {
        if (NemuMPVideoModesMatch(pMode, &pModesTable[i]))
        {
            return i;
        }
    }
 ...
 */
static uint32_t
NemuMPFillModesTable(PNEMUMP_DEVEXT pExt, int iDisplay, PVIDEO_MODE_INFORMATION pModesTable, size_t tableSize,
...
# endif
    ...
    size_t   maxModesPerColorDepth = NEMUMP_MAX_VIDEO_MODES / 2 / 4;
    ...
    for (; bytesPerPixel<=4; bytesPerPixel++)
    {
        ...
        if (!NemuMPValidateVideoModeParamsGuest(pExt, iMode, 800, 600, bitsPerPixel))
            continue;

        NemuFillVidModeInfo(&pModesTable[iMode], 800, 600, bitsPerPixel, iMode+1, 0);
    ...
    ULONG yOffset = NemuGetHeightReduction();
        ...
        for (cAdded=0, resIndex=0; resIndex<RT_ELEMENTS(resolutionMatrix) && cAdded<maxModesPerColorDepth; resIndex++)
        {
            ...
            if (
                     ...
                     * So add 1024x768 resolution for win8 guest to allow user switch to it */
                       (   (NemuQueryWinVersion() != WIN8 && NemuQueryWinVersion() != WIN81)
                        || resolutionMatrix[resIndex].xRes != 1024
                        || resolutionMatrix[resIndex].yRes != 768)
                    &&
#endif
                       !NemuLikesVideoMode(iDisplay, resolutionMatrix[resIndex].xRes,
            ...
            if (!NemuMPValidateVideoModeParamsGuest(pExt, iDisplay, resolutionMatrix[resIndex].xRes, resolutionMatrix[resIndex].yRes, bitsPerPixel))
            {
            ...
            NemuFillVidModeInfo(&pModesTable[iMode], resolutionMatrix[resIndex].xRes, resolutionMatrix[resIndex].yRes, bitsPerPixel, iMode+1, yOffset);
            ++iMode;
            ++cAdded;
        }
    }
    ...
    for (int curKey=0; curKey<128; curKey++)
    {
        ...
        rc = NemuMPCmnRegQueryDword(Registry, keyname, &xres);
        NEMUMP_CHECK_VPS_BREAK(rc);

        swprintf(keyname, L"CustomMode%dHeight", curKey);
        rc = NemuMPCmnRegQueryDword(Registry, keyname, &yres);
        NEMUMP_CHECK_VPS_BREAK(rc);

        swprintf(keyname, L"CustomMode%dBPP", curKey);
        rc = NemuMPCmnRegQueryDword(Registry, keyname, &bpp);
        NEMUMP_CHECK_VPS_BREAK(rc);
        ...
        if (!NemuLikesVideoMode(iDisplay, xres, yres, bpp))
        {
            /* host doesn't like this mode */
            break;
        }

        if (!NemuMPValidateVideoModeParamsGuest(pExt, iDisplay, xres, yres, bpp))
        {
            /* guest does not like this mode */
            continue;
        }
        ...
        NemuFillVidModeInfo(&pModesTable[iMode], xres, yres, bpp, iMode+1, yOffset);
        ...
        int foundIdx = NemuMPFindVideoMode(pModesTable, iMode, &pModesTable[iMode]);
...
static BOOLEAN NemuMPIsStartingUp(PNEMUMP_DEVEXT pExt, uint32_t iDisplay)
{
#ifdef NEMU_XPDM_MINIPORT
    return (pExt->CurrentMode == 0);
#else
    NEMUWDDM_SOURCE *pSource = &pExt->aSources[iDisplay];
    return !pSource->AllocData.SurfDesc.width || !pSource->AllocData.SurfDesc.height;
#endif
}

#ifdef NEMU_WDDM_MINIPORT
static const uint32_t g_aNemuVidModesSupportedBpps[] = {
        32
#ifndef NEMU_WITHOUT_24BPP_MODES
        , 24
#endif
        , 16
#ifdef NEMU_WITH_8BPP_MODES
...
DECLINLINE(BOOLEAN) NemuMPIsSupportedBpp(uint32_t bpp)
{
    for (int i = 0; i < RT_ELEMENTS(g_aNemuVidModesSupportedBpps); ++i)
    {
        if (bpp == g_aNemuVidModesSupportedBpps[i])
            return TRUE;
    }
    return FALSE;
}

DECLINLINE(uint32_t) NemuMPAdjustBpp(uint32_t bpp)
{
    if (NemuMPIsSupportedBpp(bpp))
        return bpp;
    Assert(g_aNemuVidModesSupportedBpps[0] == 32);
    return g_aNemuVidModesSupportedBpps[0];
}
#endif
 ...
 */
static BOOLEAN
NemuMPValidateVideoModeParams(PNEMUMP_DEVEXT pExt, uint32_t iDisplay, uint32_t &xres, uint32_t &yres, uint32_t &bpp)
{
    /* Make sure all important video mode values are set */
    if (NemuMPIsStartingUp(pExt, iDisplay))
    {
    ...
    else
    {
...
#else
        PNEMUWDDM_ALLOC_DATA pAllocData = pExt->aSources[iDisplay].pPrimaryAllocation ?
        ...
        /* Nemu WDDM driver does not allow 24 modes since OS could choose the 24bit mode as default in that case,
        ...
        bpp  = bpp  ? bpp : NemuMPAdjustBpp(pAllocData->SurfDesc.bpp);
#endif
    }
 ...
 */
static BOOLEAN
NemuMPCheckPendingVideoMode(PNEMUMP_DEVEXT pExt, PVIDEO_MODE_INFORMATION pPendingMode)
{
    uint32_t xres=0, yres=0, bpp=0, display=0;

    /* Check if there's a pending display change request for this display */
    if (NemuQueryDisplayRequest(&xres, &yres, &bpp, &display) && (xres || yres || bpp))
    {
        if (display >= RT_ELEMENTS(g_CustomVideoModes))
        {
            /*display = RT_ELEMENTS(g_CustomVideoModes) - 1;*/
            WARN(("NemuQueryDisplayRequest returned invalid display number %d", display));
            return FALSE;
        }
    }
    else
    ...
    if (NemuMPValidateVideoModeParams(pExt, display, xres, yres, bpp))
    {
        NemuFillVidModeInfo(pPendingMode, xres, yres, bpp, display, 0);
        return TRUE;
    }
...
static void NemuMPRegSaveModeInfo(PNEMUMP_DEVEXT pExt, uint32_t iDisplay, PVIDEO_MODE_INFORMATION pMode)
{
    NEMUMPCMNREGISTRY Registry;
    VP_STATUS rc;
    ...
    {
        /*First name without a suffix*/
        rc = NemuMPCmnRegSetDword(Registry, L"CustomXRes", pMode->VisScreenWidth);
        NEMUMP_WARN_VPS(rc);
        rc = NemuMPCmnRegSetDword(Registry, L"CustomYRes", pMode->VisScreenHeight);
        NEMUMP_WARN_VPS(rc);
        rc = NemuMPCmnRegSetDword(Registry, L"CustomBPP", pMode->BitsPerPlane);
        NEMUMP_WARN_VPS(rc);
    }
    else
    {
        ...
        rc = NemuMPCmnRegSetDword(Registry, keyname, pMode->VisScreenWidth);
        NEMUMP_WARN_VPS(rc);
        swprintf(keyname, L"CustomYRes%d", iDisplay);
        rc = NemuMPCmnRegSetDword(Registry, keyname, pMode->VisScreenHeight);
        NEMUMP_WARN_VPS(rc);
        swprintf(keyname, L"CustomBPP%d", iDisplay);
        rc = NemuMPCmnRegSetDword(Registry, keyname, pMode->BitsPerPlane);
        NEMUMP_WARN_VPS(rc);
    }
...
{
    return NemuMPCmnGetVideoModeInfo(pExt, pExt->CurrentMode - 1);
}
...
void NemuMPXpdmBuildVideoModesTable(PNEMUMP_DEVEXT pExt)
{
     ...
     * Up to NEMUMP_MAX_VIDEO_MODES elements can be used, the rest is reserved
     * for custom mode alternating indexes.
     */
    cStandartModes = NemuMPFillModesTable(pExt, pExt->iDevice, pExt->aVideoModes, NEMUMP_MAX_VIDEO_MODES, NULL);
    ...
    bPending = NemuMPCheckPendingVideoMode(pExt, &specialMode);
    ...
    if (!bHaveSpecial && NemuMPIsStartingUp(pExt, pExt->iDevice))
    {
        ...
        if (NemuMPValidateVideoModeParams(pExt, pExt->iDevice, xres, yres, bpp))
        {
            LOG(("Startup for screen %d validated %dx%d %d", pExt->iDevice, xres, yres, bpp));
            NemuFillVidModeInfo(&specialMode, xres, yres, bpp, 0, 0);
            bHaveSpecial = TRUE;
        }
    }
    ...
    if (bHaveSpecial)
    {
        ...
        if (!NemuMPIsStartingUp(pExt, pExt->iDevice))
        {
            if (bChanged)
            {
        ...
        NemuMPRegSaveModeInfo(pExt, pExt->iDevice, &specialMode);
    }
