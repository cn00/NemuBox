...
/** @file
 * Nemu feedback spu, context tracking.
 */
...
feedbackspu_NemuCreateContext( GLint con, const char *dpyName, GLint visual, GLint shareCtx )
{
    GLint ctx, slot;
    ...
    ctx = feedback_spu.child.NemuCreateContext(con, dpyName, visual, shareCtx);
...
{
    return feedbackspu_NemuCreateContext( 0, dpyName, visual, shareCtx );
}
