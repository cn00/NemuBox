...
public:
    ...
    void i_setAdditionsInfo(com::Utf8Str aInterfaceVersion, NEMUOSTYPE aOsType);
    void i_setAdditionsInfo2(uint32_t a_uFullVersion, const char *a_pszName, uint32_t a_uRevision, uint32_t a_fFeatures);
    bool i_facilityIsActive(NemuGuestFacilityType enmFacility);
    void i_facilityUpdate(NemuGuestFacilityType a_enmFacility, NemuGuestFacilityStatus a_enmStatus,
                          uint32_t a_fFlags, PCRTTIMESPEC a_pTimeSpecTS);
    ComObjPtr<Console> i_getConsole(void) { return mParent; }
    void i_setAdditionsStatus(NemuGuestFacilityType a_enmFacility, NemuGuestFacilityStatus a_enmStatus,
                              uint32_t a_fFlags, PCRTTIMESPEC a_pTimeSpecTS);
    void i_onUserStateChange(Bstr aUser, Bstr aDomain, NemuGuestUserState enmState, const uint8_t *puDetails, uint32_t cbDetails);
    ...
    NEMUOSTYPE  i_getGuestOSType(void) { return mData.mOSType; }
#ifdef NEMU_WITH_GUEST_CONTROL
    int         i_dispatchToSession(PNEMUGUESTCTRLHOSTCBCTX pCtxCb, PNEMUGUESTCTRLHOSTCALLBACK pSvcCb);
...
private:
    ...
    struct Data
    {
        Data() : mOSType(NEMUOSTYPE_Unknown),  mAdditionsRunLevel(AdditionsRunLevelType_None)
            , mAdditionsVersionFull(0), mAdditionsRevision(0), mAdditionsFeatures(0)
        { }

        NEMUOSTYPE                  mOSType;        /**@< For internal used. NEMUOSTYPE_Unknown if not reported. */
        Utf8Str                     mOSTypeId;
