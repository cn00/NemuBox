...
/** Caller indicator. */
typedef enum NEMUDTCALLER
{
    kNemuDtCaller_Invalid = 0,
    kNemuDtCaller_Generic,
    kNemuDtCaller_ProbeFireUser,
    kNemuDtCaller_ProbeFireKernel
} NEMUDTCALLER;
 ...
 */
typedef struct NemuDtStackData
{
    ...
    /** Credentials allocated by NemuDtGetCurrentCreds. */
    struct NemuDtCred      *pCred;
    /** Thread structure currently being held by this thread. */
    struct NemuDtThread    *pThread;
    /** Pointer to this structure.
     * This is the final bit of integrity checking. */
    struct NemuDtStackData *pSelf;
} VBDTSTACKDATA;
...
#define VBDT_CLEAR_STACK_DATA() \
    ...
    { \
        ...
        if (pStackData->pThread) \
            NemuDtReleaseThread(pStackData->pThread); \
    } while (0)
...
*********************************************************************************************************************************/
/** Per CPU information */
cpucore_t                       g_aNemuDtCpuCores[RTCPUSET_MAX_CPUS];
/** Dummy mutex. */
struct NemuDtMutex              g_DummyMtx;
/** Pointer to the tracer helpers provided by NemuDrv. */
static PCSUPDRVTRACERHLP        g_pNemuDTraceHlp;
 ...
 */
static PVBDTSTACKDATA nemuDtGetStackData(void)
{
 ...
 */
static DECLCALLBACK(void) nemuDtSyncCallback(RTCPUID idCpu, void *pvUser1, void *pvUser2)
{
...
{
    int rc = RTMpOnAll(nemuDtSyncCallback, NULL, NULL);
    AssertRC(rc);
}
...
{
    PVBDTSTACKDATA pData = nemuDtGetStackData();
    if (pData->enmCaller == kNemuDtCaller_ProbeFireKernel)
...
/* CRED implementation. */
cred_t *NemuDtGetCurrentCreds(void)
{
    PVBDTSTACKDATA pData = nemuDtGetStackData();
    if (!pData->pCred)
    {
...
static struct NemuDtThread *g_apThreadsHash[16384];
...
static struct NemuDtThread  g_aThreads[8192];


static int nemuDtInitThreadDb(void)
{
    int rc = RTSpinlockCreate(&g_hThreadSpinlock, RTSPINLOCK_FLAGS_INTERRUPT_SAFE, "NemuDtThreadDb");
    if (RT_FAILURE(rc))
        return rc;
...
static void nemuDtTermThreadDb(void)
{
...
struct NemuDtThread *NemuDtGetCurrentThread(void)
{
    ...
    struct NemuDtThread *pThread = g_apThreadsHash[iHash];
    while (pThread)
    {
        if (pThread->hNative == hNativeSelf)
        {
            ...
            /* Hold the thread in the on-stack data, making sure it does not
               get reused till the thread leaves NemuDTrace. */
     ...
     */
    pThread = RTListGetLast(&g_ThreadAgeList, struct NemuDtThread, AgeEntry);
    AssertFatal(pThread);
    RTListNodeRemove(&pThread->AgeEntry);
    if (pThread->hNative != NIL_RTNATIVETHREAD)
    {
        ...
        {
            for (struct NemuDtThread *pPrev = g_apThreadsHash[iHash2]; ; pPrev = pPrev->pNext)
            {
                AssertPtr(pPrev);
 ...
 *
 */
static void NemuDtReleaseThread(struct NemuDtThread *pThread)
{
    RTSpinlockAcquire(g_hThreadSpinlock);
...
#define NEMUDTVMEMCHUNK_BITS    ( ((_64K - 32 - sizeof(uint32_t) * 2) / sizeof(uint32_t)) * 32)

/**
 * Resource allocator chunk.
 */
typedef struct  NemuDtVMemChunk
{
    ...
    uint32_t            bm[NEMUDTVMEMCHUNK_BITS / 32];
} NEMUDTVMEMCHUNK;
/** Pointer to a resource allocator chunk. */
typedef NEMUDTVMEMCHUNK *PNEMUDTVMEMCHUNK;
...
typedef struct NemuDtVMem
{
    ...
    PNEMUDTVMEMCHUNK    apChunks[1];
} NEMUDTVMEM;
/** Pointer to a resource allocator instance. */
typedef NEMUDTVMEM *PNEMUDTVMEM;

/** Magic value for the NEMUDTVMEM structure. */
#define NEMUDTVMEM_MAGIC        RT_MAKE_U32_FROM_U8('V', 'M',  'e',  'm')
                                    ...
                                    size_t cbQCacheMax, uint32_t fFlags)
{
     ...
     */
    uint32_t cChunks = (uint32_t)cb / NEMUDTVMEMCHUNK_BITS;
    if (cb % NEMUDTVMEMCHUNK_BITS)
        cChunks++;
    PNEMUDTVMEM pThis = (PNEMUDTVMEM)RTMemAllocZ(RT_OFFSETOF(NEMUDTVMEM, apChunks[cChunks]));
    if (!pThis)
        return NULL;
    int rc = RTSpinlockCreate(&pThis->hSpinlock, RTSPINLOCK_FLAGS_INTERRUPT_SAFE, "NemuDtVMem");
    if (RT_FAILURE(rc))
    {
        RTMemFree(pThis);
        return NULL;
    }
    pThis->u32Magic     = NEMUDTVMEM_MAGIC;
    pThis->cCurFree     = 0;
    pThis->cCurChunks   = 0;
...
void  NemuDtVMemDestroy(struct NemuDtVMem *pThis)
{
    if (!pThis)
        return;
    AssertPtrReturnVoid(pThis);
    AssertReturnVoid(pThis->u32Magic == NEMUDTVMEM_MAGIC);
...
void *NemuDtVMemAlloc(struct NemuDtVMem *pThis, size_t cbMem, uint32_t fFlags)
{
    /*
     * Validate input.
     */
    ...
    AssertReturn(pThis->u32Magic == NEMUDTVMEM_MAGIC, NULL);
    ...
    for (;;)
    {
        ...
        if (RT_LIKELY(pThis->cCurFree > 0))
        {
            for (uint32_t iChunk = 0; iChunk < cChunks; iChunk++)
            {
                ...
                {
                    int iBit = ASMBitFirstClear(pChunk->bm, NEMUDTVMEMCHUNK_BITS);
                    AssertMsgReturnStmt(iBit >= 0 && (unsigned)iBit < NEMUDTVMEMCHUNK_BITS, ("%d\n", iBit),
                                        RTSpinlockRelease(pThis->hSpinlock),
                                        NULL);
         ...
         */
        uint32_t const  iFirstBit = cChunks > 0 ? pThis->apChunks[cChunks - 1]->iFirst + NEMUDTVMEMCHUNK_BITS : 0;
        uint32_t const  cFreeBits = cChunks + 1 == pThis->cMaxChunks
                                  ? pThis->cMaxItems - (iFirstBit - pThis->uBase)
                                  : NEMUDTVMEMCHUNK_BITS;
        Assert(cFreeBits <= NEMUDTVMEMCHUNK_BITS);

        RTSpinlockRelease(pThis->hSpinlock);

        pChunk = (PNEMUDTVMEMCHUNK)RTMemAllocZ(sizeof(*pChunk));
        if (!pChunk)
            return NULL;

        pChunk->iFirst   = iFirstBit;
        pChunk->cCurFree = cFreeBits;
        if (cFreeBits != NEMUDTVMEMCHUNK_BITS)
        {
            /* lazy bird. */
            uint32_t iBit = cFreeBits;
            while (iBit < NEMUDTVMEMCHUNK_BITS)
            {
                ASMBitSet(pChunk->bm, iBit);
                iBit++;
            }
...
void NemuDtVMemFree(struct NemuDtVMem *pThis, void *pvMem, size_t cbMem)
{
    /*
     * Validate input.
     */
     ...
     */
    RTSpinlockAcquire(pThis->hSpinlock);
    uint32_t const iChunk = uMem / NEMUDTVMEMCHUNK_BITS;
    if (iChunk < pThis->cCurChunks)
    {
        PNEMUDTVMEMCHUNK pChunk = pThis->apChunks[iChunk];
        uint32_t iBit = uMem - pChunk->iFirst;
        AssertReturnVoidStmt(iBit < NEMUDTVMEMCHUNK_BITS, RTSpinlockRelease(pThis->hSpinlock));
                                             ...
                                             void *pvUser, void *pvVM, uint32_t fFlags)
{
     ...
     */
    struct NemuDtMemCache *pThis = (struct NemuDtMemCache *)RTMemAlloc(sizeof(*pThis));
    if (!pThis)
        return NULL;
...
void  NemuDtKMemCacheDestroy(struct NemuDtMemCache *pThis)
{
    RTMemFree(pThis);
}
...
void *NemuDtKMemCacheAlloc(struct NemuDtMemCache *pThis, uint32_t fFlags)
{
    void    *pvMem;
#ifdef HAVE_RTMEMALLOCEX_FEATURES
    uint32_t fMemAllocFlags = (fFlags & KM_NOSLEEP ? RTMEMALLOCEX_FLAGS_ANY_CTX : 0) | RTMEMALLOCEX_FLAGS_ZEROED;
...
void  NemuDtKMemCacheFree(struct NemuDtMemCache *pThis, void *pvMem)
{
 ...
 */
static void nemuDtVtgConvAttr(dtrace_attribute_t *pDtAttr, PCVTGDESCATTR pVtgAttr)
{
 ...
 */
static const char *nemuDtVtgGetString(PVTGOBJHDR pVtgHdr, uint32_t offStrTab)
{
 ...
 */
static void     nemuDtPOps_Provide(void *pvProv, const dtrace_probedesc_t *pDtProbeDesc)
{
    ...
    for (uint32_t idxProbeLoc = 0; idxProbeLoc < cProbeLocs; idxProbeLoc++)
    {
        ...
        if (*pidProbe != 0)
         ...
         const char *pszPrbName = nemuDtVtgGetString(pProv->pHdr, pProbeDesc->offName);
         ...
         if (dtrace_probe_lookup(idProvider, pProv->pszModName, pszFnNmBuf, pszPrbName) != DTRACE_IDNONE)
         {
             ...
             if (dtrace_probe_lookup(idProvider, pProv->pszModName, pszFnNmBuf, pszPrbName) != DTRACE_IDNONE)
             {
                 ...
                 if (iOrd >= 128)
                 {
                     LogRel(("NemuDrv: More than 128 duplicate probe location instances at line %u in function %s [%s], probe %s\n",
 ...
 */
static int      nemuDtPOps_Enable(void *pvProv, dtrace_id_t idProbe, void *pvProbe)
{
    PSUPDRVVDTPROVIDERCORE  pProv   = (PSUPDRVVDTPROVIDERCORE)pvProv;
 ...
 */
static void     nemuDtPOps_Disable(void *pvProv, dtrace_id_t idProbe, void *pvProbe)
{
    PSUPDRVVDTPROVIDERCORE  pProv  = (PSUPDRVVDTPROVIDERCORE)pvProv;
    AssertPtrReturnVoid(pProv);
    LOG_DTRACE(("%s: %p / %p - %#x / %p\n", __FUNCTION__, pProv, pProv->TracerData.DTrace.idProvider, idProbe, pvProbe));
 ...
 */
static void     nemuDtPOps_GetArgDesc(void *pvProv, dtrace_id_t idProbe, void *pvProbe,
                                      dtrace_argdesc_t *pArgDesc)
{
    ...
    if (!pProv->TracerData.DTrace.fZombie)
    {
        ...
        {
            const char *pszType = nemuDtVtgGetString(pProv->pHdr, pArgList->aArgs[uArg].offType);
 ...
 */
static uint64_t nemuDtPOps_GetArgVal(void *pvProv, dtrace_id_t idProbe, void *pvProbe,
                                     int iArg, int cFrames)
{
    ...
    if (iArg >= 20)
        u64Ret = UINT64_MAX;
    else if (pData->enmCaller == kNemuDtCaller_ProbeFireKernel)
    {
#if ARCH_BITS == 64
    ...
    else if (pData->enmCaller == kNemuDtCaller_ProbeFireUser)
    {
 ...
 */
static void    nemuDtPOps_Destroy(void *pvProv, dtrace_id_t idProbe, void *pvProbe)
{
    PSUPDRVVDTPROVIDERCORE  pProv  = (PSUPDRVVDTPROVIDERCORE)pvProv;
    AssertPtrReturnVoid(pProv);
    LOG_DTRACE(("%s: %p / %p - %#x / %p\n", __FUNCTION__, pProv, pProv->TracerData.DTrace.idProvider, idProbe, pvProbe));
 ...
 */
static const dtrace_pops_t g_nemuDtVtgProvOps =
{
    /* .dtps_provide         = */ nemuDtPOps_Provide,
    /* .dtps_provide_module  = */ NULL,
    /* .dtps_enable          = */ nemuDtPOps_Enable,
    /* .dtps_disable         = */ nemuDtPOps_Disable,
    /* .dtps_suspend         = */ NULL,
    /* .dtps_resume          = */ NULL,
    /* .dtps_getargdesc      = */ nemuDtPOps_GetArgDesc,
    /* .dtps_getargval       = */ nemuDtPOps_GetArgVal,
    /* .dtps_usermode        = */ NULL,
    /* .dtps_destroy         = */ nemuDtPOps_Destroy
};
 ...
 */
static DECLCALLBACK(void) nemuDtTOps_ProbeFireKernel(struct VTGPROBELOC *pVtgProbeLoc, uintptr_t uArg0, uintptr_t uArg1, uintptr_t uArg2,
                                                     uintptr_t uArg3, uintptr_t uArg4)
{
    ...
    VBDT_SETUP_STACK_DATA(kNemuDtCaller_ProbeFireKernel);
 ...
 */
static DECLCALLBACK(void) nemuDtTOps_ProbeFireUser(PCSUPDRVTRACERREG pThis, PSUPDRVSESSION pSession, PCSUPDRVTRACERUSRCTX pCtx,
                                                   PCVTGOBJHDR pVtgHdr, PCVTGPROBELOC pProbeLocRO)
{
    ...
    VBDT_SETUP_STACK_DATA(kNemuDtCaller_ProbeFireUser);
 ...
 */
static DECLCALLBACK(int) nemuDtTOps_TracerOpen(PCSUPDRVTRACERREG pThis, PSUPDRVSESSION pSession, uint32_t uCookie,
                                               uintptr_t uArg, uintptr_t *puSessionData)
{
    ...
    VBDT_SETUP_STACK_DATA(kNemuDtCaller_Generic);

    int rc = dtrace_open((dtrace_state_t **)puSessionData, NemuDtGetCurrentCreds());
 ...
 */
static DECLCALLBACK(int) nemuDtTOps_TracerIoCtl(PCSUPDRVTRACERREG pThis, PSUPDRVSESSION pSession, uintptr_t uSessionData,
 ...
 * interface_method_impl{SUPDRVTRACERREG,pfnTracerClose}
 */
static DECLCALLBACK(void) nemuDtTOps_TracerClose(PCSUPDRVTRACERREG pThis, PSUPDRVSESSION pSession, uintptr_t uSessionData)
{
 ...
 */
static DECLCALLBACK(int) nemuDtTOps_ProviderRegister(PCSUPDRVTRACERREG pThis, PSUPDRVVDTPROVIDERCORE pCore)
{
    ...
    nemuDtVtgConvAttr(&DtAttrs.dtpa_provider, &pDesc->AttrSelf);
    nemuDtVtgConvAttr(&DtAttrs.dtpa_mod,      &pDesc->AttrModules);
    nemuDtVtgConvAttr(&DtAttrs.dtpa_func,     &pDesc->AttrFunctions);
    nemuDtVtgConvAttr(&DtAttrs.dtpa_name,     &pDesc->AttrNames);
    nemuDtVtgConvAttr(&DtAttrs.dtpa_args,     &pDesc->AttrArguments);
 ...
 */
static DECLCALLBACK(int) nemuDtTOps_ProviderDeregister(PCSUPDRVTRACERREG pThis, PSUPDRVVDTPROVIDERCORE pCore)
{
 ...
 */
static DECLCALLBACK(int) nemuDtTOps_ProviderDeregisterZombie(PCSUPDRVTRACERREG pThis, PSUPDRVVDTPROVIDERCORE pCore)
{
    uintptr_t idProvider = pCore->TracerData.DTrace.idProvider;
    LOG_DTRACE(("%s: %p / %p\n", __FUNCTION__, pThis, idProvider));
    AssertPtrReturn(idProvider, VERR_INTERNAL_ERROR_3);
...
/**
 * The tracer registration record of the Nemu DTrace implementation
 */
static SUPDRVTRACERREG g_NemuDTraceReg =
{
    SUPDRVTRACERREG_MAGIC,
    SUPDRVTRACERREG_VERSION,
    nemuDtTOps_ProbeFireKernel,
    nemuDtTOps_ProbeFireUser,
    nemuDtTOps_TracerOpen,
    nemuDtTOps_TracerIoCtl,
    nemuDtTOps_TracerClose,
    nemuDtTOps_ProviderRegister,
    nemuDtTOps_ProviderDeregister,
    nemuDtTOps_ProviderDeregisterZombie,
    SUPDRVTRACERREG_MAGIC
};
...
DECLEXPORT(int)  ModuleInit(void *hMod)
{
    ...
    if (rc == DDI_SUCCESS)
    {
        rc = SUPR0TracerRegisterImpl(hMod, NULL, &g_NemuDTraceReg, &g_pNemuDTraceHlp);
        if (RT_SUCCESS(rc))
            return rc;
