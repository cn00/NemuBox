                          ...
                          unsigned cFBInfos)
{
    ...
    for (uScreenId = 0; uScreenId < cFBInfos; uScreenId++)
    {
        ...
        {
            paFBInfos[uScreenId].pHostEvents->fu32Events |= NEMU_VIDEO_INFO_HOST_EVENTS_F_VRDP_RESET;
        }
    }
}
...
void Display::processAdapterData(void *pvVRAM, uint32_t u32VRAMSize)
{
    ...
    else
    {
         ...
         pu8 += u32VRAMSize - NEMU_VIDEO_ADAPTER_INFORMATION_SIZE;

         // @todo
         uint8_t *pu8End = pu8 + NEMU_VIDEO_ADAPTER_INFORMATION_SIZE;

         NEMUVIDEOINFOHDR *pHdr;

         for (;;)
         {
             pHdr = (NEMUVIDEOINFOHDR *)pu8;
             pu8 += sizeof(NEMUVIDEOINFOHDR);

             if (pu8 >= pu8End)
             {
                 LogRel(("NemuVideo: Guest adapter information overflow!!!\n"));
                 break;
             }

             if (pHdr->u8Type == NEMU_VIDEO_INFO_TYPE_DISPLAY)
             {
                 if (pHdr->u16Length != sizeof(NEMUVIDEOINFODISPLAY))
                 {
                     LogRel(("NemuVideo: Guest adapter information %s invalid length %d!!!\n", "DISPLAY", pHdr->u16Length));
                     break;
                 }

                 NEMUVIDEOINFODISPLAY *pDisplay = (NEMUVIDEOINFODISPLAY *)pu8;

                 if (pDisplay->u32Index >= mcMonitors)
                 {
                     LogRel(("NemuVideo: Guest adapter information invalid display index %d!!!\n", pDisplay->u32Index));
                     break;
                 }
                 ...
                 LogRelFlow(("NEMU_VIDEO_INFO_TYPE_DISPLAY: %d: at 0x%08X, size 0x%08X, info 0x%08X\n", pDisplay->u32Index,
                             pDisplay->u32Offset, pDisplay->u32FramebufferSize, pDisplay->u32InformationSize));
             }
             else if (pHdr->u8Type == NEMU_VIDEO_INFO_TYPE_QUERY_CONF32)
             {
                 if (pHdr->u16Length != sizeof(NEMUVIDEOINFOQUERYCONF32))
                 {
                     LogRel(("NemuVideo: Guest adapter information %s invalid length %d!!!\n", "CONF32", pHdr->u16Length));
                     break;
                 }

                 NEMUVIDEOINFOQUERYCONF32 *pConf32 = (NEMUVIDEOINFOQUERYCONF32 *)pu8;

                 switch (pConf32->u32Index)
                 {
                     case NEMU_VIDEO_QCI32_MONITOR_COUNT:
                     {
                         pConf32->u32Value = mcMonitors;
                     } break;

                     case NEMU_VIDEO_QCI32_OFFSCREEN_HEAP_SIZE:
                     {
                     ...
                     default:
                         LogRel(("NemuVideo: CONF32 %d not supported!!! Skipping.\n", pConf32->u32Index));
                 }
             }
             else if (pHdr->u8Type == NEMU_VIDEO_INFO_TYPE_END)
             {
                 if (pHdr->u16Length != 0)
                 {
                     LogRel(("NemuVideo: Guest adapter information %s invalid length %d!!!\n", "END", pHdr->u16Length));
             ...
             else if (pHdr->u8Type != NEMU_VIDEO_INFO_TYPE_NV_HEAP)
             {
                 /** @todo why is Additions/WINNT/Graphics/Miniport/NemuVideo. cpp pushing this to us? */
...
void Display::processDisplayData(void *pvVRAM, unsigned uScreenId)
{
    ...
    {
        LogRel(("NemuVideo: Guest display information invalid display index %d!!!\n", uScreenId));
        return;
    }
    ...
    for (;;)
    {
        ...
        if (pu8 >= pu8End)
        {
            LogRel(("NemuVideo: Guest display information overflow!!!\n"));
            break;
        }

        if (pHdr->u8Type == NEMU_VIDEO_INFO_TYPE_SCREEN)
        {
            if (pHdr->u16Length != sizeof(NEMUVIDEOINFOSCREEN))
            {
                LogRel(("NemuVideo: Guest display information %s invalid length %d!!!\n", "SCREEN", pHdr->u16Length));
                break;
            }

            NEMUVIDEOINFOSCREEN *pScreen = (NEMUVIDEOINFOSCREEN *)pu8;
            ...
            LogRelFlow(("NEMU_VIDEO_INFO_TYPE_SCREEN: (%p) %d: at %d,%d, linesize 0x%X, size %dx%d, bpp %d, flags 0x%02X\n",
        ...
        else if (pHdr->u8Type == NEMU_VIDEO_INFO_TYPE_HOST_EVENTS)
        {
            if (pHdr->u16Length != sizeof(NEMUVIDEOINFOHOSTEVENTS))
            {
                LogRel(("NemuVideo: Guest display information %s invalid length %d!!!\n", "HOST_EVENTS", pHdr->u16Length));
                break;
            }

            NEMUVIDEOINFOHOSTEVENTS *pHostEvents = (NEMUVIDEOINFOHOSTEVENTS *)pu8;

            pFBInfo->pHostEvents = pHostEvents;

            LogFlow(("NEMU_VIDEO_INFO_TYPE_HOSTEVENTS: (%p)\n",
                     pHostEvents));
        }
        else if (pHdr->u8Type == NEMU_VIDEO_INFO_TYPE_LINK)
        {
            if (pHdr->u16Length != sizeof(NEMUVIDEOINFOLINK))
            {
                LogRel(("NemuVideo: Guest adapter information %s invalid length %d!!!\n", "LINK", pHdr->u16Length));
                break;
            }

            NEMUVIDEOINFOLINK *pLink = (NEMUVIDEOINFOLINK *)pu8;
            pu8 += pLink->i32Offset;
        }
        else
        {
