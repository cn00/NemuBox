...
IEM_CIMPL_DEF_1(RT_CONCAT4(iemCImpl_repe_cmps_op,OP_SIZE,_addr,ADDR_SIZE), uint8_t, iEffSeg)
{
    ...
    uint64_t        uSrc1Base;
    NEMUSTRICTRC    rcStrict     = iemMemSegCheckReadAccessEx(pIemCpu, pSrc1Hid, iEffSeg, &uSrc1Base);
...
IEM_CIMPL_DEF_0(RT_CONCAT4(iemCImpl_repe_scas_,OP_rAX,_m,ADDR_SIZE))
{
    ...
    uint64_t        uBaseAddr;
    NEMUSTRICTRC rcStrict = iemMemSegCheckReadAccessEx(pIemCpu, &pCtx->es, X86_SREG_ES, &uBaseAddr);
    if (rcStrict != VINF_SUCCESS)
        return rcStrict;
...
IEM_CIMPL_DEF_1(RT_CONCAT4(iemCImpl_rep_movs_op,OP_SIZE,_addr,ADDR_SIZE), uint8_t, iEffSeg)
{
    ...
    uint64_t        uSrcBase;
    NEMUSTRICTRC rcStrict = iemMemSegCheckReadAccessEx(pIemCpu, pSrcHid, iEffSeg, &uSrcBase);
    if (rcStrict != VINF_SUCCESS)
        return rcStrict;
...
IEM_CIMPL_DEF_0(RT_CONCAT4(iemCImpl_stos_,OP_rAX,_m,ADDR_SIZE))
{
    ...
    uint64_t        uBaseAddr;
    NEMUSTRICTRC rcStrict = iemMemSegCheckWriteAccessEx(pIemCpu, &pCtx->es, X86_SREG_ES, &uBaseAddr);
    if (rcStrict != VINF_SUCCESS)
        return rcStrict;
...
IEM_CIMPL_DEF_1(RT_CONCAT4(iemCImpl_lods_,OP_rAX,_m,ADDR_SIZE), int8_t, iEffSeg)
{
    ...
    PCCPUMSELREGHID pSrcHid = iemSRegGetHid(pIemCpu, iEffSeg);
    uint64_t        uBaseAddr;
    NEMUSTRICTRC rcStrict = iemMemSegCheckReadAccessEx(pIemCpu, pSrcHid, iEffSeg, &uBaseAddr);
    if (rcStrict != VINF_SUCCESS)
        return rcStrict;
 ...
 */
DECLINLINE(bool) iemCImpl_ins_shouldPostponeCommitToRing3(NEMUSTRICTRC rcStrictMem)
{
    /*
 ...
 * @param   rcStrictMem     The status code returned by the memory write.
 */
DECLINLINE(NEMUSTRICTRC) iemCImpl_ins_mergePostponedCommitStatuses(NEMUSTRICTRC rcStrictPort, NEMUSTRICTRC rcStrictMem)
{
...
IEM_CIMPL_DEF_1(RT_CONCAT4(iemCImpl_ins_op,OP_SIZE,_addr,ADDR_SIZE), bool, fIoChecked)
{
    ...
    if (IOM_SUCCESS(rcStrict))
    {
        *puMem = (OP_TYPE)u32Value;
        NEMUSTRICTRC rcStrict2 = iemMemCommitAndUnmap(pIemCpu, puMem, IEM_ACCESS_DATA_W);
        ...
        else if (iemCImpl_ins_shouldPostponeCommitToRing3(rcStrict2))
        {
            ...
            Log(("%s: Postponing to ring-3; cbInstr=%#x u32Value=%#x rcStrict2=%Rrc rcStrict=%Rrc\n", __FUNCTION__,
                 cbInstr, u32Value, NEMUSTRICTRC_VAL(rcStrict2),  NEMUSTRICTRC_VAL(rcStrict)));
            rcStrict = iemCImpl_ins_mergePostponedCommitStatuses(rcStrict, rcStrict2);
        }
#endif
        else
            AssertLogRelMsgFailedReturn(("rcStrict2=%Rrc\n", NEMUSTRICTRC_VAL(rcStrict2)), RT_FAILURE_NP(rcStrict2) ? rcStrict2 : VERR_IEM_IPE_1);
    }
    return rcStrict;
}
...
{
    PCPUMCTX     pCtx     = pIemCpu->CTX_SUFF(pCtx);
    NEMUSTRICTRC rcStrict = RT_CONCAT(iemMemStoreDataU,OP_SIZE)(pIemCpu, X86_SREG_ES, pCtx->ADDR_rDI, (OP_TYPE)pIemCpu->PendingCommit.uValue);
    if (rcStrict == VINF_SUCCESS)
    {
        if (!pCtx->eflags.Bits.u1DF)
            pCtx->ADDR_rDI += OP_SIZE / 8;
