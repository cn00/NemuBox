...
/** State information needed for the service.  The main NemuClient code provides
...
static DECLCALLBACK(int) nemuClientMonitorThread(RTTHREAD self, void *pvUser)
{
    ...
    while (true)
    {
        ...
        if (   event.type != PropertyNotify
            ...
            || event.xproperty.atom != XInternAtom(pDisplay, "NEMUVIDEO_NO_VT", False))
            continue;
        LogRel(("NemuClient/Display: entered virtual terminal.\n"));
...
static int startMonitorThread(struct DISPLAYSTATE *pState)
{
    ...
    rc = RTThreadCreate(NULL, nemuClientMonitorThread, (void *)pState, 0, RTTHREADTYPE_INFREQUENT_POLLER, 0, "VT_MONITOR");
    if (rc != VINF_SUCCESS)
...
static void updateSizeHintsProperty(struct DISPLAYSTATE *pState)
{
    ...
    XChangeProperty(pState->pDisplay, DefaultRootWindow(pState->pDisplay), XInternAtom(pState->pDisplay, "NEMU_SIZE_HINTS", 0),
...
static void notifyXServerRandR11(struct DISPLAYSTATE *pState)
{
        ...
        && pState->paScreenInformation[0].cx > 0 && pState->paScreenInformation[0].cy > 0)
    {
        ...
        LogRel(("NemuClient/Display: executed \"%s\", returned %d.\n", szCommand, ret));
...
static void updateMouseCapabilities(struct DISPLAYSTATE *pState)
{
    ...
    XChangeProperty(pState->pDisplay, DefaultRootWindow(pState->pDisplay),
                    XInternAtom(pState->pDisplay, "NEMU_MOUSE_CAPABILITIES", 0), XA_INTEGER, 32, PropModeReplace,
...
static void runDisplay(struct DISPLAYSTATE *pState)
{
    ...
    /* Semantics: when NEMU_HAS_GRAPHICS is set, the X server driver assumes
     ...
     */
    XChangeProperty(pState->pDisplay, DefaultRootWindow(pState->pDisplay), XInternAtom(pState->pDisplay, "NEMU_HAS_GRAPHICS", 0),
