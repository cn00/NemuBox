...
#ifndef NEMU_WDDM_WIN8
int g_bNemuVDbgBreakF = 0;
int g_bNemuVDbgBreakFv = 0;
#else
int g_bNemuVDbgBreakF = 0;
int g_bNemuVDbgBreakFv = 0;
#endif
...
#pragma alloc_text(PAGE, NemuQueryWinVersion)
#pragma alloc_text(PAGE, NemuGetHeightReduction)
#pragma alloc_text(PAGE, NemuLikesVideoMode)
#pragma alloc_text(PAGE, NemuQueryDisplayRequest)
#pragma alloc_text(PAGE, NemuQueryHostWantsAbsolute)
#pragma alloc_text(PAGE, NemuQueryPointerPos)
...
nemuWinVersion_t NemuQueryWinVersion()
{
    ...
    static nemuWinVersion_t s_WinVersion = WINVERSION_UNKNOWN;
...
bool NemuLikesVideoMode(uint32_t display, uint32_t width, uint32_t height, uint32_t bpp)
{
    ...
    if (RT_FAILURE(rc))
    {
        ...
        /* Most likely the NemuGuest driver is not loaded.
...
static bool NemuQueryPointerPosInternal(uint16_t *pPosX, uint16_t *pPosY)
{
    bool bRC = FALSE;
...
{
    return NemuQueryPointerPosInternal(NULL, NULL);
}
...
bool NemuQueryPointerPos(uint16_t *pPosX, uint16_t *pPosY)
{
    ...
    return NemuQueryPointerPosInternal(pPosX, pPosY);
}
