...
public:
    ...
    STDMETHOD(HandleEvent)(NemuEventType_T aType, IEvent *aEvent)
    {
        switch (aType)
        {
            case NemuEventType_OnGuestFileStateChanged:
            case NemuEventType_OnGuestFileOffsetChanged:
            case NemuEventType_OnGuestFileRead:
            case NemuEventType_OnGuestFileWrite:
            {
...
NEMU_LISTENER_DECLARE(GuestFileListenerImpl)
    ...
    if (RT_SUCCESS(vrc))
    {
        try
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                eventTypes.push_back(NemuEventType_OnGuestFileStateChanged);
                eventTypes.push_back(NemuEventType_OnGuestFileOffsetChanged);
                eventTypes.push_back(NemuEventType_OnGuestFileRead);
                eventTypes.push_back(NemuEventType_OnGuestFileWrite);
...
int GuestFile::i_callbackDispatcher(PNEMUGUESTCTRLHOSTCBCTX pCbCtx, PNEMUGUESTCTRLHOSTCALLBACK pSvcCb)
{
...
int GuestFile::i_onFileNotify(PNEMUGUESTCTRLHOSTCBCTX pCbCtx, PNEMUGUESTCTRLHOSTCALLBACK pSvcCbData)
{
    AssertPtrReturn(pCbCtx, VERR_INVALID_POINTER);
    ...
    switch (dataCb.uType)
    {
        ...
        case GUEST_FILE_NOTIFYTYPE_OPEN:
        {
            if (pSvcCbData->mParms == 4)
            {
                ...
                AssertMsg(mData.mID == NEMU_GUESTCTRL_CONTEXTID_GET_OBJECT(pCbCtx->uContextID),
                          ("File ID %RU32 does not match context ID %RU32\n", mData.mID,
                           NEMU_GUESTCTRL_CONTEXTID_GET_OBJECT(pCbCtx->uContextID)));
...
int GuestFile::i_onGuestDisconnected(PNEMUGUESTCTRLHOSTCBCTX pCbCtx, PNEMUGUESTCTRLHOSTCALLBACK pSvcCbData)
{
    AssertPtrReturn(pCbCtx, VERR_INVALID_POINTER);
    AssertPtrReturn(pSvcCbData, VERR_INVALID_POINTER);
...
HRESULT GuestFile::i_setErrorExternal(VirtualBoxBase *pInterface, int guestRc)
{
    ...
    return pInterface->setError(NEMU_E_IPRT_ERROR, GuestFile::i_guestErrorToString(guestRc).c_str());
}
...
int GuestFile::i_setFileStatus(FileStatus_T fileStatus, int fileRc)
{
    ...
    if (mData.mStatus != fileStatus)
    {
        ...
        {
            hr = errorInfo->initEx(NEMU_E_IPRT_ERROR, fileRc,
                                     ...
                                     uint32_t uTimeoutMS, uint64_t *puOffset)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        if (evtType == NemuEventType_OnGuestFileOffsetChanged)
        {
            if (puOffset)
            {
                             ...
                             void *pvData, size_t cbData, uint32_t *pcbRead)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        if (evtType == NemuEventType_OnGuestFileRead)
        {
                                     ...
                                     FileStatus_T *pFileStatus, int *pGuestRc)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        Assert(evtType == NemuEventType_OnGuestFileStateChanged);
                              ...
                              uint32_t uTimeoutMS, uint32_t *pcbWritten)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        if (evtType == NemuEventType_OnGuestFileWrite)
        {
            if (pcbWritten)
            {
