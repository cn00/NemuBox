...
# endif /* NEMU_STRICT || LOG_ENABLED */
       ...
       )
    {
        ...
        if (GCPtrIDT && iGate * sizeof(NEMUIDTE) >= cbIDT)
            goto failure;
         ...
         *
         */
        pIDTEntry = (RTGCPTR)((RTGCUINTPTR)GCPtrIDT + sizeof(NEMUIDTE) * iGate);
#ifdef IN_RC
        ...
        if (    RT_SUCCESS(rc)
            ...
            &&  (GuestIdte.Gen.u5Type2 == NEMU_IDTE_TYPE2_TRAP_32 || GuestIdte.Gen.u5Type2 == NEMU_IDTE_TYPE2_INT_32)
           ...
           )
        {
            ...
            pHandler = (RTGCPTR)NEMUIDTE_OFFSET(GuestIdte);
            ...
            if (rc == VINF_SUCCESS)
            {
                ...
                if (rc == VINF_SUCCESS)
                {
                    ...
                    if (GuestIdte.Gen.u5Type2 == NEMU_IDTE_TYPE2_INT_32)
                        eflags.Bits.u1IF = 0;
            ...
            Log(("MMRamRead %RGv size %d failed with %Rrc\n", (RTGCUINTPTR)GCPtrIDT + sizeof(NEMUIDTE) * iGate, sizeof(GuestIdte), rc));
    }
    else
    {
 ...
 *
 * @returns Nemu status code fit for scheduling.
