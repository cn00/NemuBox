...
#ifndef ___Nemu_vmm_pdmifs_h
#define ___Nemu_vmm_pdmifs_h
 ...
 * which normally means the same, i.e. the Main or NemuBFE API.  Picture the
...
typedef struct PDMIMOUSEPORT
{
     ...
     *
     * @returns Nemu status code.  Return VERR_TRY_AGAIN if you cannot process the
...
typedef struct NEMUVHWACMD *PNEMUVHWACMD; /**< @todo r=bird: A line what it is to make doxygen happy. */
...
struct NEMUVDMACMD_CHROMIUM_CMD; /* <- chromium [hgsmi] command */
struct NEMUVDMACMD_CHROMIUM_CTL; /* <- chromium [hgsmi] command */


/** Pointer to a display connector interface. */
typedef struct PDMIDISPLAYCONNECTOR *PPDMIDISPLAYCONNECTOR;
struct NEMUCRCMDCTL;
typedef DECLCALLBACKPTR(void, PFNCRCTLCOMPLETION)(struct NEMUCRCMDCTL* pCmd, uint32_t cbCmd, int rc, void *pvCompletion);
/**
...
typedef struct PDMIDISPLAYCONNECTOR
{
     ...
     * @thread  The emulation thread.
     */
    DECLR3CALLBACKMEMBER(int, pfnVHWACommandProcess,(PPDMIDISPLAYCONNECTOR pInterface, PNEMUVHWACMD pCmd));
     ...
     * @thread  The emulation thread.
     */
    DECLR3CALLBACKMEMBER(void, pfnCrHgsmiCommandProcess,(PPDMIDISPLAYCONNECTOR pInterface, struct NEMUVDMACMD_CHROMIUM_CMD* pCmd, uint32_t cbCmd));
     ...
     * @thread  The emulation thread.
     */
    DECLR3CALLBACKMEMBER(void, pfnCrHgsmiControlProcess,(PPDMIDISPLAYCONNECTOR pInterface, struct NEMUVDMACMD_CHROMIUM_CTL* pCtl, uint32_t cbCtl));
     ...
     * @thread  The emulation thread.
     */
    DECLR3CALLBACKMEMBER(int, pfnCrHgcmCtlSubmit,(PPDMIDISPLAYCONNECTOR pInterface, struct NEMUCRCMDCTL *pCmd, uint32_t cbCmd,
...
typedef struct PDMIVMMDEVCONNECTOR
{
     ...
     * @thread  The emulation thread.
     */
    DECLR3CALLBACKMEMBER(void, pfnUpdateGuestInfo,(PPDMIVMMDEVCONNECTOR pInterface, const struct NemuGuestInfo *pGuestInfo));
     ...
     *                              Use NEMU_FULL_VERSION_GET_MAJOR,
     *                              NEMU_FULL_VERSION_GET_MINOR and
     *                              NEMU_FULL_VERSION_GET_BUILD to access it.
     ...
     *                              by an underscore (NEMU_VERSION_STRING fashion).
     ...
     * @return  Nemu rc. VINF_SUCCESS if VBVA was enabled.
     * @thread  The emulation thread.
     */
     ...
     * @thread  The emulation thread.
     */
    DECLR3CALLBACKMEMBER(int, pfnReportStatistics,(PPDMIVMMDEVCONNECTOR pInterface, struct NemuGuestStatistics *pGuestStats));
 ...
 */
typedef struct NEMUHGCMCMD *PNEMUHGCMCMD;
...
typedef struct PDMIHGCMPORT
{
     ...
     *
     * @param   pInterface          Pointer to this interface.
     * @param   rc                  The return code (Nemu error code).
     * @param   pCmd                A pointer that identifies the completed command.
     *
     * @returns Nemu status code
     */
    DECLR3CALLBACKMEMBER(void, pfnCompleted,(PPDMIHGCMPORT pInterface, int32_t rc, PNEMUHGCMCMD pCmd));
...
typedef struct PDMIHGCMCONNECTOR
{
     ...
     * @thread  The emulation thread.
     */
    DECLR3CALLBACKMEMBER(int, pfnConnect,(PPDMIHGCMCONNECTOR pInterface, PNEMUHGCMCMD pCmd, PHGCMSERVICELOCATION pServiceLocation, uint32_t *pu32ClientID));
     ...
     * @thread  The emulation thread.
     */
    DECLR3CALLBACKMEMBER(int, pfnDisconnect,(PPDMIHGCMCONNECTOR pInterface, PNEMUHGCMCMD pCmd, uint32_t u32ClientID));
     ...
     * @thread  The emulation thread.
     */
    DECLR3CALLBACKMEMBER(int, pfnCall,(PPDMIHGCMCONNECTOR pInterface, PNEMUHGCMCMD pCmd, uint32_t u32ClientID, uint32_t u32Function,
                                       uint32_t cParms, PNEMUHGCMSVCPARM paParms));
