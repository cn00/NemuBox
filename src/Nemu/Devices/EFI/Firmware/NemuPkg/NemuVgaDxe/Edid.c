...
/**
  ...
  @param  Private             Pointer to NEMU_VGA_PRIVATE_DATA.
...
/**
  Construct the valid video modes for NemuVga.
  ...
  )
{
  ...
  BOOLEAN                                TimingMatch;
  NEMU_VGA_VIDEO_MODES          *VideoMode;
  EDID_TIMING                            TempTiming;
  ...
  if (EdidFound && 0) {
    ...
    VideoMode = &NemuVgaVideoModes[0];
    for (Index = 0; Index < NEMU_VGA_MODE_COUNT; Index++) {

      TimingMatch = TRUE;

      //
      // Check whether match with NemuVga video mode
      //
    ...
    Private->MaxMode = NEMU_VGA_MODE_COUNT;
  }
