...
/* dump stuff */
#pragma pack(1)
typedef struct NEMU_BITMAPFILEHEADER {
        uint16_t    bfType;
        uint32_t   bfSize;
        uint16_t    bfReserved1;
        uint16_t    bfReserved2;
        uint32_t   bfOffBits;
} NEMU_BITMAPFILEHEADER;

typedef struct NEMU_BITMAPINFOHEADER {
        uint32_t      biSize;
        int32_t       biWidth;
        int32_t       biHeight;
        uint16_t       biPlanes;
...
void crDmpImgBmp(CR_BLITTER_IMG *pImg, const char *pszFilename)
{
    ...
    NEMU_BITMAPFILEHEADER bf;

    bf.bfType = 'MB';
    bf.bfSize = sizeof (NEMU_BITMAPFILEHEADER) + sizeof (NEMU_BITMAPINFOHEADER) + pImg->cbData;
    bf.bfReserved1 = 0;
    bf.bfReserved2 = 0;
    bf.bfOffBits = sizeof (NEMU_BITMAPFILEHEADER) + sizeof (NEMU_BITMAPINFOHEADER);

    NEMU_BITMAPINFOHEADER bi;
...
NEMUDUMPDECL(size_t) crDmpFormatRawArray(char *pString, size_t cbString, const char *pszElFormat, uint32_t cbEl, const void *pvVal, uint32_t cVal)
{
    if (cbString < 2)
    {
...
NEMUDUMPDECL(size_t) crDmpFormatMatrixArray(char *pString, size_t cbString, const char *pszElFormat, uint32_t cbEl, const void *pvVal, uint32_t cX, uint32_t cY)
{
    if (cbString < 2)
    {
        crWarning("too few buffer size");
...
NEMUDUMPDECL(size_t) crDmpFormatArray(char *pString, size_t cbString, const char *pszElFormat, uint32_t cbEl, const void *pvVal, uint32_t cVal)
{
    switch(cVal)
    {
        case 1:
...
void crRecDumpBuffer(CR_RECORDER *pRec, CRContext *ctx, GLint idRedirFBO, NEMUVR_TEXTURE *pRedirTex)
{
    GLenum texTarget = 0;
...
void crRecDumpProgram(CR_RECORDER *pRec, CRContext *ctx, GLint id, GLint hwid)
{
    ...
    NemuGLhandleARB *pShaders = (NemuGLhandleARB*)crCalloc(cShaders * sizeof (*pShaders));
    if (!pShaders)
    {
        crWarning("crCalloc failed");
        crDmpStrF(pRec->pDumper, "WARNING: crCalloc failed");
...
void crRecDumpTextureV(CR_RECORDER *pRec, const NEMUVR_TEXTURE *pTex, const char *pszStr, va_list pArgList)
{
    CR_BLITTER_IMG Img = {0};
...
void crRecDumpTextureF(CR_RECORDER *pRec, const NEMUVR_TEXTURE *pTex, const char *pszStr, ...)
{
    va_list pArgList;
