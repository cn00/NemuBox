 ...
 *
 * Starting at Nemu 4.2 the context ID itself consists of a session ID, an object
...
{
    /** The call handle */
    NEMUHGCMCALLHANDLE mHandle;
    /** Number of parameters */
    uint32_t mNumParms;
    /** The call parameters */
    NEMUHGCMSVCPARM *mParms;
...
typedef struct HostCommand
{
     ...
     */
    int Allocate(uint32_t uMsg, uint32_t cParms, NEMUHGCMSVCPARM paParms[])
    {
        ...
        if (mParmCount)
        {
            mpParms = (NEMUHGCMSVCPARM*)RTMemAllocZ(sizeof(NEMUHGCMSVCPARM) * mParmCount);
            if (NULL == mpParms)
                rc = VERR_NO_MEMORY;
        }
        ...
        if (RT_SUCCESS(rc))
        {
            for (uint32_t i = 0; i < mParmCount; i++)
            {
                ...
                {
                    case NEMU_HGCM_SVC_PARM_32BIT:
                        mpParms[i].u.uint32 = paParms[i].u.uint32;
                        break;

                    case NEMU_HGCM_SVC_PARM_64BIT:
                        mpParms[i].u.uint64 = paParms[i].u.uint64;
                        break;

                    case NEMU_HGCM_SVC_PARM_PTR:
     ...
     */
    int CopyTo(NEMUHGCMSVCPARM paDstParms[], uint32_t cDstParms) const
    {
        LogFlowFunc(("pHostCmd=%p, mMsgType=%RU32, mParmCount=%RU32, mContextID=%RU32 (Session %RU32)\n",
                     this, mMsgType, mParmCount, mContextID, NEMU_GUESTCTRL_CONTEXTID_GET_SESSION(mContextID)));
...
typedef struct ClientState
{
    ...
    ClientState(PNEMUHGCMSVCHELPERS pSvcHelpers, uint32_t uClientID)
    ...
    bool WantsHostCommand(const HostCommand *pHostCmd) const
    {
        ...
        LogFlowFunc(("[Client %RU32] mFlags=0x%x, mContextID=%RU32 (session %RU32), mFilterMask=0x%x, mFilterValue=%RU32, fWant=%RTbool\n",
                     ...
                     NEMU_GUESTCTRL_CONTEXTID_GET_SESSION(pHostCmd->mContextID),
    ...
    PNEMUHGCMSVCHELPERS mSvcHelpers;
    /** The client's ID. */
    uint32_t mID;
...
public:
    explicit Service(PNEMUHGCMSVCHELPERS pHelpers)
        : mpHelpers(pHelpers)
    ...
    /**
     * @interface_method_impl{NEMUHGCMSVCFNTABLE,pfnUnload}
    ...
    /**
     * @interface_method_impl{NEMUHGCMSVCFNTABLE,pfnConnect}
    ...
    /**
     * @interface_method_impl{NEMUHGCMSVCFNTABLE,pfnCall}
    ...
    /**
     * @interface_method_impl{NEMUHGCMSVCFNTABLE,pfnHostCall}
    ...
    /**
     * @interface_method_impl{NEMUHGCMSVCFNTABLE,pfnRegisterExtension}
...
private:
    ...
    int prepareExecute(uint32_t cParms, NEMUHGCMSVCPARM paParms[]);
    int clientConnect(uint32_t u32ClientID, void *pvClient);
    int clientDisconnect(uint32_t u32ClientID, void *pvClient);
    int clientGetCommand(uint32_t u32ClientID, NEMUHGCMCALLHANDLE callHandle, uint32_t cParms, NEMUHGCMSVCPARM paParms[]);
    int clientSetMsgFilterSet(uint32_t u32ClientID, NEMUHGCMCALLHANDLE callHandle, uint32_t cParms, NEMUHGCMSVCPARM paParms[]);
    int clientSetMsgFilterUnset(uint32_t u32ClientID, NEMUHGCMCALLHANDLE callHandle, uint32_t cParms, NEMUHGCMSVCPARM paParms[]);
    int clientSkipMsg(uint32_t u32ClientID, NEMUHGCMCALLHANDLE callHandle, uint32_t cParms, NEMUHGCMSVCPARM paParms[]);
    int cancelHostCmd(uint32_t u32ContextID);
    int cancelPendingWaits(uint32_t u32ClientID, int rcPending);
    int hostCallback(uint32_t eFunction, uint32_t cParms, NEMUHGCMSVCPARM paParms[]);
    int hostProcessCommand(uint32_t eFunction, uint32_t cParms, NEMUHGCMSVCPARM paParms[]);
    void call(NEMUHGCMCALLHANDLE callHandle, uint32_t u32ClientID, void *pvClient, uint32_t eFunction, uint32_t cParms, NEMUHGCMSVCPARM paParms[]);
    int hostCall(uint32_t eFunction, uint32_t cParms, NEMUHGCMSVCPARM paParms[]);
    int sessionClose(uint32_t u32ClientID, NEMUHGCMCALLHANDLE callHandle, uint32_t cParms, NEMUHGCMSVCPARM paParms[]);
    int uninit(void);
};
 ...
 */
int Service::clientGetCommand(uint32_t u32ClientID, NEMUHGCMCALLHANDLE callHandle,
...
int Service::clientSetMsgFilterSet(uint32_t u32ClientID, NEMUHGCMCALLHANDLE callHandle,
                                   uint32_t cParms, NEMUHGCMSVCPARM paParms[])
{
    /*
     * Lookup client in our list so that we can assign the context ID of
...
int Service::clientSetMsgFilterUnset(uint32_t u32ClientID, NEMUHGCMCALLHANDLE callHandle,
                                     uint32_t cParms, NEMUHGCMSVCPARM paParms[])
{
    /*
     * Lookup client in our list so that we can assign the context ID of
...
int Service::clientSkipMsg(uint32_t u32ClientID, NEMUHGCMCALLHANDLE callHandle,
                           uint32_t cParms, NEMUHGCMSVCPARM paParms[])
{
    /*
     * Lookup client in our list so that we can assign the context ID of
 ...
 * @param   paParms                 Array of parameters.
 */
int Service::hostCallback(uint32_t eFunction, uint32_t cParms, NEMUHGCMSVCPARM paParms[])
{
    ...
    if (mpfnHostCallback)
    {
        NEMUGUESTCTRLHOSTCALLBACK data(cParms, paParms);
 ...
 * @param   paParms                 Array of parameters.
 */
int Service::hostProcessCommand(uint32_t eFunction, uint32_t cParms, NEMUHGCMSVCPARM paParms[])
{
    /*
     ...
     * waiting for a response from the guest side in case NemuService on
 ...
 * @thread  HGCM
 */
void Service::call(NEMUHGCMCALLHANDLE callHandle, uint32_t u32ClientID,
 ...
 * @thread  hgcm
 */
int Service::hostCall(uint32_t eFunction, uint32_t cParms, NEMUHGCMSVCPARM paParms[])
{
 ...
 * @param   paParms                     Array of parameters.
 */
int Service::sessionClose(uint32_t u32ClientID, NEMUHGCMCALLHANDLE callHandle, uint32_t cParms, NEMUHGCMSVCPARM paParms[])
{
    if (cParms < 2)
        return VERR_INVALID_PARAMETER;
    ...
    uint32_t uSessionID = NEMU_GUESTCTRL_CONTEXTID_GET_SESSION(uContextID);
