...
#define NEMUUSBHOOK_MEMTAG 'HUBV'
...
NTSTATUS NemuUsbHookUninstall(PNEMUUSBHOOK_ENTRY pHook)
{
    ...
    NemuDrvToolRefWaitEqual(&pHook->HookRef, 1);
...
VOID NemuUsbHookInit(PNEMUUSBHOOK_ENTRY pHook, PDRIVER_OBJECT pDrvObj, UCHAR iMjFunction, PDRIVER_DISPATCH pfnHook)
{
    ...
    NemuDrvToolRefInit(&pHook->HookRef);
...
static void nemuUsbHookRequestRegisterCompletion(PNEMUUSBHOOK_ENTRY pHook, PDEVICE_OBJECT pDevObj, PIRP pIrp, PIO_COMPLETION_ROUTINE pfnCompletion, PNEMUUSBHOOK_REQUEST pRequest)
{
...
{
    Assert(pfnCompletion);
    nemuUsbHookRequestRegisterCompletion(pHook, pDevObj, pIrp, pfnCompletion, pRequest);
...
#define PNEMUUSBHOOK_REQUEST_FROM_LE(_pLe) ( (PNEMUUSBHOOK_REQUEST)( ((uint8_t*)(_pLe)) - RT_OFFSETOF(NEMUUSBHOOK_REQUEST, ListEntry) ) )
...
VOID NemuUsbHookVerifyCompletion(PNEMUUSBHOOK_ENTRY pHook, PNEMUUSBHOOK_REQUEST pRequest, PIRP pIrp)
{
    ...
    {
        PNEMUUSBHOOK_REQUEST pCur = PNEMUUSBHOOK_REQUEST_FROM_LE(pLe);
        if (pCur != pRequest)
            continue;
        if (pCur->pIrp != pIrp)
            continue;
