...
#define NEMU_HGCM_SVC_NAME_MAX_BYTES 1024
...
{
    private:
        NEMUHGCMSVCHELPERS m_svcHelpers;
        ...
        RTLDRMOD m_hLdrMod;
        PFNNEMUHGCMSVCLOAD m_pfnLoad;

        NEMUHGCMSVCFNTABLE m_fntable;
        ...
        static DECLCALLBACK(void) svcHlpCallComplete(NEMUHGCMCALLHANDLE callHandle, int32_t rc);
    ...
    public:
        ...
        int HandleAcquired();
        int HandleReleased();
        int HostFastCallAsync(uint32_t u32Function, NEMUHGCMSVCPARM *pParm, PHGCMHOSTFASTCALLCB pfnCompletion,
                              void *pvCompletion);
#endif
        ...
        int GuestCall(PPDMIHGCMPORT pHGCMPort, PNEMUHGCMCMD pCmd, uint32_t u32ClientId,
                      uint32_t u32Function, uint32_t cParms, NEMUHGCMSVCPARM aParms[]);
};
...
int HGCMService::loadServiceDLL(void)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            m_fntable.u32Version = NEMU_HGCM_SVC_VERSION;
...
#define SVC_MSG_LOAD       (0)  /* Load the service library and call NEMU_HGCM_SVCLOAD_NAME entry point. */
    ...
    while (!fQuit)
    {
        ...
        switch (u32MsgId)
        {
            ...
            case SVC_MSG_GUESTCALL:
            {
                ...
                if (pClient)
                {
                    pSvc->m_fntable.pfnCall(pSvc->m_fntable.pvService, (NEMUHGCMCALLHANDLE)pMsg, pMsg->u32ClientId,
...
/* static */ DECLCALLBACK(void) HGCMService::svcHlpCallComplete(NEMUHGCMCALLHANDLE callHandle, int32_t rc)
{
...
int HGCMService::instanceCreate(const char *pszServiceLibrary, const char *pszServiceName)
{
    ...
    if (!strncmp(pszServiceName, RT_STR_TUPLE("NemuShared")))
        RTStrPrintf(szThreadName, sizeof(szThreadName), "Sh%s", pszServiceName + 10);
    else if (!strncmp(pszServiceName, RT_STR_TUPLE("Nemu")))
...
/* static */ int HGCMService::LoadState(PSSMHANDLE pSSM)
{
    ...
    while (cServices--)
    {
        ...
        AssertRCReturn(rc, rc);
        AssertReturn(u32 <= NEMU_HGCM_SVC_NAME_MAX_BYTES, VERR_SSM_UNEXPECTED_DATA);
 ...
 * @param pCmd           The Nemu HGCM context.
 ...
 * @return Nemu rc.
 */
int HGCMService::GuestCall(PPDMIHGCMPORT pHGCMPort, PNEMUHGCMCMD pCmd, uint32_t u32ClientId, uint32_t u32Function,
 ...
 * @return Nemu rc.
 */
int HGCMService::HostCall(uint32_t u32Function, uint32_t cParms, NEMUHGCMSVCPARM *paParms)
{
...
int HGCMService::HostFastCallAsync(uint32_t u32Function, NEMUHGCMSVCPARM *pParm, PHGCMHOSTFASTCALLCB pfnCompletion,
                                   void *pvCompletion)
{
...
int HGCMHostFastCallAsync(HGCMCVSHANDLE hSvc, uint32_t function, NEMUHGCMSVCPARM *pParm, PHGCMHOSTFASTCALLCB pfnCompletion,
                          void *pvCompletion)
{
