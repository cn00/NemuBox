...
# define NEMUGUEST_WITH_INPUT_DRIVER
#endif

#include "NemuGuestInternal.h"
#ifdef NEMUGUEST_WITH_INPUT_DRIVER
...
*********************************************************************************************************************************/
/** The device name. */
#define DEVICE_NAME             "nemuguest"
/** The device name for the device node open to everyone. */
#define DEVICE_NAME_USER        "nemuuser"
...
MODULE_DEVICE_TABLE(pci, g_NemuGuestPciId);

/** Structure for registering the PCI driver. */
static struct pci_driver  g_PciDriver =
{
    name:           DRIVER_NAME,
    id_table:       g_NemuGuestPciId,
    probe:          vgdrvLinuxProbePci,
    remove:         vgdrvLinuxTermPci
};

static PNEMUGUESTSESSION        g_pKernelSession = NULL;



/**
 * Converts a Nemu status code to a linux error code.
 *
...
{
    return VGDrvCommonIoCtl(NEMUGUEST_IOCTL_SET_MOUSE_STATUS, &g_DevExt, g_pKernelSession, &fStatus, sizeof(fStatus), NULL);
}
 ...
 */
static int nemuguestOpenInputDevice(struct input_dev *pDev)
{
 ...
 */
static void nemuguestCloseInputDevice(struct input_dev *pDev)
{
    NOREF(pDev);
...
static int __init vgdrvLinuxCreateInputDevice(void)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (g_pInputDevice)
        {
            ...
            g_pInputDevice->id.version = NEMU_SHORT_VERSION;
            g_pInputDevice->open       = nemuguestOpenInputDevice;
            g_pInputDevice->close      = nemuguestCloseInputDevice;
...
#endif /* NEMUGUEST_WITH_INPUT_DRIVER */
...
static int __init vgdrvLinuxModInit(void)
{
    ...
    rc = RTLogCreate(&pRelLogger, 0 /* fFlags */, "all",
                     "NEMU_RELEASE_LOG", RT_ELEMENTS(s_apszGroups), s_apszGroups,
...
# warning "huh? which arch + version is this?"
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    if (rc >= 0)
                    {
                        ...
                        if (rc >= 0)
                        {
                            ...
                            printk(KERN_DEBUG DEVICE_NAME ": Successfully loaded version "
                                   NEMU_VERSION_STRING " (interface " RT_XSTR(VMMDEV_VERSION) ")\n");
                            return rc;
                        }
                    ...
                    {
                        LogRel((DEVICE_NAME ": nemuguestCreateInputDevice failed with rc=%Rrc\n", rc));
...
#endif
    VGDrvCommonCloseSession(&g_DevExt, (PNEMUGUESTSESSION)pFilp->private_data);
...
#endif
{
    PNEMUGUESTSESSION   pSession = (PNEMUGUESTSESSION)pFilp->private_data;
    ...
    if (RT_LIKELY(copy_from_user(pvBuf, (void *)ulArg, cbData) == 0))
    {
        ...
        else
        {
            ...
            rc = -rc; Assert(rc > 0); /* Positive returns == negated Nemu error status codes. */
        }
    }
    else
    {
...
static unsigned int vgdrvLinuxPoll(struct file *pFile, poll_table *pPt)
{
    PNEMUGUESTSESSION   pSession  = (PNEMUGUESTSESSION)pFile->private_data;
...
EXPORT_SYMBOL(NemuGuestIDCOpen);
EXPORT_SYMBOL(NemuGuestIDCClose);
EXPORT_SYMBOL(NemuGuestIDCCall);
...
MODULE_DESCRIPTION(NEMU_PRODUCT " Guest Additions for Linux Module");
MODULE_LICENSE("GPL");
