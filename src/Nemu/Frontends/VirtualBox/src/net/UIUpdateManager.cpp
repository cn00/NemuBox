...
/** @file
 * Nemu Qt GUI - UIUpdateManager class implementation.
 */
...
private:
    ...
    void prepareNetworkRequest()
    {
        /* Compose query: */
        QUrl url(m_url);
        url.addQueryItem("platform", nemuGlobal().virtualBox().GetPackageType());
        ...
        if (nemuGlobal().brandingIsActive())
        {
            ...
            url.addQueryItem("version", QString("%1_%2_%3").arg(nemuGlobal().virtualBox().GetVersion())
                                                           .arg(nemuGlobal().virtualBox().GetRevision())
                                                           .arg(nemuGlobal().brandingGetKey("VerSuffix")));
        }
        else
        {
            /* Use hard coded version set by NEMU_VERSION_STRING: */
            url.addQueryItem("version", QString("%1_%2").arg(nemuGlobal().virtualBox().GetVersion())
                                                        .arg(nemuGlobal().virtualBox().GetRevision()));
        }
        url.addQueryItem("count", QString::number(gEDataManager->applicationUpdateCheckCounter()));
        url.addQueryItem("branch", NemuUpdateData(gEDataManager->applicationUpdateData()).branchName());
        QString strUserAgent(QString("VirtualBox %1 <%2>").arg(nemuGlobal().virtualBox().GetVersion()).arg(platformInfo()));
    ...
    static QString platformInfo()
    {
        ...
        {
            /* Run script: */
            QByteArray result = QIProcess::singleShot(QString(szAppPrivPath) + "/NemuSysInfo.sh");
            if (!result.isNull())
...
private slots:
    ...
    void sltStartStep()
    {
        ...
        QString strNemuVersion(nemuGlobal().nemuVersionStringNormalized());
        QByteArray abNemuVersion = strNemuVersion.toUtf8();
        NemuVersion nemuVersion(strNemuVersion);
        ...
        /* Skip the check in unstable Nemu version and if the extension pack
           is equal to or newer than Nemu.
           ...
           Note! Use RTStrVersionCompare for the comparison here as it takes
                 ...
                 comparing versions. */
        if (   nemuVersion.z() % 2 != 0
            || RTStrVersionCompare(abExtPackVersion.constData(), abNemuVersion.constData()) >= 0)
        {
            emit sigStepComplete();
            return;
        }
        ...
        if (strExtPackEdition.contains("ENTERPRISE"))
        {
            ...
            msgCenter().askUserToDownloadExtensionPack(GUI_ExtPackName, strExtPackVersion, strNemuVersion);
...
#ifdef NEMU_WITH_UPDATE_REQUEST
    /* Ask updater to check for the first time: */
    if (gEDataManager->applicationUpdateEnabled() && !nemuGlobal().isVMConsoleProcess())
        QTimer::singleShot(0, this, SLOT(sltCheckIfUpdateIsNecessary()));
#endif /* NEMU_WITH_UPDATE_REQUEST */
}
...
void UIUpdateManager::sltCheckIfUpdateIsNecessary(bool fForceCall /* = false */)
{
    ...
    NemuUpdateData currentData(gEDataManager->applicationUpdateData());
...
void UIUpdateManager::sltHandleUpdateFinishing()
{
    ...
    NemuUpdateData newData(currentData.periodIndex(), currentData.branchIndex());
