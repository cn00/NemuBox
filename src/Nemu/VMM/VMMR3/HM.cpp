...
static int hmR3InitFinalizeR0(PVM pVM)
{
    ...
    if (    !pVM->hm.s.vmx.fSupported
        ...
        &&  RTEnvExist("NEMU_HWVIRTEX_IGNORE_SVM_IN_USE"))
    {
        LogRel(("HM: NEMU_HWVIRTEX_IGNORE_SVM_IN_USE active!\n"));
        pVM->hm.s.svm.fSupported        = true;
 ...
 * @param   pvUser  Unused.
 */
static DECLCALLBACK(NEMUSTRICTRC) hmR3RemovePatches(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
 ...
 *
 */
static DECLCALLBACK(NEMUSTRICTRC) hmR3ReplaceTprInstr(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    /*
 ...
 *
 */
static DECLCALLBACK(NEMUSTRICTRC) hmR3PatchTprInstr(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    /*
     * Only execute the handler on the VCPU the original patch request was
     * issued. (The other CPU(s) might not yet have switched to protected
 ...
 * @param   pCtx        Pointer to the guest CPU context.
 */
VMMR3_INT_DECL(NEMUSTRICTRC) HMR3RestartPendingIOInstr(PVM pVM, PVMCPU pVCpu, PCPUMCTX pCtx)
{
    ...
    if (IOM_SUCCESS(rcStrict))
    {
        ...
        {
            NEMUSTRICTRC rcStrict2 = DBGFBpCheckIo(pVM, pVCpu, pCtx, pVCpu->hm.s.PendingIO.s.Port.uPort,
                                                   pVCpu->hm.s.PendingIO.s.Port.cbSize);
 ...
 *
 * @param   pVM         The cross context VM structure.
 * @param   iStatusCode Nemu status code.
 */
