...
./testdriver/
    ...
    the Nemu centric specialization is in ./testdriver/nemu.py. Various Nemu
    API wrappers that makes things easier to use and glosses over a lot of API
    version differences that live in ./testdriver/nemuwrappers.py.

    Test VM collections are often managed thru ./testdriver/nemutestvms.py, but
    ...
    There is also a NemuTXS client implementation in txsclient.py and a stacked
    test driver for installing Nemu (nemuinstaller.py).  Most test drivers will
    use the TXS client indirectly thru nemu.py methods.  The installer driver
...
./tests/
    ...
    nemu.py of course).  Most tests use one or more VMs from a standard set of
    preconfigured VMs defined by ./testdriver/nemutestvms.py (mentioned above),
...
./vms/
    ...
    ./testdrive/nemutestvms.py.  This will also contain description of how to
...
./utils/
    ...
    zip and on the NemuValidationKit.iso respectively.

    The Test eXecution Service (NemuTXS) found in ./utils/TestExecServ is one
    ...
    ./utils/TestExecServ/nemutxs-readme.txt for more details.
...
:Status: $Id: NemuValidationKitReadMe.txt $
