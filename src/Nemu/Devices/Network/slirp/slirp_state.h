...
#if defined(RT_OS_WINDOWS)
# define NEMU_SOCKET_EVENT (pData->phEvents[NEMU_SOCKET_EVENT_INDEX])
    HANDLE phEvents[NEMU_EVENT_COUNT];
#endif
#ifdef zone_mbuf
# undef zone_mbuf
#endif
...
# define QSOCKET_FOREACH(so, sonext, label)                              \
    for ((so)  = NEMU_X2(queue_ ## label ## _label).so_next;             \
         (so) != &(NEMU_X2(queue_ ## label ## _label));                  \
         (so) = (sonext))                                                \
    {                                                                    \
        (sonext) = (so)->so_next;                                        \
...
# define SOLOOKUP(so, label, src, sport, dst, dport)                                      \
    do {                                                                                  \
        (so) = solookup(&NEMU_X2(queue_ ## label ## _label), (src), (sport), (dst), (dport)); \
    } while (0)
