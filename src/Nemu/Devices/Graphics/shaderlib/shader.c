...
static void shader_chache_put(PNEMUEXT_HASHCACHE pCache, IWineD3DBaseShaderImpl *pShader);
#endif
...
#define NEMU_SHADER_FROM_CACHE_ENTRY(_pEntry) RT_FROM_MEMBER(RT_FROM_MEMBER((_pEntry), IWineD3DBaseShaderClass, CacheEntry), IWineD3DBaseShaderImpl, baseShader)

static DECLCALLBACK(void) shader_cache_cleanup_entry(void *pvKey, struct NEMUEXT_HASHCACHE_ENTRY *pEntry)
{
    IWineD3DBaseShaderImpl *pShader = NEMU_SHADER_FROM_CACHE_ENTRY(pEntry);
...
static void shader_chache_init(PNEMUEXT_HASHCACHE pCache)
{
    NemuExtCacheInit(pCache, 1024, shader_cache_hash, shader_cache_equal, shader_cache_cleanup_entry);
}

static void shader_chache_term(PNEMUEXT_HASHCACHE pCache)
{
    NemuExtCacheTerm(pCache);
}
...
static void shader_chache_put(PNEMUEXT_HASHCACHE pCache, IWineD3DBaseShaderImpl *pShader)
{
    ...
    NemuExtCachePut(pCache, pShader, &pShader->baseShader.CacheEntry);
}

static IWineD3DBaseShaderImpl* shader_chache_get(PNEMUEXT_HASHCACHE pCache, IWineD3DBaseShaderImpl *pShader)
{
    PNEMUEXT_HASHCACHE_ENTRY pEntry = NemuExtCacheGet(pCache, pShader);
    IWineD3DBaseShaderImpl *pCachedShader;

    if (!pEntry)
        return pShader;

    pCachedShader = NEMU_SHADER_FROM_CACHE_ENTRY(pEntry);
