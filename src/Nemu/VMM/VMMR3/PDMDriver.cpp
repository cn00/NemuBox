...
VMMR3DECL(int) PDMR3DrvStaticRegistration(PVM pVM, FNPDMNEMUDRIVERSREGISTER pfnCallback)
{
    ...
    int rc = pfnCallback(&RegCB.Core, NEMU_VERSION);
    if (RT_FAILURE(rc))
        AssertMsgFailed(("NemuDriversRegister failed with rc=%Rrc\n", rc));
...
int pdmR3DrvInit(PVM pVM)
{
    ...
    if (fLoadBuiltin)
    {
        ...
        rc = pdmR3DrvLoad(pVM, &RegCB, pszFilename, "NemuDD");
...
static int pdmR3DrvLoad(PVM pVM, PPDMDRVREGCBINT pRegCB, const char *pszFilename, const char *pszName)
{
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        FNPDMNEMUDRIVERSREGISTER *pfnNemuDriversRegister;
        rc = PDMR3LdrGetSymbolR3(pVM, pszName, "NemuDriversRegister", (void **)&pfnNemuDriversRegister);
        if (RT_SUCCESS(rc))
        {
            Log(("PDM: Calling NemuDriversRegister (%p) of %s (%s)\n", pfnNemuDriversRegister, pszName, pszFilename));
            rc = pfnNemuDriversRegister(&pRegCB->Core, NEMU_VERSION);
            if (RT_SUCCESS(rc))
        ...
        {
            AssertMsgFailed(("Failed to locate 'NemuDriversRegister' in %s (%s) rc=%Rrc\n", pszName, pszFilename, rc));
 ...
 *
 * @returns Nemu status code, including informational statuses.
 *
 * @param   pVM                 The cross context VM structure.
