...
*   Global Variables                                                                                                             *
*********************************************************************************************************************************/
static MALLOC_DEFINE(M_NEMUGUEST, "nemuguest", "VirtualBox Guest Device Driver");
...
/*
 * The /dev/nemuguest character device entry points.
 */
...
static void vgdrvFreeBSDClone(void *pvArg, struct ucred *pCred, char *pszName, int cchName, struct cdev **ppDev)
{
     ...
     * /dev/nemuguest<N> where N = {0...255}.
     */
    if (!ppDev)
        return;
    if (strcmp(pszName, "nemuguest") == 0)
        iUnit =  -1;
    else if (dev_stdclone(pszName, NULL, "nemuguest", &iUnit) != 1)
        return;
    if (iUnit >= 256)
    {
...
{
    PNEMUGUESTSESSION pSession = (PNEMUGUESTSESSION)pDev->si_drv1;
...
{
    struct NemuGuestDeviceState *pState = device_get_softc(pDevice);
...
static int vgdrvFreeBSDAddIRQ(device_t pDevice, void *pvState)
{
    int iResId = 0;
    int rc = 0;
    struct NemuGuestDeviceState *pState = (struct NemuGuestDeviceState *)pvState;
...
static int vgdrvFreeBSDAttach(device_t pDevice)
{
    int rc;
    int iResId;
    struct NemuGuestDeviceState *pState;
...
static driver_t vgdrvFreeBSDDriver =
{
    DEVICE_NAME,
    vgdrvFreeBSDMethods,
    sizeof(struct NemuGuestDeviceState),
};

static devclass_t vgdrvFreeBSDClass;

DRIVER_MODULE(nemuguest, pci, vgdrvFreeBSDDriver, vgdrvFreeBSDClass, 0, 0);
MODULE_VERSION(nemuguest, 1);
