...
/** @file
 * Nemu Qt GUI - UIWizardFirstRun class implementation.
 */
...
{
    /* Get 'nemu' global object: */
    CVirtualBox nemu = nemuGlobal().virtualBox();
    /* Determine machine 'OS type': */
    const CGuestOSType &osType = nemu.GetGuestOSType(m_machine.GetOSTypeId());
    ...
    UIMedium vmedium = nemuGlobal().medium(mediumId);
...
bool UIWizardFirstRun::isBootHardDiskAttached(const CMachine &machine)
{
    ...
    CVirtualBox nemu = nemuGlobal().virtualBox();
    /* Determine machine 'OS type': */
    const CGuestOSType &osType = nemu.GetGuestOSType(machine.GetOSTypeId());
    /* Determine recommended controller's 'bus' & 'type': */
