...
static void vrdpReportDirtyPathBounds(PNEMUDISPDEV pDev, CLIPOBJ *pco, PATHOBJ *ppo)
{
    RECTFX rcfxBounds;
    RECTL rclBounds;
...
static void vrdpReportDirtyPath(PNEMUDISPDEV pDev, CLIPOBJ *pco, PATHOBJ *ppo)
{
    vrdpReportDirtyPathBounds(pDev, pco, ppo);
}

static void vrdpReportDirtyClip(PNEMUDISPDEV pDev, CLIPOBJ *pco, RECTL *prcl)
{
    if (prcl)
    {
...
static void vrdpReportDirtyRects(PNEMUDISPDEV pDev, VRDPCLIPRECTS *pClipRects)
{
    /* Ignore rects, report entire area. */
    vrdpReportDirtyRect (pDev, &pClipRects->rclDst);
}

__inline BOOL vrdpWriteHdr (PNEMUDISPDEV pDev, uint32_t u32Op)
{
    return NemuVBVAWrite(&pDev->vbvaCtx, &pDev->hgsmi.ctx, &u32Op, sizeof (u32Op));
}

static BOOL vrdpWriteBits (PNEMUDISPDEV pDev, uint8_t *pu8Bits, int lDelta, int32_t x, int32_t y, uint32_t cWidth, uint32_t cHeight, int bytesPerPixel)
{
    BOOL bRc = FALSE;
    ...
    bRc = NemuVBVAWrite(&pDev->vbvaCtx, &pDev->hgsmi.ctx, &bits, sizeof (bits));

    if (bRc)
    {
        while (cHeight--)
        {
            bRc = NemuVBVAWrite(&pDev->vbvaCtx, &pDev->hgsmi.ctx, pu8Bits, cWidth * bytesPerPixel);
 ...
 */
static BOOL vrdpReportOrder(PNEMUDISPDEV pDev, const void *pOrder, unsigned cbOrder, unsigned code)
{
    BOOL bRc = vrdpWriteHdr(pDev, VRDP_MAKE_OP(code));

    if (bRc)
    {
        NemuVBVAWrite(&pDev->vbvaCtx, &pDev->hgsmi.ctx, pOrder, cbOrder);
    }

    return bRc;
}

static BOOL vrdpReportBounds(PNEMUDISPDEV pDev, const RECTL *prcl)
{
...
static BOOL vrdpReportRepeat(PNEMUDISPDEV pDev, const CLIPRECTS *pRects)
{
    BOOL bRc = TRUE;
...
BOOL vrdpReportOrderGeneric (PNEMUDISPDEV pDev,
                             const VRDPCLIPRECTS *pClipRects,
                             const void *pvOrder,
                             unsigned cbOrder,
                             unsigned code)
...
static void vrdpReportOrderGenericBounds (PNEMUDISPDEV pDev,
                                          const VRDPCLIPRECTS *pClipRects,
                                          const VRDEORDERBOUNDS *pBounds,
                                          const void *pvOrder,
                                          unsigned cbOrder,
...
static void vrdpReportSolidRect (PNEMUDISPDEV pDev,
                                 const RECTL *prclTrg,
...
static void vrdpReportSolidBlt (PNEMUDISPDEV pDev,
                                const RECTL *prclTrg,
                                VRDPCLIPRECTS *pClipRects,
                                ULONG rgb,
                                uint8_t rop3)
...
static void vrdpReportPatBlt (PNEMUDISPDEV pDev,
                              const RECTL *prclTrg,
                              VRDPCLIPRECTS *pClipRects,
                              VRDPBRUSH *pBrush,
                              POINTL *pptlBrush,
...
static void vrdpReportDstBlt (PNEMUDISPDEV pDev,
                              const RECTL *prclTrg,
                              VRDPCLIPRECTS *pClipRects,
                              uint8_t rop3)
{
...
static void vrdpReportScreenBlt (PNEMUDISPDEV pDev,
                                 const RECTL *prclTrg,
                                 VRDPCLIPRECTS *pClipRects,
                                 POINTL *pptlSrc,
                                 uint8_t rop3)
...
static void vrdpReportMemBltRect (PNEMUDISPDEV pDev,
                                  RECTL *prcl,
                                  int xSrc,
                                  int ySrc,
                                  uint8_t rop3,
...
static void vrdpReportMemBlt (PNEMUDISPDEV pDev,
                              VRDPCLIPRECTS *pClipRects,
                              POINTL *pptlSrc,
                              const uint8_t rop3,
                              const VRDPBCHASH *phash)
...
static void vrdpReportCachedBitmap (PNEMUDISPDEV pDev,
                                    SURFOBJ *psoSrc,
                                    const VRDPBCHASH *phash)
{
    BOOL bRc;
...
static void vrdpReportDeletedBitmap (PNEMUDISPDEV pDev,
                                     const VRDPBCHASH *phash)
{
                       ...
                       BRUSHOBJ  *pbo, POINTL *pptlBrushOrg, LINEATTRS *plineattrs, MIX mix)
{
    ...
    {
        if (NemuVBVAOrderSupported(&pDev->vbvaCtx, VRDE_ORDER_ELLIPSE))
        {
                    ...
                    BRUSHOBJ *pboOpaque, POINTL *pptlOrg, MIX mix)
{
    ...
    else
    {
        ...
        LOG(("calling nemuReportText fg %x bg %x", ulForeRGB, ulBackRGB));
...
void vrdpDrvBitBlt(SURFOBJ *psoTrg, SURFOBJ *psoSrc, SURFOBJ *psoMask, CLIPOBJ *pco, XLATEOBJ *pxlo,
                   ...
                   ROP4 rop4)
{
    PNEMUDISPDEV pDev = (PNEMUDISPDEV)psoTrg->dhpdev;
    ...
    clipResult = vrdpGetIntersectingClipRects(&clipRects, psoTrg, &rclTrg, pco,
                                              NemuDispIsScreenSurface(psoSrc)? pptlSrc: NULL);
    ...
    else
    {
        ...
        if (ROP3_NEED_SRC(rop4))
        {
            ...
            /* MEMBLT or SCREENBLT. */
            if (NemuDispIsScreenSurface(psoSrc))
            {
                        ...
                        XLATEOBJ *pxlo, ULONG iHatch)
{
    ...
    NemuDispDumpPSO(psoPattern, "psoPattern");
