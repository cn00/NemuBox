...
/** @file
 * Nemu Qt GUI - UILineTextEdit class definitions.
 */
...
{
    QString fileName = QIFileDialog::getOpenFileName(nemuGlobal().documentsPath(), tr("Text (*.txt);;All (*.*)"), this, tr("Select a file to open..."));
    if (!fileName.isEmpty())
    {
        QFile file(fileName);
