...
/**
 * Worker for NemuExtPackLoadDesc that loads the plug-in descriptors.
 *
 * @returns Same as NemuExtPackLoadDesc.
 * @param   pNemuExtPackElm
 ...
 */
static RTCString *
nemuExtPackLoadPlugInDescs(const xml::ElementNode *pNemuExtPackElm,
                           uint32_t *pcPlugIns, PNEMUEXTPACKPLUGINDESC *paPlugIns)
{
    *pcPlugIns = 0;
    *paPlugIns = NULL;

    /** @todo plug-ins */
    NOREF(pNemuExtPackElm);
 ...
 */
static void nemuExtPackClearDesc(PNEMUEXTPACKDESC a_pExtPackDesc)
{
...
{
    nemuExtPackClearDesc(a_pExtPackDesc);
}
 ...
 */
static RTCString *nemuExtPackLoadDescFromDoc(xml::Document *a_pDoc, PNEMUEXTPACKDESC a_pExtPackDesc)
{
    /*
     * Get the main element and check its version.
     */
    const xml::ElementNode *pNemuExtPackElm = a_pDoc->getRootElement();
    if (   !pNemuExtPackElm
        || strcmp(pNemuExtPackElm->getName(), "VirtualBoxExtensionPack") != 0)
        return new RTCString("No VirtualBoxExtensionPack element");

    RTCString strFormatVersion;
    if (!pNemuExtPackElm->getAttributeValue("version", strFormatVersion))
     ...
     */
    const xml::ElementNode *pNameElm = pNemuExtPackElm->findChildElement("Name");
    if (!pNameElm)
        return new RTCString("The 'Name' element is missing");
    const char *pszName = pNameElm->getValue();
    if (!NemuExtPackIsValidName(pszName))
        return &(new RTCString("Invalid name: "))->append(pszName);

    const xml::ElementNode *pDescElm = pNemuExtPackElm->findChildElement("Description");
    if (!pDescElm)
    ...
    const xml::ElementNode *pVersionElm = pNemuExtPackElm->findChildElement("Version");
    if (!pVersionElm)
    ...
    if (!NemuExtPackIsValidVersionString(pszVersion))
    ...
    if (!pVersionElm->getAttributeValue("edition", pszEdition))
        pszEdition = "";
    if (!NemuExtPackIsValidEditionString(pszEdition))
        return &(new RTCString("Invalid edition string: "))->append(pszEdition);

    const xml::ElementNode *pMainModuleElm = pNemuExtPackElm->findChildElement("MainModule");
    if (!pMainModuleElm)
    ...
    if (!NemuExtPackIsValidModuleString(pszMainModule))
    ...
    const xml::ElementNode *pVrdeModuleElm = pNemuExtPackElm->findChildElement("VRDEModule");
    if (pVrdeModuleElm)
    {
        pszVrdeModule = pVrdeModuleElm->getValue();
        if (!pszVrdeModule || *pszVrdeModule == '\0')
            pszVrdeModule = NULL;
        else if (!NemuExtPackIsValidModuleString(pszVrdeModule))
     ...
     */
    const xml::ElementNode *pShowLicenseElm = pNemuExtPackElm->findChildElement("ShowLicense");
     ...
     */
    uint32_t                cPlugIns  = 0;
    PNEMUEXTPACKPLUGINDESC  paPlugIns = NULL;
    RTCString *pstrRet = nemuExtPackLoadPlugInDescs(pNemuExtPackElm, &cPlugIns, &paPlugIns);
    if (pstrRet)
    {
        RTMemFree(paPlugIns);
        return pstrRet;
...
RTCString *NemuExtPackLoadDesc(const char *a_pszDir, PNEMUEXTPACKDESC a_pExtPackDesc, PRTFSOBJINFO a_pObjInfo)
{
    ...
    int vrc = RTPathJoin(szFilePath, sizeof(szFilePath), a_pszDir, NEMU_EXTPACK_DESCRIPTION_NAME);
    if (RT_FAILURE(vrc))
     ...
     */
    return nemuExtPackLoadDescFromDoc(&Doc, a_pExtPackDesc);
}
...
RTCString *NemuExtPackLoadDescFromVfsFile(RTVFSFILE hVfsFile, PNEMUEXTPACKDESC a_pExtPackDesc, PRTFSOBJINFO a_pObjInfo)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        RTCString           strFileName = NEMU_EXTPACK_DESCRIPTION_NAME;
        try
        {
     ...
     */
    if (RT_SUCCESS(rc))
        pstrErr = nemuExtPackLoadDescFromDoc(&Doc, a_pExtPackDesc);
...
RTCString *NemuExtPackExtractNameFromTarballPath(const char *pszTarball)
{
     ...
     */
    if (   off > NEMU_EXTPACK_NAME_MAX_LEN
        || off < NEMU_EXTPACK_NAME_MIN_LEN)
        return NULL;

    /*
     * Return the unmangled name.
     */
    return NemuExtPackUnmangleName(pszSrc, off);
}
 ...
 * @sa      NemuExtPackExtractNameFromTarballPath
 */
 ...
 * @sa      NemuExtPackUnmangleName, NemuExtPackIsValidMangledName
 */
RTCString *NemuExtPackMangleName(const char *pszName)
{
    AssertReturn(NemuExtPackIsValidName(pszName), NULL);

    char    szTmp[NEMU_EXTPACK_NAME_MAX_LEN + 1];
    size_t  off = 0;
    char    ch;
    ...
    szTmp[off] = '\0';
    Assert(NemuExtPackIsValidMangledName(szTmp));

    return new RTCString(szTmp, off);
}

/**
 * Unmangle an extension pack name (reverses NemuExtPackMangleName).
 *
 * @returns String containing the mangled name on success, the caller must
 *          delete it.  NULL on failure.
 * @param   pszMangledName      The mangled name.
 * @param   cchMax              The max name length.  RTSTR_MAX is fine.
 * @sa      NemuExtPackMangleName, NemuExtPackIsValidMangledName
 */
RTCString *NemuExtPackUnmangleName(const char *pszMangledName, size_t cchMax)
{
    AssertReturn(NemuExtPackIsValidMangledName(pszMangledName, cchMax), NULL);
    ...
    szTmp[off] = '\0';
    AssertReturn(NemuExtPackIsValidName(szTmp), NULL);
 ...
 *
 * A combination of RTPathJoin and NemuExtPackMangleName.
 *
...
{
    AssertReturn(NemuExtPackIsValidName(pszName), VERR_INTERNAL_ERROR_5);

    RTCString *pstrMangledName = NemuExtPackMangleName(pszName);
    if (!pstrMangledName)
 ...
 */
static int nemuExtPackReturnError(int rc, char *pszError, size_t cbError, const char *pszFormat, ...)
{
    va_list va;
 ...
 * @param   ...                 Format arguments.
 */
static void nemuExtPackSetError(char *pszError, size_t cbError, const char *pszFormat, ...)
{
    va_list va;
    va_start(va, pszFormat);
    RTStrPrintfV(pszError, cbError, pszFormat, va);
 ...
 *
 * @returns Nemu status code, failures with message.
 ...
 */
static int nemuExtPackVerifyXml(RTVFSFILE hXmlFile, const char *pszExtPackName, char *pszError, size_t cbError)
{
    /*
     * Load the XML.
     */
    NEMUEXTPACKDESC     ExtPackDesc;
    RTCString   *pstrErr = NemuExtPackLoadDescFromVfsFile(hXmlFile, &ExtPackDesc, NULL);
    if (pstrErr)
    {
    ...
    if (   pszExtPackName
        ...
        rc = nemuExtPackReturnError(VERR_NOT_EQUAL, pszError, cbError,
 ...
 * @param   cbError             The size of the buffer @a pszError points to.
 */
static int nemuExtPackVerifyManifestAndSignature(RTMANIFEST hOurManifest, RTVFSFILE hManifestFile, RTVFSFILE hSignatureFile,
                                                 char *pszError, size_t cbError)
{
    ...
    if (RT_FAILURE(rc))
        return nemuExtPackReturnError(rc, pszError, cbError, "RTVfsFileSeek failed: %Rrc", rc);

    RTMANIFEST hTheirManifest;
    rc = RTManifestCreate(0 /*fFlags*/, &hTheirManifest);
    if (RT_FAILURE(rc))
        return nemuExtPackReturnError(rc, pszError, cbError, "RTManifestCreate failed: %Rrc", rc);
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        {
            NEMU_EXTPACK_MANIFEST_NAME,
            NEMU_EXTPACK_SIGNATURE_NAME,
            "./" NEMU_EXTPACK_MANIFEST_NAME,
            "./" NEMU_EXTPACK_SIGNATURE_NAME,
            NULL
        };
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            nemuExtPackSetError(pszError, cbError, "Manifest mismatch: %s", szError);
        else
            nemuExtPackSetError(pszError, cbError, "RTManifestEqualsEx failed: %Rrc", rc);
#if 0
        ...
        nemuExtPackSetError(pszError, cbError, "Error parsing '%s': %Rrc", NEMU_EXTPACK_MANIFEST_NAME, rc);
 ...
 */
static int nemuExtPackVerifyFileDigest(RTMANIFEST hFileManifest, const char *pszFileDigest,
                                       RTCString *pStrDigest, char *pszError, size_t cbError)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
                ...
                && *pszFileDigest != '\0')
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    {
                        nemuExtPackSetError(pszError, cbError, "The extension pack file has changed (SHA-256 mismatch)");
                        rc = VERR_NOT_EQUAL;
                    }
                }
                else
                    nemuExtPackSetError(pszError, cbError, "Bad SHA-256 '%s': %Rrc", szCalculatedDigest, rc);
            }
        ...
        nemuExtPackSetError(pszError, cbError, "RTManifestEntryGetAttr: %Rrc", rc);
    return rc;
}
 ...
 *
 * @returns Nemu status code, failure message in @a pszError.
 ...
 * @param   cbError             The size of the @a pszError buffer.
 */
static int NemuExtPackValidateStandardFile(const char *pszAdjName, RTVFSOBJTYPE enmType,
                                           PRTVFSOBJ phVfsObj, PRTVFSFILE phVfsFile, char *pszError, size_t cbError)
{
    ...
    if (phVfsFile && *phVfsFile != NIL_RTVFSFILE)
        rc = nemuExtPackReturnError(VERR_DUPLICATE, pszError, cbError,
                                    "There can only be one '%s'", pszAdjName);
    else if (enmType != RTVFSOBJTYPE_IO_STREAM && enmType != RTVFSOBJTYPE_FILE)
        rc = nemuExtPackReturnError(VERR_NOT_A_FILE, pszError, cbError,
    ...
    else
    {
        ...
        if (RT_SUCCESS(rc))
        {
            if (!RTFS_IS_FILE(ObjInfo.Attr.fMode))
                ...
                rc = nemuExtPackReturnError(VERR_OUT_OF_RANGE, pszError, cbError,
            ...
            else
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        ...
                        if (RT_SUCCESS(rc))
                        {
                            ...
                            *phVfsObj = RTVfsObjFromFile(hVfsFile);
                            ...
                            nemuExtPackSetError(pszError, cbError, "RTVfsFileSeek failed on '%s': %Rrc", pszAdjName, rc);
                    }
                    ...
                    nemuExtPackSetError(pszError, cbError, "RTVfsMemorizeIoStreamAsFile failed on '%s': %Rrc", pszAdjName, rc);
                RTVfsIoStrmRelease(hVfsIos);
            }
        }
        else
            nemuExtPackSetError(pszError, cbError, "RTVfsObjQueryInfo failed on '%s': %Rrc", pszAdjName, rc);
    }
    return rc;
}
 ...
 * @param   cbError             The size of the buffer @a pszError points to.
 */
static int nemuExtPackValidateMemberName(const char *pszName, char *pszError, size_t cbError)
{
    if (RTPathStartsWithRoot(pszName))
        return nemuExtPackReturnError(VERR_PATH_IS_NOT_RELATIVE, pszError, cbError, "'%s': starts with root spec", pszName);
    ...
    while ((ch = *psz) != '\0')
    {
        ...
        if (psz - pszName > NEMU_EXTPACK_MAX_MEMBER_NAME_LENGTH)
        {
            pszErr = "Too long";
            break;
        }
    ...
    if (pszErr)
        return nemuExtPackReturnError(VERR_INVALID_NAME, pszError, cbError,
 ...
 * @param   cbError             The size of the buffer @a pszError points to.
 */
static int nemuExtPackValidateMemberFile(const char *pszName, RTVFSOBJ hVfsObj, char *pszError, size_t cbError)
{
    int rc = nemuExtPackValidateMemberName(pszName, pszError, cbError);
    if (RT_SUCCESS(rc))
    {
        RTFSOBJINFO ObjInfo;
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            nemuExtPackSetError(pszError, cbError, "RTVfsObjQueryInfo failed on '%s': %Rrc", pszName, rc);
    }
    return rc;
}
 ...
 * @param   cbError             The size of the buffer @a pszError points to.
 */
static int nemuExtPackValidateMemberDir(const char *pszName, RTVFSOBJ hVfsObj, char *pszError, size_t cbError)
{
    int rc = nemuExtPackValidateMemberName(pszName, pszError, cbError);
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            if (!RTFS_IS_DIRECTORY(ObjInfo.Attr.fMode))
                rc = nemuExtPackReturnError(VERR_NOT_A_DIRECTORY, pszError, cbError,
...
int NemuExtPackValidateMember(const char *pszName, RTVFSOBJTYPE enmType, RTVFSOBJ hVfsObj, char *pszError, size_t cbError)
{
    ...
    *pszError = '\0';
    ...
    if (   enmType == RTVFSOBJTYPE_FILE
        ...
        rc = nemuExtPackValidateMemberFile(pszName, hVfsObj, pszError, cbError);
    else if (   enmType == RTVFSOBJTYPE_DIR
             || enmType == RTVFSOBJTYPE_BASE)
        rc = nemuExtPackValidateMemberDir(pszName, hVfsObj, pszError, cbError);
    else
        rc = nemuExtPackReturnError(VERR_UNEXPECTED_FS_OBJ_TYPE, pszError, cbError,
 ...
 */
int NemuExtPackOpenTarFss(RTFILE hTarballFile, char *pszError, size_t cbError, PRTVFSFSSTREAM phTarFss,
                          PRTMANIFEST phFileManifest)
{
    ...
    *phTarFss = NIL_RTVFSFSSTREAM;
    ...
    if (RT_FAILURE(rc))
        return nemuExtPackReturnError(rc, pszError, cbError, "Failed seeking to the start of the tarball: %Rrc", rc);
    ...
    rc = RTVfsIoStrmFromRTFile(hTarballFile, RTFILE_O_READ | RTFILE_O_DENY_WRITE | RTFILE_O_OPEN, true /*fLeaveOpen*/,
                               &hTarballIos);
    if (RT_FAILURE(rc))
        return nemuExtPackReturnError(rc, pszError, cbError, "RTVfsIoStrmFromRTFile failed: %Rrc", rc);
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                nemuExtPackSetError(pszError, cbError, "RTZipTarFsStreamFromIoStream failed: %Rrc", rc);
                RTVfsIoStrmRelease(hGunzipIos);
            }
            else
                nemuExtPackSetError(pszError, cbError, "RTZipGzipDecompressIoStream failed: %Rrc", rc);
            RTVfsIoStrmRelease(hPtIos);
        }
        else
            nemuExtPackSetError(pszError, cbError, "RTManifestEntryAddPassthruIoStream failed: %Rrc", rc);
        RTManifestRelease(hFileManifest);
    }
    else
        nemuExtPackSetError(pszError, cbError, "RTManifestCreate failed: %Rrc", rc);
                               ...
                               PRTMANIFEST phValidManifest, PRTVFSFILE phXmlFile, RTCString *pStrDigest)
{
    ...
    *pszError = '\0';
    ...
    RTVFSFSSTREAM   hTarFss;
    int rc = NemuExtPackOpenTarFss(hTarballFile, pszError, cbError, &hTarFss, &hFileManifest);
    if (RT_FAILURE(rc))
        return rc;
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        for (;;)
        {
            ...
            {
                if (rc != VERR_EOF)
                    nemuExtPackSetError(pszError, cbError, "RTVfsFsStrmNext failed: %Rrc", rc);
                else
                    rc = VINF_SUCCESS;
                break;
            }
             ...
             */
            rc = NemuExtPackValidateMember(pszName, enmType, hVfsObj, pszError, cbError);
            if (RT_SUCCESS(rc))
            {
                PRTVFSFILE phVfsFile = NULL;
                if (!strcmp(pszAdjName, NEMU_EXTPACK_DESCRIPTION_NAME))
                    phVfsFile = &hXmlFile;
                else if (!strcmp(pszAdjName, NEMU_EXTPACK_MANIFEST_NAME))
                    phVfsFile = &hManifestFile;
                else if (!strcmp(pszAdjName, NEMU_EXTPACK_SIGNATURE_NAME))
                    phVfsFile = &hSignatureFile;
                else if (!strncmp(pszAdjName, NEMU_EXTPACK_LICENSE_NAME_PREFIX, sizeof(NEMU_EXTPACK_LICENSE_NAME_PREFIX) - 1))
                    rc = NemuExtPackValidateStandardFile(pszAdjName, enmType, &hVfsObj, NULL, pszError, cbError);
                if (phVfsFile)
                    rc = NemuExtPackValidateStandardFile(pszAdjName, enmType, &hVfsObj, phVfsFile, pszError, cbError);
            }
                    ...
                    || enmType == RTVFSOBJTYPE_IO_STREAM))
            {
                ...
                if (RT_FAILURE(rc))
                    nemuExtPackSetError(pszError, cbError, "RTManifestEntryAddIoStream failed on '%s': %Rrc", pszAdjName, rc);
                RTVfsIoStrmRelease(hVfsIos);
            }
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            rc = nemuExtPackVerifyFileDigest(hFileManifest, pszTarballDigest, pStrDigest, pszError, cbError);
        }
        ...
        if (RT_SUCCESS(rc))
        {
            if (hXmlFile == NIL_RTVFSFILE)
                rc = nemuExtPackReturnError(VERR_MISSING, pszError, cbError, "Mandator file '%s' is missing",
                                            NEMU_EXTPACK_DESCRIPTION_NAME);
         ...
         */
        if (RT_SUCCESS(rc))
            rc = nemuExtPackVerifyManifestAndSignature(hOurManifest, hManifestFile, hSignatureFile, pszError, cbError);

        /*
         * Check the XML.
         */
        if (RT_SUCCESS(rc))
            rc = nemuExtPackVerifyXml(hXmlFile, pszExtPackName, pszError, cbError);
