...
main(int argc, char **argv)
{
    ...
    /*
     * Note: When adding and/or modifying parameters of the nemusf mounting
     *       options you also would have to adjust NemuServiceAutoMount.cpp
     *       to keep this code here slick without having VbglR3.
     */
    err = mount(host_name, mount_point, "nemusf", flags, &mntinf);
    ...
    if (err == -1 && errno == EPROTO)
    {
        ...
        err = mount(host_name, mount_point, "nemusf", flags, &mntinf_old);
    }
    if (err)
