/* $Id: Init.cpp $ */
/** @file
 * NemuGuestLibR0 - Library initialization.
 */
 ...
 * because these values are provided by the NemuGuest driver and it might
...
static void vbglQueryDriverInfo (void)
{
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        NemuGuestPortInfo port;

        rc = vbglDriverIOCtl (&g_vbgldata.driver,
                              NEMUGUEST_IOCTL_GETVMMDEVPORT, &port,
                              sizeof (port));
...
DECLVBGL(int) VbglInitClient(void)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            /* Try to obtain VMMDev port via IOCTL to NemuGuest main driver. */
            vbglQueryDriverInfo ();
