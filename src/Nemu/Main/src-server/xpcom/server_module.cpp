...
const char NemuSVC_exe[] = RTPATH_SLASH_STR "NemuSVC" HOSTSUFF_EXE;

enum
{
    /** Amount of time to wait for the server to establish a connection, ms */
    NemuSVC_Timeout = 30000,
    /** How often to perform a connection check, ms */
    NemuSVC_WaitSlice = 100
};

/**
 *  Full path to the NemuSVC executable.
 */
static char NemuSVCPath[RTPATH_MAX];
static bool IsNemuSVCPathSet = false;
...
static nsresult nemusvcSpawnDaemon(void)
{
    ...
    char * const args[] = { (char *)NemuSVCPath, (char *)"--auto-shutdown", 0 };
    ...
    if (PR_ProcessAttrSetInheritableFD(attr, writable, NEMUSVC_STARTUP_PIPE_NAME) != PR_SUCCESS)
        goto end;
    ...
    if (PR_CreateProcessDetached(NemuSVCPath, args, nsnull, attr) != PR_SUCCESS)
        goto end;
    ...
    if (   PR_Read(readable, msg, sizeof(msg)-1) != 5
        || strcmp(msg, "READY"))
    {
        /* If several clients start NemuSVC simultaneously only one can
                      ...
                      void **aResult)
{
    ...
    do
    {
        *aResult = NULL;
        ...
        if (!IsNemuSVCPathSet)
        {
            /* Get the directory containing XPCOM components -- the NemuSVC
            ...
            if (NS_SUCCEEDED(rc))
            {
                ...
                if (NS_SUCCEEDED(rc))
                {
                    ...
                    AssertBreakStmt(path.Length() + strlen(NemuSVC_exe) < RTPATH_MAX,
                                    rc = NS_ERROR_FAILURE);

#if defined(RT_OS_SOLARIS) && defined(NEMU_WITH_HARDENING)
                    char achKernArch[128];
                    int cbKernArch = sysinfo(SI_ARCHITECTURE_K, achKernArch, sizeof(achKernArch));
                    if (cbKernArch > 0)
                    {
                        sprintf(NemuSVCPath, "/opt/VirtualBox/%s%s", achKernArch, NemuSVC_exe);
                        IsNemuSVCPathSet = true;
                    }
                    else
                        rc = NS_ERROR_UNEXPECTED;
#else
                    strcpy(NemuSVCPath, path.get());
                    RTPathStripFilename(NemuSVCPath);
                    strcat(NemuSVCPath, NemuSVC_exe);
        ...
        /* connect to the NemuSVC server process */

        bool startedOnce = false;
        unsigned timeLeft = NemuSVC_Timeout;
        ...
        do
        {
            ...
            PRUint32 serverID = 0;
            rc = ipcServ->ResolveClientName(NEMUSVC_IPC_NAME, &serverID);
            if (NS_FAILED(rc))
            {
                ...
                do
                {
                    RTThreadSleep(NemuSVC_WaitSlice);
                    rc = ipcServ->ResolveClientName(NEMUSVC_IPC_NAME, &serverID);
                    if (NS_SUCCEEDED(rc))
                        break;
                    if (timeLeft <= NemuSVC_WaitSlice)
                    {
                        timeLeft = 0;
                        break;
                    }
                    timeLeft -= NemuSVC_WaitSlice;
                }
                while (1);
            ...
            {
                nsresult rc2 =
                    ipcServ->ResolveClientName(NEMUSVC_IPC_NAME, &serverID);
                if (NS_SUCCEEDED(rc2))
                    break;
