...
static int pdmR3DevLoadModules(PVM pVM)
{
    ...
    {
        /* make filename */
        char *pszFilename = pdmR3FileR3("NemuDD", true /*fShared*/);
        if (!pszFilename)
            return VERR_NO_TMP_MEMORY;
        rc = pdmR3DevLoad(pVM, &RegCB, pszFilename, "NemuDD");
        RTMemTmpFree(pszFilename);
        if (RT_FAILURE(rc))
            return rc;

        /* make filename */
        pszFilename = pdmR3FileR3("NemuDD2", true /*fShared*/);
        if (!pszFilename)
            return VERR_NO_TMP_MEMORY;
        rc = pdmR3DevLoad(pVM, &RegCB, pszFilename, "NemuDD2");
        RTMemTmpFree(pszFilename);
        if (RT_FAILURE(rc))
            return rc;
    }
...
static int pdmR3DevLoad(PVM pVM, PPDMDEVREGCBINT pRegCB, const char *pszFilename, const char *pszName)
{
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        FNPDMNEMUDEVICESREGISTER *pfnNemuDevicesRegister;
        rc = PDMR3LdrGetSymbolR3(pVM, pszName, "NemuDevicesRegister", (void **)&pfnNemuDevicesRegister);
        if (RT_SUCCESS(rc))
        {
            Log(("PDM: Calling NemuDevicesRegister (%p) of %s (%s)\n", pfnNemuDevicesRegister, pszName, pszFilename));
            rc = pfnNemuDevicesRegister(&pRegCB->Core, NEMU_VERSION);
            if (RT_SUCCESS(rc))
                Log(("PDM: Successfully loaded device module %s (%s).\n", pszName, pszFilename));
            else
                AssertMsgFailed(("NemuDevicesRegister failed with rc=%Rrc for module %s (%s)\n", rc, pszName, pszFilename));
        }
        else
        {
            AssertMsgFailed(("Failed to locate 'NemuDevicesRegister' in %s (%s) rc=%Rrc\n", pszName, pszFilename, rc));
