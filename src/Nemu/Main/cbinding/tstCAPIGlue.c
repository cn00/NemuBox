 ...
 */
static BOOL NEMU_WINAPI ctrlCHandler(DWORD iInfo)
{
    (void)iInfo;
    g_fStop = 1;
    return TRUE;
...
static HRESULT EventListenerDemoProcessEvent(IEvent *event)
{
    ...
    switch (evType)
    {
        ...
        case NemuEventType_OnAdditionsStateChanged:
            printf("OnAdditionsStateChanged\n");
            break;

        case NemuEventType_OnNetworkAdapterChanged:
            printf("OnNetworkAdapterChanged\n");
            break;

        case NemuEventType_OnSerialPortChanged:
            printf("OnSerialPortChanged\n");
            break;

        case NemuEventType_OnParallelPortChanged:
            printf("OnParallelPortChanged\n");
            break;

        case NemuEventType_OnStorageControllerChanged:
            printf("OnStorageControllerChanged\n");
            break;

        case NemuEventType_OnMediumChanged:
            printf("OnMediumChanged\n");
            break;

        case NemuEventType_OnVRDEServerChanged:
            printf("OnVRDEServerChanged\n");
            break;

        case NemuEventType_OnUSBControllerChanged:
            printf("OnUSBControllerChanged\n");
            break;

        case NemuEventType_OnUSBDeviceStateChanged:
            printf("OnUSBDeviceStateChanged\n");
            break;

        case NemuEventType_OnSharedFolderChanged:
...
static HRESULT LoadTypeInfo(REFIID riid, ITypeInfo **pTInfo)
{
    HRESULT rc;
    ITypeLib *pTypeLib;
    rc = LoadRegTypeLib(&LIBID_NEMU, 1 /* major */, 0 /* minor */, 0 /* lcid */, &pTypeLib);
    if (FAILED(rc))
        return rc;
    ...
    if ((SUCCEEDED(rc)) && console)
    {
        ...
        if (SUCCEEDED(rc) && es)
        {
            static const ULONG interestingEvents[] =
                {
                    NemuEventType_OnMousePointerShapeChanged,
                    NemuEventType_OnMouseCapabilityChanged,
                    NemuEventType_OnKeyboardLedsChanged,
                    NemuEventType_OnStateChanged,
                    NemuEventType_OnAdditionsStateChanged,
                    NemuEventType_OnNetworkAdapterChanged,
                    NemuEventType_OnSerialPortChanged,
                    NemuEventType_OnParallelPortChanged,
                    NemuEventType_OnStorageControllerChanged,
                    NemuEventType_OnMediumChanged,
                    NemuEventType_OnVRDEServerChanged,
                    NemuEventType_OnUSBControllerChanged,
                    NemuEventType_OnUSBDeviceStateChanged,
                    NemuEventType_OnSharedFolderChanged,
                    NemuEventType_OnRuntimeError,
                    NemuEventType_OnCanShowWindow,
                    NemuEventType_OnShowWindow
                };
            ...
            interestingEventsSA = g_pNemuFuncs->pfnSafeArrayCreateVector(VT_I4, 0, sizeof(interestingEvents) / sizeof(interestingEvents[0]));
            g_pNemuFuncs->pfnSafeArrayCopyInParamHelper(interestingEventsSA, &interestingEvents, sizeof(interestingEvents));
            ...
            if (consoleListener)
            {
                ...
                if (SUCCEEDED(rc))
                {
                    ...
                    while (!g_fStop)
                    {
                        g_pNemuFuncs->pfnProcessEventQueue(250);
                    }
            ...
            g_pNemuFuncs->pfnSafeArrayDestroy(interestingEventsSA);
...
static void PrintErrorInfo(const char *pszExecutable, const char *pszErrorMsg, HRESULT rc)
{
    ...
    rc2 = g_pNemuFuncs->pfnGetException(&ex);
    if (SUCCEEDED(rc2) && ex)
    {
        ...
        if (ei)
        {
            ...
            do
            {
                ...
                g_pNemuFuncs->pfnUtf16ToUtf8(componentUtf16, &component);
                g_pNemuFuncs->pfnComUnallocString(componentUtf16);
                fprintf(stderr, "  component=%s\n", component);
                g_pNemuFuncs->pfnUtf8Free(component);

                IVirtualBoxErrorInfo_get_Text(ei, &textUtf16);
                g_pNemuFuncs->pfnUtf16ToUtf8(textUtf16, &text);
                g_pNemuFuncs->pfnComUnallocString(textUtf16);
                fprintf(stderr, "  text=%s\n", text);
                g_pNemuFuncs->pfnUtf8Free(text);
        ...
        g_pNemuFuncs->pfnClearException();
    }
}
...
static void startVM(const char *argv0, IVirtualBox *virtualBox, ISession *session, BSTR id)
{
    ...
    BSTR env              = NULL;
    BSTR sessionType;
    SAFEARRAY *groupsSA = g_pNemuFuncs->pfnSafeArrayOutParamAlloc();
    ...
    if (SUCCEEDED(rc))
    {
        ...
        ULONG cbGroups = 0;
        ULONG i, cGroups;
        g_pNemuFuncs->pfnSafeArrayCopyOutParamHelper((void **)&groups, &cbGroups, VT_BSTR, groupsSA);
        g_pNemuFuncs->pfnSafeArrayDestroy(groupsSA);
        ...
        for (i = 0; i < cGroups; ++i)
        {
            ...
            char *group;
            g_pNemuFuncs->pfnUtf16ToUtf8(groups[i], &group);
            printf("Groups[%d]: %s\n", i, group);
            g_pNemuFuncs->pfnUtf8Free(group);
        }
        for (i = 0; i < cGroups; ++i)
            g_pNemuFuncs->pfnComUnallocString(groups[i]);
        g_pNemuFuncs->pfnArrayOutFree(groups);
    }

    g_pNemuFuncs->pfnUtf8ToUtf16("gui", &sessionType);
    rc = IMachine_LaunchVMProcess(machine, session, sessionType, env, &progress);
    g_pNemuFuncs->pfnUtf16Free(sessionType);
    if (SUCCEEDED(rc))
    {
        BOOL completed;
        LONG resultCode;
...
{
    HRESULT rc;
    SAFEARRAY *machinesSA = g_pNemuFuncs->pfnSafeArrayOutParamAlloc();
     ...
     */
    g_pNemuFuncs->pfnSafeArrayCopyOutIfaceParamHelper((IUnknown ***)&machines, &machineCnt, machinesSA);
    g_pNemuFuncs->pfnSafeArrayDestroy(machinesSA);

    if (!machineCnt)
    {
        g_pNemuFuncs->pfnArrayOutFree(machines);
        printf("\tNo VMs\n");
        return;
    }
    ...
    for (i = 0; i < machineCnt; ++i)
    {
        ...
        if (isAccessible)
        {
            ...
            g_pNemuFuncs->pfnUtf16ToUtf8(machineNameUtf16,&machineName);
            g_pNemuFuncs->pfnComUnallocString(machineNameUtf16);
            printf("\tName:        %s\n", machineName);
            g_pNemuFuncs->pfnUtf8Free(machineName);
        }
        else
        {
...

        {
            ...
            g_pNemuFuncs->pfnUtf16ToUtf8(uuidUtf16, &uuidUtf8);
            g_pNemuFuncs->pfnComUnallocString(uuidUtf16);
            printf("\tUUID:        %s\n", uuidUtf8);
            g_pNemuFuncs->pfnUtf8Free(uuidUtf8);
        }
        ...
        {
            {
                ...
                g_pNemuFuncs->pfnUtf16ToUtf8(configFileUtf16, &configFileUtf8);
                g_pNemuFuncs->pfnComUnallocString(configFileUtf16);
                printf("\tConfig file: %s\n", configFileUtf8);
                g_pNemuFuncs->pfnUtf8Free(configFileUtf8);
            }
...

            {
                ...
                g_pNemuFuncs->pfnComUnallocString(typeId);
                IGuestOSType_get_Description(osType, &osNameUtf16);
                g_pNemuFuncs->pfnUtf16ToUtf8(osNameUtf16,&osName);
                g_pNemuFuncs->pfnComUnallocString(osNameUtf16);
                printf("\tGuest OS:    %s\n\n", osName);
                g_pNemuFuncs->pfnUtf8Free(osName);
...
{
    IVirtualBoxClient *nemuclient = NULL;
    IVirtualBox *nemu            = NULL;
    ...
    if (NemuCGlueInit())
    {
        fprintf(stderr, "%s: FATAL: NemuCGlueInit failed: %s\n",
                argv[0], g_szNemuErrMsg);
        return EXIT_FAILURE;
    }

    {
        unsigned ver = g_pNemuFuncs->pfnGetVersion();
        printf("VirtualBox version: %u.%u.%u\n", ver / 1000000, ver / 1000 % 1000, ver % 1000);
        ver = g_pNemuFuncs->pfnGetAPIVersion();
        printf("VirtualBox API version: %u.%u\n", ver / 1000, ver % 1000);
    }

    g_pNemuFuncs->pfnClientInitialize(NULL, &nemuclient);
    if (!nemuclient)
    {
    ...
    rc = IVirtualBoxClient_get_VirtualBox(nemuclient, &nemu);
    if (FAILED(rc) || !nemu)
    {
        PrintErrorInfo(argv[0], "FATAL: could not get VirtualBox reference", rc);
        return EXIT_FAILURE;
    }
    rc = IVirtualBoxClient_get_Session(nemuclient, &session);
...
# endif /* WIN32 */
     ...
     * this nemu. Were not using fancy macros here so it
     * remains easy to see how we access C++'s vtable.
     */

    /* 1. Revision */

    rc = IVirtualBox_get_Revision(nemu, &revision);
    if (SUCCEEDED(rc))
        printf("\tRevision: %u\n", revision);
    else
    ...
    rc = IVirtualBox_get_Version(nemu, &versionUtf16);
    if (SUCCEEDED(rc))
    {
        char *version = NULL;
        g_pNemuFuncs->pfnUtf16ToUtf8(versionUtf16, &version);
        printf("\tVersion: %s\n", version);
        g_pNemuFuncs->pfnUtf8Free(version);
        g_pNemuFuncs->pfnComUnallocString(versionUtf16);
    }
    else
    ...
    rc = IVirtualBox_get_HomeFolder(nemu, &homefolderUtf16);
    if (SUCCEEDED(rc))
    {
        char *homefolder = NULL;
        g_pNemuFuncs->pfnUtf16ToUtf8(homefolderUtf16, &homefolder);
        printf("\tHomeFolder: %s\n", homefolder);
        g_pNemuFuncs->pfnUtf8Free(homefolder);
        g_pNemuFuncs->pfnComUnallocString(homefolderUtf16);
    }
    else
        PrintErrorInfo(argv[0], "GetHomeFolder() failed", rc);

    listVMs(argv[0], nemu, session);
...
# endif /* WIN32 */
    ...
    if (nemu)
    {
        IVirtualBox_Release(nemu);
        nemu = NULL;
    }
    if (nemuclient)
    {
        IVirtualBoxClient_Release(nemuclient);
        nemuclient = NULL;
    }

    g_pNemuFuncs->pfnClientUninitialize();
    NemuCGlueTerm();
