/* $Id: NemuGuestR3LibEvent.cpp $ */
/** @file
 * NemuGuestR3Lib - Ring-3 Support Library for VirtualBox guest additions, Events.
 */
...
VBGLR3DECL(int) VbglR3WaitEvent(uint32_t fMask, uint32_t cMillies, uint32_t *pfEvents)
{
    ...
    waitEvent.u32Result = NEMUGUEST_WAITEVENT_ERROR;
    waitEvent.u32EventFlagsOut = 0;
    int rc = vbglR3DoIOCtl(NEMUGUEST_IOCTL_WAITEVENT, &waitEvent, sizeof(waitEvent));
    if (RT_SUCCESS(rc))
    {
        /*
         ...
         */
#if 0
        AssertMsg(waitEvent.u32Result == NEMUGUEST_WAITEVENT_OK, ("%d rc=%Rrc\n", waitEvent.u32Result, rc));
#endif
        if (pfEvents)
...
/**
 * Cause any pending WaitEvent calls (NEMUGUEST_IOCTL_WAITEVENT) to return
...
{
    return vbglR3DoIOCtl(NEMUGUEST_IOCTL_CANCEL_ALL_WAITEVENTS, 0, 0);
}
