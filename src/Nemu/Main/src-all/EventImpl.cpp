...
struct NemuEvent::Data
{
    Data()
        : mType(NemuEventType_Invalid),
          mWaitEvent(NIL_RTSEMEVENT),
          mWaitable(FALSE),
          mProcessed(FALSE)
    {}

    NemuEventType_T         mType;
...
DEFINE_EMPTY_CTOR_DTOR(NemuEvent)

HRESULT NemuEvent::FinalConstruct()
{
    m = new Data;
    return BaseFinalConstruct();
}

void NemuEvent::FinalRelease()
{
    if (m)
    {
        uninit();
...
HRESULT NemuEvent::init(IEventSource *aSource, NemuEventType_T aType, BOOL aWaitable)
{
    HRESULT rc = S_OK;
...
HRESULT NemuEvent::getType(NemuEventType_T *aType)
{
...
HRESULT NemuEvent::getSource(ComPtr<IEventSource> &aSource)
{
    m->mSource.queryInterfaceTo(aSource.asOutParam());
    return S_OK;
}

HRESULT NemuEvent::getWaitable(BOOL *aWaitable)
{
    // never changes while event alive, no locking
...
HRESULT NemuEvent::setProcessed()
{
...
HRESULT NemuEvent::waitProcessed(LONG aTimeout, BOOL *aResult)
{
...
struct NemuVetoEvent::Data
{
    Data() :
        mVetoed(FALSE)
    {}
    ComObjPtr<NemuEvent>    mEvent;
    BOOL                    mVetoed;
    VetoList                mVetoList;
    ApprovalList            mApprovalList;
};

HRESULT NemuVetoEvent::FinalConstruct()
{
    m = new Data;
...
void NemuVetoEvent::FinalRelease()
{
    if (m)
    {
        uninit();
...
DEFINE_EMPTY_CTOR_DTOR(NemuVetoEvent)

HRESULT NemuVetoEvent::init(IEventSource *aSource, NemuEventType_T aType)
{
    HRESULT rc = S_OK;
...
HRESULT NemuVetoEvent::getType(NemuEventType_T *aType)
{
    return m->mEvent->COMGETTER(Type)(aType);
}

HRESULT NemuVetoEvent::getSource(ComPtr<IEventSource> &aSource)
{
    return m->mEvent->COMGETTER(Source)(aSource.asOutParam());
}

HRESULT NemuVetoEvent::getWaitable(BOOL *aWaitable)
{
    return m->mEvent->COMGETTER(Waitable)(aWaitable);
}

HRESULT NemuVetoEvent::setProcessed()
{
    return m->mEvent->SetProcessed();
}

HRESULT NemuVetoEvent::waitProcessed(LONG aTimeout, BOOL *aResult)
{
    return m->mEvent->WaitProcessed(aTimeout, aResult);
}

HRESULT NemuVetoEvent::addVeto(const com::Utf8Str &aReason)
{
    // AutoWriteLock alock(this COMMA_LOCKVAL_SRC_POS);
    if (aReason.length())
...
HRESULT NemuVetoEvent::isVetoed(BOOL *aResult)
{
    // AutoReadLock alock(this COMMA_LOCKVAL_SRC_POS);
    *aResult = m->mVetoed;
...
HRESULT NemuVetoEvent::getVetos(std::vector<com::Utf8Str> &aResult)
{
    // AutoReadLock alock(this COMMA_LOCKVAL_SRC_POS);
...
HRESULT NemuVetoEvent::addApproval(const com::Utf8Str &aReason)
{
    // AutoWriteLock alock(this COMMA_LOCKVAL_SRC_POS);
...
HRESULT NemuVetoEvent::isApproved(BOOL *aResult)
{
    // AutoReadLock alock(this COMMA_LOCKVAL_SRC_POS);
...
HRESULT NemuVetoEvent::getApprovals(std::vector<com::Utf8Str> &aResult)
{
    // AutoReadLock alock(this COMMA_LOCKVAL_SRC_POS);
...
static const int FirstEvent = (int)NemuEventType_LastWildcard + 1;
static const int LastEvent = (int)NemuEventType_Last;
...
public:
    ListenerRecord(IEventListener *aListener,
                   com::SafeArray<NemuEventType_T> &aInterested,
                   BOOL aActive,
                   EventSource *aOwner);
    ~ListenerRecord();
 ...
 */
static BOOL implies(NemuEventType_T who, NemuEventType_T what)
{
    switch (who)
    {
        case NemuEventType_Any:
            return TRUE;
        case NemuEventType_Vetoable:
            return    (what == NemuEventType_OnExtraDataCanChange)
                   || (what == NemuEventType_OnCanShowWindow);
        case NemuEventType_MachineEvent:
            return    (what == NemuEventType_OnMachineStateChanged)
                   || (what == NemuEventType_OnMachineDataChanged)
                   || (what == NemuEventType_OnMachineRegistered)
                   || (what == NemuEventType_OnSessionStateChanged)
                   || (what == NemuEventType_OnGuestPropertyChanged);
        case NemuEventType_SnapshotEvent:
            return    (what == NemuEventType_OnSnapshotTaken)
                   || (what == NemuEventType_OnSnapshotDeleted)
                   || (what == NemuEventType_OnSnapshotChanged) ;
        case NemuEventType_InputEvent:
            return    (what == NemuEventType_OnKeyboardLedsChanged)
                   || (what == NemuEventType_OnMousePointerShapeChanged)
                   || (what == NemuEventType_OnMouseCapabilityChanged);
        case NemuEventType_Invalid:
            return FALSE;
        default:
            break;
    }
    ...
    mActive(aActive), mOwner(aOwner), mWaitCnt(0), mRefCnt(0)
{
    ...
    {
        NemuEventType_T interested = aInterested[i];
        for (int j = FirstEvent; j < LastEvent; j++)
        {
            NemuEventType_T candidate = (NemuEventType_T)j;
...
{
    {
        ...
        // indefinite) waits will block NemuSVC termination (just one example)
                                      ...
                                      BOOL aActive)
{
    ...
    com::SafeArray<NemuEventType_T> interested(aInteresting);
    ...
    evDesc.init(this, NemuEventType_OnEventSourceChanged, (IEventListener *)aListener, TRUE);
    evDesc.fire(0);
...
HRESULT EventSource::unregisterListener(const ComPtr<IEventListener> &aListener)
{
    ...
    {
        rc = setError(NEMU_E_OBJECT_NOT_FOUND,
    ...
    {
        NemuEventDesc evDesc;
        evDesc.init(this, NemuEventType_OnEventSourceChanged, (IEventListener *)aListener, FALSE);
        evDesc.fire(0);
    }
...
class ATL_NO_VTABLE PassiveEventListener :
    ...
    NEMU_SCRIPTABLE_IMPL(IEventListener)
{
public:
    ...
    BEGIN_COM_MAP(PassiveEventListener)
        NEMU_DEFAULT_INTERFACE_ENTRIES(IEventListener)
    END_COM_MAP()
...
class ATL_NO_VTABLE EventSourceAggregator :
    public VirtualBoxBase,
    NEMU_SCRIPTABLE_IMPL(IEventSource)
{
...
public:
    ...
    BEGIN_COM_MAP(EventSourceAggregator)
        NEMU_DEFAULT_INTERFACE_ENTRIES(IEventSource)
    END_COM_MAP()
    ...
    STDMETHOD(RegisterListener)(IEventListener *aListener,
                                ComSafeArrayIn(NemuEventType_T, aInterested),
                                BOOL aActive);
