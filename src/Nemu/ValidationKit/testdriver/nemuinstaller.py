...
class NemuInstallerTestDriver(TestDriverBase):
    """
    ...
    ksVar_Skipped = 'nemuinstaller-skipped';
    ...
    def showUsage(self):
        ...
        reporter.log('');
        reporter.log('nemuinstaller Options:');
        reporter.log('  --nemu-build    <url[,url2[,..]]>');
        ...
        reporter.log('      The default is to install it if found in the nemu-build.');
        reporter.log('  --');
    ...
    def completeOptions(self):
        ...
        if len(self._asBuildUrls) == 0:
            reporter.error('No build files specfiied ("--nemu-build file1[,file2[...]]")');
            return False;
    ...
    def actionExtract(self):
        reporter.error('nemuinstall does not support extracting resources, you have to do that using the sub testdriver.');
        return False;

    def actionCleanupBefore(self):
        """
        Kills all Nemu process we see.

        This is only supposed to execute on a testbox so we don't need to go
        all complicated wrt other users.
        """
        return self._killAllNemuProcesses();

    def actionConfig(self):
        """
        Install Nemu and pass on the configure request to the sub testdriver.
        """
        fRc = self._installNemu();
        if fRc is None: self._persistentVarSet(self.ksVar_Skipped, 'true');
        else:           self._persistentVarUnset(self.ksVar_Skipped);

        ## @todo nemu.py still has bugs preventing us from invoking it seperately with each action.
    ...
    def actionCleanupAfter(self):
        """
        Forward this to the sub testdriver, then uninstall Nemu.
        """
        fRc = True;
        if 'execute' not in self.asActions and 'all' not in self.asActions:
            fRc = self._executeSubDriver([ 'cleanup-after', ], fMaySkip = False);

        if not self._killAllNemuProcesses():
            fRc = False;

        if not self._uninstallNemu(self._persistentVarExists(self.ksVar_Skipped)):
            fRc = False;
    ...
    def actionAbort(self):
        ...
        fRc3 = self._killAllNemuProcesses();
    ...
    def _killAllNemuProcesses(self):
        """
        ...
        for iIteration in range(22):
            ...
            for oProcess in utils.processListAll():
                ...
                if sBase is None:
                    continue;
                sBase = sBase.lower();
                if sBase in [ 'nemusvc', 'virtualbox', 'virtualboxvm', 'nemuheadless', 'nemumanage', 'nemusdl', 'nemuwebsrv',
                              'nemuautostart', 'nemuballoonctrl', 'nemubfe', 'nemuextpackhelperapp', 'nemunetdhcp',
                              'nemunetadpctl', 'nemutestogl', 'nemutunctl', 'nemuvmmpreload', 'nemuxpcomipcd', 'vmCreator', ]:
    ...
    def _installNemu(self):
        ...
        # list.  This allows us to use NemuAll*.tar.gz files.
        #
        ...
        if   sHost == 'darwin':     fRc = self._installNemuOnDarwin();
        elif sHost == 'linux':      fRc = self._installNemuOnLinux();
        elif sHost == 'solaris':    fRc = self._installNemuOnSolaris();
        elif sHost == 'win':        fRc = self._installNemuOnWindows();
        else:
        ...
        try:
            ...
            reporter.log('Disk usage after Nemu install: %d MB available at %s' % (cMbFreeSpace, self.sScratchPath,));
        except:
    ...
    def _uninstallNemu(self, fIgnoreError = False):
        """
        ...
        sHost = utils.getHostOs()
        if   sHost == 'darwin':     fRc = self._uninstallNemuOnDarwin();
        elif sHost == 'linux':      fRc = self._uninstallNemuOnLinux();
        elif sHost == 'solaris':    fRc = self._uninstallNemuOnSolaris();
        elif sHost == 'win':        fRc = self._uninstallNemuOnWindows();
        else:
            reporter.error('Unsupported host "%s".' % (sHost,));
    ...
    def _installNemuOnDarwin(self):
        """ Installs Nemu on Mac OS X."""
        ...
        # Uninstall any previous nemu version first.
    ...
    def _uninstallNemuOnDarwin(self):
        """ Uninstalls Nemu on Mac OS X."""

        # Is VirtualBox installed? If not, don't try uninstall it.
        sNemu = self._getNemuInstallPath(fFailIfNotFound = False);
        if sNemu is None:
            return True;
    ...
    def _installNemuOnLinux(self):
        """ Installs Nemu on Linux."""
    ...
    def _uninstallNemuOnLinux(self):
        """ Uninstalls Nemu on Linux."""
        ...
        for sUninstaller in [os.path.join(sNemu, 'uninstall.sh'), '/opt/VirtualBox/uninstall.sh', ]:
    ...
    def _installNemuOnSolaris(self):
        """ Installs Nemu on Solaris."""
        ...
        self._uninstallNemuOnSolaris();

        # Install the new one.
        fRc, _ = self._sudoExecuteSync(['pkgadd', '-d', sPkg, '-n', '-a', sRsp, 'SUNWnemu']);
        return fRc;

    def _uninstallNemuOnSolaris(self):
        """ Uninstalls Nemu on Solaris."""
        reporter.flushall();
        if utils.processCall(['pkginfo', '-q', 'SUNWnemu']) != 0:
            return True;
        sRsp = self._generateAutoResponseOnSolaris();
        fRc, _ = self._sudoExecuteSync(['pkgrm', '-n', '-a', sRsp, 'SUNWnemu']);
        return fRc;
    ...
    def _installNemuOnWindows(self):
        """ Installs Nemu on Windows."""
        ...
        #    sMagicScript = os.path.abspath(os.path.join(g_ksValidationKitDir, 'testdriver', 'win-nemu-net-uninstall.ps1'));
        ...
        # Uninstall any previous nemu version first.
        fRc = self._uninstallNemuOnWindows();
        if fRc is not True:
        ...
        sNemuInstallPath = os.environ.get('NEMU_INSTALL_PATH', None);
        if sNemuInstallPath is not None:
            asArgs.extend(['INSTALLDIR="%s"' % (sNemuInstallPath,)]);
        ...
        if fRc2 is False:
            if iRc == 3010: # ERROR_SUCCESS_REBOOT_REQUIRED
                ...
                fRc = False;
        sLogFile = os.path.join(tempfile.gettempdir(), 'VirtualBox', 'NemuInstallLog.txt');
    ...
    def _uninstallNemuOnWindows(self):
        """
        Uninstalls Nemu on Windows, all installations we find to be on the safe side...
        """
        ...
        # Do the uninstalling.
        fRc = True;
        sLogFile = os.path.join(self.sScratchPath, 'NemuUninstallLog.txt');
    ...
    def _getNemuInstallPath(self, fFailIfNotFound):
        """ Returns the default Nemu installation path. """
        sHost = utils.getHostOs();
        if sHost == 'win':
        ...
        if 'NEMU_INSTALL_PATH' in os.environ:
            asLocs.insert(0, os.environ.get('NEMU_INSTALL_PATH', None));
    ...
    def _installExtPack(self):
        ...
        sNemu = self._getNemuInstallPath(fFailIfNotFound = True);
        if sNemu is None:
            return False;
        sExtPackDir = os.path.join(sNemu, 'ExtensionPacks');

        if self._uninstallAllExtPacks() is not True:
            return False;

        sExtPack = self._findFile('Oracle_VM_VirtualBox_Extension_Pack.nemu-extpack');
        if sExtPack is None:
            sExtPack = self._findFile('Oracle_VM_VirtualBox_Extension_Pack.*.nemu-extpack');
        if sExtPack is None:
            return True;
...
if __name__ == '__main__':
    sys.exit(NemuInstallerTestDriver().main(sys.argv));
