...
/** @file
 * Nemu crOpenGL: Host service entry points.
 */
...
PNEMUHGCMSVCHELPERS g_pHelpers;
...
typedef struct _CRNEMUSVCBUFFER_t {
    uint32_t uiId;
    uint32_t uiSize;
    void*    pData;
    _CRNEMUSVCBUFFER_t *pNext, *pPrev;
} CRNEMUSVCBUFFER_t;

static CRNEMUSVCBUFFER_t *g_pCRNemuSVCBuffers = NULL;
static uint32_t g_CRNemuSVCBufferID = 0;

/* svcPresentFBO related data */
typedef struct _CRNEMUSVCPRESENTFBOCMD_t {
    void *pData;
    int32_t screenId, x, y, w, h;
    _CRNEMUSVCPRESENTFBOCMD_t *pNext;
} CRNEMUSVCPRESENTFBOCMD_t, *PCRNEMUSVCPRESENTFBOCMD_t;
...
static DECLCALLBACK(int) svcConnect (void *, uint32_t u32ClientID, void *pvClient)
{
    ...
    rc = crNemuServerAddClient(u32ClientID);
...
static DECLCALLBACK(int) svcDisconnect (void *, uint32_t u32ClientID, void *pvClient)
{
    ...
    crNemuServerRemoveClient(u32ClientID);
...
static DECLCALLBACK(int) svcSaveState(void *, uint32_t u32ClientID, void *pvClient, PSSMHANDLE pSSM)
{
    ...
    /* The state itself */
    rc = crNemuServerSaveState(pSSM);
    AssertRCReturn(rc, rc);

    /* Save svc buffers info */
    {
        CRNEMUSVCBUFFER_t *pBuffer = g_pCRNemuSVCBuffers;

        rc = SSMR3PutU32(pSSM, g_CRNemuSVCBufferID);
        AssertRCReturn(rc, rc);
...
static DECLCALLBACK(int) svcLoadState(void *, uint32_t u32ClientID, void *pvClient, PSSMHANDLE pSSM)
{
    ...
    AssertRCReturn(rc, rc);
    if (strcmp(gszNemuOGLSSMMagic, psz))
    ...
    /* The state itself */
    rc = crNemuServerLoadState(pSSM, ui32);
    ...
    if (rc==VERR_SSM_DATA_UNIT_FORMAT_CHANGED && ui32!=SHCROGL_SSM_VERSION)
    {
        ...
        /*NemuSharedCrOpenGL isn't last hgcm service now, so can't use SSMR3SkipToEndOfUnit*/
        {
            const char *pMatch = &gszNemuOGLSSMMagic[0];
            char current;
    ...
    if (ui32>=24)
    {
        ...
        rc = SSMR3GetU32(pSSM, &g_CRNemuSVCBufferID);
        AssertRCReturn(rc, rc);
        ...
        while (uiId)
        {
            CRNEMUSVCBUFFER_t *pBuffer = (CRNEMUSVCBUFFER_t *) RTMemAlloc(sizeof(CRNEMUSVCBUFFER_t));
            if (!pBuffer)
            {
            ...
            pBuffer->pNext = g_pCRNemuSVCBuffers;
            pBuffer->pPrev = NULL;
            if (g_pCRNemuSVCBuffers)
            {
                g_pCRNemuSVCBuffers->pPrev = pBuffer;
            }
            g_pCRNemuSVCBuffers = pBuffer;
...
static CRNEMUSVCBUFFER_t* svcGetBuffer(uint32_t iBuffer, uint32_t cbBufferSize)
{
    CRNEMUSVCBUFFER_t* pBuffer;

    if (iBuffer)
    {
        pBuffer = g_pCRNemuSVCBuffers;
        while (pBuffer)
        {
    ...
    {
        pBuffer = (CRNEMUSVCBUFFER_t*) RTMemAlloc(sizeof(CRNEMUSVCBUFFER_t));
        if (pBuffer)
        {
            ...
            pBuffer->uiId = ++g_CRNemuSVCBufferID;
            if (!pBuffer->uiId)
            {
                pBuffer->uiId = ++g_CRNemuSVCBufferID;
            }
        ...
        {
            LogRel(("OpenGL: svcGetBuffer: Not enough memory (%d)\n", sizeof(CRNEMUSVCBUFFER_t)));
        }
        return pBuffer;
    }
}

static void svcFreeBuffer(CRNEMUSVCBUFFER_t* pBuffer)
{
    Assert(pBuffer);
    ...
    {
        Assert(pBuffer==g_pCRNemuSVCBuffers);
        g_pCRNemuSVCBuffers = pBuffer->pNext;
    }
...
static DECLCALLBACK(void) svcCall (void *, NEMUHGCMCALLHANDLE callHandle, uint32_t u32ClientID, void *pvClient, uint32_t u32Function, uint32_t cParms, NEMUHGCMSVCPARM paParms[])
{
    int rc = VINF_SUCCESS;
    ...
    switch (u32Function)
    {
        case SHCRGL_GUEST_FN_WRITE:
        {
            ...
            if (    paParms[0].type != NEMU_HGCM_SVC_PARM_PTR     /* pBuffer */
               )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                rc = crNemuServerClientWrite(u32ClientID, pBuffer, cbBuffer);
                if (!RT_SUCCESS(rc))
                {
        ...
        case SHCRGL_GUEST_FN_INJECT:
        {
            ...
            if (    paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT /* u32ClientID */
                 || paParms[1].type != NEMU_HGCM_SVC_PARM_PTR   /* pBuffer */
               )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                /* Execute the function. */
                rc = crNemuServerClientWrite(u32InjectClientID, pBuffer, cbBuffer);
                if (!RT_SUCCESS(rc))
                {
        ...
        case SHCRGL_GUEST_FN_READ:
        {
            ...
            if (    paParms[0].type != NEMU_HGCM_SVC_PARM_PTR     /* pBuffer */
                 || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT   /* cbBuffer */
               )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            ...
            /* Execute the function. */
            rc = crNemuServerClientRead(u32ClientID, pBuffer, &cbBuffer);
        ...
        case SHCRGL_GUEST_FN_WRITE_READ:
        {
            ...
            if (    paParms[0].type != NEMU_HGCM_SVC_PARM_PTR     /* pBuffer */
                 || paParms[1].type != NEMU_HGCM_SVC_PARM_PTR     /* pWriteback */
                 || paParms[2].type != NEMU_HGCM_SVC_PARM_32BIT   /* cbWriteback */
               )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                rc = crNemuServerClientRead(u32ClientID, pWriteback, &cbWriteback);
        ...
        case SHCRGL_GUEST_FN_SET_VERSION:
        {
            ...
            if (    paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT     /* vMajor */
                 || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT     /* vMinor */
               )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                /* Execute the function. */
                rc = crNemuServerClientSetVersion(u32ClientID, vMajor, vMinor);
        ...
        case SHCRGL_GUEST_FN_SET_PID:
        {
            ...
            if (paParms[0].type != NEMU_HGCM_SVC_PARM_64BIT)
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                /* Execute the function. */
                rc = crNemuServerClientSetPID(u32ClientID, pid);
            }
        ...
        case SHCRGL_GUEST_FN_WRITE_BUFFER:
        {
            ...
            if (   paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT /*iBufferID*/
                || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT /*cbBufferSize*/
                || paParms[2].type != NEMU_HGCM_SVC_PARM_32BIT /*ui32Offset*/
                || paParms[3].type != NEMU_HGCM_SVC_PARM_PTR   /*pBuffer*/
               )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                /* Execute the function. */
                CRNEMUSVCBUFFER_t *pSvcBuffer = svcGetBuffer(iBuffer, cbBufferSize);
        ...
        case SHCRGL_GUEST_FN_WRITE_READ_BUFFERED:
        {
            ...
            else
            {
                ...
                CRNEMUSVCBUFFER_t *pSvcBuffer = svcGetBuffer(iBuffer, 0);
                if (!pSvcBuffer)
                {
        ...
        case SHCRGL_GUEST_FN_GET_CAPS_NEW:
        {
            ...
            if (paParms[0].type != NEMU_HGCM_SVC_PARM_PTR)
            {
            ...
            rc = crNemuServerClientGetCapsNew(u32ClientID, pInfo);
            AssertRC(rc);
        ...
        case SHCRGL_GUEST_FN_GET_CAPS_LEGACY:
        {
            ...
            else if (paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT)
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                /* Execute the function. */
                rc = crNemuServerClientGetCapsLegacy(u32ClientID, &paParms[0].u.uint32);
                AssertRC(rc);
            }
...
static void crScreenshotHandle(CRNEMUHGCMTAKESCREENSHOT *pScreenshot, uint32_t idScreen, uint64_t u64Now)
{
    if (!pScreenshot->pfnScreenshotBegin || pScreenshot->pfnScreenshotBegin(pScreenshot->pvContext, idScreen, u64Now))
    {
        ...
        int rc = crServerNemuScreenshotGet(idScreen, pScreenshot->u32Width, pScreenshot->u32Height, pScreenshot->u32Pitch, pScreenshot->pvBuffer, &Screenshot);
        if (RT_SUCCESS(rc))
        {
            ...
            crServerNemuScreenshotRelease(&Screenshot);
        }
        else
        {
 ...
 */
static int svcHostCallPerform(uint32_t u32Function, uint32_t cParms, NEMUHGCMSVCPARM paParms[])
{
    int rc = VINF_SUCCESS;
    ...
    switch (u32Function)
    {
        ...
        {
            Assert(cParms == 1 && paParms[0].type == NEMU_HGCM_SVC_PARM_PTR);
            if (cParms == 1 && paParms[0].type == NEMU_HGCM_SVC_PARM_PTR)
            {
                rc = crNemuServerCrHgsmiCmd((PNEMUVDMACMD_CHROMIUM_CMD)paParms[0].u.pointer.addr, paParms[0].u.pointer.size);
                if (VERR_NOT_SUPPORTED == rc)
                {
                    svcClientVersionUnsupported(0, 0);
                }
        ...
        case SHCRGL_HOST_FN_CRHGSMI_CTL:
        {
            Assert(cParms == 1 && paParms[0].type == NEMU_HGCM_SVC_PARM_PTR);
            if (cParms == 1 && paParms[0].type == NEMU_HGCM_SVC_PARM_PTR)
                rc = crNemuServerCrHgsmiCtl((PNEMUVDMACMD_CHROMIUM_CTL)paParms[0].u.pointer.addr, paParms[0].u.pointer.size);
            else
                rc = VERR_INVALID_PARAMETER;
        } break;
#endif
        case SHCRGL_HOST_FN_SET_CONSOLE:
        {
            ...
            else if (paParms[0].type != NEMU_HGCM_SVC_PARM_PTR)
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                else /* Execute the function. */
                {
                    ...
                    rc = crNemuServerSetScreenCount(monitorCount);
                    AssertRCReturn(rc, rc);

#if 1
                    crServerNemuCompositionSetEnableStateGlobal(GL_FALSE);
                    ...
                    for (i=0; i<monitorCount; ++i)
                    {
                        ...
                        else
                        {
                            ...
                            rc = crNemuServerMapScreen(i, xo, yo, w, h, winId);
                            AssertRCReturn(rc, rc);
                        }
                    }

                    crServerNemuCompositionSetEnableStateGlobal(GL_TRUE);
#endif
        ...
        case SHCRGL_HOST_FN_SET_VISIBLE_REGION:
        {
            ...
            if (    paParms[0].type != NEMU_HGCM_SVC_PARM_PTR     /* pRects */
               )
            {
                rc = VERR_INVALID_PARAMETER;
                break;
            ...
            rc = crNemuServerSetRootVisibleRegion(paParms[0].u.pointer.size / sizeof (RTRECT), (const RTRECT*)paParms[0].u.pointer.addr);
            break;
        }
        case SHCRGL_HOST_FN_SCREEN_CHANGED:
        {
            ...
            else
            {
                ...
                if (!pFramebuffer)
                {
                    rc = crNemuServerUnmapScreen(screenId);
                    AssertRCReturn(rc, rc);
                }
                else
                {
                    do {
                        ...
                        rc = pFramebuffer->Notify3DEvent(NEMU3D_NOTIFY_EVENT_TYPE_TEST_FUNCTIONAL, ComSafeArrayAsInParam(data));
                        ...
                        else
                        {
                            ...
                            rc = crNemuServerMapScreen(screenId, xo, yo, w, h, winId);
                            AssertRCReturn(rc, rc);
                        }
                    } while (0);
                }
        ...
        case SHCRGL_HOST_FN_TAKE_SCREENSHOT:
        {
            ...
            if (paParms->type != NEMU_HGCM_SVC_PARM_PTR)
            {
            ...
            if (paParms->u.pointer.size != sizeof (CRNEMUHGCMTAKESCREENSHOT))
            {
                AssertMsgFailed(("invalid param\n"));
                rc = VERR_INVALID_PARAMETER;
                break;
            ...
            CRNEMUHGCMTAKESCREENSHOT *pScreenshot = (CRNEMUHGCMTAKESCREENSHOT*)paParms->u.pointer.addr;
        ...
        case SHCRGL_HOST_FN_DEV_RESIZE:
        {
            ...
            if (paParms->u.pointer.size != sizeof (CRNEMUHGCMDEVRESIZE))
            {
                AssertMsgFailed(("invalid param\n"));
                return VERR_INVALID_PARAMETER;
            }

            CRNEMUHGCMDEVRESIZE *pResize = (CRNEMUHGCMDEVRESIZE*)paParms->u.pointer.addr;

            rc = crNemuServerNotifyResize(&pResize->Screen, pResize->pvVRAM);
            break;
        }
        case SHCRGL_HOST_FN_VIEWPORT_CHANGED:
        {
            ...
            {
                if (paParms[i].type != NEMU_HGCM_SVC_PARM_32BIT)
                {
            ...
            rc = crNemuServerSetScreenViewport((int)paParms[0].u.uint32,
            ...
            if (!RT_SUCCESS(rc))
            {
                LogRel(("OpenGL: SHCRGL_HOST_FN_VIEWPORT_CHANGED: crNemuServerSetScreenViewport failed, rc %d", rc));
            }
        ...
        case SHCRGL_HOST_FN_VIEWPORT_CHANGED2:
        {
            ...
            if (paParms[0].type != NEMU_HGCM_SVC_PARM_PTR
                    ...
                    || paParms[0].u.pointer.size != sizeof (CRNEMUHGCMVIEWPORT))
            {
            ...
            CRNEMUHGCMVIEWPORT *pViewportInfo = (CRNEMUHGCMVIEWPORT*)paParms[0].u.pointer.addr;

            rc = crNemuServerSetScreenViewport(pViewportInfo->u32Screen,
                    pViewportInfo->x, /* x */
                    pViewportInfo->y, /* y */
            ...
            else
            {
                ...
                else /* Execute the function. */
                {
                    if (pOutputRedirect->H3DORBegin != NULL)
                    {
                        ...
                        rc = crNemuServerOutputRedirectSet(&outputRedirect);
                        if (RT_SUCCESS(rc))
                        {
                            rc = crNemuServerSetOffscreenRendering(GL_TRUE);
                        }
                    }
                    else
                    {
                        /* Redirection is disabled. */
                        crNemuServerSetOffscreenRendering(GL_FALSE);
                        crNemuServerOutputRedirectSet(NULL);
                    }
                }
            }
            break;
        ...
        case SHCRGL_HOST_FN_WINDOWS_SHOW:
        {
            ...
            if (paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT)
            {
                WARN(("invalid parameter"));
                rc = VERR_INVALID_PARAMETER;
                break;
            ...
            rc = crServerNemuWindowsShow(!!paParms[0].u.uint32);
            if (!RT_SUCCESS(rc))
                WARN(("crServerNemuWindowsShow failed rc %d", rc));
        ...
        case SHCRGL_HOST_FN_SET_SCALE_FACTOR:
        {
            /* Verify parameter count and types. */
            if (cParms != 1
             || paParms[0].type != NEMU_HGCM_SVC_PARM_PTR
             || paParms[0].u.pointer.size != sizeof(CRNEMUHGCMSETSCALEFACTOR)
             || !paParms[0].u.pointer.addr)
            {
                WARN(("invalid parameter"));
                rc = VERR_INVALID_PARAMETER;
            ...
            CRNEMUHGCMSETSCALEFACTOR *pData = (CRNEMUHGCMSETSCALEFACTOR *)paParms[0].u.pointer.addr;
            double dScaleFactorW = (double)(pData->u32ScaleFactorWMultiplied) / NEMU_OGL_SCALE_FACTOR_MULTIPLIER;
            double dScaleFactorH = (double)(pData->u32ScaleFactorHMultiplied) / NEMU_OGL_SCALE_FACTOR_MULTIPLIER;

            rc = NemuOglSetScaleFactor(pData->u32Screen, dScaleFactorW, dScaleFactorH);
            ...
            LogRel(("OpenGL: Set 3D content scale factor to (%u, %u), multiplier %d (rc=%Rrc)\n",
                ...
                (int)NEMU_OGL_SCALE_FACTOR_MULTIPLIER,
                rc));
        ...
        case SHCRGL_HOST_FN_SET_UNSCALED_HIDPI:
        {
            ...
            if (cParms != 1
             || paParms[0].type != NEMU_HGCM_SVC_PARM_PTR
             || paParms[0].u.pointer.size != sizeof(CRNEMUHGCMSETUNSCALEDHIDPIOUTPUT)
             || !paParms[0].u.pointer.addr)
            {
                WARN(("invalid parameter"));
                rc = VERR_INVALID_PARAMETER;
            ...
            CRNEMUHGCMSETUNSCALEDHIDPIOUTPUT *pData = (CRNEMUHGCMSETUNSCALEDHIDPIOUTPUT *)paParms[0].u.pointer.addr;
...
int crNemuServerHostCtl(NEMUCRCMDCTL *pCtl, uint32_t cbCtl)
{
    if ((cbCtl - sizeof (NEMUCRCMDCTL)) % sizeof(NEMUHGCMSVCPARM))
    {
        WARN(("invalid param size"));
        return VERR_INVALID_PARAMETER;
    }
    uint32_t cParams = (cbCtl - sizeof (NEMUCRCMDCTL)) / sizeof (NEMUHGCMSVCPARM);
    bool fHasCallout = NEMUCRCMDCTL_IS_CALLOUT_AVAILABLE(pCtl);
    if (fHasCallout)
        crNemuServerCalloutEnable(pCtl);

    int rc = svcHostCallPerform(pCtl->u32Function, cParams, (NEMUHGCMSVCPARM*)(pCtl + 1));

    if (fHasCallout)
        crNemuServerCalloutDisable();

    return rc;
}

static DECLCALLBACK(int) svcHostCall(void *, uint32_t u32Function, uint32_t cParms, NEMUHGCMSVCPARM paParms[])
{
    switch (u32Function)
    {
        case SHCRGL_HOST_FN_CTL:
        {
            ...
            if (paParms->u.pointer.size < sizeof (NEMUCRCMDCTL))
            {
                WARN(("invalid param size"));
                return VERR_INVALID_PARAMETER;
            }

            NEMUCRCMDCTL *pCtl = (NEMUCRCMDCTL*)paParms->u.pointer.addr;
            switch (pCtl->enmType)
            {
                case NEMUCRCMDCTL_TYPE_HGCM:
                {
                    return crNemuServerHostCtl(pCtl, paParms->u.pointer.size);
                }
                case NEMUCRCMDCTL_TYPE_DISABLE:
                {
                    if (paParms->u.pointer.size != sizeof (NEMUCRCMDCTL_DISABLE))
                        WARN(("invalid param size"));
                    NEMUCRCMDCTL_DISABLE *pDisable = (NEMUCRCMDCTL_DISABLE*)pCtl;
                    int rc = crNemuServerHgcmDisable(&pDisable->Data);
                    if (RT_SUCCESS(rc))
                        g_u32fCrHgcmDisabled = 1;
                    else
                        WARN(("crNemuServerHgcmDisable failed %d", rc));
                    return rc;
                }
                case NEMUCRCMDCTL_TYPE_ENABLE:
                {
                    if (paParms->u.pointer.size != sizeof (NEMUCRCMDCTL_ENABLE))
                        WARN(("invalid param size"));
                    NEMUCRCMDCTL_ENABLE *pEnable = (NEMUCRCMDCTL_ENABLE*)pCtl;
                    int rc = crNemuServerHgcmEnable(&pEnable->Data);
                    if (RT_SUCCESS(rc))
                        g_u32fCrHgcmDisabled = 0;
                    else
                        WARN(("crNemuServerHgcmEnable failed %d", rc));
                    return rc;
                }
                default:
...
extern "C" DECLCALLBACK(DECLEXPORT(int)) NemuHGCMSvcLoad (NEMUHGCMSVCFNTABLE *ptable)
{
    ...
    Log(("SHARED_CROPENGL NemuHGCMSvcLoad: ptable = %p\n", ptable));
    ...
    {
        Log(("NemuHGCMSvcLoad: ptable->cbSize = %d, ptable->u32Version = 0x%08X\n", ptable->cbSize, ptable->u32Version));
        ...
        else
        {
            ...
            crServerNemuSetNotifyEventCB(svcNotifyEventCB);
        }
    }
