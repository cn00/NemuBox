/* $Id: NemuServiceAutoMount.cpp $ */
/** @file
 * NemuService - Auto-mounting for Shared Folders, only Linux & Solaris atm.
 */
...
/** @page pg_vgsvc_automount NemuService - Shared Folder Automounter
 *
 ...
 * thinking is this is better of done from NemuTray, some one argue that for
...
#ifdef RT_OS_SOLARIS
# define NEMUSERVICE_AUTOMOUNT_DEFAULT_DIR       "/mnt"
#else
# define NEMUSERVICE_AUTOMOUNT_DEFAULT_DIR       "/media"
#endif
...
static int vbsvcAutoMountPrepareMountPoint(const char *pszMountPoint, const char *pszShareName, vbsf_mount_opts *pOpts)
{
    ...
    RTFMODE fMode = RTFS_UNIX_IRWXU | RTFS_UNIX_IRWXG; /* Owner (=root) and the group (=nemusf) have full access. */
 ...
 *
 * @returns Nemu status code reflecting unmount and mount point preparation
    ...
    if (!fSkip && RT_SUCCESS(rc))
    {
        ...
        else /* r == -1, we got some error in errno.  */
        {
            if (errno == EPROTO)
            {
                ...
                r = mount(mntinf.name, pszMountPoint, "nemusf", fFlags, &mntinf);
            }
            if (errno == EPROTO)
            {
                VGSvcVerbose(3, "vbsvcAutoMountWorker: Re-trying with old mounting structure ...\n");

                /* New mount tool with old nemusf module? Try again using the old
                ...
                r = mount(mntinf_old.name, pszMountPoint, "nemusf", fFlags, &mntinf_old);
            }
                                         ...
                                         const char *pszMountDir, const char *pszSharePrefix, uint32_t uClientID)
{
     ...
     *
     * It also happily continues if the 'nemusf' group is missing, which is a waste
            ...
            && *pszShareName)
        {
            ...
            if (RTStrAPrintf(&pszShareNameFull, "%s%s", pszSharePrefix, pszShareName) > 0)
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    struct group *grp_nemusf = getgrnam("nemusf");
                    if (grp_nemusf)
                    {
                        struct vbsf_mount_opts mount_opts =
                        {
                            0,                     /* uid */
                            (int)grp_nemusf->gr_gid, /* gid */
                            0,                     /* ttl */
                            0770,                  /* dmode, owner and group "nemusf" have full access */
                            0770,                  /* fmode, owner and group "nemusf" have full access */
                            0,                     /* dmask */
                            0,                     /* fmask */
                            0,                     /* ronly */
                            0,                     /* sloppy */
                        ...
                        VGSvcError("vbsvcAutoMountWorker: Group 'nemusf' does not exist\n");
                }
                else
...
static DECLCALLBACK(int) vbsvcAutoMountWorker(bool volatile *pfShutdown)
{
        ...
        && cMappings)
    {
        ...
        if (rc == VERR_NOT_FOUND)
            rc = RTStrDupEx(&pszMountDir, NEMUSERVICE_AUTOMOUNT_DEFAULT_DIR);
        if (RT_SUCCESS(rc))
        {
