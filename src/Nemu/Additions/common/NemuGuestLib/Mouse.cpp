/* $Id: Mouse.cpp $ */
/** @file
 * NemuGuestLibR0 - Mouse Integration.
 */
...
{
    NemuGuestMouseSetNotifyCallback NotifyCallback;
    VBGLDRIVER *pDriver;
    ...
    return vbglDriverIOCtl(pDriver, NEMUGUEST_IOCTL_SET_MOUSE_NOTIFY_CALLBACK, &NotifyCallback, sizeof(NotifyCallback));
}
...
DECLVBGL(int) VbglGetMouseStatus(uint32_t *pfFeatures, uint32_t *px, uint32_t *py)
{
    ...
    Req.pointerXPos = 0;
    Req.pointerYPos = 0;
    rc = vbglDriverIOCtl(pDriver, NEMUGUEST_IOCTL_VMMREQUEST(sizeof(Req)), &Req.header, sizeof(Req));
    if (RT_FAILURE(rc))
        return rc;
...
DECLVBGL(int) VbglSetMouseStatus(uint32_t fFeatures)
{
    ...
    if (RT_FAILURE(rc))
        return rc;
    return vbglDriverIOCtl(pDriver, NEMUGUEST_IOCTL_SET_MOUSE_STATUS, &fFeatures, sizeof(fFeatures));
}
