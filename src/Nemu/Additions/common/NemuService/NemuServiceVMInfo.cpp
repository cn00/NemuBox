/* $Id: NemuServiceVMInfo.cpp $ */
/** @file
 * NemuService - Virtual Machine Information for the Host.
 */
...
/** @page pg_vgsvc_vminfo NemuService - VM Information
 *
 ...
 * service in cooperation with the Main API (NemuSVC).  Properties have a name
 ...
 * via the Main API or NemuManage on the host side and NemuControl in the guest.
 *
...
typedef struct NEMUSERVICELACLIENTINFO
{
    uint32_t    uID;
    char       *pszName;
    char       *pszLocation;
...
} NEMUSERVICELACLIENTINFO, *PNEMUSERVICELACLIENTINFO;
...
static NEMUSERVICEVEPROPCACHE   g_VMInfoPropCache;
...
static NEMUSERVICELACLIENTINFO  g_LAClientInfo;
...
/**
 * @interface_method_impl{NEMUSERVICE,pfnPreInit}
 */
...
/**
 * @interface_method_impl{NEMUSERVICE,pfnOption}
 */
...
/**
 * @interface_method_impl{NEMUSERVICE,pfnInit}
 */
static DECLCALLBACK(int) vbsvcVMInfoInit(void)
{
    ...
    /* The status code is ignored as this information is not available with Nemu < 3.2.10. */
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        char *pszValue;
        rc2 = VGSvcReadHostProp(g_uVMInfoGuestPropSvcClientID, "/VirtualBox/GuestAdd/NemuService/--vminfo-user-idle-threshold",
 ...
 * objects which need to be free'd with nemuServiceFreeLAClientInfo.
 *
 * @return  IPRT status code.
 * @param   uClientID               Client ID to retrieve information for.
 * @param   pClient                 Pointer where to store the client information.
 */
static int vgsvcGetLAClientInfo(uint32_t uClientID, PNEMUSERVICELACLIENTINFO pClient)
{
    AssertReturn(uClientID, VERR_INVALID_PARAMETER);
 ...
 */
static void vgsvcFreeLAClientInfo(PNEMUSERVICELACLIENTINFO pClient)
{
    if (pClient)
    {
        if (pClient->pszName)
 ...
 *                                  the property.
 */
int VGSvcUserUpdateF(PNEMUSERVICEVEPROPCACHE pCache, const char *pszUser, const char *pszDomain,
...
#if defined(NEMU_WITH_DBUS) && defined(RT_OS_LINUX) /* Not yet for Solaris/FreeBSB. */
/*
 * Simple wrapper to work around compiler-specific va_list madness.
 */
static dbus_bool_t nemuService_dbus_message_get_args(DBusMessage *message, DBusError *error, int first_arg_type, ...)
{
    va_list va;
            ...
            && dbus_message_get_type(pMsgSessions) == DBUS_MESSAGE_TYPE_METHOD_CALL)
        {
                ...
                && !dbus_error_is_set(&dbErr))
            {
                ...
                if (   dbus_message_get_type(pMsgSessions) == DBUS_MESSAGE_TYPE_METHOD_CALL
                    && nemuService_dbus_message_get_args(pReplySessions, &dbErr, DBUS_TYPE_ARRAY,
...
/**
 * @interface_method_impl{NEMUSERVICE,pfnWorker}
 */
static DECLCALLBACK(int) vbsvcVMInfoWorker(bool volatile *pfShutdown)
{
    ...
    for (;;)
    {
         ...
         * works with Nemu (latest) 4.1 and up. */
                        ...
                        || (g_LAClientAttachedTS != uLAClientAttachedTS)))
                {
                    ...
                    if (RT_SUCCESS(rc2))
                    {
                        VGSvcVerbose(1, "VRDP: Hotdesk client %s with ID=%RU32, Name=%s, Domain=%s\n",
                                      ...
                                      * hotdesk session when NemuService started. */
...
/**
 * @interface_method_impl{NEMUSERVICE,pfnStop}
 */
...
/**
 * @interface_method_impl{NEMUSERVICE,pfnTerm}
 */
