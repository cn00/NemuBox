...
extern void crNemuHGCMInit( CRNetReceiveFuncList *rfl, CRNetCloseFuncList *cfl, unsigned int mtu );
extern void crNemuHGCMConnection( CRConnection *conn
#if defined(NEMU_WITH_CRHGSMI) && defined(IN_GUEST)
        , struct NEMUUHGSMI *pHgsmi
#endif
        );
extern int crNemuHGCMRecv(
#if defined(NEMU_WITH_CRHGSMI) && defined(IN_GUEST)
        CRConnection *conn
#endif
        );
#ifdef IN_GUEST
extern uint32_t crNemuHGCMHostCapsGet();
#endif
extern CRConnection** crNemuHGCMDump( int *num );
extern void crNemuHGCMTearDown(void);
#endif
