...
CHAR16 CONST *maBdsLibBootFiles[] = {
  NEMU_EFI_APPLE_MEDIA_FILE_NAME,
  ...
  for (Index = 0; Index < UsbIoHandleCount; Index++) {
    ...
    if (BdsMatchUsbClass (UsbIo, (USB_CLASS_DEVICE_PATH *) ShortFormDevicePath) ||
      ...
      if (EFI_ERROR (Status)) {
        ...
        //
        NemuLogFlowFuncMark();
#ifdef NEMU /* We look for serveral files because of apple. */
        {
          INT32 IdxFile;
  ...
  )
{
  ...
  NemuLog(("BdsLibBootViaBootOption: BootCurrent=0x%x OptionName=%s Description=%s StatusString=%s Attribute=0x%x LoadOptionsSize=0x%x\n",
  ...
  *ExitData     = NULL;
  ...
  if (ImageHandle == NULL) {
    ...
    if (EFI_ERROR (Status)) {
      ...
      NemuLogFlowFuncMarkVar(Handle, "%p");
      if (Handle == NULL) {
        goto Done;
      }
#ifdef NEMU /* We have more options to try out because of apple. */
      //
      ...
      if (Option->LoadOptionsSize) {
         ...
         */
        NemuLogFlowFuncMarkVar(Option->LoadOptions, "%s");
        if (StrnCmp(L"config=", (CHAR16 *)Option->LoadOptions, 7) == 0) {
          ...
          pszTemp1 = pszTemp0;
          NemuLogFlowFuncMarkVar(pszTemp0, "%s");
          NemuLogFlowFuncMarkVar(pszTemp1, "%s");
          ...
          while (pszTemp0) {
              ...
              NemuLogFlowFuncMarkVar(pszFolder, "%s");
              NemuLogFlowFuncMarkVar(pszTemp0, "%s");
              NemuLogFlowFuncMarkVar(pszTemp1, "%s");
          ...
          if (!EFI_ERROR(Status)) {
            ...
            if (FilePath) {
              NemuLogFlowFuncMarkVar(DevicePathToStr(FilePath), "%s");
              REPORT_STATUS_CODE(EFI_PROGRESS_CODE, PcdGet32 (PcdProgressCodeOsLoaderLoad));
      ...
      if (EFI_ERROR(Status)) {
        ...
        for (IdxFile = 0; IdxFile < sizeof(maBdsLibBootFiles) / sizeof(maBdsLibBootFiles[0]); IdxFile++) {
          ...
          if (FilePath) {
            NemuLog(("BdsLibBootViaBootOption: Trying to load '%s'...\n", DevicePathToStr(FilePath)));
            REPORT_STATUS_CODE(EFI_PROGRESS_CODE, PcdGet32(PcdProgressCodeOsLoaderLoad));
            Status = gBS->LoadImage(TRUE, gImageHandle, FilePath, NULL, 0, &ImageHandle );
            if (!EFI_ERROR(Status)) {
              NemuLog(("BdsLibBootViaBootOption: Successfully loaded boot file: %s\n", DevicePathToStr(FilePath)));
              break;
            }
            NemuLog(("BdsLibBootViaBootOption: Failed loading '%s': 0x%x(%r)\n", DevicePathToStr(FilePath), Status, Status));
  ...
  )
{
  ...
  if (NULL == BootOrder) {
    NemuLogFlowFuncLeaveRC(EFI_NOT_FOUND);
    return EFI_NOT_FOUND;
  }
  ...
  while (Index < BootOrderSize / sizeof (UINT16)) {
    UnicodeSPrint (BootOption, sizeof (BootOption), L"Boot%04x", BootOrder[Index]);
    NemuLogFlowFuncMarkVar(BootOption, "%s");
    BootOptionVar = BdsLibGetVariableAndSize (
                      BootOption,
                      &gEfiGlobalVariableGuid,
                      &BootOptionSize
    ...
    if (NULL == BootOptionVar) {
      FreePool (BootOrder);
      NemuLogFlowFuncLeaveRC(EFI_OUT_OF_RESOURCES);
      return EFI_OUT_OF_RESOURCES;
    }
    ...
    if (Corrupted || !BdsLibIsValidEFIBootOptDevicePathExt (OptionDevicePath, FALSE, Description)) {
      ...
      //
      NemuLogFlowFuncMarkVar(BootOption, "DELETE %s");
      Status = gRT->SetVariable (
                      BootOption,
                      &gEfiGlobalVariableGuid,
                      EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE,
  ...
  )
{
  ...
  EFI_IMAGE_OPTIONAL_HEADER_PTR_UNION   Hdr = {0}; /* Nemu: VS2010 thinks it may be used uninitialized. */
  ...
  for (Index = 0; Index < NumberFileSystemHandles; Index++) {
    ...
    //  machinename is ia32, ia64, x64, ...
    //
#ifdef NEMU /* We try serveral variants because of apple. */
    {
      UINT32 IdxFile;
      NeedDelete = TRUE;
      for (IdxFile = 0; IdxFile < sizeof(maBdsLibBootFiles) / sizeof(maBdsLibBootFiles[0]); IdxFile++) {
  ...
  )
{
  EFI_DEVICE_PATH_PROTOCOL  *DevicePath;
  NemuLogFlowFuncEnter();
  NemuLogFlowFuncMarkVar(String, "%s");
  ...
  )
{
  ...
  NemuLogFlowFuncEnter();
  DEBUG((DEBUG_INFO, "%a:%d DevicePath:%s\n", __FUNCTION__, __LINE__, DevicePathToStr(DevicePath))); /* Nemu */
  ...
  NemuLogFlowFuncMarkVar(NumberSimpleFileSystemHandles, "%d");
  for (Index = 0; Index < NumberSimpleFileSystemHandles; Index++) {
    ...
    DEBUG((DEBUG_INFO, "%a:%d TempDevicePath:%s\n", __FUNCTION__, __LINE__, DevicePathToStr(TempDevicePath))); /* Nemu */
    //
...
# if 1
        ...
        while (!IsDevicePathEndType (pNode0)) {
            ...
            NemuLogFlowFuncMarkVar(DevicePathToStr(pNode0), "%s");
            NemuLogFlowFuncMarkVar(DevicePathType(pNode0), "%x");
            NemuLogFlowFuncMarkVar(DevicePathSubType(pNode0), "%x");
        }
        ...
        while (!IsDevicePathEndType (pNode1)) {
            ...
            NemuLogFlowFuncMarkVar(DevicePathToStr(pNode1), "%s");
            NemuLogFlowFuncMarkVar(DevicePathType(pNode1), "%x");
            NemuLogFlowFuncMarkVar(DevicePathSubType(pNode1), "%x");
        }
        if (   pTarget
            && pFs
            ...
            && DevicePathSubType(pFs) == DevicePathSubType(pTarget))
        {
            if (DevicePathSubType(pTarget) == MEDIA_HARDDRIVE_DP)
            {
                ...
                /* @todo: is it enough? */
                NemuLogFlowFuncMarkVar(pTargetHd->PartitionNumber, "%x");
                NemuLogFlowFuncMarkVar(pTargetHd->PartitionStart, "%lx");
                NemuLogFlowFuncMarkVar(pTargetHd->PartitionSize, "%lx");
                NemuLogFlowFuncMarkVar(pFsHd->PartitionNumber, "%x");
                NemuLogFlowFuncMarkVar(pFsHd->PartitionStart, "%lx");
                NemuLogFlowFuncMarkVar(pFsHd->PartitionSize, "%lx");
...
# endif
  ...
  NemuLogFlowFuncMarkVar(ReturnHandle, "%p");
  NemuLogFlowFuncLeave();
  return ReturnHandle;
}
  ...
  )
{
  ...
  NemuLog(("BdsLibIsValidEFIBootOptDevicePathExt: DevPath=%s CheckMedia=%d Description=%s\n",
  ...
  if (!EFI_ERROR (Status)) {
    if (!IsDevicePathEnd (TempDevicePath)) {
      ...
      //
      NemuLog(("BdsLibIsValidEFIBootOptDevicePathExt: returns FALSE (#1)\n"));
      return FALSE;
    }
    ...
    if (CheckMedia) {
      ...
      if (BdsLibNetworkBootWithMediaPresent(DevPath)) {
        NemuLog(("BdsLibIsValidEFIBootOptDevicePathExt: returns TRUE (#2)\n"));
        return TRUE;
      }
    } else {
      NemuLog(("BdsLibIsValidEFIBootOptDevicePathExt: returns TRUE (#3)\n"));
      return TRUE;
    }
  }
  ...
  while (!IsDevicePathEnd (TempDevicePath)) {
    ...
    if ((DevicePathType (TempDevicePath) == MESSAGING_DEVICE_PATH) &&
        ((DevicePathSubType (TempDevicePath) == MSG_USB_CLASS_DP) ||
         (DevicePathSubType (TempDevicePath) == MSG_USB_WWID_DP))) {
      NemuLog(("BdsLibIsValidEFIBootOptDevicePathExt: returns TRUE (#4)\n"));
      return TRUE;
    }
  ...
  if ((DevicePathType (LastDeviceNode) == MEDIA_DEVICE_PATH) &&
    ...
    NemuLog(("BdsLibIsValidEFIBootOptDevicePathExt: returns TRUE (#5)\n"));
    return TRUE;
  }
  ...
  if (EfiGetNameGuidFromFwVolDevicePathNode ((MEDIA_FW_VOL_FILEPATH_DEVICE_PATH *) LastDeviceNode) != NULL) {
    ...
    if (Status == EFI_ALREADY_STARTED) {
      NemuLog(("BdsLibIsValidEFIBootOptDevicePathExt: returns TRUE (#6)\n"));
      return TRUE;
    } else {
      if (Status == EFI_SUCCESS) {
        FreePool (TempDevicePath);
      }
      NemuLog(("BdsLibIsValidEFIBootOptDevicePathExt: returns FALSE (#7)\n"));
      return FALSE;
    }
  }
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if (!EFI_ERROR (Status)) {
      if (CheckMedia) {
        ...
        if (BdsLibGetBootableHandle (DevPath) != NULL) {
          NemuLog(("BdsLibIsValidEFIBootOptDevicePathExt: returns TRUE (#8)\n"));
          return TRUE;
        }
      } else {
        NemuLog(("BdsLibIsValidEFIBootOptDevicePathExt: returns TRUE (#9)\n"));
        return TRUE;
      }
    }
  } else {
    ...
    if (!EFI_ERROR (Status)) {
      if (CheckMedia) {
        ...
        //
        if (BdsLibGetBootableHandle (DevPath) != NULL) {
          NemuLog(("BdsLibIsValidEFIBootOptDevicePathExt: returns TRUE (#10)\n"));
          return TRUE;
        }
      } else {
        NemuLog(("BdsLibIsValidEFIBootOptDevicePathExt: returns TRUE (#10)\n"));
  ...
  return FALSE;
#else
  NemuLog(("BdsLibIsValidEFIBootOptDevicePathExt: returns TRUE (#11) - Status 0x%x(%r)\n", Status, Status));
  return TRUE;
#endif
}
