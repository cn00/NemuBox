/* $Id: ConsoleImpl.cpp $ */
/** @file
 * Nemu Console COM Class implementation
 */
...
#include "Nvram.h"
#ifdef NEMU_WITH_USB_CARDREADER
...
#include <Nemu/vmm/pdmasynccompletion.h>
#include <Nemu/vmm/pdmnetifs.h>
#ifdef NEMU_WITH_USB
# include <Nemu/vmm/pdmusb.h>
#endif
#ifdef NEMU_WITH_NETSHAPER
# include <Nemu/vmm/pdmnetshaper.h>
#endif /* NEMU_WITH_NETSHAPER */
#include <Nemu/vmm/mm.h>
#include <Nemu/vmm/ftm.h>
...
#include <Nemu/HostServices/DragAndDropSvc.h>
#ifdef NEMU_WITH_GUEST_PROPS
# include <Nemu/HostServices/GuestPropertySvc.h>
# include <Nemu/com/array.h>
#endif

#ifdef NEMU_OPENSSL_FIPS
...
public:
    ...
    STDMETHOD(HandleEvent)(NemuEventType_T aType, IEvent *aEvent)
    {
        switch(aType)
        {
            case NemuEventType_OnNATRedirect:
            {
                Bstr id;
            ...
            case NemuEventType_OnHostNameResolutionConfigurationChange:
            {
                mConsole->i_onNATDnsChanged();
                break;
            }

            case NemuEventType_OnHostPCIDevicePlug:
            {
                // handle if needed
                break;
            }

            case NemuEventType_OnExtraDataChanged:
            {
...
NEMU_LISTENER_DECLARE(VmEventListenerImpl)
...
HRESULT Console::init(IMachine *aMachine, IInternalMachineControl *aControl, LockType_T aLockType)
{
    ...
    rc = mptrExtPackManager->initExtPackManager(NULL, NEMUEXTPACKCTX_VM_PROCESS);
        AssertComRCReturnRC(rc);
#endif
    ...
    if (aLockType == LockType_VM)
    {
        ...
        /* VirtualBox events registration. */
        {
            ...
            com::SafeArray<NemuEventType_T> eventTypes;
            eventTypes.push_back(NemuEventType_OnNATRedirect);
            eventTypes.push_back(NemuEventType_OnHostNameResolutionConfigurationChange);
            eventTypes.push_back(NemuEventType_OnHostPCIDevicePlug);
            eventTypes.push_back(NemuEventType_OnExtraDataChanged);
 ...
 *       service to do this instead of detouring thru NemuSVC.
 *       (IMachine::SetGuestProperty ends up in NemuSVC, which in turns calls
...
{
    Bstr value;
    HRESULT hrc = mMachine->GetExtraData(Bstr("NemuInternal2/EnableGuestPropertiesVRDP").raw(),
                                         value.asOutParam());
    if (   hrc   == S_OK
        && value == "1")
        return true;
...
{
    Bstr value;
    HRESULT hrc = mMachine->GetExtraData(Bstr("NemuInternal2/TurnResetIntoPowerOff").raw(),
                                         value.asOutParam());
    if (   hrc   == S_OK
        && value == "1")
        return true;
                                                     ...
                                                     uint32_t cbParms)
{
    ...
    else
    {
        ...
        rc = Global::nemuStatusCodeFromCOM(hrc);
    }
    return rc;
}
                                              ...
                                              std::vector<Utf8Str> &aFlags)
{
    ...
    NEMUHGCMSVCPARM parm[3];

    parm[0].type = NEMU_HGCM_SVC_PARM_PTR;
    ...
    for (unsigned i = 0; i < 10 && (VERR_BUFFER_OVERFLOW == vrc); ++i)
    {
        ...
        parm[1].type = NEMU_HGCM_SVC_PARM_PTR;
        parm[1].u.pointer.addr = Utf8Buf.mutableRaw();
        parm[1].u.pointer.size = (uint32_t)cchBuf + 1024;

        parm[2].type = NEMU_HGCM_SVC_PARM_32BIT;
        parm[2].u.uint32 = 0;

        vrc = m_pVMMDev->hgcmHostCall("NemuGuestPropSvc", ENUM_PROPS_HOST, 3,
                                      &parm[0]);
        Utf8Buf.jolt();
        if (parm[2].type != NEMU_HGCM_SVC_PARM_32BIT)
...
HRESULT Console::powerDown(ComPtr<IProgress> &aProgress)
{
    ...
    switch (mMachineState)
    {
        ...
        case MachineState_Saving:
            ...
            return setError(NEMU_E_INVALID_VM_STATE, tr("Cannot power down at this point during a save state"));
        ...
        case MachineState_TeleportingPausedVM:
            ...
            return setError(NEMU_E_INVALID_VM_STATE, tr("Cannot power down at this point in a teleportation"));
        ...
        case MachineState_OnlineSnapshotting:
            ...
            return setError(NEMU_E_INVALID_VM_STATE, tr("Cannot power down at this point in an online snapshot"));
        ...
        case MachineState_LiveSnapshotting:
            ...
            return setError(NEMU_E_INVALID_VM_STATE, tr("Cannot power down at this point in a live snapshot"));
        ...
        case MachineState_FaultTolerantSyncing:
            ...
            return setError(NEMU_E_INVALID_VM_STATE, tr("Cannot power down at this point in a fault tolerant sync"));

        /* extra nice error message for a common case */
        case MachineState_Saved:
            return setError(NEMU_E_INVALID_VM_STATE, tr("Cannot power down a saved virtual machine"));
        case MachineState_Stopping:
            return setError(NEMU_E_INVALID_VM_STATE, tr("The virtual machine is being powered down"));
        default:
            return setError(NEMU_E_INVALID_VM_STATE,
...
HRESULT Console::i_doCPURemove(ULONG aCpu, PUVM pUVM)
{
    ...
    if (RT_SUCCESS(vrc) && !fLocked)
    {
         ...
         */
        PVMREQ pReq;
        vrc = VMR3ReqCallU(pUVM, 0, &pReq, 0 /* no wait! */, VMREQFLAGS_NEMU_STATUS,
                           (PFNRT)i_unplugCpu, 3,
...
HRESULT Console::i_doCPUAdd(ULONG aCpu, PUVM pUVM)
{
     ...
     */
    PVMREQ pReq;
    int vrc = VMR3ReqCallU(pUVM, 0, &pReq, 0 /* no wait! */, VMREQFLAGS_NEMU_STATUS,
                           (PFNRT)i_plugCpu, 3,
                           this, pUVM, aCpu);
...
HRESULT Console::attachUSBDevice(const com::Guid &aId, const com::Utf8Str &aCaptureFilename)
{
    ...
    if (!mfVMHasUsbController)
        return setError(NEMU_E_PDM_ERROR,
...
HRESULT Console::findUSBDeviceByAddress(const com::Utf8Str &aName, ComPtr<IUSBDevice> &aDevice)
{
    ...
    return setErrorNoLog(NEMU_E_OBJECT_NOT_FOUND,
...
HRESULT Console::createSharedFolder(const com::Utf8Str &aName, const com::Utf8Str &aHostPath, BOOL aWritable, BOOL aAutomount)
{
    ...
    if (SUCCEEDED(rc))
        return setError(NEMU_E_FILE_ERROR,
                                           ...
                                           BOOL aClearOnSuspend)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        hrc = setError(NEMU_E_OBJECT_IN_USE, tr("A password with the given ID already exists"));
                                            ...
                                            BOOL aClearOnSuspend)
{
    ...
    for (unsigned i = 0; i < aIds.size(); i++)
    {
        ...
        if (rc != VERR_NOT_FOUND)
        {
            ...
            if (pKey)
                pKey->release();
            return setError(NEMU_E_OBJECT_IN_USE, tr("A password with the given ID already exists"));
        }
    }
...
HRESULT Console::removeDiskEncryptionPassword(const com::Utf8Str &aId)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        return setError(NEMU_E_OBJECT_NOT_FOUND, tr("A password with the ID \"%s\" does not exist"),
                                                 aId.c_str());
    else
...
HRESULT Console::clearAllDiskEncryptionPasswords()
{
    ...
    if (rc == VERR_RESOURCE_IN_USE)
        return setError(NEMU_E_OBJECT_IN_USE, tr("A password is still in use by the VM"));
    ...
    *pfResume = false;
    ...
    switch (enmVMState)
    {
        ...
        case VMSTATE_RUNNING:
        {
            ...
            mVMStateChangeCallbackDisabled = false;
            if (RT_FAILURE(rc))
                return setErrorInternal(NEMU_E_INVALID_VM_STATE,
                                        COM_IIDOF(IConsole),
                                        getStaticComponentName(),
...
HRESULT Console::i_doMediumChange(IMediumAttachment *aMediumAttachment, bool fForce, PUVM pUVM)
{
     ...
     */
    PVMREQ pReq;
    int vrc = VMR3ReqCallU(pUVM, VMCPUID_ANY, &pReq, 0 /* no wait! */, VMREQFLAGS_NEMU_STATUS,
 ...
 * so we have to go back to NemuSVC and ask...  This is not ideal.
 */
...
HRESULT Console::i_clearDiskEncryptionKeysOnAllAttachmentsWithKeyId(const Utf8Str &strId)
{
    ...
    for (unsigned i = 0; i < sfaAttachments.size(); i++)
    {
        ...
        if (hrc == NEMU_E_OBJECT_NOT_FOUND)
        {
            hrc = S_OK;
            continue;
        }
            ...
            if (RT_SUCCESS(rc))
            {
                if (pIBase)
                {
                    ...
                    else
                    {
                        ...
                        {
                            hrc = setError(NEMU_E_PASSWORD_INCORRECT, tr("The provided password for ID \"%s\" is not correct for at least one disk using this ID"),
                                           strId.c_str());
                            break;
                        }
                        else if (RT_FAILURE(rc))
...
HRESULT Console::i_onExtraDataChange(IN_BSTR aMachineId, IN_BSTR aKey, IN_BSTR aVal)
{
    ...
    if (ptrVM.isOk())
    {
        ...
        if (strKey == "NemuInternal2/TurnResetIntoPowerOff")
        {
...
{
#ifndef NEMU_WITH_GUEST_PROPS
    ReturnComNotImplemented();
#else  /* NEMU_WITH_GUEST_PROPS */
    ...
    try
    {
        ...
        parm[2].type = NEMU_HGCM_SVC_PARM_64BIT;
        parm[2].u.uint64 = 0;

        parm[3].type = NEMU_HGCM_SVC_PARM_32BIT;
        parm[3].u.uint32 = 0;

        int vrc = m_pVMMDev->hgcmHostCall("NemuGuestPropSvc", GET_PROP_HOST,
                                          4, &parm[0]);
        /* The returned string should never be able to be greater than our buffer */
        AssertLogRel(vrc != VERR_BUFFER_OVERFLOW);
        AssertLogRel(RT_FAILURE(vrc) || parm[2].type == NEMU_HGCM_SVC_PARM_64BIT);
        if (RT_SUCCESS(vrc))
        {
            *aValue = szBuffer;
        ...
        else if (vrc == VERR_NOT_FOUND)
        {
            *aValue = "";
            ...
            rc = setError(NEMU_E_IPRT_ERROR,
                          tr("The NemuGuestPropSvc service call failed with the error %Rrc"),
                          vrc);
    }
...
HRESULT Console::i_setGuestProperty(const Utf8Str &aName, const Utf8Str &aValue, const Utf8Str &aFlags)
{
    ...
    if (aFlags.isEmpty())
    {
        vrc = m_pVMMDev->hgcmHostCall("NemuGuestPropSvc", SET_PROP_VALUE_HOST,
                                    2, &parm[0]);
    }
    else
    {
        parm[2].type = NEMU_HGCM_SVC_PARM_PTR;
        parm[2].u.pointer.addr = (void*)aFlags.c_str();
        parm[2].u.pointer.size = (uint32_t)aFlags.length() + 1; /* The + 1 is the null terminator */

        vrc = m_pVMMDev->hgcmHostCall("NemuGuestPropSvc", SET_PROP_HOST,
                                      3, &parm[0]);
    }

    HRESULT hrc = S_OK;
    if (RT_FAILURE(vrc))
        hrc = setError(NEMU_E_IPRT_ERROR, tr("The NemuGuestPropSvc service call failed with the error %Rrc"), vrc);
    return hrc;
#endif /* NEMU_WITH_GUEST_PROPS */
}
...
HRESULT Console::i_deleteGuestProperty(const Utf8Str &aName)
{
    ...
    NEMUHGCMSVCPARM parm[1];

    parm[0].type = NEMU_HGCM_SVC_PARM_PTR;
    parm[0].u.pointer.addr = (void*)aName.c_str();
    parm[0].u.pointer.size = (uint32_t)aName.length() + 1; /* The + 1 is the null terminator */

    int vrc = m_pVMMDev->hgcmHostCall("NemuGuestPropSvc", DEL_PROP_HOST,
                                      1, &parm[0]);
...
HRESULT Console::i_reconfigureMediumAttachments(const std::vector<ComPtr<IMediumAttachment> > &aAttachments)
{
    ...
    for (size_t i = 0; i < aAttachments.size(); ++i)
    {
         ...
         * IPC (to NemuSVC) which takes a significant amount of time.
 ...
 * in module NemuC (and ConsoleImpl.cpp is part of it and thus can call it).
 * Extension modules must not link directly against NemuC, (XP)COM is
 * handling this.
 */
...
HRESULT Console::i_pause(Reason_T aReason)
{
    ...
    switch (mMachineState)
    {
        ...
        case MachineState_OnlineSnapshotting:
            ...
            return setError(NEMU_E_INVALID_VM_STATE, tr("Already paused"));
    ...
    HRESULT hrc = S_OK;
    if (RT_FAILURE(vrc))
        hrc = setError(NEMU_E_VM_ERROR, tr("Could not suspend the machine execution (%Rrc)"), vrc);
    ...
    else
    {
        ...
        else
        {
            ...
            if (   VMR3GetStateU(ptrVM.rawUVM()) == VMSTATE_SUSPENDED
                ...
                return setError(NEMU_E_INVALID_VM_STATE, tr("VM is paused due to host power management"));

            enmReason = aReason == Reason_Snapshot ? VMRESUMEREASON_STATE_SAVED : VMRESUMEREASON_USER;
        }

        // for snapshots: no state change callback, NemuSVC does everything
...
HRESULT Console::i_saveState(Reason_T aReason, const ComPtr<IProgress> &aProgress, const Utf8Str &aStateFilePath, bool aPauseVM, bool &aLeftPaused)
{
    ...
    mMachine->GetExtraData(Bstr("NemuInternal2/DisableSaveState").raw(), strDisableSaveState.asOutParam());
    if (strDisableSaveState == "1")
        return setError(NEMU_E_VM_ERROR,
    ...
    if (aPauseVM)
    {
        ...
        if (RT_FAILURE(vrc))
            return setError(NEMU_E_VM_ERROR, tr("Could not suspend the machine execution (%Rrc)"), vrc);
        fPaused = true;
    }
 ...
 *          Additions are available (via NemuTray/NemuClient).
 */
...
HRESULT Console::i_onShowWindow(BOOL aCheck, BOOL *aCanShow, LONG64 *aWinId)
{
    ...
    *aWinId = 0;
    ...
    NemuEventDesc evDesc;
    if (aCheck)
    {
        evDesc.init(mEventSource, NemuEventType_OnCanShowWindow);
    ...
    {
        evDesc.init(mEventSource, NemuEventType_OnShowWindow, INT64_C(0));
        BOOL fDelivered = evDesc.fire(5000); /* Wait up to 5 secs for delivery */
        //Assert(fDelivered);
        if (fDelivered)
        {
...
HRESULT Console::i_consoleInitReleaseLog(const ComPtr<IMachine> aMachine)
{
    ...
    Utf8Str logFile = Utf8StrFmt("%s%cNemu.log",
                                 logDir.c_str(), RTPATH_DELIMITER);
    Utf8Str pngFile = Utf8StrFmt("%s%cNemu.png",
                                 logDir.c_str(), RTPATH_DELIMITER);
    ...
    /* If we've made any directory changes, flush the directory to increase
       ...
       Tip: Try 'export NEMU_RELEASE_LOG_FLAGS=flush' if the last bits of the log
...
HRESULT Console::i_powerUp(IProgress **aProgress, bool aPaused)
{
    ...
    try
    {
        if (Global::IsOnlineOrTransient(mMachineState))
            throw setError(NEMU_E_INVALID_VM_STATE,
...
#if 0 /** @todo we should save it afterwards, but that isn't necessarily a good idea. Find a better place for this (NemuSVC).  */
        ...
        if (mMachineState == MachineState_Saved)
        {
            ...
            if (RT_FAILURE(vrc))
                throw setError(NEMU_E_FILE_ERROR,
        ...
        Bstr value;
        HRESULT hrc = mMachine->GetExtraData(Bstr("NemuInternal2/CoreDumpEnabled").raw(), value.asOutParam());
        if (SUCCEEDED(hrc) && value == "1")
        {
            Bstr coreDumpDir, coreDumpReplaceSys, coreDumpLive;
            mMachine->GetExtraData(Bstr("NemuInternal2/CoreDumpDir").raw(), coreDumpDir.asOutParam());
            mMachine->GetExtraData(Bstr("NemuInternal2/CoreDumpReplaceSystemDump").raw(), coreDumpReplaceSys.asOutParam());
            mMachine->GetExtraData(Bstr("NemuInternal2/CoreDumpLive").raw(), coreDumpLive.asOutParam());
        ...
        /* Tell NemuSVC and Machine about the progress object so they can
             ...
             ++slot)
        {
            ...
            switch (netattach)
            {
                case NetworkAttachmentType_Bridged:
                {
                    ...
                    if (hostif.isEmpty())
                    {
                        throw setError(NEMU_E_HOST_ERROR,
                                    ...
                                    bool aSetError /* = false */)
{
    ...
    if (aSetError)
        setError(NEMU_E_FILE_ERROR,
            tr("Could not find a shared folder named '%s'."),
            strName.c_str());

    return NEMU_E_FILE_ERROR;
}
...
HRESULT Console::i_createSharedFolder(const Utf8Str &strName, const SharedFolderData &aData)
{
    ...
    NEMUHGCMSVCPARM parms[SHFL_CPARMS_ADD_MAPPING];
    SHFLSTRING *pFolderName, *pMapName;
    size_t cbString;

    Bstr value;
    HRESULT hrc = mMachine->GetExtraData(BstrFmt("NemuInternal2/SharedFoldersEnableSymlinksCreate/%s",
                                                 strName.c_str()).raw(),
                                         value.asOutParam());
    ...
    parms[0].type = NEMU_HGCM_SVC_PARM_PTR;
    ...
    vrc = m_pVMMDev->hgcmHostCall("NemuSharedFolders",
                                  SHFL_FN_ADD_MAPPING,
...
HRESULT Console::i_removeSharedFolder(const Utf8Str &strName)
{
    ...
    parms.type = NEMU_HGCM_SVC_PARM_PTR;
    parms.u.pointer.addr = pMapName;
    parms.u.pointer.size = ShflStringSizeOfBuffer(pMapName);

    int vrc = m_pVMMDev->hgcmHostCall("NemuSharedFolders",
...
void Console::i_changeClipboardMode(ClipboardMode_T aClipboardMode)
{
    ...
    NEMUHGCMSVCPARM parm;
    parm.type = NEMU_HGCM_SVC_PARM_32BIT;
    ...
    switch (aClipboardMode)
    {
        ...
        case ClipboardMode_Disabled:
            ...
            parm.u.uint32 = NEMU_SHARED_CLIPBOARD_MODE_OFF;
            break;
        case ClipboardMode_GuestToHost:
            LogRel(("Shared clipboard mode: Guest to Host\n"));
            parm.u.uint32 = NEMU_SHARED_CLIPBOARD_MODE_GUEST_TO_HOST;
            break;
        case ClipboardMode_HostToGuest:
            LogRel(("Shared clipboard mode: Host to Guest\n"));
            parm.u.uint32 = NEMU_SHARED_CLIPBOARD_MODE_HOST_TO_GUEST;
            break;
        case ClipboardMode_Bidirectional:
            LogRel(("Shared clipboard mode: Bidirectional\n"));
            parm.u.uint32 = NEMU_SHARED_CLIPBOARD_MODE_BIDIRECTIONAL;
            break;
    }

    pVMMDev->hgcmHostCall("NemuSharedClipboard", NEMU_SHARED_CLIPBOARD_HOST_FN_SET_MODE, 1, &parm);
}
...
int Console::i_changeDnDMode(DnDMode_T aDnDMode)
{
    ...
    switch (aDnDMode)
    {
        ...
        case DnDMode_Disabled:
            ...
            parm.u.uint32 = NEMU_DRAG_AND_DROP_MODE_OFF;
            break;
        case DnDMode_GuestToHost:
            LogRel(("Drag and drop mode: Guest to Host\n"));
            parm.u.uint32 = NEMU_DRAG_AND_DROP_MODE_GUEST_TO_HOST;
            break;
        case DnDMode_HostToGuest:
            LogRel(("Drag and drop mode: Host to Guest\n"));
            parm.u.uint32 = NEMU_DRAG_AND_DROP_MODE_HOST_TO_GUEST;
            break;
        case DnDMode_Bidirectional:
            LogRel(("Drag and drop mode: Bidirectional\n"));
            parm.u.uint32 = NEMU_DRAG_AND_DROP_MODE_BIDIRECTIONAL;
            break;
    }

    int rc = pVMMDev->hgcmHostCall("NemuDragAndDropSvc",
...
#if ((defined(RT_OS_LINUX) && !defined(NEMU_WITH_NETFLT)) || defined(RT_OS_FREEBSD))
/**
...
# ifdef RT_OS_LINUX
    ...
    if (RT_SUCCESS(rcNemu))
    {
        ...
        if (SUCCEEDED(rc))
        {
            ...
            rcNemu = ioctl(RTFileToNative(maTapFD[slot]), TUNSETIFF, &IfReq);
            if (rcNemu != 0)
            {
...
# elif defined(RT_OS_FREEBSD)
    ...
    int rcNemu = RTFileOpen(&maTapFD[slot], szTapdev,
...
# endif
    ...
    if (RT_FAILURE(rcNemu) && SUCCEEDED(rc))
    {
...
# endif /* NEMU_STRICT */
    ...
    if (maTapFD[slot] != NIL_RTFILE)
    {
        ...
        if (FAILED(rc) || tapDeviceName.isEmpty())
        {
            ...
            isStatic = false;
            int rcNemu = RTFileClose(maTapFD[slot]);
            AssertRC(rcNemu);
...
#endif /* (RT_OS_LINUX || RT_OS_FREEBSD) && !NEMU_WITH_NETFLT */
...
HRESULT Console::i_powerDownHostInterfaces()
{
    ...
    for (ULONG slot = 0; slot < maxNetworkAdapters; slot++)
    {
        ...
        {
#if ((defined(RT_OS_LINUX) || defined(RT_OS_FREEBSD)) && !defined(NEMU_WITH_NETFLT))
...
HRESULT Console::i_captureUSBDevices(PUVM pUVM)
{
    ...
    if (mfVMHasUsbController)
    {
        /* release the lock before calling Host in NemuSVC since Host may call
...
DECLCALLBACK(int) Console::i_powerUpThread(RTTHREAD Thread, void *pvUser)
{
    ...
    try
    {
        ...
        if (SUCCEEDED(rc) && RT_FAILURE(vrc))
        {
            ...
            if (!task->mErrorMsg.length())
            {
                 ...
                 * convert the Nemu status code into a meaningful error message.
    ...
    /* Notify NemuSVC and any waiting openRemoteSession progress object. */
...
Console::i_vmm2User_SaveState(PCVMM2USERMETHODS pThis, PUVM pUVM)
{
    ...
    return SUCCEEDED(hrc) ? VINF_SUCCESS : Global::nemuStatusCodeFromCOM(hrc);
}
