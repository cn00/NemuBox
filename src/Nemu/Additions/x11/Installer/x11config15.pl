...
# the graphics card (not of nemumouse as far as I can see).  Although this
...
# for use on a VirtualBox guest) and replace them with nemuvideo.
...
foreach $cfg (@cfg_files) {
    ...
    if (open(CFG, $cfg)) {
        ...
        while (defined ($line = <CFG>)) {
            ...
            if ($in_section) {
                if ($line =~ /^\s*driver\s+\"(fbdev|vga|vesa|nemuvideo|ChangeMe)\"/i) {
                    $line =~ s/(fbdev|vga|vesa|nemuvideo|ChangeMe)/nemuvideo/i;
                }
            }
            print TMP $line;
        }
        ...
        # We do not overwrite existing $cfg.".nemu" files because that will
