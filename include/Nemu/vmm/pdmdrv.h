...
#ifndef ___Nemu_vmm_pdmdrv_h
#define ___Nemu_vmm_pdmdrv_h
 ...
 *
 * This structure is used when registering a driver from NemuInitDrivers() (in
...
/** Pointer to callbacks provided to the NemuDriverRegister() call. */
typedef struct PDMDRVREGCB *PPDMDRVREGCB;
/** Pointer to const callbacks provided to the NemuDriverRegister() call. */
typedef const struct PDMDRVREGCB *PCPDMDRVREGCB;

/**
 * Callbacks for NemuDriverRegister().
 */
...
/**
 * The NemuDriverRegister callback function.
 *
 ...
 */
typedef DECLCALLBACK(int) FNPDMNEMUDRIVERSREGISTER(PCPDMDRVREGCB pCallbacks, uint32_t u32Version);

VMMR3DECL(int) PDMR3DrvStaticRegistration(PVM pVM, FNPDMNEMUDRIVERSREGISTER pfnCallback);
