...
private:
    ...
    void i_importMachineGeneric(const ovf::VirtualSystem &vsysThis,
                                ...
                                PVDINTERFACEIO pCallbacks,
                                PSHASTORAGE pStorage);
    void i_importNemuMachine(ComObjPtr<VirtualSystemDescription> &vsdescThis,
                             ComPtr<IMachine> &pNewMachine,
                             ImportStack &stack,
                             PVDINTERFACEIO pCallbacks,
                             PSHASTORAGE pStorage);
...
struct VirtualSystemDescriptionEntry
{
    ...
    Utf8Str strNemuSuggested;               ///< configuration value (type-dependent); original value suggested by interpret()
    Utf8Str strNemuCurrent;                 ///< configuration value (type-dependent); current value, either from interpret() or setFinalValue()
...
public:
    ...
    void i_importNemuMachineXML(const xml::ElementNode &elmMachine);
