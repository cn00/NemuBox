...
#define NemuShapeNotify 64

/**
 * Callback which provides the interface for notifying the host of changes to
 * the X11 window configuration, mainly split out from @a NemuGuestSeamlessHost
...
/** Structure containing information about a guest window's position and visible area.
    Used inside of NemuGuestWindowList. */
struct NemuGuestWinInfo {
public:
    ...
    XRectangle *mpRects;
    /** Constructor. */
    NemuGuestWinInfo(bool hasShape, int x, int y, int w, int h, int cRects,
                     XRectangle *pRects)
...
private:
    ...
    NemuGuestWinInfo(const NemuGuestWinInfo&);
    NemuGuestWinInfo& operator=(const NemuGuestWinInfo&);
};

/** Callback type used for "DoWithAll" calls */
typedef DECLCALLBACK(int) NEMUGUESTWINCALLBACK(NemuGuestWinInfo *, void *);
/** Pointer to NEMUGUESTWINCALLBACK */
typedef NEMUGUESTWINCALLBACK *PNEMUGUESTWINCALLBACK;

DECLCALLBACK(int) inline NemuGuestWinCleanup(NemuGuestWinInfo *pInfo, void *)
{
    delete pInfo;
    return VINF_SUCCESS;
}
 ...
 */
class NemuGuestWindowList
{
private:
    // We don't want a copy constructor or an assignment operator
    NemuGuestWindowList(const NemuGuestWindowList&);
    NemuGuestWindowList& operator=(const NemuGuestWindowList&);
...
public:
    // Constructor
    NemuGuestWindowList(void) : mWindows(NULL) {}
    // Destructor
    ~NemuGuestWindowList()
    {
        ...
        doWithAll(NemuGuestWinCleanup, NULL);
    }

    // Standard operations
    NemuGuestWinInfo *find(Window hWin)
    {
        return (NemuGuestWinInfo *)RTAvlU32Get(&mWindows, hWin);
    }

    void detachAll(PNEMUGUESTWINCALLBACK pCallback, void *pvParam)
    {
        RTAvlU32Destroy(&mWindows, (PAVLU32CALLBACK)pCallback, pvParam);
    }

    int doWithAll(PNEMUGUESTWINCALLBACK pCallback, void *pvParam)
    {
                   ...
                   XRectangle *pRects)
    {
        ...
        NemuGuestWinInfo *pInfo = new NemuGuestWinInfo(isMapped, x, y, w, h, cRects,
                                                       pRects);
        pInfo->Core.Key = hWin;
    ...
    NemuGuestWinInfo *removeWindow(Window hWin)
    {
        LogRelFlowFunc(("called\n"));
        return (NemuGuestWinInfo *)RTAvlU32Remove(&mWindows, hWin);
    }
};
...
private:
    ...
    NemuGuestWindowList mGuestWindows;
