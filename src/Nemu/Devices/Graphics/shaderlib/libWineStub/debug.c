...
typedef DECLCALLBACK(void) FNNEMUWINELOGBACKDOOR(char* pcszStr);
typedef FNNEMUWINELOGBACKDOOR *PFNNEMUWINELOGBACKDOOR;
static PFNNEMUWINELOGBACKDOOR nemu_log_backdoor = NULL;
...
static DECLCALLBACK(void) nemu_log_backdoor_rt(char* pcszStr)
{
    RTLogPrintf("%s", pcszStr);
}
#else
static DECLCALLBACK(void) nemu_log_backdoor_dispmp(char* pcszStr)
{
    NemuDispMpLoggerLog(pcszStr);
}
#endif
static void nemu_log_v(const char *pszFormat, va_list args)
{
    if (nemu_log_backdoor)
    {
...
static void debug_init(void)
{
    ...
    if ((wine_debug = getenv("WINEDEBUG")))
    {
        ...
        {
#ifdef NEMU_WITH_WDDM
            int rc = NemuDispMpLoggerInit();
            if (RT_SUCCESS(rc))
                nemu_log_backdoor = nemu_log_backdoor_dispmp;
//            else
#else
                nemu_log_backdoor = nemu_log_backdoor_rt;
#endif
        }
...
#if !defined(NEMU_WITH_VMSVGA) || defined(RT_OS_WINDOWS)
