/* $Id: NemuServiceToolBox.cpp $ */
/** @file
 * NemuServiceToolbox - Internal (BusyBox-like) toolbox.
 */
...
enum
{
    NEMUSERVICETOOLBOXOPT_MACHINE_READABLE = 1000,
    NEMUSERVICETOOLBOXOPT_VERBOSE
};

/** Options indices for "nemu_cat". */
typedef enum NEMUSERVICETOOLBOXCATOPT
{
    NEMUSERVICETOOLBOXCATOPT_NO_CONTENT_INDEXED = 1000
} NEMUSERVICETOOLBOXCATOPT;

/** Flags for "nemu_ls". */
typedef enum NEMUSERVICETOOLBOXLSFLAG
{
    NEMUSERVICETOOLBOXLSFLAG_NONE =             0x0,
    NEMUSERVICETOOLBOXLSFLAG_RECURSIVE =        0x1,
    NEMUSERVICETOOLBOXLSFLAG_SYMLINKS =         0x2
} NEMUSERVICETOOLBOXLSFLAG;

/** Flags for fs object output. */
typedef enum NEMUSERVICETOOLBOXOUTPUTFLAG
{
    NEMUSERVICETOOLBOXOUTPUTFLAG_NONE =         0x0,
    NEMUSERVICETOOLBOXOUTPUTFLAG_LONG =         0x1,
    NEMUSERVICETOOLBOXOUTPUTFLAG_PARSEABLE =    0x2
} NEMUSERVICETOOLBOXOUTPUTFLAG;
 ...
 */
typedef struct NEMUSERVICETOOLBOXPATHENTRY
{
    /** Our node. */
    RTLISTNODE  Node;
    /** Name of the entry. */
    char       *pszName;
} NEMUSERVICETOOLBOXPATHENTRY, *PNEMUSERVICETOOLBOXPATHENTRY;

typedef struct NEMUSERVICETOOLBOXDIRENTRY
{
    /** Our node. */
    RTLISTNODE   Node;
    /** The actual entry. */
    RTDIRENTRYEX dirEntry;
} NEMUSERVICETOOLBOXDIRENTRY, *PNEMUSERVICETOOLBOXDIRENTRY;
...
{
    RTPrintf(NEMU_PRODUCT " Guest Toolbox Version "
             NEMU_VERSION_STRING "\n"
             "(C) " NEMU_C_YEAR " " NEMU_VENDOR "\n"
             "All rights reserved.\n"
             "\n");
    RTPrintf("Usage:\n\n");
}
...
static void vgsvcToolboxShowUsage(void)
{
    ...
    RTPrintf("  NemuService [--use-toolbox] nemu_<command> [<general options>] <parameters>\n\n"
             "General options:\n\n"
...
{
    RTPrintf("%sr%d\n", NEMU_VERSION_STRING, RTBldCfgRevision());
}
...
static void vgsvcToolboxPathBufDestroy(PRTLISTNODE pList)
{
    ...
    PNEMUSERVICETOOLBOXPATHENTRY pNode = RTListGetFirst(pList, NEMUSERVICETOOLBOXPATHENTRY, Node);
    while (pNode)
    {
        PNEMUSERVICETOOLBOXPATHENTRY pNext = RTListNodeIsLast(pList, &pNode->Node)
                                           ? NULL
                                           : RTListNodeGetNext(&pNode->Node, NEMUSERVICETOOLBOXPATHENTRY, Node);
...
static int vgsvcToolboxPathBufAddPathEntry(PRTLISTNODE pList, const char *pszName)
{
    ...
    int rc = VINF_SUCCESS;
    PNEMUSERVICETOOLBOXPATHENTRY pNode = (PNEMUSERVICETOOLBOXPATHENTRY)RTMemAlloc(sizeof(NEMUSERVICETOOLBOXPATHENTRY));
    if (pNode)
    {
...
/**
 * Performs the actual output operation of "nemu_cat".
 *
 * @return  IPRT status code.
...
static char g_paszCatHelp[] =
    "  NemuService [--use-toolbox] nemu_cat [<general options>] <file>...\n\n"
    "Concatenate files, or standard input, to standard output.\n"
    "\n";


/**
 * Main function for tool "nemu_cat".
 *
 * @return  RTEXITCODE.
...
static RTEXITCODE vgsvcToolboxCat(int argc, char **argv)
{
    static const RTGETOPTDEF s_aOptions[] =
    {
        ...
        { "--no-content-indexed",  NEMUSERVICETOOLBOXCATOPT_NO_CONTENT_INDEXED,   RTGETOPT_REQ_NOTHING},
              ...
              && RT_SUCCESS(rc))
    {
        ...
        switch (ch)
        {
            ...
            case NEMUSERVICETOOLBOXCATOPT_NO_CONTENT_INDEXED:
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            PNEMUSERVICETOOLBOXPATHENTRY pNodeIt;
            RTFILE hInput = NIL_RTFILE;
            RTListForEach(&inputList, pNodeIt, NEMUSERVICETOOLBOXPATHENTRY, Node)
            {
...
static int vgsvcToolboxPrintFsInfo(const char *pszName, size_t cbName, uint32_t fOutputFlags, PRTFSOBJINFO pObjInfo)
{
    ...
    if (!(fOutputFlags & NEMUSERVICETOOLBOXOUTPUTFLAG_LONG))
    {
        if (fOutputFlags & NEMUSERVICETOOLBOXOUTPUTFLAG_PARSEABLE)
        {
        ...
        if (fOutputFlags & NEMUSERVICETOOLBOXOUTPUTFLAG_PARSEABLE) /* End of data block. */
            RTPrintf("%c%c", 0, 0);
    }
    else
    {
 ...
 * @param   fFlags                  Flags of type NEMUSERVICETOOLBOXLSFLAG.
 * @param   fOutputFlags            Flags of type  NEMUSERVICETOOLBOXOUTPUTFLAG.
 */
static int vgsvcToolboxLsHandleDir(const char *pszDir, uint32_t fFlags, uint32_t fOutputFlags)
{
    ...
    if (fFlags & NEMUSERVICETOOLBOXOUTPUTFLAG_PARSEABLE)
        RTPrintf("dname=%s%c", pszDir, 0);
    else if (fFlags & NEMUSERVICETOOLBOXLSFLAG_RECURSIVE)
        RTPrintf("%s:\n", pszDir);
    ...
    for (;RT_SUCCESS(rc);)
    {
        ...
        if (RT_SUCCESS(rc))
        {
            PNEMUSERVICETOOLBOXDIRENTRY pNode = (PNEMUSERVICETOOLBOXDIRENTRY)RTMemAlloc(sizeof(NEMUSERVICETOOLBOXDIRENTRY));
            if (pNode)
            {
    ...
    if (RT_SUCCESS(rc))
    {
        PNEMUSERVICETOOLBOXDIRENTRY pNodeIt;
        RTListForEach(&dirList, pNodeIt, NEMUSERVICETOOLBOXDIRENTRY, Node)
        {
        ...
        if (   RT_SUCCESS(rc)
            && (fFlags & NEMUSERVICETOOLBOXLSFLAG_RECURSIVE))
        {
            ...
            RTListForEach(&dirList, pNodeIt, NEMUSERVICETOOLBOXDIRENTRY, Node)
            {
                ...
                switch (fMode & RTFS_TYPE_MASK)
                {
                    case RTFS_TYPE_SYMLINK:
                        if (!(fFlags & NEMUSERVICETOOLBOXLSFLAG_SYMLINKS))
                            break;
    ...
    /* Clean up the mess. */
    PNEMUSERVICETOOLBOXDIRENTRY pNode, pSafe;
    RTListForEachSafe(&dirList, pNode, pSafe, NEMUSERVICETOOLBOXDIRENTRY, Node)
    {
        RTListNodeRemove(&pNode->Node);
        RTMemFree(pNode);
    }
...
static char g_paszLsHelp[] =
    "  NemuService [--use-toolbox] nemu_ls [<general options>] [option]...\n"
    "                                      [<file>...]\n\n"
...
/**
 * Main function for tool "nemu_ls".
 *
 * @return  RTEXITCODE.
 * @param   argc                    Number of arguments.
 * @param   argv                    Pointer to argument array.
...
static RTEXITCODE vgsvcToolboxLs(int argc, char **argv)
{
    static const RTGETOPTDEF s_aOptions[] =
    {
        { "--machinereadable", NEMUSERVICETOOLBOXOPT_MACHINE_READABLE,      RTGETOPT_REQ_NOTHING },
        { "--dereference",     'L',                                         RTGETOPT_REQ_NOTHING },
        { NULL,                'l',                                         RTGETOPT_REQ_NOTHING },
        { NULL,                'R',                                         RTGETOPT_REQ_NOTHING },
        { "--verbose",         NEMUSERVICETOOLBOXOPT_VERBOSE,               RTGETOPT_REQ_NOTHING}
    };
    ...
    bool     fVerbose     = false;
    uint32_t fFlags       = NEMUSERVICETOOLBOXLSFLAG_NONE;
    uint32_t fOutputFlags = NEMUSERVICETOOLBOXOUTPUTFLAG_NONE;
           ...
           && RT_SUCCESS(rc))
    {
        ...
        switch (ch)
        {
            ...
            case 'L': /* Dereference symlinks. */
                fFlags |= NEMUSERVICETOOLBOXLSFLAG_SYMLINKS;
                break;

            case 'l': /* Print long format. */
                fOutputFlags |= NEMUSERVICETOOLBOXOUTPUTFLAG_LONG;
                break;

            case NEMUSERVICETOOLBOXOPT_MACHINE_READABLE:
                fOutputFlags |= NEMUSERVICETOOLBOXOUTPUTFLAG_PARSEABLE;
                break;
            ...
            case NEMUSERVICETOOLBOXOPT_VERBOSE:
                fVerbose = true;
                break;
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        PNEMUSERVICETOOLBOXPATHENTRY pNodeIt;
        RTListForEach(&fileList, pNodeIt, NEMUSERVICETOOLBOXPATHENTRY, Node)
        {
        ...
        if (fOutputFlags & NEMUSERVICETOOLBOXOUTPUTFLAG_PARSEABLE) /* Output termination. */
...
static char g_paszMkTempHelp[] =
    "  NemuService [--use-toolbox] nemu_mktemp [<general options>] [<options>]\n"
    "                                          <template>\n\n"
...
/**
 * Report the result of a nemu_mktemp operation.
 *
...
/**
 * Main function for tool "nemu_mktemp".
 *
 * @return  RTEXITCODE.
 * @param   argc                    Number of arguments.
 * @param   argv                    Pointer to argument array.
...
static RTEXITCODE vgsvcToolboxMkTemp(int argc, char **argv)
{
    static const RTGETOPTDEF s_aOptions[] =
    {
        { "--machinereadable", NEMUSERVICETOOLBOXOPT_MACHINE_READABLE,
              ...
              && RT_SUCCESS(rc))
    {
        ...
        switch (ch)
        {
            ...
            case 'd':
                fFlags |= NEMUSERVICETOOLBOXMKTEMPFLAG_DIRECTORY;
                break;
            ...
            case 'm':
                ...
                break;
            case 's':
                fFlags |= NEMUSERVICETOOLBOXMKTEMPFLAG_SECURE;
                break;
    ...
    if (fFlags & NEMUSERVICETOOLBOXMKTEMPFLAG_SECURE && fModeSet)
    {
    ...
    if (fFlags & NEMUSERVICETOOLBOXMKTEMPFLAG_DIRECTORY)
    {
        rc = fFlags & NEMUSERVICETOOLBOXMKTEMPFLAG_SECURE
...
static char g_paszMkDirHelp[] =
    "  NemuService [--use-toolbox] nemu_mkdir [<general options>] [<options>]\n"
    "                                         <directory>...\n\n"
    "Options:\n\n"
...
/**
 * Main function for tool "nemu_mkdir".
 *
 * @return  RTEXITCODE.
 * @param   argc                    Number of arguments.
 * @param   argv                    Pointer to argument array.
...
static char g_paszStatHelp[] =
    "  NemuService [--use-toolbox] nemu_stat [<general options>] [<options>]\n"
    "                                        <file>...\n\n"
...
/**
 * Main function for tool "nemu_stat".
 *
 * @return  RTEXITCODE.
 * @param   argc                    Number of arguments.
 * @param   argv                    Pointer to argument array.
...
static RTEXITCODE vgsvcToolboxStat(int argc, char **argv)
{
    ...
    int rc = VINF_SUCCESS;
    bool fVerbose = false;
    uint32_t fOutputFlags = NEMUSERVICETOOLBOXOUTPUTFLAG_LONG; /* Use long mode by default. */
...
bool VGSvcToolboxMain(int argc, char **argv, RTEXITCODE *prcExit)
{
    ...
    if (!pfnHandler)
    {
         ...
         * when the first NemuService argument is --use-toolbox.
         */
     ...
     * Invoke the handler.
     */
    RTMsgSetProgName("NemuService/%s", pszTool);
