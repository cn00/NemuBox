...
int GuestFsObjData::FromStat(const GuestProcessStreamBlock &strmBlk)
{
    ...
    try
    {
         ...
         * in older NemuService (< 4.2) versions. */
...
int GuestBase::dispatchGeneric(PNEMUGUESTCTRLHOSTCBCTX pCtxCb, PNEMUGUESTCTRLHOSTCALLBACK pSvcCb)
{
...
int GuestBase::generateContextID(uint32_t uSessionID, uint32_t uObjectID, uint32_t *puContextID)
{
    ...
    if (   uSessionID >= NEMU_GUESTCTRL_MAX_SESSIONS
        || uObjectID  >= NEMU_GUESTCTRL_MAX_OBJECTS)
        return VERR_INVALID_PARAMETER;

    uint32_t uCount = ASMAtomicIncU32(&mNextContextID);
    if (uCount == NEMU_GUESTCTRL_MAX_CONTEXTS)
        uCount = 0;

    uint32_t uNewContextID =
        NEMU_GUESTCTRL_CONTEXTID_MAKE(uSessionID, uObjectID, uCount);
...
int GuestBase::signalWaitEvent(NemuEventType_T aType, IEvent *aEvent)
{
    int rc = RTCritSectEnter(&mWaitEventCritSect);
#ifdef DEBUG
    uint32_t cEvents = 0;
        ...
        if (itGroup != mWaitEventGroups.end())
        {
            ...
            while (itEvents != itGroup->second.end())
            {
                ...
                LogFlowThisFunc(("Signalling event=%p, type=%ld (CID %RU32: Session=%RU32, Object=%RU32, Count=%RU32) ...\n",
                                 ...
                                 NEMU_GUESTCTRL_CONTEXTID_GET_SESSION(itEvents->first),
                                 NEMU_GUESTCTRL_CONTEXTID_GET_OBJECT(itEvents->first),
                                 NEMU_GUESTCTRL_CONTEXTID_GET_COUNT(itEvents->first)));
#endif
...
int GuestBase::signalWaitEventInternal(PNEMUGUESTCTRLHOSTCBCTX pCbCtx,
...
int GuestBase::signalWaitEventInternalEx(PNEMUGUESTCTRLHOSTCBCTX pCbCtx,
                                         int rc, int guestRc,
...
int GuestBase::waitForEvent(GuestWaitEvent *pEvent, uint32_t uTimeoutMS,
                            NemuEventType_T *pType, IEvent **ppEvent)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        if (!pThisEvent.isNull()) /* Having a NemuEventType_ event is optional. */
        {
            if (pType)
            {
