/* $Id: NemuGuestR3LibLog.cpp $ */
/** @file
 * NemuGuestR3Lib - Ring-3 Support Library for VirtualBox guest additions, Logging.
 */
...
VBGLR3DECL(int) VbglR3WriteLog(const char *pch, size_t cch)
{
    ...
    int rc = vbglR3DoIOCtl(NEMUGUEST_IOCTL_LOG(cch), pvTmp, cch);
    RTMemFree(pvTmp);
    return rc;
     ...
     */
    return vbglR3DoIOCtl(NEMUGUEST_IOCTL_LOG(cch), pvTmp, cch);
...
# define STEP 2048
    ...
    for (size_t off = 0; off < cch && RT_SUCCESS(rc); off += STEP)
    {
        ...
        rc = vbglR3DoIOCtl(NEMUGUEST_IOCTL_LOG(cbStep), (char *)pch + off, cbStep);
    }
# undef STEP
    return rc;
#endif
