/* $Id: HGCM.cpp $ */
/** @file
 * NemuGuestLib - Host-Guest Communication Manager.
 *
 * These public functions can be only used by other drivers. They all
 * do an IOCTL to NemuGuest via IDC.
 */
...
DECLVBGL(int) VbglHGCMConnect(VBGLHGCMHANDLE *pHandle, NemuGuestHGCMConnectInfo *pData)
{
    ...
    if (pHandle && pData)
    {
        ...
        if (pHandleData)
        {
            ...
            if (RT_SUCCESS(rc))
            {
                rc = vbglDriverIOCtl(&pHandleData->driver, NEMUGUEST_IOCTL_HGCM_CONNECT, pData, sizeof(*pData));
                if (RT_SUCCESS(rc))
                    rc = pData->result;
                if (RT_SUCCESS(rc))
                {
...
{
    int rc = vbglDriverIOCtl(&handle->driver, NEMUGUEST_IOCTL_HGCM_DISCONNECT, pData, sizeof(*pData));
...
{
    VBGL_HGCM_ASSERT_MSG(cbData >= sizeof(NemuGuestHGCMCallInfo) + pData->cParms * sizeof(HGCMFunctionParameter),
                         ("cbData = %d, cParms = %d (calculated size %d)\n", cbData, pData->cParms,
                          sizeof(NemuGuestHGCMCallInfo) + pData->cParms * sizeof(NemuGuestHGCMCallInfo)));

    return vbglDriverIOCtl(&handle->driver, NEMUGUEST_IOCTL_HGCM_CALL(cbData), pData, cbData);
}
...
DECLVBGL(int) VbglHGCMCallUserData (VBGLHGCMHANDLE handle, NemuGuestHGCMCallInfo *pData, uint32_t cbData)
{
    ...
    return vbglDriverIOCtl(&handle->driver, NEMUGUEST_IOCTL_HGCM_CALL_USERDATA(cbData), pData, cbData);
}


DECLVBGL(int) VbglHGCMCallTimed(VBGLHGCMHANDLE handle, NemuGuestHGCMCallInfoTimed *pData, uint32_t cbData)
{
    uint32_t cbExpected = sizeof(NemuGuestHGCMCallInfoTimed)
                        + pData->info.cParms * sizeof(HGCMFunctionParameter);
    VBGL_HGCM_ASSERT_MSG(cbData >= cbExpected,
                         ("cbData = %d, cParms = %d (calculated size %d)\n", cbData, pData->info.cParms, cbExpected));
    NOREF(cbExpected);

    return vbglDriverIOCtl(&handle->driver, NEMUGUEST_IOCTL_HGCM_CALL_TIMED(cbData), pData, cbData);
}
