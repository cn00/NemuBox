...
/** @file
 * Nemu Qt GUI - UIFrameBuffer class implementation.
 */
...
class ATL_NO_VTABLE UIFrameBufferPrivate : public QObject,
                                           ...
                                           NEMU_SCRIPTABLE_IMPL(IFramebuffer)
{
    Q_OBJECT;
...
public:
    ...
    /** EMT callback: Notifies frame-buffer about 3D backend event.
      * @param        uType Event type. Currently only NEMU3D_NOTIFY_EVENT_TYPE_VISIBLE_3DDATA is supported.
...
class NemuOverlayFrameBuffer : public UIFrameBufferPrivate
{
    Q_OBJECT;

public:

    NemuOverlayFrameBuffer()
    {
    }
    ...
    virtual HRESULT init(UIMachineView *pView)
    {
        ...
        mOverlay.onResizeEventPostprocess (NemuFBSizeInfo(this), QPoint(mpView->contentsX(), mpView->contentsY()));
        return S_OK;
    }
    ...
    STDMETHOD(ProcessVHWACommand)(BYTE *pCommand)
    {
        ...
        rc = mOverlay.onVHWACommand ((struct NEMUVHWACMD*)pCommand);
        UIFrameBufferPrivate::unlock();
    ...
    void performResize(int iWidth, int iHeight)
    {
        ...
        mOverlay.onResizeEventPostprocess(NemuFBSizeInfo(this),
...
STDMETHODIMP UIFrameBufferPrivate::Notify3DEvent(ULONG uType, ComSafeArrayIn(BYTE, data))
{
    ...
    switch (uType)
    {
        case NEMU3D_NOTIFY_EVENT_TYPE_3DDATA_VISIBLE:
        case NEMU3D_NOTIFY_EVENT_TYPE_3DDATA_HIDDEN:
        {
            /* Notify machine-view with the async-signal
             * about 3D overlay visibility change: */
            BOOL fVisible = uType == NEMU3D_NOTIFY_EVENT_TYPE_3DDATA_VISIBLE;
            LogRel2(("GUI: UIFrameBufferPrivate::Notify3DEvent: Sending to async-handler: "
                     "(NEMU3D_NOTIFY_EVENT_TYPE_3DDATA_%s)\n",
        ...
        case NEMU3D_NOTIFY_EVENT_TYPE_TEST_FUNCTIONAL:
        {
...
void UIFrameBufferPrivate::handleNotifyChange(int iWidth, int iHeight)
{
    ...
    if (!nemuGlobal().isSeparateProcess() && !m_fPendingSourceBitmap)
    {
...
#if defined(NEMU_WITH_TRANSLUCENT_SEAMLESS)
...
UIFrameBuffer::UIFrameBuffer(bool m_fAccelerate2DVideo)
{
    ...
    {
        ComObjPtr<NemuOverlayFrameBuffer> pFrameBuffer;
