...
/** @file
 * Nemu Qt GUI - UIGlobalSettingsUpdate class implementation.
 */
...
void UIGlobalSettingsUpdate::loadToCacheFrom(QVariant &data)
{
    ...
    NemuUpdateData updateData(gEDataManager->applicationUpdateData());
...
void UIGlobalSettingsUpdate::getFromCache()
{
    ...
    if (m_pCheckBoxUpdate->isChecked())
    {
        ...
        if (m_cache.m_branchIndex == NemuUpdateData::BranchWithBetas)
            m_pRadioUpdateFilterBetas->setChecked(true);
        else if (m_cache.m_branchIndex == NemuUpdateData::BranchAllRelease)
...
void UIGlobalSettingsUpdate::saveFromCacheTo(QVariant &data)
{
    ...
    NemuUpdateData newData(m_cache.m_periodIndex, m_cache.m_branchIndex);
...
void UIGlobalSettingsUpdate::retranslateUi()
{
    ...
    NemuUpdateData::populate();
    m_pComboBoxUpdatePeriod->insertItems(0, NemuUpdateData::list());
...
{
    NemuUpdateData data(periodType(), branchType());
...
NemuUpdateData::PeriodType UIGlobalSettingsUpdate::periodType() const
{
    NemuUpdateData::PeriodType result = m_pCheckBoxUpdate->isChecked() ?
        (NemuUpdateData::PeriodType)m_pComboBoxUpdatePeriod->currentIndex() : NemuUpdateData::PeriodNever;
    return result == NemuUpdateData::PeriodUndefined ? NemuUpdateData::Period1Day : result;
}

NemuUpdateData::BranchType UIGlobalSettingsUpdate::branchType() const
{
    if (m_pRadioUpdateFilterBetas->isChecked())
        return NemuUpdateData::BranchWithBetas;
    else if (m_pRadioUpdateFilterEvery->isChecked())
        return NemuUpdateData::BranchAllRelease;
    else
        return NemuUpdateData::BranchStable;
}
