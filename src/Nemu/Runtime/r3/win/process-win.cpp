                                  ...
                                  uint32_t fFlags, const char *pszExec)
{
     ...
     * - If valid get the correct Explorer/NemuTray instance corresponding to that
     *   user. This of course is only possible if that user is logged in (over
     *   physical console or terminal services).
     * - If we found the user's Explorer/NemuTray app, use and modify the token to
     *   use it in order to allow the newly started process to access the user's
     *   desktop. If there's no Explorer/NemuTray app we cannot display the started
    ...
    if (RT_SUCCESS(rc))
    {
            ...
            if (pSid)
            {
                ...
                if (pwszDomain)
                {
                        ...
                        && IsValidSid(pSid))
                    {
                        ...
                        {
#ifdef NEMU                 /* The explorer entry is a fallback in case GA aren't installed. */
                            { "NemuTray.exe" },
#endif
                            { "explorer.exe" },
                            NULL
        ...
        {
            /*
             * If we didn't find a matching NemuTray, just use the token we got
