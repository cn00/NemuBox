...
/** @file
 * Nemu Qt GUI - UIModalWindowManager class implementation.
 */
...
{
    /* It may happen that this method is called before NemuGlobal initialization
     * or after initialization had failed (for example, to show some message).
     * Return NULL pointer in such cases: */
    if (!NemuGlobal::instance() || !nemuGlobal().isValid())
        return 0;
    ...
    if (nemuGlobal().isVMConsoleProcess())
    {
        ...
        if (nemuGlobal().activeMachineWindow() &&
            nemuGlobal().activeMachineWindow()->isVisible())
            return nemuGlobal().activeMachineWindow();
    }
