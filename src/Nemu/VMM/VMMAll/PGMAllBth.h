/* $Id: PGMAllBth.h $ */
/** @file
 * Nemu - Page Manager, Shadow+Guest Paging Template - All context code.
 *
 ...
 */
PGM_BTH_DECL(NEMUSTRICTRC, Trap0eHandlerGuestFault)(PVMCPU pVCpu, PGSTPTWALK pGstWalk, RTGCUINT uErr)
{
 ...
 *                          when we're called.
 */
static NEMUSTRICTRC PGM_BTH_NAME(Trap0eHandlerDoAccessHandlers)(PVMCPU pVCpu, RTGCUINT uErr, PCPUMCTXCORE pRegFrame,
...
#  if PGM_WITH_PAGING(PGM_GST_TYPE, PGM_SHW_TYPE)
    ...
    if (RT_FAILURE_NP(rc))
        return NEMUSTRICTRC_TODO(PGM_BTH_NAME(Trap0eHandlerGuestFault)(pVCpu, &GstWalk, uErr));
...
#   if PGM_WITH_PAGING(PGM_GST_TYPE, PGM_SHW_TYPE)
        ...
        if (RT_SUCCESS(rc) && PGM_PAGE_HAS_ACTIVE_ALL_HANDLERS(pPage))
            return NEMUSTRICTRC_TODO(PGM_BTH_NAME(Trap0eHandlerDoAccessHandlers)(pVCpu, uErr, pRegFrame, pvFault, pPage,
                                                                                 pfLockTaken, &GstWalk));
...
# if PGM_WITH_PAGING(PGM_GST_TYPE, PGM_SHW_TYPE)
        return NEMUSTRICTRC_TODO(PGM_BTH_NAME(Trap0eHandlerDoAccessHandlers)(pVCpu, uErr, pRegFrame, pvFault, pPage, pfLockTaken,
                                                                             &GstWalk));
# else
        return NEMUSTRICTRC_TODO(PGM_BTH_NAME(Trap0eHandlerDoAccessHandlers)(pVCpu, uErr, pRegFrame, pvFault, pPage, pfLockTaken));
# endif
...
#  endif /* PGM_WITH_PAGING(PGM_GST_TYPE, PGM_SHW_TYPE) */
    ...
    if (!(uErr & X86_TRAP_PF_P))
    {
        ...
        if (PGM_PAGE_IS_BALLOONED(pPage))
        {
            ...
            rc = NEMUSTRICTRC_TODO(PGMInterpretInstruction(pVM, pVCpu, pRegFrame, pvFault));
...
#   if PGM_WITH_PAGING(PGM_GST_TYPE, PGM_SHW_TYPE) && defined(NEMU_STRICT)
        /*
...
#   endif /* PGM_WITH_PAGING(PGM_GST_TYPE, PGM_SHW_TYPE) && NEMU_STRICT */
    }
...
#ifndef NEMU_WITH_NEW_LAZY_PAGE_ALLOC
...
# ifdef NEMU_WITH_REAL_WRITE_MONITORED_PAGES
 ...
 *
 * @returns Nemu status code (informational included).
 * @param   pVCpu       The cross context virtual CPU structure.
 ...
 *
 * @returns Nemu status code, no specials.
 * @param   pVCpu       The cross context virtual CPU structure.
...
#  ifndef NEMU_WITH_2X_4GB_ADDR_SPACE
 ...
 *
 * @returns Nemu status, no specials.
 * @param   pVCpu       The cross context virtual CPU structure.
 */
PGM_BTH_DECL(int, UnmapCR3)(PVMCPU pVCpu)
{
