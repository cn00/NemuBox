...
int hlpReportStatus(NemuGuestFacilityStatus statusCurrent)
{
    int rc = VbglR3ReportAdditionsStatus(NemuGuestFacilityType_NemuTrayClient,
                                         statusCurrent,
                                         0 /* Flags */);
    if (RT_FAILURE(rc))
        Log(("NemuTray: Could not report NemuTray status \"%ld\", rc=%Rrc\n", statusCurrent, rc));
    return rc;
}
