 ...
 * Generator: src/Nemu/Main/webservice/websrv-cpp.xsl
 */
 ...
 ****************************************************************************/
  ...
  <xsl:for-each select="//enum">
    ...
    <xsl:call-template name="emitNewline" />
    <xsl:value-of select="concat('nemu__', $enumname, ' ', $G_funcPrefixOutputEnumConverter, $enumname, '(', $enumname, '_T e)')" />
    <xsl:call-template name="emitNewline" />
    <xsl:text>{</xsl:text>
    <xsl:call-template name="emitNewline" />
    <xsl:value-of select="concat('    nemu__', $enumname, ' v;')" />
    <xsl:call-template name="emitNewline" />
    <xsl:call-template name="emitNewline" />
    ...
    <xsl:for-each select="const[not(@wsmap='suppress')]">
      ...
      <xsl:value-of select="concat('    v = nemu__', $enumname, '__')" />
    ...
    <xsl:call-template name="emitNewlineIndent8" />
    <xsl:value-of select="concat('    v = (nemu__', $enumname, ')0x7fffdead;')" />
    <xsl:call-template name="emitNewlineIndent8" />
    <xsl:text>break; </xsl:text>
    <xsl:call-template name="emitNewline" />
    <xsl:text>    }</xsl:text>
    ...
    <xsl:call-template name="emitNewline" />
    <xsl:value-of select="concat($enumname, '_T ', $G_funcPrefixInputEnumConverter, $enumname, '(nemu__', $enumname, ' v)')" />
    <xsl:call-template name="emitNewline" />
    <xsl:text>{</xsl:text>
    <xsl:call-template name="emitNewline" />
    ...
    <xsl:for-each select="const[not(@wsmap='suppress')]">
      <xsl:variable name="enumconst" select="@name" />
      <xsl:value-of select="concat('        case nemu__', $enumname, '__')" />
      <!-- escape all "_" in $enumconst -->
      <xsl:call-template name="escapeUnderscores">
        <xsl:with-param name="string" select="$enumconst" />
      </xsl:call-template>
 ...
 ****************************************************************************/
  ...
  <xsl:for-each select="//interface[@wsmap='struct']">
    ...
    <xsl:call-template name="emitNewline" />
    <xsl:value-of select="concat('nemu__', $structname, '* ', $G_funcPrefixOutputStructConverter, $structname, '(')" />
    <xsl:call-template name="emitNewline" />
    ...
    <xsl:value-of select="concat('    nemu__', $structname, ' *resp = NULL;')" />
    <xsl:call-template name="emitNewline" />

    <xsl:call-template name="emitPrologue"><xsl:with-param name="fSkipHRESULT" select="'1'"/></xsl:call-template>

    <xsl:value-of select="concat('        resp = soap_new_nemu__', $structname, '(soap, -1);')" />
    <xsl:call-template name="emitNewline" />
    <xsl:text>        if (!in)&#10;</xsl:text>
    <xsl:text>        {&#10;</xsl:text>
...
<xsl:template name="emitBeginOfFunctionHeader">
  ...
  <xsl:call-template name="emitNewline" />
  <xsl:value-of select="concat('int __nemu__', $ifname, '_USCORE', $method, '(')" />
  <xsl:call-template name="emitNewline" />
...
<xsl:template name="emitDocumentStyleArgStructs">
  ...
  <xsl:text>,</xsl:text>
  <xsl:call-template name="emitNewline" />
  <xsl:value-of select="concat('    _nemu__', $ifname, '_USCORE', $methodname, $G_requestMessageElementSuffix, ' *', $G_requestElementVarName)" />
  <xsl:if test="$fOutputs">
    <xsl:text>,</xsl:text>
    <xsl:call-template name="emitNewline" />
    <xsl:value-of select="concat('    _nemu__', $ifname, '_USCORE', $methodname, $G_responseMessageElementSuffix, ' *', $G_responseElementVarName)" />
...
<xsl:template name="emitObjForMethod">
  ...
  <xsl:param name="structprefix" />   <!-- with WSDL document style: req element prefix, like "nemu__IVirtualBox_USCOREcreateMachineRequestElement->" -->
