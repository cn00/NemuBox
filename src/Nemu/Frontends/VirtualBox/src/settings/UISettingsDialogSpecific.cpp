...
/** @file
 * Nemu Qt GUI - UISettingsDialogSpecific class implementation.
 */
...
#  define NEMU_GUI_WITH_TOOLBAR_SETTINGS
# endif
#endif
...
void UISettingsDialogGlobal::loadOwnData()
{
    ...
    CSystemProperties properties = nemuGlobal().virtualBox().GetSystemProperties();
    NemuGlobalSettings settings = nemuGlobal().settings();
    /* Prepare global data: */
...
void UISettingsDialogGlobal::saveOwnData()
{
    ...
    NemuGlobalSettings newSettings = varData.value<UISettingsDataGlobal>().m_settings;
    ...
    /* Else save the new settings if they were changed: */
        ...
        nemuGlobal().setSettings(newSettings);
...
{
#ifdef NEMU_GUI_WITH_TOOLBAR_SETTINGS
    return m_pSelector->itemText(m_pSelector->currentId());
#else /* NEMU_GUI_WITH_TOOLBAR_SETTINGS */
    return tr("Preferences");
#endif /* !NEMU_GUI_WITH_TOOLBAR_SETTINGS */
}
    ...
    , m_fResetFirstRunFlag(false)
{
    ...
    connect(&nemuGlobal(), SIGNAL(sigMediumEnumerationFinished()), this, SLOT(sltAllowResetFirstRunFlag()));
    ...
    m_machine = nemuGlobal().virtualBox().FindMachine(m_strMachineId);
...
void UISettingsDialogMachine::loadOwnData()
{
    ...
    m_session = configurationAccessLevel() == ConfigurationAccessLevel_Null ? CSession() :
                configurationAccessLevel() == ConfigurationAccessLevel_Full ? nemuGlobal().openSession(m_strMachineId) :
                                                                              nemuGlobal().openExistingSession(m_strMachineId);
...
void UISettingsDialogMachine::retranslateUi()
{
     ...
     * nemuGlobal which is retranslated at that point already: */
...
QString UISettingsDialogMachine::title() const
{
    ...
    const CMachine &machine = nemuGlobal().virtualBox().FindMachine(m_strMachineId);
    if (!machine.isNull())
