/* $Rev: 103607 $ */
/** @file
 * NemuDrv - The VirtualBox Support Driver - Linux specifics.
 */
...
# define NEMU_WITH_SUSPEND_NOTIFICATION
#endif
...
#endif
#ifdef CONFIG_NEMUDRV_AS_MISC
# include <linux/miscdevice.h>
#endif
#ifdef NEMU_WITH_SUSPEND_NOTIFICATION
...
/* devfs defines */
#if defined(CONFIG_DEVFS_FS) && !defined(CONFIG_NEMUDRV_AS_MISC)
# ifdef NEMU_WITH_HARDENING
#  define NEMU_DEV_FMASK     (S_IWUSR | S_IRUSR)
# else
#  define NEMU_DEV_FMASK     (S_IRUGO | S_IWUGO)
# endif
#endif /* CONFIG_DEV_FS && !CONFIG_NEMUDEV_AS_MISC */

#ifdef CONFIG_X86_HIGH_ENTRY
# error "CONFIG_X86_HIGH_ENTRY is not supported by NemuDrv at this time."
#endif
...
#define NemuDrvLinuxVersion RT_CONCAT3(RT_CONCAT(NEMU_VERSION_MAJOR, _), \
                                       RT_CONCAT(NEMU_VERSION_MINOR, _), \
                                       NEMU_VERSION_BUILD)
#define NemuDrvLinuxIOCtl RT_CONCAT(NemuDrvLinuxIOCtl_,NemuDrvLinuxVersion)
...
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static int  NemuDrvLinuxInit(void);
static void NemuDrvLinuxUnload(void);
static int  NemuDrvLinuxCreateSys(struct inode *pInode, struct file *pFilp);
static int  NemuDrvLinuxCreateUsr(struct inode *pInode, struct file *pFilp);
static int  NemuDrvLinuxClose(struct inode *pInode, struct file *pFilp);
#ifdef HAVE_UNLOCKED_IOCTL
static long NemuDrvLinuxIOCtl(struct file *pFilp, unsigned int uCmd, unsigned long ulArg);
#else
static int  NemuDrvLinuxIOCtl(struct inode *pInode, struct file *pFilp, unsigned int uCmd, unsigned long ulArg);
#endif
static int  NemuDrvLinuxIOCtlSlow(struct file *pFilp, unsigned int uCmd, unsigned long ulArg, PSUPDRVSESSION pSession);
static int  NemuDrvLinuxErr2LinuxErr(int);
#ifdef NEMU_WITH_SUSPEND_NOTIFICATION
static int  NemuDrvProbe(struct platform_device *pDev);
# if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 30)
static int  NemuDrvSuspend(struct device *pDev);
static int  NemuDrvResume(struct device *pDev);
# else
static int  NemuDrvSuspend(struct platform_device *pDev, pm_message_t State);
static int  NemuDrvResume(struct platform_device *pDev);
# endif
static void NemuDevRelease(struct device *pDev);
#endif
...
#ifndef CONFIG_NEMUDRV_AS_MISC
/** Module major number for nemudrv. */
#define DEVICE_MAJOR_SYS    234
/** Saved major device number for nemudrv. */
static int                  g_iModuleMajorSys;
/** Module major number for nemudrvu. */
#define DEVICE_MAJOR_USR    235
/** Saved major device number for nemudrvu. */
static int                  g_iModuleMajorUsr;
#endif /* !CONFIG_NEMUDRV_AS_MISC */
...
#define DEVICE_NAME_SYS     "nemudrv"
/** The user device name. */
#define DEVICE_NAME_USR     "nemudrvu"

#if (defined(RT_ARCH_AMD64) && LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 23)) || defined(NEMU_WITH_TEXT_MODMEM_HACK)
/**
...
# ifndef NEMU_WITH_TEXT_MODMEM_HACK
...
static struct file_operations gFileOpsNemuDrvSys =
{
    owner:      THIS_MODULE,
    open:       NemuDrvLinuxCreateSys,
    release:    NemuDrvLinuxClose,
#ifdef HAVE_UNLOCKED_IOCTL
    unlocked_ioctl: NemuDrvLinuxIOCtl,
#else
    ioctl:      NemuDrvLinuxIOCtl,
#endif
};

/** The file_operations structure. */
static struct file_operations gFileOpsNemuDrvUsr =
{
    owner:      THIS_MODULE,
    open:       NemuDrvLinuxCreateUsr,
    release:    NemuDrvLinuxClose,
#ifdef HAVE_UNLOCKED_IOCTL
    unlocked_ioctl: NemuDrvLinuxIOCtl,
#else
...
{
    .suspend = NemuDrvSuspend,  /* before entering deep sleep */
    .resume  = NemuDrvResume,   /* after wakeup from deep sleep */
    .freeze  = NemuDrvSuspend,  /* before creating hibernation image */
    .restore = NemuDrvResume,   /* after waking up from hibernation */
};
# endif
...
static struct platform_driver gPlatformDriver =
{
    .probe = NemuDrvProbe,
# if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 30)
    .suspend = NemuDrvSuspend,
    .resume  = NemuDrvResume,
# endif
    /** @todo .shutdown? */
    .driver =
    {
...
static struct platform_device gPlatformDevice =
{
    ...
    .dev =
    {
        .release = NemuDevRelease
    }
};
#endif /* NEMU_WITH_SUSPEND_NOTIFICATION */


DECLINLINE(RTUID) nemudrvLinuxUid(void)
{
...
DECLINLINE(RTGID) nemudrvLinuxGid(void)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 29)
# if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 5, 0)
...
DECLINLINE(RTUID) nemudrvLinuxEuid(void)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 29)
# if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 5, 0)
 ...
 */
static int __init NemuDrvLinuxInit(void)
{
    int       rc;
     ...
     */
    printk(KERN_DEBUG "nemudrv: Found %u processor cores\n", (unsigned)RTMpGetOnlineCount());
#ifdef CONFIG_NEMUDRV_AS_MISC
    rc = misc_register(&gMiscDeviceSys);
    if (rc)
    {
        printk(KERN_ERR "nemudrv: Can't register system misc device! rc=%d\n", rc);
        return rc;
    }
    rc = misc_register(&gMiscDeviceUsr);
    if (rc)
    {
        printk(KERN_ERR "nemudrv: Can't register user misc device! rc=%d\n", rc);
        misc_deregister(&gMiscDeviceSys);
        return rc;
    }
#else  /* !CONFIG_NEMUDRV_AS_MISC */
    /*
     * Register character devices and save the returned major numbers.
     */
    /* /dev/nemudrv */
    g_iModuleMajorSys = DEVICE_MAJOR_SYS;
    rc = register_chrdev((dev_t)g_iModuleMajorSys, DEVICE_NAME_SYS, &gFileOpsNemuDrvSys);
    if (rc < 0)
    {
        Log(("register_chrdev() failed with rc=%#x for nemudrv!\n", rc));
        return rc;
    }
    ...
    rc = register_chrdev((dev_t)g_iModuleMajorUsr, DEVICE_NAME_USR, &gFileOpsNemuDrvUsr);
    if (rc < 0)
    {
        Log(("register_chrdev() failed with rc=%#x for nemudrv!\n", rc));
        return rc;
...
# ifdef CONFIG_DEVFS_FS
     ...
     */
    if (   devfs_mk_cdev(MKDEV(DEVICE_MAJOR_SYS, 0), S_IFCHR | NEMU_DEV_FMASK, DEVICE_NAME_SYS) != 0
        || devfs_mk_cdev(MKDEV(DEVICE_MAJOR_USR, 0), S_IFCHR | NEMU_DEV_FMASK, DEVICE_NAME_USR) != 0)
    {
...
# endif
    ...
    if (!rc)
    {
        ...
        {
#if (defined(RT_ARCH_AMD64) && LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 23)) || defined(NEMU_WITH_TEXT_MODMEM_HACK)
# ifdef NEMU_WITH_TEXT_MODMEM_HACK
...
# endif
            ...
            printk(KERN_DEBUG "NemuDrv: dbg - g_abExecMemory=%p\n", (void *)&g_abExecMemory[0]);
#endif
            Log(("NemuDrv::ModuleInit\n"));
...
#endif
                    {
                        printk(KERN_INFO "nemudrv: TSC mode is %s, tentative frequency %llu Hz\n",
                               SUPGetGIPModeName(g_DevExt.pGip), g_DevExt.pGip->u64CpuHz);
                        LogFlow(("NemuDrv::ModuleInit returning %#x\n", rc));
                        printk(KERN_DEBUG "nemudrv: Successfully loaded version "
                                NEMU_VERSION_STRING " (interface " RT_XSTR(SUPDRV_IOC_VERSION) ")\n");
                        return rc;
                    }
#ifdef NEMU_WITH_SUSPEND_NOTIFICATION
                    else
 ...
 * Unload the module.
 */
static void __exit NemuDrvLinuxUnload(void)
{
    Log(("NemuDrvLinuxUnload\n"));
 ...
 */
static int nemudrvLinuxCreateCommon(struct inode *pInode, struct file *pFilp, bool fUnrestricted)
{
    int                 rc;
    PSUPDRVSESSION      pSession;
    Log(("NemuDrvLinuxCreate: pFilp=%p pid=%d/%d %s\n", pFilp, RTProcSelf(), current->pid, current->comm));
     ...
     */
    if (   fUnrestricted
        && nemudrvLinuxEuid() != 0 /* root */ )
    {
        Log(("NemuDrvLinuxCreate: euid=%d, expected 0 (root)\n", nemudrvLinuxEuid()));
        return -EPERM;
    }
    ...
    if (!rc)
    {
        pSession->Uid = nemudrvLinuxUid();
        pSession->Gid = nemudrvLinuxGid();
    }

    pFilp->private_data = pSession;

    Log(("NemuDrvLinuxCreate: g_DevExt=%p pSession=%p rc=%d/%d (pid=%d/%d %s)\n",
         &g_DevExt, pSession, rc, NemuDrvLinuxErr2LinuxErr(rc),
         RTProcSelf(), current->pid, current->comm));
    return NemuDrvLinuxErr2LinuxErr(rc);
}
...
static int NemuDrvLinuxCreateSys(struct inode *pInode, struct file *pFilp)
{
    return nemudrvLinuxCreateCommon(pInode, pFilp, true);
}
...
static int NemuDrvLinuxCreateUsr(struct inode *pInode, struct file *pFilp)
{
    return nemudrvLinuxCreateCommon(pInode, pFilp, false);
}
...
static int NemuDrvLinuxClose(struct inode *pInode, struct file *pFilp)
{
    Log(("NemuDrvLinuxClose: pFilp=%p pSession=%p pid=%d/%d %s\n",
...
#endif
{
    ...
    int rc;
#if defined(NEMU_STRICT) || defined(NEMU_WITH_EFLAGS_AC_SET_IN_NEMUDRV)
    ...
    {
        SUPR0Printf("NemuDrvLinuxIOCtl: EFLAGS.AC=0 detected %u times, refusing all I/O controls!\n", g_DevExt.cBadContextCalls);
        return ESPIPE;
    }
...
# endif
     ...
     * the session and iCmd, and only returns a Nemu status code.
     */
#ifdef HAVE_UNLOCKED_IOCTL
    if (RT_LIKELY(   (   uCmd == SUP_IOCTL_FAST_DO_RAW_RUN
        ...
        rc = NemuDrvLinuxIOCtlSlow(pFilp, uCmd, ulArg, pSession);
...
static int NemuDrvLinuxIOCtlSlow(struct file *pFilp, unsigned int uCmd, unsigned long ulArg, PSUPDRVSESSION pSession)
{
    ...
    Log6(("NemuDrvLinuxIOCtl: pFilp=%p uCmd=%#x ulArg=%p pid=%d/%d\n", pFilp, uCmd, (void *)ulArg, RTProcSelf(), current->pid));
    ...
    {
        Log(("NemuDrvLinuxIOCtl: copy_from_user(,%#lx,) failed; uCmd=%#x\n", ulArg, uCmd));
        return -EFAULT;
    }
    if (RT_UNLIKELY((Hdr.fFlags & SUPREQHDR_FLAGS_MAGIC_MASK) != SUPREQHDR_FLAGS_MAGIC))
    {
        Log(("NemuDrvLinuxIOCtl: bad header magic %#x; uCmd=%#x\n", Hdr.fFlags & SUPREQHDR_FLAGS_MAGIC_MASK, uCmd));
        return -EINVAL;
    }
    ...
    {
        Log(("NemuDrvLinuxIOCtl: too big cbBuf=%#x; uCmd=%#x\n", cbBuf, uCmd));
        return -E2BIG;
    }
    if (RT_UNLIKELY(_IOC_SIZE(uCmd) ? cbBuf != _IOC_SIZE(uCmd) : Hdr.cbIn < sizeof(Hdr)))
    {
        Log(("NemuDrvLinuxIOCtl: bad ioctl cbBuf=%#x _IOC_SIZE=%#x; uCmd=%#x\n", cbBuf, _IOC_SIZE(uCmd), uCmd));
        return -EINVAL;
    }
    pHdr = RTMemAlloc(cbBuf);
    if (RT_UNLIKELY(!pHdr))
    {
        OSDBGPRINT(("NemuDrvLinuxIOCtl: failed to allocate buffer of %d bytes for uCmd=%#x\n", cbBuf, uCmd));
        return -ENOMEM;
    }
    if (RT_FAILURE(RTR0MemUserCopyFrom(pHdr, ulArg, Hdr.cbIn)))
    {
        Log(("NemuDrvLinuxIOCtl: copy_from_user(,%#lx, %#x) failed; uCmd=%#x\n", ulArg, Hdr.cbIn, uCmd));
        RTMemFree(pHdr);
        return -EFAULT;
    }
    if (Hdr.cbIn < cbBuf)
    ...
    if (RT_LIKELY(!rc))
    {
        ...
        {
            /* this is really bad! */
            OSDBGPRINT(("NemuDrvLinuxIOCtl: copy_to_user(%#lx,,%#x); uCmd=%#x!\n", ulArg, cbOut, uCmd));
            rc = -EFAULT;
        }
    }
    else
    {
        Log(("NemuDrvLinuxIOCtl: pFilp=%p uCmd=%#x ulArg=%p failed, rc=%d\n", pFilp, uCmd, (void *)ulArg, rc));
        rc = -EINVAL;
    }
    RTMemFree(pHdr);

    Log6(("NemuDrvLinuxIOCtl: returns %d (pid=%d/%d)\n", rc, RTProcSelf(), current->pid));
    return rc;
}
 ...
 */
int NEMUCALL SUPDrvLinuxIDC(uint32_t uReq, PSUPDRVIDCREQHDR pReq)
{
    PSUPDRVSESSION  pSession;
...
RTCCUINTREG NEMUCALL supdrvOSChangeCR4(RTCCUINTREG fOrMask, RTCCUINTREG fAndMask)
{
 ...
 */
static int NemuDrvLinuxErr2LinuxErr(int rc)
{
    switch (rc)
    {
...
module_init(NemuDrvLinuxInit);
module_exit(NemuDrvLinuxUnload);

MODULE_AUTHOR(NEMU_VENDOR);
MODULE_DESCRIPTION(NEMU_PRODUCT " Support Driver");
MODULE_LICENSE("GPL");
#ifdef MODULE_VERSION
MODULE_VERSION(NEMU_VERSION_STRING " (" RT_XSTR(SUPDRV_IOC_VERSION) ")");
#endif
