...
#ifdef NEMU_WITH_WINE_FIX_IBMTMR
...
#ifdef NEMU
static BOOL WineD3D_CreateFakeGLContext(struct wined3d_fake_gl_ctx *ctx, struct NEMUUHGSMI *pHgsmi)
#else
...
#endif
{
    ...
    /* Create a GL context. */
#ifdef NEMU
    if (!(ctx->gl_ctx = pNemuCreateContext(ctx->dc, pHgsmi)))
#else
...
#ifdef NEMU_WITH_WDDM
        pNemuCtxChromiumParameteriCR(ctx->gl_ctx, GL_HOST_WND_CREATED_HIDDEN_CR,  GL_TRUE);
#endif
        ...
        const char *gl_vendor_string, const char *gl_renderer)
{
    ...
    {
#if 0//def NEMU_WITH_WINE_FIX_QUIRKS /* check if we still need this: device detection was improved significantly with wine */
...
#  ifdef NEMU_WDDM_WOW64
        HMODULE mod_gl = loadSystemDll("NemuOGL-x86.dll");
#  else
        HMODULE mod_gl = loadSystemDll("NemuOGL.dll");
#  endif
        if (!mod_gl)
        {
            ERR("Can't load NemuOGL.dll!\n");
            return FALSE;
        }
...
#  define NEMU_USE_FUNC(f) p##f = (void *)GetProcAddress(mod_gl, #f);
        NEMU_GL_FUNCS_GEN
#  undef NEMU_USE_FUNC
# endif
...
#ifdef NEMU_WITH_WDDM
    pHgsmi = NemuCrHgsmiCreate();
    if (!pHgsmi)
    {
        ERR("NemuCrHgsmiCreate failed");
        return FALSE;
    }
#endif
