...
<!--
    ...
    websrv-php.xsl:
        XSLT stylesheet that generates nemuServiceWrappers.php from
  ...
  <xsl:choose>
    ...
    <xsl:when test="$type='$unknown'">
      <xsl:call-template name="emitObject">
        <xsl:with-param name="type">Nemu_ManagedObject</xsl:with-param>
        <xsl:with-param name="value" select="$value" />
        <xsl:with-param name="safearray" select="$safearray"/>
      </xsl:call-template>
   </xsl:when>
...
<xsl:template name="interface">
   ...
   <xsl:text>
/**
 * Generated NemuWebService Interface Wrapper
 */
</xsl:text>
   <xsl:choose>
      <xsl:when test="($extends = '$unknown') or ($extends = '$errorinfo')">
         <xsl:value-of select="concat('class ', $ifname, ' extends Nemu_ManagedObject&#10;{&#10;')" />
      </xsl:when>
...
<xsl:template name="collection">
   ...
   <xsl:text>
/**
 * Generated NemuWebService Managed Object Collection
 */</xsl:text>
class <xsl:value-of select="$ifname"/>Collection extends Nemu_ManagedObjectCollection
{
...
<xsl:template name="interfacestruct">
   ...
   <xsl:text>
/**
 * Generated NemuWebService Struct
 */</xsl:text>
class <xsl:value-of select="$ifname"/> extends Nemu_Struct
{
...
<xsl:template name="structcollection">
   ...
   <xsl:text>
/**
 * Generated NemuWebService Struct Collection
 */</xsl:text>
class <xsl:value-of select="$ifname"/>Collection extends Nemu_StructCollection
{
    protected $_interfaceName = "<xsl:value-of select="$ifname"/>";
}
</xsl:template>
...
<xsl:template name="enum">
  <xsl:text>
/**
 * Generated NemuWebService ENUM
 */</xsl:text>
class <xsl:value-of select="@name"/> extends Nemu_Enum
{
...
<xsl:template name="enumcollection">
   ...
   <xsl:text>
/**
 * Generated NemuWebService Enum Collection
 */</xsl:text>
class <xsl:value-of select="$ifname"/>Collection extends Nemu_EnumCollection
{
    protected $_interfaceName = "<xsl:value-of select="$ifname"/>";
}
</xsl:template>
...
class Nemu_ManagedObject
{
    ...
    {
        if (is_subclass_of($class, 'Nemu_ManagedObject'))
        {
            return new $class($this->connection, $this->handle);
        }
        throw new Exception('Cannot cast Nemu_ManagedObject to non-child class Nemu_ManagedObject');
    }
...
abstract class Nemu_Collection implements ArrayAccess, Iterator, Countable
{
...
class Nemu_ManagedObjectCollection extends Nemu_Collection
{
    protected $_interfaceName = 'Nemu_ManagedObject';

    // Result is undefined if this is called AFTER any call to Nemu_Collection::offsetSet or Nemu_Collection::offsetUnset
    public function setInterfaceName($interface)
    {
        if (!is_subclass_of($interface, 'Nemu_ManagedObject'))
        {
            throw new Exception('Cannot set collection interface to non-child class of Nemu_ManagedObject');
        }
...
abstract class Nemu_StructCollection extends Nemu_Collection
{
...
abstract class Nemu_EnumCollection extends Nemu_Collection
{
}
