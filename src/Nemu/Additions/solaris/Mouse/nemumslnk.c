...
{
    int fNoLogo, hNemuMS, hConsMS, idConnection;

    handleArgs(argc, argv, &fNoLogo);
    /* Open our pointer integration driver (nemums). */
    hNemuMS = open("/dev/nemums", O_RDWR);
    if (hNemuMS < 0)
    {
        printf("Failed to open /dev/nemums - please make sure that the node exists and that\n"
    ...
    /* Link nemums to consms from below.  What this means is that nemums is
     * added to the list of input sources multiplexed by consms, and nemums
    ...
    idConnection = ioctl(hConsMS, I_PLINK, hNemuMS);
    if (idConnection < 0)
    {
        printf("Failed to add /dev/nemums (the pointer integration driver) to /dev/mouse\n"
