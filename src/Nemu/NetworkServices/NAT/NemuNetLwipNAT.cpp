/* $Id: NemuNetLwipNAT.cpp $ */
/** @file
 * NemuNetNAT - NAT Service for connecting to IntNet.
 */
...
#include "../NetLib/NemuNetLib.h"
#include "../NetLib/NemuNetBaseService.h"
#include "../NetLib/utils.h"
#include "NemuLwipCore.h"
...
#if defined(NEMU_RAWSOCK_DEBUG_HELPER)          \
    && (defined(NEMU_WITH_HARDENING)            \
        || defined(RT_OS_WINDOWS)               \
        || defined(RT_OS_DARWIN))
# error Have you forgotten to turn off NEMU_RAWSOCK_DEBUG_HELPER?
#endif

#ifdef NEMU_RAWSOCK_DEBUG_HELPER
extern "C" int getrawsock(int type);
#endif

#include "../NetLib/NemuPortForwardString.h"
...
static int nemuNetNATLogInit(int argc, char **argv);


class NemuNetLwipNAT: public NemuNetBaseService, public NATNetworkEventAdapter
{
    friend class NATNetworkListener;
  public:
    NemuNetLwipNAT(SOCKET icmpsock4, SOCKET icmpsock6);
    virtual ~NemuNetLwipNAT();
    void usage(){                /* @todo: should be implemented */ };
    int run();
    virtual int init(void);
    virtual int parseOpt(int rc, const RTGETOPTUNION& getOptVal);
    /* NemuNetNAT always needs Main */
   ...
   private:
    ...
    ComNatListenerPtr m_NemuListener;
    ComNatListenerPtr m_NemuClientListener;
    static INTNETSEG aXmitSeg[64];

    HRESULT HandleEvent(NemuEventType_T aEventType, IEvent *pEvent);
...
static NemuNetLwipNAT *g_pLwipNat;
INTNETSEG NemuNetLwipNAT::aXmitSeg[64];

/**
 * @note: this work on Event thread.
 */
HRESULT NemuNetLwipNAT::HandleEvent(NemuEventType_T aEventType, IEvent *pEvent)
{
    HRESULT hrc = S_OK;
    switch (aEventType)
    {
        case NemuEventType_OnNATNetworkSetting:
        {
        ...
        case NemuEventType_OnNATNetworkPortForward:
        {
        ...
        case NemuEventType_OnNATNetworkStartStop:
        {
        ...
        {
            LogRel(("NemuSVC became unavailable, exiting.\n"));
            shutdown();
            break;
        }
    }
...
/*static*/ DECLCALLBACK(void) NemuNetLwipNAT::onLwipTcpIpInit(void *arg)
{
    AssertPtrReturnVoid(arg);
    NemuNetLwipNAT *pNat = static_cast<NemuNetLwipNAT *>(arg);
    ...
    netif *pNetif = netif_add(&g_pLwipNat->m_LwipNetIf /* Lwip Interface */,
                              ...
                              g_pLwipNat /* state */,
                              NemuNetLwipNAT::netifInit /* netif_init_fn */,
...
/*static*/ DECLCALLBACK(void) NemuNetLwipNAT::onLwipTcpIpFini(void* arg)
{
    AssertPtrReturnVoid(arg);
    NemuNetLwipNAT *pThis = (NemuNetLwipNAT *)arg;
 ...
 */
/*static*/ err_t NemuNetLwipNAT::netifInit(netif *pNetif)
{
    err_t rcLwip = ERR_OK;

    AssertPtrReturn(pNetif, ERR_ARG);

    NemuNetLwipNAT *pNat = static_cast<NemuNetLwipNAT *>(pNetif->state);
...
/*static*/ err_t NemuNetLwipNAT::netifLinkoutput(netif *pNetif, pbuf *pPBuf)
{
    AssertPtrReturn(pNetif, ERR_ARG);
    AssertPtrReturn(pPBuf, ERR_ARG);

    NemuNetLwipNAT *self = static_cast<NemuNetLwipNAT *>(pNetif->state);
    ...
    RT_ZERO(NemuNetLwipNAT::aXmitSeg);

    size_t idx = 0;
    for (struct pbuf *q = pPBuf; q != NULL; q = q->next, ++idx)
    {
        AssertReturn(idx < RT_ELEMENTS(NemuNetLwipNAT::aXmitSeg), ERR_MEM);

#if ETH_PAD_SIZE
        if (q == pPBuf)
        {
            NemuNetLwipNAT::aXmitSeg[idx].pv = (uint8_t *)q->payload + ETH_PAD_SIZE;
            NemuNetLwipNAT::aXmitSeg[idx].cb = q->len - ETH_PAD_SIZE;
        }
        else
#endif
        {
            NemuNetLwipNAT::aXmitSeg[idx].pv = q->payload;
            NemuNetLwipNAT::aXmitSeg[idx].cb = q->len;
        }
    }

    int rc = self->sendBufferOnWire(NemuNetLwipNAT::aXmitSeg, idx,
...
NemuNetLwipNAT::NemuNetLwipNAT(SOCKET icmpsock4, SOCKET icmpsock6) : NemuNetBaseService("NemuNetNAT", "nat-network")
{
    LogFlowFuncEnter();
...
/*static*/ int NemuNetLwipNAT::natServicePfRegister(NATSEVICEPORTFORWARDRULE& natPf)
{
    int lrc;
...
/*static*/ int NemuNetLwipNAT::natServiceProcessRegisteredPf(VECNATSERVICEPF& vecRules)
{
    ITERATORNATSERVICEPF it;
 ...
 */
int NemuNetLwipNAT::init()
{
    LogFlowFuncEnter();

    /* virtualbox initialized in super class */
    int rc = ::NemuNetBaseService::init();
...

    {
        ...
        eventTypes.push_back(NemuEventType_OnNATNetworkPortForward);
        eventTypes.push_back(NemuEventType_OnNATNetworkSetting);
        rc = createNatListener(m_NatListener, virtualbox, this, eventTypes);
        AssertRCReturn(rc, rc);
    }


    // resolver changes are reported on nemu but are retrieved from
...

    {
        ...
        eventTypes.push_back(NemuEventType_OnNATNetworkStartStop);
        rc = createNatListener(m_NemuListener, virtualbox, this, eventTypes);
        AssertRCReturn(rc, rc);
    }

    {
        ...
        rc = createClientListener(m_NemuClientListener, virtualboxClient, this, eventTypes);
        AssertRCReturn(rc, rc);
    }
    ...
    nemuLwipCoreInitialize(NemuNetLwipNAT::onLwipTcpIpInit, this);
...
const char **NemuNetLwipNAT::getHostNameservers()
{
    if (m_host.isNull())
        return NULL;
...
int NemuNetLwipNAT::parseOpt(int rc, const RTGETOPTUNION& Val)
{
    switch (rc)
    {
        case 'p':
...
int NemuNetLwipNAT::processFrame(void *pvFrame, size_t cbFrame)
{
...
int NemuNetLwipNAT::processGSO(PCPDMNETWORKGSO pGso, size_t cbFrame)
{
...
int NemuNetLwipNAT::run()
{
    /* Father starts receiving thread and enter event loop. */
    NemuNetBaseService::run();

    nemuLwipCoreFinalize(NemuNetLwipNAT::onLwipTcpIpFini, this);

    m_vecPortForwardRule4.clear();
    m_vecPortForwardRule6.clear();

    destroyNatListener(m_NatListener, virtualbox);
    destroyNatListener(m_NemuListener, virtualbox);
    destroyClientListener(m_NemuClientListener, virtualboxClient);
...
extern "C" DECLEXPORT(int) TrustedMain(int argc, char **argv, char **envp)
{
    ...
    if (FAILED(hrc))
    {
        ...
        if (hrc == NS_ERROR_FILE_ACCESS_DENIED)
        {
            ...
            int vrc = com::GetNemuUserHomeDirectory(szHome, sizeof(szHome), false);
            if (RT_SUCCESS(vrc))
            {
    ...
    rc = nemuNetNATLogInit(argc, argv);
    // shall we bail if we failed to init logging?

    g_pLwipNat = new NemuNetLwipNAT(icmpsock4, icmpsock6);
...
static int nemuNetNATLogInit(int argc, char **argv)
{
    ...
    char szHome[RTPATH_MAX];
    rc = com::GetNemuUserHomeDirectory(szHome, sizeof(szHome), false);
    if (RT_FAILURE(rc))
        return rc;

    const char *pcszNetwork = NULL;

    // XXX: This duplicates information from NemuNetBaseService.cpp.
    // Perhaps option definitions should be exported as public static
    // member of NemuNetBaseService?
    ...
    cch = RTStrPrintf(szEnvVarBase, sizeof(szEnvVarBase),
                      "NEMUNET_%s_RELEASE_LOG", szNetwork);
    if (cch >= sizeof(szEnvVarBase))
        return VERR_BUFFER_OVERFLOW;

    char szError[RTPATH_MAX + 128];
    rc = com::NemuLogRelCreate("NAT Network",
                               szLogFile,
...
static LPCWSTR g_WndClassName = L"NemuNetNatLwipClass";
