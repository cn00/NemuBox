...
/** @file
 * Nemu Qt GUI - UIMachineViewScale class implementation.
 */
    ...
    if (scaledSize.isValid())
    {
        ...
        if (machine().GetAccelerate3DEnabled() && nemuGlobal().is3DAvailable())
        {
            ...
            display().NotifyScaleFactorChange(m_uScreenId,
                                              (uint32_t)(xScaleFactor * NEMU_OGL_SCALE_FACTOR_MULTIPLIER),
                                              (uint32_t)(yScaleFactor * NEMU_OGL_SCALE_FACTOR_MULTIPLIER));
        }
    }
...
{
    return nemuGlobal().availableGeometry(this);
}
