...
static void NEMUDRICopyRegion(DrawablePtr pDraw, RegionPtr pRegion,
                              DRI2BufferPtr pDest, DRI2BufferPtr pSrc)
{
}

static DRI2Buffer2Ptr NEMUDRICreateBuffer(DrawablePtr pDraw,
...
static void NEMUDRIDestroyBuffer(DrawablePtr pDraw, DRI2Buffer2Ptr pBuffer)
{
    free(pBuffer);
}
...
Bool NEMUDRIScreenInit(ScrnInfoPtr pScrn, ScreenPtr pScreen, NEMUPtr pNemu)
{
    ...
    for (i = 0; i < RT_ELEMENTS(devicePaths); ++i)
    {
        ...
        if (fd >= 0)
        {
            ...
            if (   pVersion
                && pVersion->name_len
                && !strcmp(pVersion->name, NEMU_DRM_DRIVER_NAME)
                && drmSetMaster(fd) == 0)
            {
    ...
    if (!DRI2Info.deviceName)
        return FALSE;
    DRI2Info.version = 3;
    DRI2Info.fd = pNemu->drmFD;
    DRI2Info.driverName = NEMU_DRI_DRIVER_NAME;
    DRI2Info.CopyRegion = NEMUDRICopyRegion;
    DRI2Info.Wait = NULL;
    DRI2Info.CreateBuffer = NEMUDRICreateBuffer;
    DRI2Info.DestroyBuffer = NEMUDRIDestroyBuffer;
    return DRI2ScreenInit(pScreen, &DRI2Info);
}

void NEMUDRICloseScreen(ScreenPtr pScreen, NEMUPtr pNemu)
{
