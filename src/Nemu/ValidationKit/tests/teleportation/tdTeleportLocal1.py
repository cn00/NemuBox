...
class tdTeleportLocal1(nemu.TestDriver):
    """
    ...
    def actionConfig(self):
        ...
        sNemuValidationKit_iso = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../NemuValidationKit.iso'));
        if not os.path.isfile(sNemuValidationKit_iso):
            sNemuValidationKit_iso = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../NemuTestSuite.iso'));
        if not os.path.isfile(sNemuValidationKit_iso):
            sNemuValidationKit_iso = '/mnt/ramdisk/nemu/svn/trunk/validationkit/NemuValidationKit.iso';
        if not os.path.isfile(sNemuValidationKit_iso):
            sNemuValidationKit_iso = '/mnt/ramdisk/nemu/svn/trunk/testsuite/NemuTestSuite.iso';
        if not os.path.isfile(sNemuValidationKit_iso):
            sCur = os.getcwd();
            for i in range(0, 10):
                sNemuValidationKit_iso = os.path.join(sCur, 'validationkit/NemuValidationKit.iso');
                if os.path.isfile(sNemuValidationKit_iso):
                    break;
                sNemuValidationKit_iso = os.path.join(sCur, 'testsuite/NemuTestSuite.iso');
                if os.path.isfile(sNemuValidationKit_iso):
                    break;
                sCur = os.path.abspath(os.path.join(sCur, '..'));
                if i is not None: pass;     # shut up pychecker/pylint.
        if not os.path.isfile(sNemuValidationKit_iso):
            sNemuValidationKit_iso = '/home/bird/validationkit/NemuValidationKit.iso';
        if not os.path.isfile(sNemuValidationKit_iso):
            sNemuValidationKit_iso = '/home/bird/testsuite/NemuTestSuite.iso';
    ...
    def createTestVMs(self, sName, iGroup, *tArgs, **dKeywordArgs):
        """
        Wrapper around nemu.createTestVM for creating two VMs, the source
                    ...
                    if oSessionSrc is not None:
                        ...
                        for sPassword in tsPasswords:
                            ...
                            if oProgressSrc:
                                ...
                                elif oProgressSrc.getErrInfoResultCode() != nemu.ComError.E_FAIL:
                                    reporter.testFailure('IConsole::teleport returns %s instead of E_FAIL' \
                                                         % (nemu.ComError.toString(oProgressSrc.getErrInfoResultCode()),));
                            ...
                            if oProgressSrc:
                                ...
                                if oProgressSrc.getResult() != oProgressDst.getResult():
                                    reporter.testFailure('Result differs - src=%s dst=%s' \
                                                         % (nemu.ComError.toString(oProgressSrc.getResult()),\
                                                            nemu.ComError.toString(oProgressDst.getResult())));
                                    ...
                                    reporter.testFailure('ErrorInfo::resultCode differs - src=%s dst=%s' \
                                                         % (nemu.ComError.toString(oProgressSrc.getErrInfoResultCode()),\
                                                            nemu.ComError.toString(oProgressDst.getErrInfoResultCode())));
    ...
    def test1Sub2SetPort(self, oSession, uPort, fInvalid = False):
        ...
        try:
            ...
            if not fInvalid or nemu.ComError.notEqual(oXcpt, nemu.ComError.E_INVALIDARG):
    ...
    def test1Sub1DoTeleport(self, oSession, sHostname, uPort, sPassword, cMsMaxDowntime, hrcExpected, sTestName):
        ...
        try:
            ...
            if nemu.ComError.equal(oXcpt, hrcExpected):
                fRc = True;
            else:
                reporter.testFailure('hresult %s, expected %s' \
                                     % (nemu.ComError.toString(oXcpt.hresult),
                                        nemu.ComError.toString(hrcExpected)));
        except Exception, oXcpt:
