...
#ifndef ___Nemu_vmm_dbgf_h
#define ___Nemu_vmm_dbgf_h

#include <Nemu/types.h>
#include <Nemu/log.h>                   /* LOG_ENABLED */
#include <Nemu/vmm/vmm.h>
#include <Nemu/vmm/dbgfsel.h>
 ...
 *
 * @returns Nemu status code which must be propagated up to EM if not VINF_SUCCESS.
...
VMM_INT_DECL(NEMUSTRICTRC)  DBGFBpCheckIo(PVM pVM, PVMCPU pVCpu, PCPUMCTX pCtx, RTIOPORT uIoPort, uint8_t cbValue);
...
typedef enum DBGFPLUGINOP
{
     ...
     * uArg: The full VirtualBox version, see Nemu/version.h. */
    DBGFPLUGINOP_INIT,
