...
static int vbsfTransferCommon(VBSFTRANSFERCTX *pCtx)
{
    ...
    if (VbglR0CanUsePhysPageList())
    {
        ...
        Log(("NEMUSF: vbsfTransferCommon: using page list: %d pages, offset 0x%03X\n", cPages, offFirstPage));
        ...
        if (paPages)
        {
            ...
            while (cPagesToTransfer != 0)
            {
                ...
                Log(("NEMUSF: vbsfTransferCommon: transferring %d pages at %d; %d bytes at %d\n",
                ...
                if (RT_FAILURE(rc))
                {
                    Log(("NEMUSF: vbsfTransferCommon: pfnTransferPages %Rrc, cbTransferred %d\n", rc, cbTransferred));
    ...
    if (fProcessed != TRUE)
    {
        ...
        Log(("NEMUSF: vbsfTransferCommon: using linear address\n"));

        while (cbToTransfer != 0)
        {
            cbIO = cbToTransfer;

            Log(("NEMUSF: vbsfTransferCommon: transferring %d bytes at %d\n",
            ...
            if (RT_FAILURE(rc))
            {
                Log(("NEMUSF: vbsfTransferCommon: pfnTransferBuffer %Rrc, cbTransferred %d\n", rc, cbTransferred));
...
static NTSTATUS vbsfReadInternal(IN PRX_CONTEXT RxContext)
{
    ...
    Log(("NEMUSF: vbsfReadInternal: AsyncIo = %d, Fcb->FileSize = 0x%RX64\n",
         AsyncIo, capFcb->Header.FileSize.QuadPart));
    Log(("NEMUSF: vbsfReadInternal: UserBuffer %p, BufferMdl %p\n",
         pbUserBuffer, BufferMdl));
    Log(("NEMUSF: vbsfReadInternal: ByteCount 0x%X, ByteOffset 0x%RX64, FileSize 0x%RX64\n",
...
#endif
    {
        ...
        {
            Log(("NEMUSF: vbsfReadInternal: EOF\n"));
    ...
    Log(("NEMUSF: vbsfReadInternal: Status = 0x%08X, ByteCount = 0x%X\n",
         Status, ByteCount));
...
static VOID vbsfReadWorker(VOID *pv)
{
    ...
    Log(("NEMUSF: vbsfReadWorker: calling the worker\n"));

    RxContext->IoStatusBlock.Status = vbsfReadInternal(RxContext);

    Log(("NEMUSF: vbsfReadWorker: Status 0x%08X\n",
...
NTSTATUS NemuMRxRead(IN PRX_CONTEXT RxContext)
{
    NTSTATUS Status = RxDispatchToWorkerThread(NemuMRxDeviceObject, DelayedWorkQueue,
                                               vbsfReadWorker,
                                               RxContext);

    Log(("NEMUSF: MRxRead: RxDispatchToWorkerThread: Status 0x%08X\n", Status));
...
static NTSTATUS vbsfWriteInternal(IN PRX_CONTEXT RxContext)
{
    ...
    Log(("NEMUSF: vbsfWriteInternal: AsyncIo = %d, Fcb->FileSize = 0x%RX64\n",
         AsyncIo, capFcb->Header.FileSize.QuadPart));
    Log(("NEMUSF: vbsfWriteInternal: UserBuffer %p, BufferMdl %p\n",
         pbUserBuffer, BufferMdl));
    Log(("NEMUSF: vbsfWriteInternal: ByteCount is 0x%X, ByteOffset is 0x%RX64, FileSize 0x%RX64\n",
         ByteCount, ByteOffset, FileSize));
    ...
    Log(("NEMUSF: vbsfWriteInternal: Status = 0x%08X, ByteCount = 0x%X\n",
         Status, ByteCount));
...
static VOID vbsfWriteWorker(VOID *pv)
{
    ...
    Log(("NEMUSF: vbsfWriteWorker: calling the worker\n"));

    RxContext->IoStatusBlock.Status = vbsfWriteInternal(RxContext);

    Log(("NEMUSF: vbsfWriteWorker: Status 0x%08X\n",
         RxContext->IoStatusBlock.Status));
...
NTSTATUS NemuMRxWrite(IN PRX_CONTEXT RxContext)
{
    NTSTATUS Status = RxDispatchToWorkerThread(NemuMRxDeviceObject, DelayedWorkQueue,
                                               vbsfWriteWorker,
                                               RxContext);

    Log(("NEMUSF: MRxWrite: RxDispatchToWorkerThread: Status 0x%08X\n",
         Status));
...
NTSTATUS NemuMRxLocks(IN PRX_CONTEXT RxContext)
{
    NTSTATUS Status = STATUS_SUCCESS;
    ...
    nemuRC = VbglR0SfLock(&pDeviceExtension->hgcmClient, &pNetRootExtension->map, pNemuFobx->hFile,
                          LowIoContext->ParamsFor.Locks.ByteOffset, LowIoContext->ParamsFor.Locks.Length, fu32Lock);

    Status = NemuErrorToNTStatus(nemuRC);

    Log(("NEMUSF: MRxLocks: Returned 0x%08X\n", Status));
    return Status;
}
...
NTSTATUS NemuMRxCompleteBufferingStateChangeRequest(IN OUT PRX_CONTEXT RxContext,
                                                    ...
                                                    IN PVOID pContext)
{
    Log(("NEMUSF: MRxCompleteBufferingStateChangeRequest: not implemented\n"));
    return STATUS_NOT_IMPLEMENTED;
}

NTSTATUS NemuMRxFlush (IN PRX_CONTEXT RxContext)
{
    NTSTATUS Status = STATUS_SUCCESS;
    ...
    nemuRC = VbglR0SfFlush(&pDeviceExtension->hgcmClient, &pNetRootExtension->map, pNemuFobx->hFile);

    Status = NemuErrorToNTStatus(nemuRC);

    Log(("NEMUSF: MRxFlush: Returned 0x%08X\n", Status));
    return Status;
}
                          ...
                          OUT PLARGE_INTEGER pNewAllocationSize)
{
    ...
    Log(("NEMUSF: vbsfSetEndOfFile: New size = %RX64 (%p), pNewAllocationSize = %p\n",
    ...
    Log(("NEMUSF: vbsfSetEndOfFile: VbglR0SfFsInfo returned %Rrc\n", nemuRC));

    Status = NemuErrorToNTStatus(nemuRC);
    if (Status == STATUS_SUCCESS)
    {
        Log(("NEMUSF: vbsfSetEndOfFile: VbglR0SfFsInfo new allocation size = %RX64\n",
    ...
    Log(("NEMUSF: vbsfSetEndOfFile: Returned 0x%08X\n", Status));
    return Status;
}
                           ...
                           OUT PLARGE_INTEGER pNewAllocationSize)
{
     ...
     */
    Log(("NEMUSF: MRxExtendStub: new size = %RX64\n",
