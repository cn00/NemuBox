...
HRESULT Medium::i_unlockRead(MediumState_T *aState)
{
    ...
    switch (m->state)
    {
        ...
        {
            LogFlowThisFunc(("Failing - state=%d\n", m->state));
            rc = setError(NEMU_E_INVALID_OBJECT_STATE,
...
# ifdef NEMU_WITH_EXTPACK
        ...
        if (RT_FAILURE(vrc))
            throw setError(NEMU_E_INVALID_OBJECT_STATE,
...
# ifdef NEMU_WITH_EXTPACK
        ...
        if (vrc == VERR_VD_PASSWORD_INCORRECT)
            throw setError(NEMU_E_PASSWORD_INCORRECT,
                              ...
                              bool aWait)
{
    ...
    try
    {
         ...
         * registered media are initially Inaccessible upon NemuSVC startup
        ...
        if (m->backRefs.size() != 0)
        {
            ...
            i_dumpBackRefs();
#endif
            throw setError(NEMU_E_OBJECT_IN_USE,
 ...
 * @retval  NEMU_E_INVALID_OBJECT_STATE if both media are not related to each other
 *          An error is set.
 ...
 *
 * @param aVRC  Nemu error code to use when no error message is provided.
 */
...
{
#ifdef NEMU_WITH_MAIN_LOCK_VALIDATION
...
HRESULT Medium::i_taskMergeHandler(Medium::MergeTask &task)
{
    ...
    try
    {
        ...
        {
            rcTmp = setError(NEMU_E_FILE_ERROR,
        ...
        try
        {
            ...
            if (task.mstrCipher.isNotEmpty())
            {
                    ...
                    && task.mstrCurrentPassword.isNotEmpty())
                {
                    ...
                    throw setError(NEMU_E_OBJECT_NOT_FOUND,
