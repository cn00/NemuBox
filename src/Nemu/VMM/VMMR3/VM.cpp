...
/** @page pg_vm     VM API
 *
 * This is the encapsulating bit.  It provides the APIs that Main and NemuBFE
 ...
 * mostly) and for reporting error back to the VMM user (Main/NemuBFE).
 *
 *
...
#include <Nemu/vmm/pdmcritsect.h>
...
#if defined(NEMU_WITH_DTRACE_R3) && !defined(NEMU_WITH_NATIVE_DTRACE)
    SUPR3TracerRegisterModule(~(uintptr_t)0, "NemuVMM", &g_VTGObjHeader, (uintptr_t)&g_VTGObjHeader,
 ...
 * @returns Nemu error code on failure.
                            ...
                            PVM *ppVM, PUVM *ppUVM)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
             ...
             */
            PVMREQ pReq;
            rc = VMR3ReqCallU(pUVM, VMCPUID_ANY, &pReq, RT_INDEFINITE_WAIT, VMREQFLAGS_NEMU_STATUS,
...
#ifdef RT_OS_LINUX
                    pszError = N_("VirtualBox kernel driver not loaded. The nemudrv kernel module "
                                  "was either not loaded or /dev/nemudrv is not set up properly. "
...
#else
                    /* This should only happen when mixing builds or with the usual /dev/nemudrv access issues. */
# if defined(RT_OS_DARWIN)
                    pszError = N_("VirtualBox KEXT is not accessible, permission problem. "
                                  ...
                                  "have the nemudrv KEXT from a different build or installation loaded.");
# elif defined(RT_OS_LINUX)
                    pszError = N_("VirtualBox kernel driver is not accessible, permission problem. "
                                  ...
                                  "not have the nemudrv kernel module from a different build or "
                                  "installation loaded. Also, make sure the nemudrv udev rule gives "
...
# else /* solaris, freebsd, ++. */
                    pszError = N_("VirtualBox kernel module is not accessible, permission problem. "
                                  "If you have built VirtualBox yourself, make sure that you do "
                                  "not have the nemudrv kernel module from a different install loaded.");
# endif
#endif
                    break;
...
#ifdef RT_OS_LINUX
                    pszError = N_("VirtualBox kernel driver not installed. The nemudrv kernel module "
                                  "was either not loaded or /dev/nemudrv was not created for some "
...
{
    /*
     * Load the NemuVMMR0.r0 module so that we can call GVMMR0CreateVM.
     */
    ...
    if (RT_FAILURE(rc))
    {
        ...
        return vmR3SetErrorU(pUVM, rc, RT_SRC_POS, N_("Failed to load NemuVMMR0.r0"));
    }
...
static int vmR3ReadBaseConfig(PVM pVM, PUVM pUVM, uint32_t cCpus)
{
     ...
     */
    const char *psz = RTEnvGet("NEMU_SUPLIB_FAKE");
...
static int vmR3InitRing0(PVM pVM)
{
    ...
    if (!psz || strcmp(psz, "fake"))
    {
        ...
        Log(("vmR3InitRing0: skipping because of NEMU_SUPLIB_FAKE=fake\n"));
...
static int vmR3InitRC(PVM pVM)
{
    ...
    if (!psz || strcmp(psz, "fake"))
    {
        ...
        Log(("vmR3InitRC: skipping because of NEMU_SUPLIB_FAKE=fake\n"));
 ...
 */
static DECLCALLBACK(NEMUSTRICTRC) vmR3PowerOn(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
 ...
 * @param   pvUser          Ignored.
 */
static DECLCALLBACK(NEMUSTRICTRC) vmR3Suspend(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
 ...
 *
 * @returns Nemu status code. When called on EMT, this will be a strict status
 ...
 * @param   pvUser          Reason.
 */
static DECLCALLBACK(NEMUSTRICTRC) vmR3Resume(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
 ...
 */
static DECLCALLBACK(NEMUSTRICTRC) vmR3LiveDoSuspend(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
 ...
 * @param   pvUser          The pfSuspended argument of vmR3SaveTeleport.
 */
static DECLCALLBACK(NEMUSTRICTRC) vmR3LiveDoStep1Cleanup(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
 ...
 * @param   pvUser          Ignored.
 */
static DECLCALLBACK(NEMUSTRICTRC) vmR3PowerOff(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
...
DECLCALLBACK(int) vmR3Destroy(PVM pVM)
{
    ...
    if (pVCpu->idCpu == 0)
    {
         ...
         */
#if defined(NEMU_WITH_STATISTICS) || defined(LOG_ENABLED)
...
static void vmR3DestroyUVM(PUVM pUVM, uint32_t cMilliesEMTWait)
{
    ...
    /*
     * Make sure the NemuVMMR0.r0 module and whatever else is unloaded.
     */
    PDMR3TermUVM(pUVM);
 ...
 * @param   pvUser          Ignored.
 */
static DECLCALLBACK(NEMUSTRICTRC) vmR3Reset(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    Assert(!pvUser); NOREF(pvUser);
 ...
 * @param   pvUser          Ignored.
 */
static DECLCALLBACK(NEMUSTRICTRC) vmR3SetRuntimeErrorChangeState(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    NOREF(pVCpu);
    Assert(!pvUser); NOREF(pvUser);
 ...
 *
 * @returns Nemu status code with modifications, see VMSetRuntimeErrorV.
 *
 * @param   pVM             The cross context VM structure.
 ...
 * The message is found in VMINT.
 *
 * @returns Nemu status code, see VMSetRuntimeError.
 * @param   pVM             The cross context VM structure.
 * @thread  EMT.
 */
