...
int SessionTaskCopyTo::Run(void)
{
    ...
    if (mCopyFileFlags)
    {
        setProgressErrorMsg(NEMU_E_IPRT_ERROR,
    ...
    if (!mSourceFile)
    {
        ...
        {
            rc = setProgressErrorMsg(NEMU_E_IPRT_ERROR,
    ...
    procInfo.mExecutable = Utf8Str(NEMUSERVICE_TOOL_CAT);
    ...
    if (RT_SUCCESS(rc))
    {
            ...
            || RT_SUCCESS(rc))
        {
            ...
            if (   mSourceSize > 0
                && cbWrittenTotal == 0)
            {
                /* If nothing was transfered but the file size was > 0 then "nemu_cat" wasn't able to write
...
int SessionTaskUpdateAdditions::Run(void)
{
    ...
    if (rc == VERR_TIMEOUT)
        hr = setProgressErrorMsg(NEMU_E_NOT_SUPPORTED,
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_FAILURE(rc))
        {
            hr = setProgressErrorMsg(NEMU_E_IPRT_ERROR,
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                switch (osType)
                {
                    case eOSType_Windows:
                    {
                        ...
                        {
                            /* Our certificate. */
                            mFiles.push_back(InstallerFile("CERT/ORACLE_NEMU.CER",
                                                           strUpdateDir + "oracle-nemu.cer",
                            ...
                            siCertUtilRem.mArguments.push_back(Utf8Str(strUpdateDir + "oracle-nemu.cer"));
                            siCertUtilRem.mArguments.push_back(Utf8Str(strUpdateDir + "oracle-nemu.cer"));
                            mFiles.push_back(InstallerFile("CERT/NEMUCERTUTIL.EXE",
                                                           strUpdateDir + "NemuCertUtil.exe",
                            ...
                            siCertUtilAdd.mArguments.push_back(Utf8Str(strUpdateDir + "oracle-nemu.cer"));
                            siCertUtilAdd.mArguments.push_back(Utf8Str(strUpdateDir + "oracle-nemu.cer"));
                            mFiles.push_back(InstallerFile("CERT/NEMUCERTUTIL.EXE",
                                                           strUpdateDir + "NemuCertUtil.exe",
                         ...
                         * the guest's bitness. */
                        mFiles.push_back(InstallerFile("NEMUWINDOWSADDITIONS_X86.EXE",
                                                       strUpdateDir + "NemuWindowsAdditions-x86.exe",
                                                       UPDATEFILE_FLAG_COPY_FROM_ISO));
                        mFiles.push_back(InstallerFile("NEMUWINDOWSADDITIONS_AMD64.EXE",
                                                       strUpdateDir + "NemuWindowsAdditions-amd64.exe",
                                                       UPDATEFILE_FLAG_COPY_FROM_ISO));
                        ...
                        /* Don't quit NemuService during upgrade because it still is used for this
                         * piece of code we're in right now (that is, here!) ... */
                        siInstaller.mArguments.push_back(Utf8Str("/no_nemuservice_exit"));
                        /* Tell the installer to report its current installation status
                         * using a running NemuTray instance via balloon messages in the
                         * Windows taskbar. */
                        ...
                        mFiles.push_back(InstallerFile("NEMUWINDOWSADDITIONS.EXE",
                                                       strUpdateDir + "NemuWindowsAdditions.exe",
