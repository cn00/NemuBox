/* $Id: NemuManageHelp.cpp $ */
/** @file
 * NemuManage - help and other message output.
 */
...
enum HELP_CMD_NEMUMANAGE    g_enmCurCommand = HELP_CMD_NEMUMANAGE_INVALID;
 ...
 */
void setCurrentCommand(enum HELP_CMD_NEMUMANAGE enmCommand)
{
    Assert(g_enmCurCommand == HELP_CMD_NEMUMANAGE_INVALID);
 ...
 * @param   pStrm               The output stream.
 */
static uint32_t printBriefCommandOrSubcommandHelp(enum HELP_CMD_NEMUMANAGE enmCommand, uint64_t fSubcommandScope, PRTSTREAM pStrm)
{
    uint32_t cLinesWritten = 0;
 ...
 * @param   pStrm               The output stream.
 */
static void printFullCommandOrSubcommandHelp(enum HELP_CMD_NEMUMANAGE enmCommand, uint64_t fSubcommandScope, PRTSTREAM pStrm)
{
    uint32_t cPendingBlankLines = 0;
    uint32_t cFound = 0;
    for (uint32_t i = 0; i < g_cHelpEntries; i++)
    {
        PCREFENTRY pHelp = g_apHelpEntries[i];
        if (   pHelp->idInternal == (int64_t)enmCommand
            || enmCommand == HELP_CMD_NEMUMANAGE_INVALID)
        {
            cFound++;
...
void showLogo(PRTSTREAM pStrm)
{
    ...
    if (!s_fShown)
    {
        RTStrmPrintf(pStrm, NEMU_PRODUCT " Command Line Management Interface Version "
                     NEMU_VERSION_STRING "\n"
...
void printUsage(USAGECATEGORY fCategory, uint32_t fSubCategory, PRTSTREAM pStrm)
{
    ...
    if (fCategory == USAGE_ALL)
        RTStrmPrintf(pStrm,
                     "  NemuManage [<general option>] <command>\n"
                     " \n \n"
                     "General Options:\n \n"
...
#define SEP pcszSep1, pcszSep2
    ...
    if (fCategory & USAGE_MODIFYVM)
    {
        RTStrmPrintf(pStrm,
                     ...
                     "                            [--graphicscontroller none|nemuvga|vmsvga]\n"
#else
                     "                            [--graphicscontroller none|nemuvga]\n"
#endif
                     "                            [--monitorcount <number>]\n"
                     "                            [--accelerate3d on|off]\n"
...
#if defined(NEMU_WITH_NAT_SERVICE)
...
# if !defined(RT_OS_SOLARIS) || defined(NEMU_ONLY_DOCS)
                     "                            create |\n"
                     "                            remove <name>\n"
# endif
                     "\n", SEP);
...
#ifndef NEMU_ONLY_DOCS /* Converted to man page, not needed. */
    if (fCategory == USAGE_ALL)
    {
        uint32_t cPendingBlankLines = 0;
        for (uint32_t i = 0; i < g_cHelpEntries; i++)
