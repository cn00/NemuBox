...
/** @file
 * Nemu IMachineDebugger COM class implementation (NemuC).
 */
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        else
        {
            ...
            if (SUCCEEDED(hrc))
            {
                ...
                if (RT_FAILURE(vrc))
                    hrc = setError(NEMU_E_VM_ERROR, tr("EMR3SetExecutionPolicy failed with %Rrc"), vrc);
            }
        }
    }
    return hrc;
...
HRESULT MachineDebugger::setPATMEnabled(BOOL aPATMEnabled)
{
    ...
    if (RT_FAILURE(vrc))
        return setError(NEMU_E_VM_ERROR, tr("PATMR3AllowPatching returned %Rrc"), vrc);

#else  /* !NEMU_WITH_RAW_MODE */
    if (aPATMEnabled)
        return setError(NEMU_E_VM_ERROR, tr("PATM not present"), VERR_NOT_SUPPORTED);
...
HRESULT MachineDebugger::setCSAMEnabled(BOOL aCSAMEnabled)
{
    ...
    if (RT_FAILURE(vrc))
        return setError(NEMU_E_VM_ERROR, tr("CSAMR3SetScanningEnabled returned %Rrc"), vrc);

#else  /* !NEMU_WITH_RAW_MODE */
    if (aCSAMEnabled)
        return setError(NEMU_E_VM_ERROR, tr("CASM not present"), VERR_NOT_SUPPORTED);
#endif /* !NEMU_WITH_RAW_MODE */
    return S_OK;
}
                                          ...
                                          const char *pszLogGetStr, Utf8Str *pstrSettings)
{
    ...
    for (;;)
    {
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            *pstrSettings = "";
            AssertReturn(vrc == VERR_BUFFER_OVERFLOW, setError(NEMU_E_IPRT_ERROR, tr("%s returned %Rrc"), pszLogGetStr, vrc));
        }
        else
...
HRESULT MachineDebugger::getOSName(com::Utf8Str &aOSName)
{
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            hrc = setError(NEMU_E_VM_ERROR, tr("DBGFR3OSQueryNameAndVersion failed with %Rrc"), vrc);
    }
    return hrc;
}
...
HRESULT MachineDebugger::setVirtualTimeRate(ULONG aVirtualTimeRate)
{
    ...
    else
    {
        ...
        if (SUCCEEDED(hrc))
        {
            ...
            if (RT_FAILURE(vrc))
                hrc = setError(NEMU_E_VM_ERROR, tr("TMR3SetWarpDrive(, %u) failed with rc=%Rrc"), aVirtualTimeRate, vrc);
        }
    }
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        if (SUCCEEDED(hrc))
        {
            ...
            if (RT_SUCCESS(vrc))
            {
                ...
                hrc = setError(NEMU_E_VM_ERROR, tr("DBGFR3Info failed with %Rrc"), vrc);
...
HRESULT MachineDebugger::detectOS(com::Utf8Str &aOs)
{
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        if (RT_SUCCESS(vrc) && vrc != VINF_DBGF_OS_NOT_DETCTED)
        {
            ...
            hrc = setError(NEMU_E_VM_ERROR, tr("DBGFR3OSDetect failed with %Rrc"), vrc);
    }
    return hrc;
}
...
HRESULT MachineDebugger::resetStats(const com::Utf8Str &aPattern)
{
    ...
    if (!ptrVM.isOk())
        return setError(NEMU_E_INVALID_VM_STATE, "Machine is not running");
