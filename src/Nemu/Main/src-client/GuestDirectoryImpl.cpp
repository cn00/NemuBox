                         ...
                         ULONG uDirID, const GuestDirectoryOpenInfo &openInfo)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        procInfo.mExecutable= Utf8Str(NEMUSERVICE_TOOL_LS);
...
int GuestDirectory::i_callbackDispatcher(PNEMUGUESTCTRLHOSTCBCTX pCbCtx, PNEMUGUESTCTRLHOSTCALLBACK pSvcCb)
{
...
HRESULT GuestDirectory::i_setErrorExternal(VirtualBoxBase *pInterface, int guestRc)
{
    ...
    return pInterface->setError(NEMU_E_IPRT_ERROR, GuestDirectory::i_guestErrorToString(guestRc).c_str());
}
    ...
    if (RT_FAILURE(rc)) /** @todo Add more errors here. */
    {
        switch (rc)
        {
            ...
            case VERR_ACCESS_DENIED:
                hr = setError(NEMU_E_IPRT_ERROR, tr("Reading directory \"%s\" failed: Unable to read / access denied"),
                              mData.mOpenInfo.mPath.c_str());
                break;

            case VERR_PATH_NOT_FOUND:
                hr = setError(NEMU_E_IPRT_ERROR, tr("Reading directory \"%s\" failed: Path not found"),
                              mData.mOpenInfo.mPath.c_str());
                break;

            case VERR_NO_MORE_FILES:
                /* See SDK reference. */
                hr = setError(NEMU_E_OBJECT_NOT_FOUND, tr("No more entries for directory \"%s\""),
                              mData.mOpenInfo.mPath.c_str());
                break;

            default:
                hr = setError(NEMU_E_IPRT_ERROR, tr("Error while reading directory \"%s\": %Rrc\n"),
                              mData.mOpenInfo.mPath.c_str(), rc);
                break;
        }
    }
