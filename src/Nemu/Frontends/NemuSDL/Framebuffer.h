 ...
 * Declaration of NemuSDLFB (SDL framebuffer) class
 */
...
#endif /* NEMU_SECURELABEL && !NEMU_WITH_SDL13 */

class NemuSDLFBOverlay;

class ATL_NO_VTABLE NemuSDLFB :
...
public:
    ...
    DECLARE_NOT_AGGREGATABLE(NemuSDLFB)

    DECLARE_PROTECT_FINAL_CONSTRUCT()

    BEGIN_COM_MAP(NemuSDLFB)
...
{
public:
    NemuSDLFBOverlay(ULONG x, ULONG y, ULONG width, ULONG height, BOOL visible,
                     NemuSDLFB *aParent);
    virtual ~NemuSDLFBOverlay();
...
#endif
    NEMU_SCRIPTABLE_DISPATCH_IMPL(IFramebuffer)
