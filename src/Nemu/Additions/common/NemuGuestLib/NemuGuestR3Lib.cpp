/* $Id: NemuGuestR3Lib.cpp $ */
/** @file
 * NemuGuestR3Lib - Ring-3 Support Library for VirtualBox guest additions, Core.
 */
...
# define NEMU_VBGLR3_XSERVER
#elif defined(NEMU_VBGLR3_XORG)
# include <sys/stat.h>
# include <fcntl.h>
# include <unistd.h>
# include <sys/ioctl.h>
...
*   Global Variables                                                                                                             *
*********************************************************************************************************************************/
/** The NemuGuest device handle. */
#ifdef NEMU_VBGLR3_XSERVER
static int g_File = -1;
...
static int vbglR3Init(const char *pszDeviceName)
{
...
#elif !defined (NEMU_VBGLR3_XSERVER)
    ...
    CFDictionaryRef ClassToMatch = IOServiceMatching("org_virtualbox_NemuGuest");
    if (!ClassToMatch)
        return VERR_GENERAL_FAILURE;
    ...
    kr = IOServiceOpen(ServiceObject, mach_task_self(), NEMUGUEST_DARWIN_IOSERVICE_COOKIE, &uConnection);
...
/**
 * Open the Nemu R3 Guest Library.  This should be called by system daemons
 * and processes.
 */
VBGLR3DECL(int) VbglR3Init(void)
{
    return vbglR3Init(NEMUGUEST_DEVICE_NAME);
}


/**
 * Open the Nemu R3 Guest Library.  Equivalent to VbglR3Init, but for user
 * session processes.
 */
VBGLR3DECL(int) VbglR3InitUser(void)
{
    return vbglR3Init(NEMUGUEST_USER_DEVICE_NAME);
}
...
VBGLR3DECL(void) VbglR3Term(void)
{
    ...
    if (cInits > 0)
        return;
#if !defined(NEMU_VBGLR3_XSERVER)
...
#else  /* NEMU_VBGLR3_XSERVER */
    int File = g_File;
    g_File = -1;
    if (File == -1)
        return;
    xf86close(File);
#endif /* NEMU_VBGLR3_XSERVER */
}
 ...
 *
 * @returns Nemu status code as returned by NemuGuestCommonIOCtl, or
 ...
 *
 * @remark  Exactly how the NemuGuestCommonIOCtl is ferried back
 ...
 * OS/2).  The idea is that the Nemu status codes in ring-0 should be transferred without loss down to
 ...
 * lossless, so still needs fixing. */
    ...
    APIRET rc = DosDevIOCtl((uintptr_t)g_File, NEMUGUEST_IOCTL_CATEGORY, iFunction,
 ...
 *        error checks using specific errno values, and just pass an Nemu
...
# endif
    ...
    /* Positive values are negated Nemu error status codes. */
    if (rc > 0)
        rc = -rc;
    else
# ifdef NEMU_VBGLR3_XSERVER
