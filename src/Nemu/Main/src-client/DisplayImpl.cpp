...
#include <Nemu/vmm/pdmdrv.h>
#if defined(DEBUG) || defined(NEMU_STRICT) /* for VM_ASSERT_EMT(). */
...
#if defined(NEMU_WITH_CROGL) || defined(NEMU_WITH_CRHGSMI)
...
#if defined(NEMU_WITH_VIDEOHWACCEL) || defined(NEMU_WITH_CRHGSMI)
    /** VBVA callbacks */
...
typedef struct
{
    ...
    uint32_t cxPNG;
    uint32_t cyPNG;
} NEMU_DISPLAY_SAVESCREENSHOT_DATA;
...
{
    NEMU_DISPLAY_SAVESCREENSHOT_DATA *pData = (NEMU_DISPLAY_SAVESCREENSHOT_DATA*)pvCtx;
...
DECLCALLBACK(void) Display::i_displaySSMSaveScreenshot(PSSMHANDLE pSSM, void *pvUser)
{
    ...
    if (ptrVM.isOk())
    {
#if defined(NEMU_WITH_HGCM) && defined(NEMU_WITH_CROGL)
            ...
            && that->mCrOglCallbacks.pfnHasData())
        {
            ...
            if (pVMMDev)
            {
                NEMU_DISPLAY_SAVESCREENSHOT_DATA *pScreenshot;
                pScreenshot = (NEMU_DISPLAY_SAVESCREENSHOT_DATA*)RTMemAllocZ(sizeof(*pScreenshot));
                if (pScreenshot)
                {
                    ...
                    NEMUCRCMDCTL_HGCM data;
                    data.Hdr.enmType = NEMUCRCMDCTL_TYPE_HGCM;
                    data.Hdr.u32Function = SHCRGL_HOST_FN_TAKE_SCREENSHOT;

                    data.aParms[0].type = NEMU_HGCM_SVC_PARM_PTR;
...
#endif
        {
            ...
            uint32_t cy = 0;
            bool fFreeMem = false;
            int rc = Display::i_displayTakeScreenshotEMT(that, NEMU_VIDEO_PRIMARY_SCREEN, &pbData, &cbData, &cx, &cy, &fFreeMem);

            /*
             * It is possible that success is returned but everything is 0 or NULL.
             * (no display attached if a VM is running with NemuHeadless on OSE for example)
             */
...
HRESULT Display::init(Console *aParent)
{
...

    {
        ...
        eventTypes.push_back(NemuEventType_OnStateChanged);
    ...
    GraphicsControllerType_T enmGpuType = (GraphicsControllerType_T)GraphicsControllerType_NemuVGA;
    mParent->i_machine()->COMGETTER(GraphicsControllerType)(&enmGpuType);
    mfIsCr3DEnabled = fIs3DEnabled && enmGpuType == GraphicsControllerType_NemuVGA;
...
DECLCALLBACK(void) Display::i_displayCrCmdFree(struct NEMUCRCMDCTL* pCmd, uint32_t cbCmd, int rc, void *pvCompletion)
{
    Assert(pvCompletion);
...
int Display::i_crOglWindowsShow(bool fShow)
{
    ...
    NEMUCRCMDCTL_HGCM *pData = (NEMUCRCMDCTL_HGCM*)RTMemAlloc(sizeof(NEMUCRCMDCTL_HGCM));
    if (!pData)
    {
    ...
    pData->Hdr.enmType = NEMUCRCMDCTL_TYPE_HGCM;
    pData->Hdr.u32Function = SHCRGL_HOST_FN_WINDOWS_SHOW;

    pData->aParms[0].type = NEMU_HGCM_SVC_PARM_32BIT;
...
int Display::i_notifyCroglResize(const PVBVAINFOVIEW pView, const PVBVAINFOSCREEN pScreen, void *pvVRAM)
{
    ...
    if (mfIsCr3DEnabled)
    {
        ...
        if (mhCrOglSvc)
        {
            ...
            if (pVMMDev)
            {
                NEMUCRCMDCTL_HGCM *pCtl;
                pCtl = (NEMUCRCMDCTL_HGCM*)RTMemAlloc(sizeof(CRNEMUHGCMDEVRESIZE) + sizeof(NEMUCRCMDCTL_HGCM));
                if (pCtl)
                {
                    CRNEMUHGCMDEVRESIZE *pData = (CRNEMUHGCMDEVRESIZE*)(pCtl+1);
                    pData->Screen = *pScreen;
                    pData->pvVRAM = pvVRAM;

                    pCtl->Hdr.enmType = NEMUCRCMDCTL_TYPE_HGCM;
                    pCtl->Hdr.u32Function = SHCRGL_HOST_FN_DEV_RESIZE;
                    pCtl->aParms[0].type = NEMU_HGCM_SVC_PARM_PTR;
...
#endif /* #if defined(NEMU_WITH_HGCM) && defined(NEMU_WITH_CROGL) */
    return VINF_SUCCESS;
}
                                   ...
                                   uint32_t cbLine, uint32_t w, uint32_t h, uint16_t flags)
{
    ...
    if (uScreenId == NEMU_VIDEO_PRIMARY_SCREEN)
    {
        pFBInfo->w = w;
        pFBInfo->h = h;
...
void Display::i_updateGuestGraphicsFacility(void)
{
    ...
    if (   mfVMMDevSupportsGraphics
        ...
        pGuest->i_setAdditionsStatus(NemuGuestFacilityType_Graphics,
...
int Display::i_handleSetVisibleRegion(uint32_t cRect, PRTRECT pRect)
{
    ...
    if (mfIsCr3DEnabled && vmmDev)
    {
        ...
        {
            NEMUCRCMDCTL_HGCM *pCtl;
            pCtl = (NEMUCRCMDCTL_HGCM*)RTMemAlloc(RT_MAX(cRect, 1) * sizeof(RTRECT) + sizeof(NEMUCRCMDCTL_HGCM));
            if (pCtl)
            {
                                    ...
                                    DISPLAYFBINFO *pFBInfo)
{
    ...
    {
        uint32_t fu32HostEvents = NEMU_VIDEO_INFO_HOST_EVENTS_F_VRDP_RESET;
...
HRESULT Display::attachFramebuffer(ULONG aScreenId, const ComPtr<IFramebuffer> &aFramebuffer, com::Guid &aId)
{
    ...
    if (ptrVM.isOk())
    {
        ...
        if (mfIsCr3DEnabled)
        {
            ...
            data.aParms[0].type = NEMU_HGCM_SVC_PARM_32BIT;
            data.aParms[0].u.uint32 = aScreenId;

            int vrc = i_crCtlSubmitSync(&data.Hdr, sizeof(data));
            AssertRC(vrc);
        }
#endif /* defined(NEMU_WITH_HGCM) && defined(NEMU_WITH_CROGL) */
...
HRESULT Display::setSeamlessMode(BOOL enabled)
{
    ...
    if (!enabled)
    {
        ...
        if (mfIsCr3DEnabled && vmmDev)
        {
            ...
            pData->aParms[0].type = NEMU_HGCM_SVC_PARM_PTR;
                                                ...
                                                uint32_t u32Width, uint32_t u32Height)
{
        ...
        && pDisplay->mCrOglCallbacks.pfnHasData())
    {
        ...
        if (pVMMDev)
        {
            CRNEMUHGCMTAKESCREENSHOT *pScreenshot = (CRNEMUHGCMTAKESCREENSHOT *)RTMemAlloc(sizeof(*pScreenshot));
            if (pScreenshot)
            {
                /* screen id or CRSCREEN_ALL to specify all enabled */
...
{
    int rc;
    if (   aScreenId == NEMU_VIDEO_PRIMARY_SCREEN
            ...
            if (pbDst != NULL)
            {
                ...
                else
                {
                    ...
                    if (   rc == VERR_INVALID_STATE
                        && aScreenId == NEMU_VIDEO_PRIMARY_SCREEN)
                    {
                        rc = pDisplay->mpDrv->pUpPort->pfnTakeScreenshot(pDisplay->mpDrv->pUpPort, ppbData, pcbData, pcx, pcy);
                        *pfMemFree = false;
                    }
...
HRESULT Display::drawToScreen(ULONG aScreenId, BYTE *aAddress, ULONG aX, ULONG aY, ULONG aWidth, ULONG aHeight)
{
     ...
     */
    int rcNemu = VMR3ReqCallWaitU(ptrVM.rawUVM(), VMCPUID_ANY, (PFNRT)Display::i_drawToScreenEMT, 7,
     ...
     */
    HRESULT rc = S_OK;
    if (rcNemu == VERR_NOT_SUPPORTED || rcNemu == VERR_NOT_IMPLEMENTED)
    {
        /** @todo implement generic fallback for screen blitting. */
        rc = E_NOTIMPL;
    }
    else if (RT_FAILURE(rcNemu))
        rc = setError(NEMU_E_IPRT_ERROR,
                      tr("Could not draw to the screen (%Rrc)"), rcNemu);
//@todo
//    else
//    {
...
int Display::i_InvalidateAndUpdateEMT(Display *pDisplay, unsigned uId, bool fUpdateAll)
{
    ...
    for (uScreenId = (fUpdateAll ? 0 : uId); uScreenId < pDisplay->mcMonitors; uScreenId++)
    {
        ...
        if (   !pFBInfo->fVBVAEnabled
            && uScreenId == NEMU_VIDEO_PRIMARY_SCREEN)
        {
...
HRESULT Display::invalidateAndUpdate()
{
    ...
    int rcNemu = VMR3ReqCallNoWaitU(ptrVM.rawUVM(), VMCPUID_ANY, (PFNRT)Display::i_InvalidateAndUpdateEMT,
                                    3, this, 0, true);
    alock.acquire();

    if (RT_FAILURE(rcNemu))
        rc = setError(NEMU_E_IPRT_ERROR,
                      tr("Could not invalidate and update the screen (%Rrc)"), rcNemu);
...
HRESULT Display::invalidateAndUpdateScreen(ULONG aScreenId)
{
    ...
    if (RT_FAILURE(rcNemu))
        rc = setError(NEMU_E_IPRT_ERROR,
                      tr("Could not invalidate and update the screen %d (%Rrc)"), aScreenId, rcNemu);
...
{
#ifdef NEMU_WITH_VIDEOHWACCEL
    mpDrv->pVBVACallbacks->pfnVHWACommandCompleteAsync(mpDrv->pVBVACallbacks, (PNEMUVHWACMD)aCommand);
    return S_OK;
#else
    return E_NOTIMPL;
#endif
...
#endif /* NEMU_WITH_CROGL && NEMU_WITH_HGCM */
...
{
    NemuEventType_T aType = NemuEventType_Invalid;

    aEvent->COMGETTER(Type)(&aType);
    switch (aType)
    {
        case NemuEventType_OnStateChanged:
        {
...
int Display::i_crViewportNotify(ULONG aScreenId, ULONG x, ULONG y, ULONG width, ULONG height)
{
    ...
    size_t cbData = RT_UOFFSETOF(NEMUCRCMDCTL_HGCM, aParms[5]);
    NEMUCRCMDCTL_HGCM *pData = (NEMUCRCMDCTL_HGCM *)alloca(cbData);
    ...
    pData->aParms[1].type = NEMU_HGCM_SVC_PARM_32BIT;
    pData->aParms[1].u.uint32 = x;

    pData->aParms[2].type = NEMU_HGCM_SVC_PARM_32BIT;
    pData->aParms[2].u.uint32 = y;

    pData->aParms[3].type = NEMU_HGCM_SVC_PARM_32BIT;
    pData->aParms[3].u.uint32 = width;

    pData->aParms[4].type = NEMU_HGCM_SVC_PARM_32BIT;
...
void Display::i_setupCrHgsmiData(void)
{
    ...
    if (pVMMDev)
        rc = pVMMDev->hgcmHostSvcHandleCreate("NemuSharedCrOpenGL", &mhCrOglSvc);
    else
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        NEMUVDMACMD_CHROMIUM_CTL_CRHGSMI_SETUP_MAINCB Completion;
        Completion.Hdr.enmType = NEMUVDMACMD_CHROMIUM_CTL_TYPE_CRHGSMI_SETUP_MAINCB;
        ...
        parm.type = NEMU_HGCM_SVC_PARM_PTR;
        parm.u.pointer.addr = &Completion;
        parm.u.pointer.size = 0;

        rc = pVMMDev->hgcmHostCall("NemuSharedCrOpenGL", SHCRGL_HOST_FN_CRHGSMI_CTL, 1, &parm);
        if (RT_SUCCESS(rc))
            mCrOglCallbacks = Completion.MainInterface;
        else
            AssertMsgFailed(("NEMUVDMACMD_CHROMIUM_CTL_TYPE_CRHGSMI_SETUP_COMPLETION failed (rc=%Rrc)\n", rc));
    }
                                                   ...
                                                   uint32_t bpp, void *pvVRAM, uint32_t cbLine, uint32_t cx, uint32_t cy)
{
    ...
    int rc = pThis->i_handleDisplayResize(NEMU_VIDEO_PRIMARY_SCREEN, bpp, pvVRAM, cbLine, cx, cy, VBVA_SCREEN_F_ACTIVE);
                                                    ...
                                                    uint32_t x, uint32_t y, uint32_t cx, uint32_t cy)
{
    ...
    pDrv->pDisplay->i_handleDisplayUpdate(NEMU_VIDEO_PRIMARY_SCREEN, x, y, cx, cy);
}
...
/*static*/ DECLCALLBACK(void) Display::i_displayRefreshCallback(PPDMIDISPLAYCONNECTOR pInterface)
{
    ...
    if (rc != VINF_TRY_AGAIN) /* Means 'do nothing' here. */
    {
        if (rc == VWRN_INVALID_STATE)
        {
            ...
            DISPLAYFBINFO *pFBInfo = &pDisplay->maFramebuffers[NEMU_VIDEO_PRIMARY_SCREEN];
...
# if defined(NEMU_WITH_HGCM) && defined(NEMU_WITH_CROGL)
            if (pDisplay->mfIsCr3DEnabled)
            {
                if (ASMAtomicCmpXchgU32(&pDisplay->mfCrOglVideoRecState, CRVREC_STATE_SUBMITTED, CRVREC_STATE_IDLE))
                {
                    ...
                    {
                        /* submit */
                        NEMUCRCMDCTL_HGCM *pData = &pDisplay->mCrOglScreenshotCtl;
...
# endif /* NEMU_WITH_HGCM && NEMU_WITH_CROGL */
            ...
            for (uScreenId = 0; uScreenId < pDisplay->mcMonitors; uScreenId++)
            {
                    ...
                    && !pFBInfo->fDisabled)
                {
                    ...
                    else if (uScreenId == NEMU_VIDEO_PRIMARY_SCREEN && pDrv->IConnector.pbData)
                    {
                        rc = VideoRecCopyToIntBuf(pDisplay->mpVideoRecCtx, uScreenId, 0, 0,
                                                  BitmapFormat_BGR,
...
int Display::i_handleVHWACommandProcess(PNEMUVHWACMD pCommand)
{
...
DECLCALLBACK(int) Display::i_displayVHWACommandProcess(PPDMIDISPLAYCONNECTOR pInterface, PNEMUVHWACMD pCommand)
{
    PDRVMAINDISPLAY pDrv = PDMIDISPLAYCONNECTOR_2_MAINDISPLAY(pInterface);
...
#ifdef NEMU_WITH_CRHGSMI
void Display::i_handleCrHgsmiCommandCompletion(int32_t result, uint32_t u32Function, PNEMUHGCMSVCPARM pParam)
{
    mpDrv->pVBVACallbacks->pfnCrHgsmiCommandCompleteAsync(mpDrv->pVBVACallbacks,
                                                          (PNEMUVDMACMD_CHROMIUM_CMD)pParam->u.pointer.addr, result);
}

void Display::i_handleCrHgsmiControlCompletion(int32_t result, uint32_t u32Function, PNEMUHGCMSVCPARM pParam)
{
    PNEMUVDMACMD_CHROMIUM_CTL pCtl = (PNEMUVDMACMD_CHROMIUM_CTL)pParam->u.pointer.addr;
    mpDrv->pVBVACallbacks->pfnCrHgsmiControlCompleteAsync(mpDrv->pVBVACallbacks, pCtl, result);
}

void Display::i_handleCrHgsmiCommandProcess(PNEMUVDMACMD_CHROMIUM_CMD pCmd, uint32_t cbCmd)
{
...
void Display::i_handleCrHgsmiControlProcess(PNEMUVDMACMD_CHROMIUM_CTL pCtl, uint32_t cbCtl)
{
    int rc = VERR_NOT_SUPPORTED;
    NEMUHGCMSVCPARM parm;
    parm.type = NEMU_HGCM_SVC_PARM_PTR;
    ...
    if (mhCrOglSvc)
    {
        ...
        if (pVMMDev)
        {
            bool fCheckPendingViewport = (pCtl->enmType == NEMUVDMACMD_CHROMIUM_CTL_TYPE_CRHGSMI_SETUP);
...
DECLCALLBACK(void) Display::i_displayCrHgsmiCommandProcess(PPDMIDISPLAYCONNECTOR pInterface, PNEMUVDMACMD_CHROMIUM_CMD pCmd,
                                                           uint32_t cbCmd)
{
    PDRVMAINDISPLAY pDrv = PDMIDISPLAYCONNECTOR_2_MAINDISPLAY(pInterface);
...
DECLCALLBACK(void) Display::i_displayCrHgsmiControlProcess(PPDMIDISPLAYCONNECTOR pInterface, PNEMUVDMACMD_CHROMIUM_CTL pCmd,
                                                           uint32_t cbCmd)
{
    PDRVMAINDISPLAY pDrv = PDMIDISPLAYCONNECTOR_2_MAINDISPLAY(pInterface);
...
DECLCALLBACK(void) Display::i_displayCrHgsmiCommandCompletion(int32_t result, uint32_t u32Function, PNEMUHGCMSVCPARM pParam,
                                                              void *pvContext)
{
...
DECLCALLBACK(void) Display::i_displayCrHgsmiControlCompletion(int32_t result, uint32_t u32Function, PNEMUHGCMSVCPARM pParam,
                                                              void *pvContext)
{
    Display *pDisplay = (Display *)pvContext;
...
#if defined(NEMU_WITH_HGCM) && defined(NEMU_WITH_CROGL)
DECLCALLBACK(void)  Display::i_displayCrHgcmCtlSubmitCompletion(int32_t result, uint32_t u32Function, PNEMUHGCMSVCPARM pParam,
                                                                void *pvContext)
{
    NEMUCRCMDCTL *pCmd = (NEMUCRCMDCTL*)pParam->u.pointer.addr;
    if (pCmd->u.pfnInternal)
        ((PFNCRCTLCOMPLETION)pCmd->u.pfnInternal)(pCmd, pParam->u.pointer.size, result, pvContext);
}

int  Display::i_handleCrHgcmCtlSubmit(struct NEMUCRCMDCTL* pCmd, uint32_t cbCmd,
...
int Display::i_crCtlSubmit(struct NEMUCRCMDCTL* pCmd, uint32_t cbCmd, PFNCRCTLCOMPLETION pfnCompletion, void *pvCompletion)
{
...
int Display::i_crCtlSubmitSync(struct NEMUCRCMDCTL* pCmd, uint32_t cbCmd)
{
    int rc = RTCritSectRwEnterShared(&mCrOglLock);
    if (RT_SUCCESS(rc))
    {
...
int Display::i_crCtlSubmitAsyncCmdCopy(struct NEMUCRCMDCTL* pCmd, uint32_t cbCmd)
{
    NEMUCRCMDCTL* pCmdCopy = (NEMUCRCMDCTL*)RTMemAlloc(cbCmd);
    if (!pCmdCopy)
    {
...
int Display::i_crCtlSubmitSyncIfHasDataForScreen(uint32_t u32ScreenID, struct NEMUCRCMDCTL* pCmd, uint32_t cbCmd)
{
    int rc = RTCritSectRwEnterShared(&mCrOglLock);
    AssertRCReturn(rc, rc);
...
HRESULT Display::notifyScaleFactorChange(ULONG aScreenId, ULONG aScaleFactorWMultiplied, ULONG aScaleFactorHMultiplied)
{
    ...
    if (mfIsCr3DEnabled)
    {
        /* NemuSharedCrOpenGL HGCM host service is running. */
        if (mhCrOglSvc)
        {
            VMMDev *pVMMDev = mParent->i_getVMMDev();
            if (pVMMDev)
            {
                NEMUCRCMDCTL_HGCM *pCtl;
                pCtl = (NEMUCRCMDCTL_HGCM *)RTMemAlloc(sizeof(CRNEMUHGCMSETSCALEFACTOR) + sizeof(NEMUCRCMDCTL_HGCM));
                if (pCtl)
                {
                    CRNEMUHGCMSETSCALEFACTOR *pData = (CRNEMUHGCMSETSCALEFACTOR *)(pCtl + 1);
                    int rc;
...
HRESULT Display::notifyHiDPIOutputPolicyChange(BOOL fUnscaledHiDPI)
{
    ...
    if (mfIsCr3DEnabled)
    {
        ...
        if (mhCrOglSvc)
        {
            ...
            {
                NEMUCRCMDCTL_HGCM *pCtl;
                pCtl = (NEMUCRCMDCTL_HGCM *)RTMemAlloc(sizeof(CRNEMUHGCMSETUNSCALEDHIDPIOUTPUT) + sizeof(NEMUCRCMDCTL_HGCM));
                if (pCtl)
                {
                    CRNEMUHGCMSETUNSCALEDHIDPIOUTPUT *pData = (CRNEMUHGCMSETUNSCALEDHIDPIOUTPUT *)(pCtl + 1);
                    int rc;
...
DECLCALLBACK(void) Display::i_displayVRecCompletion(struct NEMUCRCMDCTL* pCmd, uint32_t cbCmd, int rc, void *pvCompletion)
{
...
DECLCALLBACK(void) Display::i_displayVBVADisable(PPDMIDISPLAYCONNECTOR pInterface, unsigned uScreenId)
{
    ...
    if (!fRenderThreadMode && uScreenId == NEMU_VIDEO_PRIMARY_SCREEN)
    {
