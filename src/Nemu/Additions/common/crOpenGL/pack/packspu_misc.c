...
void PACKSPU_APIENTRY packspu_NemuWindowDestroy( GLint con, GLint window )
{
...
GLint PACKSPU_APIENTRY packspu_NemuWindowCreate( GLint con, const char *dpyName, GLint visBits )
{
    GET_THREAD(thread);
...
{
    return packspu_NemuWindowCreate( 0, dpyName, visBits );
}
...
GLint PACKSPU_APIENTRY packspu_NemuPackSetInjectThread(struct NEMUUHGSMI *pHgsmi)
{
    GLint con = 0;
    int i;
    GET_THREAD(thread);
...
GLuint PACKSPU_APIENTRY packspu_NemuPackGetInjectID(GLint con)
{
    GLuint ret;
    ...
    crLockMutex(&_PackMutex);
    {
        ...
        CRASSERT(thread && thread->netServer.conn && thread->netServer.conn->type==CR_NEMUHGCM);
...
void PACKSPU_APIENTRY packspu_NemuPackSetInjectID(GLuint id)
{
    crLockMutex(&_PackMutex);
    {
        GET_THREAD(thread);

        CRASSERT(thread && thread->netServer.conn && thread->netServer.conn->type==CR_NEMUHGCM && thread->bInjectThread);
        thread->netServer.conn->u32InjectClientID = id;
    }
    crUnlockMutex(&_PackMutex);
}

void PACKSPU_APIENTRY packspu_NemuAttachThread()
{
#if 0
    int i;
    GET_THREAD(thread);
...
void PACKSPU_APIENTRY packspu_NemuDetachThread()
{
    if (CRPACKSPU_IS_WDDM_CRHGSMI())
    {
        crPackSetContext(NULL);
...
void PACKSPU_APIENTRY packspu_NemuPackAttachThread()
{
}

void PACKSPU_APIENTRY packspu_NemuPackDetachThread()
{
}
#endif /*CHROMIUM_THREADSAFE*/

void PACKSPU_APIENTRY packspu_NemuPresentComposition(GLint win, const struct NEMUVR_SCR_COMPOSITOR * pCompositor, const struct NEMUVR_SCR_COMPOSITOR_ENTRY *pChangedEntry)
{
}
