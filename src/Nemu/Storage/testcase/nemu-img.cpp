...
static void showLogo(PRTSTREAM pStrm)
{
    ...
    if (!s_fShown)
    {
        RTStrmPrintf(pStrm, NEMU_PRODUCT " Disk Utility " NEMU_VERSION_STRING "\n"
...
static DECLCALLBACK(int) nemuimgDvmRead(void *pvUser, uint64_t off, void *pvBuf, size_t cbRead)
{
    int rc = VINF_SUCCESS;
    PNEMUHDD pDisk = (PNEMUHDD)pvUser;
...
static DECLCALLBACK(int) nemuimgDvmWrite(void *pvUser, uint64_t off, const void *pvBuf, size_t cbWrite)
{
    PNEMUHDD pDisk = (PNEMUHDD)pvUser;
...
static DECLCALLBACK(int) nemuimgQueryBlockStatus(void *pvUser, uint64_t off,
...
static DECLCALLBACK(int) nemuimgQueryRangeUse(void *pvUser, uint64_t off, uint64_t cb,
                                              bool *pfUsed)
{
...
typedef struct NEMUIMGVFS
{
    /** Pointer to the next VFS handle. */
    struct NEMUIMGVFS *pNext;
    /** VFS handle. */
    RTVFS              hVfs;
} NEMUIMGVFS, *PNEMUIMGVFS;
...
static int handleCompact(HandlerArg *a)
{
    ...
    PNEMUIMGVFS pNemuImgVfsHead = NULL;
        ...
        && fFilesystemAware)
    {
        ...
        if (cbDisk > 0)
        {
            rc = RTDvmCreate(&hDvm, nemuimgDvmRead, nemuimgDvmWrite, cbDisk, 512,
                             0 /* fFlags*/, pDisk);
            if (RT_SUCCESS(rc))
            {
                    ...
                    && RTDvmMapGetValidVolumes(hDvm))
                {
                    ...
                    do
                    {
                        ...
                        {
                            PNEMUIMGVFS pNemuImgVfs = (PNEMUIMGVFS)RTMemAllocZ(sizeof(NEMUIMGVFS));
                            if (!pNemuImgVfs)
                                rc = VERR_NO_MEMORY;
                            else
                            {
                                pNemuImgVfs->hVfs = hVfs;
                                pNemuImgVfs->pNext = pNemuImgVfsHead;
                                pNemuImgVfsHead = pNemuImgVfs;
                                RTDvmVolumeSetQueryBlockStatusCallback(hVol, nemuimgQueryBlockStatus, hVfs);
                            }
                        }
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        VDIfQueryRangeUse.pfnQueryRangeUse = nemuimgQueryRangeUse;
    ...
    while (pNemuImgVfsHead)
    {
        PNEMUIMGVFS pNemuImgVfsFree = pNemuImgVfsHead;

        pNemuImgVfsHead = pNemuImgVfsHead->pNext;
        RTVfsRelease(pNemuImgVfsFree->hVfs);
        RTMemFree(pNemuImgVfsFree);
    }
