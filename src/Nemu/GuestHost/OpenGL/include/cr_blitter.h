...
/** @todo r=bird: NEMUBLITTERDECL makes no sense. */
#ifndef IN_RING0
# define NEMUBLITTERDECL(_type) DECLEXPORT(_type)
#else
...
NEMUBLITTERDECL(void) CrMClrFillImgRect(PCR_BLITTER_IMG pDst, PCRTRECT pCopyRect, uint32_t u32Color);
NEMUBLITTERDECL(void) CrMClrFillImg(PCR_BLITTER_IMG pImg, uint32_t cRects, PCRTRECT pRects, uint32_t u32Color);
NEMUBLITTERDECL(void) CrMBltImgRect(PCCR_BLITTER_IMG pSrc, PCRTPOINT pSrcDataPoint, bool fSrcInvert, PCRTRECT pCopyRect,
                                    PCR_BLITTER_IMG pDst);
NEMUBLITTERDECL(void) CrMBltImg(PCCR_BLITTER_IMG pSrc, PCRTPOINT pPos, uint32_t cRects, PCRTRECT pRects, PCR_BLITTER_IMG pDst);
NEMUBLITTERDECL(void) CrMBltImgRectScaled(PCCR_BLITTER_IMG pSrc, PCRTPOINT pPos, bool fSrcInvert, PCRTRECT pCopyRect,
                                          float strX, float strY, PCR_BLITTER_IMG pDst);
NEMUBLITTERDECL(void) CrMBltImgScaled(PCCR_BLITTER_IMG pSrc, PCRTRECTSIZE pSrcRectSize, PCRTRECT pDstRect, uint32_t cRects,
...
NEMUBLITTERDECL(bool) CrGlslIsSupported(PCR_GLSL_CACHE pCache);
NEMUBLITTERDECL(int)  CrGlslProgGenAllNoAlpha(PCR_GLSL_CACHE pCache);
NEMUBLITTERDECL(int)  CrGlslProgGenNoAlpha(PCR_GLSL_CACHE pCache, GLenum enmTexTarget);
NEMUBLITTERDECL(int)  CrGlslProgUseGenNoAlpha(PCR_GLSL_CACHE pCache, GLenum enmTexTarget);
NEMUBLITTERDECL(int)  CrGlslProgUseNoAlpha(PCCR_GLSL_CACHE pCache, GLenum enmTexTarget);
NEMUBLITTERDECL(void) CrGlslProgClear(PCCR_GLSL_CACHE pCache);
NEMUBLITTERDECL(bool) CrGlslNeedsCleanup(PCCR_GLSL_CACHE pCache);
NEMUBLITTERDECL(void) CrGlslCleanup(PCR_GLSL_CACHE pCache);
NEMUBLITTERDECL(void) CrGlslTerm(PCR_GLSL_CACHE pCache);
...
typedef DECLCALLBACK(int) FNCRBLT_BLITTER(struct CR_BLITTER *pBlitter, PCNEMUVR_TEXTURE pSrc, PCRTRECT paSrcRect,
...
NEMUBLITTERDECL(int)  CrBltInit(PCR_BLITTER pBlitter, PCCR_BLITTER_CONTEXT pCtxBase, bool fCreateNewCtx,
                                bool fForceDrawBlt, PCCR_GLSL_CACHE pShaders, SPUDispatchTable *pDispatch);

NEMUBLITTERDECL(void) CrBltTerm(PCR_BLITTER pBlitter);
...
NEMUBLITTERDECL(int) CrBltMuralSetCurrentInfo(PCR_BLITTER pBlitter, PCCR_BLITTER_WINDOW pMural);
...
NEMUBLITTERDECL(void) CrBltCheckUpdateViewport(PCR_BLITTER pBlitter);

NEMUBLITTERDECL(void) CrBltLeave(PCR_BLITTER pBlitter);
NEMUBLITTERDECL(int) CrBltEnter(PCR_BLITTER pBlitter);
NEMUBLITTERDECL(void) CrBltBlitTexMural(PCR_BLITTER pBlitter, bool fBb, PCNEMUVR_TEXTURE pSrc, PCRTRECT paSrcRects,
                                        PCRTRECT paDstRects, uint32_t cRects, uint32_t fFlags);
NEMUBLITTERDECL(void) CrBltBlitTexTex(PCR_BLITTER pBlitter, PCNEMUVR_TEXTURE pSrc, PCRTRECT pSrcRect, PCNEMUVR_TEXTURE pDst,
                                      PCRTRECT pDstRect, uint32_t cRects, uint32_t fFlags);
NEMUBLITTERDECL(int) CrBltImgGetTex(PCR_BLITTER pBlitter, PCNEMUVR_TEXTURE pSrc, GLenum enmFormat, PCR_BLITTER_IMG pDst);

NEMUBLITTERDECL(int) CrBltImgGetMural(PCR_BLITTER pBlitter, bool fBb, PCR_BLITTER_IMG pDst);
NEMUBLITTERDECL(void) CrBltImgFree(PCR_BLITTER pBlitter, PCR_BLITTER_IMG pDst);
NEMUBLITTERDECL(void) CrBltPresent(PCR_BLITTER pBlitter);
/* */
struct CR_TEXDATA;
...
DECLINLINE(void) CrTdInit(PCR_TEXDATA pTex, PCNEMUVR_TEXTURE pVrTex, PCR_BLITTER pBlitter, PFNCRTEXDATA_RELEASED pfnTextureReleased)
{
...
DECLINLINE(PCNEMUVR_TEXTURE) CrTdTexGet(PCCR_TEXDATA pTex)
{
    return &pTex->Tex;
}
...
NEMUBLITTERDECL(int) CrTdBltDataAcquire(PCR_TEXDATA pTex, GLenum enmFormat, bool fInverted, PCCR_BLITTER_IMG *ppImg);

NEMUBLITTERDECL(int) CrTdBltDataAcquireScaled(PCR_TEXDATA pTex, GLenum enmFormat, bool fInverted,
                                              uint32_t width, uint32_t height, PCCR_BLITTER_IMG *ppImg);

NEMUBLITTERDECL(int) CrTdBltDataReleaseScaled(PCR_TEXDATA pTex, PCCR_BLITTER_IMG pImg);
