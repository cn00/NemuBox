...
typedef std::map<NemuLockingClass, RTLOCKVALCLASS> LockValidationClassesMap;
...
bool AutoLockHoldsLocksInClass(NemuLockingClass lockClass)
{
#ifdef NEMU_WITH_MAIN_LOCK_VALIDATION
    return RTLockValidatorHoldsLocksInClass(NIL_RTTHREAD,
                                            g_mapLockValidationClasses[lockClass]);
#else /* !NEMU_WITH_MAIN_LOCK_VALIDATION */
    return false;
#endif /* !NEMU_WITH_MAIN_LOCK_VALIDATION */
}
...
RWLockHandle::RWLockHandle(NemuLockingClass lockClass)
{
    m = new Data();
...
WriteLockHandle::WriteLockHandle(NemuLockingClass lockClass)
{
    m = new Data;
