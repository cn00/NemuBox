...
{
    renderspuNemuCompositorSet(window, NULL);
    renderspuNemuPresentBlitterCleanup(window);
...
GLboolean renderspuWinInitWithVisual( WindowInfo *window, VisualInfo *visual, GLboolean showIt, GLint id )
{
    ...
    */
    ...
    if (!renderspu_SystemNemuCreateWindow( visual, showIt, window ))
    {
...
static void RENDER_APIENTRY
renderspuNemuPresentComposition( GLint win, const struct NEMUVR_SCR_COMPOSITOR * pCompositor, const struct NEMUVR_SCR_COMPOSITOR_ENTRY *pChangedEntry )
{
    WindowInfo *window;
    CRASSERT(win >= 0);
    window = (WindowInfo *) crHashtableSearch(render_spu.windowTable, win);
    if (window) {
        if (renderspuNemuCompositorSet(window, pCompositor))
        {
            renderspu_SystemNemuPresentComposition(window, pChangedEntry);
        }
    }
    else {
...
void renderspuNemuCompositorBlitStretched ( const struct NEMUVR_SCR_COMPOSITOR * pCompositor, PCR_BLITTER pBlitter, GLfloat scaleX, GLfloat scaleY)
{
...
void renderspuNemuCompositorBlit ( const struct NEMUVR_SCR_COMPOSITOR * pCompositor, PCR_BLITTER pBlitter)
{
    NEMUVR_SCR_COMPOSITOR_CONST_ITERATOR CIter;
    const NEMUVR_SCR_COMPOSITOR_ENTRY *pEntry;
    CrVrScrCompositorConstIterInit(pCompositor, &CIter);
...
void renderspuNemuPresentBlitterCleanup( WindowInfo *window )
{
    if (!window->pBlitter)
        return;
...
PCR_BLITTER renderspuNemuPresentBlitterGet( WindowInfo *window )
{
...
int renderspuNemuPresentBlitterEnter( PCR_BLITTER pBlitter, int32_t i32MakeCurrentUserData)
{
    int rc;
...
PCR_BLITTER renderspuNemuPresentBlitterGetAndEnter( WindowInfo *window, int32_t i32MakeCurrentUserData, bool fRedraw )
{
    PCR_BLITTER pBlitter = fRedraw ? window->pBlitter : renderspuNemuPresentBlitterGet(window);
    if (pBlitter)
    {
        int rc = renderspuNemuPresentBlitterEnter(pBlitter, i32MakeCurrentUserData);
        if (RT_SUCCESS(rc))
        {
            return pBlitter;
        }
...
PCR_BLITTER renderspuNemuPresentBlitterEnsureCreated( WindowInfo *window, int32_t i32MakeCurrentUserData )
{
    if (!window->pBlitter)
    {
        const struct NEMUVR_SCR_COMPOSITOR * pTmpCompositor;
        /* just use compositor lock to synchronize */
        pTmpCompositor = renderspuNemuCompositorAcquire(window);
        CRASSERT(pTmpCompositor);
        if (pTmpCompositor)
        {
            PCR_BLITTER pBlitter = renderspuNemuPresentBlitterGet( window );
            if (pBlitter)
            {
                if (!CrBltIsEverEntered(pBlitter))
                {
                    ...
                    {
                        crWarning("renderspuNemuPresentBlitterEnter failed rc %d", rc);
                    }
                }
            }
            else
            {
                crWarning("renderspuNemuPresentBlitterGet failed");
            }

            renderspuNemuCompositorRelease(window);
        }
        else
        {
            crWarning("renderspuNemuCompositorAcquire failed");
        }
    }
    return window->pBlitter;
}

void renderspuNemuPresentCompositionGeneric( WindowInfo *window, const struct NEMUVR_SCR_COMPOSITOR * pCompositor,
        const struct NEMUVR_SCR_COMPOSITOR_ENTRY *pChangedEntry, int32_t i32MakeCurrentUserData,
        bool fRedraw )
{
    PCR_BLITTER pBlitter = renderspuNemuPresentBlitterGetAndEnter(window, i32MakeCurrentUserData, fRedraw);
    if (!pBlitter)
        return;

    renderspuNemuCompositorBlit(pCompositor, pBlitter);
...
GLboolean renderspuNemuCompositorSet( WindowInfo *window, const struct NEMUVR_SCR_COMPOSITOR * pCompositor)
{
    int rc;
    GLboolean fEmpty = pCompositor && CrVrScrCompositorIsEmpty(pCompositor);
    GLboolean fNeedPresent;

    /* renderspuNemuCompositorSet can be invoked from the chromium thread only and is not reentrant,
...
static void renderspuNemuCompositorClearAllCB(unsigned long key, void *data1, void *data2)
{
    WindowInfo *window = (WindowInfo *) data1;
    renderspuNemuCompositorSet(window, NULL);
}

void renderspuNemuCompositorClearAll()
{
     ...
     * we are can be sure noone will modify the table here since renderspuNemuCompositorClearAll can be called in the command (hgcm) thread only,
     * and the table can be modified from that thread only as well */
    crHashtableWalkUnlocked(render_spu.windowTable, renderspuNemuCompositorClearAllCB, NULL);
}

const struct NEMUVR_SCR_COMPOSITOR * renderspuNemuCompositorAcquire( WindowInfo *window)
{
    int rc = RTCritSectEnter(&window->CompositorLock);
    if (RT_SUCCESS(rc))
    {
        const NEMUVR_SCR_COMPOSITOR * pCompositor = window->pCompositor;
        if (pCompositor)
        {
...
int renderspuNemuCompositorLock(WindowInfo *window, const struct NEMUVR_SCR_COMPOSITOR **ppCompositor)
{
    int rc = RTCritSectEnter(&window->CompositorLock);
    if (RT_SUCCESS(rc))
    {
...
int renderspuNemuCompositorUnlock(WindowInfo *window)
{
...
int renderspuNemuCompositorTryAcquire(WindowInfo *window, const struct NEMUVR_SCR_COMPOSITOR **ppCompositor)
{
...
void renderspuNemuCompositorRelease( WindowInfo *window)
{
    int rc;
...
renderspuCreateFunctions(SPUNamedFunctionTable table[])
{
    ...
    FILLIN( "NemuPresentComposition", renderspuNemuPresentComposition );
    return i;
}
