...
/** @file
 *
 * Nemu frontends: NemuSDL (simple frontend based on SDL):
 * Implementation of NemuSDLFB (SDL framebuffer) class
 */
...
NS_IMPL_THREADSAFE_ISUPPORTS1_CI(NemuSDLFB, IFramebuffer)
NS_DECL_CLASSINFO(NemuSDLFB)
NS_IMPL_THREADSAFE_ISUPPORTS2_CI(NemuSDLFBOverlay, IFramebufferOverlay, IFramebuffer)
NS_DECL_CLASSINFO(NemuSDLFBOverlay)
#endif

#ifdef NEMU_SECURELABEL
/* function pointers */
extern "C"
{
...
HRESULT NemuSDLFB::FinalConstruct()
{
    return 0;
}

void NemuSDLFB::FinalRelease()
{
    return;
}
 ...
 */
HRESULT NemuSDLFB::init(uint32_t uScreenId,
...
bool NemuSDLFB::init(bool fShowSDLConfig)
{
    LogFlow(("NemuSDLFB::init\n"));
 ...
 */
STDMETHODIMP NemuSDLFB::COMGETTER(Width)(ULONG *width)
{
    LogFlow(("NemuSDLFB::GetWidth\n"));
    if (!width)
        return E_INVALIDARG;
    *width = mGuestXRes;
    return S_OK;
 ...
 */
STDMETHODIMP NemuSDLFB::COMGETTER(Height)(ULONG *height)
{
    LogFlow(("NemuSDLFB::GetHeight\n"));
    if (!height)
        return E_INVALIDARG;
    *height = mGuestYRes;
    return S_OK;
 ...
 */
STDMETHODIMP NemuSDLFB::COMGETTER(BitsPerPixel)(ULONG *bitsPerPixel)
{
    LogFlow(("NemuSDLFB::GetBitsPerPixel\n"));
    if (!bitsPerPixel)
        return E_INVALIDARG;
 ...
 */
STDMETHODIMP NemuSDLFB::COMGETTER(BytesPerLine)(ULONG *bytesPerLine)
{
    LogFlow(("NemuSDLFB::GetBytesPerLine\n"));
    if (!bytesPerLine)
        return E_INVALIDARG;
...
STDMETHODIMP NemuSDLFB::COMGETTER(PixelFormat) (BitmapFormat_T *pixelFormat)
{
    if (!pixelFormat)
        return E_POINTER;
 ...
 */
STDMETHODIMP NemuSDLFB::COMGETTER(HeightReduction)(ULONG *heightReduction)
{
    if (!heightReduction)
        return E_POINTER;
#ifdef NEMU_SECURELABEL
 ...
 */
STDMETHODIMP NemuSDLFB::COMGETTER(Overlay)(IFramebufferOverlay **aOverlay)
{
    if (!aOverlay)
        return E_POINTER;
    /* Not yet implemented */
 ...
 */
STDMETHODIMP NemuSDLFB::COMGETTER(WinId)(LONG64 *winId)
{
    if (!winId)
        return E_POINTER;
#ifdef RT_OS_DARWIN
    if (mWinId == NULL) /* (In case it failed the first time.) */
        mWinId = (intptr_t)NemuSDLGetDarwinWindowId();
#endif
    *winId = mWinId;
    return S_OK;
}

STDMETHODIMP NemuSDLFB::COMGETTER(Capabilities)(ComSafeArrayOut(FramebufferCapabilities_T, aCapabilities))
{
 ...
 *                   has to return immediately) or Nemu should wait
 *                   for a call to the update complete API before
 *                   continuing with display updates.
 */
STDMETHODIMP NemuSDLFB::NotifyUpdate(ULONG x, ULONG y,
                                     ULONG w, ULONG h)
{
    /*
     * The input values are in guest screen coordinates.
     */
    LogFlow(("NemuSDLFB::NotifyUpdate: x = %d, y = %d, w = %d, h = %d\n",
             x, y, w, h));
...
#else /* !NEMUSDL_WITH_X11 */
    update(x, y, w, h, true /* fGuestRelative */);
#endif /* !NEMUSDL_WITH_X11 */

    return S_OK;
}

STDMETHODIMP NemuSDLFB::NotifyUpdateImage(ULONG aX,
                                          ULONG aY,
                                          ULONG aWidth,
                                          ULONG aHeight,
...
STDMETHODIMP NemuSDLFB::NotifyChange(ULONG aScreenId,
                                     ULONG aXOrigin,
                                     ULONG aYOrigin,
                                     ULONG aWidth,
                                     ULONG aHeight)
 ...
 */
STDMETHODIMP NemuSDLFB::VideoModeSupported(ULONG width, ULONG height, ULONG bpp, BOOL *supported)
{
    if (!supported)
        return E_POINTER;
...
#ifdef DEBUG
        printf("NemuSDL::VideoModeSupported: we refused mode %dx%dx%d\n", width, height, bpp);
#endif
        *supported = false;
    }
    else
...
STDMETHODIMP NemuSDLFB::GetVisibleRegion(BYTE *aRectangles, ULONG aCount,
                                         ULONG *aCountCopied)
{
...
STDMETHODIMP NemuSDLFB::SetVisibleRegion(BYTE *aRectangles, ULONG aCount)
{
    PRTRECT rects = (PRTRECT)aRectangles;
...
STDMETHODIMP NemuSDLFB::ProcessVHWACommand(BYTE *pCommand)
{
    return E_NOTIMPL;
}

STDMETHODIMP NemuSDLFB::Notify3DEvent(ULONG uType, ComSafeArrayIn(BYTE, aData))
{
    return E_NOTIMPL;
}
...
void NemuSDLFB::notifyChange(ULONG aScreenId)
{
    /* Disable screen updates. */
    RTCritSectEnter(&mUpdateLock);
...
void NemuSDLFB::resizeGuest()
{
    ...
    if (mPtrVRAM)
    {
        ...
        LogFlow(("NemuSDL:: using the source bitmap\n"));
    }
    else
    {
        mSurfVRAM = SDL_CreateRGBSurface(SDL_SWSURFACE, mGuestXRes, mGuestYRes, mBitsPerPixel,
                                         Rmask, Gmask, Bmask, Amask);
        LogFlow(("NemuSDL:: using SDL_SWSURFACE\n"));
    }
    LogFlow(("NemuSDL:: created VRAM surface %p\n", mSurfVRAM));

    if (mfSameSizeRequested)
    {
        mfSameSizeRequested = false;
        LogFlow(("NemuSDL:: the same resolution requested, skipping the resize.\n"));
    }
    else
    {
 ...
 * @remarks Must be called from the SDL thread!
 */
void NemuSDLFB::resizeSDL(void)
{
    LogFlow(("NemuSDL:resizeSDL\n"));
 ...
 */
void NemuSDLFB::update(int x, int y, int w, int h, bool fGuestRelative)
{
#ifdef NEMUSDL_WITH_X11
    AssertMsg(gSdlNativeThread == RTThreadNativeSelf(), ("Wrong thread! SDL is not threadsafe!\n"));
#endif
 ...
 * @remarks Must be called from the SDL thread!
 */
void NemuSDLFB::repaint()
{
    AssertMsg(gSdlNativeThread == RTThreadNativeSelf(), ("Wrong thread! SDL is not threadsafe!\n"));
    LogFlow(("NemuSDLFB::repaint\n"));
 ...
 * @remarks Must be called from the SDL thread!
 */
void NemuSDLFB::setFullscreen(bool fFullscreen)
{
    AssertMsg(gSdlNativeThread == RTThreadNativeSelf(), ("Wrong thread! SDL is not threadsafe!\n"));
    LogFlow(("NemuSDLFB::SetFullscreen: fullscreen: %d\n", fFullscreen));
    mfFullscreen = fFullscreen;
 ...
 */
void NemuSDLFB::getFullscreenGeometry(uint32_t *width, uint32_t *height)
{
    SDL_Rect **modes;
 ...
 */
int NemuSDLFB::initSecureLabel(uint32_t height, char *font, uint32_t pointsize, uint32_t labeloffs)
{
    LogFlow(("NemuSDLFB:initSecureLabel: new offset: %d pixels, new font: %s, new pointsize: %d\n",
 ...
 * @remarks must be called from the SDL thread!
 */
void NemuSDLFB::setSecureLabelText(const char *text)
{
 ...
 * @remarks must be called from the SDL thread!
 */
void NemuSDLFB::setSecureLabelColor(uint32_t colorFG, uint32_t colorBG)
{
 ...
 * @remarks must be called from the SDL thread!
 */
void NemuSDLFB::paintSecureLabel(int x, int y, int w, int h, bool fForce)
{
#ifdef NEMUSDL_WITH_X11
    AssertMsg(gSdlNativeThread == RTThreadNativeSelf(), ("Wrong thread! SDL is not threadsafe!\n"));
#endif
...
/**
 * Constructor for the NemuSDLFBOverlay class (IFramebufferOverlay implementation)
 *
 ...
 */
NemuSDLFBOverlay::NemuSDLFBOverlay(ULONG x, ULONG y, ULONG width, ULONG height,
                                   BOOL visible, NemuSDLFB *aParent) :
...
/**
 * Destructor for the NemuSDLFBOverlay class.
 */
NemuSDLFBOverlay::~NemuSDLFBOverlay()
{
 ...
 */
STDMETHODIMP NemuSDLFBOverlay::COMGETTER(X)(ULONG *x)
{
    LogFlow(("NemuSDLFBOverlay::GetX\n"));
    if (!x)
        return E_INVALIDARG;
    *x = mOverlayX;
    return S_OK;
 ...
 * @param   height Address of result buffer.
 */
STDMETHODIMP NemuSDLFBOverlay::COMGETTER(Y)(ULONG *y)
{
    LogFlow(("NemuSDLFBOverlay::GetY\n"));
    if (!y)
        return E_INVALIDARG;
    *y = mOverlayY;
    return S_OK;
 ...
 * @param   width Address of result buffer.
 */
STDMETHODIMP NemuSDLFBOverlay::COMGETTER(Width)(ULONG *width)
{
    LogFlow(("NemuSDLFBOverlay::GetWidth\n"));
    if (!width)
        return E_INVALIDARG;
 ...
 */
STDMETHODIMP NemuSDLFBOverlay::COMGETTER(BytesPerLine)(ULONG *bytesPerLine)
{
    LogFlow(("NemuSDLFBOverlay::GetBytesPerLine\n"));
    if (!bytesPerLine)
        return E_INVALIDARG;
 ...
 * @param   height Address of result buffer.
 */
STDMETHODIMP NemuSDLFBOverlay::COMGETTER(Height)(ULONG *height)
{
    LogFlow(("NemuSDLFBOverlay::GetHeight\n"));
    if (!height)
        return E_INVALIDARG;
 ...
 */
STDMETHODIMP NemuSDLFBOverlay::COMGETTER(Visible)(BOOL *visible)
{
    LogFlow(("NemuSDLFBOverlay::GetVisible\n"));
    if (!visible)
        return E_INVALIDARG;
 ...
 */
STDMETHODIMP NemuSDLFBOverlay::COMSETTER(Visible)(BOOL visible)
{
    LogFlow(("NemuSDLFBOverlay::SetVisible\n"));
 ...
 */
STDMETHODIMP NemuSDLFBOverlay::COMGETTER(Alpha)(ULONG *alpha)
{
    LogFlow(("NemuSDLFBOverlay::GetAlpha\n"));
    return E_NOTIMPL;
}
 ...
 */
STDMETHODIMP NemuSDLFBOverlay::COMSETTER(Alpha)(ULONG alpha)
{
    LogFlow(("NemuSDLFBOverlay::SetAlpha\n"));
    return E_NOTIMPL;
}
 ...
 * @param   alpha Address of result buffer.
 */
STDMETHODIMP NemuSDLFBOverlay::COMGETTER(Address)(ULONG *address)
{
    LogFlow(("NemuSDLFBOverlay::GetAddress\n"));
    if (!address)
        return E_INVALIDARG;
 ...
 */
STDMETHODIMP NemuSDLFBOverlay::COMGETTER(BitsPerPixel)(ULONG *bitsPerPixel)
{
    LogFlow(("NemuSDLFBOverlay::GetBitsPerPixel\n"));
    if (!bitsPerPixel)
        return E_INVALIDARG;
    *bitsPerPixel = 32;
    return S_OK;
 ...
 */
STDMETHODIMP NemuSDLFBOverlay::COMGETTER(PixelFormat)(ULONG *pixelFormat)
{
    LogFlow(("NemuSDLFBOverlay::GetPixelFormat\n"));
    if (!pixelFormat)
        return E_INVALIDARG;
    *pixelFormat = BitmapFormat_BGR;
    return S_OK;
 ...
 */
STDMETHODIMP NemuSDLFBOverlay::COMGETTER(UsesGuestVRAM)(BOOL *usesGuestVRAM)
{
    LogFlow(("NemuSDLFBOverlay::GetUsesGuestVRAM\n"));
    if (!usesGuestVRAM)
        return E_INVALIDARG;
    *usesGuestVRAM = FALSE;
    return S_OK;
 ...
 */
STDMETHODIMP NemuSDLFBOverlay::COMGETTER(HeightReduction)(ULONG *heightReduction)
{
    LogFlow(("NemuSDLFBOverlay::GetHeightReduction\n"));
    if (!heightReduction)
        return E_INVALIDARG;
    *heightReduction = 0;
    return S_OK;
 ...
 */
STDMETHODIMP NemuSDLFBOverlay::COMGETTER(Overlay)(IFramebufferOverlay **aOverlay)
{
    LogFlow(("NemuSDLFBOverlay::GetOverlay\n"));
    if (!aOverlay)
        return E_INVALIDARG;
    *aOverlay = 0;
    return S_OK;
 ...
 */
STDMETHODIMP NemuSDLFBOverlay::COMGETTER(WinId)(LONG64 *winId)
{
    LogFlow(("NemuSDLFBOverlay::GetWinId\n"));
    if (!winId)
        return E_INVALIDARG;
    *winId = 0;
    return S_OK;
 ...
 * @returns COM status code
 */
STDMETHODIMP NemuSDLFBOverlay::Lock()
{
    LogFlow(("NemuSDLFBOverlay::Lock\n"));
 ...
 * @returns COM status code
 */
STDMETHODIMP NemuSDLFBOverlay::Unlock()
{
    LogFlow(("NemuSDLFBOverlay::Unlock\n"));
    AssertMsgFailed(("You should not attempt to lock an IFramebufferOverlay object -\n"
                     "lock the parent IFramebuffer object instead.\n"));
    return E_NOTIMPL;
}
 ...
 */
STDMETHODIMP NemuSDLFBOverlay::Move(ULONG x, ULONG y)
{
    mOverlayX = x;
    mOverlayY = y;
    return S_OK;
 ...
 */
STDMETHODIMP NemuSDLFBOverlay::NotifyUpdate(ULONG x, ULONG y,
                            ULONG w, ULONG h)
{
 ...
 * @retval  finished    Set if the operation has completed.
 */
STDMETHODIMP NemuSDLFBOverlay::RequestResize(ULONG aScreenId, ULONG pixelFormat, ULONG vram,
 ...
 */
STDMETHODIMP NemuSDLFBOverlay::VideoModeSupported(ULONG width, ULONG height, ULONG bpp,
                                               BOOL *supported)
{
    if (!supported)
        return E_POINTER;
