...
# ifdef MOZ_DEFAULT_NEMU_XPCOM_HOME
    rv = localFile->InitWithNativePath(nsDependentCString(MOZ_DEFAULT_NEMU_XPCOM_HOME));
    if (NS_SUCCEEDED(rv))
        *aFile = localFile;
# else
    ...
    //	- if NEMU_XPCOM_HOME is defined, that is it
    ...
    if (!fPathSet)
    {
        ...
        /*
         * Env.var. NEMU_XPCOM_HOME first.
         */
        char *psz = PR_GetEnv("NEMU_XPCOM_HOME");
        if (psz)
        {
        ...
        if (buf2[0])
        {
            ...
            if (p)
            {
                p[p == buf2] = '\0';
            #ifdef DEBUG
                printf("debug: (1) NEMU_XPCOM_HOME=%s\n", buf2);
            #endif
                strcpy(szPath, buf2);
                fPathSet = true;
            }
    ...
    // The MOZ_DEFAULT_NEMU_XPCOM_HOME variable can be set at configure time with
    // a --with-default-mozilla-five-home=foo autoconf flag.
    //
    // The idea here is to allow for builds that have a default NEMU_XPCOM_HOME
...
#ifdef MOZ_DEFAULT_NEMU_XPCOM_HOME
    if (PR_GetEnv("NEMU_XPCOM_HOME") == nsnull)
    {
        putenv("NEMU_XPCOM_HOME=" MOZ_DEFAULT_NEMU_XPCOM_HOME);
    }
#endif

    char *moz5 = PR_GetEnv("NEMU_XPCOM_HOME");
    ...
    if(!moz5 && firstWarning) {
        // Warn that NEMU_XPCOM_HOME not set, once.
        printf("Warning: NEMU_XPCOM_HOME not set.\n");
    ...
    *aFile = localFile;
    ...
    char *moz5 = getenv("NEMU_XPCOM_HOME");
    if (moz5)
    {
