/* $Id: DrvVD.cpp $ */
/** @file
 * DrvVD - Generic Nemu disk media driver.
 */
...
#include <Nemu/vmm/pdmblkcache.h>
#include <iprt/asm.h>
...
/** Converts a pointer to NEMUDISK::IMedia to a PNEMUDISK. */
#define PDMIMEDIA_2_NEMUDISK(pInterface) \
    ( (PNEMUDISK)((uintptr_t)pInterface - RT_OFFSETOF(NEMUDISK, IMedia)) )

/** Converts a pointer to NEMUDISK::IMediaAsync to a PNEMUDISK. */
#define PDMIMEDIAASYNC_2_NEMUDISK(pInterface) \
    ( (PNEMUDISK)((uintptr_t)pInterface - RT_OFFSETOF(NEMUDISK, IMediaAsync)) )

/**
 * Nemu disk container, image information, private part.
 */

typedef struct NEMUIMAGE
{
    /** Pointer to next image. */
    struct NEMUIMAGE    *pNext;
...
/**
 * Nemu disk container media main structure, private part.
 *
 ...
 */
typedef struct NEMUDISK
{
    /** The Nemu disk container. */
    PNEMUHDD                 pDisk;
    /** The media interface. */
    PDMIMEDIA                IMedia;
    /** Media port. */
 ...
 */
static PNEMUIMAGE drvvdNewImage(PNEMUDISK pThis)
{
    AssertPtr(pThis);
    PNEMUIMAGE pImage = (PNEMUIMAGE)RTMemAllocZ(sizeof(NEMUIMAGE));
    if (pImage)
    {
        pImage->pVDIfsImage = NULL;
        PNEMUIMAGE *pp = &pThis->pImages;
        while (*pp != NULL)
            pp = &(*pp)->pNext;
        *pp = pImage;
        pImage->pNext = NULL;
 ...
 */
static void drvvdFreeImages(PNEMUDISK pThis)
{
    while (pThis->pImages != NULL)
    {
        PNEMUIMAGE p = pThis->pImages;
 ...
 */
static int drvvdSetReadonly(PNEMUDISK pThis)
{
    int rc = VINF_SUCCESS;
 ...
 * @param   pThis               The driver instance data.
 */
static int drvvdSetWritable(PNEMUDISK pThis)
{
    int rc = VINF_SUCCESS;
                               ...
                               const char *pszFormat, va_list va)
{
    ...
    PNEMUDISK pThis = PDMINS_2_DATA(pDrvIns, PNEMUDISK);
...
static DECLCALLBACK(int) drvvdAsyncIOOpen(void *pvUser, const char *pszLocation,
                                          ...
                                          void **ppStorage)
{
    PNEMUDISK pThis = (PNEMUDISK)pvUser;
...
{
    PNEMUDISK pDrvVD = (PNEMUDISK)pvUser;
    PDRVVDSTORAGEBACKEND pStorageBackend = (PDRVVDSTORAGEBACKEND)pStorage;
...
#endif /* NEMU_WITH_PDM_ASYNC_COMPLETION */
 ...
 */
static int drvvdKeyCheckPrereqs(PNEMUDISK pThis)
{
    if (   pThis->pCfgCrypto
        && !pThis->pIfSecKey)
    {
                                   ...
                                   uint64_t off, void *pvBuf, size_t cbRead)
{
    ...
    PNEMUDISK pThis = PDMIMEDIA_2_NEMUDISK(pInterface);
...
{
    PNEMUDISK pThis = (PNEMUDISK)pvUser1;
                                        ...
                                        size_t cbRead, void *pvUser)
{
    ...
    int rc = VINF_SUCCESS;
    PNEMUDISK pThis = PDMIMEDIAASYNC_2_NEMUDISK(pInterface);
 ...
 */
static int drvvdLoadPlugins(PNEMUDISK pThis, PCFGMNODE pCfg)
{
    int rc = VINF_SUCCESS;
 ...
 */
static int drvvdSetupFilters(PNEMUDISK pThis, PCFGMNODE pCfg)
{
    int rc = VINF_SUCCESS;
...
/**
 * Construct a Nemu disk media driver instance.
 *
...
static DECLCALLBACK(int) drvvdConstruct(PPDMDRVINS pDrvIns, PCFGMNODE pCfg, uint32_t fFlags)
{
    ...
    while (pCurNode && RT_SUCCESS(rc))
    {
        ...
        PNEMUIMAGE pImage = drvvdNewImage(pThis);
        if (!pImage)
        {
            rc = VERR_NO_MEMORY;
            break;
...
#else /* !NEMU_WITH_PDM_ASYNC_COMPLETION */
            rc = PDMDrvHlpVMSetError(pDrvIns, VERR_PDM_DRVINS_UNKNOWN_CFG_VALUES,
...
/**
 * Nemu disk container media driver registration record.
 */
const PDMDRVREG g_DrvVD =
{
    ...
    "",
    /* pszDescription */
    "Generic Nemu disk media driver.",
    /* fFlags */
