...
/** @file
 * Nemu Miniport IOCTL related header
 */
...
#ifndef NEMUVIDEOIOCTL_H
#define NEMUVIDEOIOCTL_H
...
#define IOCTL_VIDEO_QUERY_NEMUVIDEO_INFO \
    CTL_CODE(FILE_DEVICE_VIDEO, 0x436, METHOD_BUFFERED, FILE_ANY_ACCESS)

/* ==================== Data structures used by Virtual Box VRPS's ==================== */
typedef void* HNEMUVIDEOHGSMI;

/* Complete host commands addressed to the display */
typedef DECLCALLBACK(void) FNNEMUVIDEOHGSMICOMPLETION(HNEMUVIDEOHGSMI hHGSMI, struct VBVAHOSTCMD * pCmd);
typedef FNNEMUVIDEOHGSMICOMPLETION *PFNNEMUVIDEOHGSMICOMPLETION;

/* request the host commands addressed to the display */
typedef DECLCALLBACK(int) FNNEMUVIDEOHGSMICOMMANDS(HNEMUVIDEOHGSMI hHGSMI, uint8_t u8Channel, uint32_t iDevice, struct VBVAHOSTCMD ** ppCmd);
typedef FNNEMUVIDEOHGSMICOMMANDS *PFNNEMUVIDEOHGSMICOMMANDS;

/* post guest command (offset) to the host */
typedef DECLCALLBACK(void) FNNEMUVIDEOHGSMIPOSTCOMMAND(HNEMUVIDEOHGSMI hHGSMI, HGSMIOFFSET offCmd);
typedef FNNEMUVIDEOHGSMIPOSTCOMMAND *PFNNEMUVIDEOHGSMIPOSTCOMMAND;
...
{
    HNEMUVIDEOHGSMI hContext;
    PFNNEMUVIDEOHGSMICOMPLETION pfnCompletionHandler;
    PFNNEMUVIDEOHGSMICOMMANDS   pfnRequestCommandsHandler;
} HGSMIQUERYCALLBACKS;
...
#define NEMUVIDEO_INFO_LEVEL_REGISTRY_FLAGS 1

#define NEMUVIDEO_REGISTRY_FLAGS_DISABLE_BITMAP_CACHE 0x00000001

#endif /*NEMUVIDEOIOCTL_H*/
