; $Id$
; @file
; NemuGuestAdditionsCommon.nsh - Common / shared utility functions.
;
...
!ifdef NEMU_WITH_LICENSE_INSTALL_RTF
  ; Copy license file (if any) into the installation directory
  FILE "/oname=${LICENSE_FILE_RTF}" "$%NEMU_BRAND_LICENSE_RTF%"
!endif

  FILE "$%NEMU_PATH_DIFX%\DIFxAPI.dll"
  FILE "$%PATH_OUT%\bin\additions\NemuDrvInst.exe"

  FILE "$%PATH_OUT%\bin\additions\NemuVideo.inf"
!ifdef NEMU_SIGN_ADDITIONS
  FILE "$%PATH_OUT%\bin\additions\NemuVideo.cat"
!endif
...
Function ExtractFiles
  ...
  ; Root files
  SetOutPath "$0"
!if $%NEMU_WITH_LICENSE_INSTALL_RTF% == "1"
  FILE "/oname=${LICENSE_FILE_RTF}" "$%NEMU_BRAND_LICENSE_RTF%"
!endif

  ; Video driver
  SetOutPath "$0\NemuVideo"
...
!if $%NEMU_WITH_CROGL% == "1"
  ...
  !if $%BUILD_TARGET_ARCH% == "amd64"
    ...
    SetOutPath "$0\NemuVideo\OpenGL\SysWow64"
    FILE "$%NEMU_PATH_ADDITIONS_WIN_X86%\d3d8.dll"
    FILE "$%NEMU_PATH_ADDITIONS_WIN_X86%\d3d9.dll"
    FILE "$%NEMU_PATH_ADDITIONS_WIN_X86%\NemuOGLarrayspu.dll"
    FILE "$%NEMU_PATH_ADDITIONS_WIN_X86%\NemuOGLcrutil.dll"
    FILE "$%NEMU_PATH_ADDITIONS_WIN_X86%\NemuOGLerrorspu.dll"
    FILE "$%NEMU_PATH_ADDITIONS_WIN_X86%\NemuOGLpackspu.dll"
    FILE "$%NEMU_PATH_ADDITIONS_WIN_X86%\NemuOGLpassthroughspu.dll"
    FILE "$%NEMU_PATH_ADDITIONS_WIN_X86%\NemuOGLfeedbackspu.dll"
    FILE "$%NEMU_PATH_ADDITIONS_WIN_X86%\NemuOGL.dll"
...
!if $%BUILD_TARGET_ARCH% == "x86"
  SetOutPath "$0\NemuMouse\NT4"
  FILE "$%PATH_OUT%\bin\additions\NemuMouseNT.sys"
!endif

  ; Guest driver
  SetOutPath "$0\NemuGuest"
...
!endif
  ...
  ; Shared Folders
  SetOutPath "$0\NemuSF"
  FILE "$%PATH_OUT%\bin\additions\NemuSF.sys"
  FILE "$%PATH_OUT%\bin\additions\NemuMRXNP.dll"
  !if $%BUILD_TARGET_ARCH% == "amd64"
    ; Only 64-bit installer: Also copy 32-bit DLLs on 64-bit target
    FILE "$%PATH_OUT%\bin\additions\NemuMRXNP-x86.dll"
  !endif

  ; Auto-Logon
  SetOutPath "$0\AutoLogon"
  FILE "$%PATH_OUT%\bin\additions\NemuGINA.dll"
  FILE "$%PATH_OUT%\bin\additions\NemuCredProv.dll"
...
Function ${un}GetAdditionsVersion
  ...
  ReadRegStr $0 HKLM "SOFTWARE\$%NEMU_VENDOR_SHORT%\VirtualBox Guest Additions" "Version"

  ; Get revision
  ReadRegStr $g_strAddVerRev HKLM "SOFTWARE\$%NEMU_VENDOR_SHORT%\VirtualBox Guest Additions" "Revision"
...
!macro StopNemuService un
Function ${un}StopNemuService

  Push $0   ; Temp results
  Push $1
  Push $2   ; Image name of NemuService
  Push $3   ; Safety counter

  StrCpy $3 "0" ; Init counter
  ${LogVerbose} "Stopping NemuService ..."

svc_stop:

  ${LogVerbose} "Stopping NemuService via SCM ..."
  ${If} $g_strWinVersion == "NT4"
    nsExec::Exec '"$SYSDIR\net.exe" stop NemuService'
  ${Else}
    nsExec::Exec '"$SYSDIR\SC.exe" stop NemuService'
  ${EndIf}
...
!ifdef _DEBUG
  ${LogVerbose} "Stopping NemuService (as exe) ..."
!endif
...
!endif
  ...
  ${If} $g_strWinVersion == "NT4"
    StrCpy $2 "NemuServiceNT.exe"
  ${Else}
    StrCpy $2 "NemuService.exe"
  ${EndIf}
...
exit:
  ...
  ${LogVerbose} "Stopping NemuService done"
...
FunctionEnd
!macroend
!insertmacro StopNemuService ""
!insertmacro StopNemuService "un."

!macro StopNemuTray un
Function ${un}StopNemuTray

  Push $0   ; Temp results
  Push $1   ; Safety counter

  StrCpy $1 "0" ; Init counter
  ${LogVerbose} "Stopping NemuTray ..."
...
exe_stop:
  ...
  ${nsProcess::FindProcess} "NemuTray.exe" $0
  StrCmp $0 0 0 exit

  ${nsProcess::KillProcess} "NemuTray.exe" $0
  Sleep "1000"           ; Wait a bit
  Goto exe_stop

exit:

  ${LogVerbose} "Stopping NemuTray done"
...
FunctionEnd
!macroend
!insertmacro StopNemuTray ""
!insertmacro StopNemuTray "un."

!macro StopNemuMMR un
Function ${un}StopNemuMMR

  Push $0   ; Temp results
  Push $1   ; Safety counter

  StrCpy $1 "0" ; Init counter
  DetailPrint "Stopping NemuMMR ..."
...
exe_stop:
  ...
  ${nsProcess::FindProcess} "NemuMMR.exe" $0
  StrCmp $0 0 0 exit

  ${nsProcess::KillProcess} "NemuMMR.exe" $0
  Sleep "1000"           ; Wait a bit
  Goto exe_stop

exit:

  DetailPrint "Stopping NemuMMR done."
...
FunctionEnd
!macroend
!insertmacro StopNemuMMR ""
!insertmacro StopNemuMMR "un."
...
found:
  ...
  NemuGuestInstallHelper::FileGetVendor "$0"
  ; Stack: <vendor> $1 $0
  Pop  $0 ; Get vendor
  Pop  $1 ; Restore $1
...
found:
  ...
  NemuGuestInstallHelper::FileGetArchitecture "$0"
...
!if $%NEMU_WITH_GUEST_INSTALL_HELPER% == "1"
  !ifdef WFP_FILE_EXCEPTION
    NemuGuestInstallHelper::DisableWFP "$0"
...
; a Nemu XPDM driver installation before. This might be necessary for upgrading a
...
!endif
  ...
  ${If} $0 == "1" ; D3D files are invalid
    ${LogVerbose} $(NEMU_UNINST_UNABLE_TO_RESTORE_D3D)
    MessageBox MB_ICONSTOP|MB_OK $(NEMU_UNINST_UNABLE_TO_RESTORE_D3D) /SD IDOK
  ${EndIf}
