                                         ...
                                         LONG aResultDetail /* = 0*/)
{
    ...
    if (aText.isEmpty())
    {
        ...
        switch (aResultCode)
        {
            ...
            case NEMU_E_OBJECT_NOT_FOUND:      aText = "Object corresponding to the supplied arguments does not exist"; break;
            case NEMU_E_INVALID_VM_STATE:      aText = "Current virtual machine state prevents the operation"; break;
            case NEMU_E_VM_ERROR:              aText = "Virtual machine error occurred attempting the operation"; break;
            case NEMU_E_FILE_ERROR:            aText = "File not accessible or erroneous file contents"; break;
            case NEMU_E_IPRT_ERROR:            aText = "Runtime subsystem error"; break;
            case NEMU_E_PDM_ERROR:             aText = "Pluggable Device Manager error"; break;
            case NEMU_E_INVALID_OBJECT_STATE:  aText = "Current object state prohibits operation"; break;
            case NEMU_E_HOST_ERROR:            aText = "Host operating system related error"; break;
            case NEMU_E_NOT_SUPPORTED:         aText = "Requested operation is not supported"; break;
            case NEMU_E_XML_ERROR:             aText = "Invalid XML found"; break;
            case NEMU_E_INVALID_SESSION_STATE: aText = "Current session state prohibits operation"; break;
            case NEMU_E_OBJECT_IN_USE:         aText = "Object being in use prohibits operation"; break;
...
/**
 * Converts the Nemu status code a COM one and sets the error info.
 *
 * The Nemu status code is made available to the API user via
 * IVirtualBoxErrorInfo::resultDetail attribute.
 *
 * @param   vrc             The Nemu status code.
...
{
    return setErrorInternal(Global::nemuStatusCodeToCOM(vrc),
                            this->getClassIID(),
                            this->getComponentName(),
                            Utf8Str("%Rrc", vrc),
                            false /* aWarning */,
...
HRESULT VirtualBoxBase::setErrorVrc(int vrc, const char *pcszMsgFmt, ...)
{
    ...
    HRESULT hrc = setErrorInternal(Global::nemuStatusCodeToCOM(vrc),
                                   this->getClassIID(),
                                   this->getComponentName(),
...
/**
 * Sets error info with both a COM status and an Nemu status code.
 *
 * The Nemu status code is made available to the API user via
 * IVirtualBoxErrorInfo::resultDetail attribute.
 *
...
/**
 * Sets error info with a message and both a COM status and an Nemu status code.
 *
 * The Nemu status code is made available to the API user via
 * IVirtualBoxErrorInfo::resultDetail attribute.
 *
