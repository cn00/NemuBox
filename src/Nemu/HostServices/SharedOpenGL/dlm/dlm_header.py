...
""" % os.path.basename(sys.argv[0])
    ...
    for func_name in keys:
        if apiutil.CanCompile(func_name) and not apiutil.FindSpecial("dlm", func_name):
            print "    NEMU_DL_OPCODE_%s," % func_name

    print "    NEMU_DL_OPCODE_MAX,"
    print "} NemuDLOpCode;"
...
typedef struct DLMInstanceList {
	...
	int                     cbInstance;
	NemuDLOpCode            iNemuOpCode; /* This field name should not interfere w/ OpenGL function parameters names (for example w/ param 'opcode' for glLogicOp()). */
