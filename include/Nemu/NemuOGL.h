...
/* GUI and Nemu OpenGL code require scaling factor value to be stored in container
 ...
 * value to type of 'uint32_t'. Then Main provides this data to OpenGL HGCM thread. Finally, Nemu OpenGL
...
#define NEMU_OGL_SCALE_FACTOR_MULTIPLIER    10000.0

/* 3D content scale factor range bounds. */
#define NEMU_OGL_SCALE_FACTOR_MIN           0.01
#define NEMU_OGL_SCALE_FACTOR_MAX           10.0

bool RTCALL NemuOglIsOfflineRenderingAppropriate(void);
bool RTCALL NemuOglIs3DAccelerationSupported(void);

DECLEXPORT(int) NemuOglSetScaleFactor(uint32_t idScreen, double dScaleFactorW, double dScaleFactorH);
