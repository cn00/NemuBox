 ...
 *
 * [ PT_NOTE ]       -- Only 1
 *    - Offset into CoreDescriptor followed by list of Notes (Note Hdr + data) of Nemu CPUs.
 ...
 *    - Nemu 1 Note Hdr + Data.
 *    - Nemu 2 Note Hdr + Data.
 *    ...
 * Memory dump
 *
 */
...
#include <Nemu/vmm/dbgfcorefmt.h>
...
static const char *g_pcszCoreNemuCore = "VBCORE";
static const char *g_pcszCoreNemuCpu  = "VBCPU";
...
static int dbgfR3CoreWriteWorker(PVM pVM, RTFILE hFile)
{
    ...
    CoreDescriptor.u32NemuVersion     = NEMU_FULL_VERSION;
    CoreDescriptor.u32NemuRevision    = VMMGetSvnRev();
    CoreDescriptor.cCpus              = pVM->cCpus;

    Log((DBGFLOG_NAME ": CoreDescriptor Version=%u Revision=%u\n", CoreDescriptor.u32NemuVersion, CoreDescriptor.u32NemuRevision));
    ...
    uint64_t const cbCoreDescriptor   = Elf64NoteSectionSize(g_pcszCoreNemuCore, sizeof(CoreDescriptor));
    uint64_t const offCpuDumps        = offCoreDescriptor + cbCoreDescriptor;
    uint64_t const cbCpuDumps         = pVM->cCpus * Elf64NoteSectionSize(g_pcszCoreNemuCpu, sizeof(DBGFCORECPU));
    ...
    rc = Elf64WriteNoteHdr(hFile, NT_NEMUCORE, g_pcszCoreNemuCore, &CoreDescriptor, sizeof(CoreDescriptor));
    if (RT_FAILURE(rc))
    {
        LogRel((DBGFLOG_NAME ": Elf64WriteNoteHdr failed for Note '%s' rc=%Rrc\n", g_pcszCoreNemuCore, rc));
        return rc;
    }
    ...
    for (uint32_t iCpu = 0; iCpu < pVM->cCpus; iCpu++)
    {
        ...
        rc = Elf64WriteNoteHdr(hFile, NT_NEMUCPU, g_pcszCoreNemuCpu, pDbgfCoreCpu, sizeof(*pDbgfCoreCpu));
        if (RT_FAILURE(rc))
        {
 ...
 */
static DECLCALLBACK(NEMUSTRICTRC) dbgfR3CoreWriteRendezvous(PVM pVM, PVMCPU pVCpu, void *pvData)
{
    /*
     * Validate input.
     */
