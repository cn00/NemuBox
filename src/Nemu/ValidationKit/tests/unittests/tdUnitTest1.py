...
class tdUnitTest1(nemu.TestDriver):
    """
    Unit Tests.
    """
    ...
    kdTestCasesBlackList = {
        ...
        'testcase/tstSupLoadModule': '',    # Needs parameters and nemudrv access. Covered elsewhere.
        'testcase/tstSeamlessX11': '',      # graphics test
        'testcase/tstTime-3': '',           # parameters required
        'testcase/tstNemuControl': '',      # works only inside a guest
        ...
        'testcase/tstLdr-4': '',            # failed: Failed to get bits for '/home/nemu/test/tmp/bin/testcase/tstLdrObjR0.r0'/0,
        ...
        'tstNemuAPILinux': '',              # creates VirtualBox directories for root user because of sudo (should be in nemu)
    ...
    kasHardened = [
        ...
        "testcase/tstR0ThreadPreemptionDriver", # Nemu 4.3
        "testcase/tstRTR0ThreadPreemptionDriver",
        "testcase/tstRTR0MemUserKernelDriver",
        "testcase/tstRTR0SemMutexDriver",
    ...
    def _detectPaths(self):
        ...
        This sets sNemuInstallRoot and sUnitTestsPathBase and returns True/False.
        """

        #
        # We need a Nemu install (/ build) to test.
        #
        if False:
            if not self.importNemuApi():
                reporter.error('Unabled to import the Nemu Python API.')
                return False
        else:
            self._detectBuild();
            if self.oBuild is None:
                reporter.error('Unabled to detect the Nemu build.');
                return False;
        ...
        if utils.getHostOs() == 'win':
            ...
            if len(sPath) > 0 and sPath[-1] != ';':
                sPath += ';';
            os.environ[sPathName] = sPath + self.sNemuInstallRoot + ';';
    ...
    def _figureVersion(self):
        """ Tries to figure which Nemu version this is, setting self.aiNemuVer. """
        try:
            sVer = utils.processOutputChecked(['NemuManage', '--version'])
            ...
            self.aiNemuVer = [int(sComp) for sComp in sVer.split('.')];

            reporter.log('Nemu version: %s' % (self.aiNemuVer,));
        except:
            reporter.logXcpt();
            return False;
        return True;
    ...
    def _compareVersion(self, aiVer):
        """
        Compares the give version string with the nemu version string,
        returning a result similar to C strcmp().  aiVer is on the right side.
        """
        cComponents = min(len(self.aiNemuVer), len(aiVer));
        for i in range(cComponents):
            if self.aiNemuVer[i] < aiVer[i]:
                return -1;
            if self.aiNemuVer[i] > aiVer[i]:
                return 1;
        return len(self.aiNemuVer) - len(aiVer);
            ...
            for sExpr in asExprs:
                ...
                for sSubExpr in sExpr.split('&&'):
                    ...
                    if sOp == '>=' and iCmp < 0:
                        ...
                        fResult = False;
                    reporter.log2('iCmp=%s; %s %s %s -> %s' % (iCmp, self.aiNemuVer, sOp, aiValue, fResult));
    ...
    def _executeTestCase(self, sName, sFullPath, sTestCaseSubDir, oDevNull): # pylint: disable=R0914
        ...
        # needs to be copied to and execute from the sNemuInstallRoot
        ...
        if    sName in self.kasHardened \
          and self.sUnitTestsPathBase != self.sNemuInstallRoot:

            sDstDir = os.path.join(self.sNemuInstallRoot, sTestCaseSubDir);
