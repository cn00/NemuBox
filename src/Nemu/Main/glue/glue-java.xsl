<xsl:stylesheet version = '1.0'
    ...
    xmlns:nemu="http://www.virtualbox.org/"
...
<xsl:variable name="G_virtualBoxPackage" select="concat('org.virtualbox', $G_nemuApiSuffix)" />
<xsl:variable name="G_virtualBoxPackageCom" select="concat('org.virtualbox', $G_nemuApiSuffix, '.', $G_nemuGlueStyle)" />
<xsl:variable name="G_virtualBoxWsdl" select="concat('&quot;nemuwebService', $G_nemuApiSuffix, '.wsdl&quot;')" />
 ...
 * Generated from: src/Nemu/Main/idl/VirtualBox.xidl (VirtualBox's interface definitions in XML)
 * Generator: src/Nemu/Main/glue/glue-java.xsl
 */
...
<xsl:template name="startFile">
  ...
  <xsl:choose>
    <xsl:when test="$filelistonly=''">
      <xsl:value-of select="concat('&#10;// ##### BEGINFILE &quot;', $G_nemuDirPrefix, $file, '&quot;&#10;&#10;')" />
    ...
    <xsl:otherwise>
      <xsl:value-of select="concat(' \', $G_sNewLine, '&#9;', $G_nemuDirPrefix, $file)"/>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
...
<xsl:template name="endExcWrapper">
  ...
  <xsl:choose>
    <xsl:when test="$G_nemuGlueStyle='xpcom'">
      ...
      <xsl:text>        {&#10;</xsl:text>
      <xsl:text>            throw new NemuException(e.getMessage(), e);&#10;</xsl:text>
      <xsl:text>        }&#10;</xsl:text>
    </xsl:when>
    ...
    <xsl:when test="$G_nemuGlueStyle='jaxws'">
      ...
      <xsl:text>        {&#10;</xsl:text>
      <xsl:text>            throw new NemuException(e.getMessage(), e, this.port);&#10;</xsl:text>
      <xsl:text>        }&#10;</xsl:text>
...
<xsl:template name="genMethod">
  ...
  <xsl:choose>
    <xsl:when test="(param[@mod='ptr']) or (($G_nemuGlueStyle='jaxws') and (param[@type=($G_setSuppressedInterfaces/@name)]))" >
      <xsl:value-of select="concat('    // Skipping method ', $methodname, ' for it has parameters with suppressed types&#10;')" />
    </xsl:when>
    <xsl:when test="($G_nemuGlueStyle='jaxws') and (@wsmap = 'suppress')" >
    ...
    <xsl:otherwise>
      ...
      <xsl:variable name="retValValue">
        <xsl:choose>
          <xsl:when test="(param[@dir='out']) and ($G_nemuGlueStyle='jaxws')">
...
<xsl:template name="genIfaceWrapper">
  ...
  <xsl:choose>
      ...
      <xsl:when test="($G_nemuGlueStyle='xpcom') or ($G_nemuGlueStyle='mscom')">
  ...
  <xsl:for-each select="attribute[not(@mod='ptr')]">
    ...
    <xsl:choose>
      <xsl:when test="($G_nemuGlueStyle='jaxws') and ($attrtype=($G_setSuppressedInterfaces/@name))">
...
<xsl:template name="genIface">
  ...
  <xsl:if test="$filelistonly=''">
    ...
    <xsl:choose>
      <xsl:when test="($wsmap='struct') and ($G_nemuGlueStyle='jaxws')">
...
public class Helper
{
  ...
  <xsl:call-template name="startFile">
    <xsl:with-param name="file" select="'NemuException.java'" />
    <xsl:with-param name="package" select="$G_virtualBoxPackage" />
  </xsl:call-template>
...
public class NemuException extends RuntimeException
{
    private int resultCode;
    private IVirtualBoxErrorInfo errorInfo;

    public NemuException(String message)
    {
        super(message);
        resultCode = -1;
        errorInfo = null;
    ...
    public NemuException(String message, Throwable cause)
    {
        super(message, cause);
...
public class VirtualBoxManager
{
    ...
    private VirtualBoxManager(Mozilla mozilla)
    {
        ...
        this.nemu = new IVirtualBox((org.mozilla.interfaces.IVirtualBox) this.componentManager
    ...
    {
        throw new NemuException("Connect doesn't make sense for local bindings");
    }

    public void disconnect()
    {
        throw new NemuException("Disconnect doesn't make sense for local bindings");
    }
    ...
    {
        return "]]></xsl:text>
    <xsl:value-of select="substring($G_nemuApiSuffix, 2)" />
    <xsl:text><![CDATA[";
    }

    public IVirtualBox getNemu()
    {
        return this.nemu;
    }
    ...
    {
        if (hasInstance)
            throw new NemuException("only one instance of VirtualBoxManager at a time allowed");
        if (home == null || home.equals(""))
            home = System.getProperty("nemu.home");

        if (home == null)
            throw new NemuException("nemu.home Java property must be defined to use XPCOM bridge");
...
public class EventListenerImpl extends nsISupportsBase implements org.mozilla.interfaces.IEventListener
{
  ...
  <xsl:call-template name="startFile">
    <xsl:with-param name="file" select="'NemuObjectBase.java'" />
    <xsl:with-param name="package" select="$G_virtualBoxPackage" />
  </xsl:call-template>
...
public class VirtualBoxManager
{
    ...
    {
        initPerThread();
        nemu = new IVirtualBox(new ActiveXComponent("VirtualBox.VirtualBox"));
    }
...
public class Helper
{
    ...
    public static byte[] decodeBase64(String str)
    {
        ...
        if ((validChars * 3 % 4) != 0)
            throw new NemuException("invalid base64 encoded string " + str);
...
public class NemuException extends RuntimeException
{
    ...
    public NemuException(String message, Throwable cause, VboxPortType port)
    {
        super(message, cause);
...
class NemuTLSSocketFactory extends SSLSocketFactory
{
    ...
    {
        return new NemuTLSSocketFactory();
    }
...
public class VirtualBoxManager
{
    ...
    public void connect(String url, String username, String passwd)
    {
        ...
        try
        {
            ...
            NemuTLSSocketFactory sf = new NemuTLSSocketFactory();
            ((BindingProvider)port).getRequestContext().
            ...
            this.nemu = new IVirtualBox(handle, port);
        }
        catch (Throwable t)
        {
            ...
            throw new NemuException(t.getMessage(), t, this.port);
        }
    }
    ...
    public void disconnect()
    {
        ...
        try
        {
            if (this.nemu != null && port != null)
                port.iWebsessionManagerLogoff(this.nemu.getWrapped());
        }
        catch (InvalidObjectFaultMsg e)
        {
            throw new NemuException(e.getMessage(), e, this.port);
        }
        catch (RuntimeFaultMsg e)
        {
            throw new NemuException(e.getMessage(), e, this.port);
    ...
    {
        if (this.nemu == null)
            throw new NemuException("connect first");
        try
        {
            String handle = port.iWebsessionManagerGetSessionObject(this.nemu.getWrapped());
    ...
    public IEventListener createListener(Object sink)
    {
        throw new NemuException("no active listeners here");
    }
...
<xsl:template match="/">
  ...
  <xsl:if test="not($G_nemuApiSuffix)">
    <xsl:call-template name="fatalError">
      <xsl:with-param name="msg" select="'G_nemuApiSuffix must be given'" />
    </xsl:call-template>
  </xsl:if>
