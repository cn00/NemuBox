/* $Id: NemuGuestInst.cpp $ */
/** @file
 * Small tool to (un)install the NemuGuest device driver.
 */
...
static int installDriver(void)
{
    ...
    strcat(szDriver, "\\drivers\\NemuGuestNT.sys");

    SC_HANDLE hService = CreateService(hSMgrCreate,
                                       NEMUGUEST_SERVICE_NAME,
                                       "NemuGuest Support Driver",
...
static int uninstallDriver(void)
{
    ...
    SC_HANDLE hService = OpenService(hSMgr, NEMUGUEST_SERVICE_NAME, DELETE);
    if (hService)
    {
        /*
         * Delete the service.
...
static HANDLE openDriver(void)
{
    ...
    hDevice = CreateFile(NEMUGUEST_DEVICE_NAME, // Win2k+: NEMUGUEST_DEVICE_NAME_GLOBAL
