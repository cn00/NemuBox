...
#ifndef ___Nemu_types_h
#define ___Nemu_types_h

#include <Nemu/cdefs.h>
#include <iprt/types.h>


/** @defgroup grp_types     Nemu Basic Types
 * @{
 */
...
/** @def NEMUSTRICTRC_STRICT_ENABLED
 * Indicates that NEMUSTRICTRC is in strict mode.
 */
...
# define NEMUSTRICTRC_STRICT_ENABLED 1
#endif

/** We need RTERR_STRICT_RC.  */
#if defined(NEMUSTRICTRC_STRICT_ENABLED) && !defined(RTERR_STRICT_RC)
 ...
 * handling.
 */
#ifdef NEMUSTRICTRC_STRICT_ENABLED
struct NEMUSTRICTRC
{
protected:
    /** The status code. */
...
public:
    ...
    NEMUSTRICTRC(int32_t const rc)
        : m_rc(rc)
    {
    }

    /** Getter that NEMUSTRICTRC_VAL can use. */
    ...
    bool operator==(const NEMUSTRICTRC &rRc) const  { return m_rc == rRc.m_rc; }
    bool operator!=(const NEMUSTRICTRC &rRc) const  { return m_rc != rRc.m_rc; }
    bool operator<=(const NEMUSTRICTRC &rRc) const  { return m_rc <= rRc.m_rc; }
    bool operator>=(const NEMUSTRICTRC &rRc) const  { return m_rc >= rRc.m_rc; }
    bool operator<(const NEMUSTRICTRC &rRc) const   { return m_rc <  rRc.m_rc; }
...
private:
     ...
     * @{ */
    NEMUSTRICTRC(uint8_t  rc) : m_rc(-999)          { NOREF(rc); }
    NEMUSTRICTRC(uint16_t rc) : m_rc(-999)          { NOREF(rc); }
    NEMUSTRICTRC(uint32_t rc) : m_rc(-999)          { NOREF(rc); }
    NEMUSTRICTRC(uint64_t rc) : m_rc(-999)          { NOREF(rc); }

    NEMUSTRICTRC(int8_t rc)   : m_rc(-999)          { NOREF(rc); }
    NEMUSTRICTRC(int16_t rc)  : m_rc(-999)          { NOREF(rc); }
    NEMUSTRICTRC(int64_t rc)  : m_rc(-999)          { NOREF(rc); }
    /** @} */
};
# ifdef _MSC_VER
...
# endif
#else
typedef int32_t NEMUSTRICTRC;
#endif

/** @def NEMUSTRICTRC_VAL
 * Explicit getter.
 * @param rcStrict  The strict VirtualBox status code.
 */
#ifdef NEMUSTRICTRC_STRICT_ENABLED
# define NEMUSTRICTRC_VAL(rcStrict) ( (rcStrict).getValue() )
#else
# define NEMUSTRICTRC_VAL(rcStrict) (rcStrict)
#endif

/** @def NEMUSTRICTRC_TODO
 * Returns that needs dealing with.
 * @param rcStrict  The strict VirtualBox status code.
 */
#define NEMUSTRICTRC_TODO(rcStrict) NEMUSTRICTRC_VAL(rcStrict)
...
/** @todo NEMUIDT -> NEMUDESCIDT, skip the complex variations. We'll never use them. */

/** IDT Entry, Task Gate view. */
#pragma pack(1)                         /* paranoia */
typedef struct NEMUIDTE_TASKGATE
{
    /** Reserved. */
...
typedef NEMUIDTE_TASKGATE *PNEMUIDTE_TASKGATE;


/** IDT Entry, Intertupt gate view. */
#pragma pack(1)                         /* paranoia */
typedef struct NEMUIDTE_INTERRUPTGATE
{
    /** Low offset word. */
...
} NEMUIDTE_INTERRUPTGATE;
#pragma pack()
/** Pointer to IDT Entry, Interrupt gate view. */
typedef  NEMUIDTE_INTERRUPTGATE *PNEMUIDTE_INTERRUPTGATE;

/** IDT Entry, Trap Gate view. */
#pragma pack(1)                         /* paranoia */
typedef struct NEMUIDTE_TRAPGATE
{
    /** Low offset word. */
    unsigned    u16OffsetLow : 16;
    /** Segment Selector. */
...
typedef NEMUIDTE_TRAPGATE *PNEMUIDTE_TRAPGATE;

/** IDT Entry Generic view. */
#pragma pack(1)                         /* paranoia */
typedef struct NEMUIDTE_GENERIC
{
    /** Low offset word. */
    unsigned    u16OffsetLow : 16;
    /** Segment Selector. */
...
typedef NEMUIDTE_GENERIC *PNEMUIDTE_GENERIC;

/** IDT Type1 value. (Reserved for task gate!) */
#define NEMU_IDTE_TYPE1             0
/** IDT Type2 value - Task gate. */
#define NEMU_IDTE_TYPE2_TASK        0x5
/** IDT Type2 value - 16 bit interrupt gate. */
#define NEMU_IDTE_TYPE2_INT_16      0x6
/** IDT Type2 value - 32 bit interrupt gate. */
#define NEMU_IDTE_TYPE2_INT_32      0xe
/** IDT Type2 value - 16 bit trap gate. */
#define NEMU_IDTE_TYPE2_TRAP_16     0x7
/** IDT Type2 value - 32 bit trap gate. */
#define NEMU_IDTE_TYPE2_TRAP_32     0xf

/** IDT Entry. */
#pragma pack(1)                         /* paranoia */
typedef union NEMUIDTE
{
    /** Task gate view. */
    NEMUIDTE_TASKGATE       Task;
    /** Trap gate view. */
    NEMUIDTE_TRAPGATE       Trap;
    /** Interrupt gate view. */
    NEMUIDTE_INTERRUPTGATE  Int;
    /** Generic IDT view. */
    NEMUIDTE_GENERIC        Gen;
...
typedef NEMUIDTE *PNEMUIDTE;
/** Pointer to IDT Entry. */
typedef NEMUIDTE const *PCNEMUIDTE;

/** IDT Entry, 64-bit mode, Intertupt gate view. */
#pragma pack(1)                         /* paranoia */
typedef struct NEMUIDTE64_INTERRUPTGATE
{
    /** Low offset word. */
    unsigned    u16OffsetLow : 16;
    /** Segment Selector. */
...
} NEMUIDTE64_INTERRUPTGATE;
#pragma pack()
/** Pointer to IDT Entry, 64-bit mode, Interrupt gate view. */
typedef  NEMUIDTE64_INTERRUPTGATE *PNEMUIDTE64_INTERRUPTGATE;

/** IDT Entry, 64-bit mode, Trap gate view. */
#pragma pack(1)                         /* paranoia */
typedef struct NEMUIDTE64_TRAPGATE
{
    /** Low offset word. */
    unsigned    u16OffsetLow : 16;
    /** Segment Selector. */
...
typedef  NEMUIDTE64_TRAPGATE *PNEMUIDTE64_TRAPGATE;

/** IDT Entry, 64-bit mode, Generic view. */
#pragma pack(1)                         /* paranoia */
typedef struct NEMUIDTE64_GENERIC
{
    /** Low offset word. */
    unsigned    u16OffsetLow : 16;
    /** Segment Selector. */
...
typedef NEMUIDTE64_GENERIC *PNEMUIDTE64_GENERIC;

/** IDT Entry, 64-bit mode. */
#pragma pack(1)                         /* paranoia */
typedef union NEMUIDTE64
{
    /** Trap gate view. */
    NEMUIDTE64_TRAPGATE       Trap;
    /** Interrupt gate view. */
    NEMUIDTE64_INTERRUPTGATE  Int;
    /** Generic IDT view. */
    NEMUIDTE64_GENERIC        Gen;
...
#pragma pack()
/** Pointer to IDT Entry. */
typedef NEMUIDTE64 *PNEMUIDTE64;
/** Pointer to IDT Entry. */
typedef NEMUIDTE64 const *PCNEMUIDTE64;

#pragma pack(1)
/** IDTR */
typedef struct NEMUIDTR
{
    /** Size of the IDT. */
    uint16_t    cbIdt;
    /** Address of the IDT. */
 ...
 */
#define NEMUIDTE_OFFSET(desc) \
        (  ((uint32_t)((desc).Gen.u16OffsetHigh) << 16) \
         | (           (desc).Gen.u16OffsetLow        ) )

/** @def NEMUIDTE64_OFFSET
 * Return the offset of an IDT entry.
 */
#define NEMUIDTE64_OFFSET(desc) \
...
typedef NEMUTSS *PNEMUTSS;
/** Pointer to const task segment. */
typedef const NEMUTSS *PCNEMUTSS;
