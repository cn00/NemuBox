...
# ifdef NEMU_WITH_WDDM
    if(swapchain->win_handle) {
        NemuExtWndDestroy(swapchain->win_handle, swapchain->hDC);
        ...
        struct wined3d_swapchain_desc *desc, void *parent, const struct wined3d_parent_ops *parent_ops)
{
    ...
    {
        hr = NemuExtWndCreate(desc->backbuffer_width, desc->backbuffer_height, &window, &hDC);
        if (FAILED(hr))
        {
            ERR("NemuExtWndCreate failed, hr 0x%x", hr);
            return hr;
        }
    }
    Assert(window);
...
# ifdef NEMU_WINE_WITH_SINGLE_CONTEXT
    swapchain->hDC = NemuExtGetDC(window);
    if (!swapchain->hDC)
    {
...
err:
    ...
    if (swapchain->back_buffers)
    {
        ...
        HeapFree(GetProcessHeap(), 0, swapchain->back_buffers);
#ifdef NEMU_WITH_WINE_FIX_INITCLEAR
        swapchain->back_buffers = NULL;
#endif
    }
#ifdef NEMU_WITH_WINE_FIX_INITCLEAR
...
struct wined3d_context *swapchain_get_context(struct wined3d_swapchain *swapchain)
{
    ...
    for (i = 0; i < swapchain->num_contexts; ++i)
    {
#  ifdef NEMU_WINE_WITH_SINGLE_SWAPCHAIN_CONTEXT
        if (NemuTlsRefIsFunctional(swapchain->context[i]))
#  else
...
#else /* if defined NEMU_WINE_WITH_SINGLE_CONTEXT */
struct wined3d_context *swapchain_get_context(struct wined3d_swapchain *swapchain)
{
...
{
    int32_t id = pNemuGetWindowId(swapchain->hDC);
    if (!id)
    {
        *pi32Id = 0;
