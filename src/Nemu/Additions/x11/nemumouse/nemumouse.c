...
static void
NemuReadInput(InputInfoPtr pInfo)
{
...
static void
NemuPtrCtrlProc(DeviceIntPtr device, PtrCtrl *ctrl)
{
    /* Nothing to do, dix handles all settings */
}

static int
NemuInit(DeviceIntPtr device)
{
    CARD8 map[2] = { 0, 1 };
...
#endif
#if GET_ABI_MAJOR(ABI_XINPUT_VERSION) < 2
                                 miPointerGetMotionEvents, NemuPtrCtrlProc,
                                 miPointerGetMotionBufferSize()
#elif GET_ABI_MAJOR(ABI_XINPUT_VERSION) < 3
                                 GetMotionHistory, NemuPtrCtrlProc,
                                 GetMotionHistorySize(), 2 /* Number of axes */

#elif GET_ABI_MAJOR(ABI_XINPUT_VERSION) >= 3
                                 NemuPtrCtrlProc, GetMotionHistorySize(),
                                 2 /* Number of axes */
#else
...
static int
NemuProc(DeviceIntPtr device, int what)
{
    InputInfoPtr pInfo;
    int rc, xrc;
...
static Bool
NemuConvert(InputInfoPtr pInfo, int first, int num, int v0, int v1, int v2,
...
static int
NemuPreInitInfo(InputDriverPtr drv, InputInfoPtr pInfo, int flags)
{
    const char *device;
    int rc;

    /* Initialise the InputInfoRec. */
    pInfo->device_control = NemuProc;
    pInfo->read_input = NemuReadInput;
...
#if GET_ABI_MAJOR(ABI_XINPUT_VERSION) < 12
static InputInfoPtr
NemuPreInit(InputDriverPtr drv, IDevPtr dev, int flags)
{
    InputInfoPtr pInfo;
    const char *device;
    ...
    pInfo->conf_idev = dev;
    pInfo->conversion_proc = NemuConvert;
    pInfo->flags = XI86_POINTER_CAPABLE | XI86_SEND_DRAG_EVENTS;

    xf86CollectInputOptions(pInfo, NULL, NULL);
    xf86ProcessCommonOptions(pInfo, pInfo->options);

    if (NemuPreInitInfo(drv, pInfo, flags) != Success) {
...
_X_EXPORT InputDriverRec NEMUMOUSE = {
    1,
    "nemumouse",
    NULL,
#if GET_ABI_MAJOR(ABI_XINPUT_VERSION) < 12
...
static pointer
NemuPlug(pointer module,
          pointer options,
          int *errmaj,
          int *errmin)
{
    xf86AddInputDriver(&NEMUMOUSE, module, 0);
    xf86Msg(X_CONFIG, "Load address of symbol \"NEMUMOUSE\" is %p\n",
            (void *)&NEMUMOUSE);
    return module;
}
...
_X_EXPORT XF86ModuleData nemumouseModuleData =
{
    &NemuVersionRec,
    NemuPlug,
    NULL
