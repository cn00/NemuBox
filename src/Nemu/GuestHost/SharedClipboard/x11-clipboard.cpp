...
#include <Nemu/GuestHost/SharedClipboard.h>
#include <Nemu/GuestHost/clipboard-helper.h>
 ...
 * Nemu clipboard formats (currently only Unicode) */
static struct _CLIPFORMATTABLE
{
    ...
    /** The corresponding Nemu clipboard format */
    uint32_t    u32NemuFormat;
} g_aFormats[] =
{
    { "INVALID", INVALID, 0 },
    { "UTF8_STRING", UTF8, NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT },
    { "text/plain;charset=UTF-8", UTF8,
      NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT },
    { "text/plain;charset=utf-8", UTF8,
      NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT },
    { "STRING", TEXT, NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT },
    { "TEXT", TEXT, NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT },
    { "text/plain", TEXT, NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT },
    { "image/bmp", BMP, NEMU_SHARED_CLIPBOARD_FMT_BITMAP },
    { "image/x-bmp", BMP, NEMU_SHARED_CLIPBOARD_FMT_BITMAP },
    { "image/x-MS-bmp", BMP, NEMU_SHARED_CLIPBOARD_FMT_BITMAP },
...
/** Return the atom corresponding to a supported X11 format. */
static uint32_t clipNemuFormatForX11Format(CLIPX11FORMAT format)
{
    return g_aFormats[format].u32NemuFormat;
}
...
/**
 * Enumerates supported X11 clipboard formats corresponding to a given Nemu
 * format.
 ...
 */
static CLIPX11FORMAT clipEnumX11Formats(uint32_t u32NemuFormats,
                                        CLIPX11FORMAT lastFormat)
{
    for (unsigned i = lastFormat + 1; i < RT_ELEMENTS(g_aFormats); ++i)
        if (u32NemuFormats & clipNemuFormatForX11Format(i))
            return i;
    return NIL_CLIPX11FORMAT;
}
...
struct _CLIPBACKEND
{
    ...
    NEMUCLIPBOARDCONTEXT *pFrontend;
    ...
    /** What formats does Nemu have on offer? */
    uint32_t nemuFormats;
...
/**
 * Report the formats currently supported by the X11 clipboard to Nemu.
 */
static void clipReportFormatsToNemu(CLIPBACKEND *pCtx)
{
    uint32_t u32NemuFormats = clipNemuFormatForX11Format(pCtx->X11TextFormat);
    u32NemuFormats |= clipNemuFormatForX11Format(pCtx->X11BitmapFormat);
    ClipReportX11Formats(pCtx->pFrontend, u32NemuFormats);
}
...
/** Tell Nemu that X11 currently has nothing in its clipboard. */
static void clipReportEmptyX11CB(CLIPBACKEND *pCtx)
{
    clipResetX11Formats(pCtx);
    clipReportFormatsToNemu(pCtx);
}
                                                  ...
                                                  size_t cTargets)
{
    ...
    for (unsigned i = 0; i < cTargets; ++i)
    {
        ...
        {
            if (   (clipNemuFormatForX11Format(format)
                            == NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT)
                                                    ...
                                                    size_t cTargets)
{
    ...
    for (unsigned i = 0; i < cTargets; ++i)
    {
        ...
        {
            if (   (clipNemuFormatForX11Format(format)
                            == NEMU_SHARED_CLIPBOARD_FMT_BITMAP)
...
/**
 * Notify the Nemu clipboard about available data formats, based on the
...
static void clipStopEventThreadWorker(void *pUserData, void *)
{
     ...
     * the Nemu part *must* have returned before we do this. */
...
static int clipInit(CLIPBACKEND *pCtx)
{
    ...
    pDisplay = XtOpenDisplay(pCtx->appContext, 0, 0, "NemuClipboard", 0, 0, &cArgc, &pcArgv);
    if (NULL == pDisplay)
    {
    ...
    if (RT_SUCCESS(rc))
    {
        pCtx->widget = XtVaAppCreateShell(0, "NemuClipboard",
 ...
 * @note  X11 backend code
 */
CLIPBACKEND *ClipConstructX11(NEMUCLIPBOARDCONTEXT *pFrontend, bool fHeadless)
{
    int rc;
    ...
    if (pCtx && fHeadless)
    {
         ...
         * This is important for NemuHeadless.
         */
 ...
 * @note  X11 backend code
 * @note  Any requests from this object to get clipboard data from Nemu
...
/**
 * Satisfy a request from X11 for clipboard targets supported by Nemu.
 *
                                ...
                                int *piFormatReturn)
{
    ...
    do
    {
        format = clipEnumX11Formats(pCtx->nemuFormats, format);
        if (format != NIL_CLIPX11FORMAT)
        {
 ...
 * data returned.
 */
static int clipReadNemuClipboard(CLIPBACKEND *pCtx, uint32_t u32Format,
                                 void **ppv, uint32_t *pcb)
{
    int rc = VINF_SUCCESS;
    LogRelFlowFunc(("pCtx=%p, u32Format=%02X, ppv=%p, pcb=%p\n", pCtx,
                 u32Format, ppv, pcb));
    if (u32Format == NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT)
    {
                            ...
                            size_t cbBuf, size_t *pcbActual)
{
    ...
    AssertPtr(pwszSrc);
    AssertPtr(pszBuf);
    rc = nemuClipboardUtf16GetLinSize(pwszSrc, cwSrc, &cwTmp);
    ...
    /* Convert the text. */
    if (RT_SUCCESS(rc))
        rc = nemuClipboardUtf16WinToLin(pwszSrc, cwSrc, pwszTmp, cwTmp);
    if (RT_SUCCESS(rc))
...
static int clipConvertNemuCBForX11(CLIPBACKEND *pCtx, Atom *atomTarget,
                                   Atom *atomTypeReturn,
                                   XtPointer *pValReturn,
                                   unsigned long *pcLenReturn,
                                   int *piFormatReturn)
{
    ...
    if (   ((format == UTF8) || (format == TEXT))
        && (pCtx->nemuFormats & NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT))
    {
        void *pv = NULL;
        uint32_t cb = 0;
        rc = clipReadNemuClipboard(pCtx,
                                   NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT,
                                   &pv, &cb);
        ...
        RTMemFree(pv);
             ...
             && (pCtx->nemuFormats & NEMU_SHARED_CLIPBOARD_FMT_BITMAP))
    {
        void *pv = NULL;
        uint32_t cb = 0;
        rc = clipReadNemuClipboard(pCtx,
        ...
        if (RT_SUCCESS(rc) && (format == BMP))
        {
            ...
            rc = nemuClipboardDibToBmp(pv, cb, (void **)pValReturn,
                                       (size_t *)pcLenReturn);
        }
        else
            rc = VERR_NOT_SUPPORTED;
...
/**
 * Return Nemu's clipboard data for an X11 client.
                                          ...
                                          int *piFormatReturn)
{
    ...
    if (*atomTarget == clipGetAtom(pCtx, "TARGETS"))
        ...
        rc = clipConvertNemuCBForX11(pCtx, atomTarget, atomTypeReturn,
                                     pValReturn, pcLenReturn, piFormatReturn);
    LogRelFlowFunc(("returning, internal status code %Rrc\n", rc));
    return RT_SUCCESS(rc);
}

/** Structure used to pass information about formats that Nemu supports */
typedef struct _CLIPNEWNEMUFORMATS
{
    /** Context information for the X11 clipboard */
    CLIPBACKEND *pCtx;
    /** Formats supported by Nemu */
    uint32_t formats;
} CLIPNEWNEMUFORMATS;

/** Invalidates the local cache of the data in the Nemu clipboard. */
static void clipInvalidateNemuCBCache(CLIPBACKEND *pCtx)
{
    if (pCtx->pvUnicodeCache != NULL)
    {
 ...
 * event thread.
 * @param pUserData  Pointer to a CLIPNEWNEMUFORMATS structure containing
 *                   information about the Nemu formats available and the
 *                   clipboard context data.  Must be freed by the worker.
 */
static void clipNewNemuFormatsWorker(void *pUserData,
                                     void * /* interval */)
{
    CLIPNEWNEMUFORMATS *pFormats = (CLIPNEWNEMUFORMATS *)pUserData;
    CLIPBACKEND *pCtx = pFormats->pCtx;
    uint32_t u32Formats = pFormats->formats;
    RTMemFree(pFormats);
    LogRelFlowFunc (("u32Formats=%d\n", u32Formats));
    clipInvalidateNemuCBCache(pCtx);
...
/**
 * Nemu is taking possession of the shared clipboard.
 *
 * @param u32Formats Clipboard formats that Nemu is offering
 * @note  X11 backend code
 */
                                        ...
                                        uint32_t u32Formats)
{
    ...
    CLIPNEWNEMUFORMATS *pFormats =
        (CLIPNEWNEMUFORMATS *) RTMemAlloc(sizeof(CLIPNEWNEMUFORMATS));
    if (pFormats != NULL)  /* if it is we will soon have other problems */
    {
        pFormats->pCtx = pCtx;
        pFormats->formats = u32Formats;
        clipQueueToEventThread(pCtx, clipNewNemuFormatsWorker,
                               (XtPointer) pFormats);
    }
}
                             ...
                             PRTUTF16 *ppwszDest, uint32_t *pcbDest)
{
    ...
    size_t cwcDest;
    int rc = nemuClipboardUtf16GetWinSize(pwcSrc, cwcSrc + 1, &cwcDest);
    if (RT_SUCCESS(rc))
    {
    ...
    if (RT_SUCCESS(rc))
        rc = nemuClipboardUtf16LinToWin(pwcSrc, cwcSrc + 1, pwszDest,
                                        cwcDest);
    if (RT_SUCCESS(rc))
    {
...
{
    /** The format Nemu would like the data in */
    uint32_t mFormat;
...
static void clipConvertX11CB(void *pClientData, void *pvSrc, unsigned cbSrc)
{
    ...
    if (pvSrc == NULL)
        ...
        rc = VERR_NO_DATA;
    else if (pReq->mFormat == NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT)
    {
    ...
    else if (pReq->mFormat == NEMU_SHARED_CLIPBOARD_FMT_BITMAP)
    {
        /* In which format is the clipboard data? */
        switch (clipRealFormatForX11Format(pReq->mBitmapFormat))
        {
            case BMP:
            {
                const void *pDib;
                size_t cbDibSize;
                rc = nemuClipboardBmpGetDib((const void *)pvSrc, cbSrc,
                                            &pDib, &cbDibSize);
                if (RT_SUCCESS(rc))
                {
 ...
 * thread. */
static void nemuClipboardReadX11Worker(void *pUserData,
                                       void * /* interval */)
{
    ...
    {
        /*
         * Nemu wants to read data in the given format.
         */
        pReq->mTextFormat = pCtx->X11TextFormat;
        if (pReq->mTextFormat == INVALID)
            /* Nemu thinks we have data and we don't */
            rc = VERR_NO_DATA;
        else
...
/**
 * Called when Nemu wants to read the X11 clipboard.
 *
 * @returns iprt status code
 * @param  pCtx      Context data for the clipboard backend
 * @param  u32Format The format that the Nemu would like to receive the data
 *                   in
                           ...
                           CLIPREADCBREQ *pReq)
{
    ...
    else
    {
        ...
        clipQueueToEventThread(pCtx, nemuClipboardReadX11Worker,
                               (XtPointer) pX11Req);
    }
    return rc;
}
...
/* The data in the simulated Nemu clipboard */
static int g_nemuDataRC = VINF_SUCCESS;
static void *g_nemuDatapv = NULL;
static uint32_t g_nemuDatacb = 0;

/* Set empty data in the simulated Nemu clipboard. */
static void clipEmptyNemu(CLIPBACKEND *pCtx, int retval)
{
    g_nemuDataRC = retval;
    RTMemFree(g_nemuDatapv);
    g_nemuDatapv = NULL;
    g_nemuDatacb = 0;
    ClipAnnounceFormatToX11(pCtx, 0);
}

/* Set the data in the simulated Nemu clipboard. */
static int clipSetNemuUtf16(CLIPBACKEND *pCtx, int retval,
...
/* Return the data in the simulated Nemu clipboard. */
int ClipRequestDataForX11(NEMUCLIPBOARDCONTEXT *pCtx,
                                 uint32_t u32Format, void **ppv,
                                 uint32_t *pcb)
{
    *pcb = g_nemuDatacb;
    if (g_nemuDatapv != NULL)
    {
        void *pv = RTMemDup(g_nemuDatapv, g_nemuDatacb);
        *ppv = pv;
        return pv != NULL ? g_nemuDataRC : VERR_NO_MEMORY;
    }
    *ppv = NULL;
    return g_nemuDataRC;
}
...
void ClipReportX11Formats(NEMUCLIPBOARDCONTEXT* pCtx,
                                      uint32_t u32Formats)
{
...
void ClipCompleteDataRequestFromX11(NEMUCLIPBOARDCONTEXT *pCtx, int rc,
                              ...
                              const char *pcszExp, int rcExp)
{
    ...
    if (clipQueryFormats() != NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT)
    ...
    else
    {
        ...
        ClipRequestDataFromX11(pCtx, NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT,
                               pReq);
        int rc = VINF_SUCCESS;
        uint32_t cbActual = 0;
...
static void testStringFromNemu(RTTEST hTest, CLIPBACKEND *pCtx,
...
static void testStringFromNemuFailed(RTTEST hTest, CLIPBACKEND *pCtx,
...
int main()
{
    ...
    /*** Ensure that Nemu is notified when we return the CB to X11 ***/
    ...
    /*** request for an invalid Nemu format from X11 ***/
    RTTestSub(hTest, "a request for an invalid Nemu format from X11");
    ...
    /*** Utf-8 from Nemu ***/
    RTTestSub(hTest, "reading Utf-8 from Nemu");
    /* Simple test */
    clipSetNemuUtf16(pCtx, VINF_SUCCESS, "hello world",
                     sizeof("hello world") * 2);
    testStringFromNemu(hTest, pCtx, "UTF8_STRING",
                       clipGetAtom(pCtx, "UTF8_STRING"), "hello world");
    /* With an embedded carriage return */
    clipSetNemuUtf16(pCtx, VINF_SUCCESS, "hello\r\nworld",
                     sizeof("hello\r\nworld") * 2);
    testStringFromNemu(hTest, pCtx, "text/plain;charset=UTF-8",
                       clipGetAtom(pCtx, "text/plain;charset=UTF-8"),
                       "hello\nworld");
    /* With an embedded CRCRLF */
    clipSetNemuUtf16(pCtx, VINF_SUCCESS, "hello\r\r\nworld",
                     sizeof("hello\r\r\nworld") * 2);
    testStringFromNemu(hTest, pCtx, "text/plain;charset=UTF-8",
                       clipGetAtom(pCtx, "text/plain;charset=UTF-8"),
                       "hello\r\nworld");
    ...
    testStringFromNemu(hTest, pCtx, "text/plain;charset=UTF-8",
                       ...
                       "hello\n\rworld");
    /* An empty string */
    clipSetNemuUtf16(pCtx, VINF_SUCCESS, "", 2);
    testStringFromNemu(hTest, pCtx, "text/plain;charset=utf-8",
                       clipGetAtom(pCtx, "text/plain;charset=utf-8"), "");
    /* With an embedded Utf-8 character. */
    clipSetNemuUtf16(pCtx, VINF_SUCCESS, "100\xE2\x82\xAC" /* 100 Euro */,
                     10);
    testStringFromNemu(hTest, pCtx, "STRING",
                       clipGetAtom(pCtx, "STRING"), "100\xE2\x82\xAC");
    /* A non-zero-terminated string */
    clipSetNemuUtf16(pCtx, VINF_SUCCESS, "hello world",
                     sizeof("hello world") * 2 - 2);
    testStringFromNemu(hTest, pCtx, "TEXT", clipGetAtom(pCtx, "TEXT"),
                       "hello world");

    /*** Timeout from Nemu ***/
    RTTestSub(hTest, "reading from Nemu with timeout");
    clipEmptyNemu(pCtx, VERR_TIMEOUT);
    testStringFromNemuFailed(hTest, pCtx, "UTF8_STRING");

    /*** No data in Nemu clipboard ***/
    RTTestSub(hTest, "an empty Nemu clipboard");
    clipSetSelectionValues("TEXT", XA_STRING, "", sizeof(""), 8);
    clipEmptyNemu(pCtx, VINF_SUCCESS);
    RTTEST_CHECK_MSG(hTest, g_ownsSel,
                     (hTest, "Nemu grabbed the clipboard with no data and we ignored it\n"));
    testStringFromNemuFailed(hTest, pCtx, "UTF8_STRING");

    /*** An unknown Nemu format ***/
    RTTestSub(hTest, "reading an unknown Nemu format");
    clipSetSelectionValues("TEXT", XA_STRING, "", sizeof(""), 8);
    clipSetNemuUtf16(pCtx, VINF_SUCCESS, "", 2);
    ClipAnnounceFormatToX11(pCtx, 0xa0000);
    RTTEST_CHECK_MSG(hTest, g_ownsSel,
                     (hTest, "Nemu grabbed the clipboard with unknown data and we ignored it\n"));
    testStringFromNemuFailed(hTest, pCtx, "UTF8_STRING");
    rc = ClipStopX11(pCtx);
    AssertRCReturn(rc, 1);
    ...
    /* Simple test */
    clipSetNemuUtf16(pCtx, VINF_SUCCESS, "",
                     sizeof("") * 2);
    clipSetSelectionValues("UTF8_STRING", XA_STRING, "hello world",
                           sizeof("hello world"), 8);
    ...
    /*** Read from Nemu ***/
    RTTestSub(hTest, "reading from Nemu, headless clipboard");
    /* Simple test */
    clipEmptyNemu(pCtx, VERR_WRONG_ORDER);
    clipSetSelectionValues("TEXT", XA_STRING, "", sizeof(""), 8);
    clipSetNemuUtf16(pCtx, VINF_SUCCESS, "hello world",
                     sizeof("hello world") * 2);
    testNoSelectionOwnership(pCtx, "reading from Nemu, headless clipboard");
