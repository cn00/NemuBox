...
# define NEMU_DSOUND_MAX_EVENTS 3
...
typedef struct DRVHOSTDSOUND
{
    ...
    HANDLE              aEvents[NEMU_DSOUND_MAX_EVENTS];
    /** Number of events to wait for in notification thread.
     *  Must not exceed NEMU_DSOUND_MAX_EVENTS. */
    uint8_t             cEvents;
...
static HRESULT directSoundPlayOpen(PDRVHOSTDSOUND pThis, PDSOUNDSTREAMOUT pDSoundStrmOut)
{
    ...
    do /* To use breaks. */
    {
        ...
        Assert(pThis->cEvents < NEMU_DSOUND_MAX_EVENTS);
        pThis->cEvents++;
                                              ...
                                              uint32_t *pcSamplesPlayed)
{
    ...
    do /* to use 'break' */
    {
        ...
        if (cbFree <= cbSample)
        {
            ...
            fFlags |= DSCBSTART_LOOPING;
#endif
            IDirectSoundBuffer8_Play(pDSoundStrmOut->pDSB, 0, 0, fFlags);
            break;
        }
        cbFree     -= cbSample;
...
static DECLCALLBACK(int) drvHostDSoundThread(RTTHREAD hThreadSelf, void *pvUser)
{
    ...
    do
    {
        HANDLE aEvents[NEMU_DSOUND_MAX_EVENTS];
        DWORD  cEvents = 0;
        for (uint8_t i = 0; i < NEMU_DSOUND_MAX_EVENTS; i++)
        {
            if (pThis->aEvents[i])
