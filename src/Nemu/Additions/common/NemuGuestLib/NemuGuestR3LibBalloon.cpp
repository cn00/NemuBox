/* $Id: NemuGuestR3LibBalloon.cpp $ */
/** @file
 * NemuGuestR3Lib - Ring-3 Support Library for VirtualBox guest additions, Ballooning.
 */
...
{
    NemuGuestCheckBalloonInfo Info;
    int rc = vbglR3DoIOCtl(NEMUGUEST_IOCTL_CHECK_BALLOON, &Info, sizeof(Info));
    if (RT_SUCCESS(rc))
    {
...
{
    NemuGuestChangeBalloonInfo Info;
    Info.u64ChunkAddr = (uint64_t)((uintptr_t)pv);
    Info.fInflate = fInflate;
    return vbglR3DoIOCtl(NEMUGUEST_IOCTL_CHANGE_BALLOON, &Info, sizeof(Info));
}
