 ...
 *    NemuGuestR3LibDragAndDrop.cpp) and the guest OS specific parts.
 ...
 *
 * In general I propose the following changes in the Nemu HGCM infrastructure
 * for the future:
 ...
 *   - DnD service crash in the guest during a DnD op (e.g. crash of NemuClient or X11)
    ...
    if (!m_progress.isNull())
    {
        ...
        if (!fCompleted)
        {
            switch (uStatus)
            {
                ...
                {
                    hr = m_progress->i_notifyComplete(NEMU_E_IPRT_ERROR,
                                                      COM_IIDOF(IGuest),
...
int GuestDnDResponse::onDispatch(uint32_t u32Function, void *pvParms, uint32_t cbParms)
{
    ...
    switch (u32Function)
    {
        ...
        {
            DragAndDropSvc::PNEMUDNDCBHGACKOPDATA pCBData = reinterpret_cast<DragAndDropSvc::PNEMUDNDCBHGACKOPDATA>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(DragAndDropSvc::NEMUDNDCBHGACKOPDATA) == cbParms, VERR_INVALID_PARAMETER);
        ...
        {
            DragAndDropSvc::PNEMUDNDCBHGREQDATADATA pCBData = reinterpret_cast<DragAndDropSvc::PNEMUDNDCBHGREQDATADATA>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(DragAndDropSvc::NEMUDNDCBHGREQDATADATA) == cbParms, VERR_INVALID_PARAMETER);
        ...
        {
            DragAndDropSvc::PNEMUDNDCBHGEVTPROGRESSDATA pCBData =
               reinterpret_cast<DragAndDropSvc::PNEMUDNDCBHGEVTPROGRESSDATA>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(DragAndDropSvc::NEMUDNDCBHGEVTPROGRESSDATA) == cbParms, VERR_INVALID_PARAMETER);
        ...
        {
            DragAndDropSvc::PNEMUDNDCBGHACKPENDINGDATA pCBData =
               reinterpret_cast<DragAndDropSvc::PNEMUDNDCBGHACKPENDINGDATA>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(DragAndDropSvc::NEMUDNDCBGHACKPENDINGDATA) == cbParms, VERR_INVALID_PARAMETER);
    ...
    : m_pGuest(pGuest)
{
    ...
    const com::Utf8Str arrEntries[] = { NEMU_DND_FORMATS_DEFAULT };
...
int GuestDnD::hostCall(uint32_t u32Function, uint32_t cParms, PNEMUHGCMSVCPARM paParms) const
{
    ...
    return pVMMDev->hgcmHostCall("NemuDragAndDropSvc", u32Function, cParms, paParms);
}
...
int GuestDnDBase::getProtocolVersion(uint32_t *puVersion)
{
    ...
    {
        uint32_t uNemuMajor = NEMU_FULL_VERSION_GET_MAJOR(uVerAdditions);
        uint32_t uNemuMinor = NEMU_FULL_VERSION_GET_MINOR(uVerAdditions);
...
#else
        uVer = (  uNemuMajor  >= 5)
             ? 2  /* Nemu 5.0 and up: Protocol version 2. */
             : 1; /* Nemu <= 4.3:     Protocol version 1. */
        /* Build revision is ignored. */
#endif

        LogFlowThisFunc(("uVerAdditions=%RU32 (%RU32.%RU32)\n", uVerAdditions, uNemuMajor, uNemuMinor));
        rc = VINF_SUCCESS;
    }
    else
    {
