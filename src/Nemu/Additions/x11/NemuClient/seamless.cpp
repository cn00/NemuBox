 ...
 * wrapper interface for the main code of the NemuClient deamon.  The
...
int SeamlessMain::init(void)
{
    ...
    if (RT_FAILURE(rc))
        VBClFatalError(("NemuClient (seamless): failed to start.  Stage: \"%s\"  Error: %Rrc\n",
                pcszStage, rc));
    return rc;
}
...
int SeamlessMain::run(void)
{
    ...
    if (RT_FAILURE(rc))
    {
        LogRel(("NemuClient (seamless): event loop failed with error: %Rrc\n",
                rc));
        stop();
    }
    return rc;
...
int SeamlessMain::nextStateChangeEvent(void)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        switch (newMode)
        {
             ...
             * borderless and making the guest desktop transparent. */
                LogRelFlowFunc(("\"Visible region\" mode requested (NemuClient).\n"));
                break;
            case VMMDev_Seamless_Disabled:
                LogRelFlowFunc(("\"Disabled\" mode requested (NemuClient).\n"));
                break;
            case VMMDev_Seamless_Host_Window:
            /* One host window represents one guest window.  Not yet implemented. */
                LogRelFunc(("Unsupported \"host window\" mode requested (NemuClient).\n"));
                return VERR_NOT_SUPPORTED;
            default:
                LogRelFunc(("Unsupported mode %d requested (NemuClient).\n",
                            newMode));
                return VERR_NOT_SUPPORTED;
        }
    }
    ...
    {
        LogRelFunc(("VbglR3SeamlessWaitEvent returned %Rrc (NemuClient)\n", rc));
    }
...
int SeamlessMain::startX11MonitorThread(void)
{
    ...
    rc = RTThreadCreate(&mX11MonitorThread, x11MonitorThread, this, 0,
                        ...
                        "X11 events");
    if (RT_FAILURE(rc))
        LogRelFunc(("Warning: failed to start X11 monitor thread (NemuClient).\n"));
    return rc;
}
...
int SeamlessMain::selfTest()
{
    ...
    if (RT_FAILURE(rc))
        LogRel(("NemuClient (seamless): self test failed.  Stage: \"%s\"\n",
                pcszStage));
    return rc;
}
...
/** NemuClient service class wrapping the logic for the seamless service while
...
{
    return ".nemuclient-seamless.pid";
}
