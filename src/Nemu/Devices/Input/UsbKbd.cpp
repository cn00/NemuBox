 ...
 * @{ */
/** The highest USB usage code reported by the Nemu emulated keyboard */
#define NEMU_USB_MAX_USAGE_CODE     0xE7
/** The size of an array needed to store all USB usage codes */
#define NEMU_USB_USAGE_ARRAY_SIZE   (NEMU_USB_MAX_USAGE_CODE + 1)
...
typedef struct USBHID
{
    ...
    uint8_t             abDepressedKeys[NEMU_USB_USAGE_ARRAY_SIZE];
...
static void usbHidBuildReport(PUSBHIDK_REPORT pReport, uint8_t *pabDepressedKeys)
{
    unsigned iBuf = 0;
    RT_ZERO(*pReport);
    for (unsigned iKey = 0; iKey < NEMU_USB_USAGE_ARRAY_SIZE; ++iKey)
    {
...
static DECLCALLBACK(int) usbHidKeyboardPutEvent(PPDMIKEYBOARDPORT pInterface, uint32_t u32UsageCode)
{
    ...
    AssertReturn(u8HidCode <= NEMU_USB_MAX_USAGE_CODE, VERR_INTERNAL_ERROR);
