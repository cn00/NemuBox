...
#define COBJMACROS
#ifndef NEMU_WINE_WITHOUT_LIBWINE
#include "windef.h"
#include "winbase.h"
#include "winreg.h"
#include "wingdi.h"
...
# include "nemusharedrc.h"
#endif

#if defined(NEMU_WINE_WITH_SINGLE_CONTEXT) || defined(NEMU_WINE_WITH_SINGLE_SWAPCHAIN_CONTEXT)
# define NemuTlsRefGetImpl(_tls) (TlsGetValue((DWORD)(_tls)))
# define NemuTlsRefSetImpl(_tls, _val) (TlsSetValue((DWORD)(_tls), (_val)))
# define NemuTlsRefAssertImpl Assert
...
#ifdef NEMU_WITH_WINE_FIX_QUIRKS
...
#ifdef NEMU_WITH_WINE_FIX_BUFOFFSET
    UINT offset;
#endif
    GLsizei stride;
    UINT stream_idx;
...
#endif
#if defined(NEMU_WINE_WITH_SINGLE_CONTEXT)
...
#ifdef NEMU_WITH_WINE_FIX_ZEROVERTATTR
...
#ifdef NEMU_WINE_WITH_SHADER_CACHE
    NEMUEXT_HASHCACHE vshaderCache;
    NEMUEXT_HASHCACHE pshaderCache;
#endif

#ifdef NEMU_WINE_WITH_PROFILE
    NEMUWINEPROFILE_DRAWPRIM DrawPrimProfile;
#endif
};
...
# define SFLAG_DONOTFREE_NEMUWDDM SFLAG_CLIENTMEM
#else
# define SFLAG_DONOTFREE_NEMUWDDM 0
#endif
...
#ifdef NEMU_WINE_WITH_SHADER_CACHE
    NEMUEXT_HASHCACHE_ENTRY CacheEntry;
    uint32_t u32CacheDataInited;
    uint32_t u32Hash;
    PNEMUEXT_HASHCACHE shaderCache;
#endif
};
...
# else
#  define _NEMU_BITVAL_CAST(_t, _f) (*((const _t*)((const void*)(&(_f)))))
#  define _NEMU_BITVAL_TO_FLOATL(_f) _NEMU_BITVAL_CAST(float, _f)
#  define _NEMU_BITVAL_FROM_FLOAT(_f) _NEMU_BITVAL_CAST(uint32_t, _f)
DECLINLINE(float) copysignf(float val, float sign)
{
    uint32_t u32Val = ((_NEMU_BITVAL_FROM_FLOAT(val) & 0x7fffffff) | (_NEMU_BITVAL_FROM_FLOAT(sign) & 0x80000000));
    return _NEMU_BITVAL_TO_FLOATL(u32Val);
}
# endif
