...
static PNEMUHGCMSVCHELPERS g_pHelpers = NULL;
                                  ...
                                  uint32_t cbEvent)
{
    if (cbEvent > 0)
    {
        ...
        NemuHGCMParmPtrGet(&paParms[2], &pvParm, &cbParm);
    ...
    NemuHGCMParmUInt32Set(&paParms[0], u32ChannelHandle);
    NemuHGCMParmUInt32Set(&paParms[1], u32Id);
    NemuHGCMParmUInt32Set(&paParms[3], cbEvent);
}
                                ...
                                uint32_t cbEvent)
{
    ...
    nemuHostChannelEventParmsSet(pClient->async.paParms,
                                 u32ChannelHandle,
                                 u32Id,
                                 pvEvent,
                                 cbEvent);
...
static DECLCALLBACK(int) svcDisconnect(void *pvService, uint32_t u32ClientID, void *pvClient)
{
    ...
    NEMUHOSTCHCLIENT *pClient = (NEMUHOSTCHCLIENT *)pvClient;

    nemuHostChannelClientDisconnect(pClient);

    memset(pClient, 0, sizeof(NEMUHOSTCHCLIENT));
...
static DECLCALLBACK(int) svcConnect(void *pvService, uint32_t u32ClientID, void *pvClient)
{
    ...
    rc = nemuHostChannelClientConnect(pClient);
                                  ...
                                  NEMUHGCMSVCPARM paParms[])
{
    ...
    switch (u32Function)
    {
        case NEMU_HOST_CHANNEL_FN_ATTACH:
        {
            LogRel2(("svcCall: NEMU_HOST_CHANNEL_FN_ATTACH\n"));

            if (cParms != 3)
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else if (   paParms[0].type != NEMU_HGCM_SVC_PARM_PTR     /* name */
                     || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT   /* flags */
                     || paParms[2].type != NEMU_HGCM_SVC_PARM_32BIT   /* handle */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                rc = NemuHGCMParmPtrGet(&paParms[0], &pvName, &cbName);

                if (RT_SUCCESS(rc))
                {
                    rc = NemuHGCMParmUInt32Get(&paParms[1], &u32Flags);
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        ...
                        rc = nemuHostChannelAttach(pClient, &u32Handle, (const char *)pvName, u32Flags);

                        if (RT_SUCCESS(rc))
                        {
                            NemuHGCMParmUInt32Set(&paParms[2], u32Handle);
                        }
                    }
                }
            }
        ...
        case NEMU_HOST_CHANNEL_FN_DETACH:
        {
            LogRel2(("svcCall: NEMU_HOST_CHANNEL_FN_DETACH\n"));

            if (cParms != 1)
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else if (   paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT   /* handle */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                rc = NemuHGCMParmUInt32Get(&paParms[0], &u32Handle);

                if (RT_SUCCESS(rc))
                {
                    rc = nemuHostChannelDetach(pClient, u32Handle);
                }
            }
        } break;

        case NEMU_HOST_CHANNEL_FN_SEND:
        {
            LogRel2(("svcCall: NEMU_HOST_CHANNEL_FN_SEND\n"));
            ...
            if (cParms != 2)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != NEMU_HGCM_SVC_PARM_PTR     /* data */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                if (RT_SUCCESS (rc))
                {
                    rc = NemuHGCMParmPtrGet (&paParms[1], &pvData, &cbData);

                    if (RT_SUCCESS (rc))
                    {
                        rc = nemuHostChannelSend(pClient, u32Handle, pvData, cbData);
                    }
                }
            }
        } break;

        case NEMU_HOST_CHANNEL_FN_RECV:
        {
            LogRel2(("svcCall: NEMU_HOST_CHANNEL_FN_RECV\n"));
            ...
            if (cParms != 4)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != NEMU_HGCM_SVC_PARM_PTR     /* data */
                     || paParms[2].type != NEMU_HGCM_SVC_PARM_32BIT   /* sizeReceived */
                     || paParms[3].type != NEMU_HGCM_SVC_PARM_32BIT   /* sizeRemaining */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                if (RT_SUCCESS (rc))
                {
                    ...
                    if (RT_SUCCESS (rc))
                    {
                        ...
                        rc = nemuHostChannelRecv(pClient, u32Handle,
                                                 pvData, cbData,
                                                 &u32SizeReceived, &u32SizeRemaining);

                        if (RT_SUCCESS(rc))
                        {
                            NemuHGCMParmUInt32Set(&paParms[2], u32SizeReceived);
                            NemuHGCMParmUInt32Set(&paParms[3], u32SizeRemaining);
                        }
                    }
                }
            }
        ...
        case NEMU_HOST_CHANNEL_FN_CONTROL:
        {
            LogRel2(("svcCall: NEMU_HOST_CHANNEL_FN_CONTROL\n"));
            ...
            if (cParms != 5)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT   /* code */
                     || paParms[2].type != NEMU_HGCM_SVC_PARM_PTR     /* parm */
                     || paParms[3].type != NEMU_HGCM_SVC_PARM_PTR     /* data */
                     || paParms[4].type != NEMU_HGCM_SVC_PARM_32BIT   /* sizeDataReturned */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                if (RT_SUCCESS (rc))
                {
                    rc = NemuHGCMParmUInt32Get (&paParms[1], &u32Code);

                    if (RT_SUCCESS (rc))
                    {
                        rc = NemuHGCMParmPtrGet (&paParms[2], &pvParm, &cbParm);

                        if (RT_SUCCESS (rc))
                        {
                            rc = NemuHGCMParmPtrGet (&paParms[3], &pvData, &cbData);

                            if (RT_SUCCESS (rc))
                            {
                                uint32_t u32SizeDataReturned = 0;

                                rc = nemuHostChannelControl(pClient, u32Handle, u32Code,
                                                            pvParm, cbParm,
                                                            pvData, cbData, &u32SizeDataReturned);
                                if (RT_SUCCESS(rc))
                                {
                                    NemuHGCMParmUInt32Set(&paParms[4], u32SizeDataReturned);
                                }
                            }
                        }
                    }
        ...
        case NEMU_HOST_CHANNEL_FN_EVENT_WAIT:
        {
            LogRel2(("svcCall: NEMU_HOST_CHANNEL_FN_EVENT_WAIT\n"));
            ...
            if (cParms != 4)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT   /* id */
                     || paParms[2].type != NEMU_HGCM_SVC_PARM_PTR     /* parm */
                     || paParms[3].type != NEMU_HGCM_SVC_PARM_32BIT   /* sizeReturned */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                rc = nemuHostChannelEventWait(pClient, &fEvent, callHandle, paParms);
        ...
        case NEMU_HOST_CHANNEL_FN_EVENT_CANCEL:
        {
            LogRel2(("svcCall: NEMU_HOST_CHANNEL_FN_EVENT_CANCEL\n"));
            ...
            {
                rc = nemuHostChannelEventCancel(pClient);
            }
        } break;

        case NEMU_HOST_CHANNEL_FN_QUERY:
        {
            LogRel2(("svcCall: NEMU_HOST_CHANNEL_FN_QUERY\n"));

            if (cParms != 5)
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else if (   paParms[0].type != NEMU_HGCM_SVC_PARM_PTR     /* channel name */
                     || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT   /* code */
                     || paParms[2].type != NEMU_HGCM_SVC_PARM_PTR     /* parm */
                     || paParms[3].type != NEMU_HGCM_SVC_PARM_PTR     /* data */
                     || paParms[4].type != NEMU_HGCM_SVC_PARM_32BIT   /* sizeDataReturned */
                    ...
                    if (RT_SUCCESS (rc))
                    {
                        ...
                        if (RT_SUCCESS (rc))
                        {
                            ...
                            if (RT_SUCCESS (rc))
                            {
                                ...
                                /* @todo make sure that pvName is a nul terminated */
                                rc = nemuHostChannelQuery(pClient, (const char *)pvName, u32Code,
                                                          pvParm, cbParm,
                                                          pvData, cbData, &u32SizeDataReturned);
                                if (RT_SUCCESS(rc))
                                {
                                     ...
                                     NEMUHGCMSVCPARM paParms[])
{
    ...
    switch (u32Function)
    {
        case NEMU_HOST_CHANNEL_HOST_FN_REGISTER:
        {
            LogRel2(("svcCall: NEMU_HOST_CHANNEL_HOST_FN_REGISTER\n"));
            ...
            if (cParms != 2)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != NEMU_HGCM_SVC_PARM_PTR /* iface */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
            else
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    rc = NemuHGCMParmPtrGet(&paParms[1], &pvInterface, &cbInterface);

                    if (RT_SUCCESS(rc))
                    {
                        rc = nemuHostChannelRegister((const char *)pvName,
                                                     (NEMUHOSTCHANNELINTERFACE *)pvInterface, cbInterface);
                    }
                }
            }
        } break;

        case NEMU_HOST_CHANNEL_HOST_FN_UNREGISTER:
        {
            LogRel2(("svcCall: NEMU_HOST_CHANNEL_HOST_FN_UNREGISTER\n"));
            ...
            else
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    rc = nemuHostChannelUnregister((const char *)pvName);
                }
            }
        } break;
...
void nemuSvcClipboardCompleteReadData(NEMUHOSTCHCLIENT *pClient, int rc, uint32_t cbActual)
{
...
/**
 * SSM descriptor table for the NEMUHOSTCHCLIENT structure.
 */
static SSMFIELD const g_aClipboardClientDataFields[] =
{
    SSMFIELD_ENTRY(NEMUHOSTCHCLIENT, u32ClientID),  /* for validation purposes */
    SSMFIELD_ENTRY(NEMUHOSTCHCLIENT, fMsgQuit),
    SSMFIELD_ENTRY(NEMUHOSTCHCLIENT, fMsgReadData),
    SSMFIELD_ENTRY(NEMUHOSTCHCLIENT, fMsgFormats),
    SSMFIELD_ENTRY(NEMUHOSTCHCLIENT, u32RequestedFormat),
    SSMFIELD_ENTRY_TERM()
};
...
static DECLCALLBACK(int) svcSaveState(void *pvService, uint32_t u32ClientID, void *pvClient, PSSMHANDLE pSSM)
{
    ...
    nemuSvcClipboardCompleteReadData (pClient, VINF_SUCCESS, 0);
 ...
 * NEMUHOSTCHCLIENT structure.  As the structure was saved as a whole
...
extern "C" DECLCALLBACK(DECLEXPORT(int)) NemuHGCMSvcLoad(NEMUHGCMSVCFNTABLE *pTable)
{
    ...
    else
    {
        ...
        else
        {
            ...
            pTable->cbClient = sizeof(NEMUHOSTCHCLIENT);
