...
NemuDbgConsoleOutput::NemuDbgConsoleOutput(QWidget *pParent/* = NULL*/, const char *pszName/* = NULL*/)
...
NemuDbgConsoleOutput::~NemuDbgConsoleOutput()
{
...
void
NemuDbgConsoleOutput::contextMenuEvent(QContextMenuEvent *pEvent)
{
    /*
...
void
NemuDbgConsoleOutput::setColorGreenOnBlack()
{
...
void
NemuDbgConsoleOutput::setColorBlackOnWhite()
{
...
void
NemuDbgConsoleOutput::setFontCourier()
{
#ifdef Q_WS_MAC
...
void
NemuDbgConsoleOutput::setFontMonospace()
{
    QFont Font = font();
    Font.setStyleHint(QFont::TypeWriter);
...
void
NemuDbgConsoleOutput::appendText(const QString &rStr, bool fClearSelection)
{
    Assert(m_hGUIThread == RTThreadNativeSelf());
...
NemuDbgConsoleInput::NemuDbgConsoleInput(QWidget *pParent/* = NULL*/, const char *pszName/* = NULL*/)
...
NemuDbgConsoleInput::~NemuDbgConsoleInput()
{
    Assert(m_hGUIThread == RTThreadNativeSelf());
}
...
void
NemuDbgConsoleInput::setLineEdit(QLineEdit *pEdit)
{
    Assert(m_hGUIThread == RTThreadNativeSelf());
...
void
NemuDbgConsoleInput::returnPressed()
{
    Assert(m_hGUIThread == RTThreadNativeSelf());
...
NemuDbgConsole::NemuDbgConsole(NemuDbgGui *a_pDbgGui, QWidget *a_pParent/* = NULL*/)
    : NemuDbgBaseWindow(a_pDbgGui, a_pParent), m_pOutput(NULL), m_pInput(NULL), m_fInputRestoreFocus(false),
...
{
    setWindowTitle("NemuDbg - Console");

    /*
     * Create the output text box.
     */
    m_pOutput = new NemuDbgConsoleOutput(this);
    ...
    m_pInput = new NemuDbgConsoleInput(NULL);
...
# endif
    ...
    rc = RTThreadCreate(&m_Thread, backThread, this, 0, RTTHREADTYPE_DEBUGGER, RTTHREADFLAGS_WAITABLE, "NemuDbgC");
    AssertRC(rc);
    if (RT_FAILURE(rc))
...
void
NemuDbgConsole::commandSubmitted(const QString &rCommand)
{
    Assert(isGUIThread());
    ...
    Log(("NemuDbgConsole::commandSubmitted: %s (input-enabled=%RTbool)\n", psz, m_pInput->isEnabled()));
    unlock();
}


void
NemuDbgConsole::updateOutput()
{
    Assert(isGUIThread());
...
NemuDbgConsole::backInput(PDBGCBACK pBack, uint32_t cMillies)
{
    NemuDbgConsole *pThis = NEMUDBGCONSOLE_FROM_DBGCBACK(pBack);
    pThis->lock();
...
NemuDbgConsole::backRead(PDBGCBACK pBack, void *pvBuf, size_t cbBuf, size_t *pcbRead)
{
    NemuDbgConsole *pThis = NEMUDBGCONSOLE_FROM_DBGCBACK(pBack);
 ...
 */
/*static*/ DECLCALLBACK(int)
NemuDbgConsole::backWrite(PDBGCBACK pBack, const void *pvBuf, size_t cbBuf, size_t *pcbWritten)
{
    NemuDbgConsole *pThis = NEMUDBGCONSOLE_FROM_DBGCBACK(pBack);
    int rc = VINF_SUCCESS;
    ...
    if (!pThis->m_fUpdatePending)
        QApplication::postEvent(pThis, new NemuDbgConsoleEvent(NemuDbgConsoleEvent::kUpdate));
...
NemuDbgConsole::backSetReady(PDBGCBACK pBack, bool fReady)
{
    NemuDbgConsole *pThis = NEMUDBGCONSOLE_FROM_DBGCBACK(pBack);
    if (fReady)
        QApplication::postEvent(pThis, new NemuDbgConsoleEvent(NemuDbgConsoleEvent::kInputEnable));
}
 ...
 */
/*static*/ DECLCALLBACK(int)
NemuDbgConsole::backThread(RTTHREAD Thread, void *pvUser)
{
    NemuDbgConsole *pThis = (NemuDbgConsole *)pvUser;
    ...
    if (!ASMAtomicUoReadBool(&pThis->m_fTerminate))
        QApplication::postEvent(pThis, new NemuDbgConsoleEvent(rc == VINF_SUCCESS
                                                               ? NemuDbgConsoleEvent::kTerminatedUser
                                                               : NemuDbgConsoleEvent::kTerminatedOther));
...
bool
NemuDbgConsole::event(QEvent *pGenEvent)
{
    Assert(isGUIThread());
    if (pGenEvent->type() == (QEvent::Type)NemuDbgConsoleEvent::kEventNumber)
    {
        NemuDbgConsoleEvent *pEvent = (NemuDbgConsoleEvent *)pGenEvent;

        switch (pEvent->command())
        {
            /* make update pending. */
            case NemuDbgConsoleEvent::kUpdate:
                lock();
                if (!m_fUpdatePending)
                {
            ...
            case NemuDbgConsoleEvent::kInputEnable:
                Log(("NemuDbgConsole: kInputEnable (input-enabled=%RTbool)\n", m_pInput->isEnabled()));
            ...
            case NemuDbgConsoleEvent::kTerminatedUser:
                Log(("NemuDbgConsole: kTerminatedUser (input-enabled=%RTbool)\n", m_pInput->isEnabled()));
                m_pInput->setEnabled(false);
                close();
                break;

            /* The thread terminated for some unknown reason., disable input */
            case NemuDbgConsoleEvent::kTerminatedOther:
                Log(("NemuDbgConsole: kTerminatedOther (input-enabled=%RTbool)\n", m_pInput->isEnabled()));
                m_pInput->setEnabled(false);
                break;
    ...
    return NemuDbgBaseWindow::event(pGenEvent);
}


void
NemuDbgConsole::closeEvent(QCloseEvent *a_pCloseEvt)
{
...
void
NemuDbgConsole::actFocusToInput()
{
...
void
NemuDbgConsole::actFocusToOutput()
{
