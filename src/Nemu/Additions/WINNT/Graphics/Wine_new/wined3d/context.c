...
# define nemuGetCurrentContext() NemuTlsRefGetCurrent(struct wined3d_context, wined3d_context_tls_idx)
# define nemuSetCurrentContext(_ctx) NemuTlsRefSetCurrent(struct wined3d_context, wined3d_context_tls_idx, (_ctx))
#endif
...
#ifdef NEMU_WINE_WITH_SINGLE_CONTEXT
...
#if defined(DEBUG) || !defined(NEMU_WITH_WDDM)
    {
        HWND hWnd = WindowFromDC(swapchain->hDC);
        if (hWnd != swapchain->win_handle)
        {
# if defined(NEMU_WITH_WDDM)
...
#else
    if (!(context->hdc = NemuExtGetDC(context->win_handle)))
#endif
    {
...
#  ifndef NEMU
    wined3d_release_dc(context->win_handle, context->hdc);
#  else
    NemuExtReleaseDC(context->win_handle, context->hdc);
#  endif
# endif
#endif
...
//    NemuExtReleaseContextAsync(old);
}
#endif
...
#if defined(NEMU_WINE_WITH_SINGLE_CONTEXT) || defined(NEMU_WINE_WITH_SINGLE_SWAPCHAIN_CONTEXT)
    NemuTlsRefInit(ret, context_tls_dtor);
#endif
...
#ifdef NEMU_WITH_WDDM
        pNemuCtxChromiumParameteriCR(ctx, GL_HOST_WND_CREATED_HIDDEN_CR,  GL_TRUE);
#endif
