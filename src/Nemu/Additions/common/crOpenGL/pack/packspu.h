...
extern uint32_t g_u32NemuHostCaps;
...
#define CRPACKSPU_WRITEBACK_WAIT(_thread, _writeback) do {\
        if (g_u32NemuHostCaps & CR_NEMU_CAP_CMDVBVA) { \
...
#if defined(WINDOWS) && defined(NEMU_WITH_WDDM) && defined(NEMU_WITH_CRHGSMI) && defined(IN_GUEST)
...
extern void packspuSetNemuConfiguration( const SPU *child_spu );
...
extern ThreadInfo *packspuNewCtxThread( struct NEMUUHGSMI *pHgsmi );
