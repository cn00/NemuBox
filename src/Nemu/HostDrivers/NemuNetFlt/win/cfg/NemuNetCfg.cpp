/* $Id: NemuNetCfg.cpp $ */
/** @file
 * NemuNetCfg.cpp - Network Configuration API.
 */
/*
...
#define NEMU_NETCFG_LOCK_TIME_OUT     5000  /** @todo r=bird: What does this do? */
...
static HRESULT nemuNetCfgWinINetCfgLock(IN INetCfg *pNetCfg,
...
static HRESULT nemuNetCfgWinINetCfgUnlock(IN INetCfg *pNetCfg)
{
    INetCfgLock *pLock;
    HRESULT hr = pNetCfg->QueryInterface(IID_INetCfgLock, (PVOID*)&pLock);
    if (FAILED(hr))
                                                      ...
                                                      OUT LPWSTR *ppszwClientDescription)
{
    ...
    if (fGetWriteLock)
    {
        hr = nemuNetCfgWinINetCfgLock(pNetCfg, pszwClientDescription, cmsTimeout, ppszwClientDescription);
        if (hr == S_FALSE)
        {
...
NEMUNETCFGWIN_DECL(HRESULT) NemuNetCfgWinReleaseINetCfg(IN INetCfg *pNetCfg, IN BOOL fHasWriteLock)
{
    ...
    if (fHasWriteLock)
    {
        HRESULT hr2 = nemuNetCfgWinINetCfgUnlock(pNetCfg);
        if (FAILED(hr2))
            NonStandardLogFlow(("nemuNetCfgWinINetCfgUnlock failed, hr (0x%x)\n", hr2));
        if (SUCCEEDED(hr))
            hr = hr2;
    }
...
static HRESULT nemuNetCfgWinGetComponentByGuidEnum(IEnumNetCfgComponent *pEnumNcc,
                                                   IN const GUID *pGuid,
...
NEMUNETCFGWIN_DECL(HRESULT) NemuNetCfgWinGetComponentByGuid(IN INetCfg *pNc,
                                                            ...
                                                            OUT INetCfgComponent **ppncc)
{
    ...
    if (SUCCEEDED(hr))
    {
        hr = nemuNetCfgWinGetComponentByGuidEnum(pEnumNcc, pComponentGuid, ppncc);
        if (hr == S_FALSE)
        {
        ...
        {
            NonStandardLogFlow(("nemuNetCfgWinGetComponentByGuidEnum failed, hr (0x%x)\n", hr));
        }
        pEnumNcc->Release();
    }
    else
...
static HRESULT nemuNetCfgWinQueryInstaller(IN INetCfg *pNetCfg, IN const GUID *pguidClass, INetCfgClassSetup **ppSetup)
{
...
NEMUNETCFGWIN_DECL(HRESULT) NemuNetCfgWinInstallComponent(IN INetCfg *pNetCfg, IN LPCWSTR pszwComponentId, IN const GUID *pguidClass,
                                                          OUT INetCfgComponent **ppComponent)
{
    INetCfgClassSetup *pSetup;
    HRESULT hr = nemuNetCfgWinQueryInstaller(pNetCfg, pguidClass, &pSetup);
    if (FAILED(hr))
    {
        NonStandardLogFlow(("nemuNetCfgWinQueryInstaller failed, hr (0x%x)\n", hr));
        return hr;
    }
...
static HRESULT nemuNetCfgWinInstallInfAndComponent(IN INetCfg *pNetCfg, IN LPCWSTR pszwComponentId, IN const GUID *pguidClass,
                                                   ...
                                                   OUT INetCfgComponent **ppComponent)
{
    ...
    for (; cFilesProcessed < cInfPaths; cFilesProcessed++)
    {
        ...
        hr = NemuDrvCfgInfInstall(apInfPaths[cFilesProcessed]);
        if (FAILED(hr))
        {
            NonStandardLogFlow(("NemuNetCfgWinInfInstall failed, hr (0x%x)\n", hr));
            break;
        }
    }
    ...
    if (SUCCEEDED(hr))
    {
        hr = NemuNetCfgWinInstallComponent(pNetCfg, pszwComponentId, pguidClass, ppComponent);
        if (FAILED(hr))
            NonStandardLogFlow(("NemuNetCfgWinInstallComponent failed, hr (0x%x)\n", hr));
    }
    ...
    if (FAILED(hr))
    {
        ...
        do
        {
            HRESULT hr2 = NemuDrvCfgInfUninstall(apInfPaths[cFilesProcessed], 0);
            if (FAILED(hr2))
                NonStandardLogFlow(("NemuDrvCfgInfUninstall failed, hr (0x%x)\n", hr2));
                /* Keep going. */
            if (!cFilesProcessed)
                break;
...
NEMUNETCFGWIN_DECL(HRESULT) NemuNetCfgWinUninstallComponent(IN INetCfg *pNetCfg, IN INetCfgComponent *pComponent)
{
    GUID GuidClass;
    ...
    hr = nemuNetCfgWinQueryInstaller(pNetCfg, &GuidClass, &pSetup);
    if (FAILED(hr))
    {
        NonStandardLogFlow(("nemuNetCfgWinQueryInstaller failed, hr (0x%x)\n", hr));
        return hr;
...
typedef BOOL (*NEMUNETCFGWIN_NETCFGENUM_CALLBACK) (IN INetCfg *pNetCfg, IN INetCfgComponent *pNetCfgComponent, PVOID pContext);

static HRESULT nemuNetCfgWinEnumNetCfgComponents(IN INetCfg *pNetCfg,
                                                 IN const GUID *pguidClass,
                                                 NEMUNETCFGWIN_NETCFGENUM_CALLBACK callback,
                                                 PVOID pContext)
{
...
/*
 * Forward declarations of functions used in nemuNetCfgWinRemoveAllNetDevicesOfIdCallback.
 */
NEMUNETCFGWIN_DECL(HRESULT) NemuNetCfgWinGenHostonlyConnectionName(PCWSTR DevName, WCHAR *pBuf, PULONG pcbBuf);
NEMUNETCFGWIN_DECL(HRESULT) NemuNetCfgWinRenameConnection(LPWSTR pGuid, PCWSTR NewName);

static BOOL nemuNetCfgWinRemoveAllNetDevicesOfIdCallback(HDEVINFO hDevInfo, PSP_DEVINFO_DATA pDev, PVOID pContext)
{
                                     ...
                                     &rmdParams.ClassInstallHeader, sizeof(rmdParams)))
    {
        if (SetupDiSetSelectedDevice(hDevInfo, pDev))
        {
            ...
            {
                NonStandardLogFlow(("nemuNetCfgWinRemoveAllNetDevicesOfIdCallback: SetupDiOpenDevRegKey failed with error %ld\n",
                                    GetLastError()));
            }
            else
            {
                ...
                {
                    NonStandardLogFlow(("nemuNetCfgWinRemoveAllNetDevicesOfIdCallback: Processing device ID \"%S\"\n",
                                        wszCfgGuidString));
                                                          ...
                                                          NULL                /* OUT PDWORD RequiredSize OPTIONAL*/))
                    {
                         ...
                         */
                        HRESULT hr = NemuNetCfgWinGenHostonlyConnectionName(wszDevName, wszTempName, &cbName);
                        wcscat_s(wszTempName, sizeof(wszTempName), L" removed");
                        if (SUCCEEDED(hr))
                            hr = NemuNetCfgWinRenameConnection(wszCfgGuidString, wszTempName);
                        //NonStandardLogFlow(("NemuNetCfgWinRenameConnection(%S,%S) => 0x%x\n", pWCfgGuidString, TempName, hr_tmp));
                    }
                    else
                    {
                        NonStandardLogFlow(("nemuNetCfgWinRemoveAllNetDevicesOfIdCallback: Failed to get friendly name for device \"%S\"\n",
                                            wszCfgGuidString));
                    }
                }
                else
                {
                    NonStandardLogFlow(("nemuNetCfgWinRemoveAllNetDevicesOfIdCallback: Querying instance ID failed with %d\n",
                                        ret));
                }
            ...
            if (SetupDiCallClassInstaller(DIF_REMOVE, hDevInfo, pDev))
            {
                ...
                if (SetupDiGetDeviceInstallParams(hDevInfo, pDev, &devParams))
                {
                    ...
                    {
                        NonStandardLog(("nemuNetCfgWinRemoveAllNetDevicesOfIdCallback: A reboot is required\n"));
                    }
                }
                else
                    NonStandardLogFlow(("nemuNetCfgWinRemoveAllNetDevicesOfIdCallback: SetupDiGetDeviceInstallParams failed with %ld\n",
                                        GetLastError()));
            }
            else
                NonStandardLogFlow(("nemuNetCfgWinRemoveAllNetDevicesOfIdCallback: SetupDiCallClassInstaller failed with %ld\n",
                                    GetLastError()));
        }
        else
            NonStandardLogFlow(("nemuNetCfgWinRemoveAllNetDevicesOfIdCallback: SetupDiSetSelectedDevice failed with %ld\n",
                                GetLastError()));
    }
    else
        NonStandardLogFlow(("nemuNetCfgWinRemoveAllNetDevicesOfIdCallback: SetupDiSetClassInstallParams failed with %ld\n",
                            GetLastError()));
...
typedef struct NEMUNECTFGWINPROPCHANGE
{
    NEMUNECTFGWINPROPCHANGE_TYPE enmPcType;
    HRESULT hr;
} NEMUNECTFGWINPROPCHANGE ,*PNEMUNECTFGWINPROPCHANGE;

static BOOL nemuNetCfgWinPropChangeAllNetDevicesOfIdCallback(HDEVINFO hDevInfo, PSP_DEVINFO_DATA pDev, PVOID pContext)
{
    PNEMUNECTFGWINPROPCHANGE pPc = (PNEMUNECTFGWINPROPCHANGE)pContext;
    ...
    {
        case NEMUNECTFGWINPROPCHANGE_TYPE_DISABLE:
            PcParams.StateChange = DICS_DISABLE;
            NonStandardLogFlow(("nemuNetCfgWinPropChangeAllNetDevicesOfIdCallback: Change type (DICS_DISABLE): %d\n", pPc->enmPcType));
            break;
        case NEMUNECTFGWINPROPCHANGE_TYPE_ENABLE:
            PcParams.StateChange = DICS_ENABLE;
            NonStandardLogFlow(("nemuNetCfgWinPropChangeAllNetDevicesOfIdCallback: Change type (DICS_ENABLE): %d\n", pPc->enmPcType));
            break;
        default:
            NonStandardLogFlow(("nemuNetCfgWinPropChangeAllNetDevicesOfIdCallback: Unexpected prop change type: %d\n", pPc->enmPcType));
            pPc->hr = E_INVALIDARG;
            return FALSE;
    }
            ...
            if (SetupDiCallClassInstaller(DIF_PROPERTYCHANGE, hDevInfo, pDev))
            {
                ...
                if (SetupDiGetDeviceInstallParams(hDevInfo,pDev,&devParams))
                {
                    if (   (devParams.Flags & DI_NEEDRESTART)
                        || (devParams.Flags & DI_NEEDREBOOT))
                    {
                        NonStandardLog(("nemuNetCfgWinPropChangeAllNetDevicesOfIdCallback: A reboot is required\n"));
                    }
                }
                else
                    NonStandardLogFlow(("nemuNetCfgWinPropChangeAllNetDevicesOfIdCallback: SetupDiGetDeviceInstallParams failed with %ld\n",
                                        GetLastError()));
            }
            else
                NonStandardLogFlow(("nemuNetCfgWinPropChangeAllNetDevicesOfIdCallback: SetupDiCallClassInstaller failed with %ld\n",
                                    GetLastError()));
        }
        else
...
typedef BOOL (*NEMUNETCFGWIN_NETENUM_CALLBACK) (HDEVINFO hDevInfo, PSP_DEVINFO_DATA pDev, PVOID pContext);
NEMUNETCFGWIN_DECL(HRESULT) NemuNetCfgWinEnumNetDevices(LPCWSTR pwszPnPId,
                                                        NEMUNETCFGWIN_NETENUM_CALLBACK callback, PVOID pContext)
{
    NonStandardLogFlow(("NemuNetCfgWinEnumNetDevices: Searching for: %S\n", pwszPnPId));
    ...
    if (hDevInfo != INVALID_HANDLE_VALUE)
    {
        ...
        for (;;)
        {
            ...
            NonStandardLogFlow(("NemuNetCfgWinEnumNetDevices: Enumerating device %ld ... \n", dwDevId));
            dwDevId++;
                                                   ...
                                                   &cbRequired       /* OUT PDWORD RequiredSize OPTIONAL */))
            {
                ...
                {
                    NonStandardLogFlow(("NemuNetCfgWinEnumNetDevices: SetupDiGetDeviceRegistryPropertyW (1) failed with %ld\n", winEr));
                    break;
                }
                ...
                if (!pBuffer)
                {
                    NonStandardLogFlow(("NemuNetCfgWinEnumNetDevices: Out of memory allocating %ld bytes\n",
                                        cbRequired));
                ...
                {
                    winEr = GetLastError();
                    NonStandardLogFlow(("NemuNetCfgWinEnumNetDevices: SetupDiGetDeviceRegistryPropertyW (2) failed with %ld\n",
                                        winEr));
                    break;
                }
            }
            ...
            NonStandardLogFlow(("NemuNetCfgWinEnumNetDevices: Device %ld: %S\n", dwDevId, pCurId));
        ...
        NonStandardLogFlow(("NemuNetCfgWinEnumNetDevices: Found %ld devices total\n", dwDevId));
    ...
    else
    {
        ...
        NonStandardLogFlow(("NemuNetCfgWinEnumNetDevices: SetupDiGetClassDevsExW failed with %ld\n", winEr));
        hr = HRESULT_FROM_WIN32(winEr);
    }

    NonStandardLogFlow(("NemuNetCfgWinEnumNetDevices: Ended with hr (0x%x)\n", hr));
    return hr;
}

NEMUNETCFGWIN_DECL(HRESULT) NemuNetCfgWinRemoveAllNetDevicesOfId(IN LPCWSTR lpszPnPId)
{
    return NemuNetCfgWinEnumNetDevices(lpszPnPId, nemuNetCfgWinRemoveAllNetDevicesOfIdCallback, NULL);
}
...
{
    NEMUNECTFGWINPROPCHANGE Pc;
    Pc.enmPcType = enmPcType;
    Pc.hr = S_OK;
    NonStandardLogFlow(("Calling NemuNetCfgWinEnumNetDevices with lpszPnPId =(%S) and nemuNetCfgWinPropChangeAllNetDevicesOfIdCallback\n", lpszPnPId));

    HRESULT hr = NemuNetCfgWinEnumNetDevices(lpszPnPId, nemuNetCfgWinPropChangeAllNetDevicesOfIdCallback, &Pc);
    if (!SUCCEEDED(hr))
    {
        NonStandardLogFlow(("NemuNetCfgWinEnumNetDevices failed 0x%x\n", hr));
        return hr;
    }

    if (!SUCCEEDED(Pc.hr))
    {
        NonStandardLogFlow(("nemuNetCfgWinPropChangeAllNetDevicesOfIdCallback failed 0x%x\n", Pc.hr));
        return Pc.hr;
    }
...
public:
    ...
    {
        private:
#if !defined (NEMU_WITH_XPCOM)
            STDMETHOD_(ULONG, AddRef)() = 0;
            STDMETHOD_(ULONG, Release)() = 0;
#else /* !defined (NEMU_WITH_XPCOM) */
            NS_IMETHOD_(nsrefcnt) AddRef(void) = 0;
            NS_IMETHOD_(nsrefcnt) Release(void) = 0;
#endif /* !defined (NEMU_WITH_XPCOM) */
    };
...
static HRESULT netIfWinIsHostOnly(IWbemClassObject * pAdapterConfig, BOOL * pbIsHostOnly)
{
    ...
    if (SUCCEEDED(hr))
    {
        *pbIsHostOnly = bstr_t(vtServiceName.bstrVal) == bstr_t("NemuNetAdp");
...
static HRESULT nemuNetCfgWinIsDhcpEnabled(IWbemClassObject * pAdapterConfig, BOOL *pEnabled)
{
    VARIANT vtEnabled;
...
NEMUNETCFGWIN_DECL(HRESULT) NemuNetCfgWinGetAdapterSettings(IN const GUID * pGuid, OUT PADAPTER_SETTINGS pSettings)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            hr = nemuNetCfgWinIsDhcpEnabled(pAdapterConfig, &pSettings->bDhcp);
            if (SUCCEEDED(hr))
...
NEMUNETCFGWIN_DECL(HRESULT) NemuNetCfgWinIsDhcpEnabled(const GUID * pGuid, BOOL *pEnabled)
{
    HRESULT hr;
    ComPtr <IWbemServices> pSvc;
    hr = netIfWinCreateIWbemServices(pSvc.asOutParam());
...
{
    NonStandardLogFlow(("NemuNetCfgWinEnableStaticIpConfig: ip=0x%x mask=0x%x\n", ip, mask));
    ComPtr<IWbemServices> pSvc;
    ...
    NonStandardLogFlow(("NemuNetCfgWinEnableStaticIpConfig: returns 0x%x\n", hr));
    return hr;
}
...
static const char *nemuNetCfgWinAddrToStr(char *pszBuf, LPSOCKADDR pAddr)
{
...
typedef bool (*PFNNEMUNETCFG_IPSETTINGS_CALLBACK) (ULONG ip, ULONG mask, PVOID pContext);

static void nemuNetCfgWinEnumIpConfig(PIP_ADAPTER_ADDRESSES pAddresses, PFNNEMUNETCFG_IPSETTINGS_CALLBACK pfnCallback, PVOID pContext)
{
    ...
    for (pAdapter = pAddresses; pAdapter; pAdapter = pAdapter->Next)
    {
        ...
        {
            const char *pcszAddress = nemuNetCfgWinAddrToStr(szBuf, pPrefix->Address.lpSockaddr);
...
static bool nemuNetCfgWinIpProbeCallback (ULONG ip, ULONG mask, PVOID pContext)
{
    ...
    if (dwRc == NO_ERROR)
    {
        ...
        *pNetMask = 0;
        ...
        for (int i = 0; i < 255; i++)
        {
            ...
            nemuNetCfgWinEnumIpConfig(pAddresses, nemuNetCfgWinIpProbeCallback, &Context);
 ...
 */
#define NEMUNETCFGWIN_NETFLT_ID    L"sun_NemuNetFlt"
#define NEMUNETCFGWIN_NETFLT_MP_ID L"sun_NemuNetFltmp"

static HRESULT nemuNetCfgWinNetFltUninstall(IN INetCfg *pNc, DWORD InfRmFlags)
{
    INetCfgComponent *pNcc = NULL;
    HRESULT hr = pNc->FindComponent(NEMUNETCFGWIN_NETFLT_ID, &pNcc);
    if (hr == S_OK)
    {
        NonStandardLog("NetFlt is installed currently, uninstalling ...\n");

        hr = NemuNetCfgWinUninstallComponent(pNc, pNcc);
    ...
    NemuDrvCfgInfUninstallAllF(L"NetService", NEMUNETCFGWIN_NETFLT_ID, InfRmFlags);
    NemuDrvCfgInfUninstallAllF(L"Net", NEMUNETCFGWIN_NETFLT_MP_ID, InfRmFlags);
...
{
    return nemuNetCfgWinNetFltUninstall(pNc, 0);
}

NEMUNETCFGWIN_DECL(HRESULT) NemuNetCfgWinNetFltInstall(IN INetCfg *pNc,
                                                       IN LPCWSTR const *apInfFullPaths, IN UINT cInfFullPaths)
{
    HRESULT hr = nemuNetCfgWinNetFltUninstall(pNc, SUOI_FORCEDELETE);
    if (SUCCEEDED(hr))
    {
        NonStandardLog("NetFlt will be installed ...\n");
        hr = nemuNetCfgWinInstallInfAndComponent(pNc, NEMUNETCFGWIN_NETFLT_ID,
...
#define NEMUNETCFGWIN_NETADP_ID L"sun_NemuNetAdp"
static HRESULT nemuNetCfgWinNetAdpUninstall(IN INetCfg *pNc, LPCWSTR pwszId, DWORD InfRmFlags)
{
    HRESULT hr = S_OK;
    NonStandardLog("Finding NetAdp driver package and trying to uninstall it ...\n");

    NemuDrvCfgInfUninstallAllF(L"Net", pwszId, InfRmFlags);
...
{
    return nemuNetCfgWinNetAdpUninstall(pNc, pwszId, SUOI_FORCEDELETE);
}

NEMUNETCFGWIN_DECL(HRESULT) NemuNetCfgWinNetAdpInstall(IN INetCfg *pNc,
                                                       IN LPCWSTR const pInfFullPath)
{
    NonStandardLog("NetAdp will be installed ...\n");
    HRESULT hr = nemuNetCfgWinInstallInfAndComponent(pNc, NEMUNETCFGWIN_NETADP_ID,
                                             &GUID_DEVCLASS_NET,
                                             &pInfFullPath,
                                             1,
                                             NULL);
...
#define NEMUNETCFGWIN_NETLWF_ID    L"oracle_NemuNetLwf"

static HRESULT nemuNetCfgWinNetLwfUninstall(IN INetCfg *pNc, DWORD InfRmFlags)
{
    INetCfgComponent * pNcc = NULL;
    HRESULT hr = pNc->FindComponent(NEMUNETCFGWIN_NETLWF_ID, &pNcc);
    if (hr == S_OK)
    {
    ...
    NemuDrvCfgInfUninstallAllF(L"NetService", NEMUNETCFGWIN_NETLWF_ID, InfRmFlags);
...
{
    return nemuNetCfgWinNetLwfUninstall(pNc, 0);
}

static void NemuNetCfgWinFilterLimitWorkaround(void)
{
    /*
...
NEMUNETCFGWIN_DECL(HRESULT) NemuNetCfgWinNetLwfInstall(IN INetCfg *pNc,
                                                       IN LPCWSTR const pInfFullPath)
{
    HRESULT hr = nemuNetCfgWinNetLwfUninstall(pNc, SUOI_FORCEDELETE);
    if (SUCCEEDED(hr))
    {
        NemuNetCfgWinFilterLimitWorkaround();
        NonStandardLog("NetLwf will be installed ...\n");
        hr = nemuNetCfgWinInstallInfAndComponent(pNc, NEMUNETCFGWIN_NETLWF_ID,
                                                 &GUID_DEVCLASS_NETSERVICE,
                                                 &pInfFullPath,
                                                 1,
                                                 NULL);
...
#define NEMU_CONNECTION_NAME L"VirtualBox Host-Only Network"
NEMUNETCFGWIN_DECL(HRESULT) NemuNetCfgWinGenHostonlyConnectionName(PCWSTR DevName, WCHAR *pBuf, PULONG pcbBuf)
{
    const WCHAR * pSuffix = wcsrchr( DevName, L'#' );
    ULONG cbSize = sizeof(NEMU_CONNECTION_NAME);
    ULONG cbSufSize = 0;
    ...
    wcscpy(pBuf, NEMU_CONNECTION_NAME);
    if (pSuffix)
    {
        wcscat(pBuf, L" ");
        wcscat(pBuf, pSuffix);
...
static BOOL nemuNetCfgWinAdjustHostOnlyNetworkInterfacePriority(IN INetCfg *pNc, IN INetCfgComponent *pNcc, PVOID pContext)
{
                                ...
                                if (hr == S_OK)
                                {
                                    ...
                                    if (hr == S_OK)
                                    {
                                            ...
                                            && guid == *pGuid)
                                        {
                                            ...
                                            else if (_wcsnicmp(pwszHwId, NEMUNETCFGWIN_NETLWF_ID,
                                                               sizeof(NEMUNETCFGWIN_NETLWF_ID)/2))
...
static UINT WINAPI nemuNetCfgWinPspFileCallback(
        PVOID Context,
        UINT Notification,
        UINT_PTR Param1,
        UINT_PTR Param2
...
/* The original source of the NemuNetAdp adapter creation/destruction code has the following copyright */
/*
...
#define DRIVERHWID _T("sun_NemuNetAdp")
...
{
    return NemuDrvCfgDrvUpdate(pcsxwId, pcsxwInf, pbRebootRequired);
}

static const char *nemuNetCfgWinGetStateText(DWORD dwState)
{
    switch (dwState)
    {
...
static DWORD nemuNetCfgWinGetNetSetupState(SC_HANDLE hService)
{
    SERVICE_STATUS status;
    status.dwCurrentState = SERVICE_RUNNING;
    if (hService) {
        if (QueryServiceStatus(hService, &status))
            NonStandardLogFlow(("NetSetupSvc %s\n", nemuNetCfgWinGetStateText(status.dwCurrentState)));
        else
...
DECLINLINE(bool) nemuNetCfgWinIsNetSetupRunning(SC_HANDLE hService)
{
    return nemuNetCfgWinGetNetSetupState(hService) == SERVICE_RUNNING;
}

DECLINLINE(bool) nemuNetCfgWinIsNetSetupStopped(SC_HANDLE hService)
{
    return nemuNetCfgWinGetNetSetupState(hService) == SERVICE_STOPPED;
}

static HRESULT nemuNetCfgWinCreateHostOnlyNetworkInterface(IN LPCWSTR pInfPath, IN bool bIsInfPathFile,
                                                           OUT GUID *pGuid, OUT BSTR *lppszName, OUT BSTR *pErrMsg)
{
    ...
    do
    {
        ...
        if (fResult)
        {
            ...
            if (pQueueCallbackContext)
            {
                ...
                DeviceInstallParams.InstallMsgHandler = (PSP_FILE_CALLBACK)nemuNetCfgWinPspFileCallback;
                fResult = SetupDiSetDeviceInstallParams (hDeviceInfo, &DeviceInfoData,
                                                    &DeviceInstallParams);
                if (!fResult)
                {
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        hr = NemuNetCfgWinGenHostonlyConnectionName(DevName, ConnectionName, &cbName);
        if (SUCCEEDED(hr))
            hr = NemuNetCfgWinRenameConnection(pWCfgGuidString, ConnectionName);
#endif
        if (lppszName)
        {
        ...
        hr = NemuNetCfgWinQueryINetCfg(&pNetCfg, TRUE, L"VirtualBox Host-Only Creation",
        ...
        if (hr == S_OK)
        {
            hr = nemuNetCfgWinEnumNetCfgComponents(pNetCfg,
                                                   &GUID_DEVCLASS_NETSERVICE,
                                                   nemuNetCfgWinAdjustHostOnlyNetworkInterfacePriority,
                                                   pGuid);
            if (SUCCEEDED(hr))
            {
                hr = nemuNetCfgWinEnumNetCfgComponents(pNetCfg,
            ...
            NemuNetCfgWinReleaseINetCfg(pNetCfg, TRUE);
        }
        else if (hr == NETCFG_E_NO_WRITE_LOCK && lpszApp)
        {
            ...
            NonStandardLogFlow(("NemuNetCfgWinQueryINetCfg failed, hr 0x%x\n", hr));
    }
...
NEMUNETCFGWIN_DECL(HRESULT) NemuNetCfgWinCreateHostOnlyNetworkInterface(IN LPCWSTR pInfPath, IN bool bIsInfPathFile,
                                                                        OUT GUID *pGuid, OUT BSTR *lppszName, OUT BSTR *pErrMsg)
{
    HRESULT hrc = nemuNetCfgWinCreateHostOnlyNetworkInterface(pInfPath, bIsInfPathFile, pGuid, lppszName, pErrMsg);
    if (hrc == E_ABORT)
    {
         ...
         */
        hrc = nemuNetCfgWinCreateHostOnlyNetworkInterface(pInfPath, bIsInfPathFile, pGuid, lppszName, pErrMsg);
        if (hrc == E_ABORT)
        {
            ...
            if (hSCM)
            {
                ...
                if (hService)
                {
                    for (int retries = 0; retries < 60 && !nemuNetCfgWinIsNetSetupStopped(hService); ++retries)
                        Sleep(1000);
...
#ifdef NEMUNETCFG_DELAYEDRENAME
NEMUNETCFGWIN_DECL(HRESULT) NemuNetCfgWinRenameHostOnlyConnection(IN const GUID *pGuid, IN LPCWSTR pwszId, OUT BSTR *pDevName)
{
    HRESULT hr = S_OK;
    ...
    if (hDevInfo != INVALID_HANDLE_VALUE)
    {
        ...
        if (SetupDiOpenDeviceInfo(hDevInfo, pwszId, NULL, 0, &DevInfoData))
        {
            ...
            {
                hr = NemuNetCfgWinGenHostonlyConnectionName(wszDevName, wszConnectionNewName, &cbName);
                if (SUCCEEDED(hr))
                {
                    WCHAR wszGuid[50];
                    ...
                    if (cbWGuid)
                    {
                        hr = NemuNetCfgWinRenameConnection(wszGuid, wszConnectionNewName);
                        if (FAILED(hr))
                            NonStandardLogFlow(("NemuNetCfgWinRenameHostOnlyConnection: NemuNetCfgWinRenameConnection failed (0x%x)\n", hr));
                    }
                    else
                    {
                        err = GetLastError();
                    ...
                    NonStandardLogFlow(("NemuNetCfgWinRenameHostOnlyConnection: NemuNetCfgWinGenHostonlyConnectionName failed (0x%x)\n", hr));
            ...
            {
                hr = HRESULT_FROM_WIN32(err);
                NonStandardLogFlow(("NemuNetCfgWinRenameHostOnlyConnection: SetupDiGetDeviceRegistryPropertyW failed (0x%x)\n", err));
            }
        }
        else
        {
            DWORD err = GetLastError();
            hr = HRESULT_FROM_WIN32(err);
            NonStandardLogFlow(("NemuNetCfgWinRenameHostOnlyConnection: SetupDiOpenDeviceInfo failed (0x%x)\n", err));
        }
        SetupDiDestroyDeviceInfoList(hDevInfo);
    }
