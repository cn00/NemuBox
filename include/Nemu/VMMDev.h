...
#ifndef ___Nemu_VMMDev_h
#define ___Nemu_VMMDev_h

#include <Nemu/cdefs.h>
#include <Nemu/param.h>                 /* for the PCI IDs. */
 ...
 * Host source code will use NEMU_HGCM_HOST_CODE define to catch undefined
 ...
 * @note The NEMU_MOUSE_POINTER_* flags are used in the guest video driver,
 *       values must be <= 0x8000 and must not be changed. (try make more sense
 *       of this, please).
 * @{
 */
/** pointer is visible */
#define NEMU_MOUSE_POINTER_VISIBLE (0x0001)
/** pointer has alpha channel */
#define NEMU_MOUSE_POINTER_ALPHA   (0x0002)
/** pointerData contains new pointer shape */
#define NEMU_MOUSE_POINTER_SHAPE   (0x0004)
/** @} */
...
AssertCompileSize(NemuGuestInfo, 8);
...
typedef struct NemuGuestInfo2
{
     ...
     *
     * The way it was implemented was implemented: NEMU_VERSION_STRING.
     *
...
AssertCompileSize(NemuGuestInfo2, 144);
...
typedef enum
{
    NemuGuestFacilityType_Unknown         = 0,
    NemuGuestFacilityType_NemuGuestDriver = 20,
    NemuGuestFacilityType_AutoLogon       = 90,  /* NemuGINA / NemuCredProv / pam_nemu. */
    NemuGuestFacilityType_NemuService     = 100,
    NemuGuestFacilityType_NemuTrayClient  = 101, /* NemuTray (Windows), NemuClient (Linux, Unix). */
    NemuGuestFacilityType_Seamless        = 1000,
    NemuGuestFacilityType_Graphics        = 1100,
    NemuGuestFacilityType_All             = 0x7ffffffe,
    NemuGuestFacilityType_SizeHack        = 0x7fffffff
} NemuGuestFacilityType;
AssertCompileSize(NemuGuestFacilityType, 4);
...
typedef enum
{
    NemuGuestFacilityStatus_Inactive    = 0,
    NemuGuestFacilityStatus_Paused      = 1,
    NemuGuestFacilityStatus_PreInit     = 20,
    NemuGuestFacilityStatus_Init        = 30,
    NemuGuestFacilityStatus_Active      = 50,
    NemuGuestFacilityStatus_Terminating = 100,
    NemuGuestFacilityStatus_Terminated  = 101,
    NemuGuestFacilityStatus_Failed  =     800,
    NemuGuestFacilityStatus_Unknown     = 999,
    NemuGuestFacilityStatus_SizeHack    = 0x7fffffff
} NemuGuestFacilityStatus;
AssertCompileSize(NemuGuestFacilityStatus, 4);
...
typedef enum
{
    NemuGuestFacilityClass_None       = 0,
    NemuGuestFacilityClass_Driver     = 10,
    NemuGuestFacilityClass_Service    = 30,
    NemuGuestFacilityClass_Program    = 50,
    NemuGuestFacilityClass_Feature    = 100,
    NemuGuestFacilityClass_ThirdParty = 999,
    NemuGuestFacilityClass_All        = 0x7ffffffe,
    NemuGuestFacilityClass_SizeHack   = 0x7fffffff
} NemuGuestFacilityClass;
AssertCompileSize(NemuGuestFacilityClass, 4);
 ...
 */
typedef struct NemuGuestStatus
{
    /** Facility the status is indicated for. */
    NemuGuestFacilityType facility;
    /** Current guest status. */
    NemuGuestFacilityStatus status;
    /** Flags, not used at the moment. */
    uint32_t flags;
} NemuGuestStatus;
AssertCompileSize(NemuGuestStatus, 12);
 ...
 */
typedef enum NemuGuestUserState
{
    NemuGuestUserState_Unknown            = 0,
    NemuGuestUserState_LoggedIn           = 1,
    NemuGuestUserState_LoggedOut          = 2,
    NemuGuestUserState_Locked             = 3,
    NemuGuestUserState_Unlocked           = 4,
    NemuGuestUserState_Disabled           = 5,
    NemuGuestUserState_Idle               = 6,
    NemuGuestUserState_InUse              = 7,
    NemuGuestUserState_Created            = 8,
    NemuGuestUserState_Deleted            = 9,
    NemuGuestUserState_SessionChanged     = 10,
    NemuGuestUserState_CredentialsChanged = 11,
    NemuGuestUserState_RoleChanged        = 12,
    NemuGuestUserState_GroupAdded         = 13,
    NemuGuestUserState_GroupRemoved       = 14,
    NemuGuestUserState_Elevated           = 15,
    NemuGuestUserState_SizeHack           = 0x7fffffff
} NemuGuestUserState;
AssertCompileSize(NemuGuestUserState, 4);


/**
 * Guest user status updates.
 */
typedef struct NemuGuestUserStatus
{
    /** The guest user state to send. */
    NemuGuestUserState  state;
...
AssertCompileSize(NemuGuestUserStatus, 20);
 ...
 */
typedef struct NemuGuestStatistics
{
    /** Virtual CPU ID. */
    uint32_t        u32CpuId;
    /** Reported statistics. */
...
AssertCompileSize(NemuGuestStatistics, 19*4);

/** @name Guest statistics values (NemuGuestStatistics::u32StatCaps).
 * @{ */
#define NEMU_GUEST_STAT_CPU_LOAD_IDLE       RT_BIT(0)
#define NEMU_GUEST_STAT_CPU_LOAD_KERNEL     RT_BIT(1)
#define NEMU_GUEST_STAT_CPU_LOAD_USER       RT_BIT(2)
#define NEMU_GUEST_STAT_THREADS             RT_BIT(3)
#define NEMU_GUEST_STAT_PROCESSES           RT_BIT(4)
#define NEMU_GUEST_STAT_HANDLES             RT_BIT(5)
#define NEMU_GUEST_STAT_MEMORY_LOAD         RT_BIT(6)
#define NEMU_GUEST_STAT_PHYS_MEM_TOTAL      RT_BIT(7)
#define NEMU_GUEST_STAT_PHYS_MEM_AVAIL      RT_BIT(8)
#define NEMU_GUEST_STAT_PHYS_MEM_BALLOON    RT_BIT(9)
#define NEMU_GUEST_STAT_MEM_COMMIT_TOTAL    RT_BIT(10)
#define NEMU_GUEST_STAT_MEM_KERNEL_TOTAL    RT_BIT(11)
#define NEMU_GUEST_STAT_MEM_KERNEL_PAGED    RT_BIT(12)
#define NEMU_GUEST_STAT_MEM_KERNEL_NONPAGED RT_BIT(13)
#define NEMU_GUEST_STAT_MEM_SYSTEM_CACHE    RT_BIT(14)
#define NEMU_GUEST_STAT_PAGE_FILE_SIZE      RT_BIT(15)
/** @} */
...
typedef struct
{
    ...
    VMMDevRequestHeader header;
    /** Guest information. */
    NemuGuestStatistics guestStats;
 ...
 * @{
 */
# define NEMU_HGCM_REQ_DONE      RT_BIT_32(NEMU_HGCM_REQ_DONE_BIT)
# define NEMU_HGCM_REQ_DONE_BIT  0
# define NEMU_HGCM_REQ_CANCELLED (0x2)
/** @} */
 ...
 * @{ */
#define NEMU_HGCM_F_PARM_DIRECTION_NONE      UINT32_C(0x00000000)
#define NEMU_HGCM_F_PARM_DIRECTION_TO_HOST   UINT32_C(0x00000001)
#define NEMU_HGCM_F_PARM_DIRECTION_FROM_HOST UINT32_C(0x00000002)
#define NEMU_HGCM_F_PARM_DIRECTION_BOTH      UINT32_C(0x00000003)
/** Macro for validating that the specified flags are valid. */
#define NEMU_HGCM_F_PARM_ARE_VALID(fFlags) \
    (   (fFlags) > NEMU_HGCM_F_PARM_DIRECTION_NONE \
     && (fFlags) < NEMU_HGCM_F_PARM_DIRECTION_BOTH )
/** @} */
...
typedef struct
{
    uint32_t flags;        /**< NEMU_HGCM_F_PARM_*. */
