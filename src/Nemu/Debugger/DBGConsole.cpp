 ...
 * into the NemuDbg module with a very simple Qt wrapper around it.
 *
 ...
 * \#ifdef NEMU_WITH_DEBUGGER blocks. This is mandatory for components that
 ...
 *
 * The DBGC code is linked into the NemuVMM module.
 *
...
#include <Nemu/vmm/vmapi.h> /* VMR3GetVM() */
#include <Nemu/vmm/hm.h>    /* HMR3IsEnabled */
#include <Nemu/err.h>
#include <Nemu/log.h>
 ...
 *
 * @returns Nemu status code. Any error indicates the termination of the console session.
...
int dbgcProcessInput(PDBGC pDbgc, bool fNoExecute)
{
    ...
    if (pDbgc->cInputLines)
    {
        ...
        if (    RT_SUCCESS(rc)
            ...
            &&  pDbgc->fReady)
            rc = pDbgc->CmdHlp.pfnPrintf(&pDbgc->CmdHlp, NULL, "NemuDbg> ");
 ...
 *
 * @returns Nemu status code. (output related)
 * @param   pDbgc   Debugger console instance data.
 */
static int dbgcProcessLog(PDBGC pDbgc)
{
 ...
 * @returns The Nemu status code causing the console termination.
 *
...
DBGDECL(int) DBGCCreate(PUVM pUVM, PDBGCBACK pBack, unsigned fFlags)
{
    ...
    if (RT_SUCCESS(rc) && pUVM)
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            rc = pDbgc->CmdHlp.pfnNemuError(&pDbgc->CmdHlp, rc, "When trying to attach to VM %p\n", pDbgc->pVM);
    }
