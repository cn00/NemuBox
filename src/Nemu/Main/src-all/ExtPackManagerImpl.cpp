...
/** @file
 * VirtualBox Main - interface for Extension Packs, NemuSVC & NemuC.
 */
...
# define NEMU_EXTPACK_HELPER_NAME       "NemuExtPackHelperApp.exe"
#else
# define NEMU_EXTPACK_HELPER_NAME       "NemuExtPackHelperApp"
#endif
...
public:
    ...
    PCNEMUEXTPACKREG    pReg;
    /** The current context. */
    NEMUEXTPACKCTX      enmContext;
...
struct ExtPackManager::Data
{
    ...
    /** File handle for the NemuVMM libary which we slurp because ExtPacks depend on it. */
    RTLDRMOD            hNemuVMM;
#endif
                                  ...
                                  VirtualBox *a_pVirtualBox)
{
    ...
    RTCString *pstrTarName = NemuExtPackExtractNameFromTarballPath(a_pszFile);
    if (pstrTarName)
    {
    ...
    char        szError[8192];
    RTVFSFILE   hXmlFile;
    vrc = NemuExtPackValidateTarball(m->hExtPackFile, NULL /*pszExtPackName*/, a_pszFile, a_pszDigest,
    ...
    RTCString *pStrLoadErr = NemuExtPackLoadDescFromVfsFile(hXmlFile, &m->Desc, &m->ObjInfoDesc);
                                  ...
                                  const com::Utf8Str &aFormat, com::Utf8Str &aLicenseText)
{
     ...
     */
    char szName[sizeof(NEMU_EXTPACK_LICENSE_NAME_PREFIX "-de_DE.html") + 2];
    if (aPreferredLocale.isNotEmpty() && aPreferredLanguage.isNotEmpty())
        RTStrPrintf(szName, sizeof(szName), NEMU_EXTPACK_LICENSE_NAME_PREFIX "-%s_%s.%s",
                    aPreferredLocale.c_str(), aPreferredLanguage.c_str(), aFormat.c_str());
    else if (aPreferredLocale.isNotEmpty())
        RTStrPrintf(szName, sizeof(szName), NEMU_EXTPACK_LICENSE_NAME_PREFIX "-%s.%s",
                    aPreferredLocale.c_str(), aFormat.c_str());
    else if (aPreferredLanguage.isNotEmpty())
        RTStrPrintf(szName, sizeof(szName), NEMU_EXTPACK_LICENSE_NAME_PREFIX "-_%s.%s",
                    aPreferredLocale.c_str(), aFormat.c_str());
    else
        RTStrPrintf(szName, sizeof(szName), NEMU_EXTPACK_LICENSE_NAME_PREFIX ".%s",
                    aFormat.c_str());
    /*
    ...
    else
    {
        ...
        if (RTManifestEntryExists(m->hOurManifest, szName))
        {
            ...
            char            szError[8192];
            int vrc = NemuExtPackOpenTarFss(m->hExtPackFile, szError, sizeof(szError), &hTarFss, NULL);
            if (RT_SUCCESS(vrc))
            {
                for (;;)
                {
                    ...
                    {
                        if (vrc != VERR_EOF)
                            hrc = setError(NEMU_E_IPRT_ERROR, tr("RTVfsFsStrmNext failed: %Rrc"), vrc);
                        else
                            ...
                            || enmType == RTVFSOBJTYPE_FILE))
                    {
                        ...
                        if (RT_SUCCESS(vrc))
                        {
                            ...
                            if (pvFile)
                            {
                                ...
                                if (RT_SUCCESS(vrc))
                                {
                                    ...
                                    if (bstrLicense.isNotEmpty())
                                    {
                                        ...
                                        hrc = setError(NEMU_E_IPRT_ERROR,
                                                       tr("The license file '%s' is empty or contains invalid UTF-8 encoding"),
                                                       szName);
                                }
                                else
                                    hrc = setError(NEMU_E_IPRT_ERROR, tr("Failed to read '%s': %Rrc"), szName, vrc);
                                RTMemFree(pvFile);
                            }
                            else
                            ...
                            hrc = setError(NEMU_E_IPRT_ERROR, tr("RTVfsIoStrmQueryInfo on '%s': %Rrc"), szName, vrc);
                ...
                hrc = setError(NEMU_E_OBJECT_NOT_FOUND, tr("%s"), szError);
        }
            else
            hrc = setError(NEMU_E_OBJECT_NOT_FOUND, tr("The license file '%s' was not found in '%s'"),
        ...
        try
        {
            ...
            if (SUCCEEDED(hrc))
            {
                ...
                if (RT_SUCCESS(vrc))
                {
                    ...
                    hrc = setError(NEMU_E_IPRT_ERROR, tr("RTThreadCreate failed with %Rrc"), vrc);
            }
        }
        catch (std::bad_alloc)
        {
 ...
 */
HRESULT ExtPack::initWithDir(NEMUEXTPACKCTX a_enmContext, const char *a_pszName, const char *a_pszDir)
{
    AutoInitSpan autoInitSpan(this);
    AssertReturn(autoInitSpan.isOk(), E_FAIL);

    static const NEMUEXTPACKHLP s_HlpTmpl =
    {
        /* u32Version           = */ NEMUEXTPACKHLP_VERSION,
        /* uNemuFullVersion     = */ NEMU_FULL_VERSION,
        /* uNemuVersionRevision = */ 0,
        /* u32Padding           = */ 0,
        /* pszNemuVersion       = */ "",
        ...
        /* u32EndMarker         = */ NEMUEXTPACKHLP_VERSION
    };
    ...
    m->Hlp                          = s_HlpTmpl;
    m->Hlp.pszNemuVersion           = RTBldCfgVersion();
    m->Hlp.uNemuInternalRevision    = RTBldCfgRevision();
    m->pThis                        = this;
    m->pReg                         = NULL;
...
HRESULT ExtPack::i_getVrdpLibraryName(Utf8Str *a_pstrVrdeLibrary)
{
    ...
    if (SUCCEEDED(hrc))
    {
        if (i_findModule(m->Desc.strVrdeModule.c_str(), NULL, NEMUEXTPACKMODKIND_R3,
...
{
    HRESULT hrc;
    if (i_findModule(a_pszModuleName, NULL, NEMUEXTPACKMODKIND_R3,
 ...
 * @remarks Only called in NemuSVC.
 */
HRESULT ExtPack::i_refresh(bool *a_pfCanDelete)
{
    ...
    vrc = RTPathJoin(szDescFilePath, sizeof(szDescFilePath), m->strExtPackPath.c_str(), NEMU_EXTPACK_DESCRIPTION_NAME);
    if (RT_SUCCESS(vrc))
...
void ExtPack::i_probeAndLoad(void)
{
     ...
     */
    RTCString strSavedName(m->Desc.strName);
    RTCString *pStrLoadErr = NemuExtPackLoadDesc(m->strExtPackPath.c_str(), &m->Desc, &m->ObjInfoDesc);
    if (pStrLoadErr != NULL)
    {
        m->strWhyUnusable.printf(tr("Failed to load '%s/%s': %s"),
                                 m->strExtPackPath.c_str(), NEMU_EXTPACK_DESCRIPTION_NAME, pStrLoadErr->c_str());
        m->Desc.strName = strSavedName;
        delete pStrLoadErr;
        return;
    }
     ...
     *
     *  Since this was from before NEMUEXTPACKREG::uNemuFullVersion was
     ...
     */
    bool fIsNative;
    if (!i_findModule(m->Desc.strMainModule.c_str(), NULL /* default extension */, NEMUEXTPACKMODKIND_R3,
     ...
     */
    PFNNEMUEXTPACKREGISTER pfnRegistration;
    vrc = RTLdrGetSymbol(m->hMainMod, NEMU_EXTPACK_MAIN_MOD_ENTRY_POINT, (void **)&pfnRegistration);
    if (RT_SUCCESS(vrc))
    {
        ...
        if (   RT_SUCCESS(vrc)
            ...
            && VALID_PTR(m->pReg))
        {
            if (   NEMUEXTPACK_IS_MAJOR_VER_EQUAL(m->pReg->u32Version, NEMUEXTPACKREG_VERSION)
            ...
            m->strWhyUnusable.printf(tr("%s returned %Rrc, pReg=%p ErrInfo='%s'"),
                                     NEMU_EXTPACK_MAIN_MOD_ENTRY_POINT, vrc, m->pReg, ErrInfo.Core.pszMsg);
        m->pReg = NULL;
    }
    else
        m->strWhyUnusable.printf(tr("Failed to resolve exported symbol '%s' in the main module: %Rrc"),
                                 NEMU_EXTPACK_MAIN_MOD_ENTRY_POINT, vrc);
 ...
 *                              the module. Optional.
 */
bool ExtPack::i_findModule(const char *a_pszName, const char *a_pszExt, NEMUEXTPACKMODKIND a_enmKind,
                           Utf8Str *a_pStrFound, bool *a_pfNative, PRTFSOBJINFO a_pObjInfo) const
{
    ...
    if (!a_pszExt)
    {
        ...
        switch (a_enmKind)
        {
            case NEMUEXTPACKMODKIND_RC: pszDefExt = ".rc"; break;
            case NEMUEXTPACKMODKIND_R0: pszDefExt = ".r0"; break;
            case NEMUEXTPACKMODKIND_R3: pszDefExt = RTLdrGetSuff(); break;
            default:
...
/**
 * @interface_method_impl{NEMUEXTPACKHLP,pfnFindModule}
 */
/*static*/ DECLCALLBACK(int)
ExtPack::i_hlpFindModule(PCNEMUEXTPACKHLP pHlp, const char *pszName, const char *pszExt, NEMUEXTPACKMODKIND enmKind,
                         char *pszFound, size_t cbFound, bool *pfNative)
{
    ...
    AssertReturn(enmKind > NEMUEXTPACKMODKIND_INVALID && enmKind < NEMUEXTPACKMODKIND_END, VERR_INVALID_PARAMETER);

    AssertPtrReturn(pHlp, VERR_INVALID_POINTER);
    AssertReturn(pHlp->u32Version == NEMUEXTPACKHLP_VERSION, VERR_INVALID_POINTER);
...
/*static*/ DECLCALLBACK(int)
ExtPack::i_hlpGetFilePath(PCNEMUEXTPACKHLP pHlp, const char *pszFilename, char *pszPath, size_t cbPath)
{
    /*
     * Validate the input and get our bearings.
     */
...
/*static*/ DECLCALLBACK(NEMUEXTPACKCTX)
ExtPack::i_hlpGetContext(PCNEMUEXTPACKHLP pHlp)
{
    /*
     * Validate the input and get our bearings.
     */
    AssertPtrReturn(pHlp, NEMUEXTPACKCTX_INVALID);
    AssertReturn(pHlp->u32Version == NEMUEXTPACKHLP_VERSION, NEMUEXTPACKCTX_INVALID);
    ExtPack::Data *m = RT_FROM_CPP_MEMBER(pHlp, Data, Hlp);
    AssertPtrReturn(m, NEMUEXTPACKCTX_INVALID);
    ExtPack *pThis = m->pThis;
    AssertPtrReturn(pThis, NEMUEXTPACKCTX_INVALID);

    return pThis->m->enmContext;
}

/*static*/ DECLCALLBACK(int)
ExtPack::i_hlpLoadHGCMService(PCNEMUEXTPACKHLP pHlp, NEMUEXTPACK_IF_CS(IConsole) *pConsole,
                              const char *pszServiceLibrary, const char *pszServiceName)
{
#ifdef NEMU_COM_INPROC
    /*
     * Validate the input and get our bearings.
     */
...
/*static*/ DECLCALLBACK(int)
ExtPack::i_hlpLoadVDPlugin(PCNEMUEXTPACKHLP pHlp, NEMUEXTPACK_IF_CS(IVirtualBox) *pVirtualBox, const char *pszPluginLibrary)
{
#ifndef NEMU_COM_INPROC
    /*
     * Validate the input and get our bearings.
    ...
    VirtualBox *pNemu = (VirtualBox *)pVirtualBox;
    return pNemu->i_loadVDPlugin(pszPluginLibrary);
#else
...
/*static*/ DECLCALLBACK(int)
ExtPack::i_hlpUnloadVDPlugin(PCNEMUEXTPACKHLP pHlp, NEMUEXTPACK_IF_CS(IVirtualBox) *pVirtualBox, const char *pszPluginLibrary)
{
#ifndef NEMU_COM_INPROC
    /*
     * Validate the input and get our bearings.
    ...
    VirtualBox *pNemu = (VirtualBox *)pVirtualBox;
    return pNemu->i_unloadVDPlugin(pszPluginLibrary);
#else
    NOREF(pHlp); NOREF(pVirtualBox);
#endif
    return VERR_INVALID_STATE;
...
/*static*/ DECLCALLBACK(int)
ExtPack::i_hlpReservedN(PCNEMUEXTPACKHLP pHlp)
{
    /*
     * Validate the input and get our bearings.
     */
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            if (RT_SUCCESS(vrc))
            {
                ...
                if (bstrLicense.isNotEmpty())
                {
                    ...
                    hrc = setError(NEMU_E_IPRT_ERROR, tr("The license file '%s' is empty or contains invalid UTF-8 encoding"),
                                   szPath);
                RTFileReadAllFree(pvFile, cbFile);
            }
            else if (vrc == VERR_FILE_NOT_FOUND || vrc == VERR_PATH_NOT_FOUND)
                hrc = setError(NEMU_E_OBJECT_NOT_FOUND, tr("The license file '%s' was not found in extension pack '%s'"),
                               szName, m->Desc.strName.c_str());
            else
                hrc = setError(NEMU_E_FILE_ERROR, tr("Failed to open the license file '%s': %Rrc"), szPath, vrc);
        }
        else
            hrc = setError(NEMU_E_IPRT_ERROR, tr("RTPathJoin failed: %Rrc"), vrc);
    }
    return hrc;
}
 ...
 * @param   a_enmContext            The context we're in.
 */
HRESULT ExtPackManager::initExtPackManager(VirtualBox *a_pVirtualBox, NEMUEXTPACKCTX a_enmContext)
{
    AutoInitSpan autoInitSpan(this);
    AssertReturn(autoInitSpan.isOk(), E_FAIL);
    ...
    rc = RTPathAppend(szBaseDir, sizeof(szBaseDir), NEMU_EXTPACK_INSTALL_DIR);
    AssertLogRelRCReturn(rc, E_FAIL);

    char szCertificatDir[RTPATH_MAX];
    rc = RTPathAppPrivateNoArch(szCertificatDir, sizeof(szCertificatDir));
    AssertLogRelRCReturn(rc, E_FAIL);
    rc = RTPathAppend(szCertificatDir, sizeof(szCertificatDir), NEMU_EXTPACK_CERT_DIR);
    AssertLogRelRCReturn(rc, E_FAIL);
    ...
    /*
     * Slurp in NemuVMM which is used by NemuPuelMain.
     */
#if !defined(RT_OS_WINDOWS) && !defined(RT_OS_DARWIN)
    if (a_enmContext == NEMUEXTPACKCTX_PER_USER_DAEMON)
    {
        int vrc = SUPR3HardenedLdrLoadAppPriv("NemuVMM", &m->hNemuVMM, RTLDRLOAD_FLAGS_GLOBAL, NULL);
        if (RT_FAILURE(vrc))
            m->hNemuVMM = NIL_RTLDRMOD;
        /* cleanup in ::uninit()? */
    }
#endif
    ...
    if (RT_SUCCESS(vrc))
    {
        for (;;)
        {
            ...
            if (   RTFS_IS_DIRECTORY(Entry.Info.Attr.fMode)
                ...
                && strcmp(Entry.szName, "..") != 0
                && NemuExtPackIsValidMangledName(Entry.szName) )
            {
                /*
                ...
                if (RT_SUCCESS(vrc))
                {
                    RTCString *pstrName = NemuExtPackUnmangleName(Entry.szName, RTSTR_MAX);
...
{
    Assert(m->enmContext == NEMUEXTPACKCTX_PER_USER_DAEMON);
...
{
    AssertReturn(m->enmContext == NEMUEXTPACKCTX_PER_USER_DAEMON, E_UNEXPECTED);
...
HRESULT ExtPackManager::i_runSetUidToRootHelper(Utf8Str const *a_pstrDisplayInfo, const char *a_pszCommand, ...)
{
    ...
    vrc = RTPathAppend(szExecName, sizeof(szExecName), NEMU_EXTPACK_HELPER_NAME);
    AssertLogRelRCReturn(vrc, E_UNEXPECTED);
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        hrc = setError(NEMU_E_IPRT_ERROR, tr("Failed to launch the helper application '%s' (%Rrc)"), szExecName, vrc);
...
{
    Assert(m->pVirtualBox != NULL); /* Only called from NemuSVC. */
    ...
    {
        /*
         * Do this check here, otherwise NemuExtPackCalcDir() will fail with a strange
         * error.
         */
        bool fValid = NemuExtPackIsValidName(a_pszName);
        if (!fValid)
        ...
        int vrc = NemuExtPackCalcDir(szDir, sizeof(szDir), m->strBaseDir.c_str(), a_pszName);
...
HRESULT ExtPackManager::i_doInstall(ExtPackFile *a_pExtPackFile, bool a_fReplace, Utf8Str const *a_pstrDisplayInfo)
{
    ...
    if (SUCCEEDED(hrc))
    {
         ...
         */
        if (m->enmContext == NEMUEXTPACKCTX_PER_USER_DAEMON)
        {
            autoLock.release();
 ...
 *
 * @returns Nemu status code.  Stops on the first failure, expecting the caller
...
int ExtPackManager::i_callAllVmConfigureVmmHooks(IConsole *a_pConsole, PVM a_pVM)
{
    ...
    HRESULT hrc = autoCaller.rc();
    if (FAILED(hrc))
        return Global::nemuStatusCodeFromCOM(hrc);
    AutoWriteLock           autoLock(this COMMA_LOCKVAL_SRC_POS);
    ComPtr<ExtPackManager>  ptrSelfRef = this; /* paranoia */
    ExtPackList             llExtPacks = m->llInstalledExtPacks;
...
HRESULT ExtPackManager::i_checkVrdeExtPack(Utf8Str const *a_pstrExtPack)
{
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        if (pExtPack)
            ...
            hrc = setError(NEMU_E_OBJECT_NOT_FOUND, tr("No extension pack by the name '%s' was found"), a_pstrExtPack->c_str());
    }
...
int ExtPackManager::i_getVrdeLibraryPathForExtPack(Utf8Str const *a_pstrExtPack, Utf8Str *a_pstrVrdeLibrary)
{
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        if (pExtPack)
            ...
            hrc = setError(NEMU_E_OBJECT_NOT_FOUND, tr("No extension pack by the name '%s' was found"),
                           a_pstrExtPack->c_str());
    }
