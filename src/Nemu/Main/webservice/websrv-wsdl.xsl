...
<!--
    ...
    websrv-wsdl.xsl:
        XSLT stylesheet that generates nemuweb.wsdl from
...
<xsl:template name="emitConvertedType">
  ...
  <xsl:choose>
    <xsl:when test="$type=$G_typeIsGlobalRequestElementMarker"><xsl:value-of select="concat('nemu:', $ifname, $G_classSeparator, $methodname, $G_requestMessageElementSuffix)" /></xsl:when>
    <xsl:when test="$type=$G_typeIsGlobalResponseElementMarker"><xsl:value-of select="concat('nemu:', $ifname, $G_classSeparator, $methodname, $G_responseMessageElementSuffix)" /></xsl:when>
    ...
    <xsl:when test="count(key('G_keyEnumsByName', $type)) > 0"><xsl:value-of select="concat('nemu:', $type)" /></xsl:when>
    ...
    <xsl:when test="count(key('G_keyInterfacesByName', $type)) > 0">
      ...
      <xsl:choose>
        <xsl:when test="$wsmap='struct'"><xsl:value-of select="concat('nemu:', $type)" /></xsl:when>
  ...
  <wsdl:operation>
    ...
    <wsdl:input>
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:attribute name="message">nemu:<xsl:copy-of select="$_ifname" /><xsl:value-of select="$G_classSeparator" /><xsl:value-of select="$_methodname" /><xsl:copy-of select="$G_methodRequest" /></xsl:attribute>
        </xsl:otherwise>
      </xsl:choose>
    </wsdl:input>
    <xsl:choose>
      <xsl:when test="$_resulttype">
        <wsdl:output>
          <xsl:choose>
            ...
            <xsl:otherwise>
              <xsl:attribute name="message">nemu:<xsl:copy-of select="$_ifname" /><xsl:value-of select="$G_classSeparator" /><xsl:value-of select="$_methodname" /><xsl:copy-of select="$G_methodResponse" /></xsl:attribute>
            </xsl:otherwise>
          </xsl:choose>
        </wsdl:output>
      </xsl:when>
    ...
    <xsl:choose>
      <xsl:when test="not($_fSoap)">
        <wsdl:fault name="InvalidObjectFault" message="nemu:InvalidObjectFaultMsg" />
        <wsdl:fault name="RuntimeFault" message="nemu:RuntimeFaultMsg" />
      </xsl:when>
      <xsl:otherwise>
...
<xsl:template match="/idl">
  ...
  Generator: src/Nemu/Main/webservice/websrv-wsdl.xsl
</xsl:comment>
...
<xsl:template match="library">
  ...
  ******************************************************
    ...
    <wsdl:message name="InvalidObjectFaultMsg">
      <wsdl:part name="fault" element="nemu:InvalidObjectFault" />
    </wsdl:message>
    <wsdl:message name="RuntimeFaultMsg">
      <wsdl:part name="fault" element="nemu:RuntimeFault" />
    </wsdl:message>
  ...
  ******************************************************
    ...
    <wsdl:portType>
      <xsl:attribute name="name"><xsl:copy-of select="'nemu'" /><xsl:value-of select="$G_portTypeSuffix" /></xsl:attribute>
    ...
    <wsdl:binding>
      <xsl:attribute name="name"><xsl:value-of select="concat('nemu', $G_bindingSuffix)" /></xsl:attribute>
      <xsl:attribute name="type"><xsl:value-of select="concat('nemu:nemu', $G_portTypeSuffix)" /></xsl:attribute>
