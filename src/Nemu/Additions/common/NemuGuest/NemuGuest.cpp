/* $Id: NemuGuest.cpp $ */
/** @file
 * NemuGuest - Guest Additions Driver, Common Code.
 */
...
/** @page pg_vbdrv NemuGuest
 *
 * NemuGuest is the device driver for VMMDev.
 *
 * The device driver is shipped as part of the guest additions.  It has roots in
 * the host VMM support driver (usually known as NemuDrv), so fixes in platform
 * specific code may apply to both drivers.
 *
 * The common code lives in NemuGuest.cpp and is compiled both as C++ and C.
 * The NemuGuest.cpp source file shall not contain platform specific code,
 ...
 * restrict set of functions.  These are generally referred to as 'nemuguest'
 * and 'nemuuser' respectively.  Currently, this two device approach is only
...
#include <Nemu/VMMDev.h> /* for VMMDEV_RAM_SIZE */
#include <Nemu/log.h>
#include <iprt/mem.h>
#include <iprt/time.h>
...
*********************************************************************************************************************************/
#define NEMUGUEST_ACQUIRE_STYLE_EVENTS (VMMDEV_EVENT_DISPLAY_CHANGE_REQUEST | VMMDEV_EVENT_SEAMLESS_MODE_CHANGE_REQUEST)
...
#endif
static int      vgdrvIoCtl_CancelAllWaitEvents(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession);
static void     vgdrvBitUsageTrackerClear(PNEMUGUESTBITUSAGETRACER pTracker);
static uint32_t vgdrvGetAllowedEventMaskForSession(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession);
static int      vgdrvResetEventFilterOnHost(PNEMUGUESTDEVEXT pDevExt, uint32_t fFixedEvents);
static int      vgdrvResetMouseStatusOnHost(PNEMUGUESTDEVEXT pDevExt);
static int      vgdrvResetCapabilitiesOnHost(PNEMUGUESTDEVEXT pDevExt);
static int      vgdrvSetSessionEventFilter(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession,
                                           uint32_t fOrMask, uint32_t fNotMask, bool fSessionTermination);
static int      vgdrvSetSessionMouseStatus(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession,
                                           uint32_t fOrMask, uint32_t fNotMask, bool fSessionTermination);
static int      vgdrvSetSessionCapabilities(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession,
                                            uint32_t fOrMask, uint32_t fNoMask, bool fSessionTermination);
static int      vgdrvAcquireSessionCapabilities(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession, uint32_t fOrMask,
                                                uint32_t fNotMask, NEMUGUESTCAPSACQUIRE_FLAGS enmFlags, bool fSessionTermination);
static int      vgdrvDispatchEventsLocked(PNEMUGUESTDEVEXT pDevExt, uint32_t fEvents);
 ...
 */
static int vgdrvInitFixateGuestMappings(PNEMUGUESTDEVEXT pDevExt)
{
    /*
    ...
    else
    {
        ...
        for (iTry = 0; iTry < RT_ELEMENTS(ahTries); iTry++)
        {
            ...
            if (rc == VERR_NOT_SUPPORTED)
            {
                ...
                {
                    rc = RTR0MemObjEnterPhys(&hObj, NEMUGUEST_HYPERVISOR_PHYSICAL_START, cbHypervisor + _4M, RTMEM_CACHE_POLICY_DONT_CARE);
                    if (RT_FAILURE(rc))
                        break;
                    hFictive = hObj;
                }
            ...
            if (RT_FAILURE(rc))
            {
                LogRel(("NemuGuest: Failed to reserve memory for the hypervisor: rc=%Rrc (cbHypervisor=%#x uAlignment=%#x iTry=%u)\n",
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                Log(("NemuGuest: %p LB %#x; uAlignment=%#x iTry=%u hGuestMappings=%p (%s)\n",
        ...
        if (RT_FAILURE(rc) && !fBitched)
            LogRel(("NemuGuest: Warning: failed to reserve %#d of memory for guest mappings.\n", cbHypervisor));
    }
    VbglGRFree(&pReq->header);
 ...
 * @param   pDevExt     The device extension.
 */
static void vgdrvTermUnfixGuestMappings(PNEMUGUESTDEVEXT pDevExt)
{
 ...
 */
static int vgdrvReportGuestInfo(NEMUOSTYPE enmOSType)
{
    /*
    ...
    if (RT_SUCCESS(rc))
    {
        pReqInfo2->guestInfo.additionsMajor    = NEMU_VERSION_MAJOR;
        pReqInfo2->guestInfo.additionsMinor    = NEMU_VERSION_MINOR;
        pReqInfo2->guestInfo.additionsBuild    = NEMU_VERSION_BUILD;
        pReqInfo2->guestInfo.additionsRevision = NEMU_SVN_REV;
        pReqInfo2->guestInfo.additionsFeatures = 0; /* (no features defined yet) */
        RTStrCopy(pReqInfo2->guestInfo.szName, sizeof(pReqInfo2->guestInfo.szName), NEMU_VERSION_STRING);
...
{
    /*
     * Report guest status of the Nemu driver to the host.
     */
    ...
    if (RT_SUCCESS(rc))
    {
        pReq2->guestStatus.facility = NemuGuestFacilityType_NemuGuestDriver;
        pReq2->guestStatus.status = fActive ?
                                    NemuGuestFacilityStatus_Active
                                  : NemuGuestFacilityStatus_Inactive;
 ...
 *                          (VINF_SUCCESS if set).
 */
static int vgdrvSetBalloonSizeKernel(PNEMUGUESTDEVEXT pDevExt, uint32_t cBalloonChunks, uint32_t *pfHandleInR3)
{
    int rc = VINF_SUCCESS;
    ...
    if (pDevExt->MemBalloon.fUseKernelAPI)
    {
            ...
            for (i = pDevExt->MemBalloon.cChunks; i < cBalloonChunks; i++)
            {
                ...
                if (RT_FAILURE(rc))
                {
                    if (rc == VERR_NOT_SUPPORTED)
                    {
                        ...
                        Log(("NemuGuestSetBalloonSizeKernel: PhysNC allocs not supported, falling back to R3 allocs.\n"));
                    }
                ...
                if (RT_FAILURE(rc))
                {
                    Log(("nemuGuestSetBalloonSize(inflate): failed, rc=%Rrc!\n", rc));
        ...
        else
        {
            ...
            for (i = pDevExt->MemBalloon.cChunks; i-- > cBalloonChunks;)
            {
                ...
                if (RT_FAILURE(rc))
                {
                    Log(("nemuGuestSetBalloonSize(deflate): failed, rc=%Rrc!\n", rc));
                    break;
                }
                pDevExt->MemBalloon.cChunks--;
            }
 ...
 */
static int vgdrvSetBalloonSizeFromUser(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession, uint64_t u64ChunkAddr, bool fInflate)
{
    VMMDevChangeMemBalloon *pReq;
    int rc = VINF_SUCCESS;
    uint32_t i;
    ...
    if (fInflate)
    {
        ...
        {
            LogRel(("nemuGuestSetBalloonSize: cannot inflate balloon, already have %u chunks (max=%u)\n",
        ...
        if (!pDevExt->MemBalloon.paMemObj)
        {
            ...
            if (!pDevExt->MemBalloon.paMemObj)
            {
                LogRel(("NemuGuestSetBalloonSizeFromUser: no memory for paMemObj!\n"));
                return VERR_NO_MEMORY;
            }
    ...
    else
    {
        ...
        {
            AssertMsgFailed(("nemuGuestSetBalloonSize: cannot decrease balloon, already at size 0\n"));
            return VERR_INVALID_PARAMETER;
        }
    }
 ...
 */
static void vgdrvCloseMemBalloon(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession)
{
 ...
 * @returns Nemu status code.
 */
static int vgdrvHeartbeatSend(PNEMUGUESTDEVEXT pDevExt)
{
    int rc;
...
{
    PNEMUGUESTDEVEXT pDevExt = (PNEMUGUESTDEVEXT)pvUser;
    int rc;
 ...
 */
static int vgdrvHeartbeatHostConfigure(PNEMUGUESTDEVEXT pDevExt, bool fEnabled)
{
 ...
 *
 * @returns Nemu status (ignored).
 * @param   pDevExt             The device extension.
 */
static int vgdrvHeartbeatInit(PNEMUGUESTDEVEXT pDevExt)
{
    /*
 ...
 */
int VGDrvCommonReinitDevExtAfterHibernation(PNEMUGUESTDEVEXT pDevExt, NEMUOSTYPE enmOSType)
{
...
/**
 * Initializes the NemuGuest device extension when the
 ...
 */
int VGDrvCommonInitDevExt(PNEMUGUESTDEVEXT pDevExt, uint16_t IOPortBase,
                          void *pvMMIOBase, uint32_t cbMMIO, NEMUOSTYPE enmOSType, uint32_t fFixedEvents)
{
    int rc, rc2;

#ifdef NEMU_GUESTDRV_WITH_RELEASE_LOGGER
    /*
     * Create the release log.
     */
    static const char * const s_apszGroups[] = NEMU_LOGGROUP_NAMES;
    PRTLOGGER pRelLogger;
    rc = RTLogCreate(&pRelLogger, 0 /*fFlags*/, "all", "NEMUGUEST_RELEASE_LOG", RT_ELEMENTS(s_apszGroups), s_apszGroups,
...
#endif
#ifdef NEMUGUEST_USE_DEFERRED_WAKE_UP
     ...
     */
    rc = RTSpinlockCreate(&pDevExt->EventSpinlock, RTSPINLOCK_FLAGS_INTERRUPT_SAFE, "NemuGuestEvent");
    if (RT_SUCCESS(rc))
        rc = RTSpinlockCreate(&pDevExt->SessionSpinlock, RTSPINLOCK_FLAGS_INTERRUPT_SAFE, "NemuGuestSession");
    if (RT_FAILURE(rc))
    {
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        ...
                        if (RT_SUCCESS(rc))
                        {
                            ...
                            rc = vgdrvReportDriverStatus(true /* Driver is active */);
                            if (RT_FAILURE(rc))
                                LogRel(("VGDrvCommonInitDevExt: NemuReportGuestDriverStatus failed, rc=%Rrc\n", rc));
                ...
                LogRel(("VGDrvCommonInitDevExt: NemuReportGuestInfo failed: rc=%Rrc\n", rc));
            VbglGRFree((VMMDevRequestHeader *)pDevExt->pIrqAckEvents);
        }
        else
            LogRel(("VGDrvCommonInitDevExt: NemuGRAlloc failed: rc=%Rrc\n", rc));
...
static void vgdrvDeleteWaitList(PRTLISTNODE pList)
{
    ...
    {
        int             rc2;
        PNEMUGUESTWAIT  pWait = RTListGetFirst(pList, NEMUGUESTWAIT, ListNode);
...
/**
 * Destroys the NemuGuest device extension.
 *
 ...
 * @param   pDevExt         The device extension.
 */
void VGDrvCommonDeleteDevExt(PNEMUGUESTDEVEXT pDevExt)
{
    int rc2;
    Log(("VGDrvCommonDeleteDevExt:\n"));
    Log(("NemuGuest: The additions driver is terminating.\n"));
    ...
    vgdrvCloseMemBalloon(pDevExt, (PNEMUGUESTSESSION)NULL);
...
/**
 * Creates a NemuGuest user session.
 *
 ...
 */
int VGDrvCommonCreateUserSession(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION *ppSession)
{
    PNEMUGUESTSESSION pSession = (PNEMUGUESTSESSION)RTMemAllocZ(sizeof(*pSession));
...
/**
 * Creates a NemuGuest kernel session.
 *
 ...
 * @param   ppSession       Where to store the session on success.
 */
int VGDrvCommonCreateKernelSession(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION *ppSession)
{
    PNEMUGUESTSESSION pSession = (PNEMUGUESTSESSION)RTMemAllocZ(sizeof(*pSession));
    if (RT_UNLIKELY(!pSession))
    {
...
/**
 * Closes a NemuGuest session.
 *
 * @param   pDevExt         The device extension.
 * @param   pSession        The session to close (and free).
 */
void VGDrvCommonCloseSession(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession)
{
#ifdef NEMU_WITH_HGCM
    unsigned i;
#endif
    ...
    pDevExt->cSessions--;
    RTSpinlockRelease(pDevExt->SessionSpinlock);
    vgdrvAcquireSessionCapabilities(pDevExt, pSession, 0, UINT32_MAX, NEMUGUESTCAPSACQUIRE_FLAGS_NONE,
 ...
 */
static PNEMUGUESTWAIT vgdrvWaitAlloc(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession)
{
    /*
     * Allocate it one way or the other.
     */
    PNEMUGUESTWAIT pWait = RTListGetFirst(&pDevExt->FreeList, NEMUGUESTWAIT, ListNode);
    if (pWait)
    {
        RTSpinlockAcquire(pDevExt->EventSpinlock);

        pWait = RTListGetFirst(&pDevExt->FreeList, NEMUGUESTWAIT, ListNode);
        if (pWait)
            RTListNodeRemove(&pWait->ListNode);
    ...
    if (!pWait)
    {
        ...
        pWait = (PNEMUGUESTWAIT)RTMemAlloc(sizeof(*pWait));
        if (!pWait)
        {
 ...
 */
static void vgdrvWaitFreeLocked(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTWAIT pWait)
{
    pWait->fReqEvents = 0;
    pWait->fResEvents = 0;
#ifdef NEMU_WITH_HGCM
 ...
 * @param   pWait           The wait-for-event entry to free.
 */
static void vgdrvWaitFreeUnlocked(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTWAIT pWait)
{
    RTSpinlockAcquire(pDevExt->EventSpinlock);
 ...
 * @param   pDevExt         The device extension.
 */
void VGDrvCommonWaitDoWakeUps(PNEMUGUESTDEVEXT pDevExt)
{
    if (!RTListIsEmpty(&pDevExt->WakeUpList))
    {
        ...
        {
            int            rc;
            PNEMUGUESTWAIT pWait = RTListGetFirst(&pDevExt->WakeUpList, NEMUGUESTWAIT, ListNode);
            if (!pWait)
                break;
...
#endif /* NEMUGUEST_USE_DEFERRED_WAKE_UP */
 ...
 * @param   pSession    The session.
 */
int VGDrvCommonIoCtlFast(unsigned iFunction, PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession)
{
 ...
 */
static int vgdrvIoCtl_GetVMMDevPort(PNEMUGUESTDEVEXT pDevExt, NemuGuestPortInfo *pInfo, size_t *pcbDataReturned)
{
    LogFlow(("NEMUGUEST_IOCTL_GETVMMDEVPORT\n"));
 ...
 */
int vgdrvIoCtl_SetMouseNotifyCallback(PNEMUGUESTDEVEXT pDevExt, NemuGuestMouseSetNotifyCallback *pNotify)
{
    LogFlow(("NEMUGUEST_IOCTL_SET_MOUSE_NOTIFY_CALLBACK: pfnNotify=%p pvUser=%p\n", pNotify->pfnNotify, pNotify->pvUser));
 ...
 */
DECLINLINE(int) vbdgCheckWaitEventCondition(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession,
                                            NemuGuestWaitEventInfo *pInfo, int iEvent, const uint32_t fReqEvents)
{
    uint32_t fMatches = pDevExt->f32PendingEvents & fReqEvents;
    if (fMatches & NEMUGUEST_ACQUIRE_STYLE_EVENTS)
    ...
    if (fMatches || pSession->fPendingCancelWaitEvents)
    {
        ...
        pInfo->u32Result = NEMUGUEST_WAITEVENT_OK;
        if (fReqEvents & ~((uint32_t)1 << iEvent))
            LogFlow(("NEMUGUEST_IOCTL_WAITEVENT: returns %#x\n", pInfo->u32EventFlagsOut));
        else
            LogFlow(("NEMUGUEST_IOCTL_WAITEVENT: returns %#x/%d\n", pInfo->u32EventFlagsOut, iEvent));
...
static int vgdrvIoCtl_WaitEvent(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession,
                                NemuGuestWaitEventInfo *pInfo,  size_t *pcbDataReturned, bool fInterruptible)
{
    const uint32_t  fReqEvents = pInfo->u32EventMaskIn;
    uint32_t        fResEvents;
    int             iEvent;
    PNEMUGUESTWAIT  pWait;
    int             rc;

    pInfo->u32EventFlagsOut = 0;
    pInfo->u32Result = NEMUGUEST_WAITEVENT_ERROR;
    if (pcbDataReturned)
        *pcbDataReturned = sizeof(*pInfo);
    ...
    {
        LogRel(("NEMUGUEST_IOCTL_WAITEVENT: Invalid input mask %#x!!\n", fReqEvents));
        return VERR_INVALID_PARAMETER;
    }
    ...
    {
        pInfo->u32Result = NEMUGUEST_WAITEVENT_TIMEOUT;
        LogFlow(("NEMUGUEST_IOCTL_WAITEVENT: returns VERR_TIMEOUT\n"));
        return VERR_TIMEOUT;
    }
        ...
        &&  fResEvents != UINT32_MAX)
    {
        ...
        rc = VINF_SUCCESS;
             ...
             || rc == VERR_INTERRUPTED)
    {
        pInfo->u32Result = NEMUGUEST_WAITEVENT_INTERRUPTED;
        rc = VERR_INTERRUPTED;
        LogFlow(("NEMUGUEST_IOCTL_WAITEVENT: returns VERR_INTERRUPTED\n"));
    }
    else if (rc == VERR_TIMEOUT)
    {
        pInfo->u32Result = NEMUGUEST_WAITEVENT_TIMEOUT;
        LogFlow(("NEMUGUEST_IOCTL_WAITEVENT: returns VERR_TIMEOUT (2)\n"));
    }
    else
    {
        if (RT_SUCCESS(rc))
        {
            LogRelMax(32, ("NEMUGUEST_IOCTL_WAITEVENT: returns %Rrc but no events!\n", rc));
            rc = VERR_INTERNAL_ERROR;
        }
        pInfo->u32Result = NEMUGUEST_WAITEVENT_ERROR;
        LogFlow(("NEMUGUEST_IOCTL_WAITEVENT: returns %Rrc\n", rc));
    }
...
static int vgdrvIoCtl_CancelAllWaitEvents(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession)
{
    ...
    LogFlow(("NEMUGUEST_IOCTL_CANCEL_ALL_WAITEVENTS\n"));

    /*
     * Walk the event list and wake up anyone with a matching session.
     */
    RTSpinlockAcquire(pDevExt->EventSpinlock);
    RTListForEachSafe(&pDevExt->WaitList, pWait, pSafe, NEMUGUESTWAIT, ListNode)
    {
 ...
 */
static int vgdrvCheckIfVmmReqIsAllowed(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession, VMMDevRequestType enmType,
                                       VMMDevRequestHeader const *pReqHdr)
{
    ...
    enum
    {
        kLevel_Invalid, kLevel_NoOne, kLevel_OnlyNemuGuest, kLevel_OnlyKernel, kLevel_TrustedUsers, kLevel_AllUsers
    } enmRequired;
    switch (enmType)
    {
        /*
...
# endif /* NEMU_WITH_64_BITS_GUESTS */
        ...
        case VMMDevReq_GetMemBalloonChangeRequest:
            enmRequired = kLevel_OnlyNemuGuest;
            break;
        ...
        case VMMDevReq_ReportGuestCapabilities:
            ...
            {
                case NemuGuestFacilityType_All:
                case NemuGuestFacilityType_NemuGuestDriver:
                    enmRequired = kLevel_OnlyNemuGuest;
                    break;
                case NemuGuestFacilityType_NemuService:
                    enmRequired = kLevel_TrustedUsers;
                    break;
                case NemuGuestFacilityType_NemuTrayClient:
                case NemuGuestFacilityType_Seamless:
                case NemuGuestFacilityType_Graphics:
                default:
                    enmRequired = kLevel_AllUsers;
                    break;
            }
...
static int vgdrvIoCtl_VMMRequest(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession,
                                 VMMDevRequestHeader *pReqHdr, size_t cbData, size_t *pcbDataReturned)
{
    ...
    LogFlow(("NEMUGUEST_IOCTL_VMMREQUEST: type %d\n", pReqHdr->requestType));

    if (cbReq < cbMinSize)
    {
        LogRel(("NEMUGUEST_IOCTL_VMMREQUEST: invalid hdr size %#x, expected >= %#x; type=%#x!!\n",
                cbReq, cbMinSize, enmType));
        return VERR_INVALID_PARAMETER;
    }
    if (cbReq > cbData)
    {
        LogRel(("NEMUGUEST_IOCTL_VMMREQUEST: invalid size %#x, expected >= %#x (hdr); type=%#x!!\n",
    ...
    if (RT_FAILURE(rc))
    {
        Log(("NEMUGUEST_IOCTL_VMMREQUEST: invalid header: size %#x, expected >= %#x (hdr); type=%#x; rc=%Rrc!!\n",
    ...
    if (RT_FAILURE(rc))
    {
        Log(("NEMUGUEST_IOCTL_VMMREQUEST: Operation not allowed! type=%#x rc=%Rrc\n", enmType, rc));
        return rc;
    }

    /*
     * Make a copy of the request in the physical memory heap so
     * the NemuGuestLibrary can more easily deal with the request.
    ...
    if (RT_FAILURE(rc))
    {
        Log(("NEMUGUEST_IOCTL_VMMREQUEST: failed to allocate %u (%#x) bytes to cache the request. rc=%Rrc!!\n",
             cbReq, cbReq, rc));
        return rc;
    }
        ...
        && RT_SUCCESS(pReqCopy->rc))
    {
        ...
        Log(("NEMUGUEST_IOCTL_VMMREQUEST: VbglGRPerform - rc=%Rrc!\n", rc));
    else
    {
        Log(("NEMUGUEST_IOCTL_VMMREQUEST: request execution failed; VMMDev rc=%Rrc!\n", pReqCopy->rc));
        rc = pReqCopy->rc;
    }
...
static int vgdrvHgcmAsyncWaitCallbackWorker(VMMDevHGCMRequestHeader volatile *pHdr, PNEMUGUESTDEVEXT pDevExt,
                                            bool fInterruptible, uint32_t cMillies)
{
    ...
    {
        RTSpinlockAcquire(pDevExt->EventSpinlock);
        if ((pHdr->fu32Flags & NEMU_HGCM_REQ_DONE) != 0)
        {
            RTSpinlockRelease(pDevExt->EventSpinlock);
            return VINF_SUCCESS;
        }
...
static int vgdrvIoCtl_HGCMConnect(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession,
                                  NemuGuestHGCMConnectInfo *pInfo, size_t *pcbDataReturned)
{
    int rc;
     ...
     */
    Log(("NEMUGUEST_IOCTL_HGCM_CONNECT: %.128s\n",
    ...
    if (RT_SUCCESS(rc))
    {
        Log(("NEMUGUEST_IOCTL_HGCM_CONNECT: u32Client=%RX32 result=%Rrc (rc=%Rrc)\n",
        ...
        if (RT_SUCCESS(pInfo->result))
        {
            ...
            {
                NemuGuestHGCMDisconnectInfo Info;
                LogRelMax(32, ("NEMUGUEST_IOCTL_HGCM_CONNECT: too many HGCMConnect calls for one session!\n"));
                Info.result = 0;
...
static int vgdrvIoCtl_HGCMDisconnect(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession,
                                     NemuGuestHGCMDisconnectInfo *pInfo, size_t *pcbDataReturned)
{
    /*
    ...
    if (i >= RT_ELEMENTS(pSession->aHGCMClientIds))
    {
        LogRelMax(32, ("NEMUGUEST_IOCTL_HGCM_DISCONNECT: u32Client=%RX32\n", u32ClientId));
     ...
     * to deal with cancelled requests.
     */
    Log(("NEMUGUEST_IOCTL_HGCM_DISCONNECT: u32Client=%RX32\n", pInfo->u32ClientID));
    rc = VbglR0HGCMInternalDisconnect(pInfo, vgdrvHgcmAsyncWaitCallback, pDevExt, RT_INDEFINITE_WAIT);
    if (RT_SUCCESS(rc))
    {
        LogFlow(("NEMUGUEST_IOCTL_HGCM_DISCONNECT: result=%Rrc\n", pInfo->result));
        if (pcbDataReturned)
            *pcbDataReturned = sizeof(*pInfo);
    }
...
static int vgdrvIoCtl_HGCMCall(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession, NemuGuestHGCMCallInfo *pInfo,
                               ...
                               size_t cbExtra, size_t cbData, size_t *pcbDataReturned)
{
    ...
    {
        LogRel(("NEMUGUEST_IOCTL_HGCM_CALL: cParm=%RX32 is not sane\n", pInfo->cParms));
        return VERR_INVALID_PARAMETER;
    }
    ...
    if (cbData < cbActual)
    {
        LogRel(("NEMUGUEST_IOCTL_HGCM_CALL: cbData=%#zx (%zu) required size is %#zx (%zu)\n",
    ...
    {
        LogRelMax(32, ("NEMUGUEST_IOCTL_HGCM_CALL: Invalid handle. u32Client=%RX32\n", u32ClientId));
        return VERR_INVALID_HANDLE;
    }
     ...
     */
    LogFlow(("NEMUGUEST_IOCTL_HGCM_CALL: u32Client=%RX32\n", pInfo->u32ClientID));
    ...
    if (RT_SUCCESS(rc))
    {
        LogFlow(("NEMUGUEST_IOCTL_HGCM_CALL: result=%Rrc\n", pInfo->result));
        if (pcbDataReturned)
    ...
    else
    {
        if (   rc != VERR_INTERRUPTED
            && rc != VERR_TIMEOUT)
            LogRelMax(32, ("NEMUGUEST_IOCTL_HGCM_CALL: %s Failed. rc=%Rrc.\n", f32bit ? "32" : "64", rc));
        else
            Log(("NEMUGUEST_IOCTL_HGCM_CALL: %s Failed. rc=%Rrc.\n", f32bit ? "32" : "64", rc));
    }
    return rc;
}
...
/**
 * Handle NEMUGUEST_IOCTL_CHECK_BALLOON from R3.
 *
 ...
 *                              be NULL.
 */
static int vgdrvIoCtl_CheckMemoryBalloon(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession,
                                         NemuGuestCheckBalloonInfo *pInfo, size_t *pcbDataReturned)
{
    VMMDevGetMemBalloonChangeRequest *pReq;
    int rc;

    LogFlow(("NEMUGUEST_IOCTL_CHECK_BALLOON:\n"));
    ...
    if (pDevExt->MemBalloon.pOwner == pSession)
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                if (pcbDataReturned)
                    *pcbDataReturned = sizeof(NemuGuestCheckBalloonInfo);
            }
            else
                LogRel(("NEMUGUEST_IOCTL_CHECK_BALLOON: VbglGRPerform failed. rc=%Rrc\n", rc));
            VbglGRFree(&pReq->header);
        }
    }
    else
    ...
    RTSemFastMutexRelease(pDevExt->MemBalloon.hMtx);
    LogFlow(("NEMUGUEST_IOCTL_CHECK_BALLOON returns %Rrc\n", rc));
    return rc;
}
 ...
 *                              be NULL.
 */
static int vgdrvIoCtl_ChangeMemoryBalloon(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession,
                                          NemuGuestChangeBalloonInfo *pInfo, size_t *pcbDataReturned)
{
    int rc;
    LogFlow(("NEMUGUEST_IOCTL_CHANGE_BALLOON: fInflate=%RTbool u64ChunkAddr=%#RX64\n", pInfo->fInflate, pInfo->u64ChunkAddr));
 ...
 * @param pInfo                 The output buffer.
 */
static int vgdrvIoCtl_WriteCoreDump(PNEMUGUESTDEVEXT pDevExt, NemuGuestWriteCoreDump *pInfo)
{
    VMMDevReqWriteCoreDump *pReq = NULL;
    int rc;
    LogFlow(("NEMUGUEST_IOCTL_WRITE_CORE_DUMP\n"));
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        rc = VbglGRPerform(&pReq->header);
        if (RT_FAILURE(rc))
            Log(("NEMUGUEST_IOCTL_WRITE_CORE_DUMP: VbglGRPerform failed, rc=%Rrc!\n", rc));

        VbglGRFree(&pReq->header);
    }
    else
        Log(("NEMUGUEST_IOCTL_WRITE_CORE_DUMP: failed to allocate %u (%#x) bytes to cache the request. rc=%Rrc!!\n",
 ...
 * @param   fUserSession        Copy of NEMUGUESTSESSION::fUserSession for the
 *                              call.  True normal user, false root user.
 */
static int vgdrvIoCtl_Log(PNEMUGUESTDEVEXT pDevExt, const char *pch, size_t cbData, size_t *pcbDataReturned, bool fUserSession)
{
 ...
 */
static void vgdrvBitUsageTrackerCheckMask(PCNEMUGUESTBITUSAGETRACER pTracker, uint32_t cMax, const char *pszWhat)
{
    uint32_t fMask = 0;
    uint32_t iBit;
    AssertCompile(sizeof(pTracker->acPerBitUsage) == 32 * sizeof(uint32_t));
 ...
 * @param   pszWhat             Identifies the tracker in assertions.
 */
static bool vgdrvBitUsageTrackerChange(PNEMUGUESTBITUSAGETRACER pTracker, uint32_t fChanged, uint32_t fPrevious,
 ...
 *
 * This is called in response to NEMUGUEST_IOCTL_CTL_FILTER_MASK as well as to
 * do session cleanup.
 *
 * @returns Nemu status code.
 * @param   pDevExt             The device extension.
...
/**
 * Handle NEMUGUEST_IOCTL_CTL_FILTER_MASK.
 *
 * @returns Nemu status code.
 *
 * @param   pDevExt             The device extension.
 ...
 */
static int vgdrvIoCtl_CtlFilterMask(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession, NemuGuestFilterMaskInfo *pInfo)
{
    LogFlow(("NEMUGUEST_IOCTL_CTL_FILTER_MASK: or=%#x not=%#x\n", pInfo->u32OrMask, pInfo->u32NotMask));

    if ((pInfo->u32OrMask | pInfo->u32NotMask) & ~VMMDEV_EVENT_VALID_EVENT_MASK)
    {
        Log(("NEMUGUEST_IOCTL_CTL_FILTER_MASK: or=%#x not=%#x: Invalid masks!\n", pInfo->u32OrMask, pInfo->u32NotMask));
        return VERR_INVALID_PARAMETER;
    }
 ...
 *
 * This is called in response to NEMUGUEST_IOCTL_SET_MOUSE_STATUS as well as to
 * do session cleanup.
 *
 * @returns Nemu status code.
 * @param   pDevExt             The device extension.
 ...
 */
static int vgdrvIoCtl_SetMouseStatus(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession, uint32_t fFeatures)
{
    LogFlow(("NEMUGUEST_IOCTL_SET_MOUSE_STATUS: features=%#x\n", fFeatures));
 ...
 *          NEMUGUEST_ACQUIRE_STYLE_EVENTS mask.
 */
static uint32_t vgdrvGetAllowedEventMaskForSession(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession)
{
     ...
     *       WITHOUT holding NEMUGUESTDEVEXT::SessionSpinlock.
     */
 ...
 */
static int vgdrvUpdateCapabilitiesOnHostWithReqAndLock(PNEMUGUESTDEVEXT pDevExt, VMMDevReqGuestCapabilities2 *pReq)
{
    int rc;
 ...
 * the given session.
 *
 * This is called in response to NEMUGUEST_IOCTL_GUEST_CAPS_ACQUIRE as well as
 ...
 *                              NEMUGUESTCAPSACQUIRE_FLAGS_NONE means to both
 ...
 */
static int vgdrvAcquireSessionCapabilities(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession,
                                           uint32_t fOrMask, uint32_t fNotMask, NEMUGUESTCAPSACQUIRE_FLAGS enmFlags,
                                           bool fSessionTermination)
{
    ...
    if (   enmFlags != NEMUGUESTCAPSACQUIRE_FLAGS_CONFIG_ACQUIRE_MODE
        && enmFlags != NEMUGUESTCAPSACQUIRE_FLAGS_NONE)
    {
...
/**
 * Handle NEMUGUEST_IOCTL_GUEST_CAPS_ACQUIRE.
 *
 * @returns Nemu status code.
 *
 * @param   pDevExt             The device extension.
 ...
 */
static int vgdrvIoCtl_GuestCapsAcquire(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession, NemuGuestCapsAquire *pAcquire)
{
    int rc;
    LogFlow(("NEMUGUEST_IOCTL_GUEST_CAPS_ACQUIRE: or=%#x not=%#x flags=%#x\n",
...
#ifndef NEMUGUEST_DISREGARD_ACQUIRE_MODE_GUEST_CAPS
    /*
...
/**
 * Handle NEMUGUEST_IOCTL_SET_GUEST_CAPABILITIES.
 *
 * @returns Nemu status code.
 *
 * @param   pDevExt             The device extension.
 ...
 * @param   pInfo               The request.
 */
static int vgdrvIoCtl_SetCapabilities(PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession, NemuGuestSetCapabilitiesInfo *pInfo)
{
    int rc;
    LogFlow(("NEMUGUEST_IOCTL_SET_GUEST_CAPABILITIES: or=%#x not=%#x\n", pInfo->u32OrMask, pInfo->u32NotMask));
 ...
 * @param   pcbDataReturned     Where to store the amount of returned data. Can be NULL.
 */
int VGDrvCommonIoCtl(unsigned iFunction, PNEMUGUESTDEVEXT pDevExt, PNEMUGUESTSESSION pSession,
...
#define CHECKRET_SIZE(mnemonic, cb) \
     ...
     */
    rc = VINF_SUCCESS;
    if (NEMUGUEST_IOCTL_STRIP_SIZE(iFunction) == NEMUGUEST_IOCTL_STRIP_SIZE(NEMUGUEST_IOCTL_VMMREQUEST(0)))
    {
     ...
     */
    else if (NEMUGUEST_IOCTL_STRIP_SIZE(iFunction) == NEMUGUEST_IOCTL_STRIP_SIZE(NEMUGUEST_IOCTL_HGCM_CALL(0)))
    {
        bool fInterruptible = pSession->R0Process != NIL_RTR0PROCESS;
        CHECKRET_MIN_SIZE("HGCM_CALL", sizeof(NemuGuestHGCMCallInfo));
        rc = vgdrvIoCtl_HGCMCall(pDevExt, pSession, (NemuGuestHGCMCallInfo *)pvData, RT_INDEFINITE_WAIT,
                                 fInterruptible, false /*f32bit*/, false /* fUserData */,
                                 0, cbData, pcbDataReturned);
    }
    else if (NEMUGUEST_IOCTL_STRIP_SIZE(iFunction) == NEMUGUEST_IOCTL_STRIP_SIZE(NEMUGUEST_IOCTL_HGCM_CALL_TIMED(0)))
    {
        NemuGuestHGCMCallInfoTimed *pInfo = (NemuGuestHGCMCallInfoTimed *)pvData;
        CHECKRET_MIN_SIZE("HGCM_CALL_TIMED", sizeof(NemuGuestHGCMCallInfoTimed));
        rc = vgdrvIoCtl_HGCMCall(pDevExt, pSession, &pInfo->info, pInfo->u32Timeout,
                                 ...
                                 RT_OFFSETOF(NemuGuestHGCMCallInfoTimed, info), cbData, pcbDataReturned);
    }
    else if (NEMUGUEST_IOCTL_STRIP_SIZE(iFunction) == NEMUGUEST_IOCTL_STRIP_SIZE(NEMUGUEST_IOCTL_HGCM_CALL_USERDATA(0)))
    {
...
# ifdef RT_ARCH_AMD64
    else if (NEMUGUEST_IOCTL_STRIP_SIZE(iFunction) == NEMUGUEST_IOCTL_STRIP_SIZE(NEMUGUEST_IOCTL_HGCM_CALL_32(0)))
    {
        bool fInterruptible = pSession->R0Process != NIL_RTR0PROCESS;
        CHECKRET_MIN_SIZE("HGCM_CALL", sizeof(NemuGuestHGCMCallInfo));
        rc = vgdrvIoCtl_HGCMCall(pDevExt, pSession, (NemuGuestHGCMCallInfo *)pvData, RT_INDEFINITE_WAIT,
    ...
    else if (NEMUGUEST_IOCTL_STRIP_SIZE(iFunction) == NEMUGUEST_IOCTL_STRIP_SIZE(NEMUGUEST_IOCTL_HGCM_CALL_TIMED_32(0)))
    {
        CHECKRET_MIN_SIZE("HGCM_CALL_TIMED", sizeof(NemuGuestHGCMCallInfoTimed));
        NemuGuestHGCMCallInfoTimed *pInfo = (NemuGuestHGCMCallInfoTimed *)pvData;
        rc = vgdrvIoCtl_HGCMCall(pDevExt, pSession, &pInfo->info, pInfo->u32Timeout,
...
# endif
#endif /* NEMU_WITH_HGCM */
    else if (NEMUGUEST_IOCTL_STRIP_SIZE(iFunction) == NEMUGUEST_IOCTL_STRIP_SIZE(NEMUGUEST_IOCTL_LOG(0)))
    {
    ...
    else
    {
        switch (iFunction)
        {
            case NEMUGUEST_IOCTL_GETVMMDEVPORT:
                CHECKRET_RING0("GETVMMDEVPORT");
                CHECKRET_MIN_SIZE("GETVMMDEVPORT", sizeof(NemuGuestPortInfo));
                rc = vgdrvIoCtl_GetVMMDevPort(pDevExt, (NemuGuestPortInfo *)pvData, pcbDataReturned);
                break;

#ifndef RT_OS_WINDOWS  /* Windows has its own implementation of this. */
            case NEMUGUEST_IOCTL_SET_MOUSE_NOTIFY_CALLBACK:
                CHECKRET_RING0("SET_MOUSE_NOTIFY_CALLBACK");
                CHECKRET_SIZE("SET_MOUSE_NOTIFY_CALLBACK", sizeof(NemuGuestMouseSetNotifyCallback));
                rc = vgdrvIoCtl_SetMouseNotifyCallback(pDevExt, (NemuGuestMouseSetNotifyCallback *)pvData);
                break;
#endif

            case NEMUGUEST_IOCTL_WAITEVENT:
                CHECKRET_MIN_SIZE("WAITEVENT", sizeof(NemuGuestWaitEventInfo));
                rc = vgdrvIoCtl_WaitEvent(pDevExt, pSession, (NemuGuestWaitEventInfo *)pvData,
                                          pcbDataReturned, pSession->R0Process != NIL_RTR0PROCESS);
                break;

            case NEMUGUEST_IOCTL_CANCEL_ALL_WAITEVENTS:
                CHECKRET_SIZE("CANCEL_ALL_WAITEVENTS", 0);
                rc = vgdrvIoCtl_CancelAllWaitEvents(pDevExt, pSession);
                break;

            case NEMUGUEST_IOCTL_CTL_FILTER_MASK:
                CHECKRET_MIN_SIZE("CTL_FILTER_MASK", sizeof(NemuGuestFilterMaskInfo));
                rc = vgdrvIoCtl_CtlFilterMask(pDevExt, pSession, (NemuGuestFilterMaskInfo *)pvData);
                break;

#ifdef NEMU_WITH_HGCM
            case NEMUGUEST_IOCTL_HGCM_CONNECT:
# ifdef RT_ARCH_AMD64
            case NEMUGUEST_IOCTL_HGCM_CONNECT_32:
# endif
                CHECKRET_MIN_SIZE("HGCM_CONNECT", sizeof(NemuGuestHGCMConnectInfo));
                rc = vgdrvIoCtl_HGCMConnect(pDevExt, pSession, (NemuGuestHGCMConnectInfo *)pvData, pcbDataReturned);
                break;

            case NEMUGUEST_IOCTL_HGCM_DISCONNECT:
# ifdef RT_ARCH_AMD64
            case NEMUGUEST_IOCTL_HGCM_DISCONNECT_32:
# endif
                CHECKRET_MIN_SIZE("HGCM_DISCONNECT", sizeof(NemuGuestHGCMDisconnectInfo));
                rc = vgdrvIoCtl_HGCMDisconnect(pDevExt, pSession, (NemuGuestHGCMDisconnectInfo *)pvData, pcbDataReturned);
                break;
#endif /* NEMU_WITH_HGCM */

            case NEMUGUEST_IOCTL_CHECK_BALLOON:
                CHECKRET_MIN_SIZE("CHECK_MEMORY_BALLOON", sizeof(NemuGuestCheckBalloonInfo));
                rc = vgdrvIoCtl_CheckMemoryBalloon(pDevExt, pSession, (NemuGuestCheckBalloonInfo *)pvData, pcbDataReturned);
                break;

            case NEMUGUEST_IOCTL_CHANGE_BALLOON:
                CHECKRET_MIN_SIZE("CHANGE_MEMORY_BALLOON", sizeof(NemuGuestChangeBalloonInfo));
                rc = vgdrvIoCtl_ChangeMemoryBalloon(pDevExt, pSession, (NemuGuestChangeBalloonInfo *)pvData, pcbDataReturned);
                break;

            case NEMUGUEST_IOCTL_WRITE_CORE_DUMP:
                CHECKRET_MIN_SIZE("WRITE_CORE_DUMP", sizeof(NemuGuestWriteCoreDump));
                rc = vgdrvIoCtl_WriteCoreDump(pDevExt, (NemuGuestWriteCoreDump *)pvData);
                break;

            case NEMUGUEST_IOCTL_SET_MOUSE_STATUS:
            ...
            case NEMUGUEST_IOCTL_DPC_LATENCY_CHECKER:
                CHECKRET_SIZE("DPC_LATENCY_CHECKER", 0);
                rc = VGDrvNtIOCtl_DpcLatencyChecker();
                break;
#endif

            case NEMUGUEST_IOCTL_GUEST_CAPS_ACQUIRE:
                CHECKRET_SIZE("GUEST_CAPS_ACQUIRE", sizeof(NemuGuestCapsAquire));
                rc = vgdrvIoCtl_GuestCapsAcquire(pDevExt, pSession, (NemuGuestCapsAquire *)pvData);
                *pcbDataReturned = sizeof(NemuGuestCapsAquire);
                break;

            case NEMUGUEST_IOCTL_SET_GUEST_CAPABILITIES:
                CHECKRET_MIN_SIZE("SET_GUEST_CAPABILITIES", sizeof(NemuGuestSetCapabilitiesInfo));
                rc = vgdrvIoCtl_SetCapabilities(pDevExt, pSession, (NemuGuestSetCapabilitiesInfo *)pvData);
                break;

            default:
            {
                LogRel(("VGDrvCommonIoCtl: Unknown request iFunction=%#x stripped size=%#x\n",
                        iFunction, NEMUGUEST_IOCTL_STRIP_SIZE(iFunction)));
 ...
 * @param   pDevExt             The NemuGuest device extension.
    ...
    RTListForEachSafe(&pDevExt->WaitList, pWait, pSafe, NEMUGUESTWAIT, ListNode)
    {
            ...
            &&  !pWait->fResEvents)
        {
            /* Does this one wait on any of the events we're dispatching?  We do a quick
               check first, then deal with NEMUGUEST_ACQUIRE_STYLE_EVENTS as applicable. */
            if (fHandledEvents & NEMUGUEST_ACQUIRE_STYLE_EVENTS)
 ...
 * @param   pDevExt     The NemuGuest device extension.
 */
bool VGDrvCommonISR(PNEMUGUESTDEVEXT pDevExt)
{
    ...
    if (fOurIrq)
    {
        ...
        if (RT_SUCCESS(pReq->header.rc))
        {
            ...
            if (fEvents & VMMDEV_EVENT_HGCM)
            {
                PNEMUGUESTWAIT pWait;
                PNEMUGUESTWAIT pSafe;
                RTListForEachSafe(&pDevExt->HGCMWaitList, pWait, pSafe, NEMUGUESTWAIT, ListNode)
                {
                    if (pWait->pHGCMReq->fu32Flags & NEMU_HGCM_REQ_DONE)
                    {
...
#if defined(NEMUGUEST_USE_DEFERRED_WAKE_UP) && !defined(RT_OS_DARWIN) && !defined(RT_OS_WINDOWS)
    /*
     * Do wake-ups.
