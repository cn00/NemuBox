/* $Id: NemuGuestR3LibAdditions.cpp $ */
/** @file
 * NemuGuestR3Lib - Ring-3 Support Library for VirtualBox guest additions, Additions Info.
 */
...
static int vbglR3WinOpenAdditionRegisterKey(PHKEY phKey)
{
    ...
    {
        L"SOFTWARE\\" RT_LSTR(NEMU_VENDOR_SHORT) L"\\VirtualBox Guest Additions",
#ifdef RT_ARCH_AMD64
        L"SOFTWARE\\Wow6432Node\\" RT_LSTR(NEMU_VENDOR_SHORT) L"\\VirtualBox Guest Additions",
#endif
...
static int vbglR3GetAdditionsCompileTimeVersion(char **ppszVer, char **ppszVerExt, char **ppszRev)
{
    int rc = VINF_SUCCESS;
    if (ppszVer)
        rc = RTStrDupEx(ppszVer, NEMU_VERSION_STRING_RAW);
    if (RT_SUCCESS(rc))
    {
        if (ppszVerExt)
            rc = RTStrDupEx(ppszVerExt, NEMU_VERSION_STRING);
        if (RT_SUCCESS(rc))
        {
            if (ppszRev)
                rc = RTStrDupEx(ppszRev, RT_XSTR(NEMU_SVN_REV));
            if (RT_SUCCESS(rc))
                return VINF_SUCCESS;
 ...
 */
VBGLR3DECL(int) VbglR3ReportAdditionsStatus(NemuGuestFacilityType enmFacility, NemuGuestFacilityStatus enmStatus,
                                            uint32_t fReserved)
{
