                          ...
                          ComPtr<IVirtualSystemDescription> &aDescription)
{
    ...
    try
    {
        ...
        // get guest OS
        Utf8Str strOsTypeNemu = mUserData->s.strOsType;
        // CPU count
        ...
        /* Guest OS type */
        ovf::CIMOSType_T cim = convertNemuOSType2CIMOSType(strOsTypeNemu.c_str(), fLongMode);
...
//     <const name="HardDiskControllerIDE" value="6" />
        if (!pIDEController.isNull())
        {
            ...
            switch(ctlr)
            {
                case StorageControllerType_PIIX3: strNemu = "PIIX3"; break;
                case StorageControllerType_PIIX4: strNemu = "PIIX4"; break;
                case StorageControllerType_ICH6: strNemu = "ICH6"; break;
            }
...
//     <const name="HardDiskControllerSCSI" value="8" />
        if (!pSCSIController.isNull())
        {
            ...
            if (SUCCEEDED(rc))
            {
                Utf8Str strNemu = "LsiLogic";       // the default in Nemu
                switch(ctlr)
                {
                    case StorageControllerType_LsiLogic: strNemu = "LsiLogic"; break;
                    case StorageControllerType_BusLogic: strNemu = "BusLogic"; break;
                }
            ...
            switch (deviceType)
            {
                case DeviceType_HardDisk:
                    ...
                    pNewDesc->i_addEntry(VirtualSystemDescriptionType_HardDiskImage,
                                         ...
                                         strLocation, // nemu value: media path
                                        (uint32_t)(llSize / _1M),
                                         strExtra);
                break;
                           ...
                           ovf::OVFVersion_T enFormat)
{
    ...
    pelmRoot->setAttribute("xmlns:nemu", "http://www.virtualbox.org/ovf/machine");
         ...
         ++itS)
    {
        ...
        // source path: where the Nemu image is
        const Utf8Str &strSrcFilePath = pDiskEntry->strNemuCurrent;
        ...
        // nemu: namespace since it's not part of the standard
        pelmDisk->setAttribute("nemu:uuid", Utf8StrFmt("%RTuuid", guidTarget.raw()).c_str());

        // now, we might have other XML elements from nemu:Machine pointing to this image,
                                              ...
                                              XMLStack &stack)
{
    ...
    if (llName.empty())
        throw setError(NEMU_E_NOT_SUPPORTED, tr("Missing VM name"));
    Utf8Str &strVMName = llName.back()->strNemuCurrent;
    ...
    bool fProduct    = llProduct.size()    && !llProduct.back()->strNemuCurrent.isEmpty();
    bool fProductUrl = llProductUrl.size() && !llProductUrl.back()->strNemuCurrent.isEmpty();
    bool fVendor     = llVendor.size()     && !llVendor.back()->strNemuCurrent.isEmpty();
    bool fVendorUrl  = llVendorUrl.size()  && !llVendorUrl.back()->strNemuCurrent.isEmpty();
    bool fVersion    = llVersion.size()    && !llVersion.back()->strNemuCurrent.isEmpty();
    if (fProduct || fProductUrl || fVendor || fVendorUrl || fVersion)
    {
        ...
        if (fProduct)
            pelmAnnotationSection->createChild("Product")->addContent(llProduct.back()->strNemuCurrent);
        if (fVendor)
            pelmAnnotationSection->createChild("Vendor")->addContent(llVendor.back()->strNemuCurrent);
        if (fVersion)
            pelmAnnotationSection->createChild("Version")->addContent(llVersion.back()->strNemuCurrent);
        if (fProductUrl)
            pelmAnnotationSection->createChild("ProductUrl")->addContent(llProductUrl.back()->strNemuCurrent);
        if (fVendorUrl)
            pelmAnnotationSection->createChild("VendorUrl")->addContent(llVendorUrl.back()->strNemuCurrent);
    }
    ...
    if (llDescription.size() &&
        !llDescription.back()->strNemuCurrent.isEmpty())
    {
        ...
        pelmAnnotationSection->createChild("Annotation")->addContent(llDescription.back()->strNemuCurrent);
    }

    // license
    std::list<VirtualSystemDescriptionEntry*> llLicense = vsdescThis->i_findByType(VirtualSystemDescriptionType_License);
    if (llLicense.size() &&
        !llLicense.back()->strNemuCurrent.isEmpty())
    {
        /* <EulaSection>
        ...
        pelmEulaSection->createChild("License")->addContent(llLicense.back()->strNemuCurrent);
    }

    // operating system
    std::list<VirtualSystemDescriptionEntry*> llOS = vsdescThis->i_findByType(VirtualSystemDescriptionType_OS);
    if (llOS.empty())
        throw setError(NEMU_E_NOT_SUPPORTED, tr("Missing OS type"));
    ...
    Utf8Str strOSDesc;
    convertCIMOSType2NemuOSType(strOSDesc, (ovf::CIMOSType_T)pvsdeOS->strOvf.toInt32(), "");
    ...
    xml::ElementNode *pelmNemuOSType = pelmOperatingSystemSection->createChild("nemu:OSType");
    pelmNemuOSType->setAttribute("ovf:required", "false");
    pelmNemuOSType->addContent(pvsdeOS->strNemuCurrent);
    ...
    for (size_t uLoop = 1; uLoop <= 2; ++uLoop)
    {
            ...
            ++itD, ++lIndexThis)
        {
            ...
            LogFlowFunc(("Loop %u: handling description entry ulIndex=%u, type=%s, strRef=%s, strOvf=%s, strNemu=%s, strExtraConfig=%s\n",
                         uLoop,
                         desc.ulIndex,
            ...
            switch (desc.type)
            {
                case VirtualSystemDescriptionType_CPU:
                    ...
                    if (uLoop == 1)
                    {
                        ...
                        desc.strNemuCurrent.toInt(uTemp);
                ...
                case VirtualSystemDescriptionType_HardDiskControllerIDE:
                    ...
                    if (uLoop == 1)
                    {
                        ...
                        strResourceSubType = desc.strNemuCurrent;
                ...
                case VirtualSystemDescriptionType_HardDiskControllerSATA:
                    ...
                    */
                    if (uLoop == 1)
                    {
                        ...
                        if (    desc.strNemuCurrent.isEmpty()      // AHCI is the default in VirtualBox
                             || (!desc.strNemuCurrent.compare("ahci", Utf8Str::CaseInsensitive))
                           )
                            strResourceSubType = "AHCI";
                        else
                            throw setError(NEMU_E_NOT_SUPPORTED,
                                            tr("Invalid config string \"%s\" in SATA controller"), desc.strNemuCurrent.c_str());
                ...
                case VirtualSystemDescriptionType_HardDiskControllerSAS:
                    ...
                    */
                    if (uLoop == 1)
                    {
                        ...
                        if (    desc.strNemuCurrent.isEmpty()      // LsiLogic is the default in VirtualBox
                             || (!desc.strNemuCurrent.compare("lsilogic", Utf8Str::CaseInsensitive))
                            )
                            strResourceSubType = "lsilogic";
                        else if (!desc.strNemuCurrent.compare("buslogic", Utf8Str::CaseInsensitive))
                            strResourceSubType = "buslogic";
                        else if (!desc.strNemuCurrent.compare("lsilogicsas", Utf8Str::CaseInsensitive))
                ...
                case VirtualSystemDescriptionType_CDROM:
                    ...
                    if (uLoop == 2)
                    {
                        ...
                        //skip empty Medium. There are no information to add into section <References> or <DiskSection>
                        if (desc.strNemuCurrent.isNotEmpty() &&
                            desc.skipIt == false)
                        {
                            // the following references the "<Disks>" XML block
                            strHostResource = Utf8StrFmt("/disk/%s", strDiskID.c_str());
                ...
                case VirtualSystemDescriptionType_NetworkAdapter:
                    ...
                    if (uLoop == 2)
                    {
                        ...
                        /* Set the hardware type to something useful.
                            ...
                            * E1000 cards. */
                        switch (desc.strNemuCurrent.toInt32())
                        {
    ...
    // under the nemu: namespace
    xml::ElementNode *pelmNemuMachine = pelmVirtualSystem->createChild("nemu:Machine");
    // ovf:required="false" tells other OVF parsers that they can ignore this thing
    pelmNemuMachine->setAttribute("ovf:required", "false");
    // ovf:Info element is required or VMware will bail out on the nemu:Machine element
    pelmNemuMachine->createChild("ovf:Info")->addContent("Complete VirtualBox machine configuration in VirtualBox format");
    ...
    try
    {
        ...
        // write the machine config to the nemu:Machine element
        pConfig->buildMachineXML(*pelmNemuMachine,
                                   settings::MachineConfigFile::BuildMachineXML_WriteNemuVersionAttribute
...
HRESULT Appliance::i_writeS3(TaskOVF *pTask)
{
    ...
    try
    {
        ...
        vrc = RTS3Create(&hS3, pTask->locInfo.strUsername.c_str(), pTask->locInfo.strPassword.c_str(),
                         pTask->locInfo.strHostname.c_str(), "virtualbox-agent/" NEMU_VERSION_STRING);
        if (RT_FAILURE(vrc))
