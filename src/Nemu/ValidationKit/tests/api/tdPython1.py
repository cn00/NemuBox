...
class tdPython1(nemu.TestDriver):
    """
    ...
    def testEventQueueWaitingThreadProc(self):
        ...
        try:
            rc2 = self.oNemuMgr.waitForEvents(0);
        except:
            return True;
    ...
    def testEventQueueWaiting(self):
        ...
        for cMsTimeout in (0, 1, 2, 3, 256, 1000, 0):
            ...
            while True:
                try:
                    rc = self.oNemuMgr.waitForEvents(cMsTimeout);
                except:
                    reporter.errorXcpt();
                    break;
        ...
        sNemuAssertSaved = os.environ.get('NEMU_ASSERT', 'breakpoint');
        os.environ['NEMU_ASSERT'] = 'ignore';
        oThread = threading.Thread(target=self.testEventQueueWaitingThreadProc);
        oThread.start();
        oThread.join();
        os.environ['NEMU_ASSERT'] = sNemuAssertSaved;
    ...
    def interruptWaitEventsThreadProc(self):
        ...
        time.sleep(2);
        try:
            rc2 = self.oNemuMgr.interruptWaitEvents();
        except:
            reporter.errorXcpt();
        else:
            if rc2 is True:
    ...
    def testEventQueueInterrupt(self):
        ...
        for i in range(0, 10):
            try:
                rc = self.oNemuMgr.interruptWaitEvents();
            except:
                reporter.errorXcpt();
                break;
            if rc is not True:
