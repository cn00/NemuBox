...
HRESULT Appliance::interpret()
{
    ...
    try
    {
             ...
             ++it)
        {
            ...
            // if the virtual system in OVF had a <nemu:Machine> element, have the
            // VirtualBox settings code parse that XML now
            if (vsysThis.pelmNemuMachine)
                pNewDesc->i_importNemuMachineXML(*vsysThis.pelmNemuMachine);

            // Guest OS type
            // This is taken from one of three places, in this order:
            Utf8Str strOsTypeNemu;
            Utf8StrFmt strCIMOSType("%RU32", (uint32_t)vsysThis.cimos);
            // 1) If there is a <nemu:Machine>, then use the type from there.
            if (   vsysThis.pelmNemuMachine
                && pNewDesc->m->pConfig->machineUserData.strOsType.isNotEmpty()
               )
                strOsTypeNemu = pNewDesc->m->pConfig->machineUserData.strOsType;
            // 2) Otherwise, if there is OperatingSystemSection/nemu:OSType, use that one.
            else if (vsysThis.strTypeNemu.isNotEmpty())      // OVFReader has found nemu:OSType
                strOsTypeNemu = vsysThis.strTypeNemu;
            // 3) Otherwise, make a best guess what the nemu type is from the OVF (CIM) OS type.
            else
                convertCIMOSType2NemuOSType(strOsTypeNemu, vsysThis.cimos, vsysThis.strCimosDesc);
            ...
            /* VM name */
            Utf8Str nameNemu;
            /* If there is a <nemu:Machine>, we always prefer the setting from there. */
            if (   vsysThis.pelmNemuMachine
                && pNewDesc->m->pConfig->machineUserData.strName.isNotEmpty())
                nameNemu = pNewDesc->m->pConfig->machineUserData.strName;
            else
                nameNemu = vsysThis.strName;
            /* If there isn't any name specified create a default one out
             * of the OS type */
            if (nameNemu.isEmpty())
                nameNemu = strOsTypeNemu;
            i_searchUniqueVMName(nameNemu);
            ...
            rc = mVirtualBox->ComposeMachineFilename(Bstr(nameNemu).raw(),
                                                     NULL /* aGroup */,
                                                     NULL /* aCreateFlags */,
                                                     NULL /* aBaseFolder */,
            ...
            rc = mVirtualBox->GetGuestOSType(Bstr(strOsTypeNemu).raw(), pGuestOSType.asOutParam());
            if (FAILED(rc)) throw rc;
            ...
            if (   vsysThis.pelmNemuMachine
                ...
                cpuCountNemu = pNewDesc->m->pConfig->hardwareMachine.cCPUs;
            else
                cpuCountNemu = vsysThis.cCPUs;
            /* Check for the constraints */
            if (cpuCountNemu > SchemaDefs::MaxCPUCount)
            {
                i_addWarning(tr("The virtual system \"%s\" claims support for %u CPU's, but VirtualBox has support for "
                                "max %u CPU's only."),
                                vsysThis.strName.c_str(), cpuCountNemu, SchemaDefs::MaxCPUCount);
                cpuCountNemu = SchemaDefs::MaxCPUCount;
            }
            if (vsysThis.cCPUs == 0)
                cpuCountNemu = 1;
            pNewDesc->i_addEntry(VirtualSystemDescriptionType_CPU,
                                ...
                                Utf8StrFmt("%RU32", (uint32_t)cpuCountNemu));

            /* RAM */
            uint64_t ullMemSizeNemu;
            /* If there is a <nemu:Machine>, we always prefer the setting from there. */
            if (   vsysThis.pelmNemuMachine
                && pNewDesc->m->pConfig->hardwareMachine.ulMemorySizeMB)
                ullMemSizeNemu = pNewDesc->m->pConfig->hardwareMachine.ulMemorySizeMB;
            else
                ullMemSizeNemu = vsysThis.ullMemorySize / _1M;
            /* Check for the constraints */
            if (    ullMemSizeNemu != 0
                 && (    ullMemSizeNemu < MM_RAM_MIN_IN_MB
                      || ullMemSizeNemu > MM_RAM_MAX_IN_MB
                    )
               )
            {
                i_addWarning(tr("The virtual system \"%s\" claims support for %llu MB RAM size, but VirtualBox has "
                                ...
                                vsysThis.strName.c_str(), ullMemSizeNemu, MM_RAM_MIN_IN_MB, MM_RAM_MAX_IN_MB);
                ullMemSizeNemu = RT_MIN(RT_MAX(ullMemSizeNemu, MM_RAM_MIN_IN_MB), MM_RAM_MAX_IN_MB);
            }
            if (vsysThis.ullMemorySize == 0)
            {
                /* If the RAM of the OVF is zero, use our predefined values */
                ULONG memSizeNemu2;
                rc = pGuestOSType->COMGETTER(RecommendedRAM)(&memSizeNemu2);
                if (FAILED(rc)) throw rc;
                /* Nemu stores that in MByte */
                ullMemSizeNemu = (uint64_t)memSizeNemu2;
            }
            pNewDesc->i_addEntry(VirtualSystemDescriptionType_Memory,
                                 "",
                                 Utf8StrFmt("%RU64", (uint64_t)vsysThis.ullMemorySize),
                                 Utf8StrFmt("%RU64", (uint64_t)ullMemSizeNemu));
            ...
            else if (vsysThis.llEthernetAdapters.size() >  0)
            {
                ...
                NetworkAdapterType_T defaultAdapterNemu = NetworkAdapterType_Am79C970A;
                rc = pGuestOSType->COMGETTER(AdapterType)(&defaultAdapterNemu);
                if (FAILED(rc)) throw rc;
                     ...
                     ++itEA, ++a)
                {
                    ...
                    NetworkAdapterType_T nwAdapterNemu = defaultAdapterNemu;
                    if (!ea.strAdapterType.compare("PCNet32", Utf8Str::CaseInsensitive))
                    {
                        ...
                        if (!(defaultAdapterNemu == NetworkAdapterType_Am79C970A ||
                              defaultAdapterNemu == NetworkAdapterType_Am79C973))
                            nwAdapterNemu = NetworkAdapterType_Am79C970A;
                    }
                    ...
                    /* VMWare accidentally write this with VirtualCenter 3.5,
                       so make sure in this case always to use the VMWare one */
                        ...
                        nwAdapterNemu = NetworkAdapterType_I82545EM;
                    else if (!ea.strAdapterType.compare("E1000", Utf8Str::CaseInsensitive))
                    {
                        ...
                        if (Utf8Str(vsysThis.strVirtualSystemType).contains("virtualbox", Utf8Str::CaseInsensitive))
                        {
                            ...
                            if (!(defaultAdapterNemu == NetworkAdapterType_I82540EM ||
                                  defaultAdapterNemu == NetworkAdapterType_I82543GC ||
                                  defaultAdapterNemu == NetworkAdapterType_I82545EM))
                            nwAdapterNemu = NetworkAdapterType_I82540EM;
                        }
                        else
                            /* Always use this one since it's what VMware uses */
                            nwAdapterNemu = NetworkAdapterType_I82545EM;
                    }
#endif /* NEMU_WITH_E1000 */

                    pNewDesc->i_addEntry(VirtualSystemDescriptionType_NetworkAdapter,
                                         "",      // ref
                                         ea.strNetworkName,      // orig
                                         Utf8StrFmt("%RU32", (uint32_t)nwAdapterNemu),   // conf
                                         0,
                                               ...
                                               RTCString &availableImage)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        throw setError(NEMU_E_FILE_ERROR, tr("Empty directory folder (%s) isn't allowed in the OVA package (%Rrc)"),
                       pszFilename, VERR_IS_A_DIRECTORY);
    }

    throw setError(NEMU_E_FILE_ERROR, tr("Could not open the current file in the OVA package (%Rrc)"), vrc);
}
        ...
        try
        {
            ...
            if (RTFileExists(strMfFile.c_str()))
            {
                ...
                if (RT_SUCCESS(vrc) && pFile != NULL)
                {
                    ...
                    if (RT_FAILURE(vrc))
                        throw setError(NEMU_E_IPRT_ERROR, "Creation of the VD interface failed (%Rrc)", vrc);
                ...
                && RTStrICmp(&pszName[cchName - sizeof(".ovf") + 1], ".ovf") == 0)
            {
                ...
                if (pShaIo)
                {
                    ...
                    if (RT_SUCCESS(vrc))
                        ...
                        hrc = setError(NEMU_E_IPRT_ERROR, "Creation of the VD interface failed (%Rrc)", vrc);
                    RTMemFree(pShaIo);
                }
                else
                    hrc = E_OUTOFMEMORY;
                ...
                hrc = setError(NEMU_E_FILE_ERROR,
                               tr("First file in the OVA package must have the extension 'ovf'. But the file '%s' has a different extension."),
                               pszName);
        }
        else
            hrc = setError(NEMU_E_FILE_ERROR, tr("Error reading OVA file '%s' (%Rrc)"), pTask->locInfo.strPath.c_str(), vrc);
        fssRdOnlyDestroyInterface(pTarIo);
    }
    else
        hrc = setError(NEMU_E_FILE_ERROR, tr("Could not open the OVA file '%s' (%Rrc)"), pTask->locInfo.strPath.c_str(), vrc);
    ...
    try
    {
        ...
        if (m->pReader->m_envelopeData.getOVFVersion() == ovf::OVFVersion_2_0)
        {
            ...
            if (RT_SUCCESS(vrc))
                ...
                throw setError(NEMU_E_FILE_ERROR, tr("Converting SHA256 digest to a string was failed (%Rrc)"), vrc);
...
HRESULT Appliance::i_readS3(TaskOVF *pTask)
{
    ...
    char *pszTmpDir = RTPathJoinA(szOSTmpDir, "nemu-ovf-XXXXXX");
    ...
    try
    {
        ...
        vrc = RTS3Create(&hS3,
                         ...
                         "virtualbox-agent/" NEMU_VERSION_STRING);
        if (RT_FAILURE(vrc))
            throw setError(NEMU_E_IPRT_ERROR,
...
HRESULT Appliance::i_importFSOVF(TaskOVF *pTask, AutoWriteLockBase& writeLock)
{
    ...
    catch (HRESULT rc2)
    {
         ...
         * During import Nemu creates new UUIDs for imported images and
...
HRESULT Appliance::i_importFSOVA(TaskOVF *pTask, AutoWriteLockBase& writeLock)
{
    ...
    try
    {
        ...
        if (   RT_FAILURE(vrc)
            && vrc != VERR_EOF)
            throw setError(NEMU_E_IPRT_ERROR, tr("Seeking within the archive failed (%Rrc)"), vrc);
                                      ...
                                      PSHASTORAGE pStorage)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        rc = setError(NEMU_E_IPRT_ERROR, "Seeking within the archive failed (%Rrc)", vrc);
...
HRESULT Appliance::i_verifyCertificateFile(void *pvBuf, size_t cbSize, PSHASTORAGE pStorage)
{
    ...
    if (RT_FAILURE(vrc))
    {
        rc = setError(NEMU_E_FILE_ERROR, tr("Digest type of certificate is unknown"));
    }
    else
    {
        ...
        {
            {
                ...
                if(errValue != 0)
                {
                    rc = setError(NEMU_E_FILE_ERROR, tr(errStrDesc));
...

            {
                ...
                rc = setError(NEMU_E_FILE_ERROR, tr(errStrDesc.c_str()));
            }
        }
        else
        {
    ...
    switch (hdc.system)
    {
        case ovf::HardDiskController::IDE:
            ...
            switch (ulAddressOnParent)
            {
                ...
                default:
                    throw setError(NEMU_E_NOT_SUPPORTED,
 ...
 *  --  i_importNemuMachine(); in that case, the information comes from the <nemu:Machine>
 *      tag.
 *
 ...
 * spec, even though this cannot really happen in the nemu:Machine case since such data
    ...
    else
    {
        ...
        try
        {
            ...
            if ((pszSuff = RTPathSuffix(strTargetPath->c_str()))!=NULL)
            {
                 ...
                 * or via NemuManage import command (option --importtovdi)
                                       ...
                                       PSHASTORAGE pStorage)
{
    ...
    rc = mVirtualBox->GetGuestOSType(Bstr(stack.strOsTypeNemu).raw(), osType.asOutParam());
    if (FAILED(rc)) throw rc;

    /* Create the machine */
    SafeArray<BSTR> groups; /* no groups */
    rc = mVirtualBox->CreateMachine(NULL, /* machine name: use default */
                                    Bstr(stack.strNameNemu).raw(),
                                    ComSafeArrayAsInParam(groups),
                                    Bstr(stack.strOsTypeNemu).raw(),
                                    NULL, /* aCreateFlags */
    ...
    ULONG vramNemu;
    rc = osType->COMGETTER(RecommendedVRAM)(&vramNemu);
    if (FAILED(rc)) throw rc;

    /* Set the VRAM */
    rc = pNewMachine->COMSETTER(VRAMSize)(vramNemu);
    if (FAILED(rc)) throw rc;
    ...
    if (vsdeNW.empty())
    {
        ...
        ComPtr<INetworkAdapter> nwNemu;
        rc = pNewMachine->GetNetworkAdapter(0, nwNemu.asOutParam());
        if (FAILED(rc)) throw rc;
        rc = nwNemu->COMSETTER(Enabled)(false);
        if (FAILED(rc)) throw rc;
    }
    ...
    else
    {
             ...
             ++nwIt, ++a)
        {
            ...
            const Utf8Str &nwTypeNemu = pvsys->strNemuCurrent;
            uint32_t tt1 = RTStrToUInt32(nwTypeNemu.c_str());
    ...
    if (!vsdeHDCIDE.empty())
    {
        ...
        const char *pcszIDEType = vsdeHDCIDE.front()->strNemuCurrent.c_str();
    ...
    {
        ComPtr<IStorageController> pController;
        const Utf8Str &hdcNemu = vsdeHDCSATA.front()->strNemuCurrent;
        if (hdcNemu == "AHCI")
        {
    ...
    if (!vsdeHDCSCSI.empty())
    {
        ...
        const Utf8Str &hdcNemu = vsdeHDCSCSI.front()->strNemuCurrent;
        if (hdcNemu == "LsiLogic")
            controllerType = StorageControllerType_LsiLogic;
        else if (hdcNemu == "LsiLogicSas")
        {
            // OVF treats LsiLogicSas as a SCSI controller but Nemu considers it a class of its own
            bstrName = L"SAS Controller";
            busType = StorageBus_SAS;
            controllerType = StorageControllerType_LsiLogicSas;
        }
        else if (hdcNemu == "BusLogic")
    ...
    if (!avsdeHDs.empty())
    {
        ...
        try
        {
            ...
            while (oit != stack.mapDisks.end() && cImportedDisks != avsdeHDs.size())
            {
                        ...
                        if (availableImage.compare(diCurrent.strHref, Utf8Str::CaseInsensitive) != 0)
                        {
                             ...
                             */
                            {
                                ...
                                if (!vsdeTargetHD)
                                {
                                     ...
                                     * May be Nemu imports OVA package with wrong file sequence inside the archive.
                                     */
                                    throw setError(E_FAIL,
                                                   tr("Internal inconsistency looking up disk image '%s'"),
                                                   diCurrent.strHref.c_str());
                ...
                Utf8Str savedNemuCurrent = vsdeTargetHD->strNemuCurrent;

                i_importOneDiskImage(diCurrent,
                                     &vsdeTargetHD->strNemuCurrent,
                                     pTargetHD,
                                     stack,
                                     pCallbacks,
                                     pStorage);
                ...
                vsdeTargetHD->strNemuCurrent.c_str(), mhda.lControllerPort, mhda.lDevice));
                ...
                if (vdf.compare("RAW", Utf8Str::CaseInsensitive) == 0)
                {
                    ...
                    rc = mVirtualBox->OpenMedium(Bstr(vsdeTargetHD->strNemuCurrent).raw(),
                                                 DeviceType_DVD,
                ...
                /* restore */
                vsdeTargetHD->strNemuCurrent = savedNemuCurrent;
...
/**
 * Imports one OVF virtual system (described by a nemu:Machine tag represented by the given config
 ...
 * the disk attachments used by the machine when it was exported. We also add nemu:uuid attributes
 ...
 * @param stack
 */
void Appliance::i_importNemuMachine(ComObjPtr<VirtualSystemDescription> &vsdescThis,
                                    ...
                                    PSHASTORAGE pStorage)
{
    ...
    /* OS Type */
    config.machineUserData.strOsType = stack.strOsTypeNemu;
    /* Description */
...
*/
             ...
             ++itNW)
        {
                ...
                && vsdeNW->strExtraConfigCurrent.length() > 6)
            {
                     ...
                     ++it1)
                {
                    ...
                    {
                        it1->fEnabled = true;
                        it1->type = (NetworkAdapterType_T)vsdeNW->strNemuCurrent.toUInt32();
                        break;
                    }
                }
            }
    ...
    while(oit != stack.mapDisks.end() && cImportedDisks != avsdeHDs.size())
    {
        ...
        if (!fFound)
            throw setError(E_FAIL,
                           tr("<nemu:Machine> element in OVF contains a medium attachment for the disk image %s "
    ...
    // instance that we created from the nemu:Machine
    rc = pNewMachine->init(mVirtualBox,
                           stack.strNameNemu,// name from OVF preparations; can be suffixed to avoid duplicates
                                 ...
                                 PSHASTORAGE pStorage)
{
         ...
         ++it, ++it1, ++i)
    {
        ...
        // there are two ways in which we can create a nemu machine from OVF:
        // -- either this OVF was written by nemu 3.2 or later, in which case there is a <nemu:Machine> element
        ...
        // -- or this is an OVF from an older nemu or an external source, and then we need to translate the
        //    VirtualSystemDescriptionEntry and do import work

        // Even for the nemu:Machine case, there are a number of configuration items that will be taken from
        ...
        if (vsdeName.size() < 1)
            throw setError(NEMU_E_FILE_ERROR,
                           tr("Missing VM name"));
        stack.strNameNemu = vsdeName.front()->strNemuCurrent;

        // have VirtualBox suggest where the filename would be placed so we can
        // put the disk images in the same directory
        Bstr bstrMachineFilename;
        rc = mVirtualBox->ComposeMachineFilename(Bstr(stack.strNameNemu).raw(),
                                                 NULL /* aGroup */,
                                                 NULL /* aCreateFlags */,
                                                 NULL /* aBaseFolder */,
                                                 bstrMachineFilename.asOutParam());
        ...
        LogFunc(("i=%zu strName=%s bstrMachineFilename=%ls\n", i, stack.strNameNemu.c_str(), bstrMachineFilename.raw()));
        ...
        stack.strOsTypeNemu = vsdeOS.front()->strNemuCurrent;

        // CPU count
        std::list<VirtualSystemDescriptionEntry*> vsdeCPU = vsdescThis->i_findByType(VirtualSystemDescriptionType_CPU);
        if (vsdeCPU.size() != 1)
            throw setError(NEMU_E_FILE_ERROR, tr("CPU count missing"));

        stack.cCPUs = vsdeCPU.front()->strNemuCurrent.toUInt32();
        ...
        if (vsdeRAM.size() != 1)
            throw setError(NEMU_E_FILE_ERROR, tr("RAM size missing"));
        stack.ulMemorySizeMB = (ULONG)vsdeRAM.front()->strNemuCurrent.toUInt64();
        ...
        if (!vsdeAudioAdapter.empty())
            stack.strAudioAdapter = vsdeAudioAdapter.front()->strNemuCurrent;
        ...
        if (!vsdeDescription.empty())
            stack.strDescription = vsdeDescription.front()->strNemuCurrent;

        // import nemu:machine or OVF now
        if (vsdescThis->m->pConfig)
            // nemu:Machine config
            i_importNemuMachine(vsdescThis, pNewMachine, stack, pCallbacks, pStorage);
        else
            // generic OVF config
