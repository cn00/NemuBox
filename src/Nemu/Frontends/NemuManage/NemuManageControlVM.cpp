/* $Id: NemuManageControlVM.cpp $ */
/** @file
 * NemuManage - Implementation of the controlvm command.
 */
...
unsigned int getMaxNics(IVirtualBox* nemu, IMachine* mach)
{
    ...
    do {
        CHECK_ERROR_BREAK(nemu, COMGETTER(SystemProperties)(info.asOutParam()));
...
#undef ITERATE_TO_NEXT_TERM
        ...
        else if (!strcmp(a->argv[1], "teleport"))
        {
            ...
            if (cMsTimeout)
            {
                ...
                if (FAILED(rc) && rc != NEMU_E_INVALID_OBJECT_STATE)
