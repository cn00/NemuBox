/* $Id: NemuDTraceWrapper.cpp $ */
/** @file
 * NemuDTrace - Wrapper that selects the right dtrace implemetation and adds
...
*********************************************************************************************************************************/
/** The NemuDTrace extension pack name.   */
#define NEMU_EXTPACK_NEMUDTRACE_NAME            "Oracle NemuDTrace Extension Pack"
/** The mangled version of NEMU_EXTPACK_NEMUDTRACE_NAME (also in Config.kmk). */
#define NEMU_EXTPACK_NEMUDTRACE_MANGLED_NAME    "Oracle_NemuDTrace_Extension_Pack"


/*********************************************************************************************************************************
*   Structures and Typedefs                                                                                                      *
*********************************************************************************************************************************/
/** The main function of NemuDTrace.so/dylib/dll. */
typedef int (RTCALL *PFNNEMUDTRACEMAIN)(int argc, char **argv);
...
int main(int argc, char **argv)
{
    ...
    if (!RTEnvExist("NEMU_DTRACE_NO_NATIVE"))
...
# ifdef RT_OS_LINUX
                /** @todo Warn if the dtrace modules haven't been loaded or nemudrv isn't
...
# endif
    ...
    {
        /*
         * 2. NemuDTrace extension pack installed?
         *
        ...
        if (RT_SUCCESS(rc))
            rc = RTPathAppend(szDTraceCmd, sizeof(szDTraceCmd),
                              NEMU_EXTPACK_INSTALL_DIR RTPATH_SLASH_STR NEMU_EXTPACK_NEMUDTRACE_MANGLED_NAME);
        if (RT_SUCCESS(rc))
            rc = RTPathAppend(szDTraceCmd, sizeof(szDTraceCmd), RTBldCfgTargetDotArch());
        if (RT_SUCCESS(rc))
            rc = RTPathAppend(szDTraceCmd, sizeof(szDTraceCmd), "NemuDTraceCmd");
        if (RT_SUCCESS(rc))
        ...
        if (!RTFileExists(szDTraceCmd))
            return RTMsgErrorExit(RTEXITCODE_FAILURE,
                                  "Unable to find a DTrace implementation. NemuDTrace Extension Pack installed?");
    ...
    if (RT_FAILURE(rc))
        return RTMsgErrorExit(RTEXITCODE_FAILURE, "Error constructing dtrace library path for Nemu: %Rrc", rc);
    ...
    /*
     * While the NemuDTrace we load and call the main function of.
     */
    else
    {
        ...
        if (RT_SUCCESS(rc))
        {
            PFNNEMUDTRACEMAIN pfnNemuDTraceMain;
            rc = RTLdrGetSymbol(hMod, "NemuDTraceMain", (void **)&pfnNemuDTraceMain);
            if (RT_SUCCESS(rc))
                rcExit = (RTEXITCODE)pfnNemuDTraceMain(cArgs, papszArgs);
            else
                rcExit = RTMsgErrorExit(RTEXITCODE_FAILURE, "Error locating 'NemuDTraceMain' in '%s': %Rrc", szDTraceCmd, rc);
        }
        else
