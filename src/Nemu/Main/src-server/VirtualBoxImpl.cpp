...
/** @file
 * Implementation of IVirtualBox in NemuSVC.
 */
...
#define NEMU_GLOBAL_SETTINGS_FILE "VirtualBox.xml"
...
public:
    ...
    CallbackEvent(VirtualBox *aVirtualBox, NemuEventType_T aWhat)
    ...
    virtual HRESULT prepareEventDesc(IEventSource* aSource, NemuEventDesc& aEvDesc) = 0;
...
HRESULT VirtualBox::init()
{
    ...
    if (sVersionNormalized.isEmpty())
    {
        ...
        if (tmp.endsWith(NEMU_BUILD_PUBLISHER))
            tmp = tmp.substr(0, tmp.length() - strlen(NEMU_BUILD_PUBLISHER));
        sVersionNormalized = tmp;
    }
    sRevision = RTBldCfgRevision();
    if (sPackageType.isEmpty())
        sPackageType = NEMU_PACKAGE_STRING;
    if (sAPIVersion.isEmpty())
        sAPIVersion = NEMU_API_VERSION_STRING;
    ...
    /* Get the VirtualBox home directory. */
    {
        ...
        int vrc = com::GetNemuUserHomeDirectory(szHomeDir, sizeof(szHomeDir));
        if (RT_FAILURE(vrc))
    ...
    try
    {
        ...
        if (SUCCEEDED(rc))
            rc = m->ptrExtPackManager->initExtPackManager(this, NEMUEXTPACKCTX_PER_USER_DAEMON);
        if (FAILED(rc))
            throw rc;
#endif
...
{
    AssertCompile(NEMU_VERSION_MAJOR < 128 && NEMU_VERSION_MAJOR > 0);
    AssertCompile((uint64_t)NEMU_VERSION_MINOR < 256);
    uint64_t uRevision = ((uint64_t)NEMU_VERSION_MAJOR << 56)
                       | ((uint64_t)NEMU_VERSION_MINOR << 48);

    if (NEMU_VERSION_BUILD >= 51 && (NEMU_VERSION_BUILD & 1)) /* pre-release trunk */
        uRevision |= (uint64_t)NEMU_VERSION_BUILD << 40;
...
HRESULT VirtualBox::getPerformanceCollector(ComPtr<IPerformanceCollector> &aPerformanceCollector)
{
    ...
    return S_OK;
#else /* !NEMU_WITH_RESOURCE_USAGE_API */
                                         ...
                                         BOOL *aResult)
{
    ...
    firmwareDesc[] =
    {
        ...
        {
            FirmwareType_EFI32,   "NemuEFI32.fd",   "http://virtualbox.org/firmware/NemuEFI32.fd"
        },
        {
            FirmwareType_EFI64,   "NemuEFI64.fd",   "http://virtualbox.org/firmware/NemuEFI64.fd"
        },
        {
            FirmwareType_EFIDUAL, "NemuEFIDual.fd", "http://virtualbox.org/firmware/NemuEFIDual.fd"
        }
    };
    ...
    for (size_t i = 0; i < sizeof(firmwareDesc) / sizeof(firmwareDesc[0]); i++)
    {
        ...
        char pszNemuPath[RTPATH_MAX];
        rc = RTPathExecDir(pszNemuPath, RTPATH_MAX);
        AssertRCReturn(rc, rc);
                                           ...
                                           com::Utf8Str       &aFile)
{
    ...
    if (strGroup == "/")
        strGroup.setNull();
    aFile = com::Utf8StrFmt("%s%s%c%s%c%s.nemu",
                            strBase.c_str(),
                            strGroup.c_str(),
                            RTPATH_DELIMITER,
                            strDirName.c_str(),
                                ...
                                ComPtr<IMachine> &aMachine)
{
    ...
    if (id.isValid() && !id.isZero())
        ...
        rc = i_findMachine(id,
                           ...
                           true /* setError */,
                           &pMachineFound);
        // returns NEMU_E_OBJECT_NOT_FOUND if not found and sets error
    else
    {
                               ...
                               ComPtr<IMedium> &aMedium)
{
    ...
    if (pMedium.isNull())
    {
        ...
        if (SUCCEEDED(rc))
        {
            ...
            {
                pMedium->uninit();
                rc = NEMU_E_OBJECT_NOT_FOUND;
            }
        }
        else
        {
            if (rc != NEMU_E_INVALID_OBJECT_STATE)
                rc = NEMU_E_OBJECT_NOT_FOUND;
        }
    }
...
{
    MachineEvent(VirtualBox *aVB, NemuEventType_T aWhat, const Guid &aId, BOOL aBool)
        : CallbackEvent(aVB, aWhat), id(aId.toUtf16())
        , mBool(aBool)
        { }

    MachineEvent(VirtualBox *aVB, NemuEventType_T aWhat, const Guid &aId, MachineState_T aState)
        : CallbackEvent(aVB, aWhat), id(aId.toUtf16())
        , mState(aState)
        {}

    virtual HRESULT prepareEventDesc(IEventSource* aSource, NemuEventDesc& aEvDesc)
    {
        switch (mWhat)
        {
            case NemuEventType_OnMachineDataChanged:
                aEvDesc.init(aSource, mWhat, id.raw(), mBool);
                break;

            case NemuEventType_OnMachineStateChanged:
                aEvDesc.init(aSource, mWhat, id.raw(), mState);
                break;

            case NemuEventType_OnMachineRegistered:
                aEvDesc.init(aSource, mWhat, id.raw(), mBool);
                break;
...
{
    i_postEvent(new MachineEvent(this, NemuEventType_OnMachineStateChanged, aId, aState));
}
...
{
    i_postEvent(new MachineEvent(this, NemuEventType_OnMachineDataChanged, aId, aTemporary));
}
                                        ...
                                        Bstr &aError)
{
    ...
    evDesc.init(m->pEventSource, NemuEventType_OnExtraDataCanChange, id.raw(), aKey, aValue);
...
struct ExtraDataEvent : public VirtualBox::CallbackEvent
{
    ExtraDataEvent(VirtualBox *aVB, const Guid &aMachineId,
        ...
        : CallbackEvent(aVB, NemuEventType_OnExtraDataChanged)
        , machineId(aMachineId.toUtf16()), key(aKey), val(aVal)
    {}

    virtual HRESULT prepareEventDesc(IEventSource* aSource, NemuEventDesc& aEvDesc)
    {
        return aEvDesc.init(aSource, NemuEventType_OnExtraDataChanged, machineId.raw(), key.raw(), val.raw());
    }
...
{
    i_postEvent(new MachineEvent(this, NemuEventType_OnMachineRegistered, aId, aRegistered));
}
...
struct SessionEvent : public VirtualBox::CallbackEvent
{
    SessionEvent(VirtualBox *aVB, const Guid &aMachineId, SessionState_T aState)
        : CallbackEvent(aVB, NemuEventType_OnSessionStateChanged)
        , machineId(aMachineId.toUtf16()), sessionState(aState)
    {}

    virtual HRESULT prepareEventDesc(IEventSource* aSource, NemuEventDesc& aEvDesc)
    {
        return aEvDesc.init(aSource, NemuEventType_OnSessionStateChanged, machineId.raw(), sessionState);
    }
    Bstr machineId;
...
{
    i_postEvent(new SnapshotEvent(this, aMachineId, aSnapshotId,
                                  NemuEventType_OnSnapshotDeleted));
}
...
{
    i_postEvent(new SnapshotEvent(this, aMachineId, aSnapshotId,
                                  NemuEventType_OnSnapshotRestored));
}
...
{
    i_postEvent(new SnapshotEvent(this, aMachineId, aSnapshotId,
                                  NemuEventType_OnSnapshotChanged));
}

/** Event for onGuestPropertyChange() */
struct GuestPropertyEvent : public VirtualBox::CallbackEvent
{
    GuestPropertyEvent(VirtualBox *aNemu, const Guid &aMachineId,
                       IN_BSTR aName, IN_BSTR aValue, IN_BSTR aFlags)
        : CallbackEvent(aNemu, NemuEventType_OnGuestPropertyChanged),
    ...
    virtual HRESULT prepareEventDesc(IEventSource* aSource, NemuEventDesc& aEvDesc)
    {
        return aEvDesc.init(aSource, NemuEventType_OnGuestPropertyChanged,
 ...
 *
 * @return S_OK, E_INVALIDARG or NEMU_E_OBJECT_NOT_FOUND when not found.
 *
 ...
 *
 * @return S_OK when found or E_INVALIDARG or NEMU_E_OBJECT_NOT_FOUND when not found.
 *
 * @note Locks the media tree for reading.
 */
                                           ...
                                           ComObjPtr<Medium> *aImage /* = NULL */)
{
    ...
    HRESULT rc = found ? S_OK : NEMU_E_OBJECT_NOT_FOUND;
 ...
 *
 * If none of the above apply, this returns NEMU_E_OBJECT_NOT_FOUND.
 *
...
HRESULT VirtualBox::i_registerMachine(Machine *aMachine)
{
...

    {
        ...
        ComAssertRet(rc == NEMU_E_OBJECT_NOT_FOUND, rc);
        rc = S_OK;
    }
...
HRESULT VirtualBox::i_ensureFilePathExists(const Utf8Str &strFileName, bool fCreate)
{
    ...
    if (!RTDirExists(strDir.c_str()))
    {
        if (fCreate)
        {
            ...
            if (RT_FAILURE(vrc))
                return i_setErrorStatic(NEMU_E_IPRT_ERROR,
                                         ...
                                         bool aSaveSettings /*= true*/)
{
    ...
    AutoWriteLock nemuLock(this COMMA_LOCKVAL_SRC_POS);
