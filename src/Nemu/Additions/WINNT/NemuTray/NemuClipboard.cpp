/* $Id: NemuClipboard.cpp $ */
/** @file
 * NemuClipboard - Shared clipboard, Windows Guest Implementation.
 */
...
typedef struct _NEMUCLIPBOARDCONTEXT
{
...
static int nemuClipboardChanged(NEMUCLIPBOARDCONTEXT *pCtx)
{
    ...
    else
    {
        ...
        {
            LogFlowFunc(("nemuClipboardChanged: format = 0x%08X\n", format));
            switch (format)
            {
                case CF_UNICODETEXT:
                case CF_TEXT:
...
static LRESULT nemuClipboardProcessMsg(NEMUCLIPBOARDCONTEXT *pCtx, HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    LRESULT rc = 0;
    ...
    switch (msg)
    {
        ...
        case WM_DRAWCLIPBOARD:
        {
            ...
            if (GetClipboardOwner () != hwnd)
            {
                ...
                int nemurc = nemuClipboardChanged(pCtx);
                if (RT_FAILURE(nemurc))
                    LogFlowFunc(("nemuClipboardChanged failed, rc = %Rrc\n", nemurc));
            }
            ...
            else
            {
                ...
                if (hMem)
                {
                    ...
                    if (pMem)
                    {
                        ...
                        int nemurc = VbglR3ClipboardReadData(pCtx->u32ClientID, u32Format, pMem, cbPrealloc, &cb);
                            ...
                            else if (cb > cbPrealloc)
                            {
                                ...
                                if (hMem)
                                {
                                    ...
                                    if (pMem)
                                    {
                                        /* Read the host data to the preallocated buffer. */
                                        uint32_t cbNew = 0;
                                        nemurc = VbglR3ClipboardReadData(pCtx->u32ClientID, u32Format, pMem, cb, &cbNew);
                                        LogFlowFunc(("VbglR3ClipboardReadData returned with rc = %Rrc, cb = %d, cbNew = %d\n", nemurc, cb, cbNew));

                                        if (RT_SUCCESS (nemurc) && cbNew <= cb)
                                        {
                                            cb = cbNew;
                                        }
                                        else
        ...
        case WM_USER:
        {
            ...
            else
            {
                ...
                {
                    LogFlowFunc(("WM_USER: NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT\n"));
                    hClip = SetClipboardData(CF_UNICODETEXT, NULL);
                }

                if (u32Formats & NEMU_SHARED_CLIPBOARD_FMT_BITMAP)
                {
                    LogFlowFunc(("WM_USER: NEMU_SHARED_CLIPBOARD_FMT_BITMAP\n"));
                ...
                if (u32Formats & NEMU_SHARED_CLIPBOARD_FMT_HTML)
                {
                    ...
                    LogFlowFunc(("WM_USER: NEMU_SHARED_CLIPBOARD_FMT_HTML 0x%04X\n", format));
                    if (format != 0)
                    {
        ...
        case WM_USER + 1:
        {
            ...
            else
            {
                ...
                if (u32Formats & NEMU_SHARED_CLIPBOARD_FMT_BITMAP)
                {
                    ...
                    if (hClip != NULL)
                    {
                        ...
                        if (lp != NULL)
                        {
                            ...
                            nemurc = VbglR3ClipboardWriteData(pCtx->u32ClientID, NEMU_SHARED_CLIPBOARD_FMT_BITMAP,
                                                              lp, GlobalSize(hClip));
                            GlobalUnlock(hClip);
                        }
                        else
                ...
                else if (u32Formats & NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT)
                {
                    ...
                    if (hClip != NULL)
                    {
                        ...
                        if (uniString != NULL)
                        {
                            ...
                            nemurc = VbglR3ClipboardWriteData(pCtx->u32ClientID, NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT,
                ...
                else if (u32Formats & NEMU_SHARED_CLIPBOARD_FMT_HTML)
                {
                    UINT format = RegisterClipboardFormat ("HTML Format");
                    if (format != 0)
                    {
                        ...
                        if (hClip != NULL)
                        {
                            ...
                            if (lp != NULL)
                            {
                                ...
                                nemurc = VbglR3ClipboardWriteData(pCtx->u32ClientID, NEMU_SHARED_CLIPBOARD_FMT_HTML,
                                                                  lp, GlobalSize(hClip));
                                GlobalUnlock(hClip);
                            }
                            else
...
#ifndef DEBUG_andy
    LogFlowFunc(("nemuClipboardProcessMsg returned with rc = %ld\n", rc));
#endif
    return rc;
}
...
static int nemuClipboardInit (NEMUCLIPBOARDCONTEXT *pCtx)
{
    ...
    LogFlowFunc(("nemuClipboardInit returned with rc = %Rrc\n", rc));
    return rc;
}

static void nemuClipboardDestroy(NEMUCLIPBOARDCONTEXT *pCtx)
{
    if (pCtx->hwnd)
    {
        removeFromCBChain(pCtx);
...
static NEMUCLIPBOARDCONTEXT gCtx = { NULL };

static LRESULT CALLBACK nemuClipboardWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    /* Forward with proper context. */
    return nemuClipboardProcessMsg(&gCtx, hwnd, msg, wParam, lParam);
}

int NemuClipboardInit(const NEMUSERVICEENV *pEnv, void **ppInstance, bool *pfStartThread)
{
    LogFlowFunc(("NemuClipboardInit\n"));
    if (gCtx.pEnv)
    {
    ...
    if (VbglR3AutoLogonIsRemoteSession())
    {
        ...
        LogRel(("NemuTray: clipboard has been disabled for a remote session.\n"));
        return VERR_NOT_SUPPORTED;
    }
...
unsigned __stdcall NemuClipboardThread(void *pInstance)
{
    LogFlowFunc(("NemuClipboardThread\n"));

    NEMUCLIPBOARDCONTEXT *pCtx = (NEMUCLIPBOARDCONTEXT *)pInstance;
    AssertPtr(pCtx);
    ...
    for (;;)
    {
        ...
        if (RT_FAILURE(rc))
        {
            LogFlowFunc(("NemuClipboardThread: Failed to call the driver for host message! rc = %Rrc\n", rc));
        ...
        {
            LogFlowFunc(("NemuClipboardThread: VbglR3ClipboardGetHostMsg u32Msg = %ld, u32Formats = %ld\n", u32Msg, u32Formats));
            switch (u32Msg)
            {
                ...
                default:
                {
                    LogFlowFunc(("NemuClipboardThread: Unsupported message from host! Message = %ld\n", u32Msg));
                }
            }
        }
    }
...
void NemuClipboardDestroy(const NEMUSERVICEENV *pEnv, void *pInstance)
{
    NEMUCLIPBOARDCONTEXT *pCtx = (NEMUCLIPBOARDCONTEXT *)pInstance;
    if (pCtx != &gCtx)
    {
        LogFlowFunc(("NemuClipboardDestroy: invalid instance %p (our = %p)!\n", pCtx, &gCtx));
        pCtx = &gCtx;
    }

    nemuClipboardDestroy (pCtx);
