/* $Id: SUPDrv.cpp $ */
/** @file
 * NemuDrv - The VirtualBox Support Driver - Common code.
 */
...
#include <Nemu/log.h>
#include <Nemu/err.h>
#include <Nemu/vmm/hm_svm.h>
#include <Nemu/vmm/hm_vmx.h>

#if defined(RT_OS_SOLARIS) || defined(RT_OS_DARWIN)
# include "dtrace/SUPDrv.h"
#else
# define NEMUDRV_SESSION_CREATE(pvSession, fUser) do { } while (0)
# define NEMUDRV_SESSION_CLOSE(pvSession) do { } while (0)
# define NEMUDRV_IOCTL_ENTRY(pvSession, uIOCtl, pvReqHdr) do { } while (0)
# define NEMUDRV_IOCTL_RETURN(pvSession, uIOCtl, pvReqHdr, rcRet, rcReq) do { } while (0)
#endif
 ...
 * instantiation in log-nemu.c(pp).
 */
...
int NEMUCALL supdrvInitDevExt(PSUPDRVDEVEXT pDevExt, size_t cbSession)
{
    ...
    rc = RTLogCreate(&pRelLogger, 0 /* fFlags */, "all",
                     "NEMU_RELEASE_LOG", RT_ELEMENTS(s_apszGroups), s_apszGroups, RTLOGDEST_STDOUT | RTLOGDEST_DEBUGGER, NULL);
    if (RT_SUCCESS(rc))
    ...
    if (pSession)
    {
        ...
        if (!rc)
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                NEMUDRV_SESSION_CREATE(pSession, fUser);
...
{
    NEMUDRV_SESSION_CLOSE(pSession);
 ...
 */
int NEMUCALL supdrvSessionHashTabInsert(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, PSUPDRVSESSION *ppOsSessionPtr,
                                        void *pvUser)
{
    PSUPDRVSESSION  pCur;
    unsigned        iHash;
 ...
 *
 * @returns Nemu status code that should be passed down to ring-3 unchanged.
...
{
    int rc;
    NEMUDRV_IOCTL_ENTRY(pSession, uIOCtl, pReqHdr);
    ...
    {
        OSDBGPRINT(("nemudrv: Bad ioctl request size; cbReq=%#lx\n", (long)cbReq));
        NEMUDRV_IOCTL_RETURN(pSession, uIOCtl, pReqHdr, VERR_INVALID_PARAMETER, VINF_SUCCESS);
        return VERR_INVALID_PARAMETER;
    }
    if (RT_UNLIKELY(   (pReqHdr->fFlags & SUPREQHDR_FLAGS_MAGIC_MASK) != SUPREQHDR_FLAGS_MAGIC
                    ...
                    || pReqHdr->cbOut > cbReq))
    {
        OSDBGPRINT(("nemudrv: Bad ioctl request header; cbIn=%#lx cbOut=%#lx fFlags=%#lx\n",
    ...
    {
        OSDBGPRINT(("nemudrv: Invalid pSession value %p (ioctl=%p)\n", pSession, (void *)uIOCtl));
        NEMUDRV_IOCTL_RETURN(pSession, uIOCtl, pReqHdr, VERR_INVALID_PARAMETER, VINF_SUCCESS);
        return VERR_INVALID_PARAMETER;
    }
    ...
    {
        OSDBGPRINT(("nemudrv: bad cookie %#lx / %#lx.\n", (long)pReqHdr->u32Cookie, (long)pReqHdr->u32SessionCookie));
        NEMUDRV_IOCTL_RETURN(pSession, uIOCtl, pReqHdr, VERR_INVALID_PARAMETER, VINF_SUCCESS);
        return VERR_INVALID_PARAMETER;
    }
    ...
    NEMUDRV_IOCTL_RETURN(pSession, uIOCtl, pReqHdr, pReqHdr->rc, rc);
    return rc;
}
 ...
 * @param   pReqHdr     The request header.
 */
int NEMUCALL supdrvIDC(uintptr_t uReq, PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, PSUPDRVIDCREQHDR pReqHdr)
{
    /*
    ...
    switch (uReq)
    {
        case SUPDRV_IDC_REQ_CONNECT:
        {
                ...
                ||  (pReq->u.In.uMinVersion & 0xffff0000) != (SUPDRV_IDC_VERSION & 0xffff0000))
            {
                ...
                pReq->u.Out.uDriverRevision = NEMU_SVN_REV;
                pReq->Hdr.rc = VERR_VERSION_MISMATCH;
                return VINF_SUCCESS;
            }
...
SUPR0DECL(int) SUPR0PageAllocEx(PSUPDRVSESSION pSession, uint32_t cPages, uint32_t fFlags, PRTR3PTR ppvR3, PRTR0PTR ppvR0, PRTHCPHYS paPages)
{
    ...
    if (cPages < 1 || cPages > NEMU_MAX_ALLOC_PAGE_COUNT)
    {
        Log(("SUPR0PageAlloc: Illegal request cb=%u; must be greater than 0 and smaller than %uMB (NEMU_MAX_ALLOC_PAGE_COUNT pages).\n", cPages, NEMU_MAX_ALLOC_PAGE_COUNT * (_1M / _4K)));
        return VERR_PAGE_COUNT_OUT_OF_RANGE;
    }
...
void NEMUCALL supdrvBadContext(PSUPDRVDEVEXT pDevExt, const char *pszFile, uint32_t uLine, const char *pszExtra)
{
    ...
    if (RT_VALID_PTR(pszExtra) && *pszExtra)
        SUPR0Printf("nemudrv: Bad CPU context error at line %u in %s: %s\n", uLine, pszFile, pszExtra);
    else
        SUPR0Printf("nemudrv: Bad CPU context error at line %u in %s!\n", uLine, pszFile);
...
static void supdrvLdrFree(PSUPDRVDEVEXT pDevExt, PSUPDRVLDRIMAGE pImage)
{
    ...
    /* check if this is NemuVMMR0.r0 unset its entry point pointers. */
    if (pDevExt->pvVMMR0 == pImage->pvImage)
        supdrvLdrUnsetVMMR0EPs(pDevExt);
