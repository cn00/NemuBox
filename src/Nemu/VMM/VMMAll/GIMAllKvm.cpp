 ...
 *
 * @returns Strict Nemu status code like CPUMQueryGuestMsr().
 ...
 */
VMM_INT_DECL(NEMUSTRICTRC) gimKvmReadMsr(PVMCPU pVCpu, uint32_t idMsr, PCCPUMMSRRANGE pRange, uint64_t *puValue)
{
    NOREF(pRange);
    PVM     pVM        = pVCpu->CTX_SUFF(pVM);
 ...
 *
 * @returns Strict Nemu status code like CPUMSetGuestMsr().
 ...
 */
VMM_INT_DECL(NEMUSTRICTRC) gimKvmWriteMsr(PVMCPU pVCpu, uint32_t idMsr, PCCPUMMSRRANGE pRange, uint64_t uRawValue)
{
    NOREF(pRange);
    PVM     pVM  = pVCpu->CTX_SUFF(pVM);
    PGIMKVM pKvm = &pVM->gim.s.u.Kvm;
