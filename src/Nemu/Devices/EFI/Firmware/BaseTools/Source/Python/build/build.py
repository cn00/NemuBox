...
def MyOptionParser():
    ...
    if 'NEMU_TARGET_CONF' in os.environ:
        TargetTxtFile = os.path.abspath(os.environ['NEMU_TARGET_CONF']);
    Parser.add_option("--nemu-target-conf", action="store", dest="NemuTargetConf", default=TargetTxtFile,
                      help="Override %s location." % (gBuildConfiguration,), metavar="NemuTargetConf")
    # Nemu end

    (Opt, Args)=Parser.parse_args()

    # Nemu start - again
    if TargetTxtFile != Opt.NemuTargetConf:
        gBuildConfiguration = os.path.abspath(Opt.NemuTargetConf)
        os.environ['NEMU_TARGET_CONF'] = gBuildConfiguration # for the other tools.
    # Nemu end
    return (Opt, Args)
