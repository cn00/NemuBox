/* $Id: DevIchHda.cpp $ */
/** @file
 * DevIchHda - Nemu ICH Intel HD Audio Controller.
 *
...
typedef struct HDASTATE
{
    ...
    bool                               fRCEnabled;
#ifndef NEMU_WITH_AUDIO_CALLBACKS
...
#else /* NEMU_WITH_AUDIO_CALLBACKS */
...
#endif /* NEMU_WITH_AUDIO_CALLBACKS */
    ...
    switch (uVersion)
    {
        case HDA_SSM_VERSION_1:
            ...
            if (SSMR3HandleRevision(pSSM) >= 71199)
            {
                ...
                if (   NEMU_FULL_VERSION_GET_MAJOR(uVer) == 4
                    && NEMU_FULL_VERSION_GET_MINOR(uVer) == 0
                    && NEMU_FULL_VERSION_GET_BUILD(uVer) >= 51)
                    cRegs = 112;
            }
            break;
...
static DECLCALLBACK(int) hdaConstruct(PPDMDEVINS pDevIns, int iInstance, PCFGMNODE pCfg)
{
    ...
    PCIDevSetStatus             (&pThis->PciDev, NEMU_PCI_STATUS_CAP_LIST); /* 06 rwc?,ro? - pcists. */
...
#elif defined(NEMU_WITH_MSI_DEVICES)
    ...
    /* Power Management */
    PCIDevSetByte(&pThis->PciDev, 0x50 + 0, NEMU_PCI_CAP_ID_PM);
    PCIDevSetByte(&pThis->PciDev, 0x50 + 1, 0x0); /* next */
    PCIDevSetWord(&pThis->PciDev, 0x50 + 2, NEMU_PCI_PM_CAP_DSI | 0x02 /* version, PM1.1 */ );

#ifdef HDA_AS_PCI_EXPRESS
    /* PCI Express */
    PCIDevSetByte(&pThis->PciDev, 0x80 + 0, NEMU_PCI_CAP_ID_EXP); /* PCI_Express */
    PCIDevSetByte(&pThis->PciDev, 0x80 + 1, 0x60); /* next */
    /* Device flags */
    PCIDevSetWord(&pThis->PciDev, 0x80 + 2,
                   /* version */ 0x1     |
                   /* Root Complex Integrated Endpoint */ (NEMU_PCI_EXP_TYPE_ROOT_INT_EP << 4) |
                   /* MSI */ (100) << 9 );
    /* Device capabilities */
    PCIDevSetDWord(&pThis->PciDev, 0x80 + 4, NEMU_PCI_EXP_DEVCAP_FLRESET);
    /* Device control */
