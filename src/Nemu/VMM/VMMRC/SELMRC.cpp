 ...
 *
 * @returns Nemu strict status code (appropriate for trap handling and GC
 *          return).
 * @retval  VINF_SUCCESS
 ...
 */
static NEMUSTRICTRC selmRCSyncGDTEntry(PVM pVM, PVMCPU pVCpu, PCPUMCTX pCtx, unsigned iGDTEntry)
{
 ...
 * @param   pCtx            The current CPU context.
 */
NEMUSTRICTRC selmRCGuestGdtPostWriteCheck(PVM pVM, PVMCPU pVCpu, uint32_t offGuestGdt, uint32_t cbWrite, PCPUMCTX pCtx)
{
    NEMUSTRICTRC rcStrict = VINF_SUCCESS;
    ...
    else
    {
        ...
        do
        {
            NEMUSTRICTRC rcStrict2 = selmRCSyncGDTEntry(pVM, pVCpu, pCtx, iGdt);
 ...
 */
DECLEXPORT(NEMUSTRICTRC) selmRCGuestGDTWritePfHandler(PVM pVM, PVMCPU pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pRegFrame,
                                                      RTGCPTR pvFault, RTGCPTR pvRange, uintptr_t offRange, void *pvUser)
{
     ...
     */
    uint32_t cb;
    NEMUSTRICTRC rcStrict = EMInterpretInstructionEx(pVCpu, pRegFrame, (RTGCPTR)(RTRCUINTPTR)pvFault, &cb);
 ...
 */
DECLEXPORT(NEMUSTRICTRC) selmRCGuestLDTWritePfHandler(PVM pVM, PVMCPU pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pRegFrame,
                                                      RTGCPTR pvFault, RTGCPTR pvRange, uintptr_t offRange, void *pvUser)
{
 ...
 * @returns Nemu status code (appropriate for trap handling and GC return).
 * @param   pVM         The cross context VM structure.
 ...
 *
 * @returns Strict Nemu status code appropriate for raw-mode returns.
 * @param   pVM             The cross context VM structure.
 * @param   pVCpu           The cross context virtual CPU structure.
 * @param   offGuestTss     The offset into the TSS of the write that was made.
 * @param   cbWrite         The number of bytes written.
 */
NEMUSTRICTRC selmRCGuestTssPostWriteCheck(PVM pVM, PVMCPU pVCpu, uint32_t offGuestTss, uint32_t cbWrite)
{
    NEMUSTRICTRC rcStrict = VINF_SUCCESS;
...
/** @todo just read the darn fields and put them on the stack. */
    PCNEMUTSS pGuestTss = (PNEMUTSS)(uintptr_t)pVM->selm.s.GCPtrGuestTss;
...
# endif
    ...
    else if (   offGuestTss >= RT_UOFFSETOF(NEMUTSS, esp0)
             && offGuestTss < RT_UOFFSETOF(NEMUTSS, padding_ss0))
    {
        struct
        {
            uint32_t esp0;
     ...
     * needs updating.
     */
    if (   offGuestTss >= RT_UOFFSETOF(NEMUTSS, offIoBitmap)
        && (CPUMGetGuestCR4(pVCpu) & X86_CR4_VME))
    {
        if (offGuestTss - RT_UOFFSETOF(NEMUTSS, offIoBitmap) < sizeof(pGuestTss->offIoBitmap))
        {
 ...
 */
DECLEXPORT(NEMUSTRICTRC) selmRCGuestTSSWritePfHandler(PVM pVM, PVMCPU pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pRegFrame,
                                                      RTGCPTR pvFault, RTGCPTR pvRange, uintptr_t offRange, void *pvUser)
{
 ...
 * \#PF Virtual Handler callback for Guest write access to the Nemu shadow GDT.}
 */
DECLEXPORT(NEMUSTRICTRC) selmRCShadowGDTWritePfHandler(PVM pVM, PVMCPU pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pRegFrame,
                                                       RTGCPTR pvFault, RTGCPTR pvRange, uintptr_t offRange, void *pvUser)
{
...
/**
 * @callback_method_impl{FNPGMRCVIRTPFHANDLER,
 * \#PF Virtual Handler callback for Guest write access to the Nemu shadow LDT.}
 */
DECLEXPORT(NEMUSTRICTRC) selmRCShadowLDTWritePfHandler(PVM pVM, PVMCPU pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pRegFrame,
                                                       RTGCPTR pvFault, RTGCPTR pvRange, uintptr_t offRange, void *pvUser)
{
...
/**
 * @callback_method_impl{FNPGMRCVIRTPFHANDLER,
 * \#PF Virtual Handler callback for Guest write access to the Nemu shadow TSS.}
 */
DECLEXPORT(NEMUSTRICTRC) selmRCShadowTSSWritePfHandler(PVM pVM, PVMCPU pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pRegFrame,
                                                       RTGCPTR pvFault, RTGCPTR pvRange, uintptr_t offRange, void *pvUser)
{
