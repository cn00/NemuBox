...
#if HC_ARCH_BITS == 64 || defined (NEMU_WITH_64_BITS_GUESTS)
/* Enable 64 bits guest support. */
# define NEMU_ENABLE_64_BITS_GUESTS
#endif
...
#define HM_VTX_TSS_SIZE                 (sizeof(NEMUTSS) + 2 * PAGE_SIZE + 1)
...
#define NEMU_HM_WITH_GUEST_PATCHING
...
typedef struct HMGLOBALCPUINFO
{
    ...
    bool                fConfigured;
    /** Set if the NEMU_HWVIRTEX_IGNORE_SVM_IN_USE hack is active. */
...
typedef struct HM
{
    ...
    struct
    {
        ...
        R3PTRTYPE(PNEMUTSS)         pRealModeTSS;
