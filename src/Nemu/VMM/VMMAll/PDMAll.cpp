    ...
    if (VMCPU_FF_IS_SET(pVCpu, VMCPU_FF_INTERRUPT_APIC))
    {
        ...
        if (i >= 0)
        {
            ...
            *pu8Interrupt = (uint8_t)i;
            NEMUVMM_PDM_IRQ_GET(pVCpu, RT_LOWORD(uTagSrc), RT_HIWORD(uTagSrc), i);
            return VINF_SUCCESS;
        }
    }
...
VMMDECL(int) PDMIsaSetIrq(PVM pVM, uint8_t u8Irq, uint8_t u8Level, uint32_t uTagSrc)
{
    ...
    if (!uTagSrc && (u8Level & PDM_IRQ_LEVEL_HIGH)) /* FPU IRQ */
    {
        if (u8Level == PDM_IRQ_LEVEL_HIGH)
            NEMUVMM_PDM_IRQ_HIGH(VMMGetCpu(pVM), 0, 0);
        else
            NEMUVMM_PDM_IRQ_HILO(VMMGetCpu(pVM), 0, 0);
    }
    ...
    if (!uTagSrc && u8Level == PDM_IRQ_LEVEL_LOW)
        NEMUVMM_PDM_IRQ_LOW(VMMGetCpu(pVM), 0, 0);
    pdmUnlock(pVM);
    return rc;
}
 ...
 *
 * @returns Nemu true if an IO-APIC is present.
 * @param   pVM         The cross context VM structure.
 */
 ...
 *
 * @returns Nemu true if a Local APIC is present.
 * @param   pVM         The cross context VM structure.
 */
