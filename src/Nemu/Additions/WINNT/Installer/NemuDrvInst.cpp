/* $Id: NemuDrvInst.cpp $ */
/** @file
 * NemuDrvInst - Driver and service installation helper for Windows guests.
 */
...
#define NEMU_REG_STRINGLIST_NONE              0x00000000        /* No flags set. */
#define NEMU_REG_STRINGLIST_ALLOW_DUPLICATES  0x00000001        /* Allows duplicates in list when adding a value. */

#ifdef DEBUG
# define NEMU_DRVINST_LOGFILE                 "C:\\Temp\\NemuDrvInstDIFx.log"
#endif
 ...
 *                              Optional.
 */
int NemuInstallDriver(const BOOL fInstall, const _TCHAR *pszDriverPath, BOOL fSilent,
...
static UINT WINAPI nemuDrvInstExecuteInfFileCallback(PVOID Context,
                                                     UINT Notification,
                                                     UINT_PTR Param1,
                                                     UINT_PTR Param2)
{
...
int ExecuteInfFile(const _TCHAR *pszSection, int iMode, const _TCHAR *pszInf)
{
    ...
    if (hINF != INVALID_HANDLE_VALUE)
    {
        ...
        BOOL fSuccess = SetupInstallFromInfSection(NULL,
                                                    ...
                                                    nemuDrvInstExecuteInfFileCallback,
                                                    pvQueue,
                                                    NULL,
                                                    NULL
                                                    );
    ...
    if (lRet == ERROR_SUCCESS)
    {
        ...
        while (pszToken != NULL)
        {
            ...
            if (   !wcsicmp(pszToken, pszValueToAdd)
                && (dwFlags & NEMU_REG_STRINGLIST_ALLOW_DUPLICATES))
                fAddToList = TRUE;
...
int AddNetworkProvider(const TCHAR *pszProvider, unsigned int uiOrder)
{
    ...
    int rc = RegistryAddStringToList(_T("System\\CurrentControlSet\\Control\\NetworkProvider\\Order"),
                                     _T("ProviderOrder"),
                                     pszProvider, uiOrder, NEMU_REG_STRINGLIST_NONE /* No flags set */);
    if (rc == EXIT_OK)
...
void PrintHelp(void)
{
    ...
    _tprintf(_T("\n"));
    _tprintf(_T("Drivers:\n"));
    _tprintf(_T("\tNemuDrvInst         driver install <inf-file> [log file]\n"));
    _tprintf(_T("\tNemuDrvInst         driver uninstall <inf-file> [log file]\n"));
    _tprintf(_T("\tNemuDrvInst         driver executeinf <inf-file>\n"));
    _tprintf(_T("\n"));
    _tprintf(_T("Network Provider:\n"));
    _tprintf(_T("\tNemuDrvInst         netprovider add <name> [order]\n"));
    _tprintf(_T("\tNemuDrvInst         netprovider remove <name>\n"));
    _tprintf(_T("\n"));
    _tprintf(_T("Registry:\n"));
    _tprintf(_T("\tNemuDrvInst         registry write <root> <sub key>\n")
             _T("\t                    <key name> <key type> <value>\n")
             _T("\t                    [type] [size]\n"));
    _tprintf(_T("\tNemuDrvInst         registry addmultisz <root> <sub key>\n")
             _T("\t                    <value> [order]\n"));
    _tprintf(_T("\tNemuDrvInst         registry delmultisz <root> <sub key>\n")
            ...
            && argc >= 3)
        {
                ...
                && argc >= 4)
            {
                ...
                else
                {
                    ...
                    rc = NemuInstallDriver(!_tcsicmp(argv[2], _T("install")) ? TRUE : FALSE, szINF,
