 ...
 *
 *                              This is only needed to load NemuRT.dll when
...
/**
 * Translates Nemu status code (from supHardenedWinVerifyImageTrust) to an NT
 * status.
 *
 * @returns NT status.
...
#else  /* NEMU_PERMIT_EVEN_MORE */
    /*
     * Require trusted installer + some kind of signature on everything, except
     * for the Nemu bits where we require kernel code signing and special
     * integrity checks.
     */
    uint32_t fFlags = 0;
...
supR3HardenedMonitor_LdrLoadDll(PWSTR pwszSearchPath, PULONG pfFlags, PUNICODE_STRING pName, PHANDLE phMod)
{
    ...
    else
    {
        ...
        else
        {
             ...
             * a search on the API level, all Nemu calls will have full paths.
 ...
 * the parent does.
 *
 * This is a necessary evil to make NemuNetDhcp and NemuNetNat termination from
 ...
 *
 * @param   hNemuRT             The NemuRT.dll handle.  We use RTThreadCreate to
 *                              spawn the thread to avoid duplicating thread
 *                              creation and thread naming code from IPRT.
 */
DECLHIDDEN(void) supR3HardenedWinCreateParentWatcherThread(HMODULE hNemuRT)
{
    /*
     * Resolve runtime methods that we need.
     */
    PFNRTTHREADCREATE pfnRTThreadCreate = (PFNRTTHREADCREATE)GetProcAddress(hNemuRT, "RTThreadCreate");
...
/**
 * Tries to open NemuDrvErrorInfo and read extra error info from it.
 *
...
static void supR3HardenedWinOpenStubDevice(void)
{
    ...
    /*
     * Retry if we think driver might still be initializing (STATUS_NO_SUCH_DEVICE + \Drivers\NemuDrv).
     */
    ...
    for (iTry = 0;; iTry++)
    {
        ...
        if (iTry > 0 && supR3HardenedWinGetMilliTS() - uMsTsStart > 5000)  /* 5 sec, at least two tries */
            break;
        if (!supR3HardenedWinDriverExists("NemuDrv"))
        {
            /** @todo Consider starting the Nemudrv.sys service. Requires 2nd process
    ...
    else
    {
         ...
         * extra information that goes into NemuStartup.log so that we stand a
         * better chance resolving the issue.
         */
        char szErrorInfo[16384];
        int rc = VERR_OPEN_FAILED;
        if (SUP_NT_STATUS_IS_NEMU(rcNt)) /* See NemuDrvNtErr2NtStatus. */
        {
            rc = SUP_NT_STATUS_TO_NEMU(rcNt);
            ...
            {
                SUP_DPRINTF(("Error opening NemuDrvStub: VERR_SUPDRV_APIPORT_OPEN_ERROR\n"));
                ...
                supR3HardenedFatalMsg("supR3HardenedWinReSpawn", kSupInitOp_Misc, rc,
                                      ...
                                      "\n"
                                      "Error getting %s\\ApiPort in the driver from nemudrv.\n"
                                      "\n"
                                      "Could be due to security software is redirecting access to it, so please include full "
                                      "details of such software in a bug report. NemuStartup.log may contain details important "
...
/** @todo don't fail during early init, wait till later and try load the driver if missing or at least query the service manager for additional information. */
            if (   rcNt == STATUS_NO_SUCH_DEVICE
                || rcNt == STATUS_OBJECT_NAME_NOT_FOUND)
            {
                SUP_DPRINTF(("Error opening NemuDrvStub: %s\n", pszDefine));
                if (supR3HardenedWinDriverExists("NemuDrv"))
                    supR3HardenedFatalMsg("supR3HardenedWinReSpawn", kSupInitOp_Driver, VERR_OPEN_FAILED,
                                          "NtCreateFile(%ls) failed: %#x%s (%u retries)\n"
                                          "\n"
                                          "Driver is probably stuck stopping/starting. Try 'sc.exe query nemudrv' to get more "
                    ...
                    supR3HardenedFatalMsg("supR3HardenedWinReSpawn", kSupInitOp_Driver, VERR_OPEN_FAILED,
                                          "NtCreateFile(%ls) failed: %#x%s (%u retries)\n"
                                          "\n"
                                          "Driver is does not appear to be loaded. Try 'sc.exe start nemudrv', reinstall "
...
DECLHIDDEN(int) supR3HardenedWinReSpawn(int iWhich)
{
     ...
     * support driver via /Devices/NemuDrvStub.  (We tried to do this
     * during the early init, but in case we had trouble accessing nemudrv we
...
DECLHIDDEN(void) supR3HardenedWinModifyDllSearchPath(uint32_t fFlags, const char *pszAppBinPath)
{
     ...
     * able to find the Nemu DLLs.  This is done _after_ NemuRT.dll is
 ...
 * software that is up to no good.  Once we've opened nemudrv, the process
...
DECLASM(uintptr_t) supR3HardenedEarlyProcessInit(void)
{
    ...
    {
        SUP_DPRINTF(("supR3HardenedVmProcessInit: Opening nemudrv stub...\n"));
        supR3HardenedWinOpenStubDevice();
    }
    else if (cArgs >= 1 && suplibHardenedStrCmp(papszArgs[0], SUPR3_RESPAWN_2_ARG0) == 0)
    {
        SUP_DPRINTF(("supR3HardenedVmProcessInit: Opening nemudrv...\n"));
