                                                ...
                                                uint32_t cbData)
{
     ...
     */
    if (cbData != sizeof(NEMUGUESTCTRLHOSTCALLBACK))
        return VERR_NOT_SUPPORTED;
    PNEMUGUESTCTRLHOSTCALLBACK pSvcCb = (PNEMUGUESTCTRLHOSTCALLBACK)pvData;
    AssertPtr(pSvcCb);
    ...
    LogFlowFunc(("CID=%RU32, uSession=%RU32, uObject=%RU32, uCount=%RU32\n",
                 uContextID,
                 NEMU_GUESTCTRL_CONTEXTID_GET_SESSION(uContextID),
                 NEMU_GUESTCTRL_CONTEXTID_GET_OBJECT(uContextID),
                 NEMU_GUESTCTRL_CONTEXTID_GET_COUNT(uContextID)));
#endif

    NEMUGUESTCTRLHOSTCBCTX ctxCb = { u32Function, uContextID };
...
int Guest::i_dispatchToSession(PNEMUGUESTCTRLHOSTCBCTX pCtxCb, PNEMUGUESTCTRLHOSTCALLBACK pSvcCb)
{
    ...
    uint32_t uSessionID = NEMU_GUESTCTRL_CONTEXTID_GET_SESSION(pCtxCb->uContextID);
#ifdef DEBUG
                           ...
                           const GuestCredentials &guestCreds, ComObjPtr<GuestSession> &pGuestSession)
{
    ...
    if (mData.mGuestSessions.size() >= NEMU_GUESTCTRL_MAX_SESSIONS)
        return rc;

    try
    {
        /* Create a new session ID and assign it. */
        uint32_t uNewSessionID = NEMU_GUESTCTRL_SESSION_ID_BASE;
        uint32_t uTries = 0;
        ...
        for (;;)
        {
            ...
            uNewSessionID++;
            if (uNewSessionID >= NEMU_GUESTCTRL_MAX_SESSIONS)
                uNewSessionID = NEMU_GUESTCTRL_SESSION_ID_BASE;

            if (++uTries == NEMU_GUESTCTRL_MAX_SESSIONS)
    ...
    if (RT_FAILURE(rc))
    {
        switch (rc)
        {
            case VERR_MAX_PROCS_REACHED:
                hr = setError(NEMU_E_IPRT_ERROR, tr("Maximum number of concurrent guest sessions (%ld) reached"),
                              NEMU_GUESTCTRL_MAX_SESSIONS);
                break;

            /** @todo Add more errors here. */

            default:
                hr = setError(NEMU_E_IPRT_ERROR, tr("Could not create guest session: %Rrc"), rc);
                break;
        }
    }
                                    ...
                                    const std::vector<AdditionsUpdateFlag_T> &aFlags, ComPtr<IProgress> &aProgress)
{
    ...
    else
    {
        ...
        if (RT_FAILURE(rc))
        {
            ...
            hr = setError(NEMU_E_IPRT_ERROR, tr("Could not open guest session: %Rrc"), rc);
        }
        else
        {
            try
