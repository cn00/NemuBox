...
/** @file
 * Nemu Qt GUI - UIMachineLogicFullscreen class implementation.
 */
...
Qt::WindowFlags UIMachineLogicFullscreen::windowFlags(ulong uScreenId) const
{
    Q_UNUSED(uScreenId);
#ifdef Q_WS_MAC
    return nemuGlobal().osRelease() <= MacOSXRelease_Lion ? Qt::FramelessWindowHint :
...
void UIMachineLogicFullscreen::revalidateNativeFullScreen(UIMachineWindow *pMachineWindow)
{
    ...
    else
    {
        ...
        else
        {
            ...
            const int iCurrentHostScreenIndex = nemuGlobal().screenNumber(pMachineWindow);
            const QSize frameBufferSize((int)uisession()->frameBuffer(uScreenID)->width(), (int)uisession()->frameBuffer(uScreenID)->height());
            const QSize screenSize = nemuGlobal().screenGeometry(iWantedHostScreenIndex).size();
