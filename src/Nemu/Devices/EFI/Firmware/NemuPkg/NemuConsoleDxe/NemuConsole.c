/* $Id: NemuConsole.c $ */
/** @file
 * NemuConsole.c - Helper driver waiting for Ready to Boot event to switch graphic mode into user-defined one.
 */
...
#include "NemuConsole.h"
#include "NemuPkg.h"
#include "DevEFI.h"
#include "iprt/asm.h"

/* @todo understand the reasons why TextOutputProtocol.SetMode isn't enough to switch mode. */
#define NEMU_CONSOLE_VAR L"NEMU_CONSOLE_VAR"
/*b53865fd-b76c-4433-9e85-c0cadf65aab8*/
static EFI_GUID gNemuConsoleVarGuid = { 0xb53865fd, 0xb76c, 0x4433, { 0x9e, 0x85, 0xc0, 0xca, 0xdf, 0x65, 0xaa, 0xb8}};
...
EFI_STATUS
EFIAPI
NemuConsoleInit(EFI_HANDLE hImage, EFI_SYSTEM_TABLE *pSysTable)
{
    EFI_STATUS r;
    UINT32 val;
    EFI_EVENT event;
    ...
    r = gRT->GetVariable(NEMU_CONSOLE_VAR, &gNemuConsoleVarGuid, NULL, &size, &val);
    if (   EFI_ERROR(r)
        && r == EFI_NOT_FOUND)
    {
        size = sizeof(UINT32);
        val = 1;
        r = gRT->SetVariable(NEMU_CONSOLE_VAR, &gNemuConsoleVarGuid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS, size, &val);
        if (EFI_ERROR(r))
        {
            DEBUG((DEBUG_INFO, "%a:%d - %r\n", __FILE__,  __LINE__, r));
            return r;
...
EFI_STATUS
EFIAPI
NemuConsoleFini(EFI_HANDLE hImage)
{
    return EFI_SUCCESS;
}
