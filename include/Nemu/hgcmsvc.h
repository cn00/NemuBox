...
#ifndef ___Nemu_hgcm_h
#define ___Nemu_hgcm_h
 ...
 *
 * Includes layout of both NEMUHGCMSVCFNTABLE and NEMUHGCMSVCHELPERS.
 *
 ...
 * 4.1->4.2 Because the NEMU_HGCM_SVC_PARM_CALLBACK parameter type was added
 * 4.2->5.1 Removed the NEMU_HGCM_SVC_PARM_CALLBACK parameter type, as
 *          this problem is already solved by service extension callbacks
 */
#define NEMU_HGCM_SVC_VERSION_MAJOR (0x0005)
#define NEMU_HGCM_SVC_VERSION_MINOR (0x0001)
#define NEMU_HGCM_SVC_VERSION ((NEMU_HGCM_SVC_VERSION_MAJOR << 16) + NEMU_HGCM_SVC_VERSION_MINOR)


/** Typed pointer to distinguish a call to service. */
struct NEMUHGCMCALLHANDLE_TYPEDEF;
typedef struct NEMUHGCMCALLHANDLE_TYPEDEF *NEMUHGCMCALLHANDLE;

/** Service helpers pointers table. */
typedef struct NEMUHGCMSVCHELPERS
{
    /** The service has processed the Call request. */
    DECLR3CALLBACKMEMBER(void, pfnCallComplete, (NEMUHGCMCALLHANDLE callHandle, int32_t rc));
...
typedef NEMUHGCMSVCHELPERS *PNEMUHGCMSVCHELPERS;


#define NEMU_HGCM_SVC_PARM_INVALID (0U)
#define NEMU_HGCM_SVC_PARM_32BIT (1U)
#define NEMU_HGCM_SVC_PARM_64BIT (2U)
#define NEMU_HGCM_SVC_PARM_PTR   (3U)

typedef struct NEMUHGCMSVCPARM
{
    /** NEMU_HGCM_SVC_PARM_* values. */
    uint32_t type;
    ...
    int getUInt32(uint32_t *u32)
    {
        ...
        int rc = VINF_SUCCESS;
        if (type != NEMU_HGCM_SVC_PARM_32BIT)
    ...
    int getUInt64(uint64_t *u64)
    {
        ...
        int rc = VINF_SUCCESS;
        if (type != NEMU_HGCM_SVC_PARM_64BIT)
            rc = VERR_INVALID_PARAMETER;
        if (RT_SUCCESS(rc))
            *u64 = u.uint64;
        return rc;
    ...
    int getPointer(void **ppv, uint32_t *pcb)
    {
        ...
        if (type == NEMU_HGCM_SVC_PARM_PTR)
        {
            *ppv = u.pointer.addr;
            *pcb = u.pointer.size;
            return VINF_SUCCESS;
                         ...
                         uint32_t acb, int rcExp)
    {
        ...
        type = aType;  /* in case we don't want NEMU_HGCM_SVC_PARM_PTR */
        const char *pcc = NULL;
        uint32_t cb = 0;
    ...
    void testGetString(RTTEST hTest)
    {
        ...
        doTestGetString(hTest, NEMU_HGCM_SVC_PARM_32BIT, "test", 3,
                        VERR_INVALID_PARAMETER);
        doTestGetString(hTest, NEMU_HGCM_SVC_PARM_PTR, "test", 5,
                        VINF_SUCCESS);
        doTestGetString(hTest, NEMU_HGCM_SVC_PARM_PTR, "test", 3,
                        VERR_BUFFER_OVERFLOW);
        doTestGetString(hTest, NEMU_HGCM_SVC_PARM_PTR, "test\xf0", 6,
                        VERR_INVALID_UTF8_ENCODING);
        doTestGetString(hTest, NEMU_HGCM_SVC_PARM_PTR, "test", 0,
                        VERR_INVALID_PARAMETER);
        doTestGetString(hTest, NEMU_HGCM_SVC_PARM_PTR, (const char *)0x1, 5,
                        VERR_INVALID_PARAMETER);
        RTTestSubDone(hTest);
    }
#endif

    NEMUHGCMSVCPARM() : type(NEMU_HGCM_SVC_PARM_INVALID) {}
#endif
} NEMUHGCMSVCPARM;

typedef NEMUHGCMSVCPARM *PNEMUHGCMSVCPARM;

#ifdef NEMU_WITH_CRHGSMI
typedef void * HGCMCVSHANDLE;

typedef DECLCALLBACK(void) HGCMHOSTFASTCALLCB (int32_t result, uint32_t u32Function, PNEMUHGCMSVCPARM pParam, void *pvContext);
 ...
 *
 *  HGCM will call the DLL "NemuHGCMSvcLoad"
 *  function and the DLL must fill in the NEMUHGCMSVCFNTABLE
...
typedef struct NEMUHGCMSVCFNTABLE
{
    /** @name Filled by HGCM
     * @{ */
     ...
     */
    DECLR3CALLBACKMEMBER(void, pfnCall, (void *pvService, NEMUHGCMCALLHANDLE callHandle, uint32_t u32ClientID, void *pvClient, uint32_t function, uint32_t cParms, NEMUHGCMSVCPARM paParms[]));

    /** Host Service entry point meant for privileged features invisible to the guest.
     *  Return code is passed to pfnCallComplete callback.
     */
    DECLR3CALLBACKMEMBER(int, pfnHostCall, (void *pvService, uint32_t function, uint32_t cParms, NEMUHGCMSVCPARM paParms[]));
...
typedef DECLCALLBACK(int) NEMUHGCMSVCLOAD(NEMUHGCMSVCFNTABLE *ptable);
typedef NEMUHGCMSVCLOAD *PFNNEMUHGCMSVCLOAD;
#define NEMU_HGCM_SVCLOAD_NAME "NemuHGCMSvcLoad"
