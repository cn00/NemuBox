...
/** @file
 * Nemu Qt GUI - UIGlobalSettingsExtension class implementation.
 */
                                               ...
                                               QWidget *pParent, QString *pstrExtPackName)
{
    ...
    if (extPackFile.GetShowLicense())
    {
        ...
        NemuLicenseViewer licenseViewer(pParent);
...
void UIGlobalSettingsExtension::loadToCacheFrom(QVariant &data)
{
    ...
    /* Load to cache: */
    const CExtPackManager &manager = nemuGlobal().virtualBox().GetExtensionPackManager();
...
void UIGlobalSettingsExtension::sltInstallPackage()
{
    ...
    for (int i = 0; i < NemuExtPackFileExts.size(); ++i)
        extensions << QString("*.%1").arg(NemuExtPackFileExts[i]);
