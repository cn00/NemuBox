...
class tdAddBasic1(nemu.TestDriver):                                         # pylint: disable=R0902
    """
    Additions Basics #1.
    """
    ## @todo
    ...
    def testWindowsInstallAdditions(self, oSession, oTxsSession, oTestVm):
        ...
        #
        # The actual install.
        # Enable installing the optional auto-logon modules (NemuGINA/NemuCredProv) + (Direct)3D support.
        # Also tell the installer to produce the appropriate log files.
        #
        fRc = self.txsRunTest(oTxsSession, 'NemuWindowsAdditions.exe', 5 * 60 * 1000, \
            '${CDROM}/NemuWindowsAdditions.exe', ('${CDROM}/NemuWindowsAdditions.exe', '/S', '/l', '/with_autologon'));
    ...
    def testIGuest_additionsRunLevel(self, oGuest, oTestVm):
        ...
        if oTestVm.isLoggedOntoDesktop():
            eExpectedRunLevel = nemucon.AdditionsRunLevelType_Desktop;
        else:
            eExpectedRunLevel = nemucon.AdditionsRunLevelType_Userland;
