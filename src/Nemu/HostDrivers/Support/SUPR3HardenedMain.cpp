...
*   Global Variables                                                                                                             *
*********************************************************************************************************************************/
/** The pre-init data we pass on to SUPR3 (residing in NemuRT). */
...
#ifdef RT_OS_WINDOWS
/** Pointer to NemuRT's RTLogRelPrintf function so we can write errors to the
        ...
        if (strncmp(papszArgs[iArg], s_szLogOption, sizeof(s_szLogOption) - 1) == 0)
        {
            ...
            *pcArgs -= 1;
            ...
            if (g_hStartupLog == NULL)
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    SUP_DPRINTF(("Log file opened: " NEMU_VERSION_STRING "r%u g_hStartupLog=%p g_uNtVerCombined=%#x\n",
                                 NEMU_SVN_REV, g_hStartupLog, g_uNtVerCombined));
...
/**
 * Attempts to open /dev/nemudrv (or equvivalent).
 *
...
# ifdef RT_OS_LINUX
    ...
    if (getuid() != 0)
    {
         ...
         * CAP_NET_RAW.
         * Default: enabled.
         * Can be disabled with 'export NEMU_HARD_CAP_NET_RAW=0'.
         */
        pszOpt = getenv("NEMU_HARD_CAP_NET_RAW");
        if (   !pszOpt
         ...
         * Default: disabled.
         * Can be enabled with 'export NEMU_HARD_CAP_NET_BIND_SERVICE=1'.
         */
        pszOpt = getenv("NEMU_HARD_CAP_NET_BIND_SERVICE");
        if (   pszOpt
            && memcmp(pszOpt, "0", sizeof("0")) != 0)
...
/**
 * Loads the NemuRT DLL/SO/DYLIB, hands it the open driver,
 * and calls RTR3InitEx.
 *
 * @param   fFlags      The SUPR3HardenedMain fFlags argument, passed to supR3PreInit.
 *
 * @remarks NemuRT contains both IPRT and SUPR3.
 * @remarks This function will not return on failure.
 */
static void supR3HardenedMainInitRuntime(uint32_t fFlags)
{
     ...
     */
    char szPath[RTPATH_MAX];
    supR3HardenedPathAppSharedLibs(szPath, sizeof(szPath) - sizeof("/NemuRT" SUPLIB_DLL_SUFF));
    suplibHardenedStrCat(szPath, "/NemuRT" SUPLIB_DLL_SUFF);
     ...
     *          process terminates (NemuNetDHCP, Ctrl-C, etc).
     */
 ...
 * and when using the open-nemudrv-via-root-service setup on Windows.
 *
...
DECLHIDDEN(int) SUPR3HardenedMain(const char *pszProgName, uint32_t fFlags, int argc, char **argv, char **envp)
{
    ...
    if (!(fFlags & SUPSECMAIN_FLAGS_DONT_OPEN_DEV))
    {
         ...
         * with nemudrv to monitor access to the new process from its inception.
         */
        ...
        SUP_DPRINTF(("SUPR3HardenedMain: Final process, opening NemuDrv...\n"));
        supR3HardenedWinFlushLoaderCache();

#else
        /*
         * Open the nemudrv device.
         */
