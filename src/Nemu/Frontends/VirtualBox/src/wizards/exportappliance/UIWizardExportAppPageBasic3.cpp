...
/** @file
 * Nemu Qt GUI - UIWizardExportAppPageBasic3 class implementation.
 */
...
void UIWizardExportAppPage3::refreshCurrentSettings()
{
    ...
    if (storageType == Filesystem)
        strName = QDir::toNativeSeparators(QString("%1/%2").arg(nemuGlobal().documentsPath()).arg(strName));
...
UIWizardExportAppPageBasic3::UIWizardExportAppPageBasic3()
{
    ...
    QVBoxLayout *pMainLayout = new QVBoxLayout(this);
    {
        ...
        QGridLayout *pSettingsLayout = new QGridLayout;
        {
            ...
            m_pFileSelector = new NemuEmptyFileSelector(this);
            {
                m_pFileSelector->setMode(NemuFilePathSelectorWidget::Mode_File_Save);
                m_pFileSelector->setEditable(true);
                m_pFileSelector->setButtonPosition(NemuEmptyFileSelector::RightPosition);
...
bool UIWizardExportAppPageBasic3::isComplete() const
{
    ...
    if (fResult)
    {
        ...
        fResult = NemuGlobal::hasAllowedExtension(strFile, OVFFileExts);
        if (fResult)
        {
