...
public class TestNemu
{
    ...
    static void testEvents(VirtualBoxManager mgr, IEventSource es)
    {
        ...
        es.registerListener(listener, Arrays.asList(NemuEventType.Any), false);
    ...
    static void testEnumeration(VirtualBoxManager mgr, IVirtualBox nemu)
    {
        List<IMachine> machs = nemu.getMachines();
        for (IMachine m : machs)
        {
            ...
            try
            {
                ...
                IGuestOSType foo = nemu.getGuestOSType(osType);
            }
            catch (NemuException e)
            {
                name = "<inaccessible>";
                inaccessible = true;
            }
    ...
    static void testStart(VirtualBoxManager mgr, IVirtualBox nemu)
    {
        IMachine m = nemu.getMachines().get(0);
    ...
    static void testMultiServer()
    {
        ...
        try {
            ...
            IMachine m1 = mgr1.getNemu().getMachines().get(0);
            IMachine m2 = mgr2.getNemu().getMachines().get(0);
    ...
    static void testReadLog(VirtualBoxManager mgr, IVirtualBox nemu)
    {
        IMachine m =  nemu.getMachines().get(0);
        long logNo = 0;
        long off = 0;
    ...
    static void printErrorInfo(NemuException e)
    {
        System.out.println("Nemu error: " + e.getMessage());
    ...
    public static void main(String[] args)
    {
        ...
        try
        {
            IVirtualBox nemu = mgr.getNemu();
            if (nemu != null)
            {
                System.out.println("VirtualBox version: " + nemu.getVersion() + "\n");
                testEnumeration(mgr, nemu);
                testReadLog(mgr, nemu);
                testStart(mgr, nemu);
                testEvents(mgr, nemu.getEventSource());
