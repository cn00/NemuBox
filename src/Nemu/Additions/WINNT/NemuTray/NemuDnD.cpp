/* $Id: NemuDnD.cpp $ */
/** @file
 * NemuDnD.cpp - Windows-specific bits of the drag and drop service.
 */
...
/** @todo Merge this with messages from NemuTray.h. */
#define WM_NEMUTRAY_DND_MESSAGE       WM_APP + 401
...
static LRESULT CALLBACK nemuDnDWndProcInstance(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
static LRESULT CALLBACK nemuDnDWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);

NemuDnDWnd::NemuDnDWnd(void)
      ...
      mState(Uninitialized)
{
    ...
    const RTCString arrEntries[] = { NEMU_DND_FORMATS_DEFAULT };
 ...
 */
int NemuDnDWnd::Initialize(PNEMUDNDCONTEXT pContext)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        rc = RTThreadCreate(&hThread, NemuDnDWnd::Thread, this,
 ...
 * @param   pvUser                  Pointer to NemuDnDWnd instance which
 *                                  is using the thread.
 */
/* static */
int NemuDnDWnd::Thread(RTTHREAD hThread, void *pvUser)
{
    AssertPtrReturn(pvUser, VERR_INVALID_POINTER);

    NemuDnDWnd *pThis = (NemuDnDWnd*)pvUser;
    AssertPtr(pThis);

    PNEMUDNDCONTEXT pContext = pThis->pContext;
    AssertPtr(pContext);
    ...
    wndClass.lpfnWndProc   = nemuDnDWndProc;
    wndClass.lpszClassName = "NemuTrayDnDWnd";
 ...
 */
/* static */
BOOL CALLBACK NemuDnDWnd::MonitorEnumProc(HMONITOR hMonitor, HDC hdcMonitor,
 ...
 */
LRESULT CALLBACK NemuDnDWnd::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg)
    {
        case WM_CREATE:
        ...
        case WM_MOUSEMOVE:
        {
            ...
            {
                /* Starting here NemuDnDDropTarget should
        ...
        case WM_NEMUTRAY_DND_MESSAGE:
        {
            NEMUDNDEVENT *pEvent = (PNEMUDNDEVENT)lParam;
            if (!pEvent)
 ...
 * @return  IPRT status code.
 */
int NemuDnDWnd::RegisterAsDropTarget(void)
{
    ...
    try
    {
        pDropTarget = new NemuDnDDropTarget(this /* pParent */);
 ...
 * @return  IPRT status code.
 */
int NemuDnDWnd::UnregisterAsDropTarget(void)
{
    LogFlowFuncEnter();
 ...
 */
void NemuDnDWnd::OnDestroy(void)
{
 ...
 */
int NemuDnDWnd::OnHgEnter(const RTCList<RTCString> &lstFormats, uint32_t uAllActions)
{
    ...
    try
    {
        ...
        startupInfo.pDropSource = new NemuDnDDropSource(this);
        startupInfo.pDataObject = new NemuDnDDataObject(pFormatEtc, pStgMeds, cFormatsActive);
 ...
 *                                  Currently ignored.
 */
int NemuDnDWnd::OnHgMove(uint32_t u32xPos, uint32_t u32yPos, uint32_t uAction)
{
    int rc;
        ...
        if (lstFmtActive.size() >= 1)
        {
            ...
            if (RT_SUCCESS(rc))
            {
                if (startupInfo.pDataObject)
                    startupInfo.pDataObject->SetStatus(NemuDnDDataObject::Dropping);
                else
                    rc = VERR_NOT_FOUND;
 ...
 */
int NemuDnDWnd::OnHgDataReceived(const void *pvData, uint32_t cbData)
{
 ...
 * @return  IPRT status code.
 */
int NemuDnDWnd::OnHgCancel(void)
{
 ...
 */
int NemuDnDWnd::OnGhIsDnDPending(uint32_t uScreenID)
{
 ...
 */
int NemuDnDWnd::OnGhDropped(const char *pszFormat, uint32_t cbFormat,
                            uint32_t uDefAction)
{
 ...
 */
int NemuDnDWnd::ProcessEvent(PNEMUDNDEVENT pEvent)
{
    AssertPtrReturn(pEvent, VERR_INVALID_POINTER);

    BOOL fRc = PostMessage(hWnd, WM_NEMUTRAY_DND_MESSAGE,
 ...
 * @return  IPRT status code.
 */
int NemuDnDWnd::makeFullscreen(void)
{
    int rc = VINF_SUCCESS;
    ...
    if (hDC)
    {
        fRc = s_pfnEnumDisplayMonitors
            ...
            ? s_pfnEnumDisplayMonitors(hDC, NULL, NemuDnDWnd::MonitorEnumProc, (LPARAM)&r):
              FALSE;
 ...
 */
int NemuDnDWnd::mouseMove(int x, int y, DWORD dwMouseInputFlags)
{
    int iScreenX = GetSystemMetrics(SM_CXSCREEN) - 1;
    int iScreenY = GetSystemMetrics(SM_CYSCREEN) - 1;
 ...
 * @return  IPRT status code.
 */
int NemuDnDWnd::mouseRelease(void)
{
#ifdef DEBUG_andy
    LogFlowFunc(("\n"));
#endif
 ...
 */
int NemuDnDWnd::setMode(Mode enmMode)
{
 ...
 * window instances.
 */
static LRESULT CALLBACK nemuDnDWndProcInstance(HWND hWnd, UINT uMsg,
                                               WPARAM wParam, LPARAM lParam)
{
    LONG_PTR pUserData = GetWindowLongPtr(hWnd, GWLP_USERDATA);
    AssertPtrReturn(pUserData, 0);

    NemuDnDWnd *pWnd = reinterpret_cast<NemuDnDWnd *>(pUserData);
    if (pWnd)
 ...
 */
static LRESULT CALLBACK nemuDnDWndProc(HWND hWnd, UINT uMsg,
                                       WPARAM wParam, LPARAM lParam)
{
    ...
    if (uMsg == WM_NCCREATE)
    {
        ...
        SetWindowLongPtr(hWnd, GWLP_WNDPROC, (LONG_PTR)nemuDnDWndProcInstance);

        return nemuDnDWndProcInstance(hWnd, uMsg, wParam, lParam);
    }
 ...
 */
int NemuDnDInit(const NEMUSERVICEENV *pEnv, void **ppInstance, bool *pfStartThread)
{
...
void NemuDnDStop(const NEMUSERVICEENV *pEnv, void *pInstance)
{
    ...
    PNEMUDNDCONTEXT pCtx = (PNEMUDNDCONTEXT)pInstance;
    AssertPtr(pCtx);
...
void NemuDnDDestroy(const NEMUSERVICEENV *pEnv, void *pInstance)
{
    AssertPtr(pEnv);
    AssertPtr(pInstance);
    ...
    NemuDnDWnd *pWnd = pCtx->lstWnd.first();
    if (pWnd)
        delete pWnd;
...
unsigned __stdcall NemuDnDThread(void *pInstance)
{
    ...
    do
    {
        PNEMUDNDEVENT pEvent = (PNEMUDNDEVENT)RTMemAlloc(sizeof(NEMUDNDEVENT));
        if (!pEvent)
        {
            rc = VERR_NO_MEMORY;
            break;
