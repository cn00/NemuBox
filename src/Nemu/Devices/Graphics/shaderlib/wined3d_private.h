...
struct NEMUVMSVGASHADERIF;
extern BOOL IWineD3DImpl_FillGLCaps(struct wined3d_adapter *adapter, struct NEMUVMSVGASHADERIF *pNemuShaderIf);
...
#ifdef NEMU_WITH_WDDM
HRESULT WINAPI IWineD3DResourceImpl_SetShRcState(IWineD3DResource *iface, NEMUWINEEX_SHRC_STATE enmState);
#endif
...
#define texture_gl_delete(_o, _t) do { \
        if (NEMUSHRC_IS_SHARED(_o)) GL_EXTCALL(glChromiumParameteriCR(GL_RCUSAGE_TEXTURE_CLEAR_CR, _t)); \
 ...
 */
#ifdef NEMU_WITH_VMSVGA
# define NEMU_CHECK_GL_CALL(a_Expr) \
        do { \
            GLint rcCheckCall; \
            a_Expr; \
...
#else
# define NEMU_CHECK_GL_CALL(a_Expr) do { a_Expr; } while (0)
#endif
