/* $Id: NemuPci-linux.c $ */
/** @file
 * NemuPci - PCI Driver (Host), Linux Specific Code.
 */
...
# endif
#endif /* NEMU_WITH_IOMMU */


/*********************************************************************************************************************************
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static int  NemuPciLinuxInit(void);
static void NemuPciLinuxUnload(void);


/*********************************************************************************************************************************
*   Global Variables                                                                                                             *
*********************************************************************************************************************************/
static NEMURAWPCIGLOBALS g_NemuPciGlobals;

module_init(NemuPciLinuxInit);
module_exit(NemuPciLinuxUnload);

MODULE_AUTHOR(NEMU_VENDOR);
MODULE_DESCRIPTION(NEMU_PRODUCT " PCI access Driver");
MODULE_LICENSE("GPL");
#ifdef MODULE_VERSION
MODULE_VERSION(NEMU_VERSION_STRING);
#endif
 ...
 */
static int __init NemuPciLinuxInit(void)
{
    int rc;
    /*
     * Initialize IPRT.
    ...
    LogRel(("NemuPciLinuxInit\n"));

    RT_ZERO(g_NemuPciGlobals);

    rc = nemuPciInit(&g_NemuPciGlobals);
    if (RT_FAILURE(rc))
    {
        LogRel(("cannot do NemuPciInit: %Rc\n", rc));
        goto error;
    }

#if defined(CONFIG_PCI_STUB)
    /* nothing to do, pci_stub module part of the kernel */
    g_NemuPciGlobals.fPciStubModuleAvail = true;
...
# if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 30)
        ...
        g_NemuPciGlobals.pciStubModule = find_module(PCI_STUB_MODULE_NAME);
        mutex_unlock(&module_mutex);
        if (g_NemuPciGlobals.pciStubModule)
        {
            if (try_module_get(g_NemuPciGlobals.pciStubModule))
                g_NemuPciGlobals.fPciStubModuleAvail = true;
        }
        else
            printk(KERN_INFO "nemupci: find_module %s failed\n", PCI_STUB_MODULE);
# endif
    }
    else
        printk(KERN_INFO "nemupci: cannot load %s\n", PCI_STUB_MODULE);

#else
    printk(KERN_INFO "nemupci: %s module not available, cannot detach PCI devices\n",
                      PCI_STUB_MODULE);
#endif

#ifdef NEMU_WITH_IOMMU
    if (IOMMU_PRESENT())
        printk(KERN_INFO "nemupci: IOMMU found\n");
    else
        printk(KERN_INFO "nemupci: IOMMU not found (not registered)\n");
#else
    printk(KERN_INFO "nemupci: IOMMU not found (not compiled)\n");
#endif
 ...
 * Unload the module.
 */
static void __exit NemuPciLinuxUnload(void)
{
    LogRel(("NemuPciLinuxLinuxUnload\n"));

    /*
     * Undo the work done during start (in reverse order).
     */
    nemuPciShutdown(&g_NemuPciGlobals);

    RTR0Term();

    if (g_NemuPciGlobals.pciStubModule)
    {
        module_put(g_NemuPciGlobals.pciStubModule);
        g_NemuPciGlobals.pciStubModule = NULL;
    }

    Log(("NemuPciLinuxUnload - done\n"));
}

static int nemuPciLinuxDevRegisterWithIommu(PNEMURAWPCIINS pIns)
{
#ifdef NEMU_WITH_IOMMU
    int rc = VINF_SUCCESS;
    struct pci_dev *pPciDev = pIns->pPciDev;
    PNEMURAWPCIDRVVM pData = NEMU_DRV_VMDATA(pIns);
...
static int nemuPciLinuxDevUnregisterWithIommu(PNEMURAWPCIINS pIns)
{
#ifdef NEMU_WITH_IOMMU
    int rc = VINF_SUCCESS;
    struct pci_dev *pPciDev = pIns->pPciDev;
...
static int nemuPciLinuxDevReset(PNEMURAWPCIINS pIns)
{
    int rc = VINF_SUCCESS;
    IPRT_LINUX_SAVE_EFL_AC();
...
static struct file* nemuPciFileOpen(const char* path, int flags)
{
    ...
    {
        err = PTR_ERR(filp);
        printk(KERN_DEBUG "nemuPciFileOpen: error %d\n", err);
        return NULL;
    }
...
static void  nemuPciFileClose(struct file* file)
{
    filp_close(file, NULL);
}

static int nemuPciFileWrite(struct file* file, unsigned long long offset, unsigned char* data, unsigned int size)
{
    int          ret;
    ...
    set_fs(fs_save);
    if (ret < 0)
        printk(KERN_DEBUG "nemuPciFileWrite: error %d\n", ret);

    return ret;
}

static int nemuPciLinuxDevDetachHostDriver(PNEMURAWPCIINS pIns)
{
    ...
    if (!g_NemuPciGlobals.fPciStubModuleAvail)
    {
        printk(KERN_INFO "nemupci: stub module %s not detected: cannot detach\n",
               PCI_STUB_MODULE);
    ...
    if (!pPciDev)
    {
        printk(KERN_INFO "nemupci: device at %02x:%02x.%d not found\n",
    ...
    printk(KERN_DEBUG "nemupci: detected device: %04x:%04x at %02x:%02x.%d, driver %s\n",
    ...
    if (fDetach && currentDriver)
    {
        ...
        {
            printk(KERN_DEBUG "nemupci: ERROR: %s contains invalid symbols\n", currentDriver);
            return VERR_ACCESS_DENIED;
        }
...
# endif
        ...
        RTStrPrintf(szFileBuf, cMaxBuf,
                              ...
                              PCI_STUB_MODULE);
        pFile = nemuPciFileOpen(szFileBuf, O_WRONLY);
        if (pFile)
        {
            ...
            nemuPciFileWrite(pFile, 0, szCmdBuf, iCmdLen);
            nemuPciFileClose(pFile);
        }
        else
            printk(KERN_DEBUG "nemupci: cannot open %s\n", szFileBuf);
...
static int nemuPciLinuxDevReattachHostDriver(PNEMURAWPCIINS pIns)
{
    struct pci_dev *pPciDev = pIns->pPciDev;
...
DECLHIDDEN(int) nemuPciOsDevInit(PNEMURAWPCIINS pIns, uint32_t fFlags)
{
    ...
    {
        rc = nemuPciLinuxDevDetachHostDriver(pIns);
        if (RT_FAILURE(rc))
        {
...
DECLHIDDEN(int) nemuPciOsDevDeinit(PNEMURAWPCIINS pIns, uint32_t fFlags)
{
    ...
    if (RT_LIKELY(pPciDev))
    {
        ...
        nemuPciLinuxDevUnregisterWithIommu(pIns);
        ...
        nemuPciLinuxDevReattachHostDriver(pIns);
                                      ...
                                      RTR0PTR        *pRegionBase)
{
    ...
    if (!pIns->aRegionR0Mapping[iRegion])
    {
        ...
        *pRegionBase = pIns->aRegionR0Mapping[iRegion];
        ...
        rcLnx = pci_request_region(pPciDev, iRegion, "nemupci");
        if (!rcLnx)
        {
 ...
 * @param   pvDevId         The device ID, a pointer to PNEMURAWPCIINS.
 * @param   pRegs           Register set. Removed in 2.6.19.
 */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 19) && !defined(DOXYGEN_RUNNING)
static irqreturn_t nemuPciOsIrqHandler(int iIrq, void *pvDevId)
#else
static irqreturn_t nemuPciOsIrqHandler(int iIrq, void *pvDevId, struct pt_regs *pRegs)
#endif
{
    PNEMURAWPCIINS pIns = (PNEMURAWPCIINS)pvDevId;
    bool fTaken = true;

    if (pIns && pIns->IrqHandler.pfnIrqHandler)
        fTaken = pIns->IrqHandler.pfnIrqHandler(pIns->IrqHandler.pIrqContext, iIrq);
#ifndef NEMU_WITH_SHARED_PCI_INTERRUPTS
...
static DECLCALLBACK(int) nemuPciOsContigMemInfo(PRAWPCIPERVM pVmCtx, RTHCPHYS HostStart, RTGCPHYS GuestStart,
                                                uint64_t cMemSize, PCIRAWMEMINFOACTION Action)
{
    struct iommu_domain* domain = ((PNEMURAWPCIDRVVM)(pVmCtx->pDriverData))->pIommuDomain;
    int rc = VINF_SUCCESS;
    IPRT_LINUX_SAVE_EFL_AC();
    ...
    switch (Action)
    {
        case PCIRAW_MEMINFO_MAP:
        {
            ...
            if (r)
            {
                printk(KERN_ERR "nemuPciOsContigMemInfo:"
...
DECLHIDDEN(int) nemuPciOsInitVm(PNEMURAWPCIDRVVM pThis, PVM pVM, PRAWPCIPERVM pVmData)
{
    ...
    if (IOMMU_PRESENT())
    {
        ...
        {
            pVmData->pfnContigMemInfo = nemuPciOsContigMemInfo;
