...
/** @file
 * Nemu Qt GUI - UIMachineView class implementation.
 */
...
void UIMachineView::applyMachineViewScaleFactor()
{
    ...
    const double dScaleFactor = gEDataManager->scaleFactor(nemuGlobal().managedVMUuid());
    const bool fUseUnscaledHiDPIOutput = gEDataManager->useUnscaledHiDPIOutput(nemuGlobal().managedVMUuid());
    ...
    if (machine().GetAccelerate3DEnabled() && nemuGlobal().is3DAvailable())
    {
        display().NotifyScaleFactorChange(m_uScreenId,
                                          (uint32_t)(dScaleFactor * NEMU_OGL_SCALE_FACTOR_MULTIPLIER),
                                          (uint32_t)(dScaleFactor * NEMU_OGL_SCALE_FACTOR_MULTIPLIER));
...
void UIMachineView::sltPerformGuestResize(const QSize &toSize)
{
    ...
    if (gEDataManager->autoMountGuestScreensEnabled(nemuGlobal().managedVMUuid()))
    {
...
void UIMachineView::sltHandleScaleFactorChange(const QString &strMachineID)
{
    ...
    if (strMachineID != nemuGlobal().managedVMUuid())
        return;
...
void UIMachineView::sltHandleScalingOptimizationChange(const QString &strMachineID)
{
    ...
    frameBuffer()->setScalingOptimizationType(gEDataManager->scalingOptimizationType(nemuGlobal().managedVMUuid()));
...
void UIMachineView::sltHandleHiDPIOptimizationChange(const QString &strMachineID)
{
    ...
    frameBuffer()->setHiDPIOptimizationType(gEDataManager->hiDPIOptimizationType(nemuGlobal().managedVMUuid()));
...
void UIMachineView::prepareFrameBuffer()
{
    ...
    else
    {
        ...
        m_pFrameBuffer->setHiDPIOptimizationType(gEDataManager->hiDPIOptimizationType(nemuGlobal().managedVMUuid()));

        /* Take scaling optimization type into account: */
        m_pFrameBuffer->setScalingOptimizationType(gEDataManager->scalingOptimizationType(nemuGlobal().managedVMUuid()));
...
void UIMachineView::loadMachineViewSettings()
{
    /* Global settings: */
    {
         ...
         * video modes we like: */
        QString maxGuestSize = nemuGlobal().settings().publicProperty("GUI/MaxGuestResolution");
...
void UIMachineView::cleanupFrameBuffer()
{
     ...
     * Note: NEMU_WITH_CROGL additionally requires us to call DetachFramebuffer
...
QSize UIMachineView::sizeHint() const
{
    ...
    if (size.width() < 16 || size.height() < 16)
        if (nemuGlobal().shouldStartPaused() || nemuGlobal().isDebuggerAutoShowEnabled())
            size = QSize(640, 480);
...
bool UIMachineView::guestScreenVisibilityStatus() const
{
    ...
    return gEDataManager->lastGuestScreenVisibilityStatus(m_uScreenId, nemuGlobal().managedVMUuid());
}

QSize UIMachineView::guestScreenSizeHint() const
{
    /* Load guest-screen size-hint: */
    QSize sizeHint = gEDataManager->lastGuestScreenSizeHint(m_uScreenId, nemuGlobal().managedVMUuid());
...
void UIMachineView::storeGuestSizeHint(const QSize &sizeHint)
{
    ...
    gEDataManager->setLastGuestScreenSizeHint(m_uScreenId, sizeHint, nemuGlobal().managedVMUuid());
}
    ...
    if (pWatched == machineWindow())
    {
        switch (pEvent->type())
        {
            ...
            case QEvent::Move:
            {
                ...
                const int iCurrentHostScreenNumber = nemuGlobal().screenNumber(this);
...
bool UIMachineView::winEvent(MSG *pMsg, long* /* piResult */)
{
    ...
    switch (pMsg->message)
    {
        ...
        case WM_SYSKEYUP:
        {
            ...
            if (nemuGlobal().isSeparateProcess() && InSendMessage())
            {
