...
/** @file
 * Nemu Qt GUI - UIGlobalSettingsNetwork class implementation.
 */
...
void UIGlobalSettingsNetwork::loadToCacheFrom(QVariant &data)
{
    ...
    const CNATNetworkVector &networks = nemuGlobal().virtualBox().GetNATNetworks();
    ...
    const CHostNetworkInterfaceVector &interfaces = nemuGlobal().host().GetNetworkInterfaces();
...
void UIGlobalSettingsNetwork::sltAddNetworkNAT()
{
    ...
    CNATNetwork network = nemu.CreateNATNetwork(strNameTemplate.arg(strIndex));
    if (!nemu.isOk())
        return msgCenter().cannotCreateNATNetwork(nemu, this);
...
void UIGlobalSettingsNetwork::sltDelNetworkNAT()
{
    ...
    const CNATNetwork &network = nemu.FindNATNetworkByName(strNetworkName);
    AssertReturnVoid(nemu.isOk() && !network.isNull());

    /* Remove NAT network: */
    nemu.RemoveNATNetwork(network);
    if (!nemu.isOk())
        return msgCenter().cannotRemoveNATNetwork(nemu, strNetworkName, this);
...
void UIGlobalSettingsNetwork::sltAddNetworkHost()
{
    ...
    CDHCPServer dhcp = nemu.FindDHCPServerByNetworkName(iface.GetNetworkName());
    if (dhcp.isNull())
    {
        nemu.CreateDHCPServer(iface.GetNetworkName());
        if (!nemu.isOk())
            return msgCenter().cannotCreateDHCPServer(nemu, this);
        dhcp = nemu.FindDHCPServerByNetworkName(iface.GetNetworkName());
    }
...
void UIGlobalSettingsNetwork::sltDelNetworkHost()
{
    ...
    CDHCPServer dhcp = nemu.FindDHCPServerByNetworkName(iface.GetNetworkName());
    if (nemu.isOk() && !dhcp.isNull())
    {
        nemu.RemoveDHCPServer(dhcp);
        if (!nemu.isOk())
            return msgCenter().cannotRemoveDHCPServer(nemu, strInterfaceName, this);
    }
...
void UIGlobalSettingsNetwork::saveCacheItemNetworkNAT(const UIDataNetworkNAT &data)
{
    ...
    CVirtualBox nemu = nemuGlobal().virtualBox();
    CNATNetwork network = nemu.FindNATNetworkByName(data.m_strName);
    AssertReturnVoid(nemu.isOk() && !network.isNull());
...
UIDataNetworkHost UIGlobalSettingsNetwork::generateDataNetworkHost(const CHostNetworkInterface &iface)
{
    ...
    CDHCPServer dhcp = nemuGlobal().virtualBox().FindDHCPServerByNetworkName(iface.GetNetworkName());
    if (dhcp.isNull())
    {
        CVirtualBox nemu = nemuGlobal().virtualBox();
        nemu.CreateDHCPServer(iface.GetNetworkName());
        if (!nemu.isOk())
        {
            msgCenter().cannotCreateDHCPServer(nemu, this);
            return data;
        }
        dhcp = nemuGlobal().virtualBox().FindDHCPServerByNetworkName(iface.GetNetworkName());
    }
    Assert(!dhcp.isNull());
    if (dhcp.isNull())
        return data;
...
void UIGlobalSettingsNetwork::saveCacheItemNetworkHost(const UIDataNetworkHost &data)
{
    ...
    CVirtualBox nemu = nemuGlobal().virtualBox();
    CDHCPServer dhcp = nemu.FindDHCPServerByNetworkName(iface.GetNetworkName());
    AssertReturnVoid(nemu.isOk() && !dhcp.isNull());
