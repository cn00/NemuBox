...
%ifdef NEMU
 ...
 %define NEMU_PDPT_ADDR     (0x00800000 - 0x6000)
 %define NEMU_PDPTR_ADDR    (0x00800000 - 0x2000)
 %define NEMU_PML4_ADDR     (0x00800000 - 0x1000)
...
%else ; !NEMU
    ...
    FILL_ENTRIES X86_PAGE_2M_SIZE, NEMU_PDPT_ADDR, NEMU_PDPT_ADDR + 0x4000
    ...
    mov     eax, ( X86_PDPE_P | X86_PDPE_RW | X86_PDPE_A | X86_PDPE_PCD ) + NEMU_PDPT_ADDR
    FILL_ENTRIES X86_PAGE_4K_SIZE, NEMU_PDPTR_ADDR, NEMU_PDPTR_ADDR + 4*8
    ZERO_ENTRIES NEMU_PDPTR_ADDR + 4*8, NEMU_PDPTR_ADDR + 0x1000

    ; Third, set up a PML4 with the first entry pointing to the previous table.
    mov     dword [NEMU_PML4_ADDR], ( X86_PML4E_P | X86_PML4E_PCD | X86_PML4E_A | X86_PML4E_RW ) + NEMU_PDPTR_ADDR
    mov     dword [NEMU_PML4_ADDR + 4], 0
    ZERO_ENTRIES NEMU_PML4_ADDR + 1*8, NEMU_PML4_ADDR + 0x1000

    mov     eax, NEMU_PML4_ADDR
%endif
    mov     cr3, eax
