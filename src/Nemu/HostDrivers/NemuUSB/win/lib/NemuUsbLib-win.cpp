/* $Id: NemuUsbLib-win.cpp $ */
/** @file
 * Nemu USB ring-3 Driver Interface library, Windows.
 */
...
#define NEMU_USB_USE_DEVICE_NOTIFICATION

#ifdef NEMU_USB_USE_DEVICE_NOTIFICATION
# include <Dbt.h>
#endif
...
typedef struct NEMUUSBGLOBALSTATE
{
    HANDLE hMonitor;
    HANDLE hNotifyEvent;
    ...
    HANDLE hTimer;
#endif
} NEMUUSBGLOBALSTATE, *PNEMUUSBGLOBALSTATE;

typedef struct NEMUUSB_STRING_DR_ENTRY
{
    struct NEMUUSB_STRING_DR_ENTRY *pNext;
    UCHAR iDr;
    USHORT idLang;
    USB_STRING_DESCRIPTOR StrDr;
} NEMUUSB_STRING_DR_ENTRY, *PNEMUUSB_STRING_DR_ENTRY;

/**
 * This represents NemuUsb device instance
 */
typedef struct NEMUUSB_DEV
{
    struct NEMUUSB_DEV *pNext;
    char                szName[512];
    char                szDriverRegName[512];
} NEMUUSB_DEV, *PNEMUUSB_DEV;


/*********************************************************************************************************************************
*   Global Variables                                                                                                             *
*********************************************************************************************************************************/
static NEMUUSBGLOBALSTATE g_NemuUsbGlobal;


int usbLibVuDeviceValidate(PNEMUUSB_DEV pVuDev)
{
...
static int usbLibVuDevicePopulate(PNEMUUSB_DEV pVuDev, HDEVINFO hDevInfo, PSP_DEVICE_INTERFACE_DATA pIfData)
{
...
static void usbLibVuFreeDevices(PNEMUUSB_DEV pDevInfos)
{
    while (pDevInfos)
    {
        PNEMUUSB_DEV pNext = pDevInfos->pNext;
        RTMemFree(pDevInfos);
        pDevInfos = pNext;
    }
}

static int usbLibVuGetDevices(PNEMUUSB_DEV *ppVuDevs, uint32_t *pcVuDevs)
{
    *ppVuDevs = NULL;
    *pcVuDevs = 0;

    HDEVINFO hDevInfo =  SetupDiGetClassDevs(&GUID_CLASS_NEMUUSB,
    ...
    for (int i = 0; ; ++i)
    {
        ...
        if (!SetupDiEnumDeviceInterfaces(hDevInfo,
                            ...
                            &GUID_CLASS_NEMUUSB, /* IN LPGUID InterfaceClassGuid */
                            i,
                            &IfData))
        {
            DWORD winEr = GetLastError();
        ...
        PNEMUUSB_DEV pVuDev = (PNEMUUSB_DEV)RTMemAllocZ(sizeof (*pVuDev));
        if (!pVuDev)
        {
...
static int usbLibDevPopulate(PUSBDEVICE pDev, PUSB_NODE_CONNECTION_INFORMATION_EX pConInfo, ULONG iPort, LPCSTR lpszDrvKeyName, LPCSTR lpszHubName, PNEMUUSB_STRING_DR_ENTRY pDrList)
{
 ...
 *  that stuff succeeds:
 *  http://nemu.innotek.de/pipermail/nemu-dev/2011-August/004516.html
 *
 *  */
...
static int usbLibDevStrDriverKeyGet(HANDLE hHub, ULONG iPort, LPSTR* plpszName)
{
    ...
    *plpszName = NULL;
    ...
    {
#ifdef NEMU_WITH_ANNOYING_USB_ASSERTIONS
        DWORD winEr = GetLastError();
...
static int usbLibDevStrDrEntryGet(HANDLE hHub, ULONG iPort, ULONG iDr, USHORT idLang, PNEMUUSB_STRING_DR_ENTRY *ppList)
{
    ...
    PNEMUUSB_STRING_DR_ENTRY pEntry =
        (PNEMUUSB_STRING_DR_ENTRY)RTMemAllocZ(sizeof(NEMUUSB_STRING_DR_ENTRY) + pDr->bLength + 2);
    AssertPtr(pEntry);
    if (!pEntry)
...
static void usbLibDevStrDrEntryFree(PNEMUUSB_STRING_DR_ENTRY pDr)
{
    RTMemFree(pDr);
}

static void usbLibDevStrDrEntryFreeList(PNEMUUSB_STRING_DR_ENTRY pDr)
{
    while (pDr)
    {
        PNEMUUSB_STRING_DR_ENTRY pNext = pDr->pNext;
        usbLibDevStrDrEntryFree(pDr);
        pDr = pNext;
    }
}

static int usbLibDevStrDrEntryGetForLangs(HANDLE hHub, ULONG iPort, ULONG iDr, ULONG cIdLang, const USHORT *pIdLang, PNEMUUSB_STRING_DR_ENTRY *ppList)
{
...
static int usbLibDevStrDrEntryGetAll(HANDLE hHub, ULONG iPort, PUSB_DEVICE_DESCRIPTOR pDevDr, PUSB_CONFIGURATION_DESCRIPTOR pCfgDr, PNEMUUSB_STRING_DR_ENTRY *ppList)
{
...
static int usbLibDevGetHubPortDevices(HANDLE hHub, LPCSTR lpcszHubName, ULONG iPort, PUSBDEVICE *ppDevs, uint32_t *pcDevs)
{
    ...
    PNEMUUSB_STRING_DR_ENTRY pList = NULL;
...
static int usbLibMonDevicesCmp(PUSBDEVICE pDev, PNEMUUSB_DEV pDevInfo)
{
    int iDiff;
...
static int usbLibMonDevicesUpdate(PNEMUUSBGLOBALSTATE pGlobal, PUSBDEVICE pDevs, uint32_t cDevs, PNEMUUSB_DEV pDevInfos, uint32_t cDevInfos)
{
    ...
    for (; pDevInfos; pDevInfos = pDevInfos->pNext)
    {
        for (pDevs = pDevsHead; pDevs; pDevs = pDevs->pNext)
        {
            ...
            HNEMUUSBDEVUSR hDevice = Dev.hDevice;
...
static int usbLibGetDevices(PNEMUUSBGLOBALSTATE pGlobal, PUSBDEVICE *ppDevs, uint32_t *pcDevs)
{
    *ppDevs = NULL;
    *pcDevs = 0;
...
static int usbLibStateWaitChange(PNEMUUSBGLOBALSTATE pGlobal, RTMSINTERVAL cMillies)
{
...
{
    return usbLibStateWaitChange(&g_NemuUsbGlobal, msWaitTimeout);
}

static int usbLibInterruptWaitChange(PNEMUUSBGLOBALSTATE pGlobal)
{
...
{
    return usbLibInterruptWaitChange(&g_NemuUsbGlobal);
}
...
{
    Assert(g_NemuUsbGlobal.hMonitor != INVALID_HANDLE_VALUE);
    return usbLibGetDevices(&g_NemuUsbGlobal, ppDevices, pcbNumDevices);
}
...
USBLIB_DECL(void *) USBLibAddFilter(PCUSBFILTER pFilter)
{
    ...
    if (g_NemuUsbGlobal.hMonitor == INVALID_HANDLE_VALUE)
    {
#ifdef NEMU_WITH_ANNOYING_USB_ASSERTIONS
        AssertFailed();
#endif
    ...
    if (!DeviceIoControl(g_NemuUsbGlobal.hMonitor, SUPUSBFLT_IOCTL_ADD_FILTER,
...
USBLIB_DECL(void) USBLibRemoveFilter(void *pvId)
{
    ...
    uId = (uintptr_t)pvId;
    if (!DeviceIoControl(g_NemuUsbGlobal.hMonitor, SUPUSBFLT_IOCTL_REMOVE_FILTER, &uId, sizeof(uId),  NULL, 0,&cbReturned, NULL))
...
USBLIB_DECL(int) USBLibRunFilters()
{
    ...
    if (!DeviceIoControl(g_NemuUsbGlobal.hMonitor, SUPUSBFLT_IOCTL_RUN_FILTERS,
                NULL, 0,
                NULL, 0,
                &cbReturned, NULL))
    {
...
{
    SetEvent(g_NemuUsbGlobal.hNotifyEvent);
}
...
static void usbLibOnDeviceChange(void)
{
     ...
     * */
    if (g_NemuUsbGlobal.hTimer)
    {
        if (!DeleteTimerQueueTimer(g_NemuUsbGlobal.hTimerQueue, g_NemuUsbGlobal.hTimer, NULL))
        {
            DWORD winEr = GetLastError();
    ...
    if (!CreateTimerQueueTimer(&g_NemuUsbGlobal.hTimer, g_NemuUsbGlobal.hTimerQueue,
                               usbLibTimerCallback,
                               NULL,
                               500, /* ms*/
                               0,
...
{
    static LPCSTR   s_szNemuUsbWndClassName = "NemuUsbLibClass";
    const HINSTANCE hInstance               = (HINSTANCE)GetModuleHandle(NULL);

    Assert(g_NemuUsbGlobal.hWnd == NULL);
    g_NemuUsbGlobal.hWnd = NULL;
    ...
    wc.lpszClassName = s_szNemuUsbWndClassName;
    ATOM atomWindowClass = RegisterClass(&wc);
    if (atomWindowClass != 0)
        g_NemuUsbGlobal.hWnd = CreateWindowEx(WS_EX_TOOLWINDOW | WS_EX_TRANSPARENT | WS_EX_TOPMOST,
                                              s_szNemuUsbWndClassName, s_szNemuUsbWndClassName,
                                              WS_POPUPWINDOW,
    ...
    if (g_NemuUsbGlobal.hWnd)
    {
        /* Make sure it's really hidden. */
        SetWindowPos(g_NemuUsbGlobal.hWnd, HWND_TOPMOST, -200, -200, 0, 0,
         ...
         * Clean up.
         */
        DestroyWindow(g_NemuUsbGlobal.hWnd);
    }

    if (atomWindowClass != NULL)
        UnregisterClass(s_szNemuUsbWndClassName, hInstance);

    return 0;
}

#endif /* NEMU_USB_USE_DEVICE_NOTIFICATION */
...
USBLIB_DECL(int) USBLibInit(void)
{
    ...
    RT_ZERO(g_NemuUsbGlobal);
    g_NemuUsbGlobal.hMonitor = INVALID_HANDLE_VALUE;

    /*
     * Create the notification and interrupt event before opening the device.
     */
    g_NemuUsbGlobal.hNotifyEvent = CreateEvent(NULL,  /* LPSECURITY_ATTRIBUTES lpEventAttributes */
                                               FALSE, /* BOOL bManualReset */
#ifndef NEMU_USB_USE_DEVICE_NOTIFICATION
                                               TRUE,  /* BOOL bInitialState */
#else
                                               FALSE, /* set to false since it will be initially used for notification thread startup sync */
#endif
                                               NULL   /* LPCTSTR lpName */);
    if (g_NemuUsbGlobal.hNotifyEvent)
    {
        g_NemuUsbGlobal.hInterruptEvent = CreateEvent(NULL,  /* LPSECURITY_ATTRIBUTES lpEventAttributes */
                                                      FALSE, /* BOOL bManualReset */
                                                      FALSE, /* BOOL bInitialState */
                                                      NULL   /* LPCTSTR lpName */);
        if (g_NemuUsbGlobal.hInterruptEvent)
        {
            /*
             * Open the USB monitor device, starting if needed.
             */
            g_NemuUsbGlobal.hMonitor = CreateFile(USBMON_DEVICE_NAME,
            ...
            if (g_NemuUsbGlobal.hMonitor == INVALID_HANDLE_VALUE)
            {
                HRESULT hr = NemuDrvCfgSvcStart(USBMON_SERVICE_NAME_W);
                if (hr == S_OK)
                {
                    g_NemuUsbGlobal.hMonitor = CreateFile(USBMON_DEVICE_NAME,
                                                          GENERIC_READ | GENERIC_WRITE,
            ...
            if (g_NemuUsbGlobal.hMonitor != INVALID_HANDLE_VALUE)
            {
                ...
                USBSUP_VERSION  Version = {0};
                DWORD           cbReturned = 0;
                if (DeviceIoControl(g_NemuUsbGlobal.hMonitor, SUPUSBFLT_IOCTL_GET_VERSION,
                                    NULL, 0,
                                    ...
                                    &cbReturned, NULL))
                {
                        ...
                        && Version.u32Minor >= USBMON_MINOR_VERSION)
                    {
                        ...
                        Assert(g_NemuUsbGlobal.hNotifyEvent);
                        SetEvent.u.hEvent = g_NemuUsbGlobal.hNotifyEvent;
                        if (DeviceIoControl(g_NemuUsbGlobal.hMonitor, SUPUSBFLT_IOCTL_SET_NOTIFY_EVENT,
                         ...
                         */
                        g_NemuUsbGlobal.hTimerQueue = CreateTimerQueue();
                        if (g_NemuUsbGlobal.hTimerQueue)
                        {
                            g_NemuUsbGlobal.hThread = CreateThread(
                            ...
                            if (g_NemuUsbGlobal.hThread)
                            {
                                DWORD dwResult = WaitForSingleObject(g_NemuUsbGlobal.hNotifyEvent, INFINITE);
                                ...
                                dwResult = WaitForSingleObject(g_NemuUsbGlobal.hThread, INFINITE);
                                Assert(dwResult == WAIT_OBJECT_0);
                                BOOL bRc = CloseHandle(g_NemuUsbGlobal.hThread);
                                AssertMsg(bRc, ("CloseHandle for hThread failed winEr(%d)\n", GetLastError()));
                                g_NemuUsbGlobal.hThread = INVALID_HANDLE_VALUE;
                            }
                            else
                            {
                                DWORD winEr = GetLastError();
                            ...
                            DeleteTimerQueueEx(g_NemuUsbGlobal.hTimerQueue, INVALID_HANDLE_VALUE /* see term */);
                            g_NemuUsbGlobal.hTimerQueue = NULL;
                        }
                        else
                        {
                            DWORD winEr = GetLastError();
                ...
                CloseHandle(g_NemuUsbGlobal.hMonitor);
                g_NemuUsbGlobal.hMonitor = INVALID_HANDLE_VALUE;
            }
            else
            {
            ...
            CloseHandle(g_NemuUsbGlobal.hInterruptEvent);
            g_NemuUsbGlobal.hInterruptEvent = NULL;
        }
        else
        {
            DWORD winEr = GetLastError();
        ...
        CloseHandle(g_NemuUsbGlobal.hNotifyEvent);
        g_NemuUsbGlobal.hNotifyEvent = NULL;
    }
    else
    {
        DWORD winEr = GetLastError();
...
USBLIB_DECL(int) USBLibTerm(void)
{
    if (g_NemuUsbGlobal.hMonitor == INVALID_HANDLE_VALUE)
    {
        Assert(g_NemuUsbGlobal.hInterruptEvent == NULL);
        Assert(g_NemuUsbGlobal.hNotifyEvent == NULL);
        return VINF_SUCCESS;
    }

    BOOL bRc;
#ifdef NEMU_USB_USE_DEVICE_NOTIFICATION
    bRc = PostMessage(g_NemuUsbGlobal.hWnd, WM_QUIT, 0, 0);
    AssertMsg(bRc, ("PostMessage for hWnd failed winEr(%d)\n", GetLastError()));

    if (g_NemuUsbGlobal.hThread != NULL)
    {
        DWORD dwResult = WaitForSingleObject(g_NemuUsbGlobal.hThread, INFINITE);
        Assert(dwResult == WAIT_OBJECT_0);
        bRc = CloseHandle(g_NemuUsbGlobal.hThread);
        AssertMsg(bRc, ("CloseHandle for hThread failed winEr(%d)\n", GetLastError()));
    }

    if (g_NemuUsbGlobal.hTimer)
    {
        bRc = DeleteTimerQueueTimer(g_NemuUsbGlobal.hTimerQueue, g_NemuUsbGlobal.hTimer,
