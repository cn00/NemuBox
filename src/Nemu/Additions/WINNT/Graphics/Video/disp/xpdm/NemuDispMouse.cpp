/* $Id: NemuDispMouse.cpp $ */

/** @file
 * Nemu XPDM Display driver, mouse pointer related functions
 */
...
static BOOL NemuDispFillMonoShape(PNEMUDISPDEV pDev, SURFOBJ *psoMask)
{
...
static SURFOBJ *NemuDispConvSurfTo32BPP(PNEMUDISPDEV pDev, SURFOBJ *psoScreen, SURFOBJ *psoSrc, XLATEOBJ *pxlo, HSURF *phDstSurf)
{
    *phDstSurf = NULL;
...
static BOOL NemuDispFillColorShape(PNEMUDISPDEV pDev, SURFOBJ *psoScreen, SURFOBJ *psoMask, SURFOBJ *psoColor,
                                   XLATEOBJ *pxlo, FLONG fl)
{
    ...
    else
    {
        ...
        pso32bpp = NemuDispConvSurfTo32BPP(pDev, psoScreen, psoColor, pxlo, &hSurf32bpp);
        if (!pso32bpp)
        {
...
int NemuDispInitPointerCaps(PNEMUDISPDEV pDev, DEVINFO *pDevInfo)
{
    ...
    rc = NemuDispMPGetPointerCaps(pDev->hDriver, &pDev->pointer.caps);
...
VOID APIENTRY NemuDispDrvMovePointer(SURFOBJ *pso, LONG x, LONG y, RECTL *prcl)
{
    ...
    if (-1==x) /* hide pointer */
    {
        rc = NemuDispMPDisablePointer(pDev->hDriver);
        NEMU_WARNRC(rc);
    }
    else
    {
        ...
        rc = NemuDispMPSetPointerPosition(pDev->hDriver, &pos);
        NEMU_WARNRC(rc);
    }
...
ULONG APIENTRY
NemuDispDrvSetPointerShape(SURFOBJ *pso, SURFOBJ *psoMask, SURFOBJ *psoColor, XLATEOBJ *pxlo,
                           LONG xHot, LONG yHot, LONG x, LONG y, RECTL *prcl, FLONG fl)
{
    ...
    if (!psoColor) /* Monochrome pointer */
    {
        if (!(pDev->pointer.caps.Flags & VIDEO_MODE_MONO_POINTER)
            || !NemuDispFillMonoShape(pDev, psoMask))
        {
            rc = NemuDispMPDisablePointer(pDev->hDriver);
            NEMU_WARNRC(rc);
            return SPS_DECLINE;
    ...
    {
        if (!(pDev->pointer.caps.Flags & VIDEO_MODE_COLOR_POINTER)
            || !NemuDispFillColorShape(pDev, pso, psoMask, psoColor, pxlo, fl))
        {
            rc = NemuDispMPDisablePointer(pDev->hDriver);
            NEMU_WARNRC(rc);
            return SPS_DECLINE;
    ...
    pDev->pointer.pAttrs->Enable = NEMU_MOUSE_POINTER_SHAPE;
    pDev->pointer.pAttrs->Enable |= (yHot & 0xFF) << 24;
    pDev->pointer.pAttrs->Enable |= (xHot & 0xFF) << 16;

    if (x!=-1)
    {
        pDev->pointer.pAttrs->Enable |= NEMU_MOUSE_POINTER_VISIBLE;
    }

    if (fl & SPS_ALPHA)
    {
        pDev->pointer.pAttrs->Enable |= NEMU_MOUSE_POINTER_ALPHA;
    }
    ...
    rc = NemuDispMPSetPointerAttrs(pDev);
    if (RT_FAILURE(rc))
    {
        NEMU_WARNRC(rc);
        rc = NemuDispMPDisablePointer(pDev->hDriver);
    ...
    if (x!=-1)
    {
        NemuDispDrvMovePointer(pso, x, y, NULL);
    }
