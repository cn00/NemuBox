...
/** @file
 * Nemu Qt GUI - UIGDetailsElement[Name] classes implementation.
 */
    ...
    , m_pTask(0)
{
    ...
    connect(nemuGlobal().threadPool(), SIGNAL(sigTaskComplete(UITask*)),
...
void UIGDetailsElementInterface::updateAppearance()
{
    ...
    if (!m_pTask)
    {
        ...
        nemuGlobal().threadPool()->enqueueTask(m_pTask);
    }
}
...
void UIGDetailsUpdateTaskGeneral::run()
{
    ...
    if (machine.GetAccessible())
    {
        ...
        table << UITextTableLine(QApplication::translate("UIGDetails", "Operating System", "details (general)"),
                                 nemuGlobal().vmGuestOSTypeDescription(machine.GetOSTypeId()));
...
void UIGDetailsUpdateTaskSystem::run()
{
    ...
    if (machine.GetAccessible())
    {
        ...
        for (ulong i = 1; i <= nemuGlobal().virtualBox().GetSystemProperties().GetMaxBootPosition(); ++i)
        {
        ...
        if (nemuGlobal().virtualBox().GetHost().GetProcessorFeature(KProcessorFeature_HWVirtEx))
        {
            /* VT-x/AMD-V: */
    ...
    if (machine.GetAccessible())
    {
        ...
        foreach (const CStorageController &controller, machine.GetStorageControllers())
        {
            ...
            foreach (const CMediumAttachment &attachment, machine.GetMediumAttachmentsOfController(controller.GetName()))
            {
                ...
                QString strAttachmentInfo = nemuGlobal().details(attachment.GetMedium(), false, false);
                /* That temporary hack makes sure 'Inaccessible' word is always bold: */
                { // hack
                    QString strInaccessibleString(NemuGlobal::tr("Inaccessible", "medium"));
...
void UIGDetailsUpdateTaskNetwork::run()
{
    ...
    if (machine.GetAccessible())
    {
        ...
        bool fSomeInfo = false;
        ulong uSount = nemuGlobal().virtualBox().GetSystemProperties().GetMaxNetworkAdapters(KChipsetType_PIIX3);
...
void UIGDetailsUpdateTaskSerial::run()
{
    ...
    if (machine.GetAccessible())
    {
        ...
        bool fSomeInfo = false;
        ulong uCount = nemuGlobal().virtualBox().GetSystemProperties().GetSerialPortCount();
        for (ulong uSlot = 0; uSlot < uCount; ++uSlot)
        {
            ...
            if (port.GetEnabled())
            {
                ...
                QString data = nemuGlobal().toCOMPortName(port.GetIRQ(), port.GetIOBase()) + ", ";
...
void UIGDetailsUpdateTaskParallel::run()
{
    ...
    {
        bool fSomeInfo = false;
        ulong uCount = nemuGlobal().virtualBox().GetSystemProperties().GetParallelPortCount();
        for (ulong uSlot = 0; uSlot < uCount; ++uSlot)
        {
            const CParallelPort &port = machine.GetParallelPort(uSlot);
            if (port.GetEnabled())
            {
                QString data = nemuGlobal().toLPTPortName(port.GetIRQ(), port.GetIOBase()) +
