/* $Id: NemuNetIntIf.cpp $ */
/** @file
 * NemuNetIntIf - IntNet Interface Client Routines.
 */
 ...
 */
static void nemunetIntIfCopySG(void *pvFrame, size_t cSegs, PCINTNETSEG paSegs)
{
...
int NemuNetIntIfRingWriteFrame(PINTNETBUF pBuf, PINTNETRINGBUF pRingBuf, size_t cSegs, PCINTNETSEG paSegs)
{
    ...
    if (RT_SUCCESS(rc))
    {
        nemunetIntIfCopySG(pvFrame, cSegs, paSegs);
 ...
 */
int NemuNetIntIfSend(PSUPDRVSESSION pSession, INTNETIFHANDLE hIf, PINTNETBUF pBuf,
                     size_t cSegs, PCINTNETSEG paSegs, bool fFlush)
{
    int rc = NemuNetIntIfRingWriteFrame(pBuf, &pBuf->Send, cSegs, paSegs);
    if (rc == VERR_BUFFER_OVERFLOW)
    {
        NemuNetIntIfFlush(pSession, hIf);
        rc = NemuNetIntIfRingWriteFrame(pBuf, &pBuf->Send, cSegs, paSegs);
    }
    if (RT_SUCCESS(rc) && fFlush)
        rc = NemuNetIntIfFlush(pSession, hIf);
    return rc;
}
