...
#ifndef ___NemuGuest_win_h
#define ___NemuGuest_win_h
...
typedef struct NEMUGUESTWINBASEADDRESS
{
...
} NEMUGUESTWINBASEADDRESS, *PNEMUGUESTWINBASEADDRESS;


/** Windows-specific device extension bits. */
typedef struct NEMUGUESTDEVEXTWIN
{
    NEMUGUESTDEVEXT Core;
    ...
    NEMUGUESTWINBASEADDRESS pciBaseAddress[PCI_TYPE0_ADDRESSES];
    ...
    /** Last system power action set (see NemuGuestPower). */
    ...
    struct NEMUGUESTSESSION *pKernelSession;
...
} NEMUGUESTDEVEXTWIN, *PNEMUGUESTDEVEXTWIN;
...
#define NEMUGUEST_UPDATE_DEVSTATE(a_pDevExt, a_enmNewDevState) \
    do { \
...
#define NEMU_CM_PRE_VISTA_MASK (0x3f)
...
void       vgdrvNtUnmapVMMDevMemory(PNEMUGUESTDEVEXTWIN pDevExt); /**< @todo make static once buggy vgdrvNtInit is fixed. */
