/* $Id: SUPDrv-win.cpp $ */
/** @file
 * NemuDrv - The VirtualBox Support Driver - Windows NT specifics.
 */
...
#define SERVICE_NAME    "NemuDrv"
/** The Pool tag (Nemu). */
#define SUPDRV_NT_POOL_TAG  'xoBV'

/** NT device name for user access. */
#define DEVICE_NAME_NT_USR          L"\\Device\\NemuDrvU"
#ifdef NEMU_WITH_HARDENING
/** Macro for checking for deflecting calls to the stub device. */
# define NEMUDRV_COMPLETE_IRP_AND_RETURN_IF_STUB_DEV(a_pDevObj, a_pIrp) \
 ...
 *  devices. */
# define NEMUDRV_COMPLETE_IRP_AND_RETURN_IF_STUB_OR_ERROR_INFO_DEV(a_pDevObj, a_pIrp) \
    do { if ((a_pDevObj) == g_pDevObjStub || (a_pDevObj) == g_pDevObjErrorInfo) \
            return supdrvNtCompleteRequest(STATUS_ACCESS_DENIED, a_pIrp); \
    } while (0)
#else
# define NEMUDRV_COMPLETE_IRP_AND_RETURN_IF_STUB_DEV(a_pDevObj, a_pIrp)                 do {} while (0)
# define NEMUDRV_COMPLETE_IRP_AND_RETURN_IF_STUB_OR_ERROR_INFO_DEV(a_pDevObj, a_pIrp)   do {} while (0)
#endif

/** Enables the fast I/O control code path. */
#define NEMUDRV_WITH_FAST_IO
...
*********************************************************************************************************************************/
/**
 * Device extension used by NemuDrvU.
 */
...
/** Pointer to the NemuDrvU device extension. */
...
/**
 * Device extension used by NemuDrvStub.
 */
...
/** Pointer to the NemuDrvStub device extension. */
...
/**
 * Device extension used by NemuDrvErrorInfo.
 */
...
/** Pointer to the NemuDrvErrorInfo device extension. */
...
/**
 * Error info for a failed NemuDrv or NemuDrvStub open attempt.
 */
...
typedef struct SUPDRVNTPROTECT
{
    ...
    HANDLE              hParentPid;
    /** The TID of the thread opening NemuDrv or NemuDrvStub, NULL if not opened. */
    HANDLE              hOpenTid;
...
#endif /* NEMU_WITH_HARDENINIG */


/*********************************************************************************************************************************
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static void     _stdcall   NemuDrvNtUnload(PDRIVER_OBJECT pDrvObj);
static NTSTATUS _stdcall   NemuDrvNtCreate(PDEVICE_OBJECT pDevObj, PIRP pIrp);
static NTSTATUS _stdcall   NemuDrvNtCleanup(PDEVICE_OBJECT pDevObj, PIRP pIrp);
static NTSTATUS _stdcall   NemuDrvNtClose(PDEVICE_OBJECT pDevObj, PIRP pIrp);
#ifdef NEMUDRV_WITH_FAST_IO
static BOOLEAN  _stdcall   NemuDrvNtFastIoDeviceControl(PFILE_OBJECT pFileObj, BOOLEAN fWait, PVOID pvInput, ULONG cbInput,
                                                        PVOID pvOutput, ULONG cbOutput, ULONG uCmd,
                                                        PIO_STATUS_BLOCK pIoStatus, PDEVICE_OBJECT pDevObj);
#endif
static NTSTATUS _stdcall   NemuDrvNtDeviceControl(PDEVICE_OBJECT pDevObj, PIRP pIrp);
static int                 NemuDrvNtDeviceControlSlow(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, PIRP pIrp, PIO_STACK_LOCATION pStack);
static NTSTATUS _stdcall   NemuDrvNtInternalDeviceControl(PDEVICE_OBJECT pDevObj, PIRP pIrp);
static VOID     _stdcall   NemuPowerDispatchCallback(PVOID pCallbackContext, PVOID pArgument1, PVOID pArgument2);
static NTSTATUS _stdcall   NemuDrvNtRead(PDEVICE_OBJECT pDevObj, PIRP pIrp);
static NTSTATUS _stdcall   NemuDrvNtNotSupportedStub(PDEVICE_OBJECT pDevObj, PIRP pIrp);
static NTSTATUS            NemuDrvNtErr2NtStatus(int rc);
#ifdef NEMU_WITH_HARDENING
...
static FAST_IO_DISPATCH const g_NemuDrvFastIoDispatch =
{
    /* .SizeOfFastIoDispatch            = */ sizeof(g_NemuDrvFastIoDispatch),
    ...
    /* .FastIoDeviceControl             = */ NemuDrvNtFastIoDeviceControl,
 ...
 */
static NTSTATUS nemudrvNtCreateDevices(PDRIVER_OBJECT pDrvObj)
{
    /*
     * System device.
     */
...
/**
 * Destroys the devices and links created by nemudrvNtCreateDevices.
 */
static void nemudrvNtDestroyDevices(void)
{
    if (g_pDevObjUsr)
    {
        PSUPDRVDEVEXTUSR pDevExtUsr = (PSUPDRVDEVEXTUSR)g_pDevObjUsr->DeviceExtension;
...
ULONG _stdcall DriverEntry(PDRIVER_OBJECT pDrvObj, PUNICODE_STRING pRegPath)
{
     ...
     */
#ifdef NEMUDRV_WITH_FAST_IO
    if (g_NemuDrvFastIoDispatch.FastIoDeviceControl != NemuDrvNtFastIoDeviceControl)
    {
        DbgPrint("NemuDrv: FastIoDeviceControl=%p instead of %p\n",
                 g_NemuDrvFastIoDispatch.FastIoDeviceControl, NemuDrvNtFastIoDeviceControl);
    ...
    if (RT_SUCCESS(vrc))
    {
...
#endif
        {
             ...
             */
            rcNt = nemudrvNtCreateDevices(pDrvObj);
            if (NT_SUCCESS(rcNt))
            {
                /*
                ...
                if (!vrc)
                {
                     ...
                     */
                    pDrvObj->DriverUnload                                   = NemuDrvNtUnload;
                    pDrvObj->MajorFunction[IRP_MJ_CREATE]                   = NemuDrvNtCreate;
                    pDrvObj->MajorFunction[IRP_MJ_CLEANUP]                  = NemuDrvNtCleanup;
                    pDrvObj->MajorFunction[IRP_MJ_CLOSE]                    = NemuDrvNtClose;
                    pDrvObj->MajorFunction[IRP_MJ_DEVICE_CONTROL]           = NemuDrvNtDeviceControl;
                    pDrvObj->MajorFunction[IRP_MJ_INTERNAL_DEVICE_CONTROL]  = NemuDrvNtInternalDeviceControl;
                    pDrvObj->MajorFunction[IRP_MJ_READ]                     = NemuDrvNtRead;
                    pDrvObj->MajorFunction[IRP_MJ_WRITE]                    = NemuDrvNtNotSupportedStub;

#ifdef NEMUDRV_WITH_FAST_IO
                    /* Fast I/O to speed up guest execution roundtrips. */
                    pDrvObj->FastIoDispatch = (PFAST_IO_DISPATCH)&g_NemuDrvFastIoDispatch;
#endif
                    ...
                    if (rcNt == STATUS_SUCCESS)
                        pDevExt->hPowerCallback = ExRegisterCallback(pDevExt->pObjPowerCallback,
                                                                     NemuPowerDispatchCallback,
                                                                     g_pDevObjSys);

                    /*
                     * Done! Returning success!
                     */
                    Log(("NemuDrv::DriverEntry returning STATUS_SUCCESS\n"));
                    return STATUS_SUCCESS;
                }

                Log(("supdrvInitDevExit failed with vrc=%d!\n", vrc));
                rcNt = NemuDrvNtErr2NtStatus(vrc);

                nemudrvNtDestroyDevices();
            }
#ifdef NEMU_WITH_HARDENING
            supdrvNtProtectTerm();
#endif
 ...
 */
void _stdcall NemuDrvNtUnload(PDRIVER_OBJECT pDrvObj)
{
    PSUPDRVDEVEXT pDevExt = (PSUPDRVDEVEXT)g_pDevObjSys->DeviceExtension;

    Log(("NemuDrvNtUnload at irql %d\n", KeGetCurrentIrql()));
 ...
 */
NTSTATUS _stdcall NemuDrvNtCreate(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    Log(("NemuDrvNtCreate: RequestorMode=%d\n", pIrp->RequestorMode));
     ...
     * NemuDrvNtInternalDeviceControl.  The first request will be one to
     * create a session.
     */
    NTSTATUS rcNt;
    ...
    {
#if defined(NEMU_WITH_HARDENING) && !defined(NEMU_WITHOUT_DEBUGGER_CHECKS)
        /*
        ...
        {
            LogRel(("nemudrv: Process %p is being debugged, access to nemudrv / nemudrvu declined.\n",
...
#endif
        {
            ...
            if (pDevObj == g_pDevObjStub)
            {
                ...
                LogRel(("nemudrv: Declined %p access to NemuDrvStub: rc=%d\n", PsGetProcessId(PsGetCurrentProcess()), rc));
            }
            /*
            ...
            else if (pDevObj != g_pDevObjUsr)
            {
                ...
                if (pNtProtect)
                {
                    if (pNtProtect->enmProcessKind == kSupDrvNtProtectKind_VmProcessUnconfirmed)
                    {
                        ...
                        if (RT_SUCCESS(rc))
                        {
                            ...
                            LogRel(("nemudrv: supdrvCreateSession failed for process %p: rc=%d.\n",
                                    PsGetProcessId(PsGetCurrentProcess()), rc));
                        }
                        else
                            LogRel(("nemudrv: Process %p failed process verification: rc=%d.\n",
                                    PsGetProcessId(PsGetCurrentProcess()), rc));
                    }
                    else
                    {
                        LogRel(("nemudrv: %p is not a budding VM process (enmProcessKind=%d).\n",
                ...
                {
                    LogRel(("nemudrv: %p is not a budding VM process.\n", PsGetProcessId(PsGetCurrentProcess())));
 ...
 * @param   pIrp        Request packet.
 */
NTSTATUS _stdcall NemuDrvNtCleanup(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    PSUPDRVDEVEXT       pDevExt  = SUPDRVNT_GET_DEVEXT(pDevObj);
    PIO_STACK_LOCATION  pStack   = IoGetCurrentIrpStackLocation(pIrp);
    PFILE_OBJECT        pFileObj = pStack->FileObject;
    ...
    if (pDevObj == g_pDevObjStub)
    {
        ...
        Log(("NemuDrvNtCleanup: pDevExt=%p pFileObj=%p pNtProtect=%p\n", pDevExt, pFileObj, pNtProtect));
        if (pNtProtect)
        {
            supdrvNtProtectRelease(pNtProtect);
            pFileObj->FsContext = NULL;
...
#endif
    {
        ...
        Log(("NemuDrvNtCleanup: pDevExt=%p pFileObj=%p pSession=%p\n", pDevExt, pFileObj, pSession));
        if (pSession)
        {
 ...
 * @param   pIrp        Request packet.
 */
NTSTATUS _stdcall NemuDrvNtClose(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    PSUPDRVDEVEXT       pDevExt  = SUPDRVNT_GET_DEVEXT(pDevObj);
    PIO_STACK_LOCATION  pStack   = IoGetCurrentIrpStackLocation(pIrp);
    PFILE_OBJECT        pFileObj = pStack->FileObject;
    ...
    {
        PSUPDRVNTPROTECT pNtProtect = (PSUPDRVNTPROTECT)pFileObj->FsContext;
        Log(("NemuDrvNtClose: pDevExt=%p pFileObj=%p pNtProtect=%p\n", pDevExt, pFileObj, pNtProtect));
        if (pNtProtect)
        {
            supdrvNtProtectRelease(pNtProtect);
            pFileObj->FsContext = NULL;
                                                     ...
                                                     PIO_STATUS_BLOCK pIoStatus, PDEVICE_OBJECT pDevObj)
{
    ...
    unsigned    cbOut = 0;
    int         rc = 0;
    Log2(("NemuDrvNtFastIoDeviceControl(%p): ioctl=%#x pvIn=%p cbIn=%#x pvOut=%p cbOut=%#x pSession=%p\n",
...
# endif
    {
        ...
        if ((uCmd & 0x3) == METHOD_BUFFERED)
        {
            ...
            if (NT_SUCCESS(rcNt))
            {
                    ...
                    && cbBuf < _1M*16)
                {
                    ...
                    PSUPREQHDR pHdr = (PSUPREQHDR)ExAllocatePoolWithTag(NonPagedPool, cbBuf, 'Nemu');
                    if (pHdr)
                    {
                        __try
                        {
                        ...
                        if (NT_SUCCESS(rcNt))
                        {
                            ...
                            if (RT_SUCCESS(rc))
                            {
                                ...
                                {
                                    cbOut = cbOutput;
                                    OSDBGPRINT(("NemuDrvNtFastIoDeviceControl: too much output! %#x > %#x; uCmd=%#x!\n",
                            ...
                            Log2(("NemuDrvNtFastIoDeviceControl: returns %#x cbOut=%d rc=%#x\n", rcNt, cbOut, rc));
                        }
                        else
                            Log(("NemuDrvNtFastIoDeviceControl: Error reading %u bytes of user memory at %p (uCmd=%#x)\n",
                                 cbInput, pvInput, uCmd));
                        ExFreePoolWithTag(pHdr, 'Nemu');
                    }
                    else
                ...
                {
                    Log(("NemuDrvNtFastIoDeviceControl: Mismatching sizes (%#x) - Hdr=%#lx/%#lx Irp=%#lx/%#lx!\n",
        ...
        {
            Log(("NemuDrvNtFastIoDeviceControl: not buffered request (%#x) - not supported\n", uCmd));
            rcNt = STATUS_NOT_SUPPORTED;
        }
    }
# ifdef RT_ARCH_AMD64
    ...
    {
        Log(("NemuDrvNtFastIoDeviceControl: WOW64 req - not supported\n"));
        rcNt = STATUS_NOT_SUPPORTED;
    }
# endif
 ...
 * @param   pIrp        Request packet.
 */
NTSTATUS _stdcall NemuDrvNtDeviceControl(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    NEMUDRV_COMPLETE_IRP_AND_RETURN_IF_STUB_OR_ERROR_INFO_DEV(pDevObj, pIrp);
    ...
    return NemuDrvNtDeviceControlSlow(pDevExt, pSession, pIrp, pStack);
}


/**
 * Worker for NemuDrvNtDeviceControl that takes the slow IOCtl functions.
 *
 * @returns NT status code.
 *
...
static int NemuDrvNtDeviceControlSlow(PSUPDRVDEVEXT pDevExt, PSUPDRVSESSION pSession, PIRP pIrp, PIO_STACK_LOCATION pStack)
{
    ...
    unsigned    cbOut = 0;
    int         rc = 0;
    Log2(("NemuDrvNtDeviceControlSlow(%p,%p): ioctl=%#x pBuf=%p cbIn=%#x cbOut=%#x pSession=%p\n",
...
#endif
    {
                ...
                &&  pStack->Parameters.DeviceIoControl.OutputBufferLength ==  pHdr->cbOut)
            {
                ...
                if (!rc)
                {
                    ...
                    if (cbOut > pStack->Parameters.DeviceIoControl.OutputBufferLength)
                    {
                        ...
                        OSDBGPRINT(("NemuDrvLinuxIOCtl: too much output! %#x > %#x; uCmd=%#x!\n",
                                    pHdr->cbOut, cbOut, pStack->Parameters.DeviceIoControl.IoControlCode));
                    }
                }
                else
                    rcNt = STATUS_INVALID_PARAMETER;
                Log2(("NemuDrvNtDeviceControlSlow: returns %#x cbOut=%d rc=%#x\n", rcNt, cbOut, rc));
            }
            else
            {
                Log(("NemuDrvNtDeviceControlSlow: Mismatching sizes (%#x) - Hdr=%#lx/%#lx Irp=%#lx/%#lx!\n",
        ...
        {
            Log(("NemuDrvNtDeviceControlSlow: not buffered request (%#x) - not supported\n",
                 pStack->Parameters.DeviceIoControl.IoControlCode));
            rcNt = STATUS_NOT_SUPPORTED;
        }
    }
    ...
    {
        Log(("NemuDrvNtDeviceControlSlow: WOW64 req - not supported\n"));
        rcNt = STATUS_NOT_SUPPORTED;
    }
#endif
 ...
 * @param   pIrp        Request packet.
 */
NTSTATUS _stdcall NemuDrvNtInternalDeviceControl(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    NEMUDRV_COMPLETE_IRP_AND_RETURN_IF_STUB_OR_ERROR_INFO_DEV(pDevObj, pIrp);
    ...
    unsigned            cbOut = 0;
    int                 rc = 0;
    Log2(("NemuDrvNtInternalDeviceControl(%p,%p): ioctl=%#x pBuf=%p cbIn=%#x cbOut=%#x pSession=%p\n",
          pDevExt, pIrp, pStack->Parameters.DeviceIoControl.IoControlCode,
          pIrp->AssociatedIrp.SystemBuffer, pStack->Parameters.DeviceIoControl.InputBufferLength,
          pStack->Parameters.DeviceIoControl.OutputBufferLength, pSession));
    ...
    if ((pStack->Parameters.DeviceIoControl.IoControlCode & 0x3) == METHOD_BUFFERED)
    {
           ...
           )
        {
                ...
                &&  pStack->Parameters.DeviceIoControl.OutputBufferLength == pHdr->cb)
            {
                ...
                Log2(("NemuDrvNtInternalDeviceControl: returns %#x/rc=%#x\n", rcNt, rc));
            }
            else
            {
                Log(("NemuDrvNtInternalDeviceControl: Mismatching sizes (%#x) - Hdr=%#lx Irp=%#lx/%#lx!\n",
                     pStack->Parameters.DeviceIoControl.IoControlCode,
    ...
    {
        Log(("NemuDrvNtInternalDeviceControl: not buffered request (%#x) - not supported\n",
             pStack->Parameters.DeviceIoControl.IoControlCode));
        rcNt = STATUS_NOT_SUPPORTED;
    }
...
/**
 * Implementation of the read major function for NemuDrvErrorInfo.
 *
 ...
 */
NTSTATUS _stdcall NemuDrvNtRead(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    Log(("NemuDrvNtRead\n"));
 ...
 * @param   pIrp        IRP.
 */
NTSTATUS _stdcall NemuDrvNtNotSupportedStub(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    Log(("NemuDrvNtNotSupportedStub\n"));
    NOREF(pDevObj);
 ...
 */
VOID _stdcall NemuPowerDispatchCallback(PVOID pCallbackContext, PVOID pArgument1, PVOID pArgument2)
{
    PDEVICE_OBJECT pDevObj = (PDEVICE_OBJECT)pCallbackContext;

    Log(("NemuPowerDispatchCallback: %x %x\n", pArgument1, pArgument2));

    /* Power change imminent? */
    if ((unsigned)pArgument1 == PO_CB_SYSTEM_STATE_LOCK)
    {
        if ((unsigned)pArgument2 == 0)
            Log(("NemuPowerDispatchCallback: about to go into suspend mode!\n"));
        else
            Log(("NemuPowerDispatchCallback: resumed!\n"));
...
# ifndef RT_ARCH_X86
#  error "NEMU_WITHOUT_NATIVE_R0_LOADER is only safe on x86."
# endif
    ...
    if (iDiff)
    {
        ...
        for (size_t off = uRva; cbLeft > 0; off++, cbLeft--)
            if (pbNativeBits[off] != pbImageBits[off])
            {
                ...
                SUPR0Printf("NemuDrv: %s", pReq->u.Out.szError);
                break;
            }
    }
    return iDiff;
...
void NEMUCALL   supdrvOSLdrUnload(PSUPDRVDEVEXT pDevExt, PSUPDRVLDRIMAGE pImage)
{
    if (pImage->pvNtSectionObj)
    {
        ...
        if (rcNt != STATUS_SUCCESS)
            SUPR0Printf("NemuDrv: failed to unload '%s', rcNt=%#x\n", pImage->szName, rcNt);
        pImage->pvNtSectionObj = NULL;
    }
    NOREF(pDevExt);
}
...
static bool supdrvNtProtectGetAlpcPortObjectType2(PCRTUTF16 pwszPortNm, POBJECT_TYPE *ppObjType)
{
    ...
    if (NT_SUCCESS(rcNt))
    {
        ...
        if (NT_SUCCESS(rcNt))
        {
            ...
            if (pObjType)
            {
                SUPR0Printf("nemudrv: ALPC Port Object Type %p (vs %p)\n", pObjType, *ppObjType);
                *ppObjType = pObjType;
                fDone = true;
            }
...
static POBJECT_TYPE supdrvNtProtectGetAlpcPortObjectType(uint32_t uSessionId, const char *pszSessionId)
{
        ...
        && g_pfnObGetObjectType)
    {
        ...
        rc |= RTUtf16CatAscii(wszPortNm, RT_ELEMENTS(wszPortNm), "\\NemuDrv-");
        ...
        if (!fDone)
        {
             ...
             */
            if (uSessionId == 0)
                rc  = RTUtf16CopyAscii(wszPortNm, RT_ELEMENTS(wszPortNm), "\\BaseNamedObjects\\NemuDrv-");
            else
            {
                rc  = RTUtf16CopyAscii(wszPortNm, RT_ELEMENTS(wszPortNm), "\\Sessions\\");
                rc |= RTUtf16CatAscii(wszPortNm, RT_ELEMENTS(wszPortNm), pszSessionId);
                rc |= RTUtf16CatAscii(wszPortNm, RT_ELEMENTS(wszPortNm), "\\BaseNamedObjects\\NemuDrv-");
            }
            cchTmp = RTStrFormatU32(szTmp, sizeof(szTmp), (uint32_t)(uintptr_t)PsGetProcessId(PsGetCurrentProcess()), 16, 0, 0, 0);
            Assert(cchTmp > 0);
            rc |= RTUtf16CatAscii(wszPortNm, RT_ELEMENTS(wszPortNm), szTmp);
...
/**
 * Called in the context of NemuDrvNtCreate to determin the CSRSS for the
 * current process.
 *
...
static int supdrvNtProtectFindAssociatedCsrss(PSUPDRVNTPROTECT pNtProtect)
{
    ...
    if (!NT_SUCCESS(rcNt))
    {
        SUPR0Printf("nemudrv: Error opening '%ls': %#x\n", wszApiPort, rcNt);
...
#endif /* NEMU_WITHOUT_DEBUGGER_CHECKS */
...
/**
 * Cleans up NemuDrv or NemuDrvStub error info not collected by the dead process.
 *
...
static void supdrvNtProtectVerifyNewChildProtection(PSUPDRVNTPROTECT pNtStub, PSUPDRVNTPROTECT pNtVm, bool fCallerChecks)
{
    ...
    LogRel(("nemudrv: Misdetected vm stub; hParentPid=%p hChildPid=%p\n", pNtStub->AvlCore.Key, pNtVm->AvlCore.Key));
...
supdrvNtProtectCallback_ProcessCreateNotifyEx(PEPROCESS pNewProcess, HANDLE hNewPid, PPS_CREATE_NOTIFY_INFO pInfo)
{
    ...
    if (pInfo)
    {
        ...
        Log(("nemudrv/NewProcessEx: ctx=%04zx/%p pid=%04zx ppid=%04zx ctor=%04zx/%04zx rcNt=%#x %.*ls\n",
    ...
    if (pNtProtect)
    {
        ...
        if (pOpInfo->Operation == OB_OPERATION_HANDLE_CREATE)
        {
            ...
            if ((PEPROCESS)pOpInfo->Object == PsGetCurrentProcess())
            {
                ...
                Log(("nemudrv/ProcessHandlePre: %sctx=%04zx/%p wants %#x to %p in pid=%04zx [%d] %s\n",
            ...
            else if (supdrvNtProtectIsWhitelistedDebugger(PsGetCurrentProcess()))
            {
                ...
                Log(("nemudrv/ProcessHandlePre: %sctx=%04zx/%p wants %#x to %p in pid=%04zx [%d] %s [debugger]\n",
                     pOpInfo->KernelHandle ? "k" : "", PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
                     pOpInfo->Parameters->CreateHandleInformation.DesiredAccess,
                     pOpInfo->Object, pNtProtect->AvlCore.Key, pNtProtect->enmProcessKind,
                     PsGetProcessImageFileName(PsGetCurrentProcess()) ));
            ...
            else
            {
                    ...
                    && supdrvNtProtectIsAssociatedCsrss(pNtProtect, PsGetCurrentProcess()) )
                {
                    ...
                    if (fDesiredAccess == s_fCsrssStupidDesires)
                    {
                        ...
                        fAllowedRights = fAllowedRights
                                       | PROCESS_VM_OPERATION
                                       | PROCESS_VM_WRITE
                                       | PROCESS_DUP_HANDLE /* Needed for CreateProcess/NemuTestOGL. */
                                       | 0;
                        pOpInfo->CallContext = NULL; /* don't assert this. */
                    }
                }
                ...
                Log(("nemudrv/ProcessHandlePre: %sctx=%04zx/%p wants %#x to %p/pid=%04zx [%d], allow %#x => %#x; %s [prev=%#x]\n",
                     pOpInfo->KernelHandle ? "k" : "", PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
        ...
        else
        {
            ...
            {
                Log(("nemudrv/ProcessHandlePre: ctx=%04zx/%p[%p] dup from %04zx/%p with %#x to %p in pid=%04zx [%d] %s\n",
                     PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
            ...
            else
            {
                    ...
                    && supdrvNtProtectIsAssociatedCsrss(pNtProtect, PsGetCurrentProcess()))
                {
                    if (ASMAtomicDecS32(&pNtProtect->cCsrssFirstProcessDuplicateHandle) >= 0)
                    {
                        ...
                        fAllowedRights = fAllowedRights
                                       | PROCESS_VM_OPERATION
                                       | PROCESS_VM_WRITE
                                       | PROCESS_DUP_HANDLE /* Needed for launching NemuTestOGL. */
                                       | 0;
                        pOpInfo->CallContext = NULL; /* don't assert this. */
                    }
                }
                ...
                Log(("nemudrv/ProcessHandlePre: %sctx=%04zx/%p[%p] dup from %04zx/%p with %#x to %p in pid=%04zx [%d] %s\n",
                     pOpInfo->KernelHandle ? "k" : "", PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
                     pOpInfo->Parameters->DuplicateHandleInformation.TargetProcess,
                     PsGetProcessId((PEPROCESS)pOpInfo->Parameters->DuplicateHandleInformation.SourceProcess),
                     pOpInfo->Parameters->DuplicateHandleInformation.SourceProcess,
...
supdrvNtProtectCallback_ThreadHandlePre(PVOID pvUser, POB_PRE_OPERATION_INFORMATION pOpInfo)
{
    ...
    if (pNtProtect)
    {
        ...
        if (pOpInfo->Operation == OB_OPERATION_HANDLE_CREATE)
        {
            ...
            {
                Log(("nemudrv/ThreadHandlePre: %sctx=%04zx/%p wants %#x to %p in pid=%04zx [%d] self\n",
                     pOpInfo->KernelHandle ? "k" : "", PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
                     pOpInfo->Parameters->CreateHandleInformation.DesiredAccess,
                     pOpInfo->Object, pNtProtect->AvlCore.Key, pNtProtect->enmProcessKind));
                pOpInfo->CallContext = NULL; /* don't assert */
            ...
            {
                Log(("nemudrv/ThreadHandlePre: %sctx=%04zx/%p wants %#x to %p in pid=%04zx [%d] %s [debugger]\n",
                     pOpInfo->KernelHandle ? "k" : "", PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
                     pOpInfo->Parameters->CreateHandleInformation.DesiredAccess,
                     pOpInfo->Object, pNtProtect->AvlCore.Key, pNtProtect->enmProcessKind,
                     PsGetProcessImageFileName(PsGetCurrentProcess()) ));
            ...
            else
            {
                ...
                /* Special case 2 on 8.1, possibly also Vista, 7, 8:
                   When creating a process like NemuTestOGL from the VM process,
                ...
                Log(("nemudrv/ThreadHandlePre: %sctx=%04zx/%p wants %#x to %p in pid=%04zx [%d], allow %#x => %#x; %s [prev=%#x]\n",
                     pOpInfo->KernelHandle ? "k" : "", PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
                     pOpInfo->Parameters->CreateHandleInformation.DesiredAccess,
        ...
        else
        {
            ...
            {
                Log(("nemudrv/ThreadHandlePre: %sctx=%04zx/%p[%p] dup from %04zx/%p with %#x to %p in pid=%04zx [%d] self\n",
                     pOpInfo->KernelHandle ? "k" : "", PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
                     pOpInfo->Parameters->DuplicateHandleInformation.TargetProcess,
                     PsGetProcessId((PEPROCESS)pOpInfo->Parameters->DuplicateHandleInformation.SourceProcess),
                     pOpInfo->Parameters->DuplicateHandleInformation.SourceProcess,
            ...
            else
            {
                ...
                Log(("nemudrv/ThreadHandlePre: %sctx=%04zx/%p[%p] dup from %04zx/%p with %#x to %p in pid=%04zx [%d], allow %#x => %#x; %s\n",
                     pOpInfo->KernelHandle ? "k" : "", PsGetProcessId(PsGetCurrentProcess()), PsGetCurrentProcess(),
                     pOpInfo->Parameters->DuplicateHandleInformation.TargetProcess,
                     PsGetProcessId((PEPROCESS)pOpInfo->Parameters->DuplicateHandleInformation.SourceProcess),
                     pOpInfo->Parameters->DuplicateHandleInformation.SourceProcess,
...
# endif
        ...
        {
            LogRel(("nemudrv: Found evil handle to budding VM process: pid=%p h=%p acc=%#x attr=%#x type=%s\n",
...
# ifndef NEMU_WITHOUT_DEBUGGER_CHECKS
...
# endif
    ...
    {
        LogRel(("nemudrv: Cannot locate ZwQueryVirtualMemory in ntoskrnl, nor were we able to cook up a replacement.\n"));
...
# ifdef NEMU_STRICT
    ...
    {
        LogRel(("nemudrv: g_pfnObGetObjectType=%p g_pfnZwAlpcCreatePort=%p.\n", g_pfnObGetObjectType, g_pfnZwAlpcCreatePort));
        return STATUS_PROCEDURE_NOT_FOUND;
    }
# endif
    ...
    if (RT_FAILURE(rc))
        return NemuDrvNtErr2NtStatus(rc);
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (NT_SUCCESS(rcNt))
            {
                ...
                if (g_pfnObRegisterCallbacks && g_pfnObUnRegisterCallbacks)
                {
                    ...
                    LogRel(("nemudrv: ObRegisterCallbacks failed with rcNt=%#x\n", rcNt));
                    g_pvObCallbacksCookie = NULL;
                }
                else
                {
...
# endif
                    {
                        DbgPrint("nemudrv: ObRegisterCallbacks was not found. Please make sure you got the latest updates and service packs installed\n");
                        rcNt = STATUS_SXS_VERSION_CONFLICT;
                    }
                    else
                    {
                        Log(("nemudrv: ObRegisterCallbacks was not found; ignored pre-Vista\n"));
                ...
                LogRel(("nemudrv: PsSetCreateProcessNotifyRoutine%s failed with rcNt=%#x\n",
