/* $Id: NemuServiceTimeSync.cpp $ */
/** @file
 * NemuService - Guest Additions TimeSync Service.
 */
...
/** @page pg_vgsvc_timesync     NemuService - The Time Sync Service
 *
...
static DECLCALLBACK(int) vgsvcTimeSyncPreInit(void)
{
    ...
    {
        rc = VGSvcReadPropUInt32(uGuestPropSvcClientID, "/VirtualBox/GuestAdd/NemuService/--timesync-interval",
                                 &g_TimeSyncInterval, 50, UINT32_MAX - 1);
        if (   RT_SUCCESS(rc)
            || rc == VERR_NOT_FOUND)
            rc = VGSvcReadPropUInt32(uGuestPropSvcClientID, "/VirtualBox/GuestAdd/NemuService/--timesync-min-adjust",
                                     &g_TimeSyncMinAdjust, 0, 3600000);
        if (   RT_SUCCESS(rc)
            || rc == VERR_NOT_FOUND)
            rc = VGSvcReadPropUInt32(uGuestPropSvcClientID, "/VirtualBox/GuestAdd/NemuService/--timesync-latency-factor",
                                     &g_TimeSyncLatencyFactor, 1, 1024);
        if (   RT_SUCCESS(rc)
            || rc == VERR_NOT_FOUND)
            rc = VGSvcReadPropUInt32(uGuestPropSvcClientID, "/VirtualBox/GuestAdd/NemuService/--timesync-max-latency",
                                     &g_TimeSyncMaxLatency, 1, 3600000);
        if (   RT_SUCCESS(rc)
            || rc == VERR_NOT_FOUND)
            rc = VGSvcReadPropUInt32(uGuestPropSvcClientID, "/VirtualBox/GuestAdd/NemuService/--timesync-set-threshold",
                                     &g_TimeSyncSetThreshold, 0, 7*24*60*60*1000 /* a week */);
        if (   RT_SUCCESS(rc)
            || rc == VERR_NOT_FOUND)
        {
            char *pszValue;
            rc = VGSvcReadProp(uGuestPropSvcClientID, "/VirtualBox/GuestAdd/NemuService/--timesync-set-start",
        ...
        {
            uint32_t value;
            rc = VGSvcReadPropUInt32(uGuestPropSvcClientID, "/VirtualBox/GuestAdd/NemuService/--timesync-set-on-restore",
                                     &value, 1, 1);
            if (RT_SUCCESS(rc))
