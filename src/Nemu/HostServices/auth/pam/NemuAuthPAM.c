 ...
 * environment variable NEMU_AUTH_PAM_SERVICE. If the variable
 * is not specified, then the 'login' PAM service is used.
 */
#define NEMU_AUTH_PAM_SERVICE_NAME_ENV_OLD "VRDP_AUTH_PAM_SERVICE"
#define NEMU_AUTH_PAM_SERVICE_NAME_ENV "NEMU_AUTH_PAM_SERVICE"
#define NEMU_AUTH_PAM_DEFAULT_SERVICE_NAME "login"


/* The debug log file name.
 *
 * If defined, debug messages will be written to the file specified in the
 * NEMU_AUTH_DEBUG_FILENAME (or deprecated VRDP_AUTH_DEBUG_FILENAME) environment
 * variable:
 *
 * export NEMU_AUTH_DEBUG_FILENAME=pam.log
 *
 * The above will cause writing to the pam.log.
 */
#define NEMU_AUTH_DEBUG_FILENAME_ENV_OLD "VRDP_AUTH_DEBUG_FILENAME"
#define NEMU_AUTH_DEBUG_FILENAME_ENV "NEMU_AUTH_DEBUG_FILENAME"
 ...
 * and does not harm.
 */
#define NEMU_AUTH_USE_PAM_DLLOAD


#ifdef NEMU_AUTH_USE_PAM_DLLOAD
...
# endif
#endif /* NEMU_AUTH_USE_PAM_DLLOAD */
...
{
#if defined(NEMU_AUTH_DEBUG_FILENAME_ENV) || defined(NEMU_AUTH_DEBUG_FILENAME_ENV_OLD)
    va_list va;
...
#if defined(NEMU_AUTH_DEBUG_FILENAME_ENV)
    filename = getenv (NEMU_AUTH_DEBUG_FILENAME_ENV);
#endif /* NEMU_AUTH_DEBUG_FILENAME_ENV */

#if defined(NEMU_AUTH_DEBUG_FILENAME_ENV_OLD)
    if (filename == NULL)
    {
        filename = getenv (NEMU_AUTH_DEBUG_FILENAME_ENV_OLD);
    }
#endif /* NEMU_AUTH_DEBUG_FILENAME_ENV_OLD */
    ...
    va_end (va);
#endif /* NEMU_AUTH_DEBUG_FILENAME_ENV || NEMU_AUTH_DEBUG_FILENAME_ENV_OLD */
}
...
{
    const char *service = getenv (NEMU_AUTH_PAM_SERVICE_NAME_ENV);

    if (service == NULL)
    {
        service = getenv (NEMU_AUTH_PAM_SERVICE_NAME_ENV_OLD);

        if (service == NULL)
        {
            service = NEMU_AUTH_PAM_DEFAULT_SERVICE_NAME;
        }
    }
        ...
        if (rc == PAM_SUCCESS)
        {
            ...
            if (rc == PAM_SUCCESS)
            {
                ...
                if (rc == PAM_AUTHINFO_UNAVAIL
                    &&
                    getenv("NEMU_PAM_ALLOW_INACTIVE") != NULL)
                {
