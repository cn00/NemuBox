...
typedef struct _NEMUDISPLAYCONTEXT
{
...
static NEMUDISPLAYCONTEXT gCtx = {0};

#ifdef NEMU_WITH_WDDM
typedef enum
{
    NEMUDISPLAY_DRIVER_TYPE_UNKNOWN = 0,
    NEMUDISPLAY_DRIVER_TYPE_XPDM    = 1,
    NEMUDISPLAY_DRIVER_TYPE_WDDM    = 2
} NEMUDISPLAY_DRIVER_TYPE;

static NEMUDISPLAY_DRIVER_TYPE getNemuDisplayDriverType (NEMUDISPLAYCONTEXT *pCtx);
#endif

int NemuDisplayInit(const NEMUSERVICEENV *pEnv, void **ppInstance, bool *pfStartThread)
{
    LogFlowFunc(("NemuDisplayInit ...\n"));
    ...
    else if (OSinfo.dwMajorVersion >= 5)        /* APIs available only on W2K and up! */
    {
        ...
        *(uintptr_t *)&gCtx.pfnEnumDisplayDevices = (uintptr_t)GetProcAddress(hUser, "EnumDisplayDevicesA");
        ...
        if (OSinfo.dwMajorVersion >= 6)
        {
            ...
            NEMUDISPLAY_DRIVER_TYPE enmType = getNemuDisplayDriverType (&gCtx);
            if (enmType == NEMUDISPLAY_DRIVER_TYPE_WDDM)
            {
                LogFlowFunc(("WDDM driver is installed, switching display driver if to WDDM mode\n"));
                /* this is hacky, but the most easiest way */
                NEMUDISPIF_MODE enmMode = (OSinfo.dwMajorVersion > 6 || OSinfo.dwMinorVersion > 0) ? NEMUDISPIF_MODE_WDDM_W7 : NEMUDISPIF_MODE_WDDM;
                DWORD err = NemuDispIfSwitchMode(const_cast<PNEMUDISPIF>(&pEnv->dispIf), enmMode, NULL /* old mode, we don't care about it */);
                if (err == NO_ERROR)
    ...
    NEMUDISPIFESCAPE_ISANYX IsAnyX = {0};
    IsAnyX.EscapeHdr.escapeCode = NEMUESC_ISANYX;
    DWORD err = NemuDispIfEscapeInOut(&pEnv->dispIf, &IsAnyX.EscapeHdr, sizeof (uint32_t));
    if (err == NO_ERROR)
...
static NEMUDISPLAY_DRIVER_TYPE getNemuDisplayDriverType(NEMUDISPLAYCONTEXT *pCtx)
#else
static bool isNemuDisplayDriverActive(NEMUDISPLAYCONTEXT *pCtx)
#endif
{
#ifdef NEMU_WITH_WDDM
    NEMUDISPLAY_DRIVER_TYPE enmType = NEMUDISPLAY_DRIVER_TYPE_UNKNOWN;
#else
    bool result = false;
#endif
    ...
    if( pCtx->pfnEnumDisplayDevices )
    {
        ...
        LogFlowFunc(("isNemuDisplayDriverActive: Checking for active Nemu display driver (W2K+) ...\n"));
        ...
        while (EnumDisplayDevices(NULL,
                                  ...
                                  0))
        {
            LogFlowFunc(("isNemuDisplayDriverActive: DevNum:%d\nName:%s\nString:%s\nID:%s\nKey:%s\nFlags=%08X\n\n",
                          devNum,
            ...
            {
                LogFlowFunc(("isNemuDisplayDriverActive: Primary device\n"));

                if (strcmp(&dispDevice.DeviceString[0], "VirtualBox Graphics Adapter") == 0)
#ifndef NEMU_WITH_WDDM
                    result = true;
#else
                    enmType = NEMUDISPLAY_DRIVER_TYPE_XPDM;
                 ...
                 * assume it to be WDDM */
                    ...
                    enmType = NEMUDISPLAY_DRIVER_TYPE_WDDM;
#endif
                break;
            }
    ...
    {
        LogFlowFunc(("isNemuDisplayDriverActive: Checking for active Nemu display driver (NT or older) ...\n"));
        ...
        if (strcmp((char*)&tempDevMode.dmDeviceName[0], "NemuDisp") == 0)
#ifndef NEMU_WITH_WDDM
            result = true;
#else
            enmType = NEMUDISPLAY_DRIVER_TYPE_XPDM;
                                    ...
                                    DWORD aBitsPerPixel, LONG aPosX, LONG aPosY, BOOL fEnabled, BOOL fExtDispSup)
{
    ...
    for (iter = 0; iter < totalDispNum; iter++)
    {
        ...
        {
            LogRel(("NemuTray:Initially disabling the monitor with id = %d . Total Monitor=%d\n", iter, totalDispNum));
...
static BOOL ResizeDisplayDevice(UINT Id, DWORD Width, DWORD Height, DWORD BitsPerPixel,
                                ...
                                NEMUDISPLAYCONTEXT *pCtx, BOOL fExtDispSup)
{
    ...
    NemuDispIfCancelPendingResize(&pCtx->pEnv->dispIf);
    ...
    DWORD DevNum = 0;
    DWORD DevPrimaryNum = 0;
    DWORD dwStatus = NemuGetDisplayConfig(NumDevices, &DevPrimaryNum, &DevNum, paDisplayDevices, paDeviceModes);
    if (dwStatus != NO_ERROR)
    {
        LogFlowFunc(("ResizeDisplayDevice: NemuGetDisplayConfig failed, %d\n", dwStatus));
        return dwStatus;
    }
...
#ifdef NEMU_WITH_WDDM
    NEMUDISPLAY_DRIVER_TYPE enmDriverType = getNemuDisplayDriverType (pCtx);
    if (enmDriverType == NEMUDISPLAY_DRIVER_TYPE_WDDM)
    {
        ...
        DWORD err = NemuDispIfResizeModes(&pCtx->pEnv->dispIf, Id, fEnabled, fExtDispSup, paDisplayDevices, paDeviceModes, DevNum);
        if (err == NO_ERROR || err != ERROR_RETRY)
        {
            if (err == NO_ERROR)
                LogFlowFunc(("NemuDispIfResizeModes succeeded\n"));
            else
               LogFlowFunc(("Failure NemuDispIfResizeModes (%d)\n", err));
            return FALSE;
        }
...
unsigned __stdcall NemuDisplayThread(void *pInstance)
{
    ...
    NEMUDISPLAYCONTEXT *pCtx = (NEMUDISPLAYCONTEXT *)pInstance;
    ...
    PostMessage(ghwndToolWindow, WM_NEMU_GRAPHICS_SUPPORTED, 0, 0);

    NemuDispIfResizeStarted(&pCtx->pEnv->dispIf);
    ...
    do
    {
        ...
        if (DeviceIoControl(gNemuDriver, NEMUGUEST_IOCTL_WAITEVENT, &waitEvent, sizeof(waitEvent), &waitEvent, sizeof(waitEvent), &cbReturned, NULL))
        {
            ...
            if (waitEvent.u32EventFlagsOut & VMMDEV_EVENT_DISPLAY_CHANGE_REQUEST)
            {
                ...
                fDisplayChangeQueried = DeviceIoControl(gNemuDriver, NEMUGUEST_IOCTL_VMMREQUEST(sizeof(VMMDevDisplayChangeRequestEx)), &displayChangeRequest, sizeof(VMMDevDisplayChangeRequestEx),
               ...
               if (!fDisplayChangeQueried)
               {
                    ...
                    displayChangeRequest.eventAck           = VMMDEV_EVENT_DISPLAY_CHANGE_REQUEST;
                    fDisplayChangeQueried = DeviceIoControl(gNemuDriver, NEMUGUEST_IOCTL_VMMREQUEST(sizeof(VMMDevDisplayChangeRequest2)), &displayChangeRequest, sizeof(VMMDevDisplayChangeRequest2),
                ...
                if (!fDisplayChangeQueried)
                {
                    ...
                    fExtDispSup = FALSE; /*Extended display Change request is not supported */
                    /* Try the old version of the request for old Nemu hosts. */
                    ...
                    displayChangeRequest.eventAck           = VMMDEV_EVENT_DISPLAY_CHANGE_REQUEST;
                    fDisplayChangeQueried = DeviceIoControl(gNemuDriver, NEMUGUEST_IOCTL_VMMREQUEST(sizeof(VMMDevDisplayChangeRequest)), &displayChangeRequest, sizeof(VMMDevDisplayChangeRequest),
                ...
                if (fDisplayChangeQueried)
                {
                    ...
                    for (;;)
                    {
                        ...
                        /*
                         * Only try to change video mode if the active display driver is Nemu additions.
                         */
#ifdef NEMU_WITH_WDDM
                        NEMUDISPLAY_DRIVER_TYPE enmDriverType = getNemuDisplayDriverType (pCtx);
                        ...
                        if (enmDriverType != NEMUDISPLAY_DRIVER_TYPE_UNKNOWN)
#else
                        if (isNemuDisplayDriverActive (pCtx))
#endif
                        {
                        ...
                        {
                            LogFlowFunc(("nemuDisplayDriver is not active\n"));
                        }
        ...
        else
        {
            ...
            LogFlowFunc(("error 0 from DeviceIoControl NEMUGUEST_IOCTL_WAITEVENT\n"));
#endif
    ...
    PostMessage(ghwndToolWindow, WM_NEMU_GRAPHICS_UNSUPPORTED, 0, 0);
