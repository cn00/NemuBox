...
#if   defined(NEMU_WITH_DTRACE) \
   && defined(IN_RING3) \
   && !defined(NEMU_DEVICE_STRUCT_TESTCASE)
# include "dtrace/NemuDD.h"
#else
# define NEMUDD_AHCI_REQ_SUBMIT(a,b,c,d)           do { } while (0)
# define NEMUDD_AHCI_REQ_SUBMIT_TIMESTAMP(a,b)     do { } while (0)
# define NEMUDD_AHCI_REQ_COMPLETED(a,b,c,d,e)      do { } while (0)
# define NEMUDD_AHCI_REQ_COMPLETED_TIMESTAMP(a,b)  do { } while (0)
#endif
...
#define AHCI_SAVED_STATE_VERSION_NEMU_30                2
        ...
        if (pAhciReq->enmTxDir == AHCITXDIR_READ)
        {
           Assert(cbTransfer <= pAhciReq->cbTransfer);
            ...
            if (pAhciReq->aATAPICmd[0] == SCSI_INQUIRY)
            {
                ...
                if (cbTransfer >= 8 + 8)
                    ataSCSIPadStr((uint8_t *)pvBuf + 8, "NEMU", 8);
                if (cbTransfer >= 16 + 16)
...
static bool ahciTransferComplete(PAHCIPort pAhciPort, PAHCIREQ pAhciReq, int rcReq)
{
    ...
    NEMUDD_AHCI_REQ_COMPLETED(pAhciReq, rcReq, enmTxState, pAhciReq->uOffset, pAhciReq->cbTransfer);
    NEMUDD_AHCI_REQ_COMPLETED_TIMESTAMP(pAhciReq, tsNow);
...
static bool ahciR3ReqSubmit(PAHCIPort pAhciPort, PAHCIREQ pAhciReq, AHCITXDIR enmTxDir)
{
    ...
    if (pAhciPort->fAsyncInterface)
    {
        NEMUDD_AHCI_REQ_SUBMIT(pAhciReq, pAhciReq->enmTxDir, pAhciReq->uOffset, pAhciReq->cbTransfer);
        NEMUDD_AHCI_REQ_SUBMIT_TIMESTAMP(pAhciReq, pAhciReq->tsStart);
...
static DECLCALLBACK(int) ahciR3LoadExec(PPDMDEVINS pDevIns, PSSMHANDLE pSSM, uint32_t uVersion, uint32_t uPass)
{
    ...
    if (   uVersion > AHCI_SAVED_STATE_VERSION
        || uVersion < AHCI_SAVED_STATE_VERSION_NEMU_30)
        return VERR_SSM_UNSUPPORTED_DATA_UNIT_VERSION;
    ...
    /* Verify config. */
    if (uVersion > AHCI_SAVED_STATE_VERSION_NEMU_30)
    {
        rc = SSMR3GetU32(pSSM, &u32);
        AssertRCReturn(rc, rc);
...
static int ahciR3VpdInit(PPDMDEVINS pDevIns, PAHCIPort pAhciPort, const char *pszName)
{
    ...
    rc = CFGMR3QueryStringDef(pCfgNode, "ModelNumber", pAhciPort->szModelNumber, sizeof(pAhciPort->szModelNumber),
                              pAhciPort->fATAPI ? "NEMU CD-ROM" : "NEMU HARDDISK");
    if (RT_FAILURE(rc))
    {
        if (rc == VERR_CFGM_NOT_ENOUGH_SPACE)
            return PDMDEV_SET_ERROR(pDevIns, VERR_INVALID_PARAMETER,
...
static DECLCALLBACK(int) ahciR3Construct(PPDMDEVINS pDevIns, int iInstance, PCFGMNODE pCfg)
{
    ...
    PCIDevSetStatus      (&pThis->dev, NEMU_PCI_STATUS_CAP_LIST);
    ...
    pThis->dev.config[0x70] = NEMU_PCI_CAP_ID_PM; /* Capability ID: PCI Power Management Interface */
