...
static inline void load_seg_cache_raw_dt(SegmentCache *sc, uint32_t e1, uint32_t e2)
{
    ...
    sc->newselector = 0;
    sc->fNemuFlags  = CPUMSELREG_FLAGS_VALID;
#endif
}
                       ...
                       uint32_t next_eip)
{
...
#ifndef NEMU    /* The old TSS is written first... */
    ...
    env->tr.fNemuFlags  = CPUMSELREG_FLAGS_VALID;
    ...
    env->ldt.fNemuFlags  = CPUMSELREG_FLAGS_VALID;
...
#ifdef NEMU /** @todo figure out when this is done one day... */
    if (!(e2 & DESC_A_MASK))
...
#ifdef NEMU /* See page 3-477 of 253666.pdf */
...
DECLINLINE(bool) is_vme_irq_redirected(int intno)
{
    ...
    if (!(env->tr.flags & DESC_P_MASK) ||
        ...
        goto fail;
    io_offset = lduw_kernel(env->tr.base + 0x66);
    /* Make sure the io bitmap offset is valid; anything less than sizeof(NEMUTSS) means there's none. */
...
{
# if defined(RT_OS_DARWIN) && defined(NEMU_STRICT)
    uintptr_t uSP;
#  ifdef RT_ARCH_AMD64
...
#ifdef NEMU /** @todo Why do we do this? */
    e1 = e2 = 0;
#endif
    if ((new_cs & 0xfffc) == 0)
        raise_exception_err(EXCP0D_GPF, 0);
    ...
    if (e2 & DESC_S_MASK) {
        ...
        case 12: /* 386 call gate */
            ...
            if (((e2 & DESC_C_MASK) && (dpl > cpl)) ||
                ...
                raise_exception_err(EXCP0D_GPF, gate_cs & 0xfffc);
            if (!(e2 & DESC_P_MASK))
#ifdef NEMU /* See page 3-514 of 253666.pdf */
    ...
    if (e2 & DESC_S_MASK) {
        ...
        if (!(e2 & DESC_C_MASK) && dpl < cpl) {
            ...
            if (!(ss_e2 & DESC_S_MASK) ||
                ...
                raise_exception_err(EXCP0A_TSS, ss & 0xfffc);
            if (!(ss_e2 & DESC_P_MASK))
#ifdef NEMU /* See page 3-99 of 253666.pdf */
                raise_exception_err(EXCP0C_STACK, ss & 0xfffc);
#else
                raise_exception_err(EXCP0A_TSS, ss & 0xfffc);
#endif
...
# ifndef NEMU /* The CPUMSetGuestMsr call below does this now. */
...
void helper_rdmsr(void)
{
    ...
    case MSR_VM_HSAVE_PA:
        val = env->vm_hsave;
        break;
# ifndef NEMU /* forward to CPUMQueryGuestMsr. */
...
target_ulong helper_lar(target_ulong selector1)
{
    ...
    CC_SRC = eflags | CC_Z;
#ifdef NEMU /* AMD says 0x00ffff00, while intel says 0x00fxff00. Bochs and IEM does like AMD says (x=f). */
...
#ifndef NEMU /* Unfortunately, we cannot handle isinf/isnan easily in wrapper */
...
#ifndef NEMU /* Unfortunately, we cannot easily handle isinf/isnan in wrapper */
    if (isinf(ST0) || isnan(ST0) || isnan(ST1) || (ST1 == 0.0)) {
#else
    if ((ST0 != ST0) || (ST1 != ST1) || (ST1 == 0.0)) {
#endif
...
void helper_fxrstor(target_ulong ptr, int data64)
{
    ...
    if (env->cr[4] & CR4_OSFXSR_MASK) {
        ...
        if (!(env->efer & MSR_EFER_FFXSR)
          ...
          || !(env->hflags & HF_LMA_MASK)) {
            for(i = 0; i < nb_xmm_regs; i++) {
#if !defined(NEMU) || __GNUC__ < 4
                env->xmm_regs[i].XMM_Q(0) = ldq(addr);
                env->xmm_regs[i].XMM_Q(1) = ldq(addr + 8);
#else /* NEMU + __GNUC__ >= 4: gcc 4.x compiler bug - it runs out of registers for the 64-bit value. */
# if 1
...
#if defined(NEMU) && defined(REM_PHYS_ADDR_IN_TLB)
...
#if defined(NEMU) && defined(GCC_WITH_BUGGY_REGPARM)
        int fake_ret;
