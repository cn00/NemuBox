...
def print_vars(vers, known, sep, bitness_magic):
    print "NEMU_PYTHON%s_INC=%s%s" %(vers, known[0], sep)
    if bitness_magic > 0:
        print "NEMU_PYTHON%s_LIB=%s%s" %(vers, known[2], sep)
        print "NEMU_PYTHON%s_LIB_X86=%s%s" %(vers, known[1], sep)
    else:
        print "NEMU_PYTHON%s_LIB=%s%s" %(vers, known[1], sep)
...
def main(argv):
    ...
    if target == 'darwin':
        ## @todo Pick up the locations from NEMU_PATH_MACOSX_SDK_10_*.
