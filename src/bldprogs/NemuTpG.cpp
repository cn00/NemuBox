/* $Id: NemuTpG.cpp $ */
/** @file
 * Nemu Build Tool - Nemu Tracepoint Generator.
 */
...
static enum
{
    kNemuTpGAction_Nothing,
    kNemuTpGAction_GenerateHeader,
    kNemuTpGAction_GenerateWrapperHeader,
    kNemuTpGAction_GenerateObject
}                           g_enmAction                 = kNemuTpGAction_Nothing;
...
static RTEXITCODE generateAssembly(PSCMSTREAM pStrm)
{
    ...
    RTListForEach(&g_ProviderHead, pProvider, VTGPROVIDER, ListEntry)
    {
        RTListForEach(&pProvider->ProbeHead, pProbe, VTGPROBE, ListEntry)
        {
            ScmStreamPrintf(pStrm,
                            "\n"
                            "VTG_GLOBAL VTGProbeStub_%s_%s, function; (NEMUTPGPROBELOC pVTGProbeLoc",
                            pProvider->pszName, pProbe->pszMangledName);
            RTListForEach(&pProbe->ArgHead, pArg, VTGARG, ListEntry)
            {
...
#define MY_STRMATCH(a_sz)  (cchType == sizeof(a_sz) - 1 && !memcmp(a_sz, pszType, sizeof(a_sz) - 1))
    ...
    RTMsgError("Don't know '%s' - please change or fix NemuTpG", pszType);
    g_cTypeErrors++;
            ...
            )
        {
            ...
            else
            {
                ...
                if (!strcmp(pArg->pszTracerType, "struct CPUMCTX *")|| !strcmp(pArg->pszTracerType, "PCPUMCTX"))
                    ...
                    pArg->pszArgPassingFmt = ", NemuTpG-Is-Buggy!!";
            }
        }
        else
        {
...
static RTEXITCODE parseArguments(int argc,  char **argv)
{
    ...
    enum
    {
        kNemuTpGOpt_32Bit = 1000,
        kNemuTpGOpt_64Bit,
        kNemuTpGOpt_GenerateWrapperHeader,
        kNemuTpGOpt_Assembler,
        kNemuTpGOpt_AssemblerFmtOpt,
        kNemuTpGOpt_AssemblerFmtVal,
        kNemuTpGOpt_AssemblerOutputOpt,
        kNemuTpGOpt_AssemblerOption,
        kNemuTpGOpt_Pic,
        kNemuTpGOpt_ProbeFnName,
        kNemuTpGOpt_ProbeFnImported,
        kNemuTpGOpt_ProbeFnNotImported,
        kNemuTpGOpt_Host32Bit,
        kNemuTpGOpt_Host64Bit,
        kNemuTpGOpt_RawModeContext,
        kNemuTpGOpt_Ring0Context,
        kNemuTpGOpt_Ring0ContextAgnostic,
        kNemuTpGOpt_Ring3Context,
        kNemuTpGOpt_End
    };
    ...
    static RTGETOPTDEF const s_aOpts[] =
    {
        ...
        { "-32",                                kNemuTpGOpt_32Bit,                      RTGETOPT_REQ_NOTHING },
        { "-64",                                kNemuTpGOpt_64Bit,                      RTGETOPT_REQ_NOTHING },
        ...
        /* our stuff */
        { "--generate-wrapper-header",          kNemuTpGOpt_GenerateWrapperHeader,      RTGETOPT_REQ_NOTHING },
        { "--assembler",                        kNemuTpGOpt_Assembler,                  RTGETOPT_REQ_STRING  },
        { "--assembler-fmt-opt",                kNemuTpGOpt_AssemblerFmtOpt,            RTGETOPT_REQ_STRING  },
        { "--assembler-fmt-val",                kNemuTpGOpt_AssemblerFmtVal,            RTGETOPT_REQ_STRING  },
        { "--assembler-output-opt",             kNemuTpGOpt_AssemblerOutputOpt,         RTGETOPT_REQ_STRING  },
        { "--assembler-option",                 kNemuTpGOpt_AssemblerOption,            RTGETOPT_REQ_STRING  },
        { "--pic",                              kNemuTpGOpt_Pic,                        RTGETOPT_REQ_NOTHING },
        { "--probe-fn-name",                    kNemuTpGOpt_ProbeFnName,                RTGETOPT_REQ_STRING  },
        { "--probe-fn-imported",                kNemuTpGOpt_ProbeFnImported,            RTGETOPT_REQ_NOTHING },
        { "--probe-fn-not-imported",            kNemuTpGOpt_ProbeFnNotImported,         RTGETOPT_REQ_NOTHING },
        { "--host-32-bit",                      kNemuTpGOpt_Host32Bit,                  RTGETOPT_REQ_NOTHING },
        { "--host-64-bit",                      kNemuTpGOpt_Host64Bit,                  RTGETOPT_REQ_NOTHING },
        { "--raw-mode-context",                 kNemuTpGOpt_RawModeContext,             RTGETOPT_REQ_NOTHING },
        { "--ring-0-context",                   kNemuTpGOpt_Ring0Context,               RTGETOPT_REQ_NOTHING },
        { "--ring-0-context-agnostic",          kNemuTpGOpt_Ring0ContextAgnostic,       RTGETOPT_REQ_NOTHING },
        { "--ring-3-context",                   kNemuTpGOpt_Ring3Context,               RTGETOPT_REQ_NOTHING },
    ...
    while ((rc = RTGetOpt(&GetOptState, &ValueUnion)) != 0)
    {
        switch (rc)
        {
            ...
            case 'G':
                if (   g_enmAction != kNemuTpGAction_Nothing
                    && g_enmAction != kNemuTpGAction_GenerateObject)
            ...
            case 'h':
                ...
                {
                    if (   g_enmAction != kNemuTpGAction_Nothing
                        && g_enmAction != kNemuTpGAction_GenerateHeader)
            ...
            case VINF_GETOPT_NOT_OPTION:
                if (g_enmAction == kNemuTpGAction_GenerateObject)
             ...
             * Our options.
             */
            case kNemuTpGOpt_GenerateWrapperHeader:
                if (   g_enmAction != kNemuTpGAction_Nothing
                    && g_enmAction != kNemuTpGAction_GenerateWrapperHeader)
                    return RTMsgErrorExit(RTEXITCODE_SYNTAX, "--generate-wrapper-header does not mix with -h or -G");
                g_enmAction = kNemuTpGAction_GenerateWrapperHeader;
                break;

            case kNemuTpGOpt_Assembler:
                g_pszAssembler = ValueUnion.psz;
                break;

            case kNemuTpGOpt_AssemblerFmtOpt:
                g_pszAssemblerFmtOpt = ValueUnion.psz;
                break;

            case kNemuTpGOpt_AssemblerFmtVal:
                g_pszAssemblerFmtVal = ValueUnion.psz;
                break;

            case kNemuTpGOpt_AssemblerOutputOpt:
                g_pszAssemblerOutputOpt = ValueUnion.psz;
                break;

            case kNemuTpGOpt_AssemblerOption:
            ...
            case kNemuTpGOpt_Pic:
                g_fPic = true;
                break;

            case kNemuTpGOpt_ProbeFnName:
                g_pszProbeFnName = ValueUnion.psz;
                break;

            case kNemuTpGOpt_ProbeFnImported:
                g_fProbeFnImported = true;
                break;

            case kNemuTpGOpt_ProbeFnNotImported:
                g_fProbeFnImported = false;
                break;

            case kNemuTpGOpt_Host32Bit:
                g_cHostBits = 32;
                break;

            case kNemuTpGOpt_Host64Bit:
                g_cHostBits = 64;
                break;

            case kNemuTpGOpt_RawModeContext:
                g_fTypeContext = VTG_TYPE_CTX_RC;
                g_pszContextDefine = "IN_RC";
                g_pszContextDefine2 = NULL;
                break;

            case kNemuTpGOpt_Ring0Context:
                g_fTypeContext = VTG_TYPE_CTX_R0;
                g_pszContextDefine = "IN_RING0";
                g_pszContextDefine2 = NULL;
                break;

            case kNemuTpGOpt_Ring0ContextAgnostic:
                g_fTypeContext = VTG_TYPE_CTX_R0;
                g_pszContextDefine = "IN_RING0_AGNOSTIC";
                g_pszContextDefine2 = "IN_RING0";
                break;

            case kNemuTpGOpt_Ring3Context:
...
int main(int argc, char **argv)
{
    ...
    if (rcExit == RTEXITCODE_SUCCESS)
    {
        ...
        if (rcExit == RTEXITCODE_SUCCESS)
        {
             ...
             * Take action.
             */
            if (g_enmAction == kNemuTpGAction_GenerateHeader)
                rcExit = generateFile(g_pszOutput, "header", generateHeader);
            else if (g_enmAction == kNemuTpGAction_GenerateWrapperHeader)
