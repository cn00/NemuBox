...
HRESULT SystemProperties::setExclusiveHwVirt(BOOL aExclusiveHwVirt)
{
    ...
    // VirtualBox::i_saveSettings() needs nemu write lock
...
HRESULT SystemProperties::getLoggingLevel(com::Utf8Str &aLoggingLevel)
{
    ...
    if (aLoggingLevel.isEmpty())
        aLoggingLevel = NEMUSVC_LOG_DEFAULT;
...
HRESULT SystemProperties::setDefaultVRDEExtPack(const com::Utf8Str &aExtPack)
{
    ...
    {
        if (aExtPack.equals(NEMUVRDP_KLUDGE_EXTPACK_NAME))
            hrc = S_OK;
        else
#ifdef NEMU_WITH_EXTPACK
...
HRESULT SystemProperties::i_setLoggingLevel(const com::Utf8Str &aLoggingLevel)
{
    ...
    if (useLoggingLevel.isEmpty())
        useLoggingLevel = NEMUSVC_LOG_DEFAULT;
    ...
    if (RT_FAILURE(rc))
    {
        ...
        if (!useLoggingLevel.equals(NEMUSVC_LOG_DEFAULT))
        {
            rc = RTLogGroupSettings(RTLogRelGetDefaultInstance(), NEMUSVC_LOG_DEFAULT);
    ...
    m->strLoggingLevel = (useLoggingLevel.equals(NEMUSVC_LOG_DEFAULT) ? "" : useLoggingLevel);
...
HRESULT SystemProperties::i_setWebServiceAuthLibrary(const com::Utf8Str &aPath)
{
    if (!aPath.isEmpty())
        ...
        m->strWebServiceAuthLibrary = "NemuAuth";
...
HRESULT SystemProperties::i_setDefaultAdditionsISO(const com::Utf8Str &aPath)
{
    ...
    if (path.isEmpty())
    {
        ...
        AssertRC(vrc);
        Utf8Str strSrc1 = Utf8Str(strTemp).append("/NemuGuestAdditions.iso");

        vrc = RTPathExecDir(strTemp, sizeof(strTemp));
        AssertRC(vrc);
        Utf8Str strSrc2 = Utf8Str(strTemp).append("/additions/NemuGuestAdditions.iso");

        vrc = RTPathUserHome(strTemp, sizeof(strTemp));
        AssertRC(vrc);
        Utf8Str strSrc3 = Utf8StrFmt("%s/NemuGuestAdditions_%s.iso", strTemp, VirtualBox::i_getVersionNormalized().c_str());
