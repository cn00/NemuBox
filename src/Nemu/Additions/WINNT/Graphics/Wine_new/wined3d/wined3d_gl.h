...
HGLRC (WINAPI *pNemuCreateContext)(HDC, struct NEMUUHGSMI*) DECLSPEC_HIDDEN;
void (WINAPI *pNemuCtxChromiumParameteriCR)(HGLRC , GLenum , GLint) DECLSPEC_HIDDEN;
void (WINAPI *pNemuFlushToHost)(HGLRC) DECLSPEC_HIDDEN;
GLint (WINAPI *pNemuGetWindowId)(HDC)  DECLSPEC_HIDDEN;
GLint (WINAPI *pNemuGetContextId)(HGLRC)  DECLSPEC_HIDDEN;


#define NEMU_USE_GL_FUNC(pfn) p##pfn = (void *)wglGetProcAddress(#pfn);

# define NEMU_GL_FUNCS_GEN \
        NEMU_USE_FUNC(NemuCreateContext) \
        NEMU_USE_FUNC(NemuCtxChromiumParameteriCR) \
        NEMU_USE_FUNC(NemuFlushToHost) \
        NEMU_USE_FUNC(NemuGetWindowId) \
        NEMU_USE_FUNC(NemuGetContextId)

# define NEMU_GL_EXT_FUNCS_GEN \
        NEMU_USE_GL_FUNC(glChromiumParameteriCR)
