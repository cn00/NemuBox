/* $Id: NemuNetFltInstall.cpp $ */
/** @file
 * NetFltInstall - NemuNetFlt installer command line tool
 */
...
#define NETFLT_ID L"sun_NemuNetFlt"
#define NEMU_NETCFG_APP_NAME L"NetFltInstall"
#define NEMU_NETFLT_PT_INF L".\\NemuNetFlt.inf"
#define NEMU_NETFLT_MP_INF L".\\NemuNetFltM.inf"
#define NEMU_NETFLT_RETRIES 10
...
static int NemuNetFltInstall()
{
    ...
    if(hr == S_OK)
    {
        ...
        do
        {
            hr = NemuNetCfgWinQueryINetCfg(&pnc, TRUE, NEMU_NETCFG_APP_NAME, 10000, &lpszLockedBy);
            if(hr == S_OK)
            {
                DWORD dwSize;
                dwSize = MyGetfullPathNameW(NEMU_NETFLT_PT_INF, sizeof(PtInf)/sizeof(PtInf[0]), PtInf);
                if(dwSize > 0)
                {
                    /** @todo add size check for (sizeof(PtInf)/sizeof(PtInf[0])) == dwSize (string length in sizeof(PtInf[0])) */

                    dwSize = MyGetfullPathNameW(NEMU_NETFLT_MP_INF, sizeof(MpInf)/sizeof(MpInf[0]), MpInf);
                    if(dwSize > 0)
                    {
                        /** @todo add size check for (sizeof(MpInf)/sizeof(MpInf[0])) == dwSize (string length in sizeof(MpInf[0])) */

                        LPCWSTR aInfs[] = {PtInf, MpInf};
                        hr = NemuNetCfgWinNetFltInstall(pnc, aInfs, 2);
                        if(hr == S_OK)
                        {
                        ...
                        {
                            wprintf(L"error installing NemuNetFlt (0x%x)\n", hr);
                        }
                    }
                    else
                    {
                        hr = HRESULT_FROM_WIN32(GetLastError());
                        wprintf(L"error getting full inf path for NemuNetFltM.inf (0x%x)\n", hr);
                    }
                }
                else
                {
                    hr = HRESULT_FROM_WIN32(GetLastError());
                    wprintf(L"error getting full inf path for NemuNetFlt.inf (0x%x)\n", hr);
                }
            ...
            {
                if(i < NEMU_NETFLT_RETRIES && !wcscmp(lpszLockedBy, L"6to4svc.dll"))
                {
                    wprintf(L"6to4svc.dll is holding the lock, retrying %d out of %d\n", ++i, NEMU_NETFLT_RETRIES);
