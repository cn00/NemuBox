...
#define NEMU_NO_MESA_PATCH_REPORTS
...
static struct _glapi_table* nemu_glapi_table = NULL;
...
#define NEMU_SET_MESA_FUNC(table, name, func) \
...
#define GLAPI_ENTRY(Func) NEMU_SET_MESA_FUNC(nemu_glapi_table, "gl"#Func, cr_gl##Func);

static void
nemuPatchMesaExport(const char* psFuncName, const void *pStart, const void *pEnd);

static void
nemuPatchMesaGLAPITable()
{
    void *pGLTable;

    pGLTable = (void *)_glapi_get_dispatch();
    nemu_glapi_table = crAlloc(_glapi_get_dispatch_table_size() * sizeof (void *));
    if (!nemu_glapi_table)
    {
        crError("Not enough memory to allocate dispatch table");
    }
    crMemcpy(nemu_glapi_table, pGLTable, _glapi_get_dispatch_table_size() * sizeof (void *));

    #include "fakedri_glfuncsList.h"

    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glBlendEquationSeparateEXT", cr_glBlendEquationSeparate);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glSampleMaskSGIS", cr_glSampleMaskEXT);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glSamplePatternSGIS", cr_glSamplePatternEXT);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glWindowPos2dMESA", cr_glWindowPos2d);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glWindowPos2dvMESA", cr_glWindowPos2dv);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glWindowPos2fMESA", cr_glWindowPos2f);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glWindowPos2fvMESA", cr_glWindowPos2fv);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glWindowPos2iMESA", cr_glWindowPos2i);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glWindowPos2ivMESA", cr_glWindowPos2iv);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glWindowPos2sMESA", cr_glWindowPos2s);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glWindowPos2svMESA", cr_glWindowPos2sv);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glWindowPos3dMESA", cr_glWindowPos3d);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glWindowPos3dvMESA", cr_glWindowPos3dv);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glWindowPos3fMESA", cr_glWindowPos3f);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glWindowPos3fvMESA", cr_glWindowPos3fv);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glWindowPos3iMESA", cr_glWindowPos3i);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glWindowPos3ivMESA", cr_glWindowPos3iv);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glWindowPos3sMESA", cr_glWindowPos3s);
    NEMU_SET_MESA_FUNC(nemu_glapi_table, "glWindowPos3svMESA", cr_glWindowPos3sv);

    _glapi_set_dispatch(nemu_glapi_table);
};
#undef GLAPI_ENTRY

#define GLXAPI_ENTRY(Func) pGLXTable->Func = NEMUGLXTAG(glX##Func);
static void
nemuFillGLXAPITable(fakedri_glxapi_table *pGLXTable)
{
...
static void
nemuApplyPatch(const char* psFuncName, void *pDst, const void *pSrc, unsigned long size)
{
    void *alPatch;
    int rv;
...
#ifndef NEMU_NO_MESA_PATCH_REPORTS
...
nemuPatchMesaExport(const char* psFuncName, const void *pStart, const void *pEnd)
{
...
#ifndef NEMU_NO_MESA_PATCH_REPORTS
    crDebug("\nnemuPatchMesaExport: %s", psFuncName);
#endif
...
#if NEMU_OGL_GLX_USE_CSTUBS
    {
        ...
        if (!rv || !sym1)
        {
            crError("Failed to get size for nemu %p", pStart);
            return;
        }

        pEnd = pStart + sym1->st_size;
# ifndef NEMU_NO_MESA_PATCH_REPORTS
        crDebug("Nemu Entry: %p, start: %p(%s:%s), size: %li", pStart, dlip1.dli_saddr, dlip1.dli_fname, dlip1.dli_sname, sym1->st_size);
# endif
    }
#endif
    ...
    crDebug("Nemu code: start: %p, end %p, size: %li", pStart, pEnd, pEnd-pStart);
#endif
...
# endif
    ...
    nemuApplyPatch(psFuncName, dlip.dli_saddr, pStart, pEnd-pStart);
...
nemuRepatchMesaExports(void)
{
    ...
    {
# ifndef NEMU_NO_MESA_PATCH_REPORTS
        crDebug("\nnemuRepatchMesaExports %s", pPatchNode->psFuncName);
# endif
...
# endif
        nemuApplyPatch(pPatchNode->psFuncName, pPatchNode->pDstStart, &patch[0], 5);
        ...
        patch[12] = 0xE3;
# ifndef NEMU_NO_MESA_PATCH_REPORTS
        crDebug("Adding jmp from mesa %s+%#lx to nemu %s", pFreeNode->psFuncName, pFreeNode->pDstStart-pFreeNode->pSrcStart,
                pPatchNode->psFuncName);
# endif
        nemuApplyPatch(pFreeNode->psFuncName, pFreeNode->pDstStart, &patch[0], FAKEDRI_JMP64_PATCH_SIZE);
...
static void
nemuFakeDriFreeList(FAKEDRI_PatchNode *pList)
{
    FAKEDRI_PatchNode *pNode;
...
static void
# define GLXAPI_ENTRY(Func) nemuPatchMesaExport("glX"#Func, &nemu_glX##Func, NULL);
nemuPatchMesaExports()
#else
static void
# define GLXAPI_ENTRY(Func) nemuPatchMesaExport("glX"#Func, &nemu_glX##Func, &nemu_glX##Func##_EndProc);
nemuPatchMesaExports()
#endif
{
...
#ifdef RT_ARCH_AMD64
    nemuRepatchMesaExports();
    nemuFakeDriFreeList(g_pRepatchList);
    g_pRepatchList = NULL;
    nemuFakeDriFreeList(g_pFreeList);
    g_pFreeList = NULL;
#endif
}
#undef GLXAPI_ENTRY

bool nemu_load_sw_dri()
{
...
void __attribute__ ((constructor)) nemu_install_into_mesa(void)
{
    {
        ...
        if (pxf86Msg)
        {
            pxf86Msg(X_INFO, "Next line is added to allow nemuvideo_drv.so to appear as whitelisted driver\n");
    ...
    if (!nemu_load_sw_dri())
    {
        crDebug("nemudriInitScreen: nemu_load_sw_dri failed...going to fail badly");
        return;
    }

    /* Handle gl api.
     * In the end application call would look like this:
     * app call glFoo->(mesa asm dispatch stub)->cr_glFoo(nemu asm dispatch stub)->SPU Foo function(packspuFoo or alike)
    ...
    /* Handle glx api.
     * In the end application call would look like this:
     * app call glxFoo->(mesa asm dispatch stub patched with nemu_glXFoo:jmp glxim[Foo's index])->NEMUGLXTAG(glxFoo)
     */
    /* Fill structure used by our assembly stubs */
    nemuFillGLXAPITable(&glxim);
...
static __DRIscreen *
nemudriCreateNewScreen(int screen, int fd, unsigned int sarea_handle,
...
static void 
nemudriDestroyScreen(__DRIscreen *screen)
{
    SWDRI_SAFECALL_CORE(destroyScreen, screen);
}

static const __DRIextension **
nemudriGetExtensions(__DRIscreen *screen)
{
    SWDRI_SAFERET_CORE(getExtensions, screen);
}

static int
nemudriGetConfigAttrib(const __DRIconfig *config,
                       unsigned int attrib,
                       unsigned int *value)
{
...
static int
nemudriIndexConfigAttrib(const __DRIconfig *config, int index,
...
nemudriCreateNewDrawable(__DRIscreen *screen,
...
static void 
nemudriDestroyDrawable(__DRIdrawable *drawable)
{
    SWDRI_SAFECALL_CORE(destroyDrawable, drawable);
}

static void
nemudriSwapBuffers(__DRIdrawable *drawable)
{
    SWDRI_SAFECALL_CORE(swapBuffers, drawable);
}

static __DRIcontext *
nemudriCreateNewContext(__DRIscreen *screen,
                        const __DRIconfig *config,
                        __DRIcontext *shared,
                        void *loaderPrivate)
{
...
static int 
nemudriCopyContext(__DRIcontext *dest,
                   __DRIcontext *src,
                   unsigned long mask)
{
...
static void 
nemudriDestroyContext(__DRIcontext *context)
{
    SWDRI_SAFECALL_CORE(destroyContext, context);
}

static int 
nemudriBindContext(__DRIcontext *ctx,
                   __DRIdrawable *pdraw,
                   __DRIdrawable *pread)
{
...
static int 
nemudriUnbindContext(__DRIcontext *ctx)
{
...
static __DRIscreen *
nemudriCreateNewScreen_Legacy(int scrn,
...
static __DRIdrawable *
nemudriCreateNewDrawable_Legacy(__DRIscreen *psp, const __DRIconfig *config,
...
static __DRIcontext *
nemudriCreateNewContext_Legacy(__DRIscreen *psp, const __DRIconfig *config,
                               int render_type, __DRIcontext *shared, 
                               drm_context_t hwContext, void *data)
{
    (void) render_type;
    (void) hwContext;
    return nemudriCreateNewContext(psp, config, shared, data);
}


static const __DRIlegacyExtension nemudriLegacyExtension = {
    { __DRI_LEGACY, __DRI_LEGACY_VERSION },
    nemudriCreateNewScreen_Legacy,
    nemudriCreateNewDrawable_Legacy,
    nemudriCreateNewContext_Legacy
};

static const __DRIcoreExtension nemudriCoreExtension = {
    { __DRI_CORE, __DRI_CORE_VERSION },
    nemudriCreateNewScreen, /* driCreateNewScreen */
    nemudriDestroyScreen,
    nemudriGetExtensions,
    nemudriGetConfigAttrib,
    nemudriIndexConfigAttrib,
    nemudriCreateNewDrawable, /* driCreateNewDrawable */
    nemudriDestroyDrawable,
    nemudriSwapBuffers,
    nemudriCreateNewContext,
    nemudriCopyContext,
    nemudriDestroyContext,
    nemudriBindContext,
    nemudriUnbindContext
};

/* This structure is used by dri_util from mesa, don't rename it! */
DECLEXPORT(const __DRIextension *) __driDriverExtensions[] = {
    &nemudriLegacyExtension.base,
    &nemudriCoreExtension.base,
    NULL
};
