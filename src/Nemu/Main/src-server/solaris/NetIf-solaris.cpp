...
/** @todo Unify this define with NemuNetFltBow-solaris.c */
...
static void nemuSolarisAddHostIface(char *pszIface, int Instance, void *pvHostNetworkInterfaceList)
{
    ...
    if (SolarisNICMap.empty())
    {
        ...
        SolarisNICMap.insert(NICPair("nemunet", "VirtualBox Host Ethernet"));
        SolarisNICMap.insert(NICPair(NEMUBOW_VNIC_TEMPLATE_NAME, "VirtualBox VNIC Template"));
    ...
    if (Description != "VirtualBox Host Ethernet")
    {
        ...
        else if (!strncmp(szNICInstance, RT_STR_TUPLE(NEMUBOW_VNIC_TEMPLATE_NAME)))
        {
            /*
             * We want prefix matching only for "nemuvnic_template" as it's possible to create "nemuvnic_template_abcd123",
             * which our Solaris Crossbow NetFilter driver will interpret as a VNIC template.
             */
            Description = SolarisNICMap[NEMUBOW_VNIC_TEMPLATE_NAME];
            RTStrPrintf(szNICDesc, sizeof(szNICDesc), "%s - %s", szNICInstance, Description.c_str());
        }
        else
    ...
    if (strncmp(szNICInstance, RT_STR_TUPLE("nemunet")))
...
static boolean_t nemuSolarisAddLinkHostIface(const char *pszIface, void *pvHostNetworkInterfaceList)
{
    /*
     ...
     * These names originate from NemuNetFltBow-solaris.c, hardcoded here for now.
     *                                                                           .
     * ASSUMES template name is longer than 'nemuvnic'.
     */
    if (    strncmp(pszIface, RT_STR_TUPLE(NEMUBOW_VNIC_TEMPLATE_NAME))
        && !strncmp(pszIface, RT_STR_TUPLE("nemuvnic")))
        return _B_FALSE;
     ...
     * Add the interface.
     */
    nemuSolarisAddHostIface(szIfaceName, Instance, pvHostNetworkInterfaceList);
...
static bool nemuSolarisSortNICList(const ComObjPtr<HostNetworkInterface> Iface1, const ComObjPtr<HostNetworkInterface> Iface2)
{
    Bstr Iface1Str;
...
static bool nemuSolarisSameNIC(const ComObjPtr<HostNetworkInterface> Iface1, const ComObjPtr<HostNetworkInterface> Iface2)
{
    Bstr Iface1Str;
    (*Iface1).COMGETTER(Name) (Iface1Str.asOutParam());
...
static int nemuSolarisAddPhysHostIface(di_node_t Node, di_minor_t Minor, void *pvHostNetworkInterfaceList)
{
    NOREF(Minor);
    ...
    nemuSolarisAddHostIface(pszDriverName, Instance, pvHostNetworkInterfaceList);
    return DI_WALK_CONTINUE;
}
...
int NetIfList(std::list <ComObjPtr<HostNetworkInterface> > &list)
{
    ...
    if (Root != DI_NODE_NIL)
    {
        di_walk_minor(Root, DDI_NT_NET, 0 /* flag */, &list, nemuSolarisAddPhysHostIface);
        di_fini(Root);
    }
     ...
     */
    if (NemuSolarisLibDlpiFound())
        g_pfnLibDlpiWalk(nemuSolarisAddLinkHostIface, &list, 0);
                ...
                if (!rc)
                {
                    for (int i = 0; i < cIfaces; i++)
                    {
                        ...
                        if (rc >= 0)
                        {
                            ...
                            nemuSolarisAddLinkHostIface(pszIface, &list);
                        }
#endif
     ...
     */
    list.sort(nemuSolarisSortNICList);
    list.unique(nemuSolarisSameNIC);
