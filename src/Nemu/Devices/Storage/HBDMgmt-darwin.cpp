...
/** @file
 * Nemu storage devices: Host block device management API - darwin specifics.
 */
...
/**
 * Converts a given DA return code to a Nemu status code.
 *
 * @returns Nemu status code.
 * @param   hReturn        The status code returned by a DA API call.
 */
static int hbdMgrDAReturn2NemuStatus(DAReturn hReturn)
{
    ...
    if (!pDev)
    {
        ...
        if (hDiskRef)
        {
                ...
                && CalllbackArgs.rcDA == kDAReturnSuccess)
            {
                ...
                {
                    rc = hbdMgrDAReturn2NemuStatus(CalllbackArgs.rcDA);
