...
/** @file
 * Nemu Qt GUI - UIWizardCloneVM class implementation.
 */
...
bool UIWizardCloneVM::cloneVM()
{
    ...
    {
        /* Open session: */
        CSession session = nemuGlobal().openSession(m_machine.GetId());
        if (session.isNull())
            return false;
    ...
    const QString &strSettingsFile = nemu.ComposeMachineFilename(strName, QString::null /**< @todo group support */, QString::null, QString::null);
    CMachine cloneMachine = nemu.CreateMachine(strSettingsFile, strName, QVector<QString>(), QString::null, QString::null);
    if (!nemu.isOk())
    {
    ...
    nemu.RegisterMachine(cloneMachine);
    if (!nemu.isOk())
    {
        msgCenter().cannotRegisterMachine(nemu, cloneMachine.GetName(), this);
        return false;
    }
