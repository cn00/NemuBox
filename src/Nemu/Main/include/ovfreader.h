...
struct DiskImage
{
    ...
    RTCString uuidNemu;      // optional; if the file was exported by VirtualBox >= 3.2,
...
struct VirtualSystem
{
    ...
    RTCString    strTypeNemu;            // optional type from @nemu:ostype attribute (VirtualBox 4.0 or higher)
    ...
    const xml::ElementNode *pelmNemuMachine; // pointer to <nemu:Machine> element under <VirtualSystem> element or NULL if not present
