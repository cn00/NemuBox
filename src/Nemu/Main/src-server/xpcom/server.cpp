/* $Id: server.cpp $ */
/** @file
 * XPCOM server process (NemuSVC) start point.
 */
...
static nsresult nemusvcSpawnDaemonByReExec(const char *pszPath, bool fAutoShutdown, const char *pszPidFile)
{
...
{
    /*
     * Initialize the Nemu runtime without loading
     * the support driver
     */
    ...
    if (fDaemonize)
    {
        nemusvcSpawnDaemonByReExec(argv[0], gAutoShutdown, g_pszPidFile);
        exit(126);
    }
    ...
    if (!pszLogFile)
    {
        vrc = com::GetNemuUserHomeDirectory(szLogFile, sizeof(szLogFile));
        if (RT_SUCCESS(vrc))
            vrc = RTPathAppend(szLogFile, sizeof(szLogFile), "NemuSVC.log");
    }
    else
    {
    ...
    vrc = com::NemuLogRelCreate("XPCOM Server", szLogFile,
                                RTLOGFLAGS_PREFIX_THREAD | RTLOGFLAGS_PREFIX_TIME_PROG,
                                NEMUSVC_LOG_DEFAULT, "NEMUSVC_RELEASE_LOG",
    ...
    daemon_pipe_wr = PR_GetInheritedFD(NEMUSVC_STARTUP_PIPE_NAME);
    ...
    do
    {
        ...
        rc = gIpcServ->AddName(NEMUSVC_IPC_NAME);
        if (NS_FAILED(rc))
        {
...

        {
            ...
            iSize = RTStrPrintf(szBuf, sizeof(szBuf),
                                NEMU_PRODUCT" XPCOM Server Version "
                                NEMU_VERSION_STRING);
            for (int i = iSize; i > 0; i--)
                putchar('*');
            RTPrintf("\n%s\n", szBuf);
            RTPrintf("(C) 2004-" NEMU_C_YEAR " " NEMU_VENDOR "\n"
                     "All rights reserved.\n");
#ifdef DEBUG
        ...
        gIpcServ->RemoveName(NEMUSVC_IPC_NAME);
    }
