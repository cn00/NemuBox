...
{
    ContextInfo* ctx;
    NemuTlsRefGetCurrentFunctional(ctx, ContextInfo, &g_stubCurrentContextTSD);
    return ctx;
}
# define stubSetCurrentContext(_ctx) NemuTlsRefSetCurrent(ContextInfo, &g_stubCurrentContextTSD, _ctx)
#else
