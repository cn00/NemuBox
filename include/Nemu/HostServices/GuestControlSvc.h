...
#ifndef ___Nemu_HostService_GuestControlService_h
#define ___Nemu_HostService_GuestControlService_h
...
#define NEMU_GUESTCTRL_SESSION_ID_BASE  1

/** Builds a context ID out of the session ID, object ID and an
 *  increasing count. */
#define NEMU_GUESTCTRL_CONTEXTID_MAKE(uSession, uObject, uCount) \
    (  (uint32_t)((uSession) &   0x1f) << 27 \
     | (uint32_t)((uObject)  &  0x7ff) << 16 \
     | (uint32_t)((uCount)   & 0xffff)       \
    )
/** Creates a context ID out of a session ID. */
#define NEMU_GUESTCTRL_CONTEXTID_MAKE_SESSION(uSession) \
    ((uint32_t)((uSession) & 0x1f) << 27)
/** Gets the session ID out of a context ID. */
#define NEMU_GUESTCTRL_CONTEXTID_GET_SESSION(uContextID) \
    (((uContextID) >> 27) & 0x1f)
/** Gets the process ID out of a context ID. */
#define NEMU_GUESTCTRL_CONTEXTID_GET_OBJECT(uContextID) \
    (((uContextID) >> 16) & 0x7ff)
/** Gets the context count of a process out of a context ID. */
#define NEMU_GUESTCTRL_CONTEXTID_GET_COUNT(uContextID) \
    ((uContextID) & 0xffff)
/** Filter context IDs by session. Can be used in conjunction
 *  with VbglR3GuestCtrlMsgFilterSet(). */
#define NEMU_GUESTCTRL_FILTER_BY_SESSION(uSession) \
    (NEMU_GUESTCTRL_CONTEXTID_MAKE_SESSION(uSession) | 0xF8000000)
 ...
 */
#define OUTPUT_HANDLE_ID_STDOUT_DEPRECATED  0 /* Needed for Nemu hosts < 4.1.0. */
...
/** @name Internal tools built into NemuService which are used in order to
 *        accomplish tasks host<->guest.
 * @{
 */
#define NEMUSERVICE_TOOL_CAT        "nemu_cat"
#define NEMUSERVICE_TOOL_LS         "nemu_ls"
#define NEMUSERVICE_TOOL_RM         "nemu_rm"
#define NEMUSERVICE_TOOL_MKDIR      "nemu_mkdir"
#define NEMUSERVICE_TOOL_MKTEMP     "nemu_mktemp"
#define NEMUSERVICE_TOOL_STAT       "nemu_stat"
/** @} */
 ...
 */
typedef struct NemuGuestCtrlHostCbCtx
{
...
} NEMUGUESTCTRLHOSTCBCTX, *PNEMUGUESTCTRLHOSTCBCTX;

/**
 * Structure for low level HGCM host callback from
 * the guest. No deep copy. */
typedef struct NemuGuestCtrlHostCallback
{
    NemuGuestCtrlHostCallback(uint32_t cParms, NEMUHGCMSVCPARM paParms[])
                                : mParms(cParms), mpaParms(paParms) { }

    /** Number of HGCM parameters. */
    uint32_t mParms;
    /** Actual HGCM parameters. */
    PNEMUHGCMSVCPARM mpaParms;

} NEMUGUESTCTRLHOSTCALLBACK, *PNEMUGUESTCTRLHOSTCALLBACK;
...
#endif  /* !___Nemu_HostService_GuestControlService_h */
