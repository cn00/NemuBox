...
int NemuMainDriveInfo::updateDVDs ()
{
    ...
    try
    {
        ...
        if (RT_SUCCESS(rc) && (!success || testing()))
            rc = getDriveInfoFromEnv ("NEMU_CDROM", &mDVDList, true /* isDVD */,
                                      &success);
        setNoProbe(false);
        if (RT_SUCCESS(rc) && (!success || testing()))
...
int NemuMainDriveInfo::updateFloppies ()
{
    LogFlowThisFunc(("entered\n"));
    int rc = VINF_SUCCESS;
    bool success = false;  /* Have we succeeded in finding anything yet? */
    try
    {
        mFloppyList.clear ();
        if (RT_SUCCESS(rc) && (!success || testing()))
            rc = getDriveInfoFromEnv("NEMU_FLOPPY", &mFloppyList,
                                     false /* isDVD */, &success);
        setNoProbe(false);
        if (   RT_SUCCESS(rc) && (!success || testing()))
...
class hotplugNullImpl : public NemuMainHotplugWaiterImpl
{
public:
    hotplugNullImpl(const char *) {}
    virtual ~hotplugNullImpl (void) {}
    /** @copydoc NemuMainHotplugWaiter::Wait */
    virtual int Wait (RTMSINTERVAL)
    {
        return VERR_NOT_SUPPORTED;
    }
    /** @copydoc NemuMainHotplugWaiter::Interrupt */
...
class hotplugInotifyImpl : public NemuMainHotplugWaiterImpl
{
...
# endif /* NEMU_USB_WITH_INOTIFY */
#endif  /* NEMU_USB_WTH_SYSFS */

NemuMainHotplugWaiter::NemuMainHotplugWaiter(const char *pcszDevicesRoot)
{
    try
    {
#ifdef NEMU_USB_WITH_SYSFS
