...
#define NEMUNETFLT_SOLARIS_IPV6_POLLING
#ifdef NEMUNETFLT_SOLARIS_IPV6_POLLING
...
#ifdef NEMUNETFLT_SOLARIS_IPV6_POLLING
/** Driver properties */
# define NEMUNETFLT_IP6POLLINTERVAL      "ipv6-pollinterval"
#endif

/** Maximum loopback packet queue size per interface */
#define NEMUNETFLT_LOOPBACK_SIZE        32
 ...
 * Stream Driver hooks.
 */
static int NemuNetFltSolarisGetInfo(dev_info_t *pDip, ddi_info_cmd_t enmCmd, void *pArg, void **ppvResult);
static int NemuNetFltSolarisAttach(dev_info_t *pDip, ddi_attach_cmd_t enmCmd);
static int NemuNetFltSolarisDetach(dev_info_t *pDip, ddi_detach_cmd_t enmCmd);
static int NemuNetFltSolarisQuiesceNotNeeded(dev_info_t *pDip);

/**
 * Stream Module hooks.
 */
static int NemuNetFltSolarisModOpen(queue_t *pQueue, dev_t *pDev, int fFile, int fStream, cred_t *pCred);
static int NemuNetFltSolarisModClose(queue_t *pQueue, int fFile, cred_t *pCred);
static int NemuNetFltSolarisModReadPut(queue_t *pQueue, mblk_t *pMsg);
static int NemuNetFltSolarisModWritePut(queue_t *pQueue, mblk_t *pMsg);
 ...
 * Streams: module info.
 */
static struct module_info g_NemuNetFltSolarisModInfo =
{
    0xbad,                          /* module id */
    DEVICE_NAME,
    0,                              /* min. packet size */
 ...
 */
static struct qinit g_NemuNetFltSolarisReadQ =
{
    NemuNetFltSolarisModReadPut,
    NULL,                           /* service */
    NemuNetFltSolarisModOpen,
    NemuNetFltSolarisModClose,
    NULL,                           /* admin (reserved) */
    &g_NemuNetFltSolarisModInfo,
    NULL                            /* module stats */
};
 ...
 */
static struct qinit g_NemuNetFltSolarisWriteQ =
{
    NemuNetFltSolarisModWritePut,
    NULL,                           /* service */
    NULL,                           /* open */
    NULL,                           /* close */
    NULL,                           /* admin (reserved) */
 ...
 */
static struct streamtab g_NemuNetFltSolarisStreamTab =
{
    &g_NemuNetFltSolarisReadQ,
    &g_NemuNetFltSolarisWriteQ,
    NULL,                           /* muxread init */
    NULL                            /* muxwrite init */
};
...
static struct dev_ops g_NemuNetFltSolarisDevOps =
{
    ...
    (struct bus_ops *)0,
    nodev,                          /* power */
    NemuNetFltSolarisQuiesceNotNeeded
};

/**
 * modldrv: export driver specifics to kernel
 */
static struct modldrv g_NemuNetFltSolarisDriver =
{
 ...
 */
static struct fmodsw g_NemuNetFltSolarisModOps =
{
    DEVICE_NAME,
    &g_NemuNetFltSolarisStreamTab,
 ...
 */
static struct modlstrmod g_NemuNetFltSolarisModule =
{
    &mod_strmodops,                 /* extern from kernel */
    DEVICE_DESC_MOD " " NEMU_VERSION_STRING "r" RT_XSTR(NEMU_SVN_REV),
    &g_NemuNetFltSolarisModOps
};
...
static struct modlinkage g_NemuNetFltSolarisModLinkage =
{
    ...
    {
        &g_NemuNetFltSolarisDriver,  /* streams driver framework */
        &g_NemuNetFltSolarisModule,  /* streams module framework */
        NULL                         /* terminate array of linkage structures */
    }
};

struct nemunetflt_state_t;

/**
 * nemunetflt_dladdr_t: DL SAP address format
 */
typedef struct nemunetflt_dladdr_t
{
    ether_addr_t Mac;
    uint16_t SAP;
} nemunetflt_dladdr_t;

#define NEMUNETFLT_DLADDRL        sizeof(nemunetflt_dladdr_t)

/**
 * which stream is this?
 */
typedef enum NEMUNETFLTSTREAMTYPE
{
    kUndefined = 0,
    kIp4Stream = 0x1b,
    kIp6Stream = 0xcc,
    kArpStream = 0xab,
    kPromiscStream = 0xdf
} NEMUNETFLTSTREAMTYPE;

/**
 * loopback packet identifier
 */
typedef struct NEMUNETFLTPACKETID
{
    struct NEMUNETFLTPACKETID *pNext;
    uint16_t cbPacket;
    uint16_t Checksum;
    RTMAC SrcMac;
    RTMAC DstMac;
} NEMUNETFLTPACKETID;
typedef struct NEMUNETFLTPACKETID *PNEMUNETFLTPACKETID;

/**
 * nemunetflt_stream_t: per-stream data (multiple streams per interface)
 */
typedef struct nemunetflt_stream_t
{
    int DevMinor;                         /* minor device no. (for clone) */
    queue_t *pReadQueue;                  /* read side queue */
    struct nemunetflt_stream_t *pNext;    /* next stream in list */
    PNEMUNETFLTINS volatile pThis;        /* the backend instance */
    NEMUNETFLTSTREAMTYPE Type;            /* the type of the stream */
} nemunetflt_stream_t;

/**
 * nemunetflt_promisc_stream_t: per-interface dedicated stream data
 */
typedef struct nemunetflt_promisc_stream_t
{
    nemunetflt_stream_t Stream;           /* dedicated/promiscuous stream */
    ...
    PNEMUNETFLTPACKETID pHead;            /* loopback packet identifier head */
    PNEMUNETFLTPACKETID pTail;            /* loopback packet identifier tail */
} nemunetflt_promisc_stream_t;

typedef struct nemunetflt_promisc_params_t
{
    PNEMUNETFLTINS pThis;                 /* the backend instance */
    bool fPromiscOn;                      /* whether promiscuous req. on or off */
} nemunetflt_promisc_params_t;


/*********************************************************************************************************************************
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static int nemuNetFltSolarisSetRawMode(nemunetflt_promisc_stream_t *pPromiscStream);
/* static int nemuNetFltSolarisSetFastMode(queue_t *pQueue); */

static int nemuNetFltSolarisPhysAddrReq(queue_t *pQueue);
static void nemuNetFltSolarisCachePhysAddr(PNEMUNETFLTINS pThis, mblk_t *pPhysAddrAckMsg);
static int nemuNetFltSolarisBindReq(queue_t *pQueue, int SAP);
static int nemuNetFltSolarisNotifyReq(queue_t *pQueue);

/*  static int nemuNetFltSolarisUnitDataToRaw(PNEMUNETFLTINS pThis, mblk_t *pMsg, mblk_t **ppRawMsg); */
static int nemuNetFltSolarisRawToUnitData(mblk_t *pMsg, mblk_t **ppDlpiMsg);

static inline void nemuNetFltSolarisInitPacketId(PNEMUNETFLTPACKETID pTag, mblk_t *pMsg);
static int nemuNetFltSolarisQueueLoopback(PNEMUNETFLTINS pThis, nemunetflt_promisc_stream_t *pPromiscStream, mblk_t *pMsg);
static bool nemuNetFltSolarisIsOurMBlk(PNEMUNETFLTINS pThis, nemunetflt_promisc_stream_t *pPromiscStream, mblk_t *pMsg);

static mblk_t *nemuNetFltSolarisMBlkFromSG(PNEMUNETFLTINS pThis, PINTNETSG pSG, uint32_t fDst);
static unsigned nemuNetFltSolarisMBlkCalcSGSegs(PNEMUNETFLTINS pThis, mblk_t *pMsg);
static int nemuNetFltSolarisMBlkToSG(PNEMUNETFLTINS pThis, mblk_t *pMsg, PINTNETSG pSG, unsigned cSegs, uint32_t fSrc);
static int nemuNetFltSolarisRecv(PNEMUNETFLTINS pThis, nemunetflt_stream_t *pStream, queue_t *pQueue, mblk_t *pMsg);
/* static mblk_t *nemuNetFltSolarisFixChecksums(mblk_t *pMsg); */
/* static void nemuNetFltSolarisAnalyzeMBlk(mblk_t *pMsg); */
...
nemunetflt_stream_t *g_NemuNetFltSolarisStreams = NULL;

/** Global mutex protecting open/close. */
static RTSEMFASTMUTEX g_NemuNetFltSolarisMtx = NIL_RTSEMFASTMUTEX;

/** Global credentials using during open/close. */
static cred_t *g_pNemuNetFltSolarisCred = NULL;

/**
 * g_NemuNetFltInstance is the current PNEMUNETFLTINS to be associated with the stream being created
 * in ModOpen. This is just shared global data between the dynamic attach and the ModOpen procedure.
 */
PNEMUNETFLTINS volatile g_NemuNetFltSolarisInstance = NULL;

/** Goes along with the instance to determine type of stream being opened/created. */
NEMUNETFLTSTREAMTYPE volatile g_NemuNetFltSolarisStreamType = kUndefined;

#ifdef NEMUNETFLT_SOLARIS_IPV6_POLLING
/** Global IPv6 polling interval */
static int g_NemuNetFltSolarisPollInterval = -1;
#endif
...
static int
nemuNetFltSolarisCtfGetMemberOffset(ctf_file_t *pCtfFile, const char *pszStruct, const char *pszMember, int *pOffset)
{
...
static int
nemuNetFltSolarisProbeCtf(void)
{
    /*
    ...
    if (pModCtl)
    {
        ...
        if (pCtfFile)
        {
            rc = nemuNetFltSolarisCtfGetMemberOffset(pCtfFile, "file_t", "f_vnode", &s_off_vnode);
            ctf_close(pCtfFile);
        }
        else
...
int _init(void)
{
    ...
    if (RT_SUCCESS(rc))
    {
        rc = nemuNetFltSolarisProbeCtf();
        if (RT_SUCCESS(rc))
        {
            /*
             * Initialize Solaris specific globals here.
             */
            g_NemuNetFltSolarisStreams = NULL;
            g_NemuNetFltSolarisInstance = NULL;
            g_pNemuNetFltSolarisCred = crdup(kcred);
            if (RT_LIKELY(g_pNemuNetFltSolarisCred))
            {
                rc = RTSemFastMutexCreate(&g_NemuNetFltSolarisMtx);
                if (RT_SUCCESS(rc))
                {
                    /*
                    ...
                    RTSemFastMutexDestroy(g_NemuNetFltSolarisMtx);
                    g_NemuNetFltSolarisMtx = NIL_RTSEMFASTMUTEX;
                }
            }
            else
            {
            ...
            LogRel((DEVICE_NAME ":nemuNetFltSolarisProbeCtf failed. rc=%d\n", rc));
...
int _fini(void)
{
    ...
    if (!rc)
    {
        if (g_pNemuNetFltSolarisCred)
        {
            crfree(g_pNemuNetFltSolarisCred);
            g_pNemuNetFltSolarisCred = NULL;
        }

        if (g_NemuNetFltSolarisMtx != NIL_RTSEMFASTMUTEX)
        {
            RTSemFastMutexDestroy(g_NemuNetFltSolarisMtx);
            g_NemuNetFltSolarisMtx = NIL_RTSEMFASTMUTEX;
        }
...
static int NemuNetFltSolarisAttach(dev_info_t *pDip, ddi_attach_cmd_t enmCmd)
{
    LogFunc((DEVICE_NAME ":NemuNetFltSolarisAttach pDip=%p enmCmd=%d\n", pDip, enmCmd));
    ...
    switch (enmCmd)
    {
        case DDI_ATTACH:
        {
            ...
            if (rc == DDI_SUCCESS)
            {
                 ...
                 */
                int Interval = ddi_getprop(DDI_DEV_T_ANY, pDip, DDI_PROP_DONTPASS, NEMUNETFLT_IP6POLLINTERVAL, -1 /* default */);
                if (Interval == -1)
                    Log((DEVICE_NAME ":nemuNetFltSolarisSetupIp6Polling: no poll interval property specified. Skipping Ipv6 polling.\n"));
                else if (Interval < 1 || Interval > 120)
                {
                    LogRel((DEVICE_NAME ":nemuNetFltSolarisSetupIp6Polling: Invalid polling interval %d. Expected between 1 and 120 secs.\n",
                                        Interval));
                    Interval = -1;
                }

                g_NemuNetFltSolarisPollInterval = Interval;
#endif
                ddi_report_dev(pDip);
                return DDI_SUCCESS;
            }
            else
                LogRel((DEVICE_NAME ":NemuNetFltSolarisAttach failed to create minor node. rc%d\n", rc));
            return DDI_FAILURE;
        }
...
static int NemuNetFltSolarisDetach(dev_info_t *pDip, ddi_detach_cmd_t enmCmd)
{
    LogFunc((DEVICE_NAME ":NemuNetFltSolarisDetach pDip=%p enmCmd=%d\n", pDip, enmCmd));
 ...
 * @returns  corresponding solaris error code.
 */
static int NemuNetFltSolarisGetInfo(dev_info_t *pDip, ddi_info_cmd_t enmCmd, void *pvArg, void **ppvResult)
{
    LogFunc((DEVICE_NAME ":NemuNetFltSolarisGetInfo pDip=%p enmCmd=%d pArg=%p instance=%d\n", pDip, enmCmd,
 ...
 * @returns corresponding solaris error code.
 */
static int NemuNetFltSolarisModOpen(queue_t *pQueue, dev_t *pDev, int fOpenMode, int fStreamMode, cred_t *pCred)
{
    Assert(pQueue);

    LogFunc((DEVICE_NAME ":NemuNetFltSolarisModOpen pQueue=%p pDev=%p fOpenMode=%d fStreamMode=%d\n", pQueue, pDev,
    ...
    if (pQueue->q_ptr)
    {
        LogRel((DEVICE_NAME ":NemuNetFltSolarisModOpen invalid open.\n"));
        return ENOENT;
    }
     ...
     * g_NemuNetFltSolarisMtx prevent races and that the instance gets
     * associated with the wrong streams.
     */
    if (pCred != g_pNemuNetFltSolarisCred)
    {
        LogRel((DEVICE_NAME ":NemuNetFltSolarisModOpen invalid credentials.\n"));
        return EACCES;
    }
     ...
     */
    PNEMUNETFLTINS pThis = g_NemuNetFltSolarisInstance;
    if (!pThis)
    {
        LogRel((DEVICE_NAME ":NemuNetFltSolarisModOpen failed to get VirtualBox instance.\n"));
        return ENOENT;
    }
     ...
     */
    if (   g_NemuNetFltSolarisStreamType != kPromiscStream
        && g_NemuNetFltSolarisStreamType != kArpStream
        && g_NemuNetFltSolarisStreamType != kIp6Stream
        && g_NemuNetFltSolarisStreamType != kIp4Stream)
    {
        LogRel((DEVICE_NAME ":NemuNetFltSolarisModOpen failed due to undefined VirtualBox open mode. Type=%d\n",
                g_NemuNetFltSolarisStreamType));
        return ENOENT;
    }
     ...
     */
    minor_t DevMinor = 0;
    nemunetflt_stream_t *pStream = NULL;
    nemunetflt_stream_t **ppPrevStream = &g_NemuNetFltSolarisStreams;
    ...
    if (g_NemuNetFltSolarisStreamType == kPromiscStream)
    {
        nemunetflt_promisc_stream_t *pPromiscStream = RTMemAlloc(sizeof(nemunetflt_promisc_stream_t));
        if (RT_UNLIKELY(!pPromiscStream))
        {
            LogRel((DEVICE_NAME ":NemuNetFltSolarisModOpen failed to allocate promiscuous stream data.\n"));
            return ENOMEM;
        }
...
#endif
        pStream = (nemunetflt_stream_t *)pPromiscStream;
    }
    else
    {
        /*
         ...
         */
        pStream = RTMemAlloc(sizeof(nemunetflt_stream_t));
        if (RT_UNLIKELY(!pStream))
        {
            LogRel((DEVICE_NAME ":NemuNetFltSolarisModOpen failed to allocate stream data.\n"));
            return ENOMEM;
        }
    }
    ...
    /*
     * Pick up the current global NEMUNETFLTINS instance as
     * the one that we will associate this stream with.
     */
    ASMAtomicUoWritePtr(&pStream->pThis, pThis);
    pStream->Type = g_NemuNetFltSolarisStreamType;
    switch (pStream->Type)
    {
        ...
        default:    /* Heh. */
        {
            LogRel((DEVICE_NAME ":NemuNetFltSolarisModOpen huh!? Invalid stream type %d\n", pStream->Type));
            RTMemFree(pStream);
            return EINVAL;
        }
    }
    ...
    *ppPrevStream = pStream;
    ...
    {
        nemunetflt_promisc_stream_t *pPromiscStream = (nemunetflt_promisc_stream_t *)pStream;
         ...
         */
        int rc = nemuNetFltSolarisBindReq(pStream->pReadQueue, 0 /* SAP */);
        if (RT_LIKELY(RT_SUCCESS(rc)))
        {
            /*
             * Request the physical address (we cache the acknowledgement).
             */
            rc = nemuNetFltSolarisPhysAddrReq(pStream->pReadQueue);
            if (RT_LIKELY(RT_SUCCESS(rc)))
            {
                /*
                 * Ask for DLPI link notifications, don't bother check for errors here.
                 */
                nemuNetFltSolarisNotifyReq(pStream->pReadQueue);

                /*
                 * Enable raw mode.
                 */
                rc = nemuNetFltSolarisSetRawMode(pPromiscStream);
                if (RT_FAILURE(rc))
                    LogRel((DEVICE_NAME ":nemuNetFltSolarisSetRawMode failed rc=%Rrc.\n", rc));
            }
            else
                LogRel((DEVICE_NAME ":nemuNetFltSolarisSetRawMode failed rc=%Rrc.\n", rc));
        }
        else
            LogRel((DEVICE_NAME ":nemuNetFltSolarisBindReq failed rc=%Rrc.\n", rc));
    }

    NOREF(fOpenMode);

    Log((DEVICE_NAME ":NemuNetFltSolarisModOpen returns 0, DevMinor=%d pQueue=%p\n", DevMinor, pStream->pReadQueue));
 ...
 * @returns  corresponding solaris error code.
 */
static int NemuNetFltSolarisModClose(queue_t *pQueue, int fOpenMode, cred_t *pCred)
{
    Assert(pQueue);

    LogFunc((DEVICE_NAME ":NemuNetFltSolarisModClose pQueue=%p fOpenMode=%d\n", pQueue, fOpenMode));

    nemunetflt_stream_t *pStream = NULL;
    nemunetflt_stream_t **ppPrevStream = NULL;

    /*
     * Get instance data.
     */
    pStream = (nemunetflt_stream_t *)pQueue->q_ptr;
    if (RT_UNLIKELY(!pStream))
    {
        LogRel((DEVICE_NAME ":NemuNetFltSolarisModClose failed to get stream.\n"));
        return ENXIO;
    }
    ...
    if (pStream->Type == kPromiscStream)
    {
         ...
         */
        PNEMUNETFLTPACKETID pCur = pPromiscStream->pHead;
        while (pCur)
        {
            PNEMUNETFLTPACKETID pNext = pCur->pNext;
            RTMemFree(pCur);
            pCur = pNext;
        }
        pPromiscStream->pHead = NULL;
     ...
     */
    for (ppPrevStream = &g_NemuNetFltSolarisStreams; (pStream = *ppPrevStream) != NULL; ppPrevStream = &pStream->pNext)
        if (pStream == (nemunetflt_stream_t *)pQueue->q_ptr)
            break;
    *ppPrevStream = pStream->pNext;
     ...
     */
    nemuNetFltRelease(pStream->pThis, false /* fBusy */);
...
static int NemuNetFltSolarisModReadPut(queue_t *pQueue, mblk_t *pMsg)
{
    ...
    LogFunc((DEVICE_NAME ":NemuNetFltSolarisModReadPut pQueue=%p pMsg=%p\n", pQueue, pMsg));

    bool fSendUpstream = true;
    nemunetflt_stream_t *pStream = pQueue->q_ptr;
    PNEMUNETFLTINS pThis = NULL;
    ...
    {
        fSendUpstream = false;
        pThis = ASMAtomicUoReadPtrT(&pStream->pThis, PNEMUNETFLTINS);
        if (RT_LIKELY(pThis))
        {
            /*
            ...
            nemuNetFltRetain(pThis, true /* fBusy */);
            ...
            switch (DB_TYPE(pMsg))
            {
                case M_DATA:
                {
                    Log((DEVICE_NAME ":NemuNetFltSolarisModReadPut M_DATA\n"));

                    if (   fActive
                        && pPromiscStream->fRawMode)
                    {
                        nemuNetFltSolarisRecv(pThis, pStream, pQueue, pMsg);
                    }
                    break;
                }
                ...
                case M_PCPROTO:
                {
                    ...
                    Log((DEVICE_NAME ":NemuNetFltSolarisModReadPut: M_PCPROTO %d\n", Prim));
                    switch (Prim)
                    {
                        case DL_NOTIFY_IND:
                        {
                            ...
                            {
                                LogRel((DEVICE_NAME ":NemuNetFltSolarisModReadPut: Invalid notification size; expected>=%d"
                            ...
                            switch (pNotifyInd->dl_notification)
                            {
                                case DL_NOTE_PHYS_ADDR:
                                {
                                    ...
                                    if (!cOffset || !cbAddr)
                                    {
                                        LogRel((DEVICE_NAME ":NemuNetFltSolarisModReadPut: DL_NOTE_PHYS_ADDR."
                                                "Invalid offset/addr.\n"));
                                        fSendUpstream = false;
                                        break;
                                    }
                                    ...
                                    Log((DEVICE_NAME ":NemuNetFltSolarisModReadPut: DL_NOTE_PHYS_ADDR. New Mac=%.*Rhxs\n",
                                ...
                                case DL_NOTE_LINK_UP:
                                {
                                    if (ASMAtomicXchgBool(&pThis->fDisconnectedFromHost, false))
                                        Log((DEVICE_NAME ":NemuNetFltSolarisModReadPut: DL_NOTE_LINK_UP.\n"));
                                    break;
                                }
                                ...
                                case DL_NOTE_LINK_DOWN:
                                {
                                    if (!ASMAtomicXchgBool(&pThis->fDisconnectedFromHost, true))
                                        Log((DEVICE_NAME ":NemuNetFltSolarisModReadPut: DL_NOTE_LINK_DOWN.\n"));
                                    break;
                                }
                            }
                            break;
                        ...
                        case DL_BIND_ACK:
                        {
                             ...
                             */
                            Log((DEVICE_NAME ":NemuNetFltSolarisModReadPut: DL_BIND_ACK. Bound to requested SAP!\n"));
                            break;
                        }
                        ...
                        case DL_PHYS_ADDR_ACK:
                        {
                             ...
                             */
                            nemuNetFltSolarisCachePhysAddr(pThis, pMsg);
                            break;
                        }
                        ...
                        case DL_OK_ACK:
                        {
                            ...
                            {
                                Log((DEVICE_NAME ":NemuNetFltSolarisModReadPut: M_PCPROTO: DL_OK_ACK: fPromisc is ON.\n"));
                                pPromiscStream->fPromisc = true;
                            }
                            else if (pOkAck->dl_correct_primitive == DL_PROMISCOFF_REQ)
                            {
                                Log((DEVICE_NAME ":NemuNetFltSolarisModReadPut: M_PCPROTO: DL_OK_ACK: fPromisc is OFF.\n"));
                                pPromiscStream->fPromisc = false;
                            }
                            break;
                        }
                ...
                case M_IOCACK:
                {
                    ...
                    if (pIOC->ioc_id == pPromiscStream->ModeReqId)
                    {
                        ...
                        Log((DEVICE_NAME ":NemuNetFltSolarisModReadPut: Mode acknowledgement. RawMode is %s\n",
                ...
                case M_IOCNAK:
                {
                    ...
                    {
                        pPromiscStream->fRawMode = false;
                        LogRel((DEVICE_NAME ":NemuNetFltSolarisModReadPut: WARNING! Mode not acknowledged. RawMode is %s\n",
                                pPromiscStream->fRawMode ? "ON" : "OFF"));
                    }
                    break;
                }
                ...
                case M_FLUSH:
                {
                     ...
                     */
                    Log((DEVICE_NAME ":NemuNetFltSolarisModReadPut: M_FLUSH\n"));
            ...
            LogRel((DEVICE_NAME ":NemuNetFltSolarisModReadPut: Could not find VirtualBox instance!!\n"));
    }
...
static int NemuNetFltSolarisModWritePut(queue_t *pQueue, mblk_t *pMsg)
{
    LogFunc((DEVICE_NAME ":NemuNetFltSolarisModWritePut pQueue=%p pMsg=%p\n", pQueue, pMsg));
...
static int nemuNetFltSolarisSetRawMode(nemunetflt_promisc_stream_t *pPromiscStream)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisSetRawMode pPromiscStream=%p\n", pPromiscStream));
...
static int nemuNetFltSolarisSetFastMode(queue_t *pQueue)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisSetFastMode pQueue=%p\n", pQueue));
    ...
    size_t cbReq = sizeof(dl_unitdata_req_t) + sizeof(nemunetflt_dladdr_t);
    ...
    pDataReq->dl_dest_addr_length = sizeof(nemunetflt_dladdr_t);
    pDataReq->dl_dest_addr_offset = sizeof(dl_unitdata_req_t);
    pDataReq->dl_priority.dl_min = 0;
    pDataReq->dl_priority.dl_max = 0;

    bzero(pDataReqMsg->b_rptr + sizeof(dl_unitdata_req_t), sizeof(nemunetflt_dladdr_t));
 ...
 * @returns Nemu status code.
 */
static int nemuNetFltSolarisPromiscReq(queue_t *pQueue, bool fPromisc)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisPromiscReq pQueue=%p fPromisc=%d\n", pQueue, fPromisc));
 ...
 *                          nemuNetFltSolarisPromiscReqWrap().
 */
static void nemuNetFltSolarisPromiscReqWrapExcl(queue_t *pQueue, mblk_t *pMsg)
{
    /*
     * Paranoia.
     */
    ...
    if (RT_UNLIKELY(!pMsg))
        LogRel((DEVICE_NAME ":NemuNetFltSolarisPromiscReqWrapExcl pQueue=%p missing message!\n", pQueue));

    bool fPromisc = (MBLKL(pMsg) == 1);
    freemsg(pMsg);
    pMsg = NULL;
    int rc = nemuNetFltSolarisPromiscReq(pQueue, fPromisc);
    if (RT_FAILURE(rc))
        LogRel((DEVICE_NAME ":NemuNetFltSolarisPromiscReqWrapExcl nemuNetFltSolarisPromiscReq failed. rc=%d\n", rc));
}
 ...
 *
 * @param pvData            Pointer to nemunetflt_promisc_params_t. See
 *                          nemuNetFltPortOsSetActive().
 */
static void nemuNetFltSolarisPromiscReqWrap(void *pvData)
{
    nemunetflt_promisc_params_t *pParams = pvData;
    if (RT_LIKELY(pParams))
    {
        PNEMUNETFLTINS pThis = pParams->pThis;
        nemunetflt_promisc_stream_t *pPromiscStream = ASMAtomicUoReadPtrT(&pThis->u.s.pPromiscStream,
                                                                          nemunetflt_promisc_stream_t *);
        if (   pPromiscStream
            && pPromiscStream->Stream.pReadQueue)
        {
             ...
             */
            qwriter(WR(pPromiscStream->Stream.pReadQueue), pMsg, nemuNetFltSolarisPromiscReqWrapExcl, PERIM_OUTER);
            ASMAtomicWritePtr(&pPromiscStream->TimeoutId, NULL);
        }
        RTMemFree(pParams);
    }
...
static int nemuNetFltSolarisPhysAddrReq(queue_t *pQueue)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisPhysAddrReq pQueue=%p\n", pQueue));
 ...
 */
static void nemuNetFltSolarisCachePhysAddr(PNEMUNETFLTINS pThis, mblk_t *pMsg)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisCachePhysAddr pThis=%p pMsg=%p\n", pThis, pMsg));
    ...
    if (pPhysAddrAck->dl_addr_length == sizeof(pThis->u.s.MacAddr))
    {
        ...
        Log((DEVICE_NAME ":nemuNetFltSolarisCachePhysAddr: DL_PHYS_ADDR_ACK: Mac=%.*Rhxs\n",
                 sizeof(pThis->u.s.MacAddr), &pThis->u.s.MacAddr));
    ...
    {
        LogRel((DEVICE_NAME ":nemuNetFltSolarisCachePhysAddr: Invalid address size. expected=%d got=%d\n", ETHERADDRL,
...
static int nemuNetFltSolarisBindReq(queue_t *pQueue, int SAP)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisBindReq SAP=%d\n", SAP));
...
static int nemuNetFltSolarisNotifyReq(queue_t *pQueue)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisNotifyReq\n"));
 ...
 */
static int nemuNetFltSolarisOpenDev(char *pszDev, vnode_t **ppVNode, vnode_t **ppVNodeHeld, TIUSER **ppUser)
{
    int rc;
        ...
        && pVNodeHeld)
    {
        ...
        if (!rc)
        {
            ...
            {
                LogRel((DEVICE_NAME ":nemuNetFltSolarisOpenDev failed. pUser=%p fp=%p f_vnode=%p\n", pUser,
            ...
            LogRel((DEVICE_NAME ":nemuNetFltSolarisOpenDev t_kopen failed. rc=%d\n", rc));

        VN_RELE(pVNodeHeld);
    }
    else
        LogRel((DEVICE_NAME ":nemuNetFltSolarisOpenDev lookupname failed. rc=%d pVNodeHeld=%p\n", rc, pVNodeHeld));
...
/**
 * Close the device opened using nemuNetFltSolarisOpenDev.
 *
 * @param   pVNodeHeld      Pointer to the held vnode of the device.
 * @param   pUser           Pointer to the file handle.
 */
static void nemuNetFltSolarisCloseDev(vnode_t *pVNodeHeld, TIUSER *pUser)
{
    t_kclose(pUser, 0);
    VN_RELE(pVNodeHeld);
}
 ...
 */
static int nemuNetFltSolarisAttachReq(ldi_handle_t hDevice, int PPA)
{
    int rc;
    ...
    if (!rc)
    {
        ...
        if (!rc)
        {
            ...
            if (cbMsg >= sizeof(t_uscalar_t))
            {
                ...
                {
                    LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachReq ldi_getmsg succeeded, but unsupported op.\n"));
                    rc = VERR_NOT_SUPPORTED;
                }
                else                                            /* Garbled reply */
                {
                    LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachReq ldi_getmsg succeeded, but invalid op."
                            " expected %d recvd %d\n", DL_OK_ACK, AckPrim));
            ...
            {
                LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachReq ldi_getmsg succeeded, but invalid size %d expected %d\n", cbMsg,
                            DL_OK_ACK_SIZE));
                rc = VERR_INVALID_FUNCTION;
            }
        }
        ...
        {
            LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachReq ldi_getmsg failed. rc=%d\n", rc));
            rc = VERR_INVALID_FUNCTION;
        }
    }
    else
    {
        LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachReq ldi_putmsg failed. rc=%d\n", rc));
 ...
 */
static int nemuNetFltSolarisGetIfFlags(ldi_handle_t hDevice, struct lifreq *pInterface)
{
 ...
 * @param   pInterface  Pointer to the interface.
 */
static int nemuNetFltSolarisSetMuxId(vnode_t *pVNode, struct lifreq *pInterface)
{
    struct strioctl IOCReq;
    int rc;
    int ret;
 ...
 */
static int nemuNetFltSolarisMuxIdToFd(vnode_t *pVNode, int MuxId, int *pFd)
{
    int ret;
 ...
 */
static int nemuNetFltSolarisRelinkIp4(vnode_t *pVNode, struct lifreq *pInterface, int IpMuxFd, int ArpMuxFd)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisRelinkIp4: pVNode=%p pInterface=%p IpMuxFd=%d ArpMuxFd=%d\n", pVNode,
        ...
        && !rc2)
    {
        ...
        rc = nemuNetFltSolarisSetMuxId(pVNode, pInterface);
        if (RT_SUCCESS(rc))
            return VINF_SUCCESS;

        LogRel((DEVICE_NAME ":nemuNetFltSolarisRelinkIp4: failed to set new Mux Id.\n"));
    }
    else
        LogRel((DEVICE_NAME ":nemuNetFltSolarisRelinkIp4: failed to link.\n"));
 ...
 */
static int nemuNetFltSolarisRelinkIp6(vnode_t *pVNode, struct lifreq *pInterface, int Ip6MuxFd)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisRelinkIp6: pVNode=%p pInterface=%p Ip6MuxFd=%d\n", pVNode, pInterface, Ip6MuxFd));
    ...
    if (!rc)
    {
        ...
        LogRel((DEVICE_NAME ":nemuNetFltSolarisRelinkIp6: failed to set new Mux Id.\n"));
    }
    else
        LogRel((DEVICE_NAME ":nemuNetFltSolarisRelinkIp6: failed to link.\n"));
 ...
 */
static int nemuNetFltSolarisDetermineModPos(bool fAttach, vnode_t *pVNode, int *pModPos)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisDetermineModPos: fAttach=%d pVNode=%p pModPos=%p\n", fAttach, pVNode, pModPos));
    ...
    if (!rc)
    {
        if (cMod < 1)
        {
            LogRel((DEVICE_NAME ":nemuNetFltSolarisDetermineModPos: too few modules on host interface. cMod=%d\n"));
         ...
         * the host driver.
         */
        Log((DEVICE_NAME ":nemuNetFltSolarisDetermineModPos: cMod=%d\n", cMod));
        if (fAttach)
        {
            *pModPos = cMod - 1;
            return VINF_SUCCESS;
        ...
        {
            Log((DEVICE_NAME ":nemuNetFltSolarisDetermineModPos: failed to alloc memory for StrList.\n"));
            return VERR_NO_MEMORY;
        }
        ...
        if (!rc)
        {
            ...
            for (int i = 0; i < StrList.sl_nmods; i++)
            {
                ...
                {
                    Log((DEVICE_NAME ":nemuNetFltSolarisDetermineModPos: Success! Found %s at %d.\n", DEVICE_NAME, i));
                    *pModPos = i;
            ...
            LogRel((DEVICE_NAME ":nemuNetFltSolarisDetermineModPos: failed to find %s in the host stack.\n", DEVICE_NAME));
        }
        else
            LogRel((DEVICE_NAME ":nemuNetFltSolarisDetermineModPos: failed to get module information. rc=%d\n"));

        RTMemFree(StrList.sl_modlist);
    }
    else
        LogRel((DEVICE_NAME ":nemuNetFltSolarisDetermineModPos: failed to get list of modules on host interface. rc=%d\n", rc));
    return VERR_GENERAL_FAILURE;
}
 ...
 */
static int nemuNetFltSolarisOpenStyle2(PNEMUNETFLTINS pThis, ldi_ident_t *pDevId)
{
    /*
        ...
        && ddi_strtol(pszEnd, NULL, 10, &PPA) == 0)
    {
        *pszEnd = '\0';
        ...
        if (!rc)
        {
             ...
             */
            rc = nemuNetFltSolarisAttachReq(pThis->u.s.hIface, (int)PPA);
            if (RT_SUCCESS(rc))
            {
                RTStrFree(pszDev);
                return rc;
            ...
            LogRel((DEVICE_NAME ":nemuNetFltSolarisOpenStyle2 dl_attach failed. rc=%d szDev=%s PPA=%d rc=%d\n", rc, szDev, PPA));
        }
        else
            LogRel((DEVICE_NAME ":nemuNetFltSolarisOpenStyle2 Failed to open. rc=%d szDev=%s PPA=%d\n", rc, szDev, PPA));
    }
    else
        LogRel((DEVICE_NAME ":nemuNetFltSolarisOpenStyle2 Failed to construct PPA. pszDev=%s pszEnd=%s.\n", pszDev, pszEnd));
 ...
 * @param   pThis       The instance.
 */
static int nemuNetFltSolarisOpenStream(PNEMUNETFLTINS pThis)
{
    ldi_ident_t DevId;
    ...
    if (rc)
    {
         ...
         * Try DLPI style 2.
         */
        rc = nemuNetFltSolarisOpenStyle2(pThis, &DevId);
        if (RT_FAILURE(rc))
            LogRel((DEVICE_NAME ":nemuNetFltSolarisOpenStream nemuNetFltSolarisOpenStyle2 failed. rc=%d\n", rc));
        else
            rc = 0;
    }
    ...
    if (rc)
    {
        LogRel((DEVICE_NAME ":nemuNetFltSolarisOpenStream Failed to open '%s' rc=%d pszName='%s'\n", szDev, rc, pThis->szName));
        return VERR_INTNET_FLT_IF_FAILED;
    }
    ...
    if (!rc)
    {
        if (!ret)
        {
            if (RT_LIKELY(g_pNemuNetFltSolarisCred))        /* Paranoia */
            {
                rc = RTSemFastMutexRequest(g_NemuNetFltSolarisMtx);
                AssertRCReturn(rc, rc);

                g_NemuNetFltSolarisInstance = pThis;
                g_NemuNetFltSolarisStreamType = kPromiscStream;

                rc = ldi_ioctl(pThis->u.s.hIface, I_PUSH, (intptr_t)DEVICE_NAME, FKIOCTL, g_pNemuNetFltSolarisCred, &ret);

                g_NemuNetFltSolarisInstance = NULL;
                g_NemuNetFltSolarisStreamType = kUndefined;

                RTSemFastMutexRelease(g_NemuNetFltSolarisMtx);
            }
            else
            {
                LogRel((DEVICE_NAME ":nemuNetFltSolarisOpenStream huh!? Missing credentials.\n"));
            ...
            LogRel((DEVICE_NAME ":nemuNetFltSolarisOpenStream Failed to push filter onto host interface '%s'\n", pThis->szName));
        }
        else
            return VINF_SUCCESS;
    }
    else
        LogRel((DEVICE_NAME ":nemuNetFltSolarisOpenStream Failed to search for filter in interface '%s'.\n", pThis->szName));
 ...
 * @param   pThis       The instance.
 */
static void nemuNetFltSolarisCloseStream(PNEMUNETFLTINS pThis)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisCloseStream pThis=%p\n"));
 ...
 * @param   fAttach     Is this an attach or detach.
 */
static int nemuNetFltSolarisAttachIp4(PNEMUNETFLTINS pThis, bool fAttach)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisAttachIp4 pThis=%p fAttach=%d\n", pThis, fAttach));
    ...
    if (rc)
    {
        LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp4: failed to open the IP stream on '%s'.\n", pThis->szName));
    ...
    if (rc)
    {
        LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp4: failed to open the ARP stream on '%s'.\n", pThis->szName));
        ldi_ident_release(DeviceIdent);
     ...
     */
    rc = nemuNetFltSolarisGetIfFlags(Ip4DevHandle, &Ip4Interface);
    if (RT_SUCCESS(rc))
    {
        /*
        ...
        rc = nemuNetFltSolarisOpenDev(UDP_DEV_NAME, &pUdp4VNode, &pUdp4VNodeHeld, &pUdp4User);
        if (RT_SUCCESS(rc))
        {
            /*
            ...
            if (!rc)
            {
                ...
                int Ip4MuxFd;
                int ArpMuxFd;
                rc = nemuNetFltSolarisMuxIdToFd(pUdp4VNode, Ip4Interface.lifr_ip_muxid, &Ip4MuxFd);
                rc2 = nemuNetFltSolarisMuxIdToFd(pUdp4VNode, Ip4Interface.lifr_arp_muxid, &ArpMuxFd);
                if (   RT_SUCCESS(rc)
                    && RT_SUCCESS(rc2))
                {
                    /*
                        ...
                        && !rc2)
                    {
                            ...
                            && VNODE_FOR_FILE_T(pIpFile))
                        {
                             ...
                             */
                            rc = nemuNetFltSolarisDetermineModPos(fAttach, pIp4VNode, &StrMod.pos);
                            rc2 = nemuNetFltSolarisDetermineModPos(fAttach, pArpVNode, &ArpStrMod.pos);
                            if (   RT_SUCCESS(rc)
                                && RT_SUCCESS(rc2))
                            {
                                /*
                                ...
                                if (!rc)
                                {
                                    ...
                                    if (!rc)
                                    {
                                         ...
                                         */
                                        rc = nemuNetFltSolarisRelinkIp4(pUdp4VNode, &Ip4Interface, Ip4MuxFd, ArpMuxFd);
                                        if (RT_SUCCESS(rc))
                                        {
                                            /*
                                             ...
                                             */
                                            nemuNetFltSolarisCloseDev(pUdp4VNodeHeld, pUdp4User);
                                            ldi_close(ArpDevHandle, FREAD | FWRITE, kcred);
                                            ldi_close(Ip4DevHandle, FREAD | FWRITE, kcred);
                                            releasef(Ip4MuxFd);
                                            releasef(ArpMuxFd);

                                            Log((DEVICE_NAME ":nemuNetFltSolarisAttachIp4: Success! %s %s@(IPv4:%d Arp:%d) "
                                        ...
                                        {
                                            LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp4: Relinking failed. Mode=%s rc=%d.\n",
                                    ...
                                    {
                                        LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp4: failed to %s the ARP stack. rc=%d\n",
                                    ...
                                    nemuNetFltSolarisRelinkIp4(pUdp4VNode, &Ip4Interface, Ip4MuxFd, ArpMuxFd);
                                }
                                else
                                {
                                    LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp4: failed to %s the IP stack. rc=%d\n",
                                            fAttach ? "inject into" : "eject from", rc));
                                }
                            }
                            else
                            {
                                LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp4: failed to find position. rc=%d rc2=%d\n", rc,
                                        rc2));
                            }
                            ...
                            LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp4: failed to get vnode from MuxFd.\n"));
                    }
                    else
                    {
                        LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp4: failed to unlink upper stream rc=%d rc2=%d.\n", rc,
                                rc2));
                    }
                }
                else
                    LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp4: failed to get MuxFd from MuxId. rc=%d rc2=%d\n", rc, rc2));
            }
            else
                LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp4: failed to get Mux Ids. rc=%d\n", rc));
            nemuNetFltSolarisCloseDev(pUdp4VNodeHeld, pUdp4User);
        }
        else
            LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp4: failed to open UDP. rc=%d\n", rc));
    ...
    else
    {
         ...
         */
        LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp4: Warning: seems '%s' is unplumbed.\n", pThis->szName));
        rc = VINF_SUCCESS;
    }
 ...
 * @param   fAttach     Is this an attach or detach.
 */
static int nemuNetFltSolarisAttachIp6(PNEMUNETFLTINS pThis, bool fAttach)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisAttachIp6 pThis=%p fAttach=%d\n", pThis, fAttach));
    ...
    if (rc)
    {
        LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp6: failed to open the IPv6 stream on '%s'.\n", pThis->szName));
        return VERR_INTNET_FLT_IF_FAILED;
    }
     ...
     */
    rc = nemuNetFltSolarisGetIfFlags(Ip6DevHandle, &Ip6Interface);
    if (RT_SUCCESS(rc))
    {
        /*
         * Open the UDP stream. We sort of cheat here and obtain the vnode so that we can perform
        ...
        rc = nemuNetFltSolarisOpenDev(UDP6_DEV_NAME, &pUdp6VNode, &pUdp6VNodeHeld, &pUdp6User);
        if (RT_SUCCESS(rc))
        {
            /*
             * Get the multiplexor IDs.
            ...
            if (!rc)
            {
                 ...
                 */
                int Ip6MuxFd;
                rc = nemuNetFltSolarisMuxIdToFd(pUdp6VNode, Ip6Interface.lifr_ip_muxid, &Ip6MuxFd);
                if (RT_SUCCESS(rc))
                {
                    /*
                     * We need to I_PUNLINK on these multiplexor IDs before we can start
                    ...
                    if (!rc)
                    {
                            ...
                            && VNODE_FOR_FILE_T(pIpFile))
                        {
                             ...
                             * Find the position on the host stack for attaching/detaching ourselves.
                             */
                            rc = nemuNetFltSolarisDetermineModPos(fAttach, pIp6VNode, &StrMod.pos);
                            if (RT_SUCCESS(rc))
                            {
                                /*
                                 * Set global data which will be grabbed by ModOpen.
                                ...
                                if (!rc)
                                {
                                     ...
                                     * otherwise we've pretty much screwed up the host interface.
                                     */
                                    rc = nemuNetFltSolarisRelinkIp6(pUdp6VNode, &Ip6Interface, Ip6MuxFd);
                                    if (RT_SUCCESS(rc))
                                    {
                                        /*
                                         * Close the devices ONLY during the return from function case; otherwise
                                         * we end up close twice which is an instant kernel panic.
                                         */
                                        nemuNetFltSolarisCloseDev(pUdp6VNodeHeld, pUdp6User);
                                        ldi_close(Ip6DevHandle, FREAD | FWRITE, kcred);
                                        releasef(Ip6MuxFd);

                                        Log((DEVICE_NAME ":nemuNetFltSolarisAttachIp6: Success! %s %s@(IPv6:%d) "
                                                "%s interface %s\n", fAttach ? "Injected" : "Ejected", StrMod.mod_name,
                                    ...
                                    {
                                        LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp6: Relinking failed. Mode=%s rc=%d.\n",
                                                fAttach ? "inject" : "eject", rc));
                                    }
                                    ...
                                    nemuNetFltSolarisRelinkIp6(pUdp6VNode, &Ip6Interface, Ip6MuxFd);
                                }
                                else
                                {
                                    LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp6: failed to %s the IP stack. rc=%d\n",
                                            fAttach ? "inject into" : "eject from", rc));
                                }
                            }
                            else
                                LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp6: failed to find position. rc=%d\n", rc));

                            releasef(Ip6MuxFd);
                        }
                        else
                             LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp6: failed to get vnode from MuxFd.\n"));
                    }
                    else
                        LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp6: failed to unlink upper stream rc=%d.\n", rc));
                }
                else
                    LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp6: failed to get MuxFd from MuxId. rc=%d\n", rc));
            }
            else
                LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp6: failed to get Mux Ids. rc=%d\n", rc));

            nemuNetFltSolarisCloseDev(pUdp6VNodeHeld, pUdp6User);
        }
        else
            LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachIp6: failed to open UDP. rc=%d\n", rc));

        rc = VERR_INTNET_FLT_IF_FAILED;
    }
    else
    {
        Log((DEVICE_NAME ":nemuNetFltSolarisAttachIp6: failed to get IPv6 flags.\n", pThis->szName));
 ...
 */
static void nemuNetFltSolarispIp6Timer(PRTTIMER pTimer, void *pvData, uint64_t iTick)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarispIp6Timer pTimer=%p pvData=%p\n", pTimer, pvData));

    PNEMUNETFLTINS pThis = (PNEMUNETFLTINS)pvData;
    if (   RT_LIKELY(pThis)
        && RT_LIKELY(pTimer))
    {
        nemunetflt_stream_t *pIp6Stream = ASMAtomicUoReadPtrT(&pThis->u.s.pIp6Stream, nemunetflt_stream_t *);
            ...
            && !fIp6Attaching)
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                nemuNetFltSolarisAttachIp6(pThis, true /* fAttach */);

                ASMAtomicUoWriteBool(&pThis->u.s.fAttaching, false);
                RTSemFastMutexRelease(pThis->u.s.hPollMtx);
            }
            else
                LogRel((DEVICE_NAME ":nemuNetFltSolarispIp6Timer failed to obtain mutex. rc=%Rrc\n", rc));
        }
    }
 ...
 * @param   pThis           The instance.
 */
static int nemuNetFltSolarisSetupIp6Polling(PNEMUNETFLTINS pThis)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisSetupIp6Polling pThis=%p\n", pThis));

    int rc = VERR_GENERAL_FAILURE;
    nemunetflt_promisc_stream_t *pPromiscStream = ASMAtomicUoReadPtrT(&pThis->u.s.pPromiscStream, nemunetflt_promisc_stream_t *);
    if (RT_LIKELY(pPromiscStream))
    {
        if (RT_LIKELY(pPromiscStream->pIp6Timer == NULL))
        {
             ...
             */
            int Interval = g_NemuNetFltSolarisPollInterval;
            if (Interval < 1 || Interval > 120)
            {
                LogRel((DEVICE_NAME ":nemuNetFltSolarisSetupIp6Polling: Invalid polling interval %d. Expected between"
                        " 1 and 120 secs.\n", Interval));
            ...
            rc = RTTimerCreateEx(&pPromiscStream->pIp6Timer, Interval * (uint64_t)1000000000, RTTIMER_FLAGS_CPU_ANY,
                                nemuNetFltSolarispIp6Timer, (void *)pThis);
            if (RT_SUCCESS(rc))
            {
                rc = RTTimerStart(pPromiscStream->pIp6Timer, 10 * (uint64_t)1000000000 /* 10 seconds to blastoff */);
                Log((DEVICE_NAME ":nemuNetFltSolarisSetupIp6Polling: Ipv6 %d second timer begins firing in 10 seconds.\n",
                     Interval));
            }
            else
                LogRel((DEVICE_NAME ":nemuNetFltSolarisSetupIp6Polling: Failed to create timer. rc=%d\n", rc));
        }
        else
        {
            LogRel((DEVICE_NAME ":nemuNetFltSolarisSetupIp6Polling: Polling already started.\n"));
            rc = VINF_SUCCESS;
        }
    }
    return rc;
 ...
 *          would panic the system (e.g. in nemuNetFltSolarisFindInstance).
 */
static int nemuNetFltSolarisDetachFromInterface(PNEMUNETFLTINS pThis)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisDetachFromInterface pThis=%p\n", pThis));

    ASMAtomicWriteBool(&pThis->fDisconnectedFromHost, true);
    nemuNetFltSolarisCloseStream(pThis);
    int rc = VINF_SUCCESS;
    if (pThis->u.s.pIp4Stream)
        rc = nemuNetFltSolarisAttachIp4(pThis, false /* fAttach */);
    if (pThis->u.s.pIp6Stream)
        rc = nemuNetFltSolarisAttachIp6(pThis, false /* fAttach */);
 ...
 * @param   pThis           The instance.
 */
static int nemuNetFltSolarisAttachToInterface(PNEMUNETFLTINS pThis)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisAttachToInterface pThis=%p\n", pThis));
     ...
     */
    ASMAtomicWriteBool(&pThis->fDisconnectedFromHost, true);
    int rc = nemuNetFltSolarisOpenStream(pThis);
    if (RT_SUCCESS(rc))
    {
        rc = nemuNetFltSolarisAttachIp4(pThis, true /* fAttach */);
        if (RT_SUCCESS(rc))
        {
            /*
             ...
             */
            int rc2 = nemuNetFltSolarisAttachIp6(pThis, true /* fAttach */);
            ...
            if (   rc2 == VERR_INTNET_FLT_IF_NOT_FOUND
                && g_NemuNetFltSolarisPollInterval != -1)
            {
                int rc3 = nemuNetFltSolarisSetupIp6Polling(pThis);
                if (RT_FAILURE(rc3))
                {
                    /*
                     * If we failed to setup Ip6 polling, warn in the release log and continue.
                     */
                    LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachToInterface IPv6 polling inactive. rc=%Rrc\n", rc3));
                }
            }
#endif
        ...
        LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachToInterface nemuNetFltSolarisOpenStream failed rc=%Rrc\n", rc));
...
static mblk_t *nemuNetFltSolarisMBlkFromSG(PNEMUNETFLTINS pThis, PINTNETSG pSG, uint32_t fDst)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisMBlkFromSG pThis=%p pSG=%p\n", pThis, pSG));
...
static int nemuNetFltSolarisMBlkToSG(PNEMUNETFLTINS pThis, mblk_t *pMsg, PINTNETSG pSG, unsigned cSegs, uint32_t fSrc)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisMBlkToSG pThis=%p pMsg=%p pSG=%p cSegs=%d\n", pThis, pMsg, pSG, cSegs));
...
static int nemuNetFltSolarisRawToUnitData(mblk_t *pMsg, mblk_t **ppDlpiMsg)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisRawToUnitData pMsg=%p\n", pMsg));

    if (DB_TYPE(pMsg) != M_DATA)
        return VERR_NO_MEMORY;

    size_t cbMsg = sizeof(dl_unitdata_ind_t) + 2 * sizeof(nemunetflt_dladdr_t);
    ...
    pDlpiData->dl_dest_addr_length = NEMUNETFLT_DLADDRL;
    pDlpiData->dl_dest_addr_offset = sizeof(dl_unitdata_ind_t);
    pDlpiData->dl_src_addr_length = NEMUNETFLT_DLADDRL;
    pDlpiData->dl_src_addr_offset = NEMUNETFLT_DLADDRL + sizeof(dl_unitdata_ind_t);

    PRTNETETHERHDR pEthHdr = (PRTNETETHERHDR)pMsg->b_rptr;

    nemunetflt_dladdr_t *pDlAddr = (nemunetflt_dladdr_t *)(pDlpiMsg->b_rptr + pDlpiData->dl_dest_addr_offset);
    pDlAddr->SAP = RT_BE2H_U16(pEthHdr->EtherType);
    bcopy(&pEthHdr->DstMac, &pDlAddr->Mac, sizeof(RTMAC));

    pDlAddr = (nemunetflt_dladdr_t *)(pDlpiMsg->b_rptr + pDlpiData->dl_src_addr_offset);
    pDlAddr->SAP = RT_BE2H_U16(pEthHdr->EtherType);
...
static int nemuNetFltSolarisUnitDataToRaw(PNEMUNETFLTINS pThis, mblk_t *pMsg, mblk_t **ppRawMsg)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisUnitDataToRaw pMsg=%p\n", pMsg));

    if (   !pMsg->b_cont
        || DB_TYPE(pMsg) != M_PROTO)
    {
        LogRel((DEVICE_NAME ":nemuNetFltSolarisUnitDataToRaw invalid input message.\n"));
    ...
    switch (pPrim->dl_primitive)
    {
        case DL_UNITDATA_IND:
        {
            ...
            nemunetflt_dladdr_t *pDLSapAddr = (nemunetflt_dladdr_t *)(pMsg->b_rptr + pDlpiMsg->dl_dest_addr_offset);
        ...
        default:
        {
            LogRel((DEVICE_NAME ":nemuNetFltSolarisUnitDataToRaw Unknown M_PROTO. This shouldn't be happening!!"));
            return VERR_NET_PROTOCOL_ERROR;
        }
    }
     ...
     * The nemuNetFltSolarisMBlkToSG function will handle chained mblk_t's.
     */
...
static int nemuNetFltSolarisQueueLoopback(PNEMUNETFLTINS pThis, nemunetflt_promisc_stream_t *pPromiscStream, mblk_t *pMsg)
{
    ...
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisQueueLoopback pThis=%p pPromiscStream=%p pMsg=%p\n", pThis, pPromiscStream, pMsg));
    ...
    PNEMUNETFLTPACKETID pCur = NULL;
    if (pPromiscStream->cLoopback < NEMUNETFLT_LOOPBACK_SIZE
            ...
            && pPromiscStream->pHead->cbPacket == 0))
    {
        do
        {
            ...
            {
                pCur = RTMemAlloc(sizeof(NEMUNETFLTPACKETID));
                if (RT_UNLIKELY(!pCur))
                {
                    rc = VERR_NO_MEMORY;
                    break;
                ...
                nemuNetFltSolarisInitPacketId(pCur, pMsg);
                ...
                Log((DEVICE_NAME ":nemuNetFltSolarisQueueLoopback initialized head. checksum=%u.\n",
                     ...
                     && pPromiscStream->pHead->cbPacket == 0)
            {
                ...
                Log((DEVICE_NAME ":nemuNetFltSolarisQueueLoopback re-used head checksum=%u cLoopback=%d.\n",
            ...
            else
            {
                ...
                Log((DEVICE_NAME ":nemuNetFltSolarisQueueLoopback added head checksum=%u cLoopback=%d.\n", pCur->Checksum,
                        pPromiscStream->cLoopback));
                break;
            }
        } while (0);
    ...
    else
    {
         ...
         */
        PNEMUNETFLTPACKETID pPrev = NULL;
        pCur = pPromiscStream->pHead;
        ...
        nemuNetFltSolarisInitPacketId(pCur, pMsg);
        Log((DEVICE_NAME ":nemuNetFltSolarisQueueLoopback recycled tail!! checksum=%u cLoopback=%d\n", pCur->Checksum,
                pPromiscStream->cLoopback));
    }
...
static bool nemuNetFltSolarisIsOurMBlk(PNEMUNETFLTINS pThis, nemunetflt_promisc_stream_t *pPromiscStream, mblk_t *pMsg)
{
    ...
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisIsOurMBlk pThis=%p pMsg=%p\n", pThis, pMsg));
    ...
    while (pCur)
    {
        ...
        Log((DEVICE_NAME ":nemuNetFltSolarisIsOurMBlk found packet %p Checksum=%u cLoopback=%d\n", pMsg, Checksum,
                    pPromiscStream->cLoopback));
        break;
    }

    Log((DEVICE_NAME ":nemuNetFltSolarisIsOurMBlk returns %d.\n", fIsOurPacket));
    mutex_exit(&pThis->u.s.hMtx);
    return fIsOurPacket;
}
...
static int nemuNetFltSolarisRecv(PNEMUNETFLTINS pThis, nemunetflt_stream_t *pStream, queue_t *pQueue, mblk_t *pMsg)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisRecv pThis=%p pMsg=%p\n", pThis, pMsg));
    ...
    if (RT_UNLIKELY(MBLKL(pMsg) < sizeof(RTNETETHERHDR)))
    {
        ...
        if (cbMsg < sizeof(RTNETETHERHDR))
        {
            LogRel((DEVICE_NAME ":nemuNetFltSolarisRecv %s: packet too small. Dropping packet.\n", pThis->szName));
            return VINF_SUCCESS;
        }
        ...
        {
            LogRel((DEVICE_NAME ":nemuNetFltSolarisRecv msgpullup failed.\n"));
            return VERR_NO_MEMORY;
        }
    }
     ...
     */
    if (nemuNetFltSolarisIsOurMBlk(pThis, pPromiscStream, pMsg))
    {
    ...
    PRTNETETHERHDR pEthHdr = (PRTNETETHERHDR)pMsg->b_rptr;
    if (nemuNetFltPortSolarisIsHostMac(pThis, &pEthHdr->SrcMac))
    ...
    {
        mblk_t *pCorrectedMsg = nemuNetFltSolarisFixChecksums(pMsg);
        if (pCorrectedMsg)
            pMsg = pCorrectedMsg;
    }
    nemuNetFltSolarisAnalyzeMBlk(pMsg);
#endif
        ...
        if (pEthHdr->EtherType == RT_H2BE_U16(RTNET_ETHERTYPE_VLAN))
        {
            if (msgdsize(pMsg) > sizeof(RTNETETHERHDR) + sizeof(VLANHEADER))
            {
                    ...
                    && VLAN_ID(RT_BE2H_U16(pVlanHdr->Data)) == 0)
                {
                    ...
                    {
                        LogRel((DEVICE_NAME ":nemuNetFltSolarisRecv insufficient memory for creating VLAN stripped packet"
                                " cbMsg=%u.\n", cbEthPrefix));
                        if (fCopied)
                            freemsg(pMsg);
                        return VERR_NO_MEMORY;
     ...
     */
    unsigned cSegs = nemuNetFltSolarisMBlkCalcSGSegs(pThis, pMsg);
    ...
    if (RT_SUCCESS(rc))
        ...
        LogRel((DEVICE_NAME ":nemuNetFltSolarisMBlkToSG failed. rc=%d\n", rc));
...
static mblk_t *nemuNetFltSolarisFixChecksums(mblk_t *pMsg)
{
    LogFunc((DEVICE_NAME ":nemuNetFltSolarisFixChecksums pMsg=%p\n"));

    Assert(DB_TYPE(pMsg) == M_DATA);

    if (MBLKL(pMsg) < sizeof(RTNETETHERHDR))
    {
        LogRel((DEVICE_NAME ":nemuNetFltSolarisFixChecksums Packet shorter than ethernet header size!\n"));
        return NULL;
    }
    ...
    if (pEthHdr->EtherType == RT_H2BE_U16(RTNET_ETHERTYPE_IPV4))
    {
        ...
        if (pMsg->b_cont)
        {
            ...
            {
                LogRel((DEVICE_NAME ":nemuNetFltSolarisFixChecksums failed to alloc new message of %d bytes.\n", cbFullMsg));
                return NULL;
            }
...
{
    LogFunc((DEVICE_NAME ":nemuNetFltPortOsSetActive pThis=%p fActive=%d\n", pThis, fActive));

    /*
     * Enable/disable promiscuous mode.
     */
    nemunetflt_promisc_params_t *pData = RTMemAllocZ(sizeof(nemunetflt_promisc_params_t));
    if (RT_LIKELY(pData))
    {
        /*
            ...
            && pPromiscStream->Stream.pReadQueue)
        {
            ...
            timeout_id_t TimeoutId = qtimeout(WR(pPromiscStream->Stream.pReadQueue), nemuNetFltSolarisPromiscReqWrap,
                                              pData, 1 /* ticks */);
            ASMAtomicWritePtr(&pPromiscStream->TimeoutId, TimeoutId);
            return; /* pData will be freed by nemuNetFltSolarisPromiscReqWrap() */
        }
        else
            LogRel((DEVICE_NAME ":nemuNetFltPortOsSetActive pThis=%p fActive=%d missing stream!\n", pThis, fActive));
        RTMemFree(pData);
    }
    else
        LogRel((DEVICE_NAME ":nemuNetFltPortOsSetActive out of memory!\n"));
}


int nemuNetFltOsDisconnectIt(PNEMUNETFLTINS pThis)
{
    LogFunc((DEVICE_NAME ":nemuNetFltOsDisconnectIt pThis=%p\n", pThis));

    nemuNetFltSolarisDetachFromInterface(pThis);
...
{
    LogFunc((DEVICE_NAME ":nemuNetFltOsDeleteInstance pThis=%p\n", pThis));
...
{
    LogFunc((DEVICE_NAME ":nemuNetFltOsInitInstance pThis=%p\n"));
    ...
    {
#endif
        rc = nemuNetFltSolarisAttachToInterface(pThis);
        if (RT_SUCCESS(rc))
            return rc;

        LogRel((DEVICE_NAME ":nemuNetFltSolarisAttachToInterface failed. rc=%Rrc\n", rc));
        ...
        LogRel((DEVICE_NAME ":nemuNetFltOsInitInstance failed to create poll mutex. rc=%Rrc\n", rc));
#endif
...
{
    NOREF(pvIfData);
    LogFunc((DEVICE_NAME ":nemuNetFltPortOsXmit pThis=%p pSG=%p fDst=%d\n", pThis, pSG, fDst));
    ...
    if (fDst & INTNETTRUNKDIR_WIRE)
    {
        ...
        if (RT_LIKELY(pPromiscStream))
        {
            ...
            if (RT_LIKELY(pMsg))
            {
                Log((DEVICE_NAME ":nemuNetFltPortOsXmit INTNETTRUNKDIR_WIRE\n"));

                nemuNetFltSolarisQueueLoopback(pThis, pPromiscStream, pMsg);
                putnext(WR(pPromiscStream->Stream.pReadQueue), pMsg);
            }
            else
            {
                LogRel((DEVICE_NAME ":nemuNetFltPortOsXmit nemuNetFltSolarisMBlkFromSG failed.\n"));
                return VERR_NO_MEMORY;
            }
        }
    }
    ...
    if (fDst & INTNETTRUNKDIR_HOST)
    {
         ...
         */
        nemunetflt_stream_t *pIp4Stream = ASMAtomicUoReadPtrT(&pThis->u.s.pIp4Stream, nemunetflt_stream_t *);
        if (!pIp4Stream)
            return rc;
        ...
        if (RT_LIKELY(pMsg))
        {
            ...
            {
                Log((DEVICE_NAME ":nemuNetFltPortOsXmit INTNETTRUNKDIR_HOST ARP\n"));

                nemunetflt_stream_t *pArpStream = ASMAtomicUoReadPtrT(&pThis->u.s.pArpStream, nemunetflt_stream_t *);
                if (pArpStream)
                {
                    /*
                     ...
                     */
                    mblk_t *pDlpiMsg;
                    rc = nemuNetFltSolarisRawToUnitData(pMsg, &pDlpiMsg);
                    if (RT_SUCCESS(rc))
                    {
                        pMsg = pDlpiMsg;
                    ...
                    {
                        LogRel((DEVICE_NAME ":nemuNetFltSolarisRawToUnitData failed!\n"));
                        freemsg(pMsg);
                        rc = VERR_NO_MEMORY;
                    }
                }
            ...
            else
            {
                     ...
                     && pIp6Stream)
                 {
                      ...
                      */
                     Log((DEVICE_NAME ":nemuNetFltPortOsXmit INTNETTRUNKDIR_HOST IPv6\n"));
                 ...
                 else
                 {
                     ...
                     */
                    Log((DEVICE_NAME ":nemuNetFltPortOsXmit INTNETTRUNKDIR_HOST IPv4\n"));
        ...
        {
            LogRel((DEVICE_NAME ":nemuNetFltSolarisMBlkFromSG failed.\n"));
            rc = VERR_NO_MEMORY;
        }
    }
