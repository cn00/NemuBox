...
void Guest::i_updateStats(uint64_t iTick)
{
    ...
    /*
     * Calling SessionMachine may take time as the object resides in NemuSVC
...
{
#ifndef NEMU_WITH_DRAG_AND_DROP
...
HRESULT Guest::getAdditionsStatus(AdditionsRunLevelType_T aLevel, BOOL *aActive)
{
    ...
    switch (aLevel)
    {
        ...
        default:
            rc = setError(NEMU_E_NOT_SUPPORTED,
 ...
 * @param aOsType
 */
void Guest::i_setAdditionsInfo(com::Utf8Str aInterfaceVersion, NEMUOSTYPE aOsType)
{
    ...
    if (mData.mAdditionsVersionNew.isEmpty())
    {
        ...
        else
        {
             ...
             * interface version.
             */
            i_facilityUpdate(NemuGuestFacilityType_Graphics, NemuGuestFacilityStatus_Active,  0 /*fFlags*/, &TimeSpecTS);
        }
    }
     ...
     */
    i_facilityUpdate(NemuGuestFacilityType_Graphics,
                     i_facilityIsActive(NemuGuestFacilityType_NemuGuestDriver)
                     ? NemuGuestFacilityStatus_Active : NemuGuestFacilityStatus_Inactive,
 ...
 * state).
 *
 * @param   a_uFullVersion          NemuGuestInfo2::additionsMajor,
 *                                  NemuGuestInfo2::additionsMinor and
 *                                  NemuGuestInfo2::additionsBuild combined into
 *                                  one value by NEMU_FULL_VERSION_MAKE.
 *
 ...
 * @param   a_uRevision             See NemuGuestInfo2::additionsRevision.
 * @param   a_fFeatures             See NemuGuestInfo2::additionsFeatures.
 */
void Guest::i_setAdditionsInfo2(uint32_t a_uFullVersion, const char *a_pszName, uint32_t a_uRevision, uint32_t a_fFeatures)
{
    ...
    if (a_uFullVersion)
    {
        mData.mAdditionsVersionNew  = Utf8StrFmt(*a_pszName ? "%u.%u.%u_%s" : "%u.%u.%u",
                                                 NEMU_FULL_VERSION_GET_MAJOR(a_uFullVersion),
                                                 NEMU_FULL_VERSION_GET_MINOR(a_uFullVersion),
                                                 NEMU_FULL_VERSION_GET_BUILD(a_uFullVersion),
                                                 a_pszName);
...
bool Guest::i_facilityIsActive(NemuGuestFacilityType enmFacility)
{
...
void Guest::i_facilityUpdate(NemuGuestFacilityType a_enmFacility, NemuGuestFacilityStatus a_enmStatus,
                             uint32_t a_fFlags, PCRTTIMESPEC a_pTimeSpecTS)
{
    AssertReturnVoid(   a_enmFacility < NemuGuestFacilityType_All
                     && a_enmFacility > NemuGuestFacilityType_Unknown);
 ...
 */
void Guest::i_onUserStateChange(Bstr aUser, Bstr aDomain, NemuGuestUserState enmState,
 ...
 */
void Guest::i_setAdditionsStatus(NemuGuestFacilityType a_enmFacility, NemuGuestFacilityStatus a_enmStatus,
                                 uint32_t a_fFlags, PCRTTIMESPEC a_pTimeSpecTS)
{
    Assert(   a_enmFacility > NemuGuestFacilityType_Unknown
           && a_enmFacility <= NemuGuestFacilityType_All); /* Paranoia, VMMDev checks for this. */
     ...
     */
    if (a_enmFacility == NemuGuestFacilityType_All)
        for (FacilityMapIter it = mData.mFacilityMap.begin(); it != mData.mFacilityMap.end(); ++it)
            i_facilityUpdate((NemuGuestFacilityType)it->first, a_enmStatus, a_fFlags, a_pTimeSpecTS);
     ...
     * Recalc the runlevel.
     */
    if (i_facilityIsActive(NemuGuestFacilityType_NemuTrayClient))
        mData.mAdditionsRunLevel = AdditionsRunLevelType_Desktop;
    else if (i_facilityIsActive(NemuGuestFacilityType_NemuService))
        mData.mAdditionsRunLevel = AdditionsRunLevelType_Userland;
    else if (i_facilityIsActive(NemuGuestFacilityType_NemuGuestDriver))
        mData.mAdditionsRunLevel = AdditionsRunLevelType_System;
    else
        mData.mAdditionsRunLevel = AdditionsRunLevelType_None;
}
...
void Guest::i_setSupportedFeatures(uint32_t aCaps)
{
    ...
    i_facilityUpdate(NemuGuestFacilityType_Seamless,
                     aCaps & VMMDEV_GUEST_SUPPORTS_SEAMLESS ? NemuGuestFacilityStatus_Active : NemuGuestFacilityStatus_Inactive,
                     0 /*fFlags*/, &TimeSpecTS);
