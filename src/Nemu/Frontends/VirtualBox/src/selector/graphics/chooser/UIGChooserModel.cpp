...
/** @file
 * Nemu Qt GUI - UIGChooserModel class implementation.
 */
...
void UIGChooserModel::setCurrentItem(const QString &strDefinition)
{
    ...
    else if (strItemType == "m")
    {
        ...
        CMachine machine = nemuGlobal().virtualBox().FindMachine(strItemDescriptor);
        if (!machine.isNull())
        {
...
void UIGChooserModel::sltMachineRegistered(QString strId, bool fRegistered)
{
    ...
    if (fRegistered)
    {
        ...
        CMachine machine = nemuGlobal().virtualBox().FindMachine(strId);
...
void UIGChooserModel::prepareConnections()
{
    ...
    connect(gNemuEvents, SIGNAL(sigSnapshotTake(QString, QString)),
            this, SLOT(sltSnapshotChanged(QString, QString)));
    connect(gNemuEvents, SIGNAL(sigSnapshotDelete(QString, QString)),
            this, SLOT(sltSnapshotChanged(QString, QString)));
    connect(gNemuEvents, SIGNAL(sigSnapshotChange(QString, QString)),
            this, SLOT(sltSnapshotChanged(QString, QString)));
    connect(gNemuEvents, SIGNAL(sigSnapshotRestore(QString, QString)),
            this, SLOT(sltSnapshotChanged(QString, QString)));
}
...
void UIGChooserModel::loadGroupTree()
{
    ...
    foreach (CMachine machine, nemuGlobal().virtualBox().GetMachines())
    {
