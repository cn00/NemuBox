; $Id: NemuGuestAdditions.nsi $
; @file
; NemuGuestAdditions.nsi - Main file for Windows Guest Additions installation.
;
...
!define WHQL_FAKE                   ; Enables faking of non WHQL signed / approved drivers
                                    ; Needs the NemuWHQLFake.exe in the additions output directory!
!define WFP_FILE_EXCEPTION          ; Enables setting a temporary file exception for WFP proctected files

!define VENDOR_ROOT_KEY             "SOFTWARE\$%NEMU_VENDOR_SHORT%"

; Product defines
!define PRODUCT_NAME                "$%NEMU_PRODUCT% Guest Additions"
!define PRODUCT_DESC                "$%NEMU_PRODUCT% Guest Additions"
!define PRODUCT_VERSION             "$%NEMU_VERSION_MAJOR%.$%NEMU_VERSION_MINOR%.$%NEMU_VERSION_BUILD%.0"
!define PRODUCT_PUBLISHER           "$%NEMU_VENDOR%"
!define PRODUCT_COPYRIGHT           "(C) $%NEMU_C_YEAR% $%NEMU_VENDOR%"
!define PRODUCT_OUTPUT              "NemuWindowsAdditions-$%BUILD_TARGET_ARCH%.exe"
...
VIAddVersionKey "FileVersion"       "$%NEMU_VERSION_STRING%"
...
!if $%NEMU_WITH_GUEST_INSTALL_HELPER% == "1"
  !addplugindir "$%PATH_TARGET_X86%\NemuGuestInstallHelper"
!endif
...
!ifdef USE_MUI
  ...
  ; MUI Settings
  !define MUI_WELCOMEFINISHPAGE_BITMAP "$%NEMU_BRAND_WIN_ADD_INST_DLGBMP%"
  ...
  !if $%BUILD_TARGET_ARCH% == "x86"       ; 32-bit
    !define MUI_ICON "$%NEMU_NSIS_ICON_FILE%"
    !define MUI_UNICON "$%NEMU_NSIS_ICON_FILE%"
  !else   ; 64-bit
    !define MUI_ICON "$%NEMU_WINDOWS_ADDITIONS_ICON_FILE%"
    !define MUI_UNICON "$%NEMU_WINDOWS_ADDITIONS_ICON_FILE%"
  !endif

  ; Welcome page
  !insertmacro MUI_PAGE_WELCOME
!ifdef NEMU_WITH_LICENSE_DISPLAY
  ; License page
  !insertmacro MUI_PAGE_LICENSE "$(NEMU_LICENSE)"
...
!ifdef _DEBUG
  BrandingText "VirtualBox Windows Additions $%NEMU_VERSION_STRING% (r$%NEMU_SVN_REV%) - Debug Build"
!else
  BrandingText "VirtualBox Windows Additions $%NEMU_VERSION_STRING% r$%NEMU_SVN_REV%"
!endif

!ifdef NEMU_WITH_LICENSE_DISPLAY
  ; Set license language
  LicenseLangString NEMU_LICENSE ${LANG_ENGLISH} "$%NEMU_BRAND_LICENSE_RTF%"

  ; If license files not available (OSE / PUEL) build, then use the English one as default
  !ifdef NEMU_BRAND_fr_FR_LICENSE_RTF
    LicenseLangString NEMU_LICENSE ${LANG_FRENCH} "$%NEMU_BRAND_fr_FR_LICENSE_RTF%"
  !else
    LicenseLangString NEMU_LICENSE ${LANG_FRENCH} "$%NEMU_BRAND_LICENSE_RTF%"
  !endif
  !ifdef NEMU_BRAND_de_DE_LICENSE_RTF
    LicenseLangString NEMU_LICENSE ${LANG_GERMAN} "$%NEMU_BRAND_de_DE_LICENSE_RTF%"
  !else
    LicenseLangString NEMU_LICENSE ${LANG_GERMAN} "$%NEMU_BRAND_LICENSE_RTF%"
  !endif
!endif
...
Name "${PRODUCT_NAME} $%NEMU_VERSION_STRING%"
!ifdef UNINSTALLER_ONLY
  !echo "Uninstaller only!"
  OutFile "$%PATH_TARGET%\NemuWindowsAdditions-$%BUILD_TARGET_ARCH%-uninst.exe"
!else
  OutFile "NemuWindowsAdditions-$%BUILD_TARGET_ARCH%.exe"
!endif ; UNINSTALLER_ONLY

; Define default installation directory
!if $%BUILD_TARGET_ARCH% == "x86" ; 32-bit
  InstallDir  "$PROGRAMFILES32\$%NEMU_VENDOR_SHORT%\VirtualBox Guest Additions"
!else       ; 64-bit
  InstallDir  "$PROGRAMFILES64\$%NEMU_VENDOR_SHORT%\VirtualBox Guest Additions"
!endif
...
Var g_bNoNemuServiceExit                ; Cmd line: Do not quit NemuService before updating - install on next reboot
Var g_bNoNemuTrayExit                   ; Cmd line: Do not quit NemuTray before updating - install on next reboot
Var g_bNoVideoDrv                       ; Cmd line: Do not install the NemuVideo driver
Var g_bNoGuestDrv                       ; Cmd line: Do not install the NemuGuest driver
Var g_bNoMouseDrv                       ; Cmd line: Do not install the NemuMouse driver
Var g_bWithAutoLogon                    ; Cmd line: Install NemuGINA / NemuCredProv for auto logon support
...
Var g_bPostInstallStatus                ; Cmd line: Post the overall installation status to some external program (NemuTray)

; Platform parts of this installer
!include "NemuGuestAdditionsLog.nsh"
!include "NemuGuestAdditionsExternal.nsh"
!include "NemuGuestAdditionsCommon.nsh"
!if $%BUILD_TARGET_ARCH% == "x86"       ; 32-bit only
  !include "NemuGuestAdditionsNT4.nsh"
!endif
!include "NemuGuestAdditionsW2KXP.nsh"
!include "NemuGuestAdditionsVista.nsh"
!include "NemuGuestAdditionsUninstall.nsh"    ; Product uninstallation
!include "NemuGuestAdditionsUninstallOld.nsh" ; Uninstallation of deprecated versions which must be removed first
...
Function HandleCommandLine
  ...
  ${While} $2 <= $1                           ; Loop through all params
    ...
    ${Switch} $4
      ...
      ${Case} '/no_nemuservice_exit' ; Not officially documented
        StrCpy $g_bNoNemuServiceExit "true"
        ${Break}

      ${Case} '/no_nemutray_exit' ; Not officially documented
        StrCpy $g_bNoNemuTrayExit "true"
        ${Break}
...
!if $%NEMU_WITH_GUEST_INSTALL_HELPER% == "1"
      ...
      ; - should not quit NemuTray during the update, because ...
      ; - ... it should show the overall installation status
      ;   using NemuTray's balloon message feature (since Nemu 4.0)
...
usage:
  ...
  MessageBox MB_OK "${PRODUCT_NAME} Installer$\r$\n$\r$\n \
                    Usage: NemuWindowsAdditions-$%BUILD_TARGET_ARCH% [OPTIONS] [/l] [/S] [/D=<PATH>]$\r$\n$\r$\n \
                    Options:$\r$\n \
...
sun_check:
  ...
  MessageBox MB_YESNO $(NEMU_SUN_FOUND) /SD IDYES IDYES sun_uninstall
    Pop $2
    Pop $1
    Pop $0
    MessageBox MB_ICONSTOP $(NEMU_SUN_ABORTED) /SD IDOK
    Quit
...
sun_xvm_check:
  ...
  MessageBox MB_YESNO $(NEMU_SUN_FOUND) /SD IDYES IDYES sun_xvm_uninstall
    Pop $2
    Pop $1
    Pop $0
    MessageBox MB_ICONSTOP $(NEMU_SUN_ABORTED) /SD IDOK
...
innotek_check:
  ...
  MessageBox MB_YESNO $(NEMU_INNOTEK_FOUND) /SD IDYES IDYES innotek_uninstall
    Pop $2
    Pop $1
    Pop $0
    MessageBox MB_ICONSTOP $(NEMU_INNOTEK_ABORTED) /SD IDOK
    Quit
...
Function CheckForInstalledComponents
  ...
  ; NemuGINA already installed? So we need to update the installed version as well,
...
; Main Files
Section $(NEMU_COMPONENT_MAIN) SEC01

  SectionIn RO ; Section cannot be unselected (read-only)
  ${If} $g_bPostInstallStatus == "true"
    ${LogToNemuTray} "0" "${PRODUCT_NAME} update started, please wait ..."
  ${EndIf}
  ...
  ${LogVerbose} "Version: $%NEMU_VERSION_STRING% (Rev $%NEMU_SVN_REV%)"
  ${If} $g_strAddVerMaj != ""
...
nt4: ; Windows NT4
  ...
  ${If} $R0 <> "6"
    MessageBox MB_YESNO $(NEMU_NT4_NO_SP6) /SD IDYES IDYES +2
    Quit
  ${EndIf}
...
notsupported:
  ...
  MessageBox MB_ICONSTOP $(NEMU_PLATFORM_UNSUPPORTED) /SD IDOK
  goto exit

success:

  ; Write a registry key with version and installation path for later lookup
  WriteRegStr HKLM "${PRODUCT_INSTALL_KEY}" "Version" "$%NEMU_VERSION_STRING_RAW%"
  WriteRegStr HKLM "${PRODUCT_INSTALL_KEY}" "VersionExt" "$%NEMU_VERSION_STRING%"
  WriteRegStr HKLM "${PRODUCT_INSTALL_KEY}" "Revision" "$%NEMU_SVN_REV%"
...
Section /o -$(NEMU_COMPONENT_AUTOLOGON) SEC02
  ...
  ${If} $0 != ""
    ${If} $0 != "NemuGINA.dll"
      ...
      MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON1 $(NEMU_COMPONENT_AUTOLOGON_WARN_3RDPARTY) /SD IDYES IDYES install
...
install:
  ...
  ; Do we need NemuCredProv or NemuGINA?
  ${If}   $R0 == 'Vista' ; Windows Vista.
  ${OrIf} $R0 == '7'     ; Windows 7.
  ${OrIf} $R0 == '8'     ; Windows 8.
  ${OrIf} $R0 == '8_1'   ; Windows 8.1 / Windows Server 2012 R2.
  ${OrIf} $R0 == '10'    ; Windows 10.
    ; Use NemuCredProv on Vista and up.
    ${LogVerbose} "Installing VirtualBox credential provider ..."
    !insertmacro ReplaceDLL "$%PATH_OUT%\bin\additions\NemuCredProv.dll" "$g_strSystemDir\NemuCredProv.dll" "$INSTDIR"
    WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Authentication\Credential Providers\{275D3BCC-22BB-4948-A7F6-3A3054EBA92B}" "" "NemuCredProv" ; adding to (default) key
    WriteRegStr HKCR "CLSID\{275D3BCC-22BB-4948-A7F6-3A3054EBA92B}" "" "NemuCredProv"                       ; adding to (Default) key
    WriteRegStr HKCR "CLSID\{275D3BCC-22BB-4948-A7F6-3A3054EBA92B}\InprocServer32" "" "NemuCredProv.dll"    ; adding to (Default) key
    WriteRegStr HKCR "CLSID\{275D3BCC-22BB-4948-A7F6-3A3054EBA92B}\InprocServer32" "ThreadingModel" "Apartment"
  ${Else} ; Use NemuGINA on older Windows OSes (< Vista)
    ${LogVerbose} "Installing VirtualBox GINA ..."
    !insertmacro ReplaceDLL "$%PATH_OUT%\bin\additions\NemuGINA.dll" "$g_strSystemDir\NemuGINA.dll" "$INSTDIR"
    WriteRegStr HKLM "SOFTWARE\Microsoft\Windows NT\CurrentVersion\WinLogon" "GinaDLL" "NemuGINA.dll"
    ; Add Windows notification package callbacks for NemuGINA
    WriteRegStr   HKLM "SOFTWARE\Microsoft\Windows NT\CurrentVersion\WinLogon\Notify\NemuGINA" "DLLName" "NemuGINA.dll"
    WriteRegDWORD HKLM "SOFTWARE\Microsoft\Windows NT\CurrentVersion\WinLogon\Notify\NemuGINA" "Impersonate" 0
    WriteRegStr   HKLM "SOFTWARE\Microsoft\Windows NT\CurrentVersion\WinLogon\Notify\NemuGINA" "StopScreenSaver" "WnpScreenSaverStop"
  ${EndIf}
...
; Direct3D support
Section /o $(NEMU_COMPONENT_D3D) SEC03
...
!endif
  ...
  FILE "$%PATH_OUT%\bin\additions\NemuD3D8.dll"
  FILE "$%PATH_OUT%\bin\additions\NemuD3D9.dll"
...
!if $%BUILD_TARGET_ARCH% == "amd64"
    ...
    FILE "$%NEMU_PATH_ADDITIONS_WIN_X86%\NemuD3D8.dll"
    FILE "$%NEMU_PATH_ADDITIONS_WIN_X86%\NemuD3D9.dll"
    FILE "$%NEMU_PATH_ADDITIONS_WIN_X86%\wined3d.dll"
    ...
    ${If} $g_bCapDllCache == "true"
      ${If} ${FileExists} "$g_strSysWow64\dllcache"
        ...
        ${InstallFileEx} "" "$%NEMU_PATH_ADDITIONS_WIN_X86%\d3d8.dll" "$g_strSysWow64\dllcache\d3d8.dll" "$TEMP"
        ${PrepareWRPFileEx} "" "$g_strSysWow64\dllcache\d3d9.dll"
        ${InstallFileEx} "" "$%NEMU_PATH_ADDITIONS_WIN_X86%\d3d9.dll" "$g_strSysWow64\dllcache\d3d9.dll" "$TEMP"
      ${Else}
        ${LogVerbose} "DLL cache does not exist, skipping"
      ${EndIf}
    ${EndIf}
    ...
    ${InstallFileEx} "" "$%NEMU_PATH_ADDITIONS_WIN_X86%\d3d8.dll" "$g_strSysWow64\d3d8.dll" "$TEMP"
    ${PrepareWRPFileEx} "" "$g_strSysWow64\d3d9.dll"
    ${InstallFileEx} "" "$%NEMU_PATH_ADDITIONS_WIN_X86%\d3d9.dll" "$g_strSysWow64\d3d9.dll" "$TEMP"
...
!ifndef WFP_FILE_EXCEPTION
  MessageBox MB_ICONINFORMATION|MB_OK $(NEMU_WFP_WARN_REPLACE) /SD IDOK
!endif
  Goto exit
...
!ifdef USE_MUI
  ...
  !insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${SEC01} $(NEMU_COMPONENT_MAIN_DESC)
    !insertmacro MUI_DESCRIPTION_TEXT ${SEC02} $(NEMU_COMPONENT_AUTOLOGON_DESC)
  !if $%NEMU_WITH_CROGL% == "1"
    !insertmacro MUI_DESCRIPTION_TEXT ${SEC03} $(NEMU_COMPONENT_D3D_DESC)
  !endif
...
!ifdef _DEBUG
  ...
  WriteRegDWORD HKLM "SYSTEM\CurrentControlSet\Services\NemuGuest" "LoggingEnabled" 255
!endif
...
!if $%NEMU_WITH_WDDM% == "1"
    ...
    ${If} $g_bCapWDDM == "true"
      ...
      ;
      MessageBox MB_ICONQUESTION|MB_YESNO $(NEMU_COMPONENT_D3D_OR_WDDM) /SD IDNO IDYES d3d_install
      ...
      MessageBox MB_ICONINFORMATION|MB_OK $(NEMU_COMPONENT_D3D_HINT_VRAM) /SD IDOK
      StrCpy $g_bWithWDDM "true"
      Goto exit
    ${EndIf}
...
!endif ; $%NEMU_WITH_WDDM% == "1"
    ...
    ${If} $g_bForceInstall != "true"
      ...
      ${OrIf} $g_strWinVersion == ""
        MessageBox MB_ICONINFORMATION|MB_OK $(NEMU_COMPONENT_D3D_NOT_SUPPORTED) /SD IDOK
        Goto d3d_disable
      ${EndIf}
    ${EndIf}
    ...
    ${If} $g_bForceInstall != "true"
      ...
      ${If} $g_iSystemMode == '0'
        MessageBox MB_ICONINFORMATION|MB_OK $(NEMU_COMPONENT_D3D_NO_SM) /SD IDOK
        Goto d3d_disable
      ${EndIf}
    ${EndIf}
...
Function .onInstFailed
  ...
  ${LogVerbose} "$(NEMU_ERROR_INST_FAILED)"
  MessageBox MB_ICONSTOP $(NEMU_ERROR_INST_FAILED) /SD IDOK

  ${If} $g_bPostInstallStatus == "true"
    ${LogToNemuTray} "2" "Error while installing ${PRODUCT_NAME}!"
  ${EndIf}
...
Function .onInstSuccess
  ...
  ${If} $g_bPostInstallStatus == "true"
    ${LogToNemuTray} "0" "${PRODUCT_NAME} successfully updated!"
  ${EndIf}
...
Function .onInit
  ...
  StrCpy $g_iSfOrder "0"
  StrCpy $g_bNoNemuServiceExit "false"
  StrCpy $g_bNoNemuTrayExit "false"
...
!else
  ...
  System::Call 'kernel32::CreateMutexA(i 0, i 0, t "NemuGuestInstaller") ?e'
  Pop $0
  ${If} $0 != 0
    Quit
  ${EndIf}
  ...
  ${If} $g_bOnlyExtract == "true"
    Call ExtractFiles
    MessageBox MB_OK|MB_ICONINFORMATION $(NEMU_EXTRACTION_COMPLETE) /SD IDOK
    Quit
  ${EndIf}
...
!if $%BUILD_TARGET_ARCH% == "amd64"
    MessageBox MB_ICONSTOP $(NEMU_NOTICE_ARCH_AMD64) /SD IDOK
!else
    MessageBox MB_ICONSTOP $(NEMU_NOTICE_ARCH_X86) /SD IDOK
!endif
    Abort "$(NEMU_NOTICE_ARCH_AMD64)"
  ${EndIf}
  ...
  Pop $0
  ${If} $0 != "Admin"
    MessageBox MB_ICONSTOP $(NEMU_NOADMIN) /SD IDOK
    Abort
  ${EndIf}
  ...
  ${If} $g_bUninstall == "true"
    Call Uninstall_Innotek
    Call Uninstall
    MessageBox MB_ICONINFORMATION|MB_OK $(NEMU_UNINST_SUCCESS) /SD IDOK
    Quit
  ${EndIf}
...
!ifdef USE_MUI
  ...
  !ifdef NEMU_INSTALLER_ADD_LANGUAGES
...
Function un.onInit
  ...
  MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2 $(NEMU_UNINST_CONFIRM) /SD IDYES IDYES proceed
  Quit
...
!ifndef _DEBUG
  SetAutoClose true
  MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2 $(NEMU_REBOOT_REQUIRED) /SD IDNO IDYES restart
