  ...
  )
{
  ...
  //
  NemuLogFlowFuncEnter();
  NemuLogFlowFuncMarkVar(Channel, "%d");
  *MaxDevices = 0;
  *Enabled = FALSE;
  if (Channel < ICH_IDE_MAX_CHANNEL) {
    ...
    for (;u8Device < ICH_IDE_MAX_CHANNEL; ++u8Device)
    {
        ...
        UINT64  u8DH = 0;
        ASSERT(pPciIo);
        NemuLogFlowFuncMarkVar(u8Device, "%d");
        if (u8Device == 0)
            u8DH = 0xa0;
        else if (u8Device == 1)
            u8DH = 0xb0;
        ...
        NemuLogFlowFuncMarkVar(u8DH, "%x");
        NemuLogFlowFuncMarkVar(u64IoBase, "%llx");
        ...
        NemuLogFlowFuncMarkVar(u8CS, "%x");
        NemuLogFlowFuncMarkVar(u8SN, "%x");
...
#endif /* !NEMU */
    NemuLogFlowFuncMarkVar(*MaxDevices, "%d");
    NemuLogFlowFuncMarkVar(*Enabled, "%d");
