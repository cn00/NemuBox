 ...
 *
 * @return Nemu status code. Failure on invalid CFGM data.
 ...
 */
static DECLCALLBACK(NEMUSTRICTRC) vmR3SetHaltMethodCallback(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    PUVM        pUVM = pVM->pUVM;
 ...
 *
 * @returns Nemu error status (never informational statuses).
 * @param   pVM                 The cross context VM structure.
