...
class tdCpuPae1ConsoleCallbacks(nemu.ConsoleEventHandlerBase):
    """
    For catching the PAE runtime error.
    """
    def __init__(self, dArgs):
        oTstDrv  = dArgs['oTstDrv'];
        oNemuMgr = dArgs['oNemuMgr']; _ = oNemuMgr;

        nemu.ConsoleEventHandlerBase.__init__(self, dArgs, 'tdCpuPae1');
...
class tdCpuPae1(nemu.TestDriver):
    """
    PAE Test #1.
    """
    ...
    def actionConfig(self):
        ...
        oVM = self.createTestVM('tst-bs-pae', 2, sKind = 'Other', fVirtEx = False, fPae = False, \
                                sFloppy = os.path.join(self.sNemuBootSectors, 'bootsector-pae.img') );
        if oVM is None:
            return False;
        return True;
