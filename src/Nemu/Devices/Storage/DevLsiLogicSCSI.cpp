...
#define LSILOGIC_SAVED_STATE_VERSION_NEMU_30        1
                                                              ...
                                                              int rcCompletion, bool fRedo, int rcReq)
{
    ...
    else
    {
        ...
        {
            int rc = nemuscsiRequestFinished(&pThis->NemuSCSI, pSCSIRequest, rcCompletion);
...
static void lsilogicR3InitializeConfigurationPages(PLSILOGICSCSI pThis)
{
    ...
    MPT_CONFIG_PAGE_HEADER_INIT_MANUFACTURING(&pPages->ManufacturingPage0,
                                              ...
                                              MPT_CONFIGURATION_PAGE_ATTRIBUTE_PERSISTENT_READONLY);
    strncpy((char *)pPages->ManufacturingPage0.u.fields.abChipName,          "Nemu MPT Fusion", 16);
    strncpy((char *)pPages->ManufacturingPage0.u.fields.abChipRevision,      "1.0", 8);
    strncpy((char *)pPages->ManufacturingPage0.u.fields.abBoardName,         "Nemu MPT Fusion", 16);
...
static DECLCALLBACK(int) lsilogicR3IsaIOPortRead(PPDMDEVINS pDevIns, void *pvUser, RTIOPORT Port, uint32_t *pu32, unsigned cb)
{
    ...
    int rc = nemuscsiReadRegister(&pThis->NemuSCSI, iRegister, pu32);
...
static int lsilogicR3PrepareBiosScsiRequest(PLSILOGICSCSI pThis)
{
    ...
    rc = nemuscsiSetupRequest(&pThis->NemuSCSI, &pLsiReq->PDMScsiRequest, &uTargetDevice);
    ...
    memcpy(pThis->NemuSCSI.pbBuf, &ScsiInquiryData, 5);

    rc = nemuscsiRequestFinished(&pThis->NemuSCSI, &pLsiReq->PDMScsiRequest, SCSI_STATUS_OK);
...
static DECLCALLBACK(int) lsilogicR3IsaIOPortWrite(PPDMDEVINS pDevIns, void *pvUser, RTIOPORT Port, uint32_t u32, unsigned cb)
{
    ...
    uint8_t iRegister = pThis->enmCtrlType == LSILOGICCTRLTYPE_SCSI_SPI
                      ? Port - LSILOGIC_BIOS_IO_PORT
                      : Port - LSILOGIC_SAS_BIOS_IO_PORT;
    int rc = nemuscsiWriteRegister(&pThis->NemuSCSI, iRegister, (uint8_t)u32);
                                                     ...
                                                     uint8_t const *pbSrc, uint32_t *pcTransfers, unsigned cb)
{
    ...
    uint8_t iRegister = pThis->enmCtrlType == LSILOGICCTRLTYPE_SCSI_SPI
                      ? Port - LSILOGIC_BIOS_IO_PORT
                      : Port - LSILOGIC_SAS_BIOS_IO_PORT;
    int rc = nemuscsiWriteString(pDevIns, &pThis->NemuSCSI, iRegister, pbSrc, pcTransfers, cb);
    if (rc == VERR_MORE_DATA)
    {
        rc = lsilogicR3PrepareBiosScsiRequest(pThis);
        AssertRC(rc);
                                                    ...
                                                    uint8_t *pbDst, uint32_t *pcTransfers, unsigned cb)
{
    ...
    uint8_t iRegister = pThis->enmCtrlType == LSILOGICCTRLTYPE_SCSI_SPI
                      ? Port - LSILOGIC_BIOS_IO_PORT
                      : Port - LSILOGIC_SAS_BIOS_IO_PORT;
    return nemuscsiReadString(pDevIns, &pThis->NemuSCSI, iRegister, pbDst, pcTransfers, cb);
}
...
static DECLCALLBACK(int) lsilogicR3SaveExec(PPDMDEVINS pDevIns, PSSMHANDLE pSSM)
{
    ...
    nemuscsiR3SaveExec(&pThis->NemuSCSI, pSSM);
...
static DECLCALLBACK(int) lsilogicR3LoadExec(PPDMDEVINS pDevIns, PSSMHANDLE pSSM, uint32_t uVersion, uint32_t uPass)
{
    ...
    if (    uVersion != LSILOGIC_SAVED_STATE_VERSION
        ...
        &&  uVersion != LSILOGIC_SAVED_STATE_VERSION_NEMU_30)
    ...
    if (uVersion > LSILOGIC_SAVED_STATE_VERSION_NEMU_30)
    {
        for (unsigned i = 0; i < pThis->cDeviceStates; i++)
        {
            bool fPresent;
    ...
    rc = nemuscsiR3LoadExec(&pThis->NemuSCSI, pSSM);
    if (RT_FAILURE(rc))
    {
        ...
        if (pThis->fRedo)
        {
            ...
            while (pLsiReq)
            {
                ...
                else
                {
                    ...
                    nemuscsiSetRequestRedo(&pThis->NemuSCSI, &pLsiReq->PDMScsiRequest);
                }
...
static void lsilogicR3ResetCommon(PPDMDEVINS pDevIns)
{
    ...
    nemuscsiInitialize(&pThis->NemuSCSI);
}
...
# ifdef NEMU_WITH_MSI_DEVICES
    PCIDevSetStatus(&pThis->PciDev,   NEMU_PCI_STATUS_CAP_LIST);
...
# endif
    ...
    rc = nemuscsiInitialize(&pThis->NemuSCSI);
    AssertRC(rc);
