/* $Id: NemuGuestR3LibHostChannel.cpp $ */
/** @file
 * NemuGuestR3Lib - Ring-3 Support Library for VirtualBox guest additions, Host Channel.
 */
...
{
    NemuGuestHGCMConnectInfo connectInfo;
    RT_ZERO(connectInfo);

    connectInfo.result = VERR_WRONG_ORDER;
    connectInfo.Loc.type = VMMDevHGCMLoc_LocalHost_Existing;
    strcpy(connectInfo.Loc.u.host.achName, "NemuHostChannel");
    connectInfo.u32ClientID = 0;

    int rc = vbglR3DoIOCtl(NEMUGUEST_IOCTL_HGCM_CONNECT, &connectInfo, sizeof(connectInfo));
...
{
    NemuGuestHGCMDisconnectInfo disconnectInfo;
    disconnectInfo.result = VERR_WRONG_ORDER;
    disconnectInfo.u32ClientID = u32HGCMClientId;

    vbglR3DoIOCtl(NEMUGUEST_IOCTL_HGCM_DISCONNECT, &disconnectInfo, sizeof(disconnectInfo));
}
                                        ...
                                        uint32_t u32Flags)
{
    ...
    NemuHostChannelAttach parms;

    parms.hdr.result = VERR_WRONG_ORDER;
    parms.hdr.u32ClientID = u32HGCMClientId;
    parms.hdr.u32Function = NEMU_HOST_CHANNEL_FN_ATTACH;
    parms.hdr.cParms = 3;
    ...
    int rc = vbglR3DoIOCtl(NEMUGUEST_IOCTL_HGCM_CALL(sizeof(parms)), &parms, sizeof(parms));
...
VBGLR3DECL(void) VbglR3HostChannelDetach(uint32_t u32ChannelHandle,
                                         uint32_t u32HGCMClientId)
{
    NemuHostChannelDetach parms;

    parms.hdr.result = VERR_WRONG_ORDER;
    parms.hdr.u32ClientID = u32HGCMClientId;
    parms.hdr.u32Function = NEMU_HOST_CHANNEL_FN_DETACH;
    parms.hdr.cParms = 1;

    VbglHGCMParmUInt32Set(&parms.handle, u32ChannelHandle);

    vbglR3DoIOCtl(NEMUGUEST_IOCTL_HGCM_CALL(sizeof(parms)), &parms, sizeof(parms));
}
...
VBGLR3DECL(int) VbglR3HostChannelSend(uint32_t u32ChannelHandle,
                                      ...
                                      uint32_t cbData)
{
    NemuHostChannelSend parms;

    parms.hdr.result = VERR_WRONG_ORDER;
    parms.hdr.u32ClientID = u32HGCMClientId;
    parms.hdr.u32Function = NEMU_HOST_CHANNEL_FN_SEND;
    parms.hdr.cParms = 2;
...
{
    NemuHostChannelRecv parms;

    parms.hdr.result = VERR_WRONG_ORDER;
    parms.hdr.u32ClientID = u32HGCMClientId;
    parms.hdr.u32Function = NEMU_HOST_CHANNEL_FN_RECV;
    parms.hdr.cParms = 4;
...
{
    NemuHostChannelControl parms;

    parms.hdr.result = VERR_WRONG_ORDER;
    parms.hdr.u32ClientID = u32HGCMClientId;
    parms.hdr.u32Function = NEMU_HOST_CHANNEL_FN_CONTROL;
    parms.hdr.cParms = 5;
...
{
    NemuHostChannelEventWait parms;

    parms.hdr.result = VERR_WRONG_ORDER;
    parms.hdr.u32ClientID = u32HGCMClientId;
    parms.hdr.u32Function = NEMU_HOST_CHANNEL_FN_EVENT_WAIT;
    parms.hdr.cParms = 4;
...
VBGLR3DECL(int) VbglR3HostChannelEventCancel(uint32_t u32ChannelHandle,
                                             uint32_t u32HGCMClientId)
{
    NemuHostChannelEventCancel parms;

    parms.hdr.result = VERR_WRONG_ORDER;
    parms.hdr.u32ClientID = u32HGCMClientId;
    parms.hdr.u32Function = NEMU_HOST_CHANNEL_FN_EVENT_CANCEL;
    parms.hdr.cParms = 0;
                                       ...
                                       uint32_t *pu32SizeDataReturned)
{
    ...
    NemuHostChannelQuery parms;

    parms.hdr.result = VERR_WRONG_ORDER;
    parms.hdr.u32ClientID = u32HGCMClientId;
    parms.hdr.u32Function = NEMU_HOST_CHANNEL_FN_QUERY;
    parms.hdr.cParms = 5;
