...
#ifdef USE_MEM_TABLES
  //
  // NEMU already has tables prepared in memory - just reuse them.
  //
  FillSysTablesInfo(NemuTables, sizeof(NemuTables)/sizeof(NemuTables[0]));
#else
  //
...
#ifdef USE_MEM_TABLES
    CurrentTable = (EFI_ACPI_COMMON_HEADER *)NemuTables[Instance];
