...
int DnDManager::addMessage(uint32_t uMsg, uint32_t cParms, NEMUHGCMSVCPARM paParms[], bool fAppend /* = true */)
{
    int rc = VINF_SUCCESS;
    ...
    try
    {
        ...
        switch (uMsg)
        {
            ...
            /* New since protocol version 2 (Nemu 5.0). */
            ...
            case DragAndDropSvc::HOST_DND_GH_REQ_PENDING:
            {
                ...
                if (   cParms != 1
                    || paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT /* screen id */)
                {
            ...
            case DragAndDropSvc::HOST_DND_GH_EVT_DROPPED:
            {
                ...
                if (   cParms != 3
                    ...
                    || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT /* format size */
                    || paParms[2].type != NEMU_HGCM_SVC_PARM_32BIT /* action */)
                {
                    rc = VERR_INVALID_PARAMETER;
                }
                break;
...
int DnDManager::nextMessage(uint32_t uMsg, uint32_t cParms, NEMUHGCMSVCPARM paParms[])
{
