...
#ifndef ___Nemu_vmm_cpumctx_h
#define ___Nemu_vmm_cpumctx_h
 ...
 */
#ifdef NEMU_FOR_DTRACE_LIB
# define CPUM_UNION_NAME(a_Nm)  a_Nm
#elif defined(NEMU_WITHOUT_UNNAMED_UNIONS)
# define CPUM_UNION_NAME(a_Nm)  a_Nm
#else
 ...
 * CPU context.
 */
#pragma pack(1) /* for NEMUIDTR / NEMUGDTR. */
 ...
 *
 */
#if defined(NEMU_STRICT) && defined(RT_COMPILER_SUPPORTS_LAMBDA)
...
#elif defined(NEMU_STRICT) && defined(__GNUC__)
# define CPUMCTX_XSAVE_C_PTR(a_pCtx, a_iCompBit, a_PtrType) \
    __extension__ (\
    { \
        AssertCompile((a_iCompBit) < 64U); \
 ...
 *          can in theory be changed, but keep older Nemu versions in mind.
 */
