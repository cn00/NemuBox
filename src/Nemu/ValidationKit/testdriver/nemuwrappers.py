...
def _ControllerNameToBus(sController):
    ...
    if sController == "IDE Controller":
        iType = nemucon.StorageBus_IDE;
    elif sController == "SATA Controller":
        iType = nemucon.StorageBus_SATA;
    elif sController == "Floppy Controller":
        iType = nemucon.StorageBus_Floppy;
    elif sController == "SAS Controller":
        iType = nemucon.StorageBus_SAS;
    elif sController == "SCSI Controller":
        iType = nemucon.StorageBus_SCSI;
    else:
        iType = nemucon.StorageBus_Null;
    return iType;

def _nameMachineState(eState):
    """ Gets the name (string) of a machine state."""
    if eState == nemucon.MachineState_PoweredOff: return 'PoweredOff';
    if eState == nemucon.MachineState_Saved: return 'Saved';
    if eState == nemucon.MachineState_Teleported: return 'Teleported';
    if eState == nemucon.MachineState_Aborted: return 'Aborted';
    if eState == nemucon.MachineState_Running: return 'Running';
    if eState == nemucon.MachineState_Paused: return 'Paused';
    if eState == nemucon.MachineState_Stuck: return 'GuruMeditation';
    if eState == nemucon.MachineState_Teleporting: return 'Teleporting';
    if eState == nemucon.MachineState_LiveSnapshotting: return 'LiveSnapshotting';
    if eState == nemucon.MachineState_Starting: return 'Starting';
    if eState == nemucon.MachineState_Stopping: return 'Stopping';
    if eState == nemucon.MachineState_Saving: return 'Saving';
    if eState == nemucon.MachineState_Restoring: return 'Restoring';
    if eState == nemucon.MachineState_TeleportingPausedVM: return 'TeleportingPausedVM';
    if eState == nemucon.MachineState_TeleportingIn: return 'TeleportingIn';
    if eState == nemucon.MachineState_FaultTolerantSyncing: return 'FaultTolerantSyncing';
    if eState == nemucon.MachineState_DeletingSnapshotOnline: return 'DeletingSnapshotOnline';
    if eState == nemucon.MachineState_DeletingSnapshotPaused: return 'DeletingSnapshotPaused';
    if eState == nemucon.MachineState_RestoringSnapshot: return 'RestoringSnapshot';
    if eState == nemucon.MachineState_DeletingSnapshot: return 'DeletingSnapshot';
    if eState == nemucon.MachineState_SettingUp: return 'SettingUp';
...
class VirtualBoxWrapper(object): # pylint: disable=R0903
    ...
    def __init__(self, oNemu, oNemuMgr, fpApiVer, oTstDrv):
        self.o          = oNemu;
        self.oNemuMgr   = oNemuMgr;
    ...
    def registerDerivedEventHandler(self, oSubClass, dArgs = None):
        ...
        dArgsCopy['oNemu'] = self;
        return oSubClass.registerDerivedEventHandler(self.oNemuMgr, self.fpApiVer, oSubClass, dArgsCopy,
    ...
    def deleteHdByLocation(self, sHdLocation):
        ...
        """
        try:
            oIMedium = self.oNemu.findHardDisk(sHdLocation);
        except:
            try:
                if self.fpApiVer >= 4.1:
                    oIMedium = self.oNemu.openMedium(sHdLocation, nemucon.DeviceType_HardDisk,
                                                     nemucon.AccessMode_ReadWrite, False);
                elif self.fpApiVer >= 4.0:
                    oIMedium = self.oNemu.openMedium(sHdLocation, nemucon.DeviceType_HardDisk,
                                                     nemucon.AccessMode_ReadWrite);
                else:
                    oIMedium = self.oNemu.openHardDisk(sHdLocation, nemucon.AccessMode_ReadOnly, False, "", False, "");
            except:
    ...
    def deleteHdByMedium(self, oIMedium):
        ...
        try:    oProgress = ProgressWrapper(oProgressCom, self.oNemuMgr, self.oTstDrv, 'delete disk %s' % (oIMedium.location));
...
class ProgressWrapper(TdTaskBase):
    ...
    def __init__(self, oProgress, oNemuMgr, oTstDrv, sName):
    ...
    def stringifyErrorInfo(self):
        ...
        try:
            ...
            sErr = nemu.stringifyErrorInfo(oErrInfo);
        return sErr;
    ...
    def stringifyResult(self):
        ...
        if self.isCompleted():
            if self.wasCanceled():
                sRet = 'Progress %s: Canceled, hrc=%s' % (self.sName, nemu.ComError.toString(self.getResult()));
            elif self.getResult() == 0:
                sRet = 'Progress %s: Success' % (self.sName,);
            elif self.getResult() > 0:
                sRet = 'Progress %s: Success (hrc=%s)' % (self.sName, nemu.ComError.toString(self.getResult()));
            else:
    ...
    def waitOnProgress(self, cMsInterval = 1000):
        """
        See nemu.TestDriver.waitOnProgress.
        """
        self.doQuickApiTest();
...
class SessionWrapper(TdTaskBase):
    ...
    def __init__(self, oSession, oVM, oNemu, oNemuMgr, oTstDrv, fRemoteSession, sFallbackName = None, sLogFile = None):
        """
    ...
    def __pollTask(self):
        ...
        try:
            try:
                ...
                if nemu.ComError.notEqual(oXcpt, nemu.ComError.E_UNEXPECTED):
                    reporter.logXcpt();
                return True;
        finally:
            self.oTstDrv.processPendingEvents();
        ...
        # Switch
        if eState == nemucon.MachineState_Running:
            return False;
        if eState == nemucon.MachineState_Paused:
            return False;
        if eState == nemucon.MachineState_Teleporting:
            return False;
        if eState == nemucon.MachineState_LiveSnapshotting:
            return False;
        if eState == nemucon.MachineState_Starting:
            return False;
        if eState == nemucon.MachineState_Stopping:
            return False;
        if eState == nemucon.MachineState_Saving:
            return False;
        if eState == nemucon.MachineState_Restoring:
            return False;
        if eState == nemucon.MachineState_TeleportingPausedVM:
            return False;
        if eState == nemucon.MachineState_TeleportingIn:
            return False;

        # *Beeep* fudge!
        if self.fpApiVer < 3.2 \
          and eState == nemucon.MachineState_PoweredOff \
    ...
    def waitForTask(self, cMsTimeout = 0):
        ...
        while not fState:
            ...
            if cMsElapsed > cMsTimeout:
                break;
            try:    self.oNemuMgr.waitForEvents(cMsTimeout - cMsElapsed);
            except KeyboardInterrupt: raise;
            except: pass;
            fState = self.pollTask(False);
        return fState;
    ...
    def registerEventHandlerForTask(self):
        ...
        if self.oConsoleEventHandler is not None:
            return True;
        self.oConsoleEventHandler = self.registerDerivedEventHandler(nemu.SessionConsoleEventHandler, {}, False);
    ...
    def assertPoweredOff(self):
        ...
        if eState == nemucon.MachineState_PoweredOff:
            return True;
    ...
    def getMachineStateWithName(self):
        ...
        try:
            ...
            return (None, '[error getting state: %s]' % (self.oNemuMgr.xcptToString(),));
        finally:
            self.oTstDrv.processPendingEvents();
    ...
    def reportPrematureTermination(self, sPrefix = ''):
        ...
        if    enmState is not None \
          and enmState == nemucon.MachineState_Aborted \
    ...
    def close(self):
        ...
        nemu.TestDriver.aoRemoteSessions list.
        ...
        if self.o is not None:
            ...
            try:
                ...
                # Kludge to ignore NemuSVC's closing of our session when the
                # direct session closes / VM process terminates.  Fun!
                try:    fIgnore = self.o.state == nemucon.SessionState_Unlocked;
    ...
    def enableVirtEx(self, fEnable):
        ...
        fRc = True;
        try:
            self.o.machine.setHWVirtExProperty(nemucon.HWVirtExPropertyType_Enabled, fEnable);
        except:
        ...
        # Force/unforce it.
        if fRc and hasattr(nemucon, 'HWVirtExPropertyType_Force'):
            try:
                self.o.machine.setHWVirtExProperty(nemucon.HWVirtExPropertyType_Force, fEnable);
            except:
            ...
            reporter.log('Warning! nemucon has no HWVirtExPropertyType_Force attribute.');
            ## @todo Modify CFGM to do the same for old Nemu versions?
    ...
    def enableNestedPaging(self, fEnable):
        ...
        fRc = True;
        try:
            self.o.machine.setHWVirtExProperty(nemucon.HWVirtExPropertyType_NestedPaging, fEnable);
        except:
    ...
    def enableLongMode(self, fEnable):
        ...
        """
        # Supported.
        if self.fpApiVer < 4.2  or  not hasattr(nemucon, 'HWVirtExPropertyType_LongMode'):
            return True;

        # Enable/disable it.
        fRc = True;
        try:
            self.o.machine.setCPUProperty(nemucon.CPUPropertyType_LongMode, fEnable);
        except:
    ...
    def enablePae(self, fEnable):
        ...
        try:
            if self.fpApiVer >= 3.2:    # great, ain't it?
                self.o.machine.setCPUProperty(nemucon.CPUPropertyType_PAE, fEnable);
            else:
                self.o.machine.setCpuProperty(nemucon.CpuPropertyType_PAE, fEnable);
        except:
        ...
        try:
            if fEnable:
                if self.fpApiVer >= 4.3:
                    cOhciCtls = self.o.machine.getUSBControllerCountByType(nemucon.USBControllerType_OHCI);
                    if cOhciCtls == 0:
                        self.o.machine.AddUSBController('OHCI', nemucon.USBControllerType_OHCI);
                else:
                    self.o.machine.usbController.enabled = True;

                if self.fpApiVer >= 4.2:
                    self.o.machine.pointingHIDType = nemucon.PointingHIDType_ComboMouse;
                    self.o.machine.keyboardHIDType = nemucon.KeyboardHIDType_ComboKeyboard;
                else:
                    self.o.machine.pointingHidType = nemucon.PointingHidType_ComboMouse;
                    self.o.machine.keyboardHidType = nemucon.KeyboardHidType_ComboKeyboard;
            else:
                if self.fpApiVer >= 4.2:
                    self.o.machine.pointingHIDType = nemucon.PointingHIDType_PS2Mouse;
                    self.o.machine.keyboardHIDType = nemucon.KeyboardHIDType_PS2Keyboard;
                else:
                    self.o.machine.pointingHidType = nemucon.PointingHidType_PS2Mouse;
                    self.o.machine.keyboardHidType = nemucon.KeyboardHidType_PS2Keyboard;
        except:
        ...
        try:
            if fEnable:
                if self.fpApiVer >= 4.3:
                    ...
                    cEhciCtls = self.o.machine.getUSBControllerCountByType(nemucon.USBControllerType_EHCI);
                    if cEhciCtls == 0:
                        self.o.machine.addUSBController('EHCI', nemucon.USBControllerType_EHCI);
                else:
                    self.o.machine.usbController.enabled = True;
    ...
    def enableUsbXhci(self, fEnable):
        ...
        try:
            if fEnable:
                cXhciCtls = self.o.machine.getUSBControllerCountByType(nemucon.USBControllerType_XHCI);
                if cXhciCtls == 0:
                    self.o.machine.addUSBController('XHCI', nemucon.USBControllerType_XHCI);
            else:
                cXhciCtls = self.o.machine.getUSBControllerCountByType(nemucon.USBControllerType_XHCI);
                if cXhciCtls == 1:
    ...
    def setupBootLogo(self, fEnable, cMsLogoDisplay = 0):
        ...
        try:
            ...
            if fEnable:
                self.o.machine.BIOSSettings.bootMenuMode = nemucon.BIOSBootMenuMode_Disabled;
            else:
                self.o.machine.BIOSSettings.bootMenuMode = nemucon.BIOSBootMenuMode_MessageAndMenu;
        except:
    ...
    def getNicDriverNameFromType(self, eNicType):
        ...
        """
        if    eNicType == nemucon.NetworkAdapterType_Am79C970A \
           or eNicType == nemucon.NetworkAdapterType_Am79C973:
            sName = 'pcnet';
        elif    eNicType == nemucon.NetworkAdapterType_I82540EM \
             or eNicType == nemucon.NetworkAdapterType_I82543GC \
             or eNicType == nemucon.NetworkAdapterType_I82545EM:
            sName = 'e1000';
        elif eNicType == nemucon.NetworkAdapterType_Virtio:
            sName = 'virtio-net';
        else:
    ...
    def setupNatForwardingForTxs(self, iNic = 0, iHostPort = 5042):
        ...
        for sName in ('pcnet', 'e1000', 'virtio-net'):
            for sConfig in ('NemuInternal/Devices/%s/%u/LUN#0/AttachedDriver/Config' % (sName, iNic), \
                            'NemuInternal/Devices/%s/%u/LUN#0/Config' % (sName, iNic)):
                try:
        ...
        try:
            ...
            return False;
        if eAttType != nemucon.NetworkAttachmentType_NAT:
            return True;
        ...
        if self.fpApiVer >= 4.1:
            ...
            try:
                oNatEngine.addRedirect('txs', nemucon.NATProtocol_TCP, '127.0.0.1', '%s' % (iHostPort), '', '5042');
            except:
            ...
            if fTraceEnabled:
                sConfig = 'NemuInternal/Devices/%s/%u/LUN#0/AttachedDriver/Config' % (sName, iNic)
            else:
                sConfig = 'NemuInternal/Devices/%s/%u/LUN#0/Config' % (sName, iNic)
    ...
    def getDefaultNicName(self, eAttachmentType):
        ...
        """
        sRetName = '';
        if eAttachmentType == nemucon.NetworkAttachmentType_Bridged:
            if self.oTstDrv.sDefBridgedNic is not None:
                sRetName = self.oTstDrv.sDefBridgedNic;
            else:
                sRetName = 'eth0';
                try:
                    aoHostNics = self.oNemuMgr.getArray(self.oNemu.host, 'networkInterfaces');
                    for oHostNic in aoHostNics:
                        if   oHostNic.interfaceType == nemucon.HostNetworkInterfaceType_Bridged \
                         and oHostNic.status == nemucon.HostNetworkInterfaceStatus_Up:
                            sRetName = oHostNic.name;
                            break;
                except:
                    reporter.errorXcpt();
        elif eAttachmentType == nemucon.NetworkAttachmentType_HostOnly:
            try:
                aoHostNics = self.oNemuMgr.getArray(self.oNemu.host, 'networkInterfaces');
                for oHostNic in aoHostNics:
                    if oHostNic.interfaceType == nemucon.HostNetworkInterfaceType_HostOnly:
                        if oHostNic.status == nemucon.HostNetworkInterfaceStatus_Up:
                            sRetName = oHostNic.name;
                            break;
                        if sRetName == '':
                            sRetName = oHostNic.name;
                ...
                reporter.errorXcpt();
            if sRetName == '':
                sRetName = 'HostInterfaceNetwork-nemunet0';
        elif eAttachmentType == nemucon.NetworkAttachmentType_Internal:
            sRetName = 'NemuTest';
        elif eAttachmentType == nemucon.NetworkAttachmentType_NAT:
            sRetName = '';
        else:
            ...
            if eAttachmentType is not None:
                try:
                    if self.fpApiVer >= 4.1:
                        ...
                        if eAttachmentType == nemucon.NetworkAttachmentType_NAT:
                            oNic.attachToNAT();
                        elif eAttachmentType == nemucon.NetworkAttachmentType_Bridged:
    ...
    def setBootOrder(self, iPosition, eType):
        ...
        @param eType        device type (nemucon.DeviceType_HardDisk,
                            nemucon.DeviceType_DVD, nemucon.DeviceType_Floppy)
        """
        try:
    ...
    def attachDvd(self, sImage = None, sController = "IDE Controller", iPort = 1, iDevice = 0):
        ...
        if sImage is not None:
            ...
            try:
                oImage = self.oNemu.findDVDImage(sFullName);
            except:
                try:
                    if self.fpApiVer >= 4.1:
                        oImage = self.oNemu.openMedium(sFullName, nemucon.DeviceType_DVD, nemucon.AccessMode_ReadOnly, False);
                    elif self.fpApiVer >= 4.0:
                        oImage = self.oNemu.openMedium(sFullName, nemucon.DeviceType_DVD, nemucon.AccessMode_ReadOnly);
                    else:
                        oImage = self.oNemu.openDVDImage(sFullName, "");
                except nemu.ComException, oXcpt:
                    if oXcpt.errno != -1:
        ...
        try:
            if self.fpApiVer >= 4.0:
                self.o.machine.attachDevice(sController, iPort, iDevice, nemucon.DeviceType_DVD, oImage);
            else:
                self.o.machine.attachDevice(sController, iPort, iDevice, nemucon.DeviceType_DVD, sImageUuid);
        except:
    ...
    def attachHd(self, sHd, sController = "IDE Controller", iPort = 0, iDevice = 0, fImmutable = True, fForceResource = True):
        ...
        if fForceResource:
            ...
            sFullName = sHd;
        try:
            oHd = self.oNemu.findHardDisk(sFullName);
        except:
            try:
                if self.fpApiVer >= 4.1:
                    oHd = self.oNemu.openMedium(sFullName, nemucon.DeviceType_HardDisk, nemucon.AccessMode_ReadOnly, False);
                elif self.fpApiVer >= 4.0:
                    oHd = self.oNemu.openMedium(sFullName, nemucon.DeviceType_HardDisk, nemucon.AccessMode_ReadOnly);
                else:
                    oHd = self.oNemu.openHardDisk(sFullName, nemucon.AccessMode_ReadOnly, False, "", False, "");
            except:
                reporter.errorXcpt('failed to open hd "%s"' % (sFullName));
                return False;
        try:
            if fImmutable:
                oHd.type = nemucon.MediumType_Immutable;
            else:
                oHd.type = nemucon.MediumType_Normal;
        except:
            if fImmutable:
        ...
        try:
            if self.fpApiVer >= 4.0:
                self.o.machine.attachDevice(sController, iPort, iDevice, nemucon.DeviceType_HardDisk, oHd);
            else:
                self.o.machine.attachDevice(sController, iPort, iDevice, nemucon.DeviceType_HardDisk, oHd.id);
        except:
            reporter.errorXcpt('attachDevice("%s",%s,%s,HardDisk,"%s") failed on "%s"' \
    ...
    def createBaseHd(self, sHd, sFmt = "VDI", cb = 10*1024*1024*1024):
        ...
        try:
            if self.fpApiVer >= 5.0:
                oHd = self.oNemu.createMedium(sFmt, sHd, nemucon.AccessMode_ReadWrite, nemucon.DeviceType_HardDisk);
            else:
                oHd = self.oNemu.createHardDisk(sFmt, sHd);
            oProgressXpcom = oHd.createBaseStorage(cb, (nemucon.MediumVariant_Standard, ))
            oProgress = ProgressWrapper(oProgressXpcom, self.oNemuMgr, self.oTstDrv, 'create base disk %s' % (sHd));
            oProgress.wait();
            oProgress.logResult();
        except:
    ...
    def createDiffHd(self, oParentHd, sHd, sFmt = "VDI"):
        ...
        try:
            if self.fpApiVer >= 5.0:
                ...
                oHd = self.oNemu.createHardDisk(sFmt, sHd);
            oProgressXpcom = oParentHd.createDiffStorage(oHd, (nemucon.MediumVariant_Standard, ))
            oProgress = ProgressWrapper(oProgressXpcom, self.oNemuMgr, self.oTstDrv, 'create diff disk %s' % (sHd));
            oProgress.wait();
            oProgress.logResult();
        except:
    ...
    def createAndAttachHd(self, sHd, sFmt = "VDI", sController = "IDE Controller", cb = 10*1024*1024*1024, \
        ...
        if fRc is False:
            try:
                ...
                oProgress = ProgressWrapper(oProgressCom, self.oNemuMgr, self.oTstDrv, 'delete disk %s' % (sHd));
                oProgress.wait();
                oProgress.logResult();
    ...
    def detachHd(self, sController = "IDE Controller", iPort = 0, iDevice = 0):
        ...
        try:
            ...
            if    self.oNemuMgr.xcptIsOurXcptKind() \
              and self.oNemuMgr.xcptIsEqual(None, self.oNemuMgr.constants.NEMU_E_OBJECT_NOT_FOUND):
    ...
    def attachFloppy(self, sFloppy, sController = "Floppy Controller", iPort = 0, iDevice = 0):
        ...
        try:
            oFloppy = self.oNemu.findFloppyImage(sFullName);
        except:
            try:
                if self.fpApiVer >= 4.1:
                    oFloppy = self.oNemu.openMedium(sFullName, nemucon.DeviceType_Floppy, nemucon.AccessMode_ReadOnly, False);
                elif self.fpApiVer >= 4.0:
                    oFloppy = self.oNemu.openMedium(sFullName, nemucon.DeviceType_Floppy, nemucon.AccessMode_ReadOnly);
                else:
                    oFloppy = self.oNemu.openFloppyImage(sFullName, "");
            except:
                reporter.errorXcpt('failed to open floppy "%s"' % (sFullName));
                return False;
        ## @todo the following works but causes trouble below (asserts in main).
        #try:
        #    oFloppy.type = nemucon.MediumType_Immutable;
        #except:
        ...
        try:
            if self.fpApiVer >= 4.0:
                self.o.machine.attachDevice(sController, iPort, iDevice, nemucon.DeviceType_Floppy, oFloppy);
            else:
                self.o.machine.attachDevice(sController, iPort, iDevice, nemucon.DeviceType_Floppy, oFloppy.id);
        except:
    ...
    def setupNic(self, sType, sXXX):
        ...
        Returns True on success and False on failure.  Error information is logged.
        """
        if sType == "PCNet":        enmType = nemucon.NetworkAdapterType_Am79C973;
        elif sType == "PCNetOld":   enmType = nemucon.NetworkAdapterType_Am79C970A;
        elif sType == "E1000":      enmType = nemucon.NetworkAdapterType_I82545EM; # MT Server
        elif sType == "E1000Desk":  enmType = nemucon.NetworkAdapterType_I82540EM; # MT Desktop
        elif sType == "E1000Srv2":  enmType = nemucon.NetworkAdapterType_I82543GC; # T Server
        elif sType == "Virtio":     enmType = nemucon.NetworkAdapterType_Virtio;
        else:
    ...
    def setupPreferredConfig(self):                                             # pylint: disable=R0914
        ...
        try:
            oOsType = self.oNemu.getGuestOSType(sOsTypeId);
        except:
        ...
        try:
            ...
            if self.fpApiVer >= 3.2:
                ...
                fHpet           = False;
                eFirmwareType   = -1;
                eStorCtlType    = nemucon.StorageControllerType_PIIX4;
        except:
        ...
        if  eStorCtlType == nemucon.StorageControllerType_PIIX3 \
         or eStorCtlType == nemucon.StorageControllerType_PIIX4 \
         or eStorCtlType == nemucon.StorageControllerType_ICH6:
    ...
    def teleport(self, sHostname = 'localhost', uPort = 6500, sPassword = 'password', cMsMaxDowntime = 250):
        ...
        try:
            ...
            return None;
        return ProgressWrapper(oProgress, self.oNemuMgr, self.oTstDrv, 'teleport %s' % (self.sName,));
    ...
    def powerOff(self, fFudgeOnFailure = True):
        ...
        if rc < 0:
            self.close();
            if fFudgeOnFailure:
                nemu.reportError(oProgress, 'powerDown for "%s" failed' % (self.sName));
                self.oTstDrv.waitOnDirectSessionClose(self.oVM, 5000); # fudge
            return None;
    ...
    def restoreSnapshot(self, oSnapshot, fFudgeOnFailure = True):
        ...
        if rc < 0:
            self.close();
            if fFudgeOnFailure:
                nemu.reportError(oProgress, 'restoreSnapshot for "%s" failed' % (self.sName));
            return None;
    ...
    def deleteSnapshot(self, oSnapshot, fFudgeOnFailure = True, cMsTimeout = 30 * 1000):
        ...
        try:
            ...
            oProgress = ProgressWrapper(oProgressCom, self.oNemuMgr, self.oTstDrv, 'Delete Snapshot %s' % (oSnapshot));
    ...
    def takeSnapshot(self, sName, sDescription = '', fPause = True, fFudgeOnFailure = True, cMsTimeout = 30 * 1000):
        ...
        try:
            if     fPause is True \
               and self.oVM.state is nemucon.MachineState_Running:
            ...
            oProgress = ProgressWrapper(oProgressCom, self.oNemuMgr, self.oTstDrv, 'Take Snapshot %s' % (sName));
            oProgress.wait(cMsTimeout);
            oProgress.logResult();
        except:
        ...
        if     fPause is True \
           and self.oVM.state is nemucon.MachineState_Paused:
    ...
    def getPid(self):
        ...
        if self.uPid is None and self.o is not None and self.fRemoteSession:
            try:
                ...
                if nemu.ComError.equal(oXcpt, nemu.ComError.E_UNEXPECTED):
                    try:
                        if self.fpApiVer >= 4.2:
    ...
    def addLogsToReport(self, cReleaseLogs = 1):
        ...
        for iLog in range(0, cReleaseLogs):
            try:
                if self.fpApiVer >= 3.2:
                    ...
                    sLogFile = '%s/Nemu.log' % (self.oVM.logFolder,);
                else:
                    sLogFile = '%s/Nemu.log.%u' % (self.oVM.logFolder, iLog);
            except:
        ...
        try:
            sLogFile = os.path.join(self.oVM.logFolder, 'NemuStartup.log');
        except:
            reporter.logXcpt();
            fRc = False;
        else:
    ...
    def registerDerivedEventHandler(self, oSubClass, dArgs = None, fMustSucceed = True):
        ...
        try:
            ...
            if fMustSucceed or nemu.ComError.notEqual(oXcpt, nemu.ComError.E_UNEXPECTED):
    ...
    def enableVmmDevTestingPart(self, fEnabled, fEnableMMIO = False):
        ...
        fRc = True;
        try:
            self.o.machine.setExtraData('NemuInternal/Devices/VMMDev/0/Config/TestingEnabled',
                                        '1' if fEnabled else '');
            self.o.machine.setExtraData('NemuInternal/Devices/VMMDev/0/Config/TestingMMIO',
...
class TxsConnectTask(TdTaskBase):
    ...
    class TxsConnectTaskNemuCallback(nemu.VirtualBoxEventHandlerBase):
        """ Class for looking for IPv4 address changes on interface 0."""
        def __init__(self, dArgs):
            nemu.VirtualBoxEventHandlerBase.__init__(self, dArgs);              # pylint: disable=W0233
    ...
    def __init__(self, oSession, cMsTimeout, sIpAddr, sMacAddr, fReversedSetup):
        ...
        self.oNemu              = oSession.oNemu;
        self.oNemuEventHandler  = None;
        ...
        if fReversedSetup is True:
            ...
            self.oNemuEventHandler = self.oNemu.registerDerivedEventHandler(self.TxsConnectTaskNemuCallback, dArgs);
    ...
    def toString(self):
        return '<%s cMsTimeout=%s, sIpAddr=%s, sNextIpAddr=%s, sMacAddr=%s, fReversedSetup=%s,' \
               ' oTxsSession=%s oNemuEventHandler=%s, oNemu=%s>' \
             % (TdTaskBase.toString(self), self.cMsTimeout, self.sIpAddr, self.sNextIpAddr, self.sMacAddr, self.fReversedSetup,
                self.oTxsSession, self.oNemuEventHandler, self.oNemu);

    def _deregisterEventHandler(self):
        """Deregisters the event handler."""
        fRc = True;
        if self.oNemuEventHandler is not None:
            fRc = self.oNemuEventHandler.unregister();
            self.oNemuEventHandler = None;
        return fRc;
    ...
    def _setIp(self, sIpAddr, fInitCall = False):
        ...
        if   sIpAddr is not None \
         and sIpAddr != '':
            if nemu.isIpAddrValid(sIpAddr) or fInitCall:
                try:
