...
/** @file
 * Nemu Qt GUI - UIDownloaderExtensionPack class implementation.
 */
...
UIDownloaderExtensionPack::UIDownloaderExtensionPack()
{
    ...
    QString strTemplateSourceName(QString("%1-%2.nemu-extpack").arg(strExtPackUnderscoredName));
    QString strSourcePath(strTemplateSourcePath.arg(nemuGlobal().nemuVersionStringNormalized()));
    QString strSourceName(strTemplateSourceName.arg(nemuGlobal().nemuVersionStringNormalized()));
    QString strSource(strSourcePath + strSourceName);
    QString strTargetPath(nemuGlobal().homeFolder());
