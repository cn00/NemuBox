...
#ifndef IN_RING0
# define NEMUHTABLEDECL(_type) DECLEXPORT(_type)
#else
# define NEMUHTABLEDECL(_type) RTDECL(_type)
#endif
...
NEMUHTABLEDECL(int) CrHTableCreate(PCRHTABLE pTbl, uint32_t cSize);
DECLINLINE(void) CrHTableMoveTo(PCRHTABLE pSrcTbl, PCRHTABLE pDstTbl)
{
    *pDstTbl = *pSrcTbl;
    CrHTableCreate(pSrcTbl, 0);
}
NEMUHTABLEDECL(void) CrHTableEmpty(PCRHTABLE pTbl);
NEMUHTABLEDECL(void) CrHTableDestroy(PCRHTABLE pTbl);
NEMUHTABLEDECL(int) CrHTableRealloc(PCRHTABLE pTbl, uint32_t cNewSize);
NEMUHTABLEDECL(CRHTABLE_HANDLE) CrHTablePut(PCRHTABLE pTbl, void *pvData);
NEMUHTABLEDECL(int) CrHTablePutToSlot(PCRHTABLE pTbl, CRHTABLE_HANDLE hHandle, void* pvData);

/* note: can be called for the element returned with CrHTableIterNext w/o corrupting the iterator */
NEMUHTABLEDECL(void*) CrHTableRemove(PCRHTABLE pTbl, CRHTABLE_HANDLE hHandle);
NEMUHTABLEDECL(void*) CrHTableGet(PCRHTABLE pTbl, CRHTABLE_HANDLE hHandle);
