...
/** @file
 * Nemu Qt GUI - UIMachineWindowNormal class implementation.
 */
...
void UIMachineWindowNormal::sltHandleMenuBarConfigurationChange(const QString &strMachineID)
{
    ...
    const bool fEnabled = gEDataManager->menuBarEnabled(nemuGlobal().managedVMUuid());
...
void UIMachineWindowNormal::sltHandleStatusBarConfigurationChange(const QString &strMachineID)
{
    ...
    const bool fEnabled = gEDataManager->statusBarEnabled(nemuGlobal().managedVMUuid());
    /* Update settings action 'enable' state: */
...
void UIMachineWindowNormal::prepareVisualState()
{
...
#ifdef NEMU_GUI_WITH_CUSTOMIZATIONS1
...
#endif /* NEMU_GUI_WITH_CUSTOMIZATIONS1 */
    ...
    if (nemuGlobal().osRelease() >= MacOSXRelease_Yosemite)
    {
...
void UIMachineWindowNormal::loadSettings()
{
    ...
    /* Load window geometry: */
    {
        ...
        QRect geo = gEDataManager->machineWindowGeometry(machineLogic()->visualStateType(),
                                                         m_uScreenId, nemuGlobal().managedVMUuid());
        ...
        else
        {
            ...
            QRect availableGeo = !geo.isNull() ? nemuGlobal().availableGeometry(QPoint(geo.x(), geo.y())) :
                                                 nemuGlobal().availableGeometry(this);
...
void UIMachineWindowNormal::saveSettings()
{
    /* Save window geometry: */
    {
        gEDataManager->setMachineWindowGeometry(machineLogic()->visualStateType(),
                                                ...
                                                isMaximizedChecked(), nemuGlobal().managedVMUuid());
    }
...
{
#ifndef NEMU_GUI_WITH_CUSTOMIZATIONS1
    /* Skip if maximized: */
    if (isMaximized())
        return;
    ...
    if (fAdjustPosition)
        frameGeo = NemuGlobal::normalizeGeometry(frameGeo, nemuGlobal().availableGeometry(pos()));

    /* Finally, set the frame geometry: */
    setGeometry(frameGeo.left() + dl, frameGeo.top() + dt,
                frameGeo.width() - dl - dr, frameGeo.height() - dt - db);
#else /* NEMU_GUI_WITH_CUSTOMIZATIONS1 */
