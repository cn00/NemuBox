...
#define CR_MAX_GUEST_MONITORS NEMU_VIDEO_MAX_SCREENS
 ...
 *
 * An external component registers the redirection callbacks using crNemuServerOutputRedirectSet.
 *
 ...
 */
typedef struct {
    const void *pvContext; /* Supplied by crNemuServerOutputRedirectSet. */
...
typedef struct {
    ...
    NEMUCRCMDCTL_HGCMDISABLE_DATA DisableData;

    RTSEMEVENT hCalloutCompletionEvent;
    NEMUCRCMDCTL *pCurrentCalloutCtl;
    NEMUCRCLIENT_INFO ClientInfo;
    ...
    /* OR-ed CR_NEMU_CAP_XXX cap values
     * describing Nemu Chromium functionality caps visible to guest
     * Currently can have only CR_NEMU_CAP_TEX_PRESENT cap to notify
    ...
    NEMUCRCMD_SVRENABLE_INFO CrCmdClientInfo;
...
extern DECLEXPORT(GLboolean) crNemuServerInit(void);
extern DECLEXPORT(void) crNemuServerTearDown(void);
extern DECLEXPORT(int32_t) crNemuServerAddClient(uint32_t u32ClientID);
extern DECLEXPORT(void) crNemuServerRemoveClient(uint32_t u32ClientID);
extern DECLEXPORT(int32_t) crNemuServerClientWrite(uint32_t u32ClientID, uint8_t *pBuffer, uint32_t cbBuffer);
extern DECLEXPORT(int32_t) crNemuServerClientRead(uint32_t u32ClientID, uint8_t *pBuffer, uint32_t *pcbBuffer);
extern DECLEXPORT(int32_t) crNemuServerClientSetVersion(uint32_t u32ClientID, uint32_t vMajor, uint32_t vMinor);
...
extern DECLEXPORT(int32_t) crNemuServerClientSetPID(uint32_t u32ClientID, uint64_t pid);

extern DECLEXPORT(int32_t) crNemuServerSaveState(PSSMHANDLE pSSM);
extern DECLEXPORT(int32_t) crNemuServerLoadState(PSSMHANDLE pSSM, uint32_t version);
...
extern DECLEXPORT(int) crServerNemuWindowsShow(bool fShow);
extern DECLEXPORT(int) crServerNemuScreenshotGet(uint32_t u32Screen, uint32_t width, uint32_t height, uint32_t pitch, void *pvBuffer, CR_SCREENSHOT *pScreenshot);
extern DECLEXPORT(void) crServerNemuScreenshotRelease(CR_SCREENSHOT *pScreenshot);

extern DECLEXPORT(void) crServerNemuCompositionSetEnableStateGlobal(GLboolean fEnable);
extern DECLEXPORT(int32_t) crNemuServerSetScreenCount(int sCount);
extern DECLEXPORT(int32_t) crNemuServerUnmapScreen(int sIndex);
extern DECLEXPORT(int32_t) crNemuServerMapScreen(int sIndex, int32_t x, int32_t y, uint32_t w, uint32_t h, uint64_t winID);
extern DECLEXPORT(void) crServerNemuCompositionSetEnableStateGlobal(GLboolean fEnable);
struct VBVAINFOSCREEN;
extern DECLEXPORT(int) crNemuServerNotifyResize(const struct VBVAINFOSCREEN *pScreen, void *pvVRAM);
extern DECLEXPORT(int32_t) crNemuServerSetRootVisibleRegion(GLint cRects, const RTRECT *pRects);

extern DECLEXPORT(int32_t) crNemuServerSetOffscreenRendering(GLboolean value);

extern DECLEXPORT(int32_t) crNemuServerOutputRedirectSet(const CROutputRedirect *pCallbacks);

extern DECLEXPORT(int32_t) crNemuServerSetScreenViewport(int sIndex, int32_t x, int32_t y, uint32_t w, uint32_t h);

extern DECLEXPORT(void) crServerNemuSetNotifyEventCB(PFNCRSERVERNOTIFYEVENT pfnCb);

extern DECLEXPORT(void) crNemuServerCalloutEnable(NEMUCRCMDCTL *pCtl);
 ...
 * */
extern DECLEXPORT(int32_t) crNemuServerCrHgsmiCmd(struct NEMUVDMACMD_CHROMIUM_CMD *pCmd, uint32_t cbCmd);
extern DECLEXPORT(int32_t) crNemuServerCrHgsmiCtl(struct NEMUVDMACMD_CHROMIUM_CTL *pCtl, uint32_t cbCtl);

#endif

extern DECLEXPORT(int32_t) crNemuServerHgcmEnable(NEMUCRCMDCTL_HGCMENABLE_DATA *pData);
extern DECLEXPORT(int32_t) crNemuServerHgcmDisable(NEMUCRCMDCTL_HGCMDISABLE_DATA *pData);

extern int crNemuServerHostCtl(NEMUCRCMDCTL *pCtl, uint32_t cbCtl);
