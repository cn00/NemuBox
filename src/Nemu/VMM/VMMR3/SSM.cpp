...
{
    /** Magic string which identifies this file as a version of Nemu saved state
 ...
 *
 * @param   pStrm           The stream handle.
 * @param   rc              The Nemu error status code.
 *
 * @thread  Any.
 */
 ...
 * Flush buffered data.
 *
 * @returns Nemu status code. Returns VINF_EOF if we encounter a buffer with the
 ...
 *
 * @returns Nemu status code. VERR_EOF gets translated into VINF_EOF.
 * @param   pStrm       The stream handle.
 *
 ...
 *
 * @returns Nemu status code. Sets pSSM->rc on failure.
 * @param   pSSM            The saved state handle.
 ...
 *
 * @returns Nemu status code. Will set pSSM->rc on error.
 * @param   pSSM            The saved state handle.
 */
static int ssmR3DataFlushBuffer(PSSMHANDLE pSSM)
{
...
/**
 * Saves a Nemu unsigned integer item to the current data unit.
 *
 * @returns Nemu status code.
 * @param   pSSM            The saved state handle.
...
/**
 * Saves a Nemu signed integer item to the current data unit.
 *
 * @returns Nemu status code.
 * @param   pSSM            The saved state handle.
 ...
 *
 * @returns Nemu status code (pSSM->rc).
 * @param   pVM                 The cross context VM structure.
 * @param   pSSM                The saved state handle.
 */
 ...
 *
 * @returns Nemu status code (no need to check pSSM->rc).
 * @param   pVM                 The cross context VM structure.
...
static int ssmR3WriteHeaderAndClearPerUnitData(PVM pVM, PSSMHANDLE pSSM)
{
    ...
    FileHdr.u16VerMajor  = NEMU_VERSION_MAJOR;
    FileHdr.u16VerMinor  = NEMU_VERSION_MINOR;
    FileHdr.u32VerBuild  = NEMU_VERSION_BUILD;
 ...
 *
 * @returns Nemu status code. Set pSSM->rc on error.
 ...
 *
 * @returns Nemu status code, pSSM->rc is set on error.
 *
 * @param   pSSM            The saved state handle.
 ...
 *
 * @returns Nemu status code. Does NOT set pSSM->rc.
 * @param   pSSM            The saved state handle.
 ...
 *
 * @returns Nemu status code. Sets pSSM->rc on error.
 * @param   pSSM            The saved state handle..
...
/**
 * Loads a Nemu unsigned integer item from the current data unit.
 *
 * @returns Nemu status code.
 * @param   pSSM            The saved state handle.
...
/**
 * Loads a Nemu signed integer item from the current data unit.
 *
 * @returns Nemu status code.
 * @param   pSSM            The saved state handle.
...
static int ssmR3ValidateHeaderInfo(PSSMHANDLE pSSM, bool fHaveHostBits, bool fHaveVersion)
{
    ...
    if (fHaveVersion)
    {
            ...
            ||  pSSM->u.Read.u32SvnRev > 10000000 /*100M*/)
        {
            ...
            return VERR_SSM_INTEGRITY_NEMU_VERSION;
        }
    }
    else
        AssertLogRelReturn(   pSSM->u.Read.u16VerMajor == 0
                           ...
                           && pSSM->u.Read.u32SvnRev   == 0,
                           VERR_SSM_INTEGRITY_NEMU_VERSION);
 ...
 *
 * @returns Nemu status code.  May or may not set pSSM->rc, the returned
 ...
 * @returns Nemu status code on other failures.
 *
...
/**
 * Query what the Nemu status code of the operation is.
 *
 ...
 * to SSM.
 *
 * @returns Nemu status code of the handle, or VERR_INVALID_PARAMETER.
 * @param   pSSM            The saved state handle.
 ...
 *
 * @returns NEMU_FULL_VERSION style version number.
...
VMMR3DECL(uint32_t)     SSMR3HandleVersion(PSSMHANDLE pSSM)
{
    if (pSSM->enmOp >= SSMSTATE_LOAD_PREP)
    {
        ...
        return NEMU_FULL_VERSION_MAKE(pSSM->u.Read.u16VerMajor, pSSM->u.Read.u16VerMinor, pSSM->u.Read.u32VerBuild);
    }
    return NEMU_FULL_VERSION;
}
