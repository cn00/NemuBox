...
{
    uint32_t cViews;
    VBVAVIEW aViews[NEMU_VIDEO_MAX_SCREENS];
    VBVAMOUSESHAPEINFO mouseShapeInfo;
    bool fPaused;
    uint32_t xCursor;
    uint32_t yCursor;
    VBVAMODEHINT aModeHints[NEMU_VIDEO_MAX_SCREENS];
} VBVACONTEXT;
...
static int vbvaMousePointerShape(PVGASTATE pVGAState, VBVACONTEXT *pCtx, const VBVAMOUSEPOINTERSHAPE *pShape, HGSMISIZE cbShape)
{
    ...
    const bool fVisible = RT_BOOL(parms.fu32Flags & NEMU_MOUSE_POINTER_VISIBLE);
    const bool fAlpha =   RT_BOOL(parms.fu32Flags & NEMU_MOUSE_POINTER_ALPHA);
    const bool fShape =   RT_BOOL(parms.fu32Flags & NEMU_MOUSE_POINTER_SHAPE);
...
#define NEMUVBVASAVEDSTATE_VHWAAVAILABLE_MAGIC   0x12345678
#define NEMUVBVASAVEDSTATE_VHWAUNAVAILABLE_MAGIC 0x9abcdef0

#ifdef NEMU_WITH_VIDEOHWACCEL
static void vbvaVHWAHHCommandReinit(NEMUVHWACMD* pHdr, NEMUVHWACMD_TYPE enmCmd, int32_t iDisplay)
{
    memset(pHdr, 0, NEMUVHWACMD_HEADSIZE());
    pHdr->cRefs = 1;
    pHdr->iDisplay = iDisplay;
    pHdr->rc = VERR_NOT_IMPLEMENTED;
    pHdr->enmCmd = enmCmd;
    pHdr->Flags = NEMUVHWACMD_FLAG_HH_CMD;
}

static NEMUVHWACMD* vbvaVHWAHHCommandCreate (PVGASTATE pVGAState, NEMUVHWACMD_TYPE enmCmd, int32_t iDisplay, NEMUVHWACMD_LENGTH cbCmd)
{
    NEMUVHWACMD* pHdr = (NEMUVHWACMD*)RTMemAllocZ(cbCmd + NEMUVHWACMD_HEADSIZE());
    Assert(pHdr);
    if (pHdr)
...
DECLINLINE(void) vbvaVHWAHHCommandRelease (NEMUVHWACMD* pCmd)
{
...
DECLINLINE(void) vbvaVHWAHHCommandRetain (NEMUVHWACMD* pCmd)
{
    ASMAtomicIncU32(&pCmd->cRefs);
}

static void vbvaVHWACommandComplete(PVGASTATE pVGAState, PNEMUVHWACMD pCommand, bool fAsyncCommand)
{
    if (fAsyncCommand)
    {
        Assert(pCommand->Flags & NEMUVHWACMD_FLAG_HG_ASYNCH);
    ...
    else
    {
        ...
        pCommand->Flags &= (~NEMUVHWACMD_FLAG_HG_ASYNCH);
    }
...
static void vbvaVHWACommandCompleteAllPending(PVGASTATE pVGAState, int rc)
{
    ...
    NEMU_VHWA_PENDINGCMD *pIter, *pNext;

    PDMCritSectEnter(&pVGAState->CritSect, VERR_SEM_BUSY);

    RTListForEachSafe(&pVGAState->pendingVhwaCommands.PendingList, pIter, pNext, NEMU_VHWA_PENDINGCMD, Node)
    {
...
static void vbvaVHWACommandPend(PVGASTATE pVGAState, PNEMUVHWACMD pCommand)
{
    int rc = VERR_BUFFER_OVERFLOW;

    if (ASMAtomicUoReadU32(&pVGAState->pendingVhwaCommands.cPending) < NEMU_VHWA_MAX_PENDING_COMMANDS)
    {
        NEMU_VHWA_PENDINGCMD *pPend = (NEMU_VHWA_PENDINGCMD*)RTMemAlloc(sizeof (*pPend));
        if (pPend)
        {
            pCommand->Flags |= NEMUVHWACMD_FLAG_HG_ASYNCH;
...
static bool vbvaVHWACommandCanPend(PNEMUVHWACMD pCommand)
{
...
static int vbvaVHWACommandSavePending(PVGASTATE pVGAState, PSSMHANDLE pSSM)
{
    ...
    NEMU_VHWA_PENDINGCMD *pIter;
    RTListForEach(&pVGAState->pendingVhwaCommands.PendingList, pIter, NEMU_VHWA_PENDINGCMD, Node)
    {
...
static int vbvaVHWACommandLoadPending(PVGASTATE pVGAState, PSSMHANDLE pSSM, uint32_t u32Version)
{
    ...
    for (uint32_t i = 0; i < u32; ++i)
    {
        ...
        AssertRCReturn(rc, rc);
        PNEMUVHWACMD pCommand = (PNEMUVHWACMD)(((uint8_t*)pVGAState->vram_ptrR3) + off32);
...
static bool vbvaVHWACommandSubmit(PVGASTATE pVGAState, PNEMUVHWACMD pCommand, bool fAsyncCommand)
{
...
static void vbvaVHWAHandleCommand(PVGASTATE pVGAState, PNEMUVHWACMD pCmd)
{
...
static int vbvaVHWAHHCommandPost(PVGASTATE pVGAState, NEMUVHWACMD* pCmd)
{
    ...
    if(RT_SUCCESS(rc))
    {
        ...
        NEMUVHWA_HH_CALLBACK_SET(pCmd, vbvaVHWAHHCommandSetEventCallback, (void*)hComplEvent);
        vbvaVHWAHandleCommand(pVGAState, pCmd);
        if((ASMAtomicReadU32((volatile uint32_t *)&pCmd->Flags)  & NEMUVHWACMD_FLAG_HG_ASYNCH) != 0)
        {
...
int vbvaVHWAConstruct (PVGASTATE pVGAState)
{
    ...
    NEMUVHWACMD *pCmd = vbvaVHWAHHCommandCreate(pVGAState, NEMUVHWACMD_TYPE_HH_CONSTRUCT, 0, sizeof(NEMUVHWACMD_HH_CONSTRUCT));
    Assert(pCmd);
    if(pCmd)
    {
        uint32_t iDisplay = 0;
        ...
        do
        {
            memset(pBody, 0, sizeof(NEMUVHWACMD_HH_CONSTRUCT));
            ...
            if (iDisplay >= pVGAState->cMonitors)
                break;
            vbvaVHWAHHCommandReinit(pCmd, NEMUVHWACMD_TYPE_HH_CONSTRUCT, (int32_t)iDisplay);
        } while (true);
...
int vbvaVHWAReset (PVGASTATE pVGAState)
{
    ...
    NEMUVHWACMD *pCmd = vbvaVHWAHHCommandCreate(pVGAState, NEMUVHWACMD_TYPE_HH_RESET, 0, 0);
    Assert(pCmd);
    if(pCmd)
    {
        int rc = VINF_SUCCESS;
        ...
        do
        {
            ...
            if (iDisplay >= pVGAState->cMonitors)
                break;
            vbvaVHWAHHCommandReinit(pCmd, NEMUVHWACMD_TYPE_HH_RESET, (int32_t)iDisplay);
...
typedef DECLCALLBACK(bool) FNNEMUVHWAHHCMDPRECB(PVGASTATE pVGAState, NEMUVHWACMD *pCmd, uint32_t iDisplay, void *pvContext);
typedef FNNEMUVHWAHHCMDPRECB *PFNNEMUVHWAHHCMDPRECB;

typedef DECLCALLBACK(bool) FNNEMUVHWAHHCMDPOSTCB(PVGASTATE pVGAState, NEMUVHWACMD *pCmd, uint32_t iDisplay, int rc, void *pvContext);
typedef FNNEMUVHWAHHCMDPOSTCB *PFNNEMUVHWAHHCMDPOSTCB;

int vbvaVHWAHHPost(PVGASTATE pVGAState, NEMUVHWACMD *pCmd, PFNNEMUVHWAHHCMDPRECB pfnPre, PFNNEMUVHWAHHCMDPOSTCB pfnPost, void *pvContext)
{
    const NEMUVHWACMD_TYPE enmType = pCmd->enmCmd;
    int rc = VINF_SUCCESS;
    uint32_t iDisplay = 0;
...
{
    const NEMUVHWACMD_TYPE enmType = bEnable ? NEMUVHWACMD_TYPE_HH_ENABLE : NEMUVHWACMD_TYPE_HH_DISABLE;
    NEMUVHWACMD *pCmd = vbvaVHWAHHCommandCreate(pVGAState,
                        enmType,
                    0, 0);
    Assert(pCmd);
    if(pCmd)
...
int nemuVBVASaveStatePrep (PPDMDEVINS pDevIns, PSSMHANDLE pSSM)
{
    /* ensure we have no pending commands */
    return vbvaVHWAEnable(PDMINS_2_DATA(pDevIns, PVGASTATE), false);
}

int nemuVBVASaveStateDone (PPDMDEVINS pDevIns, PSSMHANDLE pSSM)
{
    /* ensure we have no pending commands */
    return vbvaVHWAEnable(PDMINS_2_DATA(pDevIns, PVGASTATE), true);
}

DECLCALLBACK(int) vbvaVHWACommandCompleteAsync(PPDMIDISPLAYVBVACALLBACKS pInterface, PNEMUVHWACMD pCmd)
{
    int rc;
    Log(("VGA Command <<< Async rc %d %#p, %d\n", pCmd->rc, pCmd, pCmd->enmCmd));

    if((pCmd->Flags & NEMUVHWACMD_FLAG_HH_CMD) == 0)
    {
        PVGASTATE pVGAState = PPDMIDISPLAYVBVACALLBACKS_2_PVGASTATE(pInterface);
        PHGSMIINSTANCE pIns = pVGAState->pHGSMI;

        Assert(pCmd->Flags & NEMUVHWACMD_FLAG_HG_ASYNCH);
#ifdef NEMU_WITH_WDDM
        if (pVGAState->fGuestCaps & VBVACAPS_COMPLETEGCMD_BY_IOREAD)
        {
            rc = HGSMICompleteGuestCommand(pIns, pCmd, !!(pCmd->Flags & NEMUVHWACMD_FLAG_GH_ASYNCH_IRQ));
            AssertRC(rc);
        }
        else
#endif
        {
            ...
            if(pCmd->Flags & NEMUVHWACMD_FLAG_GH_ASYNCH_EVENT)
            {
            ...
            if(RT_SUCCESS(rc))
            {
                rc = HGSMIHostCommandSubmitAndFreeAsynch(pIns, pHostCmd, RT_BOOL(pCmd->Flags & NEMUVHWACMD_FLAG_GH_ASYNCH_IRQ));
                AssertRC(rc);
                if(RT_SUCCESS(rc))
                {
                    return rc;
    ...
    {
        PFNNEMUVHWA_HH_CALLBACK pfn = NEMUVHWA_HH_CALLBACK_GET(pCmd);
        if(pfn)
        {
            pfn(NEMUVHWA_HH_CALLBACK_GET_ARG(pCmd));
        }
        rc = VINF_SUCCESS;
    }
    return rc;
...
typedef struct NEMUVBVASAVEDSTATECBDATA
{
    PSSMHANDLE pSSM;
    int rc;
    bool ab2DOn[NEMU_VIDEO_MAX_SCREENS];
} NEMUVBVASAVEDSTATECBDATA, *PNEMUVBVASAVEDSTATECBDATA;

static DECLCALLBACK(bool) nemuVBVASaveStateBeginPostCb(PVGASTATE pVGAState, NEMUVHWACMD *pCmd, uint32_t iDisplay, int rc, void *pvContext)
{
    PNEMUVBVASAVEDSTATECBDATA pData = (PNEMUVBVASAVEDSTATECBDATA)pvContext;
...
static DECLCALLBACK(bool) nemuVBVASaveStatePerformPreCb(PVGASTATE pVGAState, NEMUVHWACMD *pCmd, uint32_t iDisplay, void *pvContext)
{
    PNEMUVBVASAVEDSTATECBDATA pData = (PNEMUVBVASAVEDSTATECBDATA)pvContext;
    if (RT_FAILURE(pData->rc))
        return false;
    ...
    {
        rc = SSMR3PutU32 (pData->pSSM, NEMUVBVASAVEDSTATE_VHWAAVAILABLE_MAGIC); AssertRC(rc);
        if (RT_FAILURE(rc))
        {
            pData->rc = rc;
            return false;
    ...
    rc = SSMR3PutU32 (pData->pSSM, NEMUVBVASAVEDSTATE_VHWAUNAVAILABLE_MAGIC); AssertRC(rc);
    if (RT_FAILURE(rc))
    {
        pData->rc = rc;
        return false;
...
static DECLCALLBACK(bool) nemuVBVASaveStateEndPreCb(PVGASTATE pVGAState, NEMUVHWACMD *pCmd, uint32_t iDisplay, void *pvContext)
{
    PNEMUVBVASAVEDSTATECBDATA pData = (PNEMUVBVASAVEDSTATECBDATA)pvContext;
    Assert(iDisplay < RT_ELEMENTS(pData->ab2DOn));
    if (pData->ab2DOn[iDisplay])
...
static DECLCALLBACK(bool) nemuVBVALoadStatePerformPostCb(PVGASTATE pVGAState, NEMUVHWACMD *pCmd, uint32_t iDisplay, int rc, void *pvContext)
{
    PNEMUVBVASAVEDSTATECBDATA pData = (PNEMUVBVASAVEDSTATECBDATA)pvContext;
    if (RT_FAILURE(pData->rc))
        return false;
...
static DECLCALLBACK(bool) nemuVBVALoadStatePerformPreCb(PVGASTATE pVGAState, NEMUVHWACMD *pCmd, uint32_t iDisplay, void *pvContext)
{
    PNEMUVBVASAVEDSTATECBDATA pData = (PNEMUVBVASAVEDSTATECBDATA)pvContext;
    if (RT_FAILURE(pData->rc))
        return false;
    ...
    switch (u32)
    {
        case NEMUVBVASAVEDSTATE_VHWAAVAILABLE_MAGIC:
            pData->ab2DOn[iDisplay] = true;
            return true;
        case NEMUVBVASAVEDSTATE_VHWAUNAVAILABLE_MAGIC:
...
int nemuVBVASaveDevStateExec (PVGASTATE pVGAState, PSSMHANDLE pSSM)
{
    PHGSMIINSTANCE pIns = pVGAState->pHGSMI;
...
int nemuVBVASaveStateExec (PPDMDEVINS pDevIns, PSSMHANDLE pSSM)
{
    PVGASTATE pVGAState = PDMINS_2_DATA(pDevIns, PVGASTATE);
    int rc;
#ifdef NEMU_WITH_VIDEOHWACCEL
    NEMUVBVASAVEDSTATECBDATA VhwaData = {0};
    VhwaData.pSSM = pSSM;
    uint32_t cbCmd = sizeof (NEMUVHWACMD_HH_SAVESTATE_SAVEPERFORM); /* maximum cmd size */
    NEMUVHWACMD *pCmd = vbvaVHWAHHCommandCreate(pVGAState, NEMUVHWACMD_TYPE_HH_SAVESTATE_SAVEBEGIN, 0, cbCmd);
    Assert(pCmd);
    if(pCmd)
    {
        vbvaVHWAHHPost (pVGAState, pCmd, NULL, nemuVBVASaveStateBeginPostCb, &VhwaData);
        rc = VhwaData.rc;
        AssertRC(rc);
        if (RT_SUCCESS(rc))
        {
#endif
            rc = nemuVBVASaveDevStateExec (pVGAState, pSSM);
            AssertRC(rc);
#ifdef NEMU_WITH_VIDEOHWACCEL
            if (RT_SUCCESS(rc))
            {
                vbvaVHWAHHCommandReinit(pCmd, NEMUVHWACMD_TYPE_HH_SAVESTATE_SAVEPERFORM, 0);
                NEMUVHWACMD_HH_SAVESTATE_SAVEPERFORM *pSave = NEMUVHWACMD_BODY(pCmd, NEMUVHWACMD_HH_SAVESTATE_SAVEPERFORM);
                pSave->pSSM = pSSM;
                vbvaVHWAHHPost (pVGAState, pCmd, nemuVBVASaveStatePerformPreCb, NULL, &VhwaData);
                rc = VhwaData.rc;
                AssertRC(rc);
                if (RT_SUCCESS(rc))
                {
                    ...
                    vbvaVHWAHHCommandReinit(pCmd, NEMUVHWACMD_TYPE_HH_SAVESTATE_SAVEEND, 0);
                    vbvaVHWAHHPost (pVGAState, pCmd, nemuVBVASaveStateEndPreCb, NULL, &VhwaData);
                    rc = VhwaData.rc;
                    AssertRC(rc);
                }
            }
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        {
            rc = SSMR3PutU32 (pSSM, NEMUVBVASAVEDSTATE_VHWAUNAVAILABLE_MAGIC);
            AssertRCReturn(rc, rc);
        }
    }
...
int nemuVBVALoadStateExec (PPDMDEVINS pDevIns, PSSMHANDLE pSSM, uint32_t uVersion)
{
        ...
        else
        {
            ...
            if (uVersion > VGA_SAVEDSTATE_VERSION_WDDM)
            {
...
#ifdef NEMU_WITH_VIDEOHWACCEL
                uint32_t cbCmd = sizeof (NEMUVHWACMD_HH_SAVESTATE_LOADPERFORM); /* maximum cmd size */
                NEMUVHWACMD *pCmd = vbvaVHWAHHCommandCreate(pVGAState, NEMUVHWACMD_TYPE_HH_SAVESTATE_LOADPERFORM, 0, cbCmd);
                Assert(pCmd);
                if(pCmd)
                {
                    NEMUVBVASAVEDSTATECBDATA VhwaData = {0};
                    ...
                    pLoad->pSSM = pSSM;
                    vbvaVHWAHHPost (pVGAState, pCmd, nemuVBVALoadStatePerformPreCb, nemuVBVALoadStatePerformPostCb, &VhwaData);
                    rc = VhwaData.rc;
                    vbvaVHWAHHCommandRelease(pCmd);
                    AssertRCReturn(rc, rc);
                ...
                for (uint32_t i = 0; i < pVGAState->cMonitors; ++i)
                {
                    ...
                    if (u32 != NEMUVBVASAVEDSTATE_VHWAUNAVAILABLE_MAGIC)
                    {
...
int nemuVBVALoadStateDone (PPDMDEVINS pDevIns, PSSMHANDLE pSSM)
{
    PVGASTATE pVGAState = PDMINS_2_DATA(pDevIns, PVGASTATE);
    ...
    if (pCtx)
    {
        ...
        for (iView = 0; iView < pCtx->cViews; iView++)
        {
            ...
            if (pView->vbva.guest.pVBVA)
            {
                ...
                Assert(!nemuCmdVBVAIsEnabled(pVGAState));
#endif
...
static int vbvaHandleQueryConf32(PVGASTATE pVGAState, VBVACONF32 *pConf32)
{
    ...
    if (u32Index == NEMU_VBVA_CONF32_MONITOR_COUNT)
    {
        pConf32->u32Value = pCtx->cViews;
    }
    else if (u32Index == NEMU_VBVA_CONF32_HOST_HEAP_SIZE)
    {
        /* @todo a value calculated from the vram size */
        pConf32->u32Value = 64*_1K;
    }
    else if (   u32Index == NEMU_VBVA_CONF32_MODE_HINT_REPORTING
             || u32Index == NEMU_VBVA_CONF32_GUEST_CURSOR_REPORTING)
    {
        pConf32->u32Value = VINF_SUCCESS;
    }
    else if (u32Index == NEMU_VBVA_CONF32_CURSOR_CAPABILITIES)
    {
        pConf32->u32Value = pVGAState->fHostCursorCapabilities;
    }
    else if (u32Index == NEMU_VBVA_CONF32_SCREEN_FLAGS)
    {
        pConf32->u32Value = VBVA_SCREEN_F_ACTIVE | VBVA_SCREEN_F_DISABLED | VBVA_SCREEN_F_BLANK;
    }
    else if (u32Index == NEMU_VBVA_CONF32_MAX_RECORD_SIZE)
    {
...
static int vbvaHandleSetConf32(PVGASTATE pVGAState, VBVACONF32 *pConf32)
{
    ...
    if (parms.u32Index == NEMU_VBVA_CONF32_MONITOR_COUNT)
    {
        /* do nothing. this is a const. */
    }
    else if (parms.u32Index == NEMU_VBVA_CONF32_HOST_HEAP_SIZE)
    {
        /* do nothing. this is a const. */
    }
    else
...
static int vbvaHandleQueryModeHints(PVGASTATE pVGAState, const VBVAQUERYMODEHINTS *pQueryModeHints, HGSMISIZE cbBuffer)
{
    ...
    for (iHint = 0;    iHint < parms.cHintsQueried
                    && iHint < NEMU_VIDEO_MAX_SCREENS; ++iHint)
    {
...
/*
 *
 * New VBVA uses a new interface id: #define VBE_DISPI_ID_NEMU_VIDEO         0xBE01
 *
...
{
#if defined(NEMU_WITH_HGSMI) && (defined(NEMU_WITH_VIDEOHWACCEL) || defined(NEMU_WITH_VDMA) || defined(NEMU_WITH_WDDM))
...
static DECLCALLBACK(int) vbvaChannelHandler(void *pvHandler, uint16_t u16ChannelInfo, void *pvBuffer, HGSMISIZE cbBuffer)
{
    ...
    switch (u16ChannelInfo)
    {
        ...
        {
            rc = nemuCmdVBVACmdSubmit(pVGAState);
        } break;

        case VBVA_CMDVBVA_FLUSH:
        {
            rc = nemuCmdVBVACmdFlush(pVGAState);
        } break;

        case VBVA_CMDVBVA_CTL:
        {
            if (cbBuffer < NemuSHGSMIBufferHeaderSize() + sizeof(NEMUCMDVBVA_CTL))
            {
                rc = VERR_INVALID_PARAMETER;
                break;
            }

            NEMUCMDVBVA_CTL *pCtl = (NEMUCMDVBVA_CTL*)NemuSHGSMIBufferData((PNEMUSHGSMIHEADER)pvBuffer);
            rc = nemuCmdVBVACmdCtl(pVGAState, pCtl, cbBuffer - NemuSHGSMIBufferHeaderSize());
        } break;
        ...
        case VBVA_VDMA_CMD:
        {
            if (cbBuffer < NemuSHGSMIBufferHeaderSize() + sizeof(NEMUVDMACBUF_DR))
            {
                rc = VERR_INVALID_PARAMETER;
                break;
            }

            PNEMUVDMACBUF_DR pCmd = (PNEMUVDMACBUF_DR)NemuSHGSMIBufferData((PNEMUSHGSMIHEADER)pvBuffer);
            nemuVDMACommand(pVGAState->pVdma, pCmd, cbBuffer - NemuSHGSMIBufferHeaderSize());
        } break;

        case VBVA_VDMA_CTL:
        {
            if (cbBuffer < NemuSHGSMIBufferHeaderSize() + sizeof(NEMUVDMA_CTL))
            {
                rc = VERR_INVALID_PARAMETER;
                break;
            }

            PNEMUVDMA_CTL pCmd = (PNEMUVDMA_CTL)NemuSHGSMIBufferData((PNEMUSHGSMIHEADER)pvBuffer);
            nemuVDMAControl(pVGAState->pVdma, pCmd, cbBuffer - NemuSHGSMIBufferHeaderSize());
        } break;
#endif /* NEMU_WITH_VDMA */
        ...
        {
#ifdef NEMU_WITH_CRHGSMI
            if (nemuCmdVBVAIsEnabled(pVGAState))
            {
        ...
        case VBVA_VHWA_CMD:
        {
            if (cbBuffer < NEMUVHWACMD_HEADSIZE())
            {
                rc = VERR_INVALID_PARAMETER;
                break;
            }
            vbvaVHWAHandleCommand(pVGAState, (PNEMUVHWACMD)pvBuffer);
        } break;
