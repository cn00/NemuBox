/* $Id: NemuNetDHCP.cpp $ */
/** @file
 * NemuNetDHCP - DHCP Service for connecting to IntNet.
 */
 ...
 */
class NemuNetDhcp: public NemuNetBaseService, public NATNetworkEventAdapter
{
public:
    NemuNetDhcp();
    virtual ~NemuNetDhcp();
...
protected:
     ...
     * How it will coexists with managment from NemuManagement, who should manage db
     * in that case (NemuManage, NemuSVC ???)
     */
 ...
 */
NemuNetDhcp::NemuNetDhcp()
  : NemuNetBaseService("NemuNetDhcp", "NemuNetDhcp"),
 ...
 *    for -b, -g, -l, -u, -m
 */
int NemuNetDhcp::parseOpt(int rc, const RTGETOPTUNION& Val)
{
    CMDLNPRM prm;
...
int NemuNetDhcp::init()
{
    int rc = this->NemuNetBaseService::init();
...
int  NemuNetDhcp::processUDP(void *pv, size_t cbPv)
{
    ...
    if (RTNetIPv4IsDHCPValid(NULL /* why is this here? */, pDhcpMsg, cbPv, &uMsgType))
    {
        ...
        debugPrint(1, true, "NemuNetDHCP: Skipping invalid DHCP packet.\n"); /** @todo handle pure bootp clients too? */
 ...
 */
bool NemuNetDhcp::handleDhcpMsg(uint8_t uMsgType, PCRTNETBOOTP pDhcpMsg, size_t cb)
{
 ...
 */
void NemuNetDhcp::debugPrintV(int iMinLevel, bool fMsg, const char *pszFmt, va_list va) const
{
    if (iMinLevel <= m_cVerbosity)
    {
        va_list vaCopy;                 /* This dude is *very* special, thus the copy. */
        va_copy(vaCopy, va);
        RTStrmPrintf(g_pStdErr, "NemuNetDHCP: %s: %N\n", iMinLevel >= 2 ? "debug" : "info", pszFmt, &vaCopy);
        va_end(vaCopy);
            ...
            &&  m_pCurMsg)
        {
            ...
            RTStrmPrintf(g_pStdErr, "NemuNetDHCP: debug: %8s chaddr=%.6Rhxs ciaddr=%d.%d.%d.%d yiaddr=%d.%d.%d.%d siaddr=%d.%d.%d.%d xid=%#x\n",
                         pszMsg,
                         &m_pCurMsg->bp_chaddr,
 ...
 */
/* static */ const char *NemuNetDhcp::debugDhcpName(uint8_t uMsgType)
{
    switch (uMsgType)
    {
        case 0:                         return "MT_00";
...
int NemuNetDhcp::fetchAndUpdateDnsInfo()
{
    ComHostPtr host;
...
HRESULT NemuNetDhcp::HandleEvent(NemuEventType_T aEventType, IEvent *pEvent)
{
    switch(aEventType)
    {
...
extern "C" DECLEXPORT(int) TrustedMain(int argc, char **argv)
{
    ...
    NemuNetDhcp *pDhcp = new NemuNetDhcp();
    if (!pDhcp)
    {
        RTStrmPrintf(g_pStdErr, "NemuNetDHCP: new NemuNetDhcp failed!\n");
        return 1;
    }
...
static LPCWSTR g_WndClassName = L"NemuNetDHCPClass";
