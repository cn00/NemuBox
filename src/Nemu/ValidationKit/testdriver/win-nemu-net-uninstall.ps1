...
if ($ghostcon -eq $null) {
   ...
   $ghostcon.GetEnumerator() | ForEach-Object {
      ...
      } elseif (!($prop.PnPInstanceId.ToString() -match "SUN_NEMUNETFLTMP")) {
         Write-Host "WARNING! PnPInstanceId (" $prop.PnPInstanceId.ToString() ") does not match ROOT\SUN_NEMUNETFLTMP for" $_.PSChildName
      }
