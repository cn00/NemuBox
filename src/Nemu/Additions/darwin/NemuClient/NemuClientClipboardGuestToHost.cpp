/** $Id: NemuClientClipboardGuestToHost.cpp $ */
/** @file
 * NemuClient - Shared Clipboard Guest -> Host copying, Darwin.
 */
...
#define NEMUCL_ADD_FORMAT_IF_PRESENT(a_kDarwinFmt, a_fNemuFmt) \
    if (PasteboardCopyItemFlavorData(pPasteboard, iItemID, a_kDarwinFmt, &flavorData) == noErr) \
    { \
        fFormats |= (uint32_t)a_fNemuFmt; \
    ...
    for (iItem = 1; iItem <= cItems; iItem++)
    {
        ...
        if (rc == noErr)
        {
            NEMUCL_ADD_FORMAT_IF_PRESENT(kUTTypeUTF16PlainText, NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT);
            NEMUCL_ADD_FORMAT_IF_PRESENT(kUTTypeUTF8PlainText,  NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT);
            NEMUCL_ADD_FORMAT_IF_PRESENT(kUTTypeBMP,            NEMU_SHARED_CLIPBOARD_FMT_BITMAP     );
            NEMUCL_ADD_FORMAT_IF_PRESENT(kUTTypeHTML,           NEMU_SHARED_CLIPBOARD_FMT_HTML       );
            ...
            if (rc == noErr)
            {
                NemuClientVerbose(3, "SCAN..\n");
                flavorCount = CFArrayGetCount(flavorTypeArray);
                NemuClientVerbose(3, "SCAN (%d)..\n", (int)flavorCount);
...
#undef NEMUCL_ADD_FORMAT_IF_PRESENT
...
static int vbclClipboardHostPasteText(uint32_t u32ClientId, PRTUTF16 pwszData, uint32_t cbData)
{
    ...
    int rc = nemuClipboardUtf16GetWinSize(pwszData, cbData / sizeof(RTUTF16), &cwcActual);
    ...
    rc = nemuClipboardUtf16LinToWin(pwszData, cbData / sizeof(RTUTF16), pwszWinTmp, cwcActual);
    if (RT_SUCCESS(rc))
        rc = vbclClipboardHostPasteData(u32ClientId, NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT,
...
static int vbclClipboardHostPasteBitmap(uint32_t u32ClientId, void *pvData, uint32_t cbData)
{
    const void   *pvDib;
    size_t        cbDib;
    int rc = nemuClipboardBmpGetDib(pvData, cbData, &pvDib, &cbDib);
    AssertRCReturn(rc, rc);

    rc = vbclClipboardHostPasteData(u32ClientId, NEMU_SHARED_CLIPBOARD_FMT_BITMAP, pvDib, cbDib);
...
int vbclClipboardForwardToHost(uint32_t u32ClientId, PasteboardRef pPasteboard, uint32_t fFormats)
{
    ...
    NemuClientVerbose(3, "vbclClipboardForwardToHost: %d\n", fFormats);

    /* Walk across all item(s) formats */
    uint32_t  fFormatsLeft = fFormats;
    while (fFormatsLeft)
    {
        if (fFormatsLeft & NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT)
        {
            NemuClientVerbose(3, "requested NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT: %d\n", fFormats);
            ...
            else
            {
                ...
                rc = vbclClipboardHostPasteData(u32ClientId, NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT, NULL, 0);
            }

            fFormatsLeft &= ~(uint32_t)NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT;
        }

        else if (fFormatsLeft & NEMU_SHARED_CLIPBOARD_FMT_BITMAP)
        {
            NemuClientVerbose(3, "requested NEMU_SHARED_CLIPBOARD_FMT_BITMAP: %d\n", fFormats);
            ...
            {
                /* No data found or error occurred: send empty buffer */
                rc = vbclClipboardHostPasteData(u32ClientId, NEMU_SHARED_CLIPBOARD_FMT_BITMAP, NULL, 0);
            }

            fFormatsLeft &= ~(uint32_t)NEMU_SHARED_CLIPBOARD_FMT_BITMAP;
        }

        else if (fFormatsLeft & NEMU_SHARED_CLIPBOARD_FMT_HTML)
        {
            NemuClientVerbose(3, "requested NEMU_SHARED_CLIPBOARD_FMT_HTML: %d\n", fFormats);

            rc = vbclClipboardReadGuestData(pPasteboard, kUTTypeHTML, &pvData, &cbData, &cbAlloc);
            if (RT_SUCCESS(rc))
            {
                rc = vbclClipboardHostPasteData(u32ClientId, NEMU_SHARED_CLIPBOARD_FMT_HTML, pvData, cbData);
                vbclClipboardReleaseGuestData(&pvData, cbAlloc);
            }
            else
            {
                /* No data found or error occurred: send empty buffer */
                rc = vbclClipboardHostPasteData(u32ClientId, NEMU_SHARED_CLIPBOARD_FMT_HTML, NULL, 0);
            }

            fFormatsLeft &= ~(uint32_t)NEMU_SHARED_CLIPBOARD_FMT_HTML;
        }

        else
        {
            NemuClientVerbose(3, "requested data in unsupported format: %#x\n", fFormatsLeft);
            break;
        }
    }
