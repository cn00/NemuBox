    ...
    def _CheckPcdDefineAndType(self):
        ...
        for Pa in self.AutoGenObjectList:
            ...
            for Pcd in Pa.Platform.Pcds:
                ...
                # Nemu: Don't know exactly what the logic here should be, because Pcds may appear in several
                ...
                KeyFound = False # Nemu: Must validate the whole list to catch type mismatches!
                for Package in Pa.PackageList:
                    # Key of DEC's Pcds dictionary is PcdCName, TokenSpaceGuid, PcdType
                    if (Pcd[0], Pcd[1], PcdType) in Package.Pcds:
                        KeyFound = True # Nemu: was 'break' - see above.
                    for Type in PcdTypeList:
                        if Type != PcdType and (Pcd[0], Pcd[1], Type) in Package.Pcds:
                            EdkLogger.warn( # Nemu: Was error
                                'build',
                                FORMAT_INVALID,
                                "Type [%s] of PCD [%s.%s] in DSC file (%s) doesn't match the type [%s] defined in DEC file (%s)." \
                                % (Pa.Platform.Pcds[Pcd].Type, Pcd[1], Pcd[0], Pa, Type, Package), # Nemu: File names!
                                ExtraData=None
                            )
                            #return -
                if not KeyFound: # Nemu: was 'else:' - see above.
...
class PlatformAutoGen(AutoGen):
    ...
    def _GetPackageList(self):
        if self._PackageList == None:
            # Nemu: The order of the package list is (or was) relevant to one or more
            ...
            else: # Nemu - always update by appending new dependencies.
                self._PackageList = []
                for La in self.LibraryAutoGenList:
