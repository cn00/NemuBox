/* $Id: NemuGuestR3LibGuestProp.cpp $ */
/** @file
 * NemuGuestR3Lib - Ring-3 Support Library for VirtualBox guest additions, guest properties.
 */
...
#if defined(NEMU_VBGLR3_XFREE86) || defined(NEMU_VBGLR3_XORG)
 ...
 * properties.
 *
 * @remarks typedef in NemuGuestLib.h.
 */
...
VBGLR3DECL(int) VbglR3GuestPropConnect(HGCMCLIENTID *pidClient)
{
    ...
    RT_ZERO(Info.Loc.u);
    strcpy(Info.Loc.u.host.achName, "NemuGuestPropSvc");
    Info.u32ClientID = UINT32_MAX;  /* try make valgrind shut up. */

    int rc = vbglR3DoIOCtl(NEMUGUEST_IOCTL_HGCM_CONNECT, &Info, sizeof(Info));
    if (RT_SUCCESS(rc))
    {
        rc = Info.result;
        if (RT_SUCCESS(rc))
 ...
 *
 * @returns Nemu status code. Stops on first failure.
                                    ...
                                    uint32_t *pcbBufActual)
{
    ...
    int rc = vbglR3DoIOCtl(NEMUGUEST_IOCTL_HGCM_CALL_TIMED(sizeof(Msg)), &Msg, sizeof(Msg));
    if (RT_SUCCESS(rc))
