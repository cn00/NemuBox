...
struct NetworkManager::Data
{
    ...
    const NemuNetHlpUDPService *m_service;
};
...
void NetworkManager::setService(const NemuNetHlpUDPService *srv)
{
    m->m_service = srv;
}
...
int NetworkManager::doReply(const Client& client, const std::vector<RawOption>& extra)
{
    ...
    /*
      Options....
     */
    NemuNetDhcpWriteCursor Cursor(&m->BootPReplyMsg.BootPHeader, RTNET_DHCP_NORMAL_SIZE);
    ...
    {
        rc = NemuNetUDPUnicast(m_pSession,
                               m_hIf,
                               m_pIfBuf,
                               m_OurAddress,
                               &m_OurMac,
