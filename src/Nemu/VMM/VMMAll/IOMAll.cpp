 ...
 */
VMMDECL(NEMUSTRICTRC) IOMIOPortRead(PVM pVM, PVMCPU pVCpu, RTIOPORT Port, uint32_t *pu32Value, size_t cbValue)
{
 ...
 *        handle is buggy and doesn't handle all cases. */
    ...
    if (pRange)
    {
         ...
         * Call the device.
         */
        NEMUSTRICTRC rcStrict = PDMCritSectEnter(pDevIns->CTX_SUFF(pCritSectRo), VINF_IOM_R3_IOPORT_READ);
 ...
 */
VMM_INT_DECL(NEMUSTRICTRC) IOMIOPortReadString(PVM pVM, PVMCPU pVCpu, RTIOPORT uPort,
...
# endif
        ...
        Log3(("IOMIOPortReadStr: uPort=%RTiop pvDst=%p pcTransfer=%p:{%#x->%#x} cb=%d rc=%Rrc\n",
              uPort, pvDst, pcTransfers, cRequestedTransfers, *pcTransfers, cb, NEMUSTRICTRC_VAL(rcStrict)));
        return rcStrict;
    }
 ...
 * @param   cbValue     The size of the register to read in bytes. 1, 2 or 4 bytes.
 */
VMMDECL(NEMUSTRICTRC) IOMIOPortWrite(PVM pVM, PVMCPU pVCpu, RTIOPORT Port, uint32_t u32Value, size_t cbValue)
{
    /* Take the IOM lock before performing any device I/O. */
    int rc2 = IOM_LOCK_SHARED(pVM);
#ifndef IN_RING3
 ...
 *        entries to the ring-3 node. */
    ...
    if (pRange)
    {
         ...
         * Call the device.
         */
        NEMUSTRICTRC rcStrict = PDMCritSectEnter(pDevIns->CTX_SUFF(pCritSectRo), VINF_IOM_R3_IOPORT_WRITE);
        if (rcStrict == VINF_SUCCESS)
        { /* likely */ }
        else
        {
 ...
 * @param   cb          Size of the transfer unit (1, 2 or 4 bytes).
 */
VMM_INT_DECL(NEMUSTRICTRC) IOMIOPortWriteString(PVM pVM, PVMCPU pVCpu, RTIOPORT uPort, void const *pvSrc,
...
# endif
        ...
        Log3(("IOMIOPortWriteStr: uPort=%RTiop pvSrc=%p pcTransfer=%p:{%#x->%#x} cb=%d rcStrict=%Rrc\n",
              uPort, pvSrc, pcTransfers, cRequestedTransfers, *pcTransfers, cb, NEMUSTRICTRC_VAL(rcStrict)));
        return rcStrict;
    }
 ...
 */
VMMDECL(NEMUSTRICTRC) IOMInterpretCheckPortIOAccess(PVM pVM, PCPUMCTXCORE pCtxCore, RTIOPORT Port, unsigned cb)
{
       ...
       )
    {
        ...
        if (    !fCanHaveIOBitmap
            ||  cbTss <= sizeof(NEMUTSS)) /** @todo r=bird: Should this really include the interrupt redirection bitmap? */
        {
         ...
         */
        uint16_t offIOPB;
        NEMUSTRICTRC rcStrict = PGMPhysInterpretedRead(pVCpu, pCtxCore, &offIOPB, GCPtrTss + RT_OFFSETOF(NEMUTSS, offIoBitmap), sizeof(offIOPB));
        if (rcStrict != VINF_SUCCESS)
        {
            Log(("iomInterpretCheckPortIOAccess: Port=%RTiop cb=%d GCPtrTss=%RGv %Rrc\n",
                 Port, cb, GCPtrTss, NEMUSTRICTRC_VAL(rcStrict)));
            return rcStrict;
        }
        ...
        if (rcStrict != VINF_SUCCESS)
        {
            Log(("iomInterpretCheckPortIOAccess: Port=%RTiop cb=%d GCPtrTss=%RGv offTss=%#x -> %Rrc\n",
                 Port, cb, GCPtrTss, offTss, NEMUSTRICTRC_VAL(rcStrict)));
            return rcStrict;
        }
