/* $Id: NemuModAPIMonitor.cpp $ */
/** @file
 * NemuModAPIMonitor - API monitor module for detecting host isolation.
 */
...
#define NEMU_MOD_APIMON_NAME "apimon"
...
/* Copied from NemuManageInfo.cpp. */
...
static int apimonMachineControl(const Bstr &strUuid, PNEMUWATCHDOG_MACHINE pMachine,
...
static bool apimonHandleVM(const PNEMUWATCHDOG_MACHINE pMachine)
{
...
/* Callbacks. */
static DECLCALLBACK(int) NemuModAPIMonitorPreInit(void)
{
    return VINF_SUCCESS;
}

static DECLCALLBACK(int) NemuModAPIMonitorOption(int argc, char *argv[], int *piConsumed)
{
...
static DECLCALLBACK(int) NemuModAPIMonitorInit(void)
{
    HRESULT rc = S_OK;
    ...
    do
    {
        ...
        {
            CHECK_ERROR_BREAK(g_pVirtualBox, GetExtraData(Bstr("NemuInternal2/Watchdog/APIMonitor/Groups").raw(),
                                                          strValue.asOutParam()));
            if (!strValue.isEmpty())
            {
        ...
        if (!g_ulAPIMonIslnTimeoutMS)
            cfgGetValueULong(g_pVirtualBox, NULL /* Machine */,
                             "NemuInternal2/Watchdog/APIMonitor/IsolationTimeoutMS", NULL /* Per-machine */,
        ...
        if (g_enmAPIMonIslnResp == APIMON_RESPONSE_NONE) /* Not set by command line? */
        {
            ...
            int rc2 = cfgGetValueStr(g_pVirtualBox, NULL /* Machine */,
                                     "NemuInternal2/Watchdog/APIMonitor/IsolationResponse", NULL /* Per-machine */,
        ...
        if (!g_ulAPIMonResponseTimeoutMS)
            cfgGetValueULong(g_pVirtualBox, NULL /* Machine */,
                             "NemuInternal2/Watchdog/APIMonitor/ResponseTimeoutMS", NULL /* Per-machine */,
...
static DECLCALLBACK(int) NemuModAPIMonitorMain(void)
{
...
static DECLCALLBACK(int) NemuModAPIMonitorStop(void)
{
    return VINF_SUCCESS;
}

static DECLCALLBACK(void) NemuModAPIMonitorTerm(void)
{
}

static DECLCALLBACK(int) NemuModAPIMonitorOnMachineRegistered(const Bstr &strUuid)
{
    return VINF_SUCCESS;
}

static DECLCALLBACK(int) NemuModAPIMonitorOnMachineUnregistered(const Bstr &strUuid)
{
    return VINF_SUCCESS;
}

static DECLCALLBACK(int) NemuModAPIMonitorOnMachineStateChanged(const Bstr &strUuid,
                                                                MachineState_T enmState)
{
    return VINF_SUCCESS;
}

static DECLCALLBACK(int) NemuModAPIMonitorOnServiceStateChanged(bool fAvailable)
{
    if (!fAvailable)
    {
        serviceLog(("apimon: NemuSVC became unavailable, triggering action\n"));
 ...
 */
NEMUMODULE g_ModAPIMonitor =
{
    /* pszName. */
    NEMU_MOD_APIMON_NAME,
    /* pszDescription. */
    ...
    /* methods. */
    NemuModAPIMonitorPreInit,
    NemuModAPIMonitorOption,
    NemuModAPIMonitorInit,
    NemuModAPIMonitorMain,
    NemuModAPIMonitorStop,
    NemuModAPIMonitorTerm,
    /* callbacks. */
    NemuModAPIMonitorOnMachineRegistered,
    NemuModAPIMonitorOnMachineUnregistered,
    NemuModAPIMonitorOnMachineStateChanged,
    NemuModAPIMonitorOnServiceStateChanged
};
