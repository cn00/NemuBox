...
typedef IKDClient *(*PFNCreateNemuKDClientEx)(unsigned version);
...
static DECLCALLBACK(int) vkdConstruct(PPDMDEVINS pDevIns, int iInstance, PCFGMNODE pCfg)
{
    ...
    PFNCreateNemuKDClientEx pfnInit;
    rc = RTLdrGetSymbol(pThis->hLib, "CreateNemuKDClientEx", (void **)&pfnInit);
    if (RT_FAILURE(rc))
    {
