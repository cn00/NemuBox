...
VMMR0_INT_DECL(int) CPUMR0LoadGuestFPU(PVM pVM, PVMCPU pVCpu, PCPUMCTX pCtx)
{
...
#endif
    {
         ...
         *        NEMU_WITH_KERNEL_USING_XMM handling. */
...
static DECLCALLBACK(void) cpumR0MapLocalApicCpuChecker(RTCPUID idCpu, void *pvUser1, void *pvUser2)
{
    ...
    else
    {
        ...
        SUPR0Printf("Nemu/CPUM: Unsupported APIC version %#x (iCpu=%d)\n", uApicVersion, iCpu);
    }
}
...
#ifdef LOG_ENABLED
    /*
     * Log the result (pretty useless, requires enabling CPUM in NemuDrv
     * and !NEMU_WITH_R0_LOGGING).
     */
    if (LogIsEnabled())
    {
        uint32_t cEnabled = 0;
...
#endif /* NEMU_WITH_VMMR0_DISABLE_LAPIC_NMI */
