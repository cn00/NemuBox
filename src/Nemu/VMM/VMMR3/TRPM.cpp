 ...
 */
static NEMUIDTE_GENERIC     g_aIdt[256] =
{
/* special trap handler - still, this is an interrupt gate not a trap gate... */
#define IDTE_TRAP(enm)  { (unsigned)enm,                    0, 0, NEMU_IDTE_TYPE1, NEMU_IDTE_TYPE2_INT_32, 0, 1, 0 }
/* generic trap handler. */
#define IDTE_TRAP_GEN() IDTE_TRAP(TRPM_HANDLER_TRAP)
/* special interrupt handler. */
#define IDTE_INT(enm)   { (unsigned)enm,                    0, 0, NEMU_IDTE_TYPE1, NEMU_IDTE_TYPE2_INT_32,  0, 1, 0 }
/* generic interrupt handler. */
#define IDTE_INT_GEN()  IDTE_INT(TRPM_HANDLER_INT)
/* special task gate IDT entry (for critical exceptions like #DF). */
#define IDTE_TASK(enm)  { (unsigned)enm,                    0, 0, NEMU_IDTE_TYPE1, NEMU_IDTE_TYPE2_TASK,    0, 1, 0 }
...
VMMR3DECL(void) TRPMR3Relocate(PVM pVM, RTGCINTPTR offDelta)
{
     ...
     */
    PNEMUIDTE pIdte = &pVM->trpm.s.aIdt[0];
    PNEMUIDTE_GENERIC pIdteTemplate = &g_aIdt[0];
...
# endif
    ...
    for (uint32_t iTrap = 0; iTrap < RT_ELEMENTS(pVM->trpm.s.aGuestTrapHandler); iTrap++)
    {
        ...
        {
            PNEMUIDTE   pIdteCur = &pVM->trpm.s.aIdt[iTrap];
            RTGCPTR     pHandler = NEMUIDTE_OFFSET(*pIdteCur);
...
int trpmR3ClearPassThroughHandler(PVM pVM, unsigned iTrap)
{
    ...
    RTSEL               SelCS         = CPUMGetHyperCS(pVCpu);
    PNEMUIDTE           pIdte         = &pVM->trpm.s.aIdt[iTrap];
    PNEMUIDTE_GENERIC   pIdteTemplate = &g_aIdt[iTrap];
    if (pIdte->Gen.u1Present)
    {
        ...
        if (pIdte->Gen.u5Type2 != NEMU_IDTE_TYPE2_TASK)
        {
            pIdte->Gen.u16OffsetLow  = Offset & 0xffff;
            pIdte->Gen.u16OffsetHigh = Offset >> 16;
            pIdte->Gen.u16SegSel     = SelCS;
...
VMMR3DECL(uint32_t) TRPMR3QueryGateByHandler(PVM pVM, RTRCPTR GCPtr)
{
    ...
    for (uint32_t iTrap = 0; iTrap < RT_ELEMENTS(pVM->trpm.s.aGuestTrapHandler); iTrap++)
    {
        ...
        {
            PNEMUIDTE   pIdte = &pVM->trpm.s.aIdt[iTrap];
            RTGCPTR     pHandler = NEMUIDTE_OFFSET(*pIdte);
...
VMMR3DECL(int) TRPMR3SetGuestTrapHandler(PVM pVM, unsigned iTrap, RTRCPTR pHandler)
{
    ...
    if (iTrap * sizeof(NEMUIDTE) >= cbIDT)
        ...
        && !EMIsRawRing1Enabled(pVM))   /* can't deal with the ambiguity of ring 1 & 2 in the patch code. */
    {
...
#endif
        {
            if (     GuestIdte.Gen.u1Present
                &&  (   GuestIdte.Gen.u5Type2 == NEMU_IDTE_TYPE2_TRAP_32
                     || GuestIdte.Gen.u5Type2 == NEMU_IDTE_TYPE2_INT_32)
...
VMMR3DECL(bool) TRPMR3IsGateHandler(PVM pVM, RTRCPTR GCPtr)
{
    ...
    unsigned    cEntries = (cbIDT + 1) / sizeof(NEMUIDTE);
    if (!cEntries)
        return false;
    RTGCPTR   GCPtrIDTELast = GCPtrIDTE + (cEntries - 1) * sizeof(NEMUIDTE);
    ...
    while (GCPtrIDTE <= GCPtrIDTELast)
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            while (GCPtrIDTE <= GCPtrIDTELast)
            {
                if (pIDTE->Gen.u1Present)
                {
                    RTRCPTR GCPtrHandler = NEMUIDTE_OFFSET(*pIDTE);
                ...
                /* next entry */
                if ((GCPtrIDTE & PAGE_OFFSET_MASK) + sizeof(NEMUIDTE) >= PAGE_SIZE)
                {
                    AssertMsg(!(GCPtrIDTE & (sizeof(NEMUIDTE) - 1)),
                              ("IDT is crossing pages and it's not aligned! GCPtrIDTE=%#x cbIDT=%#x\n", GCPtrIDTE, cbIDT));
                    GCPtrIDTE += sizeof(NEMUIDTE);
                    break;
                }
                GCPtrIDTE += sizeof(NEMUIDTE);
                pIDTE++;
        ...
        else
        {
            ...
            if ((GCPtrIDTELast >> PAGE_SHIFT) == (GCPtrIDTE >> PAGE_SHIFT))
                return false;
            GCPtrIDTE = RT_ALIGN_T(GCPtrIDTE, PAGE_SIZE, RTGCPTR) + PAGE_SIZE + (GCPtrIDTE & (sizeof(NEMUIDTE) - 1));
        }
    }
    return false;
}
