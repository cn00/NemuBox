...
/** @file
 * Nemu Qt GUI - UIVMCloseDialog class implementation.
 */
...
void UIVMCloseDialog::accept()
{
    ...
    gEDataManager->setLastMachineCloseAction(newCloseAction, nemuGlobal().managedVMUuid());
...
void UIVMCloseDialog::configure()
{
    ...
    /* Assign pixmap: */
    setPixmap(nemuGlobal().vmGuestOSTypeIcon(m_machine.GetOSTypeId()));

    /* Check which close-actions are resticted: */
    bool fIsDetachAllowed = nemuGlobal().isSeparateProcess() && !(m_restictedCloseActions & MachineCloseAction_Detach);
    ...
    m_lastCloseAction = gEDataManager->lastMachineCloseAction(nemuGlobal().managedVMUuid());
