/* $Id: SUPDrv-solaris.c $ */
/** @file
 * NemuDrv - The VirtualBox Support Driver - Solaris specifics.
 */
...
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static int NemuDrvSolarisOpen(dev_t *pDev, int fFlag, int fType, cred_t *pCred);
static int NemuDrvSolarisClose(dev_t Dev, int fFlag, int fType, cred_t *pCred);
static int NemuDrvSolarisRead(dev_t Dev, struct uio *pUio, cred_t *pCred);
static int NemuDrvSolarisWrite(dev_t Dev, struct uio *pUio, cred_t *pCred);
static int NemuDrvSolarisIOCtl(dev_t Dev, int Cmd, intptr_t pArgs, int mode, cred_t *pCred, int *pVal);

static int NemuDrvSolarisAttach(dev_info_t *pDip, ddi_attach_cmd_t Cmd);
static int NemuDrvSolarisDetach(dev_info_t *pDip, ddi_detach_cmd_t Cmd);
static int NemuDrvSolarisQuiesceNotNeeded(dev_info_t *pDip);

static int NemuSupDrvErr2SolarisErr(int rc);
static int NemuDrvSolarisIOCtlSlow(PSUPDRVSESSION pSession, int Cmd, int Mode, intptr_t pArgs);
 ...
 */
static struct cb_ops g_NemuDrvSolarisCbOps =
{
    NemuDrvSolarisOpen,
    NemuDrvSolarisClose,
    nodev,                        /* b strategy */
 ...
 */
static struct dev_ops g_NemuDrvSolarisDevOps =
{
    ...
    nulldev,                      /* identify */
    nulldev,                      /* probe */
    NemuDrvSolarisAttach,
    NemuDrvSolarisDetach,
    nodev,                        /* reset */
    &g_NemuDrvSolarisCbOps,
    (struct bus_ops *)0,
    nodev,                        /* power */
    NemuDrvSolarisQuiesceNotNeeded
};

/**
 * modldrv: export driver specifics to the kernel
 */
static struct modldrv g_NemuDrvSolarisModule =
{
    &mod_driverops,               /* extern from kernel */
    DEVICE_DESC " " NEMU_VERSION_STRING "r" RT_XSTR(NEMU_SVN_REV),
    &g_NemuDrvSolarisDevOps
};

/**
 * modlinkage: export install/remove/info to the kernel
 */
static struct modlinkage g_NemuDrvSolarisModLinkage =
{
    MODREV_1,                     /* loadable module system revision */
    {
        &g_NemuDrvSolarisModule,
...
static void *g_pNemuDrvSolarisState;
...
int _init(void)
{
    ...
    LogFlowFunc(("nemudrv:_init\n"));
#endif

    /*
     * Prevent module autounloading.
     */
    modctl_t *pModCtl = mod_getctl(&g_NemuDrvSolarisModLinkage);
    if (pModCtl)
        pModCtl->mod_loadflags |= MOD_NOAUTOUNLOAD;
    else
        cmn_err(CE_NOTE, "nemudrv: failed to disable autounloading!\n");
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            rc = RTSpinlockCreate(&g_Spinlock, RTSPINLOCK_FLAGS_INTERRUPT_SAFE, "NemuDrvSol");
            if (RT_SUCCESS(rc))
            {
                rc = ddi_soft_state_init(&g_pNemuDrvSolarisState, sizeof(nemu_devstate_t), 8);
                if (!rc)
                {
                    rc = mod_install(&g_NemuDrvSolarisModLinkage);
                    if (!rc)
                        return rc; /* success */

                    ddi_soft_state_fini(&g_pNemuDrvSolarisState);
                    LogRel(("nemudrv: mod_install failed! rc=%d\n", rc));
                }
                else
                    LogRel(("nemudrv: failed to initialize soft state.\n"));
            ...
            {
                LogRel(("NemuDrvSolarisAttach: RTSpinlockCreate failed\n"));
        ...
        {
            LogRel(("NemuDrvSolarisAttach: supdrvInitDevExt failed\n"));
            rc = EINVAL;
        }
        RTR0TermForced();
    }
    ...
    {
        LogRel(("NemuDrvSolarisAttach: failed to init R0Drv\n"));
        rc = RTErrConvertToErrno(rc);
    }
...
int _fini(void)
{
    LogFlowFunc(("nemudrv:_fini\n"));

    /*
     * Undo the work we did at start (in the reverse order).
     */
    int rc = mod_remove(&g_NemuDrvSolarisModLinkage);
    if (rc != 0)
        return rc;
...
int _info(struct modinfo *pModInfo)
{
    ...
    LogFlowFunc(("nemudrv:_init\n"));
#endif
    int e = mod_info(&g_NemuDrvSolarisModLinkage, pModInfo);
    return e;
}
 ...
 */
static int NemuDrvSolarisAttach(dev_info_t *pDip, ddi_attach_cmd_t enmCmd)
{
    LogFlowFunc(("NemuDrvSolarisAttach\n"));
    ...
    switch (enmCmd)
    {
        case DDI_ATTACH:
        {
            ...
            nemu_devstate_t *pState;

            if (ddi_soft_state_zalloc(g_pNemuDrvSolarisState, instance) != DDI_SUCCESS)
            {
                LogRel(("NemuDrvSolarisAttach: state alloc failed\n"));
                return DDI_FAILURE;
            }

            pState = ddi_get_soft_state(g_pNemuDrvSolarisState, instance);
#endif
            ...
            if (rc != DDI_PROP_SUCCESS)
                LogRel(("nemudrv: Suspend/Resume notification registration failed.\n"));
        ...
        case DDI_RESUME:
        {
            ...
            LogFlow(("nemudrv: Awakened from suspend.\n"));
            return DDI_SUCCESS;
        }
 ...
 * @return  corresponding solaris error code.
 */
static int NemuDrvSolarisDetach(dev_info_t *pDip, ddi_detach_cmd_t enmCmd)
{
    LogFlowFunc(("NemuDrvSolarisDetach\n"));
    switch (enmCmd)
    {
        case DDI_DETACH:
        {
...
#else
            int instance = ddi_get_instance(pDip);
            nemu_devstate_t *pState = ddi_get_soft_state(g_pNemuDrvSolarisState, instance);
            ddi_remove_minor_node(pDip, NULL);
            ddi_soft_state_free(g_pNemuDrvSolarisState, instance);
#endif
        ...
        case DDI_SUSPEND:
        {
            ...
            LogFlow(("nemudrv: Falling to suspend mode.\n"));
            return DDI_SUCCESS;
...
static int NemuDrvSolarisOpen(dev_t *pDev, int fFlag, int fType, cred_t *pCred)
{
    ...
    LogFlowFunc(("NemuDrvSolarisOpen: pDev=%p:%#x\n", pDev, *pDev));
     ...
     */
    nemu_devstate_t *pState = NULL;
    unsigned iOpenInstance;
    for (iOpenInstance = 0; iOpenInstance < 4096; iOpenInstance++)
    {
        if (    !ddi_get_soft_state(g_pNemuDrvSolarisState, iOpenInstance) /* faster */
            &&  ddi_soft_state_zalloc(g_pNemuDrvSolarisState, iOpenInstance) == DDI_SUCCESS)
        {
            pState = ddi_get_soft_state(g_pNemuDrvSolarisState, iOpenInstance);
            break;
        }
    }
    if (!pState)
    {
        LogRel(("NemuDrvSolarisOpen: too many open instances.\n"));
        return ENXIO;
    }
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        *pDev = makedevice(getmajor(*pDev), iOpenInstance);
        LogFlow(("NemuDrvSolarisOpen: Dev=%#x pSession=%p pid=%d r0proc=%p thread=%p\n",
                 *pDev, pSession, RTProcSelf(), RTR0ProcHandleSelf(), RTThreadNativeSelf() ));
        return 0;
    }

    /* failed - clean up */
    ddi_soft_state_free(g_pNemuDrvSolarisState, iOpenInstance);

#else
    /*
     * Create a new session.
     * Sessions in Solaris driver are mostly useless. It's however needed
     * in NemuDrvSolarisIOCtlSlow() while calling supdrvIOCtl()
     */
    rc = supdrvCreateSession(&g_DevExt, true /* fUser */, fUnrestricted, &pSession);
    if (RT_SUCCESS(rc))
    {
...
# error "Only one entry per process!"
        ...
        LogFlow(("NemuDrvSolarisOpen success\n"));
    }
    ...
    {
        LogRel(("NemuDrvSolarisOpen: All instances exhausted\n"));
        return ENXIO;
    }
    ...
    *pDev = makedevice(getmajor(*pDev), instance);
    ...
    return NemuSupDrvErr2SolarisErr(rc);
}


static int NemuDrvSolarisClose(dev_t Dev, int flag, int otyp, cred_t *cred)
{
    LogFlowFunc(("NemuDrvSolarisClose: Dev=%#x\n", Dev));

#ifndef USE_SESSION_HASH
    /*
     * Get the session and free the soft state item.
     */
    nemu_devstate_t *pState = ddi_get_soft_state(g_pNemuDrvSolarisState, getminor(Dev));
    if (!pState)
    {
        LogRel(("NemuDrvSolarisClose: no state data for %#x (%d)\n", Dev, getminor(Dev)));
        return EFAULT;
    }

    PSUPDRVSESSION pSession = pState->pSession;
    pState->pSession = NULL;
    ddi_soft_state_free(g_pNemuDrvSolarisState, getminor(Dev));

    if (!pSession)
    {
        LogRel(("NemuDrvSolarisClose: no session in state data for %#x (%d)\n", Dev, getminor(Dev)));
        return EFAULT;
    }
    LogFlow(("NemuDrvSolarisClose: Dev=%#x pSession=%p pid=%d r0proc=%p thread=%p\n",
    ...
    if (!pSession)
    {
        LogRel(("NemuDrvSolarisClose: WHAT?!? pSession == NULL! This must be a mistake... pid=%d (close)\n", (int)Process));
        return EFAULT;
    }
#endif
...
static int NemuDrvSolarisRead(dev_t Dev, struct uio *pUio, cred_t *pCred)
{
    LogFlowFunc(("NemuDrvSolarisRead"));
    return 0;
}
...
static int NemuDrvSolarisWrite(dev_t Dev, struct uio *pUio, cred_t *pCred)
{
    LogFlowFunc(("NemuDrvSolarisWrite"));
    return 0;
}
...
static int NemuDrvSolarisIOCtl(dev_t Dev, int Cmd, intptr_t pArgs, int Mode, cred_t *pCred, int *pVal)
{
    ...
    if (!pState)
    {
        LogRel(("NemuDrvSolarisIOCtl: no state data for %#x (%d)\n", Dev, getminor(Dev)));
        return EINVAL;
    }
    ...
    if (!pSession)
    {
        LogRel(("NemuDrvSolarisIOCtl: no session in state data for %#x (%d)\n", Dev, getminor(Dev)));
        return DDI_SUCCESS;
    }
#else
    const RTPROCESS     Process = RTProcSelf();
    ...
    if (!pSession)
    {
        LogRel(("NemuSupDrvIOCtl: WHAT?!? pSession == NULL! This must be a mistake... pid=%d iCmd=%#x Dev=%#x\n",
    ...
    return NemuDrvSolarisIOCtlSlow(pSession, Cmd, Mode, pArgs);
}
...
/**
 * Worker for NemuSupDrvIOCtl that takes the slow IOCtl functions.
 *
 ...
 */
static int NemuDrvSolarisIOCtlSlow(PSUPDRVSESSION pSession, int iCmd, int Mode, intptr_t iArg)
{
    int         rc;
    uint32_t    cbBuf = 0;
    union
    ...
    if (RT_UNLIKELY(    StackBuf.Hdr.cbIn < sizeof(StackBuf.Hdr)
                    ...
                    ||  cbBuf > _1M*16))
    {
        LogRel(("NemuDrvSolarisIOCtlSlow: max(%#x,%#x); iCmd=%#x\n", StackBuf.Hdr.cbIn, StackBuf.Hdr.cbOut, iCmd));
        return EINVAL;
    }
    ...
    else
    {
        ...
        if (RT_UNLIKELY(!pHdr))
        {
            LogRel(("NemuDrvSolarisIOCtlSlow: failed to allocate buffer of %d bytes for iCmd=%#x.\n", cbBuf, iCmd));
            return ENOMEM;
        }
    }
    ...
    if (RT_UNLIKELY(rc))
    {
        LogRel(("NemuDrvSolarisIOCtlSlow: copy_from_user(,%#lx, %#x) failed; iCmd=%#x. rc=%d\n", iArg, cbBuf, iCmd, rc));
    ...
    if (RT_LIKELY(!rc))
    {
        ...
        {
            LogRel(("NemuDrvSolarisIOCtlSlow: too much output! %#x > %#x; iCmd=%#x!\n", cbOut, cbBuf, iCmd));
            cbOut = cbBuf;
        }
        ...
        {
            /* this is really bad */
            LogRel(("NemuDrvSolarisIOCtlSlow: ddi_copyout(,%p,%d) failed. rc=%d\n", (void *)iArg, cbBuf, rc));
            rc = EFAULT;
        }
    }
    else
 ...
 */
int NEMUCALL SUPDrvSolarisIDC(uint32_t uReq, PSUPDRVIDCREQHDR pReq)
{
    PSUPDRVSESSION  pSession;
...
#if  defined(NEMU_WITH_NATIVE_SOLARIS_LOADING) \
 && !defined(NEMU_WITHOUT_NATIVE_R0_LOADER)
...
#else /* !NEMU_WITH_NATIVE_SOLARIS_LOADING */
...
#endif /* !NEMU_WITH_NATIVE_SOLARIS_LOADING */
