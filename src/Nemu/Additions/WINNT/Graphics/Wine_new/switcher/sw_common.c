...
BOOL isNemu3DEnabled(void)
{
    ...
    hDLL = loadSystemDll("NemuOGL-x86.dll");
#else
    hDLL = loadSystemDll("NemuOGL.dll");
#endif
...
void InitD3DExports(const char *nemuName, const char *msName)
{
    const char *dllName;
    HANDLE hDLL;

    if (isNemu3DEnabled() && checkOptions())
        dllName = nemuName;
    else
        dllName = msName;
