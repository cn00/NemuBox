...
PCR_BLITTER crServerNemuBlitterGet()
{
    ...
    if (!CrBltMuralGetCurrentInfo(&cr_server.Blitter)->Base.id)
    {
        ...
        CRASSERT(dummy);
        crServerNemuBlitterWinInit(&DummyInfo, dummy);
...
int crServerNemuBlitterTexInit(CRContext *ctx, CRMuralInfo *mural, PNEMUVR_TEXTURE pTex, GLboolean fDraw)
{
    CRTextureObj *tobj;
...
int crServerNemuBlitterBlitCurrentCtx(GLint srcX0, GLint srcY0, GLint srcX1, GLint srcY1,
        ...
        GLbitfield mask, GLenum filter)
{
    ...
    CRMuralInfo *mural;
    CRContext *ctx = crStateGetCurrent();
    PNEMUVR_TEXTURE pDrawTex, pReadTex;
    NEMUVR_TEXTURE DrawTex, ReadTex;
    int rc;
    ...
    rc = crServerNemuBlitterTexInit(ctx, mural, &DrawTex, GL_TRUE);
    if (RT_SUCCESS(rc))
    {
        pDrawTex = &DrawTex;
    }
    ...
    {
        crWarning("crServerNemuBlitterTexInit failed for draw");
        return rc;
    }

    rc = crServerNemuBlitterTexInit(ctx, mural, &ReadTex, GL_FALSE);
    if (RT_SUCCESS(rc))
    {
        pReadTex = &ReadTex;
    }
    ...
    {
//        crWarning("crServerNemuBlitterTexInit failed for read");
        return rc;
    }

    pBlitter = crServerNemuBlitterGet();
    if (!pBlitter)
    {
        crWarning("crServerNemuBlitterGet failed");
        return VERR_GENERAL_FAILURE;
    }

    crServerNemuBlitterWinInit(&BltInfo, mural);

    crServerNemuBlitterCtxInit(&Ctx, cr_server.curClient->currentCtxInfo);
...
//    }
    ...
    if (fTryBlitter)
    {
        int rc = crServerNemuBlitterBlitCurrentCtx(srcX0, srcY0, srcX1, srcY1, dstX0, dstY0, dstX1, dstY1, mask, filter);
        if (RT_SUCCESS(rc))
            goto my_exit;
    }
...
# ifndef NEMU_WITH_CRDUMPER
#  error "NEMU_WITH_CRDUMPER undefined!"
# endif
...
int crServerDumpCheckInit()
{
    ...
    if (!CrBltIsInitialized(&cr_server.RecorderBlitter))
    {
        ...
        crServerNemuBlitterWinInit(&BltWin, pBlitterMural);
        crServerNemuBlitterCtxInit(&BltCtx, &cr_server.MainContextInfo);
...
//    crDmpHtmlInit(&cr_server.HtmlDumper, "S:\\projects\\virtualbox\\3d\\dumps\\1", "index.html");
        crDmpHtmlInitF(&cr_server.HtmlDumper, "/Users/oracle-mac/nemu/dump/1", "index%d.html", cCounter);
...
void crServerDumpTexture(const NEMUVR_TEXTURE *pTex)
{
    ...
    crServerNemuBlitterWinInit(&BltWin, cr_server.currentMural);
    crServerNemuBlitterCtxInit(&BltCtx, pCtxInfo);
