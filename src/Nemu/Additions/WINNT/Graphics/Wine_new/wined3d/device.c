...
static void device_stream_info_from_declaration(struct wined3d_device *device, struct wined3d_stream_info *stream_info)
{
    ...
    for (i = 0, map = stream_info->use_map; map; map >>= 1, ++i)
    {
        ...
        if (
#ifdef NEMU_WITH_WINE_FIX_STRINFOBUF
...
HRESULT CDECL wined3d_device_uninit_3d(struct wined3d_device *device)
{
    ...
    NEMUWINEPROFILE_DRAWPRIM_TERM(&device->DrawPrimProfile);
#endif
...
#ifdef NEMU_WINE_WITH_PROFILE
    NEMUWINEPROFILE_DRAWPRIM_INIT(&device->DrawPrimProfile);
#endif
...
HRESULT CDECL wined3d_device_flush_to_host(struct wined3d_device *device)
{
    ...
    {
        context = device->contexts[i];
        pNemuFlushToHost(context->glCtx);
    }
    return WINED3D_OK;
}
...
{
    int32_t id = pNemuGetContextId(device->contexts[0]->glCtx);
    if (!id)
    {
        *pid = 0;
        ERR("pNemuGetContextId to get id for context 0x%x", device->contexts[0]->glCtx);
        return E_FAIL;
    }
