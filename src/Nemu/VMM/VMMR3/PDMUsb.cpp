...
static int pdmR3UsbLoad(PVM pVM, PCPDMUSBREGCBINT pRegCB, const char *pszFilename, const char *pszName)
{
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        FNPDMNEMUUSBREGISTER *pfnNemuUsbRegister;
        rc = PDMR3LdrGetSymbolR3(pVM, pszName, "NemuUsbRegister", (void **)&pfnNemuUsbRegister);
        if (RT_SUCCESS(rc))
        {
            Log(("PDM: Calling NemuUsbRegister (%p) of %s (%s)\n", pfnNemuUsbRegister, pszName, pszFilename));
            rc = pfnNemuUsbRegister(&pRegCB->Core, NEMU_VERSION);
            if (RT_SUCCESS(rc))
        ...
        {
            AssertMsgFailed(("Failed to locate 'NemuUsbRegister' in %s (%s) rc=%Rrc\n", pszName, pszFilename, rc));
            if (rc == VERR_SYMBOL_NOT_FOUND)
                rc = VERR_PDM_NO_REGISTRATION_EXPORT;
        }
    }
    else
        AssertMsgFailed(("Failed to load NemuDD!\n"));
    return rc;
}
...
int pdmR3UsbLoadModules(PVM pVM)
{
    ...
    if (fLoadBuiltin)
    {
        ...
        rc = pdmR3UsbLoad(pVM, &RegCB, pszFilename, "NemuDD");
