...
/** @file
 * Nemu Context state saving/loading used by VM snapshot
 */
...
int32_t crStateSaveContext(CRContext *pContext, PSSMHANDLE pSSM)
{
    ...
    CRASSERT(NemuTlsRefIsFunctional(pContext));
...
AssertCompile(NEMUTLSREFDATA_SIZE() <= CR_MAX_BITARRAY);
AssertCompile(NEMUTLSREFDATA_STATE_INITIALIZED != 0);
AssertCompile(RT_OFFSETOF(CRContext, shared) >= NEMUTLSREFDATA_OFFSET(CRContext) + NEMUTLSREFDATA_SIZE() + RT_SIZEOFMEMB(CRContext, bitid) + RT_SIZEOFMEMB(CRContext, neg_bitid));
...
int32_t crStateLoadContext(CRContext *pContext, CRHashTable * pCtxTable, PFNCRSTATE_CONTEXT_GET pfnCtxGet, PSSMHANDLE pSSM, uint32_t u32Version)
{
    ...
    if (u32Version <= SHCROGL_SSM_VERSION_WITH_INVALID_ERROR_STATE)
    {
        ...
        /* do not increment the saved state version due to NEMUTLSREFDATA addition to CRContext */
        rc = SSMR3GetMem(pSSM, pTmpContext, NEMUTLSREFDATA_OFFSET(CRContext));
        AssertRCReturn(rc, rc);

        /* Nemu 4.1.8 had a bug that NEMUTLSREFDATA was also stored in the snapshot,
         * thus the saved state data format was changed w/o changing the saved state version.
         * here we determine whether the saved state contains NEMUTLSREFDATA, and if so, treat it accordingly */
        rc = SSMR3GetMem(pSSM, &bitid, sizeof (bitid));
        AssertRCReturn(rc, rc);

        /* the bitid array has one bit set only. this is why if bitid.tlsRef has both cTlsRefs
         * and enmTlsRefState non-zero - this is definitely NOT a bit id and is a NEMUTLSREFDATA */
        if (bitid.tlsRef.enmTlsRefState == NEMUTLSREFDATA_STATE_INITIALIZED
                && bitid.tlsRef.cTlsRefs)
        {
            /* NEMUTLSREFDATA is stored, skip it */
            crMemcpy(&pTmpContext->bitid, ((uint8_t*)&bitid) + NEMUTLSREFDATA_SIZE(), sizeof (bitid) - NEMUTLSREFDATA_SIZE());
            rc = SSMR3GetMem(pSSM, ((uint8_t*)&pTmpContext->bitid) + sizeof (pTmpContext->bitid) - NEMUTLSREFDATA_SIZE(), sizeof (pTmpContext->neg_bitid) + NEMUTLSREFDATA_SIZE());
            AssertRCReturn(rc, rc);

            ui = NEMUTLSREFDATA_OFFSET(CRContext) + NEMUTLSREFDATA_SIZE() + sizeof (pTmpContext->bitid) + sizeof (pTmpContext->neg_bitid);
            ui = RT_OFFSETOF(CRContext, shared) - ui;
        }
        else
        {
            /* NEMUTLSREFDATA is NOT stored */
            crMemcpy(&pTmpContext->bitid, &bitid, sizeof (bitid));
            rc = SSMR3GetMem(pSSM, &pTmpContext->neg_bitid, sizeof (pTmpContext->neg_bitid));
            AssertRCReturn(rc, rc);

            /* the pre-NEMUTLSREFDATA CRContext structure might have additional allignment bits before the CRContext::shared */
            ui = NEMUTLSREFDATA_OFFSET(CRContext) + sizeof (pTmpContext->bitid) + sizeof (pTmpContext->neg_bitid);
     ...
     * for simplicity store the NEMUTLSREFDATA from the pContext to pTmpContext */
    NEMUTLSREFDATA_COPY(pTmpContext, pContext);
...
# endif
    ...
    CRASSERT(NEMUTLSREFDATA_EQUAL(pContext, pTmpContext));
