...
HRESULT MachineCloneVMPrivate::addSaveState(const ComObjPtr<Machine> &machine, bool fAttachCurrent, ULONG &uCount, ULONG &uTotalWeight)
{
    ...
    if (!bstrSrcSaveStatePath.isEmpty())
    {
        ...
        if (RT_FAILURE(vrc))
            return p->setError(NEMU_E_IPRT_ERROR, p->tr("Could not query file size of '%s' (%Rrc)"),
    ...
    try
    {
        ...
        if (d->mode == CloneMode_MachineAndChildStates)
        {
            if (d->pSrcMachine->i_isSnapshotMachine())
            {
                ...
                if (FAILED(rc)) throw rc;
                if (pCurr.isNull())
                    throw p->setError(NEMU_E_OBJECT_NOT_FOUND);
        ...
        if (RT_FAILURE(vrc))
            p->setError(NEMU_E_IPRT_ERROR, "Could not create machine clone thread (%Rrc)", vrc);
    }
    catch (HRESULT rc2)
    {
        rc = rc2;
...
HRESULT MachineCloneVM::run()
{
    ...
    try
    {
            ...
            && !RTDirExists(strTrgSnapshotFolder.c_str()))
        {
            ...
            if (RT_FAILURE(vrc))
                throw p->setError(NEMU_E_IPRT_ERROR,
    ...
    if (FAILED(rc))
    {
        ...
        for (size_t i = 0; i < newFiles.size(); ++i)
        {
            ...
            if (RT_FAILURE(vrc))
                mrc = p->setError(NEMU_E_IPRT_ERROR, p->tr("Could not delete file '%s' (%Rrc)"), newFiles.at(i).c_str(), vrc);
        }
