...
void crServerAddToRunQueue( CRClient *client )
{
    ...
    CRNEMUHGSMI_CMDDATA_CLEANUP(&client->conn->CmdData);
#endif
...
int crServerPendLoadState(PSSMHANDLE pSSM, uint32_t u32Version)
{
    ...
    do {
        rc = crNemuServerClientGet(u32, &pClient);
        AssertRCReturn(rc, rc);
...
crServerDispatchMessage(CRConnection *conn, CRMessage *msg, int cbMsg)
{
    ...
    const char *data_ptr;
#ifdef NEMU_WITH_CRHGSMI
    PCRNEMUHGSMI_CMDDATA pCmdData = NULL;
#endif
...
//            CRASSERT(!RTListIsEmpty(&conn->PendingMsgList));
    ...
    {
        int rc = VINF_SUCCESS;
        CRNEMUHGSMI_CMDDATA_ASSERT_CONSISTENT(pCmdData);
        if (CRNEMUHGSMI_CMDDATA_IS_SETWB(pCmdData))
        {
            uint32_t cbWriteback = pCmdData->cbWriteback;
            rc = crNemuServerInternalClientRead(conn->pClient, (uint8_t*)pCmdData->pWriteback, &cbWriteback);
            Assert(rc == VINF_SUCCESS || rc == VERR_BUFFER_OVERFLOW);
            *pCmdData->pcbWriteback = cbWriteback;
        }
        NEMUCRHGSMI_CMD_CHECK_COMPLETE(pCmdData, rc);
    }
#endif
}
