...
#ifndef ___Nemu_vmm_pgm_h
#define ___Nemu_vmm_pgm_h

#include <Nemu/types.h>
#include <Nemu/sup.h>
#include <Nemu/vmm/vmapi.h>
#include <Nemu/vmm/gmm.h>               /* for PGMMREGISTERSHAREDMODULEREQ */
#include <iprt/x86.h>
#include <Nemu/VMMDev.h>                /* for VMMDEVSHAREDREGIONDESC */
 ...
 *
 * @returns Strict Nemu status code (appropriate for ring-0 and raw-mode).
 * @param   pVM         The cross context VM structure.
 ...
 * @thread  EMT(pVCpu)
 */
typedef DECLCALLBACK(NEMUSTRICTRC) FNPGMRZPHYSPFHANDLER(PVM pVM, PVMCPU pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pRegFrame,
 ...
 *
 * @returns Strict Nemu status code in ring-0 and raw-mode context, in ring-3
 ...
 * @thread  EMT(pVCpu)
 */
typedef DECLCALLBACK(NEMUSTRICTRC) FNPGMPHYSHANDLER(PVM pVM, PVMCPU pVCpu, RTGCPHYS GCPhys, void *pvPhys, void *pvBuf, size_t cbBuf,
 ...
 *
 * @returns Strict Nemu status code (appropriate for raw-mode).
 * @param   pVM             The cross context VM structure.
 * @param   pVCpu           The cross context virtual CPU structure of the calling EMT.
 ...
 * @thread  EMT(pVCpu)
 */
typedef DECLCALLBACK(NEMUSTRICTRC) FNPGMRCVIRTPFHANDLER(PVM pVM, PVMCPU pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pRegFrame,
 ...
 * @thread  EMT(pVCpu)
 */
typedef DECLCALLBACK(NEMUSTRICTRC) FNPGMVIRTHANDLER(PVM pVM, PVMCPU pVCpu, RTGCPTR GCPtr, void *pvPtr, void *pvBuf, size_t cbBuf,
                                                    PGMACCESSTYPE enmAccessType, PGMACCESSORIGIN enmOrigin, void *pvUser);
/** Pointer to PGM access callback. */
...
VMMDECL(NEMUSTRICTRC)   PGMInterpretInstruction(PVM pVM, PVMCPU pVCpu, PCPUMCTXCORE pRegFrame, RTGCPTR pvFault);
...
#if defined(IN_RC) || defined(NEMU_WITH_2X_4GB_ADDR_SPACE)
...
VMMR0DECL(NEMUSTRICTRC) PGMR0Trap0eHandlerNPMisconfig(PVM pVM, PVMCPU pVCpu, PGMMODE enmShwPagingMode, PCPUMCTXCORE pRegFrame, RTGCPHYS GCPhysFault, uint32_t uErr);
...
#if defined(NEMU_WITH_RAW_MODE) || HC_ARCH_BITS == 32 /* (latter for 64-bit guests on 32-bit hosts) */
...
/** @name Page sharing
 * @{ */
VMMR3DECL(int)     PGMR3SharedModuleRegister(PVM pVM, NEMUOSFAMILY enmGuestOS, char *pszModuleName, char *pszVersion,
