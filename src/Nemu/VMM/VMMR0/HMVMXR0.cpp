...
#endif
DECLINLINE(NEMUSTRICTRC)  hmR0VmxHandleExitStep(PVMCPU pVCpu, PCPUMCTX pMixedCtx, PVMXTRANSIENT pVmxTransient,
...
#ifdef NEMU_WITH_CRASHDUMP_MAGIC
...
#if HC_ARCH_BITS == 32 && defined(NEMU_ENABLE_64_BITS_GUESTS)
...
#ifdef NEMU_ENABLE_64_BITS_GUESTS
                    break;
#endif
                default:
                    AssertFailed();
...
static int hmR0VmxLoadSharedDebugState(PVMCPU pVCpu, PCPUMCTX pMixedCtx)
{
        ...
        || (CPUMGetHyperDR7(pVCpu) & X86_DR7_ENABLED_MASK))
    {
         ...
         */
#if HC_ARCH_BITS == 32 && defined(NEMU_WITH_64_BITS_GUESTS)
        if (   CPUMIsGuestInLongModeEx(pMixedCtx)
...
static int hmR0VmxSetupVMRunHandler(PVMCPU pVCpu, PCPUMCTX pMixedCtx)
{
    if (CPUMIsGuestInLongModeEx(pMixedCtx))
    {
#ifndef NEMU_ENABLE_64_BITS_GUESTS
        return VERR_PGM_UNSUPPORTED_SHADOW_PAGING_MODE;
#endif
...
#endif /* HC_ARCH_BITS == 32 && defined(NEMU_ENABLE_64_BITS_GUESTS) */
 ...
 * IDT.
 *
 * @returns Nemu status code (informational error codes included).
                                      ...
                                      PCPUMSELREG pSelReg)
{
     ...
     *    - For SS the DPL has meaning (it -is- the CPL for Intel and Nemu).
     ...
     *
     * What should be important for the rest of the Nemu code, is that the P bit is
     * cleared.  Some of the other Nemu code recognizes the unusable bit, but
 ...
 *
 * @returns Nemu status code (OR-able).
 * @param   pVCpu       The cross context virtual CPU structure of the calling EMT.
 ...
 *
 * @returns Nemu status code (information status code included).
 ...
 * receive them.
 *
 * @returns Nemu status code (informational status codes included).
 * @param   pVCpu           The cross context virtual CPU structure.
 * @param   pMixedCtx       Pointer to the guest-CPU context. The data may be
 *                          out-of-sync. Make sure to update the required fields
 *                          before using them.
 ...
 *
 * @returns Nemu status code (informational status code included).
 * @param   pVCpu           The cross context virtual CPU structure.
 * @param   pMixedCtx       Pointer to the guest-CPU context. The data may be
 *                          out-of-sync. Make sure to update the required fields
 *                          before using them.
...
#ifdef NEMU_WITH_2X_4GB_ADDR_SPACE_IN_R0
...
static int hmR0VmxRunGuestCodeNormal(PVM pVM, PVMCPU pVCpu, PCPUMCTX pCtx)
{
    ...
    for (;; cLoops++)
    {
        ...
        NEMUVMM_R0_HMVMX_VMEXIT_NOCTX(pVCpu, pCtx, VmxTransient.uExitReason);
        if (RT_UNLIKELY(NEMUVMM_R0_HMVMX_VMEXIT_ENABLED()))
        {
            hmR0VmxReadExitQualificationVmcs(pVCpu, &VmxTransient);
            hmR0VmxSaveGuestState(pVCpu, pCtx);
            NEMUVMM_R0_HMVMX_VMEXIT(pVCpu, pCtx, VmxTransient.uExitReason, VmxTransient.uExitQualification);
        }
...
static int hmR0VmxRunGuestCodeStep(PVM pVM, PVMCPU pVCpu, PCPUMCTX pCtx)
{
    VMXTRANSIENT VmxTransient;
    VmxTransient.fUpdateTscOffsettingAndPreemptTimer = true;
    NEMUSTRICTRC rcStrict  = VERR_INTERNAL_ERROR_5;
    uint32_t     cLoops    = 0;
    ...
    for (;; cLoops++)
    {
        ...
        /* Restore any residual host-state and save any bits shared between host
           and guest into the guest-CPU state.  Re-enables interrupts! */
        hmR0VmxPostRunGuest(pVM, pVCpu, pCtx, &VmxTransient, NEMUSTRICTRC_TODO(rcStrict));

        /* Check for errors with running the VM (VMLAUNCH/VMRESUME). */
        if (RT_UNLIKELY(rcStrict != VINF_SUCCESS))
        {
            STAM_PROFILE_ADV_STOP(&pVCpu->hm.s.StatExit1, x);
            hmR0VmxReportWorldSwitchError(pVM, pVCpu, NEMUSTRICTRC_TODO(rcStrict), pCtx, &VmxTransient);
            return NEMUSTRICTRC_TODO(rcStrict);
        }
...
HMVMX_EXIT_DECL hmR0VmxExitInvlpg(PVMCPU pVCpu, PCPUMCTX pMixedCtx, PVMXTRANSIENT pVmxTransient)
{
    ...
    NEMUSTRICTRC rc2 = EMInterpretInvlpg(pVM, pVCpu, CPUMCTX2CORE(pMixedCtx), pVmxTransient->uExitQualification);
    rc = NEMUSTRICTRC_VAL(rc2);
    if (RT_LIKELY(rc == VINF_SUCCESS))
        rc = hmR0VmxAdvanceGuestRip(pVCpu, pMixedCtx, pVmxTransient);
    else
    {
...
HMVMX_EXIT_DECL hmR0VmxExitMwait(PVMCPU pVCpu, PCPUMCTX pMixedCtx, PVMXTRANSIENT pVmxTransient)
{
    ...
    PVM pVM = pVCpu->CTX_SUFF(pVM);
    NEMUSTRICTRC rc2 = EMInterpretMWait(pVM, pVCpu, CPUMCTX2CORE(pMixedCtx));
    rc = NEMUSTRICTRC_VAL(rc2);
    if (RT_LIKELY(   rc == VINF_SUCCESS
                  || rc == VINF_EM_HALT))
    {
...
HMVMX_EXIT_DECL hmR0VmxExitXsetbv(PVMCPU pVCpu, PCPUMCTX pMixedCtx, PVMXTRANSIENT pVmxTransient)
{
    ...
    NEMUSTRICTRC rcStrict = IEMExecDecodedXsetbv(pVCpu, pVmxTransient->cbInstr);
...
HMVMX_EXIT_DECL hmR0VmxExitIoInstr(PVMCPU pVCpu, PCPUMCTX pMixedCtx, PVMXTRANSIENT pVmxTransient)
{
    ...
    if (fIOString)
    {
#ifdef NEMU_WITH_2ND_IEM_STEP /* This used to gurus with debian 32-bit guest without NP (on ATA reads).
    ...
    if (IOM_SUCCESS(rcStrict))
    {
                        ...
                        || DBGFBpIsHwIoArmed(pVM)))
        {
            ...
            NEMUSTRICTRC rcStrict2 = DBGFBpCheckIo(pVM, pVCpu, pMixedCtx, uIOPort, cbValue);
...
HMVMX_EXIT_DECL hmR0VmxExitApicAccess(PVMCPU pVCpu, PCPUMCTX pMixedCtx, PVMXTRANSIENT pVmxTransient)
{
    ...
    switch (uAccessType)
    {
        ...
        case VMX_APIC_ACCESS_TYPE_LINEAR_READ:
        {
            ...
            NEMUSTRICTRC rc2 = IOMMMIOPhysHandler(pVM, pVCpu,
...
HMVMX_EXIT_DECL hmR0VmxExitEptMisconfig(PVMCPU pVCpu, PCPUMCTX pMixedCtx, PVMXTRANSIENT pVmxTransient)
{
     ...
     */
    PVM pVM = pVCpu->CTX_SUFF(pVM);
    NEMUSTRICTRC rc2 = PGMR0Trap0eHandlerNPMisconfig(pVM, pVCpu, PGMMODE_EPT, CPUMCTX2CORE(pMixedCtx), GCPhys, UINT32_MAX);
    rc = NEMUSTRICTRC_VAL(rc2);
        ...
        switch (pDis->pCurInstr->uOpcode)
        {
            ...
            case OP_POPF:
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    Eflags.u32 = 0;
                    rc = NEMUSTRICTRC_TODO(PGMPhysRead(pVM, (RTGCPHYS)GCPtrStack, &Eflags.u32, cbParm, PGMACCESSORIGIN_HM));
            ...
            case OP_PUSHF:
            {
                ...
                rc = NEMUSTRICTRC_TODO(PGMPhysWrite(pVM, (RTGCPHYS)GCPtrStack, &Eflags.u, cbParm, PGMACCESSORIGIN_HM));
                if (RT_UNLIKELY(rc != VINF_SUCCESS))
                {
                    AssertMsgFailed(("%Rrc\n", rc)); /** @todo allow strict return codes here */
                    rc = VERR_EM_INTERPRETER;
            ...
            case OP_IRET:
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    rc = NEMUSTRICTRC_TODO(PGMPhysRead(pVM, (RTGCPHYS)GCPtrStack, &aIretFrame[0], sizeof(aIretFrame),
                                                       PGMACCESSORIGIN_HM));
                    AssertMsg(rc == VINF_SUCCESS, ("%Rrc\n", rc)); /** @todo allow strict return codes here */
                }
                if (RT_FAILURE(rc))
            ...
            default:
            {
                ...
                NEMUSTRICTRC rc2 = EMInterpretInstructionDisasState(pVCpu, pDis, CPUMCTX2CORE(pMixedCtx), 0 /* pvFault */,
