...
int CrFbWindow::SetCompositor(const struct NEMUVR_SCR_COMPOSITOR * pCompositor)
{
    if (!checkInitedUpdating())
    {
        WARN(("err"));
...
bool CrFbWindow::SetScaleFactor(GLdouble scaleFactorW, GLdouble scaleFactorH)
{
    ...
    if ( !(  (scaleFactorW >= NEMU_OGL_SCALE_FACTOR_MIN && scaleFactorW <= NEMU_OGL_SCALE_FACTOR_MAX)
          && (scaleFactorH >= NEMU_OGL_SCALE_FACTOR_MIN && scaleFactorH <= NEMU_OGL_SCALE_FACTOR_MAX)))
    {
        crDebug("CrFbWindow: attempt to set scale factor out of valid values range: scaleFactorW=%d, scaleFactorH=%d, multiplier=%d.",
            (int)(scaleFactorW * NEMU_OGL_SCALE_FACTOR_MULTIPLIER), (int)(scaleFactorH * NEMU_OGL_SCALE_FACTOR_MULTIPLIER),
...
int CrFbWindow::UpdateBegin()
{
    ...
    {
        Assert(mSpuWindow);
        cr_server.head_spu->dispatch_table.NemuPresentComposition(mSpuWindow, NULL, NULL);
...
void CrFbWindow::UpdateEnd()
{
    ...
    if (mSpuWindow)
    {
        ...
        if (fPresentNeeded || mFlags.fForcePresentOnReenable)
        {
            ...
            if (mpCompositor)
            {
                CrVrScrCompositorSetStretching((NEMUVR_SCR_COMPOSITOR *)mpCompositor, scaleFactorW, scaleFactorH);
                cr_server.head_spu->dispatch_table.NemuPresentComposition(mSpuWindow, mpCompositor, NULL);
            }
            else
            {
                ...
                CrVrScrCompositorSetStretching((NEMUVR_SCR_COMPOSITOR *)&TmpCompositor, scaleFactorW, scaleFactorH);
                /* this is a cleanup operation
                 * empty compositor is guarantid to be released on NemuPresentComposition return */
                cr_server.head_spu->dispatch_table.NemuPresentComposition(mSpuWindow, &TmpCompositor, NULL);
            }
