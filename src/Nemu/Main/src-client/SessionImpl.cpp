/* $Id: SessionImpl.cpp $ */
/** @file
 * Nemu Client Session COM Class implementation in NemuC.
 */
...
HRESULT Session::setName(const com::Utf8Str &aName)
{
    ...
    if (mState != SessionState_Unlocked)
        return setError(NEMU_E_INVALID_OBJECT_STATE, tr("Trying to set name for a session which is not in state \"unlocked\""));
...
HRESULT Session::getMachine(ComPtr<IMachine> &aMachine)
{
    ...
    HRESULT rc;
#ifndef NEMU_COM_INPROC_API_CLIENT
    if (mConsole)
...
#else  /* NEMU_COM_INPROC_API_CLIENT */
    AssertFailed();
    return NEMU_E_INVALID_OBJECT_STATE;
#endif /* NEMU_COM_INPROC_API_CLIENT */
}

HRESULT Session::getNominalState(MachineState_T *aNominalState)
{
    AutoReadLock alock(this COMMA_LOCKVAL_SRC_POS);
    AssertReturn(mState == SessionState_Locked, NEMU_E_INVALID_VM_STATE);
    AssertReturn(mType == SessionType_WriteLock, NEMU_E_INVALID_OBJECT_STATE);
#ifndef NEMU_COM_INPROC_API_CLIENT
    AssertReturn(mConsole, NEMU_E_INVALID_OBJECT_STATE);
...
#endif /* !NEMU_WITH_GENERIC_SESSION_WATCHER */
{
    ...
    AssertReturn(mState == SessionState_Unlocked, NEMU_E_INVALID_VM_STATE);
    ...
    if (!aMachine)
    {
        ...
        AssertReturn(mType == SessionType_Null, NEMU_E_INVALID_OBJECT_STATE);
...

{
    ...
    AssertReturn(mState == SessionState_Unlocked ||
                 mState == SessionState_Spawning, NEMU_E_INVALID_VM_STATE);
...
# ifndef NEMU_COM_INPROC_API_CLIENT
     ...
     * handles this correctly, by doing the operation in NemuSVC. */
    if (!mConsole)
...
# else  /* NEMU_COM_INPROC_API_CLIENT */
     ...
     *        method call, NemuSVC should be clever enough to see that the
...
{
#if defined(NEMU_WITH_GUEST_PROPS) && !defined(NEMU_COM_INPROC_API_CLIENT)
    if (mState != SessionState_Locked)
        return setError(NEMU_E_INVALID_VM_STATE,
                        tr("Machine is not locked by session (session state: %s)."),
                        Global::stringifySessionState(mState));
...
#else /* NEMU_WITH_GUEST_PROPS not defined */
    ReturnComNotImplemented();
#endif /* NEMU_WITH_GUEST_PROPS not defined */
}
...
#if !defined(DEBUG_bird) && !defined(DEBUG_andy) /* I don't want clients crashing on me just because NemuSVC went belly up. */
        AssertComRC(rc);
#endif
    }
