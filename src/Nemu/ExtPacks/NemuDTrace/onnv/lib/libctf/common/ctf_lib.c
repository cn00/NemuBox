...
#ifndef NEMU /* staticly linked in */
#ifdef _LP64
...
# define mmap64(a_pvAddr, a_cb, a_fProt, a_fFlags, a_fd, a_offFile) \
	NemuCtfMap64Fake(a_pvAddr, a_cb, a_fProt, a_fFlags, a_fd, a_offFile)
...
static void *NemuCtfMap64Fake(void *pvAddr, size_t cb, int fProt, int fFlags, int fd, int64_t offFile)
{
...
#define pread64(a_fd, a_pvBuf, a_cbToRead, a_offFile) \
	NemuCtfPRead64(a_fd, a_pvBuf, a_cbToRead, a_offFile)

static ssize_t NemuCtfPRead64(int fd, void *pvBuf, size_t cbToRead, int64_t offFile)
{
	if ((off_t)offFile != offFile) {
		errno = EIO;
		return -1;
