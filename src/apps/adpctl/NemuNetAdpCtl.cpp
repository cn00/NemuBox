/* $Id: NemuNetAdpCtl.cpp $ */
/** @file
 * Apps - NemuAdpCtl, Configuration tool for nemunetX adapters.
 */
...
/** @todo These are duplicates from src/Nemu/HostDrivers/NemuNetAdp/NemuNetAdpInternal.h */
#define NEMUNETADP_CTL_DEV_NAME    "/dev/nemunetctl"
...
#define NEMUADPCTL_IFCONFIG_PATH1 "/sbin/ifconfig"
#define NEMUADPCTL_IFCONFIG_PATH2 "/bin/ifconfig"
static char *g_pszIfConfig;

#if defined(RT_OS_LINUX)
# define NEMUADPCTL_DEL_CMD "del"
# define NEMUADPCTL_ADD_CMD "add"
#elif defined(RT_OS_SOLARIS)
# define NEMUADPCTL_DEL_CMD "removeif"
# define NEMUADPCTL_ADD_CMD "addif"
#else
# define NEMUADPCTL_DEL_CMD "delete"
# define NEMUADPCTL_ADD_CMD "add"
#endif

static void showUsage(void)
{
    fprintf(stderr, "Usage: NemuNetAdpCtl <adapter> <address> ([netmask <address>] | remove)\n");
    fprintf(stderr, "     | NemuNetAdpCtl [<adapter>] add\n");
    fprintf(stderr, "     | NemuNetAdpCtl <adapter> remove\n");
}

static void setPathIfConfig(void)
{
    struct stat s;
    if (   !stat(NEMUADPCTL_IFCONFIG_PATH1, &s)
        && S_ISREG(s.st_mode))
        g_pszIfConfig = (char*)NEMUADPCTL_IFCONFIG_PATH1;
    else
        g_pszIfConfig = (char*)NEMUADPCTL_IFCONFIG_PATH2;
}
...
static bool removeAddresses(char *pszAdapterName)
{
    ...
    for (int i = 0; i < cAddrs; i++)
    {
        if (executeIfconfig(pszAdapterName, "inet6",
                            NEMUADPCTL_DEL_CMD, aszAddresses[i]) != EXIT_SUCCESS)
            return false;
    }
...
static int doIOCtl(unsigned long uCmd, NEMUNETADPREQ *pReq)
{
    int fd = open(NEMUNETADP_CTL_DEV_NAME, O_RDWR);
    if (fd == -1)
    {
        fprintf(stderr, "NemuNetAdpCtl: Error while %s %s: ",
                uCmd == NEMUNETADP_CTL_REMOVE ? "removing" : "adding",
                pReq->szName[0] ? pReq->szName : "new interface");
        perror("failed to open " NEMUNETADP_CTL_DEV_NAME);
    ...
    if (rc == -1)
    {
        fprintf(stderr, "NemuNetAdpCtl: Error while %s %s: ",
                uCmd == NEMUNETADP_CTL_REMOVE ? "removing" : "adding",
                pReq->szName[0] ? pReq->szName : "new interface");
        perror("NemuNetAdpCtl: ioctl failed for " NEMUNETADP_CTL_DEV_NAME);
...
static int checkAdapterName(const char *pcszNameIn, char *pszNameOut)
{
    ...
    if (   strlen(pcszNameIn) >= NEMUNETADP_MAX_NAME_LEN
        || sscanf(pcszNameIn, "nemunet%d", &iAdapterIndex) != 1
        || iAdapterIndex < 0 || iAdapterIndex >= NEMUNETADP_MAX_INSTANCES )
    {
        fprintf(stderr, "NemuNetAdpCtl: Setting configuration for '%s' is not supported.\n", pcszNameIn);
        return ADPCTLERR_BAD_NAME;
    }
    sprintf(pszNameOut, "nemunet%d", iAdapterIndex);
    if (strcmp(pszNameOut, pcszNameIn))
    {
        fprintf(stderr, "NemuNetAdpCtl: Invalid adapter name '%s'.\n", pcszNameIn);
        return ADPCTLERR_BAD_NAME;
    }
...
{
    char szAdapterName[NEMUNETADP_MAX_NAME_LEN];
    ...
    switch (argc)
    {
        ...
        case 3:
        {
            ...
            if (strcmp("speed", argv[2]) == 0)
            {
                ...
                if (fd < 0)
                {
                    fprintf(stderr, "NemuNetAdpCtl: Error while retrieving link "
                            "speed for %s: ", pszAdapterName);
                    perror("NemuNetAdpCtl: failed to open control socket");
                ...
                if (rc == 0)
                {
                    if (EthToolVal.data)
                    {
                        ...
                        else
                        {
                            fprintf(stderr, "NemuNetAdpCtl: Error while retrieving link "
                                    "speed for %s: ", pszAdapterName);
                            perror("NemuNetAdpCtl: ioctl failed");
                            rc = ADPCTLERR_IOCTL_FAILED;
                        }
                    }
                    else
            ...
            if (strcmp("remove", pszAddress) == 0)
            {
                ...
                return 1;
#else
                return doIOCtl(NEMUNETADP_CTL_REMOVE, &Req);
#endif
            }
            else if (strcmp("add", pszAddress) == 0)
            {
                ...
                return 1;
#else
                rc = doIOCtl(NEMUNETADP_CTL_ADD, &Req);
                if (rc == 0)
                    puts(Req.szName);
#endif
                return rc;
    ...
    if (fRemove)
    {
        if (strchr(pszAddress, ':'))
            rc = executeIfconfig(pszAdapterName, "inet6", NEMUADPCTL_DEL_CMD, pszAddress);
        else
        {
#if defined(RT_OS_LINUX)
            rc = executeIfconfig(pszAdapterName, "0.0.0.0");
#else
            rc = executeIfconfig(pszAdapterName, NEMUADPCTL_DEL_CMD, pszAddress);
#endif
    ...
    else
    {
        ...
        if (strchr(pszAddress, ':'))
        {
            ...
            if (!removeAddresses(pszAdapterName))
                ...
                rc = executeIfconfig(pszAdapterName, "inet6", NEMUADPCTL_ADD_CMD, pszAddress, pszOption, pszNetworkMask);
        }
        else
        {
#ifdef RT_OS_SOLARIS
