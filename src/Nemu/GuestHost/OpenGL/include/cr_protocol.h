...
#define CR_NEMU_CAP_TEX_PRESENT         0x00000001
/* vbva command submission mechanism supported */
#define CR_NEMU_CAP_CMDVBVA             0x00000002
 ...
 */
#define CR_NEMU_CAP_CMDBLOCKS            0x00000004
/* GetAttribsLocations support */
#define CR_NEMU_CAP_GETATTRIBSLOCATIONS  0x00000008
/* flush command blocks for execution  */
#define CR_NEMU_CAP_CMDBLOCKS_FLUSH      0x00000010
/* Notify guest if host reports minimal OpenGL capabilities. */
#define CR_NEMU_CAP_HOST_CAPS_NOT_SUFFICIENT 0x00000020

#define CR_NEMU_CAPS_ALL                 0x0000003f
...
typedef struct CRNEMUHGSMI_CMDDATA {
    union
    {
        struct NEMUVDMACMD_CHROMIUM_CMD *pHgsmiCmd;
        struct NEMUCMDVBVA_CRCMD_CMD *pVbvaCmd;
        const void *pvCmd;
    };
    int          *pCmdRc;
    char         *pWriteback;
    ...
    bool fHgsmiCmd;
} CRNEMUHGSMI_CMDDATA, *PCRNEMUHGSMI_CMDDATA;

#ifdef DEBUG
# define CRNEMUHGSMI_CMDDATA_ASSERT_CONSISTENT(_pData)  do { \
...
# define CRNEMUHGSMI_CMDDATA_ASSERT_CLEANED(_pData)  do { \
        CRASSERT(!(_pData)->pvCmd); \
...
# define CRNEMUHGSMI_CMDDATA_ASSERT_ISSET(_pData)  do { \
        CRNEMUHGSMI_CMDDATA_ASSERT_CONSISTENT(_pData); \
        CRASSERT(CRNEMUHGSMI_CMDDATA_IS_SET(_pData)); \
    } while (0)

# define CRNEMUHGSMI_CMDDATA_ASSERT_ISSETWB(_pData)  do { \
        CRNEMUHGSMI_CMDDATA_ASSERT_CONSISTENT(_pData); \
        CRASSERT(CRNEMUHGSMI_CMDDATA_IS_SETWB(_pData)); \
    } while (0)
#else
# define CRNEMUHGSMI_CMDDATA_ASSERT_CONSISTENT(_pData)  do { } while (0)
# define CRNEMUHGSMI_CMDDATA_ASSERT_CLEANED(_pData)  do { } while (0)
# define CRNEMUHGSMI_CMDDATA_ASSERT_ISSET(_pData)  do { } while (0)
# define CRNEMUHGSMI_CMDDATA_ASSERT_ISSETWB(_pData)  do { } while (0)
#endif

#define CRNEMUHGSMI_CMDDATA_IS_HGSMICMD(_pData) (!!(_pData)->fHgsmiCmd)
#define CRNEMUHGSMI_CMDDATA_IS_SET(_pData) (!!(_pData)->pvCmd)
#define CRNEMUHGSMI_CMDDATA_IS_SETWB(_pData) (!!(_pData)->pWriteback)

#define CRNEMUHGSMI_CMDDATA_CLEANUP(_pData) do { \
        crMemset(_pData, 0, sizeof (CRNEMUHGSMI_CMDDATA)); \
        CRNEMUHGSMI_CMDDATA_ASSERT_CLEANED(_pData); \
        CRNEMUHGSMI_CMDDATA_ASSERT_CONSISTENT(_pData); \
    } while (0)

#define CRNEMUHGSMI_CMDDATA_SET(_pData, _pCmd, _pHdr, _fHgsmiCmd) do { \
        CRNEMUHGSMI_CMDDATA_ASSERT_CLEANED(_pData); \
        (_pData)->pvCmd = (_pCmd); \
...
#define CRNEMUHGSMI_CMDDATA_SETWB(_pData, _pCmd, _pHdr, _pWb, _cbWb, _pcbWb, _fHgsmiCmd) do { \
        CRNEMUHGSMI_CMDDATA_SET(_pData, _pCmd, _pHdr, _fHgsmiCmd); \
...
#define CRNEMUHGSMI_CMDDATA_RC(_pData, _rc) do { \
        *(_pData)->pCmdRc = (_rc); \
    } while (0)
#endif
