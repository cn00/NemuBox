...
for func_name in keys:
	if "Chromium" == apiutil.Category(func_name):
		continue
	if "Nemu" == apiutil.Category(func_name):
		continue
	...
	name = "gl" + func_name
	address = "NEMUGLTAG(gl" + func_name + ")"
	if wrap:
...
#define GLXAPI_ENTRY(Func) if (!crStrcmp(name, "glX"#Func)) return (CR_PROC) &NEMUGLXENTRYTAG(glX##Func);
#include "fakedri_glxfuncsList.h"
#undef GLXAPI_ENTRY

    /*CR_EXT_texture_from_pixmap*/
    if (!crStrcmp( name, "glXBindTexImageEXT" )) return (CR_PROC) NEMUGLXTAG(glXBindTexImageEXT);
    if (!crStrcmp( name, "glXReleaseTexImageEXT" )) return (CR_PROC) NEMUGLXTAG(glXReleaseTexImageEXT);
     ...
     * Untill 3.0 support, it's better to go this way instead of adding an alias to src/Nemu/GuestHost/OpenGL/glapi_parser/apispec.txt.
     */
