 ...
 * Generated from: src/Nemu/Main/idl/VirtualBox.xidl
 * Generator: src/Nemu/Main/idl/apiwrap-server.xsl
 */
...
#include "Wrapper.h"
    ...
    <xsl:value-of select="concat('    NEMU_SCRIPTABLE_IMPL(', @name, ')')"/>
    ...
    <xsl:for-each select="exsl:node-set($addinterfaces)/token">
        <xsl:value-of select="concat('    NEMU_SCRIPTABLE_IMPL(', text(), ')')"/>
...
<xsl:template match="interface" mode="codefooter">
    <xsl:param name="addinterfaces"/>
    <xsl:text>#ifdef NEMU_WITH_XPCOM
</xsl:text>
    ...
    <xsl:text>#endif // NEMU_WITH_XPCOM
</xsl:text>
</xsl:template>
...
{</xsl:text>
    ...
    <xsl:if test="$attrbasename != 'MidlDoesNotLikeEmptyInterfaces'">
        ...
        <xsl:value-of select="translate(concat('NEMUAPI_', $dtracetopclass, '_GET_', $dtraceattrname, '_ENTER('), $G_lowerCase, $G_upperCase)"/>
        <xsl:text>this);
#endif</xsl:text>
    </xsl:if>
    <xsl:text>
    ...
    <xsl:if test="$attrbasename != 'MidlDoesNotLikeEmptyInterfaces'">
        ...
        <xsl:value-of select="translate(concat('NEMUAPI_', $dtracetopclass, '_GET_', $dtraceattrname, '_RETURN('), $G_lowerCase, $G_upperCase)"/>
        ...
        <xsl:text>)
{
    ...
    try
    {
        ...
        <xsl:value-of select="translate(concat('NEMUAPI_', $topclass, '_SET_', $dtraceattrname, '_ENTER('), $G_lowerCase, $G_upperCase)"/>
        ...
        <xsl:value-of select="translate(concat('NEMUAPI_', $dtracetopclass, '_SET_', $dtraceattrname, '_RETURN('), $G_lowerCase, $G_upperCase)"/>
        <xsl:text>this, hrc, 0 /*normal*/,</xsl:text>
        <xsl:call-template name="emitDTraceParamValue">
            <xsl:with-param name="dir">in</xsl:with-param>
        </xsl:call-template>
...
<xsl:template name="emitTargetBegin">
    ...
    <xsl:choose>
        ...
        <xsl:when test="$target = 'midl'">
            <xsl:text>#ifndef NEMU_WITH_XPCOM
</xsl:text>
        </xsl:when>
...
<xsl:text>
    ...
    <xsl:value-of select="translate(concat('NEMUAPI_', $dtracetopclass, '_', $dtracemethodname, substring($dtracenamehack, 2), '_ENTER('), $G_lowerCase, $G_upperCase)"/>
    <xsl:text>this</xsl:text>
    ...
    <xsl:value-of select="translate(concat('NEMUAPI_', $dtracetopclass, '_', $dtracemethodname, substring($dtracenamehack, 2), '_RETURN('), $G_lowerCase, $G_upperCase)"/>
    <xsl:text>this, hrc, 0 /*normal*/</xsl:text>
    <xsl:for-each select="param">
        <xsl:text>, </xsl:text>
        <xsl:call-template name="emitDTraceParamValue">
