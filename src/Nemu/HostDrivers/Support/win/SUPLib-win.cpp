...
#define SERVICE_NAME_W    L"NemuDrv"
...
int suplibOsHardenedVerifyInit(void)
{
    ...
    {
#if defined(NEMU_WITH_HARDENING) && !defined(IN_SUP_HARDENED_R3) && !defined(IN_SUP_R3_STATIC)
...
int suplibOsInit(PSUPLIBDATA pThis, bool fPreInited, bool fUnrestricted, SUPINITOP *penmWhat, PRTERRINFO pErrInfo)
{
    ...
    if (fPreInited)
    {
#if defined(NEMU_WITH_HARDENING) && !defined(IN_SUP_HARDENED_R3)
...
# endif
    ...
    for (;;)
    {
        ...
        static const WCHAR  s_wszName[] = L"\\Device\\NemuDrvU";
...
#ifdef IN_SUP_HARDENED_R3
        /*
         * Get more details from NemuDrvErrorInfo if present.
         */
...
int RTPathExecDirW(WCHAR *wsz, int len)
{

    HMODULE hExe = GetModuleHandle(NULL);
    if(!GetModuleFileNameW(hExe, wsz, len))
    {
...
static int suplibOsCreateService(void)
{
    ...
    if (hSMgrCreate != NULL)
    {
        WCHAR szDriver[RTPATH_MAX];
        rc = RTPathExecDirW(szDriver, sizeof(szDriver) - sizeof(L"\\NemuDrv.sys"));
		printf("RTPathExecDirW path=%S rc=%d\n", szDriver, rc);
        if (RT_SUCCESS(rc))
        {
            wcscat(szDriver, L"\\NemuDrv.sys");
            SC_HANDLE hService = CreateServiceW(hSMgrCreate,
                                               SERVICE_NAME_W,
                                               L"Nemu Support Driver",
            ...
            dwErr = GetLastError();
			printf("CreateServiceW path=%S dwErr=%d\n", szDriver, dwErr);
            if (hService)
            {
	...
	printf("suplibOsCreateService rc=%d\n", rc);
    return rc;
}
    ...
    if (hSMgr)
    {
        ...
        if (hService)
        {
            ...
            else if (ControlService(hService, SERVICE_CONTROL_STOP, &Status))
            {
                ...
                {
					printf("ControlService success, but fail to stop the service\n");
        ...
        else
        {
            ...
            {
				printf("ControlService fail, dwErr = %d\n", dwErr);
...
int suplibOsDeleteService(void)
{
    ...
    if (hSMgr)
    {
        ...
        if (hService)
        {
            ...
            {
                dwErr = GetLastError();
				printf("DeleteService failed dwErr=%Rwa\n", dwErr);
        ...
        else
        {
            ...
            {
				printf("OpenService failed dwErr=%Rwa\n", dwErr);
                AssertMsgFailed(("OpenService failed dwErr=%Rwa\n", dwErr));
                rc = RTErrConvertFromWin32(dwErr);
            }
        }
...
static int suplibOsUpdateService(void)
{
    ...
    if (hSMgr)
    {
        ...
        if (hService)
        {
            ...
            int rc = RTPathExecDir(szDriver, sizeof(szDriver) - sizeof("\\NemuDrv.sys"));
            if (RT_SUCCESS(rc))
            {
                strcat(szDriver, "\\NemuDrv.sys");
...
static int suplibOsStartService(void)
{
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    DWORD dwErr = GetLastError();
					printf("StartService error, dwErr=%d\n", dwErr);
    ...
    if (!CloseServiceHandle(hSMgr))
        AssertFailed();
	printf("suplibOsStartService rc=%d\n", rc);
    return rc;
}
...
# else
/**
 * Reverse of NemuDrvNtErr2NtStatus
 * returns Nemu status code.
...
static int suplibConvertNtStatus(NTSTATUS rcNt)
{
    ...
    /* See NemuDrvNtErr2NtStatus. */
    if (SUP_NT_STATUS_IS_NEMU(rcNt))
        return SUP_NT_STATUS_TO_NEMU(rcNt);
