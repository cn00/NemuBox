...
<!--
    ...
    websrv-wsdl.xsl:
        XSLT stylesheet that generates nemuwebService.wsdl from
        VirtualBox.xidl. That extra WSDL file includes the big
        nemuweb.wsdl file and adds a "service" section.
...
<xsl:template match="/idl">
  ...
  Generated from: src/Nemu/Main/idl/VirtualBox.xidl (VirtualBox's generic pseudo-IDL file)
  Generator: src/Nemu/Main/webservice/websrv-wsdl-service.xsl
</xsl:comment>
  <xsl:apply-templates />
</xsl:template>
...
<xsl:template match="library">
  <definitions xmlns:interface="urn:nemu"
               xmlns:soap="http://schemas.xmlsoap.org/wsdl/soap/"
    ...
    <import location="nemuweb.wsdl" namespace="urn:nemu">
      <xsl:attribute name="namespace"><xsl:value-of select="$G_targetNamespace" /></xsl:attribute>
    </import>

    <service name="nemuService">
      <port>
        <xsl:attribute name="binding"><xsl:value-of select="concat('nemu:nemu', $G_bindingSuffix)" /></xsl:attribute>
        <xsl:attribute name="name"><xsl:value-of select="concat('nemu', 'ServicePort')" /></xsl:attribute>
