...
# Nemu - begin
#filename = sys.argv[1] - Nemu changed it to:
...
#if filename.lower().find('ia32') >= 0: - Nemu changed it to:
if OnlyPaddImage:
    #d = open(sys.argv[1], 'rb').read() - Nemu changed it to:
    d = open(SrcFilename, 'rb').read()
    c = ((len(d) + 4 + 7) & ~7) - 4
    if c > len(d):
        c -= len(d)
        #f = open(sys.argv[1], 'wb') - Nemu changed it to:
