...
typedef struct NEMUVMSVGASHADERIF *PNEMUVMSVGASHADERIF;
/**
 * Interface the shader lib can use to talk back to the Nemu VMSVGA OGL 3D code.
 */
typedef struct NEMUVMSVGASHADERIF
{
    /**
     ...
     */
    DECLCALLBACKMEMBER(void, pfnSwitchInitProfile)(PNEMUVMSVGASHADERIF pThis, bool fOtherProfile);
     ...
     *                          non-default if SET.
     */
    DECLCALLBACKMEMBER(bool, pfnGetNextExtension)(PNEMUVMSVGASHADERIF pThis, void **ppvEnumCtx, char *pszBuf, size_t cbBuf,
                                                  bool fOtherProfile);
} NEMUVMSVGASHADERIF;

SHADERDECL(int) ShaderInitLib(PNEMUVMSVGASHADERIF pNemuShaderIf);
