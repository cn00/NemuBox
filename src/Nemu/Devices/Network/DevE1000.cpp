 ...
 * Ethernet Controllers Software Developers Manual").
 */
 ...
 * Ethernet Controllers Software Developers Manual" for more detailed
 * explanation.
 */
...
# define E1K_SAVEDSTATE_VERSION_NEMU_42_VTAG  3
...
DECLINLINE(void) e1kSetupGsoCtx(PPDMNETWORKGSO pGso, E1KTXCTX const *pCtx)
{
     ...
     */
    if (RT_UNLIKELY( pCtx->dw3.u8HDRLEN + pCtx->dw2.u20PAYLEN > NEMU_MAX_GSO_SIZE ))
    {
        E1kLog(("e1kSetupGsoCtx: HDRLEN(=%#x) + PAYLEN(=%#x) = %#x, max is %#x\n",
                pCtx->dw3.u8HDRLEN, pCtx->dw2.u20PAYLEN, pCtx->dw3.u8HDRLEN + pCtx->dw2.u20PAYLEN, NEMU_MAX_GSO_SIZE));
        return;
    }
...
static DECLCALLBACK(int) e1kLoadExec(PPDMDEVINS pDevIns, PSSMHANDLE pSSM, uint32_t uVersion, uint32_t uPass)
{
...
#ifdef E1K_WITH_TXD_CACHE
        &&  uVersion != E1K_SAVEDSTATE_VERSION_NEMU_42_VTAG
#endif /* E1K_WITH_TXD_CACHE */
        &&  uVersion != E1K_SAVEDSTATE_VERSION_NEMU_41
        &&  uVersion != E1K_SAVEDSTATE_VERSION_NEMU_30)
        return VERR_SSM_UNSUPPORTED_DATA_UNIT_VERSION;

    if (   uVersion > E1K_SAVEDSTATE_VERSION_NEMU_30
    ...
    if (uPass == SSM_PASS_FINAL)
    {
        ...
        AssertRCReturn(rc, rc);
        if (uVersion > E1K_SAVEDSTATE_VERSION_NEMU_41)
        {
...
#ifdef E1K_WITH_TXD_CACHE
        if (uVersion > E1K_SAVEDSTATE_VERSION_NEMU_42_VTAG)
        {
...
static DECLCALLBACK(void) e1kConfigurePciDev(PPCIDEVICE pPciDev, E1KCHIP eChip)
{
    ...
    PCIDevSetWord( pPciDev, NEMU_PCI_SUBSYSTEM_VENDOR_ID, g_Chips[eChip].uPCISubsystemVendorId);
    PCIDevSetWord( pPciDev, NEMU_PCI_SUBSYSTEM_ID, g_Chips[eChip].uPCISubsystemId);

    PCIDevSetWord( pPciDev, NEMU_PCI_COMMAND,            0x0000);
    /* DEVSEL Timing (medium device), 66 MHz Capable, New capabilities */
    PCIDevSetWord( pPciDev, NEMU_PCI_STATUS,
                   NEMU_PCI_STATUS_DEVSEL_MEDIUM | NEMU_PCI_STATUS_CAP_LIST |  NEMU_PCI_STATUS_66MHZ);
    /* Stepping A2 */
    PCIDevSetByte( pPciDev, NEMU_PCI_REVISION_ID,          0x02);
    /* Ethernet adapter */
    PCIDevSetByte( pPciDev, NEMU_PCI_CLASS_PROG,           0x00);
    PCIDevSetWord( pPciDev, NEMU_PCI_CLASS_DEVICE,       0x0200);
    /* normal single function Ethernet controller */
    PCIDevSetByte( pPciDev, NEMU_PCI_HEADER_TYPE,          0x00);
    /* Memory Register Base Address */
    PCIDevSetDWord(pPciDev, NEMU_PCI_BASE_ADDRESS_0, 0x00000000);
    /* Memory Flash Base Address */
    PCIDevSetDWord(pPciDev, NEMU_PCI_BASE_ADDRESS_1, 0x00000000);
    /* IO Register Base Address */
    PCIDevSetDWord(pPciDev, NEMU_PCI_BASE_ADDRESS_2, 0x00000001);
    /* Expansion ROM Base Address */
    PCIDevSetDWord(pPciDev, NEMU_PCI_ROM_ADDRESS,    0x00000000);
    /* Capabilities Pointer */
    PCIDevSetByte( pPciDev, NEMU_PCI_CAPABILITY_LIST,      0xDC);
    /* Interrupt Pin: INTA# */
    PCIDevSetByte( pPciDev, NEMU_PCI_INTERRUPT_PIN,        0x01);
    /* Max_Lat/Min_Gnt: very high priority and time slice */
    PCIDevSetByte( pPciDev, NEMU_PCI_MIN_GNT,              0xFF);
    PCIDevSetByte( pPciDev, NEMU_PCI_MAX_LAT,              0x00);

    /* PCI Power Management Registers ****************************************/
    /* Capability ID: PCI Power Management Registers */
    PCIDevSetByte( pPciDev, 0xDC,            NEMU_PCI_CAP_ID_PM);
    ...
    PCIDevSetByte( pPciDev, 0xE4,          NEMU_PCI_CAP_ID_PCIX);
#ifdef E1K_WITH_MSI
    ...
    PCIDevSetWord( pPciDev, 0xE4 + 2,        NEMU_PCI_X_CMD_ERO);
