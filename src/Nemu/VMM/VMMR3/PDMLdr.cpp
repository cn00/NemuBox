...
/**
 * Loads the NemuVMMR0.r0 module early in the init process.
 *
...
{
#if !defined(PDMLDR_FAKE_MODE) && defined(NEMU_WITH_RAW_MODE)
    /*
     * Load the mandatory RC module, the NemuVMMR0.r0 is loaded before VM creation.
     */
 ...
 * @returns Nemu status code.
 * @param   pVM             The cross context VM structure.
 * @param   pszModule       Module name. If NULL the main R0 module (NemuVMMR0.r0) is assumes.
 * @param   pszSymbol       Symbol name. If it's value is less than 64k it's treated like a
 *                          ordinal value rather than a string pointer.
 * @param   ppvValue        Where to store the symbol value.
 */
 ...
 * @returns Nemu status code.
 * @param   pVM             The cross context VM structure.
 * @param   pszModule       Module name. If NULL the main R0 module (NemuVMMR0.r0) is assumed.
 ...
 *                          Ignored if NEMU_PATH_SHARED_LIBS is not defined.
 */
 ...
 * @todo    Remove the fShared hack again once we don't need to link against NemuDD anymore!
 */
static char *pdmR3File(const char *pszFile, const char *pszDefaultExt, const char *pszSearchPath, bool fShared)
{
 ...
 *
 * @returns Nemu status code. Failure will stop the enumeration.
 * @param   hLdrMod         The loader module handle.
                                                ...
                                                bool fRing0)
{
    ...
    if (!fNullRun)
        pModule = pdmR3LdrFindModule(pVM->pUVM,
                                     pszModule ? pszModule : fRing0 ? "NemuVMMR0.r0" : "VMMRC.rc",
