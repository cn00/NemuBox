...
from testdriver import nemutestvms;


#
# Exception and Error Unification Hacks.
# Note! This is pretty gross stuff. Be warned!
# TODO: Find better ways of doing these things, preferrably in nemuapi.
#
...
class ComError(object):
    ...
    # The NEMU_E_XXX bits:
    __NEMU_E_BASE = -2135228416;
    NEMU_E_OBJECT_NOT_FOUND         = __NEMU_E_BASE + 1;
    NEMU_E_INVALID_VM_STATE         = __NEMU_E_BASE + 2;
    NEMU_E_VM_ERROR                 = __NEMU_E_BASE + 3;
    NEMU_E_FILE_ERROR               = __NEMU_E_BASE + 4;
    NEMU_E_IPRT_ERROR               = __NEMU_E_BASE + 5;
    NEMU_E_PDM_ERROR                = __NEMU_E_BASE + 6;
    NEMU_E_INVALID_OBJECT_STATE     = __NEMU_E_BASE + 7;
    NEMU_E_HOST_ERROR               = __NEMU_E_BASE + 8;
    NEMU_E_NOT_SUPPORTED            = __NEMU_E_BASE + 9;
    NEMU_E_XML_ERROR                = __NEMU_E_BASE + 10;
    NEMU_E_INVALID_SESSION_STATE    = __NEMU_E_BASE + 11;
    NEMU_E_OBJECT_IN_USE            = __NEMU_E_BASE + 12;
    NEMU_E_DONT_CALL_AGAIN          = __NEMU_E_BASE + 13;
    ...
    def copyErrors(oNativeComErrorClass):
        ...
        # First, add the NEMU_E_XXX constants to dDecimalToConst.
        for sAttr in dir(ComError):
            if sAttr.startswith('NEMU_E'):
        ...
        if platform.system() == 'Windows':
            ...
            ComError.E_ACCESSDENIED           = -2147024891; # see Nemu/com/defs.h
...
class Build(object): # pylint: disable=R0903
    ...
    Note! After dropping the installation of Nemu from this code and instead
          realizing that with the nemuinstall.py wrapper driver, this class is
    ...
    def __init__(self, oDriver, strInstallPath):
        ...
        if strInstallPath is None:
            ...
            sSearch = os.environ.get('NEMU_TD_DEV_TREE', os.path.dirname(__file__)); # Env.var. for older trees or testboxscript.
            sCandidat = None;
            for i in range(0, 10):                                          # pylint: disable=W0612
                sBldDir = os.path.join(sSearch, sOut);
                if os.path.isdir(sBldDir):
                    sCandidat = os.path.join(sBldDir, 'bin', 'NemuSVC' + base.exeSuff());
                    if os.path.isfile(sCandidat):
                        self.sSdkPath = os.path.join(sBldDir, 'bin/sdk');
                        break;
                    sCandidat = os.path.join(sBldDir, 'dist/VirtualBox.app/Contents/MacOS/NemuSVC');
                    if os.path.isfile(sCandidat):
            ...
            if self.sDesignation is None:
                try:
                    ...
                    oFile.close();
                    import re;
                    oMatch = re.search("NEMU_SVN_REV=(\\d+)", s);
            ...
            # in the out dir of a svn checkout, untarred NemuAll or a real
            ...
            # Do some checks.
            sVMMR0 = os.path.join(self.sInstallPath, 'NemuVMMR0.r0');
            if not os.path.isfile(sVMMR0) and utils.getHostOs() == 'solaris': # solaris is special.
                sVMMR0 = os.path.join(self.sInstallPath, 'amd64' if utils.getHostArch() == 'amd64' else 'i386', 'NemuVMMR0.r0');
        ...
        if self.sOs == 'win' and self.sKind != 'development':
            self.sGuestAdditionsIso = '%s/NemuGuestAdditions.iso' % (self.sInstallPath,);
        elif self.sOs == 'darwin':
            self.sGuestAdditionsIso = '%s/NemuGuestAdditions.iso' % (self.sInstallPath,);
        elif self.sOs == 'solaris':
            self.sGuestAdditionsIso = '%s/NemuGuestAdditions.iso' % (self.sInstallPath,);
        else:
            self.sGuestAdditionsIso = '%s/additions/NemuGuestAdditions.iso' % (self.sInstallPath,);

        # __init__ end;

    def dump(self):
        """ Status dumper for debugging. """
        print >> sys.stderr, "testdriver.nemu.Build: sInstallPath= '%s'" % self.sInstallPath;
        print >> sys.stderr, "testdriver.nemu.Build: sSdkPath    = '%s'" % self.sSdkPath;
        print >> sys.stderr, "testdriver.nemu.Build: sSrcRoot    = '%s'" % self.sSrcRoot;
        print >> sys.stderr, "testdriver.nemu.Build: sKind       = '%s'" % self.sKind;
        print >> sys.stderr, "testdriver.nemu.Build: sDesignation= '%s'" % self.sDesignation;
        print >> sys.stderr, "testdriver.nemu.Build: sType       = '%s'" % self.sType;
        print >> sys.stderr, "testdriver.nemu.Build: sOs         = '%s'" % self.sOs;
        print >> sys.stderr, "testdriver.nemu.Build: sArch       = '%s'" % self.sArch;
...
class EventHandlerBase(object):
    ...
    def threadForPassiveMode(self):
        ...
        while not self.fShutdown:
            try:
                ...
                if not self.oNemuMgr.xcptIsDeadInterface(): reporter.logXcpt();
        ...
        if self.oEventSrc is not None:
            if self.fpApiVer < 3.3:
                ...
                try:
                    ...
                    if self.oNemuMgr.xcptIsDeadInterface():
                        reporter.log('unregisterListener failed on %s because of dead interface (%s)'
                                     % (self.oListener, self.oNemuMgr.xcptToString(),));
                    else:
    ...
    @staticmethod
    def registerDerivedEventHandler(oNemuMgr, fpApiVer, oSubClass, dArgsCopy,
                                    oSrcParent, sSrcParentNm, sICallbackNm,
                                    fMustSucceed = True, sLogSuffix = ''):
        """
        Registers the callback / event listener.
        """
        dArgsCopy['oNemuMgr'] = oNemuMgr;
        dArgsCopy['oListener'] = None;
        if fpApiVer < 3.3:
            dArgsCopy['oEventSrc'] = oSrcParent;
            try:
                oRet = oNemuMgr.createCallback(sICallbackNm, oSubClass, dArgsCopy);
            except:
            ...
            try:
                ...
                if not fPassive:
                    oListener = oRet = oNemuMgr.createListener(oSubClass, dArgsCopy);
                else:
                ...
                try:
                    oEventSrc.registerListener(oListener, [nemucon.NemuEventType_Any], not fPassive);
                except Exception, oXcpt:
                    if fMustSucceed or ComError.notEqual(oXcpt, ComError.E_UNEXPECTED):
...
class ConsoleEventHandlerBase(EventHandlerBase):
    ...
    Note! This class must not inherit from object or we'll get type errors in NemuPython.
    """
    ...
    def handleEvent(self, oEvt):
        ...
        """
        try:
            oEvtBase = self.oNemuMgr.queryInterface(oEvt, 'IEvent');
            eType = oEvtBase.type;
        except:
            reporter.logXcpt();
            return None;
        if eType == nemucon.NemuEventType_OnRuntimeError:
            try:
                oEvtIt = self.oNemuMgr.queryInterface(oEvtBase, 'IRuntimeErrorEvent');
                return self.onRuntimeError(oEvtIt.fatal, oEvtIt.id, oEvtIt.message)
            except:
                reporter.logXcpt();
        ## @todo implement the other events.
        if eType != nemucon.NemuEventType_OnMousePointerShapeChanged:
...
class VirtualBoxEventHandlerBase(EventHandlerBase):
    ...
    def __init__(self, dArgs, sName = "emanon"):
        self.oNemuMgr  = dArgs['oNemuMgr'];
        self.oNemu     = dArgs['oNemu'];
        EventHandlerBase.__init__(self, dArgs, self.oNemu.fpApiVer, sName);
    ...
    def handleEvent(self, oEvt):
        ...
        try:
            ...
            reporter.logXcpt();
            return None;
        if eType == nemucon.NemuEventType_OnMachineStateChanged:
            try:
                oEvtIt = self.oNemuMgr.queryInterface(oEvtBase, 'IMachineStateChangedEvent');
                return self.onMachineStateChange(oEvtIt.machineId, oEvtIt.state)
            except:
                reporter.logXcpt();
        elif eType == nemucon.NemuEventType_OnGuestPropertyChanged:
            try:
                oEvtIt = self.oNemuMgr.queryInterface(oEvtBase, 'IGuestPropertyChangedEvent');
...
class SessionConsoleEventHandler(ConsoleEventHandlerBase):
    ...
    def onMachineStateChange(self, sMachineId, eState):                         # pylint: disable=W0613
        ...
        self.oNemuMgr.interruptWaitEvents();
...
class TestDriver(base.TestDriver):                                              # pylint: disable=R0902
    ...
    def __init__(self):
        ...
        self.oTestVmManager     = nemutestvms.TestVmManager(self.sResourcePath);
        self.oTestVmSet         = nemutestvms.TestVmSet();
        ...
        self.sNemuSvcLogFile    = None;
        self.oNemuSvcProcess    = None;
        self.sNemuSvcPidFile    = None;
        self.fNemuSvcInDebugger = False;
        self.sNemuValidationKit     = None;
        self.sNemuValidationKitIso  = None;
        self.sNemuBootSectors   = None;
        ...
        if 'NEMU_LOG_DEST' not in os.environ:
            os.environ['NEMU_LOG_DEST'] = 'dir=%s' % (self.sScratchPath);
    ...
    def dump(self):
        ...
        print >> sys.stderr, "testdriver.nemu: fImportedNemuApi  = '%s'" % self.fImportedNemuApi;
        print >> sys.stderr, "testdriver.nemu: fpApiVer          = '%s'" % self.fpApiVer;
        print >> sys.stderr, "testdriver.nemu: oBuild            = '%s'" % self.oBuild;
        print >> sys.stderr, "testdriver.nemu: oNemuMgr          = '%s'" % self.oNemuMgr;
        print >> sys.stderr, "testdriver.nemu: oNemu             = '%s'" % self.oNemu;
        print >> sys.stderr, "testdriver.nemu: aoRemoteSessions  = '%s'" % self.aoRemoteSessions;
        print >> sys.stderr, "testdriver.nemu: aoVMs             = '%s'" % self.aoVMs;
        print >> sys.stderr, "testdriver.nemu: sNemuValidationKit    = '%s'" % self.sNemuValidationKit;
        print >> sys.stderr, "testdriver.nemu: sNemuValidationKitIso = '%s'" % self.sNemuValidationKitIso;
        print >> sys.stderr, "testdriver.nemu: sNemuBootSectors  = '%s'" % self.sNemuBootSectors;
    ...
    def _detectBuild(self, fQuiet = False):
        ...
        if 'NEMU_INSTALL_PATH' in os.environ:
            asLocs.insert(0, os.environ['NEMU_INSTALL_PATH']);
    ...
    def _detectValidationKit(self, fQuiet = False):
        ...
        Nemu Validation Kit somewhere in the immediate proximity.
        """
        if self.sNemuValidationKit is not None:
            return True;
        ...
        for i in range(len(asCandidates)):
            ...
            if os.path.isfile(os.path.join(sCandidate, 'NemuValidationKit.iso')):
                break;
        ...
        if fRc is False:
            if not fQuiet:
                reporter.error('failed to find Nemu Validation Kit installation (candidates: %s)' % (asCandidates,));
            sCandidate = os.path.join(self.sScriptPath, 'validationkit'); # Don't leave the values as None.

        #
        # Set the member values.
        #
        self.sNemuValidationKit     = sCandidate;
        self.sNemuValidationKitIso  = os.path.join(sCandidate, 'NemuValidationKit.iso');
        self.sNemuBootSectors   = os.path.join(sCandidate, 'bootsectors');
        return fRc;

    def importNemuApi(self):
        """
        Import the 'nemuapi' module from the VirtualBox build we're using and
        ...
        """
        if self.fImportedNemuApi:
            return True;

        ## @todo split up this messy function.

        # Make sure we've got our own VirtualBox config and NemuSVC (on XPCOM at least).
        if not self.fUseDefaultSvc:
            os.environ['NEMU_USER_HOME']    = os.path.join(self.sScratchPath, 'NemuUserHome');
            sUser = os.environ.get('USERNAME', os.environ.get('USER', os.environ.get('LOGNAME', 'unknown')));
            os.environ['NEMU_IPC_SOCKETID'] = sUser + '-NemuTest';
        ...
        if     self.oBuild.sArch == 'x86' \
           ...
           and os.getenv('VERSIONER_PYTHON_PREFER_32_BIT') != 'yes':
            print "WARNING: 64-bit python on darwin, 32-bit Nemu development build => crash"
            print "WARNING:   bash-3.2$ /usr/bin/python2.5 ./testdriver"
            print "WARNING: or"
            print "WARNING:   bash-3.2$ VERSIONER_PYTHON_PREFER_32_BIT=yes ./testdriver"
            return False;

        # Start NemuSVC and load the nemuapi bits.
        if self._startNemuSVC() is True:
            assert(self.oNemuSvcProcess is not None);
            ...
            if self.fImportedNemuApi and not self.fUseDefaultSvc and self.fpApiVer >= 4.0:
                sNewFolder = os.path.join(self.sScratchPath, 'NemuUserHome', 'Machines');
                try:
                    self.oNemu.systemProperties.defaultMachineFolder = sNewFolder;
                except:
                    self.fImportedNemuApi = False;
                    self.oNemuMgr = None;
                    self.oNemu    = None;
            ...
            # Kill NemuSVC on failure.
            if self.oNemuMgr is None:
                self._stopNemuSVC();
        else:
            assert(self.oNemuSvcProcess is None);
        return self.fImportedNemuApi;

    def _startNemuSVC(self): # pylint: disable=R0915
        """ Starts NemuSVC. """
        assert(self.oNemuSvcProcess is None);

        # Setup nemu logging for NemuSVC now and start it manually.  This way
        # we can control both logging and shutdown.
        self.sNemuSvcLogFile = '%s/NemuSVC-debug.log' % (self.sScratchPath,);
        try:    os.remove(self.sNemuSvcLogFile);
        except: pass;
        os.environ['NEMU_LOG']       = self.sLogSvcGroups;
        os.environ['NEMU_LOG_FLAGS'] = '%s append' % (self.sLogSvcFlags,);  # Append becuse of NemuXPCOMIPCD.
        if self.sLogSvcDest:
            os.environ['NEMU_LOG_DEST'] = self.sLogSvcDest;
        else:
            os.environ['NEMU_LOG_DEST'] = 'file=%s' % (self.sNemuSvcLogFile,);
        os.environ['NEMUSVC_RELEASE_LOG_FLAGS'] = 'time append';

        # Always leave a pid file behind so we can kill it during cleanup-before.
        self.sNemuSvcPidFile = '%s/NemuSVC.pid' % (self.sScratchPath,);
        fWritePidFile = True;

        cMsFudge      = 1;
        sNemuSVC      = '%s/NemuSVC' % (self.oBuild.sInstallPath,); ## @todo .exe and stuff.
        if self.fNemuSvcInDebugger:
            if self.sHost in ('darwin', 'freebsd', 'linux', 'solaris', ):
                # Start NemuSVC in gdb in a new terminal.
                ...
                sGdbCmdLine = '%s --args %s --pidfile %s' % (sGdb, sNemuSVC, self.sNemuSvcPidFile);
                ...
                self.oNemuSvcProcess = base.Process.spawnp(sTerm, sTerm, '-e', sGdbCmdLine);
                os.environ['SHELL'] = self.sOurShell;
                if self.oNemuSvcProcess is not None:
                    reporter.log('Press enter or return after starting NemuSVC in the debugger...');
                    sys.stdin.read(1);
                fWritePidFile = False;
                ...
                self.oNemuSvcProcess = base.Process.spawn(sWinDbg, sWinDbg, sNemuSVC + base.exeSuff());
                if self.oNemuSvcProcess is not None:
                    reporter.log('Press enter or return after starting NemuSVC in the debugger...');
                    sys.stdin.read(1);
                fWritePidFile = False;
            ...
            if self.sHost in ('darwin', 'freebsd', 'linux', 'solaris', ):
                #
                # XPCOM - We can use a pipe to let NemuSVC notify us when it's ready.
                #
                iPipeR, iPipeW = os.pipe();
                os.environ['NSPR_INHERIT_FDS'] = 'nemusvc:startup-pipe:5:0x%x' % (iPipeW,);
                reporter.log2("NSPR_INHERIT_FDS=%s" % (os.environ['NSPR_INHERIT_FDS']));

                self.oNemuSvcProcess = base.Process.spawn(sNemuSVC, sNemuSVC, '--auto-shutdown'); # SIGUSR1 requirement.
                try: # Try make sure we get the SIGINT and not NemuSVC.
                    os.setpgid(self.oNemuSvcProcess.getPid(), 0); # pylint: disable=E1101
                ...
                if sResponse is None  or  sResponse.strip() != 'READY':
                    reporter.error('NemuSVC failed starting up... (sResponse=%s)' % (sResponse,));
                    if not self.oNemuSvcProcess.wait(5000):
                        self.oNemuSvcProcess.terminate(2500);
                    self.oNemuSvcProcess.wait(5000);
                    self.oNemuSvcProcess = None;
                ...
                #
                cMsFudge = 2000;
                self.oNemuSvcProcess = base.Process.spawn(sNemuSVC, sNemuSVC);
            ...
            #
            if self.oNemuSvcProcess is not None:
                self.oNemuSvcProcess.enableCrashReporting('crash/report/svc', 'crash/dump/svc');

        #
        # Fudge and pid file.
        #
        if self.oNemuSvcProcess != None and not self.oNemuSvcProcess.wait(cMsFudge):
            if fWritePidFile:
                iPid = self.oNemuSvcProcess.getPid();
                try:
                    oFile = utils.openNoInherit(self.sNemuSvcPidFile, "w+");
                    oFile.write('%s' % (iPid,));
                    oFile.close();
                except:
                    reporter.logXcpt('sPidFile=%s' % (self.sNemuSvcPidFile,));
                reporter.log('NemuSVC PID=%u' % (iPid,));

            #
            # Finally add the task so we'll notice when it dies in a relatively timely manner.
            #
            self.addTask(self.oNemuSvcProcess);
        else:
            self.oNemuSvcProcess = None;
            try:    os.remove(self.sNemuSvcPidFile);
            except: pass;

        return self.oNemuSvcProcess != None;


    def _killNemuSVCByPidFile(self, sPidFile):
        """ Kill a NemuSVC given the pid from it's pid file. """
        ...
        # Take care checking that it's NemuSVC we're about to inhume.
        if base.processCheckPidAndName(iPid, "NemuSVC") is not True:
            reporter.log('Ignoring stale NemuSVC pid file (pid=%s)' % (iPid,));
            return False;

        # Loop thru our different ways of getting NemuSVC to terminate.
        for aHow in [ [ base.sendUserSignal1,  5000, 'Dropping NemuSVC a SIGUSR1 hint...'], \
                      [ base.processInterrupt, 5000, 'Dropping NemuSVC a SIGINT hint...'], \
                      [ base.processTerminate, 7500, 'NemuSVC is still around, killing it...'] ]:
            reporter.log(aHow[2]);
            ...
            if fRc is True:
                break;
        if fRc:
            reporter.log('Successfully killed NemuSVC (pid=%s)' % (iPid,));
        else:
            reporter.log('Failed to kill NemuSVC (pid=%s)' % (iPid,));
        return fRc;

    def _stopNemuSVC(self):
        """
        Stops NemuSVC.  Try the polite way first.
        """

        if self.oNemuSvcProcess:
            self.removeTask(self.oNemuSvcProcess);
            self.oNemuSvcProcess.enableCrashReporting(None, None); # Disables it.

        fRc = False;
        if   self.oNemuSvcProcess is not None \
         and not self.fNemuSvcInDebugger:
            # by process object.
            if self.oNemuSvcProcess.isRunning():
                reporter.log('Dropping NemuSVC a SIGUSR1 hint...');
                if  not self.oNemuSvcProcess.sendUserSignal1() \
                 or not self.oNemuSvcProcess.wait(5000):
                    reporter.log('Dropping NemuSVC a SIGINT hint...');
                    if  not self.oNemuSvcProcess.interrupt() \
                     or not self.oNemuSvcProcess.wait(5000):
                        reporter.log('NemuSVC is still around, killing it...');
                        self.oNemuSvcProcess.terminate();
                        self.oNemuSvcProcess.wait(7500);
            else:
                reporter.log('NemuSVC is no longer running...');
            if not self.oNemuSvcProcess.isRunning():
                self.oNemuSvcProcess = None;
        else:
            # by pid file.
            self._killNemuSVCByPidFile('%s/NemuSVC.pid' % (self.sScratchPath,));
        return fRc;

    def _setupNemuApi(self):
        """
        Import and set up the nemuapi.
        The caller saves and restores sys.path.
        """

        # Setup nemu logging for self (the test driver).
        self.sSelfLogFile = '%s/NemuTestDriver.log' % (self.sScratchPath,);
        try:    os.remove(self.sSelfLogFile);
        except: pass;
        os.environ['NEMU_LOG']       = self.sLogSelfGroups;
        os.environ['NEMU_LOG_FLAGS'] = '%s append' % (self.sLogSelfFlags, );
        if self.sLogSelfDest:
            os.environ['NEMU_LOG_DEST'] = self.sLogSelfDest;
        else:
            os.environ['NEMU_LOG_DEST'] = 'file=%s' % (self.sSelfLogFile,);
        os.environ['NEMU_RELEASE_LOG_FLAGS'] = 'time append';

        # Hack the sys.path + environment so the nemuapi can be found.
        ...
        os.environ['NEMU_PROGRAM_PATH'] = self.oBuild.sInstallPath;
        reporter.log("sys.path: %s" % (sys.path));

        try:
            # pylint: disable=F0401
            from nemuapi import VirtualBoxManager
            if self.sHost == 'win':
        ...
        # Create the manager.
        try:
            self.oNemuMgr = VirtualBoxManager(None, None)
        except:
            self.oNemuMgr = None;
        ...
        try:
            oNemu = self.oNemuMgr.getVirtualBox();
            try:
                sVer = oNemu.version;
            except:
            ...
            from testdriver.nemuwrappers import VirtualBoxWrapper;
            self.oNemu = VirtualBoxWrapper(oNemu, self.oNemuMgr, self.fpApiVer, self);
            nemucon.goHackModuleClass.oNemuMgr  = self.oNemuMgr; # NemuConstantWrappingHack.
            nemucon.fpApiVer                    = self.fpApiVer
            self.fImportedNemuApi = True;
    ...
    def _patchNemuMgr(self):
        """
        Glosses over missing self.oNemuMgr methods on older Nemu versions.
        """
        ...
        # Add utilities found in newer nemuapi revision.
        if not hasattr(self.oNemuMgr, 'xcptIsDeadInterface'):
            import types;
            self.oNemuMgr.xcptGetResult         = types.MethodType(_xcptGetResult,       self.oNemuMgr);
            self.oNemuMgr.xcptIsDeadInterface   = types.MethodType(_xcptIsDeadInterface, self.oNemuMgr);
            self.oNemuMgr.xcptIsOurXcptKind     = types.MethodType(_xcptIsOurXcptKind,   self.oNemuMgr);
            self.oNemuMgr.xcptIsEqual           = types.MethodType(_xcptIsEqual,         self.oNemuMgr);
            self.oNemuMgr.xcptToString          = types.MethodType(_xcptToString,        self.oNemuMgr);


    def _teardownNemuApi(self):
        """
        Drop all Nemu object references and shutdown com/xpcom.
        """
        if not self.fImportedNemuApi:
            return True;
    ...
    def showUsage(self):
        ...
        reporter.log('  --nemu-session-type <type>');
        ...
        reporter.log('  --nemu-default-bridged-nic <interface>');
        reporter.log('      Sets the default interface for bridged networking.');
        reporter.log('      Default: autodetect');
        reporter.log('  --nemu-use-svc-defaults');
        reporter.log('      Use default locations and files for NemuSVC.  This is useful');
        reporter.log('      for automatically configuring the test VMs for debugging.');
        reporter.log('  --nemu-self-log');
        reporter.log('      The Nemu logger group settings for the testdriver.');
        reporter.log('  --nemu-self-log-flags');
        reporter.log('      The Nemu logger flags settings for the testdriver.');
        reporter.log('  --nemu-self-log-dest');
        reporter.log('      The Nemu logger destination settings for the testdriver.');
        reporter.log('  --nemu-session-log');
        reporter.log('      The VM session logger group settings.');
        reporter.log('  --nemu-session-log-flags');
        reporter.log('      The VM session logger flags.');
        reporter.log('  --nemu-session-log-dest');
        reporter.log('      The VM session logger destination settings.');
        reporter.log('  --nemu-svc-log');
        reporter.log('      The NemuSVC logger group settings.');
        reporter.log('  --nemu-svc-log-flags');
        reporter.log('      The NemuSVC logger flag settings.');
        reporter.log('  --nemu-svc-log-dest');
        reporter.log('      The NemuSVC logger destination settings.');
        reporter.log('  --nemu-log');
        reporter.log('      The Nemu logger group settings for everyone.');
        reporter.log('  --nemu-log-flags');
        reporter.log('      The Nemu logger flags settings for everyone.');
        reporter.log('  --nemu-log-dest');
        reporter.log('      The Nemu logger destination settings for everyone.');
        reporter.log('  --nemu-svc-debug');
        reporter.log('      Start NemuSVC in a debugger');
        reporter.log('  --nemu-always-upload-logs');
        reporter.log('      Whether to always upload log files, or only do so on failure.');
        reporter.log('  --nemu-always-upload-screenshots');
    ...
    def parseOption(self, asArgs, iArg): # pylint: disable=R0915
        if asArgs[iArg] == '--nemu-session-type':
            iArg += 1;
            if iArg >= len(asArgs):
                raise base.InvalidOption('The "--nemu-session-type" takes an argument');
        ...
        elif asArgs[iArg] == '--nemu-default-bridged-nic':
            iArg += 1;
            if iArg >= len(asArgs):
                raise base.InvalidOption('The "--nemu-default-bridged-nic" takes an argument');
            self.sDefBridgedNic = asArgs[iArg];
        elif asArgs[iArg] == '--nemu-use-svc-defaults':
            self.fUseDefaultSvc = True;
        elif asArgs[iArg] == '--nemu-self-log':
            iArg += 1;
            if iArg >= len(asArgs):
                raise base.InvalidOption('The "--nemu-self-log" takes an argument');
            self.sLogSelfGroups = asArgs[iArg];
        elif asArgs[iArg] == '--nemu-self-log-flags':
            iArg += 1;
            if iArg >= len(asArgs):
                raise base.InvalidOption('The "--nemu-self-log-flags" takes an argument');
            self.sLogSelfFlags = asArgs[iArg];
        elif asArgs[iArg] == '--nemu-self-log-dest':
            iArg += 1;
            if iArg >= len(asArgs):
                raise base.InvalidOption('The "--nemu-self-log-dest" takes an argument');
            self.sLogSelfDest = asArgs[iArg];
        elif asArgs[iArg] == '--nemu-session-log':
            iArg += 1;
            if iArg >= len(asArgs):
                raise base.InvalidOption('The "--nemu-session-log" takes an argument');
            self.sLogSessionGroups = asArgs[iArg];
        elif asArgs[iArg] == '--nemu-session-log-flags':
            iArg += 1;
            if iArg >= len(asArgs):
                raise base.InvalidOption('The "--nemu-session-log-flags" takes an argument');
            self.sLogSessionFlags = asArgs[iArg];
        elif asArgs[iArg] == '--nemu-session-log-dest':
            iArg += 1;
            if iArg >= len(asArgs):
                raise base.InvalidOption('The "--nemu-session-log-dest" takes an argument');
            self.sLogSessionDest = asArgs[iArg];
        elif asArgs[iArg] == '--nemu-svc-log':
            iArg += 1;
            if iArg >= len(asArgs):
                raise base.InvalidOption('The "--nemu-svc-log" takes an argument');
            self.sLogSvcGroups = asArgs[iArg];
        elif asArgs[iArg] == '--nemu-svc-log-flags':
            iArg += 1;
            if iArg >= len(asArgs):
                raise base.InvalidOption('The "--nemu-svc-log-flags" takes an argument');
            self.sLogSvcFlags = asArgs[iArg];
        elif asArgs[iArg] == '--nemu-svc-log-dest':
            iArg += 1;
            if iArg >= len(asArgs):
                raise base.InvalidOption('The "--nemu-svc-log-dest" takes an argument');
            self.sLogSvcDest = asArgs[iArg];
        elif asArgs[iArg] == '--nemu-log':
            iArg += 1;
            if iArg >= len(asArgs):
                raise base.InvalidOption('The "--nemu-log" takes an argument');
            self.sLogSelfGroups    = asArgs[iArg];
            self.sLogSessionGroups = asArgs[iArg];
            self.sLogSvcGroups     = asArgs[iArg];
        elif asArgs[iArg] == '--nemu-log-flags':
            iArg += 1;
            if iArg >= len(asArgs):
                raise base.InvalidOption('The "--nemu-svc-flags" takes an argument');
            self.sLogSelfFlags     = asArgs[iArg];
            self.sLogSessionFlags  = asArgs[iArg];
            self.sLogSvcFlags      = asArgs[iArg];
        elif asArgs[iArg] == '--nemu-log-dest':
            iArg += 1;
            if iArg >= len(asArgs):
                raise base.InvalidOption('The "--nemu-log-dest" takes an argument');
            self.sLogSelfDest     = asArgs[iArg];
            self.sLogSessionDest  = asArgs[iArg];
            self.sLogSvcDest      = asArgs[iArg];
        elif asArgs[iArg] == '--nemu-svc-debug':
            self.fNemuSvcInDebugger = True;
        elif asArgs[iArg] == '--nemu-always-upload-logs':
            self.fAlwaysUploadLogs = True;
        elif asArgs[iArg] == '--nemu-always-upload-screenshots':
    ...
    def actionCleanupBefore(self):
        """
        Kill any NemuSVC left behind by a previous test run.
        """
        self._killNemuSVCByPidFile('%s/NemuSVC.pid' % (self.sScratchPath,));
    ...
    def actionCleanupAfter(self):
        """
        Clean up the Nemu bits and then call the base driver.
        ...
        # Drop all Nemu object references and shutdown xpcom then
        # terminating NemuSVC,  with extreme prejudice if need be.
        self._teardownNemuApi();
        self._stopNemuSVC();

        # Add the NemuSVC and testdriver debug+release log files.
        if self.fAlwaysUploadLogs or reporter.getErrorCount() > 0:
            if self.sNemuSvcLogFile is not None  and  os.path.isfile(self.sNemuSvcLogFile):
                reporter.addLogFile(self.sNemuSvcLogFile, 'log/debug/svc', 'Debug log file for NemuSVC');
                self.sNemuSvcLogFile = None;
            ...
            sNemuSvcRelLog = os.path.join(self.sScratchPath, 'NemuUserHome', 'NemuSVC.log');
            if os.path.isfile(sNemuSvcRelLog):
                reporter.addLogFile(sNemuSvcRelLog, 'log/release/svc', 'Release log file for NemuSVC');
            for sSuff in [ '.1', '.2', '.3', '.4', '.5', '.6', '.7', '.8' ]:
                if os.path.isfile(sNemuSvcRelLog + sSuff):
                    reporter.addLogFile(sNemuSvcRelLog + sSuff, 'log/release/svc', 'Release log file for NemuSVC');
            # Testbox debugging - START - TEMPORARY, REMOVE ASAP.
            if self.sHost in ('darwin', 'freebsd', 'linux', 'solaris', ):
                try:
                    print '> ls -la %s' % (os.path.join(self.sScratchPath, 'NemuUserHome'),);
                    utils.processCall(['ls', '-la', os.path.join(self.sScratchPath, 'NemuUserHome')]);
    ...
    def actionAbort(self):
        """
        Terminate NemuSVC if we've got a pid file.
        """
        self._killNemuSVCByPidFile('%s/NemuSVC.pid' % (self.sScratchPath,));
    ...
    def onExit(self, iRc):
        """
        Stop NemuSVC if we've started it.
        """
        if self.oNemuSvcProcess is not None:
            reporter.log('*** Shutting down the Nemu API... (iRc=%s)' % (iRc,));
            self._powerOffAllVms();
            self._teardownNemuApi();
            self._stopNemuSVC();
            reporter.log('*** Nemu API shutdown done.');
    ...
    def notifyAboutReadyTask(self, oTask):
        ...
        try:
            self.oNemuMgr.interruptWaitEvents();
            reporter.log2('nemu.notifyAboutReadyTask: called interruptWaitEvents');
        except:
            reporter.logXcpt('nemu.notifyAboutReadyTask');
    ...
    def waitForTasksSleepWorker(self, cMsTimeout):
        ...
        """
        try:
            rc = self.oNemuMgr.waitForEvents(int(cMsTimeout));
            _ = rc; #reporter.log2('nemu.waitForTasksSleepWorker(%u): true (waitForEvents -> %s)' % (cMsTimeout, rc));
            return True;
        except KeyboardInterrupt:
            raise;
        except:
            reporter.logXcpt('nemu.waitForTasksSleepWorker');
            return False;
    ...
    def processEvents(self, cMsTimeout = 0):
        ...
        """
        try:
            self.oNemuMgr.waitForEvents(cMsTimeout);
        except KeyboardInterrupt:
            raise;
        except:
            pass;
    ...
    def _logVmInfoUnsafe(self, oVM):                                            # pylint: disable=R0915,R0912
        ...
        This is copy, paste, search, replace and edit of infoCmd from nemushell.py.
        """
        oOsType = self.oNemu.getGuestOSType(oVM.OSTypeId)
        ...
        reporter.log("  HwVirtEx:           %s" % (oVM.getHWVirtExProperty(nemucon.HWVirtExPropertyType_Enabled)));
        reporter.log("  VPID support:       %s" % (oVM.getHWVirtExProperty(nemucon.HWVirtExPropertyType_VPID)));
        reporter.log("  Nested paging:      %s" % (oVM.getHWVirtExProperty(nemucon.HWVirtExPropertyType_NestedPaging)));
        if self.fpApiVer >= 4.2 and hasattr(nemucon, 'CPUPropertyType_LongMode'):
            reporter.log("  Long-mode:          %s" % (oVM.getCPUProperty(nemucon.CPUPropertyType_LongMode)));
        if self.fpApiVer >= 3.2:
            reporter.log("  PAE:                %s" % (oVM.getCPUProperty(nemucon.CPUPropertyType_PAE)));
            if self.fpApiVer < 5.0:
                reporter.log("  Synthetic CPU:      %s" % (oVM.getCPUProperty(nemucon.CPUPropertyType_Synthetic)));
        else:
            reporter.log("  PAE:                %s" % (oVM.getCpuProperty(nemucon.CpuPropertyType_PAE)));
            reporter.log("  Synthetic CPU:      %s" % (oVM.getCpuProperty(nemucon.CpuPropertyType_Synthetic)));
        ...
        aoControllers = self.oNemuMgr.getArray(oVM, 'storageControllers')
        if aoControllers:
        ...
        aoAttachments = self.oNemuMgr.getArray(oVM, 'mediumAttachments')
        if aoAttachments:
        ...
        for oAtt in aoAttachments:
            ...
            oMedium = oAtt.medium
            if oAtt.type == nemucon.DeviceType_HardDisk:
            ...
            if oAtt.type == nemucon.DeviceType_DVD:
            ...
            if oAtt.type == nemucon.DeviceType_Floppy:
        ...
        for iSlot in range(0, 32):
            ...
            if not oNic.enabled:
                ...
                continue;
            if   oNic.adapterType == nemucon.NetworkAdapterType_Am79C973:   sType = "PCNet";
            elif oNic.adapterType == nemucon.NetworkAdapterType_Am79C970A:  sType = "PCNetOld";
            elif oNic.adapterType == nemucon.NetworkAdapterType_I82545EM:   sType = "E1000";
            elif oNic.adapterType == nemucon.NetworkAdapterType_I82540EM:   sType = "E1000Desk";
            elif oNic.adapterType == nemucon.NetworkAdapterType_I82543GC:   sType = "E1000Srv2";
            elif oNic.adapterType == nemucon.NetworkAdapterType_Virtio:     sType = "Virtio";
            ...
            if   oNic.attachmentType == nemucon.NetworkAttachmentType_NAT:
                reporter.log("    attachmentType: NAT (%s)" % (oNic.attachmentType));
                if self.fpApiVer >= 4.1:
                    reporter.log("    nat-network:    %s" % (oNic.NATNetwork,));
            elif oNic.attachmentType == nemucon.NetworkAttachmentType_Bridged:
            ...
            elif oNic.attachmentType == nemucon.NetworkAttachmentType_Internal:
                reporter.log("    attachmentType: Internal (%s)" % (oNic.attachmentType));
                reporter.log("    intnet-name:    %s" % (oNic.internalNetwork,));
            elif oNic.attachmentType == nemucon.NetworkAttachmentType_HostOnly:
                ...
                if self.fpApiVer >= 4.1:
                    if oNic.attachmentType == nemucon.NetworkAttachmentType_Generic:
    ...
    def logVmInfo(self, oVM):                                                   # pylint: disable=R0915,R0912
        ...
        This is copy, past, search, replace and edit of infoCmd from nemushell.py.
        """
        try:
    ...
    def tryFindGuestOsId(self, sIdOrDesc):
        ...
        try:
            aoGuestTypes = self.oNemuMgr.getArray(self.oNemu, 'GuestOSTypes');
        except:
            reporter.logXcpt();
        else:
    ...
    def createTestVM(self, sName, iGroup, sHd = None, cMbRam = None, cCpus = 1, fVirtEx = None, fNestedPaging = None, \
        ...
        try:
            if self.fpApiVer >= 4.2: # Introduces grouping (third parameter, empty for now).
                oVM = self.oNemu.createMachine("", sName, [], self.tryFindGuestOsId(sKind), "");
            elif self.fpApiVer >= 4.0:
                oVM = self.oNemu.createMachine("", sName, self.tryFindGuestOsId(sKind), "", False);
            ...
            try:
                oVM.saveSettings();
                try:
                    self.oNemu.registerMachine(oVM);
                except:
                    raise;
            except:
                reporter.logXcpt();
        ...
        if oSession is not None:
            ...
            oSession.close();
        if not fRc:
            try:    self.oNemu.unregisterMachine(oVM.id);
            except: pass;
            if self.fpApiVer >= 4.0:
                try:
                    if self.fpApiVer >= 4.3:
    ...
    def addTestMachine(self, sNameOrId, fQuiet = False):
        ...
        try:
            if self.fpApiVer >= 4.0:
                oVM = self.oNemu.findMachine(sNameOrId);
            else:
    ...
    def openSession(self, oVM):
        ...
        for i in range(10):
            try:
                if self.fpApiVer <= 3.2:
                    oSession = self.oNemuMgr.openMachineSession(sUuid);
                else:
                    oSession = self.oNemuMgr.openMachineSession(oVM);
                break;
            except:
                if i == 9:
        ...
        from testdriver.nemuwrappers import SessionWrapper;
        return SessionWrapper(oSession, oVM, self.oNemu, self.oNemuMgr, self, False);
    ...
    def waitOnDirectSessionClose(self, oVM, cMsTimeout):
        ...
        while iCurPid  == iOrgPid \
          ...
          and (   eCurState == nemucon.SessionState_Unlocking \
               or eCurState == nemucon.SessionState_Spawning \
               or eCurState == nemucon.SessionState_Locked):
    ...
    def uploadStartupLogFile(self, oVM, sVmName):
        """
        Uploads the NemuStartup.log when present.
        """
        fRc = True;
        try:
            sLogFile = os.path.join(oVM.logFolder, 'NemuStartup.log');
        except:
            reporter.logXcpt();
            fRc = False;
        else:
    ...
    def startVmEx(self, oVM, fWait = True, sType = None, sName = None, asEnv = None): # pylint: disable=R0914,R0915
        ...
        if sType == 'gui' and len(self.aoRemoteSessions) == 0:
            try:
                self.oNemu.setExtraData('GUI/Input/AutoCapture', 'false');
                if self.fpApiVer >= 3.2:
                    self.oNemu.setExtraData('GUI/LicenseAgreed', '8');
                else:
                    self.oNemu.setExtraData('GUI/LicenseAgreed', '7');
                self.oNemu.setExtraData('GUI/RegistrationData',  'triesLeft=0');
                self.oNemu.setExtraData('GUI/SUNOnlineData',     'triesLeft=0');
                self.oNemu.setExtraData('GUI/SuppressMessages',  'confirmVMReset,remindAboutMouseIntegrationOn,'
                ...
                self.oNemu.setExtraData('GUI/UpdateDate',        'never');
                self.oNemu.setExtraData('GUI/PreventBetaWarning', self.oNemu.version);
            except:
                reporter.logXcpt();
        ...
        sEnv = 'NEMU_LOG=%s\nNEMU_LOG_FLAGS=%s\nNEMU_LOG_DEST=%s\nNEMU_RELEASE_LOG_FLAGS=append time' \
             % (self.sLogSessionGroups, self.sLogSessionFlags, sLogDest,);
        if sType == 'gui':
            sEnv += '\nNEMU_GUI_DBG_ENABLED=1'
        ...
        try:
            if    oTestVM is not None \
              and oTestVM.fSnapshotRestoreCurrent is True:
                if oVM.state is nemucon.MachineState_Running:
        ...
        if oWrapped is None:
            for i in range(10):
                try:
                    if   self.fpApiVer < 4.3 \
                      or (self.fpApiVer == 4.3 and not hasattr(self.oNemuMgr, 'getSessionObject')):
                        oSession = self.oNemuMgr.mgr.getSessionObject(self.oNemu);  # pylint: disable=E1101
                    else:
                        oSession = self.oNemuMgr.getSessionObject(self.oNemu);      # pylint: disable=E1101
                    if self.fpApiVer < 3.3:
                        oProgress = self.oNemu.openRemoteSession(oSession, sUuid, sType, sEnv);
                    else:
        ...
        if oWrapped is None:
            from testdriver.nemuwrappers import SessionWrapper;
            oWrapped = SessionWrapper(oSession, oVM, self.oNemu, self.oNemuMgr, self, True, sName, sLogFile);
        ...
        from testdriver.nemuwrappers import ProgressWrapper;
        return (oWrapped, ProgressWrapper(oProgress, self.oNemuMgr, self,
    ...
    def terminateVmBySession(self, oSession, oProgress = None, fTakeScreenshot = None):
        ...
        if fClose is True:
            ...
            try:
                ...
                if eState == nemucon.MachineState_Aborted:
    ...
    #
    # Methods require the Nemu API.  If the information is provided by both
    # the testboxscript as well as Nemu API, we'll check if it matches.
    #
    ...
    def _hasHostCpuFeature(self, sEnvVar, sEnum, fpApiMinVer, fQuiet):
        ...
        fNemu = None;
        self.importNemuApi();
        if self.fpApiVer >= fpApiMinVer and hasattr(nemucon, sEnum):
            try:
                fNemu = self.oNemu.host.getProcessorFeature(getattr(nemucon, sEnum));
            except:
                if not fQuiet:
                    reporter.logXcpt();

        if fNemu is not None:
            if fEnv is not None:
                if fEnv != fNemu and not fQuiet:
                    reporter.log('TestBox configuration overwritten: fNemu=%s (%s) vs. fEnv=%s (%s)'
                                 % (fNemu, sEnum, fEnv, sEnvVar));
                return fEnv;
            return fNemu;
        if fEnv is not None:
            return fEnv;
    ...
    def getHostCpuCount(self, fQuiet = False):
        ...
        try:
            cNemu = self.oNemu.host.processorOnlineCount;
        except:
            if not fQuiet:
                reporter.logXcpt();
            cNemu = None;

        if cNemu is not None:
            if cEnv is not None:
                assert cNemu == cEnv, 'Misconfigured TestBox: Nemu: %u CPUs, testboxscript: %u CPUs' % (cNemu, cEnv);
            return cNemu;
        if cEnv is not None:
            return cEnv;
        return 1;
    ...
    def _getHostCpuDesc(self, fQuiet = False):
        """
        Internal method used for getting the host CPU description from NemuSVC.
        Returns description string, on failure an empty string is returned.
        """
        try:
            return self.oNemu.host.getProcessorDescription(0);
        except:
            if not fQuiet:
                reporter.logXcpt();
        return '';
    ...
    def hasRawModeSupport(self, fQuiet = False):
        ...
        # Starting with 5.0 GA / RC2 the API can tell us whether Nemu was built
        # with raw-mode support or not.
        self.importNemuApi();
        if self.fpApiVer >= 5.0:
            try:
                fNemu = self.oNemu.systemProperties.rawModeSupported;
            except:
                if not fQuiet:
                    reporter.logXcpt();
                fNemu = True;
    ...
    def txsCdWait(self, oSession, oTxsSession, cMsTimeout = 30000, sFileCdWait = 'nemutxs-readme.txt'):
        """
    ...
    def startVmAndConnectToTxsViaTcp(self, sVmName, fCdWait = False, cMsTimeout = 15*60000, \
                                     cMsCdWait = 30000, sFileCdWait = 'nemutxs-readme.txt', \
    ...
    def txsRebootAndReconnectViaTcp(self, oSession, oTxsSession, fCdWait = False, cMsTimeout = 15*60000, \
                                    cMsCdWait = 30000, sFileCdWait = 'nemutxs-readme.txt', fNatForwardingForTxs = False):
        """
