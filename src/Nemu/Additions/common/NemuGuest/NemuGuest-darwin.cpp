/* $Id: NemuGuest-darwin.cpp $ */
/** @file
 * NemuGuest - Darwin Specifics.
 */
...
#define DEVICE_NAME_SYS     "nemuguest"
/** The user device node name. */
#define DEVICE_NAME_USR     "nemuguestu"
...
static int              vgdrvDarwinIOCtlSlow(PNEMUGUESTSESSION pSession, u_long iCmd, caddr_t pData, struct proc *pProcess);
 ...
 */
class org_virtualbox_NemuGuest : public IOService
{
    OSDeclareDefaultStructors(org_virtualbox_NemuGuest);
...
OSDefineMetaClassAndStructors(org_virtualbox_NemuGuest, IOService);
 ...
 */
class org_virtualbox_NemuGuestClient : public IOUserClient
{
    OSDeclareDefaultStructors(org_virtualbox_NemuGuestClient);

private:
    PNEMUGUESTSESSION           m_pSession;     /**< The session. */
    task_t                      m_Task;         /**< The client task. */
    org_virtualbox_NemuGuest   *m_pProvider;    /**< The service provider. */
...
OSDefineMetaClassAndStructors(org_virtualbox_NemuGuestClient, IOUserClient);
...
KMOD_EXPLICIT_DECL(NemuGuest, NEMU_VERSION_STRING, _start, _stop)
 ...
 */
static NEMUGUESTDEVEXT  g_DevExt;
...
/** Hash table */
static PNEMUGUESTSESSION    g_apSessionHashTab[19];
...
#define NEMUGUEST_OBJECT_UNINITIALIZED  (0)
#define NEMUGUEST_OBJECT_INITIALIZING   (1)
#define NEMUGUEST_OBJECT_INITIALIZED    (2)
#define NEMUGUEST_OBJECT_INVALID        (3)
/** Atomic variable used to protect work loop allocation when multiple threads attempt to obtain it. */
static uint8_t volatile     g_fWorkLoopCreated  = NEMUGUEST_OBJECT_UNINITIALIZED;
...
static kern_return_t    vgdrvDarwinStart(struct kmod_info *pKModInfo, void *pvData)
{
    ...
    if (RT_SUCCESS(rc))
    {
        Log(("NemuGuest: driver loaded\n"));
        return KMOD_RETURN_SUCCESS;
    }

    printf("NemuGuest: RTR0Init failed with rc=%d\n", rc);
    return KMOD_RETURN_FAILURE;
}


/**
 * Register NemuGuest char device
 */
static int vgdrvDarwinCharDevInit(void)
{
    int rc = RTSpinlockCreate(&g_Spinlock, RTSPINLOCK_FLAGS_INTERRUPT_SAFE, "NemuGuestDarwin");
    if (RT_SUCCESS(rc))
    {
        /*
...
static kern_return_t vgdrvDarwinStop(struct kmod_info *pKModInfo, void *pvData)
{
    ...
    printf("NemuGuest: driver unloaded\n");
    return KMOD_RETURN_SUCCESS;
}


/* Unregister NemuGuest char device */
static int vgdrvDarwinCharDevRemove(void)
{
...
/**
 * Device open. Called on open /dev/nemuguest and (later) /dev/nemuguestu.
 *
...
static int vgdrvDarwinOpen(dev_t Dev, int fFlags, int fDevType, struct proc *pProcess)
{
    ...
    /*
     * Find the session created by org_virtualbox_NemuGuestClient, fail
    ...
    int                 rc = VINF_SUCCESS;
    PNEMUGUESTSESSION   pSession = NULL;
...
static int vgdrvDarwinClose(dev_t Dev, int fFlags, int fDevType, struct proc *pProcess)
{
    ...
    /*
     * Hand the session closing to org_virtualbox_NemuGuestClient.
     */
    org_virtualbox_NemuGuestClient::sessionClose(RTProcSelf());
    return 0;
}
 ...
 * @param   pData       Pointer to the data (if any it's a NEMUGUESTIOCTLDATA (kernel copy)).
...
static int vgdrvDarwinIOCtl(dev_t Dev, u_long iCmd, caddr_t pData, int fFlags, struct proc *pProcess)
{
    ...
    if (!pSession)
    {
        Log(("NemuDrvDarwinIOCtl: WHAT?!? pSession == NULL! This must be a mistake... pid=%d iCmd=%#lx\n", (int)Process, iCmd));
        return EINVAL;
    }
...
/*
 * The NemuGuest IDC entry points.
 *
 * This code is shared with the other unixy OSes.
 */
#include "NemuGuestIDC-unix.c.h"


void VGDrvNativeISRMousePollEvent(PNEMUGUESTDEVEXT pDevExt)
{
    NOREF(pDevExt);
}
...
{
    LogFlow(("NemuGuest: Got sleep/wake notice. Message type was %x\n", uMessageType));
 ...
 * @returns m_pWorkLoop.
 */
IOWorkLoop *org_virtualbox_NemuGuest::getWorkLoop()
{
/** @todo r=bird: This is actually a classic RTOnce scenario, except it's
 *        tied to a org_virtualbox_NemuGuest instance.  */
    /*
     * Handle the case when work loop was not created yet.
     */
    if (ASMAtomicCmpXchgU8(&g_fWorkLoopCreated, NEMUGUEST_OBJECT_INITIALIZING, NEMUGUEST_OBJECT_UNINITIALIZED))
    {
        ...
        if (m_pWorkLoop)
        {
            ...
            Log(("NemuGuest: created new work loop\n"));
            ASMAtomicWriteU8(&g_fWorkLoopCreated, NEMUGUEST_OBJECT_INITIALIZED);
        }
        else
        {
            /* Notify the rest of threads about the fact that there was
             * an error during allocation of a work loop */
            Log(("NemuGuest: failed to create new work loop!\n"));
            ASMAtomicWriteU8(&g_fWorkLoopCreated, NEMUGUEST_OBJECT_UNINITIALIZED);
        }
    }
    /*
    ...
    else
    {
        ...
        while (fWorkLoopCreated == NEMUGUEST_OBJECT_INITIALIZING)
        {
            thread_block(0);
            fWorkLoopCreated = ASMAtomicReadU8(&g_fWorkLoopCreated);
        }
        if (fWorkLoopCreated != NEMUGUEST_OBJECT_INITIALIZED)
            Log(("NemuGuest: No work loop!\n"));
    }
...
bool org_virtualbox_NemuGuest::setupVmmDevInterrupts(IOService *pProvider)
{
...
bool org_virtualbox_NemuGuest::disableVmmDevInterrupts(void)
{
...
bool org_virtualbox_NemuGuest::isVmmDev(IOPCIDevice *pIOPCIDevice)
{
 ...
 * Start this service.
 */
bool org_virtualbox_NemuGuest::start(IOService *pProvider)
{
    /*
    ...
    if (m_pIOPCIDevice)
    {
        if (isVmmDev(m_pIOPCIDevice))
        {
            ...
            if (pMem)
            {
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        ...
                        if (rc == KMOD_RETURN_SUCCESS)
                        {
                            if (setupVmmDevInterrupts(pProvider))
                            {
                                ...
                                registerService();
                                LogRel(("NemuGuest: IOService started\n"));
                                return true;
                            }

                            LogRel(("NemuGuest: Failed to set up interrupts\n"));
                            vgdrvDarwinCharDevRemove();
                        }
                        else
                            LogRel(("NemuGuest: Failed to initialize character device (rc=%d).\n", rc));

                        VGDrvCommonDeleteDevExt(&g_DevExt);
                    }
                    else
                        LogRel(("NemuGuest: Failed to initialize common code (rc=%d).\n", rc));
                ...
                LogRel(("NemuGuest: The device missing is the I/O port range (#0).\n"));
        }
        else
            LogRel(("NemuGuest: Not the VMMDev (%#x:%#x).\n",
                   m_pIOPCIDevice->configRead16(kIOPCIConfigVendorID), m_pIOPCIDevice->configRead16(kIOPCIConfigDeviceID)));
    }
    else
        LogRel(("NemuGuest: Provider is not an instance of IOPCIDevice.\n"));
...
void org_virtualbox_NemuGuest::stop(IOService *pProvider)
{
    ...
    if (!disableVmmDevInterrupts())
        printf("NemuGuest: unable to unregister interrupt handler\n");
    ...
    printf("NemuGuest: IOService stopped\n");
}
 ...
 */
bool org_virtualbox_NemuGuest::terminate(IOOptionBits fOptions)
{
    /* Do not use Log*() here (in IOService instance) because its instance
     * already terminated in BSD's module unload callback! */
#ifdef LOG_ENABLED
    printf("org_virtualbox_NemuGuest::terminate: reference_count=%d g_cSessions=%d (fOptions=%#x)\n",
...
/*
 *
 * org_virtualbox_NemuGuestClient
 *
 */
 ...
 */
bool org_virtualbox_NemuGuestClient::initWithTask(task_t OwningTask, void *pvSecurityId, UInt32 u32Type)
{
    LogFlow(("org_virtualbox_NemuGuestClient::initWithTask([%p], %#x, %p, %#x) (cur pid=%d proc=%p)\n",
    ...
    if (u32Type != NEMUGUEST_DARWIN_IOSERVICE_COOKIE)
    {
        Log(("org_virtualbox_NemuGuestClient::initWithTask: Bad cookie %#x\n", u32Type));
        return false;
    }
 ...
 */
bool org_virtualbox_NemuGuestClient::start(IOService *pProvider)
{
    LogFlow(("org_virtualbox_NemuGuestClient::start([%p], %p) (cur pid=%d proc=%p)\n",
    ...
    {
        m_pProvider = OSDynamicCast(org_virtualbox_NemuGuest, pProvider);
        if (m_pProvider)
        {
            Assert(!m_pSession);
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                PNEMUGUESTSESSION pCur = g_apSessionHashTab[iHash];
                ...
                if (!pCur)
                {
                    ...
                    m_pSession->pvNemuGuestClient = this;
                ...
                if (RT_SUCCESS(rc))
                {
                    Log(("org_virtualbox_NemuGuestClient::start: created session %p for pid %d\n", m_pSession, (int)RTProcSelf()));
                    return true;
                }

                LogFlow(("org_virtualbox_NemuGuestClient::start: already got a session for this process (%p)\n", pCur));
                VGDrvCommonCloseSession(&g_DevExt, m_pSession);
            }

            m_pSession = NULL;
            LogFlow(("org_virtualbox_NemuGuestClient::start: rc=%Rrc from supdrvCreateSession\n", rc));
        }
        else
            LogFlow(("org_virtualbox_NemuGuestClient::start: %p isn't org_virtualbox_NemuGuest\n", pProvider));
    }
    return false;
}
 ...
 */
/* static */ void org_virtualbox_NemuGuestClient::sessionClose(RTPROCESS Process)
{
    /*
    ...
    const unsigned  iHash = SESSION_HASH(Process);
    RTSpinlockAcquire(g_Spinlock);
    PNEMUGUESTSESSION  pSession = g_apSessionHashTab[iHash];
    if (pSession)
    {
        ...
        {
            PNEMUGUESTSESSION pPrev = pSession;
            pSession = pSession->pNextHash;
            while (pSession)
            {
                if (pSession->Process == Process)
    ...
    if (!pSession)
    {
        Log(("NemuGuestClient::sessionClose: pSession == NULL, pid=%d; freed already?\n", (int)Process));
        return;
    }
     ...
     */
    org_virtualbox_NemuGuestClient *pThis = (org_virtualbox_NemuGuestClient *)pSession->pvNemuGuestClient;
    pSession->pvNemuGuestClient = NULL;
    if (pThis)
    {
 ...
 */
IOReturn org_virtualbox_NemuGuestClient::clientClose(void)
{
    LogFlow(("org_virtualbox_NemuGuestClient::clientClose([%p]) (cur pid=%d proc=%p)\n", this, RTProcSelf(), RTR0ProcHandleSelf()));
