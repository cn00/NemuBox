/** @file
 *
 * Nemu Host Guest Shared Memory Interface (HGSMI).
...
#include <Nemu/HGSMI/HGSMIChSetup.h>
...
# include <Video.h>
RT_C_DECLS_END
#elif defined NEMU_GUESTR3XORGMOD
# include <compiler.h>
#else
...
# include "wddm/NemuMPShgsmi.h"
 typedef NEMUSHGSMI HGSMIGUESTCMDHEAP;
...
DECLINLINE(void) NemuVideoCmnPortWriteUchar(RTIOPORT Port, uint8_t Value)
{
#ifdef NEMU_XPDM_MINIPORT
...
DECLINLINE(void) NemuVideoCmnPortWriteUshort(RTIOPORT Port, uint16_t Value)
{
#ifdef NEMU_XPDM_MINIPORT
...
DECLINLINE(void) NemuVideoCmnPortWriteUlong(RTIOPORT Port, uint32_t Value)
{
#ifdef NEMU_XPDM_MINIPORT
...
DECLINLINE(uint8_t) NemuVideoCmnPortReadUchar(RTIOPORT Port)
{
#ifdef NEMU_XPDM_MINIPORT
...
DECLINLINE(uint16_t) NemuVideoCmnPortReadUshort(RTIOPORT Port)
{
#ifdef NEMU_XPDM_MINIPORT
...
DECLINLINE(uint32_t) NemuVideoCmnPortReadUlong(RTIOPORT Port)
{
#ifdef NEMU_XPDM_MINIPORT
...
/** Acknowlege an IRQ. */
DECLINLINE(void) NemuHGSMIClearIrq(PHGSMIHOSTCOMMANDCONTEXT pCtx)
{
    NemuVideoCmnPortWriteUlong(pCtx->port, HGSMIOFFSET_VOID);
}
...
struct VBVAINFOVIEW;
/**
 * Callback funtion called from @a NemuHGSMISendViewInfo to initialise
 * the @a VBVAINFOVIEW structure for each screen.
 *
 * @returns  iprt status code
 * @param  pvData  context data for the callback, passed to @a
 *                 NemuHGSMISendViewInfo along with the callback
...
RTDECL(bool)     NemuVideoGetModeRegisters(uint16_t *pcWidth,
                                           uint16_t *pcHeight,
