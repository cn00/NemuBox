...
*********************************************************************************************************************************/
#ifdef RT_OS_WINDOWS
# define NEMU_COM_OUTOFPROC_MODULE
#endif
...
NEMU_LISTENER_DECLARE(NATNetworkListenerImpl)
...
int createNatListener(ComNatListenerPtr& listener, const ComVirtualBoxPtr& nemuptr,
                      NATNetworkEventAdapter *adapter, /* const */ ComEventTypeArray& events)
{
    ...
    ComPtr<IEventSource> esNemu;
    hrc = nemuptr->COMGETTER(EventSource)(esNemu.asOutParam());
    AssertComRCReturn(hrc, VERR_INTERNAL_ERROR);

    listener = obj;

    hrc = esNemu->RegisterListener(listener, ComSafeArrayAsInParam(events), true);
    AssertComRCReturn(hrc, VERR_INTERNAL_ERROR);

    return VINF_SUCCESS;
}

int destroyNatListener(ComNatListenerPtr& listener, const ComVirtualBoxPtr& nemuptr)
{
    if (listener)
    {
        ComPtr<IEventSource> esNemu;
        HRESULT hrc = nemuptr->COMGETTER(EventSource)(esNemu.asOutParam());
        AssertComRCReturn(hrc, VERR_INTERNAL_ERROR);
        if (!esNemu.isNull())
        {
            hrc = esNemu->UnregisterListener(listener);
            AssertComRCReturn(hrc, VERR_INTERNAL_ERROR);
        }
        listener.setNull();
    }
...
int createClientListener(ComNatListenerPtr& listener, const ComVirtualBoxClientPtr& nemuclientptr,
                         NATNetworkEventAdapter *adapter, /* const */ ComEventTypeArray& events)
{
    ComObjPtr<NATNetworkListenerImpl> obj;
    HRESULT hrc = obj.createObject();
    ...
    ComPtr<IEventSource> esNemu;
    hrc = nemuclientptr->COMGETTER(EventSource)(esNemu.asOutParam());
    AssertComRCReturn(hrc, VERR_INTERNAL_ERROR);
...
int destroyClientListener(ComNatListenerPtr& listener, const ComVirtualBoxClientPtr& nemuclientptr)
{
    if (listener)
    {
        ComPtr<IEventSource> esNemu;
        HRESULT hrc = nemuclientptr->COMGETTER(EventSource)(esNemu.asOutParam());
        AssertComRCReturn(hrc, VERR_INTERNAL_ERROR);
        if (!esNemu.isNull())
        {
            hrc = esNemu->UnregisterListener(listener);
