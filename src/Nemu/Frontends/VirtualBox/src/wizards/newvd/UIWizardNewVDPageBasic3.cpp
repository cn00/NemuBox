...
/** @file
 * Nemu Qt GUI - UIWizardNewVDPageBasic3 class implementation.
 */
...
UIWizardNewVDPage3::UIWizardNewVDPage3(const QString &strDefaultName, const QString &strDefaultPath)
    ...
    , m_uMediumSizeMax(nemuGlobal().virtualBox().GetSystemProperties().GetInfoVDSize())
...
void UIWizardNewVDPage3::onSelectLocationButtonClicked()
{
    ...
    QString strChosenFilePath = QIFileDialog::getSaveFileName(folder.absoluteFilePath(strFileName),
                                                              ...
                                                              NemuGlobal::tr("Please choose a location for new virtual hard disk file"));
...
void UIWizardNewVDPage3::onSizeSliderValueChanged(int iValue)
{
    ...
    m_pSizeEditor->setText(nemuGlobal().formatSize(uMediumSize));
...
{
    /* Get full size: */
    qulonglong uMediumSize = nemuGlobal().parseSize(strValue);
    /* Update tooltips: */
    updateSizeToolTips(uMediumSize);
...
{
    QString strToolTip = UIWizardNewVD::tr("<nobr>%1 (%2 B)</nobr>").arg(nemuGlobal().formatSize(uSize)).arg(uSize);
    ...
    QVBoxLayout *pMainLayout = new QVBoxLayout(this);
    {
        ...
        QGridLayout *m_pSizeLayout = new QGridLayout;
        {
            ...
            m_pSizeEditor = new QILineEdit(this);
            {
                ...
                m_pSizeEditor->setValidator(new QRegExpValidator(QRegExp(nemuGlobal().sizeRegexp()), this));
            }
            QLabel *m_pSizeMin = new QLabel(this);
            {
                m_pSizeMin->setSizePolicy(QSizePolicy::Maximum, QSizePolicy::Fixed);
                m_pSizeMin->setText(nemuGlobal().formatSize(m_uMediumSizeMin));
            }
            QLabel *m_pSizeMax = new QLabel(this);
            {
                m_pSizeMax->setSizePolicy(QSizePolicy::Maximum, QSizePolicy::Fixed);
                m_pSizeMax->setText(nemuGlobal().formatSize(m_uMediumSizeMax));
            }
