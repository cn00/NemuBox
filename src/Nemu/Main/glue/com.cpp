...
int GetNemuUserHomeDirectory(char *aDir, size_t aDirLen, bool fCreateDir)
{
    ...
    *aDir = 0;
    ...
    int vrc = RTEnvGetEx(RTENV_DEFAULT, "NEMU_USER_HOME", szTmp, sizeof(szTmp), NULL);
...
static DECLCALLBACK(void) nemuHeaderFooter(PRTLOGGER pReleaseLogger, RTLOGPHASE enmPhase, PFNRTLOGPHASEMSG pfnLog)
{
...
#endif
                   "Log opened %s\n",
                   g_pszLogEntity, NEMU_VERSION_STRING, RTBldCfgRevision(),
...
int NemuLogRelCreate(const char *pcszEntity, const char *pcszLogFile,
                     ...
                     char *pszError, size_t cbError)
{
    ...
    int vrc = RTLogCreateEx(&pReleaseLogger, fFlags, pcszGroupSettings,
                            ...
                            nemuHeaderFooter, cHistory, uHistoryFileSize, uHistoryFileTime,
                            pszError, cbError,
