...
#ifndef ___Nemu_dbg_h
#define ___Nemu_dbg_h
...
typedef struct DBGCCMDHLP
{
    ...
    /**
     * Command helper for formatting and error message for a Nemu status code.
     *
     * @returns Nemu status code appropriate to return from a command.
     * @param   pCmdHlp     Pointer to the command callback structure.
     ...
     */
    DECLCALLBACKMEMBER(int, pfnNemuError)(PDBGCCMDHLP pCmdHlp, int rc, const char *pszFormat, ...) RT_IPRT_FORMAT_ATTR(3, 4);
     ...
     */
    DECLCALLBACKMEMBER(int, pfnNemuErrorV)(PDBGCCMDHLP pCmdHlp, int rc,
                                           const char *pszFormat, va_list args) RT_IPRT_FORMAT_ATTR(3, 0);
     ...
     *
     * @returns Nemu status code to pass upwards.
     *
     * @param   pCmdHlp     Pointer to the command callback structure.
...
/**
 * @copydoc DBGCCMDHLP::pfnNemuError
 */
DECLINLINE(int) RT_IPRT_FORMAT_ATTR(3, 4) DBGCCmdHlpNemuError(PDBGCCMDHLP pCmdHlp, int rc, const char *pszFormat, ...)
{
    va_list va;

    va_start(va, pszFormat);
    rc = pCmdHlp->pfnNemuErrorV(pCmdHlp, rc, pszFormat, va);
    va_end(va);
