...
/** @file
 * Nemu OpenGL GLSL related get functions
 */
...
void SERVER_DISPATCH_APIENTRY crServerDispatchGetAttachedObjectsARB(NemuGLhandleARB containerObj, GLsizei maxCount, GLsizei * count, NemuGLhandleARB * obj)
{
    GLsizei *pLocal;

    pLocal = (GLsizei*) crAlloc(maxCount*sizeof(NemuGLhandleARB)+sizeof(GLsizei));
    if (!pLocal)
    {
        GLsizei zero=0;
        crServerReturnValue(&zero, sizeof(zero));
    ...
    /* initial (fallback )value */
    *pLocal = 0;
    cr_server.head_spu->dispatch_table.GetAttachedObjectsARB(crStateGetProgramHWID(containerObj), maxCount, pLocal, (NemuGLhandleARB*)&pLocal[1]);
    ...
    crServerReturnValue(pLocal, (*pLocal)*sizeof(NemuGLhandleARB)+sizeof(GLsizei));
    crFree(pLocal);
}

AssertCompile(sizeof(GLsizei) == 4);

void SERVER_DISPATCH_APIENTRY crServerDispatchGetInfoLogARB(NemuGLhandleARB obj, GLsizei maxLength, GLsizei * length, GLcharARB * infoLog)
{
    GLsizei *pLocal;
    GLuint hwid;
...
void SERVER_DISPATCH_APIENTRY crServerDispatchGetObjectParameterfvARB( NemuGLhandleARB obj, GLenum pname, GLfloat * params )
{
...
void SERVER_DISPATCH_APIENTRY crServerDispatchGetObjectParameterivARB( NemuGLhandleARB obj, GLenum pname, GLint * params )
{
