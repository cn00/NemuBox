...
int CrFbDisplayWindowRootVr::EntryCreated(struct CR_FRAMEBUFFER *pFb, HCR_FRAMEBUFFER_ENTRY hEntry)
{
    ...
    const NEMUVR_SCR_COMPOSITOR_ENTRY* pSrcEntry = CrFbEntryGetCompositorEntry(hEntry);
    NEMUVR_SCR_COMPOSITOR_ENTRY *pMyEntry = entryAlloc();
...
int CrFbDisplayWindowRootVr::EntryAdded(struct CR_FRAMEBUFFER *pFb, HCR_FRAMEBUFFER_ENTRY hEntry)
{
    ...
    const NEMUVR_SCR_COMPOSITOR_ENTRY* pSrcEntry = CrFbEntryGetCompositorEntry(hEntry);
    NEMUVR_SCR_COMPOSITOR_ENTRY *pMyEntry = (NEMUVR_SCR_COMPOSITOR_ENTRY*)CrFbDDataEntryGet(hEntry, slotGet());
    Assert(pMyEntry);
...
int CrFbDisplayWindowRootVr::EntryReplaced(struct CR_FRAMEBUFFER *pFb, HCR_FRAMEBUFFER_ENTRY hNewEntry, HCR_FRAMEBUFFER_ENTRY hReplacedEntry)
{
    ...
    const NEMUVR_SCR_COMPOSITOR_ENTRY* pSrcNewEntry = CrFbEntryGetCompositorEntry(hNewEntry);
    NEMUVR_SCR_COMPOSITOR_ENTRY *pMyEntry = (NEMUVR_SCR_COMPOSITOR_ENTRY*)CrFbDDataEntryGet(hNewEntry, slotGet());
...
NEMUVR_SCR_COMPOSITOR_ENTRY* CrFbDisplayWindowRootVr::entryAlloc()
{
#ifndef NEMUVDBG_MEMCACHE_DISABLE
    return (NEMUVR_SCR_COMPOSITOR_ENTRY*)RTMemCacheAlloc(g_CrPresenter.CEntryLookasideList);
#else
    return (NEMUVR_SCR_COMPOSITOR_ENTRY*)RTMemAlloc(sizeof (NEMUVR_SCR_COMPOSITOR_ENTRY));
#endif
}


void CrFbDisplayWindowRootVr::entryFree(NEMUVR_SCR_COMPOSITOR_ENTRY* pEntry)
{
...
void CrFbDisplayWindowRootVr::rootVrTranslateForPos()
{
    ...
    NemuVrListTranslate(&cr_server.RootVr, dx, dy);
}


DECLCALLBACK(NEMUVR_SCR_COMPOSITOR_ENTRY*) CrFbDisplayWindowRootVr::rootVrGetCEntry(const NEMUVR_SCR_COMPOSITOR_ENTRY*pEntry, void *pvContext)
{
    ...
    NEMUVR_SCR_COMPOSITOR_ENTRY *pMyEntry = (NEMUVR_SCR_COMPOSITOR_ENTRY*)CrFbDDataEntryGet(hEntry, pThis->slotGet());
