...
//#ifdef DEBUG
//# define NEMU_WINE_DEBUG
//#endif

#ifdef NEMU_WINE_DEBUG
# ifndef NEMU_WINE_DEBUG_DEFINES
extern DWORD g_NemuVDbgBreakOnD3DErr;
# else
#  ifdef DEBUG_misha
...
#  endif
DWORD g_NemuVDbgBreakOnD3DErr = _ERR_BREAK_DEFAULT;
# endif

# define _ERR_BREAK() Assert(0)
# define _ERR_ASSERT(_e) Assert((_e))
# define _ERR_CHECK_BREAK(_t) do { \
        if (g_NemuVDbgBreakOn##_t) { _ERR_BREAK(); } \
    } while (0)
# define _ERR_CHECK_ASSERT(_t, _e) do { \
        if (g_NemuVDbgBreakOn##_t) { _ERR_ASSERT(_e); } \
    } while (0)
