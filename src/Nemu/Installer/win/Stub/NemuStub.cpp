/* $Id: NemuStub.cpp $ */
/** @file
 * NemuStub - VirtualBox's Windows installer stub.
 */
...
#include "NemuStub.h"
#include "../StubBld/NemuStubBld.h"
#include "resource.h"

#ifdef NEMU_WITH_CODE_SIGNING
# include "NemuStubCertUtil.h"
# include "NemuStubPublicCert.h"
#endif

#ifndef TARGET_NT4
/* Use an own console window if run in verbose mode. */
# define NEMU_STUB_WITH_OWN_CONSOLE
#endif
...
static RTEXITCODE ShowError(const char *pszFmt, ...)
{
    ...
    if (RTStrAPrintfV(&pszMsg, pszFmt, va))
    {
        ...
        else
        {
            ...
            if (RT_SUCCESS(rc))
            {
                MessageBoxW(GetDesktopWindow(), pwszMsg, MY_UNICODE(NEMU_STUB_TITLE), MB_ICONERROR);
                RTUtf16Free(pwszMsg);
            }
            else
                MessageBoxA(GetDesktopWindow(), pszMsg, NEMU_STUB_TITLE, MB_ICONERROR);
        }
        RTStrFree(pszMsg);
    }
...
static void ShowInfo(const char *pszFmt, ...)
{
    ...
    if (rc >= 0)
    {
        ...
        else
        {
            ...
            if (RT_SUCCESS(rc))
            {
                MessageBoxW(GetDesktopWindow(), pwszMsg, MY_UNICODE(NEMU_STUB_TITLE), MB_ICONINFORMATION);
                RTUtf16Free(pwszMsg);
            }
            else
                MessageBoxA(GetDesktopWindow(), pszMsg, NEMU_STUB_TITLE, MB_ICONINFORMATION);
        }
    }
    else /* Should never happen! */
        AssertMsgFailed(("Failed to format error text of format string: %s!\n", pszFmt));
 ...
 * @returns iprt status code.
 *
 * @param   pPackage            Pointer to a NEMUSTUBPKG struct that contains the resource.
 * @param   pszTempFile         The full file path + name to extract the resource to.
 *
 */
static int Extract(const PNEMUSTUBPKG  pPackage,
                   const char         *pszTempFile)
{
 ...
 *
 */
static bool PackageIsNeeded(PNEMUSTUBPKG pPackage)
{
    if (pPackage->byArch == NEMUSTUBPKGARCH_ALL)
        return true;
    NEMUSTUBPKGARCH enmArch = IsWow64() ? NEMUSTUBPKGARCH_AMD64 : NEMUSTUBPKGARCH_X86;
...
static RTEXITCODE ProcessMsiPackage(const char *pszMsi, const char *pszMsiArgs, bool fLogging)
{
    ...
    if (fLogging)
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            rc = RTPathAppend(szLogFile, sizeof(szLogFile), "NemuInstallLog.txt");
        }
        if (RT_FAILURE(rc))
...
static RTEXITCODE ProcessPackage(unsigned iPackage, const char *pszPkgDir, const char *pszMsiArgs, bool fLogging)
{
     ...
     */
    PNEMUSTUBPKG pPackage = FindPackageHeader(iPackage);
    if (pPackage == NULL)
        return RTEXITCODE_FAILURE;
...
static RTEXITCODE InstallCertificate(void)
{
    if (addCertToStore(CERT_SYSTEM_STORE_LOCAL_MACHINE,
                       "TrustedPublisher",
                       g_ab_NemuStubPublicCert,
                       sizeof(g_ab_NemuStubPublicCert)))
        return RTEXITCODE_SUCCESS;
    return ShowError("Failed to construct install certificate.");
}
#endif /* NEMU_WITH_CODE_SIGNING */
...
static RTEXITCODE ExtractFiles(unsigned cPackages, const char *pszDstDir, bool fExtractOnly, bool *pfCreatedExtractDir)
{
    ...
    *pfCreatedExtractDir = false;
    ...
    {
        PNEMUSTUBPKG pPackage = FindPackageHeader(k);
        if (!pPackage)
                   ...
                   int        nCmdShow)
{
     ...
     */
    HANDLE hMutexAppRunning = CreateMutex(NULL, FALSE, "NemuStubInstaller");
        ...
        switch (ch)
        {
            ...
            case 'V':
                ShowInfo("Version: %d.%d.%d.%d",
                         NEMU_VERSION_MAJOR, NEMU_VERSION_MINOR, NEMU_VERSION_BUILD,
                         NEMU_SVN_REV);
                fExitEarly = true;
                break;
            ...
            case 'h':
                ShowInfo("-- %s v%d.%d.%d.%d --\n"
                         ...
                         "Examples:\n"
                         "%s --msiparams INSTALLDIR=C:\\Nemu\n"
                         "%s --extract -path C:\\Nemu",
                         NEMU_STUB_TITLE, NEMU_VERSION_MAJOR, NEMU_VERSION_MINOR, NEMU_VERSION_BUILD, NEMU_SVN_REV,
                         argv[0], argv[0]);
                fExitEarly = true;
                break;
...
#if defined(_WIN32_WINNT) && _WIN32_WINNT >= 0x0501
# ifdef NEMU_STUB_WITH_OWN_CONSOLE /* Use an own console window if run in debug mode. */
    if (   RT_SUCCESS(vrc)
        && g_iVerbosity)
    {
        if (!AllocConsole())
...
# endif /* NEMU_STUB_WITH_OWN_CONSOLE */
#endif
...
#if defined(_WIN32_WINNT) && _WIN32_WINNT >= 0x0501
# ifdef NEMU_STUB_WITH_OWN_CONSOLE
    if (g_iVerbosity)
        FreeConsole();
# endif /* NEMU_STUB_WITH_OWN_CONSOLE */
#endif
