/* $Id: NemuServiceVMInfo-win.cpp $ */
/** @file
 * NemuService - Virtual Machine Information for the Host, Windows specifics.
 */
...
#include "NemuServiceVMInfo.h"
#include "../../WINNT/NemuTray/NemuTrayMsg.h" /* For IPC. */
...
typedef struct NEMUSERVICEVMINFOUSER
{
     ...
     *  ones. */
    ULONG ulLastSession;
} NEMUSERVICEVMINFOUSER, *PNEMUSERVICEVMINFOUSER;

/** Structure for the file information lookup. */
typedef struct NEMUSERVICEVMINFOFILE
{
    char *pszFilePath;
    char *pszFileName;
} NEMUSERVICEVMINFOFILE, *PNEMUSERVICEVMINFOFILE;

/** Structure for process information lookup. */
typedef struct NEMUSERVICEVMINFOPROC
{
    /** The PID. */
    DWORD id;
    /** The SID. */
    ...
    /** Interactive process. */
    bool fInteractive;
} NEMUSERVICEVMINFOPROC, *PNEMUSERVICEVMINFOPROC;


/*********************************************************************************************************************************
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static uint32_t vgsvcVMInfoWinSessionHasProcesses(PLUID pSession, PNEMUSERVICEVMINFOPROC const paProcs, DWORD cProcs);
static bool vgsvcVMInfoWinIsLoggedIn(PNEMUSERVICEVMINFOUSER a_pUserInfo, PLUID a_pSession);
static int  vgsvcVMInfoWinProcessesEnumerate(PNEMUSERVICEVMINFOPROC *ppProc, DWORD *pdwCount);
static void vgsvcVMInfoWinProcessesFree(DWORD cProcs, PNEMUSERVICEVMINFOPROC paProcs);
static int  vgsvcVMInfoWinWriteLastInput(PNEMUSERVICEVEPROPCACHE pCache, const char *pszUser, const char *pszDomain);
 ...
 */
static int vgsvcVMInfoWinProcessesGetModuleNameA(PNEMUSERVICEVMINFOPROC const pProc, PRTUTF16 *ppszName)
{
 ...
 */
static int vgsvcVMInfoWinProcessesGetTokenInfo(PNEMUSERVICEVMINFOPROC pProc, TOKEN_INFORMATION_CLASS tkClass)
{
    AssertPtrReturn(pProc, VERR_INVALID_POINTER);
 ...
 */
static int vgsvcVMInfoWinProcessesEnumerate(PNEMUSERVICEVMINFOPROC *ppaProcs, PDWORD pcProcs)
{
    AssertPtr(ppaProcs);
    AssertPtr(pcProcs);
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        PNEMUSERVICEVMINFOPROC paProcs;
        paProcs = (PNEMUSERVICEVMINFOPROC)RTMemAllocZ(cProcesses * sizeof(NEMUSERVICEVMINFOPROC));
        if (paProcs)
        {
 ...
 */
static bool vgsvcVMInfoWinIsLoggedIn(PNEMUSERVICEVMINFOUSER pUserInfo, PLUID pSession)
{
...
/** @todo r=bird:  Pointless if.  */
    if (RTStrPrintf(szPipeName, sizeof(szPipeName), "%s%s", NEMUTRAY_IPC_PIPE_PREFIX, pszUser))
    {
        bool fReportToHost = false;
        NemuGuestUserState userState = NemuGuestUserState_Unknown;
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            {
                /* .uMagic      = */ NEMUTRAY_IPC_HDR_MAGIC,
                /* .uHdrVersion = */ 0,
                /* .uMsgType    = */ NEMUTRAYIPCMSGTYPE_USERLASTINPUT,
                /* .cbMsgData   = */ 0 /* No msg */
            };
            ...
            if (   RT_SUCCESS(rc)
                /* If uLastInput is set to UINT32_MAX NemuTray was not able to retrieve the
                ...
                && ipcRes.uLastInput != UINT32_MAX)
            {
                userState = (ipcRes.uLastInput * 1000) < g_uVMInfoUserIdleThresholdMS
                          ? NemuGuestUserState_InUse
                          : NemuGuestUserState_Idle;

                rc = VGSvcUserUpdateF(pCache, pszUser, pszDomain, "UsageState",
                                      userState == NemuGuestUserState_InUse ? "InUse" : "Idle");

                /*
                 * Note: nemuServiceUserUpdateF can return VINF_NO_CHANGE in case there wasn't anything
                ...
                VGSvcVerbose(4, "User '%s' (domain '%s') is idle for %RU32, fReportToHost=%RTbool\n",
                    ...
                    if (userState == NemuGuestUserState_Idle)
        ...
        else
        {
            switch (rc)
            {
                ...
                {
                    /* No NemuTray (or too old version which does not support IPC) running
                       for the given user. Not much we can do then. */
                    VGSvcVerbose(4, "NemuTray for user '%s' not running (anymore), no last input available\n", pszUser);
                    ...
                    fReportToHost = rc == VINF_SUCCESS;
                    if (fReportToHost)
                        userState = NemuGuestUserState_Idle;
                    break;
                }
        ...
        if (fReportToHost)
        {
            Assert(userState != NemuGuestUserState_Unknown);
 ...
 */
int VGSvcVMInfoWinWriteUsers(PNEMUSERVICEVEPROPCACHE pCache, char **ppszUserList, uint32_t *pcUsersInList)
{
    AssertPtrReturn(pCache, VERR_INVALID_POINTER);
    ...
    {
        PNEMUSERVICEVMINFOUSER pUserInfo;
        pUserInfo = (PNEMUSERVICEVMINFOUSER)RTMemAllocZ(cSessions * sizeof(NEMUSERVICEVMINFOUSER) + 1);
        if (!pUserInfo)
        ...
        else
        {
            ...
            for (ULONG i = 0; i < cSessions; i++)
            {
                ...
                NEMUSERVICEVMINFOUSER userSession;
                if (vgsvcVMInfoWinIsLoggedIn(&userSession, &paSessions[i]))
                {
                    ...
                    {
                        PNEMUSERVICEVMINFOUSER pCurUser = &pUserInfo[a];
                        AssertPtr(pCurUser);
                    ...
                    if (!fFoundUser)
                    {
                        ...
                        memcpy(&pUserInfo[cUniqueUsers], &userSession, sizeof(NEMUSERVICEVMINFOUSER));
...
int VGSvcVMInfoWinGetComponentVersions(uint32_t uClientID)
{
    ...
    const NEMUSERVICEVMINFOFILE aNemuFiles[] =
    {
        { szSysDir, "NemuControl.exe" },
        { szSysDir, "NemuHook.dll" },
        { szSysDir, "NemuDisp.dll" },
        { szSysDir, "NemuTray.exe" },
#ifdef TARGET_NT4
        { szSysDir, "NemuServiceNT.exe" },
#else
        { szSysDir, "NemuService.exe" },
        { szSysDir, "NemuMRXNP.dll" },
        { szSysDir, "NemuGINA.dll" },
        { szSysDir, "NemuCredProv.dll" },
#endif

 /* On 64-bit we don't yet have the OpenGL DLLs in native format.
    So just enumerate the 32-bit files in the SYSWOW directory. */
#ifdef RT_ARCH_AMD64
        { szSysWowDir, "NemuOGLarrayspu.dll" },
        { szSysWowDir, "NemuOGLcrutil.dll" },
        { szSysWowDir, "NemuOGLerrorspu.dll" },
        { szSysWowDir, "NemuOGLpackspu.dll" },
        { szSysWowDir, "NemuOGLpassthroughspu.dll" },
        { szSysWowDir, "NemuOGLfeedbackspu.dll" },
        { szSysWowDir, "NemuOGL.dll" },
#else  /* !RT_ARCH_AMD64 */
# ifdef TARGET_NT4
        { szSysDir, "NemuOGLarrayspu.dll" },
        { szSysDir, "NemuOGLcrutil.dll" },
        { szSysDir, "NemuOGLerrorspu.dll" },
        { szSysDir, "NemuOGLpackspu.dll" },
        { szSysDir, "NemuOGLpassthroughspu.dll" },
        { szSysDir, "NemuOGLfeedbackspu.dll" },
        { szSysDir, "NemuOGL.dll" },
# endif
#endif /* !RT_ARCH_AMD64 */

#ifdef TARGET_NT4
        { szDriversDir, "NemuGuestNT.sys" },
        { szDriversDir, "NemuMouseNT.sys" },
#else
        { szDriversDir, "NemuGuest.sys" },
        { szDriversDir, "NemuMouse.sys" },
        { szDriversDir, "NemuSF.sys"    },
#endif
        { szDriversDir, "NemuVideo.sys" },
    };

    for (unsigned i = 0; i < RT_ELEMENTS(aNemuFiles); i++)
    {
        char szVer[128];
        VGSvcUtilWinGetFileVersionString(aNemuFiles[i].pszFilePath, aNemuFiles[i].pszFileName, szVer, sizeof(szVer));
        char szPropPath[256];
        RTStrPrintf(szPropPath, sizeof(szPropPath), "/VirtualBox/GuestAdd/Components/%s", aNemuFiles[i].pszFileName);
