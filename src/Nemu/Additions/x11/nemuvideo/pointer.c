 ...
 * @a NemuHGSMIUpdatePointerShape(). */
struct nemuCursorImage
{
    uint32_t fFlags;
    uint32_t cHotX;
    uint32_t cHotY;
...
#ifdef DEBUG_POINTER
static void
nemu_show_shape(unsigned short w, unsigned short h, CARD32 bg, unsigned char *image)
{
    size_t x, y;
    ...
    image    += sizeof(struct nemuCursorImage);
    mask      = image;
    pitch     = (w + 7) / 8;
...
void vbvxCursorTerm(NEMUPtr pNemu)
{
    TRACE_ENTRY();

    xf86DestroyCursorInfoRec(pNemu->pCurs);
    pNemu->pCurs = NULL;
    TRACE_EXIT();
}

static void
nemu_vmm_hide_cursor(ScrnInfoPtr pScrn, NEMUPtr pNemu)
{
    int rc;

    rc = NemuHGSMIUpdatePointerShape(&pNemu->guestCtx, 0, 0, 0, 0, 0, NULL, 0);
    VBVXASSERT(rc == VINF_SUCCESS, ("Could not hide the virtual mouse pointer, Nemu error %d.\n", rc));
}

static void
nemu_vmm_show_cursor(ScrnInfoPtr pScrn, NEMUPtr pNemu)
{
    int rc;

    if (!pNemu->fUseHardwareCursor)
        return;
    rc = NemuHGSMIUpdatePointerShape(&pNemu->guestCtx, NEMU_MOUSE_POINTER_VISIBLE,
                                     0, 0, 0, 0, NULL, 0);
    VBVXASSERT(rc == VINF_SUCCESS, ("Could not unhide the virtual mouse pointer.\n"));
}

static void
nemu_vmm_load_cursor_image(ScrnInfoPtr pScrn, NEMUPtr pNemu,
                           unsigned char *pvImage)
{
    int rc;
    struct nemuCursorImage *pImage;
    pImage = (struct nemuCursorImage *)pvImage;

#ifdef DEBUG_POINTER
    nemu_show_shape(pImage->cWidth, pImage->cHeight, 0, pvImage);
#endif

    rc = NemuHGSMIUpdatePointerShape(&pNemu->guestCtx, pImage->fFlags,
...
static void
nemu_set_cursor_colors(ScrnInfoPtr pScrn, int bg, int fg)
{
    NOREF(pScrn);
    NOREF(bg);
    NOREF(fg);
    /* ErrorF("nemu_set_cursor_colors NOT IMPLEMENTED\n"); */
}


static void
nemu_set_cursor_position(ScrnInfoPtr pScrn, int x, int y)
{
    NEMUPtr pNemu = pScrn->driverPrivate;

    /* This currently does nothing. */
    NemuHGSMICursorPosition(&pNemu->guestCtx, true, x, y, NULL, NULL);
}

static void
nemu_hide_cursor(ScrnInfoPtr pScrn)
{
    NEMUPtr pNemu = pScrn->driverPrivate;

    nemu_vmm_hide_cursor(pScrn, pNemu);
}

static void
nemu_show_cursor(ScrnInfoPtr pScrn)
{
    NEMUPtr pNemu = pScrn->driverPrivate;

    nemu_vmm_show_cursor(pScrn, pNemu);
}

static void
nemu_load_cursor_image(ScrnInfoPtr pScrn, unsigned char *image)
{
    NEMUPtr pNemu = pScrn->driverPrivate;

    nemu_vmm_load_cursor_image(pScrn, pNemu, image);
}

static Bool
nemu_use_hw_cursor(ScreenPtr pScreen, CursorPtr pCurs)
{
    ScrnInfoPtr pScrn = xf86Screens[pScreen->myNum];
    NEMUPtr pNemu = pScrn->driverPrivate;
    return pNemu->fUseHardwareCursor;
}
...
static unsigned char *
nemu_realize_cursor(xf86CursorInfoPtr infoPtr, CursorPtr pCurs)
{
    NEMUPtr pNemu;
    CursorBitsPtr bitsp;
    ...
    pNemu = infoPtr->pScrn->driverPrivate;
    bitsp = pCurs->bits;
    w = bitsp->width;
    h = bitsp->height;

    if (!w || !h || w > NEMU_MAX_CURSOR_WIDTH || h > NEMU_MAX_CURSOR_HEIGHT)
    ...
    pImage = (struct nemuCursorImage *)p;
    ...
    /*
     * Convert the Xorg source/mask bits to the and/xor bits Nemu needs.
     * Xorg:
    ...
    pImage->fFlags   = NEMU_MOUSE_POINTER_VISIBLE | NEMU_MOUSE_POINTER_SHAPE;
    pImage->cbLength = sizeRequest - sizeof(*pImage);

#ifdef DEBUG_POINTER
    ErrorF("shape = %p\n", p);
    nemu_show_shape(w, h, bc, c);
#endif
...
#ifdef ARGB_CURSOR
static Bool
nemu_use_hw_cursor_argb(ScreenPtr pScreen, CursorPtr pCurs)
{
    ScrnInfoPtr pScrn = xf86Screens[pScreen->myNum];
    NEMUPtr pNemu = pScrn->driverPrivate;
    ...
    if (!pNemu->fUseHardwareCursor)
        return FALSE;
    if (   (pCurs->bits->height > NEMU_MAX_CURSOR_HEIGHT)
        || (pCurs->bits->width > NEMU_MAX_CURSOR_WIDTH)
...
static void
nemu_load_cursor_argb(ScrnInfoPtr pScrn, CursorPtr pCurs)
{
    NEMUPtr pNemu;
    CursorBitsPtr bitsp;
    unsigned short w, h;
    ...
    CARD8 *p;
    int scrnIndex;
    uint32_t fFlags =   NEMU_MOUSE_POINTER_VISIBLE | NEMU_MOUSE_POINTER_SHAPE
    ...
    /* Mask must be generated for alpha cursors, that is required by Nemu. */
    ...
    NemuHGSMIUpdatePointerShape(&pNemu->guestCtx, fFlags, bitsp->xhot,
...
Bool vbvxCursorInit(ScreenPtr pScreen)
{
    ...
    TRACE_ENTRY();
    pNemu->pCurs = pCurs = xf86CreateCursorInfoRec();
    if (!pCurs) {
    ...
    if (rc) {
        pCurs->MaxWidth = NEMU_MAX_CURSOR_WIDTH;
        pCurs->MaxHeight = NEMU_MAX_CURSOR_HEIGHT;
        pCurs->Flags =   HARDWARE_CURSOR_TRUECOLOR_AT_8BPP
                       | HARDWARE_CURSOR_SOURCE_MASK_INTERLEAVE_1
                       | HARDWARE_CURSOR_BIT_ORDER_MSBFIRST
                       | HARDWARE_CURSOR_UPDATE_UNHIDDEN;

        pCurs->SetCursorColors   = nemu_set_cursor_colors;
        pCurs->SetCursorPosition = nemu_set_cursor_position;
        pCurs->LoadCursorImage   = nemu_load_cursor_image;
        pCurs->HideCursor        = nemu_hide_cursor;
        pCurs->ShowCursor        = nemu_show_cursor;
        pCurs->UseHWCursor       = nemu_use_hw_cursor;
        pCurs->RealizeCursor     = nemu_realize_cursor;

#ifdef ARGB_CURSOR
        pCurs->UseHWCursorARGB   = nemu_use_hw_cursor_argb;
        pCurs->LoadCursorARGB    = nemu_load_cursor_argb;
#endif
