...
/** @file
 * Nemu Qt GUI - UIMachineSettingsDisplay class implementation.
 */
...
void UIMachineSettingsDisplay::setGuestOSType(CGuestOSType guestOSType)
{
    ...
    m_fWddmModeSupported = NemuGlobal::isWddmCompatibleOsType(strguestOSTypeId);
#endif /* NEMU_WITH_CRHGSMI */
...
bool UIMachineSettingsDisplay::validate(QList<UIValidationMessage> &messages)
{
    ...
    /* Screen tab: */
    {
        ...
        message.first = NemuGlobal::removeAccelMark(m_pTabWidget->tabText(0));

        /* 3D acceleration test: */
        if (m_pCheckbox3D->isChecked() && !nemuGlobal().is3DAvailable())
        {
        ...
        {
            quint64 uNeedBytes = NemuGlobal::requiredVideoMemory(m_guestOSType.GetId(), m_pEditorVideoScreenCount->value());
            ...
            if ((quint64)m_pEditorVideoMemorySize->value() * _1M < uNeedBytes)
            {
                message.second << tr("The virtual machine is currently assigned less than <b>%1</b> of video memory "
                                     ...
                                     .arg(nemuGlobal().formatSize(uNeedBytes, 0, FormatSize_RoundUp));
            }
#ifdef NEMU_WITH_VIDEOHWACCEL
            /* 2D acceleration video RAM amount test: */
            else if (m_pCheckbox2DVideo->isChecked() && m_f2DVideoAccelerationSupported)
            {
                uNeedBytes += NemuGlobal::required2DOffscreenVideoMemory();
                if ((quint64)m_pEditorVideoMemorySize->value() * _1M < uNeedBytes)
                {
                    message.second << tr("The virtual machine is currently assigned less than <b>%1</b> of video memory "
...
# ifdef NEMU_WITH_CRHGSMI
            ...
            else if (m_pCheckbox3D->isChecked() && m_fWddmModeSupported)
            {
                ...
                uNeedBytes += NemuGlobal::required3DWddmOffscreenVideoMemory(m_guestOSType.GetId(), cGuestScreenCount);
...
# endif /* NEMU_WITH_CRHGSMI */
    ...
    /* Remote Display tab: */
    {
        /* Prepare message: */
        UIValidationMessage message;
        message.first = NemuGlobal::removeAccelMark(m_pTabWidget->tabText(1));

#ifdef NEMU_WITH_EXTPACK
        /* VRDE Extension Pack presence test: */
        CExtPack extPack = nemuGlobal().virtualBox().GetExtensionPackManager().Find(GUI_ExtPackName);
...
void UIMachineSettingsDisplay::retranslateUi()
{
    ...
    /* Screen stuff: */
    CSystemProperties sys = nemuGlobal().virtualBox().GetSystemProperties();
...
#ifdef NEMU_WITH_VIDEOHWACCEL
    m_pCheckbox2DVideo->setEnabled(isMachineOffline() && NemuGlobal::isAcceleration2DVideoAvailable());
#else /* !NEMU_WITH_VIDEOHWACCEL */
...
void UIMachineSettingsDisplay::prepareScreenTab()
{
    ...
    const uint cHostScreens = nemuGlobal().screenCount();
    ...
    nemuGlobal().setMinimumWidthAccordingSymbolCount(m_pEditorVideoMemorySize, 4);
    ...
    nemuGlobal().setMinimumWidthAccordingSymbolCount(m_pEditorVideoScreenCount, 3);
    ...
    nemuGlobal().setMinimumWidthAccordingSymbolCount(m_pEditorGuestScreenScale, 5);
...
void UIMachineSettingsDisplay::prepareVideoCaptureTab()
{
    ...
    m_pEditorVideoCapturePath->setMode(NemuFilePathSelectorWidget::Mode_File_Save);
    ...
    nemuGlobal().setMinimumWidthAccordingSymbolCount(m_pEditorVideoCaptureWidth, 5);
    nemuGlobal().setMinimumWidthAccordingSymbolCount(m_pEditorVideoCaptureHeight, 5);
    ...
    nemuGlobal().setMinimumWidthAccordingSymbolCount(m_pEditorVideoCaptureFrameRate, 3);
    ...
    nemuGlobal().setMinimumWidthAccordingSymbolCount(m_pEditorVideoCaptureBitRate, 5);
...
void UIMachineSettingsDisplay::checkVRAMRequirements()
{
    ...
    int cGuestScreenCount = m_pEditorVideoScreenCount->value();
    quint64 uNeedMBytes = NemuGlobal::requiredVideoMemory(m_guestOSType.GetId(), cGuestScreenCount) / _1M;
    ...
    if (m_pCheckbox2DVideo->isChecked() && m_f2DVideoAccelerationSupported)
    {
        uNeedMBytes += NemuGlobal::required2DOffscreenVideoMemory() / _1M;
    }
#endif /* NEMU_WITH_VIDEOHWACCEL */
    ...
    {
# if 0
        uNeedMBytes += NemuGlobal::required3DWddmOffscreenVideoMemory(m_guestOSType.GetId(), cGuestScreenCount) / _1M;
