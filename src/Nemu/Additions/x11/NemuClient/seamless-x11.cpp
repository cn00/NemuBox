...
int SeamlessX11::init(PFNSENDREGIONUPDATE pHostCallback)
{
    ...
    {
        LogRel(("NemuClient: ERROR: attempt to initialise seamless guest object twice!\n"));
        return VERR_INTERNAL_ERROR;
    }
    if (!(mDisplay = XOpenDisplay(NULL)))
    {
        LogRel(("NemuClient: seamless guest object failed to acquire a connection to the display.\n"));
...
void SeamlessX11::addClientWindow(const Window hWin)
{
    ...
    {
        LogRelFunc(("NemuClient: Failed to get the window attributes for window %d\n", hWin));
        fAddWin = false;
    }
...
DECLCALLBACK(int) NemuGuestWinFree(NemuGuestWinInfo *pInfo, void *pvParam)
{
...
void SeamlessX11::freeWindowTree(void)
{
    ...
    LogRelFlowFunc(("\n"));
    mGuestWindows.detachAll(NemuGuestWinFree, mDisplay);
    LogRelFlowFunc(("returning\n"));
}
...
void SeamlessX11::nextConfigurationEvent(void)
{
    ...
    case PropertyNotify:
        ...
        rebuildWindowTree();
        break;
    case NemuShapeNotify:  /* This is defined wrong in my X11 header files! */
...
{
    NemuGuestWinInfo *pInfo = mGuestWindows.find(hWin);
    if (pInfo)
    {
        XWindowAttributes winAttrib;
...
DECLCALLBACK(int) getRectsCallback(NemuGuestWinInfo *pInfo,
...
int SeamlessX11::updateRects(void)
{
    ...
    mGuestWindows.doWithAll((PNEMUGUESTWINCALLBACK)getRectsCallback,
                            &rects);
    if (mpRects)
        RTMemFree(mpRects);
