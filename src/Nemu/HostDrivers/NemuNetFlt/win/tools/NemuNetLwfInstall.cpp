/* $Id: NemuNetLwfInstall.cpp $ */
/** @file
 * NetLwfInstall - NemuNetLwf installer command line tool
 */
...
#define NEMU_NETCFG_APP_NAME L"NetLwfInstall"
#define NEMU_NETLWF_INF L".\\NemuNetLwf.inf"
#define NEMU_NETLWF_RETRIES 10
...
static int NemuNetLwfInstall()
{
    ...
    if(hr == S_OK)
    {
        ...
        do
        {
            ...
            {
                DWORD dwSize;
                dwSize = MyGetfullPathNameW(NEMU_NETLWF_INF, sizeof(Inf)/sizeof(Inf[0]), Inf);
                if(dwSize > 0)
                {
                    /** @todo add size check for (sizeof(Inf)/sizeof(Inf[0])) == dwSize (string length in sizeof(Inf[0])) */
                    hr = NemuNetCfgWinNetLwfInstall(pnc, Inf);
                    if(hr == S_OK)
                    {
                    ...
                    {
                        wprintf(L"error installing NemuNetLwf (0x%x)\n", hr);
                    }
                }
                else
                {
                    hr = HRESULT_FROM_WIN32(GetLastError());
                    wprintf(L"error getting full inf path for NemuNetLwf.inf (0x%x)\n", hr);
                }
            ...
            {
                if(i < NEMU_NETLWF_RETRIES && !wcscmp(lpszLockedBy, L"6to4svc.dll"))
                {
                    wprintf(L"6to4svc.dll is holding the lock, retrying %d out of %d\n", ++i, NEMU_NETLWF_RETRIES);
