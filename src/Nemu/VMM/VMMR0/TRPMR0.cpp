...
VMMR0DECL(void) TRPMR0DispatchHostInterrupt(PVM pVM)
{
    ...
    ASMGetIDTR(&Idtr);
# if HC_ARCH_BITS == 32
    PNEMUIDTE   pIdte = &((PNEMUIDTE)Idtr.pIdt)[uActiveVector];
# else
    PNEMUIDTE64 pIdte = &((PNEMUIDTE64)Idtr.pIdt)[uActiveVector];
# endif
    AssertMsgReturnVoid(pIdte->Gen.u1Present, ("The IDT entry (%d) is not present!\n", uActiveVector));
    AssertMsgReturnVoid(    pIdte->Gen.u3Type1 == NEMU_IDTE_TYPE1
                        ||  pIdte->Gen.u5Type2 == NEMU_IDTE_TYPE2_INT_32,
                        ("The IDT entry (%d) is not 32-bit int gate! type1=%#x type2=%#x\n",
                         uActiveVector, pIdte->Gen.u3Type1, pIdte->Gen.u5Type2));
# if HC_ARCH_BITS == 32
    RTFAR32   pfnHandler;
    pfnHandler.off = NEMUIDTE_OFFSET(*pIdte);
...
# else /* 64-bit: */
    RTFAR64   pfnHandler;
    pfnHandler.off = NEMUIDTE64_OFFSET(*pIdte);
    pfnHandler.sel = pIdte->Gen.u16SegSel;
