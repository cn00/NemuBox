...
#if defined(LOG_ENABLED) || defined(NEMU_STRICT)
static const char *emR3GetStateName(EMSTATE enmState);
#endif
static NEMUSTRICTRC emR3Debug(PVM pVM, PVMCPU pVCpu, NEMUSTRICTRC rc);
 ...
 */
static DECLCALLBACK(NEMUSTRICTRC) emR3SetExecutionPolicy(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    /*
...
#endif /* LOG_ENABLED || NEMU_STRICT */


/**
 * Debug loop.
 *
 * @returns Nemu status code for EM.
 * @param   pVM     The cross context VM structure.
 * @param   pVCpu   The cross context virtual CPU structure.
 * @param   rc      Current EM Nemu status code.
 */
static NEMUSTRICTRC emR3Debug(PVM pVM, PVMCPU pVCpu, NEMUSTRICTRC rc)
{
    for (;;)
        ...
        do
        {
            switch (NEMUSTRICTRC_VAL(rc))
            {
                ...
                case VERR_DBGF_NOT_ATTACHED:
                    switch (NEMUSTRICTRC_VAL(rcLast))
                    {
                        case VINF_EM_DBG_HYPER_STEPPED:
                        case VINF_EM_DBG_HYPER_BREAKPOINT:
                        case VINF_EM_DBG_HYPER_ASSERTION:
 ...
 *
 * @returns Nemu status code. The most important ones are: VINF_EM_STEP_EVENT,
...
#else
    int rc = NEMUSTRICTRC_TODO(IEMExecOne(pVCpu)); NOREF(pVM);
#endif
 ...
 *
 * @returns Nemu status code. The most important ones are: VINF_EM_RESCHEDULE,
...
static int emR3RemExecute(PVM pVM, PVMCPU pVCpu, bool *pfFFDone)
{
...
#if defined(NEMU_STRICT) && defined(DEBUG_bird)
    ...
    *pfFFDone = false;
    ...
    for (;;)
    {
...
#else
            rc = NEMUSTRICTRC_TODO(IEMExecLots(pVCpu));
#endif
         ...
         */
#ifdef NEMU_HIGH_RES_TIMERS_HACK
...
l_REMDoForcedActions:
            ...
            NEMUVMM_EM_FF_ALL_RET(pVCpu, rc);
 ...
 *
 * @returns Strict Nemu status code from IEMExecLots.
 * @param   pVM        The cross context VM structure.
 ...
 * @thread  EMT(pVCpu)
 */
static NEMUSTRICTRC emR3ExecuteIemThenRem(PVM pVM, PVMCPU pVCpu, bool *pfFFDone)
{
    ...
    *pfFFDone = false;
    ...
    {
        NEMUSTRICTRC rcStrict = IEMExecLots(pVCpu);
        if (rcStrict != VINF_SUCCESS)
        {
            ...
            Log(("emR3ExecuteIemThenRem: returns %Rrc after %u instructions\n",
                 NEMUSTRICTRC_VAL(rcStrict), pVCpu->em.s.cIemThenRemInstructions));
            return rcStrict;
        }
        pVCpu->em.s.cIemThenRemInstructions++;
...
int emR3HighPriorityPostForcedActions(PVM pVM, PVMCPU pVCpu, int rc)
{
    NEMUVMM_EM_FF_HIGH(pVCpu, pVM->fGlobalForcedActions, pVCpu->fLocalForcedActions, rc);
    ...
    if (VMCPU_FF_IS_PENDING(pVCpu, VMCPU_FF_IEM))
        rc = NEMUSTRICTRC_TODO(IEMR3DoPendingAction(pVCpu, rc));
 ...
 * we exit everything.
 *
 * @returns Nemu status code of equal or greater importance/severity than rc.
...
#define UPDATE_RC() \
    ...
    NEMUVMM_EM_FF_ALL(pVCpu, pVM->fGlobalForcedActions, pVCpu->fLocalForcedActions, rc);
 ...
 *
 * @returns Nemu status code, informational status codes may indicate failure.
 * @param   pVM         The cross context VM structure.
 * @param   pVCpu       The cross context virtual CPU structure.
 */
VMMR3_INT_DECL(int) EMR3ExecuteVM(PVM pVM, PVMCPU pVCpu)
{
    ...
    if (rc == 0)
    {
        ...
        for (;;)
        {
            ...
            {
                NEMUVMM_EM_STATE_CHANGED(pVCpu, enmOldState, enmNewState, rc);
                ...
                NEMUVMM_EM_STATE_UNCHANGED(pVCpu, enmNewState, rc);
            ...
            switch (enmNewState)
            {
                ...
                case EMSTATE_IEM:
                {
                    ...
                    rc = NEMUSTRICTRC_TODO(EMR3HmSingleInstruction(pVM, pVCpu, EM_ONE_INS_FLAGS_RIP_CHANGE));
                ...
                case EMSTATE_IEM_THEN_REM:
                {
                    ...
                    rc = NEMUSTRICTRC_TODO(emR3ExecuteIemThenRem(pVM, pVCpu, &fFFDone));
                ...
                case EMSTATE_DEBUG_GUEST_REM:
                    TMR3NotifySuspend(pVM, pVCpu);
                    rc = NEMUSTRICTRC_TODO(emR3Debug(pVM, pVCpu, rc));
                    TMR3NotifyResume(pVM, pVCpu);
