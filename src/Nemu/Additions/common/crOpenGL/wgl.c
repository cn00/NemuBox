 ...
 * */
#define NEMU_CROGL_USE_VBITS_SUPERSET

#ifdef NEMU_CROGL_USE_VBITS_SUPERSET
...
#ifndef NEMU_CROGL_USE_VBITS_SUPERSET
/**
...
DECLEXPORT(void) WINAPI NemuCtxChromiumParameteriCR(HGLRC hglrc, GLenum param, GLint value)
{
    ContextInfo *context;
...
DECLEXPORT(HGLRC) WINAPI NemuCreateContext( HDC hdc, struct NEMUUHGSMI *pHgsmi )
{
    char *dpyName;
    ContextInfo *context;
...
DECLEXPORT(GLint) WINAPI NemuGetWindowId( HDC hdc )
{
    WindowInfo *window;
    GLint winid = 0;
...
DECLEXPORT(GLint) WINAPI NemuGetContextId( HGLRC hglrc )
{
    ContextInfo *context;
    GLint ctxid = 0;
...
{
    return NemuCreateContext(hdc, NULL);
}

DECLEXPORT(void) WINAPI NemuFlushToHost ( HGLRC hglrc )
{
    ContextInfo *context;
