...
int NemuDispVBVAInit(PNEMUDISPDEV pDev)
{
    ...
    rc = NemuDispMPQueryHGSMIInfo(pDev->hDriver, &info);
    if (RT_SUCCESS(rc))
    {
        rc = NemuDispMPQueryHGSMICallbacks(pDev->hDriver, &callbacks);
        if (RT_SUCCESS(rc))
        {
            rc = NemuDispMPHGSMIQueryPortProcs(pDev->hDriver, &portProcs);
        }
    }
    if (RT_SUCCESS(rc))
    {
    ...
    if (pDev->hgsmi.bSupported)
    {
        ...
        dwrc = EngDeviceIoControl(pDev->hDriver, IOCTL_VIDEO_HGSMI_HANDLER_ENABLE, &HandlerReg, sizeof(HandlerReg),
                                  0, NULL, &cbReturned);
        NEMU_WARN_WINERR(dwrc);

#ifdef NEMU_WITH_VIDEOHWACCEL
        if (NO_ERROR == dwrc)
        {
            NemuDispVHWAInit(pDev);
        }
#endif
    }
    ...
    {
        ULONG cbAvailable;
        NEMUDISPVRAMLAYOUT *vram = &pDev->layout;
    ...
    NemuVBVASetupBufferContext(&pDev->vbvaCtx, pDev->layout.offVBVABuffer, pDev->layout.cbVBVABuffer);
...
void vbvaReportDirtyRect(PNEMUDISPDEV pDev, RECTL *pRectOrig)
{
    if (pDev && pRectOrig)
    {
        ...
        NemuVBVAWrite(&pDev->vbvaCtx, &pDev->hgsmi.ctx, &hdr, sizeof(hdr));
    }
}

static void vbvaReportDirtyPath(PNEMUDISPDEV pDev, PATHOBJ *ppo)
{
    RECTFX rcfxBounds;
    RECTL rclBounds;
...
static void vbvaReportDirtyClip(PNEMUDISPDEV pDev, CLIPOBJ *pco, RECTL *prcl)
{
    if (prcl)
    {
