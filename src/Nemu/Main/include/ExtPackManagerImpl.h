...
public:
    ...
    HRESULT     FinalConstruct();
    void        FinalRelease();
    HRESULT     initWithDir(NEMUEXTPACKCTX a_enmContext, const char *a_pszName, const char *a_pszDir);
    void        uninit();
...
protected:
    ...
    bool        i_findModule(const char *a_pszName, const char *a_pszExt, NEMUEXTPACKMODKIND a_enmKind,
     ...
     * @{ */
    static DECLCALLBACK(int)    i_hlpFindModule(PCNEMUEXTPACKHLP pHlp, const char *pszName, const char *pszExt,
                                                NEMUEXTPACKMODKIND enmKind, char *pszFound, size_t cbFound, bool *pfNative);
    static DECLCALLBACK(int)    i_hlpGetFilePath(PCNEMUEXTPACKHLP pHlp, const char *pszFilename, char *pszPath, size_t cbPath);
    static DECLCALLBACK(NEMUEXTPACKCTX) i_hlpGetContext(PCNEMUEXTPACKHLP pHlp);
    static DECLCALLBACK(int)    i_hlpLoadHGCMService(PCNEMUEXTPACKHLP pHlp, NEMUEXTPACK_IF_CS(IConsole) *pConsole, const char *pszServiceLibrary, const char *pszServiceName);
    static DECLCALLBACK(int)    i_hlpLoadVDPlugin(PCNEMUEXTPACKHLP pHlp, NEMUEXTPACK_IF_CS(IVirtualBox) *pVirtualBox, const char *pszPluginLibrary);
    static DECLCALLBACK(int)    i_hlpUnloadVDPlugin(PCNEMUEXTPACKHLP pHlp, NEMUEXTPACK_IF_CS(IVirtualBox) *pVirtualBox, const char *pszPluginLibrary);
    static DECLCALLBACK(int)    i_hlpReservedN(PCNEMUEXTPACKHLP pHlp);
    /** @}  */
...
public:
    ...
    HRESULT     FinalConstruct();
    void        FinalRelease();
    HRESULT     initExtPackManager(VirtualBox *a_pVirtualBox, NEMUEXTPACKCTX a_enmContext);
    void        uninit();
    /** @}  */
