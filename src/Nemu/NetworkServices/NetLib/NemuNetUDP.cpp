/* $Id: NemuNetUDP.cpp $ */
/** @file
 * NemuNetUDP - IntNet UDP Client Routines.
 */
...
void *NemuNetUDPMatch(PINTNETBUF pBuf, unsigned uDstPort, PCRTMAC pDstMac, uint32_t fFlags, PNEMUNETUDPHDRS pHdrs, size_t *pcb)
{
    ...
    *pcb = 0;
    ...
    if (fFlags & NEMUNETUDP_MATCH_PRINT_STDERR)
    {
    ...
    if (    (   !(fFlags & NEMUNETUDP_MATCH_UNICAST)
             || memcmp(&pEthHdr->DstMac, pDstMac, sizeof(pEthHdr->DstMac)))
        &&  (   !(fFlags & NEMUNETUDP_MATCH_BROADCAST)
    ...
    if (!pGso)
    {
        ...
        if (fFlags & (NEMUNETUDP_MATCH_CHECKSUM | NEMUNETUDP_MATCH_REQUIRE_CHECKSUM))
        {
            if (!RTNetIPv4IsUDPValid(pIpHdr, pUdpHdr, pUdpHdr + 1, cbFrame - offUdpHdr, true /*fChecksum*/))
                return NULL;
            if (    (fFlags & NEMUNETUDP_MATCH_REQUIRE_CHECKSUM)
                &&  !pUdpHdr->uh_sum)
                return NULL;
        }
        else
...
/** Internal worker for NemuNetUDPUnicast and NemuNetUDPBroadcast. */
static int nemunetudpSend(PSUPDRVSESSION pSession, INTNETIFHANDLE hIf, PINTNETBUF pBuf,
                          ...
                          void const *pvData, size_t cbData)
{
    ...
    /* send it */
    return NemuNetIntIfSend(pSession, hIf, pBuf, RT_ELEMENTS(aSegs), &aSegs[0], true /* fFlush */);
}
...
int     NemuNetUDPUnicast(PSUPDRVSESSION pSession, INTNETIFHANDLE hIf, PINTNETBUF pBuf,
                          ...
                          void const *pvData, size_t cbData)
{
    return nemunetudpSend(pSession, hIf, pBuf,
