/* $Id: NemuVFS-utils.cpp $ */
/** @file
 * NemuVFS - helper functions.
 */
 ...
 */
int
nemuvfs_create_vnode_internal(struct mount *mp, enum vtype type, vnode_t pParent, int fIsRoot, PSHFLSTRING Path, vnode_t *ret)
{
    int     rc;
    vnode_t vnode;
    ...
    pVnodeData = (nemuvfs_vnode_t *)RTMemAllocZ(sizeof(nemuvfs_vnode_t));
    ...
    if (pVnodeData->pLockAttr)
    {
        ...
        if (pVnodeData->pLock)
        {
            ...
            vnode_params.vnfs_vops       = g_NemuVFSVnodeDirOpsVector;
 ...
 */
int
nemuvfs_guest_path_to_char_path_internal(mount_t mp, char *pszGuestPath, int cbGuestPath, char **pszHostPath, int *cbHostPath)
{
    nemuvfs_mount_t *pMount;
    ...
    pMount = (nemuvfs_mount_t *)vfs_fsprivate(mp); AssertReturn(pMount, EINVAL); AssertReturn(pMount->pRootVnode, EINVAL);
 ...
 */
int
nemuvfs_guest_path_to_shflstring_path_internal(mount_t mp, char *pszGuestPath, int cbGuestPath, PSHFLSTRING *ppResult)
{
    nemuvfs_mount_t *pMount;
    ...
    rc = nemuvfs_guest_path_to_char_path_internal(mp, pszGuestPath, cbGuestPath, &pszHostPath, &cbHostPath);
    if (rc == 0)
    {
        ...
        if (pSFPath)
        {
            ...
            nemuvfs_put_path_internal((void **)&pszHostPath);
 ...
 */
int
nemuvfs_guest_vnode_to_char_path_internal(vnode_t vnode, char **ppHostPath, int *pcbHostPath)
{
    mount_t     mp;
    int         rc;
    ...
    if (pszPath)
    {
        ...
        if (rc == 0)
        {
            return nemuvfs_guest_path_to_char_path_internal(mp, pszPath, cbPath, ppHostPath, pcbHostPath);
        }
    }
    else
    {
 ...
 */
int
nemuvfs_guest_vnode_to_shflstring_path_internal(vnode_t vnode, PSHFLSTRING *ppResult)
{
    mount_t     mp;
    int         rc;
    ...
    if (pszPath)
    {
        ...
        if (rc == 0)
        {
            return nemuvfs_guest_path_to_shflstring_path_internal(mp, pszPath, cbPath, ppResult);
        }
    }
    else
    {
 ...
 */
void
nemuvfs_put_path_internal(void **ppHandle)
{
...
static void
nemuvfs_g2h_mode_dump_inernal(uint32_t fHostMode)
{
 ...
 */
int
nemuvfs_open_internal(nemuvfs_mount_t *pMount, PSHFLSTRING pPath, uint32_t fFlags, SHFLHANDLE *pHandle)
{
    ...
    nemuvfs_g2h_mode_dump_inernal(fFlags);

    parms.Handle        = SHFL_HANDLE_NIL;
    parms.Info.cbObject = 0;
    parms.CreateFlags   = fFlags;

    rc = VbglR0SfCreate(&g_nemuSFClient, &pMount->pMap, pPath, &parms);
    if (RT_SUCCESS(rc))
    {
    ...
    {
        PDEBUG("nemuvfs_open_internal() failed: %d", rc);
    }
 ...
 */
int
nemuvfs_close_internal(nemuvfs_mount_t *pMount, SHFLHANDLE pHandle)
{
    AssertReturn(pMount, EINVAL);
    return VbglR0SfClose(&g_nemuSFClient, &pMount->pMap, pHandle);
}
 ...
 */
int
nemuvfs_get_info_internal(mount_t mp, PSHFLSTRING pSHFLDPath, PSHFLFSOBJINFO Info)
{
    nemuvfs_mount_t        *pMount;
    SHFLCREATEPARMS         parms;
    ...
    rc = VbglR0SfCreate(&g_nemuSFClient, &pMount->pMap, pSHFLDPath, &parms);
    if (rc == 0)
        *Info = parms.Info;
 ...
 */
int
nemuvfs_exist_internal(vnode_t vnode)
{
    int rc;
    ...
    rc = nemuvfs_guest_vnode_to_shflstring_path_internal(vnode, &pSFPath); AssertReturn(rc == 0, rc);
    if (rc == 0)
    {
        rc = nemuvfs_open_internal(pMount, pSFPath, fFlags, &handle);
        if (rc == 0)
        {
            rc = nemuvfs_close_internal(pMount, handle);
            if (rc != 0)
            {
                PDEBUG("Unable to close() NemuVFS object handle while checking if object exist on host: %d", rc);
            }
        }
    }

    nemuvfs_put_path_internal((void **)&pSFPath);
 ...
 */
mode_t
nemuvfs_h2g_mode_inernal(RTFMODE fHostMode)
{
    mode_t fGuestMode = 0;
 ...
 */
uint32_t
nemuvfs_g2h_mode_inernal(mode_t fGuestMode)
{
...
/**
 * Mount helper: Contruct SHFLSTRING which contains Nemu share name or path.
 *
 * @param szName        Char * string containing share name.
 * @param cbName        Length of pShareName.
 *
 * @return Allocated SHFLSTRING which contains Nemu share name or path, NULL otherwise.
 */
SHFLSTRING *
nemuvfs_construct_shflstring(char *szName, size_t cbName)
{
    size_t      cbSHFLString;
