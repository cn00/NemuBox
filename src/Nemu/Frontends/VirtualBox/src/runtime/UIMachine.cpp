/* $Id: UIMachine.cpp $ */
/** @file
 * Nemu Qt GUI - UIMachine class implementation.
 */
...
bool UIMachine::startMachine(const QString &strID)
{
    ...
    if (nemuGlobal().shouldRestoreCurrentSnapshot())
    {
        /* Create temporary session: */
        CSession session = nemuGlobal().openSession(strID, KLockType_VM);
        if (session.isNull())
            return false;
        ...
        nemuGlobal().setShouldRestoreCurrentSnapshot(false);
    }
    ...
    if (nemuGlobal().isSeparateProcess())
    {
        ...
        CMachine machine = nemuGlobal().virtualBox().FindMachine(nemuGlobal().managedVMUuid());
        AssertMsgReturn(!machine.isNull(), ("NemuGlobal::managedVMUuid() should have filter that case before!\n"), false);

        /* Try to launch corresponding machine: */
        if (!nemuGlobal().launchMachine(machine, NemuGlobal::LaunchMode_Separate))
            return false;
    }
...
bool UIMachine::prepare()
{
    ...
    nemuGlobal().startMediumEnumeration(false /* force start */);
...
void UIMachine::prepareMachineLogic()
{
    ...
    UIVisualStateType restrictedVisualStates = gEDataManager->restrictedVisualStates(nemuGlobal().managedVMUuid());
    ...
    UIVisualStateType requestedVisualState = gEDataManager->requestedVisualState(nemuGlobal().managedVMUuid());
...
void UIMachine::cleanupMachineLogic()
{
    ...
    if (uisession())
    {
        ...
        gEDataManager->setRequestedVisualState(requestedVisualState, nemuGlobal().managedVMUuid());
    }
