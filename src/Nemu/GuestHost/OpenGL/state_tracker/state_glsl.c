...
/** @file
 * Nemu OpenGL: GLSL state tracking
 */
...
DECLEXPORT(void) STATE_APIENTRY crStateGLSLDestroy(CRContext *ctx)
{
    ...
    CRASSERT(g != ctx);
    NemuTlsRefAddRef(ctx); /* <- this is a hack to avoid subsequent SetCurrentContext(g) do recursive Destroy for ctx */
    if (g)
        NemuTlsRefAddRef(g); /* <- ensure the g is not destroyed by the following SetCurrentContext call */
    ...
    if (g)
        NemuTlsRefRelease(g);
    NemuTlsRefRelease(ctx); /* <- restore back the cRefs (see above) */
#else
    __currentContext = g;
#endif
...
DECLEXPORT(GLuint) STATE_APIENTRY crStateDeleteObjectARB( NemuGLhandleARB obj )
{
