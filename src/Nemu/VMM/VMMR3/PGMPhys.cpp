...
{
    NEMUSTRICTRC rcStrict = PGMPhysRead(pVM, *pGCPhys, pvBuf, cbRead, enmOrigin);
                                                   ...
                                                   PGMACCESSORIGIN enmOrigin)
{
    ...
    NEMUSTRICTRC rcStrict = PGMPhysWrite(pVM, *pGCPhys, pvBuf, cbWrite, enmOrigin);
    AssertMsg(rcStrict == VINF_SUCCESS, ("%Rrc\n", NEMUSTRICTRC_VAL(rcStrict))); NOREF(rcStrict);
    return VINF_SUCCESS;
}
 ...
 *
 * @returns VINF_SUCCESS (Nemu strict status code).
 * @param   pVM         The cross context VM structure.
 * @param   pVCpu       The cross context virtual CPU structure of the calling EMT. Unused.
 * @param   pvUser      User parameter
 */
static DECLCALLBACK(NEMUSTRICTRC) pgmR3PhysChangeMemBalloonRendezvous(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
 ...
 */
static DECLCALLBACK(NEMUSTRICTRC) pgmR3PhysWriteProtectRAMRendezvous(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    int rc = VINF_SUCCESS;
...
#if defined(NEMU_WITH_MORE_RING0_MEM_MAPPINGS)
                              &R0PtrChunk,
#elif defined(NEMU_WITH_2X_4GB_ADDR_SPACE)
 ...
 *
 */
static DECLCALLBACK(NEMUSTRICTRC) pgmR3PhysUnmapChunkRendezvous(PVM pVM, PVMCPU pVCpu, void *pvUser)
{
    int rc = VINF_SUCCESS;
    pgmLock(pVM);
