...
static DECLCALLBACK(bool) pcirawr0Isr(void* pContext, int32_t iHostIrq)
{
    ...
    Loc.cb = 2;
    rc = pThis->pPort->pfnPciCfgRead(pThis->pPort, NEMU_PCI_STATUS, &Loc);
...
PCIRAWR0DECL(int)  PciRawR0InitVM(PVM pVM)
{
    ...
    rc = SUPR0ComponentQueryFactory(pVM->pSession, "NemuRawPci", RAWPCIFACTORY_UUID_STR, (void **)&pFactory);
...
static DECLCALLBACK(int) dummyPciDevInit(PRAWPCIDEVPORT pPort, uint32_t fFlags)
{
    ...
    dummyPciSetWord(pThis, NEMU_PCI_VENDOR_ID, 0xccdd);
    dummyPciSetWord(pThis, NEMU_PCI_DEVICE_ID, 0xeeff);
    dummyPciSetWord(pThis, NEMU_PCI_COMMAND,   PCI_COMMAND_IOACCESS | PCI_COMMAND_MEMACCESS | PCI_COMMAND_BUSMASTER);
    dummyPciSetByte(pThis, NEMU_PCI_INTERRUPT_PIN, 1);
                              ...
                              uint32_t        *pfDevFlags)
{
    ...
    rc = SUPR0ComponentQueryFactory(pSession, "NemuRawPci", RAWPCIFACTORY_UUID_STR, (void **)&pFactory);
