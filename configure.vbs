...
sub DisableCOM(strReason)
   if g_blnDisableCOM = False then
      ...
      CfgPrint "NEMU_WITH_MAIN="
      CfgPrint "NEMU_WITH_QTGUI="
      CfgPrint "NEMU_WITH_NEMUSDL="
      CfgPrint "NEMU_WITH_DEBUGGER_GUI="
   end if
end sub
...
sub CheckForVisualCPP(strOptVC, strOptVCCommon, blnOptVCExpressEdition)
   ...
   if   (InStr(1, g_strShellOutput, "±‡“Î∆˜ 16.") <= 0) _
    And (InStr(1, g_strShellOutput, "±‡“Î∆˜ 17.") <= 0) then
   ...
   '
   if InStr(1, g_strShellOutput, "±‡“Î∆˜ 16.") > 0 then
      ...
      elseif   LogFileExists(g_strPathDDK, "inc/atl71/atlbase.h") _
         ...
         CfgPrint "NEMU_WITHOUT_COMPILER_REDIST=1"
...
sub CheckForPlatformSDK(strOptSDK)
   ...
   CfgPrint "NEMU_WINPSDK          := WINPSDK" & g_strVerPSDK
...
sub CheckForMidl()
   ...
   CfgPrint "NEMU_MAIN_IDL         := " & strMidl
...
sub CheckForXml2(strOptXml2)
   ...
   CfgPrint "SDK_NEMU_LIBXML2_INCS  := " & strPathXml2 & "/include"
   CfgPrint "SDK_NEMU_LIBXML2_LIBS  := " & strPathXml2 & "/lib/libxml2.lib"
...
sub CheckForXslt(strOptXslt)
   ...
   CfgPrint "SDK_NEMU_LIBXSLT_INCS   := " & strPathXslt & "/include"
   CfgPrint "SDK_NEMU_LIBXSLT_LIBS   := " & strPathXslt & "/lib/libxslt.lib"
...
sub CheckForSsl(strOptSsl)
   ...
   CfgPrint "SDK_NEMU_OPENSSL_INCS := " & strPathSsl & "/include"
   CfgPrint "SDK_NEMU_OPENSSL_LIBS := " & strPathSsl & "/lib/ssleay32.lib" & " " & strPathSsl & "/lib/libeay32.lib"
   CfgPrint "SDK_NEMU_BLD_OPENSSL_LIBS := " & strPathSsl & "/lib/ssleay32.lib" & " " & strPathSsl & "/lib/libeay32.lib"
...
sub CheckForCurl(strOptCurl)
   ...
   CfgPrint "SDK_NEMU_LIBCURL_INCS := " & strPathCurl & "/include"
   CfgPrint "SDK_NEMU_LIBCURL_LIBS := " & strPathCurl & "/libcurl.lib"
...
function CheckForQt4Sub(strPathQt4)
   ...
   if   LogFileExists(strPathQt4, "bin/moc.exe") _
    ...
    And (   LogFileExists(strPathQt4, "lib/QtCore4.lib") _
         Or LogFileExists(strPathQt4, "lib/NemuQtCore4.lib") _
         Or LogFileExists(strPathQt4, "lib/QtCoreNemu4.lib")) _
    And (   LogFileExists(strPathQt4, "lib/QtNetwork4.lib") _
         Or LogFileExists(strPathQt4, "lib/NemuQtNetwork4.lib") _
         Or LogFileExists(strPathQt4, "lib/QtNetworkNemu4.lib")) _
      then
         CheckForQt4Sub = True
   end if
...
function CheckForPython(strPathPython)
   ...
   if LogFileExists(strPathPython, "python.exe") then
      CfgPrint "NEMU_BLD_PYTHON       := " & strPathPython & "\python.exe"
      CheckForPython = True
   end if
...
function CheckForMkisofs(strFnameMkisofs)
   ...
   if FileExists(strFnameMkisofs) = false then
      ...
      CfgPrint "NEMU_MKISOFS          := " & strFnameMkisofs
...
Sub Main
   ...
   CfgPrint "NEMU_WITH_WDDM_W8     := " '' @todo look for WinDDKv8; Check with Misha if we _really_ need the v8 DDK...
   ...
   CfgPrint "NEMU_WITH_OPEN_WATCOM := " '' @todo look for openwatcom 1.9+
   ...
   Print ""
   Print "Execute env.bat once before you start to build Nemu:"
   Print ""
   Print "  env.bat"
   Print "  kmk"
   Print ""
