...
/** @file
 * Nemu Qt GUI - UIGlobalSettingsInput class implementation.
 */
...
void UIGlobalSettingsInput::loadToCacheFrom(QVariant &data)
{
    ...
    foreach (const QString &strShortcutKey, shortcutKeys)
    {
        ...
        m_cache.m_shortcuts << UIShortcutCacheItem(strShortcutKey, NemuGlobal::removeAccelMark(shortcut.description()),
...
bool UIGlobalSettingsInput::validate(QList<UIValidationMessage> &messages)
{
    ...
    if (!m_pSelectorModel->isAllShortcutsUnique())
    {
        ...
        message.first = NemuGlobal::removeAccelMark(m_pTabWidget->tabText(UIHotKeyTableIndex_Selector));
    ...
    {
        UIValidationMessage message;
        message.first = NemuGlobal::removeAccelMark(m_pTabWidget->tabText(UIHotKeyTableIndex_Machine));
        message.second << tr("Some items have the same shortcuts assigned.");
        messages << message;
        fPass = false;
    }
