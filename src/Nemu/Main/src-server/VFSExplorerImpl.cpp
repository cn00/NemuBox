...
HRESULT VFSExplorer::i_updateFS(TaskVFSExplorer *aTask)
{
    ...
    try
    {
        ...
        if (RT_FAILURE(vrc))
            throw setError(NEMU_E_FILE_ERROR, tr ("Can't open directory '%s' (%Rrc)"), pszPath, vrc);
...
HRESULT VFSExplorer::i_deleteFS(TaskVFSExplorer *aTask)
{
    ...
    try
    {
             ...
             ++it, ++i)
        {
            ...
            if (RT_FAILURE(vrc))
                throw setError(NEMU_E_FILE_ERROR, tr("Can't delete file '%s' (%Rrc)"), szPath, vrc);
            if (aTask->progress)
...
HRESULT VFSExplorer::i_updateS3(TaskVFSExplorer *aTask)
{
    ...
    try
    {
        int vrc = RTS3Create(&hS3, m->strUsername.c_str(), m->strPassword.c_str(),
                             m->strHostname.c_str(), "virtualbox-agent/" NEMU_VERSION_STRING);
        if (RT_FAILURE(vrc))
...
HRESULT VFSExplorer::i_deleteS3(TaskVFSExplorer *aTask)
{
    ...
    try
    {
             ...
             ++it, ++i)
        {
            ...
            if (RT_FAILURE(vrc))
                throw setError(NEMU_E_FILE_ERROR, tr ("Can't delete file '%s' (%Rrc)"), (*it).c_str(), vrc);
            if (aTask->progress)
                aTask->progress->SetCurrentOperationProgress((ULONG)(fPercentStep * i));
        }
    }
