/* $Id: nemu-greeter.cpp $ */
/** @file
 * nemu-greeter - an own LightDM greeter module supporting auto-logons
...
#define NEMU_MODULE_NAME "nemu-lightdm-greeter"

/* UI elements used in this greeter. */
#define NEMU_GREETER_UI_WND_GREETER     "wnd_greeter"

#define NEMU_GREETER_UI_EDT_USER        "edt_username"
#define NEMU_GREETER_UI_EDT_PASSWORD    "edt_password"
#define NEMU_GREETER_UI_BTN_LOGIN       "btn_login"
#define NEMU_GREETER_UI_LBL_INFO        "lbl_info"

/* UI display options. */
/** Show the restart menu entry / button. */
#define NEMU_GREETER_UI_SHOW_RESTART    RT_BIT(0)
/** Show the shutdown menu entry / button. */
#define NEMU_GREETER_UI_SHOW_SHUTDOWN   RT_BIT(1)
/** Show the (customized) top banner.  */
#define NEMU_GREETER_UI_SHOW_BANNER     RT_BIT(2)
/** Enable custom colors */
#define NEMU_GREETER_UI_USE_THEMING     RT_BIT(3)

/** Extracts the 8-bit red component from an uint32_t. */
#define NEMU_RGB_COLOR_RED(uColor)      uColor & 0xFF
/** Extracts the 8-bit green component from an uint32_t. */
#define NEMU_RGB_COLOR_GREEN(uColor)    (uColor >> 8)  & 0xFF
/** Extracts the 8-bit blue component from an uint32_t. */
#define NEMU_RGB_COLOR_BLUE(uColor)     (uColor >> 16) & 0xFF
...
typedef struct NEMUGREETERCTX
{
    ...
    char           *pszPassword;
} NEMUGREETERCTX, *PNEMUGREETERCTX;

static void nemuGreeterError(const char *pszFormat, ...)
{
    va_list va;
    char *buf;
    ...
    {
        RTLogRelPrintf("%s: error: %s", NEMU_MODULE_NAME, buf);
        RTStrFree(buf);
    }
    va_end(va);
}

static void nemuGreeterLog(const char *pszFormat, ...)
{
    if (g_iVerbosity)
    {
        va_list va;
        ...
        if (RTStrAPrintfV(&buf, pszFormat, va))
        {
             ...
             * sensitive data! */
            RTLogRelPrintf("%s: %s", NEMU_MODULE_NAME, buf);
            RTStrFree(buf);
        }
        va_end(va);
    }
...
/** @tood Move the following two functions to VbglR3 (also see pam_nemu). */
#ifdef NEMU_WITH_GUEST_PROPS
/**
 ...
 *                                  retrieved. Optional.
 */
static int nemu_read_prop(uint32_t uClientID,
...
#ifdef DEBUG
    nemuGreeterLog("Read guest property \"%s\"=\"%s\" (Flags: %s, TS: %RU64): %Rrc\n",
 ...
 */
static int nemu_wait_prop(uint32_t uClientID,
 ...
 */
static int nemuGreeterCheckCreds(PNEMUGREETERCTX pCtx)
{
    ...
    if (RT_FAILURE(rc))
    {
        if (rc != VERR_NOT_FOUND)
            nemuGreeterError("nemuGreeterCheckCreds: could not query for credentials! rc=%Rrc. Aborting\n", rc);
        else if (!s_fCredsNotFoundMsgShown)
        {
            nemuGreeterLog("nemuGreeterCheckCreds: no credentials available\n");
    ...
    else
    {
        ...
        if (RT_FAILURE(rc))
        {
            nemuGreeterError("nemuGreeterCheckCreds: could not retrieve credentials! rc=%Rrc. Aborting\n", rc);
        }
        else
        {
            nemuGreeterLog("nemuGreeterCheckCreds: credentials retrieved: user=%s, password=%s, domain=%s\n",
                           pszUsername,
#ifdef DEBUG
                           pCtx->pszPassword,
#else
...
#ifdef DEBUG
    nemuGreeterLog("nemuGreeterCheckCreds: returned with rc=%Rrc\n", rc);
#endif
    return rc;
}
...
static void cb_lightdm_show_prompt(LightDMGreeter *pGreeter,
                                   ...
                                   gpointer pvData)
{
    nemuGreeterLog("cb_lightdm_show_prompt: text=%s, type=%d\n", pszText, enmType);

    PNEMUGREETERCTX pCtx = (PNEMUGREETERCTX)pvData;
    AssertPtr(pCtx);
...
static void cb_lightdm_show_message(LightDMGreeter *pGreeter,
                                    ...
                                    gpointer pvData)
{
    nemuGreeterLog("cb_lightdm_show_message: text=%s, type=%d\n", pszText, enmType);
...
static void cb_lightdm_auth_complete(LightDMGreeter *pGreeter)
{
    ...
    if (lightdm_greeter_get_is_authenticated(pGreeter))
    {
        ...
        if (pszSession)
        {
            ...
            nemuGreeterError("unable to get default session\n");
    }
    else
        nemuGreeterLog("user not authenticated successfully (yet)\n");
}
...
#endif
{
    ...
    const char *pszPwd = pCtx->pEdtPassword->value();
#else
    GtkEntry *pEdtUser = GTK_ENTRY(gtk_builder_get_object(pCtx->pBuilder, NEMU_GREETER_UI_EDT_USER));
    AssertPtr(pEdtUser);
    const gchar *pszUser = gtk_entry_get_text(pEdtUser);

    GtkEntry *pEdtPwd = GTK_ENTRY(gtk_builder_get_object(pCtx->pBuilder, NEMU_GREETER_UI_EDT_PASSWORD));
    AssertPtr(pEdtPwd);
    const gchar *pszPwd = gtk_entry_get_text(pEdtPwd);
#endif

    /** @todo Add domain handling? */
    nemuGreeterLog("login button pressed: greeter=%p, user=%s, password=%s\n",
                   pCtx->pGreeter,
...
#endif
{
    nemuGreeterLog("menu button pressed\n");
}
...
#endif
{
    nemuGreeterLog("restart button pressed\n");
    ...
    if (fRestart)
    {
        nemuGreeterLog("restart requested\n");
#ifndef DEBUG
...
#endif
{
    nemuGreeterLog("shutdown button pressed\n");
    ...
    if (fShutdown)
    {
        nemuGreeterLog("shutdown requested\n");
#ifndef DEBUG
...
#endif
{
    nemuGreeterLog("cb_edt_username called\n");
...
#endif
{
    nemuGreeterLog("cb_edt_password called\n");
...
#endif
{
...
#ifdef DEBUG
    nemuGreeterLog("cb_check_creds called, clientId=%RU32, timeoutMS=%RU32\n",
    ...
    {
        uint64_t tsAbort;
        rc = nemu_read_prop(pCtx->uClientId,
        ...
        switch (rc)
        {
            case VINF_SUCCESS:
# ifdef DEBUG
                nemuGreeterLog("cb_check_creds: tsAbort %RU64 <-> %RU64\n",
...
# endif
            ...
            case VERR_TOO_MUCH_DATA:
                nemuGreeterError("cb_check_creds: temporarily unable to get abort notification\n");
                break;
            ...
            default:
                nemuGreeterError("cb_check_creds: the abort notification request failed with rc=%Rrc\n", rc);
    ...
    if (fAbort)
    {
        ...
        szVal[0] = '\0';
        int rc2 = nemu_read_prop(pCtx->uClientId,
                                 "/VirtualBox/GuestAdd/PAM/CredsMsgWaitAbort",
                                 true /* Read-only on guest */,
                                 szVal, sizeof(szVal), NULL /* Timestamp. */);
        if (   RT_FAILURE(rc2)
            && rc2 != VERR_NOT_FOUND)
            nemuGreeterError("cb_check_creds: getting wait abort message failed with rc=%Rrc\n", rc2);
# ifdef NEMU_WITH_FLTK
        AssertPtr(pCtx->pLblInfo);
        pCtx->pLblInfo->copy_label(szVal);
# else /* !NEMU_WITH_FLTK */
        GtkLabel *pLblInfo = GTK_LABEL(gtk_builder_get_object(pCtx->pBuilder, NEMU_GREETER_UI_LBL_INFO));
        AssertPtr(pLblInfo);
        gtk_label_set_text(pLblInfo, szVal);
# endif /* !NEMU_WITH_FLTK */
        nemuGreeterLog("cb_check_creds: got notification from host to abort waiting\n");
    }
    else
    {
#endif /* NEMU_WITH_GUEST_PROPS */
        rc = nemuGreeterCheckCreds(pCtx);
        if (RT_SUCCESS(rc))
        {
    ...
    if (rc == VERR_NOT_FOUND) /* No credential found this round. */
    {
        ...
        if (pCtx->uTimeoutMS < u64Elapsed)
        {
            ...
            if (   RT_FAILURE(rc2)
                && rc2 != VERR_NOT_FOUND)
                nemuGreeterError("cb_check_creds: getting wait timeout message failed with rc=%Rrc\n", rc2);
# ifdef NEMU_WITH_FLTK
            AssertPtr(pCtx->pLblInfo);
            pCtx->pLblInfo->copy_label(szVal);
# else
...
# endif
#endif /* NEMU_WITH_GUEST_PROPS */
            nemuGreeterLog("cb_check_creds: no credentials retrieved within time (%RU32ms), giving up\n",
                           pCtx->uTimeoutMS);
            rc = VERR_TIMEOUT;
        }
    }
...
#ifdef DEBUG
    nemuGreeterLog("cb_check_creds returned with rc=%Rrc\n", rc);
#endif
 ...
 * @param   pfnLog
 */
static DECLCALLBACK(void) nemuGreeterLogHeaderFooter(PRTLOGGER pLoggerRelease, RTLOGPHASE enmPhase, PFNRTLOGPHASEMSG pfnLog)
{
    ...
    switch (enmPhase)
    {
        case RTLOGPHASE_BEGIN:
        {
            pfnLog(pLoggerRelease,
                   "nemu-greeter %s r%s (verbosity: %d) %s (%s %s) release log\n"
                   "Log opened %s\n",
                   RTBldCfgVersion(), RTBldCfgRevisionStr(), g_iVerbosity, NEMU_BUILD_TARGET,
 ...
 */
static int nemuGreeterLogCreate(const char *pszLogFile)
{
    ...
    int rc = RTLogCreateEx(&g_pLoggerRelease, fFlags, "all",
                           "NEMUGREETER_RELEASE_LOG", RT_ELEMENTS(s_apszGroups), s_apszGroups,
                           RTLOGDEST_STDOUT,
                           nemuGreeterLogHeaderFooter, g_cHistory, g_uHistoryFileSize, g_uHistoryFileTime,
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        /* Explicitly flush the log in case of NEMUGREETER_RELEASE_LOG_FLAGS=buffered. */
...
static void nemuGreeterLogDestroy(void)
{
    RTLogDestroy(RTLogRelSetDefaultInstance(NULL));
}

static int nemuGreeterUsage(void)
{
    RTPrintf("Usage:\n"
    ...
    RTPrintf("\n"
             " Copyright (C) 2012-" NEMU_C_YEAR " " NEMU_VENDOR "\n");
...
int main(int argc, char **argv)
{
    ...
    rc = nemuGreeterLogCreate(strlen(szLogFile) ? szLogFile : NULL);
    if (RT_FAILURE(rc))
    ...
    uint32_t uOptsUI = NEMU_GREETER_UI_SHOW_RESTART
                     | NEMU_GREETER_UI_SHOW_SHUTDOWN;
#ifdef NEMU_WITH_GUEST_PROPS
    uint32_t uClientId = 0;
    rc = VbglR3GuestPropConnect(&uClientId);
    if (RT_SUCCESS(rc))
    {
        nemuGreeterLog("clientId=%RU32\n", uClientId);

        ctx.uClientId = uClientId;

        char szVal[256];
        int rc2 = nemu_read_prop(uClientId,
        ...
        if (   RT_SUCCESS(rc2)
            && !RTStrICmp(szVal, "1"))
        {
            uOptsUI &= ~NEMU_GREETER_UI_SHOW_RESTART;
        }

        rc2 = nemu_read_prop(uClientId,
        ...
        if (   RT_SUCCESS(rc2)
            && !RTStrICmp(szVal, "1"))
        {
            uOptsUI &= ~NEMU_GREETER_UI_SHOW_SHUTDOWN;
        }
...
# ifdef NEMU_GREETER_WITH_PNG_SUPPORT
        ...
        if (RT_SUCCESS(rc2))
        {
            ...
            {
                nemuGreeterLog("showing banner from '%s'\n", szBannerPath);
                uOptsUI |= NEMU_GREETER_UI_SHOW_BANNER;
            }
            else
                nemuGreeterLog("warning: unable to find banner at '%s', skipping\n", szBannerPath);
        }
# endif /* NEMU_GREETER_WITH_PNG_SUPPORT */
        ...
        if (   RT_SUCCESS(rc2)
            && !RTStrICmp(szVal, "1"))
        {
            nemuGreeterLog("custom theming enabled\n");
                           uOptsUI |= NEMU_GREETER_UI_USE_THEMING;
        }

        if (uOptsUI & NEMU_GREETER_UI_USE_THEMING)
        {
        ...
        nemuGreeterError("unable to connect to guest property service, rc=%Rrc\n", rc);
#endif
    nemuGreeterLog("UI options are: %RU32\n", uOptsUI);

#ifdef NEMU_WITH_FLTK
    int rc2 = Fl::scheme("plastic");
    if (!rc2)
        nemuGreeterLog("warning: unable to set visual scheme\n");

    Fl::visual(FL_DOUBLE | FL_INDEX);
    Fl_Double_Window *pWndMain = new Fl_Double_Window(Fl::w(), Fl::h(), "VirtualBox Guest Additions");
    AssertPtr(pWndMain);
    if (uOptsUI & NEMU_GREETER_UI_USE_THEMING)
        pWndMain->color(fl_rgb_color(NEMU_RGB_COLOR_RED(uBgColor),
                                     NEMU_RGB_COLOR_GREEN(uBgColor),
                                     NEMU_RGB_COLOR_BLUE(uBgColor)));
    else /* Default colors. */
    ...
    if (uOptsUI & NEMU_GREETER_UI_USE_THEMING)
        pWndGreeter->color(fl_rgb_color(NEMU_RGB_COLOR_RED(uLogonDlgBgColor),
                                        NEMU_RGB_COLOR_GREEN(uLogonDlgBgColor),
                                        NEMU_RGB_COLOR_BLUE(uLogonDlgBgColor)));
    else /* Default colors. */
...
# ifdef NEMU_GREETER_WITH_PNG_SUPPORT
    ...
    if (uOptsUI & NEMU_GREETER_UI_SHOW_BANNER)
    {
...
# endif
    ...
    if (uOptsUI & NEMU_GREETER_UI_USE_THEMING)
        pLblHeader->labelcolor(fl_rgb_color(NEMU_RGB_COLOR_RED(uLogonDlgHdrColor),
                                            NEMU_RGB_COLOR_GREEN(uLogonDlgHdrColor),
                                            NEMU_RGB_COLOR_BLUE(uLogonDlgHdrColor)));
    else /* Default color. */
    ...
    if (uOptsUI & NEMU_GREETER_UI_USE_THEMING)
        pBtnLogin->color(fl_rgb_color(NEMU_RGB_COLOR_RED(uLogonDlgBtnColor),
                                      NEMU_RGB_COLOR_GREEN(uLogonDlgBtnColor),
                                      NEMU_RGB_COLOR_BLUE(uLogonDlgBtnColor)));
    else /* Default color. */
    ...
    if (uOptsUI & NEMU_GREETER_UI_USE_THEMING)
        pBtnMenu->color(fl_rgb_color(NEMU_RGB_COLOR_RED(uLogonDlgBtnColor),
                                     NEMU_RGB_COLOR_GREEN(uLogonDlgBtnColor),
                                     NEMU_RGB_COLOR_BLUE(uLogonDlgBtnColor)));
    else /* Default color. */
        pBtnMenu->color(fl_rgb_color(255, 255, 255));

    if (uOptsUI & NEMU_GREETER_UI_SHOW_RESTART)
        pBtnMenu->add("Restart", "" /* Shortcut */, cb_btn_restart, &ctx, 0 /* Flags */);
    if (uOptsUI & NEMU_GREETER_UI_SHOW_SHUTDOWN)
    ...
    AssertPtr(pBuilder);
    if (!gtk_builder_add_from_file(pBuilder, "/usr/share/xgreeters/nemu-greeter.ui", &pError))
    {
        AssertPtr(pError);
        nemuGreeterError("unable to load UI: %s", pError->message);
        return RTEXITCODE_FAILURE;
    }

    GtkWindow *pWndGreeter = GTK_WINDOW(gtk_builder_get_object(pBuilder, NEMU_GREETER_UI_WND_GREETER));
    AssertPtr(pWndGreeter);
    GtkButton *pBtnLogin = GTK_BUTTON(gtk_builder_get_object(pBuilder, NEMU_GREETER_UI_BTN_LOGIN));
    AssertPtr(pBtnLogin);
    GtkLabel *pLblInfo = GTK_LABEL(gtk_builder_get_object(pBuilder, NEMU_GREETER_UI_LBL_INFO));
    AssertPtr(pLblInfo);
    ...
    {
        nemuGreeterError("unable to connect to LightDM server, aborting\n");
        return RTEXITCODE_FAILURE;
    }

    nemuGreeterLog("connected to LightDM server\n");
    ...
    if (uClientId) /* Connected to guest property service? */
    {
        ...
        if (RT_SUCCESS(rc2))
        {
            ...
            if (RT_SUCCESS(rc2))
            {
                ...
                if (!uTimeoutMS)
                {
                    nemuGreeterError("pam_nemu_authenticate: invalid waiting timeout value specified, defaulting to infinite timeout\n");
...
# endif
        ...
        if (fCheckCreds)
        {
            nemuGreeterLog("No credentials available on startup, starting to check periodically ...\n");
# ifdef NEMU_WITH_FLTK
...
# endif /* NEMU_GREETER_WITH_PNG_SUPPORT */
    ...
    nemuGreeterLog("terminated with exit code %ld (rc=%Rrc)\n",
                   rcExit, rc);

    nemuGreeterLogDestroy();
