...
AssertCompileSize(MsixTableRecord, NEMU_MSIX_ENTRY_SIZE);
#pragma pack()
...
{
    return PCIDevGetWord(pDev, pDev->Int.s.u8MsixCapOffset + NEMU_MSIX_CAP_MESSAGE_CONTROL);
}

DECLINLINE(bool)      msixIsEnabled(PPCIDEVICE pDev)
{
    return (msixGetMessageControl(pDev) & NEMU_PCI_MSIX_FLAGS_ENABLE) != 0;
}

DECLINLINE(bool)      msixIsMasked(PPCIDEVICE pDev)
{
    return (msixGetMessageControl(pDev) & NEMU_PCI_MSIX_FLAGS_FUNCMASK) != 0;
}
...
{
    return (MsixTableRecord*)msixGetPageOffset(pDev, iVector * NEMU_MSIX_ENTRY_SIZE);
}
...
PDMBOTHCBDECL(int) msixMMIOWrite(PPDMDEVINS pDevIns, void *pvUser, RTGCPHYS GCPhysAddr, void const *pv, unsigned cb)
{
    ...
    *(uint32_t*)msixGetPageOffset(pPciDev, off) = *(uint32_t*)pv;
    ...
    msixCheckPendingVector(pDevIns, (PCPDMPCIHLP)pPciDev->Int.s.pPciBusPtrR3, pPciDev, off / NEMU_MSIX_ENTRY_SIZE);
...
int MsixInit(PCPDMPCIHLP pPciHlp, PPCIDEVICE pDev, PPDMMSIREG pMsiReg)
{
    ...
    if (cVectors > NEMU_MSIX_MAX_ENTRIES)
    {
    ...
    pDev->Int.s.u8MsixCapSize   = NEMU_MSIX_CAP_SIZE;
    ...
    PCIDevSetByte(pDev,  iCapOffset + 0, NEMU_PCI_CAP_ID_MSIX);
    PCIDevSetByte(pDev,  iCapOffset + 1, iNextOffset); /* next */
    PCIDevSetWord(pDev,  iCapOffset + NEMU_MSIX_CAP_MESSAGE_CONTROL, cVectors - 1);

    uint32_t offTable = 0, offPBA = 0x800;

    PCIDevSetDWord(pDev,  iCapOffset + NEMU_MSIX_TABLE_BIROFFSET, offTable | iBar);
    PCIDevSetDWord(pDev,  iCapOffset + NEMU_MSIX_PBA_BIROFFSET,   offPBA   | iBar);
    ...
    for (uint32_t i = 0; i < len; i++)
    {
        ...
        switch (reg)
        {
            ...
            case 1: /* Next pointer,  ro */
                break;
            case NEMU_MSIX_CAP_MESSAGE_CONTROL:
                /* don't change read-only bits: 0-7 */
                break;
            case NEMU_MSIX_CAP_MESSAGE_CONTROL + 1:
            {
                ...
                fJustEnabled |= msixBitJustCleared(pDev->config[uAddr], u8NewVal, NEMU_PCI_MSIX_FLAGS_ENABLE >> 8);
                fJustEnabled |= msixBitJustCleared(pDev->config[uAddr], u8NewVal, NEMU_PCI_MSIX_FLAGS_FUNCMASK >> 8);
