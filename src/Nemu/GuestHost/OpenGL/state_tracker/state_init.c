...
GLboolean g_bNemuEnableDiffOnMakeCurrent = GL_TRUE;
                                             ...
                                             GLint visBits, CRContext *shareCtx)
{
    ...
    ctx->id = i;
#ifdef CHROMIUM_THREADSAFE
    NemuTlsRefInit(ctx, crStateContextDtor);
#endif
    ctx->flush_func = NULL;
...
# endif
    ...
    if (defaultContext) {
        ...
        NemuTlsRefRelease(defaultContext);
#else
...
void crStateDestroy(void)
{
    ...
    for (i = CR_MAX_CONTEXTS-1; i >= 0; i--)
    {
        ...
        {
#ifdef CHROMIUM_THREADSAFE
            if (NemuTlsRefIsFunctional(g_pAvailableContexts[i]))
                NemuTlsRefRelease(g_pAvailableContexts[i]);
#else
...
#ifdef CHROMIUM_THREADSAFE
    NemuTlsRefMarkDestroy(ctx);
# ifdef IN_GUEST
    if (NemuTlsRefCountGet(ctx) > 1 && ctx->shared == gSharedState)
    {
...
{
    GLboolean bOld = g_bNemuEnableDiffOnMakeCurrent;
    g_bNemuEnableDiffOnMakeCurrent = fEnable;
    return bOld;
}
...
void crStateMakeCurrent( CRContext *ctx )
{
    ...
    if (g_bNemuEnableDiffOnMakeCurrent && current) {
        /* Check to see if the differencer exists first,
           we may not have one, aka the packspu */
        if (diff_api.AlphaFunc)
...
void crStateNemuAttachThread()
{
}

GLint crStateNemuCreateContext( GLint con, const char * dpyName, GLint visual, GLint shareCtx )
{
    return 0;
}

GLint crStateNemuWindowCreate( GLint con, const char *dpyName, GLint visBits  )
{
    return 0;
}

void crStateNemuWindowDestroy( GLint con, GLint window )
{
}

GLint crStateNemuConCreate(struct NEMUUHGSMI *pHgsmi)
{
    return 0;
}

void crStateNemuConDestroy(GLint con)
{
}

void crStateNemuConFlush(GLint con)
{
}
