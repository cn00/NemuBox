 ...
 *
 * Nemu uses the IPRT logging system which supports group level flags and multiple
 ...
 *
 * The default logger uses NEMU_LOG_DEST as destination specifier. File output is
 * enabled by default and goes to a file "./Nemu-\<pid\>.log".
 *
 ...
 * NEMU_LOG environment variable and the .log debugger command can be used to
 ...
 * Example:
 *
 *      NEMU_LOG=+all+pgm.e.s.b.z.l-qemu
 *
...
/**
 * Creates the default logger instance for a Nemu process.
 *
...
# ifndef IN_GUEST
    ...
    if (!RTProcGetExecutablePath(szExecName, sizeof(szExecName)))
        strcpy(szExecName, "Nemu");
    RTTIMESPEC TimeSpec;
    RTTIME Time;
    RTTimeExplode(&Time, RTTimeNow(&TimeSpec));
    rc = RTLogCreate(&pLogger, 0, NULL, "NEMU_LOG", RT_ELEMENTS(g_apszGroups), &g_apszGroups[0], RTLOGDEST_FILE,
...
# else  /* IN_GUEST */
    ...
    rc = RTLogCreate(&pLogger, 0, NULL, "NEMU_LOG", RT_ELEMENTS(g_apszGroups), &g_apszGroups[0], RTLOGDEST_USER, "Nemu.log");
# endif /* IN_GUEST */
...
# ifndef IN_GUEST
    rc = RTLogCreate(&pLogger, 0, NULL, "NEMU_LOG", RT_ELEMENTS(g_apszGroups), &g_apszGroups[0], RTLOGDEST_FILE, "Nemu-ring0.log");
# else  /* IN_GUEST */
    rc = RTLogCreate(&pLogger, 0, NULL, "NEMU_LOG", RT_ELEMENTS(g_apszGroups), &g_apszGroups[0], RTLOGDEST_USER, "Nemu-ring0.log");
# endif /* IN_GUEST */
    if (RT_SUCCESS(rc))
    {
        /*
...
# if defined(DEBUG_michael) && defined(IN_GUEST)
        ...
        pLogger->fDestFlags |= RTLOGDEST_DEBUGGER | RTLOGDEST_STDOUT;
# endif
# if 0 /* nemudrv logging - ATTENTION: this is what we're referring to guys! Change to '# if 1'. */
        RTLogGroupSettings(pLogger, "all=~0 -default.l6.l5.l4.l3");
