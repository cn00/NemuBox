...
static NemuDLMExecuteFn crDLMGetExecuteRoutine(NemuDLOpCode opcode)
{
    if (opcode < NEMU_DL_OPCODE_MAX)
        return g_NemuDLMExecuteFns[opcode];
...
crDLMLoadListInstance(PSSMHANDLE pSSM, DLMListInfo *pListInfo, SPUDispatchTable *dispatchTable)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (pInstance)
        {
            ...
            if (RT_SUCCESS(rc))
            {
                pInstance->execute = crDLMGetExecuteRoutine(pInstance->iNemuOpCode);
                if (pInstance->execute)
                {
                    ...
                    crError("Restoring Display Lists: unknown list item (opcode=%u).", pInstance->iNemuOpCode);
            }
            else
