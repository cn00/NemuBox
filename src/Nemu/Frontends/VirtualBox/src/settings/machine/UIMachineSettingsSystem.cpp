...
/** @file
 * Nemu Qt GUI - UIMachineSettingsSystem class implementation.
 */
...
void UIMachineSettingsSystem::loadToCacheFrom(QVariant &data)
{
    ...
    /* Load support flags: */
    systemData.m_fSupportedPAE = nemuGlobal().host().GetProcessorFeature(KProcessorFeature_PAE);
    systemData.m_fSupportedHwVirtEx = nemuGlobal().host().GetProcessorFeature(KProcessorFeature_HWVirtEx);
...
bool UIMachineSettingsSystem::validate(QList<UIValidationMessage> &messages)
{
    ...
    /* Motherboard tab: */
    {
        ...
        message.first = NemuGlobal::removeAccelMark(m_pTabWidgetSystem->tabText(0));

        /* RAM amount test: */
        ulong uFullSize = nemuGlobal().host().GetMemorySize();
        if (m_pSliderMemorySize->value() > (int)m_pSliderMemorySize->maxRAMAlw())
        {
            message.second << tr(
                ...
                .arg(nemuGlobal().formatSize((uint64_t)uFullSize * _1M));
            fPass = false;
        }
    ...
    /* CPU tab: */
    {
        /* Prepare message: */
        UIValidationMessage message;
        message.first = NemuGlobal::removeAccelMark(m_pTabWidgetSystem->tabText(1));

        /* VCPU amount test: */
        int cTotalCPUs = nemuGlobal().host().GetProcessorOnlineCoreCount();
...
void UIMachineSettingsSystem::prepareTabMotherboard()
{
    ...
    nemuGlobal().setMinimumWidthAccordingSymbolCount(m_pEditorMemorySize, 5);
...
void UIMachineSettingsSystem::prepareTabProcessor()
{
    /* Load configuration: */
    CSystemProperties properties = nemuGlobal().virtualBox().GetSystemProperties();
    uint hostCPUs = nemuGlobal().host().GetProcessorOnlineCoreCount();
    ...
    nemuGlobal().setMinimumWidthAccordingSymbolCount(m_pEditorCPUCount, 4);
    ...
    nemuGlobal().setMinimumWidthAccordingSymbolCount(m_pEditorCPUExecCap, 4);
