/** @file
 *
 * nemusf -- VirtualBox Guest Additions for Linux:
...
MODULE_DESCRIPTION(NEMU_PRODUCT " VFS Module for Host File System Access");
MODULE_AUTHOR(NEMU_VENDOR);
MODULE_LICENSE("GPL");
#ifdef MODULE_VERSION
MODULE_VERSION(NEMU_VERSION_STRING " (interface " RT_XSTR(VMMDEV_VERSION) ")");
#endif
...
static int sf_glob_alloc(struct vbsf_mount_info_new *info, struct sf_glob_info **sf_gp)
{
    ...
    {
        /* An old version of mount.nemusf made the syscall. Translate the
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 0)
static DECLARE_FSTYPE(nemusf_fs_type, "nemusf", sf_read_super_24, 0);
#else
static int
...
static struct file_system_type nemusf_fs_type =
{
    .owner   = THIS_MODULE,
    .name    = "nemusf",
...
static int __init init(void)
{
    ...
    err = register_filesystem(&nemusf_fs_type);
    if (err)
    {
    ...
    rcNemu = VbglR0SfConnect(&client_handle);
    if (RT_FAILURE(rcNemu))
    {
    ...
    rcNemu = VbglR0SfSetUtf8(&client_handle);
    if (RT_FAILURE(rcNemu))
    {
    ...
    if (!follow_symlinks)
    {
        rcNemu = VbglR0SfSetSymlinks(&client_handle);
        if (RT_FAILURE(rcNemu))
        {
            printk(KERN_WARNING
                     "nemusf: Host unable to show symlinks, rc=%d\n",
                     rcNemu);
        }
    }
#endif
    ...
    printk(KERN_DEBUG
            "nemusf: Successfully loaded version " NEMU_VERSION_STRING
...
fail0:
    unregister_filesystem(&nemusf_fs_type);
    return rcRet;
}
