/* $Id: NemuGuest-win.cpp $ */
/** @file
 * NemuGuest - Windows specifics.
 */
...
static NTSTATUS vgdrvNtScanPCIResourceList(PCM_RESOURCE_LIST pResList, PNEMUGUESTDEVEXTWIN pDevExt);
static NTSTATUS vgdrvNtMapVMMDevMemory(PNEMUGUESTDEVEXTWIN pDevExt, PHYSICAL_ADDRESS PhysAddr, ULONG cbToMap,
...
ULONG DriverEntry(PDRIVER_OBJECT pDrvObj, PUNICODE_STRING pRegPath)
{
    ...
    /* Use RTLogBackdoorPrintf to make sure that this goes to Nemu.log */
    RTLogBackdoorPrintf("NemuGuest: Windows version %u.%u, build %u\n", ulMajorVer, ulMinorVer, ulBuildNo);
    if (fCheckedBuild)
        RTLogBackdoorPrintf("NemuGuest: Windows checked build\n");
 ...
 *
 * @remarks Parts of this is duplicated in NemuGuest-win-legacy.cpp.
 */
static NTSTATUS vgdrvNtAddDevice(PDRIVER_OBJECT pDrvObj, PDEVICE_OBJECT pDevObj)
{
    NTSTATUS rc;
    ...
    RtlInitUnicodeString(&DevName, NEMUGUEST_DEVICE_NAME_NT);
    PDEVICE_OBJECT pDeviceObject = NULL;
    rc = IoCreateDevice(pDrvObj, sizeof(NEMUGUESTDEVEXTWIN), &DevName, FILE_DEVICE_UNKNOWN, 0, FALSE, &pDeviceObject);
    if (NT_SUCCESS(rc))
    {
        /*
        ...
        RtlInitUnicodeString(&DosName, NEMUGUEST_DEVICE_NAME_DOS);
        ...
        if (NT_SUCCESS(rc))
        {
             ...
             */
            PNEMUGUESTDEVEXTWIN pDevExt = (PNEMUGUESTDEVEXTWIN)pDeviceObject->DeviceExtension;
            RT_ZERO(*pDevExt);
            ...
            if (pDevExt->pNextLowerDriver != NULL)
            {
                ...
                if (NT_SUCCESS(rc))
                {
                    /* NemuGuestPower is pageable; ensure we are not called at elevated IRQL */
...
#endif
{
    PNEMUGUESTDEVEXTWIN pDevExt = (PNEMUGUESTDEVEXTWIN)pDevObj->DeviceExtension;
#ifndef TARGET_NT4
     ...
     */
    LogFlowFunc(("Starting to scan PCI resources of NemuGuest ...\n"));

    /* Assign the PCI resources. */
    PCM_RESOURCE_LIST pResourceList = NULL;
    UNICODE_STRING classNameString;
    RtlInitUnicodeString(&classNameString, L"NemuGuestAdapter");
...
# endif
    ...
    {
        ULONG uValue = 0;
        NTSTATUS rcNt2 = vgdrvNtRegistryReadDWORD(RTL_REGISTRY_SERVICES, L"NemuGuest", L"LoggingEnabled", &uValue);
        if (NT_SUCCESS(rcNt2))
        {
        ...
        NEMUGUEST_UPDATE_DEVSTATE(pDevExt, VGDRVNTDEVSTATE_WORKING);
    }
    else
...
static void vgdrvNtUnload(PDRIVER_OBJECT pDrvObj)
{
    ...
    if (pDrvObj->DeviceObject && pDrvObj->DeviceObject->DeviceExtension)
        VGDrvCommonDeleteDevExt((PNEMUGUESTDEVEXT)pDrvObj->DeviceObject->DeviceExtension);
...
static NTSTATUS vgdrvNtClose(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    ...
    PNEMUGUESTSESSION pSession = (PNEMUGUESTSESSION)pFileObj->FsContext;
    if (pSession)
...
static NTSTATUS vgdrvNtIOCtl(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    ...
    if (pFileObj) /* ... then we might have a session object as well! */
        pSession = (PNEMUGUESTSESSION)pFileObj->FsContext;
...
static NTSTATUS vgdrvNtInternalIOCtl(PDEVICE_OBJECT pDevObj, PIRP pIrp)
{
    ...
    switch (uCmd)
    {
        case NEMUGUEST_IOCTL_SET_MOUSE_NOTIFY_CALLBACK:
        {
            ...
            fProcessed = true;
            if (cbData != sizeof(NemuGuestMouseSetNotifyCallback))
            {
                AssertFailed();
                Status = STATUS_INVALID_PARAMETER;
                break;
            ...
            NemuGuestMouseSetNotifyCallback *pInfo = (NemuGuestMouseSetNotifyCallback*)pvBuf;
...
{
    PNEMUGUESTDEVEXTWIN pDevExt = (PNEMUGUESTDEVEXTWIN)pServiceContext;
    if (pDevExt == NULL)
        return FALSE;
...
static NTSTATUS vgdrvNtScanPCIResourceList(PCM_RESOURCE_LIST pResList, PNEMUGUESTDEVEXTWIN pDevExt)
{
    ...
    ULONG rangeCount = 0;
    ULONG cMMIORange = 0;
    PNEMUGUESTWINBASEADDRESS pBaseAddress = pDevExt->pciBaseAddress;
            ...
            case CmResourceTypeMemory:
            {
                ...
                if (rangeCount < PCI_TYPE0_ADDRESSES)
                {
                    ...
                    if (   cMMIORange == 0 /* Only care about the first MMIO range (!!!). */
                        && (pPartialData->Flags & NEMU_CM_PRE_VISTA_MASK) == CM_RESOURCE_MEMORY_READ_WRITE)
                    {
 ...
 * @param   pDevExt     The device extension.
 */
void vgdrvNtUnmapVMMDevMemory(PNEMUGUESTDEVEXTWIN pDevExt)
{
...
/**
 * Translates NT version to Nemu OS.
 *
 * @returns Nemu OS type.
 * @param   enmNtVer            The NT version.
 */
NEMUOSTYPE vgdrvNtVersionToOSType(VGDRVNTVER enmNtVer)
{
    NEMUOSTYPE enmOsType;
    switch (enmNtVer)
    {
        case VGDRVNTVER_WINNT4:
            enmOsType = NEMUOSTYPE_WinNT4;
            break;

        case VGDRVNTVER_WIN2K:
            enmOsType = NEMUOSTYPE_Win2k;
            break;

        case VGDRVNTVER_WINXP:
#if ARCH_BITS == 64
            enmOsType = NEMUOSTYPE_WinXP_x64;
#else
            enmOsType = NEMUOSTYPE_WinXP;
#endif
            break;

        case VGDRVNTVER_WIN2K3:
#if ARCH_BITS == 64
            enmOsType = NEMUOSTYPE_Win2k3_x64;
#else
            enmOsType = NEMUOSTYPE_Win2k3;
#endif
            break;

        case VGDRVNTVER_WINVISTA:
#if ARCH_BITS == 64
            enmOsType = NEMUOSTYPE_WinVista_x64;
#else
            enmOsType = NEMUOSTYPE_WinVista;
#endif
            break;

        case VGDRVNTVER_WIN7:
#if ARCH_BITS == 64
            enmOsType = NEMUOSTYPE_Win7_x64;
#else
            enmOsType = NEMUOSTYPE_Win7;
#endif
            break;

        case VGDRVNTVER_WIN8:
#if ARCH_BITS == 64
            enmOsType = NEMUOSTYPE_Win8_x64;
#else
            enmOsType = NEMUOSTYPE_Win8;
#endif
            break;

        case VGDRVNTVER_WIN81:
#if ARCH_BITS == 64
            enmOsType = NEMUOSTYPE_Win81_x64;
#else
            enmOsType = NEMUOSTYPE_Win81;
#endif
            break;

        case VGDRVNTVER_WIN10:
#if ARCH_BITS == 64
            enmOsType = NEMUOSTYPE_Win10_x64;
#else
            enmOsType = NEMUOSTYPE_Win10;
#endif
            break;

        default:
            /* We don't know, therefore NT family. */
            enmOsType = NEMUOSTYPE_WinNT;
            break;
    }
    return enmOsType;
}
...
int VGDrvNtIOCtl_DpcLatencyChecker(void)
{
     ...
     */
    DPCDATA *pData = (DPCDATA *)ExAllocatePoolWithTag(NonPagedPool, sizeof(DPCDATA), NEMUGUEST_DPC_TAG);
    if (!pData)
    {
        RTLogBackdoorPrintf("NemuGuest: DPC: DPCDATA allocation failed.\n");
    ...
    ExFreePoolWithTag(pData, NEMUGUEST_DPC_TAG);
    return VINF_SUCCESS;
}
