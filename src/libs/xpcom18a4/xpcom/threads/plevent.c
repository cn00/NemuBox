...
struct PLEventQueue {
    ...
    CFStringRef         mRunLoopModeStr;                                                                /* nemu */
...
_pl_CleanupNativeNotifier(PLEventQueue* self)
{
    ...
    CFRunLoopRemoveSource(self->mMainRunLoop, self->mRunLoopSource, self->mRunLoopModeStr);             /* nemu */
    CFRelease(self->mRunLoopSource);
    CFRelease(self->mMainRunLoop);
    CFRelease(self->mRunLoopModeStr);                                                                   /* nemu */
...
_pl_AcknowledgeNativeNotify(PLEventQueue* self)
{
    ...
    */
    ...
    if ((count == -1) && ((errno == EAGAIN) || (errno == EWOULDBLOCK)))
        return PR_SUCCESS;
    return PR_FAILURE;
#elif defined(MAC_USE_CFRUNLOOPSOURCE)                                                                  /* nemu */
                                                                                                        /* nemu */
    CFRunLoopRunInMode(self->mRunLoopModeStr, 0.0, 1);                                                  /* nemu */
    return PR_SUCCESS;                                                                                  /* nemu */
#else
...
static void _md_CreateEventQueue( PLEventQueue *eventQueue )
{
    ...
    {                                                                                                   /* nemu */
        char szModeName[80];                                                                            /* nemu */
        snprintf(szModeName, sizeof(szModeName), "NemuXPCOMQueueMode-%p", eventQueue);                  /* nemu */
        eventQueue->mRunLoopModeStr = CFStringCreateWithCString(kCFAllocatorDefault,                    /* nemu */
                                                                szModeName, kCFStringEncodingASCII);    /* nemu */
        CFRunLoopAddSource(eventQueue->mMainRunLoop,                                                    /* nemu */
                           eventQueue->mRunLoopSource, eventQueue->mRunLoopModeStr);                    /* nemu */
    }                                                                                                   /* nemu */
