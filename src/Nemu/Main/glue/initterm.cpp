 ...
 *  We get down to [NemuNsxp]NS_NewNativeLocalFile and that file isn't
 ...
 * XPCOM runtime:
 *
 * 1. If the NEMU_APP_HOME environment variable is set, the path it specifies
 ...
 *
 * 2. If NEMU_APP_HOME is not set, this methods tries the following paths in the
 *    given order:
 *
...
HRESULT Initialize(bool fGui)
{
    ...
    for (size_t i = 0; i < RT_ELEMENTS(kAppPathsToProbe); ++ i)
    {
        ...
        if (i == 0)
        {
            /* Use NEMU_APP_HOME if present */
            vrc = RTEnvGetEx(RTENV_DEFAULT, "NEMU_APP_HOME", szAppHomeDir, sizeof(szAppHomeDir), NULL);
        ...
        /* Set NEMU_XPCOM_HOME to the same app path to make XPCOM sources that
         * still use it instead of the directory service happy */
        vrc = RTEnvSetEx(RTENV_DEFAULT, "NEMU_XPCOM_HOME", szAppHomeDir);
        AssertRC(vrc);
        ...
        if (i == 0)
        {
            /* We failed with NEMU_APP_HOME, don't probe other paths */
            break;
        }
    }
