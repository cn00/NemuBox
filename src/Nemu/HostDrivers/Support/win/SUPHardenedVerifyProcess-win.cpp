...
#endif
#ifdef NEMU_PERMIT_MORE
# define NEMU_PERMIT_MORE_FIRST_IDX 5
    "sfc.dll",
    "sfc_os.dll",
    "user32.dll",
    "acres.dll",
...
/**
 * Nemu executables allowed to start VMs.
    ...
    for (uint32_t i = 0; i < cSections; i++)
    {
        ...
        if (!pImage->fApiSetSchemaOnlySection1 || i == 0)
        {
            ...
            /* The section bits. Child purification verifies all, normal
               ...
               concerned (due to opening nemudrv during early process init). */
...
#ifndef NEMU_PERMIT_VISUAL_STUDIO_PROFILING
...
# ifdef NEMU_PERMIT_MORE
            if (g_uNtVerCombined < SUP_NT_VER_W70 && i >= NEMU_PERMIT_MORE_FIRST_IDX)
                pImage->pszName = NULL; /* hard limit: user32.dll is unwanted prior to w7. */
# endif

#endif /* NEMU_PERMIT_VISUAL_STUDIO_PROFILING */
            break;
        }
    if (!pImage->pszName)
    {
    ...
    if (NT_SUCCESS(rcNt) && (pThis->fFlags & SUPHARDNTVP_F_EXEC_ALLOC_REPLACE_WITH_RW))
    {
        ...
        if (!NT_SUCCESS(rcNt))
        {
            supHardNtVpSetInfo2(pThis, VERR_SUP_VP_REPLACE_VIRTUAL_MEMORY_FAILED,
                                ...
                                "See NemuStartup.log for more details",
                                pvAlloc, cbFree, rcNt);
            supR3HardenedLogFlush();
 ...
 * after respawning and the second is when we open the nemudrv device for
 * unrestricted access.
 */
