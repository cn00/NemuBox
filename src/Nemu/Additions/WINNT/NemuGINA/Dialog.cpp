/* $Id: Dialog.cpp $ */
/** @file
 * NemuGINA - Windows Logon DLL for VirtualBox, Dialog Code.
 */
...
void hookDialogBoxes(PVOID pWinlogonFunctions, DWORD dwWlxVersion)
{
    ...
    NemuGINAVerbose(0, "NemuGINA::hookDialogBoxes\n");
    ...
    switch (dwWlxVersion)
    {
        ...
        default:
        {
            NemuGINAVerbose(0, "NemuGINA::hookDialogBoxes: unrecognized version '%d', nothing hooked!\n", dwWlxVersion);
                    ...
                    PCRTUTF16 pwszUser, PCRTUTF16 pwszPassword, PCRTUTF16 pwszDomain)
{
    ...
    if (hwndDomain)
    {
        ...
        NemuGINAVerbose(0, "NemuGINA::MyWlxLoggedOutSASDlgProc: Trying to find domain entry in combo box ...\n");
        DWORD dwIndex = (DWORD) SendMessage(hwndDomain, CB_FINDSTRING,
                                            0, (LPARAM)pwszDomain);
        if (dwIndex != CB_ERR)
        {
            NemuGINAVerbose(0, "NemuGINA::MyWlxLoggedOutSASDlgProc: Found domain at combo box pos %ld\n", dwIndex);
            SendMessage(hwndDomain, CB_SETCURSEL, (WPARAM) dwIndex, 0);
            EnableWindow(hwndDomain, FALSE);
        }
        else
        {
            NemuGINAVerbose(0, "NemuGINA::MyWlxLoggedOutSASDlgProc: Domain not found in combo box ...\n");
            ...
            if (l > 255)
                NemuGINAVerbose(0, "NemuGINA::MyWlxLoggedOutSASDlgProc: Warning! FQDN (domain) is too long (max 255 bytes), will be truncated!\n");
            ...
            if (wcslen(pwszUser) > 0) /* We need a user name that we can use in caes of a FQDN */
            {
                ...
                {
                    NemuGINAVerbose(0, "NemuGINA::MyWlxLoggedOutSASDlgProc: Domain seems to be a FQDN (length)!\n");
                    bIsFQDN = TRUE;
                }
                else if (   l > 0
                         && wcsstr(pwszDomain, L".") != NULL) /* if we found a dot (.) in the domain name, this has to be a FQDN */
                {
                    NemuGINAVerbose(0, "NemuGINA::MyWlxLoggedOutSASDlgProc: Domain seems to be a FQDN (dot)!\n");
                    bIsFQDN = TRUE;
                }
                ...
                if (bIsFQDN)
                {
                    ...
                    NemuGINAVerbose(0, "NemuGINA::MyWlxLoggedOutSASDlgProc: FQDN user name is now: %s!\n", szUserFQDN);
                }
            }
        }
    }
                      ...
                      WORD wButtonToPress)
{
    ...
    if (!NemuGINAHandleCurrentSession())
        rc = VERR_NOT_FOUND;
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_FAILURE(rc))
        {
            if (rc != VERR_NOT_FOUND)
                NemuGINAVerbose(0, "NemuGINA::credentialsHandle: error querying for credentials, rc=%Rrc\n", rc);
        }
    }

    if (RT_SUCCESS(rc))
    {
        NemuGINAVerbose(0, "NemuGINA::credentialsHandle: credentials available\n");
         ...
         */
        NemuGINAReportStatus(NemuGuestFacilityStatus_Terminating);
        ...
        {
#ifdef DEBUG
            NemuGINAVerbose(0, "NemuGINA::credentialsHandle: retrieved credentials: user=%ls, password=%ls, domain=%ls\n",
                            pwszUser, pwszPassword, pwszDomain);
#else
            NemuGINAVerbose(0, "NemuGINA::credentialsHandle: retrieved credentials: user=%ls, password=XXX, domain=%ls\n",
                            pwszUser, pwszDomain);
#endif
...
#ifdef DEBUG
    NemuGINAVerbose(3, "NemuGINA::credentialsHandle: returned with rc=%Rrc\n", rc);
#endif
    return rc;
}
                                          ...
                                          LPARAM lParam)   // second message parameter
{
    ...
    /*NemuGINAVerbose(0, "NemuGINA::MyWlxLoggedOutSASDlgProc\n");*/
    ...
    switch (uMsg)
    {
        case WM_INITDIALOG:
        {
            NemuGINAVerbose(0, "NemuGINA::MyWlxLoggedOutSASDlgProc: got WM_INITDIALOG\n");
            ...
            NemuGINAVerbose(0, "NemuGINA::MyWlxLoggedOutSASDlgProc: hwndUserId: %x, hwndPassword: %d, hwndDomain: %d\n",
                            s_hwndUserId, s_hwndPassword, s_hwndDomain);

            /* terminate the credentials poller thread, it's done is job */
            NemuGINACredentialsPollerTerminate();
            ...
            if (RT_FAILURE(rc))
            {
                ...
                if (!uTimer)
                    NemuGINAVerbose(0, "NemuGINA::MyWlxLoggedOutSASDlgProc: failed creating timer! Last error: %ld\n",
                                    GetLastError());
            }
            break;
        }
                                       ...
                                       LPARAM lParam)   // second message parameter
{
    ...
    /*NemuGINAVerbose(0, "NemuGINA::MyWlxLockedSASDlgProc\n");*/
    ...
    switch (uMsg)
    {
        case WM_INITDIALOG:
        {
            NemuGINAVerbose(0, "NemuGINA::MyWlxLockedSASDlgProc: WM_INITDIALOG\n");

            /* get the entry fields */
            s_hwndPassword = GetDlgItem(hwndDlg, IDC_WKSTALOCKED_PASSWORD);
            NemuGINAVerbose(0, "NemuGINA::MyWlxLockedSASDlgProc: hwndPassword: %d\n", s_hwndPassword);
            ...
            if (RT_FAILURE(rc))
            {
                ...
                if (!uTimer)
                    NemuGINAVerbose(0, "NemuGINA::MyWlxLockedSASDlgProc: failed creating timer! Last error: %ld\n",
                         GetLastError());
            }
            break;
        }
        ...
        case WM_DESTROY:
        {
            NemuGINAVerbose(0, "NemuGINA::MyWlxLockedSASDlgProc: WM_DESTROY\n");
...
int WINAPI MyWlxDialogBoxParam(HANDLE  hWlx,
                               ...
                               LPARAM  dwInitParam)
{
    NemuGINAVerbose(0, "NemuGINA::MyWlxDialogBoxParam: lpszTemplate=%ls\n", lpszTemplate);

    NemuGINAReportStatus(NemuGuestFacilityStatus_Active);
    ...
    if (!HIWORD((int)(void*)lpszTemplate))
    {
        ...
        switch ((DWORD) lpszTemplate)
        {
            case IDD_WLXDIAPLAYSASNOTICE_DIALOG:
                NemuGINAVerbose(0, "NemuGINA::MyWlxDialogBoxParam: SAS notice dialog displayed; not handled\n");
                break;

            case IDD_WLXLOGGEDOUTSAS_DIALOG:     /* Windows NT 4.0. */
            case IDD_WLXLOGGEDOUTSAS_DIALOG2:    /* Windows 2000 and up. */
            {
                NemuGINAVerbose(0, "NemuGINA::MyWlxDialogBoxParam: returning hooked SAS logged out dialog\n");
            ...
            case IDD_SECURITY_DIALOG:
                NemuGINAVerbose(0, "NemuGINA::MyWlxDialogBoxParam: Security dialog displayed; not handled\n");
                break;

            case IDD_WLXWKSTALOCKEDSAS_DIALOG:   /* Windows NT 4.0. */
            case IDD_WLXWKSTALOCKEDSAS_DIALOG2:  /* Windows 2000 and up. */
            {
                NemuGINAVerbose(0, "NemuGINA::MyWlxDialogBoxParam: returning hooked SAS locked dialog\n");
            ...
            default:
                NemuGINAVerbose(0, "NemuGINA::MyWlxDialogBoxParam: dialog %ld not handled\n", (DWORD)lpszTemplate);
                break;
        }
    }
