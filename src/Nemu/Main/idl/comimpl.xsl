 ...
 * Generator: src/Nemu/Main/idl/comimpl.xsl
 */
...
<xsl:template name="genImplList">
  ...
  <xsl:choose>
    <xsl:when test="$name='IEvent'">
      <xsl:value-of select="       '#ifdef NEMU_WITH_XPCOM&#10;'" />
...
<xsl:template name="genEventImpl">
  ...
  <xsl:value-of select="concat('        NEMU_DEFAULT_INTERFACE_ENTRIES(', @name, ')&#10;')" />
  ...
  <xsl:text><![CDATA[
    ...
    STDMETHOD(COMGETTER(Type))(NemuEventType_T *aType)
    {
...
<xsl:text><![CDATA[
    HRESULT init(IEventSource* aSource, NemuEventType_T aType, BOOL aWaitable = TRUE)
    {
        NOREF(aWaitable);
...
private:
    ComObjPtr<NemuVetoEvent>      mEvent;
]]></xsl:text>
    </xsl:when>
    <xsl:when test="$isReusable='yes'">
      <xsl:text>
<![CDATA[
    HRESULT init(IEventSource* aSource, NemuEventType_T aType, BOOL aWaitable = FALSE)
    {
        mGeneration = 1;
...
<xsl:template name="genSwitchCase">
  ...
  <xsl:value-of select="concat('         case NemuEventType_', @id, ':&#10;')"/>
  <xsl:value-of select="       '         {&#10;'"/>
  <xsl:choose>
...
<xsl:template name="genCommonEventCode">
  <xsl:call-template name="fileheader">
    <xsl:with-param name="name" select="'NemuEvents.cpp'" />
  </xsl:call-template>
...
#include "EventImpl.h"
  ...
  <xsl:for-each select="//interface[@autogen=$G_kind]">
    ...
    <xsl:choose>
      <xsl:when test="$G_kind='NemuEvent'">
  ...
  <xsl:text><![CDATA[
HRESULT NemuEventDesc::init(IEventSource *aSource, NemuEventType_T aType, ...)
{
    va_list args;
 ...
 <xsl:text><![CDATA[
HRESULT NemuEventDesc::reinit(NemuEventType_T aType, ...)
{
    va_list args;
...
<xsl:template name="genCommonEventHeader">
  <xsl:call-template name="fileheader">
    <xsl:with-param name="name" select="'NemuEvents.h'" />
  </xsl:call-template>
...
#include "EventImpl.h"
  ...
  <!-- Interfaces -->
  <xsl:for-each select="//interface[@autogen='NemuEvent']">
    ...
    <xsl:value-of select="       '    NemuEventDesc evDesc;&#10;'"/>
    <xsl:value-of select="concat('    evDesc.init(aSource, NemuEventType_',$evid)"/>
    <xsl:call-template name="genFactParams">
      <xsl:with-param name="name" select="$ifname" />
    </xsl:call-template>
    <xsl:value-of select="');&#10;'"/>
...
<xsl:template match="/">
  <!-- Global code -->
   <xsl:choose>
      ...
      <xsl:when test="$G_kind='NemuEventHeader'">
