...
# endif
# include <Nemu/log.h>
# include <Nemu/vmm/pgm.h> /* PGM_DYNAMIC_RAM_ALLOC */
#endif /* NEMU */
...
#if 1
# define NEMU_RAMLIST_DIRTY_BOUNDS_CHECK_RET(addr,rv) \
    do { \
...
# define NEMU_RAMLIST_DIRTY_BOUNDS_CHECK_RETV(addr) \
    do { \
        if (RT_UNLIKELY( ((addr) >> TARGET_PAGE_BITS) >= ram_list.phys_dirty_size)) { \
            Log(("%s: %RGp\n", __FUNCTION__, (RTGCPHYS)addr)); \
            return; \
...
# endif
#else
# define NEMU_RAMLIST_DIRTY_BOUNDS_CHECK_RET(addr,rv) do {} while()
# define NEMU_RAMLIST_DIRTY_BOUNDS_CHECK_RETV(addr) do {} while()
#endif /* NEMU */
...
{
    NEMU_RAMLIST_DIRTY_BOUNDS_CHECK_RET(addr, 0);
    return ram_list.phys_dirty[addr >> TARGET_PAGE_BITS] == 0xff;
}

static inline int cpu_physical_memory_get_dirty_flags(ram_addr_t addr)
{
    NEMU_RAMLIST_DIRTY_BOUNDS_CHECK_RET(addr, 0xff);
...
static inline int cpu_physical_memory_get_dirty(ram_addr_t addr,
                                                int dirty_flags)
{
    NEMU_RAMLIST_DIRTY_BOUNDS_CHECK_RET(addr, 0xff & dirty_flags);
    return ram_list.phys_dirty[addr >> TARGET_PAGE_BITS] & dirty_flags;
}

static inline void cpu_physical_memory_set_dirty(ram_addr_t addr)
{
    NEMU_RAMLIST_DIRTY_BOUNDS_CHECK_RETV(addr);
                                                        ...
                                                        int dirty_flags)
{
    ...
    NEMU_RAMLIST_DIRTY_BOUNDS_CHECK_RETV(start);
