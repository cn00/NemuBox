...
PDMBOTHCBDECL(void) ioapicSendMsi(PPDMDEVINS pDevIns, RTGCPHYS GCAddr, uint32_t uValue, uint32_t uTagSrc)
{
    ...
    uint8_t  dest = (GCAddr & NEMU_MSI_ADDR_DEST_ID_MASK) >> NEMU_MSI_ADDR_DEST_ID_SHIFT;
    uint8_t  vector_num = (uValue & NEMU_MSI_DATA_VECTOR_MASK) >> NEMU_MSI_DATA_VECTOR_SHIFT;
    uint8_t  dest_mode = (GCAddr >> NEMU_MSI_ADDR_DEST_MODE_SHIFT) & 0x1;
    uint8_t  trigger_mode = (uValue >> NEMU_MSI_DATA_TRIGGER_SHIFT) & 0x1;
    uint8_t  delivery_mode = (uValue >> NEMU_MSI_DATA_DELIVERY_MODE_SHIFT) & 0x7;
#if 0
    /*
     ...
     */
    uint8_t  redir_hint = (GCAddr >> NEMU_MSI_ADDR_REDIRECTION_SHIFT) & 0x1;
#endif
