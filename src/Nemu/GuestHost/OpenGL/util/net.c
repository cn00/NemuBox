        ...
        )
{
    ...
    else if (!crStrcmp(protocol, "nemuhgcm"))
    {
        cr_net.use_hgcm++;
        crNemuHGCMInit(cr_net.recv_list, cr_net.close_list, mtu);
        crNemuHGCMConnection(conn
#if defined(NEMU_WITH_CRHGSMI) && defined(IN_GUEST)
                    , pHgsmi
#endif
                );
...
void crNetInit( CRNetReceiveFunc recvFunc, CRNetCloseFunc closeFunc )
{
    ...
    {
#ifdef WINDOWS
        /* @todo: do we actually need that WSA stuff with Nemu at all? */
...
void crNetTearDown()
{
     ...
     * but Nemu doesn't use them, so no other checks.
     */  
    if (cr_net.use_hgcm)
        crNemuHGCMTearDown();
