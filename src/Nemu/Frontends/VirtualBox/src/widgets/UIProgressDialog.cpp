...
/** @file
 * Nemu Qt GUI - UIProgressDialog class implementation.
 */
...
void UIProgressDialog::handleTimerEvent()
{
    ...
    if (!m_progress.GetCanceled())
    {
        ...
        QString strDays = NemuGlobal::daysToString(days);
        QString strHours = NemuGlobal::hoursToString(hours);
        QString strMinutes = NemuGlobal::minutesToString(minutes);
        QString strSeconds = NemuGlobal::secondsToString(seconds);
