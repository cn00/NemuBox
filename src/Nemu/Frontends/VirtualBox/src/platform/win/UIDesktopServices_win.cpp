...
/** @file
 * Nemu Qt GUI - Qt GUI - Utility Classes and Functions specific to Windows..
 */
...
bool UIDesktopServices::createMachineShortcut(const QString & /* strSrcFile */, const QString &strDstPath, const QString &strName, const QString &strUuid)
{
    ...
    QString strNemu = QDir::toNativeSeparators(QCoreApplication::applicationFilePath());
    QFileInfo fi(strNemu);
    QString strNemuDir = QDir::toNativeSeparators(fi.absolutePath());
    HRESULT rc = CoCreateInstance(CLSID_ShellLink, NULL, CLSCTX_INPROC_SERVER, IID_IShellLink, (void**)(&pShl));
    if (FAILED(rc))
        return false;
    do
    {
        rc = pShl->SetPath(strNemu.utf16());
        if (FAILED(rc))
            break;
        rc = pShl->SetWorkingDirectory(strNemuDir.utf16());
        if (FAILED(rc))
            break;
