...
*   Global Variables                                                                                                             *
*********************************************************************************************************************************/
#if defined(LOG_ENABLED) && defined(NEMU_WITH_RAW_MODE_NOT_R0)
...
#  else  /* IN_RC: */
    ...
    NEMUSTRICTRC rcStrict = selmRCGuestGdtPostWriteCheck(pVM, pVCpu, offGuestGdt, cbBuf, pCtx);
                                                   ...
                                                   RTGCPTR GCPtrDesc, RTSEL const Sel, uint32_t const iSReg)
{
     ...
     */
    X86DESC GstDesc;
    NEMUSTRICTRC rcStrict = PGMPhysReadGCPtr(pVCpu, &GstDesc, GCPtrDesc, sizeof(GstDesc), PGMACCESSORIGIN_SELM);
    ...
    else
    {
        ...
        Log(("SELMLoadHiddenSelectorReg: Error reading descriptor %s=%#x: %Rrc\n",
             g_aszSRegNms[iSReg], Sel, NEMUSTRICTRC_VAL(rcStrict) ));
...
l_tryagain:
        PNEMUTSS pTss = (PNEMUTSS)(uintptr_t)GCPtrTss;
...
# else /* !IN_RC */
        ...
        rc = PGMPhysSimpleReadGCPtr(pVCpu, &tss, GCPtrTss, sizeof(NEMUTSS));
        if (RT_FAILURE(rc))
        {
...
#if defined(NEMU_WITH_RAW_MODE) || (HC_ARCH_BITS != 64 && defined(NEMU_WITH_64_BITS_GUESTS))
...
#endif /* defined(NEMU_WITH_RAW_MODE) || (HC_ARCH_BITS != 64 && defined(NEMU_WITH_64_BITS_GUESTS)) */
