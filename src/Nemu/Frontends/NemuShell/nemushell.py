...
def printErr(_ctx, e):
    oNemuMgr = _ctx['global'];
    if oNemuMgr.errIsOurXcptKind(e):
        print colored('%s: %s' % (oNemuMgr.xcptToString(e), oNemuMgr.xcptGetMessage(e)), 'red');
    else:
...
def createVm(ctx, name, kind):
    nemu = ctx['vb']
    mach = nemu.createMachine("", name, [], kind, "")
    mach.saveSettings()
    print "created machine with UUID", mach.id
    nemu.registerMachine(mach)
    # update cache
...
def startVm(ctx, mach, vmtype):
    nemu = ctx['vb']
    perf = ctx['perf']
    session = ctx['global'].getSessionObject(nemu)
...
def monitorSource(ctx, eventSource, active, dur):
    def handleEventImpl(event):
        evtype = event.type
        print "got event: %s %s" % (str(evtype), asEnumElem(ctx, 'NemuEventType', evtype))
        if evtype == ctx['global'].constants.NemuEventType_OnMachineStateChanged:
        ...
        elif  evtype == ctx['global'].constants.NemuEventType_OnSnapshotTaken:
        ...
        elif  evtype == ctx['global'].constants.NemuEventType_OnGuestPropertyChanged:
        ...
        elif  evtype == ctx['global'].constants.NemuEventType_OnMousePointerShapeChanged:
        ...
        elif evtype == ctx['global'].constants.NemuEventType_OnGuestMouse:
            mev = ctx['global'].queryInterface(event, 'IGuestMouseEvent')
            if mev:
                printMouseEvent(ctx, mev)
        elif evtype == ctx['global'].constants.NemuEventType_OnGuestKeyboard:
            kev = ctx['global'].queryInterface(event, 'IGuestKeyboardEvent')
            if kev:
                printKbdEvent(ctx, kev)
        elif evtype == ctx['global'].constants.NemuEventType_OnGuestMultiTouch:
    ...
    if dur == -1:
        ...
        dur = 100000
    try:
        eventSource.registerListener(listener, [ctx['global'].constants.NemuEventType_Any], active)
        registered = True
        end = time.time() + dur
        while  time.time() < end:
            if active:
...
def recordDemo(ctx, console, filename, dur):
    ...
    def handleEventImpl(event):
        evtype = event.type
        #print "got event: %s %s" % (str(evtype), asEnumElem(ctx, 'NemuEventType', evtype))
        if evtype == ctx['global'].constants.NemuEventType_OnGuestMouse:
            mev = ctx['global'].queryInterface(event, 'IGuestMouseEvent')
            if mev:
    ...
    if dur == -1:
        ...
        dur = 100000
    try:
        agg.registerListener(listener, [ctx['global'].constants.NemuEventType_Any], False)
        registered = True
        end = time.time() + dur
        while  time.time() < end:
...
class XPathNodeHolderVM(XPathNodeHolder):
    def __init__(self, parent, nemu):
        XPathNodeHolder.__init__(self, parent, nemu, 'machines', XPathNodeVM, 'vms')
...
def execInGuest(ctx, console, args, env, user, passwd, tmo, inputPipe=None, outputPipe=None):
    if len(args) < 1:
        ...
        return
    guest = console.guest
    guestSession = guest.createSession(user, passwd, "", "nemushell guest exec")
...
def hostCmd(ctx, args):
    nemu = ctx['vb']
    try:
        print "VirtualBox version %s" % (colored(nemu.version, 'blue'))
    except Exception, e:
        printErr(ctx, e)
        if g_fVerbose:
            traceback.print_exc()
    props = nemu.systemProperties
    print "Machines: %s" % (colPath(ctx, props.defaultMachineFolder))

    #print "Global shared folders:"
    #for ud in ctx['global'].getArray(nemu, 'sharedFolders'):
    #    printSf(ctx, sf)
    host = nemu.host
...
def monitorNemuCmd(ctx, args):
    if (len(args) > 2):
        print "usage: monitorNemu (duration)"
        return 0
    dur = 5
    if len(args) > 1:
        dur = float(args[1])
    nemu = ctx['vb']
    active = False
    monitorSource(ctx, nemu.eventSource, active, dur)
    return 0
...
def portForwardCmd(ctx, args):
    ...
    config = "NemuInternal/Devices/" + adapterType + "/"
...
def connectCmd(ctx, args):
    ...
    nemu = ctx['global'].platform.connect(url, user, passwd)
    ctx['vb'] = nemu
    try:
        print "Running VirtualBox version %s" % (nemu.version)
    except Exception, e:
        printErr(ctx, e)
        if g_fVerbose:
            traceback.print_exc()
...
def registerHddCmd(ctx, args):
    ...
    setParentId = False
    parentId = ""
    hdd = nemu.openMedium(loc, ctx['global'].constants.DeviceType_HardDisk, ctx['global'].constants.AccessMode_ReadWrite, False)
...
def registerIsoCmd(ctx, args):
    ...
    nemu = ctx['vb']
    loc = args[1]
    iso = nemu.openMedium(loc, ctx['global'].constants.DeviceType_DVD, ctx['global'].constants.AccessMode_ReadOnly, False)
...
def unregisterIsoCmd(ctx, args):
    ...
    loc = args[1]
    try:
        dvd = nemu.openMedium(loc, ctx['global'].constants.DeviceType_DVD, ctx['global'].constants.AccessMode_ReadOnly, False)
    except:
...
def guiCmd(ctx, args):
    ...
    nemu = os.path.join(binDir, 'VirtualBox')
    try:
        os.system(nemu)
    except KeyboardInterrupt:
...
commands = {'help':['Prints help information', helpCmd, 0],
            ...
            'monitorNemu':['Monitor what happens with Virtual Box for some time: monitorNemu 10', monitorNemuCmd, 0],
            ...
            'runScript':['Run Nemu script: runScript script.nemu', runScriptCmd, 0],
            ...
            'listMedia': ['List media known to this Nemu instance', listMediaCmd, 0],
...
#
# To write your own custom commands to nemushell, create
...
def getHomeFolder(ctx):
    if ctx['remote'] or ctx['vb'] is None:
        if 'NEMU_USER_HOME' in os.environ:
            return os.path.join(os.environ['NEMU_USER_HOME'])
...
def interpret(ctx):
    if ctx['remote']:
        commands['connect'] = ["Connect to remote Nemu instance: connect http://server:18083 user password", connectCmd, 0]
        commands['disconnect'] = ["Disconnect from remote Nemu instance", disconnectCmd, 0]
        commands['reconnect'] = ["Reconnect to remote Nemu instance", reconnectCmd, 0]
    ...
    if nemu is not None:
        try:
            ...
            if g_fVerbose:
                traceback.print_exc()
        ctx['perf'] = None # ctx['global'].getPerfCollector(nemu)
    else:
        ctx['perf'] = None
    ...
    if platform.system() in ['Windows', 'Microsoft']:
        global g_fHasColors
        g_fHasColors = False
    hist_file = os.path.join(home, ".nemushellhistory")
    autoCompletion(commands, ctx)
    ...
    if ctx['perf']:
        try:
            ctx['perf'].setup(['*'], [nemu.host], 10, 15)
        except:
            pass
    cmds = []
...
def main(argv):
    ...
    if options.autopath:
        ...
        sPath = os.environ.get("NEMU_PROGRAM_PATH")
        if sPath is None:
            for sCurLoc in asLocations:
                if   os.path.isfile(os.path.join(sCurLoc, "VirtualBox")) \
                  or os.path.isfile(os.path.join(sCurLoc, "Nemu.exe")):
                    print "Autodetected NEMU_PROGRAM_PATH as", sCurLoc
                    os.environ["NEMU_PROGRAM_PATH"] = sCurLoc
                    sPath = sCurLoc
                    break;
        if sPath:
            sys.path.append(os.path.join(sPath, "sdk", "installer"))

        sPath = os.environ.get("NEMU_SDK_PATH")
        if sPath is None:
            for sCurLoc in asLocations:
                if os.path.isfile(os.path.join(sCurLoc, "sdk", "bindings", "VirtualBox.xidl")):
                    sCurLoc = os.path.join(sCurLoc, "sdk");
                    print "Autodetected NEMU_SDK_PATH as", sCurLoc
                    os.environ["NEMU_SDK_PATH"] = sCurLoc
                    sPath = sCurLoc;
                    break;
        if sPath:
    ...
    oNemuMgr = VirtualBoxManager(style, params)
    ctx = {
        'global':       oNemuMgr,
        'vb':           oNemuMgr.nemu,
        'const':        oNemuMgr.constants,
        'remote':       oNemuMgr.remote,
        'type':         oNemuMgr.type,
