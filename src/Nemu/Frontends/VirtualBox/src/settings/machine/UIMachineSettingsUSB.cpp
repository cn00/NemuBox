...
/** @file
 * Nemu Qt GUI - UIMachineSettingsUSB class implementation.
 */
 ...
 */
class NemuUSBMenu : public QMenu
{
    Q_OBJECT;
    ...
    void processAboutToShow()
    {
        ...
        else
        {
            ...
            for (int i = 0; i < devvec.size(); ++i)
            {
                ...
                CUSBDevice usb(dev);
                QAction *pAction = addAction(nemuGlobal().details(usb));
    ...
    bool event(QEvent *pEvent)
    {
        ...
        if (pEvent->type() == QEvent::ToolTip)
        {
            ...
            if (pAction)
            {
                ...
                if (!usb.isNull())
                {
                    QToolTip::showText(pHelpEvent->globalPos(), nemuGlobal().toolTip(usb));
                    return true;
                }
            }
        }
    ...
    , mUSBDevicesMenu(0)
{
    ...
    mUSBDevicesMenu = new NemuUSBMenu (this);
...
#ifndef NEMU_WITH_EHCI
    mCbUSB2->setHidden(true);
#endif /* NEMU_WITH_EHCI */
}
...
void UIMachineSettingsUSB::addConfirmed(QAction *pAction)
{
    ...
    UIDataSettingsMachineUSBFilter usbFilterData;
    usbFilterData.m_fActive = true;
    usbFilterData.m_strName = nemuGlobal().details(usb);
    usbFilterData.m_fHostUSBDevice = false;
