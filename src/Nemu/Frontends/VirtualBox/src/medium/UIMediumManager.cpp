...
/** @file
 * Nemu Qt GUI - UIMediumManager class implementation.
 */
...
public:
    ...
    bool operator<(const QTreeWidgetItem &other) const
    {
        ...
        ULONG64 thisValue = nemuGlobal().parseSize(      text(column));
        ULONG64 thatValue = nemuGlobal().parseSize(other.text(column));
...
private:
    ...
    {
        /* Open session: */
        CSession session = nemuGlobal().openSession(strMachineID);
        if (session.isNull())
            return false;
...
protected:
    ...
    bool remove()
    {
        ...
        nemuGlobal().deleteMedium(strMediumID);
...
void UIMediumManager::sltHandleMediumCreated(const QString &strMediumID)
{
    ...
    UIMedium medium = nemuGlobal().medium(strMediumID);
    ...
    if (   !m_fPreventChangeCurrentItem
        && (   !nemuGlobal().isMediumEnumerationInProgress()
...
void UIMediumManager::sltHandleMediumEnumerationStart()
{
    ...
    if (m_pProgressBar)
    {
        m_pProgressBar->setMaximum(nemuGlobal().mediumIDs().size());
...
void UIMediumManager::sltRefreshAll()
{
    ...
    nemuGlobal().startMediumEnumeration();
}
...
void UIMediumManager::prepare()
{
    ...
    NemuGlobal::centerWidget(this, m_pPseudoParentWidget, false);

    /* Initialize information-panes: */
    updateInformationFields(UIMediumType_All);

    /* Start medium-enumeration (if necessary): */
    if (m_fRefresh && !nemuGlobal().isMediumEnumerationInProgress())
        nemuGlobal().startMediumEnumeration();
    ...
    else
    {
        ...
        if (!nemuGlobal().isMediumEnumerationInProgress())
...
void UIMediumManager::prepareConnections()
{
    ...
    connect(&nemuGlobal(), SIGNAL(sigMediumCreated(const QString&)),
            this, SLOT(sltHandleMediumCreated(const QString&)));
    connect(&nemuGlobal(), SIGNAL(sigMediumDeleted(const QString&)),
            this, SLOT(sltHandleMediumDeleted(const QString&)));

    /* Configure medium-enumeration connections: */
    connect(&nemuGlobal(), SIGNAL(sigMediumEnumerationStarted()),
            this, SLOT(sltHandleMediumEnumerationStart()));
    connect(&nemuGlobal(), SIGNAL(sigMediumEnumerated(const QString&)),
            this, SLOT(sltHandleMediumEnumerated(const QString&)));
    connect(&nemuGlobal(), SIGNAL(sigMediumEnumerationFinished()),
...
void UIMediumManager::repopulateTreeWidgets()
{
    ...
    foreach (const QString &strMediumID, nemuGlobal().mediumIDs())
...
void UIMediumManager::updateActions()
{
    ...
    bool fNotInEnumeration = !nemuGlobal().isMediumEnumerationInProgress();
    ...
    switch (action)
    {
        case Action_Add:
        {
            ...
            *pfInaccessible = true;
            ...
            if (m_pTabWidget)
                m_pTabWidget->setTabIcon(tabIndex(mediumType), nemuGlobal().warningIcon());
...
void UIMediumManager::updateInformationFieldsHD()
{
    ...
    else
    {
        ...
        const QString strUsage = pCurrentItem->usage().isNull() ?
                                 formatFieldText(QApplication::translate("NemuMediaManagerDlg", "<i>Not&nbsp;Attached</i>"), false) :
                                 formatFieldText(pCurrentItem->usage());
        const QString strEncryptionPasswordID = pCurrentItem->encryptionPasswordID().isNull() ?
                                                formatFieldText(QApplication::translate("NemuMediaManagerDlg", "<i>Not&nbsp;Encrypted</i>"), false) :
...
void UIMediumManager::retranslateUi()
{
    ...
    /* Most of these translations were moved from NemuMediaManagerDlg.ui file
     * to keep old translation context.. */

    /* Translate window title: */
    setWindowTitle(QApplication::translate("NemuMediaManagerDlg", "Virtual Media Manager"));

    /* Translate menu: */
    if (m_pMenu)
        m_pMenu->setTitle(QApplication::translate("NemuMediaManagerDlg", "&Actions"));

    /* Translate actions: */
    if (m_pActionCopy)
    {
        m_pActionCopy->setText(QApplication::translate("NemuMediaManagerDlg", "&Copy..."));
        m_pActionCopy->setToolTip(m_pActionCopy->text().remove('&') + QString(" (%1)").arg(m_pActionCopy->shortcut().toString()));
        m_pActionCopy->setStatusTip(QApplication::translate("NemuMediaManagerDlg", "Copy an existing disk image file"));
    }
    if (m_pActionModify)
    {
        m_pActionModify->setText(QApplication::translate("NemuMediaManagerDlg", "&Modify..."));
        m_pActionModify->setToolTip(m_pActionModify->text().remove('&') + QString(" (%1)").arg(m_pActionModify->shortcut().toString()));
        m_pActionModify->setStatusTip(QApplication::translate("NemuMediaManagerDlg", "Modify the attributes of the selected disk image file"));
    }
    if (m_pActionRemove)
    {
        m_pActionRemove->setText(QApplication::translate("NemuMediaManagerDlg", "R&emove"));
        m_pActionRemove->setToolTip(m_pActionRemove->text().remove('&') + QString(" (%1)").arg(m_pActionRemove->shortcut().toString()));
        m_pActionRemove->setStatusTip(QApplication::translate("NemuMediaManagerDlg", "Remove the selected disk image file"));
    }
    if (m_pActionRelease)
    {
        m_pActionRelease->setText(QApplication::translate("NemuMediaManagerDlg", "Re&lease"));
        m_pActionRelease->setToolTip(m_pActionRelease->text().remove('&') + QString(" (%1)").arg(m_pActionRelease->shortcut().toString()));
        m_pActionRelease->setStatusTip(QApplication::translate("NemuMediaManagerDlg", "Release the selected disk image file by detaching it from the machines"));
    }
    if (m_pActionRefresh)
    {
        m_pActionRefresh->setText(QApplication::translate("NemuMediaManagerDlg", "Re&fresh"));
        m_pActionRefresh->setToolTip(m_pActionRefresh->text().remove('&') + QString(" (%1)").arg(m_pActionRefresh->shortcut().toString()));
        m_pActionRefresh->setStatusTip(QApplication::translate("NemuMediaManagerDlg", "Refresh the list of disk image files"));
    }
...
# endif /* QT_MAC_USE_COCOA */
    ...
    if (pTreeWidgetHD)
    {
        pTreeWidgetHD->headerItem()->setText(0, QApplication::translate("NemuMediaManagerDlg", "Name"));
        pTreeWidgetHD->headerItem()->setText(1, QApplication::translate("NemuMediaManagerDlg", "Virtual Size"));
        pTreeWidgetHD->headerItem()->setText(2, QApplication::translate("NemuMediaManagerDlg", "Actual Size"));
    }

    /* Translate HD information-labels: */
    if (infoLabel(UIMediumType_HardDisk, 0))
        infoLabel(UIMediumType_HardDisk, 0)->setText(QApplication::translate("NemuMediaManagerDlg", "Type:"));
    if (infoLabel(UIMediumType_HardDisk, 1))
        infoLabel(UIMediumType_HardDisk, 1)->setText(QApplication::translate("NemuMediaManagerDlg", "Location:"));
    if (infoLabel(UIMediumType_HardDisk, 2))
        infoLabel(UIMediumType_HardDisk, 2)->setText(QApplication::translate("NemuMediaManagerDlg", "Format:"));
    if (infoLabel(UIMediumType_HardDisk, 3))
        infoLabel(UIMediumType_HardDisk, 3)->setText(QApplication::translate("NemuMediaManagerDlg", "Storage details:"));
    if (infoLabel(UIMediumType_HardDisk, 4))
        infoLabel(UIMediumType_HardDisk, 4)->setText(QApplication::translate("NemuMediaManagerDlg", "Attached to:"));
    if (infoLabel(UIMediumType_HardDisk, 5))
        infoLabel(UIMediumType_HardDisk, 5)->setText(QApplication::translate("NemuMediaManagerDlg", "Encrypted with key:"));
    if (infoLabel(UIMediumType_HardDisk, 6))
        infoLabel(UIMediumType_HardDisk, 6)->setText(QApplication::translate("NemuMediaManagerDlg", "UUID:"));

    /* Translate CD tree-widget: */
    QTreeWidget *pTreeWidgetCD = treeWidget(UIMediumType_DVD);
    if (pTreeWidgetCD)
    {
        pTreeWidgetCD->headerItem()->setText(0, QApplication::translate("NemuMediaManagerDlg", "Name"));
        pTreeWidgetCD->headerItem()->setText(1, QApplication::translate("NemuMediaManagerDlg", "Size"));
    }

    /* Translate CD information-labels: */
    if (infoLabel(UIMediumType_DVD, 0))
        infoLabel(UIMediumType_DVD, 0)->setText(QApplication::translate("NemuMediaManagerDlg", "Location:"));
    if (infoLabel(UIMediumType_DVD, 1))
        infoLabel(UIMediumType_DVD, 1)->setText(QApplication::translate("NemuMediaManagerDlg", "Attached to:"));
    if (infoLabel(UIMediumType_DVD, 2))
        infoLabel(UIMediumType_DVD, 2)->setText(QApplication::translate("NemuMediaManagerDlg", "UUID:"));

    /* Translate FD tree-widget: */
    QTreeWidget *pTreeWidgetFD = treeWidget(UIMediumType_Floppy);
    if (pTreeWidgetFD)
    {
        pTreeWidgetFD->headerItem()->setText(0, QApplication::translate("NemuMediaManagerDlg", "Name"));
        pTreeWidgetFD->headerItem()->setText(1, QApplication::translate("NemuMediaManagerDlg", "Size"));
    }

    /* Translate FD information-labels: */
    if (infoLabel(UIMediumType_Floppy, 0))
        infoLabel(UIMediumType_Floppy, 0)->setText(QApplication::translate("NemuMediaManagerDlg", "Location:"));
    if (infoLabel(UIMediumType_Floppy, 1))
        infoLabel(UIMediumType_Floppy, 1)->setText(QApplication::translate("NemuMediaManagerDlg", "Attached to:"));
    if (infoLabel(UIMediumType_Floppy, 2))
        infoLabel(UIMediumType_Floppy, 2)->setText(QApplication::translate("NemuMediaManagerDlg", "UUID:"));

    /* Translate progress-bar: */
    if (m_pProgressBar)
    {
        m_pProgressBar->setText(QApplication::translate("NemuMediaManagerDlg", "Checking accessibility"));
#ifdef Q_WS_MAC
        ...
        if (!pMediumItem)
        {
            ...
            if (medium.parentID() != UIMedium::nullID())
            {
                ...
                if (!pParentMediumItem)
                {
                    ...
                    UIMedium parentMedium = nemuGlobal().medium(medium.parentID());
                                         ...
                                         const QString &strElipsis /* = "middle" */)
{
    ...
    QString strInfo = QString("<nobr>%1%2%3</nobr>")
                              ...
                              .arg(strText.isEmpty() ?
                                   QApplication::translate("NemuMediaManagerDlg", "--", "no info") :
                                   strText)
