...
#include <Nemu/vmm/pdmcritsectrw.h>
...
#ifdef NEMU_WITH_RC_RELEASE_LOGGING
    /*
...
#endif /* NEMU_WITH_RC_RELEASE_LOGGING */
    return VINF_SUCCESS;
}
...
static DECLCALLBACK(int) vmmR3Load(PVM pVM, PSSMHANDLE pSSM, uint32_t uVersion, uint32_t uPass)
{
    ...
    if (uVersion <= VMM_SAVED_STATE_VERSION_3_0)
    {
        ...
        SSMR3GetRCPtr(pSSM, &RCPtrIgnored);
#ifdef RT_OS_DARWIN
        if (   SSMR3HandleVersion(pSSM)  >= NEMU_FULL_VERSION_MAKE(3,0,0)
            && SSMR3HandleVersion(pSSM)  <  NEMU_FULL_VERSION_MAKE(3,1,0)
 ...
 *
 * @returns Nemu strict informational status code for EM scheduling. No failures
                                    ...
                                    uint32_t fFlags, PFNVMMEMTRENDEZVOUS pfnRendezvous, void *pvUser)
{
    ...
    {
        NEMUSTRICTRC rcStrict = pfnRendezvous(pVM, pVCpu, pvUser);
        if (rcStrict != VINF_SUCCESS)
        {
            ...
            do
            {
                ...
                if (    rcStrict == i32RendezvousStatus
                    ...
                    break;
            } while (!ASMAtomicCmpXchgS32(&pVM->vmm.s.i32RendezvousStatus, NEMUSTRICTRC_VAL(rcStrict), i32RendezvousStatus));
        }
    }
 ...
 *
 * @returns Nemu strict status code - EM scheduling.  No errors will be returned
 ...
 *
 * @returns Nemu strict status code.  This will be the first error,
