...
void CrMClrFillImg(CR_BLITTER_IMG *pImg, uint32_t cRects, const RTRECT *pRects, uint32_t u32Color)
{
    ...
    for (uint32_t i = 0; i < cRects; ++i)
    {
        ...
        NemuRectIntersected(pRect, &Rect, &Intersection);
...
void CrMBltImg(const CR_BLITTER_IMG *pSrc, const RTPOINT *pPos, uint32_t cRects, const RTRECT *pRects, CR_BLITTER_IMG *pDst)
{
    ...
    {
        const RTRECT * pRect = &pRects[i];
        NemuRectIntersected(pRect, &RestrictDstRect, &Intersection);

        RTRECT TranslatedSrc;
        NemuRectTranslated(&RestrictSrcRect, pPos->x, pPos->y, &TranslatedSrc);

        NemuRectIntersect(&Intersection, &TranslatedSrc);
...
void CrMBltImgRectScaled(const CR_BLITTER_IMG *pSrc, const RTPOINT *pPos, bool fSrcInvert, const RTRECT *pCopyRect, float strX, float strY, CR_BLITTER_IMG *pDst)
{
    ...
    NemuRectUnscaled(pCopyRect, strX, strY, &UnscaledCopyRect);

    if (NemuRectIsZero(&UnscaledCopyRect))
    {
        WARN(("ups"));
        return;
    }
...
void CrMBltImgScaled(const CR_BLITTER_IMG *pSrc, const RTRECTSIZE *pSrcRectSize, const RTRECT *pDstRect, uint32_t cRects, const RTRECT *pRects, CR_BLITTER_IMG *pDst)
{
    ...
    for (uint32_t i = 0; i < cRects; ++i)
    {
        ...
        RTRECT TranslatedSrc;
        NemuRectTranslated(&ScaledRestrictSrcRect, Pos.x, Pos.y, &TranslatedSrc);
 ...
 * @param   pDispatch
 */
NEMUBLITTERDECL(int) CrBltInit(PCR_BLITTER pBlitter, const CR_BLITTER_CONTEXT *pCtxBase,
...
NEMUBLITTERDECL(int) CrBltCleanup(PCR_BLITTER pBlitter)
{
...
static DECLCALLBACK(int) crBltBlitTexBufImplFbo(PCR_BLITTER pBlitter, const NEMUVR_TEXTURE *pSrc, const RTRECT *paSrcRect, const RTRECTSIZE *pDstSize, const RTRECT *paDstRect, uint32_t cRects, uint32_t fFlags)
{
...
static DECLCALLBACK(int) crBltBlitTexBufImplDraw2D(PCR_BLITTER pBlitter, const NEMUVR_TEXTURE *pSrc, const RTRECT *paSrcRect, const RTRECTSIZE *pDstSize, const RTRECT *paDstRect, uint32_t cRects, uint32_t fFlags)
{
...
static void crBltBlitTexBuf(PCR_BLITTER pBlitter, const NEMUVR_TEXTURE *pSrc, const RTRECT *paSrcRects, GLenum enmDstBuff, const RTRECTSIZE *pDstSize, const RTRECT *paDstRects, uint32_t cRects, uint32_t fFlags)
{
#ifdef IN_VMSVGA3D
    /** @todo IN_VMSVGA3D */
#else
...
void CrBltBlitTexMural(PCR_BLITTER pBlitter, bool fBb, const NEMUVR_TEXTURE *pSrc, const RTRECT *paSrcRects, const RTRECT *paDstRects, uint32_t cRects, uint32_t fFlags)
{
    if (!CrBltIsEntered(pBlitter))
    {
...
void CrBltBlitTexTex(PCR_BLITTER pBlitter, const NEMUVR_TEXTURE *pSrc, const RTRECT *pSrcRect, const NEMUVR_TEXTURE *pDst, const RTRECT *pDstRect, uint32_t cRects, uint32_t fFlags)
{
    if (!CrBltIsEntered(pBlitter))
    {
...
static int crBltImgInitBaseForTex(const NEMUVR_TEXTURE *pSrc, CR_BLITTER_IMG *pDst, GLenum enmFormat)
{
...
static int crBltImgCreateForTex(const NEMUVR_TEXTURE *pSrc, CR_BLITTER_IMG *pDst, GLenum enmFormat)
{
...
NEMUBLITTERDECL(int) CrBltImgGetTex(PCR_BLITTER pBlitter, const NEMUVR_TEXTURE *pSrc, GLenum enmFormat, CR_BLITTER_IMG *pDst)
{
    if (!CrBltIsEntered(pBlitter))
    {
...
NEMUBLITTERDECL(int) CrBltImgGetMural(PCR_BLITTER pBlitter, bool fBb, CR_BLITTER_IMG *pDst)
{
    if (!CrBltIsEntered(pBlitter))
    {
...
NEMUBLITTERDECL(void) CrBltImgFree(PCR_BLITTER pBlitter, CR_BLITTER_IMG *pDst)
{
    if (!CrBltIsEntered(pBlitter))
    {
...
NEMUBLITTERDECL(bool) CrGlslIsSupported(CR_GLSL_CACHE *pCache)
{
...
NEMUBLITTERDECL(int) CrGlslProgGenNoAlpha(CR_GLSL_CACHE *pCache, GLenum enmTexTarget)
{
...
NEMUBLITTERDECL(int) CrGlslProgGenAllNoAlpha(CR_GLSL_CACHE *pCache)
{
...
NEMUBLITTERDECL(void) CrGlslProgClear(const CR_GLSL_CACHE *pCache)
{
    pCache->pDispatch->UseProgram(0);
}

NEMUBLITTERDECL(int) CrGlslProgUseNoAlpha(const CR_GLSL_CACHE *pCache, GLenum enmTexTarget)
{
...
NEMUBLITTERDECL(int) CrGlslProgUseGenNoAlpha(CR_GLSL_CACHE *pCache, GLenum enmTexTarget)
{
    GLuint uiProg = crGlslProgGetNoAlpha(pCache, enmTexTarget);
    if (!uiProg)
    {
...
NEMUBLITTERDECL(bool) CrGlslNeedsCleanup(const CR_GLSL_CACHE *pCache)
{
    return pCache->uNoAlpha2DProg || pCache->uNoAlpha2DRectProg;
}

NEMUBLITTERDECL(void) CrGlslCleanup(CR_GLSL_CACHE *pCache)
{
...
NEMUBLITTERDECL(void) CrGlslTerm(CR_GLSL_CACHE *pCache)
{
...
NEMUBLITTERDECL(int) CrTdBltDataRelease(PCR_TEXDATA pTex)
{
...
NEMUBLITTERDECL(int) CrTdBltDataFree(PCR_TEXDATA pTex)
{
    if (!pTex->Flags.Entered)
    {
        WARN(("tex not entered"));
...
NEMUBLITTERDECL(void) CrTdBltDataInvalidateNe(PCR_TEXDATA pTex)
{
    crTdBltImgRelease(pTex);
...
NEMUBLITTERDECL(int) CrTdBltDataFreeNe(PCR_TEXDATA pTex)
{
    if (!pTex->Img.pvData)
        return VINF_SUCCESS;
...
NEMUBLITTERDECL(int) CrTdBltDataCleanup(PCR_TEXDATA pTex)
{
    if (!pTex->Flags.Entered)
    {
        WARN(("tex not entered"));
...
NEMUBLITTERDECL(int) CrTdBltDataCleanupNe(PCR_TEXDATA pTex)
{
    bool fEntered = false;
 ...
 * */
NEMUBLITTERDECL(int) CrTdBltDataAcquire(PCR_TEXDATA pTex, GLenum enmFormat, bool fInverted, const CR_BLITTER_IMG**ppImg)
{
    if (!pTex->Flags.Entered)
    {
        WARN(("tex not entered"));
...
DECLINLINE(void) crTdResize(PCR_TEXDATA pTex, const NEMUVR_TEXTURE *pVrTex)
{
    crTdBltDataCleanup(pTex);
...
NEMUBLITTERDECL(int) CrTdBltDataAcquireScaled(PCR_TEXDATA pTex, GLenum enmFormat, bool fInverted, uint32_t width, uint32_t height, const CR_BLITTER_IMG**ppImg)
{
...
NEMUBLITTERDECL(int) CrTdBltDataReleaseScaled(PCR_TEXDATA pTex, const CR_BLITTER_IMG *pImg)
{
...
NEMUBLITTERDECL(void) CrTdBltScaleCacheMoveTo(PCR_TEXDATA pTex, PCR_TEXDATA pDstTex)
{
    if (!pTex->pScaledCache)
        return;
