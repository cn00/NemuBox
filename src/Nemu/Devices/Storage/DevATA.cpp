/* $Id: DevATA.cpp $ */
/** @file
 * Nemu storage devices: ATA/ATAPI controller device (disk and cdrom).
 */
...
#undef NEMU_INSTRUMENT_DMA_WRITES
...
#define ATA_SAVED_STATE_VERSION_NEMU_30                 19
...
#ifdef NEMU_INSTRUMENT_DMA_WRITES
...
# endif
                ...
                && s->fOverwriteInquiry)
            {
                ...
                ataR3SCSIPadStr(s->CTX_SUFF(pbIOBuffer) + 8, "NEMU", 8);
...
# if 1/*ndef NEMU*/  /** @todo implement MESN + AENC. (async notification on removal and stuff.) */
    pbBuf[2] = 0x00; /* ISO */
...
static int ataIOPortReadU8(PATACONTROLLER pCtl, uint32_t addr, uint32_t *pu32)
{
    ...
    switch (addr & 7)
    {
        ...
        case 7: /* primary status */
        {
             ...
             * this is no guarantee for any other Nemu thread getting
...
static DECLCALLBACK(int) ataR3LoadExec(PPDMDEVINS pDevIns, PSSMHANDLE pSSM, uint32_t uVersion, uint32_t uPass)
{
    ...
    if (   uVersion != ATA_SAVED_STATE_VERSION
        && uVersion != ATA_SAVED_STATE_VERSION_NEMU_30
     ...
     */
    if (uVersion > ATA_SAVED_STATE_VERSION_NEMU_30)
    {
        uint8_t u8Type;
...
#ifdef NEMU_WITH_STATISTICS /** @todo release too. */
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    rc = CFGMR3QueryStringDef(pCfgNode, "ModelNumber", pIf->szModelNumber, sizeof(pIf->szModelNumber),
                                              pIf->fATAPI ? "NEMU CD-ROM" : "NEMU HARDDISK");
                    if (RT_FAILURE(rc))
                    {
                        if (rc == VERR_CFGM_NOT_ENOUGH_SPACE)
                            return PDMDEV_SET_ERROR(pDevIns, VERR_INVALID_PARAMETER,
