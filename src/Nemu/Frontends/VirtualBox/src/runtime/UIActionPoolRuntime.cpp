...
/** @file
 * Nemu Qt GUI - UIActionPoolRuntime class implementation.
 */
...
void UIActionPoolRuntime::sltHandleActionTriggerViewScaleFactor(QAction *pAction)
{
    ...
    gEDataManager->setScaleFactor(dScaleFactor, nemuGlobal().managedVMUuid());
}
...
void UIActionPoolRuntime::updateConfiguration()
{
    ...
    bool fAllCloseActionsRestricted =    (!nemuGlobal().isSeparateProcess() || (restrictedCloseActions & MachineCloseAction_Detach))
...
#ifdef NEMU_WITH_DEBUGGER_GUI
    /* 'Debug' menu: */
    addMenu(m_mainMenus, action(UIActionIndexRT_M_Debug), nemuGlobal().isDebuggerEnabled());
    updateMenuDebug();
#endif /* NEMU_WITH_DEBUGGER_GUI */
...
void UIActionPoolRuntime::updateMenuViewScaleFactor()
{
    ...
    const double dCurrentScaleFactor = gEDataManager->scaleFactor(nemuGlobal().managedVMUuid());
