 ...
 *
 * @returns Nemu status code. If success, then the search is completed.
...
int dbgfR3AsInit(PUVM pUVM)
{
    ...
    {
        { RTDBGCFGPROP_FLAGS,               "NEMUDBG_FLAGS",            "Flags"             },
        { RTDBGCFGPROP_PATH,                "NEMUDBG_PATH",             "Path"              },
        { RTDBGCFGPROP_SUFFIXES,            "NEMUDBG_SUFFIXES",         "Suffixes"          },
        { RTDBGCFGPROP_SRC_PATH,            "NEMUDBG_SRC_PATH",         "SrcPath"           },
    };
    ...
    /*
     * Prepend the NoArch and NemuDbgSyms directories to the path.
     */
    char szPath[RTPATH_MAX];
    rc = RTPathAppPrivateNoArch(szPath, sizeof(szPath));
    AssertRCReturn(rc, rc);
#ifdef RT_OS_DARWIN
    rc = RTPathAppend(szPath, sizeof(szPath), "../Resources/NemuDbgSyms/");
#else
    rc = RTDbgCfgChangeString(pUVM->dbgf.s.hDbgCfg, RTDBGCFGPROP_PATH, RTDBGCFGOP_PREPEND, szPath);
    AssertRCReturn(rc, rc);

    rc = RTPathAppend(szPath, sizeof(szPath), "NemuDbgSyms/");
#endif
    AssertRCReturn(rc, rc);
    rc = RTDbgCfgChangeString(pUVM->dbgf.s.hDbgCfg, RTDBGCFGPROP_PATH, RTDBGCFGOP_PREPEND, szPath);
    AssertRCReturn(rc, rc);
 ...
 *
 * @returns Nemu status code. See RTDbgAsSymbolByAddr.
 *
 * @param   pUVM            The user mode VM handle.
 * @param   hDbgAs          The address space handle.
