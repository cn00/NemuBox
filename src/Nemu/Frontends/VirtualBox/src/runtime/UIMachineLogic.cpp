...
/** @file
 * Nemu Qt GUI - UIMachineLogic class implementation.
 */
...
void UIMachineLogic::sltHandleNemuSVCAvailabilityChange()
{
    /* Do nothing if NemuSVC still availabile: */
    if (nemuGlobal().isNemuSVCAvailable())
        return;

    /* Warn user about that: */
    msgCenter().warnAboutNemuSVCUnavailable();
...
void UIMachineLogic::sltMachineStateChanged()
{
    ...
    switch (state)
    {
        case KMachineState_Stuck:
        {
            ...
            takeScreenshot(strLogFolder + "/Nemu.png", "png");
            /* How should we handle Guru Meditation? */
            switch (gEDataManager->guruMeditationHandlerType(nemuGlobal().managedVMUuid()))
            {
...
void UIMachineLogic::sltUSBDeviceStateChange(const CUSBDevice &device, bool fIsAttached, const CVirtualBoxErrorInfo &error)
{
    ...
    {
        if (fIsAttached)
            msgCenter().cannotAttachUSBDevice(error, nemuGlobal().details(device), machineName());
        else
            msgCenter().cannotDetachUSBDevice(error, nemuGlobal().details(device), machineName());
    }
}
...
{
#ifdef Q_WS_MAC
# ifdef NEMU_WITH_ICHAT_THEATER
    /* Init shared AV manager: */
    initSharedAVManager();
# endif /* NEMU_WITH_ICHAT_THEATER */
#endif /* Q_WS_MAC */
}

void UIMachineLogic::prepareSessionConnections()
{
    /* We should watch for NemuSVC availability changes: */
    connect(&nemuGlobal(), SIGNAL(sigNemuSVCAvailabilityChange()),
            this, SLOT(sltHandleNemuSVCAvailabilityChange()));
...
void UIMachineLogic::prepareDock()
{
    ...
    if (cGuestScreens > 1)
    {
        ...
        m_DockIconPreviewMonitor = qMin(gEDataManager->realtimeDockIconUpdateMonitor(nemuGlobal().managedVMUuid()),
                                        cGuestScreens - 1);
    ...
    m_pDockIconPreview = new UIDockIconPreview(uisession(), nemuGlobal().vmGuestOSTypePixmapHiDPI(osTypeId, QSize(42, 42)));

    /* Should the dock-icon be updated at runtime? */
    bool fEnabled = gEDataManager->realtimeDockIconUpdateEnabled(nemuGlobal().managedVMUuid());
    if (fEnabled)
...
{
    if (nemuGlobal().isDebuggerAutoShowEnabled())
    {
        if (nemuGlobal().isDebuggerAutoShowStatisticsEnabled())
            sltShowDebugStatistics();
        if (nemuGlobal().isDebuggerAutoShowCommandLineEnabled())
...
void UIMachineLogic::loadSettings()
{
    ...
    m_fIsHidLedsSyncEnabled = gEDataManager->hidLedsSyncState(nemuGlobal().managedVMUuid());
...
{
    /* We should stop watching for NemuSVC availability changes: */
    disconnect(&nemuGlobal(), SIGNAL(sigNemuSVCAvailabilityChange()),
               this, SLOT(sltHandleNemuSVCAvailabilityChange()));
...
void UIMachineLogic::sltTakeSnapshot()
{
    ...
    QPointer<NemuTakeSnapshotDlg> pDlg = new NemuTakeSnapshotDlg(pDlgParent, machine());
    windowManager().registerNewParent(pDlg, pDlgParent);

    /* Assign corresponding icon: */
    QString strTypeId = machine().GetOSTypeId();
    pDlg->mLbIcon->setPixmap(nemuGlobal().vmGuestOSTypeIcon(strTypeId));

    /* Search for the max available filter index: */
    QString strNameTemplate = NemuTakeSnapshotDlg::tr("Snapshot %1");
...
void UIMachineLogic::sltMountStorageMedium()
{
    ...
    nemuGlobal().updateMachineStorage(machine(), target);
}
...
void UIMachineLogic::sltAttachUSBDevice()
{
    ...
    if (target.attach)
    {
        ...
        if (!console().isOk())
        {
            ...
            msgCenter().cannotAttachUSBDevice(console(), nemuGlobal().details(device));
        }
    }
    /* Detach USB device: */
    else
    {
        ...
        {
            /* Show a message about procedure failure: */
            msgCenter().cannotDetachUSBDevice(console(), nemuGlobal().details(device));
        }
    }
}
...
void UIMachineLogic::sltInstallGuestAdditions()
{
    ...
    const QString &name = QString("NemuGuestAdditions_%1.iso").arg(nemuGlobal().nemuVersionStringNormalized());

    CMediumVector vec = nemu.GetDVDImages();
...
void UIMachineLogic::sltDockPreviewModeChanged(QAction *pAction)
{
    ...
    gEDataManager->setRealtimeDockIconUpdateEnabled(fEnabled, nemuGlobal().managedVMUuid());
    updateDockOverlay();
}

void UIMachineLogic::sltDockPreviewMonitorChanged(QAction *pAction)
{
    gEDataManager->setRealtimeDockIconUpdateMonitor(pAction->data().toInt(), nemuGlobal().managedVMUuid());
    updateDockOverlay();
}
...
void UIMachineLogic::updateMenuDevicesStorage(QMenu *pMenu)
{
    ...
    foreach (const CMediumAttachment &attachment, machine().GetMediumAttachments())
    {
        ...
        if (!controller.isNull() && attachment.GetType() == deviceType)
        {
            ...
            nemuGlobal().prepareStorageMenu(*pStorageMenu,
...
void UIMachineLogic::updateMenuDevicesNetwork(QMenu *pMenu)
{
    ...
    const ULONG uCount = qMin((ULONG)4, nemuGlobal().virtualBox().GetSystemProperties().GetMaxNetworkAdapters(chipsetType));
...
void UIMachineLogic::updateMenuDevicesUSB(QMenu *pMenu)
{
    ...
    else
    {
        ...
        foreach (const CHostUSBDevice& hostDevice, devices)
        {
            ...
            QAction *pAttachUSBAction = pMenu->addAction(nemuGlobal().details(device),
                                                         this, SLOT(sltAttachUSBDevice()));
            pAttachUSBAction->setToolTip(nemuGlobal().toolTip(device));
...
void UIMachineLogic::updateMenuDevicesWebCams(QMenu *pMenu)
{
    ...
    else
    {
        ...
        foreach (const CHostVideoInputDevice &webcam, webcams)
        {
            ...
            pAttachWebcamAction->setToolTip(nemuGlobal().toolTip(webcam));
...
bool UIMachineLogic::dbgCreated()
{
    ...
    RTLDRMOD hLdrMod = nemuGlobal().getDebuggerModule();
