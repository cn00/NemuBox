...
int NemuDbgGui::init(ISession *pSession)
{
...
void
NemuDbgGui::setParent(QWidget *pParent)
{
    m_pParent = pParent;
}
...
void
NemuDbgGui::setMenu(QMenu *pMenu)
{
    m_pMenu = pMenu;
}
...
int
NemuDbgGui::showStatistics()
{
    if (!m_pDbgStats)
    {
        m_pDbgStats = new NemuDbgStats(this, "*", 2, m_pParent);
...
void
NemuDbgGui::repositionStatistics(bool fResize/* = true*/)
{
    /*
     * Move it to the right side of the Nemu console,
...
int
NemuDbgGui::showConsole()
{
    if (!m_pDbgConsole)
    {
        m_pDbgConsole = new NemuDbgConsole(this, m_pParent);
...
void
NemuDbgGui::repositionConsole(bool fResize/* = true*/)
{
    /*
     * Move it to the bottom of the Nemu console,
...
void
NemuDbgGui::adjustRelativePos(int x, int y, unsigned cx, unsigned cy)
{
...
void
NemuDbgGui::notifyChildDestroyed(QObject *pObj)
{
    if (m_pDbgStats == pObj)
        m_pDbgStats = NULL;
