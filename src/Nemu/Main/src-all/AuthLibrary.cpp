...
int AuthLibLoad(AUTHLIBRARYCONTEXT *pAuthLibCtx, const char *pszLibrary)
{
    ...
    else
    {
        ...
        if (RT_FAILURE(rc))
        {
             ...
             * Try to load new default 'NemuAuth' instead.
             */
            if (RTStrICmp(pszLibrary, "VRDPAuth") == 0)
            {
                LogRel(("AUTH: Loading external authentication library 'NemuAuth'\n"));
                rc = RTLdrLoadAppPriv("NemuAuth", &pAuthLibCtx->hAuthLibrary);
            }
        }
    }
