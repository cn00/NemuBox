...
#define NEMU_XDND_VERSION    (4)
#define NEMU_MAX_XPROPERTIES (LONG_MAX-1)
...
/** List of Atoms. */
#define NemuDnDAtomList RTCList<Atom>
...
public:
    ...
    inline RTCString xAtomListToString(const NemuDnDAtomList &formatList)
    {
        RTCString format;
...
#ifdef DEBUG
# define NEMU_DND_FN_DECL_LOG(x) inline x /* For LogFlowXXX logging. */
#else
# define NEMU_DND_FN_DECL_LOG(x) x
#endif

/** @todo Move all proxy window-related stuff into this class! Clean up this mess. */
class NemuDnDProxyWnd
{

public:

    NemuDnDProxyWnd(void);

    virtual ~NemuDnDProxyWnd(void);
...
public:
    ...
    /* Logging. */
    NEMU_DND_FN_DECL_LOG(void) logInfo(const char *pszFormat, ...);
    NEMU_DND_FN_DECL_LOG(void) logError(const char *pszFormat, ...);
    ...
    int wndXDnDGetActionList(Window wndThis, NemuDnDAtomList &lstActions) const;
    int wndXDnDGetFormatList(Window wndThis, NemuDnDAtomList &lstTypes) const;
    int wndXDnDSetActionList(Window wndThis, const NemuDnDAtomList &lstActions) const;
    int wndXDnDSetFormatList(Window wndThis, Atom atmProp, const NemuDnDAtomList &lstFormats) const;

    /* Atom / HGCM formatting helpers. */
    int             toAtomList(const RTCList<RTCString> &lstFormats, NemuDnDAtomList &lstAtoms) const;
    int             toAtomList(const void *pvData, uint32_t cbData, NemuDnDAtomList &lstAtoms) const;
    static Atom     toAtomAction(uint32_t uAction);
    static int      toAtomActions(uint32_t uActions, NemuDnDAtomList &lstAtoms);
    static uint32_t toHGCMAction(Atom atom);
    static uint32_t toHGCMActions(const NemuDnDAtomList &actionsList);
...
protected:
    ...
    NemuDnDAtomList             m_lstFormats;
    /** List of (Atom) actions the source window supports. */
    NemuDnDAtomList             m_lstActions;
...
int DragInstance::init(uint32_t u32ScreenID)
{
    ...
    do
    {
        ...
        XStoreName(m_pDisplay, m_wndProxy.hWnd, "NemuClientWndDnD");

        /* Make the new window Xdnd aware. */
        Atom ver = NEMU_XDND_VERSION;
 ...
 */
NEMU_DND_FN_DECL_LOG(void) DragInstance::logError(const char *pszFormat, ...)
{
    va_list args;
 ...
 * @param   pszFormat               Format string to log.
 */
NEMU_DND_FN_DECL_LOG(void) DragInstance::logInfo(const char *pszFormat, ...)
{
    va_list args;
    va_start(args, pszFormat);
    char *psz = NULL;
...
#endif /* NEMU_WITH_DRAG_AND_DROP_GH */
...
int DragInstance::hgMove(uint32_t u32xPos, uint32_t u32yPos, uint32_t uDefaultAction)
{
        ...
        && newVer    != -1)
    {
        ...
        m.data.l[XdndEnterFlags]  = RT_MAKE_U32_FROM_U8(
                                    ...
                                    RT_MIN(NEMU_XDND_VERSION, newVer));
        ...
        if (waitForX11Msg(evSelNotify, SelectionNotify, 5 * 1000 /* 5s timeout */))
        {
                ...
                && evSelNotify.xselection.target    == aFormat)
            {
                ...
                int xRc = XGetWindowProperty(m_pDisplay, m_wndProxy.hWnd,
                                             ...
                                             0                        /* Offset */,
                                             NEMU_MAX_XPROPERTIES     /* Length of 32-bit multiples */,
 ...
 * @param   lstActions              Reference to NemuDnDAtomList to store the action into.
 */
int DragInstance::wndXDnDGetActionList(Window wndThis, NemuDnDAtomList &lstActions) const
{
    Atom iActType = None;
    int iActFmt;
    ...
    if (cItems > 0)
    {
        ...
        for (unsigned i = 0; i < RT_MIN(NEMU_MAX_XPROPERTIES, cItems); i++)
        {
 ...
 * @param   lstTypes                Reference to NemuDnDAtomList to store the formats into.
 */
int DragInstance::wndXDnDGetFormatList(Window wndThis, NemuDnDAtomList &lstTypes) const
{
    Atom iActType = None;
    int iActFmt;
    unsigned long cItems, cbData;
 ...
 * @remark
 */
int DragInstance::wndXDnDSetActionList(Window wndThis, const NemuDnDAtomList &lstActions) const
{
 ...
 */
int DragInstance::wndXDnDSetFormatList(Window wndThis, Atom atmProp, const NemuDnDAtomList &lstFormats) const
{
    if (lstFormats.isEmpty())
        return VERR_INVALID_PARAMETER;

    /* We support TARGETS and the data types. */
    NemuDnDAtomList lstFormatsExt(lstFormats.size() + 1);
...
/**
 * Converts a RTCString list to NemuDnDAtomList list.
 *
 * @returns IPRT status code.
 * @param   lstFormats              Reference to RTCString list to convert.
 * @param   lstAtoms                Reference to NemuDnDAtomList list to store results in.
 */
int DragInstance::toAtomList(const RTCList<RTCString> &lstFormats, NemuDnDAtomList &lstAtoms) const
{
    for (size_t i = 0; i < lstFormats.size(); ++i)
...
/**
 * Converts a raw-data string list to NemuDnDAtomList list.
 *
 * @returns IPRT status code.
 ...
 * @param   lstAtoms                Reference to NemuDnDAtomList list to store results in.
 */
int DragInstance::toAtomList(const void *pvData, uint32_t cbData, NemuDnDAtomList &lstAtoms) const
{
...
/**
 * Converts HGCM-based drag'n drop actions to a NemuDnDAtomList list.
 *
 * @returns IPRT status code.
 * @param   uActions                HGCM drag'n drop actions to convert.
 * @param   lstAtoms                Reference to NemuDnDAtomList to store actions in.
 */
/* static */
int DragInstance::toAtomActions(uint32_t uActions, NemuDnDAtomList &lstAtoms)
{
...
/**
 * Converts an NemuDnDAtomList list to an HGCM action list.
 *
 * @returns ORed HGCM action list.
 * @param   actionsList             List of Atom-based actions to convert.
 */
/* static */
uint32_t DragInstance::toHGCMActions(const NemuDnDAtomList &lstActions)
{
...
/*******************************************************************************
 * NemuDnDProxyWnd implementation.
 ******************************************************************************/

NemuDnDProxyWnd::NemuDnDProxyWnd(void)
    : pDisp(NULL)
    , hWnd(0)
    , iX(0)
    , iY(0)
...
int NemuDnDProxyWnd::init(Display *pDisplay)
{
    /** @todo What about multiple screens? Test this! */
...
void NemuDnDProxyWnd::destroy(void)
{

}

int NemuDnDProxyWnd::sendFinished(Window hWndSource, uint32_t uAction)
{
...
int DragAndDropService::run(bool fDaemonised /* = false */)
{
    ...
    do
    {
        ...
        /* Note: For multiple screen support in Nemu it is not necessary to use
         * another screen number than zero. Maybe in the future it will become
         * necessary if Nemu supports multiple X11 screens. */
        rc = m_pCurDnD->init(0);
        if (RT_FAILURE(rc))
            break;
...
/** NemuClient service class wrapping the logic for the service while
 *  the main NemuClient code provides the daemon logic needed by all services.
 */
...
{
    return ".nemuclient-draganddrop.pid";
}
