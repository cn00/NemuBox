...
/** @file
 * Nemu WDDM Miniport registry related functions
 */
...
VP_STATUS NemuMPCmnRegInit(IN PNEMUMP_DEVEXT pExt, OUT NEMUMPCMNREGISTRY *pReg)
{
    ...
    NTSTATUS Status = nemuWddmRegQueryDrvKeyName(pExt, cbBuf, Buf, &cbBuf);
    Assert(Status == STATUS_SUCCESS);
    if (Status == STATUS_SUCCESS)
    {
        Status = nemuWddmRegOpenKey(pReg, Buf, GENERIC_READ | GENERIC_WRITE);
        Assert(Status == STATUS_SUCCESS);
        if(Status == STATUS_SUCCESS)
            return NO_ERROR;
    }
...
VP_STATUS NemuMPCmnRegQueryDword(IN NEMUMPCMNREGISTRY Reg, PWSTR pName, uint32_t *pVal)
{
    ...
    *pVal = 0;
    ...
    NTSTATUS Status = nemuWddmRegQueryValueDword(Reg, pName, (PDWORD)pVal);
    return Status == STATUS_SUCCESS ? NO_ERROR : ERROR_INVALID_PARAMETER;
}
...
VP_STATUS NemuMPCmnRegSetDword(IN NEMUMPCMNREGISTRY Reg, PWSTR pName, uint32_t Val)
{
    ...
    NTSTATUS Status = nemuWddmRegSetValueDword(Reg, pName, Val);
    return Status == STATUS_SUCCESS ? NO_ERROR : ERROR_INVALID_PARAMETER;
}
