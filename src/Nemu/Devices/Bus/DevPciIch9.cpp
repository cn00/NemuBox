...
/** @def NEMU_ICH9PCI_SAVED_STATE_VERSION
 * Saved state version of the ICH9 PCI bus device.
 */
#define NEMU_ICH9PCI_SAVED_STATE_VERSION_NOMSI 1
#define NEMU_ICH9PCI_SAVED_STATE_VERSION_MSI   2
#define NEMU_ICH9PCI_SAVED_STATE_VERSION_CURRENT NEMU_ICH9PCI_SAVED_STATE_VERSION_MSI
...
DECLINLINE(PPCIDEVICE) ich9pciFindBridge(PICH9PCIBUS pBus, uint8_t iBus)
{
    ...
    for (uint32_t iBridge = 0; iBridge < pBus->cBridges; iBridge++)
    {
        ...
        uint32_t uSecondary   = PCIDevGetByte(pBridge, NEMU_PCI_SECONDARY_BUS);
        uint32_t uSubordinate = PCIDevGetByte(pBridge, NEMU_PCI_SUBORDINATE_BUS);
...
static void ich9pciUpdateMappings(PCIDevice* pDev)
{
    ...
    int iCmd = ich9pciGetWord(pDev, NEMU_PCI_COMMAND);
...
static DECLCALLBACK(void) ich9pcibridgeConfigWrite(PPDMDEVINSR3 pDevIns, uint8_t iBus, uint8_t iDevice, uint32_t u32Address, uint32_t u32Value, unsigned cb)
{
    ...
    if (iBus != PCIDevGetByte(&pBus->aPciDev, NEMU_PCI_SECONDARY_BUS))
    {
...
static DECLCALLBACK(int) ich9pciR3CommonLoadExec(PICH9PCIBUS pBus, PSSMHANDLE pSSM, uint32_t uVersion, uint32_t uPass)
{
    ...
    if (uVersion != NEMU_ICH9PCI_SAVED_STATE_VERSION_CURRENT)
    ...
    for (i = 0; i < RT_ELEMENTS(pBus->apDevices); i++)
    {
        ...
        if (pDev)
        {
            ...
            pDev->Int.s.pfnConfigWrite(pDev, NEMU_PCI_COMMAND, 0, 2);
...
static DECLCALLBACK(int) ich9pciR3LoadExec(PPDMDEVINS pDevIns, PSSMHANDLE pSSM, uint32_t uVersion, uint32_t uPass)
{
    ...
    if (uVersion == NEMU_ICH9PCI_SAVED_STATE_VERSION_NOMSI)
        return VERR_SSM_UNSUPPORTED_DATA_UNIT_VERSION;
    if (uVersion > NEMU_ICH9PCI_SAVED_STATE_VERSION_MSI)
        return VERR_SSM_UNSUPPORTED_DATA_UNIT_VERSION;
...
static void ich9pciSetRegionAddress(PICH9PCIGLOBALS pGlobals, uint8_t uBus, uint8_t uDevFn, int iRegion, uint64_t addr)
{
    ...
    uint16_t uCmd = ich9pciConfigRead(pGlobals, uBus, uDevFn, NEMU_PCI_COMMAND, 2);
    ...
    ich9pciConfigWrite(pGlobals, uBus, uDevFn, NEMU_PCI_COMMAND, uCmd, 2);
}
...
static void ich9pciBiosInitBridge(PICH9PCIGLOBALS pGlobals, uint8_t uBus, uint8_t uDevFn)
{
    ...
    ich9pciConfigWrite(pGlobals, uBus, uDevFn, NEMU_PCI_IO_BASE, (pGlobals->uPciBiosIo >> 8) & 0xf0, 1);
    ...
    ich9pciConfigWrite(pGlobals, uBus, uDevFn, NEMU_PCI_MEMORY_BASE, (pGlobals->uPciBiosMmio >> 16) & UINT32_C(0xffff0), 2);
    ...
    uint8_t uBridgeBus = ich9pciConfigRead(pGlobals, uBus, uDevFn, NEMU_PCI_SECONDARY_BUS, 1);
    ...
    ich9pciConfigWrite(pGlobals, uBus, uDevFn, NEMU_PCI_IO_LIMIT, ((pGlobals->uPciBiosIo >> 8) & 0xf0) - 1, 1);
    ...
    ich9pciConfigWrite(pGlobals, uBus, uDevFn, NEMU_PCI_MEMORY_LIMIT, ((pGlobals->uPciBiosMmio >> 16) & UINT32_C(0xfff0)) - 1, 2);
     ...
     */
    ich9pciConfigWrite(pGlobals, uBus, uDevFn, NEMU_PCI_PREF_MEMORY_BASE, 0xfff0, 2);
    ich9pciConfigWrite(pGlobals, uBus, uDevFn, NEMU_PCI_PREF_MEMORY_LIMIT, 0x0, 2);
    ich9pciConfigWrite(pGlobals, uBus, uDevFn, NEMU_PCI_PREF_BASE_UPPER32, 0x00, 4);
    ich9pciConfigWrite(pGlobals, uBus, uDevFn, NEMU_PCI_PREF_LIMIT_UPPER32, 0x00, 4);
}
...
static void ich9pciBiosInitDevice(PICH9PCIGLOBALS pGlobals, uint8_t uBus, uint8_t uDevFn)
{
    ...
    uDevClass  = ich9pciConfigRead(pGlobals, uBus, uDevFn, NEMU_PCI_CLASS_DEVICE, 2);
    uVendor    = ich9pciConfigRead(pGlobals, uBus, uDevFn, NEMU_PCI_VENDOR_ID, 2);
    uDevice    = ich9pciConfigRead(pGlobals, uBus, uDevFn, NEMU_PCI_DEVICE_ID, 2);
    ...
    switch (uDevClass)
    {
        ...
        case 0x0300:
             ...
             */
            uCmd = ich9pciConfigRead(pGlobals, uBus, uDevFn, NEMU_PCI_COMMAND, 1);
            ich9pciConfigWrite(pGlobals, uBus, uDevFn, NEMU_PCI_COMMAND,
    ...
    /* map the interrupt */
    uint32_t iPin = ich9pciConfigRead(pGlobals, uBus, uDevFn, NEMU_PCI_INTERRUPT_PIN, 1);
    if (iPin != 0)
    {
        iPin--;
        ...
        if (uBus != 0)
        {
            ...
            while (1)
            {
                ...
                if (uBus == PCIDevGetByte(pBridge, NEMU_PCI_SECONDARY_BUS))
                {
        ...
        ich9pciConfigWrite(pGlobals, uBus, uDevFn, NEMU_PCI_INTERRUPT_LINE, iIrq, 1);
    }
}
                                      ...
                                      unsigned uBusSecondary)
{
    ...
    if (uBusSecondary != 0)
    {
        PCIDevSetByte(pBridgeDev, NEMU_PCI_PRIMARY_BUS, uBusPrimary);
        PCIDevSetByte(pBridgeDev, NEMU_PCI_SECONDARY_BUS, uBusSecondary);
    }
    ...
    PCIDevSetByte(pBridgeDev, NEMU_PCI_SUBORDINATE_BUS, pGlobals->uBus);
    Log2(("ich9pciInitBridgeTopology: for bus %p: primary=%d secondary=%d subordinate=%d\n",
          pBus,
          PCIDevGetByte(pBridgeDev, NEMU_PCI_PRIMARY_BUS),
          PCIDevGetByte(pBridgeDev, NEMU_PCI_SECONDARY_BUS),
          PCIDevGetByte(pBridgeDev, NEMU_PCI_SUBORDINATE_BUS)
          ));
}
                                                ...
                                                uint32_t val, unsigned len)
{
    ...
    uint8_t  u8HeaderType = ich9pciGetByte(aDev, NEMU_PCI_HEADER_TYPE);
    ...
    for (uint32_t i = 0; i < len; i++)
    {
        ...
        switch (u8HeaderType)
        {
            ...
            case 0x80: /* multi-function device */
                ...
                {
                    /* Read-only registers  */
                    case NEMU_PCI_VENDOR_ID: case NEMU_PCI_VENDOR_ID+1:
                    case NEMU_PCI_DEVICE_ID: case NEMU_PCI_DEVICE_ID+1:
                    case NEMU_PCI_REVISION_ID:
                    case NEMU_PCI_CLASS_PROG:
                    case NEMU_PCI_CLASS_SUB:
                    case NEMU_PCI_CLASS_BASE:
                    case NEMU_PCI_HEADER_TYPE:
                    case NEMU_PCI_SUBSYSTEM_VENDOR_ID: case NEMU_PCI_SUBSYSTEM_VENDOR_ID+1:
                    case NEMU_PCI_SUBSYSTEM_ID: case NEMU_PCI_SUBSYSTEM_ID+1:
                    case NEMU_PCI_ROM_ADDRESS: case NEMU_PCI_ROM_ADDRESS+1: case NEMU_PCI_ROM_ADDRESS+2: case NEMU_PCI_ROM_ADDRESS+3:
                    case NEMU_PCI_CAPABILITY_LIST:
                    case NEMU_PCI_INTERRUPT_PIN:
                        fWritable = false;
                        break;
            ...
            case 0x01: /* PCI-PCI bridge */
                ...
                switch (addr)
                {
                    ...
                    case NEMU_PCI_CLASS_BASE:
                    case NEMU_PCI_HEADER_TYPE:
                    case NEMU_PCI_ROM_ADDRESS_BR: case NEMU_PCI_ROM_ADDRESS_BR+1: case NEMU_PCI_ROM_ADDRESS_BR+2: case NEMU_PCI_ROM_ADDRESS_BR+3:
                    case NEMU_PCI_INTERRUPT_PIN:
                        fWritable = false;
                        break;
                    default:
        ...
        switch (addr)
        {
            case NEMU_PCI_COMMAND: /* Command register, bits 0-7. */
                fUpdateMappings = true;
                goto default_case;
            case NEMU_PCI_COMMAND+1: /* Command register, bits 8-15. */
                /* don't change reserved bits (11-15) */
                u8Val &= UINT32_C(~0xf8);
                fUpdateMappings = true;
                goto default_case;
            case NEMU_PCI_STATUS:  /* Status register, bits 0-7. */
                ...
                break;
            case NEMU_PCI_STATUS+1:  /* Status register, bits 8-15. */
            ...
            case NEMU_PCI_ROM_ADDRESS:    case NEMU_PCI_ROM_ADDRESS   +1: case NEMU_PCI_ROM_ADDRESS   +2: case NEMU_PCI_ROM_ADDRESS   +3:
                fRom = true;
            case NEMU_PCI_BASE_ADDRESS_0: case NEMU_PCI_BASE_ADDRESS_0+1: case NEMU_PCI_BASE_ADDRESS_0+2: case NEMU_PCI_BASE_ADDRESS_0+3:
            case NEMU_PCI_BASE_ADDRESS_1: case NEMU_PCI_BASE_ADDRESS_1+1: case NEMU_PCI_BASE_ADDRESS_1+2: case NEMU_PCI_BASE_ADDRESS_1+3:
            case NEMU_PCI_BASE_ADDRESS_2: case NEMU_PCI_BASE_ADDRESS_2+1: case NEMU_PCI_BASE_ADDRESS_2+2: case NEMU_PCI_BASE_ADDRESS_2+3:
            case NEMU_PCI_BASE_ADDRESS_3: case NEMU_PCI_BASE_ADDRESS_3+1: case NEMU_PCI_BASE_ADDRESS_3+2: case NEMU_PCI_BASE_ADDRESS_3+3:
            case NEMU_PCI_BASE_ADDRESS_4: case NEMU_PCI_BASE_ADDRESS_4+1: case NEMU_PCI_BASE_ADDRESS_4+2: case NEMU_PCI_BASE_ADDRESS_4+3:
            case NEMU_PCI_BASE_ADDRESS_5: case NEMU_PCI_BASE_ADDRESS_5+1: case NEMU_PCI_BASE_ADDRESS_5+2: case NEMU_PCI_BASE_ADDRESS_5+3:
            {
                ...
                {
                    int iRegion = fRom ? NEMU_PCI_ROM_SLOT : (addr - NEMU_PCI_BASE_ADDRESS_0) >> 2;
    ...
    for (uint32_t iDev = 0; iDev < RT_ELEMENTS(pBus->apDevices); iDev++)
    {
        ...
        if (pPciDev != NULL)
        {
            ...
            pHlp->pfnPrintf(pHlp, "%02x:%02x:%02x %s%s: %04x-%04x",
                            ...
                            ich9pciGetWord(pPciDev, NEMU_PCI_VENDOR_ID), ich9pciGetWord(pPciDev, NEMU_PCI_DEVICE_ID)
                            );
            if (ich9pciGetByte(pPciDev, NEMU_PCI_INTERRUPT_PIN) != 0)
                pHlp->pfnPrintf(pHlp, " IRQ%d", ich9pciGetByte(pPciDev, NEMU_PCI_INTERRUPT_LINE));
            pHlp->pfnPrintf(pHlp, "\n");
            ...
            uint16_t iCmd = ich9pciGetWord(pPciDev, NEMU_PCI_COMMAND);
            if ((iCmd & (NEMU_PCI_COMMAND_IO | NEMU_PCI_COMMAND_MEMORY)) != 0)
            {
                for (int iRegion = 0; iRegion < PCI_NUM_REGIONS; iRegion++)
                {
            ...
            printIndent(pHlp, iIndent + 2);
            uint16_t iStatus = ich9pciGetWord(pPciDev, NEMU_PCI_STATUS);
            pHlp->pfnPrintf(pHlp, "Command: %04X, Status: %04X\n",
                            iCmd, iStatus);
            printIndent(pHlp, iIndent + 2);
            pHlp->pfnPrintf(pHlp, "Bus master: %s\n",
                            iCmd & NEMU_PCI_COMMAND_MASTER ? "Yes" : "No");
                                          ...
                                          PCFGMNODE  pCfg)
{
    ...
    PCIDevSetWord(&pBus->aPciDev,  NEMU_PCI_SEC_STATUS, 0x0280);  /* secondary status */
    ...
    rc = PDMDevHlpSSMRegisterEx(pDevIns, NEMU_ICH9PCI_SAVED_STATE_VERSION_CURRENT,
...
static void ich9pciResetDevice(PPCIDEVICE pDev)
{
    ...
    else
    {
        PCIDevSetCommand(pDev,
                         ...
                         &
                         ~(NEMU_PCI_COMMAND_IO | NEMU_PCI_COMMAND_MEMORY |
                           NEMU_PCI_COMMAND_MASTER | NEMU_PCI_COMMAND_SPECIAL |
                           NEMU_PCI_COMMAND_PARITY | NEMU_PCI_COMMAND_SERR |
                           NEMU_PCI_COMMAND_FAST_BACK | NEMU_PCI_COMMAND_INTX_DISABLE));

        /* Bridge device reset handlers processed later */
        if (!pciDevIsPci2PciBridge(pDev))
        {
            PCIDevSetByte(pDev, NEMU_PCI_CACHE_LINE_SIZE, 0x0);
        ...
        if (pciDevIsMsiCapable(pDev))
        {
            ...
            pDev->config[pDev->Int.s.u8MsiCapOffset + NEMU_MSI_CAP_MESSAGE_CONTROL] &= 0x8e;
        }
        ...
        if (pciDevIsMsixCapable(pDev))
        {
            ...
            pDev->config[pDev->Int.s.u8MsixCapOffset + NEMU_MSIX_CAP_MESSAGE_CONTROL + 1] &= 0x3f;
        }
    }
}
...
static void ich9pcibridgeReset(PPDMDEVINS pDevIns)
{
    ...
    PCIDevSetByte(&pBus->aPciDev, NEMU_PCI_PRIMARY_BUS, 0);
    PCIDevSetByte(&pBus->aPciDev, NEMU_PCI_SECONDARY_BUS, 0);
    PCIDevSetByte(&pBus->aPciDev, NEMU_PCI_SUBORDINATE_BUS, 0);
