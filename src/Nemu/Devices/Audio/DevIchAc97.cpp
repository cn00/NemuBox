/* $Id: DevIchAc97.cpp $ */
/** @file
 * DevIchAc97 - Nemu ICH AC97 Audio Controller.
 */
...
static DECLCALLBACK(int) ichac97Construct(PPDMDEVINS pDevIns, int iInstance, PCFGMNODE pCfg)
{
    ...
    PCIDevSetStatus           (&pThis->PciDev, NEMU_PCI_STATUS_DEVSEL_MEDIUM |  NEMU_PCI_STATUS_FAST_BACK); /* 06 rwc?,ro? - pcists. */      Assert(pThis->PciDev.config[0x06] == 0x80); Assert(pThis->PciDev.config[0x07] == 0x02);
