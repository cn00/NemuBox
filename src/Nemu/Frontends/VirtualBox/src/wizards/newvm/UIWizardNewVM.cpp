...
/** @file
 * Nemu Qt GUI - UIWizardNewVM class implementation.
 */
...
bool UIWizardNewVM::createVM()
{
    ...
    if (m_machine.isNull())
    {
        ...
        if (!m_strGroup.isEmpty())
            groups << m_strGroup;
        m_machine = nemu.CreateMachine(QString(), field("name").toString(), groups, strTypeId, QString::null);
        if (!nemu.isOk())
        {
            msgCenter().cannotCreateMachine(nemu, this);
            return false;
        }
    ...
    m_machine.SetVRAMSize(qMax(type.GetRecommendedVRAM(), (ULONG)(NemuGlobal::requiredVideoMemory(strTypeId) / _1M)));
    ...
    if (!usbDeviceFilters.isNull() && type.GetRecommendedUSB() && m_machine.GetUSBProxyAvailable())
    {
        ...
        CExtPackManager manager = nemuGlobal().virtualBox().GetExtensionPackManager();
    ...
    nemu.RegisterMachine(m_machine);
    if (!nemu.isOk())
    {
        msgCenter().cannotRegisterMachine(nemu, m_machine.GetName(), this);
        return false;
    }
    ...
    /* Attach default devices: */
    {
        ...
        if (!session.isNull())
        {
            ...
            if (!strId.isNull())
            {
                UIMedium vmedium = nemuGlobal().medium(strId);
