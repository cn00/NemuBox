/* $Id: NemuDD.cpp $ */
/** @file
 * NemuDD - Built-in drivers & devices (part 1).
 */
...
*   Global Variables                                                                                                             *
*********************************************************************************************************************************/
const void *g_apvNemuDDDependencies[] =
{
    NULL,
};
...
extern "C" DECLEXPORT(int) NemuDevicesRegister(PPDMDEVREGCB pCallbacks, uint32_t u32Version)
{
    ...
    if (RT_FAILURE(rc))
        return rc;
#endif
#ifdef NEMU_WITH_PCI_PASSTHROUGH_IMPL
 ...
 * @param   u32Version      Nemu version number.
 */
extern "C" DECLEXPORT(int) NemuDriversRegister(PCPDMDRVREGCB pCallbacks, uint32_t u32Version)
{
    LogFlow(("NemuDriversRegister: u32Version=%#x\n", u32Version));
    AssertReleaseMsg(u32Version == NEMU_VERSION, ("u32Version=%#x NEMU_VERSION=%#x\n", u32Version, NEMU_VERSION));
...
# endif
    ...
    if (RT_FAILURE(rc))
        return rc;
#endif
#ifdef NEMU_WITH_USB_VIDEO_IMPL
    ...
    if (RT_FAILURE(rc))
        return rc;
#if defined(RT_OS_LINUX) || defined(NEMU_WITH_WIN_PARPORT_SUP)
 ...
 * @param   u32Version      Nemu version number.
 */
extern "C" DECLEXPORT(int) NemuUsbRegister(PCPDMUSBREGCB pCallbacks, uint32_t u32Version)
{
    int rc = VINF_SUCCESS;
