...
/** @file
 * Nemu Qt GUI - UIMachineSettingsParallel class implementation.
 */
    ...
    , m_iSlot(-1)
{
    ...
    mCbNumber->insertItem (0, nemuGlobal().toCOMPortName (0, 0));
    mCbNumber->insertItems (0, nemuGlobal().COMPortNames());
...
void UIMachineSettingsParallel::polishTab()
{
    /* Polish port page: */
    ulong uIRQ, uIOBase;
    bool fStd = nemuGlobal().toCOMPortNumbers(mCbNumber->currentText(), uIRQ, uIOBase);
...
void UIMachineSettingsParallel::fetchPortData(const UICacheSettingsMachineParallelPort &portCache)
{
    ...
    mCbNumber->setCurrentIndex(mCbNumber->findText(nemuGlobal().toCOMPortName(portData.m_uIRQ, portData.m_uIOBase)));
...
{
    ulong a, b;
    return !nemuGlobal().toCOMPortNumbers (mCbNumber->currentText(), a, b);
}
...
void UIMachineSettingsParallel::retranslateUi()
{
    ...
    mCbNumber->setItemText (mCbNumber->count() - 1, nemuGlobal().toCOMPortName (0, 0));
}
...
{
    ulong IRQ, IOBase;
    bool std = nemuGlobal().toCOMPortNumbers (aText, IRQ, IOBase);
...
bool UIMachineSettingsParallelPage::validate(QList<UIValidationMessage> &messages)
{
    ...
    for (int iIndex = 0; iIndex < mTabWidget->count(); ++iIndex)
    {
        ...
        message.first = nemuGlobal().removeAccelMark(mTabWidget->tabText(mTabWidget->indexOf(pTab)));
