...
static DECLCALLBACK(void) svcCall (void *, NEMUHGCMCALLHANDLE callHandle, uint32_t u32ClientID, void *pvClient, uint32_t u32Function, uint32_t cParms, NEMUHGCMSVCPARM paParms[])
{
    ...
    switch (u32Function)
    {
        case SHFL_FN_QUERY_MAPPINGS:
        {
            ...
            else if (   paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT   /* flags */
                     || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT   /* numberOfMappings */
                     || paParms[2].type != NEMU_HGCM_SVC_PARM_PTR     /* mappings */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_QUERY_MAP_NAME:
        {
            ...
            else if (   paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT /* Root. */
                     || paParms[1].type != NEMU_HGCM_SVC_PARM_PTR   /* Name. */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_CREATE:
        {
            ...
            if (cParms != SHFL_CPARMS_CREATE)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != NEMU_HGCM_SVC_PARM_PTR   /* path */
                     || paParms[2].type != NEMU_HGCM_SVC_PARM_PTR   /* parms */
                    )
            {
        ...
        case SHFL_FN_CLOSE:
        {
            ...
            if (cParms != SHFL_CPARMS_CLOSE)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != NEMU_HGCM_SVC_PARM_64BIT   /* handle */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_READ:
            ...
            if (   paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT   /* root */
                || paParms[1].type != NEMU_HGCM_SVC_PARM_64BIT   /* handle */
                || paParms[2].type != NEMU_HGCM_SVC_PARM_64BIT   /* offset */
                || paParms[3].type != NEMU_HGCM_SVC_PARM_32BIT   /* count */
                || paParms[4].type != NEMU_HGCM_SVC_PARM_PTR     /* buffer */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_LOCK:
            ...
            if (   paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT   /* root */
                || paParms[1].type != NEMU_HGCM_SVC_PARM_64BIT   /* handle */
                || paParms[2].type != NEMU_HGCM_SVC_PARM_64BIT   /* offset */
                || paParms[3].type != NEMU_HGCM_SVC_PARM_64BIT   /* length */
                || paParms[4].type != NEMU_HGCM_SVC_PARM_32BIT   /* flags */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_LIST:
        {
            ...
            if (   paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT   /* root */
                || paParms[1].type != NEMU_HGCM_SVC_PARM_64BIT   /* handle */
                || paParms[2].type != NEMU_HGCM_SVC_PARM_32BIT   /* flags */
                || paParms[3].type != NEMU_HGCM_SVC_PARM_32BIT   /* cb */
                || paParms[4].type != NEMU_HGCM_SVC_PARM_PTR     /* pPath */
                || paParms[5].type != NEMU_HGCM_SVC_PARM_PTR     /* buffer */
                || paParms[6].type != NEMU_HGCM_SVC_PARM_32BIT   /* resumePoint */
                || paParms[7].type != NEMU_HGCM_SVC_PARM_32BIT   /* cFiles (out) */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_READLINK:
        {
            ...
            if (   paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT   /* root */
                || paParms[1].type != NEMU_HGCM_SVC_PARM_PTR     /* path */
                || paParms[2].type != NEMU_HGCM_SVC_PARM_PTR     /* buffer */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_MAP_FOLDER_OLD:
        {
            ...
            else if (   paParms[0].type != NEMU_HGCM_SVC_PARM_PTR     /* path */
                     || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT   /* root */
                     || paParms[2].type != NEMU_HGCM_SVC_PARM_32BIT   /* delimiter */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_MAP_FOLDER:
        {
            ...
            if (cParms != SHFL_CPARMS_MAP_FOLDER)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT   /* root */
                     || paParms[2].type != NEMU_HGCM_SVC_PARM_32BIT   /* delimiter */
                     || paParms[3].type != NEMU_HGCM_SVC_PARM_32BIT   /* fCaseSensitive */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
        ...
        case SHFL_FN_RENAME:
        {
            ...
            if (cParms != SHFL_CPARMS_RENAME)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != NEMU_HGCM_SVC_PARM_PTR   /* src */
                     || paParms[2].type != NEMU_HGCM_SVC_PARM_PTR   /* dest */
        ...
        case SHFL_FN_SYMLINK:
        {
            ...
            if (cParms != SHFL_CPARMS_SYMLINK)
            {
                rc = VERR_INVALID_PARAMETER;
                     ...
                     || paParms[1].type != NEMU_HGCM_SVC_PARM_PTR     /* newPath */
                     || paParms[2].type != NEMU_HGCM_SVC_PARM_PTR     /* oldPath */
                     || paParms[3].type != NEMU_HGCM_SVC_PARM_PTR     /* info */
                    )
            {
                rc = VERR_INVALID_PARAMETER;
            }
...
static DECLCALLBACK(int) svcHostCall (void *, uint32_t u32Function, uint32_t cParms, NEMUHGCMSVCPARM paParms[])
{
    ...
    case SHFL_FN_ADD_MAPPING:
    {
        ...
        else if (   paParms[0].type != NEMU_HGCM_SVC_PARM_PTR     /* host folder name */
                 || paParms[1].type != NEMU_HGCM_SVC_PARM_PTR     /* guest map name */
                 || paParms[2].type != NEMU_HGCM_SVC_PARM_32BIT   /* fFlags */
                )
        {
            rc = VERR_INVALID_PARAMETER;
        }
    ...
    case SHFL_FN_REMOVE_MAPPING:
    {
        ...
        else if (   paParms[0].type != NEMU_HGCM_SVC_PARM_PTR     /* folder name */
                )
        {
            rc = VERR_INVALID_PARAMETER;
        }
...
extern "C" DECLCALLBACK(DECLEXPORT(int)) NemuHGCMSvcLoad (NEMUHGCMSVCFNTABLE *ptable)
{
    ...
    Log(("SharedFolders host service: NemuHGCMSvcLoad: ptable = %p\n", ptable));
    ...
    {
        Log(("SharedFolders host service: NemuHGCMSvcLoad: ptable->cbSize = %u, ptable->u32Version = 0x%08X\n",
            ...
            ||  ptable->u32Version != NEMU_HGCM_SVC_VERSION)
        {
            LogRelFunc(("SharedFolders host service: Version mismatch while loading: ptable->cbSize = %u (should be %u), ptable->u32Version = 0x%08X (should be 0x%08X)\n",
                        ptable->cbSize, sizeof (NEMUHGCMSVCFNTABLE), ptable->u32Version, NEMU_HGCM_SVC_VERSION));
