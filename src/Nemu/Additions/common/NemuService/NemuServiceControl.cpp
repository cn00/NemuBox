/* $Id: NemuServiceControl.cpp $ */
/** @file
 * NemuServiceControl - Host-driven Guest Control.
 */
...
/** @page pg_vgsvc_gstctrl NemuService - Guest Control
 *
 ...
 * operates directly as the NemuService user (root / system service account).
 *
 ...
 * directly from NemuManage by the user by prepending the 'nemu_' prefix to the
 * unix command.
 *
 */
...
/** List of guest control session threads (NEMUSERVICECTRLSESSIONTHREAD).
 ...
 *  under behalf of the NemuService main process. On newer protocol versions
 *  each session is a forked version of NemuService using the appropriate
 *  user credentials for opening a guest session. These forked sessions then
 *  are kept in NEMUSERVICECTRLSESSIONTHREAD structures. */
NEMUSERVICECTRLSESSION      g_Session;
...
static DECLCALLBACK(int) vgsvcGstCtrlOption(const char **ppszShort, int argc, char **argv, int *pi)
{
    ...
    {
        g_Session.fFlags |= NEMUSERVICECTRLSESSION_FLAG_DUMPSTDOUT;
        rc = 0; /* Flag this command as parsed. */
    }
    else if (!strcmp(argv[*pi], "--control-dump-stderr"))
    {
        g_Session.fFlags |= NEMUSERVICECTRLSESSION_FLAG_DUMPSTDERR;
        rc = 0; /* Flag this command as parsed. */
    }
#endif
    return rc;
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            switch (uMsg)
            {
                ...
                default:
                {
                     ...
                     * under behalf of NemuService's main executable.
                     *
                    ...
                    else
                    {
                         ...
                         * up to the guest session fork of NemuService, so just
...
static int vgsvcGstCtrlHandleSessionOpen(PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    ...
    NEMUSERVICECTRLSESSIONSTARTUPINFO ssInfo = { 0 };
...
static int vgsvcGstCtrlHandleSessionClose(PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        PNEMUSERVICECTRLSESSIONTHREAD pThread;
        RTListForEach(&g_lstControlSessionThreads, pThread, NEMUSERVICECTRLSESSIONTHREAD, Node)
        {
