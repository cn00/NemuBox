...
static char gachWindowClassName[] = "NemuSharedClipboardClass";
...
#ifdef LOG_ENABLED
void nemuClipboardDump(const void *pv, size_t cb, uint32_t u32Format)
{
    if (u32Format & NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT)
    {
        Log(("DUMP: NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT:\n"));
        if (pv && cb)
        {
            Log(("%ls\n", pv));
        }
    ...
    else if (u32Format & NEMU_SHARED_CLIPBOARD_FMT_BITMAP)
    {
        dprintf(("DUMP: NEMU_SHARED_CLIPBOARD_FMT_BITMAP\n"));
    }
    else if (u32Format & NEMU_SHARED_CLIPBOARD_FMT_HTML)
    {
        Log(("DUMP: NEMU_SHARED_CLIPBOARD_FMT_HTML:\n"));
        if (pv && cb)
        {
            Log(("%s\n", pv));
        }
...
#else
#define nemuClipboardDump(__pv, __cb, __format) do { NOREF(__pv); NOREF(__cb); NOREF(__format); } while (0)
#endif /* LOG_ENABLED */

static void nemuClipboardGetData (uint32_t u32Format, const void *pvSrc, uint32_t cbSrc,
                                  void *pvDst, uint32_t cbDst, uint32_t *pcbActualDst)
{
    dprintf (("nemuClipboardGetData.\n"));

    *pcbActualDst = cbSrc;

    LogFlow(("nemuClipboardGetData cbSrc = %d, cbDst = %d\n", cbSrc, cbDst));
    ...
    nemuClipboardDump(pvDst, cbSrc, u32Format);

    return;
}

static int nemuClipboardReadDataFromClient (NEMUCLIPBOARDCONTEXT *pCtx, uint32_t u32Format)
{
    Assert(pCtx->pClient);
    Assert(pCtx->pClient->data.pv == NULL && pCtx->pClient->data.cb == 0 && pCtx->pClient->data.u32Format == 0);

    LogFlow(("nemuClipboardReadDataFromClient u32Format = %02X\n", u32Format));

    ResetEvent (pCtx->hRenderEvent);

    nemuSvcClipboardReportMsg (pCtx->pClient, NEMU_SHARED_CLIPBOARD_HOST_MSG_READ_DATA, u32Format);

    WaitForSingleObject(pCtx->hRenderEvent, INFINITE);

    LogFlow(("nemuClipboardReadDataFromClient wait completed\n"));

    return VINF_SUCCESS;
}

static void nemuClipboardChanged (NEMUCLIPBOARDCONTEXT *pCtx)
{
    LogFlow(("nemuClipboardChanged\n"));
    ...
    if (OpenClipboard (pCtx->hwnd))
    {
        ...
        {
            LogFlow(("nemuClipboardChanged format %#x\n", format));
            switch (format)
            {
                case CF_UNICODETEXT:
                case CF_TEXT:
                    u32Formats |= NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT;
                    break;

                case CF_DIB:
                case CF_BITMAP:
                    u32Formats |= NEMU_SHARED_CLIPBOARD_FMT_BITMAP;
                    break;
                ...
                default:
                    if (format >= 0xC000)
                    {
                        ...
                        if (cActual)
                        {
                            ...
                            {
                                u32Formats |= NEMU_SHARED_CLIPBOARD_FMT_HTML;
                            }
                        }
                    }
                    break;
        ...
        LogFlow(("nemuClipboardChanged u32Formats %02X\n", u32Formats));

        nemuSvcClipboardReportMsg (pCtx->pClient, NEMU_SHARED_CLIPBOARD_HOST_MSG_FORMATS, u32Formats);
    }
}

/* Add ourselves into the chain of cliboard listeners */
static void addToCBChain (NEMUCLIPBOARDCONTEXT *pCtx)
{
    pCtx->hwndNextInChain = SetClipboardViewer (pCtx->hwnd);
}

/* Remove ourselves from the chain of cliboard listeners */
static void removeFromCBChain (NEMUCLIPBOARDCONTEXT *pCtx)
{
...
VOID CALLBACK CBChainPingProc(HWND hwnd, UINT uMsg, ULONG_PTR dwData, LRESULT lResult)
{
    ...
    (void) uMsg;
    (void) lResult;
    NEMUCLIPBOARDCONTEXT *pCtx = (NEMUCLIPBOARDCONTEXT *)dwData;
    pCtx->fCBChainPingInProcess = FALSE;
}

static LRESULT CALLBACK nemuClipboardWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    LRESULT rc = 0;

    NEMUCLIPBOARDCONTEXT *pCtx = &g_ctx;
    ...
    switch (msg)
    {
        ...
        case WM_RENDERFORMAT:
        {
            ...
            {
                case CF_UNICODETEXT:
                    u32Format |= NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT;
                    break;

                case CF_DIB:
                    u32Format |= NEMU_SHARED_CLIPBOARD_FMT_BITMAP;
                    break;
                ...
                default:
                    if (format >= 0xC000)
                    {
                        ...
                        if (cActual)
                        {
                            if (strcmp (szFormatName, "HTML Format") == 0)
                            {
                                u32Format |= NEMU_SHARED_CLIPBOARD_FMT_HTML;
                            }
                        }
                    }
                    break;
            ...
            {
                int nemurc = nemuClipboardReadDataFromClient (pCtx, u32Format);

                dprintf(("nemuClipboardReadDataFromClient nemurc = %d\n", nemurc));

                if (   RT_SUCCESS (nemurc)
                    ...
                    && pCtx->pClient->data.u32Format == u32Format)
                {
                    ...
                    if (hMem)
                    {
                        ...
                        if (pMem)
                        {
                            ...
                            dprintf(("nemuClipboardHostEvent hClip %p\n", hClip));
        ...
        case WM_USER:
        {
            ...
            if (OpenClipboard (hwnd))
            {
                ...
                if (u32Formats & NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT)
                {
                    dprintf(("window proc WM_USER: NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT\n"));

                    hClip = SetClipboardData (CF_UNICODETEXT, NULL);
                }

                if (u32Formats & NEMU_SHARED_CLIPBOARD_FMT_BITMAP)
                {
                    dprintf(("window proc WM_USER: NEMU_SHARED_CLIPBOARD_FMT_BITMAP\n"));

                    hClip = SetClipboardData (CF_DIB, NULL);
                }

                if (u32Formats & NEMU_SHARED_CLIPBOARD_FMT_HTML)
                {
                    UINT format = RegisterClipboardFormat ("HTML Format");
                    dprintf(("window proc WM_USER: NEMU_SHARED_CLIPBOARD_FMT_HTML 0x%04X\n", format));
                    if (format != 0)
                    {
...
DECLCALLBACK(int) NemuClipboardThread (RTTHREAD ThreadSelf, void *pInstance)
{
    /* Create a window and make it a clipboard viewer. */
    int rc = VINF_SUCCESS;

    LogFlow(("NemuClipboardThread\n"));
    ...
    wc.style         = CS_NOCLOSE;
    wc.lpfnWndProc   = nemuClipboardWndProc;
    wc.cbClsExtra    = 0;
    wc.cbWndExtra    = 0;
...
int nemuClipboardInit (void)
{
    ...
    rc = RTThreadCreate (&g_ctx.thread, NemuClipboardThread, NULL, 65536,
...
{
    /* Sync the host clipboard content with the client. */
    nemuClipboardChanged (pClient->pCtx);
...
{
    LogFlow(("nemuClipboardReadData: u32Format = %02X\n", u32Format));
    ...
    if (OpenClipboard (pClient->pCtx->hwnd))
    {
        ...
        if (u32Format & NEMU_SHARED_CLIPBOARD_FMT_BITMAP)
        {
            ...
            if (hClip != NULL)
            {
                ...
                if (lp != NULL)
                {
                    ...
                    nemuClipboardGetData (NEMU_SHARED_CLIPBOARD_FMT_BITMAP, lp, GlobalSize (hClip),
                                          pv, cb, pcbActual);
        ...
        else if (u32Format & NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT)
        {
            ...
            if (hClip != NULL)
            {
                ...
                if (uniString != NULL)
                {
                    ...
                    nemuClipboardGetData (NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT, uniString, (lstrlenW (uniString) + 1) * 2,
                                          pv, cb, pcbActual);
            ...
            if (format != 0)
            {
                ...
                if (hClip != NULL)
                {
                    ...
                    if (lp != NULL)
                    {
                        ...
                        nemuClipboardGetData (NEMU_SHARED_CLIPBOARD_FMT_HTML, lp, GlobalSize (hClip),
                                              pv, cb, pcbActual);
...
{
    LogFlow(("nemuClipboardWriteData\n"));
    ...
    nemuClipboardDump(pv, cb, u32Format);
