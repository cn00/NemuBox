 ...
 *      Dummy for forcing ring-3 handling of the access.}
 */
VMMDECL(NEMUSTRICTRC) pgmPhysPfHandlerRedirectToHC(PVM pVM, PVMCPU pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pRegFrame,
 ...
 */
DECLEXPORT(NEMUSTRICTRC) pgmPhysRomWritePfHandler(PVM pVM, PVMCPU pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pRegFrame,
                                                  RTGCPTR pvFault, RTGCPHYS GCPhysFault, void *pvUser)
{
    int             rc;
 ...
 *
 * @returns The following Nemu status codes.
...
# endif
     ...
     * When NEMU_WITH_NEW_LAZY_PAGE_ALLOC isn't defined, there shouldn't be any.
     */
...
#if defined(IN_RC) || defined(NEMU_WITH_2X_4GB_ADDR_SPACE_IN_R0)
    /*
     * Map it by HCPhys.
     */
...
#endif /* !IN_RC && !NEMU_WITH_2X_4GB_ADDR_SPACE_IN_R0 */
...
#if 0 /*defined(IN_RC) || defined(NEMU_WITH_2X_4GB_ADDR_SPACE_IN_R0)*/
 ...
 *                      NEMU_WITH_2X_4GB_ADDR_SPACE_IN_R0 build option.
 *
 ...
 *
 * @returns Strict Nemu status code in ring-0 and raw-mode, ignorable in ring-3.
 ...
 */
static NEMUSTRICTRC pgmPhysReadHandler(PVM pVM, PPGMPAGE pPage, RTGCPHYS GCPhys, void *pvBuf, size_t cb,
...
/** @todo Check how this can work for MMIO pages? */
    ...
    NEMUSTRICTRC rcStrict = VINF_PGM_HANDLER_DO_DEFAULT;
        ...
        || PGM_PAGE_IS_MMIO_OR_SPECIAL_ALIAS(pPage))
    {
        ...
        AssertLogRelMsg(PGM_HANDLER_PHYS_IS_VALID_STATUS(rcStrict, false),
                        ("rcStrict=%Rrc GCPhys=%RGp\n", NEMUSTRICTRC_VAL(rcStrict), GCPhys));
...
#if !defined(IN_RING0) && defined(NEMU_WITH_RAW_MODE)
    /*
...
# endif
        ...
        NEMUSTRICTRC rcStrict2 = pVirtType->CTX_SUFF(pfnHandler)(pVM, pVCpu, GCPtr, (void *)pvSrc, pvBuf, cb,
        ...
        else if (rcStrict2 != VINF_PGM_HANDLER_DO_DEFAULT)
        {
            AssertLogRelMsg(PGM_HANDLER_VIRT_IS_VALID_STATUS(rcStrict2, false),
                            ("rcStrict2=%Rrc (rcStrict=%Rrc) GCPhys=%RGp pPage=%R[pgmpage] %s\n",
                             NEMUSTRICTRC_VAL(rcStrict2), NEMUSTRICTRC_VAL(rcStrict), GCPhys, pPage, pVirt->pszDesc));
...
#endif /* !IN_RING0 && NEMU_WITH_RAW_MODE */
 ...
 *
 * @returns Strict Nemu status code in raw-mode and ring-0, normal Nemu status
 ...
 * @param   enmOrigin       The origin of this call.
 */
VMMDECL(NEMUSTRICTRC) PGMPhysRead(PVM pVM, RTGCPHYS GCPhys, void *pvBuf, size_t cbRead, PGMACCESSORIGIN enmOrigin)
{
    ...
    for (;;)
    {
        ...
        if (pRam && GCPhys >= pRam->GCPhys)
        {
            ...
            while (off < pRam->cb)
            {
                ...
                {
                    NEMUSTRICTRC rcStrict2 = pgmPhysReadHandler(pVM, pPage, pRam->GCPhys + off, pvBuf, cb, enmOrigin);
                    if (PGM_PHYS_RW_IS_SUCCESS(rcStrict2))
                        PGM_PHYS_RW_DO_UPDATE_STRICT_RC(rcStrict, rcStrict2);
                    else
                    {
 ...
 * @param   enmOrigin   The origin of this call.
 */
static NEMUSTRICTRC pgmPhysWriteHandler(PVM pVM, PPGMPAGE pPage, RTGCPHYS GCPhys, void const *pvBuf, size_t cbWrite,
                                        PGMACCESSORIGIN enmOrigin)
{
    PGMPAGEMAPLOCK  PgMpLck;
    void           *pvDst = NULL;
                ...
                if (rcStrict == VINF_PGM_HANDLER_DO_DEFAULT)
                {
                    ...
                    AssertLogRelMsg(PGM_HANDLER_PHYS_IS_VALID_STATUS(rcStrict, true),
                                    ("rcStrict=%Rrc GCPhys=%RGp pPage=%R[pgmpage] %s\n",
                                     NEMUSTRICTRC_VAL(rcStrict), GCPhys, pPage, pCur ? R3STRING(pCur->pszDesc) : ""));
            }
            else
                AssertLogRelMsgFailedReturn(("pgmPhysGCPhys2CCPtrInternal failed on %RGp / %R[pgmpage] -> %Rrc\n",
                                             GCPhys, pPage, NEMUSTRICTRC_VAL(rcStrict)), rcStrict);
...
# endif
            ...
            if (RT_SUCCESS(rcStrict))
            {
                ...
                if (rcStrict == VINF_PGM_HANDLER_DO_DEFAULT)
                {
                    ...
                    AssertLogRelMsg(PGM_HANDLER_VIRT_IS_VALID_STATUS(rcStrict, true),
                                    ("rcStrict=%Rrc GCPhys=%RGp pPage=%R[pgmpage] %s\n",
                                     NEMUSTRICTRC_VAL(rcStrict), GCPhys, pPage, R3STRING(pVirt->pszDesc)));
            }
            else
                AssertLogRelMsgFailedReturn(("pgmPhysGCPhys2CCPtrInternal failed on %RGp / %R[pgmpage] -> %Rrc\n",
                                             GCPhys, pPage, NEMUSTRICTRC_VAL(rcStrict)), rcStrict);
    ...
    for (;;)
    {
         ...
         */
        NEMUSTRICTRC rcStrict2 = VINF_PGM_HANDLER_DO_DEFAULT;
        ...
        else if (!offPhys && offVirt)
        {
            ...
            AssertLogRelMsg(PGM_HANDLER_PHYS_IS_VALID_STATUS(rcStrict2, true),
                            ("rcStrict2=%Rrc (rcStrict=%Rrc) GCPhys=%RGp pPage=%R[pgmpage] %s\n", NEMUSTRICTRC_VAL(rcStrict2),
                             NEMUSTRICTRC_VAL(rcStrict), GCPhys, pPage,  pPhys ? R3STRING(pPhys->pszDesc) : ""));
        }
#if !defined(IN_RING0) && defined(NEMU_WITH_RAW_MODE)
        /*
         * Virtual handler.
...
# endif
            ...
            if (rcStrict2 == VINF_PGM_HANDLER_DO_DEFAULT || PGM_PHYS_RW_IS_SUCCESS(rcStrict2))
            {
                ...
                NEMUSTRICTRC rcStrict3 = pVirtType->CTX_SUFF(pfnHandler)(pVM, pVCpu, GCPtr, pvDst, (void *)pvBuf, cbRange,
                ...
                else if (rcStrict3 != VINF_PGM_HANDLER_DO_DEFAULT)
                {
                    AssertLogRelMsg(PGM_HANDLER_VIRT_IS_VALID_STATUS(rcStrict3, true),
                                    ("rcStrict3=%Rrc (rcStrict2=%Rrc) (rcStrict=%Rrc) GCPhys=%RGp pPage=%R[pgmpage] %s\n",
                                     NEMUSTRICTRC_VAL(rcStrict3), NEMUSTRICTRC_VAL(rcStrict2), NEMUSTRICTRC_VAL(rcStrict),
 ...
 */
VMMDECL(NEMUSTRICTRC) PGMPhysWrite(PVM pVM, RTGCPHYS GCPhys, const void *pvBuf, size_t cbWrite, PGMACCESSORIGIN enmOrigin)
{
    ...
    for (;;)
    {
        ...
        if (pRam && GCPhys >= pRam->GCPhys)
        {
            ...
            while (off < pRam->cb)
            {
                ...
                {
                    NEMUSTRICTRC rcStrict2 = pgmPhysWriteHandler(pVM, pPage, pRam->GCPhys + off, pvBuf, cb, enmOrigin);
                    if (PGM_PHYS_RW_IS_SUCCESS(rcStrict2))
                        PGM_PHYS_RW_DO_UPDATE_STRICT_RC(rcStrict, rcStrict2);
                    else
                    {
 ...
 *
 * @returns Strict Nemu status, see PGMPhysRead for details.
 ...
 * @thread  EMT(pVCpu)
 */
VMMDECL(NEMUSTRICTRC) PGMPhysReadGCPtr(PVMCPU pVCpu, void *pvDst, RTGCPTR GCPtrSrc, size_t cb, PGMACCESSORIGIN enmOrigin)
{
    RTGCPHYS    GCPhys;
    uint64_t    fFlags;
    int         rc;
    ...
    for (;;)
    {
        ...
        if (cbRead < cb)
        {
            NEMUSTRICTRC rcStrict = PGMPhysRead(pVM, GCPhys, pvDst, cbRead, enmOrigin);
 ...
 *
 * @returns Strict Nemu status, see PGMPhysWrite for details.
 * @retval  VERR_PAGE_TABLE_NOT_PRESENT if there is no page mapped at the
 *          specified virtual address.
 *
 * @param   pVCpu       The cross context virtual CPU structure of the calling EMT.
 ...
 * @param   enmOrigin       Who is calling.
 */
VMMDECL(NEMUSTRICTRC) PGMPhysWriteGCPtr(PVMCPU pVCpu, RTGCPTR GCPtrDst, const void *pvSrc, size_t cb, PGMACCESSORIGIN enmOrigin)
{
    RTGCPHYS    GCPhys;
    uint64_t    fFlags;
    int         rc;
    ...
    for (;;)
    {
        ...
        if (cbWrite < cb)
        {
            NEMUSTRICTRC rcStrict = PGMPhysWrite(pVM, GCPhys, pvSrc, cbWrite, enmOrigin);
            if (RT_LIKELY(rcStrict == VINF_SUCCESS))
            { /* likely */ }
            else
                return rcStrict;
 ...
 *
 *
 * @returns Nemu status code suitable to scheduling.
 ...
 * additional checks.
 *
 * @returns Nemu status code (no informational statuses).
 * @retval  VINF_SUCCESS on success.
