...
/** @file
 * Nemu Qt GUI - UISelectorWindow class implementation.
 */
...
void UISelectorWindow::sltHandleMediumEnumerationFinish()
{
    ...
    {
        if (nemuGlobal().medium(strMediumID).state() == KMediumState_Inaccessible)
        {
...
void UISelectorWindow::sltOpenUrls(QList<QUrl> list /* = QList<QUrl>() */)
{
    ...
    if (list.isEmpty())
    {
        list = nemuGlobal().argUrlList();
        nemuGlobal().argUrlList().clear();
    }
        ...
        if (!strFile.isEmpty() && QFile::exists(strFile))
        {
            if (NemuGlobal::hasAllowedExtension(strFile, NemuFileExts))
            {
                ...
                if (!machine.isNull())
                {
                    ...
                    CMachine machine = nemu.FindMachine(strFile);
                    if (!machine.isNull())
                        nemuGlobal().launchMachine(machine);
                }
                else
                    sltOpenAddMachineDialog(strFile);
            }
            else if (NemuGlobal::hasAllowedExtension(strFile, OVFFileExts))
            {
                /* OVF/OVA. Only one file at the time. */
                sltOpenImportApplianceWizard(strFile);
                break;
            }
            else if (NemuGlobal::hasAllowedExtension(strFile, NemuExtPackFileExts))
            {
...
void UISelectorWindow::sltOpenAddMachineDialog(const QString &strFileName /* = QString() */)
{
    ...
    {
        QString strBaseFolder = nemu.GetSystemProperties().GetDefaultMachineFolder();
        QString strTitle = tr("Select a virtual machine file");
        QStringList extensions;
        for (int i = 0; i < NemuFileExts.size(); ++i)
            extensions << QString("*.%1").arg(NemuFileExts[i]);
    ...
    CMachine newMachine = nemu.OpenMachine(strTmpFile);
    if (!nemu.isOk())
    {
        msgCenter().cannotOpenMachine(nemu, strTmpFile);
        return;
    }

    /* Make sure this machine was NOT registered already: */
    CMachine oldMachine = nemu.FindMachine(newMachine.GetId());
    ...
    nemu.RegisterMachine(newMachine);
}
                                                    ...
                                                    const QString &strID /* = QString() */)
{
    ...
    {
        nemuGlobal().openURL(strCategoryRef);
        return;
    }
...
void UISelectorWindow::sltPerformStartOrShowMachine()
{
    ...
    foreach (UIVMItem *pItem, items)
    {
        ...
        nemuGlobal().launchMachine(machine,
                                   UIVMItem::isItemRunningHeadless(pItem)         ? NemuGlobal::LaunchMode_Separate :
                                   qApp->keyboardModifiers() == Qt::ShiftModifier ? NemuGlobal::LaunchMode_Headless :
                                                                                    NemuGlobal::LaunchMode_Default);
    }
}
...
void UISelectorWindow::sltPerformStartMachineNormal()
{
    ...
    foreach (UIVMItem *pItem, items)
    {
        ...
        /* Launch/show current VM: */
        CMachine machine = pItem->machine();
        nemuGlobal().launchMachine(machine, NemuGlobal::LaunchMode_Default);
    }
}
...
void UISelectorWindow::sltPerformStartMachineHeadless()
{
    ...
    foreach (UIVMItem *pItem, items)
    {
        ...
        /* Launch/show current VM: */
        CMachine machine = pItem->machine();
        nemuGlobal().launchMachine(machine, NemuGlobal::LaunchMode_Headless);
    }
}
...
void UISelectorWindow::sltPerformStartMachineDetachable()
{
    ...
    foreach (UIVMItem *pItem, items)
    {
        ...
        /* Launch/show current VM: */
        CMachine machine = pItem->machine();
        nemuGlobal().launchMachine(machine, NemuGlobal::LaunchMode_Separate);
    }
}
...
void UISelectorWindow::sltPerformDiscardMachineState()
{
    ...
    foreach (UIVMItem *pItem, itemsToDiscard)
    {
        ...
        CSession session = nemuGlobal().openSession(pItem->id());
        if (session.isNull())
            return;
...
void UISelectorWindow::sltPerformPauseOrResumeMachine(bool fPause)
{
    ...
    foreach (UIVMItem *pItem, items)
    {
        ...
        CSession session = nemuGlobal().openExistingSession(pItem->id());
        if (session.isNull())
            return;
...
void UISelectorWindow::prepareConnections()
{
    ...
    connect(&nemuGlobal(), SIGNAL(sigMediumEnumerationFinished()), this, SLOT(sltHandleMediumEnumerationFinish()));
    ...
    connect(gNemuEvents, SIGNAL(sigMachineStateChange(QString, KMachineState)), this, SLOT(sltHandleStateChange(QString)));
    connect(gNemuEvents, SIGNAL(sigSessionStateChange(QString, KSessionState)), this, SLOT(sltHandleStateChange(QString)));
    connect(gNemuEvents, SIGNAL(sigSnapshotTake(QString, QString)), this, SLOT(sltHandleSnapshotChange(QString)));
    connect(gNemuEvents, SIGNAL(sigSnapshotDelete(QString, QString)), this, SLOT(sltHandleSnapshotChange(QString)));
    connect(gNemuEvents, SIGNAL(sigSnapshotChange(QString, QString)), this, SLOT(sltHandleSnapshotChange(QString)));
    connect(gNemuEvents, SIGNAL(sigSnapshotRestore(QString, QString)), this, SLOT(sltHandleSnapshotChange(QString)));
}
...
bool UISelectorWindow::isAtLeastOneItemSupportsShortcuts(const QList<UIVMItem*> &items)
{
    foreach (UIVMItem *pItem, items)
        if (pItem->accessible()
            ...
            && pItem->settingsFile().endsWith(".nemu", Qt::CaseInsensitive)
#endif /* Q_WS_MAC */
            )
            return true;
    return false;
