...
static int collectNetIfInfo(Bstr &strName, Guid &guid, PNETIFINFO pInfo, int iDefault)
{
    ...
    if (dwRc == NO_ERROR)
    {
        ...
        HRESULT hr = NemuNetCfgWinGetAdapterSettings((const GUID *)guid.raw(), &Settings);
        if (hr == S_OK)
        {
            if (Settings.ip)
            {
...
//            if (RT_FAILURE(vrc)) break;
            ...
            Bstr devId;
            hrc = NemuNetCfgWinCreateHostOnlyNetworkInterface(NULL, false, guid.asOutParam(), devId.asOutParam(),
                                                              bstrErr.asOutParam());
#else /* !NEMUNETCFG_DELAYEDRENAME */
            hrc = NemuNetCfgWinCreateHostOnlyNetworkInterface(NULL, false, guid.asOutParam(), name.asOutParam(),
                                                              bstrErr.asOutParam());
            ...
            {
                ULONG ip, mask;
                hrc = NemuNetCfgWinGenHostOnlyNetworkNetworkIp(&ip, &mask);
                if (hrc == S_OK)
                {
                    ...
                    ip = ip | (1 << 24);
                    hrc = NemuNetCfgWinEnableStaticIpConfig((const GUID*)guid.raw(), ip, mask);
                    if (hrc != S_OK)
                        LogRel(("NemuNetCfgWinEnableStaticIpConfig failed (0x%x)\n", hrc));
                }
                else
                    LogRel(("NemuNetCfgWinGenHostOnlyNetworkNetworkIp failed (0x%x)\n", hrc));
#ifdef NEMUNETCFG_DELAYEDRENAME
                hrc = NemuNetCfgWinRenameHostOnlyConnection((const GUID*)guid.raw(), devId.raw(), name.asOutParam());
                if (hrc != S_OK)
                    LogRel(("NemuNetCfgWinRenameHostOnlyConnection failed, error = 0x%x", hrc));
#endif /* NEMUNETCFG_DELAYEDRENAME */
        ...
        case SVCHlpMsg::RemoveHostOnlyNetworkInterface:
        {
            ...
            Utf8Str errMsg;
            hrc = NemuNetCfgWinRemoveHostOnlyNetworkInterface((const GUID*)guid.raw(), bstrErr.asOutParam());
        ...
        case SVCHlpMsg::EnableDynamicIpConfig:
        {
            ...
            Utf8Str errMsg;
            hrc = NemuNetCfgWinEnableDynamicIpConfig((const GUID *)guid.raw());
        ...
        case SVCHlpMsg::DhcpRediscover:
        {
            ...
            Utf8Str errMsg;
            hrc = NemuNetCfgWinDhcpRediscover((const GUID *)guid.raw());
                                  ...
                                  int iDefaultInterface)
{
    ...
    if (hr == S_OK)
    {
        ...
        if (hr == S_OK)
        {
            ...
            if (RT_FAILURE(rc))
            {
                LogRel(("nemuNetWinAddComponent: collectNetIfInfo() -> %Rrc\n", rc));
            }
            Log(("nemuNetWinAddComponent: adding %ls\n", lpszName));
            ...
            {
                LogRel(("nemuNetWinAddComponent: HostNetworkInterface::init() -> %Rrc\n", rc));
                Assert(0);
            }
        }
        else
            LogRel(("nemuNetWinAddComponent: failed to get device instance GUID (0x%x)\n", hr));
        CoTaskMemFree(lpszName);
    }
    else
        LogRel(("nemuNetWinAddComponent: failed to get device display name (0x%x)\n", hr));
...
static int netIfListHostAdapters(INetCfg *pNc, std::list<ComObjPtr<HostNetworkInterface> > &list)
{
    ...
    /* NemuNetAdp is available only when NEMU_WITH_NETFLT is enabled */
            ...
            if (hr == S_OK)
            {
                if (uComponentStatus == 0)
                {
                    ...
                    if (hr == S_OK)
                    {
                        ...
                        if (!_wcsnicmp(pId, L"sun_NemuNetAdp", sizeof(L"sun_NemuNetAdp")/2))
                        {
                            nemuNetWinAddComponent(&list, pMpNcc, HostNetworkInterfaceType_HostOnly, -1);
                        }
                        CoTaskMemFree(pId);
                    }
                    else
...
static int netIfGetUnboundHostOnlyAdapters(INetCfg *pNetCfg, std::list<BoundAdapter> &adapters)
{
    ...
    else
    {
        while ((hr = pEnumComponent->Next(1, &pMiniport, NULL)) == S_OK)
        {
            ...
            else if (_wcsnicmp(adapter.pHwId, L"sun_NemuNetAdp", sizeof(L"sun_NemuNetAdp")/2))
...
static HRESULT netIfGetBoundAdapters(std::list<BoundAdapter> &boundAdapters)
{
    ...
    hr = NemuNetCfgWinQueryINetCfg(&pNetCfg, FALSE,
                       NEMU_APP_NAME,
                       10000,
                       &lpszApp);
    Assert(hr == S_OK);
    ...
    if ((hr = pNetCfg->FindComponent(L"oracle_NemuNetLwf", &pFilter)) != S_OK
        /* fall back to NDIS5 miniport lookup */
        && (hr = pNetCfg->FindComponent(L"sun_NemuNetFlt", &pFilter)))
        LogRel(("netIfGetBoundAdapters: could not find either 'oracle_NemuNetLwf' or 'sun_NemuNetFlt' components (0x%x)\n", hr));
    else
    {
                                    ...
                                    else
                                    {
                                        ...
                                        if ((hr = pAdapter->GetId(&pwszHwId)) != S_OK)
                                            LogRel(("netIfGetBoundAdapters: %ls: failed to get hardware id (0x%x)\n",
                                                      pwszName, hr));
                                        else if (!_wcsnicmp(pwszHwId, L"sun_NemuNetAdp", sizeof(L"sun_NemuNetAdp")/2))
    ...
    NemuNetCfgWinReleaseINetCfg(pNetCfg, FALSE);
        ...
        else
        {
            ...
            for (it = boundAdapters.begin(); it != boundAdapters.end(); ++it)
            {
                ...
                HostNetworkInterfaceType enmType =
                    _wcsnicmp((*it).pHwId, L"sun_NemuNetAdp", sizeof(L"sun_NemuNetAdp")/2) ?
