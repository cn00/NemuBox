...
{
    /* Event classes */
    kEventClassNemu         = 'nemu',
    /* Event kinds */
    kEventNemuShowWindow    = 'swin',
    kEventNemuHideWindow    = 'hwin',
    kEventNemuMoveWindow    = 'mwin',
    kEventNemuResizeWindow  = 'rwin',
    kEventNemuDisposeWindow = 'dwin',
    kEventNemuUpdateDock    = 'udck',
    kEventNemuUpdateContext = 'uctx',
    kEventNemuBoundsChanged = 'bchg'
};
...
//    int id; /**< integer window ID */
    ...
    const NEMUVR_SCR_COMPOSITOR *pCompositor;
...
*/
#define WM_NEMU_RENDERSPU_CREATE_WINDOW (WM_APP+1)

typedef struct _NEMU_RENDERSPU_DESTROY_WINDOW {
    HWND hWnd; /* handle to window to destroy */
} NEMU_RENDERSPU_DESTROY_WINDOW;

/* Asks window thread to destroy previously created window.
   msg.lParam - holds pointer to RENDERSPU_NEMU_WINDOW_DESTROY structure
   msg.wParam - unused, should be NULL
*/
#define WM_NEMU_RENDERSPU_DESTROY_WINDOW (WM_APP+2)
...
extern void renderspuSetNemuConfiguration( RenderSPU *spu );
...
extern GLboolean renderspu_SystemNemuCreateWindow( VisualInfo *visual, GLboolean showIt, WindowInfo *window );
...
extern void renderspu_SystemNemuPresentComposition( WindowInfo *window, const struct NEMUVR_SCR_COMPOSITOR_ENTRY *pChangedEntry );
...
extern GLboolean renderspuNemuCompositorSet( WindowInfo *window, const struct NEMUVR_SCR_COMPOSITOR * pCompositor);
extern void renderspuNemuCompositorClearAll();
extern int renderspuNemuCompositorLock(WindowInfo *window, const struct NEMUVR_SCR_COMPOSITOR **ppCompositor);
extern int renderspuNemuCompositorUnlock(WindowInfo *window);
extern const struct NEMUVR_SCR_COMPOSITOR * renderspuNemuCompositorAcquire( WindowInfo *window);
extern int renderspuNemuCompositorTryAcquire(WindowInfo *window, const struct NEMUVR_SCR_COMPOSITOR **ppCompositor);
extern void renderspuNemuCompositorRelease( WindowInfo *window);
extern void renderspuNemuPresentCompositionGeneric( WindowInfo *window, const struct NEMUVR_SCR_COMPOSITOR * pCompositor,
        const struct NEMUVR_SCR_COMPOSITOR_ENTRY *pChangedEntry, int32_t i32MakeCurrentUserData,
        bool fRedraw);
extern PCR_BLITTER renderspuNemuPresentBlitterGet( WindowInfo *window );
void renderspuNemuPresentBlitterCleanup( WindowInfo *window );
extern int renderspuNemuPresentBlitterEnter( PCR_BLITTER pBlitter, int32_t i32MakeCurrentUserData );
extern PCR_BLITTER renderspuNemuPresentBlitterGetAndEnter( WindowInfo *window, int32_t i32MakeCurrentUserData, bool fRedraw );
extern PCR_BLITTER renderspuNemuPresentBlitterEnsureCreated( WindowInfo *window, int32_t i32MakeCurrentUserData );
...
extern void renderspuNemuCompositorBlit ( const struct NEMUVR_SCR_COMPOSITOR * pCompositor, PCR_BLITTER pBlitter);
extern void renderspuNemuCompositorBlitStretched ( const struct NEMUVR_SCR_COMPOSITOR * pCompositor, PCR_BLITTER pBlitter, GLfloat scaleX, GLfloat scaleY);
