...
const struct NEMUVR_SCR_COMPOSITOR* CrFbGetCompositor(CR_FRAMEBUFFER *pFb)
{
    return &pFb->Compositor;
}

DECLINLINE(CR_FRAMEBUFFER*) CrFbFromCompositor(const struct NEMUVR_SCR_COMPOSITOR* pCompositor)
{
...
static int8_t crFbImgFromDimOffVramBGRA(NEMUCMDVBVAOFFSET offVRAM, uint32_t width, uint32_t height, CR_BLITTER_IMG *pImg)
{
...
static int8_t crFbImgFromDescBGRA(const NEMUCMDVBVA_ALLOCDESC *pDesc, CR_BLITTER_IMG *pImg)
{
...
static int crFbBltGetContentsScaledDirect(HCR_FRAMEBUFFER hFb, const RTRECTSIZE *pSrcRectSize, const RTRECT *pDstRect, uint32_t cRects, const RTRECT *pRects, CR_BLITTER_IMG *pDst)
{
    ...
    NEMUVR_SCR_COMPOSITOR_CONST_ITERATOR Iter;
    ...
    NemuVrListInit(&List);
    int rc = NemuVrListRectsAdd(&List, cRects, pRects, NULL);
    if (!RT_SUCCESS(rc))
    {
    ...
    for(const NEMUVR_SCR_COMPOSITOR_ENTRY *pEntry = CrVrScrCompositorConstIterNext(&Iter);
            pEntry;
            pEntry = CrVrScrCompositorConstIterNext(&Iter))
    {
        ...
        if (pEnteredTex != pTex)
        {
            ...
            const NEMUVR_TEXTURE *pVrTex = CrTdTexGet(pTex);
        ...
        for (uint32_t j = 0; j < cRegions; ++j)
        {
            ...
            RTRECT ScaledReg;
            /* scale */
            NemuRectScaled(pReg, strX, strY, &ScaledReg);
            /* translate */
            NemuRectTranslate(&ScaledReg, pDstRect->xLeft, pDstRect->yTop);
             ...
             */
            rc = NemuVrListRectsSubst(&List, 1, &ScaledReg, NULL);
            if (!RT_SUCCESS(rc))
            {
            ...
            for (uint32_t i = 0; i < cRects; ++i)
            {
                ...
                RTRECT Intersection;
                NemuRectIntersected(pRect, &ScaledReg, &Intersection);
                if (NemuRectIsZero(&Intersection))
                    continue;

                NemuRectIntersect(&Intersection, &ScaledSrcRect);
                if (NemuRectIsZero(&Intersection))
                    continue;
    ...
    c2DRects = NemuVrListRectsCount(&List);
    if (c2DRects)
    {
        ...
        rc = NemuVrListRectsGet(&List, c2DRects, p2DRects);
        if (!RT_SUCCESS(rc))
        {
...
static int CrFbBltGetContents(HCR_FRAMEBUFFER hFb, const RTPOINT *pPos, uint32_t cRects, const RTRECT *pRects, CR_BLITTER_IMG *pDst)
{
            ...
            pEntry = CrVrScrCompositorConstIterNext(&Iter))
    {
        ...
        for (uint32_t j = 0; j < cRegions; ++j)
        {
            ...
            RTRECT SrcReg;
            /* translate */
            NemuRectTranslated(pReg, pPos->x, pPos->y, &SrcReg);

            /* Exclude the pEntry rectangle, because it will be updated now in pDst.
             * List uses dst coords and pRegions use hFb coords, therefore use
             * SrcReg which is already translated to dst.
             */
            rc = NemuVrListRectsSubst(&List, 1, &SrcReg, NULL);
            if (!RT_SUCCESS(rc))
            {
                WARN(("NemuVrListRectsSubst failed rc %d", rc));
                goto end;
            ...
            for (uint32_t i = 0; i < cRects; ++i)
            {
                ...
                RTRECT Intersection;
                NemuRectIntersected(pRect, &SrcReg, &Intersection);
                if (NemuRectIsZero(&Intersection))
                    continue;
...
static int crFbRegionsIsIntersectRects(HCR_FRAMEBUFFER hFb, uint32_t cRects, const RTRECT *pRects, bool *pfRegChanged)
{
    ...
    for (uint32_t i = 0; i < cCompRects; ++i)
    {
        ...
        for (uint32_t j = 0; j < cRects; ++j)
        {
            ...
            if (NemuRectIsIntersect(pCompRect, pRect))
            {
                *pfRegChanged = true;
                return VINF_SUCCESS;
            }
...
void CrFbTexDataInit(CR_TEXDATA* pFbTex, const NEMUVR_TEXTURE *pTex, PFNCRTEXDATA_RELEASED pfnTextureReleased)
{
    PCR_BLITTER pBlitter = crServerNemuBlitterGet();

    CrTdInit(pFbTex, pTex, pBlitter, pfnTextureReleased);
}

static CR_FBTEX* crFbTexCreate(const NEMUVR_TEXTURE *pTex)
{
...
CR_TEXDATA* CrFbTexDataCreate(const NEMUVR_TEXTURE *pTex)
{
...
static DECLCALLBACK(void) crFbEntryReleased(const struct NEMUVR_SCR_COMPOSITOR *pCompositor, struct NEMUVR_SCR_COMPOSITOR_ENTRY *pEntry, struct NEMUVR_SCR_COMPOSITOR_ENTRY *pReplacingEntry)
{
...
static int8_t crNemuServerCrCmdBltPrimaryVramGenericProcess(uint32_t u32PrimaryID, NEMUCMDVBVAOFFSET offVRAM, uint32_t width, uint32_t height, const RTPOINT *pPos, uint32_t cRects, const RTRECT *pRects, bool fToPrimary);
...
int CrFbRegionsClear(HCR_FRAMEBUFFER hFb)
{
    ...
    NEMUCMDVBVAOFFSET offVRAM = (NEMUCMDVBVAOFFSET)(((uintptr_t)CrFbGetVRAM(hFb)) - ((uintptr_t)g_pvVRamBase));
    RTPOINT Pos = {0,0};
    int8_t i8Result = crNemuServerCrCmdBltPrimaryVramGenericProcess(pScreen->u32ViewIndex, offVRAM, pScreen->u32Width, pScreen->u32Height, &Pos, cRegions, pRegions, true);
    if (i8Result)
    {
        WARN(("crNemuServerCrCmdBltPrimaryVramGenericProcess failed"));
...
int CrFbEntryRegionsAdd(CR_FRAMEBUFFER *pFb, HCR_FRAMEBUFFER_ENTRY hEntry, const RTPOINT *pPos, uint32_t cRegions, const RTRECT *paRegions, bool fPosRelated)
{
    ...
    NEMUVR_SCR_COMPOSITOR_ENTRY *pReplacedScrEntry = NULL;
    NEMUVR_SCR_COMPOSITOR_ENTRY *pNewEntry;
    ...
    if (RT_SUCCESS(rc))
    {
        if (fChangeFlags & NEMUVR_COMPOSITOR_CF_REGIONS_CHANGED)
        {
        ...
        else if (fChangeFlags & NEMUVR_COMPOSITOR_CF_ENTRY_REPLACED)
        {
            Assert(pReplacedScrEntry);
...
const struct NEMUVR_SCR_COMPOSITOR_ENTRY* CrFbEntryGetCompositorEntry(HCR_FRAMEBUFFER_ENTRY hEntry)
{
    return &hEntry->Entry;
}

HCR_FRAMEBUFFER_ENTRY CrFbEntryFromCompositorEntry(const struct NEMUVR_SCR_COMPOSITOR_ENTRY* pCEntry)
{
...
int CrPMgrInit()
{
    ...
    if (g_CrPresenter.pFbTexMap)
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (RT_SUCCESS(rc))
            {
                rc = RTMemCacheCreate(&g_CrPresenter.CEntryLookasideList, sizeof (NEMUVR_SCR_COMPOSITOR_ENTRY),
                                            0, /* size_t cbAlignment */
                                            UINT32_MAX, /* uint32_t cMaxObjects */
                                            NULL, /* PFNMEMCACHECTOR pfnCtor*/
                                            NULL, /* PFNMEMCACHEDTOR pfnDtor*/
...
HCR_FRAMEBUFFER CrPMgrFbGetEnabledByVramStart(NEMUCMDVBVAOFFSET offVRAM)
{
...
extern "C" DECLEXPORT(int) NemuOglSetScaleFactor(uint32_t idScreen, double dScaleFactorW, double dScaleFactorH)
{
...
int CrPMgrResize(const struct VBVAINFOSCREEN *pScreen, void *pvVRAM, const uint32_t *pTargetMap)
{
    ...
    NEMUCMDVBVA_SCREENMAP_DECL(uint32_t, aRemovedTargetMap);
    NEMUCMDVBVA_SCREENMAP_DECL(uint32_t, aAddedTargetMap);
    ...
    if (fFbInfoChanged)
    {
        ...
        crNemuServerMuralFbResizeBegin(hFb);
        ...
        crNemuServerMuralFbResizeEnd(hFb);
...
{
    const struct NEMUVR_SCR_COMPOSITOR_ENTRY *pEntry = CrFbEntryGetCompositorEntry(hEntry);
...
int CrFbSaveState(CR_FRAMEBUFFER *pFb, PSSMHANDLE pSSM)
{
    ...
    const NEMUVR_SCR_COMPOSITOR_ENTRY *pEntry;
    uint32_t u32 = 0;
...
int CrPMgrLoadState(PSSMHANDLE pSSM, uint32_t version)
{
    ...
    for (i = 0; i < cDisplays; ++i)
    {
        ...
        else
        {
            ...
            {
                NEMUCMDVBVA_SCREENMAP_DECL(uint32_t, aEmptyTargetMap);
...
crServerDispatchNemuTexPresent(GLuint texture, GLuint cfg, GLint xPos, GLint yPos, GLint cRects, const GLint *pRects)
{
...
DECLINLINE(void) crNemuPRectUnpack(const NEMUCMDVBVA_RECT *pVbvaRect, RTRECT *pRect)
{
...
DECLINLINE(void) crNemuPRectUnpacks(const NEMUCMDVBVA_RECT *paVbvaRects, RTRECT *paRects, uint32_t cRects)
{
    uint32_t i = 0;
    for (; i < cRects; ++i)
    {
        crNemuPRectUnpack(&paVbvaRects[i], &paRects[i]);
    }
}

static RTRECT * crNemuServerCrCmdBltRecsUnpack(const NEMUCMDVBVA_RECT *pPRects, uint32_t cRects)
{
    ...
    crNemuPRectUnpacks(pPRects, pRects, cRects);
...
static int8_t crNemuServerCrCmdBltPrimaryProcess(const NEMUCMDVBVA_BLT_PRIMARY *pCmd, uint32_t cbCmd)
{
    ...
    uint32_t cRects;
    const NEMUCMDVBVA_RECT *pPRects = pCmd->aRects;
    if ((cbCmd - RT_OFFSETOF(NEMUCMDVBVA_BLT_PRIMARY, aRects)) % sizeof (NEMUCMDVBVA_RECT))
    {
        WARN(("invalid argument size"));
        return -1;
    }

    cRects = (cbCmd - RT_OFFSETOF(NEMUCMDVBVA_BLT_PRIMARY, aRects)) / sizeof (NEMUCMDVBVA_RECT);

    RTRECT *pRects = crNemuServerCrCmdBltRecsUnpack(pPRects, cRects);
    if (!pRects)
    {
        WARN(("crNemuServerCrCmdBltRecsUnpack failed"));
        return -1;
    }

    uint8_t u8Flags = pCmd->Hdr.Hdr.u8Flags;

    if (u8Flags & NEMUCMDVBVA_OPF_OPERAND2_ISID)
    {
        ...
        if (u8Flags & NEMUCMDVBVA_OPF_BLT_DIR_IN_2)
        {
            WARN(("blit from primary to texture not implemented"));
            return -1;
        }

        crServerDispatchNemuTexPresent(texId, u32PrimaryID, pCmd->Hdr.Pos.x, pCmd->Hdr.Pos.y, cRects, (const GLint*)pRects);
    ...
    else
    {
        ...
        NEMUCMDVBVAOFFSET offVRAM = pCmd->alloc.u.offVRAM;

        bool fToPrymary = !(u8Flags & NEMUCMDVBVA_OPF_BLT_DIR_IN_2);
        RTPOINT Pos = {pCmd->Hdr.Pos.x, pCmd->Hdr.Pos.y};
        int8_t i8Result = crNemuServerCrCmdBltPrimaryVramGenericProcess(u32PrimaryID, offVRAM, width, height, &Pos, cRects, pRects, fToPrymary);
        if (i8Result < 0)
        {
            WARN(("crNemuServerCrCmdBltPrimaryVramGenericProcess failed"));
            return i8Result;
...
static int8_t crNemuServerCrCmdBltIdToVramMem(uint32_t hostId, NEMUCMDVBVAOFFSET offVRAM, uint32_t width, uint32_t height, const RTPOINT *pPos, uint32_t cRects, const RTRECT *pRects)
{
...
static int8_t crNemuServerCrCmdBltIdToVram(uint32_t hostId, NEMUCMDVBVAOFFSET offVRAM, uint32_t width, uint32_t height, const RTPOINT *pPos, uint32_t cRects, const RTRECT *pRects)
{
    ...
    if (hFb)
    {
        ...
        crServerDispatchNemuTexPresent(hostId, pScreen->u32ViewIndex, pPos->x, pPos->y, cRects, (const GLint*)pRects);
        return 0;
    }

    return crNemuServerCrCmdBltIdToVramMem(hostId, offVRAM, width, height, pPos, cRects, pRects);
}

static int8_t crNemuServerCrCmdBltVramToVramMem(NEMUCMDVBVAOFFSET offSrcVRAM, uint32_t srcWidth, uint32_t srcHeight, NEMUCMDVBVAOFFSET offDstVRAM, uint32_t dstWidth, uint32_t dstHeight, const RTPOINT *pPos, uint32_t cRects, const RTRECT *pRects)
{
...
static int8_t crNemuServerCrCmdBltVramToVram(NEMUCMDVBVAOFFSET offSrcVRAM, uint32_t srcWidth, uint32_t srcHeight,
        NEMUCMDVBVAOFFSET offDstVRAM, uint32_t dstWidth, uint32_t dstHeight,
        const RTPOINT *pPos, uint32_t cRects, const RTRECT *pRects)
{
    ...
    if (hDstFb)
    {
        ...
        else
        {
            ...
            rc = crNemuServerCrCmdBltVramToVramMem(offSrcVRAM, srcWidth, srcHeight, offDstVRAM, dstWidth, dstHeight, pPos, cRects, pRects);
            if (RT_FAILURE(rc))
            {
                WARN(("crNemuServerCrCmdBltVramToVramMem failed, %d", rc));
                return -1;
            }
        }
    ...
    return crNemuServerCrCmdBltVramToVramMem(offSrcVRAM, srcWidth, srcHeight, offDstVRAM, dstWidth, dstHeight, pPos, cRects, pRects);
}


static int8_t crNemuServerCrCmdBltOffIdProcess(const NEMUCMDVBVA_BLT_OFFPRIMSZFMT_OR_ID *pCmd, uint32_t cbCmd)
{
    uint32_t cRects;
    const NEMUCMDVBVA_RECT *pPRects = pCmd->aRects;
    if ((cbCmd - RT_OFFSETOF(NEMUCMDVBVA_BLT_OFFPRIMSZFMT_OR_ID, aRects)) % sizeof (NEMUCMDVBVA_RECT))
    {
        WARN(("invalid argument size"));
        return -1;
    }

    cRects = (cbCmd - RT_OFFSETOF(NEMUCMDVBVA_BLT_OFFPRIMSZFMT_OR_ID, aRects)) / sizeof (NEMUCMDVBVA_RECT);
    ...
    Assert(u8Flags & NEMUCMDVBVA_OPF_OPERAND2_ISID);
    ...
    if (u8Flags & NEMUCMDVBVA_OPF_OPERAND1_ISID)
    {
    ...
    RTPOINT Pos = {pCmd->Hdr.Pos.x, pCmd->Hdr.Pos.y};
    return crNemuServerCrCmdBltIdToVram(hostId, offVRAM, 0, 0, &Pos, cRects, pRects);
}

static int8_t crNemuServerCrCmdBltSameDimOrId(const NEMUCMDVBVA_BLT_SAMEDIM_A8R8G8B8 *pCmd, uint32_t cbCmd)
{
    uint32_t cRects;
    const NEMUCMDVBVA_RECT *pPRects = pCmd->aRects;
    if ((cbCmd - RT_OFFSETOF(NEMUCMDVBVA_BLT_SAMEDIM_A8R8G8B8, aRects)) % sizeof (NEMUCMDVBVA_RECT))
    {
        WARN(("invalid argument size"));
        return -1;
    }

    cRects = (cbCmd - RT_OFFSETOF(NEMUCMDVBVA_BLT_SAMEDIM_A8R8G8B8, aRects)) / sizeof (NEMUCMDVBVA_RECT);
    ...
    uint8_t u8Flags = pCmd->Hdr.Hdr.u8Flags;
    NEMUCMDVBVAOFFSET offVRAM = pCmd->alloc1.Info.u.offVRAM;
    ...
    if (u8Flags & NEMUCMDVBVA_OPF_OPERAND2_ISID)
    {
        ...
        return crNemuServerCrCmdBltIdToVram(hostId, offVRAM, width, height, &Pos, cRects, pRects);
    }
    ...
    if (u8Flags & NEMUCMDVBVA_OPF_OPERAND1_ISID)
    {
        ...
        return crNemuServerCrCmdBltIdToVram(pCmd->alloc1.Info.u.id, pCmd->info2.u.offVRAM, width, height, &Pos, cRects, pRects);
    }

    if (u8Flags & NEMUCMDVBVA_OPF_BLT_DIR_IN_2)
        crNemuServerCrCmdBltVramToVram(offVRAM, width, height, pCmd->info2.u.offVRAM, width, height, &Pos, cRects, pRects);
    else
        crNemuServerCrCmdBltVramToVram(pCmd->info2.u.offVRAM, width, height, offVRAM, width, height, &Pos, cRects, pRects);

    return 0;
}

static int8_t crNemuServerCrCmdBltGenericBGRAProcess(const NEMUCMDVBVA_BLT_GENERIC_A8R8G8B8 *pCmd, uint32_t cbCmd)
{
    uint32_t cRects;
    const NEMUCMDVBVA_RECT *pPRects = pCmd->aRects;
    if ((cbCmd - RT_OFFSETOF(NEMUCMDVBVA_BLT_GENERIC_A8R8G8B8, aRects)) % sizeof (NEMUCMDVBVA_RECT))
    {
        WARN(("invalid argument size"));
        return -1;
    }

    cRects = (cbCmd - RT_OFFSETOF(NEMUCMDVBVA_BLT_GENERIC_A8R8G8B8, aRects)) / sizeof (NEMUCMDVBVA_RECT);
    ...
    if (u8Flags & NEMUCMDVBVA_OPF_OPERAND2_ISID)
    {
        ...
        return crNemuServerCrCmdBltIdToVram(pCmd->alloc2.Info.u.id, pCmd->alloc1.Info.u.offVRAM, pCmd->alloc1.u16Width, pCmd->alloc1.u16Height, &Pos, cRects, pRects);
    }
    else
    {
        if (u8Flags & NEMUCMDVBVA_OPF_OPERAND1_ISID)
        {
            ...
            RTPOINT Pos = {pCmd->Hdr.Pos.x, pCmd->Hdr.Pos.y};
            return crNemuServerCrCmdBltIdToVram(pCmd->alloc1.Info.u.id, pCmd->alloc2.Info.u.offVRAM, pCmd->alloc2.u16Width, pCmd->alloc2.u16Height, &Pos, cRects, pRects);
        }

        if (u8Flags & NEMUCMDVBVA_OPF_BLT_DIR_IN_2)
            crNemuServerCrCmdBltVramToVram(pCmd->alloc1.Info.u.offVRAM, pCmd->alloc1.u16Width, pCmd->alloc1.u16Height, pCmd->alloc2.Info.u.offVRAM, pCmd->alloc2.u16Width, pCmd->alloc2.u16Height, &Pos, cRects, pRects);
        else
            crNemuServerCrCmdBltVramToVram(pCmd->alloc2.Info.u.offVRAM, pCmd->alloc2.u16Width, pCmd->alloc2.u16Height, pCmd->alloc1.Info.u.offVRAM, pCmd->alloc1.u16Width, pCmd->alloc1.u16Height, &Pos, cRects, pRects);

        return 0;
    }
}

static int8_t crNemuServerCrCmdClrFillPrimaryGenericProcess(uint32_t u32PrimaryID, const RTRECT *pRects, uint32_t cRects, uint32_t u32Color)
{
    HCR_FRAMEBUFFER hFb = CrPMgrFbGetEnabled(u32PrimaryID);
    if (!hFb)
    {
...
static int8_t crNemuServerCrCmdClrFillVramGenericProcess(NEMUCMDVBVAOFFSET offVRAM, uint32_t width, uint32_t height, const RTRECT *pRects, uint32_t cRects, uint32_t u32Color)
{
    CR_BLITTER_IMG Img;
    int8_t i8Result = crFbImgFromDimOffVramBGRA(offVRAM, width, height, &Img);
    if (i8Result)
...
static int8_t crNemuServerCrCmdClrFillGenericBGRAProcess(const NEMUCMDVBVA_CLRFILL_GENERIC_A8R8G8B8 *pCmd, uint32_t cbCmd)
{
    uint32_t cRects;
    const NEMUCMDVBVA_RECT *pPRects = pCmd->aRects;
    if ((cbCmd - RT_OFFSETOF(NEMUCMDVBVA_CLRFILL_GENERIC_A8R8G8B8, aRects)) % sizeof (NEMUCMDVBVA_RECT))
    {
        WARN(("invalid argument size"));
        return -1;
    }

    cRects = (cbCmd - RT_OFFSETOF(NEMUCMDVBVA_CLRFILL_GENERIC_A8R8G8B8, aRects)) / sizeof (NEMUCMDVBVA_RECT);
...
//    uint8_t u8Flags = pCmd->Hdr.Hdr.u8Flags;
    int8_t i8Result = crNemuServerCrCmdClrFillVramGenericProcess(pCmd->dst.Info.u.offVRAM, pCmd->dst.u16Width, pCmd->dst.u16Height, pRects, cRects, pCmd->Hdr.u32Color);
    if (i8Result < 0)
    {
        WARN(("crNemuServerCrCmdClrFillVramGenericProcess failed"));
        return i8Result;
    }
...
int8_t crNemuServerCrCmdClrFillProcess(const NEMUCMDVBVA_CLRFILL_HDR *pCmd, uint32_t cbCmd)
{
    uint8_t u8Flags = pCmd->Hdr.u8Flags;
    uint8_t u8Cmd = (NEMUCMDVBVA_OPF_CLRFILL_TYPE_MASK & u8Flags);

    switch (u8Cmd)
    {
        case NEMUCMDVBVA_OPF_CLRFILL_TYPE_GENERIC_A8R8G8B8:
        {
            if (cbCmd < sizeof (NEMUCMDVBVA_CLRFILL_GENERIC_A8R8G8B8))
            {
                WARN(("NEMUCMDVBVA_CLRFILL_GENERIC_A8R8G8B8: invalid command size"));
                return -1;
            }

            return crNemuServerCrCmdClrFillGenericBGRAProcess((const NEMUCMDVBVA_CLRFILL_GENERIC_A8R8G8B8*)pCmd, cbCmd);
        }
        default:
...
int8_t crNemuServerCrCmdBltProcess(const NEMUCMDVBVA_BLT_HDR *pCmd, uint32_t cbCmd)
{
    uint8_t u8Flags = pCmd->Hdr.u8Flags;
    uint8_t u8Cmd = (NEMUCMDVBVA_OPF_BLT_TYPE_MASK & u8Flags);

    switch (u8Cmd)
    {
        case NEMUCMDVBVA_OPF_BLT_TYPE_SAMEDIM_A8R8G8B8:
        {
            if (cbCmd < sizeof (NEMUCMDVBVA_BLT_SAMEDIM_A8R8G8B8))
            {
                WARN(("NEMUCMDVBVA_BLT_SAMEDIM_A8R8G8B8: invalid command size"));
                return -1;
            }

            return crNemuServerCrCmdBltSameDimOrId((const NEMUCMDVBVA_BLT_SAMEDIM_A8R8G8B8 *)pCmd, cbCmd);
        }
        case NEMUCMDVBVA_OPF_BLT_TYPE_OFFPRIMSZFMT_OR_ID:
        {
            if (cbCmd < sizeof (NEMUCMDVBVA_BLT_OFFPRIMSZFMT_OR_ID))
            {
                WARN(("NEMUCMDVBVA_OPF_BLT_TYPE_OFFPRIMSZFMT_OR_ID: invalid command size"));
                return -1;
            }

            return crNemuServerCrCmdBltOffIdProcess((const NEMUCMDVBVA_BLT_OFFPRIMSZFMT_OR_ID *)pCmd, cbCmd);
        }
        case NEMUCMDVBVA_OPF_BLT_TYPE_GENERIC_A8R8G8B8:
        {
            if (cbCmd < sizeof (NEMUCMDVBVA_BLT_GENERIC_A8R8G8B8))
            {
                WARN(("NEMUCMDVBVA_OPF_BLT_TYPE_GENERIC_A8R8G8B8: invalid command size"));
                return -1;
            }

            return crNemuServerCrCmdBltGenericBGRAProcess((const NEMUCMDVBVA_BLT_GENERIC_A8R8G8B8 *)pCmd, cbCmd);
        }
        default:
            WARN(("unsupported command"));
            return -1;
...
int8_t crNemuServerCrCmdFlipProcess(const NEMUCMDVBVA_FLIP *pFlip, uint32_t cbCmd)
{
    uint32_t hostId;
    const NEMUCMDVBVA_RECT *pPRects = pFlip->aRects;
    uint32_t cRects;

    if (pFlip->Hdr.u8Flags & NEMUCMDVBVA_OPF_OPERAND1_ISID)
    {
    ...
    {
        WARN(("NEMUCMDVBVA_OPF_ALLOC_SRCID not specified"));
        hostId = 0;
    }
    ...
    cRects = (cbCmd - NEMUCMDVBVA_SIZEOF_FLIPSTRUCT_MIN) / sizeof (NEMUCMDVBVA_RECT);
    if (cRects > 0)
    {
        RTRECT *pRects = crNemuServerCrCmdBltRecsUnpack(pPRects, cRects);
        if (pRects)
        {
            crServerDispatchNemuTexPresent(hostId, idFb, 0, 0, cRects, (const GLint*)pRects);
            return 0;
        }
    }
    else
    {
        ...
        crServerDispatchNemuTexPresent(hostId, idFb, 0, 0, 1, (const GLint*)pRect);
    }
...
{
    NEMUCRCMDCTL_CALLOUT_LISTENTRY Entry;
    PFNVCRSERVER_CLIENT_CALLOUT_CB pfnCb;
    void*pvCb;
} CRSERVER_CLIENT_CALLOUT;

static DECLCALLBACK(void) crServerClientCalloutCb(struct NEMUCRCMDCTL_CALLOUT_LISTENTRY *pEntry)
{
...
DECLEXPORT(void) crNemuServerCalloutEnable(NEMUCRCMDCTL *pCtl)
{
...
extern DECLEXPORT(void) crNemuServerCalloutDisable()
{
#if 1 //def CR_SERVER_WITH_CLIENT_CALLOUTS
    Assert(cr_server.pCurrentCalloutCtl);
