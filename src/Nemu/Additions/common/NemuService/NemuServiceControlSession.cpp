/* $Id: NemuServiceControlSession.cpp $ */
/** @file
 * NemuServiceControlSession - Guest session handling. Also handles the spawned session processes.
 */
...
enum
{
    NEMUSERVICESESSIONOPT_FIRST = 1000, /* For initialization. */
#ifdef DEBUG
    NEMUSERVICESESSIONOPT_DUMP_STDOUT,
    NEMUSERVICESESSIONOPT_DUMP_STDERR,
#endif
    NEMUSERVICESESSIONOPT_LOG_FILE,
    NEMUSERVICESESSIONOPT_USERNAME,
    NEMUSERVICESESSIONOPT_SESSION_ID,
    NEMUSERVICESESSIONOPT_SESSION_PROTO,
    NEMUSERVICESESSIONOPT_THREAD_ID
};



static int vgsvcGstCtrlSessionFileDestroy(PNEMUSERVICECTRLFILE pFile)
{
...
static PNEMUSERVICECTRLFILE vgsvcGstCtrlSessionFileGetLocked(const PNEMUSERVICECTRLSESSION pSession, uint32_t uHandle)
{
    AssertPtrReturn(pSession, NULL);

    /** @todo Use a map later! */
    PNEMUSERVICECTRLFILE pFileCur;
    RTListForEach(&pSession->lstFiles, pFileCur, NEMUSERVICECTRLFILE, Node)
    {
...
static int vgsvcGstCtrlSessionHandleDirRemove(PNEMUSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
...
static int vgsvcGstCtrlSessionHandleFileOpen(PNEMUSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
    ...
    if (RT_SUCCESS(rc))
    {
        PNEMUSERVICECTRLFILE pFile = (PNEMUSERVICECTRLFILE)RTMemAllocZ(sizeof(NEMUSERVICECTRLFILE));
        if (pFile)
        {
            if (!strlen(szFile))
            ...
            if (RT_SUCCESS(rc))
            {
                uHandle = NEMU_GUESTCTRL_CONTEXTID_GET_OBJECT(pHostCtx->uContextID);
...
static int vgsvcGstCtrlSessionHandleFileClose(const PNEMUSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);

    PNEMUSERVICECTRLFILE pFile = NULL;
...
static int vgsvcGstCtrlSessionHandleFileRead(const PNEMUSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx,
...
static int vgsvcGstCtrlSessionHandleFileReadAt(const PNEMUSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                               void *pvScratchBuf, size_t cbScratchBuf)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
...
static int vgsvcGstCtrlSessionHandleFileWrite(const PNEMUSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                              void *pvScratchBuf, size_t cbScratchBuf)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
...
static int vgsvcGstCtrlSessionHandleFileWriteAt(const PNEMUSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                                void *pvScratchBuf, size_t cbScratchBuf)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
...
static int vgsvcGstCtrlSessionHandleFileSeek(const PNEMUSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
...
static int vgsvcGstCtrlSessionHandleFileTell(const PNEMUSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
...
static int vgsvcGstCtrlSessionHandlePathRename(PNEMUSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
 ...
 */
static int vgsvcGstCtrlSessionHandleProcExec(PNEMUSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
    ...
    if (RT_SUCCESS(rc))
    {
        NEMUSERVICECTRLPROCSTARTUPINFO startupInfo;
        RT_ZERO(startupInfo);
        ...
        rc = VbglR3GuestCtrlProcGetStart(pHostCtx,
                                         ...
                                         /* Credentials; for hosts with Nemu < 4.3 (protocol version 1).
 ...
 *                              data.
 */
static int vgsvcGstCtrlSessionHandleProcInput(PNEMUSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                              void *pvScratchBuf, size_t cbScratchBuf)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
    ...
    else
    {
        ...
        PNEMUSERVICECTRLPROCESS pProcess = VGSvcGstCtrlSessionRetainProcess(pSession, uPID);
        if (pProcess)
        {
 ...
 * @param   pHostCtx            The host context to use.
 */
static int vgsvcGstCtrlSessionHandleProcOutput(PNEMUSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
 ...
 * @param   pHostCtx            The host context to use.
 */
static int vgsvcGstCtrlSessionHandleProcTerminate(const PNEMUSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
...
static int vgsvcGstCtrlSessionHandleProcWaitFor(const PNEMUSERVICECTRLSESSION pSession, PVBGLR3GUESTCTRLCMDCTX pHostCtx)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pHostCtx, VERR_INVALID_POINTER);
...
int VGSvcGstCtrlSessionHandler(PNEMUSERVICECTRLSESSION pSession, uint32_t uMsg, PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                               void *pvScratchBuf, size_t cbScratchBuf, volatile bool *pfShutdown)
{
     ...
     * operations.
     */
    bool const fImpersonated = RT_BOOL(pSession->fFlags & (  NEMUSERVICECTRLSESSION_FLAG_SPAWN
                                                           | NEMUSERVICECTRLSESSION_FLAG_ANONYMOUS));
    int rc;
    switch (uMsg)
    {
 ...
 * @param   pvUser          Pointer to a NEMUSERVICECTRLSESSIONTHREAD structure.
 *
 */
static DECLCALLBACK(int) vgsvcGstCtrlSessionThread(RTTHREAD hThreadSelf, void *pvUser)
{
    PNEMUSERVICECTRLSESSIONTHREAD pThread = (PNEMUSERVICECTRLSESSIONTHREAD)pvUser;
        ...
        for (;;)
        {
            ...
            if (ASMAtomicReadBool(&pThread->fShutdown))
            {
                if (!u64TimeoutStart)
                {
                    ...
                    {
                        /* .idClient  = */  uClientID,
                        /* .idContext = */  NEMU_GUESTCTRL_CONTEXTID_MAKE_SESSION(uSessionID),
    ...
    VBGLR3GUESTCTRLCMDCTX ctx = { uClientID, NEMU_GUESTCTRL_CONTEXTID_MAKE_SESSION(uSessionID) };
...
static RTEXITCODE vgsvcGstCtrlSessionSpawnWorker(PNEMUSERVICECTRLSESSION pSession)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        uint32_t uFilterAdd = NEMU_GUESTCTRL_FILTER_BY_SESSION(pSession->StartupInfo.uSessionID);
        rc = VbglR3GuestCtrlMsgFilterSet(uClientID,
                                         NEMU_GUESTCTRL_CONTEXTID_MAKE_SESSION(pSession->StartupInfo.uSessionID),
    ...
    VBGLR3GUESTCTRLCMDCTX ctx = { uClientID, NEMU_GUESTCTRL_CONTEXTID_MAKE_SESSION(pSession->StartupInfo.uSessionID) };
 ...
 */
PNEMUSERVICECTRLPROCESS VGSvcGstCtrlSessionRetainProcess(PNEMUSERVICECTRLSESSION pSession, uint32_t uPID)
{
    AssertPtrReturn(pSession, NULL);

    PNEMUSERVICECTRLPROCESS pProcess = NULL;
    int rc = RTCritSectEnter(&pSession->CritSect);
    if (RT_SUCCESS(rc))
    {
        PNEMUSERVICECTRLPROCESS pCurProcess;
        RTListForEach(&pSession->lstProcesses, pCurProcess, NEMUSERVICECTRLPROCESS, Node)
        {
...
int VGSvcGstCtrlSessionClose(PNEMUSERVICECTRLSESSION pSession)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        size_t cProcesses = 0;
        PNEMUSERVICECTRLPROCESS pProcess;
        RTListForEach(&pSession->lstProcesses, pProcess, NEMUSERVICECTRLPROCESS, Node)
        {
        ...
        pProcess = RTListGetFirst(&pSession->lstProcesses, NEMUSERVICECTRLPROCESS, Node);
        while (pProcess)
        {
            PNEMUSERVICECTRLPROCESS pNext = RTListNodeGetNext(&pProcess->Node, NEMUSERVICECTRLPROCESS, Node);
        ...
        PNEMUSERVICECTRLFILE pFile;
        pFile = RTListGetFirst(&pSession->lstFiles, NEMUSERVICECTRLFILE, Node);
        while (pFile)
        {
            PNEMUSERVICECTRLFILE pNext = RTListNodeGetNext(&pFile->Node, NEMUSERVICECTRLFILE, Node);
...
int VGSvcGstCtrlSessionDestroy(PNEMUSERVICECTRLSESSION pSession)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
...
int VGSvcGstCtrlSessionInit(PNEMUSERVICECTRLSESSION pSession, uint32_t fFlags)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
 ...
 */
int VGSvcGstCtrlSessionProcessAdd(PNEMUSERVICECTRLSESSION pSession, PNEMUSERVICECTRLPROCESS pProcess)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
 ...
 */
int VGSvcGstCtrlSessionProcessRemove(PNEMUSERVICECTRLSESSION pSession, PNEMUSERVICECTRLPROCESS pProcess)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
 ...
 */
int VGSvcGstCtrlSessionProcessStartAllowed(const PNEMUSERVICECTRLSESSION pSession, bool *pbAllowed)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
 ...
 */
static int vgsvcVGSvcGstCtrlSessionThreadCreateProcess(const PNEMUSERVICECTRLSESSIONSTARTUPINFO pSessionStartupInfo,
                                                       PNEMUSERVICECTRLSESSIONTHREAD pSessionThread, uint32_t uCtrlSessionThread)
{
    /*
     * Is this an anonymous session?  Anonymous sessions run with the same
     * privileges as the main NemuService executable.
     */
    ...
    if (pszExeName)
    {
        ...
        if (RT_SUCCESS(rc))
        {
            if (g_Session.fFlags & NEMUSERVICECTRLSESSION_FLAG_DUMPSTDOUT)
                apszArgs[idxArg++] = "--dump-stdout";
            if (g_Session.fFlags & NEMUSERVICECTRLSESSION_FLAG_DUMPSTDERR)
 ...
 *
 * This will spawn a new NemuService.exe instance under behalf of the given user
 ...
 *                                  Optional.
 */
int VGSvcGstCtrlSessionThreadCreate(PRTLISTANCHOR pList, const PNEMUSERVICECTRLSESSIONSTARTUPINFO pSessionStartupInfo,
                                    PNEMUSERVICECTRLSESSIONTHREAD *ppSessionThread)
{
     ...
     * Main consistency. */
    PNEMUSERVICECTRLSESSIONTHREAD pSessionCur;
    RTListForEach(pList, pSessionCur, NEMUSERVICECTRLSESSIONTHREAD, Node)
    {
     ...
     */
    PNEMUSERVICECTRLSESSIONTHREAD pSessionThread = (PNEMUSERVICECTRLSESSIONTHREAD)RTMemAllocZ(sizeof(*pSessionThread));
    if (pSessionThread)
    {
        /* Copy over session startup info. */
        memcpy(&pSessionThread->StartupInfo, pSessionStartupInfo, sizeof(NEMUSERVICECTRLSESSIONSTARTUPINFO));
 ...
 */
int VGSvcGstCtrlSessionThreadWait(PNEMUSERVICECTRLSESSIONTHREAD pThread, uint32_t uTimeoutMS, uint32_t fFlags)
{
    AssertPtrReturn(pThread, VERR_INVALID_POINTER);
 ...
 * @param   fFlags                  Closing flags.
 */
int VGSvcGstCtrlSessionThreadDestroy(PNEMUSERVICECTRLSESSIONTHREAD pThread, uint32_t fFlags)
{
    AssertPtrReturn(pThread, VERR_INVALID_POINTER);
...
int VGSvcGstCtrlSessionThreadDestroyAll(PRTLISTANCHOR pList, uint32_t fFlags)
{
    ...
    PNEMUSERVICECTRLSESSIONTHREAD pSessIt;
    PNEMUSERVICECTRLSESSIONTHREAD pSessItNext;
    RTListForEachSafe(pList, pSessIt, pSessItNext, NEMUSERVICECTRLSESSIONTHREAD, Node)
    {
...
RTEXITCODE VGSvcGstCtrlSessionSpawnInit(int argc, char **argv)
{
    ...
    {
#ifdef DEBUG
        { "--dump-stdout",     NEMUSERVICESESSIONOPT_DUMP_STDOUT,     RTGETOPT_REQ_NOTHING },
        { "--dump-stderr",     NEMUSERVICESESSIONOPT_DUMP_STDERR,     RTGETOPT_REQ_NOTHING },
#endif
        { "--logfile",         NEMUSERVICESESSIONOPT_LOG_FILE,        RTGETOPT_REQ_STRING },
        { "--user",            NEMUSERVICESESSIONOPT_USERNAME,        RTGETOPT_REQ_STRING },
        { "--session-id",      NEMUSERVICESESSIONOPT_SESSION_ID,      RTGETOPT_REQ_UINT32 },
        { "--session-proto",   NEMUSERVICESESSIONOPT_SESSION_PROTO,   RTGETOPT_REQ_UINT32 },
#ifdef DEBUG
        { "--thread-id",       NEMUSERVICESESSIONOPT_THREAD_ID,       RTGETOPT_REQ_UINT32 },
#endif /* DEBUG */
    ...
    uint32_t fSession = NEMUSERVICECTRLSESSION_FLAG_SPAWN;
    ...
    while ((ch = RTGetOpt(&GetState, &ValueUnion)) != 0)
    {
        ...
        switch (ch)
        {
            case NEMUSERVICESESSIONOPT_LOG_FILE:
            {
...
#ifdef DEBUG
            case NEMUSERVICESESSIONOPT_DUMP_STDOUT:
                fSession |= NEMUSERVICECTRLSESSION_FLAG_DUMPSTDOUT;
                break;

            case NEMUSERVICESESSIONOPT_DUMP_STDERR:
                fSession |= NEMUSERVICECTRLSESSION_FLAG_DUMPSTDERR;
                break;
#endif
            case NEMUSERVICESESSIONOPT_USERNAME:
                /* Information not needed right now, skip. */
                break;

            case NEMUSERVICESESSIONOPT_SESSION_ID:
                g_Session.StartupInfo.uSessionID = ValueUnion.u32;
                break;

            case NEMUSERVICESESSIONOPT_SESSION_PROTO:
                g_Session.StartupInfo.uProtocol = ValueUnion.u32;
                break;

#ifdef DEBUG
            case NEMUSERVICESESSIONOPT_THREAD_ID:
