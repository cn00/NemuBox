...
CHAR16 *NemuDebugDevicePath2Str(IN EFI_DEVICE_PATH_PROTOCOL *pDevicePath);
CHAR16 *NemuDebugHandleDevicePath2Str(IN EFI_HANDLE hHandle);
# endif
VOID EFIAPI NemuLogWorker(const char *pszFormat, ...);

/** See RT_XSTR */
# define NEMU_XSTR(str)                         NEMU_STR(str)
/** See RT_STR */
# define NEMU_STR(str)                          #str
# if defined(EFI_LOG_ENABLED)
#  define NemuLogFlowFuncEnter()                DEBUG((DEBUG_INFO, "%a:" NEMU_XSTR(__LINE__) ": ENTER\n", __FUNCTION__))
#  define NemuLogFlowFuncLeave()                DEBUG((DEBUG_INFO, "%a:" NEMU_XSTR(__LINE__) ": LEAVE\n", __FUNCTION__))
#  define NemuLogFlowFuncMark()                 DEBUG((DEBUG_INFO, "%a:" NEMU_XSTR(__LINE__) "\n", __FUNCTION__))
#  define NemuLogFlowFuncLeaveRC(rc) \
    do { \
        EFI_STATUS rcLog = (rc); \
        NemuLog(("%a:" NEMU_XSTR(__LINE__) ": LEAVE " #rc "=0x%x (%r)\n", __FUNCTION__, rcLog, rcLog)); \
    } while (0)
#  define NemuLogFlowFuncMarkVar(var, varfmt) \
    DEBUG((DEBUG_INFO, "%a:" NEMU_XSTR(__LINE__) ": " #var "=" varfmt "\n", __FUNCTION__, (var)))
#  define NemuLogFlowFuncMarkRC(rc) \
    do { \
        EFI_STATUS rcLog = (rc); \
        NemuLog(("%a:" NEMU_XSTR(__LINE__) ": " #rc "=0x%x (%r)\n", __FUNCTION__, rcLog, rcLog)); \
    } while (0)
#  define NemuLogFlowFuncMarkDP(dp) \
    NemuLog(("%a:" NEMU_XSTR(__LINE__) ": " #dp "=%s\n", __FUNCTION__, NemuDebugDevicePath2Str(dp)))
#  define NemuLogFlowFuncMarkHandleDP(dp)\
    NemuLog(("%a:" NEMU_XSTR(__LINE__) ": " #dp "=%s\n", __FUNCTION__, NemuDebugHandleDevicePath2Str(dp)))
#  define NemuLog(a)                            NemuLogWorker a
# else
#  define NemuLogFlowFuncEnter()                do {} while (0)
#  define NemuLogFlowFuncLeave()                do {} while (0)
#  define NemuLogFlowFuncLeaveRC(rc)            do {} while (0)
#  define NemuLogFlowFuncMark()                 do {} while (0)
#  define NemuLogFlowFuncMarkVar(var, varfmt)   do {} while (0)
#  define NemuLogFlowFuncMarkRC(rc)             do {} while (0)
#  define NemuLogFlowFuncMarkDP(dp)             do {} while (0)
#  define NemuLogLogFlowFuncMarkHandleDP(dp)    do {} while (0)
#  define NemuLog(a)                            do {} while (0)
# endif
#endif
#endif
