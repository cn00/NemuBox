...
/** @file
 * Nemu Console COM Class implementation - Guest drag and drop source.
 */
...
{
#if !defined(NEMU_WITH_DRAG_AND_DROP) || !defined(NEMU_WITH_DRAG_AND_DROP_GH)
    ReturnComNotImplemented();
#else /* NEMU_WITH_DRAG_AND_DROP */
...
{
#if !defined(NEMU_WITH_DRAG_AND_DROP)
    ReturnComNotImplemented();
#else /* NEMU_WITH_DRAG_AND_DROP */
...
HRESULT GuestDnDSource::drop(const com::Utf8Str &aFormat, DnDAction_T aAction, ComPtr<IProgress> &aProgress)
{
    ...
    try
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            hr = setError(NEMU_E_IPRT_ERROR, tr("Starting thread failed (%Rrc)"), rc);
    }
    catch(std::bad_alloc &)
    {
...
DECLCALLBACK(int) GuestDnDSource::i_receiveRawDataCallback(uint32_t uMsg, void *pvParms, size_t cbParms, void *pvUser)
{
    ...
    switch (uMsg)
    {
        ...
        {
            DragAndDropSvc::PNEMUDNDCBSNDDATADATA pCBData = reinterpret_cast<DragAndDropSvc::PNEMUDNDCBSNDDATADATA>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(DragAndDropSvc::NEMUDNDCBSNDDATADATA) == cbParms, VERR_INVALID_PARAMETER);
        ...
        {
            DragAndDropSvc::PNEMUDNDCBEVTERRORDATA pCBData = reinterpret_cast<DragAndDropSvc::PNEMUDNDCBEVTERRORDATA>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(DragAndDropSvc::NEMUDNDCBEVTERRORDATA) == cbParms, VERR_INVALID_PARAMETER);
...
DECLCALLBACK(int) GuestDnDSource::i_receiveURIDataCallback(uint32_t uMsg, void *pvParms, size_t cbParms, void *pvUser)
{
    ...
    switch (uMsg)
    {
        ...
        {
            DragAndDropSvc::PNEMUDNDCBSNDDIRDATA pCBData = reinterpret_cast<DragAndDropSvc::PNEMUDNDCBSNDDIRDATA>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(DragAndDropSvc::NEMUDNDCBSNDDIRDATA) == cbParms, VERR_INVALID_PARAMETER);
        ...
        {
            DragAndDropSvc::PNEMUDNDCBSNDFILEHDRDATA pCBData = reinterpret_cast<DragAndDropSvc::PNEMUDNDCBSNDFILEHDRDATA>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(DragAndDropSvc::NEMUDNDCBSNDFILEHDRDATA) == cbParms, VERR_INVALID_PARAMETER);
        ...
        {
            DragAndDropSvc::PNEMUDNDCBSNDFILEDATADATA pCBData = reinterpret_cast<DragAndDropSvc::PNEMUDNDCBSNDFILEDATADATA>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(DragAndDropSvc::NEMUDNDCBSNDFILEDATADATA) == cbParms, VERR_INVALID_PARAMETER);
            ...
            {
                /**
                 * Notes for protocol v1 (< Nemu 5.0):
