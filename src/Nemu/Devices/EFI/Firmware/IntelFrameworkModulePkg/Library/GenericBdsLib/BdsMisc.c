  ...
  )
{
  ...
  for (Index = 0; Index < TempOptionSize / sizeof (UINT16); Index++) {
    ...
    NemuLogFlowFuncMarkVar(OptionName, "%s");
    ...
    NemuLogFlowFuncMarkVar(OptionPtr, "%p");
    if (OptionPtr == NULL) {
      continue;
    }
    ...
    if (CompareMem (OptionDevicePath, DevicePath, GetDevicePathSize (OptionDevicePath)) == 0) {
      if (CompareMem (Description, String, StrSize (Description)) == 0) {
        ...
        NemuLogFlowFuncLeaveRC(EFI_SUCCESS);
        return EFI_SUCCESS;
      } else {
        //
  ...
  )
{
  ...
  NemuLogFlowFuncEnter();
  NemuLogFlowFuncMarkVar(Variable, "%s");
  NemuLogFlowFuncMarkVar(VariableSize, "%d");
  //
  ...
  )
{
  ...
  if (Variable == NULL) {
    NemuLogFlowFuncMarkVar(Variable, "%p");
    NemuLogFlowFuncLeave();
    return NULL;
  }
...
#ifdef NEMU  /* We need the OptionName later for Mac OS x related "Boot0080" hacks. */
  ...
  if ((Option->Attribute & LOAD_OPTION_ACTIVE) == LOAD_OPTION_ACTIVE) {
    ...
    FreePool (Variable);
    NemuLogFlowFuncMarkVar(Option, "%p");
    NemuLogFlowFuncLeave();
    return Option;
  }
  ...
  )
{
  ...
  ThisFile = NULL;
  NemuLogFlowFuncEnter();
  NemuLogFlowFuncMarkVar(FileName, "%s");
  //
