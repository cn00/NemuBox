...
static HRESULT WINAPI nemuD3D8GetSWInfoStub(void)
{
    return E_FAIL;
}
...
static IDirect3D8* WINAPI nemuDirect3DCreate8Stub(UINT SDKVersion)
{
    return NULL;
}

static HRESULT WINAPI nemuValidatePixelShaderStub(DWORD* pixelshader, DWORD* reserved1, BOOL bool, DWORD* toto)
{
    return E_FAIL;
}

static HRESULT WINAPI nemuValidateVertexShaderStub(DWORD* vertexshader, DWORD* reserved1, DWORD* reserved2, BOOL bool, DWORD* toto)
{
    return E_FAIL;
}
...
static D3D8Export g_swd3d8 = {0, "NemuD3D8.dll", "MSD3D8.dll",};
