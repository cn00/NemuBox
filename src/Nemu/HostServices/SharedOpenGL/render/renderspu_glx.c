        ...
        if (dpyName)
        {
            ...
            if (bRc)
            {
                ...
                if (bRc)
                {
                    ...
                    render_spu.WinCmdAtom = XInternAtom(render_spu.WinCmdVisual.dpy, "NemuWinCmd", False);
    	...
    	switch (event.type)
    	{
            ...
            case Expose:
            {
                ...
                {    
                    ...
                    if (pWindow)
                    {
                        const struct NEMUVR_SCR_COMPOSITOR * pCompositor;

                        pCompositor = renderspuNemuCompositorAcquire(pWindow);
                        if (pCompositor)
                        {
                            renderspuNemuPresentCompositionGeneric(pWindow, pCompositor, NULL, 0, false);
                            renderspuNemuCompositorRelease(pWindow);
                        }
                    }
                }
                break;
...
int renderspu_SystemInit()
{
    ...
    if (RT_SUCCESS(rc))
    {
        rc = RTThreadCreate(&render_spu.hWinCmdThread, renderspuWinCmdThreadProc, NULL, 0, RTTHREADTYPE_DEFAULT, RTTHREADFLAGS_WAITABLE, "NemuCrWinCmd");
	if (RT_SUCCESS(rc))
	{
...
createWindow( VisualInfo *visual, GLboolean showIt, WindowInfo *window )
{
    ...
    {
        crDebug("Render SPU: Nemu parent window_id is: %x", render_spu_parent_window_id);
    ...
    {
        /* This should happen only at the call from crNemuServerInit. At this point we don't
...
GLboolean
renderspu_SystemNemuCreateWindow( VisualInfo *visual, GLboolean showIt, WindowInfo *window )
{
...
void renderspu_SystemNemuPresentComposition( WindowInfo *window, const struct NEMUVR_SCR_COMPOSITOR_ENTRY *pChangedEntry )
{
    ...
    int rc = renderspuNemuCompositorTryAcquire(window, &pCompositor);
    if (RT_SUCCESS(rc))
    {
...
//        renderspuNemuPresentBlitterEnsureCreated(window, 0);
    ...
    else
    {
        ...
        WARN(("renderspuNemuCompositorTryAcquire failed rc %d", rc));
    }
#endif
}
