/* $Id: NemuNetLwfUninstall.cpp $ */
/** @file
 * NetLwfUninstall - NemuNetLwf uninstaller command line tool
 */
...
#define NEMU_NETCFG_APP_NAME L"NetLwfUninstall"
...
static int NemuNetLwfUninstall()
{
    INetCfg *pnc;
    ...
    if(hr == S_OK)
    {
        ...
        do
        {
            ...
            if(hr == S_OK)
            {
                hr = NemuNetCfgWinNetLwfUninstall(pnc);
                if(hr != S_OK)
                {
                    wprintf(L"error uninstalling NemuNetLwf (0x%x)\n", hr);
                    r = 1;
                }
                else
                {
