/* $Id: */
/** @file
 * NemuGuestR3Lib - Ring-3 Support Library for VirtualBox guest additions, host version check.
 */
...
VBGLR3DECL(int) VbglR3HostVersionCheckForUpdate(HGCMCLIENTID idClient, bool *pfUpdate, char **ppszHostVersion, char **ppszGuestVersion)
{
    ...
    *pfUpdate = true;
    ...
    if (RT_SUCCESS(rc) && *pfUpdate)
    {
        ...
        rc = VbglR3GuestPropReadValueAlloc(idClient, "/VirtualBox/HostInfo/NemuVer", ppszHostVersion);
        if (RT_FAILURE(rc))
        {
            LogFlow(("Could not read Nemu host version! rc = %Rrc\n", rc));
        }
        else
        {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (RT_FAILURE(rc))
                LogFlow(("Could not read Nemu guest version! rc = %Rrc\n", rc));
        }
    }
