...
typedef struct DMIOEMSTRINGS
{
    DMIHDR          header;
    uint8_t         u8Count;
    uint8_t         u8NemuVersion;
    uint8_t         u8NemuRevision;
} *PDMIOEMSTRINGS;
...
# endif
...
#ifdef NEMU_WITH_DMI_OEMSTRINGS
        ...
        char szTmp[64];
        RTStrPrintf(szTmp, sizeof(szTmp), "nemuVer_%u.%u.%u",
                    RTBldCfgVersionMajor(), RTBldCfgVersionMinor(), RTBldCfgVersionBuild());
        DMI_READ_CFG_STR_DEF(pOEMStrings->u8NemuVersion, "DmiOEMNemuVer", szTmp);
        RTStrPrintf(szTmp, sizeof(szTmp), "nemuRev_%u", RTBldCfgRevision());
        DMI_READ_CFG_STR_DEF(pOEMStrings->u8NemuRevision, "DmiOEMNemuRev", szTmp);
        DMI_TERM_STRUCT;
...
void FwCommonPlantSmbiosAndDmiHdrs(PPDMDEVINS pDevIns, uint16_t cbDmiTables, uint16_t cNumDmiTables)
{
    ...
    aBiosHeaders =
    {
        // The SMBIOS header
        {
            ...
            NEMU_SMBIOS_MAJOR_VER,             // SMBIOS major version
            NEMU_SMBIOS_MINOR_VER,             // SMBIOS minor version
            NEMU_SMBIOS_MAXSS,                 // Maximum structure size
        ...
        // The DMI header
        {
            ...
            0x00,                              // checksum
            0,                                 // DMI tables length
            NEMU_DMI_TABLE_BASE,               // DMI tables base
            0,                                 // DMI tables entries
            NEMU_DMI_TABLE_VER,                // DMI version
        }
    };
...
void FwCommonPlantMpsTable(PPDMDEVINS pDevIns, uint8_t *pTable, unsigned cbMax, uint16_t cCpus)
{
    ...
    memcpy(pCfgTab->au8OemId, "NEMUCPU ", 8);
    ...
    AssertMsg(pCfgTab->u16Length < cbMax,
              ("NEMU_MPS_TABLE_SIZE=%d, maximum allowed size is %d",
...
void FwCommonPlantMpsFloatPtr(PPDMDEVINS pDevIns)
{
    ...
    floatPtr.u32MPSAddr            = NEMU_MPS_TABLE_BASE;
