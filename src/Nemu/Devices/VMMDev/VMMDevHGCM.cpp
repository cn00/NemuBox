...
# include "dtrace/NemuDD.h"
#else
# define NEMUDD_HGCMCALL_ENTER(a,b,c,d)             do { } while (0)
# define NEMUDD_HGCMCALL_COMPLETED_REQ(a,b)         do { } while (0)
# define NEMUDD_HGCMCALL_COMPLETED_EMT(a,b)         do { } while (0)
# define NEMUDD_HGCMCALL_COMPLETED_DONE(a,b,c,d)    do { } while (0)
#endif
...
*********************************************************************************************************************************/
typedef enum NEMUHGCMCMDTYPE
{
    NEMUHGCMCMDTYPE_LOADSTATE = 0,
    NEMUHGCMCMDTYPE_CONNECT,
    NEMUHGCMCMDTYPE_DISCONNECT,
    NEMUHGCMCMDTYPE_CALL,
    NEMUHGCMCMDTYPE_SizeHack = 0x7fffffff
} NEMUHGCMCMDTYPE;

/**
 * Information about a linear ptr parameter.
 */
typedef struct NEMUHGCMLINPTR
{
...
struct NEMUHGCMCMD
{
    ...
    struct NEMUHGCMCMD *pNext;
    struct NEMUHGCMCMD *pPrev;

    /** The type of the command. */
    NEMUHGCMCMDTYPE enmCmdType;
...
static int vmmdevHGCMAddCommand (PVMMDEV pThis, PNEMUHGCMCMD pCmd, RTGCPHYS GCPhys, uint32_t cbSize, NEMUHGCMCMDTYPE enmCmdType)
{
    ...
    if (RT_SUCCESS (rc))
    {
        ...
        if (enmCmdType != NEMUHGCMCMDTYPE_LOADSTATE)
        {
        ...
        if (   enmCmdType == NEMUHGCMCMDTYPE_CONNECT
            || enmCmdType == NEMUHGCMCMDTYPE_DISCONNECT
            || enmCmdType == NEMUHGCMCMDTYPE_CALL)
        {
...
static int vmmdevHGCMRemoveCommand (PVMMDEV pThis, PNEMUHGCMCMD pCmd)
{
    /* PPDMDEVINS pDevIns = pThis->pDevIns; */
 ...
 *                          for.
 */
DECLINLINE(PNEMUHGCMCMD) vmmdevHGCMFindCommandLocked (PVMMDEV pThis, RTGCPHYS GCPhys)
{
    for (PNEMUHGCMCMD pCmd = pThis->pHGCMCmdList;
         pCmd;
         pCmd = pCmd->pNext)
    {
                                 ...
                                 RTGCPHYS **ppPages)
{
    ...
    NEMUHGCMLINPTR *pLinPtr = &paLinPtrs[iLinPtr];
...
static void vmmdevRestoreSavedCommand(NEMUHGCMCMD *pCmd, NEMUHGCMCMD *pSavedCmd)
{
...
int vmmdevHGCMConnect (PVMMDEV pThis, VMMDevHGCMConnect *pHGCMConnect, RTGCPHYS GCPhys)
{
    ...
    uint32_t cbCmdSize = sizeof (struct NEMUHGCMCMD) + pHGCMConnect->header.header.size;

    PNEMUHGCMCMD pCmd = (PNEMUHGCMCMD)RTMemAllocZ (cbCmdSize);

    if (pCmd)
    {
        VMMDevHGCMConnect *pHGCMConnectCopy = (VMMDevHGCMConnect *)(pCmd+1);

        vmmdevHGCMAddCommand (pThis, pCmd, GCPhys, pHGCMConnect->header.header.size, NEMUHGCMCMDTYPE_CONNECT);
...
static int vmmdevHGCMConnectSaved (PVMMDEV pThis, VMMDevHGCMConnect *pHGCMConnect, RTGCPHYS GCPhys, bool *pfHGCMCalled, NEMUHGCMCMD *pSavedCmd, NEMUHGCMCMD **ppCmd)
{
    int rc = VINF_SUCCESS;
...
int vmmdevHGCMDisconnect (PVMMDEV pThis, VMMDevHGCMDisconnect *pHGCMDisconnect, RTGCPHYS GCPhys)
{
    ...
    uint32_t cbCmdSize = sizeof (struct NEMUHGCMCMD);

    PNEMUHGCMCMD pCmd = (PNEMUHGCMCMD)RTMemAllocZ (cbCmdSize);

    if (pCmd)
    {
        vmmdevHGCMAddCommand (pThis, pCmd, GCPhys, pHGCMDisconnect->header.header.size, NEMUHGCMCMDTYPE_DISCONNECT);
...
static int vmmdevHGCMDisconnectSaved (PVMMDEV pThis, VMMDevHGCMDisconnect *pHGCMDisconnect, RTGCPHYS GCPhys, bool *pfHGCMCalled, NEMUHGCMCMD *pSavedCmd, NEMUHGCMCMD **ppCmd)
{
    int rc = VINF_SUCCESS;
...
int vmmdevHGCMCall (PVMMDEV pThis, VMMDevHGCMCall *pHGCMCall, uint32_t cbHGCMCall, RTGCPHYS GCPhys, bool f64Bits)
{
    ...
    uint32_t cbCmdSize = sizeof (struct NEMUHGCMCMD) + cParms * sizeof (NEMUHGCMSVCPARM);
    ...
    if (cLinPtrs > 0)
    {
        pCmd->paLinPtrs = (NEMUHGCMLINPTR *)RTMemAllocZ(  sizeof (NEMUHGCMLINPTR) * cLinPtrs
    ...
    NEMUDD_HGCMCALL_ENTER(pCmd, pHGCMCall->u32Function, pHGCMCall->u32ClientID, cbCmdSize);
    ...
    if (cParms != 0)
    {
        ...
        NEMUHGCMSVCPARM *pHostParm = (NEMUHGCMSVCPARM *)((char *)pCmd + sizeof (struct NEMUHGCMCMD));

        uint8_t *pcBuf = (uint8_t *)pHostParm + cParms * sizeof (NEMUHGCMSVCPARM);

        pCmd->paHostParms = pHostParm;
        pCmd->cHostParms  = cParms;

        uint32_t iLinPtr = 0;
        RTGCPHYS *pPages  = (RTGCPHYS *)((uint8_t *)pCmd->paLinPtrs + sizeof (NEMUHGCMLINPTR) *cLinPtrs);
            ...
            for (i = 0; i < cParms && RT_SUCCESS(rc); i++, pGuestParm++, pHostParm++)
            {
                switch (pGuestParm->type)
                {
                     case VMMDevHGCMParmType_32bit:
                     {
                         ...
                         pHostParm->type = NEMU_HGCM_SVC_PARM_32BIT;
                     ...
                     case VMMDevHGCMParmType_64bit:
                     {
                         ...
                         pHostParm->type = NEMU_HGCM_SVC_PARM_64BIT;
                     ...
                     case VMMDevHGCMParmType_LinAddr:     /* In & Out */
                     {
                         ...
                         pHostParm->type = NEMU_HGCM_SVC_PARM_PTR;
                     ...
                     case VMMDevHGCMParmType_PageList:
                     {
                         ...
                         {
                             if (pPageListInfo->flags & NEMU_HGCM_F_PARM_DIRECTION_TO_HOST)
                             {
    ...
    if (RT_SUCCESS (rc))
    {
        vmmdevHGCMAddCommand (pThis, pCmd, GCPhys, pHGCMCall->header.header.size, NEMUHGCMCMDTYPE_CALL);
...
static int vmmdevHGCMCallSaved (PVMMDEV pThis, VMMDevHGCMCall *pHGCMCall, RTGCPHYS GCPhys, uint32_t cbHGCMCall, bool f64Bits, bool *pfHGCMCalled, NEMUHGCMCMD *pSavedCmd, NEMUHGCMCMD **ppCmd)
{
    int rc = VINF_SUCCESS;
    ...
    *ppCmd = pCmd;
    ...
    {
        /* Compute addresses of host parms array and first memory buffer. */
        NEMUHGCMSVCPARM *pHostParm = (NEMUHGCMSVCPARM *)((uint8_t *)pCmd + sizeof (struct NEMUHGCMCMD));

        uint8_t *pu8Buf = (uint8_t *)pHostParm + cParms * sizeof (NEMUHGCMSVCPARM);
                     ...
                     case VMMDevHGCMParmType_LinAddr:     /* In & Out */
                     {
                         ...
                         else
                         {
                             ...
                             if (pGuestParm->type != VMMDevHGCMParmType_LinAddr_Out)
                             {
                                 ...
                                 {
                                     NEMUHGCMLINPTR *pLinPtr = &pCmd->paLinPtrs[iLinPtr];
...
int vmmdevHGCMCancel2 (PVMMDEV pThis, RTGCPHYS GCPhys)
{
    ...
    PNEMUHGCMCMD pCmd = vmmdevHGCMFindCommandLocked (pThis, GCPhys);
    if (pCmd)
    {
...
#ifdef NEMU_WITH_64_BITS_GUESTS
static int vmmdevHGCMParmVerify64(HGCMFunctionParameter64 *pGuestParm, NEMUHGCMSVCPARM *pHostParm)
{
    ...
    {
        case VMMDevHGCMParmType_32bit:
            if (pHostParm->type == NEMU_HGCM_SVC_PARM_32BIT)
                rc = VINF_SUCCESS;
            break;

        case VMMDevHGCMParmType_64bit:
            if (pHostParm->type == NEMU_HGCM_SVC_PARM_64BIT)
                rc = VINF_SUCCESS;
            break;
        ...
        case VMMDevHGCMParmType_LinAddr_Out: /* Out (write) */
        case VMMDevHGCMParmType_LinAddr:     /* In & Out */
            if (   pHostParm->type == NEMU_HGCM_SVC_PARM_PTR
...
#ifdef NEMU_WITH_64_BITS_GUESTS
static int vmmdevHGCMParmVerify32(HGCMFunctionParameter32 *pGuestParm, NEMUHGCMSVCPARM *pHostParm)
#else
static int vmmdevHGCMParmVerify32(HGCMFunctionParameter *pGuestParm, NEMUHGCMSVCPARM *pHostParm)
#endif
{
    int rc = VERR_INVALID_PARAMETER;
...
DECLCALLBACK(void) hgcmCompletedWorker (PPDMIHGCMPORT pInterface, int32_t result, PNEMUHGCMCMD pCmd)
{
    ...
    if (result == VINF_HGCM_SAVE_STATE)
    {
         ...
         * and, therefore, NEMUHGCMCMD structures are not removed by
     ...
     */
    NEMUDD_HGCMCALL_COMPLETED_EMT(pCmd, result);
    vmmdevHGCMRemoveCommand (pThis, pCmd);
    ...
    else
    {
        ...
        if (RT_SUCCESS (rc))
        {
            ...
            case VMMDevReq_HGCMCall64:
            {
                ...
                NEMUHGCMSVCPARM *pHostParm = pCmd->paHostParms;
                ...
                for (i = 0; i < cParms && RT_SUCCESS(rc); i++, pGuestParm++, pHostParm++)
                {
                    ...
                    switch (pGuestParm->type)
                    {
                        ...
                        case VMMDevHGCMParmType_PageList:
                        {
                            ...
                            if (size > 0)
                            {
                                if (pPageListInfo->flags & NEMU_HGCM_F_PARM_DIRECTION_FROM_HOST)
                                {
...
# endif
        ...
        pHeader->fu32Flags |= NEMU_HGCM_REQ_DONE;
    ...
    NEMUDD_HGCMCALL_COMPLETED_DONE(pCmd, idFunction, idClient, result);
    return;
}

DECLCALLBACK(void) hgcmCompleted (PPDMIHGCMPORT pInterface, int32_t result, PNEMUHGCMCMD pCmd)
{
    PVMMDEV pThis = RT_FROM_MEMBER(pInterface, VMMDevState, IHGCMPort);

    NEMUDD_HGCMCALL_COMPLETED_REQ(pCmd, result);
...
int vmmdevHGCMSaveState(PVMMDEV pThis, PSSMHANDLE pSSM)
{
    ...
    PNEMUHGCMCMD pIter = pThis->pHGCMCmdList;
    ...
    if (cCmds > 0)
    {
        ...
        while (pIter)
        {
            PNEMUHGCMCMD pNext = pIter->pNext;
            ...
            {
                /* Pointer to descriptions of linear pointers.  */
                NEMUHGCMLINPTR *pLinPtr = &pIter->paLinPtrs[i];
...
int vmmdevHGCMLoadState(PVMMDEV pThis, PSSMHANDLE pSSM, uint32_t uVersion)
{
    ...
    if (uVersion < 9)
    {
        ...
        while (cCmds--)
        {
            ...
            PNEMUHGCMCMD pCmd = (PNEMUHGCMCMD)RTMemAllocZ (sizeof (struct NEMUHGCMCMD));
            AssertReturn(pCmd, VERR_NO_MEMORY);

            pCmd->enmCmdType = NEMUHGCMCMDTYPE_LOADSTATE; /* This marks the "old" saved command. */

            vmmdevHGCMAddCommand (pThis, pCmd, GCPhys, cbSize, NEMUHGCMCMDTYPE_LOADSTATE);
        }
    }
    else
    {
        ...
        while (cCmds--)
        {
            ...
            /* Allocate only NEMUHGCMCMD structure. vmmdevHGCMLoadStateDone will reallocate the command
             * with additional space for parameters and for pointer/pagelists buffer.
             */
            PNEMUHGCMCMD pCmd = (PNEMUHGCMCMD)RTMemAllocZ (sizeof (NEMUHGCMCMD));
            AssertReturn(pCmd, VERR_NO_MEMORY);

            /* The type of the command. */
            rc = SSMR3GetU32(pSSM, &u32);
            AssertRCReturn(rc, rc);
            pCmd->enmCmdType = (NEMUHGCMCMDTYPE)u32;
            ...
            if (pCmd->cLinPtrs > 0)
            {
                ...
                pCmd->paLinPtrs = (NEMUHGCMLINPTR *)RTMemAllocZ (  sizeof (NEMUHGCMLINPTR) * pCmd->cLinPtrs
                                                                 + sizeof (RTGCPHYS) * pCmd->cLinPtrPages);
                AssertReturn(pCmd->paLinPtrs, VERR_NO_MEMORY);

                RTGCPHYS *pPages = (RTGCPHYS *)((uint8_t *)pCmd->paLinPtrs + sizeof (NEMUHGCMLINPTR) * pCmd->cLinPtrs);
                int cPages = 0;
                ...
                {
                    /* Pointer to descriptions of linear pointers. */
                    NEMUHGCMLINPTR *pLinPtr = &pCmd->paLinPtrs[i];
...
int vmmdevHGCMLoadStateDone(PVMMDEV pThis, PSSMHANDLE pSSM)
{
    ...
    if (RT_SUCCESS (rc))
    {
         ...
         * this loop.
         *
         * Note: The list contains only NEMUHGCMCMD structures, place for HGCM parameters
        ...
        while (pIter)
        {
            ...
            PNEMUHGCMCMD pCmd = NULL; /* Resubmitted command. */
            ...
            else
            {
                ...
                else
                {
                    ...
                    switch (pIter->enmCmdType)
                    {
                        ...
                        case NEMUHGCMCMDTYPE_LOADSTATE:
                        {
                            /* Old saved state. */
            ...
            if (pIter->enmCmdType == NEMUHGCMCMDTYPE_LOADSTATE)
            {
                /* Old saved state. */
            ...
            else
            {
                if (!fHGCMCalled)
                {
                   ...
                   requestHeader->fu32Flags |= NEMU_HGCM_REQ_DONE;
                }
