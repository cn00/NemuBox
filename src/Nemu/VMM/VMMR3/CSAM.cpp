...
static DECLCALLBACK(int) csamR3Load(PVM pVM, PSSMHANDLE pSSM, uint32_t uVersion, uint32_t uPass)
{
    ...
    else
    {
        ...
        CSAM csamInfo;
        RT_ZERO(csamInfo);
        if (   SSMR3HandleVersion(pSSM)  >= NEMU_FULL_VERSION_MAKE(4, 3, 51)
 ...
 *
 * @returns Nemu status code. (trap handled or not)
 * @param   pVM         The cross context VM structure.
...
#if 0 /* def NEMU_STRICT -> very annoying) */
    if (pPageRec->page.fMonitorActive)
    {
        uint64_t fPageShw;
        RTHCPHYS GCPhys;
...
VMMR3_INT_DECL(int) CSAMR3CheckGates(PVM pVM, uint32_t iGate, uint32_t cGates)
{
    ...
    maxGates   = (cbIDT+1) / sizeof(NEMUIDTE);
    ...
    GCPtrIDT   = GCPtrIDT + iGate * sizeof(NEMUIDTE);
     ...
     * Get IDT entries.
     */
    rc = PGMPhysSimpleReadGCPtr(pVCpu, aIDT, GCPtrIDT,  cGates*sizeof(NEMUIDTE));
    if (RT_FAILURE(rc))
    {
    ...
    for (/*iGate*/; iGate<iGateEnd; iGate++, pGuestIdte++)
    {
        ...
        if (    pGuestIdte->Gen.u1Present
            &&  (pGuestIdte->Gen.u5Type2 == NEMU_IDTE_TYPE2_TRAP_32 || pGuestIdte->Gen.u5Type2 == NEMU_IDTE_TYPE2_INT_32)
           ...
           )
        {
            ...
            pHandler = NEMUIDTE_OFFSET(*pGuestIdte);
            ...
            if (pGuestIdte->Gen.u5Type2 == NEMU_IDTE_TYPE2_TRAP_32)
            {
                Log(("CSAMCheckGates: check trap gate %d at %04X:%08X (flat %RRv)\n", iGate, pGuestIdte->Gen.u16SegSel, NEMUIDTE_OFFSET(*pGuestIdte), pHandler));
            }
            else
            {
            ...
            Log(("Installing %s gate handler for 0x%X at %RRv\n", (pGuestIdte->Gen.u5Type2 == NEMU_IDTE_TYPE2_TRAP_32) ? "trap" : "intr", iGate, pHandler));
