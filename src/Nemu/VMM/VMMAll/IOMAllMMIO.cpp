 ...
 * IEM deployment step. */
#if  ((defined(IN_RING3) || defined(IN_RING0)) && defined(NEMU_WITH_2ND_IEM_STEP)) \
  || defined(NEMU_WITH_3RD_IEM_STEP) || defined(DOXYGEN_RUNNING)
 ...
 *
 * @returns Strict Nemu status code. Any EM scheduling status code,
 ...
 */
static NEMUSTRICTRC iomMMIODoComplicatedWrite(PVM pVM, PIOMMMIORANGE pRange, RTGCPHYS GCPhys, void const *pvValue, unsigned cbValue)
{
 ...
 */
static NEMUSTRICTRC iomMMIODoWrite(PVM pVM, PVMCPU pVCpu, PIOMMMIORANGE pRange, RTGCPHYS GCPhysFault,
 ...
 */
static NEMUSTRICTRC iomMMIODoComplicatedRead(PVM pVM, PIOMMMIORANGE pRange, RTGCPHYS GCPhys, void *pvValue, unsigned cbValue)
{
 ...
 */
DECLINLINE(NEMUSTRICTRC) iomMMIODoRead(PVM pVM, PVMCPU pVCpu, PIOMMMIORANGE pRange, RTGCPHYS GCPhys,
...
# endif
    ...
    {
        switch (NEMUSTRICTRC_VAL(rcStrict))
        {
                                 ...
                                 PIOMMMIORANGE pRange, RTGCPHYS GCPhysFault)
{
    ...
    uint64_t u64Data = 0;
    int rc = NEMUSTRICTRC_TODO(iomMMIODoRead(pVM, pVCpu, pRange, GCPhysFault, &u64Data, cb));
    if (rc == VINF_SUCCESS)
    {
        /*
                                  ...
                                  PIOMMMIORANGE pRange, RTGCPHYS GCPhysFault)
{
    ...
    int rc = NEMUSTRICTRC_TODO(iomMMIODoWrite(pVM, pVCpu, pRange, GCPhysFault, &u64Data, cb));
    if (rc == VINF_SUCCESS)
        iomMMIOStatLength(pVM, cb);
    return rc;
}
...
DECLINLINE(int) iomRamRead(PVMCPU pVCpu, void *pDest, RTGCPTR GCSrc, uint32_t cb)
{
    ...
    return rc;
#else
    return NEMUSTRICTRC_VAL(PGMPhysReadGCPtr(pVCpu, pDest, GCSrc, cb, PGMACCESSORIGIN_IOM));
#endif
}
...
#else
    NOREF(pCtxCore);
    return NEMUSTRICTRC_VAL(PGMPhysWriteGCPtr(pVCpu, GCPtrDst, pvSrc, cb, PGMACCESSORIGIN_IOM));
#endif
}
...
/** @todo re-evaluate on page boundaries. */
    ...
    if (fWriteAccess)
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            while (cTransfers)
            {
                ...
                if (rc != VINF_SUCCESS)
                    break;
                rc = NEMUSTRICTRC_TODO(iomMMIODoWrite(pVM, pRange, Phys, &u32Data, cb));
                if (rc != VINF_SUCCESS)
                    break;
    ...
    else
    {
            ...
            &&  (pMMIODst = iomMmioGetRangeWithRef(pVM, PhysDst)))
        {
            ...
            {
                uint32_t u32Data;
                rc = NEMUSTRICTRC_TODO(iomMMIODoRead(pVM, pRange, Phys, &u32Data, cb));
                if (rc != VINF_SUCCESS)
                    break;
                rc = NEMUSTRICTRC_TODO(iomMMIODoWrite(pVM, pMMIODst, PhysDst, &u32Data, cb));
                if (rc != VINF_SUCCESS)
                    break;
...
/** @todo r=bird: bounds checks! */
    ...
    else
    {
        ...
        do
        {
            rc = NEMUSTRICTRC_TODO(iomMMIODoWrite(pVM, pVCpu, pRange, Phys, &u64Data, cb));
            if (rc != VINF_SUCCESS)
                break;
                            ...
                            PIOMMMIORANGE pRange)
{
     ...
     * Perform read.
     */
    int rc = NEMUSTRICTRC_TODO(iomMMIODoRead(pVM, pVCpu, pRange, GCPhysFault, &pRegFrame->rax, cb));
    if (rc == VINF_SUCCESS)
    {
        uint64_t const fAddrMask = iomDisModeToMask((DISCPUMODE)pCpu->uAddrMode);
                           ...
                           PIOMMMIORANGE pRange)
{
    ...
    if (iomGetRegImmData(pCpu, &pCpu->Param1, pRegFrame, &uData1, &cb))
        /* cmp reg, [MMIO]. */
        rc = NEMUSTRICTRC_TODO(iomMMIODoRead(pVM, pVCpu, pRange, GCPhysFault, &uData2, cb));
    else if (iomGetRegImmData(pCpu, &pCpu->Param2, pRegFrame, &uData2, &cb))
        /* cmp [MMIO], reg|imm. */
        rc = NEMUSTRICTRC_TODO(iomMMIODoRead(pVM, pVCpu, pRange, GCPhysFault, &uData1, cb));
    else
    {
                                ...
                                PIOMMMIORANGE pRange, PFNEMULATEPARAM3 pfnEmulate)
{
    ...
    if (rc == VINF_SUCCESS)
    {
        ...
        if (fAndWrite)
            ...
            rc = NEMUSTRICTRC_TODO(iomMMIODoWrite(pVM, pVCpu, pRange, GCPhysFault, &uData1, cb));
        else
        {
                          ...
                          PIOMMMIORANGE pRange)
{
    ...
    /* bt [MMIO], reg|imm. */
    int rc = NEMUSTRICTRC_TODO(iomMMIODoRead(pVM, pVCpu, pRange, GCPhysFault, &uData, cbData));
    if (rc == VINF_SUCCESS)
    {
                            ...
                            PIOMMMIORANGE pRange)
{
    ...
    else if (iomGetRegImmData(pCpu, &pCpu->Param2, pRegFrame, &uData2, &cb))
    {
        ...
        {
            /* Store result to MMIO. */
            rc = NEMUSTRICTRC_TODO(iomMMIODoWrite(pVM, pVCpu, pRange, GCPhysFault, &uData2, cb));
            if (rc == VINF_SUCCESS)
            {
                /* Store result to register. */
 ...
 *
 * @returns Nemu status code (appropriate for GC return).
 * @param   pVM         The cross context VM structure.
 * @param   pVCpu       The cross context virtual CPU structure of the calling EMT.
 ...
 */
static NEMUSTRICTRC iomMmioCommonPfHandler(PVM pVM, PVMCPU pVCpu, uint32_t uErrorCode, PCPUMCTXCORE pCtxCore,
...
# endif
     ...
     */
    NEMUSTRICTRC rcStrict = IEMExecOne(pVCpu);
...
# endif
    ...
    {
        STAM_COUNTER_INC(&pVM->iom.s.StatRZMMIOFailures);
# if defined(NEMU_WITH_STATISTICS) && !defined(IN_RING3)
        switch (rc)
        {
            case VINF_IOM_R3_MMIO_READ:
            case VINF_IOM_R3_MMIO_READ_WRITE:
 ...
 */
DECLEXPORT(NEMUSTRICTRC) iomMmioPfHandler(PVM pVM, PVMCPU pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pCtxCore, RTGCPTR pvFault,
                                          RTGCPHYS GCPhysFault, void *pvUser)
{
 ...
 */
VMMDECL(NEMUSTRICTRC) IOMMMIOPhysHandler(PVM pVM, PVMCPU pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pCtxCore, RTGCPHYS GCPhysFault)
{
    /*
    ...
    NEMUSTRICTRC rcStrict = iomMmioCommonPfHandler(pVM, pVCpu, (uint32_t)uErrorCode, pCtxCore, GCPhysFault, pRange);

    iomMmioReleaseRange(pVM, pRange);
    return NEMUSTRICTRC_VAL(rcStrict);
}
 ...
 */
PGM_ALL_CB2_DECL(NEMUSTRICTRC) iomMmioHandler(PVM pVM, PVMCPU pVCpu, RTGCPHYS GCPhysFault, void *pvPhys, void *pvBuf,
                                              size_t cbBuf, PGMACCESSTYPE enmAccessType, PGMACCESSORIGIN enmOrigin, void *pvUser)
{
    ...
    PPDMDEVINS pDevIns = pRange->CTX_SUFF(pDevIns);
    IOM_UNLOCK_SHARED(pVM);
    NEMUSTRICTRC rcStrict = PDMCritSectEnter(pDevIns->CTX_SUFF(pCritSectRo), VINF_IOM_R3_MMIO_READ_WRITE);
    if (rcStrict == VINF_SUCCESS)
    {
        /*
         * Perform the access.
...
#ifdef IN_RING3
        AssertMsg(rcStrict == VINF_SUCCESS, ("%Rrc - %RGp - %s\n", NEMUSTRICTRC_VAL(rcStrict), GCPhysFault, pRange->pszDesc));
#else
        AssertMsg(   rcStrict == VINF_SUCCESS
                  ...
                  , ("%Rrc - %RGp - %p\n", NEMUSTRICTRC_VAL(rcStrict), GCPhysFault, pDevIns));
#endif
 ...
 */
VMMDECL(NEMUSTRICTRC) IOMMMIORead(PVM pVM, PVMCPU pVCpu, RTGCPHYS GCPhys, uint32_t *pu32Value, size_t cbValue)
{
    /* Take the IOM lock before performing any MMIO. */
    NEMUSTRICTRC rc = IOM_LOCK_SHARED(pVM);
#ifndef IN_RING3
    if (rc == VERR_SEM_BUSY)
        return VINF_IOM_R3_MMIO_WRITE;
#endif
    AssertRC(NEMUSTRICTRC_VAL(rc));
 ...
 * @param   cbValue     The size of the register to read in bytes. 1, 2 or 4 bytes.
 */
VMMDECL(NEMUSTRICTRC) IOMMMIOWrite(PVM pVM, PVMCPU pVCpu, RTGCPHYS GCPhys, uint32_t u32Value, size_t cbValue)
{
    /* Take the IOM lock before performing any MMIO. */
    NEMUSTRICTRC rc = IOM_LOCK_SHARED(pVM);
#ifndef IN_RING3
 ...
 *
 * @returns Strict Nemu status code. Informational status codes other than the one documented
 ...
 */
VMMDECL(NEMUSTRICTRC) IOMInterpretINSEx(PVM pVM, PVMCPU pVCpu, PCPUMCTXCORE pRegFrame, uint32_t uPort, uint32_t uPrefix,
                                        DISCPUMODE enmAddrMode, uint32_t cbTransfer)
{
    ...
    NEMUSTRICTRC rcStrict = VINF_SUCCESS;
    if (cTransfers > 1)
    {
        /*
        ...
        for (;;)
        {
            ...
            if (RT_SUCCESS(rc2))
            {
                ...
                AssertRC(NEMUSTRICTRC_VAL(rcStrict));
 ...
 */
VMMDECL(NEMUSTRICTRC) IOMInterpretOUTSEx(PVM pVM, PVMCPU pVCpu, PCPUMCTXCORE pRegFrame, uint32_t uPort, uint32_t uPrefix,
                                         DISCPUMODE enmAddrMode, uint32_t cbTransfer)
{
 ...
 *
 * @returns Nemu status code.  This API may return VINF_SUCCESS even if no
 *          remapping is made,.
 *
 * @param   pVM             The cross context VM structure.
