...
void stubForcedFlush(GLint con)
{
    ...
    if (con)
    {
        stub.spu->dispatch_table.NemuConFlush(con);
    }
    else
    {
        stub.spu->dispatch_table.Flush();
...
{
//    if (con)
        stub.spu->dispatch_table.NemuConChromiumParameteriCR(con, param, value);
//    else
//        crError("NemuConChromiumParameteriCR called with null connection");
}

void stubConChromiumParametervCR(GLint con, GLenum target, GLenum type, GLsizei count, const GLvoid *values)
{
//    if (con)
        stub.spu->dispatch_table.NemuConChromiumParametervCR(con, target, type, count, values);
//    else
//        crError("NemuConChromiumParameteriCR called with null connection");
}
...
void stubDestroyWindow( GLint con, GLint window )
{
    ...
    if (winInfo && winInfo->type == CHROMIUM && stub.spu)
    {
        ...
        stub.spu->dispatch_table.NemuWindowDestroy(con, winInfo->spuWindow );
...
stubNewWindow( const char *dpyName, GLint visBits )
{
    ...
    {
        crError("Should not be here: WindowCreate/Destroy & NemuPackGetInjectID require connection id!");
        winInfo->mapped = 0;
    }
    else
#endif
...
#endif
#ifdef CR_NEWWINTRACK
    winInfo->u32ClientID = stub.spu->dispatch_table.NemuPackGetInjectID(0);
#endif
...
stubDestroyContextLocked( ContextInfo *context )
{
    ...
    if (context->type == NATIVE) {
        ...
        {
            stub.spu->dispatch_table.NemuConDestroy(context->spuConnection);
    ...
    )
{
    ...
    if (type == CHROMIUM) {
        ...
        if (pHgsmi)
        {
            spuConnection = stub.spu->dispatch_table.NemuConCreate(pHgsmi);
            if (!spuConnection)
            {
                crWarning("NemuConCreate failed");
                return NULL;
            }
        }
#endif
        spuContext
            = stub.spu->dispatch_table.NemuCreateContext(spuConnection, dpyName, visBits, spuShareCtx);
        if (spuContext < 0)
        {
            crWarning("NemuCreateContext failed");
#if defined(NEMU_WITH_CRHGSMI) && defined(IN_GUEST)
            if (spuConnection)
...
#ifdef CHROMIUM_THREADSAFE
    NemuTlsRefInit(context, stubContextDtor);
#endif
...
GLboolean stubCtxCreate(ContextInfo *context)
{
    ...
    if (context->pHgsmi)
    {
        spuConnection = stub.spu->dispatch_table.NemuConCreate(context->pHgsmi);
        if (!spuConnection)
        {
            crError("NemuConCreate failed");
            return GL_FALSE;
        }
        context->spuConnection = spuConnection;
    }
    ...
    context->spuContext
        = stub.spu->dispatch_table.NemuCreateContext(spuConnection, context->dpyName,
                                            context->visBits,
                                            spuShareCtx);
    ...
    if (context->type == UNDECIDED) {
        ...
        if (stubCheckUseChromium(window)) {
            ...
            if (window->spuWindow == -1)
            {
                ...
                window->spuWindow = stub.spu->dispatch_table.NemuWindowCreate(spuConnection, window->dpyName, context->visBits );
#ifdef CR_NEWWINTRACK
                window->u32ClientID = stub.spu->dispatch_table.NemuPackGetInjectID(spuConnection);
#endif
            }
        }
        else {
    ...
    if (context->type == NATIVE) {
        ...
        if (window->type == NATIVE) {
            ...
            if (window->spuWindow == -1)
            {
                ...
                window->spuWindow = stub.spu->dispatch_table.NemuWindowCreate(
#if defined(NEMU_WITH_CRHGSMI) && defined(IN_GUEST)
...
#ifdef CR_NEWWINTRACK
                window->u32ClientID = stub.spu->dispatch_table.NemuPackGetInjectID(
# if defined(NEMU_WITH_CRHGSMI) && defined(IN_GUEST)
                        context->spuConnection
# else
                        0
