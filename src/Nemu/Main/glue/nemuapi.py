...
#
import platform;
NemuBinDir = os.environ.get("NEMU_PROGRAM_PATH", None)
NemuSdkDir = os.environ.get("NEMU_SDK_PATH", None)

if NemuBinDir is None:
    if platform.system() == 'Darwin':
        NemuBinDir = '/Applications/VirtualBox.app/Contents/MacOS'
    else: # Will be set by the installer
        NemuBinDir = "%NEMU_INSTALL_PATH%"
else:
    NemuBinDir = os.path.abspath(NemuBinDir)

if NemuSdkDir is None:
    if platform.system() == 'Darwin':
        NemuSdkDir = '/Applications/VirtualBox.app/Contents/MacOS/sdk'
    else: # Will be set by the installer
        NemuSdkDir = "%NEMU_SDK_PATH%"
else:
    NemuSdkDir = os.path.abspath(NemuSdkDir)

os.environ["NEMU_PROGRAM_PATH"] = NemuBinDir
os.environ["NEMU_SDK_PATH"] = NemuSdkDir
sys.path.append(NemuBinDir)
...
class PerfCollector(object):
    ...
    def __init__(self, mgr, nemu):
        """ Initializes the instance.

        """
        self.mgr = mgr
        self.isMscom = (mgr.type == 'MSCOM')
        self.collector = nemu.performanceCollector
...
class PlatformBase(object):
    ...
    def getSessionObject(self, oINemu):
        """
        Get a session object that can be used for opening machine sessions.

        The oINemu parameter is an getVirtualBox() return value, i.e. an
    ...
    def xcptGetStatus(self, oXcpt):
        """
        Returns the COM status code from the Nemu API given exception.
        """
        return None
...
class PlatformMSCOM(PlatformBase):
    ...
    # @{
    NEMU_TLB_GUID = '{D7569351-1750-46F0-936E-BD127D5BC264}'
    NEMU_TLB_LCID = 0
    NEMU_TLB_MAJOR = 1
    NEMU_TLB_MINOR = 3
    ## @}
    ...
    def flushGenPyCache(self, oGenCache):
        """
        Flushes Nemu related files in the win32com gen_py cache.
        ...
        #
        sName = oGenCache.GetGeneratedFileName(self.NEMU_TLB_GUID, self.NEMU_TLB_LCID,
                                               self.NEMU_TLB_MAJOR, self.NEMU_TLB_MINOR)
        ...
        #
        return oGenCache.EnsureModule(self.NEMU_TLB_GUID, self.NEMU_TLB_LCID, self.NEMU_TLB_MAJOR, self.NEMU_TLB_MINOR)
    ...
    def createListener(self, oImplClass, dArgs):
        ...
        d['dArgs'] = dArgs
        d['tlb_guid'] = PlatformMSCOM.NEMU_TLB_GUID
        d['tlb_major'] = PlatformMSCOM.NEMU_TLB_MAJOR
        d['tlb_minor'] = PlatformMSCOM.NEMU_TLB_MINOR
        str_ = ""
...
class PlatformXPCOM(PlatformBase):
    ...
    def __init__(self, dParams):
        PlatformBase.__init__(self, dParams)
        sys.path.append(NemuSdkDir + '/bindings/xpcom/python/')
        import xpcom.nemuxpcom
        import xpcom
    ...
    def xcptSetupConstants(self, oDst):
        ...
        oDst.E_ACCESSDENIED = -2147024891  # see Nemu/com/defs.h
...
class PlatformWEBSERVICE(PlatformBase):
    ...
    def __init__(self, dParams):
        ...
        sWebServLib = os.path.join(NemuSdkDir, 'bindings', 'webservice', 'python', 'lib')
    ...
    def getSessionObject(self, oINemu):
        return self.wsmgr.getSessionObject(oINemu)
    ...
    def isRemote(self):
        """ Returns True if remote Nemu host, False if local. """
        return True
    ...
    def connect(self, url, user, passwd):
        ...
        self.nemu = self.wsmgr.logon(self.user, self.password)
        if not self.nemu.handle:
            raise Exception("cannot connect to '" + self.url + "' as '" + self.user + "'")
        return self.nemu

    def disconnect(self):
        if self.nemu is not None and self.wsmgr is not None:
            self.wsmgr.logoff(self.nemu)
            self.nemu = None
            self.wsmgr = None
...
# was used.  Most clients does talk to multiple Nemu instance on different
...
class VirtualBoxManager(object):
    ...
    def __init__(self, sStyle=None, dPlatformParams=None):
        ...
        ## @todo Add NEMU_E_XXX to statuses? They're already in constants...
        ...
        try:
            self.nemu = self.platform.getVirtualBox()
        except NameError:
    ...
    def getSessionObject(self, oINemu):
        ...
        return self.platform.getSessionObject(oINemu)
    ...
    def openMachineSession(self, oIMachine, fPermitSharing=True):
        ...
        """
        oSession = self.getSessionObject(self.nemu);
        if fPermitSharing:
    ...
    def xcptIsOurXcptKind(self, oXcpt=None):
        """
        Checks if the exception is one that could come from the Nemu API. If
