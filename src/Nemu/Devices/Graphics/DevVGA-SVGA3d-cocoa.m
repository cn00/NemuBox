...
- (void)nemuScheduleCtxUpdate;
- (void)nemuReshapePerform;
- (void)nemuReshape;
- (void)nemuBoundsDidChange:(NSNotification *)pNotification;
- (void)nemuFrameDidChange:(NSNotification *)pNotification;
- (void)nemuFrameDidChangeGlobal:(NSNotification *)pNotification;
- (BOOL)postsFrameChangedNotifications;
- (void)nemuRemoveFromSuperviewAndHide;
- (void)nemuUpdateCtxIfNecessary;
- (void)nemuClearBackBufferIfNecessary;
    ...
    if (self)
    {
        ...
        if (pParentView)
        {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(nemuBoundsDidChange:)
                                                         name:NSViewBoundsDidChangeNotification
                                                       object:self];
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(nemuFrameDidChange:)
            ...
            //                                           object:self];
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(nemuFrameDidChangeGlobal:)
...
- (void)nemuSetPos:(NSPoint)pos
{
    Log(("OvlView(%p) nemuSetPos: (%d,%d)\n", (void *)self, (int)pos.x, (int)pos.y));

    m_Pos = pos;
    [self nemuReshape];

    LogFlow(("OvlView(%p) nemuSetPos: returns\n", (void *)self));
}


- (void)nemuSetSize:(NSSize)size
{
    Log(("OvlView(%p) nemuSetSize: (%d,%d):\n", (void *)self, (int)size.width, (int)size.height));
    m_Size = size;
    [self nemuReshape];
    LogFlow(("OvlView(%p) nemuSetSize: returns\n", (void *)self));
}
...
- (void)nemuUpdateCtxIfNecessary
{
    if (m_fUpdateCtx)
    {
        Log(("OvlView(%p) nemuUpdateCtxIfNecessary: m_fUpdateCtx\n", (void *)self));
...
- (void)nemuClearBackBufferIfNecessary
{
    ...
    if (m_cClears > 0)
    {
        ...
        Log(("OvlView(%p) nemuClearBackBufferIfNecessary: m_cClears=%d\n", (void *)self, m_cClears));
        m_cClears--;
...
- (void)nemuReshapePerform
{
    ...
    {
        LogFlow(("OvlView(%p) nemuReshapePerform: moving (%d,%d) -> (%d,%d)\n",
    ...
    {
        LogFlow(("OvlView(%p) nemuReshapePerform: resizing (%d,%d) -> (%d,%d)\n",
    ...
    m_fUpdateCtx = true;
    LogFlow(("OvlView(%p) nemuReshapePerform: returns\n", self));
}


- (void)nemuReshape
{
    LogFlow(("OvlView(%p) nemuReshape:\n", (void *)self));
    ...
    LogFlow(("OvlView(%p) nemuReshape: returns\n", (void *)self));
}
...
- (void)nemuBoundsDidChange:(NSNotification *)pNotification
{
    LogFlow(("OvlView(%p) nemuBoundsDidChange:\n", (void *)self));
...
- (void)nemuFrameDidChange:(NSNotification *)pNotification
{
    LogFlow(("OvlView(%p) nemuFrameDidChange:\n", (void *)self));
    self->m_fUpdateCtx = true;
}
...
- (void)nemuFrameDidChangeGlobal:(NSNotification *)pNotification
{
    LogFlow(("OvlView(%p) nemuFrameDidChangeGlobal:\n", (void *)self));
    self->m_fUpdateCtx = true;
}

/** This enables the nemuFrameDidChange notification. */
- (BOOL)postsFrameChangedNotifications
{
...
- (void)nemuRemoveFromSuperviewAndHide
{
    LogFlow(("OvlView(%p) nemuRemoveFromSuperviewAndHide:\n", (void *)self));
    if (m_pParentView)
    {
        /*
         ...
         *   #4  0x000000010cffb2bb in -[VMSVGA3DOverlayView nemuRemoveFromSuperviewAndHide] (self=0x10a1da550, _cmd=0x10cffd734) at DevVGA-SVGA3d-cocoa.m:467
        ...
        if ([NSThread isMainThread])
        {
            LogFlow(("OvlView(%p) nemuRemoveFromSuperviewAndHide: calling removeFromSuperview\n", (void *)self));
            [self removeFromSuperview];
            LogFlow(("OvlView(%p) nemuRemoveFromSuperviewAndHide: calling setHidden\n", (void *)self));
            [self setHidden:YES];
            LogFlow(("OvlView(%p) nemuRemoveFromSuperviewAndHide: calling setHidden\n", (void *)self));
        ...
        {
            LogFlow(("OvlView(%p) nemuRemoveFromSuperviewAndHide: defering to main thread\n", (void *)self));
            vmsvga3dCocoaServiceRunLoop();
            [self performSelectorOnMainThread:@selector(nemuRemoveFromSuperviewAndHide) withObject:nil waitUntilDone:YES];
            vmsvga3dCocoaServiceRunLoop();
            LogFlow(("OvlView(%p) nemuRemoveFromSuperviewAndHide: main thread done\n", (void *)self));
        }
    }
}
...
VMSVGA3DCOCOA_DECL(void) vmsvga3dCocoaDestroyViewAndContext(NativeNSViewRef pView, NativeNSOpenGLContextRef pCtx)
{
    ...
    [pOvlView nemuRemoveFromSuperviewAndHide];
...
VMSVGA3DCOCOA_DECL(void) vmsvga3dCocoaViewSetPosition(NativeNSViewRef pView, NativeNSViewRef pParentView, int x, int y)
{
    ...
    [(VMSVGA3DOverlayView *)pView nemuSetPos:NSMakePoint(x, y)];
...
VMSVGA3DCOCOA_DECL(void) vmsvga3dCocoaViewUpdateViewport(NativeNSViewRef pView)
{
    ...
    [pOverlayView nemuScheduleCtxUpdate];
...
void vmsvga3dCocoaViewMakeCurrentContext(NativeNSViewRef pView, NativeNSOpenGLContextRef pCtx)
{
    ...
    if (pOverlayView)
    {
        ...
        [pOverlayView nemuUpdateCtxIfNecessary];
    }
    else
        [NSOpenGLContext clearCurrentContext];
...
void vmsvga3dCocoaSwapBuffers(NativeNSViewRef pView, NativeNSOpenGLContextRef pCtx)
{
    ...
    [pMyView nemuUpdateCtxIfNecessary];
    [pMyView nemuClearBackBufferIfNecessary];
