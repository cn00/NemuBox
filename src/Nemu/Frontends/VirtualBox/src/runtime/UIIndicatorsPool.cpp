...
/** @file
 * Nemu Qt GUI - UIIndicatorsPool class implementation.
 */
        ...
        foreach (const CStorageController &controller, machine.GetStorageControllers())
        {
            ...
            foreach (const CMediumAttachment &attachment, machine.GetMediumAttachmentsOfController(controller.GetName()))
            {
                ...
                if (attachment.GetType() != KDeviceType_DVD)
                    continue;
                /* Append attachment data: */
                UIMedium nemuMedium(attachment.GetMedium(), UIMediumType_DVD);
                strAttData += QString("<br>&nbsp;<nobr>%1:&nbsp;%2</nobr>")
                    .arg(gpConverter->toString(StorageSlot(controller.GetBus(), attachment.GetPort(), attachment.GetDevice())))
                    .arg(nemuMedium.isNull() || nemuMedium.isHostDrive() ? nemuMedium.name() : nemuMedium.location());
                fAttachmentsPresent = true;
                if (!nemuMedium.isNull())
        ...
        foreach (const CStorageController &controller, machine.GetStorageControllers())
        {
            ...
            foreach (const CMediumAttachment &attachment, machine.GetMediumAttachmentsOfController(controller.GetName()))
            {
                ...
                if (attachment.GetType() != KDeviceType_Floppy)
                    continue;
                /* Append attachment data: */
                UIMedium nemuMedium(attachment.GetMedium(), UIMediumType_Floppy);
                strAttData += QString("<br>&nbsp;<nobr>%1:&nbsp;%2</nobr>")
                    .arg(gpConverter->toString(StorageSlot(controller.GetBus(), attachment.GetPort(), attachment.GetDevice())))
                    .arg(nemuMedium.isNull() || nemuMedium.isHostDrive() ? nemuMedium.name() : nemuMedium.location());
                fAttachmentsPresent = true;
...
public:
        ...
        , m_cMaxNetworkAdapters(0)
    {
        ...
        const CMachine machine = m_pSession->machine();
        m_cMaxNetworkAdapters = nemu.GetSystemProperties().GetMaxNetworkAdapters(machine.GetChipsetType());
    ...
    void updateAppearance()
    {
        ...
        if (fUSBEnabled)
        {
            ...
            foreach (const CUSBDevice &usbDevice, console.GetUSBDevices())
                strFullData += QString("<br><b><nobr>%1</nobr></b>").arg(nemuGlobal().details(usbDevice));
...
private:
    ...
    void updateAppearance()
    {
        ...
        for (QMap<QString, QString>::const_iterator it = sfs.constBegin(); it != sfs.constEnd(); ++it)
        {
            ...
            if (NemuGlobal::isDOSType(guest.GetOSTypeId()))
                strFullData += QString("<br><nobr><b>\\\\nemusvr\\%1&nbsp;</b></nobr><nobr>%2</nobr>")
                                       .arg(it.key(), it.value());
            else
...
private:
    ...
    void updateAppearance()
    {
        ...
        const QString strVRAMSize = NemuGlobal::tr("<nobr>%1 MB</nobr>", "details report").arg(uVRAMSize);
        strFullData += QString("<br><nobr><b>%1:</b>&nbsp;%2</nobr>")
                               .arg(NemuGlobal::tr("Video Memory", "details report"), strVRAMSize);
        ...
        if (uMonitorCount > 1)
        {
            ...
            strFullData += QString("<br><nobr><b>%1:</b>&nbsp;%2</nobr>")
                                   .arg(NemuGlobal::tr("Screens", "details report"), strMonitorCount);
        }

        /* 3D acceleration: */
        const bool fAcceleration3D = machine.GetAccelerate3DEnabled() && nemuGlobal().is3DAvailable();
        if (fAcceleration3D)
        {
            const QString strAcceleration3D = fAcceleration3D
                ? NemuGlobal::tr("Enabled", "details report (3D Acceleration)")
                : NemuGlobal::tr("Disabled", "details report (3D Acceleration)");
            strFullData += QString("<br><nobr><b>%1:</b>&nbsp;%2</nobr>")
                                   .arg(NemuGlobal::tr("3D Acceleration", "details report"), strAcceleration3D);
        }
...
private:
    ...
    void updateAppearance()
    {
        ...
        QString virtualization = bVirtEnabled ?
            NemuGlobal::tr("Active", "details report (VT-x/AMD-V)") :
            NemuGlobal::tr("Inactive", "details report (VT-x/AMD-V)");

        /* Nested Paging feature: */
        bool bNestEnabled = debugger.GetHWVirtExNestedPagingEnabled();
        QString nestedPaging = bNestEnabled ?
            NemuGlobal::tr("Active", "details report (Nested Paging)") :
            NemuGlobal::tr("Inactive", "details report (Nested Paging)");

        /* Unrestricted Execution feature: */
        bool bUXEnabled = debugger.GetHWVirtExUXEnabled();
        QString unrestrictExec = bUXEnabled ?
            NemuGlobal::tr("Active", "details report (Unrestricted Execution)") :
            NemuGlobal::tr("Inactive", "details report (Unrestricted Execution)");
        ...
        QString tip(QApplication::translate("UIIndicatorsPool",
                    ...
                    .arg(NemuGlobal::tr("VT-x/AMD-V", "details report"), virtualization)
                    .arg(NemuGlobal::tr("Nested Paging"), nestedPaging)
                    .arg(NemuGlobal::tr("Unrestricted Execution"), unrestrictExec)
                    .arg(NemuGlobal::tr("Execution Cap", "details report"), strCPUExecCap));
        ...
        tip += QApplication::translate("UIIndicatorsPool", "<br><nobr><b>%1:</b>&nbsp;%2</nobr>", "Virtualization Stuff LED")
                                      .arg(NemuGlobal::tr("Paravirtualization Interface", "details report"), strParavirt);

        /* CPU count: */
        int cpuCount = machine.GetCPUCount();
        if (cpuCount > 1)
            tip += QApplication::translate("UIIndicatorsPool", "<br><nobr><b>%1:</b>&nbsp;%2</nobr>", "Virtualization Stuff LED")
                      .arg(NemuGlobal::tr("Processor(s)", "details report")).arg(cpuCount);
...
public:
    ...
    UIIndicatorKeyboardExtension()
    {
        ...
        connect(&nemuGlobal().settings(), SIGNAL(propertyChanged(const char *, const char *)),
...
public slots:
    ...
    {
        setText(UIHostCombo::toReadableString(nemuGlobal().settings().hostCombo()));
    }
...
void UIIndicatorsPool::updatePool()
{
    ...
    m_fEnabled = gEDataManager->statusBarEnabled(nemuGlobal().managedVMUuid());
    ...
    m_restrictions = gEDataManager->restrictedStatusBarIndicators(nemuGlobal().managedVMUuid());
    ...
    m_order = gEDataManager->statusBarIndicatorOrder(nemuGlobal().managedVMUuid());
