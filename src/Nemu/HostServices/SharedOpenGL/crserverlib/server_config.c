 ...
 * card drivers installed or drivers were not properly signed or Nemu
...
void crServerSetNemuConfiguration()
{
    ...
    if (env && env[0] != '\0')
    {
        ...
        cr_server.u32Caps &= CR_NEMU_CAPS_ALL;
    }
    else
    {
        cr_server.u32Caps = CR_NEMU_CAP_TEX_PRESENT
                | CR_NEMU_CAP_CMDVBVA
                | CR_NEMU_CAP_CMDBLOCKS
                | CR_NEMU_CAP_GETATTRIBSLOCATIONS
                | CR_NEMU_CAP_CMDBLOCKS_FLUSH
                ;
    }
    ...
    if (crServerHasInsufficientCaps())
    {
        ...
        cr_server.u32Caps |= CR_NEMU_CAP_HOST_CAPS_NOT_SUFFICIENT;
    }
...
void crServerSetNemuConfigurationHGCM()
{
    ...
    strcpy(cr_server.protocol, "nemuhgcm");
