...
static void shader_chache_put(PNEMUEXT_HASHCACHE pCache, struct wined3d_shader *shader);
#endif
...
#define NEMU_SHADER_FROM_CACHE_ENTRY(_pEntry) RT_FROM_MEMBER((_pEntry), struct wined3d_shader, CacheEntry)

static DECLCALLBACK(void) shader_cache_cleanup_entry(void *pvKey, struct NEMUEXT_HASHCACHE_ENTRY *pEntry)
{
    struct wined3d_shader *shader = NEMU_SHADER_FROM_CACHE_ENTRY(pEntry);
    shader_cleanup(shader);
    if (shader->parent)
        shader->parent_ops->wined3d_object_destroyed(shader->parent);
    HeapFree(GetProcessHeap(), 0, shader);
...
static void shader_chache_put(PNEMUEXT_HASHCACHE pCache, struct wined3d_shader *shader)
{
    ...
    NemuExtCachePut(pCache, shader, &shader->CacheEntry);
}

static struct wined3d_shader* shader_chache_get(PNEMUEXT_HASHCACHE pCache, struct wined3d_shader *shader)
{
    PNEMUEXT_HASHCACHE_ENTRY pEntry = NemuExtCacheGet(pCache, shader);
    struct wined3d_shader *cached_shader;

    if (!pEntry)
        return shader;

    cached_shader = NEMU_SHADER_FROM_CACHE_ENTRY(pEntry);
