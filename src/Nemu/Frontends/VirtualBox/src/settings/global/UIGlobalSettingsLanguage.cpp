...
/** @file
 * Nemu Qt GUI - UIGlobalSettingsLanguage class implementation.
 */
...
extern const char *gNemuLangSubDir;
extern const char *gNemuLangFileBase;
extern const char *gNemuLangFileExt;
extern const char *gNemuLangIDRegExp;
extern const char *gNemuBuiltInLangName;
...
public:
        ...
        : QTreeWidgetItem(pParent, UILanguageItemType), m_fBuiltIn(fBuiltIn)
    {
         ...
         * used in NemuGlobal::languageName() and friends (the latter are the
        ...
        if (text(1) == NemuGlobal::languageId())
        {
            QFont fnt = font(0);
            fnt.setBold(true);
            setFont(0, fnt);
...
void UIGlobalSettingsLanguage::reload(const QString &strLangId)
{
    ...
    AssertRC(rc);
    QString strNlsPath = QString(szNlsPath) + gNemuLangSubDir;
    QDir nlsDir(strNlsPath);
    QStringList files = nlsDir.entryList(QStringList(QString("%1*%2").arg(gNemuLangFileBase, gNemuLangFileExt)), QDir::Files);

    QTranslator translator;
    /* Add the default language: */
    new UILanguageItem(m_pLanguageTree);
    /* Add the built-in language: */
    new UILanguageItem(m_pLanguageTree, translator, gNemuBuiltInLangName, true /* built-in */);
    /* Add all existing languages */
    for (QStringList::Iterator it = files.begin(); it != files.end(); ++it)
    {
        QString strFileName = *it;
        QRegExp regExp(QString(gNemuLangFileBase) + gNemuLangIDRegExp);
