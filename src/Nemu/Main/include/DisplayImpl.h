...
typedef struct _DISPLAYFBINFO
{
    ...
    NEMUVIDEOINFOHOSTEVENTS *pHostEvents;
...
public:
    ...
    int  i_handleVHWACommandProcess(PNEMUVHWACMD pCommand);
#endif
#ifdef NEMU_WITH_CRHGSMI
    void i_handleCrHgsmiCommandCompletion(int32_t result, uint32_t u32Function, PNEMUHGCMSVCPARM pParam);
    void i_handleCrHgsmiControlCompletion(int32_t result, uint32_t u32Function, PNEMUHGCMSVCPARM pParam);
    void i_handleCrHgsmiCommandProcess(PNEMUVDMACMD_CHROMIUM_CMD pCmd, uint32_t cbCmd);
    void i_handleCrHgsmiControlProcess(PNEMUVDMACMD_CHROMIUM_CTL pCtl, uint32_t cbCtl);
#endif
#if defined(NEMU_WITH_HGCM) && defined(NEMU_WITH_CROGL)
    int  i_handleCrHgcmCtlSubmit(struct NEMUCRCMDCTL* pCmd, uint32_t cbCmd,
...
private:
...
#ifdef NEMU_WITH_VIDEOHWACCEL
    static DECLCALLBACK(int)  i_displayVHWACommandProcess(PPDMIDISPLAYCONNECTOR pInterface, PNEMUVHWACMD pCommand);
#endif

#ifdef NEMU_WITH_CRHGSMI
    static DECLCALLBACK(void)  i_displayCrHgsmiCommandProcess(PPDMIDISPLAYCONNECTOR pInterface,
                                                              PNEMUVDMACMD_CHROMIUM_CMD pCmd, uint32_t cbCmd);
    static DECLCALLBACK(void)  i_displayCrHgsmiControlProcess(PPDMIDISPLAYCONNECTOR pInterface, PNEMUVDMACMD_CHROMIUM_CTL pCtl,
                                                              uint32_t cbCtl);

    static DECLCALLBACK(void)  i_displayCrHgsmiCommandCompletion(int32_t result, uint32_t u32Function, PNEMUHGCMSVCPARM pParam,
                                                                 void *pvContext);
    static DECLCALLBACK(void)  i_displayCrHgsmiControlCompletion(int32_t result, uint32_t u32Function, PNEMUHGCMSVCPARM pParam,
                                                                 void *pvContext);
#endif
#if defined(NEMU_WITH_HGCM) && defined(NEMU_WITH_CROGL)
    static DECLCALLBACK(int)  i_displayCrHgcmCtlSubmit(PPDMIDISPLAYCONNECTOR pInterface,
                                                       ...
                                                       PFNCRCTLCOMPLETION pfnCompletion,
                                                       void *pvCompletion);
    static DECLCALLBACK(void) i_displayCrHgcmCtlSubmitCompletion(int32_t result, uint32_t u32Function, PNEMUHGCMSVCPARM pParam,
                                                                 void *pvContext);
#endif
#ifdef NEMU_WITH_HGSMI
    ...
    static DECLCALLBACK(void) i_displayVRecCompletion(struct NEMUCRCMDCTL* pCmd, uint32_t cbCmd, int rc, void *pvCompletion);
#endif
    static DECLCALLBACK(void) i_displayCrCmdFree(struct NEMUCRCMDCTL* pCmd, uint32_t cbCmd, int rc, void *pvCompletion);
    ...
    /** Accelerate3DEnabled = true && GraphicsControllerType == NemuVGA. */
    ...
    CRNEMUHGCMTAKESCREENSHOT mCrOglScreenshotData;
    NEMUCRCMDCTL_HGCM mCrOglScreenshotCtl;
#endif
...
public:
    ...
    int i_crCtlSubmit(struct NEMUCRCMDCTL* pCmd, uint32_t cbCmd, PFNCRCTLCOMPLETION pfnCompletion, void *pvCompletion);
    int i_crCtlSubmitSync(struct NEMUCRCMDCTL* pCmd, uint32_t cbCmd);
    /* copies the given command and submits it asynchronously,
     * i.e. the pCmd data may be discarded right after the call returns */
    int i_crCtlSubmitAsyncCmdCopy(struct NEMUCRCMDCTL* pCmd, uint32_t cbCmd);
    ...
    int i_crCtlSubmitSyncIfHasDataForScreen(uint32_t u32ScreenID, struct NEMUCRCMDCTL* pCmd, uint32_t cbCmd);
#endif
