/* $Id: NemuGuestR0LibSharedFolders.c $ */
/** @file
 * NemuGuestR0LibSharedFolders - Ring 0 Shared Folders calls.
 */
...
/* Entire file is ifdef'ed with !VBGL_NEMUGUEST */
#ifndef VBGL_NEMUGUEST
...
DECLVBGL(int) VbglR0SfConnect(PVBGLSFCLIENT pClient)
{
    ...
    strcpy (data.Loc.u.host.achName, "NemuSharedFolders");

    rc = VbglHGCMConnect(&pClient->handle, &data);
/*    Log(("NEMUSF: VbglR0SfConnect: VbglHGCMConnect rc = %#x, result = %#x\n", rc, data.result)); */
    if (RT_SUCCESS(rc))
        rc = data.result;
    if (RT_SUCCESS(rc))
    {
...
{
    int rc;
    NemuGuestHGCMDisconnectInfo data;
    ...
    NOREF(rc);
/*    Log(("NEMUSF: VbglR0SfDisconnect: VbglHGCMDisconnect rc = %#x, result = %#x\n", rc, data.result)); */
    return;
}
...
{
    int rc;
    NemuSFQueryMappings data;

    NEMU_INIT_CALL(&data.callInfo, QUERY_MAPPINGS, pClient);
...
{
    int rc;
    NemuSFQueryMapName data;

    NEMU_INIT_CALL(&data.callInfo, QUERY_MAP_NAME, pClient);
    ...
    rc = VbglHGCMCall(pClient->handle, &data.callInfo, sizeof(data));
/*    Log(("NEMUSF: VbglR0SfQueryMapName: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS(rc))
        rc = data.callInfo.result;
...
{
    int rc;
    NemuSFMapFolder data;

    NEMU_INIT_CALL(&data.callInfo, MAP_FOLDER, pClient);
    ...
    rc = VbglHGCMCall(pClient->handle, &data.callInfo, sizeof(data));
/*    Log(("NEMUSF: VbglR0SfMapFolder: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS(rc))
    {
    ...
    else if (rc == VERR_NOT_IMPLEMENTED)
    {
        ...
        NemuSFMapFolder_Old OldData;

        NEMU_INIT_CALL(&OldData.callInfo, MAP_FOLDER_OLD, pClient);
...
{
    int rc;
    NemuSFUnmapFolder data;

    NEMU_INIT_CALL(&data.callInfo, UNMAP_FOLDER, pClient);

    data.root.type                      = VMMDevHGCMParmType_32bit;
    data.root.u.value32                 = pMap->root;

    rc = VbglHGCMCall(pClient->handle, &data.callInfo, sizeof(data));
/*    Log(("NEMUSF: VbglR0SfUnmapFolder: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS(rc))
        rc = data.callInfo.result;
    return rc;
}
...
DECLVBGL(int) VbglR0SfCreate(PVBGLSFCLIENT pClient, PVBGLSFMAP pMap, PSHFLSTRING pParsedPath, PSHFLCREATEPARMS pCreateParms)
{
    ...
    NEMU_INIT_CALL(&data.callInfo, CREATE, pClient);
    ...
    rc = VbglHGCMCall(pClient->handle, &data.callInfo, sizeof(data));
/*    Log(("NEMUSF: VbglR0SfCreate: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS(rc))
        rc = data.callInfo.result;
    return rc;
}
...
DECLVBGL(int) VbglR0SfClose(PVBGLSFCLIENT pClient, PVBGLSFMAP pMap, SHFLHANDLE Handle)
{
    ...
    NEMU_INIT_CALL(&data.callInfo, CLOSE, pClient);
    ...
    rc = VbglHGCMCall(pClient->handle, &data.callInfo, sizeof(data));
/*    Log(("NEMUSF: VbglR0SfClose: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS(rc))
        rc = data.callInfo.result;
    return rc;
}
...
DECLVBGL(int) VbglR0SfRemove(PVBGLSFCLIENT pClient, PVBGLSFMAP pMap, PSHFLSTRING pParsedPath, uint32_t flags)
{
    ...
    NEMU_INIT_CALL(&data.callInfo, REMOVE, pClient);
    ...
    rc = VbglHGCMCall(pClient->handle, &data.callInfo, sizeof(data));
/*    Log(("NEMUSF: VbglR0SfRemove: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS (rc))
        rc = data.callInfo.result;
    return rc;
}
...
DECLVBGL(int) VbglR0SfRename(PVBGLSFCLIENT pClient, PVBGLSFMAP pMap, PSHFLSTRING pSrcPath, PSHFLSTRING pDestPath, uint32_t flags)
{
    ...
    NEMU_INIT_CALL(&data.callInfo, RENAME, pClient);
    ...
    rc = VbglHGCMCall(pClient->handle, &data.callInfo, sizeof(data));
/*    Log(("NEMUSF: VbglR0SfRename: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS (rc))
        rc = data.callInfo.result;
    return rc;
}
                           ...
                           uint64_t offset, uint32_t *pcbBuffer, uint8_t *pBuffer, bool fLocked)
{
    ...
    NEMU_INIT_CALL(&data.callInfo, READ, pClient);
    ...
    rc = VbglHGCMCall(pClient->handle, &data.callInfo, sizeof(data));
/*    Log(("NEMUSF: VbglR0SfRead: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS(rc))
    {
        rc = data.callInfo.result;
                                   ...
                                   uint16_t offFirstPage, uint16_t cPages, RTGCPHYS64 *paPages)
{
    ...
    uint32_t            cbData    = (uint32_t)(sizeof(NemuSFRead) + RT_UOFFSETOF(HGCMPageListInfo, aPages[cPages]));
    NemuSFRead         *pData     = (NemuSFRead *)RTMemTmpAlloc(cbData);
    ...
    NEMU_INIT_CALL(&pData->callInfo, READ, pClient);
    ...
    pData->buffer.u.PageList.offset       = sizeof(NemuSFRead);

    pPgLst->flags = NEMU_HGCM_F_PARM_DIRECTION_FROM_HOST;
...
/*    Log(("NEMUSF: VbglR0SfReadPageList: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS(rc))
    {
                            ...
                            uint64_t offset, uint32_t *pcbBuffer, uint8_t *pBuffer, bool fLocked)
{
    ...
    NEMU_INIT_CALL(&data.callInfo, WRITE, pClient);
    ...
    rc = VbglHGCMCall(pClient->handle, &data.callInfo, sizeof(data));
/*    Log(("NEMUSF: VbglR0SfWrite: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS(rc))
    {
        rc = data.callInfo.result;
        *pcbBuffer = data.cb.u.value32;
                                    ...
                                    uint32_t *pcbBuffer, RTCCPHYS PhysBuffer)
{
    ...
    uint32_t            cbData    = (uint32_t)(sizeof(NemuSFWrite) + RT_UOFFSETOF(HGCMPageListInfo, aPages[cPages]));
    NemuSFWrite        *pData     = (NemuSFWrite *)RTMemTmpAlloc(cbData);
    HGCMPageListInfo   *pPgLst    = (HGCMPageListInfo *)(pData + 1);
    uint32_t            iPage;
    int                 rc;
    ...
    NEMU_INIT_CALL(&pData->callInfo, WRITE, pClient);
    ...
    pData->buffer.u.PageList.offset       = sizeof(NemuSFWrite);

    pPgLst->flags = NEMU_HGCM_F_PARM_DIRECTION_TO_HOST;
    ...
    rc = VbglHGCMCall(pClient->handle, &pData->callInfo, cbData);
/*    Log(("NEMUSF: VbglR0SfWritePhysCont: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS(rc))
    {
        rc = pData->callInfo.result;
        *pcbBuffer = pData->cb.u.value32;
                                    ...
                                    uint16_t offFirstPage, uint16_t cPages, RTGCPHYS64 *paPages)
{
    ...
    rc = VbglHGCMCall(pClient->handle, &pData->callInfo, cbData);
/*    Log(("NEMUSF: VbglR0SfWritePageList: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS(rc))
    {
        rc = pData->callInfo.result;
        *pcbBuffer = pData->cb.u.value32;
...
DECLVBGL(int) VbglR0SfFlush(PVBGLSFCLIENT pClient, PVBGLSFMAP pMap, SHFLHANDLE hFile)
{
    ...
    NEMU_INIT_CALL(&data.callInfo, FLUSH, pClient);
    ...
    rc = VbglHGCMCall(pClient->handle, &data.callInfo, sizeof(data));
/*    Log(("NEMUSF: VbglR0SfFlush: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS(rc))
        rc = data.callInfo.result;
    return rc;
}
    ...
    uint32_t *pcFiles)
{
    ...
    NEMU_INIT_CALL(&data.callInfo, LIST, pClient);
    ...
    rc = VbglHGCMCall(pClient->handle, &data.callInfo, sizeof(data));
/*    Log(("NEMUSF: VbglR0SfDirInfo: rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS(rc))
        rc = data.callInfo.result;
    *pcbBuffer = data.cb.u.value32;
...
{
    int rc;
    NemuSFInformation data;

    NEMU_INIT_CALL(&data.callInfo, INFORMATION, pClient);
    ...
    rc = VbglHGCMCall(pClient->handle, &data.callInfo, sizeof(data));
/*    Log(("NEMUSF: VbglR0SfFsInfo: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS(rc))
    {
        rc = data.callInfo.result;
        *pcbBuffer = data.cb.u.value32;
                           ...
                           uint64_t offset, uint64_t cbSize, uint32_t fLock)
{
    ...
    NEMU_INIT_CALL(&data.callInfo, LOCK, pClient);
    ...
    rc = VbglHGCMCall(pClient->handle, &data.callInfo, sizeof(data));
/*    Log(("NEMUSF: VbglR0SfLock: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS (rc))
        rc = data.callInfo.result;
    return rc;
}
...
DECLVBGL(int) VbglR0SfSetUtf8(PVBGLSFCLIENT pClient)
{
    ...
    NEMU_INIT_CALL(&callInfo, SET_UTF8, pClient);
    rc = VbglHGCMCall(pClient->handle, &callInfo, sizeof(callInfo));
/*    Log(("NEMUSF: VbglR0SfSetUtf8: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS(rc))
        rc = callInfo.result;
    return rc;
}
...
DECLVBGL(int) VbglR0SfReadLink(PVBGLSFCLIENT pClient, PVBGLSFMAP pMap, PSHFLSTRING pParsedPath, uint32_t cbBuffer, uint8_t *pBuffer)
{
    ...
    NEMU_INIT_CALL(&data.callInfo, READLINK, pClient);
    ...
    rc = VbglHGCMCall(pClient->handle, &data.callInfo, sizeof(data));
/*    Log(("NEMUSF: VbglR0SfReadLink: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS (rc))
        rc = data.callInfo.result;
    return rc;
}
                              ...
                              PSHFLFSOBJINFO pBuffer)
{
    ...
    NEMU_INIT_CALL(&data.callInfo, SYMLINK, pClient);
    ...
    rc = VbglHGCMCall(pClient->handle, &data.callInfo, sizeof(data));
/*    Log(("NEMUSF: VbglR0SfSymlink: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS (rc))
        rc = data.callInfo.result;
    return rc;
}
...
DECLVBGL(int) VbglR0SfSetSymlinks(PVBGLSFCLIENT pClient)
{
    ...
    NEMU_INIT_CALL(&callInfo, SET_SYMLINKS, pClient);
    rc = VbglHGCMCall(pClient->handle, &callInfo, sizeof(callInfo));
/*    Log(("NEMUSF: VbglR0SfSetSymlinks: VbglHGCMCall rc = %#x, result = %#x\n", rc, data.callInfo.result)); */
    if (RT_SUCCESS(rc))
        rc = callInfo.result;
    return rc;
}
