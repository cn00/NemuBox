...
#include <Nemu/err.h>
#include <Nemu/vd-ifs.h>

RT_C_DECLS_BEGIN

#ifdef IN_RING0
# error "There are no Nemu HDD Container APIs available in Ring-0 Host Context!"
#endif

/** @defgroup grp_vd            Nemu HDD Container
 * @{
 */
...
/** @name Nemu HDD container image flags
 ...
 */
typedef struct NEMUHDDRAWPARTDESC
{
    ...
    uint64_t        cbData;
    /** Flags for special treatment, see NEMUHDDRAW_FLAGS_*. */
    uint32_t        uFlags;
} NEMUHDDRAWPARTDESC, *PNEMUHDDRAWPARTDESC;

/**
 * Auxiliary data structure for difference between GPT and MBR
 * disks.
 */
typedef enum NEMUHDDPARTTYPE
{
    MBR,
    GPT
} NEMUHDDPARTTYPE;
...
typedef struct NEMUHDDRAW
{
    ...
    PNEMUHDDRAWPARTDESC pPartDescs;
    /** Partitioning type of the disk */
    NEMUHDDPARTTYPE uPartitioningType;
} NEMUHDDRAW, *PNEMUHDDRAW;


/** @name Nemu HDD container image open mode flags
 * @{
 */
...
/** @name Nemu HDD container filter flags
 * @{
 */
...
/** @name Nemu HDD container backend capability flags
 * @{
 */
...
/**
 * Nemu HDD Container main structure.
 */
/* Forward declaration, NEMUHDD structure is visible only inside Nemu HDD module. */
struct NEMUHDD;
typedef struct NEMUHDD NEMUHDD;
typedef NEMUHDD *PNEMUHDD;
 ...
 * @param   pVDIfsOperation Pointer to the per-operation VD interface list.
 */
NEMUDDU_DECL(int) VDCompact(PNEMUHDD pDisk, unsigned nImage, PVDINTERFACE pVDIfsOperation);
 ...
 *                          Must be aligned to a sector boundary.
 */
NEMUDDU_DECL(int) VDRead(PNEMUHDD pDisk, uint64_t off, void *pvBuffer, size_t cbBuffer);
 ...
 *                          Must be aligned to a sector boundary.
 */
NEMUDDU_DECL(int) VDWrite(PNEMUHDD pDisk, uint64_t off, const void *pvBuffer, size_t cbBuffer);
 ...
 */
NEMUDDU_DECL(int) VDGetPCHSGeometry(PNEMUHDD pDisk, unsigned nImage, PVDGEOMETRY pPCHSGeometry);
 ...
 */
NEMUDDU_DECL(int) VDSetPCHSGeometry(PNEMUHDD pDisk, unsigned nImage, PCVDGEOMETRY pPCHSGeometry);
 ...
 */
NEMUDDU_DECL(int) VDGetLCHSGeometry(PNEMUHDD pDisk, unsigned nImage, PVDGEOMETRY pLCHSGeometry);
 ...
 */
NEMUDDU_DECL(int) VDSetLCHSGeometry(PNEMUHDD pDisk, unsigned nImage, PCVDGEOMETRY pLCHSGeometry);
 ...
 */
NEMUDDU_DECL(int) VDGetVersion(PNEMUHDD pDisk, unsigned nImage, unsigned *puVersion);
 ...
 */
NEMUDDU_DECL(int) VDBackendInfoSingle(PNEMUHDD pDisk, unsigned nImage, PVDBACKENDINFO pBackendInfo);
 ...
 */
NEMUDDU_DECL(int) VDGetImageFlags(PNEMUHDD pDisk, unsigned nImage, unsigned *puImageFlags);
 ...
 */
NEMUDDU_DECL(int) VDGetOpenFlags(PNEMUHDD pDisk, unsigned nImage, unsigned *puOpenFlags);
 ...
 * @param   uOpenFlags      Image file open mode, see VD_OPEN_FLAGS_* constants.
 */
NEMUDDU_DECL(int) VDSetOpenFlags(PNEMUHDD pDisk, unsigned nImage, unsigned uOpenFlags);
 ...
 */
NEMUDDU_DECL(int) VDGetFilename(PNEMUHDD pDisk, unsigned nImage, char *pszFilename, unsigned cbFilename);
 ...
 */
NEMUDDU_DECL(int) VDGetComment(PNEMUHDD pDisk, unsigned nImage, char *pszComment, unsigned cbComment);
 ...
 */
NEMUDDU_DECL(int) VDSetComment(PNEMUHDD pDisk, unsigned nImage, const char *pszComment);
 ...
 */
NEMUDDU_DECL(int) VDGetParentUuid(PNEMUHDD pDisk, unsigned nImage, PRTUUID pUuid);
 ...
 */
NEMUDDU_DECL(int) VDSetParentUuid(PNEMUHDD pDisk, unsigned nImage, PCRTUUID pUuid);
 ...
 */
NEMUDDU_DECL(int) VDAsyncRead(PNEMUHDD pDisk, uint64_t off, size_t cbRead,
                              PCRTSGBUF pcSgBuf,
 ...
 * @param   pvUser2         User data which is passed on completion.
 */
NEMUDDU_DECL(int) VDAsyncWrite(PNEMUHDD pDisk, uint64_t off, size_t cbWrite,
                               PCRTSGBUF pcSgBuf,
                               PFNVDASYNCTRANSFERCOMPLETE pfnComplete,
                               void *pvUser1, void *pvUser2);
 ...
 * @param   pvUser2         User data which is passed on completion.
 */
NEMUDDU_DECL(int) VDAsyncFlush(PNEMUHDD pDisk,
                               PFNVDASYNCTRANSFERCOMPLETE pfnComplete,
                               void *pvUser1, void *pvUser2);
 ...
 *                          success.
 */
NEMUDDU_DECL(int) VDCreateVfsFileFromDisk(PNEMUHDD pDisk, uint32_t fFlags,
