; $Id: NemuGuestAdditionsLog.nsh $
;; @file
; NemuGuestAdditionLog.nsh - Logging functions.
;
...
;
; Sends a logging text to the running instance of NemuTray
...
;
!macro _logToNemuTray type text
...
!if $%NEMU_WITH_GUEST_INSTALL_HELPER% == "1"
    ...
    NemuGuestInstallHelper::NemuTrayShowBallonMsg "${text}" "VirtualBox Guest Additions Setup" ${type} 5000
    Pop $0 ; Get return value (ignored for now)
    Pop $0 ; Restore original $0 from stack
!endif

!macroend
!define LogToNemuTray "!insertmacro _logToNemuTray"