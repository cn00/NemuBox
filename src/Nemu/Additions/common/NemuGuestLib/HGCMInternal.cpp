/* $Id: HGCMInternal.cpp $ */
/** @file
 * NemuGuestLib - Host-Guest Communication Manager internal functions, implemented by NemuGuest
 */
...
/* These functions can be only used by NemuGuest. */

DECLVBGL(int) VbglR0HGCMInternalConnect (NemuGuestHGCMConnectInfo *pConnectInfo,
 ...
 *                          physical page lists.
 */
static int vbglR0HGCMInternalPreprocessCall(NemuGuestHGCMCallInfo const *pCallInfo, uint32_t cbCallInfo,
                                            bool fIsUser, struct VbglR0ParmInfo *pParmInfo,  size_t *pcbExtra)
{
    HGCMFunctionParameter const *pSrcParm = NEMUGUEST_HGCM_CALL_PARMS(pCallInfo);
        ...
        switch (pSrcParm->type)
        {
            ...
            case VMMDevHGCMParmType_PageList:
                ...
                if (cb)
                {
                    ...
                    AssertMsgReturn(NEMU_HGCM_F_PARM_ARE_VALID(pPgLst->flags), ("%#x\n", pPgLst->flags), VERR_INVALID_PARAMETER);
...
static uint32_t vbglR0HGCMInternalLinAddrTypeToPageListFlags(HGCMFunctionParameterType enmType)
{
    switch (enmType)
    {
        case VMMDevHGCMParmType_LinAddr_In:
        case VMMDevHGCMParmType_LinAddr_Locked_In:
            return NEMU_HGCM_F_PARM_DIRECTION_TO_HOST;

        case VMMDevHGCMParmType_LinAddr_Out:
        case VMMDevHGCMParmType_LinAddr_Locked_Out:
            return NEMU_HGCM_F_PARM_DIRECTION_FROM_HOST;

        default: AssertFailed();
        case VMMDevHGCMParmType_LinAddr:
        case VMMDevHGCMParmType_LinAddr_Locked:
            return NEMU_HGCM_F_PARM_DIRECTION_BOTH;
    }
}
 ...
 *                          physical page lists.
 */
static void vbglR0HGCMInternalInitCall(VMMDevHGCMCall *pHGCMCall, NemuGuestHGCMCallInfo const *pCallInfo,
 ...
 *
 * @returns Nemu status code of this operation, not necessarily the call.
 *
                                    ...
                                    void *pvAsyncData, uint32_t u32AsyncData, bool *pfLeakIt)
{
    ...
    if (    RT_FAILURE(rc)
        &&  rc == pHGCMCall->header.result)
    {
        Assert(pHGCMCall->header.fu32Flags & NEMU_HGCM_REQ_DONE);
        rc = VINF_SUCCESS;
    }
    ...
    *pfLeakIt = false;
    if (rc == VINF_HGCM_ASYNC_EXECUTE)
    {
        ...
        if (pHGCMCall->header.fu32Flags & NEMU_HGCM_REQ_DONE)
        {
            Assert(!(pHGCMCall->header.fu32Flags & NEMU_HGCM_REQ_CANCELLED));
            rc = VINF_SUCCESS;
        }
        else
        {
            ...
            if (rc2 == VERR_NOT_IMPLEMENTED)
            {
                ...
                pHGCMCall->header.fu32Flags |= NEMU_HGCM_REQ_CANCELLED;
...
#endif
            if (RT_SUCCESS(rc)) rc = VERR_INTERRUPTED; /** @todo weed this out from the WINNT NemuGuest code. */
            if (RT_SUCCESS(rc2))
            {
 ...
 */
static int vbglR0HGCMInternalCopyBackResult(NemuGuestHGCMCallInfo *pCallInfo, VMMDevHGCMCall const *pHGCMCall,
                                            struct VbglR0ParmInfo *pParmInfo, bool fIsUser, int rc)
{
    HGCMFunctionParameter const *pSrcParm = VMMDEV_HGCM_CALL_PARMS(pHGCMCall);
    HGCMFunctionParameter       *pDstParm = NEMUGUEST_HGCM_CALL_PARMS(pCallInfo);
    uint32_t    cParms   = pCallInfo->cParms;
#ifdef USE_BOUNCE_BUFFERS
    uint32_t    iLockBuf = 0;
#endif
...
DECLR0VBGL(int) VbglR0HGCMInternalCall32(NemuGuestHGCMCallInfo *pCallInfo, uint32_t cbCallInfo, uint32_t fFlags,
                                         PFNVBGLHGCMCALLBACK pfnAsyncCallback, void *pvAsyncData, uint32_t u32AsyncData)
{
    NemuGuestHGCMCallInfo   *pCallInfo64 = NULL;
     ...
     */
    pCallInfo64 = (NemuGuestHGCMCallInfo *)RTMemTmpAllocZ(sizeof(*pCallInfo64) + cParms * sizeof(HGCMFunctionParameter));
    if (!pCallInfo64)
        return VERR_NO_TMP_MEMORY;

    *pCallInfo64 = *pCallInfo;
    pParm32 = NEMUGUEST_HGCM_CALL_PARMS32(pCallInfo);
    pParm64 = NEMUGUEST_HGCM_CALL_PARMS(pCallInfo64);
