/* $Id: NemuGuestR3LibMisc.cpp $ */
/** @file
 * NemuGuestR3Lib - Ring-3 Support Library for VirtualBox guest additions, Misc.
 */
...
{
    NemuGuestFilterMaskInfo Info;
    Info.u32OrMask = fOr;
    Info.u32NotMask = fNot;
    return vbglR3DoIOCtl(NEMUGUEST_IOCTL_CTL_FILTER_MASK, &Info, sizeof(Info));
}
...
{
    NemuGuestSetCapabilitiesInfo Info;
    Info.u32OrMask = fOr;
    Info.u32NotMask = fNot;
    return vbglR3DoIOCtl(NEMUGUEST_IOCTL_SET_GUEST_CAPABILITIES, &Info,
                         sizeof(Info));
}
 ...
 *                              NemuServiceVMInfoWorker() for an example.)
 */
