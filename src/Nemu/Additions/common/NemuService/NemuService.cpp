/* $Id: NemuService.cpp $ */
/** @file
 * NemuService - Guest Additions Service Skeleton.
 */
...
/** @page pg_vgsvc NemuService
 *
 * NemuService is a root daemon for implementing guest additions features.
 *
 ...
 * reason is that the NemuService binary is typically statically linked, at
...
/**
 * @interface_method_impl{NEMUSERVICE,pfnPreInit, Default Implementation}
 */
...
/**
 * @interface_method_impl{NEMUSERVICE,pfnOption, Default Implementation}
 */
...
/**
 * @interface_method_impl{NEMUSERVICE,pfnInit, Default Implementation}
 */
...
/**
 * @interface_method_impl{NEMUSERVICE,pfnTerm, Default Implementation}
 */
    ...
    switch (enmPhase)
    {
        case RTLOGPHASE_BEGIN:
        {
            pfnLog(pLoggerRelease,
                   "NemuService %s r%s (verbosity: %u) %s (%s %s) release log\n"
                   "Log opened %s\n",
                   RTBldCfgVersion(), RTBldCfgRevisionStr(), g_cVerbosity, NEMU_BUILD_TARGET,
...
int VGSvcLogCreate(const char *pszLogFile)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        /* Explicitly flush the log in case of NEMUSERVICE_RELEASE_LOG=buffered. */
 ...
 * Create the PID file.
 */
int nemuServiceCreatePidFile(const char *pszPidFile)
{
    RTFILE hFile;
...
static int vgsvcUsage(void)
{
    ...
    RTPrintf("\n"
             " Copyright (C) 2009-" NEMU_C_YEAR " " NEMU_VENDOR "\n");
...
/**
 * Reports the current NemuService status to the host.
 *
 ...
 */
int VGSvcReportStatus(NemuGuestFacilityStatus enmStatus)
{
    /*
     * NemuGuestFacilityStatus_Failed is sticky.
     */
    static NemuGuestFacilityStatus s_enmLastStatus = NemuGuestFacilityStatus_Inactive;
    VGSvcVerbose(4, "Setting NemuService status to %u\n", enmStatus);
    if (s_enmLastStatus != NemuGuestFacilityStatus_Failed)
    {
        int rc = VbglR3ReportAdditionsStatus(NemuGuestFacilityType_NemuService, enmStatus, 0 /* Flags */);
        if (RT_FAILURE(rc))
        {
            VGSvcError("Could not report NemuService status (%u), rc=%Rrc\n", enmStatus, rc);
            return rc;
        }
 ...
 *
 * @returns Nemu status code, error message displayed.
 */
 ...
 * Starts the service.
 *
 * @returns Nemu status code, errors are fully bitched.
 *
 * @remarks Also called from NemuService-win.cpp, thus not static.
 */
int VGSvcStartServices(void)
{
    int rc;

    VGSvcReportStatus(NemuGuestFacilityStatus_Init);
        ...
        if (g_aServices[j].fEnabled)
        {
            ...
            if (RT_FAILURE(rc))
            {
                if (rc != VERR_SERVICE_DISABLED)
                {
                    ...
                    VGSvcReportStatus(NemuGuestFacilityStatus_Failed);
                    return rc;
                }
 ...
 *
 * This should be called even when NemuServiceStartServices fails so it can
...
{
    VGSvcReportStatus(NemuGuestFacilityStatus_Terminating);
     ...
     * sleeping in NemuServiceMainWait).
     */
    ...
    VGSvcReportStatus(RT_SUCCESS(rc) ? NemuGuestFacilityStatus_Paused : NemuGuestFacilityStatus_Failed);
    return rc;
}
...
void VGSvcMainWait(void)
{
    ...
    VGSvcReportStatus(NemuGuestFacilityStatus_Active);
...
int main(int argc, char **argv)
{
     ...
     * NemuService and shouldn't be subject to /dev/nemuguest, pid-files and
...
#ifdef NEMU_WITH_GUEST_CONTROL
    /*
     * Check if we're the specially spawned NemuService.exe process that
...
#undef MATCHES
...
#ifdef RT_OS_WINDOWS
    /*
     * Make sure only one instance of NemuService runs at a time.  Create a
    ...
    if (    GetVersionEx((LPOSVERSIONINFO)&OSInfoEx)
        ...
        hMutexAppRunning = CreateMutex(NULL, FALSE, "Global\\" NEMUSERVICE_NAME);
    else
        hMutexAppRunning = CreateMutex(NULL, FALSE, NEMUSERVICE_NAME);
...
# endif /* !RT_OS_NT4 */
    ...
    VGSvcReportStatus(NemuGuestFacilityStatus_Terminated);
