/* $Id: SysHlp.cpp $ */
/** @file
 * NemuGuestLibR0 - IDC with NemuGuest and HGCM helpers.
 */
...
# ifdef RT_OS_OS2
#  include <Nemu/NemuGuest.h> /* for NEMUGUESTOS2IDCCONNECT */
RT_C_DECLS_BEGIN
/*
 * On OS/2 we'll do the connecting in the assembly code of the
 * client driver, exporting a g_NemuGuestIDC symbol containing
...
RT_C_DECLS_BEGIN
extern DECLVBGL(void *) NemuGuestIDCOpen(uint32_t *pu32Version);
extern DECLVBGL(void)   NemuGuestIDCClose(void *pvOpaque);
extern DECLVBGL(int)    NemuGuestIDCCall(void *pvOpaque, unsigned int iCmd, void *pvData, size_t cbSize, size_t *pcbReturn);
RT_C_DECLS_END
# endif
...
# ifdef RT_OS_WINDOWS
    ...
    RtlInitUnicodeString(&uszDeviceName, L"\\Device\\NemuGuest");
    ...
    if (NT_SUCCESS(rc))
    {
        Log(("vbglDriverOpen NemuGuest successful pDeviceObject=%x\n", pDeviceObject));
        pDriver->pDeviceObject = pDeviceObject;
        pDriver->pFileObject = pFileObject;
        return VINF_SUCCESS;
    }
    /** @todo return RTErrConvertFromNtStatus(rc)! */
    Log(("vbglDriverOpen NemuGuest failed with ntstatus=%x\n", rc));
    return rc;
...
# elif defined (RT_OS_OS2)
    ...
    if (   g_NemuGuestIDC.u32Version == VMMDEV_VERSION
        && RT_VALID_PTR(g_NemuGuestIDC.u32Session)
        && RT_VALID_PTR(g_NemuGuestIDC.pfnServiceEP))
    {
        pDriver->u32Session = g_NemuGuestIDC.u32Session;
        return VINF_SUCCESS;
    }
...
# else
    ...
    pDriver->pvOpaque = NemuGuestIDCOpen(&u32VMMDevVersion);
    if (   pDriver->pvOpaque
...
# elif defined (RT_OS_OS2)
    if (    pDriver->u32Session
        &&  pDriver->u32Session == g_NemuGuestIDC.u32Session)
        return g_NemuGuestIDC.pfnServiceEP(pDriver->u32Session, u32Function, pvData, cbData, NULL);

    Log(("vbglDriverIOCtl: No connection\n"));
    return VERR_WRONG_ORDER;

# else
    return NemuGuestIDCCall(pDriver->pvOpaque, u32Function, pvData, cbData, NULL);
# endif
}
...
# else
    NemuGuestIDCClose(pDriver->pvOpaque);
