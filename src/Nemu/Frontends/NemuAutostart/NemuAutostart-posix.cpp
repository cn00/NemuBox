/* $Id: NemuAutostart-posix.cpp $ */
/** @file
 * NemuAutostart - VirtualBox Autostart service.
 */
...
# define NEMUAUTOSTART_DAEMONIZE
#endif
...
{
    RTStrmPrintf(g_pStdErr, NEMU_PRODUCT " Autostart " NEMU_VERSION_STRING "\n"
...
static void displayHelp(const char *pszImage)
{
    ...
    RTStrmPrintf(g_pStdErr, "\nUse environment variable NEMUAUTOSTART_RELEASE_LOG for logging options.\n");
}
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    rc = com::GetNemuUserHomeDirectory(szUserHomeDir, sizeof(szUserHomeDir), false /* fCreateDir */);
    if (RT_FAILURE(rc))
    ...
    rc = com::NemuLogRelCreate("Autostart", g_fDaemonize ? NULL : pszLogFile,
                               RTLOGFLAGS_PREFIX_THREAD | RTLOGFLAGS_PREFIX_TIME_PROG,
                               "all", "NEMUAUTOSTART_RELEASE_LOG",
    ...
    if (g_fDaemonize)
    {
        ...
        if (!pszLogFile || !*pszLogFile)
        {
            ...
            rc = RTPathAppend(szLogFile, sizeof(szLogFile), "nemuautostart.log");
            if (RT_FAILURE(rc))
        ...
        rc = com::NemuLogRelCreate("Autostart", pszLogFile,
                                   RTLOGFLAGS_PREFIX_THREAD | RTLOGFLAGS_PREFIX_TIME_PROG,
                                   "all", "NEMUAUTOSTART_RELEASE_LOG",
