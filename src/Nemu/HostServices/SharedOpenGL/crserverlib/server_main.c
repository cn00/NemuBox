...
static DECLCALLBACK(int8_t) crNemuCrCmdCmd(HNEMUCRCMDSVR hSvr, const NEMUCMDVBVA_HDR *pCmd, uint32_t cbCmd);

DECLINLINE(CRClient*) crNemuServerClientById(uint32_t u32ClientID)
{
    int32_t i;
...
int32_t crNemuServerClientGet(uint32_t u32ClientID, CRClient **ppClient)
{
    CRClient *pClient = NULL;

    pClient = crNemuServerClientById(u32ClientID);
...
static int crNemuServerCrCmdDisablePostProcess(NEMUCRCMDCTL_HGCMENABLE_DATA *pData);
...
static void crServerTearDown( void )
{
    ...
    if (cr_server.fCrCmdEnabled)
    {
        NEMUCRCMDCTL_HGCMENABLE_DATA EnableData;
        /* crNemuServerHgcmEnable will erase the DisableData, preserve it here */
        NEMUCRCMDCTL_HGCMDISABLE_DATA DisableData = cr_server.DisableData;
        int rc;
        ...
        crNemuServerCrCmdDisablePostProcess(&EnableData);
    ...
    if (!fContextsDeleted)
    {
#ifndef NEMU_WITH_CR_DISPLAY_LISTS
    ...
    NemuVrListClear(&cr_server.RootVr);
...
crServerInit(int argc, char *argv[])
{
    ...
    if (!RT_SUCCESS(rc))
    {
        crWarning("NemuVrInit failed, rc %d", rc);
        return;
    }
    ...
    NemuVrListInit(&cr_server.RootVr);
...
void crNemuServerTearDown(void)
{
    crServerTearDown();
}
 ...
 * Do CRServer initializations.  After this, we can begin servicing clients.
 */
GLboolean crNemuServerInit(void)
{
    CRMuralInfo *defaultMural;
    const char*env;
    int rc = NemuVrInit();
    ...
    crServerSetNemuConfigurationHGCM();
...
static int32_t crNemuServerAddClientObj(uint32_t u32ClientID, CRClient **ppNewClient)
{
    CRClient *newClient;
...
int32_t crNemuServerAddClient(uint32_t u32ClientID)
{
    CRClient *newClient;
...
static void crNemuServerRemoveClientObj(CRClient *pClient)
{
#ifdef NEMU_WITH_CRHGSMI
    CRNEMUHGSMI_CMDDATA_ASSERT_CLEANED(&pClient->conn->CmdData);
#endif
...
static void crNemuServerRemoveAllClients()
{
    int32_t i;
    for (i = cr_server.numClients - 1; i >= 0; --i)
    {
        Assert(cr_server.clients[i]);
        crNemuServerRemoveClientObj(cr_server.clients[i]);
    }
}

void crNemuServerRemoveClient(uint32_t u32ClientID)
{
    CRClient *pClient=NULL;
    int32_t i;
    ...
    if (!pClient)
    {
        WARN(("Invalid client id %u passed to crNemuServerRemoveClient", u32ClientID));
        return;
    }

    crNemuServerRemoveClientObj(pClient);
}

static void crNemuServerInternalClientWriteRead(CRClient *pClient)
{
#ifdef NEMUCR_LOGFPS
...
#ifdef NEMU_WITH_CRHGSMI
    CRNEMUHGSMI_CMDDATA_ASSERT_CONSISTENT(&pClient->conn->CmdData);
#endif

    if (
#ifdef NEMU_WITH_CRHGSMI
         !CRNEMUHGSMI_CMDDATA_IS_SET(&pClient->conn->CmdData) &&
#endif
...
int32_t crNemuServerClientWrite(uint32_t u32ClientID, uint8_t *pBuffer, uint32_t cbBuffer)
{
    CRClient *pClient=NULL;
    int32_t rc = crNemuServerClientGet(u32ClientID, &pClient);
    ...
    crNemuServerInternalClientWriteRead(pClient);

    return VINF_SUCCESS;
}

int32_t crNemuServerInternalClientRead(CRClient *pClient, uint8_t *pBuffer, uint32_t *pcbBuffer)
{
...
int32_t crNemuServerClientRead(uint32_t u32ClientID, uint8_t *pBuffer, uint32_t *pcbBuffer)
{
    CRClient *pClient=NULL;
    int32_t rc = crNemuServerClientGet(u32ClientID, &pClient);
    ...
    return crNemuServerInternalClientRead(pClient, pBuffer, pcbBuffer);
}

extern DECLEXPORT(int32_t) crNemuServerClientGetCapsLegacy(uint32_t u32ClientID, uint32_t *pu32Caps)
{
    uint32_t u32Caps = cr_server.u32Caps;
    u32Caps &= ~CR_NEMU_CAP_CMDVBVA;
    *pu32Caps = u32Caps;
    return VINF_SUCCESS;
}

extern DECLEXPORT(int32_t) crNemuServerClientGetCapsNew(uint32_t u32ClientID, CR_CAPS_INFO *pInfo)
{
...
static int32_t crNemuServerClientObjSetVersion(CRClient *pClient, uint32_t vMajor, uint32_t vMinor)
{
...
int32_t crNemuServerClientSetVersion(uint32_t u32ClientID, uint32_t vMajor, uint32_t vMinor)
{
    CRClient *pClient=NULL;
    int32_t i;
    ...
    return crNemuServerClientObjSetVersion(pClient, vMajor, vMinor);
}

static int32_t crNemuServerClientObjSetPID(CRClient *pClient, uint64_t pid)
{
    pClient->pid = pid;
...
int32_t crNemuServerClientSetPID(uint32_t u32ClientID, uint64_t pid)
{
    CRClient *pClient=NULL;
    int32_t i;
    ...
    return crNemuServerClientObjSetPID(pClient, pid);
}
...
static void crNemuServerSaveMuralCB(unsigned long key, void *data1, void *data2)
{
...
static void crNemuServerSaveCreateInfoCB(unsigned long key, void *data1, void *data2)
{
...
static void crNemuServerSaveCreateInfoFromMuralInfoCB(unsigned long key, void *data1, void *data2)
{
    ...
    crNemuServerSaveCreateInfoCB(key, &CreateInfo, data2);
}

static void crNemuServerSaveCreateInfoFromCtxInfoCB(unsigned long key, void *data1, void *data2)
{
...
static void crNemuServerSyncTextureCB(unsigned long key, void *data1, void *data2)
{
...
typedef struct CRNEMU_SAVE_STATE_GLOBAL
{
    ...
    int rc;
} CRNEMU_SAVE_STATE_GLOBAL, *PCRNEMU_SAVE_STATE_GLOBAL;


typedef struct CRNEMU_CTXWND_CTXWALKER_CB
{
    PCRNEMU_SAVE_STATE_GLOBAL pGlobal;
    CRHashTable *usedMuralTable;
    GLuint cAdditionalMurals;
} CRNEMU_CTXWND_CTXWALKER_CB, *PCRNEMU_CTXWND_CTXWALKER_CB;

static void crNemuServerBuildAdditionalWindowContextMapCB(unsigned long key, void *data1, void *data2)
{
    CRMuralInfo * pMural = (CRMuralInfo *) data1;
    PCRNEMU_CTXWND_CTXWALKER_CB pData = (PCRNEMU_CTXWND_CTXWALKER_CB)data2;
...
typedef struct CRNEMU_CTXWND_WNDWALKER_CB
{
    PCRNEMU_SAVE_STATE_GLOBAL pGlobal;
    CRHashTable *usedMuralTable;
    CRContextInfo *pContextInfo;
    CRMuralInfo * pMural;
} CRNEMU_CTXWND_WNDWALKER_CB, *PCRNEMU_CTXWND_WNDWALKER_CB;

static void crNemuServerBuildContextWindowMapWindowWalkerCB(unsigned long key, void *data1, void *data2)
{
    CRMuralInfo * pMural = (CRMuralInfo *) data1;
    PCRNEMU_CTXWND_WNDWALKER_CB pData = (PCRNEMU_CTXWND_WNDWALKER_CB)data2;
...
static void crNemuServerBuildContextUsedWindowMapCB(unsigned long key, void *data1, void *data2)
{
    CRContextInfo *pContextInfo = (CRContextInfo *)data1;
    PCRNEMU_CTXWND_CTXWALKER_CB pData = (PCRNEMU_CTXWND_CTXWALKER_CB)data2;
...
static void crNemuServerBuildContextUnusedWindowMapCB(unsigned long key, void *data1, void *data2)
{
    CRContextInfo *pContextInfo = (CRContextInfo *)data1;
    PCRNEMU_CTXWND_CTXWALKER_CB pData = (PCRNEMU_CTXWND_CTXWALKER_CB)data2;
    ...
    {
        CRNEMU_CTXWND_WNDWALKER_CB MuralData;
        ...
        crHashtableWalk(cr_server.muralTable, crNemuServerBuildContextWindowMapWindowWalkerCB, &MuralData);
...
static void crNemuServerBuildSaveStateGlobal(PCRNEMU_SAVE_STATE_GLOBAL pGlobal)
{
    CRNEMU_CTXWND_CTXWALKER_CB Data;
    GLuint cMurals;
    ...
    crHashtableWalk(cr_server.contextTable, crNemuServerBuildContextUsedWindowMapCB, &Data);
    ...
    {
        Data.cAdditionalMurals = 0;
        crHashtableWalk(cr_server.contextTable, crNemuServerBuildContextUnusedWindowMapCB, &Data);
    }
    ...
    {
        crHashtableWalk(cr_server.muralTable, crNemuServerBuildAdditionalWindowContextMapCB, &Data);
...
static void crNemuServerFBImageDataTerm(CRFBData *pData)
{
    GLuint i;
...
static int crNemuAddFBDataElement(CRFBData *pData, GLint idFBO, GLenum enmBuffer, GLint width, GLint height, GLenum enmFormat, GLenum enmType)
{
    CRFBDataElement *pEl;
    ...
    if (!pEl->pvData)
    {
        crNemuServerFBImageDataTerm(pData);
        crWarning(": crCalloc failed");
...
static int crNemuServerFBImageDataInitEx(CRFBData *pData, CRContextInfo *pCtxInfo, CRMuralInfo *pMural, GLboolean fWrite, uint32_t version, GLuint overrideWidth, GLuint overrideHeight)
{
    CRContext *pContext;
    GLuint i;
    GLfloat *pF;
    ...
    rc = crNemuAddFBDataElement(pData, pMural && pMural->fRedirected ? pMural->aidFBOs[CR_SERVER_FBO_FB_IDX(pMural)] : 0,
    ...
    {
        rc = crNemuAddFBDataElement(pData, pMural && pMural->fRedirected ? pMural->aidFBOs[CR_SERVER_FBO_BB_IDX(pMural)] : 0,
    ...
    if (version < SHCROGL_SSM_VERSION_WITH_SINGLE_DEPTH_STENCIL)
    {
        ...
        /* Init to default depth value, just in case. "pData->cElements - 1" because we incremented counter in crNemuAddFBDataElement(). */
    ...
    if (version < SHCROGL_SSM_VERSION_WITH_SEPARATE_DEPTH_STENCIL_BUFFERS)
    {
        ...
        {
            rc = crNemuAddFBDataElement(pData, pMural && pMural->fRedirected ? pMural->aidFBOs[CR_SERVER_FBO_FB_IDX(pMural)] : 0, 0,
...
static int crNemuServerSaveFBImage(PSSMHANDLE pSSM)
{
    ...
    rc = crNemuServerFBImageDataInitEx(&Data.data, pCtxInfo, pMural, GL_FALSE, SHCROGL_SSM_VERSION, 0, 0);
    if (!RT_SUCCESS(rc))
    {
        crWarning("crNemuServerFBImageDataInit failed rc %d", rc);
        return rc;
    }
    ...
    crNemuServerFBImageDataTerm(&Data.data);
...
static void crNemuServerSaveAdditionalMuralsCB(unsigned long key, void *data1, void *data2)
{
    CRContextInfo *pContextInfo = (CRContextInfo *) data1;
    PCRNEMU_SAVE_STATE_GLOBAL pData = (PCRNEMU_SAVE_STATE_GLOBAL)data2;
    ...
    pData->rc = crNemuServerSaveFBImage(pSSM);
...
static void crNemuServerSaveContextStateCB(unsigned long key, void *data1, void *data2)
{
    CRContextInfo *pContextInfo = (CRContextInfo *) data1;
...
static uint32_t g_hackNemuServerSaveLoadCallsLeft = 0;

static int32_t crNemuServerSaveStatePerform(PSSMHANDLE pSSM)
{
    int32_t  rc, i;
    uint32_t ui32;
    GLboolean b;
...
#endif
    CRNEMU_SAVE_STATE_GLOBAL Data;

    crMemset(&Data, 0, sizeof (Data));

#if 0
    crNemuServerCheckConsistency();
#endif
    ...
    if (!cr_server.bIsInSavingState) /* It's first call */
    {
        ...
        g_hackNemuServerSaveLoadCallsLeft = cr_server.fCrCmdEnabled ? 1 : cr_server.numClients;
    }

    g_hackNemuServerSaveLoadCallsLeft--;

    /* Do nothing until we're being called last time */
    if (g_hackNemuServerSaveLoadCallsLeft>0)
    {
        return VINF_SUCCESS;
    }

#ifdef DEBUG_misha
#define CR_DBG_STR_STATE_SAVE_START "Nemu.Cr.StateSaveStart"
#define CR_DBG_STR_STATE_SAVE_STOP "Nemu.Cr.StateSaveStop"
    ...
    AssertRCReturn(rc, rc);
    crHashtableWalk(cr_server.contextTable, crNemuServerSaveCreateInfoFromCtxInfoCB, pSSM);
    ...
    AssertRCReturn(rc, rc);
    crHashtableWalk(cr_server.muralTable, crNemuServerSaveCreateInfoFromMuralInfoCB, pSSM);
    ...
    rc = SSMR3PutU32(pSSM, (uint32_t) ui32-1);
    AssertRCReturn(rc, rc);
    crHashtableWalk(cr_server.muralTable, crNemuServerSaveMuralCB, pSSM);

    /* we need to save front & backbuffer data for each mural first create a context -> mural association */
    crNemuServerBuildSaveStateGlobal(&Data);
     ...
     */
    crHashtableWalk(cr_server.contextTable, crNemuServerSaveContextStateCB, &Data);
    ...
    crHashtableWalk(Data.additionalMuralContextTable, crNemuServerSaveAdditionalMuralsCB, &Data);
    AssertRCReturn(Data.rc, Data.rc);
...
DECLEXPORT(int32_t) crNemuServerSaveState(PSSMHANDLE pSSM)
{
    if (cr_server.fCrCmdEnabled)
    {
    ...
    return crNemuServerSaveStatePerform(pSSM);
}

static DECLCALLBACK(CRContext*) crNemuServerGetContextCB(void* pvData)
{
...
static int32_t crNemuServerLoadMurals(CR_SERVER_LOADSTATE_READER *pReader, uint32_t version)
{
    unsigned long key;
...
static int crNemuServerLoadFBImage(PSSMHANDLE pSSM, uint32_t version,
        CRContextInfo* pContextInfo, CRMuralInfo *pMural)
{
    ...
    if (version >= SHCROGL_SSM_VERSION_WITH_SAVED_DEPTH_STENCIL_BUFFER)
    {
        ...
        rc = crNemuServerFBImageDataInitEx(&Data.data, pContextInfo, pMural, GL_TRUE, version, 0, 0);
        if (!RT_SUCCESS(rc))
        {
            crWarning("crNemuServerFBImageDataInit failed rc %d", rc);
            return rc;
    ...
    else
    {
        ...
        rc = crNemuServerFBImageDataInitEx(&Data.data, pContextInfo, pMural, GL_TRUE, version, storedWidth, storedHeight);
        if (!RT_SUCCESS(rc))
        {
            crWarning("crNemuServerFBImageDataInit failed rc %d", rc);
            return rc;
...
static int32_t crNemuServerLoadStatePerform(PSSMHANDLE pSSM, uint32_t version)
{
    int32_t  rc, i;
    uint32_t ui, uiNumElems;
    unsigned long key;
    ...
    if (!cr_server.bIsInLoadingState)
    {
        ...
        rc = SSMR3GetU32(pSSM, &g_hackNemuServerSaveLoadCallsLeft);
        AssertLogRelRCReturn(rc, rc);

        Assert(g_hackNemuServerSaveLoadCallsLeft);
        /* we get called only once for CrCmd */
        if (cr_server.fCrCmdEnabled)
            g_hackNemuServerSaveLoadCallsLeft = 1;
    }
...
#ifdef DEBUG_misha
#define CR_DBG_STR_STATE_LOAD_START "Nemu.Cr.StateLoadStart"
#define CR_DBG_STR_STATE_LOAD_STOP "Nemu.Cr.StateLoadStop"
    ...
    if (version > SHCROGL_SSM_VERSION_WITH_BUGGY_FB_IMAGE_DATA)
    {
        ...
        rc = crNemuServerLoadMurals(&Reader, version);
        AssertLogRelRCReturn(rc, rc);
        CRASSERT(!Reader.pu8Buffer);
    }
    ...
    for (ui=0; ui<uiNumElems; ++ui)
    {
        ...
        rc = crStateLoadContext(pContext, cr_server.contextTable, crNemuServerGetContextCB, pSSM, version);
        AssertLogRelRCReturn(rc, rc);

        /*Restore front/back buffer images*/
        rc = crNemuServerLoadFBImage(pSSM, version, pContextInfo, pMural);
        AssertLogRelRCReturn(rc, rc);
    }
...
DECLEXPORT(int32_t) crNemuServerLoadState(PSSMHANDLE pSSM, uint32_t version)
{
    if (cr_server.fCrCmdEnabled)
    {
        WARN(("CrCmd enabled"));
    ...
    return crNemuServerLoadStatePerform(pSSM, version);
}

#define SCREEN(i) (cr_server.screen[i])
#define MAPPED(screen) ((screen).winID != 0)

extern DECLEXPORT(void) crServerNemuSetNotifyEventCB(PFNCRSERVERNOTIFYEVENT pfnCb)
{
    cr_server.pfnNotifyEventCB = pfnCb;
}

void crNemuServerNotifyEvent(int32_t idScreen, uint32_t uEvent, void* pvData, uint32_t cbData)
{
...
static void crNemuServerReparentMuralCB(unsigned long key, void *data1, void *data2)
{
    CRMuralInfo *pMI = (CRMuralInfo*) data1;
...
DECLEXPORT(int32_t) crNemuServerSetScreenCount(int sCount)
{
    int i;
...
DECLEXPORT(int32_t) crNemuServerUnmapScreen(int sIndex)
{
    crDebug("crNemuServerUnmapScreen(%i)", sIndex);
    ...
    if (MAPPED(SCREEN(sIndex)))
    {
        ...
        crHashtableWalk(cr_server.muralTable, crNemuServerReparentMuralCB, &sIndex);

        crHashtableWalk(cr_server.dummyMuralTable, crNemuServerReparentMuralCB, &sIndex);
...
DECLEXPORT(int32_t) crNemuServerMapScreen(int sIndex, int32_t x, int32_t y, uint32_t w, uint32_t h, uint64_t winID)
{
    crDebug("crNemuServerMapScreen(%i) [%i,%i:%u,%u %x]", sIndex, x, y, w, h, winID);
    ...
    if (MAPPED(SCREEN(sIndex)) && SCREEN(sIndex).winID!=winID)
    {
        ...
        crNemuServerUnmapScreen(sIndex);
    }
...
DECLEXPORT(int32_t) crNemuServerSetRootVisibleRegion(GLint cRects, const RTRECT *pRects)
{
    int32_t rc = VINF_SUCCESS;
    ...
    {
        crMemset(&cr_server.RootVrCurPoint, 0, sizeof (cr_server.RootVrCurPoint));
        rc = NemuVrListRectsSet(&cr_server.RootVr, cRects, pRects, NULL);
        if (!RT_SUCCESS(rc))
        {
            crWarning("NemuVrListRectsSet failed! rc %d", rc);
            return rc;
        }
...
DECLEXPORT(int32_t) crNemuServerSetOffscreenRendering(GLboolean value)
{
    return CrPMgrModeVrdp(value);
}

DECLEXPORT(int32_t) crNemuServerOutputRedirectSet(const CROutputRedirect *pCallbacks)
{
...
DECLEXPORT(int32_t) crNemuServerSetScreenViewport(int sIndex, int32_t x, int32_t y, uint32_t w, uint32_t h)
{
    CRScreenViewportInfo *pViewport;
    RTRECT NewRect;
    int rc;

    crDebug("crNemuServerSetScreenViewport(%i)", sIndex);

    if (sIndex<0 || sIndex>=cr_server.screenCount)
    {
        crWarning("crNemuServerSetScreenViewport: invalid screen id %d", sIndex);
        return VERR_INVALID_PARAMETER;
    }
...
static void crNemuServerDeleteMuralCb(unsigned long key, void *data1, void *data2)
{
...
static void crNemuServerDefaultContextClear()
{
    HCR_FRAMEBUFFER hFb;
...
//    crStateCleanupCurrent();
    ...
    crHashtableWalk(cr_server.dummyMuralTable, crNemuServerDeleteMuralCb, cr_server.dummyMuralTable);

    cr_server.head_spu->dispatch_table.ChromiumParameteriCR(GL_HH_RENDERTHREAD_INFORM, 0);
}

static void crNemuServerDefaultContextSet()
{
...
static int32_t crNemuServerCmdVbvaCrCmdProcess(const struct NEMUCMDVBVA_CRCMD_CMD *pCmd, uint32_t cbCmd)
{
    int32_t rc;
    ...
    if (cbCmd < RT_OFFSETOF(NEMUCMDVBVA_CRCMD_CMD, aBuffers[cBuffers]))
    {
    ...
    pHdr = NEMUCRHGSMI_PTR_SAFE(pCmd->aBuffers[0].offBuffer, cbHdr, CRNEMUHGSMIHDR);
    if (!pHdr)
    {
    ...
    switch (u32Function)
    {
        case SHCRGL_GUEST_FN_WRITE:
        {
            ...
            if (cParams == 1)
            {
                CRNEMUHGSMIWRITE* pFnCmd = (CRNEMUHGSMIWRITE*)pHdr;
                const NEMUCMDVBVA_CRCMD_BUFFER *pBuf = &pCmd->aBuffers[1];
                /* Fetch parameters. */
                uint32_t cbBuffer = pBuf->cbBuffer;
                uint8_t *pBuffer  = NEMUCRHGSMI_PTR_SAFE(pBuf->offBuffer, cbBuffer, uint8_t);
                ...
                rc = crNemuServerClientGet(u32ClientID, &pClient);
                if (RT_FAILURE(rc))
                {
                    WARN(("crNemuServerClientGet failed %d", rc));
                    break;
                }
                ...
                pClient->conn->pBuffer = pBuffer;
                pClient->conn->cbBuffer = cbBuffer;
                CRNEMUHGSMI_CMDDATA_SET(&pClient->conn->CmdData, pCmd, pHdr, false);
                crNemuServerInternalClientWriteRead(pClient);
                CRNEMUHGSMI_CMDDATA_ASSERT_CLEANED(&pClient->conn->CmdData);
                return VINF_SUCCESS;
            }
        ...
        case SHCRGL_GUEST_FN_INJECT:
        {
            ...
            if (cParams == 1)
            {
                CRNEMUHGSMIINJECT *pFnCmd = (CRNEMUHGSMIINJECT*)pHdr;
                /* Fetch parameters. */
                ...
                rc = crNemuServerClientGet(u32InjectClientID, &pClient);
                if (RT_FAILURE(rc))
                {
                    WARN(("crNemuServerClientGet failed %d", rc));
                    break;
        ...
        case SHCRGL_GUEST_FN_READ:
        {
            ...
            if (cParams == 1)
            {
                CRNEMUHGSMIREAD *pFnCmd = (CRNEMUHGSMIREAD*)pHdr;
                const NEMUCMDVBVA_CRCMD_BUFFER *pBuf = &pCmd->aBuffers[1];
                /* Fetch parameters. */
                uint32_t cbBuffer = pBuf->cbBuffer;
                uint8_t *pBuffer  = NEMUCRHGSMI_PTR_SAFE(pBuf->offBuffer, cbBuffer, uint8_t);
                ...
                rc = crNemuServerInternalClientRead(pClient, pBuffer, &cbBuffer);
                ...
                if (RT_FAILURE(rc))
                {
                    WARN(("crNemuServerInternalClientRead failed %d", rc));
                    break;
                }
        ...
        case SHCRGL_GUEST_FN_WRITE_READ:
        {
            ...
            if (cParams == 2)
            {
                CRNEMUHGSMIWRITEREAD *pFnCmd = (CRNEMUHGSMIWRITEREAD*)pHdr;
                const NEMUCMDVBVA_CRCMD_BUFFER *pBuf = &pCmd->aBuffers[1];
                const NEMUCMDVBVA_CRCMD_BUFFER *pWbBuf = &pCmd->aBuffers[2];
                ...
                char *pWriteback  = NEMUCRHGSMI_PTR_SAFE(pWbBuf->offBuffer, cbWriteback, char);
                ...
                pClient->conn->pBuffer = pBuffer;
                pClient->conn->cbBuffer = cbBuffer;
                CRNEMUHGSMI_CMDDATA_SETWB(&pClient->conn->CmdData, pCmd, pHdr, pWriteback, cbWriteback, &pFnCmd->cbWriteback, false);
                crNemuServerInternalClientWriteRead(pClient);
                CRNEMUHGSMI_CMDDATA_ASSERT_CLEANED(&pClient->conn->CmdData);
                return VINF_SUCCESS;
            }
...
static DECLCALLBACK(int) crNemuCrCmdEnable(HNEMUCRCMDSVR hSvr, NEMUCRCMD_SVRENABLE_INFO *pInfo)
{
    Assert(!cr_server.fCrCmdEnabled);
...
static DECLCALLBACK(int) crNemuCrCmdDisable(HNEMUCRCMDSVR hSvr)
{
    Assert(cr_server.fCrCmdEnabled);

    crNemuServerRemoveAllClients();

    CrHTableEmpty(&cr_server.clientTable);

    crNemuServerDefaultContextClear();
...
static DECLCALLBACK(int) crNemuCrCmdHostCtl(HNEMUCRCMDSVR hSvr, uint8_t* pCmd, uint32_t cbCmd)
{
    return crNemuServerHostCtl((NEMUCRCMDCTL*)pCmd, cbCmd);
}

static int crNemuCrDisconnect(uint32_t u32Client)
{
...
static int crNemuCrConnectEx(NEMUCMDVBVA_3DCTL_CONNECT *pConnect, uint32_t u32ClientId)
{
    CRClient *pClient;
    int rc;
    ...
    rc = crNemuServerAddClientObj(u32ClientId, &pClient);
    if (RT_SUCCESS(rc))
    {
        rc = crNemuServerClientObjSetVersion(pClient, pConnect->u32MajorVersion, pConnect->u32MinorVersion);
        if (RT_SUCCESS(rc))
        {
            rc = crNemuServerClientObjSetPID(pClient, pConnect->u64Pid);
            if (RT_SUCCESS(rc))
            {
                ...
                WARN(("crNemuServerClientObjSetPID failed %d", rc));
        }
        else
            WARN(("crNemuServerClientObjSetVersion failed %d", rc));

        crNemuServerRemoveClientObj(pClient);
    }
    else
        WARN(("crNemuServerAddClientObj failed %d", rc));
...
static int crNemuCrConnect(NEMUCMDVBVA_3DCTL_CONNECT *pConnect)
{
    return crNemuCrConnectEx(pConnect, CRHTABLE_HANDLE_INVALID);
}

static DECLCALLBACK(int) crNemuCrCmdGuestCtl(HNEMUCRCMDSVR hSvr, uint8_t* pCmd, uint32_t cbCmd)
{
    NEMUCMDVBVA_3DCTL *pCtl = (NEMUCMDVBVA_3DCTL*)pCmd;
    if (cbCmd < sizeof (NEMUCMDVBVA_3DCTL))
    {
        WARN(("invalid buffer size"));
        return VERR_INVALID_PARAMETER;
    }
    ...
    switch (pCtl->u32Type)
    {
        case NEMUCMDVBVA3DCTL_TYPE_CONNECT:
        {
            if (cbCmd != sizeof (NEMUCMDVBVA_3DCTL_CONNECT))
            {
                WARN(("invalid command size"));
                return VERR_INVALID_PARAMETER;
            }

            return crNemuCrConnect((NEMUCMDVBVA_3DCTL_CONNECT*)pCtl);
        }
        case NEMUCMDVBVA3DCTL_TYPE_DISCONNECT:
        {
            if (cbCmd != sizeof (NEMUCMDVBVA_3DCTL))
            {
                WARN(("invalid command size"));
            ...
            return crNemuCrDisconnect(pCtl->u32CmdClientId);
        }
        case NEMUCMDVBVA3DCTL_TYPE_CMD:
        {
            NEMUCMDVBVA_3DCTL_CMD *p3DCmd;
            if (cbCmd < sizeof (NEMUCMDVBVA_3DCTL_CMD))
            {
                WARN(("invalid size"));
                return VERR_INVALID_PARAMETER;
            }

            p3DCmd = (NEMUCMDVBVA_3DCTL_CMD*)pCmd;

            return crNemuCrCmdCmd(NULL, &p3DCmd->Cmd, cbCmd - RT_OFFSETOF(NEMUCMDVBVA_3DCTL_CMD, Cmd));
        }
        default:
            WARN(("crNemuCrCmdGuestCtl: invalid function %d", pCtl->u32Type));
            return VERR_INVALID_PARAMETER;
    }
}

static DECLCALLBACK(int) crNemuCrCmdResize(HNEMUCRCMDSVR hSvr, const struct VBVAINFOSCREEN *pScreen, const uint32_t *pTargetMap)
{
    CRASSERT(cr_server.fCrCmdEnabled);
    return CrPMgrResize(pScreen, NULL, pTargetMap);
}

static const char* gszNemuOGLSSMMagic = "***OpenGL state data***";

static int crNemuCrCmdSaveClients(PSSMHANDLE pSSM)
{
    int i;
...
static int crNemuCrCmdLoadClients(PSSMHANDLE pSSM, uint32_t u32Version)
{
    uint32_t i;
    uint32_t u32;
    ...
    {
        uint32_t u32ClientID;
        Connect.Hdr.u32Type = NEMUCMDVBVA3DCTL_TYPE_CONNECT;
        ...
        rc = crNemuCrConnectEx(&Connect, u32ClientID);
        AssertLogRelRCReturn(rc, rc);
    }
...
static DECLCALLBACK(int) crNemuCrCmdSaveState(HNEMUCRCMDSVR hSvr, PSSMHANDLE pSSM)
{
    int rc = VINF_SUCCESS;
    ...
    /* Start*/
    rc = SSMR3PutStrZ(pSSM, gszNemuOGLSSMMagic);
    AssertRCReturn(rc, rc);
    ...
    rc = crNemuCrCmdSaveClients(pSSM);
    AssertRCReturn(rc, rc);

    /* The state itself */
    rc = crNemuServerSaveStatePerform(pSSM);
    AssertRCReturn(rc, rc);
...
static DECLCALLBACK(int) crNemuCrCmdLoadState(HNEMUCRCMDSVR hSvr, PSSMHANDLE pSSM, uint32_t u32Version)
{
    int rc = VINF_SUCCESS;
    ...
    AssertLogRelRCReturn(rc, rc);
    AssertLogRelMsgReturn(!strcmp(gszNemuOGLSSMMagic, szBuf), ("Unexpected data1: '%s'\n", szBuf), VERR_SSM_UNEXPECTED_DATA);
    ...
    if (!ui32)
    {
        ...
        rc = SSMR3GetStrZEx(pSSM, szBuf, sizeof(szBuf), NULL);
        AssertLogRelRCReturn(rc, rc);
        AssertLogRelMsgReturn(!strcmp(gszNemuOGLSSMMagic, szBuf), ("Unexpected data2: '%s'\n", szBuf), VERR_SSM_UNEXPECTED_DATA);
    ...
    rc = crNemuCrCmdLoadClients(pSSM, u32Version);
    AssertLogRelRCReturn(rc, rc);

    /* The state itself */
    rc = crNemuServerLoadStatePerform(pSSM, ui32);
    AssertLogRelRCReturn(rc, rc);
    ...
    rc = SSMR3GetStrZEx(pSSM, szBuf, sizeof(szBuf), NULL);
    AssertLogRelRCReturn(rc, rc);
    AssertLogRelMsgReturn(!strcmp(gszNemuOGLSSMMagic, szBuf), ("Unexpected data5: '%s'\n", szBuf), VERR_SSM_UNEXPECTED_DATA);
...
static DECLCALLBACK(int8_t) crNemuCrCmdCmd(HNEMUCRCMDSVR hSvr, const NEMUCMDVBVA_HDR *pCmd, uint32_t cbCmd)
{
    ...
    {
        case NEMUCMDVBVA_OPTYPE_CRCMD:
        {
            const NEMUCMDVBVA_CRCMD *pCrCmdDr;
            const NEMUCMDVBVA_CRCMD_CMD *pCrCmd;
            int rc;
            pCrCmdDr = (const NEMUCMDVBVA_CRCMD*)pCmd;
            pCrCmd = &pCrCmdDr->Cmd;
            if (cbCmd < sizeof (NEMUCMDVBVA_CRCMD))
            {
                WARN(("invalid buffer size"));
                return -1;
            }
            rc = crNemuServerCmdVbvaCrCmdProcess(pCrCmd, cbCmd - RT_OFFSETOF(NEMUCMDVBVA_CRCMD, Cmd));
            if (RT_SUCCESS(rc))
            {
                /* success */
                return 0;
            ...
            WARN(("crNemuServerCmdVbvaCrCmdProcess failed, rc %d", rc));
            return -1;
        }
        case NEMUCMDVBVA_OPTYPE_FLIP:
        {
            const NEMUCMDVBVA_FLIP *pFlip;

            if (cbCmd < NEMUCMDVBVA_SIZEOF_FLIPSTRUCT_MIN)
            {
                WARN(("invalid buffer size (cbCmd(%u) < sizeof(NEMUCMDVBVA_FLIP)(%u))", cbCmd, sizeof(NEMUCMDVBVA_FLIP)));
                return -1;
            }

            pFlip = (const NEMUCMDVBVA_FLIP*)pCmd;
            return crNemuServerCrCmdFlipProcess(pFlip, cbCmd);
        }
        case NEMUCMDVBVA_OPTYPE_BLT:
        {
            if (cbCmd < sizeof (NEMUCMDVBVA_BLT_HDR))
            {
                WARN(("invalid buffer size"));
                return -1;
            }

            return crNemuServerCrCmdBltProcess((const NEMUCMDVBVA_BLT_HDR*)pCmd, cbCmd);
        }
        case NEMUCMDVBVA_OPTYPE_CLRFILL:
        {
            if (cbCmd < sizeof (NEMUCMDVBVA_CLRFILL_HDR))
            {
                WARN(("invalid buffer size"));
                return -1;
            }

            return crNemuServerCrCmdClrFillProcess((const NEMUCMDVBVA_CLRFILL_HDR*)pCmd, cbCmd);
        }
        default:
 ...
 *
 * NOTE: it is ALWAYS responsibility of the crNemuServerCrHgsmiCmd to complete the command!
 * */


int32_t crNemuServerCrHgsmiCmd(struct NEMUVDMACMD_CHROMIUM_CMD *pCmd, uint32_t cbCmd)
{
    ...
    switch (u32Function)
    {
        case SHCRGL_GUEST_FN_WRITE:
        {
            ...
            {
                CRNEMUHGSMIWRITE* pFnCmd = (CRNEMUHGSMIWRITE*)pHdr;
                NEMUVDMACMD_CHROMIUM_BUFFER *pBuf = &pCmd->aBuffers[1];
                /* Fetch parameters. */
                uint32_t cbBuffer = pBuf->cbBuffer;
                uint8_t *pBuffer  = NEMUCRHGSMI_PTR_SAFE(pBuf->offBuffer, cbBuffer, uint8_t);
                ...
                pClient->conn->pBuffer = pBuffer;
                pClient->conn->cbBuffer = cbBuffer;
                CRNEMUHGSMI_CMDDATA_SET(&pClient->conn->CmdData, pCmd, pHdr, true);
                crNemuServerInternalClientWriteRead(pClient);
                CRNEMUHGSMI_CMDDATA_ASSERT_CLEANED(&pClient->conn->CmdData);
                return VINF_SUCCESS;
            }
        ...
        case SHCRGL_GUEST_FN_WRITE_READ:
        {
            ...
            if (cParams == 2)
            {
                CRNEMUHGSMIWRITEREAD *pFnCmd = (CRNEMUHGSMIWRITEREAD*)pHdr;
                NEMUVDMACMD_CHROMIUM_BUFFER *pBuf = &pCmd->aBuffers[1];
                NEMUVDMACMD_CHROMIUM_BUFFER *pWbBuf = &pCmd->aBuffers[2];
                ...
                pClient->conn->pBuffer = pBuffer;
                pClient->conn->cbBuffer = cbBuffer;
                CRNEMUHGSMI_CMDDATA_SETWB(&pClient->conn->CmdData, pCmd, pHdr, pWriteback, cbWriteback, &pFnCmd->cbWriteback, true);
                crNemuServerInternalClientWriteRead(pClient);
                CRNEMUHGSMI_CMDDATA_ASSERT_CLEANED(&pClient->conn->CmdData);
                return VINF_SUCCESS;
            }
        ...
        case SHCRGL_GUEST_FN_SET_VERSION:
        {
            WARN(("crNemuServerCrHgsmiCmd, SHCRGL_GUEST_FN_SET_VERSION: invalid function"));
            rc = VERR_NOT_IMPLEMENTED;
            break;
        }
        ...
        case SHCRGL_GUEST_FN_SET_PID:
        {
            WARN(("crNemuServerCrHgsmiCmd, SHCRGL_GUEST_FN_SET_PID: invalid function"));
            rc = VERR_NOT_IMPLEMENTED;
            break;
        }
        ...
        default:
        {
            WARN(("crNemuServerCrHgsmiCmd: invalid functionm %d", u32Function));
            rc = VERR_NOT_IMPLEMENTED;
            break;
        }
...
static DECLCALLBACK(bool) crNemuServerHasDataForScreen(uint32_t u32ScreenID)
{
...
static DECLCALLBACK(bool) crNemuServerHasData()
{
...
int32_t crNemuServerCrHgsmiCtl(struct NEMUVDMACMD_CHROMIUM_CTL *pCtl, uint32_t cbCtl)
{
    int rc = VINF_SUCCESS;

    switch (pCtl->enmType)
    {
        case NEMUVDMACMD_CHROMIUM_CTL_TYPE_CRHGSMI_SETUP:
        {
            PNEMUVDMACMD_CHROMIUM_CTL_CRHGSMI_SETUP pSetup = (PNEMUVDMACMD_CHROMIUM_CTL_CRHGSMI_SETUP)pCtl;
            ...
            pSetup->CrCmdServerInfo.pfnEnable = crNemuCrCmdEnable;
            pSetup->CrCmdServerInfo.pfnDisable = crNemuCrCmdDisable;
            pSetup->CrCmdServerInfo.pfnCmd = crNemuCrCmdCmd;
            pSetup->CrCmdServerInfo.pfnHostCtl = crNemuCrCmdHostCtl;
            pSetup->CrCmdServerInfo.pfnGuestCtl = crNemuCrCmdGuestCtl;
            pSetup->CrCmdServerInfo.pfnResize = crNemuCrCmdResize;
            pSetup->CrCmdServerInfo.pfnSaveState = crNemuCrCmdSaveState;
            pSetup->CrCmdServerInfo.pfnLoadState = crNemuCrCmdLoadState;
            rc = VINF_SUCCESS;
            break;
        }
        case NEMUVDMACMD_CHROMIUM_CTL_TYPE_SAVESTATE_BEGIN:
        case NEMUVDMACMD_CHROMIUM_CTL_TYPE_SAVESTATE_END:
            rc = VINF_SUCCESS;
            break;
        case NEMUVDMACMD_CHROMIUM_CTL_TYPE_CRHGSMI_SETUP_MAINCB:
        {
            PNEMUVDMACMD_CHROMIUM_CTL_CRHGSMI_SETUP_MAINCB pSetup = (PNEMUVDMACMD_CHROMIUM_CTL_CRHGSMI_SETUP_MAINCB)pCtl;
            g_hCrHgsmiCompletion = pSetup->hCompletion;
            g_pfnCrHgsmiCompletion = pSetup->pfnCompletion;

            pSetup->MainInterface.pfnHasData = crNemuServerHasData;
            pSetup->MainInterface.pfnHasDataForScreen = crNemuServerHasDataForScreen;
...
static int crNemuServerCrCmdDisablePostProcess(NEMUCRCMDCTL_HGCMENABLE_DATA *pData)
{
    ...
    uint8_t* pCtl;
    uint32_t cbCtl;
    HNEMUCRCMDCTL_REMAINING_HOST_COMMAND hRHCmd = pData->hRHCmd;
    PFNNEMUCRCMDCTL_REMAINING_HOST_COMMAND pfnRHCmd = pData->pfnRHCmd;
    ...
    {
        rc = crNemuCrCmdHostCtl(NULL, pCtl, cbCtl);
    }
...
int32_t crNemuServerHgcmEnable(NEMUCRCMDCTL_HGCMENABLE_DATA *pData)
{
    int rc = crNemuServerCrCmdDisablePostProcess(pData);
    if (RT_FAILURE(rc))
    {
        WARN(("crNemuServerCrCmdDisablePostProcess failed %d", rc));
        return rc;
    }
...
int32_t crNemuServerHgcmDisable(NEMUCRCMDCTL_HGCMDISABLE_DATA *pData)
{
    Assert(!cr_server.fCrCmdEnabled);
