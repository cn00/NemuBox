...
/** @file
 * Nemu Qt GUI - UIMachineSettingsGeneral class implementation.
 */
...
void UIMachineSettingsGeneral::getFromCache()
{
    ...
    m_pNameAndSystemEditor->setType(nemuGlobal().vmGuestOSType(generalData.m_strGuestOsTypeId));
    ...
    if (m_cache.wasChanged())
    {
        ...
        if (isMachineOffline())
        {
            ...
            if (generalData.m_strGuestOsTypeId != m_cache.base().m_strGuestOsTypeId)
            {
                ...
                CGuestOSType newType = nemu.GetGuestOSType(generalData.m_strGuestOsTypeId);
...
bool UIMachineSettingsGeneral::validate(QList<UIValidationMessage> &messages)
{
    ...
    message.first = NemuGlobal::removeAccelMark(mTwGeneral->tabText(0));
    ...
    message.first = NemuGlobal::removeAccelMark(mTwGeneral->tabText(3));
    message.second.clear();
    ...
    if (m_pCheckBoxEncryption->isChecked())
    {
        ...
        const CExtPack extPack = nemuGlobal().virtualBox().GetExtensionPackManager().Find(GUI_ExtPackName);
