...
#ifndef ___Nemu_HostService_DnD_dndmanager_h
#define ___Nemu_HostService_DnD_dndmanager_h
...
{
public:
    DnDGenericMessage(uint32_t uMsg, uint32_t cParms, NEMUHGCMSVCPARM paParms[])
    {
...
public:
    ...
    int addMessage(uint32_t uMsg, uint32_t cParms, NEMUHGCMSVCPARM paParms[], bool fAppend = true);

    HGCM::Message *nextHGCMMessage(void);
    int nextMessageInfo(uint32_t *puMsg, uint32_t *pcParms);
    int nextMessage(uint32_t uMsg, uint32_t cParms, NEMUHGCMSVCPARM paParms[]);
...
#endif /* ___Nemu_HostService_DnD_dndmanager_h */
