...
# define SUP_NT_STATUS_IS_NEMU(a_rcNt)          ( ((uint32_t)(a_rcNt) & 0xffff0000) == SUP_NT_STATUS_BASE )
# define SUP_NT_STATUS_TO_NEMU(a_rcNt)          ( (int)((uint32_t)(a_rcNt) | UINT32_C(0xffff0000)) )

/** NT device name for system access. */
# define SUPDRV_NT_DEVICE_NAME_SYS              L"\\Device\\NemuDrv"
/** NT device name for user access. */
# define SUPDRV_NT_DEVICE_NAME_USR              L"\\Device\\NemuDrvU"
# ifdef NEMU_WITH_HARDENING
/** NT device name for hardened stub access. */
#  define SUPDRV_NT_DEVICE_NAME_STUB            L"\\Device\\NemuDrvStub"
/** NT device name for getting error information for failed NemuDrv or
 * NemuDrvStub open. */
#  define SUPDRV_NT_DEVICE_NAME_ERROR_INFO      L"\\Device\\NemuDrvErrorInfo"
# endif
...
typedef struct SUPREQHDR
{
    ...
    uint32_t        fFlags;
    /** The Nemu status code of the operation, out direction only. */
    int32_t         rc;
} SUPREQHDR;
 ...
 *       when accessing certain nemudrv functions.
 *
 * @{
 */
