...
typedef DECLCALLBACK(int) FNNEMUHOSTWEBCAMADD(void *pvUser,
                                              const char *pszName,
                                              const char *pszPath,
                                              const char *pszAlias,
                                              uint64_t *pu64Result);
typedef FNNEMUHOSTWEBCAMADD *PFNNEMUHOSTWEBCAMADD;

typedef DECLCALLBACK(int) FNNEMUHOSTWEBCAMLIST(PFNNEMUHOSTWEBCAMADD pfnWebcamAdd,
                                               void *pvUser,
                                               uint64_t *pu64WebcamAddResult);
typedef FNNEMUHOSTWEBCAMLIST *PFNNEMUHOSTWEBCAMLIST;

static int loadHostWebcamLibrary(const char *pszPath, RTLDRMOD *phmod, PFNNEMUHOSTWEBCAMLIST *ppfn)
{
    int rc = VINF_SUCCESS;
    ...
    if (RT_SUCCESS(rc))
    {
        static const char *pszSymbol = "NemuHostWebcamList";
...
static HRESULT fillDeviceList(VirtualBox *pVirtualBox, HostVideoInputDeviceList *pList)
{
    ...
    hr = pExtPackMgr->i_getLibraryPathForExtPack("NemuHostWebcam", &strExtPackPuel, &strLibrary);
#else
    hr = E_NOTIMPL;
#endif
    ...
    if (SUCCEEDED(hr))
    {
        PFNNEMUHOSTWEBCAMLIST pfn = NULL;
        RTLDRMOD hmod = NIL_RTLDRMOD;
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            Log(("NemuHostWebcamList rc %Rrc, result 0x%08X\n", rc, u64Result));
            if (RT_FAILURE(rc))
            {
        ...
        {
            if (RT_FAILURE(rc))
                hr = pVirtualBox->setError(NEMU_E_IPRT_ERROR,
