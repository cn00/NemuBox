...
{
    PVMCPU          pVCpu = (PVMCPU)pvUser;
    NEMUGDTR const *pGdtr = (NEMUGDTR const *)((uint8_t const *)&pVCpu->cpum + pDesc->offRegister);
...
{
    PVMCPU          pVCpu = (PVMCPU)pvUser;
    NEMUIDTR const *pIdtr = (NEMUIDTR const *)((uint8_t const *)&pVCpu->cpum + pDesc->offRegister);
...
static DECLCALLBACK(int) cpumR3RegGstGet_msr(void *pvUser, PCDBGFREGDESC pDesc, PDBGFREGVAL pValue)
{
    ...
    uint64_t u64Value;
    NEMUSTRICTRC rcStrict = CPUMQueryGuestMsr(pVCpu, pDesc->offRegister, &u64Value);
...
static DECLCALLBACK(int) cpumR3RegGstSet_msr(void *pvUser, PCDBGFREGDESC pDesc, PCDBGFREGVAL pValue, PCDBGFREGVAL pfMask)
{
    ...
    {
        uint64_t u64FullValue;
        NEMUSTRICTRC rcStrict = CPUMQueryGuestMsr(pVCpu, pDesc->offRegister, &u64FullValue);
        if (rcStrict != VINF_SUCCESS)
        {
     ...
     * Perform the assignment.
     */
    NEMUSTRICTRC rcStrict = CPUMSetGuestMsr(pVCpu, pDesc->offRegister, u64Value);
    if (rcStrict == VINF_SUCCESS)
        return VINF_SUCCESS;
    AssertRC(RT_FAILURE_NP(rcStrict));
    return NEMUSTRICTRC_VAL(rcStrict);
