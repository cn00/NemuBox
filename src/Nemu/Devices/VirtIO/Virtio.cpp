                                        ...
                                        uint16_t uClass)
{
    ...
    vpciCfgSetU16(pci, NEMU_PCI_SUBSYSTEM_VENDOR_ID, DEVICE_PCI_SUBSYSTEM_VENDOR_ID);
    vpciCfgSetU16(pci, NEMU_PCI_SUBSYSTEM_ID, uSubsystemId);

    /* ABI version, must be equal 0 as of 2.6.30 kernel. */
    vpciCfgSetU8( pci, NEMU_PCI_REVISION_ID,          0x00);
    /* Ethernet adapter */
    vpciCfgSetU8( pci, NEMU_PCI_CLASS_PROG,           0x00);
    vpciCfgSetU16(pci, NEMU_PCI_CLASS_DEVICE,       uClass);
    /* Interrupt Pin: INTA# */
    vpciCfgSetU8( pci, NEMU_PCI_INTERRUPT_PIN,        0x01);

#ifdef NEMU_WITH_MSI_DEVICES
    PCIDevSetCapabilityList     (&pci, 0x80);
    PCIDevSetStatus             (&pci, NEMU_PCI_STATUS_CAP_LIST);
#endif
}
...
#if defined(NEMU_WITH_STATISTICS)
