...
#define NEMUCRHGSMI_PTR(_off, _t) ((_t*)(g_pvVRamBase + (_off)))
#define NEMUCRHGSMI_PTR_SAFE(_off, _cb, _t) ((_t*)crServerCrHgsmiPtrGet(_off, _cb))

DECLINLINE(void*) crServerCrHgsmiPtrGet(NEMUVIDEOOFFSET offBuffer, uint32_t cbBuffer)
{
    return ((offBuffer) + (cbBuffer) <= g_cbVRam ? NEMUCRHGSMI_PTR(offBuffer, void) : NULL);
}

DECLINLINE(void) crServerCrHgsmiCmdComplete(struct NEMUVDMACMD_CHROMIUM_CMD *pCmd, int cmdProcessingRc)
{
    g_pfnCrHgsmiCompletion(g_hCrHgsmiCompletion, pCmd, cmdProcessingRc);
}

#define NEMUCRHGSMI_CMD_COMPLETE(_pData, _rc) do { \
        CRNEMUHGSMI_CMDDATA_ASSERT_ISSET(_pData); \
        CRNEMUHGSMI_CMDDATA_RC(_pData, _rc); \
        if (CRNEMUHGSMI_CMDDATA_IS_HGSMICMD(_pData)) { \
            Assert(CRNEMUHGSMI_CMDDATA_IS_HGSMICMD(_pData)); \
            crServerCrHgsmiCmdComplete((_pData)->pHgsmiCmd, VINF_SUCCESS); \
        } \
    } while (0)

#define NEMUCRHGSMI_CMD_CHECK_COMPLETE(_pData, _rc) do { \
        if (CRNEMUHGSMI_CMDDATA_IS_SET(_pData)) {\
            NEMUCRHGSMI_CMD_COMPLETE(_pData, _rc); \
        } \
    } while (0)
...
void crServerSetNemuConfiguration();
void crServerSetNemuConfigurationHGCM();
...
void crNemuServerMuralFbResizeBegin(HCR_FRAMEBUFFER hFb);
void crNemuServerMuralFbResizeEnd(HCR_FRAMEBUFFER hFb);
...
PCR_BLITTER crServerNemuBlitterGet();
PCR_BLITTER crServerNemuBlitterGetInitialized();

DECLINLINE(void) crServerNemuBlitterWinInit(CR_BLITTER_WINDOW *win, CRMuralInfo *mural)
{
...
DECLINLINE(void) crServerNemuBlitterCtxInit(CR_BLITTER_CONTEXT *ctx, CRContextInfo *ctxInfo)
{
...
DECLINLINE(void) crServerRpwEntryDrawSettingsToTex(const CR_SERVER_RPW_ENTRY *pEntry, NEMUVR_TEXTURE *pTex)
{
...
const struct NEMUVR_SCR_COMPOSITOR* CrFbGetCompositor(HCR_FRAMEBUFFER hFb);
...
//#define NEMU_WITH_CRSERVER_DUMPER
...
void crServerDumpTexture(const NEMUVR_TEXTURE *pTex);
...
#else /* if !defined NEMU_WITH_CRSERVER_DUMPER */
...
#endif /* !NEMU_WITH_CRSERVER_DUMPER */
