...
int HGCMGuestConnect (PPDMIHGCMPORT pHGCMPort, PNEMUHGCMCMD pCmdPtr, const char *pszServiceName, uint32_t *pClientID);
int HGCMGuestDisconnect (PPDMIHGCMPORT pHGCMPort, PNEMUHGCMCMD pCmdPtr, uint32_t clientID);
int HGCMGuestCall (PPDMIHGCMPORT pHGCMPort, PNEMUHGCMCMD pCmdPtr, uint32_t clientID, uint32_t function, uint32_t cParms, NEMUHGCMSVCPARM *paParms);

int HGCMHostCall (const char *pszServiceName, uint32_t function, uint32_t cParms, NEMUHGCMSVCPARM aParms[]);
...
int HGCMHostFastCallAsync (HGCMCVSHANDLE hSvc, uint32_t function, PNEMUHGCMSVCPARM pParm, PHGCMHOSTFASTCALLCB pfnCompletion, void *pvCompletion);
#endif
