...
/** @file
 * Nemu Qt GUI - UIMachineSettingsStorage class implementation.
 */
...
{
    return nemuGlobal().virtualBox().GetSystemProperties().GetDeviceTypesForStorageBus (mBusType).toList();
}
    ...
    , mUseIoCache (false)
{
    ...
    mUseIoCache = nemuGlobal().virtualBox().GetSystemProperties().GetDefaultIoCacheSettingForStorageController (aControllerType);
}
...
{
    return (uint)nemuGlobal().virtualBox().GetSystemProperties().GetMaxPortCountForStorageBus(ctrBusType());
}
...
void ControllerItem::setPortCount (uint aPortCount)
{
    ...
    mPortCount = qMin(aPortCount, (uint)nemuGlobal().virtualBox().GetSystemProperties().GetMaxPortCountForStorageBus(ctrBusType()));
}
...
{
    SlotsList allSlots;
    CSystemProperties sp = nemuGlobal().virtualBox().GetSystemProperties();
...
void AttachmentItem::setAttMediumId (const QString &aAttMediumId)
{
    ...
    mAttMediumId = nemuGlobal().medium(aAttMediumId).id();
    cache();
}
...
{
    UIMedium medium = nemuGlobal().medium(mAttMediumId);
        ...
        case R_IsMoreIDEControllersPossible:
        {
            return (m_configurationAccessLevel == ConfigurationAccessLevel_Full) &&
                   (static_cast<RootItem*>(mRootItem)->childCount(KStorageBus_IDE) <
                    nemuGlobal().virtualBox().GetSystemProperties().GetMaxInstancesOfStorageBus(chipsetType(), KStorageBus_IDE));
        }
        case R_IsMoreSATAControllersPossible:
        {
            return (m_configurationAccessLevel == ConfigurationAccessLevel_Full) &&
                   (static_cast<RootItem*>(mRootItem)->childCount(KStorageBus_SATA) <
                    nemuGlobal().virtualBox().GetSystemProperties().GetMaxInstancesOfStorageBus(chipsetType(), KStorageBus_SATA));
        }
        case R_IsMoreSCSIControllersPossible:
        {
            return (m_configurationAccessLevel == ConfigurationAccessLevel_Full) &&
                   (static_cast<RootItem*>(mRootItem)->childCount(KStorageBus_SCSI) <
                    nemuGlobal().virtualBox().GetSystemProperties().GetMaxInstancesOfStorageBus(chipsetType(), KStorageBus_SCSI));
        }
        case R_IsMoreFloppyControllersPossible:
        {
            return (m_configurationAccessLevel == ConfigurationAccessLevel_Full) &&
                   (static_cast<RootItem*>(mRootItem)->childCount(KStorageBus_Floppy) <
                    nemuGlobal().virtualBox().GetSystemProperties().GetMaxInstancesOfStorageBus(chipsetType(), KStorageBus_Floppy));
        }
        case R_IsMoreSASControllersPossible:
        {
            return (m_configurationAccessLevel == ConfigurationAccessLevel_Full) &&
                   (static_cast<RootItem*>(mRootItem)->childCount(KStorageBus_SAS) <
                    nemuGlobal().virtualBox().GetSystemProperties().GetMaxInstancesOfStorageBus(chipsetType(), KStorageBus_SAS));
        }
        case R_IsMoreUSBControllersPossible:
        {
            return (m_configurationAccessLevel == ConfigurationAccessLevel_Full) &&
                   (static_cast<RootItem*>(mRootItem)->childCount(KStorageBus_USB) <
                    nemuGlobal().virtualBox().GetSystemProperties().GetMaxInstancesOfStorageBus(chipsetType(), KStorageBus_USB));
        }
...
QMap<KStorageBus, int> StorageModel::maximumControllerTypes() const
{
    ...
    for (int iStorageBusType = KStorageBus_IDE; iStorageBusType <= KStorageBus_USB; ++iStorageBusType)
    {
        maximumMap.insert((KStorageBus)iStorageBusType,
                          nemuGlobal().virtualBox().GetSystemProperties().GetMaxInstancesOfStorageBus(chipsetType(), (KStorageBus)iStorageBusType));
    }
    return maximumMap;
}
...
bool UIMachineSettingsStorage::validate(QList<UIValidationMessage> &messages)
{
    ...
    for (int i = 0; i < mStorageModel->rowCount (rootIndex); ++ i)
    {
        ...
        for (int j = 0; j < mStorageModel->rowCount (ctrIndex); ++ j)
        {
            ...
            /* Check for emptiness: */
            if (nemuGlobal().medium(key).isNull() && attDevice == KDeviceType_HardDisk)
            {
                message.second << tr("No hard disk is selected for <i>%1</i>.").arg (value);
                fPass = false;
            }
            /* Check for coincidence: */
            if (!nemuGlobal().medium(key).isNull() && config.contains (key))
            {
...
void UIMachineSettingsStorage::sltChooseExistingMedium()
{
    ...
    QString strMediumId = nemuGlobal().openMediumWithFileOpenDialog(m_pMediumIdHolder->type(), this, strMachineFolder);
    if (!strMediumId.isNull())
        m_pMediumIdHolder->setId(strMediumId);
}
...
void UIMachineSettingsStorage::sltChooseRecentMedium()
{
    ...
    if (pChooseRecentMediumAction)
    {
        ...
        QString strMediumId = nemuGlobal().openMedium(mediumType, strMediumLocation, this);
        if (!strMediumId.isNull())
            m_pMediumIdHolder->setId(strMediumId);
    }
}
    ...
    switch (deviceType)
    {
        case KDeviceType_HardDisk:
        {
            ...
            if (iAnswer == AlertButton_Choice1)
                ...
                strMediumId = nemuGlobal().openMediumWithFileOpenDialog(UIMediumType_HardDisk, this, strMachineFolder);
            break;
        }
        case KDeviceType_DVD:
        {
            ...
            if (iAnswer == AlertButton_Choice1)
                strMediumId = nemuGlobal().medium(strMediumId).id();
            else if (iAnswer == AlertButton_Choice2)
                strMediumId = nemuGlobal().openMediumWithFileOpenDialog(UIMediumType_DVD, this, strMachineFolder);
            break;
        }
        case KDeviceType_Floppy:
        {
            ...
            if (iAnswer == AlertButton_Choice1)
                ...
                strMediumId = nemuGlobal().openMediumWithFileOpenDialog(UIMediumType_Floppy, this, strMachineFolder);
            break;
        }
    }
...
{
    /* Initialize variables: */
    CGuestOSType guestOSType = nemuGlobal().virtualBox().GetGuestOSType(m_strMachineGuestOSTypeId);
...
void UIMachineSettingsStorage::addChooseHostDriveActions(QMenu *pOpenMediumMenu)
{
    ...
    {
        const UIMedium medium = nemuGlobal().medium(strMediumID);
                                                       ...
                                                       const UICacheSettingsMachineStorageAttachment &attachmentCache)
{
    ...
    if (fSuccess)
    {
        ...
        UIMedium nemuMedium = nemuGlobal().medium(strAttachmentMediumId);
        /* Get COM medium object: */
        CMedium comMedium = nemuMedium.medium();
