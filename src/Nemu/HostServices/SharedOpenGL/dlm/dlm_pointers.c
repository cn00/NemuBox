...
/*
 * Prototypes for functions below are auto-generated and definded at out/<os.arch>/<build type>/obj/NemuOGLgen/dlm_generated.h.
 *
 * All non-pointer structure fields are already assifned to *instance in out/<os.arch>/<build type>/obj/NemuOGLgen/dlm_generated.c.
...
#define _NEMU_crdlm_pointers_UniformX(_uniformType) \
...
#define _NEMU_crdlm_pointers_UniformMatrixX(_uniformMatrixType) \
...
{
    _NEMU_crdlm_pointers_UniformMatrixX(GLfloat);
}
...
#undef _NEMU_crdlm_pointers_UniformMatrixX

#if 0
NemuConCreate
NemuCreateContext
NemuPackSetInjectThread
NemuPresentComposition
NemuWindowCreate
#endif

int crdlm_pointers_NemuConCreate(struct instanceNemuConCreate *instance, struct NEMUUHGSMI * pHgsmi)
{
    CRASSERT(0);
    return 0;
}

int crdlm_pointers_NemuCreateContext(struct instanceNemuCreateContext *instance, GLint con, const char * dpyName, GLint visual, GLint shareCtx)
{
...
int crdlm_pointers_NemuPackSetInjectThread(struct instanceNemuPackSetInjectThread *instance, struct NEMUUHGSMI * pHgsmi)
{
    CRASSERT(0);
    return 0;
}

int crdlm_pointers_NemuPresentComposition(struct instanceNemuPresentComposition *instance, GLint win,
    const struct NEMUVR_SCR_COMPOSITOR * pCompositor, const struct NEMUVR_SCR_COMPOSITOR_ENTRY * pChangedEntry)
{
    CRASSERT(0);
    return 0;
}

int crdlm_pointers_NemuWindowCreate(struct instanceNemuWindowCreate *instance, GLint con, const char * dpyName, GLint visBits)
{
    int cbExtraSpace = (dpyName ? crStrlen(dpyName) + 1 : 0);
