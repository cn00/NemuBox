/** @file
 * NemuGuestLib - VirtualBox Guest Additions Library.
 */
...
#ifndef ___Nemu_NemuGuestLib_h
#define ___Nemu_NemuGuestLib_h

#include <Nemu/types.h>
#include <Nemu/VMMDev2.h>
#include <Nemu/VMMDev.h>     /* grumble */
#ifdef IN_RING0
...
/** @defgroup grp_nemuguest_lib     VirtualBox Guest Additions Library
 ...
 *  - NEMU_LIB_VBGL_R0_BASE / NemuGuestR0LibBase for the NemuGuest main driver,
 *    who is responsible for managing the VMMDev virtual hardware.
 *  - NEMU_LIB_VBGL_R0 / NemuGuestR0Lib for other (client) guest drivers.
 *
 *
 * The library source code and the header have a define VBGL_NEMUGUEST, which is
 * defined for NemuGuest and undefined for other drivers. Drivers must choose
 * right library in their makefiles and set VBGL_NEMUGUEST accordingly.
 *
 * The libraries consists of:
 *  - common code to be used by both NemuGuest and other drivers;
 *  - NemuGuest specific code;
 *  - code for other drivers which communicate with NemuGuest via an IOCTL.
 *
 *
 * @section sec_guest_lib_ring3     Ring-3
 *
 * There are more variants of the library here:
 *  - NEMU_LIB_VBGL_R3 / NemuGuestR3Lib for programs.
 *  - NEMU_LIB_VBGL_R3_XFREE86 / NemuGuestR3LibXFree86 for old style XFree
 *    drivers which uses special loader and or symbol resolving strategy.
 *  - NEMU_LIB_VBGL_R3_SHARED / NemuGuestR3LibShared for shared objects / DLLs /
 *    Dylibs.
 *
 */
...
/** HGCM client ID.
 * @todo Promote to Nemu/types.h  */
typedef uint32_t HGCMCLIENTID;


/** @defgroup grp_nemuguest_lib_r0     Ring-0 interface.
 * @{
 */
...
#  define DECLR0VBGL(type) DECLHIDDEN(type) NEMUCALL
# else
#  define DECLR0VBGL(type) type NEMUCALL
# endif
# define DECLVBGL(type) DECLR0VBGL(type)


# ifdef VBGL_NEMUGUEST

/**
 * The library initialization function to be used by the main
 * NemuGuest system driver.
 *
 ...
 * other than the main NemuGuest system driver.
 *
 * @return Nemu status code.
 */
 ...
 * @param   pvData      NemuGuest pointer to be passed to callback.
 * @param   u32Data     NemuGuest 32 bit value to be passed to callback.
 */
 ...
 *                              host returns VINF_HGCM_ASYNC_EXECUTE. NemuGuest
 *                              implements waiting for an IRQ in this function.
 * @param   pvAsyncData         An arbitrary NemuGuest pointer to be passed to callback.
 * @param   u32AsyncData        An arbitrary NemuGuest 32 bit value to be passed to callback.
 *
 * @return  Nemu status code.
 */

DECLR0VBGL(int) VbglR0HGCMInternalConnect(NemuGuestHGCMConnectInfo *pConnectInfo,
 ...
 *                              implements waiting for an IRQ in this function.
 * @param   pvAsyncData         An arbitrary NemuGuest pointer to be passed to callback.
 * @param   u32AsyncData        An arbitrary NemuGuest 32 bit value to be passed to
 *                              callback.
 *
 * @return  Nemu status code.
 */

DECLR0VBGL(int) VbglR0HGCMInternalDisconnect(NemuGuestHGCMDisconnectInfo *pDisconnectInfo,
                                             PFNVBGLHGCMCALLBACK pfnAsyncCallback, void *pvAsyncData, uint32_t u32AsyncData);
 ...
 * @return Nemu status code.
 */
DECLR0VBGL(int) VbglR0HGCMInternalCall(NemuGuestHGCMCallInfo *pCallInfo, uint32_t cbCallInfo, uint32_t fFlags,
                                       PFNVBGLHGCMCALLBACK pfnAsyncCallback, void *pvAsyncData, uint32_t u32AsyncData);
 ...
 * @return  Nemu status code.
 */
DECLR0VBGL(int) VbglR0HGCMInternalCall32(NemuGuestHGCMCallInfo *pCallInfo, uint32_t cbCallInfo, uint32_t fFlags,
                                         PFNVBGLHGCMCALLBACK pfnAsyncCallback, void *pvAsyncData, uint32_t u32AsyncData);
 ...
 * @return Nemu status code.
 */
DECLVBGL(int) VbglHGCMConnect(VBGLHGCMHANDLE *pHandle, NemuGuestHGCMConnectInfo *pData);
 ...
 * @return Nemu status code.
 */
DECLVBGL(int) VbglHGCMDisconnect(VBGLHGCMHANDLE handle, NemuGuestHGCMDisconnectInfo *pData);
 ...
 * @return Nemu status code.
 */
DECLVBGL(int) VbglHGCMCall(VBGLHGCMHANDLE handle, NemuGuestHGCMCallInfo *pData, uint32_t cbData);
 ...
 * @return Nemu status code.
 */
DECLVBGL(int) VbglHGCMCallUserData(VBGLHGCMHANDLE handle, NemuGuestHGCMCallInfo *pData, uint32_t cbData);
 ...
 * @return Nemu status code.
 */
DECLVBGL(int) VbglHGCMCallTimed(VBGLHGCMHANDLE handle, NemuGuestHGCMCallInfoTimed *pData, uint32_t cbData);
/** @} */
...
DECLVBGL(int) VbglR0CrCtlConCall(VBGLCRCTLHANDLE hCtl, struct NemuGuestHGCMCallInfo *pCallInfo, int cbCallInfo);
DECLVBGL(int) VbglR0CrCtlConCallUserData(VBGLCRCTLHANDLE hCtl, struct NemuGuestHGCMCallInfo *pCallInfo, int cbCallInfo);
/** @} */

#  endif /* !VBGL_NEMUGUEST */
...
/** @name Mouse
 * @{ */
DECLVBGL(int)       VbglSetMouseNotifyCallback(PFNNEMUGUESTMOUSENOTIFY pfnNotify, void *pvUser);
...
/** @defgroup grp_nemuguest_lib_r3      Ring-3 interface.
 * @{
 */
#ifdef IN_RING3
 ...
 */
# define VBGLR3DECL(type) type NEMUCALL
...
VBGLR3DECL(int)     VbglR3ReportAdditionsStatus(NemuGuestFacilityType Facility, NemuGuestFacilityStatus StatusCurrent,
                                                uint32_t fFlags);
...
#define VBGLR3HOSTDISPSOCKETPATH    "/tmp/.NemuService"
...
#define VBGLR3HOSTDISPSAVEDMODEPATH "/var/lib/NemuGuestAdditions"
 ...
 * @{ */
VBGLR3DECL(int)     VbglR3GuestUserReportState(const char *pszUser, const char *pszDomain, NemuGuestUserState enmState,
...
/** @}  */
# endif /* NEMU_WITH_SHARED_FOLDERS defined */
