...
/** @file
 * Nemu Console COM Class implementation - Guest drag'n drop target.
 */
...
#include <Nemu/HostServices/Service.h>
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                if (lstFormats.size() == 1) /* Exactly one format to use specified? */
                {
                    ...
                    hr = setError(NEMU_E_IPRT_ERROR, tr("Guest returned invalid drop formats (%zu formats)"), lstFormats.size());
            }
            else
                hr = setError(NEMU_E_IPRT_ERROR, tr("Waiting for response of dropped event failed (%Rrc)"), rc);
        }
        else
            hr = setError(NEMU_E_IPRT_ERROR, tr("Sending dropped event to guest failed (%Rrc)"), rc);
    }
    else
...
DECLCALLBACK(int) GuestDnDTarget::i_sendURIDataCallback(uint32_t uMsg, void *pvParms, size_t cbParms, void *pvUser)
{
    ...
    switch (uMsg)
    {
        ...
        {
            DragAndDropSvc::PNEMUDNDCBHGGETNEXTHOSTMSG pCBData = reinterpret_cast<DragAndDropSvc::PNEMUDNDCBHGGETNEXTHOSTMSG>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(DragAndDropSvc::NEMUDNDCBHGGETNEXTHOSTMSG) == cbParms, VERR_INVALID_PARAMETER);
        ...
        {
            DragAndDropSvc::PNEMUDNDCBHGGETNEXTHOSTMSGDATA pCBData
                = reinterpret_cast<DragAndDropSvc::PNEMUDNDCBHGGETNEXTHOSTMSGDATA>(pvParms);
            AssertPtr(pCBData);
            AssertReturn(sizeof(DragAndDropSvc::NEMUDNDCBHGGETNEXTHOSTMSGDATA) == cbParms, VERR_INVALID_PARAMETER);
...
HRESULT GuestDnDTarget::cancel(BOOL *aVeto)
{
    ...
    HRESULT hr = RT_SUCCESS(rc) ? S_OK : NEMU_E_IPRT_ERROR;
