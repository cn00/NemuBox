 ...
 * NemuSVC daemons requests it. It is also responsible for arbitrating
...
#include "NemuUSBInterface.h"
#include "NemuUSBFilterMgr.h"
#include <Nemu/version.h>
#include <Nemu/usblib-darwin.h>
#include <Nemu/log.h>
...
*********************************************************************************************************************************/
/** Locks the lists. */
#define NEMUUSB_LOCK()      do { int rc = RTSemFastMutexRequest(g_Mtx); AssertRC(rc); } while (0)
/** Unlocks the lists. */
#define NEMUUSB_UNLOCK()    do { int rc = RTSemFastMutexRelease(g_Mtx); AssertRC(rc); } while (0)
...
*********************************************************************************************************************************/
RT_C_DECLS_BEGIN
static kern_return_t    NemuUSBStart(struct kmod_info *pKModInfo, void *pvData);
static kern_return_t    NemuUSBStop(struct kmod_info *pKModInfo, void *pvData);
RT_C_DECLS_END
 ...
 * The service class.
 *
 * This is the management service that NemuSVC and the VMs speak to.
 *
 ...
 */
class org_virtualbox_NemuUSB : public IOService
{
    OSDeclareDefaultStructors(org_virtualbox_NemuUSB);
...
OSDefineMetaClassAndStructors(org_virtualbox_NemuUSB, IOService);


/**
 * The user client class that pairs up with org_virtualbox_NemuUSB.
 */
class org_virtualbox_NemuUSBClient : public IOUserClient
{
    OSDeclareDefaultStructors(org_virtualbox_NemuUSBClient);
...
public:
     ...
     * @{ */
    IOReturn addFilter(PUSBFILTER pFilter, PNEMUUSBADDFILTEROUT pOut, IOByteCount cbFilter, IOByteCount *pcbOut);
...
private:
    ...
    org_virtualbox_NemuUSB *m_pProvider;
    /** The client task. */
    task_t m_Task;
    /** The client process. */
    RTPROCESS m_Process;
    /** Pointer to the next user client. */
    org_virtualbox_NemuUSBClient * volatile m_pNext;
    /** List of user clients. Protected by g_Mtx. */
    static org_virtualbox_NemuUSBClient * volatile s_pHead;
};
OSDefineMetaClassAndStructors(org_virtualbox_NemuUSBClient, IOUserClient);
 ...
 */
class org_virtualbox_NemuUSBDevice : public IOUSBUserClientInit
{
    OSDeclareDefaultStructors(org_virtualbox_NemuUSBDevice);
...
private:
    ...
    /** The owner process, meaning the NemuSVC process. */
    ...
    org_virtualbox_NemuUSBDevice * volatile m_pNext;
    /** Pointer to the list head. Protected by g_Mtx. */
    static org_virtualbox_NemuUSBDevice * volatile s_pHead;
...
OSDefineMetaClassAndStructors(org_virtualbox_NemuUSBDevice, IOUSBUserClientInit);
 ...
 * by org_virtualbox_NemuUSBDevice.
 *
 * @remarks See org_virtualbox_NemuUSBDevice for why we use IOUSBUserClientInit.
 */
class org_virtualbox_NemuUSBInterface : public IOUSBUserClientInit
{
    OSDeclareDefaultStructors(org_virtualbox_NemuUSBInterface);
...
OSDefineMetaClassAndStructors(org_virtualbox_NemuUSBInterface, IOUSBUserClientInit);
...
KMOD_EXPLICIT_DECL(NemuDrv, NEMU_VERSION_STRING, _start, _stop)
DECLHIDDEN(kmod_start_func_t *) _realmain = NemuUSBStart;
DECLHIDDEN(kmod_stop_func_t  *) _antimain = NemuUSBStop;
DECLHIDDEN(int)                 _kext_apple_cc = __APPLE_CC__;
RT_C_DECLS_END

/** Mutex protecting the lists. */
static RTSEMFASTMUTEX g_Mtx = NIL_RTSEMFASTMUTEX;
org_virtualbox_NemuUSBClient * volatile org_virtualbox_NemuUSBClient::s_pHead = NULL;
org_virtualbox_NemuUSBDevice * volatile org_virtualbox_NemuUSBDevice::s_pHead = NULL;
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                uintptr_t uId;
                rc = NemuUSBFilterAdd(&Flt, 1, &uId);
                printf("NemuUSB: NemuUSBFilterAdd #1 -> %d + %p\n", rc, uId);

                USBFilterInit(&Flt, USBFILTERTYPE_CAPTURE);
                USBFilterSetStringPattern(&Flt, USBFILTERIDX_PRODUCT_STR, "*DISK*", true);
                rc = NemuUSBFilterAdd(&Flt, 2, &uId);
                printf("NemuUSB: NemuUSBFilterAdd #2 -> %d + %p\n", rc, uId);
#endif
                return KMOD_RETURN_SUCCESS;
            }
            printf("NemuUSB: NemuUSBFilterInit failed (rc=%d)\n", rc);
            RTSemFastMutexDestroy(g_Mtx);
            g_Mtx = NIL_RTSEMFASTMUTEX;
        }
        else
            printf("NemuUSB: RTSemFastMutexCreate failed (rc=%d)\n", rc);
        RTR0Term();
    }
    else
        printf("NemuUSB: failed to initialize IPRT (rc=%d)\n", rc);
...
{
    int rc;
    Log(("NemuUSBStop: g_cInstances=%d\n", g_cInstances));
 ...
 */
bool
org_virtualbox_NemuUSB::init(OSDictionary *pDictionary)
{
    uint32_t cInstances = ASMAtomicIncU32(&g_cInstances);
    Log(("NemuUSB::init([%p], %p) new g_cInstances=%d\n", this, pDictionary, cInstances));
 ...
 */
bool
org_virtualbox_NemuUSB::start(IOService *pProvider)
{
    Log(("NemuUSB::start([%p], %p {%s})\n", this, pProvider, pProvider->getName()));
 ...
 */
void
org_virtualbox_NemuUSB::stop(IOService *pProvider)
{
    Log(("NemuUSB::stop([%p], %p (%s))\n", this, pProvider, pProvider->getName()));
 ...
 */
bool
org_virtualbox_NemuUSB::open(IOService *pForClient, IOOptionBits fOptions/* = 0*/, void *pvArg/* = 0*/)
{
    Log(("NemuUSB::open([%p], %p, %#x, %p)\n", this, pForClient, fOptions, pvArg));
    bool fRc = IOService::open(pForClient, fOptions, pvArg);
    Log(("NemuUSB::open([%p], %p, %#x, %p) -> %d\n", this, pForClient, fOptions, pvArg, fRc));
    return fRc;
}
 ...
 */
void
org_virtualbox_NemuUSB::close(IOService *pForClient, IOOptionBits fOptions/* = 0*/)
{
    Log(("NemuUSB::close([%p], %p, %#x)\n", this, pForClient, fOptions));
 ...
 */
bool
org_virtualbox_NemuUSB::terminate(IOOptionBits fOptions)
{
    Log(("NemuUSB::terminate([%p], %#x): g_cInstances=%d\n", this, fOptions, g_cInstances));
    bool fRc = IOService::terminate(fOptions);
    Log(("NemuUSB::terminate([%p], %#x): returns %d\n", this, fOptions, fRc));
    return fRc;
}
 ...
 */
bool
org_virtualbox_NemuUSBClient::initWithTask(task_t OwningTask, void *pvSecurityId, UInt32 u32Type)
{
    if (!OwningTask)
    {
        Log(("NemuUSBClient::initWithTask([%p], %p, %p, %#x) -> false (no task)\n", this, OwningTask, pvSecurityId, u32Type));
        return false;
    }
    if (u32Type != NEMUUSB_DARWIN_IOSERVICE_COOKIE)
    {
        Log(("NemuUSBClient::initWithTask: Bade cookie %#x\n", u32Type));
        return false;
    }

    proc_t pProc = (proc_t)get_bsdtask_info(OwningTask); /* we need the pid */
    Log(("NemuUSBClient::initWithTask([%p], %p(->%p:{.pid=%d}, %p, %#x)\n",
    ...
    if (IOUserClient::initWithTask(OwningTask, pvSecurityId , u32Type))
    {
        ...
        uint32_t cInstances = ASMAtomicIncU32(&g_cInstances);
        Log(("NemuUSBClient::initWithTask([%p], %p(->%p:{.pid=%d}, %p, %#x) -> true; new g_cInstances=%d\n",
                 this, OwningTask, pProc, pProc ? proc_pid(pProc) : -1, pvSecurityId, u32Type, cInstances));
        return true;
    }

    Log(("NemuUSBClient::initWithTask([%p], %p(->%p:{.pid=%d}, %p, %#x) -> false\n",
             this, OwningTask, pProc, pProc ? proc_pid(pProc) : -1, pvSecurityId, u32Type));
    return false;
}
 ...
 */
void
org_virtualbox_NemuUSBClient::free()
{
    uint32_t cInstances = ASMAtomicDecU32(&g_cInstances); NOREF(cInstances);
    Log(("NemuUSBClient::free([%p]) new g_cInstances=%d\n", this, cInstances));
    IOUserClient::free();
}
 ...
 */
bool
org_virtualbox_NemuUSBClient::start(IOService *pProvider)
{
    Log(("NemuUSBClient::start([%p], %p)\n", this, pProvider));
    if (IOUserClient::start(pProvider))
    {
        m_pProvider = OSDynamicCast(org_virtualbox_NemuUSB, pProvider);
        if (m_pProvider)
        {
            /*
        ...
        Log(("NemuUSBClient::start: %p isn't org_virtualbox_NemuUSB\n", pProvider));
    }
    return false;
}
 ...
 */
IOReturn
org_virtualbox_NemuUSBClient::clientClose(void)
{
    Log(("NemuUSBClient::clientClose([%p:{.m_Process=%d}])\n", this, (int)m_Process));
    ...
    org_virtualbox_NemuUSBClient *pPrev = NULL;
    for (org_virtualbox_NemuUSBClient *pCur = s_pHead; pCur; pCur = pCur->m_pNext)
    {
        if (pCur == this)
        {
            if (pPrev)
    ...
    if (m_Process != NIL_RTPROCESS)
        NemuUSBFilterRemoveOwner(m_Process);
     ...
     */
    if (m_Process != NIL_RTPROCESS)
        org_virtualbox_NemuUSBDevice::scheduleReleaseByOwner(m_Process);
 ...
 */
IOReturn
org_virtualbox_NemuUSBClient::clientDied(void)
{
    Log(("NemuUSBClient::clientDied([%p]) m_Task=%p R0Process=%p Process=%d\n",
 ...
 */
bool
org_virtualbox_NemuUSBClient::finalize(IOOptionBits fOptions)
{
    Log(("NemuUSBClient::finalize([%p], %#x)\n", this, fOptions));
 ...
 */
void
org_virtualbox_NemuUSBClient::stop(IOService *pProvider)
{
    Log(("NemuUSBClient::stop([%p])\n", this));
 ...
 */
IOExternalMethod *
org_virtualbox_NemuUSBClient::getTargetAndMethodForIndex(IOService **ppService, UInt32 iMethod)
{
    static IOExternalMethod s_aMethods[NEMUUSBMETHOD_END] =
    {
        /*[NEMUUSBMETHOD_ADD_FILTER] = */
        {
            (IOService *)0,                                         /* object */
            (IOMethod)&org_virtualbox_NemuUSBClient::addFilter,     /* func */
            kIOUCStructIStructO,                                    /* flags - struct input (count0) and struct output (count1) */
            sizeof(USBFILTER),                                      /* count0 - size of the input struct. */
            sizeof(NEMUUSBADDFILTEROUT)                             /* count1 - size of the return struct. */
        },
        /* [NEMUUSBMETHOD_FILTER_REMOVE] = */
        {
            (IOService *)0,                                         /* object */
            (IOMethod)&org_virtualbox_NemuUSBClient::removeFilter,  /* func */
            kIOUCStructIStructO,                                    /* flags - struct input (count0) and struct output (count1) */
 ...
 */
IOReturn
org_virtualbox_NemuUSBClient::addFilter(PUSBFILTER pFilter, PNEMUUSBADDFILTEROUT pOut, IOByteCount cbFilter, IOByteCount *pcbOut)
{
    Log(("NemuUSBClient::addFilter: [%p:{.m_Process=%d}] pFilter=%p pOut=%p\n", this, (int)m_Process, pFilter, pOut));
    ...
    if (RT_UNLIKELY(    cbFilter != sizeof(*pFilter)
                    ||  *pcbOut != sizeof(*pOut)))
    {
        printf("NemuUSBClient::addFilter: cbFilter=%#x expected %#x; *pcbOut=%#x expected %#x\n",
     ...
     */
#ifdef DEBUG
    Log2(("NemuUSBClient::addFilter: idVendor=%#x idProduct=%#x bcdDevice=%#x bDeviceClass=%#x bDeviceSubClass=%#x bDeviceProtocol=%#x bBus=%#x bPort=%#x\n",
    ...
    Log2(("NemuUSBClient::addFilter: Manufacturer=%s Product=%s Serial=%s\n",
     ...
     */
    pOut->uId = 0;
    pOut->rc = NemuUSBFilterAdd(pFilter, m_Process, &pOut->uId);

    Log(("NemuUSBClient::addFilter: returns *pOut={.rc=%d, .uId=%p}\n", pOut->rc, (void *)pOut->uId));
    return kIOReturnSuccess;
}
 ...
 */
IOReturn
org_virtualbox_NemuUSBClient::removeFilter(uintptr_t *puId, int *prc, IOByteCount cbIn, IOByteCount *pcbOut)
{
    Log(("NemuUSBClient::removeFilter: [%p:{.m_Process=%d}] *puId=%p m_Proc\n", this, (int)m_Process, *puId));
    ...
    if (RT_UNLIKELY(    cbIn != sizeof(*puId)
                    ||  *pcbOut != sizeof(*prc)))
    {
        printf("NemuUSBClient::removeFilter: cbIn=%#x expected %#x; *pcbOut=%#x expected %#x\n",
     ...
     * Remove the filter.
     */
    *prc = NemuUSBFilterRemove(m_Process, *puId);

    Log(("NemuUSBClient::removeFilter: returns *prc=%d\n", *prc));
    return kIOReturnSuccess;
}


/**
 * Checks whether the specified task is a NemuUSB client task or not.
 *
 ...
 */
/* static*/ bool
org_virtualbox_NemuUSBClient::isClientTask(task_t ClientTask)
{
    NEMUUSB_LOCK();
 ...
 */
bool
org_virtualbox_NemuUSBDevice::init(OSDictionary *pDictionary)
{
    uint32_t cInstances = ASMAtomicIncU32(&g_cInstances);
    Log(("NemuUSBDevice::init([%p], %p) new g_cInstances=%d\n", this, pDictionary, cInstances));
 ...
 */
void
org_virtualbox_NemuUSBDevice::free()
{
    uint32_t cInstances = ASMAtomicDecU32(&g_cInstances); NOREF(cInstances);
    Log(("NemuUSBDevice::free([%p]) new g_cInstances=%d\n", this, cInstances));
 ...
 */
IOService *
org_virtualbox_NemuUSBDevice::probe(IOService *pProvider, SInt32 *pi32Score)
{
    Log(("NemuUSBDevice::probe([%p], %p {%s}, %p={%d})\n", this,
     ...
     */
    uintptr_t uId = 0;
    RTPROCESS Owner = NemuUSBFilterMatch(&Device, &uId);
    USBFilterDelete(&Device);
    if (Owner == NIL_RTPROCESS)
    {
        Log(("NemuUSBDevice::probe: returns NULL uId=%d\n", uId));
        return NULL;
    }
    ...
    *pi32Score = _1G;
    Log(("NemuUSBDevice::probe: returns %p and *pi32Score=%d\n", pRet, *pi32Score));
    return pRet;
}
 ...
 */
bool
org_virtualbox_NemuUSBDevice::start(IOService *pProvider)
{
    Log(("NemuUSBDevice::start([%p:{.m_Owner=%d, .m_uId=%p}], %p {%s})\n",
             this, m_Owner, m_uId, pProvider, pProvider->getName()));

    m_pDevice = OSDynamicCast(IOUSBDevice, pProvider);
    if (!m_pDevice)
    {
        printf("NemuUSBDevice::start([%p], %p {%s}): failed!\n", this, pProvider, pProvider->getName());
        return false;
    }
    ...
    m_pNotifier = pProvider->registerInterest(gIOGeneralInterest,
                                              &org_virtualbox_NemuUSBDevice::MyInterestHandler,
                                              this,     /* pvTarget */
                                              NULL);    /* pvRefCon */
#endif
    ...
    /*
     * Set the NemuUSB properties.
     */
    if (!setProperty(NEMUUSB_OWNER_KEY, (unsigned long long)m_Owner, sizeof(m_Owner) * 8 /* bits */))
        Log(("NemuUSBDevice::start: failed to set the '" NEMUUSB_OWNER_KEY "' property!\n"));
    if (!setProperty(NEMUUSB_CLIENT_KEY, (unsigned long long)m_Client, sizeof(m_Client) * 8 /* bits */))
        Log(("NemuUSBDevice::start: failed to set the '" NEMUUSB_CLIENT_KEY "' property!\n"));
    if (!setProperty(NEMUUSB_FILTER_KEY, (unsigned long long)m_uId, sizeof(m_uId) * 8 /* bits */))
        Log(("NemuUSBDevice::start: failed to set the '" NEMUUSB_FILTER_KEY "' property!\n"));
    ...
    if (!m_fOpen)
        Log(("NemuUSBDevice::start: failed to open the device!\n"));
    m_fOpenOnWasClosed = !m_fOpen;

    Log(("NemuUSBDevice::start: returns %d\n", true));
    return true;
}
 ...
 */
void
org_virtualbox_NemuUSBDevice::stop(IOService *pProvider)
{
    Log(("NemuUSBDevice::stop([%p], %p {%s})\n", this, pProvider, pProvider->getName()));
    ...
    org_virtualbox_NemuUSBDevice *pPrev = NULL;
    for (org_virtualbox_NemuUSBDevice *pCur = s_pHead; pCur; pCur = pCur->m_pNext)
    {
        if (pCur == this)
        {
            if (pPrev)
    ...
    if (m_fBeingUnloaded)
    {
        if (m_pDevice)
        {
            ...
            Log(("NemuUSBDevice::stop([%p], %p {%s}): m_pDevice=%p unload & ReEnumerateDevice -> %#x\n",
        ...
        else
        {
            ...
            if (pDevice)
            {
                ...
                Log(("NemuUSBDevice::stop([%p], %p {%s}): pDevice=%p unload & ReEnumerateDevice -> %#x\n",
                         this, pProvider, pProvider->getName(), pDevice, irc));
            }
            else
                Log(("NemuUSBDevice::stop([%p], %p {%s}): failed to cast provider to IOUSBDevice\n",
                         this, pProvider, pProvider->getName()));
        }
    }
    else if (m_fReleaseOnClose)
    {
        ...
        {
            IOReturn irc = m_pDevice->ReEnumerateDevice(0); NOREF(irc);
            Log(("NemuUSBDevice::stop([%p], %p {%s}): m_pDevice=%p close & ReEnumerateDevice -> %#x\n",
                     this, pProvider, pProvider->getName(), m_pDevice, irc));
        }
    }
    ...
    Log(("NemuUSBDevice::stop: returns void\n"));
}
 ...
 */
bool
org_virtualbox_NemuUSBDevice::terminate(IOOptionBits fOptions)
{
    /* kIOServiceRecursing, kIOServiceRequired, kIOServiceTerminate, kIOServiceSynchronous - interesting option bits */
    Log(("NemuUSBDevice::terminate([%p], %#x)\n", this, fOptions));
 ...
 */
IOReturn
org_virtualbox_NemuUSBDevice::message(UInt32 enmMsg, IOService *pProvider, void *pvArg)
{
    Log(("NemuUSBDevice::message([%p], %#x {%s}, %p {%s}, %p) - pid=%d\n",
    ...
    switch (enmMsg)
    {
        ...
        case kIOMessageServiceIsRequestingClose:
            ...
            {
                Log(("NemuUSBDevice::message([%p],%p {%s}, %p) - pid=%d: not seize - closing...\n",
            ...
            {
                if (org_virtualbox_NemuUSBClient::isClientTask(current_task()))
                {
                    Log(("NemuUSBDevice::message([%p],%p {%s}, %p) - pid=%d task=%p: client process, closing.\n",
                    ...
                    Log(("NemuUSBDevice::message([%p],%p {%s}, %p) - pid=%d task=%p: not client process!\n",
                             this, pProvider, pProvider->getName(), pvArg, RTProcSelf(), current_task()));
            }
            if (!setProperty(NEMUUSB_CLIENT_KEY, (unsigned long long)m_Client, sizeof(m_Client) * 8 /* bits */))
                Log(("NemuUSBDevice::message: failed to set the '" NEMUUSB_CLIENT_KEY "' property!\n"));
            break;
        ...
        case kIOMessageServiceWasClosed:
            ...
            if (m_pDevice)
            {
                ...
                if (ASMAtomicXchgBool(&m_fReleaseOnClose, false))
                {
                    ...
                    Log(("NemuUSBDevice::message([%p], %p {%s}) - ReEnumerateDevice() -> %#x\n",
                ...
                {
                    Log(("NemuUSBDevice::message: attempting to re-open the device...\n"));
                    m_fOpenOnWasClosed = false;
                    m_fOpen = m_pDevice->open(this, kIOServiceSeize, 0);
                    if (!m_fOpen)
                        Log(("NemuUSBDevice::message: failed to open the device!\n"));
                    m_fOpenOnWasClosed = !m_fOpen;
                }
            }
        ...
        case kIOMessageServiceIsTerminated:
            ...
            if (m_pDevice)
            {
                ...
                if (m_fOpen)
                {
                    m_fOpen = false;
                    m_fOpenOnWasClosed = false;
                    Log(("NemuUSBDevice::message: closing the device (%p)...\n", m_pDevice));
                    m_pDevice->close(this, 0);
                }

                /* release it (see start()) */
                Log(("NemuUSBDevice::message: releasing the device (%p)...\n", m_pDevice));
                m_pDevice->release();
                m_pDevice = NULL;
            }
    ...
    Log(("NemuUSBDevice::message([%p], %#x {%s}, %p {%s}, %p) -> %#x\n",
 ...
 */
/* static */ void
org_virtualbox_NemuUSBDevice::scheduleReleaseByOwner(RTPROCESS Owner)
{
    Log2(("NemuUSBDevice::scheduleReleaseByOwner: Owner=%d\n", Owner));
    ...
    org_virtualbox_NemuUSBDevice *pCur;
    do
    {
        for (pCur = s_pHead; pCur; pCur = pCur->m_pNext)
        {
            Log2(("NemuUSBDevice::scheduleReleaseByOwner: pCur=%p m_Owner=%d (%s) m_fReleaseOnClose=%d\n",
            ...
            if (pCur->m_Owner == Owner)
            {
                    ...
                    &&  !pCur->m_fReleaseOnClose)
                {
                    ...
                    if (pCur->m_Client != NIL_RTPROCESS)
                    {
                        ...
                        Log(("NemuUSBDevice::scheduleReleaseByOwner: %p {%s} - used by %d\n",
                    ...
                    else
                    {
                        ...
                        IOReturn irc = pDevice->ReEnumerateDevice(0); NOREF(irc);
                        Log(("NemuUSBDevice::scheduleReleaseByOwner: %p {%s} - ReEnumerateDevice -> %#x\n",
...
#ifdef DEBUG
/*static*/ IOReturn
org_virtualbox_NemuUSBDevice::MyInterestHandler(void *pvTarget, void *pvRefCon, UInt32 enmMsgType,
                                                IOService *pProvider, void * pvMsgArg, vm_size_t cbMsgArg)
{
    org_virtualbox_NemuUSBDevice *pThis = (org_virtualbox_NemuUSBDevice *)pvTarget;
    if (!pThis)
    ...
    switch (enmMsgType)
    {
        case kIOMessageServiceIsAttemptingOpen:
            ...
            Log(("NemuUSBDevice::MyInterestHandler: kIOMessageServiceIsAttemptingOpen - pvRefCon=%p pProvider=%p pvMsgArg=%p cbMsgArg=%d\n",
                     pvRefCon, pProvider, pvMsgArg, cbMsgArg));
            break;

        case kIOMessageServiceWasClosed:
            Log(("NemuUSBDevice::MyInterestHandler: kIOMessageServiceWasClosed - pvRefCon=%p pProvider=%p pvMsgArg=%p cbMsgArg=%d\n",
                     pvRefCon, pProvider, pvMsgArg, cbMsgArg));
            break;

        case kIOMessageServiceIsTerminated:
            Log(("NemuUSBDevice::MyInterestHandler: kIOMessageServiceIsTerminated - pvRefCon=%p pProvider=%p pvMsgArg=%p cbMsgArg=%d\n",
                     pvRefCon, pProvider, pvMsgArg, cbMsgArg));
            break;

        case kIOUSBMessagePortHasBeenReset:
            Log(("NemuUSBDevice::MyInterestHandler: kIOUSBMessagePortHasBeenReset - pvRefCon=%p pProvider=%p pvMsgArg=%p cbMsgArg=%d\n",
                     pvRefCon, pProvider, pvMsgArg, cbMsgArg));
            break;

        default:
            Log(("NemuUSBDevice::MyInterestHandler: %#x (%s) - pvRefCon=%p pProvider=%p pvMsgArg=%p cbMsgArg=%d\n",
...
/*
 *
 * org_virtualbox_NemuUSBInterface
 *
 */
 ...
 */
bool
org_virtualbox_NemuUSBInterface::init(OSDictionary *pDictionary)
{
    uint32_t cInstances = ASMAtomicIncU32(&g_cInstances);
    Log(("NemuUSBInterface::init([%p], %p) new g_cInstances=%d\n", this, pDictionary, cInstances));
 ...
 */
void
org_virtualbox_NemuUSBInterface::free()
{
    uint32_t cInstances = ASMAtomicDecU32(&g_cInstances); NOREF(cInstances);
    Log(("NemuUSBInterfaces::free([%p]) new g_cInstances=%d\n", this, cInstances));
    IOUSBUserClientInit::free();
}
 ...
 *
 * We implement this similarly to org_virtualbox_NemuUSBDevice, except that
 * we don't bother matching filters but instead just check if the parent is
 * handled by org_virtualbox_NemuUSBDevice or not.
 */
IOService *
org_virtualbox_NemuUSBInterface::probe(IOService *pProvider, SInt32 *pi32Score)
{
    Log(("NemuUSBInterface::probe([%p], %p {%s}, %p={%d})\n", this,
             pProvider, pProvider->getName(), pi32Score, pi32Score ? *pi32Score : 0));

    /*
     * Check if NemuUSBDevice is the parent's driver.
     */
    ...
    if (pParent)
    {
        Log(("NemuUSBInterface::probe: pParent=%p {%s}\n", pParent, pParent->getName()));
        ...
        if (pSiblings)
        {
            ...
            while ( (pSibling = OSDynamicCast(IORegistryEntry, pSiblings->getNextObject())) )
            {
                ...
                if (pMetaClass == &org_virtualbox_NemuUSBDevice::gMetaClass)
                {
                    fHijackIt = true;
                    break;
                }
    ...
    if (!fHijackIt)
    {
        Log(("NemuUSBInterface::probe: returns NULL\n"));
        return NULL;
    }
    ...
    pRet = this;
    *pi32Score = _1G;
    Log(("NemuUSBInterface::probe: returns %p and *pi32Score=%d - hijack it.\n", pRet, *pi32Score));
    return pRet;
}
 ...
 */
bool
org_virtualbox_NemuUSBInterface::start(IOService *pProvider)
{
    Log(("NemuUSBInterface::start([%p], %p {%s})\n", this, pProvider, pProvider->getName()));
    ...
    if (m_pInterface)
    {
        ...
        if (!m_fOpen)
            Log(("NemuUSBInterface::start: failed to open the interface!\n"));
        m_fOpenOnWasClosed = !m_fOpen;
    }
    else
    {
        printf("NemuUSBInterface::start([%p], %p {%s}): failed!\n", this, pProvider, pProvider->getName());
        fRc = false;
    }

    Log(("NemuUSBInterface::start: returns %d\n", fRc));
    return fRc;
}
 ...
 */
void
org_virtualbox_NemuUSBInterface::stop(IOService *pProvider)
{
    Log(("org_virtualbox_NemuUSBInterface::stop([%p], %p {%s})\n", this, pProvider, pProvider->getName()));
    ...
    IOUSBUserClientInit::stop(pProvider);
    Log(("NemuUSBInterface::stop: returns void\n"));
}
 ...
 */
bool
org_virtualbox_NemuUSBInterface::terminate(IOOptionBits fOptions)
{
    /* kIOServiceRecursing, kIOServiceRequired, kIOServiceTerminate, kIOServiceSynchronous - interesting option bits */
    Log(("NemuUSBInterface::terminate([%p], %#x)\n", this, fOptions));
    return IOUSBUserClientInit::terminate(fOptions);
}


/**
 * @copydoc org_virtualbox_NemuUSBDevice::message
 */
IOReturn
org_virtualbox_NemuUSBInterface::message(UInt32 enmMsg, IOService *pProvider, void *pvArg)
{
    Log(("NemuUSBInterface::message([%p], %#x {%s}, %p {%s}, %p)\n",
    ...
    {
        /*
         * See explanation in org_virtualbox_NemuUSBDevice::message.
         */
        case kIOMessageServiceIsRequestingClose:
            irc = kIOReturnExclusiveAccess;
            if (!((uintptr_t)pvArg & kIOServiceSeize))
            {
                Log(("NemuUSBInterface::message([%p],%p {%s}, %p) - pid=%d: not seize - closing...\n",
                         this, pProvider, pProvider->getName(), pvArg, RTProcSelf()));
                m_fOpen = false;
                m_fOpenOnWasClosed = false;
                if (m_pInterface)
            ...
            else
            {
                if (org_virtualbox_NemuUSBClient::isClientTask(current_task()))
                {
                    Log(("NemuUSBInterface::message([%p],%p {%s}, %p) - pid=%d task=%p: client process, closing.\n",
                             this, pProvider, pProvider->getName(), pvArg, RTProcSelf(), current_task()));
                    m_fOpen = false;
                    m_fOpenOnWasClosed = false;
                    if (m_pInterface)
                    ...
                    Log(("NemuUSBInterface::message([%p],%p {%s}, %p) - pid=%d task=%p: not client process!\n",
                             this, pProvider, pProvider->getName(), pvArg, RTProcSelf(), current_task()));
            }
            break;
        ...
        case kIOMessageServiceWasClosed:
            ...
            {
                Log(("NemuUSBInterface::message: attempting to re-open the interface...\n"));
                m_fOpenOnWasClosed = false;
                m_fOpen = m_pInterface->open(this, kIOServiceSeize, 0);
                if (!m_fOpen)
                    Log(("NemuUSBInterface::message: failed to open the interface!\n"));
                m_fOpenOnWasClosed = !m_fOpen;
            }
    ...
    Log(("NemuUSBInterface::message([%p], %#x {%s}, %p {%s}, %p) -> %#x\n",
             this, enmMsg, DbgGetIOKitMessageName(enmMsg), pProvider, pProvider->getName(), pvArg, irc));
    return irc;
}
