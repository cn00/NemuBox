/* $Id: SUPDrvGip.cpp $ */
/** @file
 * NemuDrv - The VirtualBox Support Driver - Common code for GIP.
 */
...
static DECLCALLBACK(void) supdrvInitRefineInvariantTscFreqTimer(PRTTIMER pTimer, void *pvUser, uint64_t iTick)
{
        ...
        && pGip->enmUseTscDelta > SUPGIPUSETSCDELTA_ZERO_CLAIMED)
    {
        ...
        {
            SUPR0Printf("nemudrv: Failed to refine invariant TSC frequency because deltas are unavailable after %u (%u) seconds\n",
                        (uint32_t)(cNsElapsed / RT_NS_1SEC), GIP_TSC_REFINE_PERIOD_IN_SECS);
            SUPR0Printf("nemudrv: start: %u, %u, %#llx  stop: %u, %u, %#llx\n",
...
static void supdrvGipInitStartTimerForRefiningInvariantTscFreq(PSUPDRVDEVEXT pDevExt, PSUPGLOBALINFOPAGE pGip)
{
    ...
    OSDBGPRINT(("nemudrv: Failed to create or start TSC frequency refinement timer: rc=%Rrc\n", rc));
}
...
/** @todo This section of code is never reached atm, consider dropping it later on... */
                if (pGip->enmUseTscDelta > SUPGIPUSETSCDELTA_ZERO_CLAIMED)
                {
                    ...
                    {
                        SUPR0Printf("nemudrv: Unexpected CPU index in supdrvGipInitMeasureTscFreq.\n");
                        SUPR0Printf("nemudrv: start: %u, %u, %#llx  stop: %u, %u, %#llx\n",
                                    iStartCpuSet, iStartGipCpu, iStartTscDelta, iStopCpuSet, iStopGipCpu, iStopTscDelta);
...
static bool supdrvGipInitDetermineAsyncTsc(uint64_t *poffMin)
{
    ...
    OSDBGPRINT(("nemudrv: fAsync=%d offMin=%#lx offMax=%#lx\n", fAsync, (long)offMin, (long)offMax));
#endif
    return fAsync;
}
...
int NEMUCALL supdrvGipCreate(PSUPDRVDEVEXT pDevExt)
{
    ...
    {
        SUPR0Printf("NemuDrv: Too many CPUs (%u) for the GIP (max %u)\n", cCpus, RT_MIN(RTCPUSET_MAX_CPUS, 256));
 ...
 * @{
 */
#if defined(DEBUG_bird) /* || defined(NEMU_STRICT) */
...
static int supdrvTscDeltaThreadInit(PSUPDRVDEVEXT pDevExt)
{
    ...
    rc = RTSpinlockCreate(&pDevExt->hTscDeltaSpinlock, RTSPINLOCK_FLAGS_INTERRUPT_UNSAFE, "NemuTscSpnLck");
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            rc = RTThreadCreate(&pDevExt->hTscDeltaThread, supdrvTscDeltaThread, pDevExt, 0 /* cbStack */,
                                RTTHREADTYPE_DEFAULT, RTTHREADFLAGS_WAITABLE, "NemuTscThread");
            if (RT_SUCCESS(rc))
            {
