...
/** @file
 * Nemu Qt GUI - UIMouseHandler class implementation.
 */
...
void UIMouseHandler::captureMouse(ulong uScreenId)
{
    ...
    if (m_viewports.contains(uScreenId))
    {
        ...
        visibleRectangle = visibleRectangle.intersected(nemuGlobal().availableGeometry(machineLogic()->machineWindows()[m_iMouseCaptureViewIndex]));
...
void UIMouseHandler::updateMouseCursorClipping()
{
    ...
    if (uisession()->isMouseCaptured())
    {
        ...
        viewportRectangle = viewportRectangle.intersected(nemuGlobal().availableGeometry(machineLogic()->machineWindows()[m_iMouseCaptureViewIndex]));
