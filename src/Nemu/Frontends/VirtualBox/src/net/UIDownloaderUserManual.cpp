...
/** @file
 * Nemu Qt GUI - UIDownloaderUserManual class implementation.
 */
...
UIDownloaderUserManual::UIDownloaderUserManual()
{
    ...
    QString strUserManualFullFileName = nemuGlobal().helpFile();
    QString strUserManualShortFileName = QFileInfo(strUserManualFullFileName).fileName();

    /* Add sources: */
    addSource(QString("http://download.virtualbox.org/virtualbox/%1/").arg(nemuGlobal().nemuVersionStringNormalized()) + strUserManualShortFileName);
    addSource(QString("http://download.virtualbox.org/virtualbox/") + strUserManualShortFileName);

    /* Set target: */
    QString strUserManualDestination = QDir(nemuGlobal().homeFolder()).absoluteFilePath(strUserManualShortFileName);
