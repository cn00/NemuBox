 ...
 * "nemufs_prov.h" and the module implementing them is shipped as part of the
 ...
 *
 * We suppport only read-only mmap (NEMUVFS_WITH_MMAP) i.e. MAP_SHARED,
...
#include <vm/pvn.h>
#if !defined(NEMU_VFS_SOLARIS_10U6)
...
sffs_purge(struct sffs_data *sffs)
{
	...
	for (prev = NULL;;) {
		...
		if (node->sf_sffs == sffs) {
			if (node->sf_vnode != NULL)
				panic("nemufs: purge hit active vnode");
			sfnode_make_stale(node);
		} else {
			prev = node;
		}
