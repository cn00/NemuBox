...
GLboolean renderspu_SystemNemuCreateWindow(VisualInfo *pVisInfo, GLboolean fShowIt, WindowInfo *pWinInfo)
{
    CRASSERT(pVisInfo);
    CRASSERT(pWinInfo);
...
void renderspu_SystemNemuPresentComposition( WindowInfo *window, const struct NEMUVR_SCR_COMPOSITOR_ENTRY *pChangedEntry )
{
...
static NemuGLhandleARB crHndlSearchNemu(GLhandleARB hNative)
{
    CRASSERT(!(((uintptr_t)hNative) >> 32));
    return (NemuGLhandleARB)((uintptr_t)hNative);
}

static GLhandleARB crHndlSearchNative(NemuGLhandleARB hNemu)
{
    return (GLhandleARB)((uintptr_t)hNemu);
}

static NemuGLhandleARB crHndlAcquireNemu(GLhandleARB hNative)
{
    CRASSERT(!(((uintptr_t)hNative) >> 32));
    return (NemuGLhandleARB)((uintptr_t)hNative);
}

static GLhandleARB crHndlReleaseNemu(NemuGLhandleARB hNemu)
{
    return (GLhandleARB)((uintptr_t)hNemu);
}

static void SPU_APIENTRY renderspu_SystemDeleteObjectARB(NemuGLhandleARB obj)
{
    GLhandleARB hNative = crHndlReleaseNemu(obj);
    if (!hNative)
    {
...
static void SPU_APIENTRY renderspu_SystemGetAttachedObjectsARB( NemuGLhandleARB containerObj, GLsizei maxCount, GLsizei * pCount, NemuGLhandleARB * obj )
{
    ...
    for (i = 0; i < count; ++i)
    {
        obj[i] = crHndlSearchNemu(paAttachments[i]);
        CRASSERT(obj[i]);
    }
...
static NemuGLhandleARB SPU_APIENTRY renderspu_SystemGetHandleARB(GLenum pname)
{
    GLhandleARB hNative = render_spu.pfnGetHandle(pname);
    NemuGLhandleARB hNemu;
    if (!hNative)
    {
        crWarning("pfnGetHandle failed");
        return 0;
    }
    hNemu = crHndlAcquireNemu(hNative);
    CRASSERT(hNemu);
    return hNemu;
}

static void SPU_APIENTRY renderspu_SystemGetInfoLogARB( NemuGLhandleARB obj, GLsizei maxLength, GLsizei * length, GLcharARB * infoLog )
{
...
static void SPU_APIENTRY renderspu_SystemGetObjectParameterfvARB( NemuGLhandleARB obj, GLenum pname, GLfloat * params )
{
    GLhandleARB hNative = crHndlSearchNative(obj);
    if (!hNative)
    {
...
static void SPU_APIENTRY renderspu_SystemGetObjectParameterivARB( NemuGLhandleARB obj, GLenum pname, GLint * params )
{
    GLhandleARB hNative = crHndlSearchNative(obj);
    if (!hNative)
    {
