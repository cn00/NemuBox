                                  ...
                                  const xml::ElementNode *pSectionElem)
{
    ...
    while ((pelmDisk = loopDisks.forAllNodes()))
    {
        ...
        else
        {
            ...
            // optional nemu:uuid attribute (if OVF was exported by VirtualBox != 3.2)
            pelmDisk->getAttributeValue("uuid", d.uuidNemu, "nemu");
...
void OVFReader::HandleVirtualSystemContent(const xml::ElementNode *pelmVirtualSystem)
{
    ...
    // peek under the <VirtualSystem> node whether we have a <nemu:Machine> node;
    // that case case, the caller can completely ignore the OVF but only load the Nemu machine XML
    vsys.pelmNemuMachine = pelmVirtualSystem->findChildElementNS("nemu", "Machine");
    ...
    while ((pelmThis = loop.forAllNodes()))
    {
                ...
                )
        {
            ...
            const xml::ElementNode *pelmNemuOSType;
            if ((pelmNemuOSType = pelmThis->findChildElementNS("nemu",            // namespace
                                                               "OSType")))        // element name
                vsys.strTypeNemu = pelmNemuOSType->getValue();
        }
