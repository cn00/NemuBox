...
/** @file
 * Nemu Qt GUI - UIMediumEnumerator class implementation.
 */
    ...
    : m_fMediumEnumerationInProgress(false)
{
    ...
    connect(gNemuEvents, SIGNAL(sigMachineDataChange(QString)), this, SLOT(sltHandleMachineUpdate(QString)));
    connect(gNemuEvents, SIGNAL(sigSnapshotTake(QString, QString)), this, SLOT(sltHandleMachineUpdate(QString)));
    connect(gNemuEvents, SIGNAL(sigSnapshotDelete(QString, QString)), this, SLOT(sltHandleSnapshotDeleted(QString, QString)));
    connect(gNemuEvents, SIGNAL(sigSnapshotChange(QString, QString)), this, SLOT(sltHandleMachineUpdate(QString)));
    connect(gNemuEvents, SIGNAL(sigSnapshotRestore(QString, QString)), this, SLOT(sltHandleSnapshotDeleted(QString, QString)));
    connect(gNemuEvents, SIGNAL(sigMachineRegistered(QString, bool)), this, SLOT(sltHandleMachineRegistration(QString, bool)));

    /* Listen for global thread-pool: */
    connect(nemuGlobal().threadPool(), SIGNAL(sigTaskComplete(UITask*)), this, SLOT(sltHandleMediumEnumerationTaskComplete(UITask*)));
}
...
void UIMediumEnumerator::enumerateMediums()
{
    ...
    addHardDisksToMap(nemuGlobal().virtualBox().GetHardDisks(), mediums);
    addMediumsToMap(nemuGlobal().host().GetDVDDrives(), mediums, UIMediumType_DVD);
    addMediumsToMap(nemuGlobal().virtualBox().GetDVDImages(), mediums, UIMediumType_DVD);
    addMediumsToMap(nemuGlobal().host().GetFloppyDrives(), mediums, UIMediumType_Floppy);
    addMediumsToMap(nemuGlobal().virtualBox().GetFloppyImages(), mediums, UIMediumType_Floppy);
    if (nemuGlobal().isCleaningUp())
        return; /* NemuGlobal is cleaning up, abort immediately. */
    m_mediums = mediums;
...
void UIMediumEnumerator::createMediumEnumerationTask(const UIMedium &medium)
{
    ...
    nemuGlobal().threadPool()->enqueueTask(pTask);
}
...
void UIMediumEnumerator::addMediumsToMap(const CMediumVector &inputMediums, UIMediumMap &outputMediums, UIMediumType mediumType)
{
    ...
    {
        /* If NemuGlobal is cleaning up, abort immediately: */
        if (nemuGlobal().isCleaningUp())
            break;
...
void UIMediumEnumerator::calculateActualUsage(const QString &strMachineID, CMediumMap &currentCMediums, QStringList &currentCMediumIDs, bool fTakeIntoAccountCurrentStateOnly) const
{
    ...
    CMachine machine = nemuGlobal().virtualBox().FindMachine(strMachineID);
    if (machine.isNull())
    {
