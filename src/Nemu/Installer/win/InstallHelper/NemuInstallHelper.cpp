/* $Id: NemuInstallHelper.cpp $ */
/** @file
 * NemuInstallHelper - Various helper routines for Windows host installer.
 */
...
#include "NemuCommon.h"

#ifndef NEMU_OSE
# include "internal/NemuSerial.h"
#endif
...
UINT __stdcall InstallPythonAPI(MSIHANDLE hModule)
{
    ...
    {
        /* Get the NemuAPI setup string. */
        WCHAR wszPathTargetDir[MAX_PATH] = {0};
        NemuGetProperty(hModule, L"CustomActionData", wszPathTargetDir, sizeof(wszPathTargetDir));
        if (wcslen(wszPathTargetDir))
        {
            ...
            swprintf_s(wszCmdLine, RT_ELEMENTS(wszCmdLine), L"%s\\python.exe nemuapisetup.py install", wszVal);

            /* Set required environment variables. */
            if (!SetEnvironmentVariableW(L"NEMU_INSTALL_PATH", wszPathTargetDir))
                logStringW(hModule, L"InstallPythonAPI: Could set environment variable NEMU_INSTALL_PATH!");
            else
            {
                ...
                {
                    /* All done! */
                    logStringW(hModule, L"InstallPythonAPI: NemuAPI for Python successfully installed.");
                    bInstalled = TRUE;
                }
                else
                {
                    if (dwRetExec)
                        logStringW(hModule, L"InstallPythonAPI: Error while executing installation of Nemu API: %ld", dwRetExec);
                    else
                        logStringW(hModule, L"InstallPythonAPI: Python reported an error while installing Nemu API: %ld", dwExitCode);
                }
            }
        }
        else
            logStringW(hModule, L"InstallPythonAPI: Unable to retrieve Nemu installation path!");
    }

    NemuSetProperty(hModule, L"NEMU_PYTHON_IS_INSTALLED", bInstalled ? L"1" : L"0");

    if (!bInstalled)
        logStringW(hModule, L"InstallPythonAPI: Nemu API not installed.");
                                 ...
                                 const WCHAR *pwszValue)
{
                                 ...
                                 wszValue, sizeof(wszValue), pwszFileName) > 0)
    {
        ...
        if (wcsicmp(L"General", pwszSection) != 0)
            swprintf_s(wszKey, RT_ELEMENTS(wszKey), L"SOFTWARE\\%s\\VirtualBox\\Branding\\%s", NEMU_VENDOR_SHORT, pwszSection);
        else
            swprintf_s(wszKey, RT_ELEMENTS(wszKey), L"SOFTWARE\\%s\\VirtualBox\\Branding", NEMU_VENDOR_SHORT);
...
UINT __stdcall UninstallBranding(MSIHANDLE hModule)
{
    ...
    rc = NemuGetProperty(hModule, L"CustomActionData", wszPathTargetDir, sizeof(wszPathTargetDir));
    if (rc == ERROR_SUCCESS)
    {
...
UINT __stdcall InstallBranding(MSIHANDLE hModule)
{
    ...
    rc = NemuGetProperty(hModule, L"SOURCEDIR", wszPathMSI, sizeof(wszPathMSI));
    if (rc == ERROR_SUCCESS)
    {
        WCHAR wszPathTargetDir[_MAX_PATH];
        rc = NemuGetProperty(hModule, L"CustomActionData", wszPathTargetDir, sizeof(wszPathTargetDir));
...
/** @todo should use some real Nemu app name */
#define NEMU_NETCFG_APP_NAME L"VirtualBox Installer"
#define NEMU_NETCFG_MAX_RETRIES 10
#define NETFLT_PT_INF_REL_PATH L"NemuNetFlt.inf"
#define NETFLT_MP_INF_REL_PATH L"NemuNetFltM.inf"
#define NETFLT_ID  L"sun_NemuNetFlt" /** @todo Needs to be changed (?). */
#define NETADP_ID  L"sun_NemuNetAdp" /** @todo Needs to be changed (?). */

#define NETLWF_INF_NAME L"NemuNetLwf.inf"

static MSIHANDLE g_hCurrentModule = NULL;

static VOID nemuDrvLoggerCallback(NEMUDRVCFG_LOG_SEVERITY enmSeverity, char * msg, void * pvContext)
{
    switch (enmSeverity)
    {
...
static VOID netCfgLoggerDisable()
{
    if (g_hCurrentModule)
    {
        NemuNetCfgWinSetLogging((LOG_ROUTINE)NULL);
...
static VOID netCfgLoggerEnable(MSIHANDLE hModule)
{
    ...
    NemuNetCfgWinSetLogging((LOG_ROUTINE)netCfgLoggerCallback);
    /* uncomment next line if you want to add logging information from NemuDrvCfg.cpp */
//    NemuDrvCfgLoggerSet(nemuDrvLoggerCallback, NULL);
}
...
static UINT doNetCfgInit(MSIHANDLE hModule, INetCfg **ppnc, BOOL bWrite)
{
    ...
    {
        LPWSTR lpszLockedBy;
        HRESULT hr = NemuNetCfgWinQueryINetCfg(ppnc, bWrite, NEMU_NETCFG_APP_NAME, 10000, &lpszLockedBy);
        if (hr != NETCFG_E_NO_WRITE_LOCK)
        {
            if (FAILED(hr))
                logStringW(hModule, L"doNetCfgInit: NemuNetCfgWinQueryINetCfg failed, error = 0x%x", hr);
        ...
        if (   cRetries < NEMU_NETCFG_MAX_RETRIES
            && !wcscmp(lpszLockedBy, L"6to4svc.dll"))
        {
            cRetries++;
            logStringW(hModule, L"doNetCfgInit: lpszLockedBy is 6to4svc.dll, retrying %d out of %d", cRetries, NEMU_NETCFG_MAX_RETRIES);
            MsgResult = IDRETRY;
        }
        else
        {
...
static UINT nemuNetFltQueryInfArray(MSIHANDLE hModule, OUT LPWSTR pwszPtInf, OUT LPWSTR pwszMpInf, DWORD dwSize)
{
        ...
        && dwBuf)
    {
        ...
        logStringW(hModule, L"nemuNetFltQueryInfArray: INF 1: %s", pwszPtInf);

        wcsncat(pwszMpInf, NETFLT_MP_INF_REL_PATH, sizeof(NETFLT_MP_INF_REL_PATH));
        logStringW(hModule, L"nemuNetFltQueryInfArray: INF 2: %s", pwszMpInf);
    }
    else if (uErr != ERROR_SUCCESS)
        logStringW(hModule, L"nemuNetFltQueryInfArray: MsiGetPropertyW failed, error = 0x%x", uErr);
    else
    {
        logStringW(hModule, L"nemuNetFltQueryInfArray: Empty installation directory");
...
UINT __stdcall UninstallNetFlt(MSIHANDLE hModule)
{
    ...
    __try
    {
        ...
        if (uErr == ERROR_SUCCESS)
        {
            HRESULT hr = NemuNetCfgWinNetFltUninstall(pNetCfg);
            if (hr != S_OK)
                logStringW(hModule, L"UninstallNetFlt: NemuNetCfgWinUninstallComponent failed, error = 0x%x", hr);
...
UINT __stdcall InstallNetFlt(MSIHANDLE hModule)
{
    ...
    __try
    {
        ...
        if (uErr == ERROR_SUCCESS)
        {
            ...
            uErr = nemuNetFltQueryInfArray(hModule, wszPtInf, wszMpInf, sizeof(wszMpInf));
            if (uErr == ERROR_SUCCESS)
            {
                LPCWSTR const apwszInfs[] = { wszPtInf, wszMpInf };
                HRESULT hr = NemuNetCfgWinNetFltInstall(pNetCfg, &apwszInfs[0], RT_ELEMENTS(apwszInfs));
                if (FAILED(hr))
                    logStringW(hModule, L"InstallNetFlt: NemuNetCfgWinNetFltInstall failed, error = 0x%x", hr);

                uErr = errorConvertFromHResult(hModule, hr);
            }
            else
                logStringW(hModule, L"InstallNetFlt: nemuNetFltQueryInfArray failed, error = 0x%x", uErr);
...
UINT __stdcall UninstallNetLwf(MSIHANDLE hModule)
{
    ...
    __try
    {
        ...
        if (uErr == ERROR_SUCCESS)
        {
            HRESULT hr = NemuNetCfgWinNetLwfUninstall(pNetCfg);
            if (hr != S_OK)
                logStringW(hModule, L"UninstallNetLwf: NemuNetCfgWinUninstallComponent failed, error = 0x%x", hr);
        ...
        if (uErr == ERROR_SUCCESS)
        {
            ...
            if (uErr == ERROR_SUCCESS)
            {
                if (cchInf)
                {
                    ...
                    HRESULT hr = NemuNetCfgWinNetLwfInstall(pNetCfg, wszInf);
                    if (FAILED(hr))
                        logStringW(hModule, L"InstallNetLwf: NemuNetCfgWinNetLwfInstall failed, error = 0x%x", hr);
            ...
            ))
    {
        ...
        if (hKey != INVALID_HANDLE_VALUE)
        {
            ...
            if (winEr == ERROR_SUCCESS)
            {
                ...
                HRESULT hr = NemuNetCfgWinGenHostonlyConnectionName (DevName, ConnectoinName, &cbName);
                NonStandardAssert(hr == S_OK);
                if (SUCCEEDED(hr))
                {
                    hr = NemuNetCfgWinRenameConnection(guid, ConnectoinName);
                    NonStandardAssert(hr == S_OK);
                }
            }
        }
...
static UINT _createHostOnlyInterface(MSIHANDLE hModule, LPCWSTR pwszId, LPCWSTR pwszInfName)
{
    ...
    if (uErr == ERROR_SUCCESS)
    {
        if (cchMpInf)
        {
            ...
            logStringW(hModule, L"CreateHostOnlyInterface: Nemu installation path is empty");
    }
    else
        logStringW(hModule, L"CreateHostOnlyInterface: Unable to retrieve Nemu installation path, error = 0x%x", uErr);

    /* Make sure the inf file is installed. */
    if (pwszInfPath != NULL && bIsFile)
    {
        logStringW(hModule, L"CreateHostOnlyInterface: Calling NemuDrvCfgInfInstall(%s)", pwszInfPath);
        hr = NemuDrvCfgInfInstall(pwszInfPath);
        logStringW(hModule, L"CreateHostOnlyInterface: NemuDrvCfgInfInstall returns 0x%x", hr);
        if (FAILED(hr))
    ...
    //first, try to update Host Only Network Interface
        ...
        hr = NemuNetCfgWinUpdateHostOnlyNetworkInterface(pwszInfPath, &fRebootRequired, pwszId);
        if (SUCCEEDED(hr))
        {
            if (fRebootRequired)
            {
            ...
            logStringW(hModule, L"UpdateHostOnlyInterfaces: NemuNetCfgWinUpdateHostOnlyNetworkInterface failed, hr = 0x%x", hr);
    //in fail case call CreateHostOnlyInterface
        if (FAILED(hr))
        {
            logStringW(hModule, L"CreateHostOnlyInterface: calling NemuNetCfgWinCreateHostOnlyNetworkInterface");
#ifdef NEMUNETCFG_DELAYEDRENAME
            BSTR devId;
            hr = NemuNetCfgWinCreateHostOnlyNetworkInterface(pwszInfPath, bIsFile, &guid, &devId, NULL);
#else /* !NEMUNETCFG_DELAYEDRENAME */
            hr = NemuNetCfgWinCreateHostOnlyNetworkInterface(pwszInfPath, bIsFile, &guid, NULL, NULL);
#endif /* !NEMUNETCFG_DELAYEDRENAME */
            logStringW(hModule, L"CreateHostOnlyInterface: NemuNetCfgWinCreateHostOnlyNetworkInterface returns 0x%x", hr);
            if (SUCCEEDED(hr))
            {
                ULONG ip = inet_addr("192.168.56.1");
                ULONG mask = inet_addr("255.255.255.0");
                logStringW(hModule, L"CreateHostOnlyInterface: calling NemuNetCfgWinEnableStaticIpConfig");
                hr = NemuNetCfgWinEnableStaticIpConfig(&guid, ip, mask);
                logStringW(hModule, L"CreateHostOnlyInterface: NemuNetCfgWinEnableStaticIpConfig returns 0x%x", hr);
                if (FAILED(hr))
                    logStringW(hModule, L"CreateHostOnlyInterface: NemuNetCfgWinEnableStaticIpConfig failed, error = 0x%x", hr);
#ifdef NEMUNETCFG_DELAYEDRENAME
                hr = NemuNetCfgWinRenameHostOnlyConnection(&guid, devId, NULL);
                if (FAILED(hr))
                    logStringW(hModule, L"CreateHostOnlyInterface: NemuNetCfgWinRenameHostOnlyConnection failed, error = 0x%x", hr);
                SysFreeString(devId);
#endif /* NEMUNETCFG_DELAYEDRENAME */
            }
            else
                logStringW(hModule, L"CreateHostOnlyInterface: NemuNetCfgWinCreateHostOnlyNetworkInterface failed, error = 0x%x", hr);
        }
    }
...
{
    return _createHostOnlyInterface(hModule, NETADP_ID, L"NemuNetAdp.inf");
}

UINT __stdcall Ndis6CreateHostOnlyInterface(MSIHANDLE hModule)
{
    return _createHostOnlyInterface(hModule, NETADP_ID, L"NemuNetAdp6.inf");
}
...
static UINT _removeHostOnlyInterfaces(MSIHANDLE hModule, LPCWSTR pwszId)
{
    ...
    HRESULT hr = NemuNetCfgWinRemoveAllNetDevicesOfId(pwszId);
    if (SUCCEEDED(hr))
    {
        hr = NemuDrvCfgInfUninstallAllSetupDi(&GUID_DEVCLASS_NET, L"Net", pwszId, SUOI_FORCEDELETE/* could be SUOI_FORCEDELETE */);
        if (FAILED(hr))
        {
...
static UINT _stopHostOnlyInterfaces(MSIHANDLE hModule, LPCWSTR pwszId)
{
    ...
    HRESULT hr = NemuNetCfgWinPropChangeAllNetDevicesOfId(pwszId, NEMUNECTFGWINPROPCHANGE_TYPE_DISABLE);
    if (SUCCEEDED(hr))
    {
    ...
    if (uErr == ERROR_SUCCESS)
    {
        if (cchMpInf)
        {
            ...
            else
            {
                ...
                BOOL fRebootRequired = FALSE;
                HRESULT hr = NemuNetCfgWinUpdateHostOnlyNetworkInterface(pwszInfPath, &fRebootRequired, pwszId);
                if (SUCCEEDED(hr))
                {
                    if (fRebootRequired)
                    {
            ...
            logStringW(hModule, L"UpdateHostOnlyInterfaces: Nemu installation path is empty");
    }
    else
        logStringW(hModule, L"UpdateHostOnlyInterfaces: Unable to retrieve Nemu installation path, error = 0x%x", uErr);
...
{
    return _updateHostOnlyInterfaces(hModule, L"NemuNetAdp.inf", NETADP_ID);
}

UINT __stdcall Ndis6UpdateHostOnlyInterfaces(MSIHANDLE hModule)
{
    return _updateHostOnlyInterfaces(hModule, L"NemuNetAdp6.inf", NETADP_ID);
}
...
static UINT _uninstallNetAdp(MSIHANDLE hModule, LPCWSTR pwszId)
{
    ...
    __try
    {
        ...
        if (uErr == ERROR_SUCCESS)
        {
            HRESULT hr = NemuNetCfgWinNetAdpUninstall(pNetCfg, pwszId);
            if (hr != S_OK)
                logStringW(hModule, L"UninstallNetAdp: NemuNetCfgWinUninstallComponent failed, error = 0x%x", hr);
                ...
                && dwKeyType == REG_SZ)
            {
                ...
                if (   !wcscmp(wszNetCfgInstanceId, pwszGUID)
                    ...
                    && (   (!wcscmp(wszNetProviderName, L"innotek GmbH")) /* Legacy stuff. */
                        ...
                        || (!wcscmp(wszNetProviderName, MY_WTEXT(NEMU_VENDOR))) /* Reflects current vendor string. */
                       )
                   )
                {
                    bIsTapDevice = true;
    ...
    do
    {
        ...
        do /* break-loop */
        {
            ...
            if ((lStatus != ERROR_SUCCESS) || !hkeyNetwork)
                SetErrBreak((L"Nemu HostInterfaces: Host interface network was not found in registry (%s)! [1]", wszRegLocation));

            lStatus = RegOpenKeyExW(hkeyNetwork, L"Connection", 0, KEY_READ, &hkeyConnection);
            if ((lStatus != ERROR_SUCCESS) || !hkeyConnection)
                SetErrBreak((L"Nemu HostInterfaces: Host interface network was not found in registry (%s)! [2]", wszRegLocation));
            ...
            if ((lStatus != ERROR_SUCCESS) || (dwKeyType != REG_SZ))
                SetErrBreak((L"Nemu HostInterfaces: Host interface network was not found in registry (%s)! [3]", wszRegLocation));
        }
        while (0);
        ...
        do
        {
            ...
            {
                logStringW(hModule, L"Nemu HostInterfaces: SetupDiGetClassDevs failed (0x%08X)!", GetLastError());
                SetErrBreak(L"Nemu HostInterfaces: Uninstallation failed!");
            }
            ...
            if (fFoundDevice)
            {
                ...
                if (!fResult)
                {
                    logStringW(hModule, L"Nemu HostInterfaces: SetupDiSetSelectedDevice failed (0x%08X)!", GetLastError());
                    SetErrBreak(L"Nemu HostInterfaces: Uninstallation failed!");
                }
                ...
                if (!fResult)
                {
                    logStringW(hModule, L"Nemu HostInterfaces: SetupDiCallClassInstaller (DIF_REMOVE) failed (0x%08X)!", GetLastError());
                    SetErrBreak(L"Nemu HostInterfaces: Uninstallation failed!");
                }
            }
            else
                SetErrBreak(L"Nemu HostInterfaces: Host interface network device not found!");
        }
        while (0);
...
UINT __stdcall UninstallTAPInstances(MSIHANDLE hModule)
{
    ...
    if (lStatus == ERROR_SUCCESS)
    {
        logStringW(hModule, L"Nemu HostInterfaces: Enumerating interfaces ...");
        for (int i = 0; ; ++i)
        {
            ...
            if (lStatus != ERROR_SUCCESS)
            {
                switch (lStatus)
                {
                    case ERROR_NO_MORE_ITEMS:
                        logStringW(hModule, L"Nemu HostInterfaces: No interfaces found.");
                        break;
                    default:
                        logStringW(hModule, L"Nemu HostInterfaces: Enumeration failed: %ld", lStatus);
                        break;
                }
                break;
            }
            ...
            {
                logStringW(hModule, L"Nemu HostInterfaces: Removing interface \"%s\" ...", wszNetworkGUID);
                removeNetworkInterface(hModule, wszNetworkGUID);
                lStatus = RegDeleteKeyW(hCtrlNet, wszNetworkGUID);
            }
        }
        RegCloseKey(hCtrlNet);
        logStringW(hModule, L"Nemu HostInterfaces: Removing interfaces done.");
    }
    return ERROR_SUCCESS;
}
