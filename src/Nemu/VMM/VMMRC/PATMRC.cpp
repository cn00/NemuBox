 ...
 *          monitored.
 */
DECLEXPORT(NEMUSTRICTRC) patmRCVirtPagePfHandler(PVM pVM, PVMCPU pVCpu, RTGCUINT uErrorCode, PCPUMCTXCORE pCtxCore,
 ...
 *
 */
VMMRC_INT_DECL(NEMUSTRICTRC) PATMRCHandleWriteToPatchPage(PVM pVM, PCPUMCTXCORE pCtxCore, RTRCPTR GCPtr, uint32_t cbWrite)
{
    Assert(cbWrite > 0);
    ...
    if (pPatchPage)
    {
            ...
            || (RTRCUINTPTR)pPatchPage->pHighestAddrGC < (RTRCUINTPTR)GCPtr)
        {
            ...
            int rc = NEMUSTRICTRC_TODO(EMInterpretInstruction(VMMGetCpu0(pVM), pCtxCore, (RTGCPTR)(RTRCUINTPTR)GCPtr));
            if (rc == VINF_SUCCESS)
            {
                STAM_COUNTER_INC(&pVM->patm.s.StatPatchWriteInterpreted);
                STAM_PROFILE_ADV_STOP(&pVM->patm.s.StatPatchWriteDetect, a);
 ...
 *
 * @returns Strict Nemu status, includes all statuses that
...
VMMRC_INT_DECL(int) PATMRCHandleInt3PatchTrap(PVM pVM, PCPUMCTXCORE pCtxCore)
{
    ...
    if (pRec && pRec->patch.uState == PATCH_ENABLED)
    {
        ...
        if (pRec->patch.flags & PATMFL_INT3_REPLACEMENT)
        {
            ...
            rc = NEMUSTRICTRC_TODO(EMInterpretInstructionDisasState(pVCpu, &cpu, pCtxCore, 0 /* not relevant here */,
