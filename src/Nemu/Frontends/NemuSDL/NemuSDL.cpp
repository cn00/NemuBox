...
#if defined(NEMUSDL_WITH_X11)
# include <Nemu/NemuKeyboard.h>

# include <X11/Xlib.h>
# include <X11/cursorfont.h>      /* for XC_left_ptr */
# if !defined(NEMU_WITHOUT_XCURSOR)
...
static void    SendMouseEvent(NemuSDLFB *fb, int dz, int button, int down);
...
static NemuSDLFB * getFbFromWinId(SDL_WindowID id);
#endif
...
static ComObjPtr<NemuSDLFB> gpFramebuffer[64];
...
#if defined(NEMUSDL_WITH_X11) && !defined(NEMU_WITH_SDL13)
 ...
 */
class NemuSDLClientEventListener
{
public:
    NemuSDLClientEventListener()
    {
    }

    virtual ~NemuSDLClientEventListener()
    {
    }
    ...
    STDMETHOD(HandleEvent)(NemuEventType_T aType, IEvent * aEvent)
    {
        switch (aType)
        {
            case NemuEventType_OnNemuSVCAvailabilityChanged:
            {
                ComPtr<INemuSVCAvailabilityChangedEvent> pVSACEv = aEvent;
                Assert(pVSACEv);
                BOOL fAvailable = FALSE;
                pVSACEv->COMGETTER(Available)(&fAvailable);
                if (!fAvailable)
                {
                    LogRel(("NemuSDL: NemuSVC became unavailable, exiting.\n"));
                    RTPrintf("NemuSVC became unavailable, exiting.\n");
                    /* Send QUIT event to terminate the VM as cleanly as possible
                     * given that NemuSVC is no longer present. */
                    SDL_Event event = {0};
                    event.type = SDL_QUIT;
 ...
 */
class NemuSDLEventListener
{
public:
    NemuSDLEventListener()
    {
    }

    virtual ~NemuSDLEventListener()
    {
    }
            ...
            case NemuEventType_OnExtraDataChanged:
            {
                ...
                if (gpMachine)
                {
                    ...
                    if (bstrOurId == bstrMachineId)
                    {
                        ...
                        if (bstrKey == NEMUSDL_SECURELABEL_EXTRADATA)
                        {
                            /*
 ...
 */
class NemuSDLConsoleEventListener
{
public:
    NemuSDLConsoleEventListener() : m_fIgnorePowerOffEvents(false)
    {
    }

    virtual ~NemuSDLConsoleEventListener()
    {
    }
    ...
    STDMETHOD(HandleEvent)(NemuEventType_T aType, IEvent * aEvent)
    {
        ...
        switch (aType)
        {
            case NemuEventType_OnMousePointerShapeChanged:
            {
            ...
            case NemuEventType_OnMouseCapabilityChanged:
            {
            ...
            case NemuEventType_OnKeyboardLedsChanged:
            {
            ...
            case NemuEventType_OnRuntimeError:
            {
            ...
            case NemuEventType_OnCanShowWindow:
            {
            ...
            case NemuEventType_OnShowWindow:
            {
...
typedef ListenerImpl<NemuSDLClientEventListener>  NemuSDLClientEventListenerImpl;
typedef ListenerImpl<NemuSDLEventListener>        NemuSDLEventListenerImpl;
typedef ListenerImpl<NemuSDLConsoleEventListener> NemuSDLConsoleEventListenerImpl;
...
#endif
#ifdef NEMUSDL_ADVANCED_OPTIONS
...
#if defined(DEBUG) || defined(NEMU_WITH_STATISTICS)
             "\n"
...
static void PrintError(const char *pszName, CBSTR pwszDescr, CBSTR pwszComponent=NULL)
{
    ...
    if (   (pszFile = strstr(pszBuffer, "At '"))
        ...
        && (pszStat = strstr(pszBuffer, "Nemu status code: ")))
        RTPrintf("  %.*s  %.*s\n  In%.*s  %s",
...
DECLEXPORT(int) TrustedMain(int argc, char **argv, char **envp)
{
    ...
    uint32_t vramSize = 0;
    ComPtr<IEventListener> pNemuClientListener;
    ComPtr<IEventListener> pNemuListener;
    ComObjPtr<NemuSDLConsoleEventListenerImpl> pConsoleListener;
    ...
    RTPrintf(NEMU_PRODUCT " SDL GUI version %s\n"
...
#endif /* NEMUSDL_ADVANCED_OPTIONS */
#ifdef NEMU_WIN32_UI
    ...
    if (!NemuSDLFB::init(fShowSDLConfig))
        goto leave;
    ...
    if (fSecureLabel)
    {
        ...
        Bstr bstrLabel;
        gpMachine->GetExtraData(Bstr(NEMUSDL_SECURELABEL_EXTRADATA).raw(), bstrLabel.asOutParam());
...

    {
        ...
        ComObjPtr<NemuSDLClientEventListenerImpl> listener;
        listener.createObject();
        listener->init(new NemuSDLClientEventListener());
        pNemuClientListener = listener;
        com::SafeArray<NemuEventType_T> eventTypes;
        eventTypes.push_back(NemuEventType_OnNemuSVCAvailabilityChanged);
        CHECK_ERROR(pES, RegisterListener(pNemuClientListener, ComSafeArrayAsInParam(eventTypes), true));
    }

    {
        ...
        ComObjPtr<NemuSDLEventListenerImpl> listener;
        listener.createObject();
        listener->init(new NemuSDLEventListener());
        pNemuListener = listener;
        com::SafeArray<NemuEventType_T> eventTypes;
        eventTypes.push_back(NemuEventType_OnExtraDataChanged);
        CHECK_ERROR(pES, RegisterListener(pNemuListener, ComSafeArrayAsInParam(eventTypes), true));
    }

    {
        ...
        pConsoleListener->init(new NemuSDLConsoleEventListener());
        com::SafeArray<NemuEventType_T> eventTypes;
        eventTypes.push_back(NemuEventType_OnMousePointerShapeChanged);
        eventTypes.push_back(NemuEventType_OnMouseCapabilityChanged);
        eventTypes.push_back(NemuEventType_OnKeyboardLedsChanged);
        eventTypes.push_back(NemuEventType_OnStateChanged);
        eventTypes.push_back(NemuEventType_OnRuntimeError);
        eventTypes.push_back(NemuEventType_OnCanShowWindow);
        eventTypes.push_back(NemuEventType_OnShowWindow);
...
# endif /* NEMUSDL_WITH_X11 */
    ...
    LogRel(("NemuSDL: NUM lock initially %s, CAPS lock initially %s\n",
            ...
            )
        {
            ...
            if (WaitSDLEvent(&event))
            {
                switch (event.type)
                {
                    ...
                    {
                        if (event.user.code != NEMUSDL_TERM_NORMAL)
                        {
                    ...
                    default:
                    {
                        Log8(("NemuSDL: Unknown SDL event %d (pre)\n", event.type));
                        break;
                    }
                }
...
#endif
    LogFlow(("NemuSDL: Entering big event loop\n"));
        ...
        switch (event.type)
        {
            ...
            case SDL_WINDOWEVENT:
            {
                switch (event.window.event)
                {
                    ...
                    {
                        NemuSDLFB *fb = getFbFromWinId(event.window.windowID);
                        if (fb)
                            fb->repaint();
                        break;
                    }
...
leave:
    ...
    LogFlow(("leaving...\n"));
#if defined(NEMU_WITH_XPCOM) && !defined(RT_OS_DARWIN) && !defined(RT_OS_OS2)
...
# endif /* NEMUSDL_WITH_X11 */
        SDL_SetCursor(gpDefaultCursor);
# if defined(NEMUSDL_WITH_X11) && !defined(NEMU_WITHOUT_XCURSOR)
        if (gfXCursorEnabled)
            XFreeCursor(gSdlInfo.info.x11.display, pDefaultTempX11Cursor);
# endif /* NEMUSDL_WITH_X11 && !NEMU_WITHOUT_XCURSOR */
    }
...
# elif defined(NEMUSDL_WITH_X11) && !defined(NEMU_WITHOUT_XCURSOR)
            if (gfXCursorEnabled)
...
# endif /* NEMUSDL_WITH_X11 && !NEMU_WITHOUT_XCURSOR */
    ...
    if (pNemuListener)
    {
        ...
        CHECK_ERROR(pVirtualBox, COMGETTER(EventSource)(pES.asOutParam()));
        if (!pES.isNull())
            CHECK_ERROR(pES, UnregisterListener(pNemuListener));
        pNemuListener.setNull();
    }

    /* VirtualBoxClient listener unregistration. */
    if (pNemuClientListener)
    {
        ComPtr<IEventSource> pES;
        CHECK_ERROR(pVirtualBoxClient, COMGETTER(EventSource)(pES.asOutParam()));
        if (!pES.isNull())
            CHECK_ERROR(pES, UnregisterListener(pNemuClientListener));
        pNemuClientListener.setNull();
    }
...
static void ProcessKey(SDL_KeyboardEvent *ev)
{
#if (defined(DEBUG) || defined(NEMU_WITH_STATISTICS)) && !defined(NEMU_WITH_SDL13)
...
#endif // DEBUG || NEMU_WITH_STATISTICS
 ...
 */
static void SendMouseEvent(NemuSDLFB *fb, int dz, int down, int button)
{
    ...
    if (abs)
    {
         ...
         * following problem: Some guests switch off the Nemu hardware mouse
...
static void UpdateTitlebar(TitlebarMode mode, uint32_t u32User)
{
    ...
    RTStrPrintf(szTitle, sizeof(szTitle), "%s - " NEMU_PRODUCT,
    ...
    setUITitle(szTitle);
#else
    SDL_WM_SetCaption(szTitle, NEMU_PRODUCT);
#endif
}

#if 0
static void nemu_show_shape(unsigned short w, unsigned short h,
...
static void SetPointerShape(const PointerShapeChangeData *data)
{
    ...
    if (data->shape.size() > 0)
    {
        ...
        // nemu_show_shape(data->width, data->height, 0, data->shape);
