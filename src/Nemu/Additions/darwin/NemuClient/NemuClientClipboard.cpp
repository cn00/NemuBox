/** $Id: NemuClientClipboard.cpp $ */
/** @file
 * NemuClient - Shared Slipboard Dispatcher, Darwin.
 */
...
#define NEMUCLIENT_SERVICE_NAME     "clipboard"
...
static DECLCALLBACK(int) vbclClipboardDispatcher(RTTHREAD ThreadSelf, void *pvUser)
{
    ...
    NemuClientVerbose(2, "starting host clipboard polling thread\n");
    ...
    while (!fQuit && !ASMAtomicReadBool(&g_fShouldStop))
    {
        ...
        NemuClientVerbose(2, "waiting for new host request\n");
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            switch (Msg)
            {
                ...
                case NEMU_SHARED_CLIPBOARD_HOST_MSG_QUIT:
                    NemuClientVerbose(2, "host requested quit\n");
                    fQuit = true;
                    break;

                /* The host needs data in the specified format */
                case NEMU_SHARED_CLIPBOARD_HOST_MSG_READ_DATA:
                    NemuClientVerbose(2, "host requested guest's clipboard read\n");
                ...
                case NEMU_SHARED_CLIPBOARD_HOST_MSG_FORMATS:
                    NemuClientVerbose(2, "host requested guest's clipboard write\n");
                    rc = vbclClipboardForwardToGuest(g_u32ClientId, g_PasteboardRef, fFormats);
                    AssertMsg(RT_SUCCESS(rc), ("paste to guest failed\n"));
                    break;

                default:
                    NemuClientVerbose(2, "received unknow command from host service\n");
                    RTThreadSleep(1000);
            }
    ...
    NemuClientVerbose(2, "host clipboard polling thread stopped\n");
...
static DECLCALLBACK(int) vbclGuestPasteboardPoll(RTTHREAD ThreadSelf, void *pvUser)
{
    ...
    NemuClientVerbose(2, "starting guest clipboard polling thread\n");
    ...
    while (!ASMAtomicReadBool(&g_fShouldStop))
    {
        ...
        if (fSyncFlags & kPasteboardModified)
        {
            ...
            if (RT_FAILURE(rc))
            {
                NemuClientVerbose(2, "failed to report pasteboard update (%Rrc)\n", rc);
            }
            else
            {
                NemuClientVerbose(2, "guest clipboard update reported: %d\n", (int)fFormats);
            }
        }
    ...
    NemuClientVerbose(2, "guest clipboard polling thread stopped\n");
...
static int vbclClipboardStart(void)
{
    ...
    NemuClientVerbose(2, "starting clipboard\n");
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (rc == noErr)
        {
            ...
            NemuClientVerbose(2, "unable create dispatcher thread\n");
        ...
        else
        {
            ...
            NemuClientVerbose(2, "unable access guest clipboard\n");
        }
    ...
    {
        NemuClientVerbose(2, "unable to establish connection to clipboard service: %Rrc\n", rc);
    }
...
static int vbclClipboardStop(void)
{
    ...
    NemuClientVerbose(2, "stopping clipboard\n");
    ...
    if (RT_SUCCESS(rc))
        ...
        NemuClientVerbose(2, "unable to close clipboard service connection: %Rrc\n", rc);
    ...
    if (RT_FAILURE(rc))
        NemuClientVerbose(2, "failed to stop dispatcher thread");

    /* Stop Pasteboard polling thread */
    rc = RTThreadWait(g_GuestPasteboardThread, 10 * 1000 /* Wait 10 seconds */, NULL);
    if (RT_FAILURE(rc))
        NemuClientVerbose(2, "failed to stop pasteboard polling thread");
...
NEMUCLIENTSERVICE g_ClipboardService =
{
    /* pszName */
    NEMUCLIENT_SERVICE_NAME,
