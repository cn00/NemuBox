...
#ifndef ___Nemu_vmm_pdmusb_h
#define ___Nemu_vmm_pdmusb_h
 ...
 *
 * This structure is used when registering a device from NemuUsbRegister() in HC Ring-3.
...
/**
 * NEMU_STRICT wrapper for pHlpR3->pfnDBGFStopV.
 *
 * @returns Nemu status code which must be passed up to the VMM.
...
/** Pointer to callbacks provided to the NemuUsbRegister() call. */
typedef const struct PDMUSBREGCB *PCPDMUSBREGCB;

/**
 * Callbacks for NemuUSBDeviceRegister().
 */
...
/**
 * The NemuUsbRegister callback function.
 *
 ...
 */
typedef DECLCALLBACK(int) FNPDMNEMUUSBREGISTER(PCPDMUSBREGCB pCallbacks, uint32_t u32Version);
