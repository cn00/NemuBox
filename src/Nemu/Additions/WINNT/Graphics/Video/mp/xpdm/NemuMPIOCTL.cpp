...
/** @file
 * Nemu XPDM Miniport IOCTL handlers
 */
...
/* Note: in/out parameters passed to NemuDrvStartIO point to the same memory location.
 ...
 */
#define NEMUMPIOCTL_HIDE(_var) \
    {                          \
        PVOID (_var);          \
        (VOID)(_var)

#define NEMUMPIOCTL_UNHIDE()   \
    }

/* Called for IOCTL_VIDEO_RESET_DEVICE.
 * Reset device to a state it comes at system boot time.
 * @todo It doesn't do anythyng at the moment, but it looks like the same as NemuDrvResetHW.
 */
BOOLEAN NemuMPResetDevice(PNEMUMP_DEVEXT pExt, PSTATUS_BLOCK pStatus)
{
   ...
   VideoPortWritePortUshort((PUSHORT)VBE_DISPI_IOPORT_DATA, VBE_DISPI_DISABLED);
    ...
    /* @todo NemuMPSetGraphicsCap(FALSE); */
                             ...
                             PVIDEO_MEMORY_INFORMATION pMapInfo, PSTATUS_BLOCK pStatus)
{
    ...
    framebuffer.QuadPart = NemuCommonFromDeviceExt(pExt)->phVRAM.QuadPart + pExt->ulFrameBufferOffset;

    pMapInfo->VideoRamBase = pRequestedAddress->RequestedVirtualAddress;
    NEMUMPIOCTL_HIDE(pRequestedAddress);
    ...
    if (NO_ERROR == pStatus->Status)
    {
        ...
        pMapInfo->FrameBufferLength =
            NemuMPXpdmCurrentVideoMode(pExt)->VisScreenHeight*
            NemuMPXpdmCurrentVideoMode(pExt)->ScreenStride;
                               ...
                               PVIDEO_SHARE_MEMORY_INFORMATION pShareMemInfo, PSTATUS_BLOCK pStatus)
{
    ...
    shareAddress.QuadPart = NemuCommonFromDeviceExt(pExt)->phVRAM.QuadPart + pExt->ulFrameBufferOffset;
...
BOOLEAN NemuMPSetCurrentMode(PNEMUMP_DEVEXT pExt, PVIDEO_MODE pMode, PSTATUS_BLOCK pStatus)
{
    ...
    pModeInfo = NemuMPCmnGetVideoModeInfo(pExt, RequestedMode-1);
    if (!pModeInfo)
    {
        pStatus->Status = ERROR_INVALID_PARAMETER;
        return FALSE;
    ...
    NemuVideoSetModeRegisters((USHORT)pModeInfo->VisScreenWidth, (USHORT)pModeInfo->VisScreenHeight,
...
BOOLEAN NemuMPQueryCurrentMode(PNEMUMP_DEVEXT pExt, PVIDEO_MODE_INFORMATION pModeInfo, PSTATUS_BLOCK pStatus)
{
    ...
    VideoPortMoveMemory(pModeInfo, NemuMPXpdmCurrentVideoMode(pExt), sizeof(VIDEO_MODE_INFORMATION));
...
BOOLEAN NemuMPQueryNumAvailModes(PNEMUMP_DEVEXT pExt, PVIDEO_NUM_MODES pNumModes, PSTATUS_BLOCK pStatus)
{
    ...
    NemuMPXpdmBuildVideoModesTable(pExt);

    pNumModes->NumModes = NemuMPXpdmGetVideoModesCount(pExt);
...
BOOLEAN NemuMPQueryAvailModes(PNEMUMP_DEVEXT pExt, PVIDEO_MODE_INFORMATION pModes, PSTATUS_BLOCK pStatus)
{
    ...
    ULONG ulSize = NemuMPXpdmGetVideoModesCount(pExt)*sizeof(VIDEO_MODE_INFORMATION);
    pStatus->Information = ulSize;
    VideoPortMoveMemory(pModes, NemuMPCmnGetVideoModeInfo(pExt, 0), ulSize);
...
BOOLEAN NemuMPSetColorRegisters(PNEMUMP_DEVEXT pExt, PVIDEO_CLUT pClut, PSTATUS_BLOCK pStatus)
{
    ...
    {
      NemuVideoCmnPortWriteUchar(VBE_DISPI_IOPORT_DAC_WRITE_INDEX, (UCHAR)entry);
      NemuVideoCmnPortWriteUchar(VBE_DISPI_IOPORT_DAC_DATA, pClut->LookupTable[entry].RgbArray.Red);
      NemuVideoCmnPortWriteUchar(VBE_DISPI_IOPORT_DAC_DATA, pClut->LookupTable[entry].RgbArray.Green);
      NemuVideoCmnPortWriteUchar(VBE_DISPI_IOPORT_DAC_DATA, pClut->LookupTable[entry].RgbArray.Blue);
    }
...
BOOLEAN NemuMPSetPointerAttr(PNEMUMP_DEVEXT pExt, PVIDEO_POINTER_ATTRIBUTES pPointerAttrs, uint32_t cbLen, PSTATUS_BLOCK pStatus)
{
    ...
    {
        bRC = NemuMPCmnUpdatePointerShape(NemuCommonFromDeviceExt(pExt), pPointerAttrs, cbLen);
    }
    else
    {
...
BOOLEAN NemuMPEnablePointer(PNEMUMP_DEVEXT pExt, BOOLEAN bEnable, PSTATUS_BLOCK pStatus)
{
    ...
    if (NemuQueryHostWantsAbsolute())
    {
        ...
        if (bEnable == pExt->pPrimary->u.primary.fMouseHidden)
        {
             ...
             */
            attrs.Enable = bEnable ? NEMU_MOUSE_POINTER_VISIBLE:0;


            /* Pass info to the host. */
            bRC = NemuMPCmnUpdatePointerShape(NemuCommonFromDeviceExt(pExt), &attrs, sizeof(attrs));
...
BOOLEAN NemuMPQueryPointerPosition(PNEMUMP_DEVEXT pExt, PVIDEO_POINTER_POSITION pPos, PSTATUS_BLOCK pStatus)
{
    ...
    if (NemuQueryPointerPos(&PosX, &PosY))
    {
        PVIDEO_MODE_INFORMATION pMode = NemuMPXpdmCurrentVideoMode(pExt);
...
BOOLEAN NemuMPVBVAEnable(PNEMUMP_DEVEXT pExt, BOOLEAN bEnable, VBVAENABLERESULT *pResult, PSTATUS_BLOCK pStatus)
{
    ...
    rc = NemuVbvaEnable(pExt, bEnable, pResult);
...
/* Called for IOCTL_VIDEO_NEMU_SETVISIBLEREGION.
...
BOOLEAN NemuMPHGSMIQueryPortProcs(PNEMUMP_DEVEXT pExt, HGSMIQUERYCPORTPROCS *pProcs, PSTATUS_BLOCK pStatus)
{
    ...
    if (NemuCommonFromDeviceExt(pExt)->bHGSMI)
    {
...
BOOLEAN NemuMPHGSMIQueryCallbacks(PNEMUMP_DEVEXT pExt, HGSMIQUERYCALLBACKS *pCallbacks, PSTATUS_BLOCK pStatus)
{
    ...
    if (NemuCommonFromDeviceExt(pExt)->bHGSMI)
    {
        pCallbacks->hContext = NemuCommonFromDeviceExt(pExt);
        pCallbacks->pfnCompletionHandler = NemuMPHGSMIHostCmdCompleteCB;
        pCallbacks->pfnRequestCommandsHandler = NemuMPHGSMIHostCmdRequestCB;
...
BOOLEAN NemuMPQueryHgsmiInfo(PNEMUMP_DEVEXT pExt, QUERYHGSMIRESULT *pResult, PSTATUS_BLOCK pStatus)
{
    ...
    if (NemuCommonFromDeviceExt(pExt)->bHGSMI)
    {
        ...
        pResult->IOPortGuestCommand = NemuCommonFromDeviceExt(pExt)->guestCtx.port;
...
BOOLEAN NemuMPHgsmiHandlerEnable(PNEMUMP_DEVEXT pExt, HGSMIHANDLERENABLE *pChannel, PSTATUS_BLOCK pStatus)
{
    ...
    if (NemuCommonFromDeviceExt(pExt)->bHGSMI)
    {
        int rc = NemuVbvaChannelDisplayEnable(NemuCommonFromDeviceExt(pExt), pExt->iDevice, pChannel->u8Channel);
        if (RT_FAILURE(rc))
        {
...
BOOLEAN NemuMPQueryRegistryFlags(PNEMUMP_DEVEXT pExt, ULONG *pulFlags, PSTATUS_BLOCK pStatus)
{
    ...
    int rc = NemuMPCmnRegInit(pExt, &Registry);
    NEMUMP_WARN_VPS_NOBP(rc);

    if (rc == NO_ERROR)
    {
        uint32_t u32Flags = 0;
        rc = NemuMPCmnRegQueryDword(Registry, L"NemuVideoFlags", &u32Flags);
        NEMUMP_WARN_VPS_NOBP(rc);
        if (rc != NO_ERROR)
        {
            u32Flags = 0;
