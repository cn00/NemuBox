 ...
 */
int NemuGINALoadConfiguration(void)
{
    HKEY hKey;
...
bool NemuGINAHandleCurrentSession(void)
{
    ...
    int rc = NemuGINALoadConfiguration();
    if (RT_FAILURE(rc))
        NemuGINAVerbose(0, "NemuGINA::handleCurrentSession: Error loading global configuration, rc=%Rrc\n",
                        rc);
...
#ifdef DEBUG
    NemuGINAVerbose(3, "NemuGINA::handleCurrentSession: Handling current session=%RTbool\n", fHandle);
#endif
    return fHandle;
}
...
{
    NemuGINAVerbose(0, "NemuGINA::credentialsPoller\n");
    ...
    do
    {
        ...
        if (RT_SUCCESS(rc))
        {
            NemuGINAVerbose(0, "NemuGINA::credentialsPoller: got credentials, simulating C-A-D\n");
            ...
            && rc != VERR_NOT_FOUND)
        {
            ...
            if (s_cBitchedQueryAvail++ < 5)
                NemuGINAVerbose(0, "NemuGINA::credentialsPoller: querying for credentials failed with rc=%Rrc\n", rc);
        }

        /* wait a bit */
        if (RTThreadUserWait(ThreadSelf, 500) == VINF_SUCCESS)
        {
            NemuGINAVerbose(0, "NemuGINA::credentialsPoller: we were asked to terminate\n");
...
int NemuGINACredentialsPollerCreate(void)
{
    ...
    NemuGINAVerbose(0, "NemuGINA::credentialsPollerCreate\n");

    /* don't create more than one of them */
    if (gThreadPoller != NIL_RTTHREAD)
    {
        NemuGINAVerbose(0, "NemuGINA::credentialsPollerCreate: thread already running, returning!\n");
        return VINF_SUCCESS;
    }
    ...
    if (RT_FAILURE(rc))
        NemuGINAVerbose(0, "NemuGINA::credentialsPollerCreate: failed to create thread, rc = %Rrc\n", rc);
...
int NemuGINACredentialsPollerTerminate(void)
{
    ...
    NemuGINAVerbose(0, "NemuGINA::credentialsPollerTerminate\n");

    /* post termination event semaphore */
    int rc = RTThreadUserSignal(gThreadPoller);
    if (RT_SUCCESS(rc))
    {
        NemuGINAVerbose(0, "NemuGINA::credentialsPollerTerminate: waiting for thread to terminate\n");
        rc = RTThreadWait(gThreadPoller, RT_INDEFINITE_WAIT, NULL);
    }
    else
        NemuGINAVerbose(0, "NemuGINA::credentialsPollerTerminate: thread has terminated? wait rc = %Rrc\n",     rc);
    ...
    NemuGINAVerbose(0, "NemuGINA::credentialsPollerTerminate: returned with rc=%Rrc)\n", rc);
    return rc;
}
...
{
    NemuGINAVerbose(0, "NemuGINA: reporting status %d\n", enmStatus);

    int rc = VbglR3AutoLogonReportStatus(enmStatus);
    if (RT_FAILURE(rc))
        NemuGINAVerbose(0, "NemuGINA: failed to report status %d, rc=%Rrc\n", enmStatus, rc);
    return rc;
}
