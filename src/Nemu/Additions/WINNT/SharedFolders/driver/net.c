...
NTSTATUS NemuMRxUpdateNetRootState(IN OUT PMRX_NET_ROOT pNetRoot)
{
    Log(("NEMUSF: MRxUpdateNetRootState\n"));
    return STATUS_NOT_IMPLEMENTED;
}

static void vbsfUpdateNetRoot(PMRX_NET_ROOT pNetRoot)
{
    Log(("NEMUSF: vbsfUpdateNetRoot: NetRoot = 0x%x Type = 0x%x\n",
    ...
    switch (pNetRoot->Type)
    {
        ...
        case NET_ROOT_WILD:
             ...
             * This NetRoot has the same remote path (\\nemusrv\dir) as other NetRoots,
            ...
            break;
        default:
            AssertMsgFailed(("NEMUSF: vbsfUpdateNetRoot: Invalid net root type! Type = 0x%x\n",
                             pNetRoot->Type));
            break;
    }

    Log(("NEMUSF: vbsfUpdateNetRoot: leaving pNetRoot->DeviceType = 0x%x\n",
         pNetRoot->DeviceType));
}

NTSTATUS NemuMRxCreateVNetRoot(IN PMRX_CREATENETROOT_CONTEXT pCreateNetRootContext)
{
    NTSTATUS Status;

    PMRX_V_NET_ROOT pVNetRoot = (PMRX_V_NET_ROOT)pCreateNetRootContext->pVNetRoot;

    PMRX_NEMU_DEVICE_EXTENSION pDeviceExtension = NemuMRxGetDeviceExtension(pCreateNetRootContext->RxContext);
    PMRX_NEMU_NETROOT_EXTENSION pNetRootExtension = NemuMRxGetNetRootExtension(pVNetRoot->pNetRoot);
    ...
    Log(("NEMUSF: MRxCreateVNetRoot: pNetRoot = %p, pNetRootExtension = %p, name = [%.*ls]\n",
    ...
    {
        /* NemuSF claims everything which starts with '\nemusrv'.
         *
         * So sometimes the system tries to open \nemusrv\ipc$ pipe for DFS
        ...
        if (pNetRoot->pNetRootName->Length >= 13 * sizeof (WCHAR)) /* Number of bytes in '\nemusrv\ipc$' unicode string. */
        {
            const WCHAR *Suffix = &pNetRoot->pNetRootName->Buffer[8]; /* Number of chars in '\nemusrv' */
               ...
               )
            {
                ...
                {
                    /* It is '\nemusrv\IPC$[\*]'. */
                    Log(("NEMUSF: MRxCreateVNetRoot: IPC$\n"));
        ...
        Log(("NEMUSF: MRxCreateVNetRoot: Pipe open not supported!\n"));
        Status = STATUS_NOT_SUPPORTED;
        goto l_Exit;
    }
    else if (pNetRoot->Type == NET_ROOT_MAILSLOT)
    {
        Log(("NEMUSF: MRxCreateVNetRoot: Mailslot open not supported!\n"));
        pVNetRoot->Context = NULL;
        Status = STATUS_NOT_SUPPORTED;
        goto l_Exit;
    }
    ...
    if (!pNetRoot->Context)
    {
        /* MRxNetRootSize is not zero in NemuSF, so it is expected
         * that the Context, which is NetRootExtension, is already allocated.
         */
        Log(("NEMUSF: MRxCreateVNetRoot: NULL netroot context\n"));
        pVNetRoot->Context = NULL;
        Status = STATUS_NOT_SUPPORTED;
        goto l_Exit;
    }
    ...
    if (fInitializeNetRoot)
    {
        ...
        Log(("NEMUSF: MRxCreateVNetRoot: initialize NET_ROOT\n"));
        ...
        if (RootNameLength < sizeof(WCHAR))
        {
            ...
            Log(("NEMUSF: MRxCreateVNetRoot: Empty shared folder name!\n"));
        ...
        {
            Log(("NEMUSF: MRxCreateVNetRoot: Initialize netroot length = %d, name = %.*ls\n",
            ...
            nemuRC = VbglR0SfMapFolder(&pDeviceExtension->hgcmClient, ParsedPath, &pNetRootExtension->map);
            vbsfFreeNonPagedMem(ParsedPath);
            if (nemuRC != VINF_SUCCESS)
            {
                Log(("NEMUSF: MRxCreateVNetRoot: VbglR0SfMapFolder failed with %d\n", nemuRC));
                Status = STATUS_BAD_NETWORK_NAME;
            }
            else
            {
        ...
        Log(("NEMUSF: MRxCreateVNetRoot: Creating V_NET_ROOT on existing NET_ROOT!\n"));
...
l_Exit:
    ...
    {
        Log(("NEMUSF: MRxCreateVNetRoot: Returning 0x%08X\n", Status));
    ...
    Log(("NEMUSF: MRxCreateVNetRoot: Returned STATUS_PENDING\n"));
    return Status;
}

NTSTATUS NemuMRxFinalizeVNetRoot(IN PMRX_V_NET_ROOT pVNetRoot,
                                 IN PBOOLEAN ForceDisconnect)
{
    Log(("NEMUSF: MRxFinalizeVNetRoot: V_NET_ROOT %p, NET_ROOT %p\n",
         pVNetRoot, pVNetRoot->pNetRoot));

    return STATUS_SUCCESS;
}

NTSTATUS NemuMRxFinalizeNetRoot(IN PMRX_NET_ROOT pNetRoot,
                                IN PBOOLEAN ForceDisconnect)
{
    PMRX_NEMU_NETROOT_EXTENSION pNetRootExtension = NemuMRxGetNetRootExtension(pNetRoot);

    Log(("NEMUSF: MRxFinalizeNetRoot: NET_ROOT %p\n", pNetRoot));

    if (pNetRootExtension->phgcmClient)
    {
        int nemuRC = VbglR0SfUnmapFolder(pNetRootExtension->phgcmClient, &pNetRootExtension->map);
        if (nemuRC != VINF_SUCCESS)
            Log(("NEMUSF: MRxFinalizeVNetRoot: VbglR0SfUnmapFolder failed with %d\n",
                 nemuRC));
                               ...
                               OUT PUNICODE_STRING RestOfName OPTIONAL)
{
    ...
    /* Split "\nemusvr\share\path" to
     * NetRootName = "\share"
     * RestOfName = "\path"
     *
     * Note that SrvCall->pSrvCallName contains "\nemusrv".
     */

    Log(("NEMUSF: MRxExtractNetRootName: [%.*ls], RestOfName %p\n",
    ...
    Log(("NEMUSF: MRxExtractNetRootName: cChars %d, iNetRoot %d, iRest %d\n",
         cChars, iNetRoot, i));
    ...
    Log(("NEMUSF: MRxExtractNetRootName: Srv = %.*ls, Root = %.*ls\n",
    ...
    if (RestOfName)
    {
        ...
        Log(("NEMUSF: MRxExtractNetRootName: Rest = %.*ls\n",
...
static VOID vbsfExecuteCreateSrvCall(PMRX_SRVCALL_CALLBACK_CONTEXT pCallbackContext)
{
    ...
    /* Validate the server name with the test name of 'nemusvr'. */
    Log(("NEMUSF: vbsfExecuteCreateSrvCall: Connection Name %.*ls Length: %d, pSrvCall = %p\n",
    ...
    {
        Log(("NEMUSF: vbsfExecuteCreateSrvCall: Principal name = %.*ls\n",
    ...
    {
        Log(("NEMUSF: vbsfExecuteCreateSrvCall: Domain name = %.*ls\n",
    ...
    if (Verifier)
    {
        Log(("NEMUSF: vbsfExecuteCreateSrvCall: Verifier succeeded!\n"));
        Status = STATUS_SUCCESS;
    }
    else
    {
        Log(("NEMUSF: vbsfExecuteCreateSrvCall: Verifier failed!\n"));
                              ...
                              PMRX_SRVCALL_CALLBACK_CONTEXT pCallbackContext)
{
    ...
    Log(("NEMUSF: MRxCreateSrvCall: %p.\n", pSrvCall));

    if (IoGetCurrentProcess() == RxGetRDBSSProcess())
    {
        Log(("NEMUSF: MRxCreateSrvCall: Called in context of RDBSS process\n"));
    ...
    else
    {
        ...
        Log(("NEMUSF: MRxCreateSrvCall: Dispatching to worker thread\n"));

        Status = RxDispatchToWorkerThread(NemuMRxDeviceObject, DelayedWorkQueue,
                                          (PWORKER_THREAD_ROUTINE)vbsfExecuteCreateSrvCall,
                                          pCallbackContext);

        if (Status == STATUS_SUCCESS)
            Log(("NEMUSF: MRxCreateSrvCall: queued\n"));
        else
        {
...
NTSTATUS NemuMRxFinalizeSrvCall (PMRX_SRV_CALL pSrvCall,
                                 BOOLEAN Force)
{
    Log(("NEMUSF: MRxFinalizeSrvCall %p, ctx = %p.\n", pSrvCall, pSrvCall->Context));
...
NTSTATUS NemuMRxSrvCallWinnerNotify(IN PMRX_SRV_CALL pSrvCall,
                                    IN BOOLEAN ThisMinirdrIsTheWinner,
                                    IN OUT PVOID pSrvCallContext)
{
    NTSTATUS Status = STATUS_SUCCESS;

    Log(("NEMUSF: MRxSrvCallWinnerNotify: pSrvCall %p, pSrvCall->Ctx %p, winner %d, context %p\n",
