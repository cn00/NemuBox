...
/** @file
 * Nemu Qt GUI - UIMachineWindow class implementation.
 */
    ...
    , m_pRightSpacer(0)
{
    ...
    /* Or set default machine-window icon: */
        ...
        setWindowIcon(nemuGlobal().vmGuestOSTypeIcon(machine().GetOSTypeId()));
#endif /* !Q_WS_MAC */
}
...
void UIMachineWindow::closeEvent(QCloseEvent *pCloseEvent)
{
    ...
    const QString strScript = gEDataManager->machineCloseHookScript(nemuGlobal().managedVMUuid());
    ...
    if (closeAction == MachineCloseAction_Invalid)
    {
        ...
        {
            /* Else user misconfigured .nemu file, we will reject closing UI: */
            closeAction = MachineCloseAction_Invalid;
        }
...
void UIMachineWindow::prepareMachineView()
{
    ...
    bool bAccelerate2DVideo = machine().GetAccelerate2DVideoEnabled() && NemuGlobal::isAcceleration2DVideoAvailable();
