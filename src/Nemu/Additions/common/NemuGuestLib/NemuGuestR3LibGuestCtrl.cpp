/* $Id: NemuGuestR3LibGuestCtrl.cpp $ */
/** @file
 * NemuGuestR3Lib - Ring-3 Support Library for VirtualBox guest additions, guest control.
 */
...
VBGLR3DECL(int) VbglR3GuestCtrlConnect(uint32_t *puClientId)
{
    ...
    RT_ZERO(Info.Loc.u);
    strcpy(Info.Loc.u.host.achName, "NemuGuestControlSvc");
                                              ...
                                              uint32_t *pfFlags,     uint32_t *pidSession)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        else
        {
            ...
            if (pidSession)
                *pidSession = NEMU_GUESTCTRL_CONTEXTID_GET_SESSION(pCtx->uContextID);
        }
    }
