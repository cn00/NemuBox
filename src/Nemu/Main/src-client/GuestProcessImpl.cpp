...
public:
    ...
    STDMETHOD(HandleEvent)(NemuEventType_T aType, IEvent *aEvent)
    {
        switch (aType)
        {
            case NemuEventType_OnGuestProcessStateChanged:
            case NemuEventType_OnGuestProcessInputNotify:
            case NemuEventType_OnGuestProcessOutput:
            {
...
NEMU_LISTENER_DECLARE(GuestProcessListenerImpl)
    ...
    if (RT_SUCCESS(vrc))
    {
        try
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                eventTypes.push_back(NemuEventType_OnGuestProcessStateChanged);
                eventTypes.push_back(NemuEventType_OnGuestProcessInputNotify);
                eventTypes.push_back(NemuEventType_OnGuestProcessOutput);
...
{
#ifndef NEMU_WTIH_GUEST_CONTROL
...
int GuestProcess::i_callbackDispatcher(PNEMUGUESTCTRLHOSTCBCTX pCbCtx, PNEMUGUESTCTRLHOSTCALLBACK pSvcCb)
{
...
int GuestProcess::i_onGuestDisconnected(PNEMUGUESTCTRLHOSTCBCTX pCbCtx, PNEMUGUESTCTRLHOSTCALLBACK pSvcCbData)
{
    AssertPtrReturn(pCbCtx, VERR_INVALID_POINTER);
...
int GuestProcess::i_onProcessInputStatus(PNEMUGUESTCTRLHOSTCBCTX pCbCtx, PNEMUGUESTCTRLHOSTCALLBACK pSvcCbData)
{
    AssertPtrReturn(pCbCtx, VERR_INVALID_POINTER);
    AssertPtrReturn(pSvcCbData, VERR_INVALID_POINTER);
...
int GuestProcess::i_onProcessNotifyIO(PNEMUGUESTCTRLHOSTCBCTX pCbCtx, PNEMUGUESTCTRLHOSTCALLBACK pSvcCbData)
{
    AssertPtrReturn(pCbCtx, VERR_INVALID_POINTER);
    AssertPtrReturn(pSvcCbData, VERR_INVALID_POINTER);
...
int GuestProcess::i_onProcessStatusChange(PNEMUGUESTCTRLHOSTCBCTX pCbCtx, PNEMUGUESTCTRLHOSTCALLBACK pSvcCbData)
{
    AssertPtrReturn(pCbCtx, VERR_INVALID_POINTER);
    AssertPtrReturn(pSvcCbData, VERR_INVALID_POINTER);
...
int GuestProcess::i_onProcessOutput(PNEMUGUESTCTRLHOSTCBCTX pCbCtx, PNEMUGUESTCTRLHOSTCALLBACK pSvcCbData)
{
    AssertPtrReturn(pSvcCbData, VERR_INVALID_POINTER);
...
int GuestProcess::i_setProcessStatus(ProcessStatus_T procStatus, int procRc)
{
    ...
    if (mData.mStatus != procStatus) /* Was there a process status change? */
    {
        ...
        {
            hr = errorInfo->initEx(NEMU_E_IPRT_ERROR, mData.mLastError,
...
HRESULT GuestProcess::i_setErrorExternal(VirtualBoxBase *pInterface, int guestRc)
{
    ...
    return pInterface->setError(NEMU_E_IPRT_ERROR, GuestProcess::i_guestErrorToString(guestRc).c_str());
}
...
int GuestProcess::i_startProcess(uint32_t uTimeoutMS, int *pGuestRc)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        ...
        if (uProtocol < 2)
        {
            /* In protocol v1 (Nemu < 4.3) the credentials were part of the execution
...
int GuestProcess::i_terminateProcess(uint32_t uTimeoutMS, int *pGuestRc)
{
    ...
    {
        AssertPtr(mSession);
        /* Note: Nemu < 4.3 (aka protocol version 1) does not
                                       ...
                                       ProcessInputStatus_T *pInputStatus, uint32_t *pcbProcessed)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        if (evtType == NemuEventType_OnGuestProcessInputNotify)
        {
                                  ...
                                  void *pvData, size_t cbData, uint32_t *pcbRead)
{
    ...
    do
    {
        ...
        if (RT_SUCCESS(vrc))
        {
            if (evtType == NemuEventType_OnGuestProcessOutput)
            {
                                        ...
                                        ProcessStatus_T *pProcessStatus, int *pGuestRc)
{
    ...
    if (RT_SUCCESS(vrc))
    {
        Assert(evtType == NemuEventType_OnGuestProcessStateChanged);
