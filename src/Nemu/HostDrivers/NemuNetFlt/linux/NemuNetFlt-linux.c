/* $Id: NemuNetFlt-linux.c $ */
/** @file
 * NemuNetFlt - Network Filter Driver (Host), Linux Specific Code.
 */
...
#define NEMUNETFLT_LINUX_NO_XMIT_QUEUE
...
#include <Nemu/vmm/pdmnetinline.h>
...
typedef struct NEMUNETFLTNOTIFIER {
    struct notifier_block Notifier;
    PNEMUNETFLTINS pThis;
} NEMUNETFLTNOTIFIER;
typedef struct NEMUNETFLTNOTIFIER *PNEMUNETFLTNOTIFIER;


/*********************************************************************************************************************************
*   Defined Constants And Macros                                                                                                 *
*********************************************************************************************************************************/
#define NEMU_FLT_NB_TO_INST(pNB)    RT_FROM_MEMBER(pNB, NEMUNETFLTINS, u.s.Notifier)
#define NEMU_FLT_PT_TO_INST(pPT)    RT_FROM_MEMBER(pPT, NEMUNETFLTINS, u.s.PacketType)
#ifndef NEMUNETFLT_LINUX_NO_XMIT_QUEUE
# define NEMU_FLT_XT_TO_INST(pXT)   RT_FROM_MEMBER(pXT, NEMUNETFLTINS, u.s.XmitTask)
#endif

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 11, 0)
# define NEMU_NETDEV_NOTIFIER_INFO_TO_DEV(ptr) netdev_notifier_info_to_dev(ptr)
#else
# define NEMU_NETDEV_NOTIFIER_INFO_TO_DEV(ptr) ((struct net_device *)ptr)
#endif

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 34)
# define NEMU_NETDEV_NAME(dev)              netdev_name(dev)
#else
# define NEMU_NETDEV_NAME(dev)              ((dev)->reg_state != NETREG_REGISTERED ? "(unregistered net_device)" : (dev)->name)
#endif

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 25)
# define NEMU_IPV4_IS_LOOPBACK(addr)        ipv4_is_loopback(addr)
# define NEMU_IPV4_IS_LINKLOCAL_169(addr)   ipv4_is_linklocal_169(addr)
#else
# define NEMU_IPV4_IS_LOOPBACK(addr)        ((addr & htonl(IN_CLASSA_NET)) == htonl(0x7f000000))
# define NEMU_IPV4_IS_LINKLOCAL_169(addr)   ((addr & htonl(IN_CLASSB_NET)) == htonl(0xa9fe0000))
#endif

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 22)
# define NEMU_SKB_RESET_NETWORK_HDR(skb)    skb_reset_network_header(skb)
# define NEMU_SKB_RESET_MAC_HDR(skb)        skb_reset_mac_header(skb)
#else
# define NEMU_SKB_RESET_NETWORK_HDR(skb)    skb->nh.raw = skb->data
# define NEMU_SKB_RESET_MAC_HDR(skb)        skb->mac.raw = skb->data
#endif

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 19)
# define NEMU_SKB_CHECKSUM_HELP(skb)        skb_checksum_help(skb)
#else
# define CHECKSUM_PARTIAL                   CHECKSUM_HW
# if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 10)
#  define NEMU_SKB_CHECKSUM_HELP(skb)       skb_checksum_help(skb, 0)
# else
...
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static int      NemuNetFltLinuxInit(void);
static void     NemuNetFltLinuxUnload(void);
static void     nemuNetFltLinuxForwardToIntNet(PNEMUNETFLTINS pThis, struct sk_buff *pBuf);
...
module_init(NemuNetFltLinuxInit);
module_exit(NemuNetFltLinuxUnload);

MODULE_AUTHOR(NEMU_VENDOR);
MODULE_DESCRIPTION(NEMU_PRODUCT " Network Filter Driver");
MODULE_LICENSE("GPL");
 ...
 */
static int __init NemuNetFltLinuxInit(void)
{
    int rc;
    /*
     * Initialize IPRT.
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        rc = nemuNetFltInitGlobalsAndIdc(&g_NemuNetFltGlobals);
        if (RT_SUCCESS(rc))
        {
            LogRel(("NemuNetFlt: Successfully started.\n"));
            return 0;
        }

        LogRel(("NemuNetFlt: failed to initialize device extension (rc=%d)\n", rc));
        RTR0Term();
    }
    else
        LogRel(("NemuNetFlt: failed to initialize IPRT (rc=%d)\n", rc));
 ...
 */
static void __exit NemuNetFltLinuxUnload(void)
{
    int rc;
    Log(("NemuNetFltLinuxUnload\n"));
    Assert(nemuNetFltCanUnload(&g_NemuNetFltGlobals));

    /*
     * Undo the work done during start (in reverse order).
     */
    rc = nemuNetFltTryDeleteIdcAndGlobals(&g_NemuNetFltGlobals);
    ...
    Log(("NemuNetFltLinuxUnload - done\n"));
}
 ...
 */
#define NEMUNETFLT_WITH_HOST2WIRE_FILTER

#ifdef NEMUNETFLT_WITH_HOST2WIRE_FILTER
 ...
 */
typedef struct NemuNetDeviceOpsOverride
{
    /** Our overridden ops. */
    OVR_OPSTYPE                     Ops;
    /** Magic word. */
...
# endif /* LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 29) */
    ...
    PNEMUNETFLTINS                  pNemuNetFlt;
    /** The number of filtered packages. */
    uint64_t                        cFiltered;
    /** The total number of packets */
    uint64_t                        cTotal;
} NEMUNETDEVICEOPSOVERRIDE, *PNEMUNETDEVICEOPSOVERRIDE;
/** NEMUNETDEVICEOPSOVERRIDE::u32Magic value. */
#define NEMUNETDEVICEOPSOVERRIDE_MAGIC  UINT32_C(0x00c0ffee)
 ...
 */
static int nemuNetFltLinuxStartXmitFilter(struct sk_buff *pSkb, struct net_device *pDev)
{
    PNEMUNETDEVICEOPSOVERRIDE   pOverride = (PNEMUNETDEVICEOPSOVERRIDE)pDev->OVR_OPS;
     ...
     *
     * Note! We're racing nemuNetFltLinuxUnhookDev here.  If this was supposed
     *       to be production quality code, we would have to be much more
     *       careful here and avoid the race.
     */
    if (   !VALID_PTR(pOverride)
        || pOverride->u32Magic != NEMUNETDEVICEOPSOVERRIDE_MAGIC
# if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 29)
        || !VALID_PTR(pOverride->pOrgOps)
# endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 29) */
        )
    {
        printk("nemuNetFltLinuxStartXmitFilter: bad override %p\n", pOverride);
        dev_kfree_skb(pSkb);
        return NETDEV_TX_OK;
    }
    pOverride->cTotal++;
    ...
    if (   pEtherHdr
        && VALID_PTR(pOverride->pNemuNetFlt)
        && (pSwitchPort = pOverride->pNemuNetFlt->pSwitchPort) != NULL
 ...
 * @param   pDev                The net device.
 */
static void nemuNetFltLinuxHookDev(PNEMUNETFLTINS pThis, struct net_device *pDev)
{
    PNEMUNETDEVICEOPSOVERRIDE   pOverride;
...
# else /* LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 29) */
    pOverride->Ops.ndo_start_xmit   = nemuNetFltLinuxStartXmitFilter;
# endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 29) */
    pOverride->u32Magic             = NEMUNETDEVICEOPSOVERRIDE_MAGIC;
    pOverride->cTotal               = 0;
    pOverride->cFiltered            = 0;
    pOverride->pNemuNetFlt          = pThis;

    RTSpinlockAcquire(pThis->hSpinlock); /* (this isn't necessary, but so what) */
    ASMAtomicWritePtr((void * volatile *)&pDev->OVR_OPS, pOverride);
# if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 29)
    ASMAtomicXchgPtr((void * volatile *)&pDev->hard_start_xmit, nemuNetFltLinuxStartXmitFilter);
# endif /* LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 29) */
    RTSpinlockRelease(pThis->hSpinlock);
}

/**
 * Undos what nemuNetFltLinuxHookDev did.
 *
 * @param   pThis               The net filter instance.
 * @param   pDev                The net device.  Can be NULL, in which case
 *                              we'll try retrieve it from @a pThis.
 */
static void nemuNetFltLinuxUnhookDev(PNEMUNETFLTINS pThis, struct net_device *pDev)
{
    PNEMUNETDEVICEOPSOVERRIDE   pOverride;
    ...
    if (VALID_PTR(pDev))
    {
        pOverride = (PNEMUNETDEVICEOPSOVERRIDE)pDev->OVR_OPS;
        if (    VALID_PTR(pOverride)
            &&  pOverride->u32Magic == NEMUNETDEVICEOPSOVERRIDE_MAGIC
            &&  VALID_PTR(pOverride->pOrgOps)
           )
...
# endif /* LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 29) */
    ...
    if (pOverride)
    {
        printk("nemunetflt: %llu out of %llu packets were not sent (directed to host)\n", pOverride->cFiltered, pOverride->cTotal);
        RTMemFree(pOverride);
    }
}

#endif /* NEMUNETFLT_WITH_HOST2WIRE_FILTER */
 ...
 * @param   pThis
 */
DECLINLINE(struct net_device *) nemuNetFltLinuxRetainNetDev(PNEMUNETFLTINS pThis)
{
#if 0
    struct net_device *pDev = NULL;

    Log(("nemuNetFltLinuxRetainNetDev\n"));
    /*
    ...
    if (!ASMAtomicUoReadBool(&pThis->fDisconnectedFromHost))
    {
        ...
        {
            dev_hold(pDev);
            Log(("nemuNetFltLinuxRetainNetDev: Device %p(%s) retained. ref=%d\n",
                 pDev, pDev->name,
    ...
    Log(("nemuNetFltLinuxRetainNetDev - done\n"));
    return pDev;
#else
 ...
 * by nemuNetFltLinuxRetainNetDev.
 *
 * @param   pThis           The instance.
 * @param   pDev            The nemuNetFltLinuxRetainNetDev
 *                          return value, NULL is fine.
 */
DECLINLINE(void) nemuNetFltLinuxReleaseNetDev(PNEMUNETFLTINS pThis, struct net_device *pDev)
{
#if 0
    Log(("nemuNetFltLinuxReleaseNetDev\n"));
    NOREF(pThis);
    if (pDev)
    {
        dev_put(pDev);
        Log(("nemuNetFltLinuxReleaseNetDev: Device %p(%s) released. ref=%d\n",
             pDev, pDev->name,
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 37)
             netdev_refcnt_read(pDev)
#else
    ...
    Log(("nemuNetFltLinuxReleaseNetDev - done\n"));
#endif
}

#define NEMUNETFLT_CB_TAG(skb) (0xA1C90000 | (skb->dev->ifindex & 0xFFFF))
#define NEMUNETFLT_SKB_TAG(skb) (*(uint32_t*)&((skb)->cb[sizeof((skb)->cb)-sizeof(uint32_t)]))

/**
 * Checks whether this is an mbuf created by nemuNetFltLinuxMBufFromSG,
 * i.e. a buffer which we're pushing and should be ignored by the filter callbacks.
 *
 * @returns true / false accordingly.
 * @param   pBuf            The sk_buff.
 */
DECLINLINE(bool) nemuNetFltLinuxSkBufIsOur(struct sk_buff *pBuf)
{
    return NEMUNETFLT_SKB_TAG(pBuf) == NEMUNETFLT_CB_TAG(pBuf);
}
 ...
 */
DECLINLINE(bool) nemuNetFltLinuxIsGso(PINTNETSG pSG)
{
#if defined(NEMUNETFLT_WITH_GSO_XMIT_WIRE) || defined(NEMUNETFLT_WITH_GSO_XMIT_HOST)
    return !((PDMNETWORKGSOTYPE)pSG->GsoCtx.u8Type == PDMNETWORKGSOTYPE_INVALID);
#else /* !NEMUNETFLT_WITH_GSO_XMIT_WIRE && !NEMUNETFLT_WITH_GSO_XMIT_HOST */
    return false;
#endif /* !NEMUNETFLT_WITH_GSO_XMIT_WIRE && !NEMUNETFLT_WITH_GSO_XMIT_HOST */
}
 ...
 * @param   pSG             The (scatter/)gather list.
 */
DECLINLINE(uint32_t) nemuNetFltLinuxFrameSize(PINTNETSG pSG)
{
    uint16_t u16Type = 0;
    ...
    if (u16Type == RTNET_ETHERTYPE_VLAN)
        cbVlanTag = 4;
    return (nemuNetFltLinuxIsGso(pSG) ? (uint32_t)pSG->GsoCtx.cbMaxSeg + pSG->GsoCtx.cbHdrsTotal : pSG->cbTotal) - cbVlanTag;
}
 ...
 */
static struct sk_buff *nemuNetFltLinuxSkBufFromSG(PNEMUNETFLTINS pThis, PINTNETSG pSG, bool fDstWire)
{
    struct sk_buff *pPkt;
    ...
    if (pSG->cbTotal == 0)
    {
        LogRel(("NemuNetFlt: Dropped empty packet coming from internal network.\n"));
        return NULL;
    }
    Log5(("NemuNetFlt: Packet to %s of %d bytes (frame=%d).\n", fDstWire?"wire":"host", pSG->cbTotal, nemuNetFltLinuxFrameSize(pSG)));
    if (fDstWire && (nemuNetFltLinuxFrameSize(pSG) > ASMAtomicReadU32(&pThis->u.s.cbMtu) + 14))
    {
        static bool s_fOnce = true;
        if (s_fOnce)
        {
            s_fOnce = false;
            printk("NemuNetFlt: Dropped over-sized packet (%d bytes) coming from internal network.\n", nemuNetFltLinuxFrameSize(pSG));
        }
        return NULL;
    }
    ...
    if (RT_UNLIKELY(!pPkt))
    {
        Log(("nemuNetFltLinuxSkBufFromSG: Failed to allocate sk_buff(%u).\n", pSG->cbTotal));
        pSG->pvUserData = NULL;
        return NULL;
    }
    pPkt->dev       = pDev;
...
# endif
    ...
    if (fDstWire)
    {
        NEMU_SKB_RESET_NETWORK_HDR(pPkt);

        /* Restore ethernet header back. */
        skb_push(pPkt, ETH_HLEN); /** @todo VLAN: +4 if VLAN? */
        NEMU_SKB_RESET_MAC_HDR(pPkt);
    }
    NEMUNETFLT_SKB_TAG(pPkt) = NEMUNETFLT_CB_TAG(pPkt);
 ...
 */
DECLINLINE(void) nemuNetFltLinuxSkBufToSG(PNEMUNETFLTINS pThis, struct sk_buff *pBuf, PINTNETSG pSG,
                                          unsigned cSegs, uint32_t fSrc, PCPDMNETWORKGSO pGsoCtx)
{
    ...
    Log4(("nemuNetFltLinuxSkBufToSG: allocated=%d, segments=%d frags=%d next=%p frag_list=%p pkt_type=%x fSrc=%x\n",
          pSG->cSegsAlloc, pSG->cSegsUsed, skb_shinfo(pBuf)->nr_frags, pBuf->next, skb_shinfo(pBuf)->frag_list, pBuf->pkt_type, fSrc));
    for (i = 0; i < pSG->cSegsUsed; i++)
        Log4(("nemuNetFltLinuxSkBufToSG:   #%d: cb=%d pv=%p\n",
...
static int nemuNetFltLinuxPacketHandler(struct sk_buff *pBuf,
...
#endif
{
    PNEMUNETFLTINS pThis;
    struct net_device *pDev;
    LogFlow(("nemuNetFltLinuxPacketHandler: pBuf=%p pSkbDev=%p pPacketType=%p\n",
             pBuf, pSkbDev, pPacketType));
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 18)
    Log3(("nemuNetFltLinuxPacketHandler: skb len=%u data_len=%u truesize=%u next=%p nr_frags=%u gso_size=%u gso_seqs=%u gso_type=%x frag_list=%p pkt_type=%x\n",
...
# if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 22)
    Log4(("nemuNetFltLinuxPacketHandler: packet dump follows:\n%.*Rhxd\n", pBuf->len-pBuf->data_len, skb_mac_header(pBuf)));
# endif
#else
    Log3(("nemuNetFltLinuxPacketHandler: skb len=%u data_len=%u truesize=%u next=%p nr_frags=%u tso_size=%u tso_seqs=%u frag_list=%p pkt_type=%x\n",
    ...
    if (pBuf->pkt_type == PACKET_LOOPBACK)
    {
         ...
         */
        Log2(("nemuNetFltLinuxPacketHandler: dropped loopback packet (cb=%u)\n", pBuf->len));
        dev_kfree_skb(pBuf); /* We must 'consume' all packets we get (@bugref{6539})! */
        return 0;
    }

    pThis = NEMU_FLT_PT_TO_INST(pPacketType);
    pDev = ASMAtomicUoReadPtrT(&pThis->u.s.pDev, struct net_device *);
    if (pDev != pSkbDev)
    {
        Log(("nemuNetFltLinuxPacketHandler: Devices do not match, pThis may be wrong! pThis=%p\n", pThis));
        kfree_skb(pBuf); /* This is a failure, so we use kfree_skb instead of dev_kfree_skb. */
        return 0;
    }

    Log4(("nemuNetFltLinuxPacketHandler: pBuf->cb dump:\n%.*Rhxd\n", sizeof(pBuf->cb), pBuf->cb));
    if (nemuNetFltLinuxSkBufIsOur(pBuf))
    {
        Log2(("nemuNetFltLinuxPacketHandler: got our own sk_buff, drop it.\n"));
        dev_kfree_skb(pBuf);
        return 0;
    }

#ifndef NEMUNETFLT_SG_SUPPORT
    {
        /*
        ...
        if (!pCopy)
        {
            LogRel(("NemuNetFlt: Failed to allocate packet buffer, dropping the packet.\n"));
            return 0;
        }
        pBuf = pCopy;
...
# if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 18)
        Log3(("nemuNetFltLinuxPacketHandler: skb copy len=%u data_len=%u truesize=%u next=%p nr_frags=%u gso_size=%u gso_seqs=%u gso_type=%x frag_list=%p pkt_type=%x\n",
              pBuf->len, pBuf->data_len, pBuf->truesize, pBuf->next, skb_shinfo(pBuf)->nr_frags, skb_shinfo(pBuf)->gso_size, skb_shinfo(pBuf)->gso_segs, skb_shinfo(pBuf)->gso_type, skb_shinfo(pBuf)->frag_list, pBuf->pkt_type));
#  if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 22)
        Log4(("nemuNetFltLinuxPacketHandler: packet dump follows:\n%.*Rhxd\n", pBuf->len-pBuf->data_len, skb_mac_header(pBuf)));
#  endif
# else
        Log3(("nemuNetFltLinuxPacketHandler: skb copy len=%u data_len=%u truesize=%u next=%p nr_frags=%u tso_size=%u tso_seqs=%u frag_list=%p pkt_type=%x\n",
              pBuf->len, pBuf->data_len, pBuf->truesize, pBuf->next, skb_shinfo(pBuf)->nr_frags, skb_shinfo(pBuf)->tso_size, skb_shinfo(pBuf)->tso_segs, skb_shinfo(pBuf)->frag_list, pBuf->pkt_type));
# endif
    }
#endif

#ifdef NEMUNETFLT_LINUX_NO_XMIT_QUEUE
    /* Forward it to the internal network. */
    nemuNetFltLinuxForwardToIntNet(pThis, pBuf);
#else
    ...
    Log4(("nemuNetFltLinuxPacketHandler: scheduled work %p for sk_buff %p\n",
 ...
 */
DECLINLINE(unsigned) nemuNetFltLinuxCalcSGSegments(struct sk_buff *pBuf)
{
#ifdef NEMUNETFLT_SG_SUPPORT
...
#endif
#ifdef PADD_RUNT_FRAMES_FROM_HOST
    /* nemuNetFltLinuxSkBufToSG adds a padding segment if it's a runt. */
    if (pBuf->len < 60)
        cSegs++;
#endif
    return cSegs;
 ...
 * nemuNetFltLinuxSkBufToSG.
 */
static void nemuNetFltLinuxDestroySG(PINTNETSG pSG)
{
#ifdef NEMUNETFLT_SG_SUPPORT
    int i;
 ...
 * Logging helper.
 */
static void nemuNetFltDumpPacket(PINTNETSG pSG, bool fEgress, const char *pszWhere, int iIncrement)
{
    int i, offSeg;
    uint8_t *pInt, *pExt;
    ...
    Log(("NemuNetFlt: (int)%02x:%02x:%02x:%02x:%02x:%02x"
...
#else
# define nemuNetFltDumpPacket(a, b, c, d) do {} while (0)
#endif

#ifdef NEMUNETFLT_WITH_GSO_RECV

/**
 * Worker for nemuNetFltLinuxForwardToIntNet that checks if we can forwards a
 ...
 */
static bool nemuNetFltLinuxCanForwardAsGso(PNEMUNETFLTINS pThis, struct sk_buff *pSkb, uint32_t fSrc,
                                           PPDMNETWORKGSO pGsoCtx)
{
    ...
    {
        Log5(("nemuNetFltLinuxCanForwardAsGso: gso_type=%#x\n", skb_shinfo(pSkb)->gso_type));
        return false;
    }
    if (RT_UNLIKELY(   skb_shinfo(pSkb)->gso_size < 1
                    || pSkb->len > NEMU_MAX_GSO_SIZE ))
    {
        Log5(("nemuNetFltLinuxCanForwardAsGso: gso_size=%#x skb_len=%#x (max=%#x)\n", skb_shinfo(pSkb)->gso_size, pSkb->len, NEMU_MAX_GSO_SIZE));
        return false;
    }
    /*
    ...
    {
        Log5(("nemuNetFltLinuxCanForwardAsGso: fSrc=wire\n"));
#ifdef NEMUNETFLT_WITH_GRO
        /*
        ...
        Log5(("nemuNetFltLinuxCanForwardAsGso: mac_len=%d data=%p mac_header=%p network_header=%p\n",
              pSkb->mac_len, pSkb->data, skb_mac_header(pSkb), skb_network_header(pSkb)));
#else /* !NEMUNETFLT_WITH_GRO */
        /* Older kernels didn't have GRO. */
        return false;
#endif /* !NEMUNETFLT_WITH_GRO */
    }
    else
    {
        /*
    ...
    switch (uEtherType)
    {
        case RT_H2N_U16_C(RTNET_ETHERTYPE_IPV4):
        {
            ...
            {
                Log5(("nemuNetFltLinuxCanForwardAsGso: failed to access IPv4 hdr\n"));
                return false;
            }
            ...
            if (RT_UNLIKELY(   cbHdr < RTNETIPV4_MIN_LEN
                            || cbHdr > cbTransport ))
            {
                Log5(("nemuNetFltLinuxCanForwardAsGso: invalid IPv4 lengths: ip_hl=%u ip_len=%u\n", pIPv4->ip_hl, RT_N2H_U16(pIPv4->ip_len)));
                return false;
            }
            cbTransport -= cbHdr;
        ...
        case RT_H2N_U16_C(RTNET_ETHERTYPE_IPV6):
        {
            ...
            {
                Log5(("nemuNetFltLinuxCanForwardAsGso: failed to access IPv6 hdr\n"));
                return false;
            }
        ...
        default:
            Log5(("nemuNetFltLinuxCanForwardAsGso: uEtherType=%#x\n", RT_H2N_U16(uEtherType)));
            return false;
    }

    if (enmGsoType == PDMNETWORKGSOTYPE_INVALID)
    {
        Log5(("nemuNetFltLinuxCanForwardAsGso: Unsupported protocol %d\n", uProtocol));
        return false;
    }
    ...
    {
        Log5(("nemuNetFltLinuxCanForwardAsGso: Bad transport length; off=%#x + cb=%#x => %#x; skb_len=%#x (%s)\n",
    ...
    if (uProtocol == RTNETIPV4_PROT_TCP)
    {
        ...
        if (RT_UNLIKELY(!pTcp))
        {
            Log5(("nemuNetFltLinuxCanForwardAsGso: failed to access TCP hdr\n"));
            return false;
        }
        ...
        {
            Log5(("nemuNetFltLinuxCanForwardAsGso: No space for TCP header; off=%#x cb=%#x skb_len=%#x\n", offTransport, cbTransportHdr, pSkb->len));
            return false;
        }
    ...
    else
    {
        ...
        if (RT_UNLIKELY(   offTransport + cbTransportHdr >= UINT8_MAX
                        || offTransport + cbTransportHdr >= pSkb->len ))
        {
            Log5(("nemuNetFltLinuxCanForwardAsGso: No space for UDP header; off=%#x skb_len=%#x\n", offTransport, pSkb->len));
            return false;
        }
    }
 ...
 * @param   pGsoCtx             Where to return the GSO context on success.
 */
static int nemuNetFltLinuxForwardAsGso(PNEMUNETFLTINS pThis, struct sk_buff *pSkb, uint32_t fSrc, PCPDMNETWORKGSO pGsoCtx)
{
    int         rc;
    unsigned    cSegs = nemuNetFltLinuxCalcSGSegments(pSkb);
    if (RT_LIKELY(cSegs <= MAX_SKB_FRAGS + 1))
    {
        PINTNETSG pSG = (PINTNETSG)alloca(RT_OFFSETOF(INTNETSG, aSegs[cSegs]));
        if (RT_LIKELY(pSG))
        {
            nemuNetFltLinuxSkBufToSG(pThis, pSkb, pSG, cSegs, fSrc, pGsoCtx);

            nemuNetFltDumpPacket(pSG, false, (fSrc & INTNETTRUNKDIR_HOST) ? "host" : "wire", 1);
            pThis->pSwitchPort->pfnRecv(pThis->pSwitchPort, NULL /* pvIf */, pSG, fSrc);

            nemuNetFltLinuxDestroySG(pSG);
            rc = VINF_SUCCESS;
        }
        else
        {
            Log(("NemuNetFlt: Dropping the sk_buff (failure case).\n"));
            rc = VERR_NO_MEMORY;
        }
    }
    else
    {
        Log(("NemuNetFlt: Bad sk_buff? cSegs=%#x.\n", cSegs));
        rc = VERR_INTERNAL_ERROR_3;
    }

    Log4(("NemuNetFlt: Dropping the sk_buff.\n"));
    dev_kfree_skb(pSkb);
    return rc;
}

#endif /* NEMUNETFLT_WITH_GSO_RECV */

/**
 * Worker for nemuNetFltLinuxForwardToIntNet.
 *
 ...
 * @param   fSrc                The source.
 */
static int nemuNetFltLinuxForwardSegment(PNEMUNETFLTINS pThis, struct sk_buff *pBuf, uint32_t fSrc)
{
    int         rc;
    unsigned    cSegs = nemuNetFltLinuxCalcSGSegments(pBuf);
    if (cSegs <= MAX_SKB_FRAGS + 1)
    {
        ...
        if (RT_LIKELY(pSG))
        {
            ...
            nemuNetFltLinuxSkBufToSG(pThis, pBuf, pSG, cSegs, fSrc, NULL /*pGsoCtx*/);
        ...
        {
            Log(("NemuNetFlt: Failed to allocate SG buffer.\n"));
            rc = VERR_NO_MEMORY;
        }
    }
    else
...
#ifdef NEMUNETFLT_WITH_GSO
    if (skb_is_gso(pBuf))
    {
        PDMNETWORKGSO GsoCtx;
        Log3(("nemuNetFltLinuxForwardToIntNet: skb len=%u data_len=%u truesize=%u next=%p nr_frags=%u gso_size=%u gso_seqs=%u gso_type=%x frag_list=%p pkt_type=%x ip_summed=%d\n",
...
# ifdef NEMUNETFLT_WITH_GSO_RECV
        if (   (skb_shinfo(pBuf)->gso_type & (SKB_GSO_UDP | SKB_GSO_TCPV6 | SKB_GSO_TCPV4))
            && nemuNetFltLinuxCanForwardAsGso(pThis, pBuf, fSrc, &GsoCtx) )
            nemuNetFltLinuxForwardAsGso(pThis, pBuf, fSrc, &GsoCtx);
        else
# endif
        {
            ...
            {
                dev_kfree_skb(pBuf);
                LogRel(("NemuNetFlt: Failed to segment a packet (%d).\n", PTR_ERR(pSegment)));
                return;
            }

            for (; pSegment; pSegment = pNext)
            {
                Log3(("nemuNetFltLinuxForwardToIntNet: segment len=%u data_len=%u truesize=%u next=%p nr_frags=%u gso_size=%u gso_seqs=%u gso_type=%x frag_list=%p pkt_type=%x\n",
                ...
                pNext = pSegment->next;
                pSegment->next = 0;
                nemuNetFltLinuxForwardSegment(pThis, pSegment, fSrc);
            }
            dev_kfree_skb(pBuf);
        }
    }
    else
#endif /* NEMUNETFLT_WITH_GSO */
    {
...
#endif /* LINUX_VERSION_CODE <= KERNEL_VERSION(2, 6, 18) */
            if (NEMU_SKB_CHECKSUM_HELP(pBuf))
            {
                LogRel(("NemuNetFlt: Failed to compute checksum, dropping the packet.\n"));
                dev_kfree_skb(pBuf);
                return;
            }
#if LINUX_VERSION_CODE <= KERNEL_VERSION(2, 6, 18)
        ...
        nemuNetFltLinuxForwardSegment(pThis, pBuf, fSrc);
    }
}
 ...
 * nemuNetFltLinuxPacketHandler to the internal network.
 *
 * @param   pWork               The work queue.
 */
# if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 20)
static void nemuNetFltLinuxXmitTask(struct work_struct *pWork)
# else
static void nemuNetFltLinuxXmitTask(void *pWork)
# endif
{
    PNEMUNETFLTINS  pThis   = NEMU_FLT_XT_TO_INST(pWork);
    struct sk_buff *pBuf;

    Log4(("nemuNetFltLinuxXmitTask: Got work %p.\n", pWork));
...
#endif /* !NEMUNETFLT_LINUX_NO_XMIT_QUEUE */
 ...
 *                              reference to this.
 */
static void nemuNetFltLinuxReportNicGsoCapabilities(PNEMUNETFLTINS pThis)
{
#ifdef NEMUNETFLT_WITH_GSO_XMIT_WIRE
...
# endif
            Log3(("nemuNetFltLinuxReportNicGsoCapabilities: reporting wire %s%s%s%s\n",
...
#endif /* NEMUNETFLT_WITH_GSO_XMIT_WIRE */
}
 ...
 */
static bool nemuNetFltLinuxPromiscuous(PNEMUNETFLTINS pThis)
{
    bool                fRc  = false;
    struct net_device * pDev = nemuNetFltLinuxRetainNetDev(pThis);
    if (pDev)
    {
        fRc = !!(pDev->promiscuity - (ASMAtomicUoReadBool(&pThis->u.s.fPromiscuousSet) & 1));
        LogFlow(("nemuNetFltPortOsIsPromiscuous: returns %d, pDev->promiscuity=%d, fPromiscuousSet=%d\n",
                 fRc, pDev->promiscuity, pThis->u.s.fPromiscuousSet));
        nemuNetFltLinuxReleaseNetDev(pThis, pDev);
    }
    return fRc;
}
 ...
 * Currently we need it for our own NemuNetAdp and TAP.
 */
static bool nemuNetFltNeedsLinkState(PNEMUNETFLTINS pThis, struct net_device *pDev)
{
    if (pDev->ethtool_ops && pDev->ethtool_ops->get_drvinfo)
    {
        ...
        if (!strncmp(Info.driver, "nemunet", sizeof(Info.driver)))
            return true;
 ...
 */
static void nemuNetFltSetLinkState(PNEMUNETFLTINS pThis, struct net_device *pDev, bool fLinkUp)
{
    if (nemuNetFltNeedsLinkState(pThis, pDev))
    {
...
/**
 * Internal worker for nemuNetFltLinuxNotifierCallback.
 *
 * @returns Nemu status code.
 * @param   pThis           The instance.
 * @param   pDev            The device to attach to.
 */
static int nemuNetFltLinuxAttachToInterface(PNEMUNETFLTINS pThis, struct net_device *pDev)
{
    LogFlow(("nemuNetFltLinuxAttachToInterface: pThis=%p (%s)\n", pThis, pThis->szName));
    ...
    Log(("nemuNetFltLinuxAttachToInterface: Device %p(%s) retained. ref=%d\n",
          pDev, pDev->name,
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 37)
          netdev_refcnt_read(pDev)
#else
    ...
    Log(("nemuNetFltLinuxAttachToInterface: Got pDev=%p pThis=%p pThis->u.s.pDev=%p\n",
    ...
    pThis->u.s.PacketType.func = nemuNetFltLinuxPacketHandler;
    dev_add_pack(&pThis->u.s.PacketType);
    ASMAtomicUoWriteBool(&pThis->u.s.fPacketHandler, true);
    Log(("nemuNetFltLinuxAttachToInterface: this=%p: Packet handler installed.\n", pThis));

#ifdef NEMUNETFLT_WITH_HOST2WIRE_FILTER
    nemuNetFltLinuxHookDev(pThis, pDev);
#endif

    /*
     * Are we the "carrier" for this device (e.g. nemunet or tap)?
     */
    nemuNetFltSetLinkState(pThis, pDev, true);
    ...
    if (!pDev)
    {
        ...
        if (nemuNetFltTryRetainBusyNotDisconnected(pThis))
        {
            nemuNetFltLinuxReportNicGsoCapabilities(pThis);
            pThis->pSwitchPort->pfnReportMacAddress(pThis->pSwitchPort, &pThis->u.s.MacAddr);
            pThis->pSwitchPort->pfnReportPromiscuousMode(pThis->pSwitchPort, nemuNetFltLinuxPromiscuous(pThis));
    ...
    {
#ifdef NEMUNETFLT_WITH_HOST2WIRE_FILTER
        nemuNetFltLinuxUnhookDev(pThis, pDev);
#endif
        RTSpinlockAcquire(pThis->hSpinlock);
        ASMAtomicUoWriteNullPtr(&pThis->u.s.pDev);
        RTSpinlockRelease(pThis->hSpinlock);
        dev_put(pDev);
        Log(("nemuNetFltLinuxAttachToInterface: Device %p(%s) released. ref=%d\n",
             pDev, pDev->name,
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 37)
             netdev_refcnt_read(pDev)
#else
    ...
    LogRel(("NemuNetFlt: attached to '%s' / %RTmac\n", pThis->szName, &pThis->u.s.MacAddr));
    return VINF_SUCCESS;
}


static int nemuNetFltLinuxUnregisterDevice(PNEMUNETFLTINS pThis, struct net_device *pDev)
{
    bool fRegistered;
    ...
    if (ASMAtomicCmpXchgBool(&pThis->u.s.fPacketHandler, false, true))
    {
        ...
        Log(("nemuNetFltLinuxUnregisterDevice: this=%p: packet handler removed.\n", pThis));
    }
...
#endif
        Log(("nemuNetFltLinuxUnregisterDevice: this=%p: xmit queue purged.\n", pThis));
        Log(("nemuNetFltLinuxUnregisterDevice: Device %p(%s) released. ref=%d\n",
             pDev, pDev->name,
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 37)
             netdev_refcnt_read(pDev)
#else
...
static int nemuNetFltLinuxDeviceIsUp(PNEMUNETFLTINS pThis, struct net_device *pDev)
{
        ...
        && !ASMAtomicUoReadBool(&pThis->u.s.fPromiscuousSet))
    {
        ...
        Log(("nemuNetFltLinuxDeviceIsUp: enabled promiscuous mode on %s (%d)\n", pThis->szName, pDev->promiscuity));
    }
    else
        Log(("nemuNetFltLinuxDeviceIsUp: no need to enable promiscuous mode on %s (%d)\n", pThis->szName, pDev->promiscuity));
    return NOTIFY_OK;
}

static int nemuNetFltLinuxDeviceGoingDown(PNEMUNETFLTINS pThis, struct net_device *pDev)
{
    ...
    if (ASMAtomicUoReadBool(&pThis->u.s.fPromiscuousSet))
    {
        ...
        Log(("nemuNetFltLinuxDeviceGoingDown: disabled promiscuous mode on %s (%d)\n", pThis->szName, pDev->promiscuity));
    }
    else
        Log(("nemuNetFltLinuxDeviceGoingDown: no need to disable promiscuous mode on %s (%d)\n", pThis->szName, pDev->promiscuity));
    return NOTIFY_OK;
}
 ...
 */
static int nemuNetFltLinuxDeviceMtuChange(PNEMUNETFLTINS pThis, struct net_device *pDev)
{
    ASMAtomicWriteU32(&pThis->u.s.cbMtu, pDev->mtu);
    Log(("nemuNetFltLinuxDeviceMtuChange: set MTU for %s to %d\n", pThis->szName, pDev->mtu));
    return NOTIFY_OK;
}

#ifdef LOG_ENABLED
/** Stringify the NETDEV_XXX constants. */
static const char *nemuNetFltLinuxGetNetDevEventName(unsigned long ulEventType)
{
 ...
 *                              relates to.
 */
static int nemuNetFltLinuxNotifierCallback(struct notifier_block *self, unsigned long ulEventType, void *ptr)

{
    PNEMUNETFLTINS      pThis = NEMU_FLT_NB_TO_INST(self);
    struct net_device  *pMyDev = ASMAtomicUoReadPtrT(&pThis->u.s.pDev, struct net_device *);
    struct net_device  *pDev  = NEMU_NETDEV_NOTIFIER_INFO_TO_DEV(ptr);
    int                 rc    = NOTIFY_OK;

    Log(("NemuNetFlt: got event %s(0x%lx) on %s, pDev=%p pThis=%p pThis->u.s.pDev=%p\n",
         nemuNetFltLinuxGetNetDevEventName(ulEventType), ulEventType, pDev->name, pDev, pThis, pMyDev));

    if (ulEventType == NETDEV_REGISTER)
    {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 24) /* cgroups/namespaces introduced */
# if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 26)
#  define NEMU_DEV_NET(dev)             dev_net(dev)
#  define NEMU_NET_EQ(n1, n2)           net_eq((n1), (n2))
# else
#  define NEMU_DEV_NET(dev)             ((dev)->nd_net)
#  define NEMU_NET_EQ(n1, n2)           ((n1) == (n2))
# endif
        struct net *pMyNet = current->nsproxy->net_ns;
        struct net *pDevNet = NEMU_DEV_NET(pDev);

        if (NEMU_NET_EQ(pDevNet, pMyNet))
#endif  /* namespaces */
        {
            if (strcmp(pDev->name, pThis->szName) == 0)
            {
                nemuNetFltLinuxAttachToInterface(pThis, pDev);
            }
        }
    }
    else
        ...
        if (pDev == pMyDev)
        {
            switch (ulEventType)
            {
                case NETDEV_UNREGISTER:
                    rc = nemuNetFltLinuxUnregisterDevice(pThis, pDev);
                    break;
                case NETDEV_UP:
                    rc = nemuNetFltLinuxDeviceIsUp(pThis, pDev);
                    break;
                case NETDEV_GOING_DOWN:
                    rc = nemuNetFltLinuxDeviceGoingDown(pThis, pDev);
                    break;
                case NETDEV_CHANGEMTU:
                    rc = nemuNetFltLinuxDeviceMtuChange(pThis, pDev);
                    break;
 ...
 */
static int nemuNetFltLinuxEnumeratorCallback(struct notifier_block *self, unsigned long ulEventType, void *ptr)
{
    PNEMUNETFLTINS pThis = ((PNEMUNETFLTNOTIFIER)self)->pThis;
    struct net_device *dev  = NEMU_NETDEV_NOTIFIER_INFO_TO_DEV(ptr);
    ...
    {
        for_ifa(in_dev) {
            if (NEMU_IPV4_IS_LOOPBACK(ifa->ifa_address))
                return NOTIFY_OK;

            if (   dev != pThis->u.s.pDev
                && NEMU_IPV4_IS_LINKLOCAL_169(ifa->ifa_address))
                continue;

            Log(("%s: %s: IPv4 addr %RTnaipv4 mask %RTnaipv4\n",
                 __FUNCTION__, NEMU_NETDEV_NAME(dev),
...
static int nemuNetFltLinuxNotifierIPv4Callback(struct notifier_block *self, unsigned long ulEventType, void *ptr)
{
    PNEMUNETFLTINS     pThis = RT_FROM_MEMBER(self, NEMUNETFLTINS, u.s.NotifierIPv4);
    struct net_device *pDev, *pEventDev;
    struct in_ifaddr  *ifa   = (struct in_ifaddr *)ptr;
    bool               fMyDev;
    int                rc    = NOTIFY_OK;

    pDev = nemuNetFltLinuxRetainNetDev(pThis);
    pEventDev = ifa->ifa_dev->dev;
    fMyDev = (pDev == pEventDev);
    Log(("NemuNetFlt: %s: IPv4 event %s(0x%lx) %s: addr %RTnaipv4 mask %RTnaipv4\n",
         pDev ? NEMU_NETDEV_NAME(pDev) : "<???>",
         nemuNetFltLinuxGetNetDevEventName(ulEventType), ulEventType,
         pEventDev ? NEMU_NETDEV_NAME(pEventDev) : "<???>",
         ifa->ifa_address, ifa->ifa_mask));
...
static int nemuNetFltLinuxNotifierIPv6Callback(struct notifier_block *self, unsigned long ulEventType, void *ptr)
{
    PNEMUNETFLTINS       pThis = RT_FROM_MEMBER(self, NEMUNETFLTINS, u.s.NotifierIPv6);
    struct net_device   *pDev, *pEventDev;
    ...
    fMyDev = (pDev == pEventDev);
    Log(("NemuNetFlt: %s: IPv6 event %s(0x%lx) %s: %RTnaipv6\n",
         pDev ? NEMU_NETDEV_NAME(pDev) : "<???>",
         nemuNetFltLinuxGetNetDevEventName(ulEventType), ulEventType,
         pEventDev ? NEMU_NETDEV_NAME(pEventDev) : "<???>",
         &ifa->addr));
...
int  nemuNetFltPortOsXmit(PNEMUNETFLTINS pThis, void *pvIfData, PINTNETSG pSG, uint32_t fDst)
{
    ...
    LogFlow(("nemuNetFltPortOsXmit: pThis=%p (%s)\n", pThis, pThis->szName));
    ...
    if (pDev)
    {
        ...
        {
            struct sk_buff *pBuf = nemuNetFltLinuxSkBufFromSG(pThis, pSG, true);
            if (pBuf)
            {
                nemuNetFltDumpPacket(pSG, true, "wire", 1);
                Log4(("nemuNetFltPortOsXmit: pBuf->cb dump:\n%.*Rhxd\n", sizeof(pBuf->cb), pBuf->cb));
                Log4(("nemuNetFltPortOsXmit: dev_queue_xmit(%p)\n", pBuf));
        ...
        {
            struct sk_buff *pBuf = nemuNetFltLinuxSkBufFromSG(pThis, pSG, false);
            if (pBuf)
            {
                nemuNetFltDumpPacket(pSG, true, "host", (fDst & INTNETTRUNKDIR_WIRE) ? 0 : 1);
                Log4(("nemuNetFltPortOsXmit: pBuf->cb dump:\n%.*Rhxd\n", sizeof(pBuf->cb), pBuf->cb));
                Log4(("nemuNetFltPortOsXmit: netif_rx_ni(%p)\n", pBuf));
...
void nemuNetFltPortOsSetActive(PNEMUNETFLTINS pThis, bool fActive)
{
    ...
    LogFlow(("nemuNetFltPortOsSetActive: pThis=%p (%s), fActive=%RTbool, fDisablePromiscuous=%RTbool\n",
    ...
    if (pDev)
    {
         ...
         * host the interface promiscuity for nemuNetFltPortOsIsPromiscuous.
         */
#ifdef LOG_ENABLED
        u_int16_t fIf;
        ...
        if (fActive)
        {
            ...
            Log(("nemuNetFltPortOsSetActive: enabled promiscuous mode on %s (%d)\n", pThis->szName, pDev->promiscuity));
        }
        else
        {
            if (pThis->u.s.fPromiscuousSet)
            {
                ...
                dev_set_promiscuity(pDev, -1);
                rtnl_unlock();
                Log(("nemuNetFltPortOsSetActive: disabled promiscuous mode on %s (%d)\n", pThis->szName, pDev->promiscuity));
            }
            pThis->u.s.fPromiscuousSet = false;

#ifdef LOG_ENABLED
            fIf = dev_get_flags(pDev);
            Log(("NemuNetFlt: fIf=%#x; %d->%d\n", fIf, cPromiscBefore, pDev->promiscuity));
#endif
        }
...
int nemuNetFltOsDisconnectIt(PNEMUNETFLTINS pThis)
{
    ...
    if (ASMAtomicCmpXchgBool(&pThis->u.s.fPacketHandler, false, true))
    {
        IPRT_LINUX_SAVE_EFL_AC();
        dev_remove_pack(&pThis->u.s.PacketType);
        Log(("nemuNetFltOsDisconnectIt: this=%p: Packet handler removed.\n", pThis));
        IPRT_LINUX_RESTORE_EFL_AC();
    }
    return VINF_SUCCESS;
}
...
#if defined(NEMUNETFLT_WITH_GSO_XMIT_HOST)
    Log3(("nemuNetFltOsConnectIt: reporting host tso tso6 ufo\n"));
...
void nemuNetFltOsDeleteInstance(PNEMUNETFLTINS pThis)
{
...
#ifdef NEMUNETFLT_WITH_HOST2WIRE_FILTER
    nemuNetFltLinuxUnhookDev(pThis, NULL);
#endif

    /** @todo This code may race nemuNetFltLinuxUnregisterDevice (very very
    ...
    if (fRegistered)
    {
        nemuNetFltSetLinkState(pThis, pDev, false);

#ifndef NEMUNETFLT_LINUX_NO_XMIT_QUEUE
        skb_queue_purge(&pThis->u.s.XmitQueue);
#endif
        Log(("nemuNetFltOsDeleteInstance: this=%p: xmit queue purged.\n", pThis));
        Log(("nemuNetFltOsDeleteInstance: Device %p(%s) released. ref=%d\n",
             pDev, pDev->name,
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 37)
             netdev_refcnt_read(pDev)
#else
    ...
    Log(("nemuNetFltOsDeleteInstance: this=%p: Notifier removed.\n", pThis));
...
int  nemuNetFltOsInitInstance(PNEMUNETFLTINS pThis, void *pvContext)
{
    ...
    pThis->u.s.Notifier.notifier_call = nemuNetFltLinuxNotifierCallback;
    ...
    {
        unregister_netdevice_notifier(&pThis->u.s.Notifier);
        LogRel(("NemuNetFlt: failed to find %s.\n", pThis->szName));
        IPRT_LINUX_RESTORE_EFL_AC();
        return VERR_INTNET_FLT_IF_NOT_FOUND;
    }

    Log(("nemuNetFltOsInitInstance: this=%p: Notifier installed.\n", pThis));
    ...
    if (pThis->pSwitchPort->pfnNotifyHostAddress)
    {
         ...
         */
        RT_ZERO(Enumerator);
        Enumerator.Notifier.notifier_call = nemuNetFltLinuxEnumeratorCallback;
        ...
        pThis->u.s.NotifierIPv4.notifier_call = nemuNetFltLinuxNotifierIPv4Callback;
        err = register_inetaddr_notifier(&pThis->u.s.NotifierIPv4);
        if (err)
            LogRel(("%s: failed to register IPv4 notifier: error %d\n", __FUNCTION__, err));

        pThis->u.s.NotifierIPv6.notifier_call = nemuNetFltLinuxNotifierIPv6Callback;
...
# if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 20)
    INIT_WORK(&pThis->u.s.XmitTask, nemuNetFltLinuxXmitTask);
# else
    INIT_WORK(&pThis->u.s.XmitTask, nemuNetFltLinuxXmitTask, &pThis->u.s.XmitTask);
# endif
#endif
...
int nemuNetFltPortOsConnectInterface(PNEMUNETFLTINS pThis, void *pvIf, void **pvIfData)
{
    /* Nothing to do */
