...
#ifndef _EXT_H_
#define _EXT_H_
# define fprintf nemu_slirp_fprintf
# define printf nemu_slirp_printf
#ifndef nemu_slirp_printfV
static void nemu_slirp_printV(char *format, va_list args)
{
    char buffer[1024];
...
# ifndef nemu_slirp_printf
static void nemu_slirp_printf(char *format, ...)
{
    va_list args;
    va_start(args, format);
    nemu_slirp_printV(format, args);
    va_end(args);
}
# endif

# ifndef nemu_slirp_fprintf
static void nemu_slirp_fprintf(void *ignored, char *format, ...)
{
#  ifdef LOG_ENABLED
    va_list args;
    NOREF(ignored);
