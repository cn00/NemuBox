/* $Id: NemuServiceControlProcess.cpp $ */
/** @file
 * NemuServiceControlThread - Guest process handling.
 */
...
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
static int                  vgsvcGstCtrlProcessAssignPID(PNEMUSERVICECTRLPROCESS pThread, uint32_t uPID);
static int                  vgsvcGstCtrlProcessLock(PNEMUSERVICECTRLPROCESS pProcess);
static int                  vgsvcGstCtrlProcessRequest(PNEMUSERVICECTRLPROCESS pProcess, const PVBGLR3GUESTCTRLCMDCTX pHostCtx,
...
static int                  vgsvcGstCtrlProcessSetupPipe(const char *pszHowTo, int fd, PRTHANDLE ph, PRTHANDLE *pph,
                                                         PRTPIPE phPipe);
static int                  vgsvcGstCtrlProcessUnlock(PNEMUSERVICECTRLPROCESS pProcess);
/* Request handlers. */
static DECLCALLBACK(int)    vgsvcGstCtrlProcessOnInput(PNEMUSERVICECTRLPROCESS pThis, const PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                                       bool fPendingClose, void *pvBuf, uint32_t cbBuf);
static DECLCALLBACK(int)    vgsvcGstCtrlProcessOnOutput(PNEMUSERVICECTRLPROCESS pThis, const PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                                        uint32_t uHandle, uint32_t cbToRead, uint32_t uFlags);
static DECLCALLBACK(int)    vgsvcGstCtrlProcessOnTerm(PNEMUSERVICECTRLPROCESS pThis);
 ...
 */
static int vgsvcGstCtrlProcessInit(PNEMUSERVICECTRLPROCESS pProcess,
                                   const PNEMUSERVICECTRLSESSION pSession,
                                   const PNEMUSERVICECTRLPROCSTARTUPINFO pStartupInfo,
                                   uint32_t u32ContextID)
{
    ...
    memcpy(&pProcess->StartupInfo, pStartupInfo, sizeof(NEMUSERVICECTRLPROCSTARTUPINFO));
 ...
 */
int VGSvcGstCtrlProcessFree(PNEMUSERVICECTRLPROCESS pProcess)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
 ...
 */
int VGSvcGstCtrlProcessStop(PNEMUSERVICECTRLPROCESS pProcess)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
 ...
 */
void VGSvcGstCtrlProcessRelease(PNEMUSERVICECTRLPROCESS pProcess)
{
 ...
 *                              Optional.
 */
int VGSvcGstCtrlProcessWait(const PNEMUSERVICECTRLPROCESS pProcess, RTMSINTERVAL msTimeout, int *prc)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
 ...
 */
static int vgsvcGstCtrlProcessPollsetCloseInput(PNEMUSERVICECTRLPROCESS pProcess, PRTPIPE phStdInW)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
    AssertPtrReturn(phStdInW, VERR_INVALID_POINTER);

    int rc = RTPollSetRemove(pProcess->hPollSet, NEMUSERVICECTRLPIPEID_STDIN);
...
static const char *vgsvcGstCtrlProcessPollHandleToString(uint32_t idPollHnd)
{
    switch (idPollHnd)
    {
        case NEMUSERVICECTRLPIPEID_UNKNOWN:
            return "unknown";
        case NEMUSERVICECTRLPIPEID_STDIN:
            return "stdin";
        case NEMUSERVICECTRLPIPEID_STDIN_WRITABLE:
            return "stdin_writable";
        case NEMUSERVICECTRLPIPEID_STDOUT:
            return "stdout";
        case NEMUSERVICECTRLPIPEID_STDERR:
            return "stderr";
        case NEMUSERVICECTRLPIPEID_IPC_NOTIFY:
            return "ipc_notify";
        default:
            return "unknown";
    }
 ...
 * @param   phStdInW            The standard input pipe handle.
 */
static int vgsvcGstCtrlProcessPollsetOnInput(PNEMUSERVICECTRLPROCESS pProcess, uint32_t fPollEvt, PRTPIPE phStdInW)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
 ...
 */
static int vgsvcGstCtrlProcessHandleOutputError(PNEMUSERVICECTRLPROCESS pProcess,
 ...
 *
 */
static int vgsvcGstCtrlProcessPollsetOnOutput(PNEMUSERVICECTRLPROCESS pProcess,
                                              uint32_t fPollEvt, PRTPIPE phPipeR, uint32_t idPollHnd)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
 ...
 */
static int vgsvcGstCtrlProcessProcLoop(PNEMUSERVICECTRLPROCESS pProcess)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
 ...
 * resolves internal NemuService tools to its appropriate executable path + name if
 * NEMUSERVICE_NAME is specified as pszFileName.
 *
 * @return  IPRT status code.
...
static int vgsvcGstCtrlProcessResolveExecutable(const char *pszFileName, char *pszResolved, size_t cbResolved)
{
    ...
    if (    (g_pszProgName && (RTStrICmp(pszFileName, g_pszProgName) == 0))
        || !RTStrICmp(pszFileName, NEMUSERVICE_NAME))
    {
 ...
 */
static int vgsvcGstCtrlProcessAssignPID(PNEMUSERVICECTRLPROCESS pProcess, uint32_t uPID)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         * not used anymore. */
        PNEMUSERVICECTRLPROCESS pProcessCur;
        bool fTryAgain;
        do
        {
            fTryAgain = false;
            RTListForEach(&pProcess->pSession->lstProcesses, pProcessCur, NEMUSERVICECTRLPROCESS, Node)
            {
                                            ...
                                            const char *pszPassword, PRTPROCESS phProcess)
{
...
#ifdef RT_OS_WINDOWS
    /*
     * If sysprep should be executed do this in the context of NemuService, which
    ...
    if (!RTStrICmp(pszExec, "sysprep"))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (RT_SUCCESS(rc))
            {
                 ...
                 * run under behalf of the account NemuService was started under, most
                 * likely local system. */
    ...
    if (RTStrStr(pszExec, "nemu_") == pszExec)
    {
        /* We want to use the internal toolbox (all internal
         * tools are starting with "nemu_" (e.g. "nemu_cat"). */
        rc = vgsvcGstCtrlProcessResolveExecutable(NEMUSERVICE_NAME, szExecExp, sizeof(szExecExp));
    }
    else
    {
#endif
 ...
 */
static int vgsvcGstCtrlProcessProcessWorker(PNEMUSERVICECTRLPROCESS pProcess)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
                        ...
                        if (RT_SUCCESS(rc))
                        {
                            ...
                            if (RT_SUCCESS(rc))
                                rc = RTPollSetAddPipe(pProcess->hPollSet,
                                                      pProcess->hPipeStdInW, RTPOLL_EVT_ERROR, NEMUSERVICECTRLPIPEID_STDIN);
                            /* Stdout. */
                            if (RT_SUCCESS(rc))
                                rc = RTPollSetAddPipe(pProcess->hPollSet,
                                                      pProcess->hPipeStdOutR, uFlags, NEMUSERVICECTRLPIPEID_STDOUT);
                            /* Stderr. */
                            if (RT_SUCCESS(rc))
                                rc = RTPollSetAddPipe(pProcess->hPollSet,
                                                      pProcess->hPipeStdErrR, uFlags, NEMUSERVICECTRLPIPEID_STDERR);
                            /* IPC notification pipe. */
                            if (RT_SUCCESS(rc))
                                rc = RTPipeCreate(&pProcess->hNotificationPipeR, &pProcess->hNotificationPipeW, 0 /* Flags */);
                            if (RT_SUCCESS(rc))
                                rc = RTPollSetAddPipe(pProcess->hPollSet,
                                                      pProcess->hNotificationPipeR, RTPOLL_EVT_READ, NEMUSERVICECTRLPIPEID_IPC_NOTIFY);
                            if (RT_SUCCESS(rc))
                            {
                                AssertPtr(pProcess->pSession);
                                bool fNeedsImpersonation = !(pProcess->pSession->fFlags & NEMUSERVICECTRLSESSION_FLAG_SPAWN);
                                ...
                                if (RT_SUCCESS(rc))
                                {
                                    ...
                                    if (RT_FAILURE(RTPollSetQueryHandle(pProcess->hPollSet,
                                                                        NEMUSERVICECTRLPIPEID_IPC_NOTIFY, NULL)))
                                    {
                                        pProcess->hNotificationPipeR = NIL_RTPIPE;
                                        pProcess->hNotificationPipeW = NIL_RTPIPE;
                                    }
                                    if (RT_FAILURE(RTPollSetQueryHandle(pProcess->hPollSet,
                                                                        NEMUSERVICECTRLPIPEID_STDERR, NULL)))
                                        pProcess->hPipeStdErrR = NIL_RTPIPE;
                                    if (RT_FAILURE(RTPollSetQueryHandle(pProcess->hPollSet,
                                                                        NEMUSERVICECTRLPIPEID_STDOUT, NULL)))
                                        pProcess->hPipeStdOutR = NIL_RTPIPE;
                                    if (RT_FAILURE(RTPollSetQueryHandle(pProcess->hPollSet,
                                                                        NEMUSERVICECTRLPIPEID_STDIN, NULL)))
                                        pProcess->hPipeStdInW = NIL_RTPIPE;
                                }
                            }
 ...
 * @param  pvUser           Pointer to a NEMUSERVICECTRLPROCESS structure.
 *
 */
static DECLCALLBACK(int) vgsvcGstCtrlProcessThread(RTTHREAD hThreadSelf, void *pvUser)
{
    PNEMUSERVICECTRLPROCESS pProcess = (PNEMUSERVICECTRLPROCESS)pvUser;
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
 ...
 */
int VGSvcGstCtrlProcessStart(const PNEMUSERVICECTRLSESSION pSession,
                             const PNEMUSERVICECTRLPROCSTARTUPINFO pStartupInfo, uint32_t uContextID)
{
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    AssertPtrReturn(pStartupInfo, VERR_INVALID_POINTER);
     ...
     */
    PNEMUSERVICECTRLPROCESS pProcess = (PNEMUSERVICECTRLPROCESS)RTMemAlloc(sizeof(NEMUSERVICECTRLPROCESS));
    if (!pProcess)
        return VERR_NO_MEMORY;
...
static DECLCALLBACK(int) vgsvcGstCtrlProcessOnInput(PNEMUSERVICECTRLPROCESS pThis,
...
static DECLCALLBACK(int) vgsvcGstCtrlProcessOnOutput(PNEMUSERVICECTRLPROCESS pThis,
                                                     const PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                                     uint32_t uHandle, uint32_t cbToRead, uint32_t fFlags)
{
    ...
    const PNEMUSERVICECTRLSESSION pSession = pThis->pSession;
    AssertPtrReturn(pSession, VERR_INVALID_POINTER);
    ...
    if (pvBuf)
    {
        ...
        if (*phPipe != NIL_RTPIPE)
        {
            ...
            if (RT_FAILURE(rc))
            {
                RTPollSetRemove(pThis->hPollSet,   uHandle == OUTPUT_HANDLE_ID_STDERR
                                                 ? NEMUSERVICECTRLPIPEID_STDERR : NEMUSERVICECTRLPIPEID_STDOUT);
                RTPipeClose(*phPipe);
                *phPipe = NIL_RTPIPE;
        ...
        if (RT_SUCCESS(rc))
        {
            if (   pSession->fFlags & NEMUSERVICECTRLSESSION_FLAG_DUMPSTDOUT
               ...
               )
            {
                ...
                if (!RTStrPrintf(szDumpFile, sizeof(szDumpFile), "NemuService_Session%RU32_PID%RU32_StdOut.txt",
            ...
            else if (   pSession->fFlags & NEMUSERVICECTRLSESSION_FLAG_DUMPSTDERR
                     && uHandle == OUTPUT_HANDLE_ID_STDERR)
            {
                char szDumpFile[RTPATH_MAX];
                if (!RTStrPrintf(szDumpFile, sizeof(szDumpFile), "NemuService_Session%RU32_PID%RU32_StdErr.txt",
...
static int vgsvcGstCtrlProcessRequestExV(PNEMUSERVICECTRLPROCESS pProcess, const PVBGLR3GUESTCTRLCMDCTX pHostCtx, bool fAsync,
...
static int vgsvcGstCtrlProcessRequestAsync(PNEMUSERVICECTRLPROCESS pProcess, const PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                           PFNRT pfnFunction, unsigned cArgs, ...)
{
    AssertPtrReturn(pProcess, VERR_INVALID_POINTER);
    /* pHostCtx is optional. */
...
#if 0 /* unused */
static int vgsvcGstCtrlProcessRequestWait(PNEMUSERVICECTRLPROCESS pProcess, const PVBGLR3GUESTCTRLCMDCTX pHostCtx,
...
int VGSvcGstCtrlProcessHandleInput(PNEMUSERVICECTRLPROCESS pProcess, PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                   bool fPendingClose, void *pvBuf, uint32_t cbBuf)
{
    if (!ASMAtomicReadBool(&pProcess->fShutdown))
...
int VGSvcGstCtrlProcessHandleOutput(PNEMUSERVICECTRLPROCESS pProcess, PVBGLR3GUESTCTRLCMDCTX pHostCtx,
                                    uint32_t uHandle, uint32_t cbToRead, uint32_t fFlags)
{
    if (!ASMAtomicReadBool(&pProcess->fShutdown))
...
int VGSvcGstCtrlProcessHandleTerm(PNEMUSERVICECTRLPROCESS pProcess)
{
    if (!ASMAtomicReadBool(&pProcess->fShutdown))
