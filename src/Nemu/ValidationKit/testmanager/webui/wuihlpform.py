...
class WuiHlpForm(object):
    ...
    def addListOfTestCaseArgs(self, sName, aoVariations, sLabel): # pylint: disable=R0915
        ...
        if not fReadOnly:
            ...
            sHtml += '}\n';
            if config.g_kfNemuSpecific:
                sSecTimeoutDef = escapeAttr(str(oVarDefaults.cSecTimeout));
                sHtml += 'function nemu_%s_add_uni()\n' % (sName,);
                sHtml += '{\n';
                ...
                sHtml += '}\n';
                sHtml += 'function nemu_%s_add_uni_amd64()\n' % (sName,);
                sHtml += '{\n';
                sHtml += '    %s_extendListEx("1", "%s", "--cpu-counts 1 --virt-modes hwvirt", ' \
                         ' "fCpuHwVirt is True", "");\n' % (sName, sSecTimeoutDef);
                sHtml += '    %s_extendListEx("1", "%s", "--cpu-counts 1 --virt-modes hwvirt-np", ' \
                         ' "fCpuNestedPaging is True", "");\n' % (sName, sSecTimeoutDef);
                sHtml += '}\n';
                sHtml += 'function nemu_%s_add_smp()\n' % (sName,);
                sHtml += '{\n';
        ...
        if not fReadOnly:
            ...
            if config.g_kfNemuSpecific:
                sHtml += '    [<a href="#" onClick="nemu_%s_add_uni()">Single CPU Variations</a>\n' % (sName,);
                sHtml += '    <a href="#" onClick="nemu_%s_add_uni_amd64()">amd64</a>]\n' % (sName,);
                sHtml += '    [<a href="#" onClick="nemu_%s_add_smp()">SMP Variations</a>]\n' % (sName,);
            sHtml += '  </caption>\n';
