...
#define NEMU_WITH_KBD_LEDS_SYNC
//#define NEMU_WITHOUT_KBD_LEDS_SYNC_FILTERING
...
#define K_EX            NEMUKEY_EXTENDED
/** short hand for a modifier key. */
#define K_MOD           NEMUKEY_MODIFIER
/** short hand for a lock key. */
#define K_LOCK          NEMUKEY_LOCK
...
#define NEMU_BOOL_TO_STR_STATE(x) (x) ? "ON" : "OFF"
/* HID LEDs synchronization data: LED states. */
typedef struct NemuLedState_t {
...
typedef struct NemuKbdState_t {
    IOHIDDeviceRef    pDevice;              /** A reference to IOKit HID device */
    NemuLedState_t    LED;                  /** LED states */
    void             *pParentContainer;     /** A pointer to a NemuHidsState_t instance where NemuKbdState_t instance is stored */
...
typedef struct NemuHidsState_t {
    IOHIDManagerRef     hidManagerRef;      /** IOKit HID manager reference */
    CFMutableArrayRef   pDeviceCollection;  /** This array consists of NemuKbdState_t elements */
    NemuLedState_t      guestState;         /** LED states that were stored during last broadcast and reflect a guest LED states */
...
{
#ifndef NEMU_WITHOUT_KBD_LEDS_SYNC_FILTERING
    ...
    return fSupported;
#else /* !NEMU_WITH_KBD_LEDS_SYNC_FILTERING */
    return true;
#endif
}
...
static void darwinHidInputCbKeyDown(NemuKbdState_t *pKbd, uint32_t iKeyCode, NemuHidsState_t *pHidState)
{
    NemuKbdEvent_t *pEvent = (NemuKbdEvent_t *)malloc(sizeof(NemuKbdEvent_t));
...
static bool darwinKbdCapsEventMatches(NemuKbdEvent_t *pEvent, bool fCapsLed)
{
 ...
 * This code should be executed within a critical section under pHidState->fifoEventQueueLock. */
static void darwinHidInputCbKeyUp(NemuKbdState_t *pKbd, uint32_t iKeyCode, NemuHidsState_t *pHidState)
{
    CFIndex         iQueue = 0;
    NemuKbdEvent_t *pEvent = NULL;
    ...
    {
        NemuKbdEvent_t *pCachedEvent = (NemuKbdEvent_t *)CFArrayGetValueAtIndex(pHidState->pFifoEventQueue, i);
...
static void darwinHidInputCallback(void *pData, IOReturn unused, void *unused1, IOHIDValueRef pValueRef)
{
    ...
    if (IOHIDElementGetUsagePage(pElementRef) == kHIDPage_KeyboardOrKeypad)    /* Keyboard or keypad event */
        ...
        {
            NemuKbdState_t *pKbd = (NemuKbdState_t *)pData;

            if (pKbd && pKbd->pParentContainer)
            {
                bool             fKeyDown  = (IOHIDValueGetIntegerValue(pValueRef) == 1);
                NemuHidsState_t *pHidState = (NemuHidsState_t *)pKbd->pParentContainer;
...
static NemuKbdEvent_t *darwinCarbonCbFindEvent(NemuHidsState_t *pHidState)
{
    NemuKbdEvent_t *pEvent = NULL;

    for (CFIndex i = 0; i < CFArrayGetCount(pHidState->pFifoEventQueue); i++)
    {
        pEvent = (NemuKbdEvent_t *)CFArrayGetValueAtIndex(pHidState->pFifoEventQueue, i);
...
static CGEventRef darwinCarbonCallback(CGEventTapProxy unused, CGEventType unused1, CGEventRef pEventRef, void *pData)
{
    ...
    NemuHidsState_t *pHidState = (NemuHidsState_t *)pData; AssertReturn(pHidState, pEventRef);
        ...
        key == kHIDUsage_KeypadNumLock)
    {
        ...
        NemuKbdEvent_t *pEvent = darwinCarbonCbFindEvent(pHidState);
        if (pEvent)
        {
            NemuKbdState_t *pKbd = pEvent->pKbd;

            LogRel2(("CARBON: KBD %d: caps=%s, num=%s. tsKeyDown=%llu, tsKeyUp=%llu [tsDiff=%llu ms]. %d events in queue.\n",
                (int)pKbd->idxPosition, NEMU_BOOL_TO_STR_STATE(fCaps), NEMU_BOOL_TO_STR_STATE(fNum),
            ...
            LogRel2(("CARBON: No KBD to take care when modifier key has been pressed: caps=%s, num=%s (%d events in queue)\n",
                NEMU_BOOL_TO_STR_STATE(fCaps), NEMU_BOOL_TO_STR_STATE(fNum), CFArrayGetCount(pHidState->pFifoEventQueue)));
    }
...
static void darwinUsbHidResyncLeds(NemuKbdState_t *pKbd)
{
...
static NemuKbdState_t * darwinUsbHidQueryKbdByLocationId(uint32_t idLocation, NemuHidsState_t *pHidState)
{
    AssertReturn(pHidState, NULL);

    for (CFIndex i = 0; i < CFArrayGetCount(pHidState->pDeviceCollection); i++)
    {
        NemuKbdState_t *pKbd = (NemuKbdState_t *)CFArrayGetValueAtIndex(pHidState->pDeviceCollection, i);
...
static void darwinUsbHidDeviceMatchCb(void *pData, io_iterator_t iter)
{
    ...
    NemuHidsState_t *pHidState = (NemuHidsState_t *)pData;
    ...
    while ((service = IOIteratorNext(iter)))
    {
        ...
        if (ppUsbDeviceInterface)
        {
            ...
            {
                NemuKbdState_t *pKbd = darwinUsbHidQueryKbdByLocationId((uint32_t)idLocation, pHidState);
...
static int darwinUsbHidSubscribeInterestNotifications(NemuHidsState_t *pHidState)
{
...
static void darwinUsbHidUnsubscribeInterestNotifications(NemuHidsState_t *pHidState)
{
    AssertReturnVoid(pHidState);
...
static void darwinHidRemovalCallback(void *pData, IOReturn unused, void *unused1)
{
    ...
    NemuKbdState_t  *pKbd      = (NemuKbdState_t  *)pData;                  AssertReturnVoid(pKbd);
    NemuHidsState_t *pHidState = (NemuHidsState_t *)pKbd->pParentContainer; AssertReturnVoid(pHidState);
...
static bool darwinIsDeviceInCache(NemuHidsState_t *pState, IOHIDDeviceRef pDevice)
{
    ...
    {
        NemuKbdState_t *pKbd = (NemuKbdState_t *)CFArrayGetValueAtIndex(pState->pDeviceCollection, i);
...
/** Add device to cache. */
static void darwinHidAddDevice(NemuHidsState_t *pHidState, IOHIDDeviceRef pDevice, bool fApplyLedState)
{
    int rc;
    ...
    if (!darwinIsDeviceInCache(pHidState, pDevice))
    {
        ...
        {
            NemuKbdState_t *pKbd = (NemuKbdState_t *)malloc(sizeof(NemuKbdState_t));
            if (pKbd)
            {
                ...
                if (elementMatchingDict)
                {
                    ...
                    LogRel2(("Saved LEDs for KBD %d (%p): fNumLockOn=%s, fCapsLockOn=%s, fScrollLockOn=%s\n",
                        (int)pKbd->idxPosition, pKbd, NEMU_BOOL_TO_STR_STATE(pKbd->LED.fNumLockOn), NEMU_BOOL_TO_STR_STATE(pKbd->LED.fCapsLockOn),
                        NEMU_BOOL_TO_STR_STATE(pKbd->LED.fScrollLockOn)));
...
static int darwinAddCarbonHandler(NemuHidsState_t *pHidState)
{
    CFMachPortRef pTapRef;
...
static void darwinRemoveCarbonHandler(NemuHidsState_t *pHidState)
{
    AssertReturnVoid(pHidState);
...
#ifdef NEMU_WITH_KBD_LEDS_SYNC
    IOReturn         rc;
    NemuHidsState_t *pHidState;

    pHidState = (NemuHidsState_t *)malloc(sizeof(NemuHidsState_t));
    AssertReturn(pHidState, NULL);
...
{
#ifdef NEMU_WITH_KBD_LEDS_SYNC
    NemuHidsState_t *pHidState = (NemuHidsState_t *)pState;
    IOReturn         rc, rc2 = 0;
    ...
    if (elementMatchingDict)
    {
        ...
        for (CFIndex i = 0; i < CFArrayGetCount(pHidState->pDeviceCollection); i++)
        {
            ...
            NemuKbdState_t *pKbd;
            pKbd = (NemuKbdState_t *)CFArrayGetValueAtIndex(pHidState->pDeviceCollection, i);
            ...
            if (pKbd)
            {
                ...
                LogRel2(("Restored LEDs for KBD %d (%p): fNumLockOn=%s, fCapsLockOn=%s, fScrollLockOn=%s\n",
                     (int)i, pKbd, NEMU_BOOL_TO_STR_STATE(pKbd->LED.fNumLockOn), NEMU_BOOL_TO_STR_STATE(pKbd->LED.fCapsLockOn),
                     NEMU_BOOL_TO_STR_STATE(pKbd->LED.fScrollLockOn)));
...
/* Temporary disabled */
    ...
    if (elementMatchingDict)
    {
        LogRel2(("Start LEDs broadcast: fNumLockOn=%s, fCapsLockOn=%s, fScrollLockOn=%s\n",
            NEMU_BOOL_TO_STR_STATE(fNumLockOn), NEMU_BOOL_TO_STR_STATE(fCapsLockOn), NEMU_BOOL_TO_STR_STATE(fScrollLockOn)));
