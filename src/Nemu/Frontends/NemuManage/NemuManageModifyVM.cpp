/* $Id: NemuManageModifyVM.cpp $ */
/** @file
 * NemuManage - Implementation of modifyvm command.
 */
           ...
           && (c = RTGetOpt(&GetOptState, &ValueUnion)))
    {
        switch (c)
        {
            ...
            case MODIFYVM_GRAPHICSCONTROLLER:
            {
                ...
                else if (   !RTStrICmp(ValueUnion.psz, "nemuvga")
                         || !RTStrICmp(ValueUnion.psz, "nemu")
                         || !RTStrICmp(ValueUnion.psz, "vga")
                         || !RTStrICmp(ValueUnion.psz, "vesa"))
                    CHECK_ERROR(sessionMachine, COMSETTER(GraphicsControllerType)(GraphicsControllerType_NemuVGA));
...
#endif /* !RT_OS_DARWIN */
# if defined(RT_OS_FREEBSD) || defined(RT_OS_LINUX) || defined(NEMU_WITH_SOLARIS_OSS)
                else if (!RTStrICmp(ValueUnion.psz, "oss"))
                {
                    CHECK_ERROR(audioAdapter, COMSETTER(AudioDriver)(AudioDriverType_OSS));
                    CHECK_ERROR(audioAdapter, COMSETTER(Enabled)(true));
...
#endif /* NEMU_WITH_USB_CARDREADER */
