...
/** @file
 * Nemu Console VRDP helper class.
 */
...
#include <Nemu/err.h>
#include <Nemu/RemoteDesktop/VRDEOrders.h>
...
NEMU_LISTENER_DECLARE(VRDPConsoleListenerImpl)
...
int ConsoleVRDPServer::Launch(void)
{
    ...
    if (strExtPack.equals(NEMUVRDP_KLUDGE_EXTPACK_NAME))
        strVrdeLibrary = "NemuVRDP";
    else
    {
#ifdef NEMU_WITH_EXTPACK
...
typedef struct TSMFVRDPCTX
{
    ...
    NEMUHOSTCHANNELCALLBACKS *pCallbacks;
    void *pvCallbacks;
                                                                         ...
                                                                         uint32_t *pcbDataReturned)
{
    ...
    if (!pvChannel)
    {
        ...
        if (u32Code == NEMU_HOST_CHANNEL_CTRL_EXISTS)
        {
            *pcbDataReturned = 0;
            return VINF_SUCCESS;
        }
...
void ConsoleVRDPServer::setupTSMF(void)
{
    ...
    NEMUHOSTCHANNELINTERFACE hostChannelInterface =
    {
        this,
    ...
    int rc = pVMMDev->hgcmHostCall("NemuHostChannel",
                                   NEMU_HOST_CHANNEL_HOST_FN_REGISTER,
                                   2,
                                   &parms.name);

    if (!RT_SUCCESS(rc))
    {
        Log(("NEMU_HOST_CHANNEL_HOST_FN_REGISTER failed with %Rrc\n", rc));
        return;
    }
...
#define NEMU_TSMF_HCH_CREATE_ACCEPTED (NEMU_HOST_CHANNEL_EVENT_USER + 0)
#define NEMU_TSMF_HCH_CREATE_DECLINED (NEMU_HOST_CHANNEL_EVENT_USER + 1)
#define NEMU_TSMF_HCH_DISCONNECTED    (NEMU_HOST_CHANNEL_EVENT_USER + 2)
                                           ...
                                           uint32_t u32ClientId)
{
...
#ifdef NEMU_WITH_VRDEAUTH_IN_NEMUSVC
    try
    {
    ...
    result = AuthLibAuthenticate(&mAuthLibCtx,
                                 ...
                                 u32ClientId);
#endif /* !NEMU_WITH_VRDEAUTH_IN_NEMUSVC */
                                                       ...
                                                       uint32_t cbData)
{
    ...
    switch (u32Function)
    {
        case VRDE_CLIPBOARD_FUNCTION_FORMAT_ANNOUNCE:
        {
            ...
            {
                pServer->mpfnClipboardCallback(NEMU_CLIPBOARD_EXT_FN_FORMAT_ANNOUNCE,
                                               u32Format,
                                               (void *)pvData,
                                               cbData);
            }
        ...
        case VRDE_CLIPBOARD_FUNCTION_DATA_READ:
        {
            if (pServer->mpfnClipboardCallback)
            {
                pServer->mpfnClipboardCallback(NEMU_CLIPBOARD_EXT_FN_DATA_READ,
                                               u32Format,
                                               (void *)pvData,
                                               cbData);
            }
                                                               ...
                                                               uint32_t cbParms)
{
    ...
    NEMUCLIPBOARDEXTPARMS *pParms = (NEMUCLIPBOARDEXTPARMS *)pvParms;

    switch (u32Function)
    {
        case NEMU_CLIPBOARD_EXT_FN_SET_CALLBACK:
        {
        ...
        case NEMU_CLIPBOARD_EXT_FN_DATA_WRITE:
        {
            if (mpEntryPoints && pServer->mhServer)
            {
                mpEntryPoints->VRDEClipboard(pServer->mhServer,
...
void ConsoleVRDPServer::ClipboardCreate(uint32_t u32ClientId)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        {
            rc = HGCMHostRegisterServiceExtension(&mhClipboard, "NemuSharedClipboard", ClipboardServiceExtension, this);
