/* $Id: NemuDnDDataObject.cpp $ */
/** @file
 * NemuDnDDataObject.cpp - IDataObject implementation.
 */
...
//# define NEMU_DND_DEBUG_FORMATS
#endif

/** @todo Implement IDataObjectAsyncCapability interface? */

NemuDnDDataObject::NemuDnDDataObject(LPFORMATETC pFormatEtc, LPSTGMEDIUM pStgMed, ULONG cFormats)
...
NemuDnDDataObject::~NemuDnDDataObject(void)
{
    if (mpFormatEtc)
        delete[] mpFormatEtc;
...
STDMETHODIMP_(ULONG) NemuDnDDataObject::AddRef(void)
{
    return InterlockedIncrement(&mRefCount);
}

STDMETHODIMP_(ULONG) NemuDnDDataObject::Release(void)
{
...
STDMETHODIMP NemuDnDDataObject::QueryInterface(REFIID iid, void **ppvObject)
{
 ...
 * @param   pMedium
 */
STDMETHODIMP NemuDnDDataObject::GetData(LPFORMATETC pFormatEtc, LPSTGMEDIUM pMedium)
{
    ...
    if (mStatus == Dropped)
    {
        LogRel3(("DnD: cfFormat=%RI16, sFormat=%s, tyMed=%RU32, dwAspect=%RU32\n",
                 pThisFormat->cfFormat, NemuDnDDataObject::ClipboardFormatToString(pFormatEtc->cfFormat),
 ...
 * @param   pMedium
 */
STDMETHODIMP NemuDnDDataObject::GetDataHere(LPFORMATETC pFormatEtc, LPSTGMEDIUM pMedium)
{
    LogFlowFunc(("\n"));
 ...
 * @param   pFormatEtc
 */
STDMETHODIMP NemuDnDDataObject::QueryGetData(LPFORMATETC pFormatEtc)
{
    LogFlowFunc(("\n"));
    return (LookupFormatEtc(pFormatEtc, NULL /* puIndex */)) ? S_OK : DV_E_FORMATETC;
}

STDMETHODIMP NemuDnDDataObject::GetCanonicalFormatEtc(LPFORMATETC pFormatEct, LPFORMATETC pFormatEtcOut)
{
    LogFlowFunc(("\n"));
...
STDMETHODIMP NemuDnDDataObject::SetData(LPFORMATETC pFormatEtc, LPSTGMEDIUM pMedium,  BOOL fRelease)
{
    return E_NOTIMPL;
}

STDMETHODIMP NemuDnDDataObject::EnumFormatEtc(DWORD dwDirection, IEnumFORMATETC **ppEnumFormatEtc)
{
    ...
    {
        hr = NemuDnDEnumFormatEtc::CreateEnumFormatEtc(mcFormats, mpFormatEtc, ppEnumFormatEtc);
    }
    else
        hr = E_NOTIMPL;
...
STDMETHODIMP NemuDnDDataObject::DAdvise(LPFORMATETC pFormatEtc, DWORD advf, IAdviseSink *pAdvSink, DWORD *pdwConnection)
{
    return OLE_E_ADVISENOTSUPPORTED;
}

STDMETHODIMP NemuDnDDataObject::DUnadvise(DWORD dwConnection)
{
    return OLE_E_ADVISENOTSUPPORTED;
}

STDMETHODIMP NemuDnDDataObject::EnumDAdvise(IEnumSTATDATA **ppEnumAdvise)
{
    return OLE_E_ADVISENOTSUPPORTED;
}
...
int NemuDnDDataObject::Abort(void)
{
...
/* static */
const char* NemuDnDDataObject::ClipboardFormatToString(CLIPFORMAT fmt)
{
#if 0
    char szFormat[128];
...
bool NemuDnDDataObject::LookupFormatEtc(LPFORMATETC pFormatEtc, ULONG *puIndex)
{
    ...
    for (ULONG i = 0; i < mcFormats; i++)
    {
            ...
            && pFormatEtc->dwAspect == mpFormatEtc[i].dwAspect)
        {
            LogRel3(("DnD: Format found: tyMed=%RI32, cfFormat=%RI16, sFormats=%s, dwAspect=%RI32, ulIndex=%RU32\n",
                      pFormatEtc->tymed, pFormatEtc->cfFormat, NemuDnDDataObject::ClipboardFormatToString(mpFormatEtc[i].cfFormat),
    ...
    LogRel3(("DnD: Format NOT found: tyMed=%RI32, cfFormat=%RI16, sFormats=%s, dwAspect=%RI32\n",
             pFormatEtc->tymed, pFormatEtc->cfFormat, NemuDnDDataObject::ClipboardFormatToString(pFormatEtc->cfFormat),
             pFormatEtc->dwAspect));
...
/* static */
HGLOBAL NemuDnDDataObject::MemDup(HGLOBAL hMemSource)
{
...
void NemuDnDDataObject::RegisterFormat(LPFORMATETC pFormatEtc, CLIPFORMAT clipFormat,
                                       ...
                                       DVTARGETDEVICE *pTargetDevice)
{
    ...
    LogFlowFunc(("Registered format=%ld, sFormat=%s\n",
                 pFormatEtc->cfFormat, NemuDnDDataObject::ClipboardFormatToString(pFormatEtc->cfFormat)));
}

void NemuDnDDataObject::SetStatus(Status status)
{
    LogFlowFunc(("Setting status to %ld\n", status));
    mStatus = status;
}

int NemuDnDDataObject::Signal(const RTCString &strFormat,
