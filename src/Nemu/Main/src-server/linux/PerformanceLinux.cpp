...
/** @file
 *
 * Nemu Linux-specific Performance Classes implementation.
 */
...
#define NEMUVOLINFO_NAME "NemuVolInfo"
...
void CollectorLinux::addVolumeDependencies(const char *pcszVolume, DiskList& listDisks)
{
    ...
    int rc = RTPathAppPrivateArch(szVolInfo,
                                  sizeof(szVolInfo) - sizeof("/" NEMUVOLINFO_NAME " ") - strlen(pcszVolume));
    if (RT_FAILURE(rc))
    {
        LogRel(("VolInfo: Failed to get program path, rc=%Rrc\n", rc));
        return;
    }
    strcat(szVolInfo, "/" NEMUVOLINFO_NAME " ");
