 ...
 * The runtime lives here (in NemuVMMR0.r0) and NemuDD*R0.r0 links against us. */
PFNRT g_VMMR0Deps[] =
{
    (PFNRT)RTCrc32,
    (PFNRT)RTOnce,
...
extern "C" { char _depends_on[] = "nemudrv"; }
#endif
 ...
 * @returns Nemu status on failure.
...
#ifdef NEMU_WITH_64ON32_CMOS_DEBUG
    /*
                        ...
                        if (RT_SUCCESS(rc))
                        {
                            ...
                            if (RT_SUCCESS(rc))
                            {
                                ...
                                if (RT_SUCCESS(rc))
                                {
                                    ...
                                    if (RT_SUCCESS(rc))
                                    {
#ifdef NEMU_WITH_TRIPLE_FAULT_HACK
                                        VMM_CHECK_SMAP_CHECK(RT_NOTHING);
...
# endif
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    NEMUVMM_R0_VMM_RETURN_TO_RING3_RC(pVCpu, CPUMQueryGuestCtxPtr(pVCpu), rc);
# ifdef NEMU_WITH_STATISTICS
...
# endif
        ...
        case VMMR0_DO_HM_RUN:
        {
                          ...
                          && SUPIsTscDeltaAvailableForCpuSetIndex(iHostCpuSet)))
            {
                ...
                /* Fire dtrace probe and collect statistics. */
                NEMUVMM_R0_VMM_RETURN_TO_RING3_HM(pVCpu, CPUMQueryGuestCtxPtr(pVCpu), rc);
#ifdef NEMU_WITH_STATISTICS
                vmmR0RecordRC(pVM, pVCpu, rc);
#endif
            }
