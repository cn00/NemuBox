/** @file
 *
 * Nemu frontends: Framebuffer (FB, DirectFB):
 * Implementation of NemuDirectFB class
 */
...
NS_IMPL_ISUPPORTS1_CI(NemuDirectFB, IFramebuffer)
NS_DECL_CLASSINFO(NemuDirectFB)

NemuDirectFB::NemuDirectFB(IDirectFB *aDFB, IDirectFBSurface *aSurface)
{
    dfb = aDFB;
    surface = aSurface;
...
NS_IMETHODIMP NemuDirectFB::GetWidth(uint32 *width)
{
    if (!width)
...
NS_IMETHODIMP NemuDirectFB::GetHeight(PRUint32 *height)
{
    if (!height)
        return NS_ERROR_INVALID_POINTER;
    *height = fbHeight;
...
NS_IMETHODIMP NemuDirectFB::Lock()
{
...
NS_IMETHODIMP NemuDirectFB::Unlock()
{
    // do we have an internal framebuffer?
    if (fbInternalSurface)
    {
...
NS_IMETHODIMP NemuDirectFB::GetAddress(PRUint8 **address)
{
    if (!address)
        return NS_ERROR_INVALID_POINTER;
...
NS_IMETHODIMP NemuDirectFB::GetBitsPerPixel(PRUint32 *bitsPerPixel)
{
    if (!bitsPerPixel)
        return NS_ERROR_INVALID_POINTER;
...
NS_IMETHODIMP NemuDirectFB::GetBytesPerLine(PRUint32 *bytesPerLine)
{
    if (!bytesPerLine)
        return NS_ERROR_INVALID_POINTER;
...
NS_IMETHODIMP NemuDirectFB::GetPixelFormat (PRUint32 *pixelFormat)
{
    if (!pixelFormat)
        return NS_ERROR_INVALID_POINTER;
...
NS_IMETHODIMP NemuDirectFB::GetUsesGuestVRAM (PRBool *usesGuestVRAM)
{
    if (!usesGuestVRAM)
        return NS_ERROR_INVALID_POINTER;
    *usesGuestVRAM = false;
...
NS_IMETHODIMP NemuDirectFB::GetHeightReduction(PRUint32 *heightReduction)
{
    if (!heightReduction)
        return NS_ERROR_INVALID_POINTER;
    *heightReduction = 0;
...
NS_IMETHODIMP NemuDirectFB::GetOverlay(IFramebufferOverlay **overlay)
{
    if (!overlay)
        return NS_ERROR_INVALID_POINTER;
    /* Not yet implemented */
...
NS_IMETHODIMP NemuDirectFB::GetWinId(PRint64 *winId)
{
    if (!winId)
        return NS_ERROR_INVALID_POINTER;
    *winId = 0;
...
NS_IMETHODIMP NemuDirectFB::NotifyUpdate(PRUint32 x, PRUint32 y,
...
NS_IMETHODIMP NemuDirectFB::RequestResize(PRUint32 aScreenId, PRUint32 pixelFormat, PRUint8 *vram,
...
NS_IMETHODIMP NemuDirectFB::VideoModeSupported(PRUint32 w, PRUint32 h, PRUint32 bpp, PRBool *supported)
{
    if (!supported)
        return NS_ERROR_INVALID_POINTER;
    *supported = true;
...
NS_IMETHODIMP NemuDirectFB::GetVisibleRegion(PRUint8 *rectangles, PRUint32 count, PRUint32 *countCopied)
{
...
NS_IMETHODIMP NemuDirectFB::SetVisibleRegion(PRUint8 *rectangles, PRUint32 count)
{
    PRTRECT rects = (PRTRECT)rectangles;
...
NS_IMETHODIMP NemuDirectFB::ProcessVHWACommand(PRUint8 *command)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}

NS_IMETHODIMP NemuDirectFB::Notify3DEvent(PRUint32 type, PRUint8 *reserved)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}

int NemuDirectFB::createSurface(uint32_t w, uint32_t h)
{
