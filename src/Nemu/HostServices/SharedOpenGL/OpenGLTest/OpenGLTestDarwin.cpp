 ...
 *  For determining the cached NemuOglIsOfflineRenderingAppropriate result.}
 */
static DECLCALLBACK(int32_t) nemuOglIsOfflineRenderingAppropriateOnce(void *pvUser)
{
...
bool RTCALL NemuOglIsOfflineRenderingAppropriate(void)
{
    ...
    int rc = RTOnce(&s_Once, nemuOglIsOfflineRenderingAppropriateOnce, &s_fCached);
    AssertRC(rc);
    return s_fCached;
}


bool RTCALL NemuOglIs3DAccelerationSupported(void)
{
    if (RTEnvExist("NEMU_CROGL_FORCE_SUPPORTED"))
    {
        LogRel(("NEMU_CROGL_FORCE_SUPPORTED is specified, skipping 3D test, and treating as supported\n"));
        return true;
    }
    ...
    CGLPixelFormatAttribute aAttribs[] =
    {
        ...
        kCGLPFAAccelerated,
        kCGLPFADoubleBuffer,
        NemuOglIsOfflineRenderingAppropriate() ? kCGLPFAAllowOfflineRenderers : (CGLPixelFormatAttribute)NULL,
