...
static DECLCALLBACK(int) pcbiosInitComplete(PPDMDEVINS pDevIns)
{
    ...
    /* Bochs/Nemu BIOS specific way of specifying memory above 4GB in 64KB units.
...
static DECLCALLBACK(void) pcbiosMemSetup(PPDMDEVINS pDevIns, PDMDEVMEMSETUPCTX enmCtx)
{
    ...
    RTGCPHYS    GCPhys = NEMU_LANBOOT_SEG << 4;
    while (cPages > 0)
    {
...
static DECLCALLBACK(int)  pcbiosConstruct(PPDMDEVINS pDevIns, int iInstance, PCFGMNODE pCfg)
{
    ...
    rc = FwCommonPlantDMITable(pDevIns, pThis->au8DMIPage, NEMU_DMI_TABLE_SIZE,
    ...
    if (pThis->u8IOAPIC)
    {
        FwCommonPlantMpsTable(pDevIns, pThis->au8DMIPage + NEMU_DMI_TABLE_SIZE,
                              _4K - NEMU_DMI_TABLE_SIZE, pThis->cCpus);
        LogRel(("PcBios: MPS table at %08x\n", NEMU_DMI_TABLE_BASE + NEMU_DMI_TABLE_SIZE));
    }
    ...
    if (pThis->pszLanBootFile)
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (RT_SUCCESS(rc))
            {
                if (cbFileLanBoot > _64K - (NEMU_LANBOOT_SEG << 4 & 0xffff))
    ...
    if (pu8LanBootBinary)
    {
        ...
        rc = PDMDevHlpROMRegister(pDevIns, NEMU_LANBOOT_SEG << 4,
                                  RT_MAX(cbLanBootBinary, _64K - (NEMU_LANBOOT_SEG << 4 & 0xffff)),
