...
int _init(void)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        rc = ddi_soft_state_init(&g_pvgdrvSolarisState, sizeof(nemuguest_state_t), 1);
        if (!rc)
        {
...
static int vgdrvSolarisClose(dev_t Dev, int flag, int fType, cred_t *pCred)
{
    ...
    PNEMUGUESTSESSION pSession = NULL;
    nemuguest_state_t *pState = ddi_get_soft_state(g_pvgdrvSolarisState, getminor(Dev));
    if (!pState)
    {
...
static int vgdrvSolarisRead(dev_t Dev, struct uio *pUio, cred_t *pCred)
{
    ...
    PNEMUGUESTSESSION pSession = pState->pSession;
...
static int vgdrvSolarisIOCtl(dev_t Dev, int Cmd, intptr_t pArg, int Mode, cred_t *pCred, int *pVal)
{
    ...
    /*
     * Read the request payload if any; requests like NEMUGUEST_IOCTL_CANCEL_ALL_WAITEVENTS have no data payload.
     */
    void *pvBuf = NULL;
    ...
    {
        /*
         * We Log() instead of LogRel() here because NEMUGUEST_IOCTL_WAITEVENT can return VERR_TIMEOUT,
         * NEMUGUEST_IOCTL_CANCEL_ALL_EVENTS can return VERR_INTERRUPTED and possibly more in the future;
...
static int vgdrvSolarisPoll(dev_t Dev, short fEvents, int fAnyYet, short *pReqEvents, struct pollhead **ppPollHead)
{
    ...
    if (RT_LIKELY(pState))
    {
        PNEMUGUESTSESSION pSession  = (PNEMUGUESTSESSION)pState->pSession;
        uint32_t u32CurSeq = ASMAtomicUoReadU32(&g_DevExt.u32MousePosChangedSeq);
        if (pSession->u32MousePosChangedSeq != u32CurSeq)
        {
