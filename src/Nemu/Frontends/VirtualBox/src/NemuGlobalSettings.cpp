/* $Id: NemuGlobalSettings.cpp $ */
/** @file
 * Nemu Qt GUI - NemuGlobalSettingsData, NemuGlobalSettings class implementation.
 */
...
/** @class NemuGlobalSettingsData
 *
 *  The NemuGlobalSettingsData class encapsulates the global settings
 *  of the VirtualBox.
 */

NemuGlobalSettingsData::NemuGlobalSettingsData()
{
    /* default settings */
#if defined (Q_WS_WIN)
...
NemuGlobalSettingsData::NemuGlobalSettingsData (const NemuGlobalSettingsData &that)
{
...
bool NemuGlobalSettingsData::operator== (const NemuGlobalSettingsData &that) const
{
    return this == &that ||
        (hostCombo == that.hostCombo &&
         autoCapture == that.autoCapture &&
...
/** @class NemuGlobalSettings
 *
 *  The NemuGlobalSettings class is a wrapper class for NemuGlobalSettingsData
 *  to implement implicit sharing of VirtualBox global data.
 */

/* Defined in NemuGlobal.cpp */
extern const char *gNemuLangIDRegExp;
...
gPropertyMap[] =
{
    ...
    { "GUI/LanguageID",                            "languageId",              gNemuLangIDRegExp, true },
...
void NemuGlobalSettings::setHostCombo (const QString &hostCombo)
{
...
bool NemuGlobalSettings::isFeatureActive (const char *aFeature) const
{
 ...
 *  returns and the nemu argument will hold all error info (and therefore
 *  nemu.isOk() will be false to indicate this).
 *
 ...
 */
void NemuGlobalSettings::load (CVirtualBox &nemu)
{
    for (size_t i = 0; i < SIZEOF_ARRAY(gPropertyMap); i++)
    {
        QString value = nemu.GetExtraData(gPropertyMap[i].publicName);
        if (!nemu.isOk())
            return;
        /* Check for the host key upgrade path. */
        if (   value.isEmpty()
            && QString(gPropertyMap[i].publicName) == "GUI/Input/HostKeyCombination")
            value = nemu.GetExtraData("GUI/Input/HostKey");
 ...
 *  nemu.isOk() will be false to indicate this).
 */
void NemuGlobalSettings::save (CVirtualBox &nemu) const
{
    for (size_t i = 0; i < SIZEOF_ARRAY (gPropertyMap); i++)
    {
        ...
        nemu.SetExtraData (gPropertyMap [i].publicName, value.toString());
        if (!nemu.isOk())
            return;
    }
}
 ...
 */
QString NemuGlobalSettings::publicProperty (const QString &publicName) const
{
    for (size_t i = 0; i < SIZEOF_ARRAY (gPropertyMap); i++)
    {
 ...
 *  @note   This method emits the #propertyChanged() signal.
 */
bool NemuGlobalSettings::setPublicProperty (const QString &publicName, const QString &value)
{
    for (size_t i = 0; i < SIZEOF_ARRAY (gPropertyMap); i++)
    {
        if (gPropertyMap [i].publicName == publicName)
...
void NemuGlobalSettings::setPropertyPrivate (size_t index, const QString &value)
{
    if (value.isEmpty())
    {
