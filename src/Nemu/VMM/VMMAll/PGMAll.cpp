...
#if 0 /* def NEMU_STRICT; triggers too often now */
    /*
 ...
 *
 * @returns Strict Nemu status code, special care required.
...
#if !defined(IN_RING3) && defined(NEMU_WITH_REM)
    /*
...
{
    NOREF(pVM);
    NEMUSTRICTRC rc = EMInterpretInstruction(pVCpu, pRegFrame, pvFault);
    if (rc == VERR_EM_INTERPRETER)
        rc = VINF_EM_RAW_EMULATE_INSTR;
    if (rc != VINF_SUCCESS)
        Log(("PGMInterpretInstruction: returns %Rrc (pvFault=%RGv)\n", NEMUSTRICTRC_VAL(rc), pvFault));
    return rc;
}
...
#ifndef NEMU_WITH_2X_4GB_ADDR_SPACE_IN_R0
...
#endif /* !NEMU_WITH_2X_4GB_ADDR_SPACE_IN_R0 */
#if !defined(IN_RC) && !defined(NEMU_WITH_2X_4GB_ADDR_SPACE_IN_R0)
/**
 * Performs the lazy mapping of the 32-bit guest PD.
 *
 ...
 *
 * @returns Nemu status code, with the following informational code for
 *          VM scheduling.
 ...
 */
#if (defined(NEMU_STRICT) && defined(IN_RING3)) || defined(DOXYGEN_RUNNING)
