/* $Id: NemuVgaMiniPortDxe.c $ */
/** @file
 * NemuVgaMiniPortDxe.c - VgaMiniPort Protocol Implementation.
 */
 ...
 */
typedef struct NEMUVGAMINIPORT
{
    /** The VGA Mini Port Protocol. */
    EFI_VGA_MINI_PORT_PROTOCOL  VgaMiniPort;
    /** Magic value, NEMU_VGA_MINI_PORT_MAGIC. */
    UINT32                      u32Magic;
...
/** Pointer to a NEMUVGAMINIPORT structure. */
typedef NEMUVGAMINIPORT *PNEMUVGAMINIPORT;

/** NEMUVGAMINIPORT::u32Magic value (Isaac Asimov). */
#define NEMU_VGA_MINI_PORT_MAGIC        0x19200102
/** NEMUVGAMINIPORT::u32Magic dead value. */
#define NEMU_VGA_MINI_PORT_MAGIC_DEAD   0x19920406
...
NemuVgaMiniPortDB_Supported(IN EFI_DRIVER_BINDING_PROTOCOL *This, IN EFI_HANDLE ControllerHandle,
                            IN EFI_DEVICE_PATH_PROTOCOL *RemainingDevicePath OPTIONAL);
static EFI_STATUS EFIAPI
NemuVgaMiniPortDB_Start(IN EFI_DRIVER_BINDING_PROTOCOL *This, IN EFI_HANDLE ControllerHandle,
                        IN EFI_DEVICE_PATH_PROTOCOL *RemainingDevicePath OPTIONAL);
static EFI_STATUS EFIAPI
NemuVgaMiniPortDB_Stop(IN EFI_DRIVER_BINDING_PROTOCOL *This, IN EFI_HANDLE ControllerHandle,
                       IN UINTN NumberOfChildren, IN EFI_HANDLE *ChildHandleBuffer OPTIONAL);


static EFI_STATUS EFIAPI
NemuVgaMiniPortVMP_SetMode(IN EFI_VGA_MINI_PORT_PROTOCOL *This, IN UINTN ModeNumber);


static EFI_STATUS EFIAPI
NemuVgaMiniPortCN_GetDriverName(IN EFI_COMPONENT_NAME_PROTOCOL *This,
                                IN CHAR8 *Language, OUT CHAR16 **DriverName);
static EFI_STATUS EFIAPI
NemuVgaMiniPortCN_GetControllerName(IN EFI_COMPONENT_NAME_PROTOCOL *This,
...
static EFI_STATUS EFIAPI
NemuVgaMiniPortCN2_GetDriverName(IN EFI_COMPONENT_NAME2_PROTOCOL *This,
                                 IN CHAR8 *Language, OUT CHAR16 **DriverName);
static EFI_STATUS EFIAPI
NemuVgaMiniPortCN2_GetControllerName(IN EFI_COMPONENT_NAME2_PROTOCOL *This,
                                     IN EFI_HANDLE ControllerHandle,
                                     IN EFI_HANDLE ChildHandle OPTIONAL,
                                     IN CHAR8 *Language,  OUT CHAR16 **ControllerName);
...
static EFI_DRIVER_BINDING_PROTOCOL          g_NemuVgaMiniPortDB =
{
    NemuVgaMiniPortDB_Supported,
    NemuVgaMiniPortDB_Start,
    NemuVgaMiniPortDB_Stop,
...
static const EFI_COMPONENT_NAME_PROTOCOL    g_NemuVgaMiniPortCN =
{
    NemuVgaMiniPortCN_GetDriverName,
    NemuVgaMiniPortCN_GetControllerName,
    "eng"
};

/** EFI Component Name 2 Protocol. */
static const EFI_COMPONENT_NAME2_PROTOCOL   g_NemuVgaMiniPortCN2 =
{
    NemuVgaMiniPortCN2_GetDriverName,
    NemuVgaMiniPortCN2_GetControllerName,
    "en"
};

/** Driver name translation table. */
static CONST EFI_UNICODE_STRING_TABLE       g_aNemuMiniPortDriverLangAndNames[] =
{
 ...
 */
EFI_STATUS EFIAPI
DxeInitializeNemuVgaMiniPort(IN EFI_HANDLE ImageHandle, IN EFI_SYSTEM_TABLE *SystemTable)
{
    EFI_STATUS  rc;
    DEBUG((DEBUG_INFO, "DxeInitializeNemuVgaMiniPort\n"));

    rc = EfiLibInstallDriverBindingComponentName2(ImageHandle, SystemTable,
                                                  &g_NemuVgaMiniPortDB, ImageHandle,
                                                  &g_NemuVgaMiniPortCN, &g_NemuVgaMiniPortCN2);
    ASSERT_EFI_ERROR(rc);
    return rc;
}
                        ...
                        IN EFI_DEVICE_PATH_PROTOCOL *RemainingDevicePath OPTIONAL)
{
    ...
    if (!EFI_ERROR(rc))
    {
         ...
         */
        PNEMUVGAMINIPORT pThisDev;
        rc = gBS->AllocatePool(EfiBootServicesData, sizeof(*pThisDev), (VOID **)&pThisDev);
        if (!EFI_ERROR(rc))
        {
            pThisDev->VgaMiniPort.SetMode                   = NemuVgaMiniPortVMP_SetMode;
            ...
            pThisDev->u32Magic                              = NEMU_VGA_MINI_PORT_MAGIC;
                       ...
                       IN UINTN NumberOfChildren, IN EFI_HANDLE *ChildHandleBuffer OPTIONAL)
{
    ...
    if (!EFI_ERROR(rc))
    {
        ASSERT(pThisDev->u32Magic == NEMU_VGA_MINI_PORT_MAGIC);
        ASSERT(pThisDev->hController == ControllerHandle);
        if (    pThisDev->u32Magic == NEMU_VGA_MINI_PORT_MAGIC
            &&  pThisDev->hController == ControllerHandle)
        {
            /*
            ...
            if (!EFI_ERROR(rc))
            {
                 ...
                 */
                pThisDev->u32Magic = NEMU_VGA_MINI_PORT_MAGIC_DEAD;
                gBS->FreePool(pThisDev);
...
NemuVgaMiniPortVMP_SetMode(IN EFI_VGA_MINI_PORT_PROTOCOL *This, IN UINTN ModeNumber)
{
    PNEMUVGAMINIPORT    pThisDev = (PNEMUVGAMINIPORT)This;
    UINT8               r[64];
    int                 i;
    ...
    if (pThisDev->u32Magic != NEMU_VGA_MINI_PORT_MAGIC)
    {
        DEBUG((DEBUG_INFO, "%a: u32Magic=%x/%x\n", __FUNCTION__, pThisDev->u32Magic, NEMU_VGA_MINI_PORT_MAGIC));
...
#define BOUTB(count, aport, dport)  \
     ...
     */
     ...
     /* set all Nemu extended registers */
     r[0] = 1;
                                ...
                                IN CHAR8 *Language, OUT CHAR16 **DriverName)
{
    return LookupUnicodeString2(Language,
                                ...
                                &g_aNemuMiniPortDriverLangAndNames[0],
                                DriverName,
                                TRUE);
}
