...
static DECLCALLBACK(int) drvHostBaseGetMediaSize(PDRVHOSTBASE pThis, uint64_t *pcb)
{
    ...
    /* convert nt status code to Nemu status code. */
 ...
 * @returns Some other Nemu status code on failures without sense code.
 *
...
static DECLCALLBACK(int) drvHostBaseMediaThread(RTTHREAD ThreadSelf, void *pvUser)
{
    ...
    if (s_hAtomDeviceChange == 0)
    {
        ...
        s_classDeviceChange.lpszClassName = "NEMU_DeviceChangeClass";
        s_classDeviceChange.hInstance     = GetModuleHandle("NemuDD.dll");
