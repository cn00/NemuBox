...
#define NEMU_EXPANDOSPU_SSM_MAGIC           0x3d3d3d3d
/* Modify Nemu Expando SPU SSM version if SSM data structure changed. */
#define NEMU_EXPANDOSPU_SSM_VERSION_ONE     1
#define NEMU_EXPANDOSPU_SSM_VERSION         NEMU_EXPANDOSPU_SSM_VERSION_ONE
 ...
 *
 * <NEMU_EXPANDOSPU_SSM_MAGIC>
 * <NEMU_EXPANDOSPU_SSM_VERSION>
...
{
    uint32_t    magic   = NEMU_EXPANDOSPU_SSM_MAGIC;
    uint32_t    version = NEMU_EXPANDOSPU_SSM_VERSION;
...
expandoSPULoadState(void *pData)
{
    ...
    if (magic == NEMU_EXPANDOSPU_SSM_MAGIC)
    {
        rc = SSMR3GetU32(pSSM, &version);
        AssertRCReturn(rc, rc);

        if (version >= NEMU_EXPANDOSPU_SSM_VERSION_ONE)
        {
            uint32_t cStates = 0;
            uint32_t i;
            bool     fSuccess = false;
