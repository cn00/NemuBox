...
/* Context activation is done by the caller. */
BOOL IWineD3DImpl_FillGLCaps(struct wined3d_adapter *adapter, struct NEMUVMSVGASHADERIF *pNemuShaderIf)
{
    ...
    NEMU_CHECK_GL_CALL(gl_renderer_str = (const char *)glGetString(GL_RENDERER));
    ...
    NEMU_CHECK_GL_CALL(gl_vendor_str = (const char *)glGetString(GL_VENDOR));
    ...
    NEMU_CHECK_GL_CALL(gl_version_str = (const char *)glGetString(GL_VERSION));
    ...
    NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_CLIP_PLANES, &gl_max));
    gl_info->limits.clipplanes = min(WINED3DMAXUSERCLIPPLANES, gl_max);
    TRACE_(d3d_caps)("ClipPlanes support - num Planes=%d\n", gl_max);

#ifdef NEMU_VMSVGA3D_DUAL_OPENGL_PROFILE
    glGetIntegerv(GL_MAX_LIGHTS, &gl_max);
    if (glGetError() != GL_NO_ERROR)
    {
        pNemuShaderIf->pfnSwitchInitProfile(pNemuShaderIf, true /*fOtherProfile*/);
        NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_LIGHTS, &gl_max));
        pNemuShaderIf->pfnSwitchInitProfile(pNemuShaderIf, false /*fOtherProfile*/);
    }
#else
    NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_LIGHTS, &gl_max));
#endif
    ...
    NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_TEXTURE_SIZE, &gl_max));
    ...
    {
        pNemuShaderIf->pfnSwitchInitProfile(pNemuShaderIf, true /*fOtherProfile*/);
        NEMU_CHECK_GL_CALL(glGetFloatv(GL_ALIASED_POINT_SIZE_RANGE, gl_floatv));
        if (glGetError() != GL_NO_ERROR)
    ...
    gl_info->supported[NEMU_SHARED_CONTEXTS] = TRUE;

#ifdef NEMU_WITH_VMSVGA
    {
        void *pvEnumCtx = NULL;
        char  szCurExt[256];
        while (pNemuShaderIf->pfnGetNextExtension(pNemuShaderIf, &pvEnumCtx, szCurExt, sizeof(szCurExt), false /*fOtherProfile*/))
            check_gl_extension(gl_info, szCurExt);

        /* The cheap way out. */
        pvEnumCtx = NULL;
        while (pNemuShaderIf->pfnGetNextExtension(pNemuShaderIf, &pvEnumCtx, szCurExt, sizeof(szCurExt), true /*fOtherProfile*/))
            check_gl_extension(gl_info, szCurExt);
    }
#else /* NEMU_WITH_VMSVGA */
    while (*GL_Extensions)
    {
        const char *start;
...
#undef USE_GL_FUNC
    ...
    if (gl_info->supported[NV_REGISTER_COMBINERS])
    {
        NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_GENERAL_COMBINERS_NV, &gl_max));
        gl_info->limits.general_combiners = gl_max;
        TRACE_(d3d_caps)("Max general combiners: %d.\n", gl_max);
    }
    if (gl_info->supported[ARB_DRAW_BUFFERS])
    {
        NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_DRAW_BUFFERS_ARB, &gl_max));
    ...
    if (gl_info->supported[ARB_MULTITEXTURE])
    {
        ...
        if (glGetError() != GL_NO_ERROR)
            NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_TEXTURE_IMAGE_UNITS, &gl_max));
#else
        NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_TEXTURE_UNITS_ARB, &gl_max));
#endif
        ...
        {
            GLint tmp;
            NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_TEXTURE_IMAGE_UNITS_ARB, &tmp));
        ...
        {
            GLint tmp;
            NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_VERTEX_TEXTURE_IMAGE_UNITS_ARB, &tmp));
            gl_info->limits.vertex_samplers = tmp;
            NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS_ARB, &tmp));
    ...
    if (gl_info->supported[ARB_VERTEX_BLEND])
    {
        ...
        {
            pNemuShaderIf->pfnSwitchInitProfile(pNemuShaderIf, true /*fOtherProfile*/);
            NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_VERTEX_UNITS_ARB, &gl_max));
            pNemuShaderIf->pfnSwitchInitProfile(pNemuShaderIf, false /*fOtherProfile*/);
        }
#else
        NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_VERTEX_UNITS_ARB, &gl_max));
    ...
    {
        NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_3D_TEXTURE_SIZE_EXT, &gl_max));
        gl_info->limits.texture3d_size = gl_max;
        TRACE_(d3d_caps)("Max texture3D size: %d.\n", gl_info->limits.texture3d_size);
    }
    if (gl_info->supported[EXT_TEXTURE_FILTER_ANISOTROPIC])
    {
        NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, &gl_max));
    ...
    if (gl_info->supported[ARB_FRAGMENT_PROGRAM])
    {
        ...
        if (glGetError() != GL_NO_ERROR)
            pNemuShaderIf->pfnSwitchInitProfile(pNemuShaderIf, true /*fOtherProfile*/);
#endif
        NEMU_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_FRAGMENT_PROGRAM_ARB, GL_MAX_PROGRAM_ENV_PARAMETERS_ARB, &gl_max)));
        gl_info->limits.arb_ps_float_constants = gl_max;
        TRACE_(d3d_caps)("Max ARB_FRAGMENT_PROGRAM float constants: %d.\n", gl_info->limits.arb_ps_float_constants);
        NEMU_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_FRAGMENT_PROGRAM_ARB, GL_MAX_PROGRAM_NATIVE_PARAMETERS_ARB, &gl_max)));
        gl_info->limits.arb_ps_native_constants = gl_max;
        TRACE_(d3d_caps)("Max ARB_FRAGMENT_PROGRAM native float constants: %d.\n",
                gl_info->limits.arb_ps_native_constants);
        NEMU_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_FRAGMENT_PROGRAM_ARB, GL_MAX_PROGRAM_NATIVE_TEMPORARIES_ARB, &gl_max)));
        gl_info->limits.arb_ps_temps = gl_max;
        TRACE_(d3d_caps)("Max ARB_FRAGMENT_PROGRAM native temporaries: %d.\n", gl_info->limits.arb_ps_temps);
        NEMU_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_FRAGMENT_PROGRAM_ARB, GL_MAX_PROGRAM_NATIVE_INSTRUCTIONS_ARB, &gl_max)));
        gl_info->limits.arb_ps_instructions = gl_max;
        TRACE_(d3d_caps)("Max ARB_FRAGMENT_PROGRAM native instructions: %d.\n", gl_info->limits.arb_ps_instructions);
        NEMU_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_FRAGMENT_PROGRAM_ARB, GL_MAX_PROGRAM_LOCAL_PARAMETERS_ARB, &gl_max)));
    ...
    if (gl_info->supported[ARB_VERTEX_PROGRAM])
    {
        ...
        if (glGetError() != GL_NO_ERROR)
            pNemuShaderIf->pfnSwitchInitProfile(pNemuShaderIf, true /*fOtherProfile*/);
#endif
        NEMU_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_VERTEX_PROGRAM_ARB, GL_MAX_PROGRAM_ENV_PARAMETERS_ARB, &gl_max)));
        gl_info->limits.arb_vs_float_constants = gl_max;
        TRACE_(d3d_caps)("Max ARB_VERTEX_PROGRAM float constants: %d.\n", gl_info->limits.arb_vs_float_constants);
        NEMU_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_VERTEX_PROGRAM_ARB, GL_MAX_PROGRAM_NATIVE_PARAMETERS_ARB, &gl_max)));
        gl_info->limits.arb_vs_native_constants = gl_max;
        TRACE_(d3d_caps)("Max ARB_VERTEX_PROGRAM native float constants: %d.\n",
                gl_info->limits.arb_vs_native_constants);
        NEMU_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_VERTEX_PROGRAM_ARB, GL_MAX_PROGRAM_NATIVE_TEMPORARIES_ARB, &gl_max)));
        gl_info->limits.arb_vs_temps = gl_max;
        TRACE_(d3d_caps)("Max ARB_VERTEX_PROGRAM native temporaries: %d.\n", gl_info->limits.arb_vs_temps);
        NEMU_CHECK_GL_CALL(GL_EXTCALL(glGetProgramivARB(GL_VERTEX_PROGRAM_ARB, GL_MAX_PROGRAM_NATIVE_INSTRUCTIONS_ARB, &gl_max)));
    ...
    if (gl_info->supported[ARB_VERTEX_SHADER])
    {
        NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_VERTEX_UNIFORM_COMPONENTS_ARB, &gl_max));
    ...
    {
        NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_FRAGMENT_UNIFORM_COMPONENTS_ARB, &gl_max));
        ...
        {
            pNemuShaderIf->pfnSwitchInitProfile(pNemuShaderIf, true /*fOtherProfile*/);
            NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_VARYING_FLOATS_ARB, &gl_max));
            pNemuShaderIf->pfnSwitchInitProfile(pNemuShaderIf, false /*fOtherProfile*/);
        }
#else
        NEMU_CHECK_GL_CALL(glGetIntegerv(GL_MAX_VARYING_FLOATS_ARB, &gl_max));
    ...
    if (gl_info->supported[NV_LIGHT_MAX_EXPONENT])
    {
        ...
        {
            pNemuShaderIf->pfnSwitchInitProfile(pNemuShaderIf, true /*fOtherProfile*/);
            NEMU_CHECK_GL_CALL(glGetFloatv(GL_MAX_SHININESS_NV, &gl_info->limits.shininess));
            pNemuShaderIf->pfnSwitchInitProfile(pNemuShaderIf, false /*fOtherProfile*/);
        }
#else
        NEMU_CHECK_GL_CALL(glGetFloatv(GL_MAX_SHININESS_NV, &gl_info->limits.shininess));
