...
#include "Nemu/hgcmsvc.h" /* For PNEMUHGCMSVCPARM. */
...
public:
    ...
    virtual ~GuestDnDMsg(void)
    {
        if (paParms)
        {
            ...
            {
                if (   paParms[i].type == NEMU_HGCM_SVC_PARM_PTR
...
public:
    ...
    PNEMUHGCMSVCPARM getNextParam(void)
    {
        if (cParms >= cParmsAlloc)
        {
            paParms = (PNEMUHGCMSVCPARM)RTMemRealloc(paParms, (cParmsAlloc + 4) * sizeof(NEMUHGCMSVCPARM));
            if (!paParms)
                throw VERR_NO_MEMORY;
            RT_BZERO(&paParms[cParmsAlloc], 4 * sizeof(NEMUHGCMSVCPARM));
            cParmsAlloc += 4;
        }
    ...
    PNEMUHGCMSVCPARM getParms(void) const { return paParms; }
    ...
    int setNextPointer(void *pvBuf, uint32_t cbBuf)
    {
        ...
        PNEMUHGCMSVCPARM pParm = getNextParam();
        if (!pParm)
            return VERR_NO_MEMORY;
...
public:
    ...
    int                        hostCall(uint32_t u32Function, uint32_t cParms, PNEMUHGCMSVCPARM paParms) const;
