...
packspu_NemuConCreate(struct NEMUUHGSMI *pHgsmi)
{
#if defined(NEMU_WITH_CRHGSMI) && defined(IN_GUEST)
    ThreadInfo * thread;
...
packspu_NemuConFlush(GLint con)
{
#if defined(NEMU_WITH_CRHGSMI) && defined(IN_GUEST)
...
#else
    crError("NemuConFlush not implemented!");
#endif
}

void PACKSPU_APIENTRY
packspu_NemuConDestroy(GLint con)
{
#if defined(NEMU_WITH_CRHGSMI) && defined(IN_GUEST)
    GET_THREAD_ID(thread, con);
    CRASSERT(con);
...
packspu_NemuConChromiumParameteriCR(GLint con, GLenum param, GLint value)
{
    GET_THREAD(thread);
...
GLvoid PACKSPU_APIENTRY
packspu_NemuConChromiumParametervCR(GLint con, GLenum target, GLenum type, GLsizei count, const GLvoid *values)
{
    GET_THREAD(thread);
    CRPackContext * curPacker = crPackGetContext();
    ThreadInfo *curThread = thread;
...
GLint PACKSPU_APIENTRY
packspu_NemuCreateContext( GLint con, const char *dpyName, GLint visual, GLint shareCtx )
{
    GET_THREAD(thread);
    CRPackContext * curPacker = crPackGetContext();
    ThreadInfo *curThread = thread;
...
{
    return packspu_NemuCreateContext( 0, dpyName, visual, shareCtx );
}
