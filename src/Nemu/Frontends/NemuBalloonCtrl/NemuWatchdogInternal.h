/* $Id: NemuWatchdogInternal.h $ */
/** @file
 * NemuWatchdog - VirtualBox Watchdog Service.
 */
...
#ifndef ___H_NEMUWATCHDOG
#define ___H_NEMUWATCHDOG
 ...
 */
typedef struct NEMUWATCHDOG_MODULE_PAYLOAD
{
...
} NEMUWATCHDOG_MODULE_PAYLOAD, *PNEMUWATCHDOG_MODULE_PAYLOAD;
 ...
 */
typedef std::map<const char*, NEMUWATCHDOG_MODULE_PAYLOAD> mapPayload;
typedef std::map<const char*, NEMUWATCHDOG_MODULE_PAYLOAD>::iterator mapPayloadIter;
typedef std::map<const char*, NEMUWATCHDOG_MODULE_PAYLOAD>::const_iterator mapPayloadIterConst;
...
typedef struct NEMUWATCHDOG_MACHINE
{
    ...
    mapPayload payload;
} NEMUWATCHDOG_MACHINE, *PNEMUWATCHDOG_MACHINE;
typedef std::map<Bstr, NEMUWATCHDOG_MACHINE> mapVM;
typedef std::map<Bstr, NEMUWATCHDOG_MACHINE>::iterator mapVMIter;
typedef std::map<Bstr, NEMUWATCHDOG_MACHINE>::const_iterator mapVMIterConst;
...
/** Pointer to a NEMUMODULE. */
typedef NEMUMODULE *PNEMUMODULE;
/** Pointer to a const NEMUMODULE. */
typedef NEMUMODULE const *PCNEMUMODULE;
...
extern NEMUMODULE g_ModBallooning;
extern NEMUMODULE g_ModAPIMonitor;
...
extern int getMetric(PNEMUWATCHDOG_MACHINE pMachine, const Bstr& strName, LONG *pulData);
void* payloadFrom(PNEMUWATCHDOG_MACHINE pMachine, const char *pszModule);
int payloadAlloc(PNEMUWATCHDOG_MACHINE pMachine, const char *pszModule, size_t cbSize, void **ppszPayload);
void payloadFree(PNEMUWATCHDOG_MACHINE pMachine, const char *pszModule);

PNEMUWATCHDOG_MACHINE getMachine(const Bstr& strUuid);
MachineState_T getMachineState(const PNEMUWATCHDOG_MACHINE pMachine);

int cfgGetValueStr(const ComPtr<IVirtualBox> &rptrNemu, const ComPtr<IMachine> &rptrMachine,
                   const char *pszGlobal, const char *pszVM, Utf8Str &strValue, Utf8Str strDefault);
int cfgGetValueULong(const ComPtr<IVirtualBox> &rptrNemu, const ComPtr<IMachine> &rptrMachine,
                     const char *pszGlobal, const char *pszVM, unsigned long *pulValue, unsigned long ulDefault);
RT_C_DECLS_END

#endif /* !___H_NEMUWATCHDOG */
