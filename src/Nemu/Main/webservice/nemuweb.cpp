 ...
 *      generated code in out/.../src/Nemu/Main/webservice/methodmaps.cpp
...
extern const unsigned char g_abNemuWebWSDL[];
extern const unsigned g_cbNemuWebWSDL;
...
// globals for nemuweb command-line arguments
...
static const char       *g_pcszSID = "nemuwebsrv";      // server ID for SSL session cache
#endif /* WITH_OPENSSL */
...
public:
    ...
    static int fnHttpGet(struct soap *soap)
    {
        ...
        soap_send_raw(soap, (const char *)g_abNemuWebWSDL, g_cbNemuWebWSDL);
        soap_end_send(soap);
        return SOAP_OK;
    }
    ...
    STDMETHOD(HandleEvent)(NemuEventType_T aType, IEvent *aEvent)
    {
        switch (aType)
        {
            case NemuEventType_OnNemuSVCAvailabilityChanged:
            {
                ...
                if (!fAvailable)
                {
                    WebLog("NemuSVC became unavailable\n");
                    {
                ...
                {
                    WebLog("NemuSVC became available\n");
                    util::AutoWriteLock vlock(g_pVirtualBoxLockHandle COMMA_LOCKVAL_SRC_POS);
...
int main(int argc, char *argv[])
{
    ...
    RTStrmPrintf(g_pStdErr, NEMU_PRODUCT " web service Version " NEMU_VERSION_STRING "\n"
                            "(C) 2007-" NEMU_C_YEAR " " NEMU_VENDOR "\n"
                            "All rights reserved.\n");
    ...
    rc = com::NemuLogRelCreate("web service", g_fDaemonize ? NULL : pszLogFile,
                               RTLOGFLAGS_PREFIX_THREAD | RTLOGFLAGS_PREFIX_TIME_PROG,
                               "all", "NEMUWEBSRV_RELEASE_LOG",
    ...
    if (g_fDaemonize)
    {
        ...
        {
            rc = com::GetNemuUserHomeDirectory(szLogFile, sizeof(szLogFile));
            if (RT_FAILURE(rc))
                 return RTMsgErrorExit(RTEXITCODE_FAILURE, "could not get base directory for logging: %Rrc", rc);
            rc = RTPathAppend(szLogFile, sizeof(szLogFile), "nemuwebsrv.log");
            if (RT_FAILURE(rc))
        ...
        rc = com::NemuLogRelCreate("web service", pszLogFile,
                                   RTLOGFLAGS_PREFIX_THREAD | RTLOGFLAGS_PREFIX_TIME_PROG,
                                   "all", "NEMUWEBSRV_RELEASE_LOG",
    ...
    ComPtr<IEventListener> nemuClientListener;
    {
        ...
        nemuClientListener = clientListener;
...
void RaiseSoapInvalidObjectFault(struct soap *soap,
                                 WSDLT_ID obj)
{
    _nemu__InvalidObjectFault *ex = soap_new__nemu__InvalidObjectFault(soap, 1);
    ex->badObjectID = obj;
    ...
    RaiseSoapFault(soap,
                   str.c_str(),
                   SOAP_TYPE__nemu__InvalidObjectFault,
                   ex);
}
                           ...
                           const com::Guid &iid)
{
    ...
    _nemu__RuntimeFault *ex = soap_new__nemu__RuntimeFault(soap, 1);
 ...
 *      findObjectFromRef() template function in nemuweb.h
 ...
 *  createOrFindRefFromComPtr() template function in nemuweb.h, which
 ...
 * @return
 */
int __nemu__IManagedObjectRef_USCOREgetInterfaceName(
    struct soap *soap,
    _nemu__IManagedObjectRef_USCOREgetInterfaceName *req,
    _nemu__IManagedObjectRef_USCOREgetInterfaceNameResponse *resp)
{
    HRESULT rc = S_OK;
    WEBDEBUG(("-- entering %s\n", __FUNCTION__));
 ...
 * @return
 */
int __nemu__IManagedObjectRef_USCORErelease(
    struct soap *soap,
 ...
 *
 * @param
 * @param nemu__IWebsessionManager_USCORElogon
 * @param nemu__IWebsessionManager_USCORElogonResponse
 * @return
 */
int __nemu__IWebsessionManager_USCORElogon(
        struct soap *soap,
 ...
 */
int __nemu__IWebsessionManager_USCOREgetSessionObject(
        struct soap*,
 ...
 * @return
 */
int __nemu__IWebsessionManager_USCORElogoff(
        struct soap*,
