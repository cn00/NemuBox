...
/** @file
 * Nemu Qt GUI - UIVMLogViewer class implementation.
 */
...
#ifdef NEMU_DARWIN_USE_NATIVE_CONTROLS
...
#endif /* NEMU_DARWIN_USE_NATIVE_CONTROLS */
...
void UIVMLogViewer::refresh()
{
    ...
    const CSystemProperties &sys = nemuGlobal().virtualBox().GetSystemProperties();
    unsigned cMaxLogs = sys.GetLogHistoryCount() + 1 /*Nemu.log*/ + 1 /*NemuHardening.log*/; /** @todo Add api for getting total possible log count! */
    ...
    if (!isAnyLogPresent)
    {
        QTextEdit *pDummyLog = createLogPage("Nemu.log");
