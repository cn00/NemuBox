...
/** Waiting time between probing whether NemuSVC is alive. */
#define NEMUCLIENT_DEFAULT_INTERVAL 30000
...
HRESULT VirtualBoxClient::init()
{
    ...
    /* Setting up the NemuSVC watcher thread. If anything goes wrong here it
    ...
    if (RT_SUCCESS(vrc))
    {
        vrc = RTThreadCreate(&mData.m_ThreadWatcher, SVCWatcherThread,
                             ...
                             RTTHREADFLAGS_WAITABLE, "NemuSVCWatcher");
        AssertRC(vrc);
    }
    else
    {
                                                     ...
                                                     void *pvUser)
{
    ...
    RTMSINTERVAL cMillies = NEMUCLIENT_DEFAULT_INTERVAL;
    int vrc;
    ...
    {
        {
            ...
            if (!pV.isNull())
            {
                ...
                {
                    LogRel(("VirtualBoxClient: detected unresponsive NemuSVC (rc=%Rhrc)\n", rc));
                    {
                        AutoWriteLock alock(pThis COMMA_LOCKVAL_SRC_POS);
                        /* Throw away the VirtualBox reference, it's no longer
                         * usable as NemuSVC terminated in the mean time. */
                        pThis->mData.m_pVirtualBox.setNull();
                    }
                    fireNemuSVCAvailabilityChangedEvent(pThis->mData.m_pEventSource, FALSE);
                }
            }
            else
            {
                ...
                if (FAILED(rc))
                    cMillies = 3 * NEMUCLIENT_DEFAULT_INTERVAL;
                else
                {
                    LogRel(("VirtualBoxClient: detected working NemuSVC (rc=%Rhrc)\n", rc));
                    {
                        AutoWriteLock alock(pThis COMMA_LOCKVAL_SRC_POS);
                        /* Update the VirtualBox reference, there's a working
                         * NemuSVC again from now on. */
                        pThis->mData.m_pVirtualBox = pVirtualBox;
                    }
                    fireNemuSVCAvailabilityChangedEvent(pThis->mData.m_pEventSource, TRUE);
                    cMillies = NEMUCLIENT_DEFAULT_INTERVAL;
                }
            }
        }
