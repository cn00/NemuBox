/* $Id: NemuNetBaseService.cpp $ */
/** @file
 * NemuNetBaseService - common services for NemuNetDHCP and NemuNetNAT.
 */
...
#include "NemuNetLib.h"
#include "NemuNetBaseService.h"
...
*********************************************************************************************************************************/
struct NemuNetBaseService::Data
{
...
DECLCALLBACK(int) NemuNetBaseService::Data::recvLoop(RTTHREAD, void *pvUser)
{
    NemuNetBaseService *pThis = static_cast<NemuNetBaseService *>(pvUser);
...
NemuNetBaseService::NemuNetBaseService(const std::string& aName, const std::string& aNetworkName):m(NULL)
{
    m = new NemuNetBaseService::Data(aName, aNetworkName);
...
NemuNetBaseService::~NemuNetBaseService()
{
    /*
...
bool NemuNetBaseService::isMainNeeded() const
{
    return m->m_fNeedMain;
}


int NemuNetBaseService::run()
{
    /**
 ...
 */
int NemuNetBaseService::parseArgs(int argc, char **argv)
{
...
int NemuNetBaseService::tryGoOnline(void)
{
    /*
    ...
    {
        m->m_pSession = NIL_RTR0PTR;
        LogRel(("NemuNetBaseService: SUPR3Init -> %Rrc\n", rc));
        return rc;
    }

    char szPath[RTPATH_MAX];
    rc = RTPathExecDir(szPath, sizeof(szPath) - sizeof("/NemuVMMR0.r0"));
    if (RT_FAILURE(rc))
    {
        LogRel(("NemuNetBaseService: RTPathExecDir -> %Rrc\n", rc));
        return rc;
    }

    rc = SUPR3LoadVMM(strcat(szPath, "/NemuVMMR0.r0"));
    if (RT_FAILURE(rc))
    {
        LogRel(("NemuNetBaseService: SUPR3LoadVMM(\"%s\") -> %Rrc\n", szPath, rc));
        return rc;
    }
    ...
    if (RT_FAILURE(rc))
    {
        Log2(("NemuNetBaseService: SUPR3CallVMMR0Ex(,VMMR0_DO_INTNET_OPEN,) failed, rc=%Rrc\n", rc));
        return rc;
    }
    m->m_hIf = OpenReq.hIf;
    ...
    if (RT_FAILURE(rc))
    {
        Log2(("NemuNetBaseService: SUPR3CallVMMR0Ex(,VMMR0_DO_INTNET_IF_GET_BUFFER_PTRS,) failed, rc=%Rrc\n", rc));
        return rc;
    }
    ...
    /* bail out */
    Log2(("NemuNetBaseService: SUPR3CallVMMR0Ex(,VMMR0_DO_INTNET_IF_SET_PROMISCUOUS_MODE,) failed, rc=%Rrc\n", rc));
...
void NemuNetBaseService::shutdown(void)
{
    syncEnter();
    if (!m->fShutdown)
    {
...
int NemuNetBaseService::syncEnter()
{
    return RTCritSectEnter(&m->m_csThis);
}


int NemuNetBaseService::syncLeave()
{
    return RTCritSectLeave(&m->m_csThis);
}


int NemuNetBaseService::waitForIntNetEvent(int cMillis)
{
...
int NemuNetBaseService::abortWait()
{
...
/* S/G API */
int NemuNetBaseService::sendBufferOnWire(PCINTNETSEG pcSg, int cSg, size_t cbFrame)
{
 ...
 */
void NemuNetBaseService::flushWire()
{
...
int NemuNetBaseService::hlpUDPBroadcast(unsigned uSrcPort, unsigned uDstPort,
                                         void const *pvData, size_t cbData) const
{
    return NemuNetUDPBroadcast(m->m_pSession, m->m_hIf, m->m_pIfBuf,
...
const std::string NemuNetBaseService::getServiceName() const
{
    return m->m_ServiceName;
}


void NemuNetBaseService::setServiceName(const std::string& aName)
{
    m->m_ServiceName = aName;
}


const std::string NemuNetBaseService::getNetworkName() const
{
    return m->m_NetworkName;
}


void NemuNetBaseService::setNetworkName(const std::string& aName)
{
    m->m_NetworkName = aName;
}


const RTMAC NemuNetBaseService::getMacAddress() const
{
    return m->m_MacAddress;
}


void NemuNetBaseService::setMacAddress(const RTMAC& aMac)
{
    m->m_MacAddress = aMac;
}


const RTNETADDRIPV4 NemuNetBaseService::getIpv4Address() const
{
    return m->m_Ipv4Address;
}


void NemuNetBaseService::setIpv4Address(const RTNETADDRIPV4& aAddress)
{
    m->m_Ipv4Address = aAddress;
}


const RTNETADDRIPV4 NemuNetBaseService::getIpv4Netmask() const
{
    return m->m_Ipv4Netmask;
}


void NemuNetBaseService::setIpv4Netmask(const RTNETADDRIPV4& aNetmask)
{
    m->m_Ipv4Netmask = aNetmask;
}


uint32_t NemuNetBaseService::getSendBufSize() const
{
    return m->m_cbSendBuf;
}


void NemuNetBaseService::setSendBufSize(uint32_t cbBuf)
{
    m->m_cbSendBuf = cbBuf;
}


uint32_t NemuNetBaseService::getRecvBufSize() const
{
    return m->m_cbRecvBuf;
}


void NemuNetBaseService::setRecvBufSize(uint32_t cbBuf)
{
    m->m_cbRecvBuf = cbBuf;
}


int32_t NemuNetBaseService::getVerbosityLevel() const
{
    return m->m_cVerbosity;
}


void NemuNetBaseService::setVerbosityLevel(int32_t aVerbosity)
{
    m->m_cVerbosity = aVerbosity;
}


void NemuNetBaseService::addCommandLineOption(const PRTGETOPTDEF optDef)
{
    m->m_vecOptionDefs.push_back(optDef);
}


void NemuNetBaseService::doReceiveLoop()
{
    int rc;
    /* Well we're ready */
    ...
    for (;;)
    {
        ...
        if (RT_FAILURE(rc))
        {
            ...
            LogRel(("NemuNetBaseService: waitForIntNetEvent returned %Rrc\n", rc));
            AssertRCReturnVoid(rc);
        }
            ...
            switch (u8Type)
            {
                case INTNETHDR_TYPE_FRAME:
                {
                    ...
                    if (RT_FAILURE(rc) && rc == VERR_IGNORED)
                    {
                        ...
                        NEMUNETUDPHDRS Hdrs;
                        size_t  cb;
                        void   *pv = NemuNetUDPMatch(m->m_pIfBuf, RTNETIPV4_PORT_BOOTPS, &m->m_MacAddress,
                                                       NEMUNETUDP_MATCH_UNICAST
                                                     | NEMUNETUDP_MATCH_BROADCAST
                                                     | NEMUNETUDP_MATCH_CHECKSUM
                                                     | (m->m_cVerbosity > 2 ? NEMUNETUDP_MATCH_PRINT_STDERR : 0),
                                                     &Hdrs, &cb);
                        if (pv && cb)
                            processUDP(pv, cb);
                        else
                            NemuNetArpHandleIt(m->m_pSession, m->m_hIf, m->m_pIfBuf, &m->m_MacAddress, m->m_Ipv4Address);
                    }
                    break;
                }
...
int NemuNetBaseService::startReceiveThreadAndEnterEventLoop()
{
    ...
    int rc = RTThreadCreate(&m->m_hThrRecv, /* thread handle*/
                            &NemuNetBaseService::Data::recvLoop,  /* routine */
                            this, /* user data */
                            128 * _1K, /* stack size */
...
void NemuNetBaseService::debugPrint(int32_t iMinLevel, bool fMsg, const char *pszFmt, ...) const
{
 ...
 */
void NemuNetBaseService::debugPrintV(int iMinLevel, bool fMsg, const char *pszFmt, va_list va) const
{
    if (iMinLevel <= m->m_cVerbosity)
    {
...
PRTGETOPTDEF NemuNetBaseService::getOptionsPtr()
{
