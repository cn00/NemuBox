...
int emR3RawStep(PVM pVM, PVMCPU pVCpu)
{
    ...
    if (fGuest)
    {
            ...
            ||  VMCPU_FF_IS_PENDING(pVCpu, VMCPU_FF_HIGH_PRIORITY_PRE_RAW_MASK))
        {
            ...
            NEMUVMM_EM_FF_RAW_RET(pVCpu, rc);
            if (rc != VINF_SUCCESS)
                return rc;
        }
...
#endif
{
     ...
     */
//#define NEMU_WITH_FIRST_IEM_STEP_B
#if defined(NEMU_WITH_FIRST_IEM_STEP_B) || !defined(NEMU_WITH_REM)
...
# ifndef NEMU_WITH_FIRST_IEM_STEP_B
    ...
    if (u8TrapNo == 6) /* (#UD) Invalid opcode. */
    {
            ...
            && (cpu.pCurInstr->uOpcode == OP_MONITOR || cpu.pCurInstr->uOpcode == OP_MWAIT))
        {
            ...
            if (u32ExtFeatures & X86_CPUID_FEATURE_ECX_MONITOR)
            {
                ...
                rc = NEMUSTRICTRC_TODO(EMInterpretInstructionDisasState(pVCpu, &cpu, CPUMCTX2CORE(pCtx), 0, EMCODETYPE_SUPERVISOR));
                if (RT_SUCCESS(rc))
                    return rc;
...
static int emR3RawForcedActions(PVM pVM, PVMCPU pVCpu, PCPUMCTX pCtx)
{
     ...
     */
    NEMUVMM_EM_FF_RAW(pVCpu, pVM->fGlobalForcedActions, pVCpu->fLocalForcedActions);
    ...
    {
        NEMUSTRICTRC rcStrict = SELMR3UpdateFromCPUM(pVM, pVCpu);
...
# endif
        ...
        if (RT_LIKELY(emR3IsExecutionAllowed(pVM, pVCpu)))
        {
            ...
            NEMUVMM_EM_RAW_RUN_PRE(pVCpu, pCtx);
            rc = VMMR3RawRunGC(pVM, pVCpu);
            NEMUVMM_EM_RAW_RUN_RET(pVCpu, pCtx, rc);
