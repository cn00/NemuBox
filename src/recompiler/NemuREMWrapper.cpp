/* $Id: NemuREMWrapper.cpp $ */
/** @file
 *
 * NemuREM Win64 DLL Wrapper.
 */
/*
...
/** @page pg_nemurem_amd64      NemuREM Hacks on AMD64
 *
 ...
 * NemuREM code as a relocatable module and use our ELF loader to load it.
 *
 ...
 *         IPRT. Part of IPRT will be linked into NemuREM2.rel, this will be yet another
 ...
 *         in libgcc). Call this NemuREM2.rel.
 *       - Write a wrapper dll, NemuREM.dll, for which during REMR3Init() will load
 *         NemuREM2.rel (using IPRT) and generate calling convention wrappers
 *         for all IPRT functions and NemuVMM functions that it uses. All exports
 ...
 *
 *
 * @section sec_nemurem_amd64_compare   Comparing the GCC and MSC calling conventions
 *
...
/** @def NEMU_USE_BITNESS_SELECTOR
 * Define NEMU_USE_BITNESS_SELECTOR to build this module as a bitness selector
 * between NemuREM32 and NemuREM64.
 */
#if defined(DOXYGEN_RUNNING)
# define NEMU_USE_BITNESS_SELECTOR
#endif

/** @def NEMU_WITHOUT_REM_LDR_CYCLE
 * Define NEMU_WITHOUT_REM_LDR_CYCLE dynamically resolve any dependencies on
 * NemuVMM and thus avoid the cyclic dependency between NemuREM and NemuVMM.
 */
#if defined(DOXYGEN_RUNNING)
# define NEMU_WITHOUT_REM_LDR_CYCLE
#endif
...
/** Loader handle of the NemuVMM DLL. */
...
# ifndef NEMU_USE_BITNESS_SELECTOR
...
#  if defined(NEMU_WITH_DEBUGGER) && !(defined(RT_OS_WINDOWS) && defined(RT_ARCH_AMD64)) /* the callbacks are problematic */
...
# endif /* !NEMU_USE_BITNESS_SELECTOR */
...
/**
 * Descriptors for the functions imported from NemuVMM.
 */
...
/**
 * Descriptors for the functions imported from NemuRT.
 */
...
#  ifdef NEMU_WITHOUT_REM_LDR_CYCLE
    /*
     * Resolve all the NemuVMM references.
     */
    if (g_ModVMM != NIL_RTLDRMOD)
    {
        rc = SUPR3HardenedLdrLoadAppPriv("NemuVMM", &g_ModVMM, RTLDRLOAD_FLAGS_LOCAL, NULL);
        AssertRCReturn(rc, rc);
        for (size_t i = 0; i < RT_ELEMENTS(g_aVMMImports); i++)
        {
            rc = RTLdrGetSymbol(g_ModVMM, g_aVMMImports[i].pszName, &g_aVMMImports[i].pv);
            AssertLogRelMsgRCReturn(rc, ("RTLdrGetSymbol(NemuVMM,%s,) -> %Rrc\n", g_aVMMImports[i].pszName, rc), rc);
        }
    }
#  endif

    /*
     * Load the NemuREM2.rel object/DLL.
     */
    strcpy(&szPath[offFilename], "/NemuREM2.rel");
...
#  ifdef DEBUG /* How to load the NemuREM2.rel symbols into the GNU debugger. */
            RTPrintf("NemuREMWrapper: (gdb) add-symbol-file %s 0x%p\n", szPath, g_pvREM2);
#  endif
...
# else  /* NEMU_USE_BITNESS_SELECTOR */
...
{
    /*
     * Load the NemuREM32/64 object/DLL.
     */
    const char *pszModule = remIs64bitEnabled(pVM) ? "NemuREM64" : "NemuREM32";
...
#elif defined(NEMU_USE_BITNESS_SELECTOR)
    if (!pfnREMR3Init)
    {
