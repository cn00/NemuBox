...
#ifdef RT_OS_SOLARIS
typedef struct NEMUSOLMAPINFO
{
    prmap_t                         pMap;                       /* Proc description of this mapping */
    int                             fError;                     /* Any error reading this mapping (errno) */
    struct NEMUSOLMAPINFO          *pNext;                      /* Pointer to the next mapping */
} NEMUSOLMAPINFO;
typedef NEMUSOLMAPINFO *PNEMUSOLMAPINFO;

typedef struct NEMUSOLTHREADINFO
{
    ...
    struct NEMUSOLTHREADINFO       *pNext;                      /* Pointer to the next thread */
} NEMUSOLTHREADINFO;
typedef NEMUSOLTHREADINFO *PNEMUSOLTHREADINFO;
#endif

typedef int (*PFNCOREREADER)(RTFILE hFile, void *pv, size_t cb);
typedef int (*PFNCOREWRITER)(RTFILE hFile, const void *pcv, size_t cb);

typedef struct NEMUPROCESS
{
    ...
    PNEMUSOLMAPINFO                 pMapInfoHead;               /* Pointer to the head of list of mappings */
    uint32_t                        cMappings;                  /* Number of mappings (count of pMapInfoHead list) */
    PNEMUSOLTHREADINFO              pThreadInfoHead;            /* Pointer to the head of list of threads */
...
typedef NEMUPROCESS *PNEMUPROCESS;

typedef struct NEMUCORE
{
    char                            szCorePath[PATH_MAX];       /* Path of the core file */
    NEMUPROCESS                     NemuProc;                   /* Current process information */
...
typedef NEMUCORE *PNEMUCORE;

typedef int (*PFNCOREACCUMULATOR)(PNEMUCORE pNemuCOre);
