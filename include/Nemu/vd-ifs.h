...
#ifndef ___Nemu_VD_Interfaces_h
#define ___Nemu_VD_Interfaces_h
...
typedef enum VDINTERFACETYPE
{
    ...
    /** Interface for I/O between the generic NemuHDD code and the backend. Per-image (internal).
 ...
 *                          success.
 */
NEMUDDU_DECL(int) VDIfCreateVfsStream(PVDINTERFACEIO pVDIfsIo, void *pvStorage, uint32_t fFlags, PRTVFSIOSTREAM phVfsIos);
 ...
 */
NEMUDDU_DECL(int) VDIfCreateVfsFile(PVDINTERFACEIO pVDIfs, struct VDINTERFACEIOINT *pVDIfsInt, void *pvStorage, uint32_t fFlags, PRTVFSFILE phVfsFile);
