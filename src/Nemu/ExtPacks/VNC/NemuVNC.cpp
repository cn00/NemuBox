...
DECLCALLBACK(int) VNCServerImpl::VRDEEnableConnections(HVRDESERVER hServer, bool fEnable)
{
    ...
    vncServer->desktopName = "NemuVNC";

#ifndef NEMU_USE_IPV6
...
DECLCALLBACK(void) VNCServerImpl::VRDEResize(HVRDESERVER hServer)
{
    ...
    {
        // Convert RGB (windows/nemu) to BGR(vnc)
        uint32_t i, j;
 ...
 * @param pvUpdate  Pointer to NemuGuest.h::VRDEORDERHDR structure with extra data.
