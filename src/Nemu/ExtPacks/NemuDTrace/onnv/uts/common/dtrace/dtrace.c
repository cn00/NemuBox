...
# define dtrace_casptr(a_ppvDst, a_pvOld, a_pvNew) \
	NemuDtCompareAndSwapPtr((void * volatile *)a_ppvDst, a_pvOld, a_pvNew)
DECLINLINE(void *) NemuDtCompareAndSwapPtr(void * volatile *ppvDst, void *pvOld, void *pvNew)
{
	void *pvRet;
...
# define dtrace_cas32(a_pu32Dst, a_pu32Old, a_pu32New) \
	NemuDtCompareAndSwapU32(a_pu32Dst, a_pu32Old, a_pu32New)
DECLINLINE(uint32_t) NemuDtCompareAndSwapU32(uint32_t volatile *pu32Dst, uint32_t u32Old, uint32_t u32New)
{
	uint32_t u32Ret;
...
#ifndef NEMU /* We have our own assertion machinery. */
int
...
#ifndef NEMU /* no destructive stuff */
...
#endif
#ifndef NEMU /* Avoid idProbe = UINT32_MAX as it is used as invalid value by VTG. */
...
#ifdef NEMU
        if (   NemuDtMutexInit(&dtrace_lock)
            || NemuDtMutexInit(&dtrace_provider_lock)
            || NemuDtMutexInit(&dtrace_meta_lock)
# ifdef DEBUG
            || NemuDtMutexInit(&dtrace_errlock)
# endif
            )
            return (DDI_FAILURE);
#endif
...
#ifndef NEMU /* Reduce the area a bit just to be sure our vmem fake doesn't blow up. */
...
#endif
{
	...
	case DTRACEIOC_BUFSNAP: {
		...
		if (/*Nemu value is is unsigned: desc.dtbd_cpu < 0 ||*/ desc.dtbd_cpu >= NCPU)
			return (EINVAL);
...
#endif
{
	...
	mutex_exit(&dtrace_provider_lock);
#ifdef NEMU
    NemuDtMutexDelete(&dtrace_lock);
    NemuDtMutexDelete(&dtrace_provider_lock);
    NemuDtMutexDelete(&dtrace_meta_lock);
# ifdef DEBUG
    NemuDtMutexDelete(&dtrace_errlock);
# endif
#endif
