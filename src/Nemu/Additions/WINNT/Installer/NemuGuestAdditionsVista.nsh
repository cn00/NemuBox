; $Id$
; @file
; NemuGuestAdditionsVista.nsh - Guest Additions installation for Windows Vista/7.
;
...
Function Vista_CheckForRequirements
  ...
  ${If} $0 == "1" ; D3D files are invalid, notify user
    MessageBox MB_ICONSTOP|MB_OKCANCEL $(NEMU_COMPONENT_D3D_INVALID) /SD IDOK IDCANCEL failure
    ; Offer to open up the Nemu online manual on how to fix missing/corrupted D3D files
    MessageBox MB_ICONQUESTION|MB_YESNO $(NEMU_COMPONENT_D3D_INVALID_MANUAL) /SD IDNO IDYES open_handbook_d3d_invalid    
  ${EndIf}
  Goto success
...
open_handbook_d3d_invalid:
  ...
  IfErrors 0 +2
    MessageBox MB_ICONSTOP|MB_OK $(NEMU_ERROR_OPEN_LINK) /SD IDOK
  Call SetAppMode32
  Goto failure
...
Function Vista_CopyFiles
  ...
  ; NemuNET drivers are not tested yet - commented out until officially supported and released
  ;FILE "$%PATH_OUT%\bin\additions\NemuNET.inf"
  ;FILE "$%PATH_OUT%\bin\additions\NemuNET.sys"
...
Function ${un}Vista_Uninstall
   ...
   Delete /REBOOTOK "$g_strSystemDir\NemuCredProv.dll"

   Call ${un}NemuMMR_Uninstall
...
!macro NemuMMR_Uninstall un
Function ${un}NemuMMR_Uninstall

  ; Remove NemuMMR always

  DetailPrint "Uninstalling NemuMMR."
  Call ${un}StopNemuMMR

  DeleteRegValue HKLM "Software\Microsoft\Windows\CurrentVersion\Run" "NemuMMR"

  Delete /REBOOTOK "$g_strSystemDir\NemuMMR.exe"

  !if $%BUILD_TARGET_ARCH% == "amd64"
    Delete /REBOOTOK "$g_strSysWow64\NemuMMRHook.dll"
    Delete /REBOOTOK "$INSTDIR\NemuMMR-x86.exe"
    Delete /REBOOTOK "$INSTDIR\NemuMMRHook-x86.dll"
  !else
    Delete /REBOOTOK "$g_strSystemDir\NemuMMRHook.dll"
    Delete /REBOOTOK "$INSTDIR\NemuMMR.exe"
    Delete /REBOOTOK "$INSTDIR\NemuMMRHook.dll"
  !endif

FunctionEnd
!macroend
!insertmacro NemuMMR_Uninstall ""
!insertmacro NemuMMR_Uninstall "un."
