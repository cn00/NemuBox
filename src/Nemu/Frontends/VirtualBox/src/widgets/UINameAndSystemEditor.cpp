...
/** @file
 * Nemu Qt GUI - UINameAndSystemEditor class implementation.
 */
...
void UINameAndSystemEditor::sltFamilyChanged(int iIndex)
{
    ...
    QList<CGuestOSType> types(nemuGlobal().vmGuestOSTypeList(strFamilyId));
...
void UINameAndSystemEditor::sltTypeChanged(int iIndex)
{
    ...
    m_type = nemuGlobal().vmGuestOSType(m_pTypeCombo->itemData(iIndex, TypeID).toString(),
                                        m_pFamilyCombo->itemData(m_pFamilyCombo->currentIndex(), TypeID).toString());
    m_pTypeIcon->setPixmap(nemuGlobal().vmGuestOSTypeIcon(m_type.GetId()));
