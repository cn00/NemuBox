/* $Id: NemuSnapshotDetailsDlg.cpp $ */
/** @file
 * Nemu Qt GUI - NemuSnapshotDetailsDlg class implementation.
 */
...
NemuSnapshotDetailsDlg::NemuSnapshotDetailsDlg (QWidget *aParent)
    : QIWithRetranslateUI <QDialog> (aParent)
{
    /* Apply UI decorations */
    Ui::NemuSnapshotDetailsDlg::setupUi (this);
...
void NemuSnapshotDetailsDlg::getFromSnapshot (const CSnapshot &aSnapshot)
{
    mSnapshot = aSnapshot;
...
void NemuSnapshotDetailsDlg::putBackToSnapshot()
{
    AssertReturn (!mSnapshot.isNull(), (void) 0);

    /* We need a session when we manipulate the snapshot data of a machine. */
    CSession session = nemuGlobal().openExistingSession(mSnapshot.GetMachine().GetId());
    if (session.isNull())
        return;
...
void NemuSnapshotDetailsDlg::retranslateUi()
{
    /* Translate uic generated strings */
    Ui::NemuSnapshotDetailsDlg::retranslateUi (this);
    ...
    mTeDetails->setText (nemuGlobal().detailsReport (machine, false /* with links? */));
}

bool NemuSnapshotDetailsDlg::eventFilter (QObject *aObject, QEvent *aEvent)
{
    Assert (aObject == mLbThumbnail);
    if (aEvent->type() == QEvent::MouseButtonPress && !mScreenshot.isNull())
    {
        NemuScreenshotViewer *viewer = new NemuScreenshotViewer (this, mScreenshot, mSnapshot.GetMachine().GetName(), mSnapshot.GetName());
        viewer->show();
    }
    return QDialog::eventFilter (aObject, aEvent);
}

void NemuSnapshotDetailsDlg::showEvent (QShowEvent *aEvent)
{
...
void NemuSnapshotDetailsDlg::onNameChanged (const QString &aText)
{
    mButtonBox->button (QDialogButtonBox::Ok)->setEnabled (!aText.trimmed().isEmpty());
}

NemuScreenshotViewer::NemuScreenshotViewer (QWidget *aParent, const QPixmap &aScreenshot,
...
void NemuScreenshotViewer::retranslateUi()
{
    setWindowTitle (tr ("Screenshot of %1 (%2)").arg (mSnapshotName).arg (mMachineName));
}

void NemuScreenshotViewer::showEvent (QShowEvent *aEvent)
{
    adjustPicture();
    QIWithRetranslateUI2 <QWidget>::showEvent (aEvent);
}

void NemuScreenshotViewer::resizeEvent (QResizeEvent *aEvent)
{
    adjustPicture();
    QIWithRetranslateUI2 <QWidget>::resizeEvent (aEvent);
}

void NemuScreenshotViewer::mousePressEvent (QMouseEvent *aEvent)
{
    mZoomMode = !mZoomMode;
    adjustPicture();
...
void NemuScreenshotViewer::keyPressEvent (QKeyEvent *aEvent)
{
...
void NemuScreenshotViewer::adjustPicture()
{
    if (mZoomMode)
    {
