...
/**
 * @interface_method_impl{DBGCCMDHLP,pfnNemuErrorV}
 */
static DECLCALLBACK(int) dbgcHlpNemuErrorV(PDBGCCMDHLP pCmdHlp, int rc, const char *pszFormat, va_list args)
{
    switch (rc)
    {
        case VINF_SUCCESS:
...
/**
 * @interface_method_impl{DBGCCMDHLP,pfnNemuError}
 */
static DECLCALLBACK(int) dbgcHlpNemuError(PDBGCCMDHLP pCmdHlp, int rc, const char *pszFormat, ...)
{
    va_list args;
    va_start(args, pszFormat);
    int rcRet = pCmdHlp->pfnNemuErrorV(pCmdHlp, rc, pszFormat, args);
    va_end(args);
    return rcRet;
}
...
void dbgcInitCmdHlp(PDBGC pDbgc)
{
    ...
    pDbgc->CmdHlp.pfnNemuErrorV         = dbgcHlpNemuErrorV;
    pDbgc->CmdHlp.pfnNemuError          = dbgcHlpNemuError;
