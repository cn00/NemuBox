...
/** @file
 * Nemu Qt GUI - UIKeyboardHandler class implementation.
 */
...
bool UIKeyboardHandler::x11EventFilter(XEvent *pEvent, ulong uScreenId)
{
    ...
    switch (pEvent->type)
    {
        ...
        case XFocusIn:
        {
            if (isSessionRunning())
            {
                if (NemuGlobal::qtRTVersion() < ((4 << 16) | (5 << 8) | 0))
                {
...
UIKeyboardHandler::UIKeyboardHandler(UIMachineLogic *pMachineLogic)
    ...
    , m_iKeyboardCaptureViewIndex(-1)
    , m_globalSettings(nemuGlobal().settings())
    , m_fIsKeyboardCaptured(false)
    , m_bIsHostComboPressed(false)
...
void UIKeyboardHandler::loadSettings()
{
    ...
    initMappedX11Keyboard(QX11Info::display(), nemuGlobal().settings().publicProperty("GUI/RemapScancodes"));
#endif

    /* Extra data settings: */
    {
        /* CAD setting: */
        m_fPassCADtoGuest = gEDataManager->passCADtoGuest(nemuGlobal().managedVMUuid());
    }
}
    ...
    if (EventKind != kEventRawKeyModifiersChanged)
    {
        ...
        if (scanCode)
        {
            ...
            if (EventKind != kEventRawKeyUp)
                flags |= KeyPressed;
            if (scanCode & NEMUKEY_EXTENDED)
                flags |= KeyExtended;
            /** @todo KeyPause, KeyPrint. */
            scanCode &= NEMUKEY_SCANCODE_MASK;
            ...
            if (isSessionRunning())
            {
                ...
                if (ok)
                {
                    ...
                    if (!uisession()->isMouseSupportsAbsolute() || !uisession()->isMouseIntegrated())
                    {
                        ...
                        if (m_fIsKeyboardCaptured)
                        {
                            const MouseCapturePolicy mcp = gEDataManager->mouseCapturePolicy(nemuGlobal().managedVMUuid());
