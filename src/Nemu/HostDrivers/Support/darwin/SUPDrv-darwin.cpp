...
#define NEMU_PROC_SELFNAME_LEN  (20)
#define NEMU_RETRIEVE_CUR_PROC_NAME(_name)  char _name[NEMU_PROC_SELFNAME_LEN]; \
                                            proc_selfname(pszProcName, NEMU_PROC_SELFNAME_LEN)
...
*********************************************************************************************************************************/
RT_C_DECLS_BEGIN
static kern_return_t    NemuDrvDarwinStart(struct kmod_info *pKModInfo, void *pvData);
static kern_return_t    NemuDrvDarwinStop(struct kmod_info *pKModInfo, void *pvData);

static int              NemuDrvDarwinOpen(dev_t Dev, int fFlags, int fDevType, struct proc *pProcess);
static int              NemuDrvDarwinClose(dev_t Dev, int fFlags, int fDevType, struct proc *pProcess);
static int              NemuDrvDarwinIOCtl(dev_t Dev, u_long iCmd, caddr_t pData, int fFlags, struct proc *pProcess);
static int              NemuDrvDarwinIOCtlSMAP(dev_t Dev, u_long iCmd, caddr_t pData, int fFlags, struct proc *pProcess);
static int              NemuDrvDarwinIOCtlSlow(PSUPDRVSESSION pSession, u_long iCmd, caddr_t pData, struct proc *pProcess);

static int              NemuDrvDarwinErr2DarwinErr(int rc);

static IOReturn         NemuDrvDarwinSleepHandler(void *pvTarget, void *pvRefCon, UInt32 uMessageType, IOService *pProvider, void *pvMessageArgument, vm_size_t argSize);
RT_C_DECLS_END

static int              nemudrvDarwinResolveSymbols(void);
static bool             nemudrvDarwinCpuHasSMAP(void);
...
DECLHIDDEN(kmod_start_func_t *) _realmain = NemuDrvDarwinStart;
DECLHIDDEN(kmod_stop_func_t *)  _antimain = NemuDrvDarwinStop;
...
static struct cdevsw    g_DevCW =
{
    ...
    /*.d_open  = */NemuDrvDarwinOpen,
    /*.d_close = */NemuDrvDarwinClose,
    /*.d_read  = */eno_rdwrt,
    /*.d_write = */eno_rdwrt,
    /*.d_ioctl = */NemuDrvDarwinIOCtl,
    /*.d_stop  = */eno_stop,
    /*.d_reset = */eno_reset,
    /*.d_ttys  = */NULL,
    /*.d_select= */eno_select,
...
static kern_return_t    NemuDrvDarwinStart(struct kmod_info *pKModInfo, void *pvData)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            rc = RTSpinlockCreate(&g_Spinlock, RTSPINLOCK_FLAGS_INTERRUPT_SAFE, "NemuDrvDarwin");
            if (RT_SUCCESS(rc))
            {
                if (nemudrvDarwinCpuHasSMAP())
                {
                    LogRel(("disabling SMAP for NemuDrvDarwinIOCtl\n"));
                    g_DevCW.d_ioctl = NemuDrvDarwinIOCtlSMAP;
                }

                /*
                 * Resolve some extra kernel symbols.
                 */
                rc = nemudrvDarwinResolveSymbols();
                if (RT_SUCCESS(rc))
                {
                    ...
                    if (g_iMajorDeviceNo >= 0)
                    {
                        ...
                        if (g_hDevFsDeviceSys)
                        {
                            ...
                            {
                                LogRel(("NemuDrv: version " NEMU_VERSION_STRING " r%d; IOCtl version %#x; IDC version %#x; dev major=%d\n",
                                        NEMU_SVN_REV, SUPDRV_IOC_VERSION, SUPDRV_IDC_VERSION, g_iMajorDeviceNo));

                                /* Register a sleep/wakeup notification callback */
                                g_pSleepNotifier = registerPrioritySleepWakeInterest(&NemuDrvDarwinSleepHandler, &g_DevExt, NULL);
                                if (g_pSleepNotifier == NULL)
                                    LogRel(("NemuDrv: register for sleep/wakeup events failed\n"));

                                return KMOD_RETURN_SUCCESS;
                            }

                            LogRel(("NemuDrv: devfs_make_node(makedev(%d,1),,,,%s) failed\n", g_iMajorDeviceNo, DEVICE_NAME_USR));
                            devfs_remove(g_hDevFsDeviceSys);
                            g_hDevFsDeviceSys = NULL;
                        }
                        else
                            LogRel(("NemuDrv: devfs_make_node(makedev(%d,0),,,,%s) failed\n", g_iMajorDeviceNo, DEVICE_NAME_SYS));

                        cdevsw_remove(g_iMajorDeviceNo, &g_DevCW);
                        g_iMajorDeviceNo = -1;
                    }
                    else
                        LogRel(("NemuDrv: cdevsw_add failed (%d)\n", g_iMajorDeviceNo));
                }
                RTSpinlockDestroy(g_Spinlock);
                g_Spinlock = NIL_RTSPINLOCK;
            }
            else
                LogRel(("NemuDrv: RTSpinlockCreate failed (rc=%d)\n", rc));
            supdrvDeleteDevExt(&g_DevExt);
        }
        else
            printf("NemuDrv: failed to initialize device extension (rc=%d)\n", rc);
        RTR0TermForced();
    }
    else
        printf("NemuDrv: failed to initialize IPRT (rc=%d)\n", rc);
...
static int nemudrvDarwinResolveSymbols(void)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        {
            LogRel(("NemuDrv: vmx_resume=%p vmx_suspend=%p vmx_use_count=%p (%d) cr4=%#x\n",
                    g_pfnVmxResume, g_pfnVmxSuspend, g_pVmxUseCount, *g_pVmxUseCount, ASMGetCR4() ));
        }
        else
        {
            LogRel(("NemuDrv: failed to resolve vmx stuff: vmx_resume=%Rrc vmx_suspend=%Rrc vmx_use_count=%Rrc", rc1, rc2, rc3));
            g_pfnVmxResume  = NULL;
...
# endif
            ...
            LogRel(("NemuDrv: g_pfnRdMsrCarefully=%p g_pfnRdMsr64Carefully=%p g_pfnWrMsr64Carefully=%p\n",
        ...
        LogRel(("NemuDrv: Failed to open kernel symbols, rc=%Rrc\n", rc));
    return rc;
}
...
/**
 * Device open. Called on open /dev/nemudrv
 *
...
#ifdef DEBUG_DARWIN_GIP
    OSDBGPRINT(("NemuDrvDarwinOpen: pid=%d '%s' pSession=%p rc=%d\n", proc_pid(pProcess), szName, pSession, rc));
#else
    Log(("NemuDrvDarwinOpen: g_DevExt=%p pSession=%p rc=%d pid=%d\n", &g_DevExt, pSession, rc, proc_pid(pProcess)));
#endif
    return NemuDrvDarwinErr2DarwinErr(rc);
}
...
static int NemuDrvDarwinIOCtl(dev_t Dev, u_long iCmd, caddr_t pData, int fFlags, struct proc *pProcess)
{
...
#ifdef NEMU_WITH_EFLAGS_AC_SET_IN_NEMUDRV
    /*
    ...
    {
        SUPR0Printf("NemuDrvDarwinIOCtl: EFLAGS.AC=0 detected %u times, refusing all I/O controls!\n", g_DevExt.cBadContextCalls);
        return EDEVERR;
    }
#endif
    ...
    {
        OSDBGPRINT(("NemuDrvDarwinIOCtl: WHAT?!? pSession == NULL! This must be a mistake... pid=%d iCmd=%#lx\n",
                    (int)Process, iCmd));
        return EINVAL;
    }
    ...
    if (   (    iCmd == SUP_IOCTL_FAST_DO_RAW_RUN
        ...
        rc = NemuDrvDarwinIOCtlSlow(pSession, iCmd, pData, pProcess);
...
{
    /*
     * Allow Nemu R0 code to touch R3 memory. Setting the AC bit disables the
     * SMAP check.
     */
    RTCCUINTREG fSavedEfl = ASMAddFlags(X86_EFL_AC);

    int rc = NemuDrvDarwinIOCtl(Dev, iCmd, pData, fFlags, pProcess);
...
/**
 * Worker for NemuDrvDarwinIOCtl that takes the slow IOCtl functions.
 *
...
static int NemuDrvDarwinIOCtlSlow(PSUPDRVSESSION pSession, u_long iCmd, caddr_t pData, struct proc *pProcess)
{
    LogFlow(("NemuDrvDarwinIOCtlSlow: pSession=%p iCmd=%p pData=%p pProcess=%p\n", pSession, iCmd, pData, pProcess));
    ...
    if ((IOC_DIRMASK & iCmd) == IOC_INOUT)
    {
        ...
        {
            OSDBGPRINT(("NemuDrvDarwinIOCtlSlow: cbReq=%#x < %#x; iCmd=%#lx\n", cbReq, (int)sizeof(*pHdr), iCmd));
            return EINVAL;
        }
        if (RT_UNLIKELY((pHdr->fFlags & SUPREQHDR_FLAGS_MAGIC_MASK) != SUPREQHDR_FLAGS_MAGIC))
        {
            OSDBGPRINT(("NemuDrvDarwinIOCtlSlow: bad magic fFlags=%#x; iCmd=%#lx\n", pHdr->fFlags, iCmd));
            return EINVAL;
        }
        ...
        {
            OSDBGPRINT(("NemuDrvDarwinIOCtlSlow: max(%#x,%#x) != %#x; iCmd=%#lx\n", pHdr->cbIn, pHdr->cbOut, cbReq, iCmd));
            return EINVAL;
        }
    }
    else if ((IOC_DIRMASK & iCmd) == IOC_VOID && !cbReq)
    {
        ...
        if (RT_UNLIKELY(rc))
        {
            OSDBGPRINT(("NemuDrvDarwinIOCtlSlow: copyin(%llx,Hdr,) -> %#x; iCmd=%#lx\n", (unsigned long long)pUser, rc, iCmd));
        ...
        if (RT_UNLIKELY(    Hdr.cbIn < sizeof(Hdr)
                        ...
                        ||  cbReq > _1M*16))
        {
            OSDBGPRINT(("NemuDrvDarwinIOCtlSlow: max(%#x,%#x); iCmd=%#lx\n", Hdr.cbIn, Hdr.cbOut, iCmd));
            IPRT_DARWIN_SAVE_EFL_AC();
            return EINVAL;
        }
        ...
        if (RT_UNLIKELY(!pHdr))
        {
            OSDBGPRINT(("NemuDrvDarwinIOCtlSlow: failed to allocate buffer of %d bytes; iCmd=%#lx\n", cbReq, iCmd));
            IPRT_DARWIN_RESTORE_EFL_AC();
            return ENOMEM;
        }
        ...
        if (RT_UNLIKELY(rc))
        {
            OSDBGPRINT(("NemuDrvDarwinIOCtlSlow: copyin(%llx,%p,%#x) -> %#x; iCmd=%#lx\n",
    ...
    {
        Log(("NemuDrvDarwinIOCtlSlow: huh? cbReq=%#x iCmd=%#lx\n", cbReq, iCmd));
        return EINVAL;
    }
    ...
    if (RT_LIKELY(!rc))
    {
        ...
        if (pUser)
        {
            ...
            if (cbOut > cbReq)
            {
                OSDBGPRINT(("NemuDrvDarwinIOCtlSlow: too much output! %#x > %#x; uCmd=%#lx!\n", cbOut, cbReq, iCmd));
                cbOut = cbReq;
            }
            rc = copyout(pHdr, pUser, cbOut);
            if (RT_UNLIKELY(rc))
                OSDBGPRINT(("NemuDrvDarwinIOCtlSlow: copyout(%p,%llx,%#x) -> %d; uCmd=%#lx!\n",
    ...
    else
    {
        ...
        Log(("NemuDrvDarwinIOCtlSlow: pid=%d iCmd=%lx pData=%p failed, rc=%d\n", proc_pid(pProcess), iCmd, (void *)pData, rc));
        rc = EINVAL;
    }

    Log2(("NemuDrvDarwinIOCtlSlow: returns %d\n", rc));
    return rc;
}
 ...
 */
DECLEXPORT(int) NEMUCALL SUPDrvDarwinIDC(uint32_t uReq, PSUPDRVIDCREQHDR pReq)
{
    PSUPDRVSESSION  pSession;
 ...
 */
IOReturn NemuDrvDarwinSleepHandler(void * /* pvTarget */, void *pvRefCon, UInt32 uMessageType, IOService * /* pProvider */, void * /* pvMessageArgument */, vm_size_t /* argSize */)
{
    LogFlow(("NemuDrv: Got sleep/wake notice. Message type was %X\n", (uint)uMessageType));
 ...
 */
static DECLCALLBACK(void) nemudrvDarwinVmxEnableFix(RTCPUID idCpu, void *pvUser1, void *pvUser2)
{
 ...
 */
int NEMUCALL supdrvOSEnableVTx(bool fEnable)
{
#ifdef NEMU_WITH_HOST_VMX
    int rc;
...
# endif
       )
    {
        ...
        if (fEnable)
        {
                ...
                if (version_major == 14 /* 14 = 10.10 = yosemite */)
                {
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        if (fCaps & SUPVTCAPS_VT_X)
                            rc = RTMpOnAll(nemudrvDarwinVmxEnableFix, NULL, NULL);
                        else
                            rc = VERR_VMX_NO_VMX;
                    }
                    if (RT_FAILURE(rc))
            ...
            LogRel(("NemuDrv: host_vmxon  -> vmx_use_count=%d rc=%Rrc\n", *g_pVmxUseCount, rc));
        }
        else
        {
            ...
            host_vmxoff();
            rc = VINF_SUCCESS;
            LogRel(("NemuDrv: host_vmxoff -> vmx_use_count=%d\n", *g_pVmxUseCount));
        }
        IPRT_DARWIN_RESTORE_EFL_AC();
    }
    else
 ...
 */
bool NEMUCALL supdrvOSSuspendVTxOnCpu(void)
{
#ifdef NEMU_WITH_HOST_VMX
    /*
 ...
 */
void NEMUCALL   supdrvOSResumeVTxOnCpu(bool fSuspended)
{
#ifdef NEMU_WITH_HOST_VMX
    /*
...
void NEMUCALL   supdrvOSLdrNotifyOpened(PSUPDRVDEVEXT pDevExt, PSUPDRVLDRIMAGE pImage)
{
    ...
    {
        char szVar[80];
        RTStrPrintf(szVar, sizeof(szVar), "nemuimage"/*-%s*/, pImage->szName);
        char szValue[48];
 ...
 */
int NEMUCALL    supdrvDarwinResumeSuspendedKbds(void)
{
    IPRT_DARWIN_SAVE_EFL_AC();
...
{
    uint32_t fFlags = 0;
    if (g_DevCW.d_ioctl == NemuDrvDarwinIOCtlSMAP)
...
bool org_virtualbox_SupDrvClient::initWithTask(task_t OwningTask, void *pvSecurityId, UInt32 u32Type)
{
    ...
    NEMU_RETRIEVE_CUR_PROC_NAME(pszProcName);
...
/**
 * Common worker for clientClose and NemuDrvDarwinClose.
 */
/* static */ void org_virtualbox_SupDrvClient::sessionClose(RTPROCESS Process)
{
     ...
     * NemuDrvDarwinOpen makes sure this is so.)
     */
    const unsigned  iHash = SESSION_HASH(Process);
    RTSpinlockAcquire(g_Spinlock);
