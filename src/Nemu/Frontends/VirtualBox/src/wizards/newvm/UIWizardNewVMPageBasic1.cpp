...
/** @file
 * Nemu Qt GUI - UIWizardNewVMPageBasic1 class implementation.
 */
...
void UIWizardNewVMPage1::onNameChanged(QString strNewName)
{
    ...
    for (size_t i = 0; i < RT_ELEMENTS(gs_OSTypePattern); ++i)
        if (strNewName.contains(gs_OSTypePattern[i].pattern))
        {
            ...
            m_pNameAndSystemEditor->setType(nemuGlobal().vmGuestOSType(gs_OSTypePattern[i].pcstId));
...
bool UIWizardNewVMPage1::createMachineFolder()
{
    ...
    QString strDefaultMachinesFolder = nemu.GetSystemProperties().GetDefaultMachineFolder();
    /* Compose machine filename: */
    QString strMachineFilename = nemu.ComposeMachineFilename(m_pNameAndSystemEditor->name(), m_strGroup, QString::null, strDefaultMachinesFolder);
