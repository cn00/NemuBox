  ...
  )
{
  ...
  if (RemainingDevicePath != NULL) {
    ...
    //
    NemuLogFlowFuncMarkDP(RemainingDevicePath);
    if (!IsDevicePathEnd (RemainingDevicePath)) {
      ...
      if (     Node->DevPath.Type != MEDIA_DEVICE_PATH
        ...
        NemuLogFlowFuncLeaveRC(EFI_UNSUPPORTED);
  ...
  NemuLogFlowFuncMarkDP(ParentDevicePath);
  if (Status == EFI_ALREADY_STARTED) {
    NemuLogFlowFuncLeaveRC(EFI_SUCCESS);
    return EFI_SUCCESS;
  }
  ...
  if (BlockIo->Media->MediaPresent ||
    ...
    while (*Routine != NULL) {
      ...
      if (!EFI_ERROR (Status) || Status == EFI_MEDIA_CHANGED || Status == EFI_NO_MEDIA) {
        NemuLogFlowFuncMarkVar(idxRoutine, "%d");
        break;
      }
      Routine++;
      idxRoutine++;
