...
int main (int argc, char* argv[])
{
    ...
    char szSetupStub[_MAX_PATH] = {"NemuStub.exe"};
    char szOutput[_MAX_PATH] = {"VirtualBox-MultiArch.exe"};
    HANDLE hUpdate = NULL;

    do
    {
        printf(NEMU_PRODUCT " Stub Builder v%d.%d.%d.%d\n",
        ...
        NEMUSTUBBUILDPKG stbBuildPkg[NEMUSTUB_MAX_PACKAGES] = {0};
        NEMUSTUBPKG stbPkg[NEMUSTUB_MAX_PACKAGES] = {0};
        NEMUSTUBPKGHEADER stbHeader =
        {
            "nemu$tub",    /* File magic. */
            1,             /* Version. */
            0              /* No files yet. */
        ...
        for (int i=1; i<argc; i++)
        {
            ...
            else if (0 == stricmp(argv[i], "-target-all") && argc > i+1)
            {
                ...
                stbBuildPkg[stbHeader.byCntPkgs].byArch = NEMUSTUBPKGARCH_ALL;
                stbHeader.byCntPkgs++;
                i++;
            }
            ...
            {
                hr = ::StringCchCopy(stbBuildPkg[stbHeader.byCntPkgs].szSourcePath, _MAX_PATH, argv[i+1]);
                stbBuildPkg[stbHeader.byCntPkgs].byArch = NEMUSTUBPKGARCH_X86;
                stbHeader.byCntPkgs++;
                i++;
            }
            ...
            {
                hr = ::StringCchCopy(stbBuildPkg[stbHeader.byCntPkgs].szSourcePath, _MAX_PATH, argv[i+1]);
                stbBuildPkg[stbHeader.byCntPkgs].byArch = NEMUSTUBPKGARCH_AMD64;
                stbHeader.byCntPkgs++;
                i++;
            }
            else
        ...
        for (BYTE i=0; i<stbHeader.byCntPkgs; i++)
        {
            ...
            hr = UpdateResource(hUpdate, RT_RCDATA, szHeaderName, MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), pPackage, sizeof(NEMUSTUBPKG));
        ...
        if (!::UpdateResource(hUpdate, RT_RCDATA, "MANIFEST", MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), &stbHeader, sizeof(NEMUSTUBPKGHEADER)))
        {
            hr = HRESULT_FROM_WIN32(GetLastError());
            break;
        }
