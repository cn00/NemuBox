...
#include <Nemu/NemuVideo.h> /* required by DevVGA.h */
...
*********************************************************************************************************************************/
#ifndef NEMU_VMSVGA3D_DEFAULT_OGL_PROFILE
# define NEMU_VMSVGA3D_DEFAULT_OGL_PROFILE 1.0
#endif
...
/* Generated by NemuDef2LazyLoad from the NemuSVGA3D.def and NemuSVGA3DObjC.def files. */
extern "C" int ExplicitlyLoadNemuSVGA3D(bool fResolveAllImports, PRTERRINFO pErrInfo);
...
static bool vmsvga3dCheckGLExtension(PVMSVGA3DSTATE pState, float fMinGLVersion, const char *pszWantedExtension)
{
    ...
    AssertMsg(   fMinGLVersion == 0.0
              ...
              || NEMU_VMSVGA3D_DEFAULT_OGL_PROFILE == 2.1,
...
/**
 * @interface_method_impl{NEMUVMSVGASHADERIF,pfnSwitchInitProfile}
 */
static DECLCALLBACK(void) vmsvga3dShaderIfSwitchInitProfile(PNEMUVMSVGASHADERIF pThis, bool fOtherProfile)
{
#ifdef NEMU_VMSVGA3D_DUAL_OPENGL_PROFILE
...
/**
 * @interface_method_impl{NEMUVMSVGASHADERIF,pfnGetNextExtension}
 */
static DECLCALLBACK(bool) vmsvga3dShaderIfGetNextExtension(PNEMUVMSVGASHADERIF pThis, void **ppvEnumCtx,
...
int vmsvga3dInit(PVGASTATE pThis)
{
    ...
    int rc = ExplicitlyLoadNemuSVGA3D(true /*fResolveAllImports*/, RTErrInfoInitStatic(&ErrInfo));
    if (RT_FAILURE(rc))
    {
        LogRel(("VMSVGA3d: Error loading NemuSVGA3D and resolving necessary functions: %Rrc - %s\n", rc, ErrInfo.Core.pszMsg));
        return rc;
    }
#ifdef RT_OS_DARWIN
    rc = ExplicitlyLoadNemuSVGA3DObjC(true /*fResolveAllImports*/, RTErrInfoInitStatic(&ErrInfo));
    if (RT_FAILURE(rc))
    {
        LogRel(("VMSVGA3d: Error loading NemuSVGA3DObjC and resolving necessary functions: %Rrc - %s\n", rc, ErrInfo.Core.pszMsg));
        return rc;
    }
#endif
...
int vmsvga3dPowerOn(PVGASTATE pThis)
{
    ...
    rc = vmsvga3dGatherExtensions(&pState->pszExtensions, NEMU_VMSVGA3D_DEFAULT_OGL_PROFILE);
    AssertRCReturn(rc, rc);
    ...
    rc = vmsvga3dGatherExtensions(&pState->pszOtherExtensions, NEMU_VMSVGA3D_OTHER_OGL_PROFILE);
    AssertRCReturn(rc, rc);
...
#if NEMU_VMSVGA3D_GL_HACK_LEVEL < 0x102
...
#if NEMU_VMSVGA3D_GL_HACK_LEVEL < 0x103
...
#ifdef NEMU_VMSVGA3D_DUAL_OPENGL_PROFILE /* The alternative profile has a higher number here (ati/darwin). */
    VMSVGA3D_SET_CURRENT_CONTEXT(pState, pOtherCtx);
                                  ...
                                  SVGA3dCopyBox const *pBox, PVMSVGA3DCONTEXT pContext, int rc, int iBox)
{
    ...
    case SVGA3D_SURFACE_HINT_INDEXBUFFER:
    {
        ...
        if (VMSVGA3D_GL_IS_SUCCESS(pContext))
        {
            ...
            {
#if defined(NEMU_STRICT) && defined(RT_OS_DARWIN)
...
#if !defined(NEMU_VMSVGA3D_DUAL_OPENGL_PROFILE) || !(defined(RT_OS_DARWIN))
