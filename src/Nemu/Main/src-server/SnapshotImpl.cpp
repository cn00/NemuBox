/* $Id: SnapshotImpl.cpp $ */
/** @file
 * COM class implementation for Snapshot and SnapshotMachine in NemuSVC.
 */
...
void SessionMachine::i_takeSnapshotHandler(TakeSnapshotTask &task)
{
    ...
    try
    {
        ...
        {
            Bstr value;
            rc = GetExtraData(Bstr("NemuInternal2/ForceTakeSnapshotWithoutState").raw(),
                              value.asOutParam());
...
HRESULT SessionMachine::finishOnlineMergeMedium()
{
    ...
    else
    {
        ...
        if (pDeleteRec->mpChildrenToReparent && !pDeleteRec->mpChildrenToReparent->IsEmpty())
        {
            ...
            // need manual fixing via NemuManage to adjust the parent UUID.
            treeLock.release();
