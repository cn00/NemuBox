...
# if defined(RT_OS_DARWIN) && defined(NEMU_WITH_CROGL)
...
#ifdef RT_OS_OS2
# define NEMUSHAREDFOLDERS_DLL "NemuSFld"
#else
# define NEMUSHAREDFOLDERS_DLL "NemuSharedFolders"
#endif
                                           ...
                                           uint32_t fFlags, PCRTTIMESPEC pTimeSpecTS)
{
    ...
    guest->i_setAdditionsStatus((NemuGuestFacilityType)uFacility, (NemuGuestFacilityStatus)uStatus, fFlags, pTimeSpecTS);
                                              ...
                                              const uint8_t *puDetails, uint32_t cbDetails)
{
    ...
    pGuest->i_onUserStateChange(Bstr(pszUser), Bstr(pszDomain), (NemuGuestUserState)uState,
                                puDetails, cbDetails);
}
 ...
 */
DECLCALLBACK(void) vmmdevUpdateGuestInfo(PPDMIVMMDEVCONNECTOR pInterface, const NemuGuestInfo *guestInfo)
{
    ...
    else
    {
        ...
        guest->i_setAdditionsStatus(NemuGuestFacilityType_All, NemuGuestFacilityStatus_Inactive, 0 /*fFlags*/, &TimeSpecTS);
        pConsole->i_onAdditionsStateChange();
    }
}
 ...
 * @thread  The emulation thread.
 */
DECLCALLBACK(int) vmmdevReportStatistics(PPDMIVMMDEVCONNECTOR pInterface, NemuGuestStatistics *pGuestStats)
{
    PDRVMAINVMMDEV pDrv = RT_FROM_MEMBER(pInterface, DRVMAINVMMDEV, Connector);
    Console *pConsole = pDrv->pVMMDev->getParent();
...
static DECLCALLBACK(int) iface_hgcmConnect(PPDMIHGCMCONNECTOR pInterface, PNEMUHGCMCMD pCmd,
...
static DECLCALLBACK(int) iface_hgcmDisconnect(PPDMIHGCMCONNECTOR pInterface, PNEMUHGCMCMD pCmd, uint32_t u32ClientID)
{
    Log9(("Enter\n"));
...
static DECLCALLBACK(int) iface_hgcmCall(PPDMIHGCMCONNECTOR pInterface, PNEMUHGCMCMD pCmd, uint32_t u32ClientID,
                                        uint32_t u32Function, uint32_t cParms, PNEMUHGCMSVCPARM paParms)
{
    Log9(("Enter\n"));
...
int VMMDev::hgcmHostFastCallAsync(HGCMCVSHANDLE hSvc, uint32_t function, PNEMUHGCMSVCPARM pParm,
...
DECLCALLBACK(int) VMMDev::drvConstruct(PPDMDRVINS pDrvIns, PCFGMNODE pCfgHandle, uint32_t fFlags)
{
...
#ifdef NEMU_WITH_HGCM
    rc = pThis->pVMMDev->hgcmLoadService(NEMUSHAREDFOLDERS_DLL,
                                         "NemuSharedFolders");
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc) && pLed)
        {
            ...
            rc = HGCMHostCall("NemuSharedFolders", SHFL_FN_SET_STATUS_LED, 1, &parm);
        }
        else
