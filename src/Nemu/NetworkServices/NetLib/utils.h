...
inline int findDhcpServer(const ComVirtualBoxPtr& nemu, const std::string& name, ComDhcpServerPtr& dhcp)
{
    HRESULT hrc = nemu->FindDHCPServerByNetworkName(com::Bstr(name.c_str()).raw(),
                                                          dhcp.asOutParam());
...
inline int findNatNetwork(const ComVirtualBoxPtr& nemu, const std::string& name, ComNatPtr& nat)
{
    HRESULT hrc = nemu->FindNATNetworkByName(com::Bstr(name.c_str()).raw(),
                                                   nat.asOutParam());
...
{
    public:
    virtual HRESULT HandleEvent(NemuEventType_T aEventType, IEvent *pEvent) = 0;
};
...
typedef com::SafeArray<NemuEventType_T> ComEventTypeArray;
