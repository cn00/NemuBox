...
/** @file
 * Nemu Qt GUI - UIApplianceEditorWidget class implementation.
 */
    ...
    switch (role)
    {
        ...
        case Qt::DisplayRole:
        {
            ...
            else if (column == ConfigValueSection)
            {
                switch (m_type)
                {
                    ...
                    case KVirtualSystemDescriptionType_OS:             v = nemuGlobal().vmGuestOSTypeDescription(m_strConfigValue); break;
                    case KVirtualSystemDescriptionType_Memory:         v = m_strConfigValue + " " + NemuGlobal::tr("MB", "size suffix MBytes=1024 KBytes"); break;
        ...
        case Qt::DecorationRole:
        {
                     ...
                     m_type == KVirtualSystemDescriptionType_OS)
            {
                ...
                v = nemuGlobal().vmGuestOSTypeIcon(m_strConfigValue).scaledToHeight(iIconMetric, Qt::SmoothTransformation);
            }
            break;
        }
        case Qt::FontRole:
...
QWidget *HardwareItem::createEditor(QWidget *pParent, const QStyleOptionViewItem & /* styleOption */, const QModelIndex &idx) const
{
    ...
    if (idx.column() == ConfigValueSection)
    {
        switch (m_type)
        {
            ...
            {
                NemuOSTypeSelectorButton *e = new NemuOSTypeSelectorButton(pParent);
            ...
            case KVirtualSystemDescriptionType_Memory:
            {
                ...
                e->setSuffix(" " + NemuGlobal::tr("MB", "size suffix MBytes=1024 KBytes"));
                editor = e;
                break;
            }
            ...
            {
                /* disabled for now
                   NemuFilePathSelectorWidget *e = new NemuFilePathSelectorWidget(pParent);
                   e->setMode(NemuFilePathSelectorWidget::Mode_File);
...
bool HardwareItem::setEditorData(QWidget *pEditor, const QModelIndex & /* idx */) const
{
    ...
    switch (m_type)
    {
        case KVirtualSystemDescriptionType_OS:
        {
            if (NemuOSTypeSelectorButton *e = qobject_cast<NemuOSTypeSelectorButton*>(pEditor))
            {
        ...
        {
            /* disabled for now
               if (NemuFilePathSelectorWidget *e = qobject_cast<NemuFilePathSelectorWidget*>(pEditor))
               {
...
bool VirtualSystemDelegate::eventFilter(QObject *pObject, QEvent *pEvent)
{
    if (pEvent->type() == QEvent::FocusOut)
    {
        ...
        NemuOSTypeSelectorButton *pButton = qobject_cast<NemuOSTypeSelectorButton*>(pObject);
