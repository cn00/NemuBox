...
*********************************************************************************************************************************/
#include "GuestImpl.h"
#ifndef NEMU_WITH_GUEST_CONTROL
# error "NEMU_WITH_GUEST_CONTROL must defined in this file"
#endif
...
public:
    ...
    STDMETHOD(HandleEvent)(NemuEventType_T aType, IEvent *aEvent)
    {
        switch (aType)
        {
            case NemuEventType_OnGuestSessionStateChanged:
            {
...
NEMU_LISTENER_DECLARE(GuestSessionListenerImpl)
                       ...
                       const GuestCredentials &guestCreds)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (SUCCEEDED(hr))
        {
            try
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    eventTypes.push_back(NemuEventType_OnGuestSessionStateChanged);
        ...
        rc = Global::nemuStatusCodeFromCOM(hr);
    }
...
HRESULT GuestSession::getEnvironmentChanges(std::vector<com::Utf8Str> &aEnvironmentChanges)
{
    ...
    return Global::nemuStatusCodeToCOM(vrc);
}
...
HRESULT GuestSession::getEnvironmentBase(std::vector<com::Utf8Str> &aEnvironmentBase)
{
    ...
    if (mData.mpBaseEnvironment)
    {
        ...
        hrc = Global::nemuStatusCodeToCOM(vrc);
    }
    else if (mData.mProtocolVersion < 99999)
        hrc = setError(NEMU_E_NOT_SUPPORTED, tr("The base environment feature is not supported by the guest additions"));
    else
        hrc = setError(NEMU_E_INVALID_OBJECT_STATE, tr("The base environment has not yet been reported by the guest"));
...
{
    NEMUOSTYPE enmOsType = mParent->i_getGuestOSType();
    if (    enmOsType < NEMUOSTYPE_DOS)
    {
        *aPathStyle = PathStyle_Unknown;
        LogFlowFunc(("returns PathStyle_Unknown\n"));
    }
    else if (enmOsType < NEMUOSTYPE_Linux)
    {
                                          ...
                                          uint32_t uFlags, int *pGuestRc)
{
    ...
    procInfo.mExecutable = Utf8Str(NEMUSERVICE_TOOL_MKDIR);
                                             ...
                                             bool fDirectory, Utf8Str &strName, int *pGuestRc)
{
    ...
    procInfo.mExecutable = Utf8Str(NEMUSERVICE_TOOL_MKTEMP);
    procInfo.mArguments.push_back(procInfo.mExecutable);
                                          ...
                                          ComObjPtr<GuestDirectory> &pDirectory, int *pGuestRc)
{
    ...
    if (mData.mNumObjects >= NEMU_GUESTCTRL_MAX_OBJECTS)
        return rc;
    ...
    for (;;)
    {
        ...
        uNewDirID++;
        if (uNewDirID == NEMU_GUESTCTRL_MAX_OBJECTS)
            uNewDirID = 0;
    ...
    try
    {
        ...
        mData.mNumObjects++;
        Assert(mData.mNumObjects <= NEMU_GUESTCTRL_MAX_OBJECTS);
        ...
        /** @todo Fire off a NemuEventType_OnGuestDirectoryRegistered event? */
    }
    catch (std::bad_alloc &)
    {
        rc = VERR_NO_MEMORY;
...
int GuestSession::i_dispatchToDirectory(PNEMUGUESTCTRLHOSTCBCTX pCtxCb, PNEMUGUESTCTRLHOSTCALLBACK pSvcCb)
{
    ...
    uint32_t uDirID = NEMU_GUESTCTRL_CONTEXTID_GET_OBJECT(pCtxCb->uContextID);
#ifdef DEBUG
...
int GuestSession::i_dispatchToFile(PNEMUGUESTCTRLHOSTCBCTX pCtxCb, PNEMUGUESTCTRLHOSTCALLBACK pSvcCb)
{
    LogFlowFunc(("pCtxCb=%p, pSvcCb=%p\n", pCtxCb, pSvcCb));
    ...
    uint32_t uFileID = NEMU_GUESTCTRL_CONTEXTID_GET_OBJECT(pCtxCb->uContextID);
#ifdef DEBUG
...
int GuestSession::i_dispatchToObject(PNEMUGUESTCTRLHOSTCBCTX pCtxCb, PNEMUGUESTCTRLHOSTCALLBACK pSvcCb)
{
    LogFlowFunc(("pCtxCb=%p, pSvcCb=%p\n", pCtxCb, pSvcCb));
    ...
    int rc;
    uint32_t uObjectID = NEMU_GUESTCTRL_CONTEXTID_GET_OBJECT(pCtxCb->uContextID);
...
int GuestSession::i_dispatchToProcess(PNEMUGUESTCTRLHOSTCBCTX pCtxCb, PNEMUGUESTCTRLHOSTCALLBACK pSvcCb)
{
    LogFlowFunc(("pCtxCb=%p, pSvcCb=%p\n", pCtxCb, pSvcCb));
    ...
    uint32_t uProcessID = NEMU_GUESTCTRL_CONTEXTID_GET_OBJECT(pCtxCb->uContextID);
#ifdef DEBUG
...
int GuestSession::i_dispatchToThis(PNEMUGUESTCTRLHOSTCBCTX pCbCtx, PNEMUGUESTCTRLHOSTCALLBACK pSvcCb)
{
...
int GuestSession::i_fileRemoveInternal(const Utf8Str &strPath, int *pGuestRc)
{
    ...
    procInfo.mFlags      = ProcessCreateFlag_WaitForStdOut;
    procInfo.mExecutable = Utf8Str(NEMUSERVICE_TOOL_RM);
    procInfo.mArguments.push_back(procInfo.mExecutable);
                                     ...
                                     ComObjPtr<GuestFile> &pFile, int *pGuestRc)
{
    ...
    for (;;)
    {
        ...
        uNewFileID++;
        if (uNewFileID == NEMU_GUESTCTRL_MAX_OBJECTS)
            uNewFileID = 0;
...
int GuestSession::i_fsQueryInfoInternal(const Utf8Str &strPath, bool fFollowSymlinks, GuestFsObjData &objData, int *pGuestRc)
{
    ...
    GuestProcessStartupInfo procInfo;
    procInfo.mFlags      = ProcessCreateFlag_WaitForStdOut;
    procInfo.mExecutable = Utf8Str(NEMUSERVICE_TOOL_STAT);
    procInfo.mArguments.push_back(procInfo.mExecutable);
...
/** No locking! */
int GuestSession::i_onSessionStatusChange(PNEMUGUESTCTRLHOSTCBCTX pCbCtx, PNEMUGUESTCTRLHOSTCALLBACK pSvcCbData)
{
    AssertPtrReturn(pCbCtx, VERR_INVALID_POINTER);
...
int GuestSession::i_processCreateExInternal(GuestProcessStartupInfo &procInfo, ComObjPtr<GuestProcess> &pProcess)
{
    ...
    for (;;)
    {
        ...
        uNewProcessID++;
        if (uNewProcessID == NEMU_GUESTCTRL_MAX_OBJECTS)
            uNewProcessID = 0;

        if (++uTries == NEMU_GUESTCTRL_MAX_OBJECTS)
            break; /* Don't try too hard. */
    }
...
int GuestSession::i_sendCommand(uint32_t uFunction,
                                uint32_t uParms, PNEMUHGCMSVCPARM paParms)
{
    LogFlowThisFuncEnter();

#ifndef NEMU_GUESTCTRL_TEST_CASE
...
HRESULT GuestSession::i_setErrorExternal(VirtualBoxBase *pInterface, int guestRc)
{
    ...
    return pInterface->setError(NEMU_E_IPRT_ERROR, GuestSession::i_guestErrorToString(guestRc).c_str());
}
...
int GuestSession::i_setSessionStatus(GuestSessionStatus_T sessionStatus, int sessionRc)
{
    ...
    if (mData.mStatus != sessionStatus)
    {
        ...
        ComAssertComRC(hr);
        int rc2 = errorInfo->initEx(NEMU_E_IPRT_ERROR, sessionRc,
...
int GuestSession::i_determineProtocolVersion(void)
{
     ...
     * ASSUMING that NemuService and NemuDrv are at the same version.
     */
    ...
    if (uGaVersion >= NEMU_FULL_VERSION_MAKE(4,3,0))
        mData.mProtocolVersion = 2; /* Guest control 2.0. */

    LogFlowThisFunc(("uGaVersion=%u.%u.%u => mProtocolVersion=%u\n",
                     NEMU_FULL_VERSION_GET_MAJOR(uGaVersion), NEMU_FULL_VERSION_GET_MINOR(uGaVersion),
                     NEMU_FULL_VERSION_GET_BUILD(uGaVersion), mData.mProtocolVersion));
    ...
    if (RT_FAILURE(rc))
    {
        switch (rc)
        {
            case VERR_GSTCTL_GUEST_ERROR:
                ...
                hr = setError(NEMU_E_IPRT_ERROR, tr("Directory creation failed: Could not create directory"));
                break;

            case VERR_INVALID_PARAMETER:
               hr = setError(NEMU_E_IPRT_ERROR, tr("Directory creation failed: Invalid parameters given"));
               break;

            case VERR_BROKEN_PIPE:
               hr = setError(NEMU_E_IPRT_ERROR, tr("Directory creation failed: Unexpectedly aborted"));
               break;

            default:
               hr = setError(NEMU_E_IPRT_ERROR, tr("Directory creation failed: %Rrc"), rc);
               break;
        }
    }
                                          ...
                                          BOOL aSecure, com::Utf8Str &aDirectory)
{
    ...
    if (!RT_SUCCESS(rc))
    {
        switch (rc)
        {
            ...
            default:
               hr = setError(NEMU_E_IPRT_ERROR, tr("Temporary directory creation \"%s\" with template \"%s\" failed: %Rrc"),
...
HRESULT GuestSession::directoryExists(const com::Utf8Str &aPath, BOOL aFollowSymlinks, BOOL *aExists)
{
    ...
    else
    {
        ...
        switch (rc)
        {
            ...
            default:
               hr = setError(NEMU_E_IPRT_ERROR, tr("Querying directory existence \"%s\" failed: %Rrc"),
                             aPath.c_str(), rc);
               break;
        }
    }
                                    ...
                                    const std::vector<DirectoryOpenFlag_T> &aFlags, ComPtr<IGuestDirectory> &aDirectory)
{
    ...
    else
    {
        ...
        {
            case VERR_INVALID_PARAMETER:
               hr = setError(NEMU_E_IPRT_ERROR, tr("Opening directory \"%s\" failed; invalid parameters given"),
                                                   aPath.c_str());
               break;
            ...
            default:
               hr = setError(NEMU_E_IPRT_ERROR, tr("Opening directory \"%s\" failed: %Rrc"),
                             aPath.c_str(),rc);
               break;
        }
    }
...
HRESULT GuestSession::directoryRemove(const com::Utf8Str &aPath)
{
    ...
    if (RT_FAILURE(vrc))
    {
        switch (vrc)
        {
            ...
            default:
                hr = setError(NEMU_E_IPRT_ERROR, tr("Removing guest directory \"%s\" failed: %Rrc"),
                              aPath.c_str(), vrc);
                break;
        }
    }
 ...
 *        further down!! */
    ...
    if (RT_FAILURE(vrc))
    {
        switch (vrc)
        {
            ...
            default:
                hr = setError(NEMU_E_IPRT_ERROR, tr("Recursively removing guest directory \"%s\" failed: %Rrc"),
                              aPath.c_str(), vrc);
                break;
        }
    }
 ...
 *        Where does that get converted to *aExists = FALSE? */
        ...
        default:
            hr = setError(NEMU_E_IPRT_ERROR, tr("Querying file information for \"%s\" failed: %Rrc"),
                          aPath.c_str(), vrc);
            break;
    }
                                 ...
                                 const std::vector<FileOpenExFlags_T> &aFlags, ComPtr<IGuestFile> &aFile)
{
    ...
    else
    {
        switch (vrc)
        {
            ...
            default:
                hr = setError(NEMU_E_IPRT_ERROR, tr("Opening guest file \"%s\" failed: %Rrc"),
                              aPath.c_str(), vrc);
                break;
        }
    }
...
HRESULT GuestSession::fileQuerySize(const com::Utf8Str &aPath, BOOL aFollowSymlinks, LONG64 *aSize)
{
    ...
    else
    {
        switch (vrc)
        {
            ...
            default:
               hr = setError(NEMU_E_IPRT_ERROR, tr("Querying file size failed: %Rrc"), vrc);
               break;
        }
    }
...
HRESULT GuestSession::fsObjRemove(const com::Utf8Str &aPath)
{
    ...
    if (RT_FAILURE(vrc))
    {
        switch (vrc)
        {
            ...
            default:
                hr = setError(NEMU_E_IPRT_ERROR, tr("Removing file \"%s\" failed: %Rrc"),
                              aPath.c_str(), vrc);
                break;
        }
    }
                                  ...
                                  const std::vector<FsObjRenameFlag_T> &aFlags)
{
    ...
    if (RT_FAILURE(vrc))
    {
        switch (vrc)
        {
            ...
            default:
                hr = setError(NEMU_E_IPRT_ERROR, tr("Renaming guest directory \"%s\" failed: %Rrc"),
                              aSource.c_str(), vrc);
                break;
        }
    }
                                      ...
                                      ComPtr<IGuestProcess> &aGuestProcess)
{
     ...
     * without any validation.  Flags not existing in this nemu version are
