...
/** @file
 * Nemu Qt GUI - UINetworkReply stuff implementation.
 */
...
# ifndef NEMU_GUI_IN_TST_SSL_CERT_DOWNLOADS
#  include "NemuGlobal.h"
#  include "NemuUtils.h"
# else /* NEMU_GUI_IN_TST_SSL_CERT_DOWNLOADS */
#  include <Nemu/log.h>
# endif /* NEMU_GUI_IN_TST_SSL_CERT_DOWNLOADS */
...
#ifdef NEMU_GUI_IN_TST_SSL_CERT_DOWNLOADS
public:
...
/* static */
const QString UINetworkReplyPrivateThread::s_strCertificateFileName = QString("nemu-ssl-cacertificate.crt");
...
int UINetworkReplyPrivateThread::applyProxyRules()
{
    ...
    UIProxyManager proxyManager(nemuGlobal().settings().proxySettings());
...
{
#ifndef NEMU_GUI_IN_TST_SSL_CERT_DOWNLOADS
    const QDir homeDir(QDir::toNativeSeparators(nemuGlobal().homeFolder()));
