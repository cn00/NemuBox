/* $Id: DevBusLogic.cpp $ */
/** @file
 * Nemu storage devices - BusLogic SCSI host adapter BT-958.
 *
...
static int buslogicR3HwReset(PBUSLOGIC pBusLogic, bool fResetIO)
{
    ...
    nemuscsiInitialize(&pBusLogic->NemuSCSI);
...
static int buslogicR3PrepareBIOSSCSIRequest(PBUSLOGIC pBusLogic)
{
    ...
    rc = nemuscsiSetupRequest(&pBusLogic->NemuSCSI, &pTaskState->PDMScsiRequest, &uTargetDevice);
    ...
    if (!pTaskState->CTX_SUFF(pTargetDevice)->fPresent)
    {
        ...
        memcpy(pBusLogic->NemuSCSI.pbBuf, &ScsiInquiryData, 5);

        rc = nemuscsiRequestFinished(&pBusLogic->NemuSCSI, &pTaskState->PDMScsiRequest, SCSI_STATUS_OK);
...
static DECLCALLBACK(int) buslogicR3BiosIoPortRead(PPDMDEVINS pDevIns, void *pvUser, RTIOPORT Port, uint32_t *pu32, unsigned cb)
{
    ...
    rc = nemuscsiReadRegister(&pBusLogic->NemuSCSI, (Port - BUSLOGIC_BIOS_IO_PORT), pu32);
...
static DECLCALLBACK(int) buslogicR3BiosIoPortWrite(PPDMDEVINS pDevIns, void *pvUser, RTIOPORT Port, uint32_t u32, unsigned cb)
{
    ...
    rc = nemuscsiWriteRegister(&pBusLogic->NemuSCSI, (Port - BUSLOGIC_BIOS_IO_PORT), (uint8_t)u32);
                                                      ...
                                                      uint8_t const *pbSrc, uint32_t *pcTransfers, unsigned cb)
{
    ...
    int rc = nemuscsiWriteString(pDevIns, &pBusLogic->NemuSCSI, (Port - BUSLOGIC_BIOS_IO_PORT), pbSrc, pcTransfers, cb);
    if (rc == VERR_MORE_DATA)
    {
        rc = buslogicR3PrepareBIOSSCSIRequest(pBusLogic);
        AssertRC(rc);
                                                     ...
                                                     uint8_t *pbDst, uint32_t *pcTransfers, unsigned cb)
{
    ...
    return nemuscsiReadString(pDevIns, &pBusLogic->NemuSCSI, (Port - BUSLOGIC_BIOS_IO_PORT),
                                                              ...
                                                              int rcCompletion, bool fRedo, int rcReq)
{
    ...
    else
    {
        if (pTaskState->fBIOS)
        {
            rc = nemuscsiRequestFinished(&pBusLogic->NemuSCSI, pSCSIRequest, rcCompletion);
...
static DECLCALLBACK(int) buslogicR3SaveExec(PPDMDEVINS pDevIns, PSSMHANDLE pSSM)
{
    ...
    nemuscsiR3SaveExec(&pBusLogic->NemuSCSI, pSSM);
...
static DECLCALLBACK(int) buslogicR3LoadExec(PPDMDEVINS pDevIns, PSSMHANDLE pSSM, uint32_t uVersion, uint32_t uPass)
{
    ...
    rc = nemuscsiR3LoadExec(&pBusLogic->NemuSCSI, pSSM);
    if (RT_FAILURE(rc))
    {
