 ...
 * @note    prints its own diagnostic information to stdout.
 */
static void testSetPropsHost(NEMUHGCMSVCFNTABLE *ptable)
{
    RTTestISub("SET_PROPS_HOST");
    RTTESTI_CHECK_RETV(RT_VALID_PTR(ptable->pfnHostCall));

    NEMUHGCMSVCPARM aParms[4];
 ...
 * @note    prints its own diagnostic information to stdout.
 */
static void testEnumPropsHost(NEMUHGCMSVCFNTABLE *ptable)
{
 ...
 */
int doSetProperty(NEMUHGCMSVCFNTABLE *pTable, const char *pcszName,
 ...
 * @note    prints its own diagnostic information to stdout.
 */
static void testSetProp(NEMUHGCMSVCFNTABLE *pTable)
{
 ...
 *                    than the guest one
 */
static int doDelProp(NEMUHGCMSVCFNTABLE *pTable, const char *pcszName, bool isHost)
{
    NEMUHGCMCALLHANDLE_TYPEDEF callHandle = { VINF_SUCCESS };
 ...
 * @note    prints its own diagnostic information to stdout.
 */
static void testDelProp(NEMUHGCMSVCFNTABLE *pTable)
{
 ...
 * @note    prints its own diagnostic information to stdout.
 */
static void testGetProp(NEMUHGCMSVCFNTABLE *pTable)
{
 ...
 * @note    prints its own diagnostic information to stdout.
 */
static void testGetNotification(NEMUHGCMSVCFNTABLE *pTable)
{
...
struct asyncNotification_
{
    ...
    /** Return value */
    NEMUHGCMCALLHANDLE_TYPEDEF callHandle;
} g_AsyncNotification;

/**
 * Set up the test for the asynchronous GET_NOTIFICATION function.
 */
static void setupAsyncNotification(NEMUHGCMSVCFNTABLE *pTable)
{
 ...
 */
static void testAsyncNotification(NEMUHGCMSVCFNTABLE *pTable)
{
    uint64_t u64Timestamp;
    uint32_t u32Size;
...
static void test2(void)
{
    ...
    RTTESTI_CHECK_RC_OK_RETV(NemuHGCMSvcLoad(&svcTable));
 ...
 */
static int doSetGlobalFlags(NEMUHGCMSVCFNTABLE *pTable, ePropFlags eFlags)
{
 ...
 * @note    prints its own diagnostic information to stdout.
 */
static void testSetPropROGuest(NEMUHGCMSVCFNTABLE *pTable)
{
    ...
    RTTESTI_CHECK_RC_OK_RETV(NemuHGCMSvcLoad(pTable));
 ...
 * @note    prints its own diagnostic information to stdout.
 */
static void testDelPropROGuest(NEMUHGCMSVCFNTABLE *pTable)
{
