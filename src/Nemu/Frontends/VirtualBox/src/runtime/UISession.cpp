/* $Id: UISession.cpp $ */
/** @file
 * Nemu Qt GUI - UISession class implementation.
 */
...
# ifdef NEMU_GUI_WITH_KEYS_RESET_HANDLER
#  include "UIKeyboardHandler.h"
#  include <signal.h>
# endif /* NEMU_GUI_WITH_KEYS_RESET_HANDLER */
...
# include <X11/Xlib.h>
# include <X11/Xutil.h>
# ifndef NEMU_WITHOUT_XCURSOR
#  include <X11/Xcursor/Xcursor.h>
# endif /* NEMU_WITHOUT_XCURSOR */
#endif /* Q_WS_X11 */
...
bool UISession::initialize()
{
    ...
    if (nemuGlobal().settings().autoCapture())
    ...
    {
        if (nemuGlobal().isPatmDisabled())
            debugger().SetPATMEnabled(false);
        if (nemuGlobal().isCsamDisabled())
            debugger().SetCSAMEnabled(false);
        if (nemuGlobal().isSupervisorCodeExecedRecompiled())
            debugger().SetRecompileSupervisor(true);
        if (nemuGlobal().isUserCodeExecedRecompiled())
            debugger().SetRecompileUser(true);
        if (nemuGlobal().areWeToExecuteAllInIem())
            debugger().SetExecuteAllInIEM(true);
        if (!nemuGlobal().isDefaultWarpPct())
            debugger().SetVirtualTimeRate(nemuGlobal().getWarpPct());
    }

    /* Apply ad-hoc reconfigurations from the command line: */
    if (nemuGlobal().hasFloppyImageToMount())
        mountAdHocImage(KDeviceType_Floppy, UIMediumType_Floppy, nemuGlobal().getFloppyImage());
    if (nemuGlobal().hasDvdImageToMount())
        mountAdHocImage(KDeviceType_DVD, UIMediumType_DVD, nemuGlobal().getDvdImage());

    /* Power UP if this is NOT separate process: */
    if (!nemuGlobal().isSeparateProcess())
        if (!powerUp())
            return false;
    ...
    LogRel(("GUI: 2D video acceleration is %s\n",
           machine().GetAccelerate2DVideoEnabled() && NemuGlobal::isAcceleration2DVideoAvailable()
           ? "enabled" : "disabled"));
#endif /* NEMU_WITH_VIDEOHWACCEL */
...
{
    /* Power UP machine: */
    CProgress progress = nemuGlobal().shouldStartPaused() ? console().PowerUpPaused() : console().PowerUp();

    /* Check for immediate failure: */
    if (!console().isOk() || progress.isNull())
    {
        if (nemuGlobal().showStartVMErrors())
...
bool UISession::powerOff(bool fIncludingDiscard, bool &fServerCrashed)
{
    ...
    else
    {
        ...
        if (!console().isNull())
        {
           ...
           /* This can happen if NemuSVC is not running: */
...
bool UISession::restoreCurrentSnapshot()
{
    ...
    do
    {
        ...
        CVirtualBox nemu = nemuGlobal().virtualBox();
        const QString strMachineID = nemuGlobal().managedVMUuid();
        const CMachine mach = nemu.FindMachine(strMachineID);
        if (!nemu.isOk() || mach.isNull())
        {
            /* Unable to find VM: */
            msgCenter().cannotFindMachineById(nemu, strMachineID);
            break;
        }

        /* Open a direct session to modify that VM: */
        CSession sess = nemuGlobal().openSession(nemuGlobal().managedVMUuid(),
                                                 nemuGlobal().isSeparateProcess()
...
void UISession::sltInstallGuestAdditionsFrom(const QString &strSource)
{
...
#ifndef NEMU_WITH_ADDITIONS_AUTOUPDATE_UI
    fDoMount = true;
#else /* NEMU_WITH_ADDITIONS_AUTOUPDATE_UI */
    ...
    if (fResult)
    {
        ...
        {
            /* If we got back a NEMU_E_NOT_SUPPORTED we don't complain (guest OS
             * simply isn't supported yet), so silently fall back to "old" .ISO
             * mounting method. */
            if (   !SUCCEEDED_WARNING(rc)
                && rc != NEMU_E_NOT_SUPPORTED)
            {
...
#endif /* NEMU_WITH_ADDITIONS_AUTOUPDATE_UI */
    ...
    QString strMediumID;
    CVirtualBox nemu = nemuGlobal().virtualBox();
    CMedium image = nemu.OpenMedium(strSource, KDeviceType_DVD, KAccessMode_ReadWrite, false /* fForceNewUuid */);
    if (nemu.isOk() && !image.isNull())
        strMediumID = image.GetId();
    else
    {
        msgCenter().cannotOpenMedium(nemu, UIMediumType_DVD, strSource, mainMachineWindow());
        return;
    }
    ...
    if (medium.isNull())
    {
        ...
        nemuGlobal().createMedium(medium);
    }
...
void UISession::sltHandleMenuBarConfigurationChange(const QString &strMachineID)
{
    ...
    if (nemuGlobal().managedVMUuid() != strMachineID)
        return;
...
void UISession::sltCheckIfHostDisplayChanged()
{
    ...
    if (nemuGlobal().screenCount() != m_hostScreens.size())
    {
        /* Reset watchdog: */
    ...
    else
    {
        ...
        for (int iScreenIndex = 0; iScreenIndex < nemuGlobal().screenCount(); ++iScreenIndex)
        {
            if (nemuGlobal().screenGeometry(iScreenIndex) != m_hostScreens.at(iScreenIndex))
            {
                /* Reset watchdog: */
                m_pWatchdogDisplayChange->setProperty("tryNumber", 0);
...
{
    /* Open session: */
    m_session = nemuGlobal().openSession(nemuGlobal().managedVMUuid(),
                                         nemuGlobal().isSeparateProcess()
...
void UISession::prepareScreens()
{
    ...
    else if (nemuGlobal().isSeparateProcess())
    {
...
void UISession::loadSessionSettings()
{
    /* Load extra-data settings: */
    {
        ...
        /* Menu-bar options: */
        {
            const bool fEnabledGlobally = !nemuGlobal().settings().isFeatureActive("noMenuBar");
        ...
        /* Status-bar options: */
        {
            const bool fEnabledGlobally = !nemuGlobal().settings().isFeatureActive("noStatusBar");
        ...
        m_fAllCloseActionsRestricted =  (!nemuGlobal().isSeparateProcess() || (m_restrictedCloseActions & MachineCloseAction_Detach))
...
void UISession::saveSessionSettings()
{
    /* Save extra-data settings: */
    {
        ...
        gEDataManager->setMachineFirstTimeStarted(false, nemuGlobal().managedVMUuid());
        ...
        if (actionPool())
        {
            ...
            gEDataManager->setGuestScreenAutoResizeEnabled(pGuestAutoresizeSwitch->isChecked(), nemuGlobal().managedVMUuid());
        }
...
void UISession::cleanupSession()
{
    ...
    /* Close session: */
    if (!m_session.isNull() && nemuGlobal().isNemuSVCAvailable())
    {
...
#elif defined (Q_WS_X11) && !defined (NEMU_WITHOUT_XCURSOR)
...
bool UISession::preprocessInitialization()
{
    ...
    foreach (const CHostNetworkInterface &iface, nemuGlobal().host().GetNetworkInterfaces())
    {
        availableInterfaceNames << iface.GetName();
        availableInterfaceNames << iface.GetShortName();
    }

    ulong cCount = nemuGlobal().virtualBox().GetSystemProperties().GetMaxNetworkAdapters(machine().GetChipsetType());
...
bool UISession::mountAdHocImage(KDeviceType enmDeviceType, UIMediumType enmMediumType, const QString &strImage)
{
    ...
    if (strImage != "none")
    {
        /* Open the image: */
        CVirtualBox nemu = nemuGlobal().virtualBox();
        CMedium nemuImage = nemu.OpenMedium(strImage, enmDeviceType, KAccessMode_ReadWrite, false /* fForceNewUuid */);
        if (!nemu.isOk() || nemuImage.isNull())
        {
            msgCenter().cannotOpenMedium(nemu, enmMediumType, strImage);
            return false;
        }

        /* Work the cache and use the cached image if possible: */
        uiImage = nemuGlobal().medium(nemuImage.GetId());
        if (uiImage.isNull())
        {
            uiImage = UIMedium(nemuImage, enmMediumType, KMediumState_Created);
            nemuGlobal().createMedium(uiImage);
        }
    }
    if (nemu.isOk())
    {
...
bool UISession::postprocessInitialization()
{
    ...
    const bool fIs64BitsGuest = nemuGlobal().virtualBox().GetGuestOSType(guest().GetOSTypeId()).GetIs64Bit();
    const bool fRecommendVirtEx = nemuGlobal().virtualBox().GetGuestOSType(guest().GetOSTypeId()).GetRecommendedVirtEx();
    ...
    if (fRecommendVirtEx && !fIsVirtActive)
    {
        ...
        bool fVTxAMDVSupported = nemuGlobal().host().GetProcessorFeature(KProcessorFeature_HWVirtEx);
...
void UISession::setScreenVisible(ulong uScreenId, bool fIsMonitorVisible)
{
    ...
    gEDataManager->setLastGuestScreenVisibilityStatus(uScreenId, fIsMonitorVisible, nemuGlobal().managedVMUuid());
}
...
void UISession::updateHostScreenData()
{
    m_hostScreens.clear();
    for (int iScreenIndex = 0; iScreenIndex < nemuGlobal().screenCount(); ++iScreenIndex)
        m_hostScreens << nemuGlobal().screenGeometry(iScreenIndex);
}

void UISession::updateActionRestrictions()
{
    /* Get host and prepare restrictions: */
    const CHost host = nemuGlobal().host();
    ...
    /* Network stuff: */
    {
        ...
        ULONG uSlots = nemuGlobal().virtualBox().GetSystemProperties().GetMaxNetworkAdapters(chipsetType);
