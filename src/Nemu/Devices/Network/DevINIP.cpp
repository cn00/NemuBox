 ...
 *
 * @returns Nemu status code. VINF_SUCCESS means there is at least one receive descriptor available.
...
static DECLCALLBACK(int) devINIPDestruct(PPDMDEVINS pDevIns)
{
    ...
    if (g_pDevINIPData != NULL)
        nemuLwipCoreFinalize(devINIPTcpipFiniDone, pThis);
...
static DECLCALLBACK(int) devINIPConstruct(PPDMDEVINS pDevIns, int iInstance, PCFGMNODE pCfg)
{
     ...
     * Initialize lwIP.
     */
    nemuLwipCoreInitialize(devINIPTcpipInitDone, pThis);
