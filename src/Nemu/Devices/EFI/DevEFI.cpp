...
# define DEVEFI_WITH_NEMUDBG_SCRIPT
#endif
...
typedef struct DEVEFI
{
    ...
    /** EFI message buffer. */
    char                    szMsg[NEMU_EFI_DEBUG_BUFFER];
...
/** The saved state version from Nemu 4.2. */
#define EFI_SSM_VERSION_4_2 1

/** Non-volatile EFI variable. */
#define NEMU_EFI_VARIABLE_NON_VOLATILE  UINT32_C(0x00000001)
/** Non-volatile EFI variable. */
#define NEMU_EFI_VARIABLE_READ_ONLY     UINT32_C(0x00000008)
...
static int nvramStore(PDEVEFI pThis)
{
    ...
    RTListForEach(&pThis->NVRAM.VarList, pEfiVar, EFIVAR, ListNode)
        if (pEfiVar->fAttributes & NEMU_EFI_VARIABLE_NON_VOLATILE)
            cNonVolatile++;
...
static uint8_t efiInfoNextByte(PDEVEFI pThis)
{
    switch (pThis->iInfoSelector)
    {
        ...
        case EFI_INFO_INDEX_TEMPMEM_BASE:       return efiInfoNextByteU32(pThis, NEMU_EFI_TOP_OF_STACK); /* just after stack */
        ...
        case EFI_INFO_INDEX_STACK_BASE:         return efiInfoNextByteU32(pThis,  NEMU_EFI_TOP_OF_STACK - _128K); /* 2M - 128 K */
...
#ifdef IN_RING3
static void efiNemuDbgScript(PDEVEFI pThis, const char *pszFormat, ...)
{
# ifdef DEVEFI_WITH_NEMUDBG_SCRIPT
    PRTSTREAM pStrm;
    int rc2 = RTStrmOpen("./DevEFI.NemuDbg", "a", &pStrm);
    if (RT_SUCCESS(rc2))
    {
        va_list va;
 ...
 *
 * @returns Nemu status suitable for I/O port write handler.
 *
 * @param   pThis               The EFI state.
        ...
        case EFI_IMAGE_EVT_CMD_COMPLETE:
        {
            ...
            if (s_cImageEvtLogged < 2048)
            {
                ...
                switch (pThis->ImageEvt.uEvt)
                {
                    ...
                    case EFI_IMAGE_EVT_CMD_START_LOAD32:
                        LogRel(("EFI: NemuDbg> loadimage32 '%.*s.efi' %#llx LB %#llx\n",
                                pThis->ImageEvt.offName - 4, pThis->ImageEvt.szName, pThis->ImageEvt.uAddr0, pThis->ImageEvt.cb0));
                        if (pThis->ImageEvt.offName > 4)
                            efiNemuDbgScript(pThis, "loadimage32 '%.*s.efi' %#llx\n",
                                             pThis->ImageEvt.offName - 4, pThis->ImageEvt.szName, pThis->ImageEvt.uAddr0);
                        break;
                    case EFI_IMAGE_EVT_CMD_START_LOAD64:
                        LogRel(("EFI: NemuDbg> loadimage64 '%.*s.efi' %#llx LB %#llx\n",
                                pThis->ImageEvt.offName - 4, pThis->ImageEvt.szName, pThis->ImageEvt.uAddr0, pThis->ImageEvt.cb0));
                        if (pThis->ImageEvt.offName > 4)
                            efiNemuDbgScript(pThis, "loadimage64 '%.*s.efi' %#llx\n",
                                             pThis->ImageEvt.offName - 4, pThis->ImageEvt.szName, pThis->ImageEvt.uAddr0);
                        break;
                    case EFI_IMAGE_EVT_CMD_START_UNLOAD32:
                    case EFI_IMAGE_EVT_CMD_START_UNLOAD64:
                    {
                        LogRel(("EFI: NemuDbg> unload '%.*s.efi' # %#llx LB %#llx\n",
                        ...
                        if (pThis->ImageEvt.offName > 4)
                            efiNemuDbgScript(pThis, "unload '%.*s.efi'\n",
                                             pThis->ImageEvt.offName - 4 - pThis->ImageEvt.offNameLastComponent,
                                             &pThis->ImageEvt.szName[pThis->ImageEvt.offNameLastComponent]);
                        break;
                    }
...
static DECLCALLBACK(int)  efiConstruct(PPDMDEVINS pDevIns, int iInstance, PCFGMNODE pCfg)
{
    ...
    if (rc == VERR_CFGM_VALUE_NOT_FOUND)
    {
        ...
        AssertRCReturn(rc, rc);
        rc = RTPathAppend(pThis->pszEfiRomFile, RTPATH_MAX, "NemuEFI32.fd");
        AssertRCReturn(rc, rc);
    }
    ...
    rc = FwCommonPlantDMITable(pDevIns, pThis->au8DMIPage, NEMU_DMI_TABLE_SIZE, &pThis->aUuid,
                               pDevIns->pCfg, pThis->cCpus, &pThis->cbDmiTables, &pThis->cNumDmiTables);
    AssertRCReturn(rc, rc);
    if (pThis->u8IOAPIC)
        FwCommonPlantMpsTable(pDevIns,
                              pThis->au8DMIPage + NEMU_DMI_TABLE_SIZE,
                              _4K - NEMU_DMI_TABLE_SIZE, pThis->cCpus);
    rc = PDMDevHlpROMRegister(pDevIns, NEMU_DMI_TABLE_BASE, _4K, pThis->au8DMIPage, _4K,
