...
static DECLCALLBACK(void) callComplete(NEMUHGCMCALLHANDLE callHandle, int32_t rc)
{
    callHandle->rc = rc;
}
 ...
 */
void initTable(NEMUHGCMSVCFNTABLE *pTable, NEMUHGCMSVCHELPERS *pHelpers)
{
    pTable->cbSize              = sizeof (NEMUHGCMSVCFNTABLE);
...
static SHFLROOT initWithWritableMapping(RTTEST hTest,
                                        NEMUHGCMSVCFNTABLE *psvcTable,
                                        NEMUHGCMSVCHELPERS *psvcHelpers,
                                        const char *pcszFolderName,
                                        const char *pcszMapping)
{
    NEMUHGCMSVCPARM aParms[RT_MAX(SHFL_CPARMS_ADD_MAPPING,
                                  SHFL_CPARMS_MAP_FOLDER)];
    struct TESTSHFLSTRING FolderName;
    struct TESTSHFLSTRING Mapping;
    NEMUHGCMCALLHANDLE_TYPEDEF callHandle = { VINF_SUCCESS };
    int rc;

    initTable(psvcTable, psvcHelpers);
    AssertReleaseRC(NemuHGCMSvcLoad(psvcTable));
...
static void unmapAndRemoveMapping(RTTEST hTest, NEMUHGCMSVCFNTABLE *psvcTable,
                                  SHFLROOT root, const char *pcszFolderName)
{
    NEMUHGCMSVCPARM aParms[RT_MAX(SHFL_CPARMS_UNMAP_FOLDER,
...
static int createFile(NEMUHGCMSVCFNTABLE *psvcTable, SHFLROOT Root,
                      const char *pcszFilename, uint32_t fCreateFlags,
                      SHFLHANDLE *pHandle, SHFLCREATERESULT *pResult)
{
    NEMUHGCMSVCPARM aParms[SHFL_CPARMS_CREATE];
...
static int readFile(NEMUHGCMSVCFNTABLE *psvcTable, SHFLROOT Root,
                    SHFLHANDLE hFile, uint64_t offSeek, uint32_t cbRead,
                    uint32_t *pcbRead, void *pvBuf, uint32_t cbBuf)
{
    NEMUHGCMSVCPARM aParms[SHFL_CPARMS_READ];
    NEMUHGCMCALLHANDLE_TYPEDEF callHandle = { VINF_SUCCESS };
...
static int writeFile(NEMUHGCMSVCFNTABLE *psvcTable, SHFLROOT Root,
                     SHFLHANDLE hFile, uint64_t offSeek, uint32_t cbWrite,
                     uint32_t *pcbWritten, const void *pvBuf, uint32_t cbBuf)
{
    NEMUHGCMSVCPARM aParms[SHFL_CPARMS_WRITE];
    NEMUHGCMCALLHANDLE_TYPEDEF callHandle = { VINF_SUCCESS };
...
static int flushFile(NEMUHGCMSVCFNTABLE *psvcTable, SHFLROOT root,
                     SHFLHANDLE handle)
{
    NEMUHGCMSVCPARM aParms[SHFL_CPARMS_FLUSH];
    NEMUHGCMCALLHANDLE_TYPEDEF callHandle = { VINF_SUCCESS };
...
static int listDir(NEMUHGCMSVCFNTABLE *psvcTable, SHFLROOT root,
...
{
    NEMUHGCMSVCPARM aParms[SHFL_CPARMS_LIST];
    struct TESTSHFLSTRING Path;
    NEMUHGCMCALLHANDLE_TYPEDEF callHandle = { VINF_SUCCESS };
...
static int sfInformation(NEMUHGCMSVCFNTABLE *psvcTable, SHFLROOT root,
                         SHFLHANDLE handle, uint32_t fFlags, uint32_t cb,
                         SHFLFSOBJINFO *pInfo)
{
    NEMUHGCMSVCPARM aParms[SHFL_CPARMS_INFORMATION];
    NEMUHGCMCALLHANDLE_TYPEDEF callHandle = { VINF_SUCCESS };
...
static int lockFile(NEMUHGCMSVCFNTABLE *psvcTable, SHFLROOT root,
                    SHFLHANDLE handle, int64_t offLock, uint64_t cbLock,
                    uint32_t fFlags)
{
    NEMUHGCMSVCPARM aParms[SHFL_CPARMS_LOCK];
    NEMUHGCMCALLHANDLE_TYPEDEF callHandle = { VINF_SUCCESS };
...
{
    NEMUHGCMSVCFNTABLE  svcTable;
    NEMUHGCMSVCHELPERS  svcHelpers;
    SHFLROOT Root;
