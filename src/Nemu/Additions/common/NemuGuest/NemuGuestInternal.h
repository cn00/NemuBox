/* $Id: NemuGuestInternal.h $ */
/** @file
 * NemuGuest - Guest Additions Driver, Internal Header.
 */
...
#ifndef ___NemuGuestInternal_h
#define ___NemuGuestInternal_h
...
/** @def NEMUGUEST_USE_DEFERRED_WAKE_UP
...
# define NEMUGUEST_USE_DEFERRED_WAKE_UP
#endif


/** Pointer to the NemuGuest per session data. */
typedef struct NEMUGUESTSESSION *PNEMUGUESTSESSION;

/** Pointer to a wait-for-event entry. */
typedef struct NEMUGUESTWAIT *PNEMUGUESTWAIT;

/**
 * Nemu guest wait for event entry.
 *
 * Each waiting thread allocates one of these items and adds
 * it to the wait list before going to sleep on the event sem.
 */
typedef struct NEMUGUESTWAIT
{
    /** The list node. */
    RTLISTNODE                  ListNode;
...
/**
 * Nemu guest memory balloon.
 */
typedef struct NEMUGUESTMEMBALLOON
{
...
typedef NEMUGUESTMEMBALLOON *PNEMUGUESTMEMBALLOON;
 ...
 */
typedef struct NEMUGUESTBITUSAGETRACER
{
    /** Per bit usage counters. */
    uint32_t        acPerBitUsage[32];
    /** The current mask according to acPerBitUsage. */
    uint32_t        fMask;
} NEMUGUESTBITUSAGETRACER;
/** Pointer to a per bit usage tracker.  */
typedef NEMUGUESTBITUSAGETRACER *PNEMUGUESTBITUSAGETRACER;
/** Pointer to a const per bit usage tracker.  */
typedef NEMUGUESTBITUSAGETRACER const *PCNEMUGUESTBITUSAGETRACER;


/**
 * Nemu guest device (data) extension.
 */
typedef struct NEMUGUESTDEVEXT
{
    ...
    /** List of free wait-for-event entries (NEMUGUESTWAIT). */
    ...
    /** Spinlock various items in the NEMUGUESTSESSION. */
    RTSPINLOCK                  SessionSpinlock;
    /** List of guest sessions (NEMUGUESTSESSION).  We currently traverse this
    ...
    NEMUGUESTMEMBALLOON         MemBalloon;
    /** Callback and user data for a kernel mouse handler. */
    NemuGuestMouseSetNotifyCallback MouseNotifyCallback;
    ...
    NEMUGUESTBITUSAGETRACER     EventFilterTracker;
    ...
    NEMUGUESTBITUSAGETRACER     MouseStatusTracker;
     ...
     *
     * Protected by NEMUGUESTDEVEXT::SessionSpinlock, but is unfortunately read
    ...
    NEMUGUESTBITUSAGETRACER     SetGuestCapsTracker;
...
/** Pointer to the NemuGuest driver data. */
typedef NEMUGUESTDEVEXT *PNEMUGUESTDEVEXT;


/**
 * The NemuGuest per session data.
 */
typedef struct NEMUGUESTSESSION
{
    /** The list node. */
    RTLISTNODE                  ListNode;
     ...
     * filter.  Protected by NEMUGUESTDEVEXT::SessionSpinlock. */
     ...
     * These accumulated for sessions via NEMUGUESTDEVEXT::acGuestCapsSet and
     * reported to the host.  Protected by NEMUGUESTDEVEXT::SessionSpinlock.  */
     ...
     * the checks.
     * Use under the NEMUGUESTDEVEXT#SessionSpinlock lock. */
    uint32_t                    fMouseStatus;
#ifdef RT_OS_DARWIN
    /** Pointer to the associated org_virtualbox_NemuGuestClient object. */
    void                       *pvNemuGuestClient;
...
int  VGDrvCommonInitDevExt(PNEMUGUESTDEVEXT pDevExt, uint16_t IOPortBase, void *pvMMIOBase, uint32_t cbMMIO,
                           NEMUOSTYPE enmOSType, uint32_t fEvents);
