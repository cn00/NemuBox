 ...
 */
int initTable(NEMUHGCMSVCFNTABLE *pTable, NEMUHGCMSVCHELPERS *pHelpers)
{
 ...
 */
static int testHostCmd(const NEMUHGCMSVCFNTABLE *pTable, const PCMDHOST pCmd, uint32_t uNumTests)
{
    int rc = VINF_SUCCESS;
...
static int testHost(const NEMUHGCMSVCFNTABLE *pTable)
{
    RTTestSub(g_hTest, "Testing host commands ...");

    static NEMUHGCMSVCPARM s_aParms[1];
...
static int testClient(const NEMUHGCMSVCFNTABLE *pTable)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        static NEMUHGCMSVCPARM s_aParmsGuest[8];
        ...
        static NEMUHGCMSVCPARM s_aParmsHost[8];
...
int main(int argc, char **argv)
{
    ...
    do
    {
        RTTESTI_CHECK_RC_BREAK(NemuHGCMSvcLoad(&svcTable), VINF_SUCCESS);
