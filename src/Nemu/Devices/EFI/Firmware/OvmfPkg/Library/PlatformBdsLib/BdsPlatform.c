...
NemuConsoleInit()
{
    ...
    gBS->CreateEventEx(EVT_NOTIFY_SIGNAL, TPL_NOTIFY, NemuConsoleSwitchMode, NULL, &gEfiEventReadyToBootGuid, &event);
    return EFI_SUCCESS;
}
#endif /* NEMU */
...
--*/
{
  ...
  BdsLibConnectAll ();
#ifdef NEMU
    {
        ...
        DEBUG ((EFI_D_INFO, "------------------ Nemu Platform Specific Initialization Start -----------------------\n"));
        ...
        if (!BootOption0080)
        {
            rc = gBS->LocateHandleBuffer (ByProtocol,
                                          ...
                                          &cFileSystem,
                                          &phFileSystem);
            NemuLogFlowFuncMarkRC(rc);
            NemuLogFlowFuncMarkVar(cFileSystem, "%d");
            if (   rc == EFI_SUCCESS
                && cFileSystem > 0)
            {
                ...
                for (iFileSystem = 0; iFileSystem < cFileSystem; ++iFileSystem)
                {
                    ...
                    rc = gBS->HandleProtocol (phFileSystem[iFileSystem],
                                              &gEfiSimpleFileSystemProtocolGuid,
                                              (VOID *) &pFSVolume);
                    NemuLogFlowFuncMarkVar(iFileSystem, "%d");
                    NemuLogFlowFuncMarkRC(rc);
                    if (EFI_ERROR(rc))
                        continue;
                    ...
                    NemuLogFlowFuncMarkVar(pDevicePath,"%p");
                    if (!pDevicePath)
                        continue;
        ...
        {
            NemuLogFlowFuncMarkVar(BootOption0080->LoadOptionsSize, "%d");
            if (BootOption0080->LoadOptionsSize)
                NemuLogFlowFuncMarkVar(BootOption0080->LoadOptions, "%s");
#if 0
        ...
        DEBUG ((EFI_D_INFO, "------------------ Nemu Platform Specific Initialization End -----------------------\n"));
    }
#endif
  ...
  )
{
  ...
  NemuLogFlowFuncLeaveRC(retStatus);
  return retStatus;
}
#endif /* !NEMU */
