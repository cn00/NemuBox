/* $Id: NemuGuestR3LibClipboard.cpp $ */
/** @file
 * NemuGuestR3Lib - Ring-3 Support Library for VirtualBox guest additions, Clipboard.
 */
...
VBGLR3DECL(int) VbglR3ClipboardConnect(HGCMCLIENTID *pidClient )
{
    ...
    RT_ZERO(Info.Loc.u);
    strcpy(Info.Loc.u.host.achName, "NemuSharedClipboard");
...
{
    NemuClipboardGetHostMsg Msg;

    Msg.hdr.result = VERR_WRONG_ORDER;
    Msg.hdr.u32ClientID = idClient;
    Msg.hdr.u32Function = NEMU_SHARED_CLIPBOARD_FN_GET_HOST_MSG;
    Msg.hdr.cParms = 2;
...
{
    NemuClipboardReadData Msg;

    Msg.hdr.result = VERR_WRONG_ORDER;
    Msg.hdr.u32ClientID = idClient;
    Msg.hdr.u32Function = NEMU_SHARED_CLIPBOARD_FN_READ_DATA;
    Msg.hdr.cParms = 3;
...
{
    NemuClipboardFormats Msg;

    Msg.hdr.result = VERR_WRONG_ORDER;
    Msg.hdr.u32ClientID = idClient;
    Msg.hdr.u32Function = NEMU_SHARED_CLIPBOARD_FN_FORMATS;
    Msg.hdr.cParms = 1;
 ...
 *
 * This is usually called in reply to a NEMU_SHARED_CLIPBOARD_HOST_MSG_READ_DATA message
 * from the host.
 *
 * @returns Nemu status code.
 * @param   idClient        The client id returned by VbglR3ClipboardConnect().
...
{
    NemuClipboardWriteData Msg;
    Msg.hdr.result = VERR_WRONG_ORDER;
    Msg.hdr.u32ClientID = idClient;
    Msg.hdr.u32Function = NEMU_SHARED_CLIPBOARD_FN_WRITE_DATA;
    Msg.hdr.cParms = 2;
    VbglHGCMParmUInt32Set(&Msg.format, fFormat);
    VbglHGCMParmPtrSet(&Msg.ptr, pv, cb);
