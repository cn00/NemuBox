/* $Id: NemuGuestInstallHelper.cpp $ */
/** @file
 * NemuGuestInstallHelper - Various helper routines for Windows guest installer.
 */
...
/* Required structures/defines of NemuTray. */
#include "../../NemuTray/NemuTrayMsg.h"


/*********************************************************************************************************************************
*   Defined Constants And Macros                                                                                                 *
*********************************************************************************************************************************/
#define NEMUINSTALLHELPER_EXPORT extern "C" void __declspec(dllexport)
...
static HRESULT nemuPopULong(PULONG pulValue)
{
    HRESULT hr = S_OK;
    if (!g_stacktop || !*g_stacktop)
...
static void nemuPushResultAsString(HRESULT hr)
{
...
/**
 * Connects to NemuTray IPC under the behalf of the user running
 * in the current thread context.
 *
 * @return  IPRT status code.
 * @param   phSession               Where to store the IPC session.
 */
static int nemuConnectToNemuTray(RTLOCALIPCSESSION *phSession)
{
    int rc = VINF_SUCCESS;
...
static void nemuChar2WCharFree(PWCHAR pwString)
{
    if (pwString)
        HeapFree(GetProcessHeap(), 0, pwString);
}

static HRESULT nemuChar2WCharAlloc(const char *pszString, PWCHAR *ppwString)
{
    HRESULT hr;
 ...
 */
NEMUINSTALLHELPER_EXPORT DisableWFP(HWND hwndParent, int string_size,
                                    TCHAR *variables, stack_t **stacktop)
{
    EXDLL_INIT();

    TCHAR szFile[MAX_PATH + 1];
    HRESULT hr = nemuPopString(szFile, sizeof(szFile) / sizeof(TCHAR));
    if (SUCCEEDED(hr))
    {
        ...
        {
            WCHAR *pwszFile;
            hr = nemuChar2WCharAlloc(szFile, &pwszFile);
            if (SUCCEEDED(hr))
            {
                if (g_pfnSfcFileException(0, pwszFile, -1) != 0)
                    hr = HRESULT_FROM_WIN32(GetLastError());
                nemuChar2WCharFree(pwszFile);
            }
        }
 ...
 * @param   stacktop            Pointer to a pointer to the current stack.
 */
NEMUINSTALLHELPER_EXPORT FileGetArchitecture(HWND hwndParent, int string_size,
                                             TCHAR *variables, stack_t **stacktop)
{
    EXDLL_INIT();
 ...
 * @param   stacktop            Pointer to a pointer to the current stack.
 */
NEMUINSTALLHELPER_EXPORT FileGetVendor(HWND hwndParent, int string_size,
                                       TCHAR *variables, stack_t **stacktop)
{
    EXDLL_INIT();
...
/**
 * Shows a balloon message using NemuTray's notification area in the
 * Windows task bar.
 *
 * @param   hwndParent          Window handle of parent.
 * @param   string_size         Size of variable string.
 ...
 * @param   stacktop            Pointer to a pointer to the current stack.
 */
NEMUINSTALLHELPER_EXPORT NemuTrayShowBallonMsg(HWND hwndParent, int string_size,
                                               TCHAR *variables, stack_t **stacktop)
{
    EXDLL_INIT();
    ...
    char szMsg[256];
    char szTitle[128];
    HRESULT hr = nemuPopString(szMsg, sizeof(szMsg) / sizeof(char));
    if (SUCCEEDED(hr))
        hr = nemuPopString(szTitle, sizeof(szTitle) / sizeof(char));
    ...
    if (SUCCEEDED(hr))
    {
        ...
        uint32_t cbMsg = sizeof(NEMUTRAYIPCMSG_SHOWBALLOONMSG)
                       + strlen(szMsg) + 1    /* Include terminating zero */
                       + strlen(szTitle) + 1; /* Dito. */
        Assert(cbMsg);
        PNEMUTRAYIPCMSG_SHOWBALLOONMSG pIpcMsg =
            (PNEMUTRAYIPCMSG_SHOWBALLOONMSG)RTMemAlloc(cbMsg);
        if (pIpcMsg)
        {
            ...
            if (SUCCEEDED(hr))
                hr = nemuPopULong((ULONG*)&pIpcMsg->uType);
            if (SUCCEEDED(hr))
                hr = nemuPopULong((ULONG*)&pIpcMsg->uShowMS);

            if (SUCCEEDED(hr))
            {
                RTLOCALIPCSESSION hSession = 0;
                int rc = nemuConnectToNemuTray(&hSession);
                if (RT_SUCCESS(rc))
                {
                    NEMUTRAYIPCHEADER ipcHdr = { NEMUTRAY_IPC_HDR_MAGIC, 0 /* Header version */,
                                                 NEMUTRAYIPCMSGTYPE_SHOWBALLOONMSG, cbMsg };
