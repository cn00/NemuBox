...
static RTEXITCODE DoStandardOption(int ch)
{
    switch (ch)
    {
        case 'h':
        {
            RTMsgInfo(NEMU_PRODUCT " Extension Pack Helper App\n"
                      "(C) " NEMU_C_YEAR " " NEMU_VENDOR "\n"
                      "All rights reserved.\n"
                      "\n"
                      "This NOT intended for general use, please use NemuManage instead\n"
...
static bool IsValidCertificateDir(const char *pszCertDir)
{
    ...
    if (RT_FAILURE(rc))
        return false;
    rc = RTPathAppend(szCorrect, sizeof(szCorrect), NEMU_EXTPACK_CERT_DIR);
    if (RT_FAILURE(rc))
        return false;
...
static bool IsValidBaseDir(const char *pszBaseDir)
{
    ...
    if (RT_FAILURE(rc))
        return false;
    rc = RTPathAppend(szCorrect, sizeof(szCorrect), NEMU_EXTPACK_INSTALL_DIR);
    if (RT_FAILURE(rc))
        return false;
...
static RTEXITCODE CommonUninstallWorker(const char *pszExtPackDir)
{
    ...
    if (RT_FAILURE(rc))
        return RTMsgErrorExit(RTEXITCODE_FAILURE,
                              ...
                              "If the problem persists, try running the command: NemuManage extpack cleanup", rc);
...
/**
 * Wrapper around NemuExtPackOpenTarFss.
 *
 * @returns success or failure, message displayed on failure.
...
{
    char szError[8192];
    int rc = NemuExtPackOpenTarFss(hTarballFile, szError, sizeof(szError), phTarFss, NULL);
    if (RT_FAILURE(rc))
    {
        Assert(szError[0]);
...
/**
 * Wrapper around NemuExtPackValidateMember.
 *
...
{
    char szError[8192];
    int rc = NemuExtPackValidateMember(pszName, enmType, hVfsObj, szError, sizeof(szError));
    if (RT_FAILURE(rc))
    {
        Assert(szError[0]);
        return RTMsgErrorExit(RTEXITCODE_FAILURE, "%s", szError);
                                ...
                                const char *pszTarball)
{
    ...
    int rc = RTPathAbs(pszDirDst, szDstPath, sizeof(szDstPath) - NEMU_EXTPACK_MAX_MEMBER_NAME_LENGTH - 2);
    if (RT_FAILURE(rc))
...
/**
 * Wrapper around NemuExtPackValidateTarball.
 *
 * @returns The program exit code.
                                         ...
                                         const char *pszTarballDigest, PRTMANIFEST phValidManifest)
{
    *phValidManifest = NIL_RTMANIFEST;
    ...
    char szError[8192];
    int rc = NemuExtPackValidateTarball(hTarballFile, pszExtPackName, pszTarball, pszTarballDigest,
    ...
    for (;;)
    {
            ...
            && strcmp(Entry.szName, "..") != 0)
        {
            ...
            if (   pszMarker
                ...
                fCandidate = NemuExtPackIsValidMangledName(Entry.szName, pszMarker - &Entry.szName[0]);
            if (fCandidate)
            {
                /*
 ...
 *                              may be missing when running under NemuSVC.)
 */
                                         ...
                                         int iCmd, const char *pszDisplayInfoHack)
{
             ...
             && FindExecTool(szXterm, sizeof(szXterm), "xterm"))
    {
        ...
        RTMsgError("Unable to locate 'pkexec', 'gksu' or 'su+xterm'. Try perform the operation using NemuManage running as root");
    else
        RTMsgError("Unable to locate 'pkexec'. Try perform the operation using NemuManage running as root");
    if (iSuArg != cSuArgs)
    {
...
static RTEXITCODE RelaunchElevated(int argc, char **argv, int iCmd, const char *pszDisplayInfoHack)
{
    ...
    rc = RTPathAppend(szTempDir, sizeof(szTempDir), "NemuExtPackHelper-XXXXXX");
    if (RT_FAILURE(rc))
