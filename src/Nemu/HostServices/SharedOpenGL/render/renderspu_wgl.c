...
{
    NEMU_RENDERSPU_DESTROY_WINDOW vrdw;
    ...
    {
        PostThreadMessage(render_spu.dwWinThreadId, WM_NEMU_RENDERSPU_DESTROY_WINDOW, 0, (LPARAM) &vrdw);
            ...
            if (pWindow)
            {
                ...
                if (pCompositor)
                {
                    ...
                    if (hDC)
                    {
                        ...
                        renderspuNemuPresentCompositionGeneric(pWindow, pCompositor, NULL, 1, true);
...
#ifdef NEMU_CR_SERVER_FORCE_WGL
...
GLboolean renderspu_SystemNemuCreateWindow( VisualInfo *visual, GLboolean showIt, WindowInfo *window )
{
#if 0
    HDESK     desktop;
#endif
            ...
            (void*) render_spu_parent_window_id, NULL, hinstance, &render_spu );*/
    {
        ...
        if (render_spu.dwWinThreadId)
        {
            ...
            if (!PostThreadMessage(render_spu.dwWinThreadId, WM_NEMU_RENDERSPU_CREATE_WINDOW, 0, (LPARAM) &cs))
            {
...
void renderspu_SystemNemuPresentComposition( WindowInfo *window, const struct NEMUVR_SCR_COMPOSITOR_ENTRY *pChangedEntry )
{
    ...
    else
    {
        ...
        crWarning("renderspuNemuCompositorTryAcquire failed rc %d", rc);
    }
#endif
}
...
void renderspu_SystemMakeCurrent( WindowInfo *window, GLint nativeWindow, ContextInfo *context )
{
    ...
    if (context && window) {
        if (window->visual != context->visual) {
            ...
            renderspu_SystemNemuCreateWindow( context->visual, window->visible, window );
        }
