/* $Id: UIVMItem.cpp $ */
/** @file
 * Nemu Qt GUI - UIVMItem class implementation.
 */
...
static WId FindWindowIdFromPid(ULONG aPid)
{
     ...
     * Or at least check that it's not a NemuVRDP process. */
    NOREF (aPid);
    return (WId) 0;
...
{
    return m_fAccessible ? nemuGlobal().vmGuestOSTypeIcon(m_strOSTypeId, pLogicalSize) :
                           nemuGlobal().vmGuestOSTypeIcon("Other", pLogicalSize);
}
...
bool UIVMItem::recache()
{
    ...
    else
    {
        ...
        QString name = NemuGlobal::hasAllowedExtension(fi.completeSuffix(), NemuFileExts) ?
