...
#define NEMUNETADPCTL_NAME "NemuNetAdpCtl"
...
static int NetIfAdpCtl(const char * pcszIfName, const char *pszAddr, const char *pszOption, const char *pszMask)
{
    ...
    int rc = RTPathExecDir(szAdpCtl, sizeof(szAdpCtl) - sizeof("/" NEMUNETADPCTL_NAME));
    if (RT_FAILURE(rc))
    {
        LogRel(("NetIfAdpCtl: failed to get program path, rc=%Rrc.\n", rc));
        return rc;
    }
    strcat(szAdpCtl, "/" NEMUNETADPCTL_NAME);
    args[0] = szAdpCtl;
    if (!RTPathExists(szAdpCtl))
    {
        LogRel(("NetIfAdpCtl: path %s does not exist. Failed to run " NEMUNETADPCTL_NAME " helper.\n",
                szAdpCtl));
...
{
    char szAdpCtl[RTPATH_MAX];
    int rc = RTPathExecDir(szAdpCtl, sizeof(szAdpCtl) - sizeof("/" NEMUNETADPCTL_NAME " ") - strlen(pcszCmd));
    if (RT_FAILURE(rc))
    {
    ...
    if (fp)
    {
        ...
        {
            if (!strncmp(NEMUNETADPCTL_NAME ":", pszBuffer, sizeof(NEMUNETADPCTL_NAME)))
            {
        ...
        {
            LogRel(("NetIfAdpCtlOut: No output from " NEMUNETADPCTL_NAME));
            rc = VERR_INTERNAL_ERROR;
        }
        pclose(fp);
    }
                                        ...
                                        const char *pcszName)
{
    ...
    if (SUCCEEDED(hrc))
    {
        ...
        if (SUCCEEDED(hrc))
        {
            ...
            char szAdpCtl[RTPATH_MAX];
            int rc = RTPathExecDir(szAdpCtl, sizeof(szAdpCtl) - sizeof("/" NEMUNETADPCTL_NAME " add"));
            if (RT_FAILURE(rc))
            {
            ...
            if (fp)
            {
                ...
                if (fgets(szBuf, sizeof(szBuf), fp))
                {
                    ...
                    if (!strncmp(NEMUNETADPCTL_NAME ":", szBuf, sizeof(NEMUNETADPCTL_NAME)))
                    {
                        progress->i_notifyComplete(E_FAIL,
                                                   COM_IIDOF(IHostNetworkInterface),
                                                   HostNetworkInterface::getStaticComponentName(),
                    ...
                    else
                    {
                        ...
                        if (RT_FAILURE(rc))
                        {
                            progress->i_notifyComplete(E_FAIL,
                                                       COM_IIDOF(IHostNetworkInterface),
                                                       HostNetworkInterface::getStaticComponentName(),
                                                       "Failed to get config info for %s (as reported by '" NEMUNETADPCTL_NAME " add')\n", szBuf);
                        }
                        else
                        {
                            Bstr IfName(szBuf);
                    ...
                    if ((rc = pclose(fp)) != 0)
                    {
                        progress->i_notifyComplete(E_FAIL,
                                                   COM_IIDOF(IHostNetworkInterface),
                                                   HostNetworkInterface::getStaticComponentName(),
                                                   "Failed to execute '" NEMUNETADPCTL_NAME " add' (exit status: %d)", rc);
                        rc = VERR_INTERNAL_ERROR;
                    }
                }
                else
                {
                    ...
                    progress->i_notifyComplete(E_FAIL,
                                               COM_IIDOF(IHostNetworkInterface),
                                               HostNetworkInterface::getStaticComponentName(),
                                               "Failed to execute '" NEMUNETADPCTL_NAME " add' (exit status: %d). Check permissions!", rc);
                    pclose(fp);
                }
            }
            if (RT_SUCCESS(rc))
        ...
        if (SUCCEEDED(rc))
        {
            ...
            if (RT_FAILURE(rc))
                progress->i_notifyComplete(E_FAIL,
                                           COM_IIDOF(IHostNetworkInterface),
                                           HostNetworkInterface::getStaticComponentName(),
                                           "Failed to execute '" NEMUNETADPCTL_NAME "' (exit status: %d)", rc);
            else
