/* $Id: RTAssertShouldPanic-nemu.cpp $ */
/** @file
...
/** @def NEMU_RTASSERT_WITH_GDB
 * Enables the 'gdb' NEMU_ASSERT option.
 */
#if defined(DOXYGEN_RUNNING) \
 || (   !defined(NEMU_RTASSERT_WITH_GDB) \
     && !defined(IN_GUEST) \
     && !defined(RT_OS_OS2) \
     && !defined(RT_OS_WINDOWS))
# define NEMU_RTASSERT_WITH_GDB
#endif

#ifdef NEMU_RTASSERT_WITH_GDB
...
{
    /*
     * Check for the NEMU_ASSERT variable.
     */
    const char *psz = RTEnvGet("NEMU_ASSERT");
    ...
    if (!strcmp(psz, "gdb"))
    {
        ...
        const char *pszTerm = RTEnvGet("NEMU_ASSERT_TERM");
        if (    !pszTerm
        ...
        /* And find gdb. */
        const char *pszGdb = RTEnvGet("NEMU_ASSERT_GDB");
        if (    !pszGdb
            ||  !RTPathExists(pszGdb))
        {
            pszGdb = "/usr/bin/gdb";
