...
#else  /* NEMU - Resettable U32 stats, see NemuRecompiler.c. */
...
static void page_init(void)
{
    ...
    while ((1 << qemu_host_page_bits) < NEMU_ONLY((int))qemu_host_page_size)
        qemu_host_page_bits++;
    qemu_host_page_mask = ~(qemu_host_page_size - 1);

#ifndef NEMU /* We use other means to set reserved bit on our pages */
...
#ifdef NEMU /*  We don't need such huge codegen buffer size, as execute
...
#if defined(NEMU) && defined(USE_STATIC_CODE_GEN_BUFFER)
# error "Nemu allocates codegen buffer dynamically"
#endif
...
#ifndef NEMU /** @todo r=bird: why are we different? */
...
TranslationBlock *tb_alloc(target_ulong pc)
{
    ...
    if (nb_tbs >= code_gen_max_blocks ||
        (code_gen_ptr - code_gen_buffer) >= NEMU_ONLY((uintptr_t))code_gen_buffer_max_size)
        return NULL;
    tb = &tbs[nb_tbs++];
    tb->pc = pc;
    tb->cflags = 0;
...
static void tlb_protect_code(ram_addr_t ram_addr)
{
    cpu_physical_memory_reset_dirty(ram_addr,
                                    ...
                                    CODE_DIRTY_FLAG);
#if defined(NEMU) && defined(REM_MONITOR_CODE_PAGES)
