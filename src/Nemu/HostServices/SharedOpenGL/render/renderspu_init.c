...
static DWORD WINAPI renderSPUWindowThreadProc(void* unused)
{
    ...
    while( (bRet = GetMessage( &msg, 0, 0, 0 )) != 0)
    {
        ...
        {
            if (msg.message == WM_NEMU_RENDERSPU_CREATE_WINDOW)
            {
            ...
            else if (msg.message == WM_NEMU_RENDERSPU_DESTROY_WINDOW)
            {
                CRASSERT(msg.lParam && !msg.wParam);

                DestroyWindow(((NEMU_RENDERSPU_DESTROY_WINDOW*) msg.lParam)->hWnd);
               ...
               unsigned int context_id, unsigned int num_contexts )
{
    ...
    render_spu.id = id;
    renderspuSetNemuConfiguration(&render_spu);
...
# else /* NEMU_WITH_COCOA_QT */
    ...
    {
        {kEventClassNemu, kEventNemuUpdateContext}, /* Update the context after show/size/move events */
        {kEventClassNemu, kEventNemuBoundsChanged}  /* Clip/Pos the OpenGL windows when the main window is changed in pos/size */
    };
...
{
    renderspuNemuCompositorClearAll();
