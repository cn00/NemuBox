...
class InstallTestVm(nemutestvms.TestVm):
    ...
    kasIdeIrqDelay   = [ 'NemuInternal/Devices/piix3ide/0/Config/IRQDelay:1', ];
    ...
    def __init__(self, oSet, sVmName, sKind, sInstallIso, sHdCtrlNm, cGbHdd, fFlags):
        nemutestvms.TestVm.__init__(self, oSet, sVmName, sKind = sKind, sHddControllerType = sHdCtrlNm,
        ...
        if oVM is not None:
            ...
            if oSession is not None:
                ...
                if fRc is True and oHd is not None:
                    ...
                    fRc = fRc and oTestDrv.oNemu.deleteHdByMedium(oHd);
    ...
    def getReconfiguredVm(self, oTestDrv, cCpus, sVirtMode, sParavirtMode=None):
        ...
        #
        (fRc, oVM) = nemutestvms.TestVm.getReconfiguredVm(self, oTestDrv, cCpus, sVirtMode, sParavirtMode);
        ...
        if fRc is True:
            ...
            if oSession is not None:
                if self.sHddControllerType == self.ksSataController:
                    fRc = fRc and oSession.setStorageControllerType(nemucon.StorageControllerType_IntelAhci,
                ...
                fRc = fRc and oSession.setBootOrder(1, nemucon.DeviceType_HardDisk)
                fRc = fRc and oSession.setBootOrder(2, nemucon.DeviceType_DVD)
    ...
    def isHostCpuAffectedByUbuntuNewAmdBug(self, oTestDrv):
        ...
        if not oTestDrv.isHostCpuAmd():
            return False;
        try:
            (uMaxExt, _, _, _) = oTestDrv.oNemu.host.getProcessorCPUIDLeaf(0, 0x80000000, 0);
            (uFamilyModel, _, _, _) = oTestDrv.oNemu.host.getProcessorCPUIDLeaf(0, 0x80000001, 0);
        except:
            reporter.logXcpt();
            return False;
...
class tdGuestOsInstTest1(nemu.TestDriver):
    """
    ...
    def __init__(self):
        ...
        #
        oSet = nemutestvms.TestVmSet(self.oTestVmManager, fIgnoreSkippedVm = True);
    ...
    def actionConfig(self):
        if not self.importNemuApi(): # So we can use the constant below.
            return False;
        return self.oTestVmSet.actionConfig(self, eNic0AttachType = nemucon.NetworkAttachmentType_NAT);
