/* $Id: NemuService-win.cpp $ */
/** @file
 * NemuService - Guest Additions Service Skeleton, Windows Specific Parts.
 */
...
static RTEXITCODE vgsvcWinSetDesc(SC_HANDLE hService)
{
    ...
    desc.lpDescription = NEMUSERVICE_DESCRIPTION;
...
RTEXITCODE VGSvcWinInstall(void)
{
    ...
    SC_HANDLE  hService = CreateService(hSCManager,
                                        NEMUSERVICE_NAME, NEMUSERVICE_FRIENDLY_NAME,
                                        SERVICE_ALL_ACCESS,
    ...
    else
    {
        ...
        switch (dwErr)
        {
            case ERROR_SERVICE_EXISTS:
                ...
                hService = OpenService(hSCManager, NEMUSERVICE_NAME, SERVICE_ALL_ACCESS);
                if (hService)
                {
...
RTEXITCODE VGSvcWinUninstall(void)
{
    ...
    RTEXITCODE rcExit;
    SC_HANDLE  hService = OpenService(hSCManager, NEMUSERVICE_NAME, SERVICE_ALL_ACCESS );
    if (hService != NULL)
    {
        if (DeleteService(hService))
        {
            ...
            if (RegOpenKeyEx(HKEY_LOCAL_MACHINE,
                ...
                == ERROR_SUCCESS)
            {
                RegDeleteKey(hKey, NEMUSERVICE_NAME);
                RegCloseKey(hKey);
            }
...
static int vgsvcWinStart(void)
{
    ...
    {
        DWORD dwRes = vgsvcWinAddAceToObjectsSecurityDescriptor(TEXT("\\\\.\\NemuMiniRdrDN"),
                                                                      SE_FILE_OBJECT,
        ...
        if (dwRes != ERROR_SUCCESS)
        {
            if (dwRes == ERROR_FILE_NOT_FOUND)
            {
                /* If we don't find our "NemuMiniRdrDN" (for Shared Folders) object above,
...
#endif
{
    ...
    switch (dwControl)
    {
        ...
        case SERVICE_CONTROL_SHUTDOWN:
        {
            ...
            {
                rc2 = VGSvcReportStatus(NemuGuestFacilityStatus_Terminated);
                AssertRC(rc2);
            }
...
#ifdef TARGET_NT4
    g_hWinServiceStatus = RegisterServiceCtrlHandler(NEMUSERVICE_NAME, vgsvcWinCtrlHandler);
#else
    g_hWinServiceStatus = RegisterServiceCtrlHandlerEx(NEMUSERVICE_NAME, vgsvcWinCtrlHandler, NULL);
#endif
