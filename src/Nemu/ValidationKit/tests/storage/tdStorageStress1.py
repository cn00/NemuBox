...
def _ControllerTypeToName(eControllerType):
    ...
    if eControllerType == nemucon.StorageControllerType_PIIX3 or eControllerType == nemucon.StorageControllerType_PIIX4:
        sType = "IDE Controller";
    elif eControllerType == nemucon.StorageControllerType_IntelAhci:
        sType = "SATA Controller";
    elif eControllerType == nemucon.StorageControllerType_LsiLogicSas:
        sType = "SAS Controller";
    elif eControllerType == nemucon.StorageControllerType_LsiLogic or eControllerType == nemucon.StorageControllerType_BusLogic:
        sType = "SCSI Controller";
    else:
        sType = "Storage Controller";
    return sType;

class tdStorageStress(nemu.TestDriver):                                      # pylint: disable=R0902
    """
    Storage testcase.
    """
    ...
    def completeOptions(self):
        ...
        return nemu.TestDriver.completeOptions(self);
    ...
    def actionConfig(self):
        ...
        sNemuValidationKit_iso = os.path.abspath(os.path.join(os.path.dirname(__file__),
                                                              '../../NemuValidationKitStorIo.iso'));
        if not os.path.isfile(sNemuValidationKit_iso):
            sNemuValidationKit_iso = os.path.abspath(os.path.join(os.path.dirname(__file__),
                                                                  '../../NemuValidationKitStorIo.iso'));
        if not os.path.isfile(sNemuValidationKit_iso):
            sNemuValidationKit_iso = '/mnt/ramdisk/nemu/svn/trunk/validationkit/NemuValidationKitStorIo.iso';
        if not os.path.isfile(sNemuValidationKit_iso):
            sNemuValidationKit_iso = '/mnt/ramdisk/nemu/svn/trunk/testsuite/NemuTestSuiteStorIo.iso';
        if not os.path.isfile(sNemuValidationKit_iso):
            sCur = os.getcwd();
            for i in range(0, 10):
                sNemuValidationKit_iso = os.path.join(sCur, 'validationkit/NemuValidationKitStorIo.iso');
                if os.path.isfile(sNemuValidationKit_iso):
                    break;
                sNemuValidationKit_iso = os.path.join(sCur, 'testsuite/NemuTestSuiteStorIo.iso');
                if os.path.isfile(sNemuValidationKit_iso):
                    break;
                sCur = os.path.abspath(os.path.join(sCur, '..'));
                if i is None: pass; # shut up pychecker/pylint.
        if not os.path.isfile(sNemuValidationKit_iso):
            sNemuValidationKit_iso = '/mnt/VirtualBox/NemuValidationKitStorIo.iso';
        if not os.path.isfile(sNemuValidationKit_iso):
            sNemuValidationKit_iso = '/mnt/VirtualBox/NemuTestSuiteStorIo.iso';
        ...
        if 'tst-debian' in self.asTestVMs:
            oVM = self.createTestVM('tst-debian', 1, '4.2/storage/debian.vdi', sKind = 'Debian_64', fIoApic = True, \
                                    ...
                                    eNic0Type = nemucon.NetworkAdapterType_Am79C973, \
                                    sDvdImage = sNemuValidationKit_iso);
            if oVM is None:
                return False;
        ...
        if oSession is not None:
            ...
            if sDiskFormat == "iSCSI":
                ...
                if self.fpApiVer >= 5.0:
                    oHd = oSession.oNemu.createMedium(sDiskFormat, sDiskPath1, nemucon.AccessMode_ReadWrite, \
                                                      nemucon.DeviceType_HardDisk);
                else:
                    oHd = oSession.oNemu.createHardDisk(sDiskFormat, sDiskPath1);
                    ...
                    try:
                        if oSession.fpApiVer >= 4.0:
                            oSession.o.machine.attachDevice(_ControllerTypeToName(eStorageController), \
                                                            1, 0, nemucon.DeviceType_HardDisk, oHd);
                        else:
                            oSession.o.machine.attachDevice(_ControllerTypeToName(eStorageController), \
                                                            1, 0, nemucon.DeviceType_HardDisk, oHd.id);
                    except:
            ...
            if oSession is not None:
                ...
                if oSession is not None:
                    try:
                        ...
                        if     eStorageController is not nemucon.StorageControllerType_PIIX3 \
                           and eStorageController is not nemucon.StorageControllerType_PIIX4:
                            oSession.o.machine.removeStorageController(_ControllerTypeToName(eStorageController));

                        oSession.saveSettings();
                        oSession.oNemu.deleteHdByLocation(sDiskPath1);
                        oSession.oNemu.deleteHdByLocation(sDiskPath2);
                        oSession.saveSettings();
                        oSession.close();
                        oSession = None;
                    except:
    ...
    def test1OneVM(self, sVmName):
        ...
        for sStorageCtrl in self.asStorageCtrls:
            ...
            if sStorageCtrl == 'AHCI':
                eStorageCtrl = nemucon.StorageControllerType_IntelAhci;
            elif sStorageCtrl == 'IDE':
                eStorageCtrl = nemucon.StorageControllerType_PIIX4;
            elif sStorageCtrl == 'LsiLogicSAS':
                eStorageCtrl = nemucon.StorageControllerType_LsiLogicSas;
            elif sStorageCtrl == 'LsiLogic':
                eStorageCtrl = nemucon.StorageControllerType_LsiLogic;
            elif sStorageCtrl == 'BusLogic':
                eStorageCtrl = nemucon.StorageControllerType_BusLogic;
            else:
                eStorageCtrl = None;
