/* $Id: tstNemuAPIXPCOM.cpp $ */
/** @file
 *
 * tstNemuAPIXPCOM - sample program to illustrate the VirtualBox
 *                   XPCOM API for machine management.
 *                   It only uses standard C/C++ and XPCOM semantics,
 *                   no additional Nemu classes/macros/helpers.
 */
 ...
 * applications such as the VirtualBox GUI frontend or the NemuManage command
 * line frontend.
 *
 *
 ...
 *
 *   $ env NEMU_XPCOM_HOME=../../.. LD_LIBRARY_PATH=../../.. ./tstNemuAPIXPCOM
 *
 * The above command assumes that NemuRT.so, NemuXPCOM.so and others reside in
 * the directory ../../..
 */
...
void createVM(IVirtualBox *virtualBox)
{
    ...
    rc = virtualBox->OpenMedium(NS_LITERAL_STRING("/home/nemu/isos/winnt4ger.iso").get(),
                                DeviceType_DVD,
                                AccessMode_ReadOnly,
                                false /* fForceNewUuid */,
