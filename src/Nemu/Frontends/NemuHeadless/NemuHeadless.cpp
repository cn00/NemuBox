/* $Id: NemuHeadless.cpp $ */
/** @file
 * NemuHeadless - The VirtualBox Headless frontend for running VMs on servers.
 */
...
//#define NEMU_WITH_SAVESTATE_ON_SIGNAL
#ifdef NEMU_WITH_SAVESTATE_ON_SIGNAL
#include <signal.h>
#endif
...
#define LogError(m,rc) \
    do { \
        Log(("NemuHeadless: ERROR: " m " [rc=0x%08X]\n", rc)); \
        RTPrintf("%s\n", m); \
    } while (0)
    ...
    STDMETHOD(HandleEvent)(NemuEventType_T aType, IEvent *aEvent)
    {
        switch (aType)
        {
            case NemuEventType_OnNemuSVCAvailabilityChanged:
            {
                ...
                if (!fAvailable)
                {
                    LogRel(("NemuHeadless: NemuSVC became unavailable, exiting.\n"));
                    RTPrintf("NemuSVC became unavailable, exiting.\n");
                    /* Terminate the VM as cleanly as possible given that NemuSVC
...
public:
    ...
    STDMETHOD(HandleEvent)(NemuEventType_T aType, IEvent *aEvent)
    {
        switch (aType)
        {
            ...
            case NemuEventType_OnVRDEServerInfoChanged:
            {
            ...
            case NemuEventType_OnGuestPropertyChanged:
            {
...
NEMU_LISTENER_DECLARE(VirtualBoxClientEventListenerImpl)
NEMU_LISTENER_DECLARE(ConsoleEventListenerImpl)
...
#endif /* NEMU_WITH_SAVESTATE_ON_SIGNAL */
 ...
 *        Use RTEnvGetEx instead! */
    if ((pszEnvTemp = RTEnvGet("NEMU_CAPTUREWIDTH")) != 0)
    {
        errno = 0;
        unsigned long ulFrameWidth = strtoul(pszEnvTemp, 0, 10);
        if (errno != 0)
            LogError("NemuHeadless: ERROR: invalid NEMU_CAPTUREWIDTH environment variable", 0);
        else
            *pulFrameWidth = ulFrameWidth;
    }
    if ((pszEnvTemp = RTEnvGet("NEMU_CAPTUREHEIGHT")) != 0)
    {
        errno = 0;
        unsigned long ulFrameHeight = strtoul(pszEnvTemp, 0, 10);
        if (errno != 0)
            LogError("NemuHeadless: ERROR: invalid NEMU_CAPTUREHEIGHT environment variable", 0);
        else
            *pulFrameHeight = ulFrameHeight;
    }
    if ((pszEnvTemp = RTEnvGet("NEMU_CAPTUREBITRATE")) != 0)
    {
        errno = 0;
        unsigned long ulBitRate = strtoul(pszEnvTemp, 0, 10);
        if (errno != 0)
            LogError("NemuHeadless: ERROR: invalid NEMU_CAPTUREBITRATE environment variable", 0);
        else
            *pulBitRate = ulBitRate;
    }
    if ((pszEnvTemp = RTEnvGet("NEMU_CAPTUREFILE")) != 0)
        *ppszFileName = pszEnvTemp;
}
#endif /* NEMU_WITH_VPX defined */
...
extern "C" DECLEXPORT(int) TrustedMain(int argc, char **argv, char **envp)
{
    ...
    const char *pszFileNameParam = "Nemu-%d.vob";
#endif /* NEMU_WITH_VPX */

    LogFlow(("NemuHeadless STARTED.\n"));
    RTPrintf(NEMU_PRODUCT " Headless Interface " NEMU_VERSION_STRING "\n"
    ...
    {
        LogError("NemuHeadless: ERROR: please specify an even frame width between 512 and 2048", 0);
        return 1;
    }
    if (ulFrameHeight < 384 || ulFrameHeight > 1536 || ulFrameHeight % 2)
    {
        LogError("NemuHeadless: ERROR: please specify an even frame height between 384 and 1536", 0);
        return 1;
    }
    if (ulBitRate < 300000 || ulBitRate > 1000000)
    {
        LogError("NemuHeadless: ERROR: please specify an even bitrate between 300000 and 1000000", 0);
        return 1;
    }
    ...
    {
        LogError("NemuHeadless: ERROR: Only %%d and %%u are allowed in the capture file name.", -1);
        return 1;
    }
    /* And no more than one % in the name */
    if (pcPercent != 0 && strchr(pcPercent + 1, '%') != 0)
    {
        LogError("NemuHeadless: ERROR: Only one format modifier is allowed in the capture file name.", -1);
        return 1;
    }
    RTStrPrintf(&szMpegFile[0], RTPATH_MAX, pszFileNameParam, RTProcSelf());
#endif /* defined NEMU_WITH_VPX */
    ...
    if (FAILED(rc))
    {
        RTPrintf("NemuHeadless: ERROR: failed to initialize COM!\n");
        return 1;
    }
    ...
    ComPtr<IEventListener> nemuClientListener;
    ComPtr<IEventListener> nemuListener;
    ...
    do
    {
        ...
        if (FAILED(rc))
        {
            RTPrintf("NemuHeadless: ERROR: failed to create the VirtualBoxClient object!\n");
            com::ErrorInfo info;
        ...
        Log(("NemuHeadless: Opening a session with machine (id={%s})...\n",
              Utf8Str(id).c_str()));
        ...
        /* VirtualBoxClient events registration. */
        {
            ...
            nemuClientListener = listener;
            com::SafeArray<NemuEventType_T> eventTypes;
            eventTypes.push_back(NemuEventType_OnNemuSVCAvailabilityChanged);
            CHECK_ERROR(pES, RegisterListener(nemuClientListener, ComSafeArrayAsInParam(eventTypes), true));
        }
        ...
        /* Console events registration. */
        {
            ...
            eventTypes.push_back(NemuEventType_OnVRDEServerInfoChanged);
            eventTypes.push_back(NemuEventType_OnCanShowWindow);
            eventTypes.push_back(NemuEventType_OnShowWindow);
            eventTypes.push_back(NemuEventType_OnGuestPropertyChanged);
        ...
        Log(("NemuHeadless: enmVRDE %d, fVRDEEnabled %d\n", enmVRDEOption, fVRDEEnabled));
        ...
        Log(("NemuHeadless: Powering up the machine...\n"));
        ...
        Log(("NemuHeadless: Waiting for PowerDown...\n"));

        while (   !g_fTerminateFE
               && RT_SUCCESS(gEventQ->processEventQueue(RT_INDEFINITE_WAIT)))
            /* nothing */ ;

        Log(("NemuHeadless: event loop has terminated...\n"));
    ...
    do
    {
        ...
        if (FAILED(hrc))
        {
            RTPrintf("NemuHeadless: ERROR: Failed to power down VM!");
            com::ErrorInfo info;
            if (!info.isFullAvailable() && !info.isBasicAvailable())
    ...
    if (nemuListener)
    {
        ...
        if (!es.isNull())
            CHECK_ERROR(es, UnregisterListener(nemuListener));
        nemuListener.setNull();
    }
    ...
    if (nemuClientListener)
    {
        ComPtr<IEventSource> pES;
        CHECK_ERROR(pVirtualBoxClient, COMGETTER(EventSource)(pES.asOutParam()));
        if (!pES.isNull())
            CHECK_ERROR(pES, UnregisterListener(nemuClientListener));
        nemuClientListener.setNull();
    }
    ...
    if (fSessionOpened)
    {
         ...
         */
        Log(("NemuHeadless: Closing the session...\n"));
...
int main(int argc, char **argv, char **envp)
{
    ...
    if (RT_FAILURE(rc))
    {
        RTPrintf("NemuHeadless: Runtime Error:\n"
                 " %Rrc -- %Rrf\n", rc, rc);
        switch (rc)
        {
