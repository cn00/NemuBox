...
protected:
        ...
        , m_cRefs(1)
    {
        ...
        if (RT_FAILURE(rc))
            throw (Global::nemuStatusCodeToCOM(rc));
    }
 ...
 * environment block (in NemuService before launching a new process).
 */
...
struct GuestFsObjData
{
     ...
     *  a certin NemuService tool's guest stream block. */
...
/**
 * Class for parsing machine-readable guest process output by NemuService'
 * toolbox commands ("nemu_ls", "nemu_stat" etc), aka "guest stream".
 */
...
typedef std::list < NemuEventType_T > GuestEventTypes;
...
typedef std::map < NemuEventType_T, GuestWaitEvents > GuestEventGroup;
...
public:
    ...
    int signalWaitEvent(NemuEventType_T aType, IEvent *aEvent);
    /** Signals a wait event using a guest rc. */
    int signalWaitEventInternal(PNEMUGUESTCTRLHOSTCBCTX pCbCtx, int guestRc, const GuestWaitEventPayload *pPayload);
    /** Signals a wait event without letting public guest events know,
     *  extended director's cut version. */
    int signalWaitEventInternalEx(PNEMUGUESTCTRLHOSTCBCTX pCbCtx, int rc, int guestRc, const GuestWaitEventPayload *pPayload);
public:

    int baseInit(void);
    void baseUninit(void);
    int cancelWaitEvents(void);
    int dispatchGeneric(PNEMUGUESTCTRLHOSTCBCTX pCtxCb, PNEMUGUESTCTRLHOSTCALLBACK pSvcCb);
    ...
    int waitForEvent(GuestWaitEvent *pEvent, uint32_t uTimeoutMS, NemuEventType_T *pType, IEvent **ppEvent);
...
protected:
    ...
    virtual int i_callbackDispatcher(PNEMUGUESTCTRLHOSTCBCTX pCbCtx, PNEMUGUESTCTRLHOSTCALLBACK pSvcCb) = 0;
...
protected:
    ...
    int sendCommand(uint32_t uFunction, uint32_t uParms, PNEMUHGCMSVCPARM paParms);
