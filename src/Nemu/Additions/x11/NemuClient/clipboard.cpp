 ...
 */
static int nemuClipboardSendData(uint32_t u32Format, void *pv, uint32_t cb)
{
    int rc;
 ...
 *                    returned
 */
int ClipRequestDataForX11(NEMUCLIPBOARDCONTEXT *pCtx, uint32_t u32Format,
 ...
 */
void ClipReportX11Formats(NEMUCLIPBOARDCONTEXT *pCtx, uint32_t u32Formats)
{
    int rc;
...
{
    if (RT_SUCCESS(rc))
        nemuClipboardSendData(pReq->u32Format, pv, cb);
    else
        nemuClipboardSendData(0, NULL, 0);
    RTMemFree(pReq);
}
        ...
        if (RT_SUCCESS(rc))
        {
            switch (Msg)
            {
                case NEMU_SHARED_CLIPBOARD_HOST_MSG_FORMATS:
                {
                     ...
                     */
                    LogRelFlowFunc(("NEMU_SHARED_CLIPBOARD_HOST_MSG_FORMATS fFormats=%x\n", fFormats));
                ...
                case NEMU_SHARED_CLIPBOARD_HOST_MSG_READ_DATA:
                {
                    ...
                    LogRelFlowFunc(("NEMU_SHARED_CLIPBOARD_HOST_MSG_READ_DATA fFormats=%x\n", fFormats));
                    CLIPREADCBREQ *pReq;
...
{
    return ".nemuclient-clipboard.pid";
}
