/* $Id: */
/** @file
 * NemuWatchdogUtils - Misc. utility functions for modules.
 */
 ...
 */
int getMetric(PNEMUWATCHDOG_MACHINE pMachine, const Bstr& strName, LONG *pulData)
{
...
int payloadAlloc(PNEMUWATCHDOG_MACHINE pMachine, const char *pszModule,
...
{
    AssertReturn(!rptrNemu.isNull(), VERR_INVALID_POINTER);
    ...
    if (strValue.isEmpty()) /* Not set by per-VM value? */
    {
        ...
        /* Try global approach. */
        hr = rptrNemu->GetExtraData(Bstr(pszGlobal).raw(),
                                    strTemp.asOutParam());
        if (   SUCCEEDED(hr)
            && !strTemp.isEmpty())
        {
...
{
    Utf8Str strValue;
    int rc = cfgGetValueStr(rptrNemu, rptrMachine, pszGlobal, pszVM, strValue, "" /* Default */);
    if (RT_SUCCESS(rc))
    {
