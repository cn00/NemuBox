...
int cpu_exec(CPUState *env1)
{
    ...
    int ret NEMU_ONLY(= 0), interrupt_request;
    TranslationBlock *tb;
    uint8_t *tc_ptr;
    uintptr_t next_tb;
...
#endif
#ifndef NEMU /* NEMU: We need to raise traps and suchlike from the outside. */
    env->exception_index = -1;
#endif /* !NEMU */
...
#define env cpu_single_env
            ...
            if (env->exception_index >= 0) {
                if (env->exception_index >= EXCP_INTERRUPT) {
                    ...
                    ret = env->exception_index;
#ifdef NEMU /* because of the above stuff */
                    env->exception_index = -1;
#endif
                    if (ret == EXCP_DEBUG)
...
# ifndef NEMU /** @todo reconcile our code with the following...  */
...
# ifndef NEMU /* we might be using elsewhere, we only have one. */
