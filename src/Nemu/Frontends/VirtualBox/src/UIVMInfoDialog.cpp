...
/** @file
 * Nemu Qt GUI - UIVMInfoDialog class implementation.
 */
...
void UIVMInfoDialog::retranslateUi()
{
    ...
    /* Network statistics: */
    ulong count = nemuGlobal().virtualBox().GetSystemProperties().GetMaxNetworkAdapters(KChipsetType_PIIX3);
...
{
    /* Details page update: */
    setText(m_browsers[0], nemuGlobal().detailsReport(m_session.GetMachine(), false /* with links */));
}
...
void UIVMInfoDialog::prepareThis()
{
    ...
    connect(&nemuGlobal(), SIGNAL(sigMediumEnumerationFinished()), this, SLOT(sltUpdateDetails()));
...
void UIVMInfoDialog::loadSettings()
{
    ...
    {
        /* Load geometry: */
        m_geometry = gEDataManager->informationWindowGeometry(this, m_pMachineWindow, nemuGlobal().managedVMUuid());
#ifdef Q_WS_MAC
        ...
        if (gEDataManager->informationWindowShouldBeMaximized(nemuGlobal().managedVMUuid()))
            showMaximized();
    }
}
...
void UIVMInfoDialog::saveSettings()
{
    /* Save window geometry: */
    {
        /* Save geometry: */
#ifdef Q_WS_MAC
        gEDataManager->setInformationWindowGeometry(m_geometry, ::darwinIsWindowMaximized(this), nemuGlobal().managedVMUuid());
#else /* Q_WS_MAC */
        gEDataManager->setInformationWindowGeometry(m_geometry, isMaximized(), nemuGlobal().managedVMUuid());
#endif /* !Q_WS_MAC */
...
void UIVMInfoDialog::refreshStatistics()
{
    ...
    /* Runtime Information: */
    {
        ...
        for (ULONG iScreen = 0; iScreen < cGuestScreens; ++iScreen)
        {
            ...
            if (monitorStatus == KGuestMonitorStatus_Disabled)
            {
                ...
                strResolution += QString(NemuGlobal::tr("off", "guest monitor status"));
            }
        ...
        if (strOSType.isEmpty())
            ...
            strOSType = nemuGlobal().vmGuestOSTypeDescription(strOSType);
        ...
        strResult += formatValue(NemuGlobal::tr("VT-x/AMD-V", "details report"), strVirtualization, iMaxLength);
        strResult += formatValue(NemuGlobal::tr("Nested Paging", "details report"), strNestedPaging, iMaxLength);
        strResult += formatValue(NemuGlobal::tr("Unrestricted Execution", "details report"), strUnrestrictedExecution, iMaxLength);
        strResult += formatValue(NemuGlobal::tr("Paravirtualization Interface", "details report"), strParavirtProvider, iMaxLength);
        ...
        strResult += formatValue(NemuGlobal::tr("Remote Desktop Server Port", "details report (VRDE Server)"), strVRDEInfo, iMaxLength);
    ...
    /* Network statistics: */
    {
        ...
        ulong uCount = nemuGlobal().virtualBox().GetSystemProperties().GetMaxNetworkAdapters(m.GetChipsetType());
                                             ...
                                             const QString &strBelongsTo)
{
    ...
    QString strHeader = "<tr><td></td><td colspan=2><nobr>%1</nobr></td></tr>";
    QString strName = NemuGlobal::tr("Adapter %1", "details report (network)").arg(uSlot + 1);
