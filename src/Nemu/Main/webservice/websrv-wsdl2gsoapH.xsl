 ...
 * Generated from: nemuweb.wsdl (generated WSDL file)
 * Generator: src/Nemu/Main/webservice/websrv-gsoapH.xsl
 *
...
#import "stlvector.h"
    ...
    <xsl:value-of select="concat('//gsoap nemu  schema namespace: ', $G_targetNamespace)" />
    <xsl:value-of select="concat('//gsoap nemu  schema form:  unqualified', '')" />
...
class _nemu__InvalidObjectFault;
class _nemu__RuntimeFault;

struct SOAP_ENV__Detail
{
    _nemu__InvalidObjectFault *nemu__InvalidObjectFault;
    _nemu__RuntimeFault *nemu__RuntimeFault;
    int __type;
    void *fault;
    _XML __any;
};
...
<xsl:template name="convertSequence">
  ...
  <xsl:for-each select="xsd:element">
    ...
    <xsl:variable name="withoutnemutypefield" select="substring($typefield, 6)" /><!-- remove "nemu:" prefix-->
    ...
    <xsl:choose>
      ...
      <xsl:when test="count(key('G_keySimpleTypesByName', $withoutnemutypefield)) > 0">
        <xsl:variable name="enumname">
          <xsl:value-of select="concat('enum nemu__', $withoutnemutypefield)" />
        </xsl:variable>
        <xsl:choose>
          <xsl:when test="@maxOccurs='unbounded'">
      ...
      <!-- is this one of the nemu types? (look up in complex types) -->
      <xsl:when test="count(key('G_keyComplexTypesByName', $withoutnemutypefield)) > 0">
        <!-- array or simple type: depends on whether maxOccurs="unbounded" is in WSDL -->
        <xsl:choose>
          <xsl:when test="@maxOccurs='unbounded'">
            <xsl:value-of select="concat('std::vector&lt;nemu__', $withoutnemutypefield, '*&gt;')" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="concat('nemu__', $withoutnemutypefield, '*')" />
          </xsl:otherwise>
        </xsl:choose>
      </xsl:when>
      <xsl:otherwise>
...
<xsl:template match="wsdl:types/xsd:schema">
  ...
  <xsl:for-each select="xsd:simpleType">
    <xsl:variable name="ctype" select="concat('nemu__', @name)" />
      ...
      <xsl:text>
{
  ...
  <xsl:for-each select="xsd:complexType">
    ...
    <xsl:variable name="ctype" select="concat('nemu__', $xmltype)" />
  ...
  <xsl:for-each select="xsd:element">
    ...
    <xsl:variable name="ctype" select="concat('_nemu__', $underscoredname)" />
    <xsl:for-each select="xsd:complexType">
      <xsl:for-each select="xsd:sequence">
        <xsl:call-template name="convertSequence">
          <xsl:with-param name="xmltype" select="$xmltype" />
...
<xsl:template match="wsdl:portType">
  ...
  <xsl:value-of select="concat('//gsoap nemu service name: nemu', $G_bindingSuffix, $G_sNewLine)" />
  <xsl:value-of select="concat('//gsoap nemu service type: nemu', $G_portTypeSuffix, $G_sNewLine)" />
  <xsl:value-of select="concat('//gsoap nemu service namespace: ', $G_targetNamespace, $G_targetNamespaceSeparator, $G_sNewLine)" />
  <xsl:value-of select="concat('//gsoap nemu service transport: ', 'http://schemas.xmlsoap.org/soap/http', $G_sNewLine)" />
  ...
  <xsl:for-each select="wsdl:operation">
    ...
    <xsl:value-of select="concat($G_sNewLine, '//gsoap nemu service method-style:    ', $cmethodname, ' ', $G_basefmt)" />
    <xsl:value-of select="concat($G_sNewLine, '//gsoap nemu service method-encoding: ', $cmethodname, ' ', $G_parmfmt)" />
    <xsl:value-of select="concat($G_sNewLine, '//gsoap nemu service method-action:   ', $cmethodname, ' &quot;&quot;')" />
    <xsl:value-of select="concat($G_sNewLine, '//gsoap nemu service method-fault:    ', $cmethodname, ' nemu__InvalidObjectFault')" />
    <xsl:value-of select="concat($G_sNewLine, '//gsoap nemu service method-fault:    ', $cmethodname, ' nemu__RuntimeFault')" />
    <xsl:value-of select="concat($G_sNewLine, 'int __nemu__', $cmethodname, '(', $G_sNewLine)" />
    ...
    <xsl:value-of select="concat('    _nemu__', $creqtype, '* nemu__', $creqtype, ',', $G_sNewLine)"/>
    <!-- response element -->
    ...
    <xsl:value-of select="concat('    _nemu__', $cresptype, '* nemu__', $cresptype, $G_sNewLine)"/>
