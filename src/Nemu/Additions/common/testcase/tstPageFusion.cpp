...
/** @file
 * NemuService - Guest page sharing testcase
 */
 ...
 */
void NemuServicePageSharingCheckModule(MODULEENTRY32 *pModule)
{
 ...
 */
void NemuServicePageSharingInspectModules(DWORD dwProcessId)
{
    HANDLE hSnapshot = CreateToolhelp32Snapshot(TH32CS_SNAPMODULE, dwProcessId);
    if (hSnapshot == INVALID_HANDLE_VALUE)
    {
        printf("NemuServicePageSharingInspectModules: CreateToolhelp32Snapshot failed with %d\n", GetLastError());
        return;
    }

    printf("NemuServicePageSharingInspectModules\n");
    ...
    do
    {
        /** todo when changing this make sure NemuService.exe is excluded! */
        char *pszDot = strrchr(ModuleInfo.szModule, '.');
        if (    pszDot
            &&  (pszDot[1] == 'e' || pszDot[1] == 'E'))
            continue;   /* ignore executables for now. */

        NemuServicePageSharingCheckModule(&ModuleInfo);
    }
 ...
 *
 */
void NemuServicePageSharingInspectGuest()
{
    NemuServicePageSharingInspectModules(GetCurrentProcessId());
...
/** @copydoc NEMUSERVICE::pfnInit */
static DECLCALLBACK(int) NemuServicePageSharingInit(void)
{
    printf("NemuServicePageSharingInit\n");
...
static DECLCALLBACK(void) NemuServicePageSharingTerm(void)
{
    printf("NemuServicePageSharingTerm\n");
...
int main(int argc, char **argv)
{
    ...
    NemuServicePageSharingInit();

    NemuServicePageSharingInspectGuest();

    NemuServicePageSharingTerm();
    return 0;
}
