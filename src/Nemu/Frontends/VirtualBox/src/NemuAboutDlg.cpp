/* $Id: NemuAboutDlg.cpp $ */
/** @file
 * Nemu Qt GUI - NemuAboutDlg class implementation.
 */
...
/* Other Nemu includes: */
# include <iprt/path.h>
# include <Nemu/version.h> /* NEMU_VENDOR */

#endif /* !NEMU_WITH_PRECOMPILED_HEADERS */

NemuAboutDlg::NemuAboutDlg(QWidget *pParent, const QString &strVersion)
...
bool NemuAboutDlg::event(QEvent *pEvent)
{
...
void NemuAboutDlg::paintEvent(QPaintEvent* /* pEvent */)
{
...
void NemuAboutDlg::retranslateUi()
{
    ...
    const QString strVersionText = "EXPERIMENTAL build %1 - " + QString(NEMU_BLEEDING_EDGE);
    ...
    m_strAboutText = strAboutText + " " + strVersionText.arg(m_strVersion) + "\n" +
                     QString("%1 2004-" NEMU_C_YEAR " " NEMU_VENDOR).arg(QChar(0xa9));
#else /* !NEMU_OSE */
    ...
    m_strAboutText = m_strAboutText + "\n" + QString("Copyright %1 %2 %3 and/or its affiliates. All rights reserved.")
                                                     .arg(QChar(0xa9)).arg(NEMU_C_YEAR).arg(NEMU_VENDOR);
    AssertPtrReturnVoid(m_pLabel);
    m_pLabel->setText(m_strAboutText);
}

void NemuAboutDlg::prepare()
{
    ...
    const QString strSplash = nemuGlobal().brandingGetKey("UI/AboutSplash");
    if (nemuGlobal().brandingIsActive() && !strSplash.isEmpty())
    {
...
void NemuAboutDlg::prepareMainLayout()
{
    /* Create main-layout: */
...
void NemuAboutDlg::prepareLabel()
{
    ...
    AssertPtrReturnVoid(m_pLabel);
    {
        ...
        const QString strColor = nemuGlobal().brandingGetKey("UI/AboutTextColor");
        if (!strColor.isEmpty())
...
void NemuAboutDlg::prepareCloseButton()
{
    /* Create button-box: */
