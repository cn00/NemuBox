/* $Id: WinKeyboard.cpp $ */
/** @file
 * Nemu Qt GUI - Windows keyboard handling..
 */
...
static void winSetModifierState(int idModifier, bool fState)
{
    ...
    if (winGetModifierState(idModifier) != fState)
    {
        ...
        LogRel2(("HID LEDs sync: setting %s state to %s (0x%X).\n",
                 NEMU_CONTROL_TO_STR_NAME(idModifier), NEMU_BOOL_TO_STR_STATE(fState), MapVirtualKey(idModifier, MAPVK_VK_TO_VSC)));
    }
    else
    {
...
{
    NemuModifiersState_t *pState;

    pState = (NemuModifiersState_t *)malloc(sizeof(NemuModifiersState_t));
    if (pState)
    {
        ...
        LogRel2(("HID LEDs sync: host state captured: NUM(%s) CAPS(%s) SCROLL(%s)\n",
                 NEMU_BOOL_TO_STR_STATE(pState->fNumLockOn),
                 NEMU_BOOL_TO_STR_STATE(pState->fCapsLockOn),
                 NEMU_BOOL_TO_STR_STATE(pState->fScrollLockOn)));
...
{
    NemuModifiersState_t *pState = (NemuModifiersState_t *)pData;
...
void WinHidDevicesBroadcastLeds(bool fNumLockOn, bool fCapsLockOn, bool fScrollLockOn)
{
    LogRel2(("HID LEDs sync: start broadcast guest modifier states: NUM(%s) CAPS(%s) SCROLL(%s)\n",
             NEMU_BOOL_TO_STR_STATE(fNumLockOn),
             NEMU_BOOL_TO_STR_STATE(fCapsLockOn),
             NEMU_BOOL_TO_STR_STATE(fScrollLockOn)));
