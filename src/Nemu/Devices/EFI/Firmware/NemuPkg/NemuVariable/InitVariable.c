...
static UINT32 NemuReadNVRAM(UINT8 *pu8Buffer, UINT32 cbBuffer)
{
    UINT32 idxBuffer = 0;
...
DECLINLINE(void) NemuWriteNVRAMU32Param(UINT32 u32CodeParam, UINT32 u32Param)
{
    ASMOutU32(EFI_PORT_VARIABLE_OP, u32CodeParam);
    ASMOutU32(EFI_PORT_VARIABLE_PARAM, u32Param);
}

static UINT32 NemuWriteNVRAMByteArrayParam(const UINT8 *pbParam, UINT32 cbParam)
{
    UINT32 idxParam = 0;
...
static void NemuWriteNVRAMNameParam(const CHAR16 *pwszName)
{
    UINTN i;
...
DECLINLINE(UINT32) NemuWriteNVRAMGuidParam(const EFI_GUID *pGuid)
{
    ASMOutU32(EFI_PORT_VARIABLE_OP, EFI_VM_VARIABLE_OP_GUID);
    return NemuWriteNVRAMByteArrayParam((UINT8 *)pGuid, sizeof(EFI_GUID));
}

static UINT32 NemuWriteNVRAMDoOp(UINT32 u32Operation)
{
    UINT32 u32Rc;
    NemuLogFlowFuncEnter();
    NemuLogFlowFuncMarkVar(u32Operation, "%x");
    NemuWriteNVRAMU32Param(EFI_VM_VARIABLE_OP_START, u32Operation);
    ...
    NemuLogFlowFuncMarkVar(u32Rc, "%x");
  ...
  )
{
  ...
  return EmuGetVariable (
     ...
     */
    NemuWriteNVRAMGuidParam(VendorGuid);
    NemuWriteNVRAMNameParam(VariableName);
    u32Rc = NemuWriteNVRAMDoOp(EFI_VARIABLE_OP_QUERY);
    if (u32Rc == EFI_VARIABLE_OP_STATUS_OK)
    {
        ...
        NemuLogFlowFuncMarkVar(*DataSize, "%d");
        NemuLogFlowFuncMarkVar(VarLen, "%d");
        if (   VarLen <= *DataSize
            && Data)
        {
            /*
  ...
  )
{
  ...
  return EmuGetNextVariableName (
    ...
    {
        NemuLogFlowFuncLeaveRC(EFI_INVALID_PARAMETER);
     ...
     */
    if (!VariableName[0])
        u32Rc = NemuWriteNVRAMDoOp(EFI_VARIABLE_OP_QUERY_REWIND);
    else
    {
        NemuWriteNVRAMGuidParam(VendorGuid);
        NemuWriteNVRAMNameParam(VariableName);
    ...
    if (u32Rc == EFI_VARIABLE_OP_STATUS_OK)
        u32Rc = NemuWriteNVRAMDoOp(EFI_VARIABLE_OP_QUERY_NEXT);
    ...
    if (u32Rc == EFI_VARIABLE_OP_STATUS_OK)
    {
        ...
        if ((cwcName + 1) * 2 <= *VariableNameSize) /* ASSUMES byte size is specified */
        {
             ...
             */
            ASMOutU32(EFI_PORT_VARIABLE_OP, EFI_VM_VARIABLE_OP_GUID);
            NemuReadNVRAM((UINT8 *)VendorGuid, sizeof(EFI_GUID));
  ...
  )
{
  ...
  return EmuSetVariable (
    ...
    UINT32 u32Rc;
    NemuLogFlowFuncEnter();
    NemuLogFlowFuncMarkVar(VendorGuid, "%g");
    NemuLogFlowFuncMarkVar(VariableName, "%s");
    NemuLogFlowFuncMarkVar(DataSize, "%d");
    /* set guid */
    NemuWriteNVRAMGuidParam(VendorGuid);
    /* set name */
    NemuWriteNVRAMNameParam(VariableName);
    /* set attribute */
    NemuWriteNVRAMU32Param(EFI_VM_VARIABLE_OP_ATTRIBUTE, Attributes);
    /* set value length */
    NemuWriteNVRAMU32Param(EFI_VM_VARIABLE_OP_VALUE_LENGTH, (UINT32)DataSize);
    /* fill value bytes */
    ASMOutU32(EFI_PORT_VARIABLE_OP, EFI_VM_VARIABLE_OP_VALUE);
    NemuWriteNVRAMByteArrayParam(Data, (UINT32)DataSize);
    /* start fetch operation */
    u32Rc = NemuWriteNVRAMDoOp(EFI_VARIABLE_OP_ADD);
    /* process errors */
    NemuLogFlowFuncLeave();
    switch (u32Rc)
    {
  ...
  /* Self Test */
    {
        ...
        rc = RuntimeServiceGetVariable(
            ...
            &size,
            (void *)szTestVariable);
        NemuLogFlowFuncMarkVar(szTestVariable, "%a");
        ...
        rc = RuntimeServiceGetVariable(
            ...
            &size,
            (void *)szTestVariable);
        NemuLogFlowFuncMarkVar((CHAR16 *)szTestVariable, "%s");
