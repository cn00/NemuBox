...
#endif
#ifdef NEMU_WITH_IEM
//# define NEMU_COMPARE_IEM_AND_EM /* debugging... */
//# define NEMU_SAME_AS_EM
//# define NEMU_COMPARE_IEM_LAST
#endif
...
*   Internal Functions                                                                                                           *
*********************************************************************************************************************************/
#if !defined(NEMU_WITH_IEM) || defined(NEMU_COMPARE_IEM_AND_EM)
DECLINLINE(NEMUSTRICTRC) emInterpretInstructionCPUOuter(PVMCPU pVCpu, PDISCPUSTATE pDis, PCPUMCTXCORE pRegFrame,
...
#if defined(NEMU_COMPARE_IEM_FIRST) || defined(NEMU_COMPARE_IEM_LAST)
...
#if defined(NEMU_WITH_RAW_MODE) && (defined(IN_RC) || defined(IN_RING3))
    /*
 ...
 *
 * @returns Nemu status code, see SELMToFlatEx and EMInterpretDisasOneEx for
 *          details.
 *
 * @param   pVM             The cross context VM structure.
 * @param   pVCpu           The cross context virtual CPU structure.
...
static void emCompareWithIem(PVMCPU pVCpu, PCCPUMCTX pEmCtx, PCCPUMCTX pIemCtx,
                             NEMUSTRICTRC rcEm, NEMUSTRICTRC rcIem,
                             uint32_t cbEm, uint32_t cbIem)
{
    ...
    if (rcEm != rcIem)
        RTLogPrintf(" * rcIem=%Rrc rcEm=%Rrc\n", NEMUSTRICTRC_VAL(rcIem), NEMUSTRICTRC_VAL(rcEm));
    else if (cbEm != cbIem)
 ...
 */
VMM_INT_DECL(NEMUSTRICTRC) EMInterpretInstruction(PVMCPU pVCpu, PCPUMCTXCORE pRegFrame, RTGCPTR pvFault)
{
...
#  ifdef NEMU_COMPARE_IEM_FIRST
    /* IEM */
    NEMUSTRICTRC rcIem = IEMExecOneBypassEx(pVCpu, pRegFrame, NULL);
...
#  endif
    ...
    /* EM */
    RTGCPTR pbCode;
    NEMUSTRICTRC rcEm = SELMToFlatEx(pVCpu, DISSELREG_CS, pRegFrame, pRegFrame->rip, 0, &pbCode);
    if (RT_SUCCESS(rcEm))
    {
        uint32_t     cbOp;
...
#  if defined(NEMU_COMPARE_IEM_LAST) || defined(NEMU_COMPARE_IEM_FIRST)
    emCompareWithIem(pVCpu, &g_EmCtx, &g_IemCtx, rcEm, rcIem, 0, 0);
#  endif

# else
    NEMUSTRICTRC rc = IEMExecOneBypassEx(pVCpu, pRegFrame, NULL);
...
#else
    RTGCPTR pbCode;
    NEMUSTRICTRC rc = SELMToFlatEx(pVCpu, DISSELREG_CS, pRegFrame, pRegFrame->rip, 0, &pbCode);
    if (RT_SUCCESS(rc))
    {
        uint32_t     cbOp;
        PDISCPUSTATE pDis = &pVCpu->em.s.DisState;
 ...
 *          to worry about e.g. invalid modrm combinations (!)
 */
VMM_INT_DECL(NEMUSTRICTRC) EMInterpretInstructionEx(PVMCPU pVCpu, PCPUMCTXCORE pRegFrame, RTGCPTR pvFault, uint32_t *pcbWritten)
{
...
#  ifdef NEMU_COMPARE_IEM_FIRST
    ...
    NEMUSTRICTRC rcIem = IEMExecOneBypassEx(pVCpu, pRegFrame, &cbIemWritten);
    if (RT_UNLIKELY(   rcIem == VERR_IEM_ASPECT_NOT_IMPLEMENTED
                    || rcIem == VERR_IEM_INSTR_NOT_IMPLEMENTED))
        rcIem = VERR_EM_INTERPRETER;
    g_IemCtx = *pCtx;
...
# else
    NEMUSTRICTRC rc = IEMExecOneBypassEx(pVCpu, pRegFrame, pcbWritten);
    if (RT_UNLIKELY(   rc == VERR_IEM_ASPECT_NOT_IMPLEMENTED
                    || rc == VERR_IEM_INSTR_NOT_IMPLEMENTED))
        rc = VERR_EM_INTERPRETER;
# endif
 ...
 */
VMM_INT_DECL(NEMUSTRICTRC) EMInterpretInstructionDisasState(PVMCPU pVCpu, PDISCPUSTATE pDis, PCPUMCTXCORE pRegFrame,
...
#  ifdef NEMU_COMPARE_IEM_FIRST
    NEMUSTRICTRC rcIem = IEMExecOneBypassWithPrefetchedByPC(pVCpu, pRegFrame, pRegFrame->rip, pDis->abInstr, pDis->cbCachedInstr);
    if (RT_UNLIKELY(   rcIem == VERR_IEM_ASPECT_NOT_IMPLEMENTED
                    || rcIem == VERR_IEM_INSTR_NOT_IMPLEMENTED))
        rcIem = VERR_EM_INTERPRETER;
    g_IemCtx = *pCtx;
...
#  endif
    ...
    /* EM */
    uint32_t cbIgnored;
    NEMUSTRICTRC rcEm = emInterpretInstructionCPUOuter(pVCpu, pDis, pRegFrame, pvFault, enmCodeType, &cbIgnored);
    if (RT_SUCCESS(rcEm))
...
# else
    NEMUSTRICTRC rc = IEMExecOneBypassWithPrefetchedByPC(pVCpu, pRegFrame, pRegFrame->rip, pDis->abInstr, pDis->cbCachedInstr);
    if (RT_UNLIKELY(   rc == VERR_IEM_ASPECT_NOT_IMPLEMENTED
                    || rc == VERR_IEM_INSTR_NOT_IMPLEMENTED))
        rc = VERR_EM_INTERPRETER;
# endif
...
#else
    uint32_t cbIgnored;
    NEMUSTRICTRC rc = emInterpretInstructionCPUOuter(pVCpu, pDis, pRegFrame, pvFault, enmCodeType, &cbIgnored);
    if (RT_SUCCESS(rc))
        pRegFrame->rip += pDis->cbInstr; /* Move on to the next instruction. */
    return rc;
#endif
...
VMM_INT_DECL(int) EMInterpretRdtscp(PVM pVM, PVMCPU pVCpu, PCPUMCTX pCtx)
{
    ...
    NEMUSTRICTRC rc2 = CPUMQueryGuestMsr(pVCpu, MSR_K8_TSC_AUX, &pCtx->rcx); Assert(rc2 == VINF_SUCCESS);
 ...
 * MWAIT Emulation.
 */
VMM_INT_DECL(NEMUSTRICTRC) EMInterpretMWait(PVM pVM, PVMCPU pVCpu, PCPUMCTXCORE pRegFrame)
{
    Assert(pRegFrame == CPUMGetGuestCtxCore(pVCpu));
 ...
 *
 */
VMM_INT_DECL(NEMUSTRICTRC) EMInterpretInvlpg(PVM pVM, PVMCPU pVCpu, PCPUMCTXCORE pRegFrame, RTGCPTR pAddrGC)
{
...
#endif
        rc = VINF_SUCCESS;
#if !defined(NEMU_COMPARE_IEM_AND_EM) || !defined(NEMU_COMPARE_IEM_LAST)
...
VMM_INT_DECL(int) EMInterpretRdmsr(PVM pVM, PVMCPU pVCpu, PCPUMCTXCORE pRegFrame)
{
    ...
    uint64_t uValue;
    NEMUSTRICTRC rcStrict = CPUMQueryGuestMsr(pVCpu, pRegFrame->ecx, &uValue);
...
VMM_INT_DECL(int) EMInterpretWrmsr(PVM pVM, PVMCPU pVCpu, PCPUMCTXCORE pRegFrame)
{
    ...
    NEMUSTRICTRC rcStrict = CPUMSetGuestMsr(pVCpu, pRegFrame->ecx, RT_MAKE_U64(pRegFrame->eax, pRegFrame->edx));
...
#ifndef NEMU_COMPARE_IEM_AND_EM
/**
...
#if defined(IN_RC) && defined(NEMU_WITH_RAW_RING1)
 ...
 * INVLPG Emulation.
 */
static NEMUSTRICTRC emInterpretInvlPg(PVM pVM, PVMCPU pVCpu, PDISCPUSTATE pDis, PCPUMCTXCORE pRegFrame, RTGCPTR pvFault, uint32_t *pcbSize)
{
    DISQPVPARAMVAL param1;
    RTGCPTR     addr;
    NOREF(pvFault); NOREF(pVM); NOREF(pcbSize);

    NEMUSTRICTRC rc = DISQueryParamVal(pRegFrame, pDis, &pDis->Param1, &param1, DISQPVWHICH_SRC);
    if(RT_FAILURE(rc))
        return VERR_EM_INTERPRETER;
    ...
    if (    rc == VINF_SUCCESS
        ...
        return VINF_SUCCESS;
    AssertMsgReturn(rc == VINF_EM_RAW_EMULATE_INSTR,
                    ("%Rrc addr=%RGv\n", NEMUSTRICTRC_VAL(rc), addr),
                    VERR_EM_INTERPRETER);
    return rc;
}

/** @todo change all these EMInterpretXXX methods to NEMUSTRICTRC. */
...
static NEMUSTRICTRC emInterpretMWait(PVM pVM, PVMCPU pVCpu, PDISCPUSTATE pDis, PCPUMCTXCORE pRegFrame, RTGCPTR pvFault, uint32_t *pcbSize)
{
    NOREF(pDis); NOREF(pvFault); NOREF(pcbSize);
 ...
 * @param   pVM     The cross context VM structure.
 */
DECLINLINE(NEMUSTRICTRC) emInterpretInstructionCPU(PVM pVM, PVMCPU pVCpu, PDISCPUSTATE pDis, PCPUMCTXCORE pRegFrame,
                                                   RTGCPTR pvFault, EMCODETYPE enmCodeType, uint32_t *pcbSize)
{
    Assert(pRegFrame == CPUMGetGuestCtxCore(pVCpu));
                                                        ...
                                                        RTGCPTR pvFault, EMCODETYPE enmCodeType, uint32_t *pcbSize)
{
    ...
    NEMUSTRICTRC rc = emInterpretInstructionCPU(pVCpu->CTX_SUFF(pVM), pVCpu, pDis, pRegFrame, pvFault, enmCodeType, pcbSize);
