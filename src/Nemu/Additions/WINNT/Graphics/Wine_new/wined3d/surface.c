...
{
    NEMUSHRC_UNLOCK(surf);
    if (NEMUSHRC_IS_LOCKED(surf))
        return;
...
//    DWORD alloc_flag = srgb ? SFLAG_SRGBALLOCATED : SFLAG_ALLOCATED;
    ...
    if(surface->container)
    {
        ...
        pglChromiumParameteriCR(GL_RCUSAGE_TEXTURE_SET_CR, (GLuint)NEMUSHRC_GET_SHAREHANDLE(surface));
        context_release(context);
    ...
    surface->texture_name = (GLuint)NEMUSHRC_GET_SHAREHANDLE(surface);
...
static void surface_bind(struct wined3d_surface *surface, struct wined3d_context *context, BOOL srgb)
{
    ...
    else
    {
        ...
        {
#ifdef NEMU_WITH_WDDM
            if (NEMUSHRC_IS_SHARED_OPENED(surface))
            {
...
#ifdef NEMU_WITH_WDDM
            if (NEMUSHRC_IS_SHARED(surface))
            {
                NEMUSHRC_SET_SHAREHANDLE(surface, surface->texture_name);
            }
#endif
            }
        }
...
#ifndef NEMU_WINE_WITH_SINGLE_CONTEXT
...
void surface_set_texture_name(struct wined3d_surface *surface, GLuint new_name, BOOL srgb)
{
    ...
    if (NEMUSHRC_IS_SHARED(surface))
    {
        Assert(NEMUSHRC_GET_SHAREHANDLE(surface) == NULL
                || (GLuint)NEMUSHRC_GET_SHAREHANDLE(surface) == new_name
                || new_name == 0 /* on cleanup */);
        NEMUSHRC_SET_SHAREHANDLE(surface, new_name);
    }
#endif
    *name = new_name;
    surface_force_reload(surface);
...
#ifdef NEMU_WITH_WINE_FIX_SURFUPDATA
...
#ifdef NEMU_WITH_WINE_FIX_PBOPSM
...
#ifdef NEMU_WITH_WDDM
    if(NEMUSHRC_IS_SHARED_UNLOCKED(surface)) {
        ...
        )
{
    ...
    if (NEMUSHRC_IS_SHARED(surface))
    {
        ...
        if (!NEMUSHRC_IS_SHARED_OPENED(surface))
        {
            surface_shrc_unlock(surface);
            Assert(!(*shared_handle));
            *shared_handle = NEMUSHRC_GET_SHAREHANDLE(surface);
            pglChromiumParameteriCR(GL_PIN_TEXTURE_SET_CR, (GLuint)NEMUSHRC_GET_SHAREHANDLE(surface));
        }
        else
        {
            NEMUSHRC_UNLOCK(surface);
            Assert(!NEMUSHRC_IS_LOCKED(surface));
#ifdef DEBUG_misha
            ERR("test this!");
#endif
            surface_setup_location_onopen(surface);
            Assert(*shared_handle);
            Assert(*shared_handle == NEMUSHRC_GET_SHAREHANDLE(surface));
        }
