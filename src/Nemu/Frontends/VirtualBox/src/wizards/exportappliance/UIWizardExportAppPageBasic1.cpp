...
/** @file
 * Nemu Qt GUI - UIWizardExportAppPageBasic1 class implementation.
 */
...
void UIWizardExportAppPage1::populateVMSelectorItems(const QStringList &selectedVMNames)
{
    ...
    foreach (const CMachine &machine, nemuGlobal().virtualBox().GetMachines())
    {
        QPixmap pixIcon;
        QString strName;
        QString strUuid;
        ...
        {
            pixIcon = nemuGlobal().vmGuestOSTypeIcon(machine.GetOSTypeId()).scaled(iIconMetric, iIconMetric, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
        ...
        else
        {
            ...
            strName = NemuGlobal::hasAllowedExtension(fi.completeSuffix(), NemuFileExts) ? fi.completeBaseName() : fi.fileName();
