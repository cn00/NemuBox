...

                {
                    ...
                    if (pReq)
                    {
                        ...
                        if (RT_SUCCESS(rc))
                        {
                            ...
                            Log(("NemuMouseNT: ERROR querying mouse capabilities from VMMDev. rc = %Rrc\n", rc));
                    }
                }
                QueueInput(pDevObj);
                pDevExt->MouExt.InputState = MouseIdle;
...
#define NAME_MAX 256
    ...
    LogFlow(("NemuMouseNT::DriverEntry: enter\n"));
    ...
    for (unsigned i = 0; i < KEYBOARD_PORTS_MAXIMUM; i++)
    {
        ...
        LogFlow(("NemuMouseNT::DriverEntry: Creating device object named %S\n", KbdNameFull.Buffer));
    ...
    *pDevExt = pInit->DevExt;
    ...
    if (pDevExt->HardwarePresent & MOUSE_HARDWARE_PRESENT)
    {
        ...
        for (unsigned i = 0; i < POINTER_PORTS_MAXIMUM; i++)
        {
            ...
            LogFlow(("NemuMouseNT::DriverEntry: pointer port name (symbolic link) = %S\n", MouNameFull.Buffer));
    ...
    int rcNemu = VbglInitClient();
    if (RT_FAILURE(rcNemu))
    {
        Log(("NemuMouseNT::DriverEntry: could not initialize guest library, rc = %Rrc\n", rcNemu));
        /* Continue working in non-Nemu mode. */
    }
    else
    {
        VMMDevReqMouseStatus *pReq = NULL;

        rcNemu = VbglGRAlloc((VMMDevRequestHeader**)&pReq, sizeof(VMMDevReqMouseStatus), VMMDevReq_SetMouseStatus);
        if (RT_SUCCESS(rcNemu))
        {
            ...
            pReq->pointerXPos = 0;
            pReq->pointerYPos = 0;
            rcNemu = VbglGRPerform(&pReq->header);
            if (RT_FAILURE(rcNemu))
                Log(("NemuMouseNT::DriverEntry: ERROR communicating new mouse capabilities to VMMDev. rc = %Rrc\n", rcNemu));
            else
            {
        ...
        {
            VbglTerminate();
            Log(("NemuMouseNT::DriverEntry: could not allocate request buffer, rc = %Rrc\n", rcNemu));
            /* Continue working in non-Nemu mode. */
        }
    }
...
fail:
    ...
    LogFlow(("NemuMouseNT::DriverEntry: leave, status = %d\n", status));
