 ...
 * - Host:   A host process (e.g. NemuManage or another tool utilizing the Main API)
 *           which wants to control something on the guest.
 * - Client: A client (e.g. NemuService) running inside the guest OS waiting for
...
public:
    ...
    explicit DragAndDropService(PNEMUHGCMSVCHELPERS pHelpers)
        : HGCM::AbstractService<DragAndDropService>(pHelpers)
        , m_pManager(NULL) {}

protected:

    int  init(NEMUHGCMSVCFNTABLE *pTable);
    int  uninit(void);
    int  clientConnect(uint32_t u32ClientID, void *pvClient);
    int  clientDisconnect(uint32_t u32ClientID, void *pvClient);
    void guestCall(NEMUHGCMCALLHANDLE callHandle, uint32_t u32ClientID, void *pvClient, uint32_t u32Function, uint32_t cParms, NEMUHGCMSVCPARM paParms[]);
    int  hostCall(uint32_t u32Function, uint32_t cParms, NEMUHGCMSVCPARM paParms[]);
...
int DragAndDropService::init(NEMUHGCMSVCFNTABLE *pTable)
{
    /* Register functions. */
    ...
    modeSet(NEMU_DRAG_AND_DROP_MODE_OFF);
...
int DragAndDropService::modeSet(uint32_t u32Mode)
{
    ...
    switch (u32Mode)
    {
        case NEMU_DRAG_AND_DROP_MODE_OFF:
        case NEMU_DRAG_AND_DROP_MODE_HOST_TO_GUEST:
        case NEMU_DRAG_AND_DROP_MODE_GUEST_TO_HOST:
        case NEMU_DRAG_AND_DROP_MODE_BIDIRECTIONAL:
            m_u32Mode = u32Mode;
            break;

        default:
            m_u32Mode = NEMU_DRAG_AND_DROP_MODE_OFF;
            break;
    }
...
void DragAndDropService::guestCall(NEMUHGCMCALLHANDLE callHandle, uint32_t u32ClientID,
                                   void *pvClient, uint32_t u32Function,
                                   uint32_t cParms, NEMUHGCMSVCPARM paParms[])
{
    ...
    switch (u32Function)
    {
        ...
        {
            if (modeGet() != NEMU_DRAG_AND_DROP_MODE_OFF)
            {
                rc = VINF_SUCCESS;
            }
            else
        ...
        {
            if (   modeGet() == NEMU_DRAG_AND_DROP_MODE_BIDIRECTIONAL
                || modeGet() == NEMU_DRAG_AND_DROP_MODE_HOST_TO_GUEST)
            {
                rc = VINF_SUCCESS;
            }
            else
        ...
        case DragAndDropSvc::GUEST_DND_GH_EVT_ERROR:
        {
            ...
            if (   modeGet() == NEMU_DRAG_AND_DROP_MODE_BIDIRECTIONAL
                || modeGet() == NEMU_DRAG_AND_DROP_MODE_GUEST_TO_HOST)
            {
                rc = VINF_SUCCESS;
            }
            else
...
#define DO_HOST_CALLBACK();                                                     \
    ...
    if (rc == VINF_SUCCESS) /* Note: rc might be VINF_HGCM_ASYNC_EXECUTE! */
    {
        ...
        {
            /*
             * Note: Older Nemu versions with enabled DnD guest->host support (< 5.0)
            ...
            case DragAndDropSvc::GUEST_DND_GET_NEXT_HOST_MSG:
            {
                ...
                if (   cParms != 3
                    || paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT /* message */
                    || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT /* parameter count */
                    || paParms[2].type != NEMU_HGCM_SVC_PARM_32BIT /* blocking */)
                {
                ...
                else
                {
                    ...
                    if (RT_FAILURE(rc)) /* No queued messages available? */
                    {
                        ...
                        {
                            DragAndDropSvc::NEMUDNDCBHGGETNEXTHOSTMSG data;
            ...
            case DragAndDropSvc::GUEST_DND_CONNECT:
            {
                ...
                if (   cParms != 2
                    || paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT /* protocol version */
                    || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT /* additional connection flags */)
                    rc = VERR_INVALID_PARAMETER;
                else
                {
                    uint32_t uProtocol;
            ...
            case DragAndDropSvc::GUEST_DND_HG_ACK_OP:
            {
                ...
                if (   cParms != 1
                    || paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT /* action */)
                    rc = VERR_INVALID_PARAMETER;
                else
                {
                    DragAndDropSvc::NEMUDNDCBHGACKOPDATA data;
            ...
            case DragAndDropSvc::GUEST_DND_HG_REQ_DATA:
            {
                ...
                if (   cParms != 1
                    || paParms[0].type != NEMU_HGCM_SVC_PARM_PTR /* format */)
                    rc = VERR_INVALID_PARAMETER;
                else
                {
                    DragAndDropSvc::NEMUDNDCBHGREQDATADATA data;
            ...
            case DragAndDropSvc::GUEST_DND_HG_EVT_PROGRESS:
            {
                ...
                if (   cParms != 3
                    || paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT /* status */
                    || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT /* percent */
                    || paParms[2].type != NEMU_HGCM_SVC_PARM_32BIT /* rc */)
                    rc = VERR_INVALID_PARAMETER;
                else
                {
                    DragAndDropSvc::NEMUDNDCBHGEVTPROGRESSDATA data;
            ...
            case DragAndDropSvc::GUEST_DND_GH_ACK_PENDING:
            {
                ...
                if (   cParms != 3
                    || paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT /* defaction */
                    || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT /* alloctions */
                    || paParms[2].type != NEMU_HGCM_SVC_PARM_PTR   /* format */)
                    rc = VERR_INVALID_PARAMETER;
                else
                {
                    DragAndDropSvc::NEMUDNDCBGHACKPENDINGDATA data;
            ...
            case DragAndDropSvc::GUEST_DND_GH_SND_DATA:
            {
                ...
                if (   cParms != 2
                    || paParms[0].type != NEMU_HGCM_SVC_PARM_PTR   /* data */
                    || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT /* size */)
                    rc = VERR_INVALID_PARAMETER;
                else
                {
                    DragAndDropSvc::NEMUDNDCBSNDDATADATA data;
            ...
            case DragAndDropSvc::GUEST_DND_GH_SND_DIR:
            {
                ...
                if (   cParms != 3
                    || paParms[0].type != NEMU_HGCM_SVC_PARM_PTR   /* path */
                    || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT /* path length */
                    || paParms[2].type != NEMU_HGCM_SVC_PARM_32BIT /* creation mode */)
                    rc = VERR_INVALID_PARAMETER;
                else
                {
                    DragAndDropSvc::NEMUDNDCBSNDDIRDATA data;
            ...
            /* Note: Since protocol v2 (>= Nemu 5.0). */
            case DragAndDropSvc::GUEST_DND_GH_SND_FILE_HDR:
            {
                LogFlowFunc(("GUEST_DND_GH_SND_FILE_HDR\n"));
                if (   cParms != 6
                    || paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT /* context ID */
                    || paParms[1].type != NEMU_HGCM_SVC_PARM_PTR   /* file path */
                    || paParms[2].type != NEMU_HGCM_SVC_PARM_32BIT /* file path length  */
                    || paParms[3].type != NEMU_HGCM_SVC_PARM_32BIT /* flags */
                    || paParms[4].type != NEMU_HGCM_SVC_PARM_32BIT /* file mode */
                    || paParms[5].type != NEMU_HGCM_SVC_PARM_64BIT /* file size */)
                    rc = VERR_INVALID_PARAMETER;
                else
                {
                    DragAndDropSvc::NEMUDNDCBSNDFILEHDRDATA data;
            ...
            case DragAndDropSvc::GUEST_DND_GH_SND_FILE_DATA:
            {
                ...
                switch (pClient->protocol())
                {
                    case 2: /* Protocol version 2 only sends the next data chunks to reduce traffic. */
                    {
                        if (   cParms != 3
                            /* paParms[0] is context ID; unused yet. */
                            || paParms[1].type != NEMU_HGCM_SVC_PARM_PTR   /* file data */
                            || paParms[2].type != NEMU_HGCM_SVC_PARM_32BIT /* file data length */)
                        {
                            rc = VERR_INVALID_PARAMETER;
                        }
                        else
                        {
                            DragAndDropSvc::NEMUDNDCBSNDFILEDATADATA data;
                    ...
                    {
                        if (   cParms != 5
                            || paParms[0].type != NEMU_HGCM_SVC_PARM_PTR   /* file path */
                            || paParms[1].type != NEMU_HGCM_SVC_PARM_32BIT /* file path length */
                            || paParms[2].type != NEMU_HGCM_SVC_PARM_PTR   /* file data */
                            || paParms[3].type != NEMU_HGCM_SVC_PARM_32BIT /* file data length */
                            || paParms[4].type != NEMU_HGCM_SVC_PARM_32BIT /* creation mode */)
                        {
                            rc = VERR_INVALID_PARAMETER;
                        }
                        else
            ...
            case DragAndDropSvc::GUEST_DND_GH_EVT_ERROR:
            {
                ...
                if (   cParms != 1
                    || paParms[0].type != NEMU_HGCM_SVC_PARM_32BIT /* rc */)
                    rc = VERR_INVALID_PARAMETER;
                else
                {
                    DragAndDropSvc::NEMUDNDCBEVTERRORDATA data;
            ...
            default:
            {
                ...
                if (rc == VERR_NO_DATA) /* Manager has no new messsages? Try asking the host. */
                {
                    ...
                    {
                        DragAndDropSvc::NEMUDNDCBHGGETNEXTHOSTMSGDATA data;
                                 ...
                                 uint32_t cParms, NEMUHGCMSVCPARM paParms[])
{
    ...
    else if (modeGet() != NEMU_DRAG_AND_DROP_MODE_OFF)
    {
        ...
        else
        {
             ...
             * not running NemuTray/NemuClient.
             */
            rc = VERR_NOT_SUPPORTED;
        }
    }
