/** $Id: NemuClient.cpp $ */
/** @file
 * NemuClient - User specific services, Darwin.
 */
...
static NEMUCLIENTSERVICE    g_aServices[] =
{
    g_ClipboardService
};
...
static int vbclInitLogger(char *szLogFileName)
{
    ...
    rc = RTLogCreateEx(&g_pLogger,
                       ...
                       "NEMUCLIENT_RELEASE_LOG",        /* Base name for the environment variables for this instance */
    ...
    /* Explicitly flush the log in case of NEMUCLIENT_RELEASE_LOG=buffered. */
 ...
 */
void NemuClientVerbose(int iLevel, const char *pszFormat, ...)
{
...
static int vbclStartServices(void)
{
    ...
    if (RT_SUCCESS(rc))
        NemuClientVerbose(1, "Services start completed.\n");
...
static void vbclStopServices(void)
{
    ...
    NemuClientVerbose(1, "Stopping services...\n");
    for (iServiceId = 0; iServiceId < RT_ELEMENTS(g_aServices); iServiceId++)
    {
        NemuClientVerbose(1, "Stopping service: %s\n", g_aServices[iServiceId].pszName);
    ...
    NemuClientVerbose(1, "Services stop completed\n");
}
