...
#ifdef NEMU
nsresult
InitXPCOMNemu_Impl(JNIEnv* env, jobject aNemuBinDirectory)
{
...
{
#ifdef NEMU
  nsresult rv = InitXPCOMNemu_Impl(env, aMozBinDirectory);
  if (NS_SUCCEEDED(rv))
    return nsnull;
#else
  jobject servMan;
...
#if defined(XP_MACOSX) && !defined(NEMU)
  JAWT awt;
