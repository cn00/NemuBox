/* $Id: NemuCpuReport.cpp $ */
/** @file
 * NemuCpuReport - Produces the basis for a CPU DB entry.
 */
 ...
 *
 * @returns Nemu status code (failure if bad MSR behavior).
 * @param   uMsr                The MSR.
...
static int produceCpuReport(void)
{
    ...
    if (g_pReportOut)
    {
        ...
        vbCpuRepPrintf("/* $" "Id" "$ */\n"
                       ...
                       " * Generated at %s by NemuCpuReport v%sr%s on %s.%s.\n"
                       " */\n"
                       "\n"
                       "/*\n"
                       " * Copyright (C) 2013-2015 Oracle Corporation\n"
                       ...
                       " */\n"
                       "\n"
                       "#ifndef NEMU_CPUDB_%s\n"
                       "#define NEMU_CPUDB_%s\n"
                       "\n",
                       pszName,
...
int main(int argc, char **argv)
{
    ...
    while ((iOpt = RTGetOpt(&State, &ValueUnion)) != 0)
    {
        switch (iOpt)
        {
            ...
            case 'h':
                RTPrintf("Usage: NemuCpuReport [-m|--msrs-only] [-d|--msrs-dev] [-h|--help] [-V|--version] [-o filename.h] [-l debug.log]\n");
