/* $Id: MachineImpl.cpp $ */
/** @file
 * Implementation of IMachine in NemuSVC.
 */
...
#include "Nemu/com/MultiResult.h"

#include <algorithm>

#ifdef NEMU_WITH_DTRACE_R3_MAIN
# include "dtrace/NemuAPI.h"
#endif
...
Machine::HWData::HWData()
{
    ...
    mGraphicsControllerType = GraphicsControllerType_NemuVGA;
    mVRAMSize = 8;
                             ...
                             LockType_T aLockType)
{
    ...
    if (state != SessionState_Unlocked)
        return setError(NEMU_E_INVALID_OBJECT_STATE,
       ...
       )
    {
        ...
        // 1) NemuSVC (where this code runs);
        ...
        if (mData->mSession.mState != SessionState_Locked)
        {
            ...
            return setError(NEMU_E_INVALID_SESSION_STATE,
    ...
    else
    {
...
#if defined(NEMU_WITH_HARDENING) && defined(RT_OS_WINDOWS)
        ...
        if (SUCCEEDED(rc))
        {
...
#ifndef NEMU_WITH_GENERIC_SESSION_WATCHER
...
#else /* NEMU_WITH_GENERIC_SESSION_WATCHER */
            ...
            if (!pToken.isNull())
                pToken->Release();
#endif /* NEMU_WITH_GENERIC_SESSION_WATCHER */
                              ...
                              const ComPtr<IMedium> &aMedium)
{
    ...
    strReconfig = i_getExtraData(Utf8Str("NemuInternal2/SilentReconfigureWhilePaused"));
                                        ...
                                        aDevice)))
    {
        ...
        if (pMedium)
        {
            ...
            return setError(NEMU_E_OBJECT_IN_USE,
                              ...
                              LONG aDevice)
{
    ...
    MediumAttachment *pAttach = i_findAttachment(mMediaData->mAttachments,
                                                 ...
                                                 aDevice);
    if (!pAttach)
        return setError(NEMU_E_OBJECT_NOT_FOUND,
    ...
    if (fHotplug && !pAttach->i_getHotPluggable())
        return setError(NEMU_E_NOT_SUPPORTED,
...
HRESULT Machine::getNetworkAdapter(ULONG aSlot, ComPtr<INetworkAdapter> &aAdapter)
{
    /* Do not assert if slot is out of range, just return the advertised
       status.  testdriver/nemu.py triggers this in logVmInfo. */
...
HRESULT Machine::setExtraData(const com::Utf8Str &aKey, const com::Utf8Str &aValue)
{
    ...
    if ((fChanged = (strOldValue != aValue)))
    {
        ...
        if (fNeedsGlobalSaveSettings)
        {
            ...
            alock.release();
            AutoWriteLock nemulock(mParent COMMA_LOCKVAL_SRC_POS);
...
void Machine::i_deleteConfigHandler(DeleteConfigTask &task)
{
    ...
    try
    {
        ...
        if (mData->pMachineConfigFile->fileExists())
        {
            ...
            {
                /* Delete all Nemu.log[.N] files from the Logs folder
                 * (this must be in sync with the rotation logic in
                 * Console::powerUpThread()). Also, delete the Nemu.png[.N]
                 * files that may have been created by the GUI. */
                Utf8Str log = Utf8StrFmt("%s%cNemu.log",
                                         logFolder.c_str(), RTPATH_DELIMITER);
                RTFileDelete(log.c_str());
                log = Utf8StrFmt("%s%cNemu.png",
                                 logFolder.c_str(), RTPATH_DELIMITER);
                RTFileDelete(log.c_str());
                for (int i = uLogHistoryCount; i > 0; i--)
                {
                    log = Utf8StrFmt("%s%cNemu.log.%d",
                                     logFolder.c_str(), RTPATH_DELIMITER, i);
                    RTFileDelete(log.c_str());
                    log = Utf8StrFmt("%s%cNemu.png.%d",
                                     logFolder.c_str(), RTPATH_DELIMITER, i);
                    RTFileDelete(log.c_str());
                }
#if defined(RT_OS_WINDOWS)
                log = Utf8StrFmt("%s%cNemuStartup.log", logFolder.c_str(), RTPATH_DELIMITER);
                RTFileDelete(log.c_str());
                log = Utf8StrFmt("%s%cNemuHardening.log", logFolder.c_str(), RTPATH_DELIMITER);
                RTFileDelete(log.c_str());
#endif
...
#ifdef NEMU_WITH_GUEST_PROPS
/**
 * Look up a guest property in NemuSVC's internal structures.
 */
...
#ifdef NEMU_WITH_GUEST_PROPS
/**
 * Set a guest property in NemuSVC's internal structures.
 */
...
#ifdef NEMU_WITH_GUEST_PROPS
/**
 * Enumerate the guest properties in NemuSVC's internal structures.
 */
                                           ...
                                           BOOL  *aEnabled)
{
    ...
    if (RT_FAILURE(vrc))
    {
        ...
        *aEnabled = TRUE;
#endif
        return setError(NEMU_E_IPRT_ERROR,
...
HRESULT Machine::hotPlugCPU(ULONG aCpu)
{
    ...
    if (mHWData->mCPUAttached[aCpu])
        return setError(NEMU_E_OBJECT_IN_USE, tr("CPU %lu is already attached"), aCpu);
...
HRESULT Machine::hotUnplugCPU(ULONG aCpu)
{
    ...
    if (!mHWData->mCPUAttached[aCpu])
        return setError(NEMU_E_OBJECT_NOT_FOUND, tr("CPU %lu is not attached"), aCpu);
...
HRESULT Machine::detachHostPCIDevice(LONG aHostAddress)
{
    ...
    return fRemoved ? S_OK : setError(NEMU_E_OBJECT_NOT_FOUND,
...
HRESULT Machine::setAutostartEnabled(BOOL aAutostartEnabled)
{
        ...
        && mHWData->mAutostart.fAutostartEnabled != !!aAutostartEnabled)
    {
        ...
        if (RT_SUCCESS(vrc))
        {
            ...
            hrc = setError(NEMU_E_NOT_SUPPORTED,
...
void Machine::i_getLogFolder(Utf8Str &aLogFolder)
{
    ...
    int vrc = RTEnvGetEx(RTENV_DEFAULT, "NEMU_USER_VMLOGDIR", szTmp, sizeof(szTmp), NULL);
    if (RT_SUCCESS(vrc))
    {
    ...
    if (RT_FAILURE(vrc))
    {
        // fallback if NEMU_USER_LOGHOME is not set or invalid
        aLogFolder = mData->m_strConfigFileFull;    // path/to/machinesfolder/vmname/vmname.nemu
...
Utf8Str Machine::i_getLogFilename(ULONG idx)
{
    ...
    Utf8Str log;
    if (idx == 0)
        log = Utf8StrFmt("%s%cNemu.log", logFolder.c_str(), RTPATH_DELIMITER);
#if defined(RT_OS_WINDOWS) && defined(NEMU_WITH_HARDENING)
    else if (idx == 1)
        log = Utf8StrFmt("%s%cNemuHardening.log", logFolder.c_str(), RTPATH_DELIMITER);
    else
        log = Utf8StrFmt("%s%cNemu.log.%u", logFolder.c_str(), RTPATH_DELIMITER, idx - 1);
#else
    else
        log = Utf8StrFmt("%s%cNemu.log.%u", logFolder.c_str(), RTPATH_DELIMITER, idx);
#endif
    return log;
}
...
Utf8Str Machine::i_getHardeningLogFilename(void)
{
    ...
    strFilename.append(RTPATH_SLASH_STR "NemuHardening.log");
    return strFilename;
}
...
void Machine::i_getDefaultVideoCaptureFile(Utf8Str &strFile)
{
    ...
    strFile = mData->m_strConfigFileFull;       // path/to/machinesfolder/vmname/vmname.nemu
                                   ...
                                   ProgressProxy *aProgress)
{
    ...
    Utf8Str strSupHardeningLogArg("--sup-hardening-log=");
    {
        ...
        strOldStartupLogFile.append(RTPATH_SLASH_STR "NemuStartup.log");
...
#  define OSX_APP_PATH_FMT "/Resources/%s.app/Contents/MacOS/VirtualBoxVM"
        ...
        Utf8Str strAppOverride = i_getExtraData(Utf8Str("NemuInternal2/VirtualBoxVMAppOverride"));
...
# else
        static const char s_szVirtualBox_exe[] = "Nemu" HOSTSUFF_EXE;
...
# endif
        ...
        || !strFrontend.compare("GUI/SDL/separate", Utf8Str::CaseInsensitive))
    {
        ...
        static const char s_szNemuSDL_exe[] = "NemuSDL" HOSTSUFF_EXE;
        Assert(cchBufLeft >= sizeof(s_szNemuSDL_exe));
        strcpy(pszNamePart, s_szNemuSDL_exe);
...
#else /* !NEMU_WITH_NEMUSDL */
    if (0)
        ;
#endif /* !NEMU_WITH_NEMUSDL */
       ...
       )
    {
        ...
        /* On pre-4.0 the "headless" type was used for passing "--vrdp off" to NemuHeadless to let it work in OSE,
         * which did not contain VRDP server. In Nemu 4.0 the remote desktop server (VRDE) is optional,
         * and a VM works even if the server has not been installed.
         * So in 4.0 the "headless" behavior remains the same for default Nemu installations.
         * Only if a VRDE has been installed and the VM enables it, the "headless" will work
         * differently in 4.0 and 3.x.
         */
        static const char s_szNemuHeadless_exe[] = "NemuHeadless" HOSTSUFF_EXE;
        Assert(cchBufLeft >= sizeof(s_szNemuHeadless_exe));
        strcpy(pszNamePart, s_szNemuHeadless_exe);
...
#else /* !NEMU_WITH_HEADLESS */
    if (0)
        ;
#endif /* !NEMU_WITH_HEADLESS */
    else
    {
        RTEnvDestroy(env);
        return setError(E_INVALIDARG,
...
#ifdef NEMU_WITH_DTRACE_R3_MAIN
        NEMUAPI_MACHINE_STATE_CHANGED(this, aMachineState, enmOldState, mData->mUuid.toStringCurly().c_str());
#endif
 ...
 *                          not found
 *  @return
 *      S_OK when found or NEMU_E_OBJECT_NOT_FOUND when not found
 *
 *  @note
...
{
    HRESULT rc = NEMU_E_OBJECT_NOT_FOUND;
                                      ...
                                      bool aSetError /* = false */)
{
    ...
    if (!mData->mFirstSnapshot)
    {
        ...
        return NEMU_E_OBJECT_NOT_FOUND;
    }
...
HRESULT Machine::i_prepareSaveSettings(bool *pfNeedsGlobalSaveSettings)
{
       ...
       )
    {
        ...
        do
        {
            ...
            newConfigFile = Utf8StrFmt("%s%c%s.nemu",
    ...
    for (MediaData::AttachmentList::const_iterator it = oldAtts.begin(); it != oldAtts.end(); ++it)
    {
        ...
        if (pAttach->i_getType() == DeviceType_HardDisk && pMedium)
        {
            ...
            if (aOnline)
            {
                ...
                MediumLockList *pMediumLockList;
                rc = mData->mSession.mLockedMedia.Get(pAttach, pMediumLockList);
                if (RT_UNLIKELY(rc == NEMU_E_INVALID_OBJECT_STATE))
                {
...
{
    Utf8Str strMachineDirName(mData->m_strConfigFileFull);  // path/to/machinesfolder/vmname/vmname.nemu
    ...
    Utf8Str strConfigFileOnly(mData->m_strConfigFileFull);  // path/to/machinesfolder/vmname/vmname.nemu
    strConfigFileOnly.stripPath()                           // vmname.nemu
                     .stripSuffix();                        // vmname
    /** @todo hack, make somehow use of ComposeMachineFilename */
    if (mUserData->s.fDirectoryIncludesUUID)
...
#endif /* NEMU_WITH_RESOURCE_USAGE_API */
...
HRESULT SessionMachine::beginPowerUp(const ComPtr<IProgress> &aProgress)
{
    ...
    if (mData->mSession.mState != SessionState_Locked)
        return NEMU_E_INVALID_OBJECT_STATE;
