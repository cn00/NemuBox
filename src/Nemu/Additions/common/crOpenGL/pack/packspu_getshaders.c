...
/** @file
 * Nemu OpenGL GLSL related functions
 */
...
void PACKSPU_APIENTRY packspu_GetAttachedObjectsARB(NemuGLhandleARB containerObj, GLsizei maxCount, GLsizei * count, NemuGLhandleARB * obj)
{
    GET_THREAD(thread);
    int writeback = 1;
    GLsizei *pLocal;
    ...
    if (count) *count=*pLocal;
    crMemcpy(obj, &pLocal[1], *pLocal*sizeof(NemuGLhandleARB));
    crFree(pLocal);
}

AssertCompile(sizeof(GLsizei) == 4);

void PACKSPU_APIENTRY packspu_GetInfoLogARB(NemuGLhandleARB obj, GLsizei maxLength, GLsizei * length, GLcharARB * infoLog)
{
    GET_THREAD(thread);
    int writeback = 1;
    GLsizei *pLocal;
