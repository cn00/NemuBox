/* $Id: NemuMediaComboBox.cpp $ */
/** @file
 * Nemu Qt GUI - NemuMediaComboBox class implementation.
 */
...
/* GUI includes: */
# include "NemuMediaComboBox.h"
# include "UIMedium.h"

#endif /* !NEMU_WITH_PRECOMPILED_HEADERS */


NemuMediaComboBox::NemuMediaComboBox (QWidget *aParent)
    : QComboBox (aParent)
 ...
 */
void NemuMediaComboBox::refresh()
{
    /* Clearing lists */
    clear(), mMedia.clear();
 ...
 * results.
 */
void NemuMediaComboBox::repopulate()
{
...
QString NemuMediaComboBox::id (int aIndex /* = -1*/) const
{
...
QString NemuMediaComboBox::location (int aIndex /* = -1*/) const
{
    AssertReturn (aIndex == -1 ||
                  (aIndex >= 0 && aIndex < mMedia.size()),
                  QString::null);
...
void NemuMediaComboBox::setCurrentItem (const QString &aId)
{
    mLastId = aId;
...
void NemuMediaComboBox::setType (UIMediumType aType)
{
    mType = aType;
}

void NemuMediaComboBox::setMachineId (const QString &aMachineId)
{
    mMachineId = aMachineId;
}

void NemuMediaComboBox::setNullItemPresent (bool aNullItemPresent)
{
 ...
 */
void NemuMediaComboBox::setShowDiffs (bool aShowDiffs)
{
...
void NemuMediaComboBox::sltHandleMediumCreated(const QString &strMediumID)
{
...
void NemuMediaComboBox::sltHandleMediumEnumerated(const QString &strMediumID)
{
    /* Search for corresponding medium: */
    UIMedium medium = nemuGlobal().medium(strMediumID);
...
void NemuMediaComboBox::sltHandleMediumDeleted(const QString &strMediumID)
{
    /* Search for corresponding item index: */
    int index;
...
void NemuMediaComboBox::sltHandleMediumEnumerationStart()
{
    refresh();
}


void NemuMediaComboBox::processActivated (int aIndex)
{
...
void NemuMediaComboBox::updateToolTip (int aIndex)
{
...
void NemuMediaComboBox::processOnItem (const QModelIndex &aIndex)
{
...
void NemuMediaComboBox::appendItem (const UIMedium &aMedium)
{
...
void NemuMediaComboBox::replaceItem (int aIndex, const UIMedium &aMedium)
{
    AssertReturnVoid (aIndex >= 0 && aIndex < mMedia.size());
 ...
 */
bool NemuMediaComboBox::findMediaIndex (const QString &aId, int &aIndex)
{
    aIndex = 0;
