...
#ifdef NEMU_WITH_WINE_FIX_TEXCLEAR
...
#ifndef NEMU_WITH_WINE_FIX_TEXCLEAR
    wined3d_texture_unload(texture);
#endif
        ...
        struct wined3d_context *context, BOOL srgb, BOOL *set_surface_desc)
{
    ...
    {
#ifdef NEMU_WITH_WDDM
        if (NEMUSHRC_IS_SHARED_OPENED(texture))
        {
            ERR("should not be here!");
            gl_tex->name = (GLuint)NEMUSHRC_GET_SHAREHANDLE(texture);
...
#ifdef NEMU_WITH_WDDM
            if (NEMUSHRC_IS_SHARED(texture))
            {
                NEMUSHRC_SET_SHAREHANDLE(texture, gl_tex->name);
            }
#endif
        }
#ifndef NEMU
        ...
        struct wined3d_context *context, BOOL srgb)
{
    ...
    if (set_gl_texture_desc && SUCCEEDED(hr))
    {
        ...
        {
#ifdef NEMU_WITH_WDDM
            Assert(!NEMUSHRC_IS_SHARED_OPENED(texture));
            if (!NEMUSHRC_IS_SHARED_OPENED(texture))
#endif
            {
            GLenum target = texture->target;
        ...
        )
{
    ...
    if (NEMUSHRC_IS_SHARED(texture))
    {
        ...
        for (i = 0; i < texture->level_count; ++i)
        {
            ...
            {
                UINT idx = j * texture->level_count + i;
                NEMUSHRC_COPY_SHAREDATA(surface_from_resource(texture->sub_resources[idx]), texture);
            }
        }
#ifdef DEBUG
        for (i = 0; i < texture->level_count; ++i)
        ...
        if (!NEMUSHRC_IS_SHARED_OPENED(texture))
        {
            ...
            Assert(!(*shared_handle));
            *shared_handle = NEMUSHRC_GET_SHAREHANDLE(texture);
            pglChromiumParameteriCR(GL_PIN_TEXTURE_SET_CR, (GLuint)NEMUSHRC_GET_SHAREHANDLE(texture));
        }
        else
        {
            struct wined3d_context *context = NULL;
            ...
            texture->texture_rgb.name = (GLuint)NEMUSHRC_GET_SHAREHANDLE(texture);
            gl_tex->states[WINED3DTEXSTA_ADDRESSU] = WINED3D_TADDRESS_WRAP;
            gl_tex->states[WINED3DTEXSTA_ADDRESSV] = WINED3D_TADDRESS_WRAP;
            gl_tex->states[WINED3DTEXSTA_ADDRESSW] = WINED3D_TADDRESS_WRAP;
            gl_tex->states[WINED3DTEXSTA_BORDERCOLOR] = 0;
            ...
            context = context_acquire(device, NULL);
            pglChromiumParameteriCR(GL_RCUSAGE_TEXTURE_SET_CR, (GLuint)NEMUSHRC_GET_SHAREHANDLE(texture));
            ...
            for (i = 0; i < texture->level_count; ++i)
            {
                for (j = 0; j < 6; ++j)
                {
                    ...
                    Assert(*shared_handle);
                    Assert(*shared_handle == NEMUSHRC_GET_SHAREHANDLE(texture));
                }
            }
        }
#ifdef DEBUG
        ...
        pNemuFlushToHost(device->contexts[0]->glCtx);
    }
    else
    {
        Assert(!shared_handle);
        ...
        )
{
    ...
    if (NEMUSHRC_IS_SHARED(texture))
    {
        ...
        for (i = 0; i < texture->level_count; ++i)
        {
            NEMUSHRC_COPY_SHAREDATA(surface_from_resource(texture->sub_resources[i]), texture);
        }
#ifdef DEBUG
        for (i = 0; i < texture->level_count; ++i)
        {
...
HRESULT CDECL wined3d_device_blt_voltex(struct wined3d_device *device, struct wined3d_texture *src, struct wined3d_texture *dst,
        const struct wined3d_box *pSrcBoxArg, const NEMUPOINT3D *pDstPoin3D)
{
    ...
    for (i = 0; i < level_count; ++i)
    {
        ...
        NEMUPOINT3D DstPoint, *pDstPoint;

        if (pSrcBoxArg)
        {
            nemuWddmBoxDivided((NEMUBOX3D*)&SrcBox, (NEMUBOX3D*)pSrcBoxArg, i + 1, true);
            pSrcBox = &SrcBox;
        }
        else
        {
        ...
        if (pDstPoin3D)
        {
            nemuWddmPoint3DDivided(&DstPoint, pDstPoin3D, i + 1, true);
            pDstPoint = &DstPoint;
        }
        else
        {
