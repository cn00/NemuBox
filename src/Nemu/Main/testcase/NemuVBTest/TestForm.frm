...
Private Sub Form_Load()
    ...
    'SetEnvironmentVariable "NEMU_USER_HOME", "E:\VirtualBoxHome\win"
    
    ' Setup debug logging (available only in debug builds)
    
    'PATH_OUT_BASE = "D:/Coding/innotek/nemu/out"
    
    'SetEnvironmentVariable "NEMU_LOG", "main.e.l.f + gui.e.l.f"
    'SetEnvironmentVariable "NEMU_LOG_FLAGS", "time tid thread"
    'SetEnvironmentVariable "NEMU_LOG_DEST", "dir:" + PATH_OUT_BASE + "/logs"
...
Private Sub getMachieListCmd_Click()
    ...
    Dim nemu As VirtualBox.VirtualBox
    Set nemu = New VirtualBox.VirtualBox
    
    ' Get the VirtualBox server version
    
    versionLabel.Caption = nemu.Version
    
    ' Obtain a list of registered machines
    
    Dim machines() As VirtualBox.IMachine
    machines = nemu.Machines2
