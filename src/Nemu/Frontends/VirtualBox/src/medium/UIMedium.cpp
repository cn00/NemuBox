/* $Id: UIMedium.cpp $ */
/** @file
 * Nemu Qt GUI - UIMedium class implementation.
 */
...
void UIMedium::refresh()
{
    ...
    m_strName = NemuGlobal::tr("Empty", "medium");
    ...
    if (!m_medium.isNull())
    {
        ...
        if (!m_fHostDrive)
            ...
            m_strName = NemuGlobal::tr("Host Drive '%1'", "medium").arg(QDir::toNativeSeparators(m_medium.GetLocation()));
        else
            m_strName = NemuGlobal::tr("Host Drive %1 (%2)", "medium").arg(m_medium.GetDescription(), m_medium.GetName());
        ...
        if (!m_fHostDrive)
        {
            ...
            {
                m_strSize = nemuGlobal().formatSize(m_medium.GetSize());
                if (m_type == UIMediumType_HardDisk)
                    m_strLogicalSize = nemuGlobal().formatSize(m_medium.GetLogicalSize());
                else
        ...
        if (m_type == UIMediumType_HardDisk)
        {
            ...
            m_strHardDiskType = nemuGlobal().mediumTypeString(m_medium);
        ...
        if (m_type == UIMediumType_HardDisk)
        {
            m_strToolTip += m_sstrRow.arg(NemuGlobal::tr("<p style=white-space:pre>Type (Format):  %1 (%2)</p>", "medium")
                                                         .arg(m_strHardDiskType).arg(m_strHardDiskFormat));
        }
        m_strToolTip += m_sstrRow.arg(NemuGlobal::tr("<p>Attached to:  %1</p>", "image")
                                                     .arg(m_strUsage.isNull() ? NemuGlobal::tr("<i>Not Attached</i>", "image") : m_strUsage));
        switch (m_state)
        {
            case KMediumState_NotCreated:
            {
                m_strToolTip += m_sstrRow.arg(NemuGlobal::tr("<i>Checking accessibility...</i>", "medium"));
                break;
            }
            case KMediumState_Inaccessible:
            {
                ...
                {
                    /* Not Accessible: */
                    m_strToolTip += m_sstrRow.arg("<hr>") + m_sstrRow.arg(NemuGlobal::highlight(m_strLastAccessError, true /* aToolTip */));
                }
                else
                {
                    /* Accessibility check (eg GetState()) itself failed: */
                    m_strToolTip += m_sstrRow.arg("<hr>") + m_sstrRow.arg(NemuGlobal::tr("Failed to check accessibility of disk image files.", "medium")) +
...
QString UIMedium::toolTip(bool fNoDiffs /* = false */, bool fCheckRO /* = false */, bool fNullAllowed /* = false */) const
{
    ...
    if (m_medium.isNull())
    {
        strTip = fNullAllowed ? m_sstrRow.arg(NemuGlobal::tr("<b>No disk image file selected</b>", "medium")) +
                                m_sstrRow.arg(NemuGlobal::tr("You can also change this while the machine is running.")) :
                                m_sstrRow.arg(NemuGlobal::tr("<b>No disk image files available</b>", "medium")) +
                                m_sstrRow.arg(NemuGlobal::tr("You can create or add disk image files in the virtual machine settings."));
    }
    else
    {
        ...
        if (fCheckRO && m_fReadOnly)
            strTip += m_sstrRow.arg("<hr>") +
                      m_sstrRow.arg(NemuGlobal::tr("Attaching this hard disk will be performed indirectly using "
...
QPixmap UIMedium::icon(bool fNoDiffs /* = false */, bool fCheckRO /* = false */) const
{
    ...
    if (state(fNoDiffs) == KMediumState_Inaccessible)
        pixmap = result(fNoDiffs).isOk() ? nemuGlobal().warningIcon() : nemuGlobal().errorIcon();

    if (fCheckRO && m_fReadOnly)
    {
        QIcon icon = UIIconPool::iconSet(":/hd_new_16px.png");
        pixmap = NemuGlobal::joinPixmaps(pixmap, icon.pixmap(icon.availableSizes().first()));
    }
                          ...
                          bool fUseHTML /* = false */) const
{
    ...
    if (m_type == UIMediumType_HardDisk)
    {
        ...
        if (m_fEncrypted)
            strDetails += QString("%1, ").arg(NemuGlobal::tr("Encrypted", "medium"));
    }
    ...
    {
        case KMediumState_NotCreated:
            strText = NemuGlobal::tr("Checking...", "medium");
            strDetails += fUseHTML ? QString("<i>%1</i>").arg(strText) : strText;
            break;
        case KMediumState_Inaccessible:
            strText = NemuGlobal::tr("Inaccessible", "medium");
    ...
    strDetails = fUseHTML ?
        QString("%1 (<nobr>%2</nobr>)").arg(NemuGlobal::locationForHTML(rootMedium.m_strName), strDetails) :
        QString("%1 (%2)").arg(NemuGlobal::locationForHTML(rootMedium.m_strName), strDetails);
...
{
    /* Redirect call to NemuGlobal: */
    return nemuGlobal().medium(m_strRootId);
}

UIMedium UIMedium::parent() const
{
    /* Redirect call to NemuGlobal: */
    return nemuGlobal().medium(m_strParentId);
}
...
void UIMedium::checkNoDiffs(bool fNoDiffs)
{
    ...
    for (UIMedium parentMedium = parent(); !parentMedium.isNull(); parentMedium = parentMedium.parent())
    {
        if (parentMedium.m_state == KMediumState_Inaccessible)
        {
            ...
            if (m_noDiffs.toolTip.isNull())
                m_noDiffs.toolTip = m_sstrRow.arg(NemuGlobal::tr("Some of the files in this hard disk chain "
    ...
    if (parentID() != nullID() && !m_fReadOnly)
    {
        m_noDiffs.toolTip = root().tip() +
                            m_sstrRow.arg("<hr>") +
                            m_sstrRow.arg(NemuGlobal::tr("This base hard disk is indirectly attached using "
