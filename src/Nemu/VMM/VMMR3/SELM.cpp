...
VMMR3DECL(int) SELMR3Init(PVM pVM)
{
    ...
    /* The I/O bitmap starts right after the virtual interrupt redirection
       ...
       I/O operations. */
    pVM->selm.s.Tss.offIoBitmap = sizeof(NEMUTSS);
...
static void selmR3SetupHyperGDTSelectors(PVM pVM)
{
    ...
    pDesc->Gen.u16LimitLow      = sizeof(NEMUTSS) - 1;
...
/** @todo PGM knows the proper CR3 values these days, not CPUM. */
        ...
        pVM->selm.s.TssTrap08.offIoBitmap = sizeof(NEMUTSS);
 ...
 * @param   pVCpu               The cross context virtual CPU structure of the calling EMT.
 */
static NEMUSTRICTRC selmR3UpdateSegmentRegisters(PVM pVM, PVMCPU pVCpu)
{
 ...
 */
VMMR3DECL(NEMUSTRICTRC) SELMR3UpdateFromCPUM(PVM pVM, PVMCPU pVCpu)
{
     ...
     */
    NEMUSTRICTRC rcStrict = selmR3UpdateSegmentRegisters(pVM, pVCpu);
...
/** @todo use the hidden bits, not shadow GDT. */
    ...
    if (cbMonitoredTss)
    {
        ...
        rc = PGMPhysSimpleReadGCPtr(pVCpu, &Tss, GCPtrTss, RT_OFFSETOF(NEMUTSS, IntRedirBitmap));
        if (    !(cr4 & X86_CR4_VME)
            ||   (  RT_SUCCESS(rc)
                 && Tss.offIoBitmap < sizeof(NEMUTSS) /* too small */
            ...
            cbMonitoredTss = RT_UOFFSETOF(NEMUTSS, padding_ss0);
        else if (RT_SUCCESS(rc))
        {
            /*
        ...
        {
            cbMonitoredTss = RT_OFFSETOF(NEMUTSS, IntRedirBitmap);
            pVM->selm.s.offGuestIoBitmap = 0;
 ...
 * This is a NEMU_STRICT only function.
 *
 * @returns Nemu status code.
 * @param   pVM         The cross context VM structure.
 */
...
{
# if defined(NEMU_STRICT) && defined(SELM_TRACK_GUEST_TSS_CHANGES)
    PVMCPU pVCpu = VMMGetCpu(pVM);
    ...
    if (cbMonitoredTss)
    {
        NEMUTSS Tss;
        uint32_t cr4 = CPUMGetGuestCR4(pVCpu);
        int rc = PGMPhysSimpleReadGCPtr(pVCpu, &Tss, GCPtrTss, RT_OFFSETOF(NEMUTSS, IntRedirBitmap));
 ...
 *
 * @returns Nemu status code, see SELMR3GetSelectorInfo for details.
 *
 * @param   pVCpu       The cross context virtual CPU structure.
 ...
 * @returns Other Nemu status code on other errors.
 *
 * @param   pVM         The cross context VM structure.
 * @param   pVCpu       The cross context virtual CPU structure.
 * @param   Sel         The selector to get info about.
