...
struct _NEMUCLIPBOARDREQFROMNEMU;
typedef struct _NEMUCLIPBOARDREQFROMNEMU NEMUCLIPBOARDREQFROMNEMU;
...
struct _NEMUCLIPBOARDCONTEXT
{
    ...
    /** The currently pending request for data from Nemu.  NULL if there is
    ...
    NEMUCLIPBOARDREQFROMNEMU *pReq;

    /** Pointer to the opaque X11 backend structure */
    CLIPBACKEND *pBackend;
    /** Pointer to the Nemu host client data structure. */
...
/**
 * Report formats available in the X11 clipboard to Nemu.
                                      ...
                                      uint32_t u32Formats)
{
    ...
    nemuSvcClipboardReportMsg(pCtx->pClient,
                              NEMU_SHARED_CLIPBOARD_HOST_MSG_FORMATS,
                              u32Formats);
}
...
int nemuClipboardConnect (NEMUCLIPBOARDCLIENTDATA *pClient, bool fHeadless)
{
    ...
    NEMUCLIPBOARDCONTEXT *pCtx =
        (NEMUCLIPBOARDCONTEXT *) RTMemAllocZ(sizeof(NEMUCLIPBOARDCONTEXT));
    if (!pCtx)
        rc = VERR_NO_MEMORY;
    else
    {
...
int nemuClipboardSync (NEMUCLIPBOARDCLIENTDATA *pClient)
{
    ...
    nemuSvcClipboardReportMsg (pClient,
                               NEMU_SHARED_CLIPBOARD_HOST_MSG_FORMATS, 0);
    return VINF_SUCCESS;
}
...
{
    LogRelFlow(("nemuClipboardDisconnect\n"));

    LogRel(("Stopping the host clipboard service\n"));
    NEMUCLIPBOARDCONTEXT *pCtx = pClient->pCtx;
    ...
    nemuClipboardWriteData(pClient, NULL, 0, 0);
 ...
 * @note  Host glue code
 */
void nemuClipboardFormatAnnounce (NEMUCLIPBOARDCLIENTDATA *pClient,
                                  uint32_t u32Formats)
{
 ...
 *
 */
int nemuClipboardReadData (NEMUCLIPBOARDCLIENTDATA *pClient,
...
/**
 * Complete a request from Nemu for the X11 clipboard data.  The data should
                                    ...
                                    uint32_t cb)
{
    ...
    RTMemFree(pReq);
    nemuSvcClipboardCompleteReadData(pCtx->pClient, rc, cb);
}

/** A request for clipboard data from Nemu */
struct _NEMUCLIPBOARDREQFROMNEMU
{
    /** Data received */
    void *pv;
...
/** Wait for clipboard data requested from Nemu to arrive. */
static int clipWaitForDataFromNemu(NEMUCLIPBOARDCONTEXT *pCtx,
                                   NEMUCLIPBOARDREQFROMNEMU *pReq,
                                   uint32_t u32Format)
{
    int rc = VINF_SUCCESS;
    LogRelFlowFunc(("pCtx=%p, pReq=%p, u32Format=%02X\n", pCtx, pReq, u32Format));
    /* Request data from Nemu */
    nemuSvcClipboardReportMsg(pCtx->pClient,
                              NEMU_SHARED_CLIPBOARD_HOST_MSG_READ_DATA,
                              u32Format);
...
/** Post a request for clipboard data to Nemu/the guest and wait for it to be
 * completed. */
static int clipRequestDataFromNemu(NEMUCLIPBOARDCONTEXT *pCtx,
                                          NEMUCLIPBOARDREQFROMNEMU *pReq,
                                          uint32_t u32Format)
{
    int rc = VINF_SUCCESS;
    ...
    RTCritSectLeave(&pCtx->clipboardMutex);
    if (RT_SUCCESS(rc))
        rc = clipWaitForDataFromNemu(pCtx, pReq, u32Format);
    LogRelFlowFunc(("returning %Rrc\n", rc));
    return rc;
}

/**
 * Send a request to Nemu to transfer the contents of its clipboard to X11.
 *
...
int ClipRequestDataForX11 (NEMUCLIPBOARDCONTEXT *pCtx,
                                   ...
                                   uint32_t *pcb)
{
    NEMUCLIPBOARDREQFROMNEMU request = { NULL, 0, 0, NIL_RTSEMEVENT };
    ...
    if (RT_SUCCESS(rc))
    {
        rc = clipRequestDataFromNemu(pCtx, &request, u32Format);
...
/**
 * Called when we have requested data from Nemu and that data has arrived.
 *
 * @param  pClient   Context information about the guest VM
 ...
 * @note   Host glue code
 */
void nemuClipboardWriteData (NEMUCLIPBOARDCLIENTDATA *pClient,
                             void *pv, uint32_t cb, uint32_t u32Format)
{
     ...
     */
    RTCritSectEnter(&pCtx->clipboardMutex);
    NEMUCLIPBOARDREQFROMNEMU *pReq = pCtx->pReq;
    if (pReq != NULL)
    {
        if (cb > 0)
        {
...
void nemuSvcClipboardReportMsg (NEMUCLIPBOARDCLIENTDATA *pClient, uint32_t u32Msg, uint32_t u32Formats)
{
    ...
    if (   (u32Msg == NEMU_SHARED_CLIPBOARD_HOST_MSG_READ_DATA)
        && !pBackend->writeData.timeout)
        nemuClipboardWriteData(pClient, pBackend->writeData.pv,
...
CLIPBACKEND *ClipConstructX11(NEMUCLIPBOARDCONTEXT *pFrontend, bool)
{
...
int main()
{
    NEMUCLIPBOARDCLIENTDATA client;
    unsigned cErrors = 0;
    int rc = RTR3InitExeNoArguments(0);
    RTPrintf(TEST_NAME ": TESTING\n");
    AssertRCReturn(rc, 1);
    rc = nemuClipboardConnect(&client, false);
    CLIPBACKEND *pBackend = client.pCtx->pBackend;
    AssertRCReturn(rc, 1);
    nemuClipboardFormatAnnounce(&client,
                                NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT);
    if (pBackend->formats != NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT)
    {
        RTPrintf(TEST_NAME ": nemuClipboardFormatAnnounce failed with NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT\n");
        ++cErrors;
    }
    pBackend->readData.rc = VINF_SUCCESS;
    client.asyncRead.callHandle = (NEMUHGCMCALLHANDLE)pBackend;
    client.asyncRead.paParms = (NEMUHGCMSVCPARM *)&client;
    uint32_t u32Dummy;
    rc = nemuClipboardReadData(&client, NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT,
                               &u32Dummy, 42, &u32Dummy);
    if (rc != VINF_HGCM_ASYNC_EXECUTE)
    {
        RTPrintf(TEST_NAME ": nemuClipboardReadData returned %Rrc\n", rc);
        ++cErrors;
    }
    else
    {
    ...
    nemuClipboardWriteData(&client, (void *)"tested", sizeof("tested"), 999);
    nemuClipboardDisconnect(&client);
    if (cErrors > 0)
