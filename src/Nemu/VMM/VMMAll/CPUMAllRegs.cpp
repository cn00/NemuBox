...
#if defined(NEMU_WITH_RAW_MODE) && !defined(IN_RING0)
...
VMMDECL(void) CPUMGetGuestGDTR(PVMCPU pVCpu, PNEMUGDTR pGDTR)
{
 ...
 * combined register set when the Nemu debugger is actually using hardware BPs,
 ...
 *
 * @returns Nemu status code. (recompiler failure)
 * @param   pVCpu       The cross context virtual CPU structure.
 * @see     @ref pg_raw
 */
...
VMMDECL(uint32_t) CPUMGetGuestCPL(PVMCPU pVCpu)
{
     ...
     *         or not.  Nemu or qemu bug must've lead to this misconception.
     *
