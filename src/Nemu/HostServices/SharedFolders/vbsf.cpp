/* $Id: vbsf.cpp $ */
/** @file
 * Shared Folders - Nemu Shared Folders.
 */
 ...
 * @param  pParms->CreateFlags   Creation or open parameters, see include/Nemu/shflsvc.h
 ...
 *                               initial size.
 * @retval pParms->Result        Shared folder status code, see include/Nemu/shflsvc.h
    ...
    if (RT_FAILURE(rc))
    {
        ...
        case VERR_FILE_NOT_FOUND:
            ...
            /* This actually isn't an error, so correct the rc before return later,
               because the driver (NemuSF.sys) expects rc = VINF_SUCCESS and checks the result code. */
            fNoError = true;
            break;
 ...
 *                         these attributes
 * @retval  pParms->Result Shared folder result code, see include/Nemu/shflsvc.h
