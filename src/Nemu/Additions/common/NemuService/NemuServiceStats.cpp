/* $Id: NemuServiceStats.cpp $ */
/** @file
 * NemuStats - Guest statistics notification
 */
...
/** @page pg_vgsvc_vmstats NemuService - VM Statistics
 *
...
*********************************************************************************************************************************/
typedef struct _NEMUSTATSCONTEXT
{
...
*********************************************************************************************************************************/
/** Global data. */
static NEMUSTATSCONTEXT g_VMStat = {0};
...
static void vgsvcVMStatsReport(void)
{
    ...
    req.guestStats.u32StatCaps          = NEMU_GUEST_STAT_PHYS_MEM_TOTAL
                                        | NEMU_GUEST_STAT_PHYS_MEM_AVAIL
                                        | NEMU_GUEST_STAT_PAGE_FILE_SIZE
                                        | NEMU_GUEST_STAT_MEMORY_LOAD;
# ifdef NEMU_WITH_MEMBALLOON
    req.guestStats.u32PhysMemBalloon    = VGSvcBalloonQueryPages(_4K);
    req.guestStats.u32StatCaps         |= NEMU_GUEST_STAT_PHYS_MEM_BALLOON;
# else
...
# endif
    ...
    if (g_VMStat.pfnGetPerformanceInfo)
    {
        ...
        if (g_VMStat.pfnGetPerformanceInfo(&perfInfo, sizeof(perfInfo)))
        {
            ...
            req.guestStats.u32StatCaps |= NEMU_GUEST_STAT_PROCESSES | NEMU_GUEST_STAT_THREADS | NEMU_GUEST_STAT_HANDLES
                                       |  NEMU_GUEST_STAT_MEM_COMMIT_TOTAL | NEMU_GUEST_STAT_MEM_KERNEL_TOTAL
                                       |  NEMU_GUEST_STAT_MEM_KERNEL_PAGED | NEMU_GUEST_STAT_MEM_KERNEL_NONPAGED
                                       |  NEMU_GUEST_STAT_MEM_SYSTEM_CACHE;
        }
        else
        ...
        &&  cbReturned == cbStruct)
    {
        for (uint32_t i = 0; i < systemInfo.dwNumberOfProcessors; i++)
        {
            ...
            req.guestStats.u32StatCaps      |= NEMU_GUEST_STAT_CPU_LOAD_IDLE | NEMU_GUEST_STAT_CPU_LOAD_KERNEL
                                            |  NEMU_GUEST_STAT_CPU_LOAD_USER;
        ...
        for (;;)
        {
                ...
                && RT_C_IS_DIGIT(szLine[3]))
            {
                ...
                if (u32CpuId < VMM_MAX_CPU_COUNT)
                {
                    ...
                    req.guestStats.u32StatCaps |= NEMU_GUEST_STAT_CPU_LOAD_IDLE
                                               |  NEMU_GUEST_STAT_CPU_LOAD_KERNEL
                                               |  NEMU_GUEST_STAT_CPU_LOAD_USER;
                    fCpuInfoAvail = true;
                    rc = VbglR3StatReport(&req);
                    if (RT_SUCCESS(rc))
...
DECLCALLBACK(int) vgsvcVMStatsWorker(bool volatile *pfShutdown)
{
    ...
    VGSvcVerbose(3, "NemuStatsThread: finished statistics change request thread\n");
    return 0;
}
