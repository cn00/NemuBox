...
/** @file
 * Nemu Qt GUI - UIExtraDataManager class implementation.
 */
...
void UIExtraDataEventHandler::sltPreprocessExtraDataCanChange(QString strMachineID, QString strKey, QString strValue, bool &fVeto, QString &strVetoReason)
{
    ...
    if (QUuid(strMachineID).isNull())
    {
        if (strKey.startsWith("GUI/"))
        {
            ...
            NemuGlobalSettings gs(false /* non-null */);
...
void UIExtraDataEventHandler::sltPreprocessExtraDataChange(QString strMachineID, QString strKey, QString strValue)
{
    ...
    if (QUuid(strMachineID).isNull())
    {
        if (strKey.startsWith("GUI/"))
        {
            ...
            m_mutex.lock();
            nemuGlobal().settings().setPublicProperty(strKey, strValue);
            m_mutex.unlock();
            AssertMsgReturnVoid(!!nemuGlobal().settings(), ("Failed to apply global property.\n"));
        }
    }
...
void UIChooserPaneDelegate::fetchPixmapInfo(const QModelIndex &index, QPixmap &pixmap, QSize &pixmapSize)
{
    ...
    if (index.data(Field_ID).toString() != UIExtraDataManager::GlobalID)
        pixmap = nemuGlobal().vmGuestOSTypeIcon(index.data(Field_OsTypeID).toString(), &pixmapSize);
    else
    {
...
//    NemuGlobal::centerWidget(this, pCenterWidget, false);
}
...
void UIExtraDataManagerWindow::sltMachineRegistered(QString strID, bool fRegistered)
{
    ...
    if (fRegistered)
    {
        ...
        /* Get machine items: */
        const CMachineVector machines = nemuGlobal().virtualBox().GetMachines();
...
void UIExtraDataManagerWindow::sltSave()
{
    ...
    const QString strInitialFileName = QDir(nemuGlobal().homeFolder()).absoluteFilePath(QString("%1_ExtraData.xml").arg(currentChooserName()));
...
{
    /* Prepare connections: */
    connect(gNemuEvents, SIGNAL(sigMachineRegistered(QString, bool)),
                         ...
                         pLayout == m_pPaneOfChooser->layout());
        {
            ...
            AssertPtrReturnVoid(m_pViewOfChooser);
            {
                ...
                AssertPtrReturnVoid(m_pModelSourceOfChooser);
                {
                    ...
                    CMachineVector machines = nemuGlobal().virtualBox().GetMachines();
                    foreach (const CMachine &machine, machines)
                                                  ...
                                                  const int iPosition /* = -1 */)
{
    ...
    const CMachine machine = nemu.FindMachine(strID);
    /* Make sure VM is accessible: */
    if (nemu.isOk() && !machine.isNull() && machine.GetAccessible())
...
void UIExtraDataManager::hotloadMachineExtraDataMap(const QString &strID)
{
    ...
    CVirtualBox nemu = nemuGlobal().virtualBox();
    CMachine machine = nemu.FindMachine(strID);
    AssertReturnVoid(nemu.isOk() && !machine.isNull());
...
{
    /* Make sure NemuSVC is available: */
    ...
    if (strID == GlobalID)
    {
        ...
        nemu.SetExtraData(strKey, strValue);
        if (!nemu.isOk())
            msgCenter().cannotSetExtraData(nemu, strKey, strValue);
    }
    /* Machine extra-data: */
    else
    {
        ...
        if (cLevel == ConfigurationAccessLevel_Full)
            session = nemuGlobal().openSession(strID);
        else
            session = nemuGlobal().openExistingSession(strID);
...
void UIExtraDataManager::setExtraDataStringList(const QString &strKey, const QStringList &value, const QString &strID /* = GlobalID */)
{
    ...
    if (strID == GlobalID)
    {
        ...
        CVirtualBox nemu = nemuGlobal().virtualBox();
        /* Update global extra-data: */
        nemu.SetExtraDataStringList(strKey, value);
        if (!nemu.isOk())
            msgCenter().cannotSetExtraData(nemu, strKey, value.join(","));
    }
    /* Machine extra-data: */
    else
    {
...
QRect UIExtraDataManager::selectorWindowGeometry(QWidget *pWidget)
{
    ...
    const QRect availableGeometry = fOk ? nemuGlobal().availableGeometry(QPoint(iX, iY)) :
                                          nemuGlobal().availableGeometry();
...
#ifdef Q_WS_WIN
    /* Get screen-geometry [of screen with point (iX, iY) if possible]: */
    const QRect availableGeometry = nemuGlobal().availableGeometry(QPoint(iX, iY));
...
void UIExtraDataManager::sltExtraDataChange(QString strMachineID, QString strKey, QString strValue)
{
    ...
    {
        /* Current VM only: */
        if (   nemuGlobal().isVMConsoleProcess()
            && strMachineID == nemuGlobal().managedVMUuid())
        {
...
void UIExtraDataManager::prepareGlobalExtraDataMap()
{
    ...
    foreach (const QString &strKey, nemu.GetExtraDataKeys())
        m_data[GlobalID][strKey] = nemu.GetExtraData(strKey);
}
...
void UIExtraDataManager::prepareMainEventListener()
{
    ...
    QVector<KNemuEventType> events;
    events
        << KNemuEventType_OnExtraDataCanChange
        << KNemuEventType_OnExtraDataChanged;
    nemu.GetEventSource().RegisterListener(m_listener, events, TRUE);
...
void UIExtraDataManager::cleanupMainEventListener()
{
    ...
    const CVirtualBox nemu = nemuGlobal().virtualBox();
    nemu.GetEventSource().UnregisterListener(m_listener);
}
