...
windowEvtHndlr(EventHandlerCallRef myHandler, EventRef event, void* userData)
{
    ...
    switch (class)
    {
        case kEventClassNemu:
        {
            switch (kind)
            {
                case kEventNemuUpdateContext:
                {
                        ...
                        context->context)
                    {
                        ...
                        DEBUG_MSG_POETZSCH (("kEventNemuUpdateContext %x %x\n", wi1, context->context));
                        RTSemFastMutexRequest(render_spu.syncMutex);
                        GLboolean result = render_spu.ws.aglSetCurrentContext(context->context);
                        if (result)
                        {
                ...
                case kEventNemuBoundsChanged:
                {
                    ...
                    DEBUG_MSG_POETZSCH (("kEventNemuBoundsChanged %f %f %f %f\n", p.x, p.y, s.width, s.height));
                    GLint l[4] = { 0,
                                   0,
                                   r.size.width,
                                   r.size.height };
...
renderspu_SystemDestroyWindow(WindowInfo *window)
{
    ...
    {
        EventRef evt;
        OSStatus status = CreateEvent(NULL, kEventClassNemu, kEventNemuDisposeWindow, 0, kEventAttributeNone, &evt);
                               ...
                               GLint x, GLint y)
{
     ...
     * thread safe */
    EventRef evt;
    status = CreateEvent(NULL, kEventClassNemu, kEventNemuMoveWindow, 0, kEventAttributeNone, &evt);
...
renderspu_SystemWindowSize(WindowInfo *window, GLint w, GLint h)
{
     ...
     * thread safe */
    EventRef evt;
    status = CreateEvent(NULL, kEventClassNemu, kEventNemuResizeWindow, 0, kEventAttributeNone, &evt);
...
renderspu_SystemShowWindow(WindowInfo *window, GLboolean showIt)
{
    ...
    if(showIt)
    {
         ...
         * aren't thread safe */
        EventRef evt;
        status = CreateEvent(NULL, kEventClassNemu, kEventNemuShowWindow, 0, kEventAttributeNone, &evt);
    ...
    {
        EventRef evt;
        OSStatus status = CreateEvent(NULL, kEventClassNemu, kEventNemuHideWindow, 0, kEventAttributeNone, &evt);
        CHECK_CARBON_RC_RETURN_VOID (status, "Render SPU (renderspu_SystemShowWindow): CreateEvent Failed");
        status = SetEventParameter(evt, kEventParamWindowRef, typeWindowRef, sizeof (window->window), &window->window);
        CHECK_CARBON_RC_RETURN_VOID (status, "Render SPU (renderspu_SystemShowWindow): SetEventParameter Failed");
        status = PostEventToQueue(GetMainEventQueue(), evt, kEventPriorityStandard);
...
void renderspu_SystemNemuPresentComposition( WindowInfo *window, const struct NEMUVR_SCR_COMPOSITOR * pCompositor, const struct NEMUVR_SCR_COMPOSITOR_ENTRY *pChangedEntry )
{
    renderspuNemuPresentCompositionGeneric(window, pCompositor, pChangedEntry, 0, false);
}
...
//    DEBUG_MSG_POETZSCH (("Swapped %d context %x visible: %d\n", window->BltInfo.Base.id, context->context, IsWindowVisible (window->window)));
    ...
    if ((curTS - render_spu.uiDockUpdateTS) > 500)
    {
         ...
         * thread safe */
        EventRef evt;
        status = CreateEvent(NULL, kEventClassNemu, kEventNemuUpdateDock, 0, kEventAttributeNone, &evt);
...
GLboolean
renderspu_SystemNemuCreateWindow(VisualInfo *visual, GLboolean showIt,
                                 WindowInfo *window)
{
    CRASSERT(visual);
    CRASSERT(window);
    ...
    {
        EventRef evt;
        status = CreateEvent(NULL, kEventClassNemu, kEventNemuDisposeWindow, 0, kEventAttributeNone, &evt);
        CHECK_CARBON_RC_RETURN (status, "Render SPU (renderspu_SystemNemuCreateWindow): CreateEvent Failed", false);
        status = SetEventParameter(evt, kEventParamWindowRef, typeWindowRef, sizeof (window->window), &window->window);
        CHECK_CARBON_RC_RETURN (status, "Render SPU (renderspu_SystemNemuCreateWindow): SetEventParameter Failed", false);
        status = PostEventToQueue(GetMainEventQueue(), evt, kEventPriorityStandard);
        CHECK_CARBON_RC_RETURN (status, "Render SPU (renderspu_SystemNemuCreateWindow): PostEventToQueue Failed", false);
    }
    ...
    CHECK_CARBON_RC_RETURN (status, "Render SPU (renderspu_SystemNemuCreateWindow): CreateNewWindow Failed", GL_FALSE);
    ...
    crDebug("Render SPU (renderspu_SystemNemuCreateWindow): actual window (x, y, width, height): %d, %d, %d, %d",
