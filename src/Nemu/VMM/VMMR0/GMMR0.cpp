 ...
 * daemon the will keep NemuVMMR0.r0 in memory and enable the security measures.
 *
 *
 *
...
*********************************************************************************************************************************/
/** @def NEMU_USE_CRIT_SECT_FOR_GIANT
...
# define NEMU_USE_CRIT_SECT_FOR_GIANT
#endif
...
#ifdef NEMU_USE_CRIT_SECT_FOR_GIANT
 ...
 * @param   rc      The return value on failure. Use VERR_GMM_INSTANCE for Nemu
 *                  status codes.
 */
 ...
 * @param   pGMM    The name of the pGMM variable.
 */
#if defined(NEMU_STRICT) && defined(GMMR0_WITH_SANITY_CHECK) && 0
 ...
 *
 * This is called when the NemuVMMR0.r0 module is loaded and protected by the
 * loader semaphore.
 *
 ...
 *
 * @returns Nemu status code (assuming success is ok).
 * @param   pMtxState   Pointer to the chunk mutex state.
 */
 ...
 *
 * @returns Nemu status code.  On success, the giant GMM lock will be held, the
...
GMMR0DECL(int) GMMR0AllocateHandyPages(PVM pVM, VMCPUID idCpu, uint32_t cPagesToUpdate, uint32_t cPagesToAlloc, PGMMPAGEDESC paPages)
{
    ...
    if (GMM_CHECK_SANITY_UPON_ENTERING(pGMM))
    {
                      ...
                      &&  pGVM->gmm.s.Stats.Reserved.cShadowPages))
        {
            ...
            {
#if defined(NEMU_STRICT) && 0 /** @todo re-test this later. Appeared to be a PGM init bug. */
    ...
    for (iPage = 0; iPage < cPages; iPage++)
    {
        ...
        if (RT_LIKELY(pPage))
        {
            ...
            else if (RT_LIKELY(GMM_PAGE_IS_SHARED(pPage)))
            {
                Assert(pGVM->gmm.s.Stats.cSharedPages);
                Assert(pPage->Shared.cRefs);
#if defined(NEMU_WITH_PAGE_SHARING) && defined(NEMU_STRICT) && HC_ARCH_BITS == 64
...
#endif /* NEMU_WITH_PAGE_SHARING || (NEMU_STRICT && 64-BIT) */
 ...
 */
static PGMMSHAREDMODULE gmmR0ShModFindGlobal(PGMM pGMM, uint32_t uHash, uint32_t cbModule, NEMUOSFAMILY enmGuestOS,
 ...
 */
static int gmmR0ShModNewGlobal(PGMM pGMM, uint32_t uHash, uint32_t cbModule, NEMUOSFAMILY enmGuestOS,
                               uint32_t cRegions, const char *pszModuleName, const char *pszVersion,
 ...
 */
GMMR0DECL(int) GMMR0RegisterSharedModule(PVM pVM, VMCPUID idCpu, NEMUOSFAMILY enmGuestOS, char *pszModuleName,
    ...
    if (GMM_CHECK_SANITY_UPON_ENTERING(pGMM))
    {
        ...
        else
        {
            ...
            else
            {
                 ...
                 * by NemuService crashing and being restarted... */
...
#if defined(NEMU_STRICT) && HC_ARCH_BITS == 64
...
#endif /* NEMU_STRICT && HC_ARCH_BITS == 64 */
