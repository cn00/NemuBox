/* $Id: NemuManageGuestCtrl.cpp $ */
/** @file
 * NemuManage - Implementation of guestcontrol command.
 */
 ...
 */
NEMU_LISTENER_DECLARE(GuestFileEventListenerImpl)
NEMU_LISTENER_DECLARE(GuestProcessEventListenerImpl)
NEMU_LISTENER_DECLARE(GuestSessionEventListenerImpl)
NEMU_LISTENER_DECLARE(GuestEventListenerImpl)
 ...
 *
 * @todo Make this handler available for all NemuManage modules?
 */
...
static int gctlPrintError(com::ErrorInfo &errorInfo)
{
    ...
    {
        /* If we got a NEMU_E_IPRT error we handle the error in a more gentle way
         * because it contains more accurate info about what went wrong. */
        if (errorInfo.getResultCode() == NEMU_E_IPRT_ERROR)
...
static RTEXITCODE gctlCtxInitGuestSession(PGCTLCMDCTX pCtx)
{
    ...
    if (RTStrAPrintf(&pszSessionName,
                     "[%RU32] NemuManage Guest Control [%s] - %s",
 ...
 * process to the command line NemuManage was started from.  Useful for e.g.
 * scripting.
 *
 ...
 *
 * We ASSUME that all NemuManage status codes are in the range 0 thru 32.
 *
...
/**
 * Calculates the exit code of NemuManage.
 *
...
static RTEXITCODE gctlHandleRunCommon(PGCTLCMDCTX pCtx, int argc, char **argv, bool fRunCmd, uint32_t fHelp)
{
    ...
    try
    {
        /* Wait for process start in any case. This is useful for scripting NemuManage
                             ...
                             const char *pszSubDir /* For recursion. */)
{
    ...
    if (RT_UNLIKELY(FAILED(rc)))
    {
        switch (rc)
        {
            ...
            case NEMU_E_FILE_ERROR: /* Current entry cannot be accessed to
                                       to missing rights. */
            {
...
static RTEXITCODE gctlHandleCopy(PGCTLCMDCTX pCtx, int argc, char **argv, bool fHostToGuest)
{
     ...
     * what and how to implement the file enumeration/recursive lookup, like NemuManage
     * does in here.
     */
    enum GETOPTDEF_COPY
    {
    ...
    vrc = gctlCopyContextCreate(pCtx, fDryRun, fHostToGuest,
                                  fHostToGuest
                                ? "NemuManage Guest Control - Copy to guest"
                                : "NemuManage Guest Control - Copy from guest", &pContext);
    if (RT_FAILURE(vrc))
    {
 ...
 * while being totaly buggy about the behavior. 'NemuGuest guestcontrol ren dir1 dir2 dstdir' will
...
static DECLCALLBACK(RTEXITCODE) gctlHandleWatch(PGCTLCMDCTX pCtx, int argc, char **argv)
{
    ...
    try
    {
        ...
        do
        {
            ...
            eventTypes.push_back(NemuEventType_OnGuestSessionRegistered);
            /** @todo Also register for NemuEventType_OnGuestUserStateChanged on demand? */
...
RTEXITCODE handleGuestControl(HandlerArg *pArg)
{
    ...
    /*
     * NemuManage guestcontrol [common-options] <VM> [common-options] <sub-command> ...
     *
