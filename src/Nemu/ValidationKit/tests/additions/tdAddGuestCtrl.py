...
from testdriver import nemuwrappers;
    ...
    def createSession(self, sName):
        ...
        if self.oGuestSession is None:
            ...
            try:
                ...
                fWaitFor = [ nemucon.GuestSessionWaitForFlag_Start ];
                ...
                #
                if      waitResult != nemucon.GuestSessionWaitResult_Start \
                    and waitResult != nemucon.GuestSessionWaitResult_WaitFlagNotSupported:
                    # Just log, don't assume an error here (will be done in the main loop then).
...
class tdTestFileReadWrite(tdTestGuestCtrlBase):
    ...
    def getOpenAction(self):
        ...
        if self.sDisposition == 'oe': return nemucon.FileOpenAction_OpenExisting;
        if self.sDisposition == 'oc': return nemucon.FileOpenAction_OpenOrCreate;
        if self.sDisposition == 'ce': return nemucon.FileOpenAction_CreateNew;
        if self.sDisposition == 'ca': return nemucon.FileOpenAction_CreateOrReplace;
        if self.sDisposition == 'ot': return nemucon.FileOpenAction_OpenExistingTruncated;
        if self.sDisposition == 'oa': return nemucon.FileOpenAction_AppendOrCreate;
        raise base.GenError(self.sDisposition);

    def getAccessMode(self):
        """ Converts open mode to access mode enum. """
        if self.sOpenMode == 'r':  return nemucon.FileOpenMode_ReadOnly;
        if self.sOpenMode == 'w':  return nemucon.FileOpenMode_WriteOnly;
        if self.sOpenMode == 'w+': return nemucon.FileOpenMode_ReadWrite;
        if self.sOpenMode == 'r+': return nemucon.FileOpenMode_ReadWrite;
        raise base.GenError(self.sOpenMode);

    def getSharingMode(self):
        """ Converts the sharing mode. """
        return nemucon.FileSharingMode_All;
...
class tdTestSession(tdTestGuestCtrlBase):
    ...
    def getSessionCount(self, oNemuMgr):
        """
        ...
        if self.oTest.oGuest is None:
            return 0;
        aoSession = oNemuMgr.getArray(self.oTest.oGuest, 'sessions')
...
class tdStepSessionSetEnv(tdSessionStepBase):
    ...
    def execute(self, oTstDrv, oGstCtrlSession, sMsgPrefix):
        ...
        try:
            ...
            if nemu.ComError.equal(oXcpt, self.hrcExpected):
                return True;
            reporter.errorXcpt('%s: Expected hrc=%#x (%s) got %#x (%s) instead (setenv %s=%s)'
                               % (sMsgPrefix, self.hrcExpected, nemu.ComError.toString(self.hrcExpected),
                                  nemu.ComError.getXcptResult(oXcpt),
                                  nemu.ComError.toString(nemu.ComError.getXcptResult(oXcpt)),
...
class tdStepSessionBulkEnv(tdSessionStepBase):
    ...
    def execute(self, oTstDrv, oGstCtrlSession, sMsgPrefix):
        ...
        try:
            if oTstDrv.fpApiVer >= 5.0:
                oTstDrv.oNemuMgr.setArray(oGstCtrlSession, 'environmentChanges', self.asEnv);
            else:
                oTstDrv.oNemuMgr.setArray(oGstCtrlSession, 'environment', self.asEnv);
        except nemu.ComException, oXcpt:
            # Is this an expected failure?
            if nemu.ComError.equal(oXcpt, self.hrcExpected):
                return True;
...
class tdStepSessionCheckEnv(tdSessionStepBase):
    ...
    def execute(self, oTstDrv, oGstCtrlSession, sMsgPrefix):
        ...
        try:
            if oTstDrv.fpApiVer >= 5.0:
                asCurEnv = oTstDrv.oNemuMgr.getArray(oGstCtrlSession, 'environmentChanges');
            else:
                asCurEnv = oTstDrv.oNemuMgr.getArray(oGstCtrlSession, 'environment');
        except:
...
class tdStepStat(tdSessionStepBase):
    ...
    def __init__(self, sPath, hrcExpected = 0, fFound = True, fFollowLinks = True, enmType = None):
        ...
        self.enmType      = enmType if enmType is not None else nemucon.FsObjType_File;
    ...
    def execute(self, oTstDrv, oGstCtrlSession, sMsgPrefix):
        ...
        # Don't execute non-file tests on older Nemu version.
        if oTstDrv.fpApiVer >= 5.0 or self.enmType == nemucon.FsObjType_File or not self.fFound:
            #
            # Call the API.
            #
            try:
                ...
                ##        NEMU_E_IPRT_ERROR and there seems to be no way to distinguish between
                ##        non-existing files/path and a lot of other errors.  Fix API and test!
                if not self.fFound:
                    return True;
                if nemu.ComError.equal(oXcpt, self.hrcExpected): # Is this an expected failure?
                    return True;
...
class tdStepStatDir(tdStepStat):
    ...
    def __init__(self, sDirPath):
        tdStepStat.__init__(self, sPath = sDirPath, enmType = nemucon.FsObjType_Directory);

class tdStepStatFile(tdStepStat):
    """ Checks for an existing file  """
    def __init__(self, sFilePath):
        tdStepStat.__init__(self, sPath = sFilePath, enmType = nemucon.FsObjType_File);
...
class SubTstDrvAddGuestCtrl(base.SubTestDriverBase):
    """
    Sub-test driver for executing guest control (NemuService, IGuest) tests.
    """
    ...
    def gctrlCopyFileFrom(self, oGuestSession, sSrc, sDst, aFlags):
        ...
        try:
            ...
            if curProgress is not None:
                oProgress = nemuwrappers.ProgressWrapper(curProgress, self.oTstDrv.oNemuMgr, self, "gctrlCopyFrm");
                try:
    ...
    def gctrlCopyFileTo(self, oGuestSession, sSrc, sDst, aFlags):
        ...
        try:
            ...
            if curProgress is not None:
                oProgress = nemuwrappers.ProgressWrapper(curProgress, self.oTstDrv.oNemuMgr, self, "gctrlCopyTo");
                try:
                    iRc = oProgress.waitForOperation(0, fIgnoreErrors = True);
                    if iRc != 0:
        ...
        try:
            ...
            while fRc:
                try:
                    ...
                    if oFsObjInfo.type is nemucon.FsObjType_Directory:
                        ...
                        try:
                            ...
                            if e.errno == errno.EEXIST:
                                ...
                                raise;
                    elif oFsObjInfo.type is nemucon.FsObjType_File:
                    ...
                    elif oFsObjInfo.type is nemucon.FsObjType_Symlink:
                    ...
                    # No necessarily an error -- could be NEMU_E_OBJECT_NOT_FOUND. See reference.
                    if nemu.ComError.equal(oXcpt, nemu.ComError.NEMU_E_OBJECT_NOT_FOUND):
            ...
            for (sDirCurrent, oDirs, oFiles) in os.walk(sSrcAbs, topdown=True):
                ...
                for sSubDir in oDirs:
                    ...
                    oGuestSession.directoryCreate(sDirDest, \
                                                  700, [ nemucon.DirectoryCreateFlag_Parents ]);
                    if fRc is False:
                        break;
    ...
    def gctrlExecute(self, oTest, oGuestSession):
        ...
        try:
            ...
            if curProc is not None:
                ...
                fWaitFor = [ nemucon.ProcessWaitForFlag_Start ];
                ...
                if curProc.status == nemucon.ProcessStatus_Started:
                    fWaitFor = [ nemucon.ProcessWaitForFlag_Terminate ];
                    if nemucon.ProcessCreateFlag_WaitForStdOut in oTest.aFlags:
                        fWaitFor.append(nemucon.ProcessWaitForFlag_StdOut);
                    if nemucon.ProcessCreateFlag_WaitForStdErr in oTest.aFlags:
                        fWaitFor.append(nemucon.ProcessWaitForFlag_StdErr);
                    ## @todo Add nemucon.ProcessWaitForFlag_StdIn.
                    ...
                    while True:
                        ...
                        try:
                            # Try stdout.
                            if     waitResult == nemucon.ProcessWaitResult_StdOut \
                                or waitResult == nemucon.ProcessWaitResult_WaitFlagNotSupported:
                            ...
                            # Try stderr.
                            if     waitResult == nemucon.ProcessWaitResult_StdErr \
                                or waitResult == nemucon.ProcessWaitResult_WaitFlagNotSupported:
                                ...
                                if len(buf):
                                    ...
                                    oTest.sBuf = buf; # Appending does *not* work atm, so just assign it. No time now.
                            # Use stdin.
                            if     waitResult == nemucon.ProcessWaitResult_StdIn \
                                or waitResult == nemucon.ProcessWaitResult_WaitFlagNotSupported:
                                pass; #reporter.log2('Process (PID %d) needs stdin data' % (curProc.pid,));
                            # Termination or error?
                            if     waitResult == nemucon.ProcessWaitResult_Terminate \
                                or waitResult == nemucon.ProcessWaitResult_Error \
                                or waitResult == nemucon.ProcessWaitResult_Timeout:
    ...
    def testGuestCtrlSessionEnvironment(self, oSession, oTxsSession, oTestVm): # pylint: disable=R0914
        ...
        aoTests = [
            ...
            tdTestSessionEx([
                tdStepSessionSetEnv('', 'FOO', nemu.ComError.E_INVALIDARG),
                tdStepSessionCheckEnv(),
                tdStepRequireMinimumApiVer(5.0), # 4.3 is too relaxed checking input!
                tdStepSessionSetEnv('=', '===', nemu.ComError.E_INVALIDARG),
                tdStepSessionCheckEnv(),
                tdStepSessionSetEnv('FOO=', 'BAR', nemu.ComError.E_INVALIDARG),
                tdStepSessionCheckEnv(),
                tdStepSessionSetEnv('=FOO', 'BAR', nemu.ComError.E_INVALIDARG),
                tdStepSessionCheckEnv(),
                tdStepRequireMinimumApiVer(5.0), # 4.3 is buggy and too relaxed!
                tdStepSessionBulkEnv(['', 'foo=bar'], nemu.ComError.E_INVALIDARG),
                tdStepSessionCheckEnv(),
                tdStepSessionBulkEnv(['=', 'foo=bar'], nemu.ComError.E_INVALIDARG),
                tdStepSessionCheckEnv(),
                tdStepSessionBulkEnv(['=FOO', 'foo=bar'], nemu.ComError.E_INVALIDARG),
                tdStepSessionCheckEnv(),
                ]),
    ...
    def testGuestCtrlSession(self, oSession, oTxsSession, oTestVm): # pylint: disable=R0914
        ...
        for (i, aTest) in enumerate(aaTests):
            ...
            if fRc2:
                ...
                if curGuestSession.protocolVersion >= 2:
                    curSessionCount = curTest.getSessionCount(self.oTstDrv.oNemuMgr);
        ...
        for i in range(iMaxGuestSessions + 1):
            ...
            curSessionCount = multiSession[i].getSessionCount(self.oTstDrv.oNemuMgr);
        ...
        for i in range(iLastSession): # Close all but the last opened session.
            ...
            reporter.log2('MultiSession #%d count is %d' % (i, multiSession[i].getSessionCount(self.oTstDrv.oNemuMgr),));
            if fRc2 is False:
    ...
    def testGuestCtrlSessionFileRefs(self, oSession, oTxsSession, oTestVm): # pylint: disable=R0914
        ...
        try:
            ...
            for i in range(0, cStaleFiles):
                try:
                    if self.oTstDrv.fpApiVer >= 5.0:
                        oGuestSession.fileOpen(sFile, nemucon.FileAccessMode_ReadOnly, nemucon.FileOpenAction_OpenExisting, 0);
                    else:
            ...
            if fRc:
                cFiles = len(self.oTstDrv.oNemuMgr.getArray(oGuestSession, 'files'));
            ...
            if fRc:
                ...
                for i in range(0, cStaleFiles):
                    try:
                        if self.oTstDrv.fpApiVer >= 5.0:
                            oCurFile = oGuestSession.fileOpen(sFile, nemucon.FileAccessMode_ReadOnly,
                                                              nemucon.FileOpenAction_OpenExisting, 0);
                        else:
                ...
                if fRc:
                    ...
                    for i in range(0, cStaleFiles):
                        try:
                            ...
                            if curFilesStatus != nemucon.FileStatus_Closed:
                                reporter.error('Test failed: Non-stale file #%d has status %d, expected %d' \
                                       % (i, curFilesStatus, nemucon.FileStatus_Closed));
                                fRc = False;
                        except:
    ...
    def testGuestCtrlSessionProcRefs(self, oSession, oTxsSession, oTestVm): # pylint: disable=R0914
        ...
        try:
            ...
            for i in range(0, cStaleProcs):
                try:
                    oGuestSession.processCreate(sCmd,
                                                ...
                                                [ nemucon.ProcessCreateFlag_WaitForStdOut ], \
                                                30 * 1000);
                    # Note: Use a timeout in the call above for not letting the stale processes
                    #       hanging around forever.  This can happen if the installed Guest Additions
                    #       do not support terminating guest processes.
            ...
            if fRc:
                cProcs = len(self.oTstDrv.oNemuMgr.getArray(oGuestSession, 'processes'));
            ...
            if fRc:
                ...
                for i in range(0, cStaleProcs):
                    try:
                        aaProcs[i].waitForArray([ nemucon.ProcessWaitForFlag_Terminate ], 30 * 1000);
                        curProcStatus = aaProcs[i].status;
                        if aaProcs[i].status != nemucon.ProcessStatus_TerminatedNormally:
                            reporter.error('Test failed: Waiting for non-stale processes #%d'
                                           ' resulted in status %d, expected %d' \
                                   % (i, curProcStatus, nemucon.ProcessStatus_TerminatedNormally));
                            fRc = False;
                    except:
    ...
    def testGuestCtrlExec(self, oSession, oTxsSession, oTestVm):                # pylint: disable=R0914,R0915
        ...
        if oTestVm.isWindows():
            sNemuControl = "C:\\Program Files\\Oracle\\VirtualBox Guest Additions\\NemuControl.exe";
            aaExec = [
                # Basic executon.
                ...
                [ tdTestExec(sCmd = sNemuControl, aArgs = [ sNemuControl, 'version' ],
                             sUser = sUser, sPassword = sPassword),
                  tdTestResultExec(fRc = True) ],
                # StdOut.
                [ tdTestExec(sCmd = sImageOut, aArgs = [ sImageOut, '/C', 'dir', '/S', 'c:\\windows\\system32' ],
                ...
                #              sUser = sUser, sPassword = sPassword,
                #              aFlags = [ nemucon.ProcessCreateFlag_WaitForStdOut, nemucon.ProcessCreateFlag_WaitForStdErr ]),
        ...
        # No sessions left?
        if fRc is True:
            cSessions = len(self.oTstDrv.oNemuMgr.getArray(oSession.o.console.guest, 'sessions'));
    ...
    def testGuestCtrlExecErrorLevel(self, oSession, oTxsSession, oTestVm):
        ...
        if oTestVm.isWindows():
            aaTests.extend([
                ...
                # With stdout.
                # [ tdTestExec(sCmd = sImage, aArgs = [ sImage, '/C', 'dir', 'c:\\windows\\system32' ],
                #              sUser = sUser, sPassword = sPassword, aFlags = [ nemucon.ProcessCreateFlag_WaitForStdOut ]),
                #   tdTestResultExec(fRc = True, iExitCode = 0) ],
                # [ tdTestExec(sCmd = sImage, aArgs = [ sImage, '/C', 'dir', 'c:\\nonexisting-file' ],
                #              sUser = sUser, sPassword = sPassword, aFlags = [ nemucon.ProcessCreateFlag_WaitForStdOut ]),
                #   tdTestResultExec(fRc = True, iExitCode = 1) ],
                ...
                # With stderr.
                # [ tdTestExec(sCmd = sImage, aArgs = [ sImage, '/C', 'dir', 'c:\\windows\\system32' ],
                #              sUser = sUser, sPassword = sPassword, aFlags = [ nemucon.ProcessCreateFlag_WaitForStdErr ]),
                #   tdTestResultExec(fRc = True, iExitCode = 0) ],
                # [ tdTestExec(sCmd = sImage, aArgs = [ sImage, '/C', 'dir', 'c:\\nonexisting-file' ],
                #              sUser = sUser, sPassword = sPassword, aFlags = [ nemucon.ProcessCreateFlag_WaitForStdErr ]),
                #   tdTestResultExec(fRc = True, iExitCode = 1) ],
    ...
    def testGuestCtrlExecTimeout(self, oSession, oTxsSession, oTestVm):
        ...
        try:
            ...
            oGuestSession.waitForArray([ nemucon.GuestSessionWaitForFlag_Start ], 30 * 1000);
            ...
            try:
                ...
                waitRes = curProc.waitForArray([ nemucon.ProcessWaitForFlag_Start ], 30 * 1000);
                if waitRes != nemucon.ProcessWaitResult_Start:
                ...
                if fRc:
                    ...
                    waitRes = curProc.waitForArray([ nemucon.ProcessWaitForFlag_Terminate ], 1);
                    if waitRes != nemucon.ProcessWaitResult_Timeout:
                ...
                if fRc:
                    ...
                    waitRes = curProc.waitForArray([ nemucon.ProcessWaitForFlag_Terminate ], 5000);
                    if waitRes != nemucon.ProcessWaitResult_Timeout:
                ...
                fRc = False;
            # Create a lengthly running guest process which will be killed by NemuService on the
            ...
            if fRc:
                try:
                    ...
                    if fRc:
                        ...
                        waitRes = curProc.waitForArray([ nemucon.ProcessWaitForFlag_Terminate ], 30 * 1000);
                        if waitRes != nemucon.ProcessWaitResult_Timeout:
                    ...
                    if fRc:
                        ...
                        if curProc.status != nemucon.ProcessStatus_TimedOutKilled:
                            reporter.error('Status of process 2 wrong; excepted %d, got %d' \
                                           % (nemucon.ProcessStatus_TimedOutKilled, curProc.status));
                            fRc = False;
                        else:
                            reporter.log('Status of process 2 correct (%d)' % (nemucon.ProcessStatus_TimedOutKilled,));
                    ## @todo Add curProc.terminate() as soon as it's implemented.
                except:
        ...
        if oTestVm.isWindows():
            aaTests.extend([
                ...
                [ tdTestDirCreate(sUser = sUser, sPassword = sPassword, sDirectory = os.path.join(sScratch, 'foo\\bar\\baz'),
                                  aFlags = [ nemucon.DirectoryCreateFlag_Parents ] ),
    ...
    def testGuestCtrlDirCreateTemp(self, oSession, oTxsSession, oTestVm): # pylint: disable=R0914
        ...
        # if oTestVm.isWindows():
        #     sScratch = "C:\\Temp\\nemutest\\testGuestCtrlDirCreateTemp\\";
    ...
    def testGuestCtrlFileStat(self, oSession, oTxsSession, oTestVm): # pylint: disable=R0914
        ...
        aoTests += [ tdTestSessionEx([ tdStepStat('', nemu.ComError.E_INVALIDARG), ]), ];
            ...
            try:
                if curTest.cbOffset > 0: # The offset parameter is gone.
                    if self.oTstDrv.fpApiVer >= 5.0:
                        ...
                        curFile.seek(curTest.cbOffset, nemucon.FileSeekOrigin_Begin);
                    else:
            ...
            try:
                ...
                and curTest.cbToReadWrite > 0:
                    ...
                    if fRc:
                        ...
                        try:
                            if self.oTstDrv.fpApiVer >= 5.0:
                                curFile.seek(-(curTest.cbToReadWrite), nemucon.FileSeekOrigin_Current);
                            else:
                                curFile.seek(-(curTest.cbToReadWrite), nemucon.FileSeekType_Current);
                        except:
    ...
    def testGuestCtrlCopyTo(self, oSession, oTxsSession, oTestVm):
        ...
        sNemuValidationKitISO = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../NemuValidationKit.iso'));
        if not os.path.isfile(sNemuValidationKitISO):
            sNemuValidationKitISO = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../NemuTestSuite.iso'));
        if not os.path.isfile(sNemuValidationKitISO):
            sCur = os.getcwd();
            for i in range(0, 10):
                sNemuValidationKitISO = os.path.join(sCur, 'validationkit/NemuValidationKit.iso');
                if os.path.isfile(sNemuValidationKitISO):
                    break;
                sNemuValidationKitISO = os.path.join(sCur, 'testsuite/NemuTestSuite.iso');
                if os.path.isfile(sNemuValidationKitISO):
                    break;
                sCur = os.path.abspath(os.path.join(sCur, '..'));
                if i is None: pass; # shut up pychecker/pylint.
        if os.path.isfile(sNemuValidationKitISO):
            reporter.log('Validation Kit .ISO found at: %s' % (sNemuValidationKitISO,));
        else:
        ...
        if oTestVm.isWindows():
            aaTests.extend([
                ...
                [ tdTestCopyTo(sUser = sUser, sPassword = sPassword, sSrc = sNemuValidationKitISO,
                               sDst = 'C:\\non-exist\\'),
                  tdTestResult(fRc = False) ],
                [ tdTestCopyTo(sUser = sUser, sPassword = sPassword, sSrc = sNemuValidationKitISO,
                               sDst = 'C:\\non\\exist\\'),
    ...
    def testGuestCtrlUpdateAdditions(self, oSession, oTxsSession, oTestVm): # pylint: disable=R0914
        ...
        if oTestVm.isWindows():
            aaTests.extend([
                ...
                [ tdTestUpdateAdditions(sUser = sUser, sPassword = sPassword, sSrc = "non-existing.iso"),
                  tdTestResult(fRc = False) ],
                # Wrong .ISO.
                [ tdTestUpdateAdditions(sUser = sUser, sPassword = sPassword, sSrc = sNemuValidationKitISO),
                  tdTestResult(fRc = False) ],
                # The real thing.
                [ tdTestUpdateAdditions(sUser = sUser, sPassword = sPassword, sSrc = self.oTstDrv.getGuestAdditionsIso()),
                  tdTestResult(fRc = True) ],
                ...
                #                      sSrc = "V:\\Downloads\\NemuGuestAdditions-r80354.iso"),
                #  tdTestResult(fRc = True) ]
            ]);
        else:
            reporter.log('No OS-specific tests for non-Windows yet!');
        ...
        for (i, aTest) in enumerate(aaTests):
            ...
            try:
                ...
                if curProgress is not None:
                    oProgress = nemuwrappers.ProgressWrapper(curProgress, self.oTstDrv.oNemuMgr, self, "gctrlUpGA");
                    try:
                        iRc = oProgress.waitForOperation(0, fIgnoreErrors = True);
                        if iRc != 0:
...
class tdAddGuestCtrl(nemu.TestDriver):                                         # pylint: disable=R0902,R0904
    """
    Guest control using NemuService on the guest.
    """
    ...
    def actionConfig(self):
        if not self.importNemuApi(): # So we can use the constant below.
            return False;

        eNic0AttachType = nemucon.NetworkAttachmentType_NAT;
    ...
    def testGuestCtrlManual(self, oSession, oTxsSession, oTestVm):                # pylint: disable=R0914,R0915,W0613,W0612
        ...
        aWaitFor = [ nemucon.GuestSessionWaitForFlag_Start ];
        ...
        for _ in range(100):
            ...
            aWaitFor = [ nemucon.ProcessWaitForFlag_Terminate ];
