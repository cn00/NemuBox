/* $Id: NemuModBallooning.cpp $ */
/** @file
 * NemuModBallooning - Module for handling the automatic ballooning of VMs.
 */
...
#include "NemuWatchdogInternal.h"
#include <iprt/system.h>

using namespace com;

#define NEMU_MOD_BALLOONING_NAME "balloonctrl"
 ...
 *  "NemuInternal/Guest/BalloonSizeMax" value. */
...
typedef struct NEMUWATCHDOG_BALLOONCTRL_PAYLOAD
{
    /** The maximum ballooning size for the VM.
     *  Specify 0 for ballooning disabled. */
    unsigned long ulBalloonSizeMax;
} NEMUWATCHDOG_BALLOONCTRL_PAYLOAD, *PNEMUWATCHDOG_BALLOONCTRL_PAYLOAD;
...
static unsigned long balloonGetMaxSize(const ComPtr<IMachine> &rptrMachine)
{
     ...
     *  Legacy (NemuBalloonCtrl):
     *  - per-VM parameter ("NemuInternal/Guest/BalloonSizeMax")
     *  Global:
     *  - global parameter ("NemuInternal/Guest/BalloonSizeMax")
     *  New:
     *  - per-VM parameter ("NemuInternal2/Watchdog/BalloonCtrl/BalloonSizeMax")
     *
    ...
    if (!ulBalloonMax)
    {
        int vrc = cfgGetValueULong(g_pVirtualBox, rptrMachine,
                                  "NemuInternal/Guest/BalloonSizeMax", "NemuInternal/Guest/BalloonSizeMax", &ulBalloonMax, 0 /* Ballooning disabled */);
        if (RT_FAILURE(vrc))
        {
            /* Try (new) NemuWatch per-VM approach. */
            Bstr strValue;
            HRESULT rc = rptrMachine->GetExtraData(Bstr("NemuInternal2/Watchdog/BalloonCtrl/BalloonSizeMax").raw(),
                                                   strValue.asOutParam());
            if (   SUCCEEDED(rc)
                && !strValue.isEmpty())
            {
 ...
 */
static bool balloonIsRequired(PNEMUWATCHDOG_MACHINE pMachine)
{
    AssertPtrReturn(pMachine, false);

    /* Only do ballooning if we have a maximum balloon size set. */
    PNEMUWATCHDOG_BALLOONCTRL_PAYLOAD pData = (PNEMUWATCHDOG_BALLOONCTRL_PAYLOAD)
                                              payloadFrom(pMachine, NEMU_MOD_BALLOONING_NAME);
    AssertPtr(pData);
...
int balloonMachineSetup(const Bstr& strUuid)
{
    ...
    do
    {
        PNEMUWATCHDOG_MACHINE pMachine = getMachine(strUuid);
...
#ifdef NEMU_WATCHDOG_GLOBAL_PERFCOL
 ...
 */
static int balloonMachineUpdate(const Bstr &strUuid, PNEMUWATCHDOG_MACHINE pMachine)
{
...
/* Callbacks. */
static DECLCALLBACK(int) NemuModBallooningPreInit(void)
{
    return VINF_SUCCESS;
}

static DECLCALLBACK(int) NemuModBallooningOption(int argc, char *argv[], int *piConsumed)
{
...
static DECLCALLBACK(int) NemuModBallooningInit(void)
{
    if (!g_ulMemoryBalloonTimeoutMS)
        cfgGetValueULong(g_pVirtualBox, NULL /* Machine */,
                         "NemuInternal2/Watchdog/BalloonCtrl/TimeoutMS", NULL /* Per-machine */,
                         &g_ulMemoryBalloonTimeoutMS, 30 * 1000 /* Default is 30 seconds timeout. */);

    if (!g_ulMemoryBalloonIncrementMB)
        cfgGetValueULong(g_pVirtualBox, NULL /* Machine */,
                         "NemuInternal2/Watchdog/BalloonCtrl/BalloonIncrementMB", NULL /* Per-machine */,
                         &g_ulMemoryBalloonIncrementMB, 256);

    if (!g_ulMemoryBalloonDecrementMB)
        cfgGetValueULong(g_pVirtualBox, NULL /* Machine */,
                         "NemuInternal2/Watchdog/BalloonCtrl/BalloonDecrementMB", NULL /* Per-machine */,
                         &g_ulMemoryBalloonDecrementMB, 128);

    if (!g_ulMemoryBalloonLowerLimitMB)
        cfgGetValueULong(g_pVirtualBox, NULL /* Machine */,
                         "NemuInternal2/Watchdog/BalloonCtrl/BalloonLowerLimitMB", NULL /* Per-machine */,
                         &g_ulMemoryBalloonLowerLimitMB, 128);

    return VINF_SUCCESS;
}

static DECLCALLBACK(int) NemuModBallooningMain(void)
{
...
static DECLCALLBACK(int) NemuModBallooningStop(void)
{
    return VINF_SUCCESS;
}

static DECLCALLBACK(void) NemuModBallooningTerm(void)
{
}

static DECLCALLBACK(int) NemuModBallooningOnMachineRegistered(const Bstr &strUuid)
{
    PNEMUWATCHDOG_MACHINE pMachine = getMachine(strUuid);
    AssertPtrReturn(pMachine, VERR_INVALID_PARAMETER);

    PNEMUWATCHDOG_BALLOONCTRL_PAYLOAD pData;
    int rc = payloadAlloc(pMachine, NEMU_MOD_BALLOONING_NAME,
                          sizeof(NEMUWATCHDOG_BALLOONCTRL_PAYLOAD), (void**)&pData);
    if (RT_SUCCESS(rc))
...
static DECLCALLBACK(int) NemuModBallooningOnMachineUnregistered(const Bstr &strUuid)
{
    PNEMUWATCHDOG_MACHINE pMachine = getMachine(strUuid);
    AssertPtrReturn(pMachine, VERR_INVALID_PARAMETER);

    payloadFree(pMachine, NEMU_MOD_BALLOONING_NAME);

    return VINF_SUCCESS;
}

static DECLCALLBACK(int) NemuModBallooningOnMachineStateChanged(const Bstr &strUuid,
...
static DECLCALLBACK(int) NemuModBallooningOnServiceStateChanged(bool fAvailable)
{
    return VINF_SUCCESS;
}
 ...
 */
NEMUMODULE g_ModBallooning =
{
    /* pszName. */
    NEMU_MOD_BALLOONING_NAME,
    /* pszDescription. */
...
#if 1
    /* (Legacy) note. */
    "Set \"NemuInternal/Guest/BalloonSizeMax\" for a per-VM maximum ballooning size.\n"
#endif
    "--balloon-safety-margin Free memory when deflating a balloon in MB (1024 MB).\n"
    ,
    /* methods. */
    NemuModBallooningPreInit,
    NemuModBallooningOption,
    NemuModBallooningInit,
    NemuModBallooningMain,
    NemuModBallooningStop,
    NemuModBallooningTerm,
    /* callbacks. */
    NemuModBallooningOnMachineRegistered,
    NemuModBallooningOnMachineUnregistered,
    NemuModBallooningOnMachineStateChanged,
    NemuModBallooningOnServiceStateChanged
};
