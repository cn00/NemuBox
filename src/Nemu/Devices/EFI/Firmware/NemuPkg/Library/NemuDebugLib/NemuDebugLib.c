/* $Id: NemuDebugLib.c $ */
/** @file
 * NemuDebugLib.c - Debug logging and assertions support routines using DevEFI.
 */
...
DebugPrint(IN UINTN ErrorLevel, IN CONST CHAR8 *Format, ...)
{
    ...
    NemuPrintString("dbg/");
    NemuPrintHex(ErrorLevel, sizeof(ErrorLevel));
    NemuPrintChar(' ');
 ...
 */
VOID EFIAPI
NemuLogWorker(const char *pszFormat, ...)
{
    CHAR8       szBuf[384];
    VA_LIST     va;
    RTCCUINTREG SavedFlags;
 ...
 */
static void
NemuPanicMsgString(const char *pszString)
{
    char ch;
 ...
 */
static void
NemuPanicMsgDecimalU32(uint32_t uValue)
{
    char     szTmp[32];
...
DebugAssert(IN CONST CHAR8 *FileName, IN UINTN LineNumber, IN CONST CHAR8 *Description)
{
    ...
    NemuPanicMsgString("EFI Assertion failed!"
                       "\nFile:  ");
    NemuPanicMsgString(FileName ? FileName : "<NULL>");
    NemuPanicMsgString("\nLine:  ");
    NemuPanicMsgDecimalU32((uint32_t)LineNumber);
    NemuPanicMsgString("\nEDescription: ");
    NemuPanicMsgString(Description ? Description : "<NULL>");
...
CHAR16 *NemuDebugHandleDevicePath2Str(IN EFI_HANDLE hHandle)
{
    ...
    psz16TxtDevicePath = NemuDebugHandleDevicePath2Str(pDevicePath);
    return psz16TxtDevicePath;
#else
    return NULL;
#endif
}
CHAR16 *NemuDebugPrintDevicePath(IN EFI_DEVICE_PATH_PROTOCOL  *pDevicePath)
{
#if 0
    EFI_STATUS rc;
    if (!g_DevPath2Txt)
