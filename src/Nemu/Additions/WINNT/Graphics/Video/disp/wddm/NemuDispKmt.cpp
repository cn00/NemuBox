...
HRESULT nemuDispKmtCallbacksInit(PNEMUDISPKMT_CALLBACKS pCallbacks)
{
    ...
    if (pCallbacks->hGdi32 != NULL)
    {
        ...
        {
            if (bSupportedWin8)
                pCallbacks->enmVersion = NEMUDISPKMT_CALLBACKS_VERSION_WIN8;
            else
                pCallbacks->enmVersion = NEMUDISPKMT_CALLBACKS_VERSION_VISTA_WIN7;
            return S_OK;
        }
        else
        {
...
HRESULT nemuDispKmtAdpHdcCreate(HDC *phDc)
{
    HRESULT hr = E_FAIL;
    DISPLAY_DEVICE DDev;
...
static HRESULT nemuDispKmtOpenAdapterViaHdc(const NEMUDISPKMT_CALLBACKS *pCallbacks, PNEMUDISPKMT_ADAPTER pAdapter)
{
    D3DKMT_OPENADAPTERFROMHDC OpenAdapterData = {0};
    HRESULT hr = nemuDispKmtAdpHdcCreate(&OpenAdapterData.hDc);
    if (!SUCCEEDED(hr))
        return hr;
...
static HRESULT nemuDispKmtOpenAdapterViaLuid(const NEMUDISPKMT_CALLBACKS *pCallbacks, PNEMUDISPKMT_ADAPTER pAdapter)
{
    if (pCallbacks->enmVersion < NEMUDISPKMT_CALLBACKS_VERSION_WIN8)
        return E_NOTIMPL;
...
{
    HRESULT hr = nemuDispKmtOpenAdapterViaHdc(pCallbacks, pAdapter);
    if (SUCCEEDED(hr))
        return S_OK;

    hr = nemuDispKmtOpenAdapterViaLuid(pCallbacks, pAdapter);
    if (SUCCEEDED(hr))
        return S_OK;
                                    ...
                                    HANDLE hEvent, uint64_t u64UmInfo)
{
    ...
    ContextData.NodeOrdinal = NEMUWDDM_NODE_ID_3D_KMT;
    ContextData.EngineAffinity = NEMUWDDM_ENGINE_ID_3D_KMT;
    ContextData.pPrivateDriverData = &Info;
    ContextData.PrivateDriverDataSize = sizeof (Info);
    ContextData.ClientHint = enmType == NEMUWDDM_CONTEXT_TYPE_CUSTOM_UHGSMI_GL ? D3DKMT_CLIENTHINT_OPENGL : D3DKMT_CLIENTHINT_DX9;
