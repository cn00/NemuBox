...
#ifdef NEMU_WITH_LINUX_COMPILER_H
 ...
 *
 * @returns Nemu status code. pu8Port is set on success.
...
#  ifdef UNIT_TEST
    ...
    /** The value of the NEMU_USB environment variable. */
    const char *s_pcszTestEnvUsb;
    /** The value of the NEMU_USB_ROOT environment variable. */
 ...
 * return test values. */
# define RTEnvGet(a) \
    (  !RTStrCmp(a, "NEMU_USB") ? s_pcszTestEnvUsb \
     : !RTStrCmp(a, "NEMU_USB_ROOT") ? s_pcszTestEnvUsbRoot \
     : NULL)
                              ...
                              const char **ppcszDevicesRoot)
{
     ...
     * The user may also specify a method and root using the NEMU_USB and
     * NEMU_USB_ROOT environment variables.  In this case we don't check
     * the root they provide for validity.
     */
    bool fUsbfsChosen = false, fSysfsChosen = false;
    const char *pcszUsbFromEnv = RTEnvGet("NEMU_USB");
    const char *pcszUsbRoot = NULL;
    if (pcszUsbFromEnv)
    {
        bool fValidNemuUSB = true;

        pcszUsbRoot = RTEnvGet("NEMU_USB_ROOT");
        ...
        {
            LogRel(("Invalid NEMU_USB environment variable setting \"%s\"\n",
                    pcszUsbFromEnv));
            fValidNemuUSB = false;
            pcszUsbFromEnv = NULL;
        }
        if (!fValidNemuUSB && pcszUsbRoot)
            pcszUsbRoot = NULL;
    }
    if (!pcszUsbRoot)
    {
        if (   !fUsbfsChosen
            && USBProxyLinuxCheckDeviceRoot("/dev/nemuusb", true))
        {
            fSysfsChosen = true;
            pcszUsbRoot = "/dev/nemuusb";
        }
    ...
    return   pcszUsbFromEnv ? VERR_NOT_FOUND
           : RTDirExists("/dev/nemuusb") ? VERR_VUSB_USB_DEVICE_PERMISSION
