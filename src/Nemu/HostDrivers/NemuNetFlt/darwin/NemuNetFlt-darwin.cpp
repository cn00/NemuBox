/* $Id: NemuNetFlt-darwin.cpp $ */
/** @file
 * NemuNetFlt - Network Filter Driver (Host), Darwin Specific Code.
 */
...
#define NEMUNETFLT_DARWIN_MAX_SEGS      32

#if 0
/** For testing extremely segmented frames. */
#define NEMUNETFLT_DARWIN_TEST_SEG_SIZE 14
#endif

/* XXX: hidden undef #ifdef __APPLE__ */
#define NEMU_IN_LOOPBACK(addr)  (((addr) & IN_CLASSA_NET) == 0x7f000000)
#define NEMU_IN_LINKLOCAL(addr) (((addr) & IN_CLASSB_NET) == 0xa9fe0000)
...
*********************************************************************************************************************************/
RT_C_DECLS_BEGIN
static kern_return_t    NemuNetFltDarwinStart(struct kmod_info *pKModInfo, void *pvData);
static kern_return_t    NemuNetFltDarwinStop(struct kmod_info *pKModInfo, void *pvData);

static void nemuNetFltDarwinSysSockUpcall(socket_t pSysSock, void *pvData, int fWait);
RT_C_DECLS_END
 ...
 */
typedef struct NEMUNETFLTTAG
{
    /** The ethernet header of the outgoing frame. */
    RTNETETHERHDR EthHdr;
} NEMUNETFLTTAG;
/** Pointer to a NemuNetFlt mbuf tag. */
typedef NEMUNETFLTTAG *PNEMUNETFLTTAG;
/** Pointer to a const NemuNetFlt mbuf tag. */
typedef NEMUNETFLTTAG const *PCNEMUNETFLTTAG;
...
KMOD_EXPLICIT_DECL(NemuNetFlt, NEMU_VERSION_STRING, _start, _stop)
DECLHIDDEN(kmod_start_func_t *) _realmain = NemuNetFltDarwinStart;
DECLHIDDEN(kmod_stop_func_t  *) _antimain = NemuNetFltDarwinStop;
...
#define NEMU_GET_PCOUNT(pIfNet) ( *(int *)((uintptr_t)pIfNet + g_offIfNetPCount) )
/** The size of area of ifnet structure we try to locate if_pcount in. */
#define NEMUNETFLT_DARWIN_IFNET_SIZE 256
 ...
 */
static unsigned nemuNetFltDarwinSetAndDiff(ifnet_t pIfNet, int iPromisc)
{
    int aiSavedState[NEMUNETFLT_DARWIN_IFNET_SIZE / sizeof(int)];
 ...
 * @param   pIfNet      The interface we're attaching to.
 */
static void nemuNetFltDarwinDetectPCountOffset(ifnet_t pIfNet)
{
    ...
    {
        offTry1 = nemuNetFltDarwinSetAndDiff(pIfNet, 1);
        offTry2 = nemuNetFltDarwinSetAndDiff(pIfNet, 1);
        offTry3 = nemuNetFltDarwinSetAndDiff(pIfNet, 0);
        offTry4 = nemuNetFltDarwinSetAndDiff(pIfNet, 0);
        if (offTry1 == offTry2 && offTry2 == offTry3 && offTry3 == offTry4)
        {
            if (g_offIfNetPCount != offTry1)
            {
                Log(("NemuNetFltDarwinDetectPCountOffset: Adjusted if_pcount offset to %x from %x.\n", offTry1, g_offIfNetPCount));
    ...
    if (g_offIfNetPCount != offTry1)
        LogRel(("NemuNetFlt: Failed to detect promiscuous count, all traffic may reach wire (%x != %x).\n", g_offIfNetPCount, offTry1));
}
...
static kern_return_t    NemuNetFltDarwinStart(struct kmod_info *pKModInfo, void *pvData)
{
    ...
    if (RT_SUCCESS(rc))
    {
        Log(("NemuNetFltDarwinStart\n"));
        errno_t err = mbuf_tag_id_find("org.VirtualBox.kext.NemuFltDrv", &g_idTag);
        if (!err)
        {
            /*
            ...
            if (RT_SUCCESS(rc))
            {
                LogRel(("NemuFltDrv: version " NEMU_VERSION_STRING " r%d\n", NEMU_SVN_REV));
                return KMOD_RETURN_SUCCESS;
            }

            LogRel(("NemuFltDrv: failed to initialize device extension (rc=%d)\n", rc));
        }
        else
            LogRel(("NemuFltDrv: mbuf_tag_id_find failed, err=%d\n", err));
        RTR0Term();
    }
    else
        printf("NemuFltDrv: failed to initialize IPRT (rc=%d)\n", rc);
...
static kern_return_t NemuNetFltDarwinStop(struct kmod_info *pKModInfo, void *pvData)
{
     ...
     * tracking for us!
     */
    int rc = nemuNetFltTryDeleteIdcAndGlobals(&g_NemuNetFltGlobals);
    if (RT_FAILURE(rc))
    {
        Log(("NemuNetFltDarwinStop - failed, busy.\n"));
        return KMOD_RETURN_FAILURE;
    }
 ...
 * @param   pThis
 */
DECLINLINE(ifnet_t) nemuNetFltDarwinRetainIfNet(PNEMUNETFLTINS pThis)
{
    ifnet_t pIfNet = NULL;
 ...
 * by nemuNetFltDarwinRetainIfNet.
 *
 * @param   pThis           The instance.
 * @param   pIfNet          The nemuNetFltDarwinRetainIfNet return value, NULL is fine.
 */
DECLINLINE(void) nemuNetFltDarwinReleaseIfNet(PNEMUNETFLTINS pThis, ifnet_t pIfNet)
{
    NOREF(pThis);
    if (pIfNet)
...
/**
 * Checks whether this is an mbuf created by nemuNetFltDarwinMBufFromSG,
 ...
 */
DECLINLINE(bool) nemuNetFltDarwinMBufIsOur(PNEMUNETFLTINS pThis, mbuf_t pMBuf, void *pvFrame)
{
    NOREF(pThis);

    /*
     * Lookup the tag set by nemuNetFltDarwinMBufFromSG.
     */
    PCNEMUNETFLTTAG pTagData;
    size_t cbTagData;
 ...
 */
static mbuf_t nemuNetFltDarwinMBufFromSG(PNEMUNETFLTINS pThis, PINTNETSG pSG)
{
    ...
    if (!err)
    {
        ...
        if (!err)
        {
             ...
             */
            PNEMUNETFLTTAG pTagData;
            err = mbuf_tag_allocate(pPkt, g_idTag, 0 /* type */, sizeof(NEMUNETFLTTAG) /* tag len */, How, (void **)&pTagData);
            if (!err)
            {
 ...
 * @param   pvFrame             The frame pointer, optional.
 */
DECLINLINE(unsigned) nemuNetFltDarwinMBufCalcSGSegs(PNEMUNETFLTINS pThis, mbuf_t pMBuf, void *pvFrame)
{
    NOREF(pThis);
...
#ifdef NEMUNETFLT_DARWIN_TEST_SEG_SIZE
    /* maximize the number of segments. */
    cSegs = RT_MAX(NEMUNETFLT_DARWIN_MAX_SEGS - 1, cSegs);
#endif
 ...
 */
DECLINLINE(void) nemuNetFltDarwinMBufToSG(PNEMUNETFLTINS pThis, mbuf_t pMBuf, void *pvFrame, PINTNETSG pSG, unsigned cSegs, uint32_t fSrc)
{
    NOREF(pThis);
    ...
    if (pSG->cSegsUsed < pSG->cSegsAlloc)
    {
               ...
               &&   iDst < pSG->cSegsAlloc)
        {
            ...
            pSG->aSegs[iDst].cb = RT_MIN(pSG->aSegs[iSrc].cb, NEMUNETFLT_DARWIN_TEST_SEG_SIZE);
 ...
 * promiscuous.
 */
static bool nemuNetFltDarwinIsPromiscuous(PNEMUNETFLTINS pThis)
{
    bool fRc = false;
    ifnet_t pIfNet = nemuNetFltDarwinRetainIfNet(pThis);
    if (pIfNet)
    {
        /* gather the data */
        uint16_t fIf = ifnet_flags(pIfNet);
        unsigned cPromisc = NEMU_GET_PCOUNT(pIfNet);
        bool fSetPromiscuous = ASMAtomicUoReadBool(&pThis->u.s.fSetPromiscuous);
        nemuNetFltDarwinReleaseIfNet(pThis, pIfNet);
 ...
 */
static void nemuNetFltDarwinIffDetached(void *pvThis, ifnet_t pIfNet)
{
    PNEMUNETFLTINS pThis = (PNEMUNETFLTINS)pvThis;
    uint64_t NanoTS = RTTimeSystemNanoTS();
    LogFlow(("nemuNetFltDarwinIffDetached: pThis=%p NanoTS=%RU64 (%d)\n",
             pThis, NanoTS, VALID_PTR(pIfNet) ? NEMU_GET_PCOUNT(pIfNet) :  -1));
    ...
    pIfNet = ASMAtomicUoReadPtrT(&pThis->u.s.pIfNet, ifnet_t);
    int cPromisc = VALID_PTR(pIfNet) ? NEMU_GET_PCOUNT(pIfNet) : - 1;
    ...
    if (pIfNet)
        ifnet_release(pIfNet);
    LogRel(("NemuNetFlt: was detached from '%s' (%d)\n", pThis->szName, cPromisc));
}
 ...
 */
static errno_t nemuNetFltDarwinIffIoCtl(void *pvThis, ifnet_t pIfNet, protocol_family_t eProtocol, u_long uCmd, void *pvArg)
{
    PNEMUNETFLTINS pThis = (PNEMUNETFLTINS)pvThis;
    LogFlow(("nemuNetFltDarwinIffIoCtl: pThis=%p uCmd=%lx\n", pThis, uCmd));
 ...
 */
static void nemuNetFltDarwinIffEvent(void *pvThis, ifnet_t pIfNet, protocol_family_t eProtocol, const struct kev_msg *pEvMsg)
{
    PNEMUNETFLTINS pThis = (PNEMUNETFLTINS)pvThis;
    LogFlow(("nemuNetFltDarwinIffEvent: pThis=%p\n", pThis));
            ...
            if (pEvMsg->event_code == KEV_DL_LINK_ON)
            {
                if (ASMAtomicUoReadBool(&pThis->u.s.fNeedSetPromiscuous))
                {
                    ...
                    if (!err)
                    {
                        ...
                        ASMAtomicWriteBool(&pThis->u.s.fNeedSetPromiscuous, false);
                        Log(("nemuNetFltDarwinIffEvent: enabled promiscuous mode on %s (%d)\n", pThis->szName, NEMU_GET_PCOUNT(pIfNet)));
                    }
                    else
                        Log(("nemuNetFltDarwinIffEvent: ifnet_set_promiscuous failed on %s, err=%d (%d)\n", pThis->szName, err, NEMU_GET_PCOUNT(pIfNet)));
                }
                         ...
                         && !(ifnet_flags(pIfNet) & IFF_PROMISC))
                {
                    ...
                    if (!err && (ifnet_flags(pIfNet) & IFF_PROMISC))
                        Log(("nemuNetFltDarwinIffEvent: fixed IFF_PROMISC on %s (%d)\n", pThis->szName, NEMU_GET_PCOUNT(pIfNet)));
                    else
                        Log(("nemuNetFltDarwinIffEvent: failed to fix IFF_PROMISC on %s, err=%d flags=%#x (%d)\n",
                             pThis->szName, err, ifnet_flags(pIfNet), NEMU_GET_PCOUNT(pIfNet)));
                }
                else
                    Log(("nemuNetFltDarwinIffEvent: online, '%s'. flags=%#x (%d)\n", pThis->szName, ifnet_flags(pIfNet), NEMU_GET_PCOUNT(pIfNet)));
            }
            else if (pEvMsg->event_code == KEV_DL_LINK_OFF)
                Log(("nemuNetFltDarwinIffEvent: %s goes down (%d)\n", pThis->szName, NEMU_GET_PCOUNT(pIfNet)));
/** @todo KEV_DL_LINK_ADDRESS_CHANGED  -> pfnReportMacAddress */
/** @todo KEV_DL_SIFFLAGS              -> pfnReportPromiscuousMode */
        }
        else
            Log(("nemuNetFltDarwinIffEvent: pThis->u.s.pIfNet=%p pIfNet=%p (%d)\n", pThis->u.s.pIfNet, pIfNet, VALID_PTR(pIfNet) ? NEMU_GET_PCOUNT(pIfNet) : -1));
    }
    else if (VALID_PTR(pEvMsg))
        Log(("nemuNetFltDarwinIffEvent: vendor_code=%#x kev_class=%#x kev_subclass=%#x event_code=%#x\n",
             pEvMsg->vendor_code, pEvMsg->kev_class, pEvMsg->kev_subclass, pEvMsg->event_code));
}


/**
 * Internal worker for  nemuNetFltDarwinIffInput and nemuNetFltDarwinIffOutput,
 *
 ...
 */
static errno_t nemuNetFltDarwinIffInputOutputWorker(PNEMUNETFLTINS pThis, mbuf_t pMBuf, void *pvFrame,
                                                    uint32_t fSrc, protocol_family_t eProtocol)
{
    /*
     * Drop it immediately?
     */
    Log2(("nemuNetFltDarwinIffInputOutputWorker: pThis=%p pMBuf=%p pvFrame=%p fSrc=%#x cbPkt=%x\n",
...
#endif
    if (nemuNetFltDarwinMBufIsOur(pThis, pMBuf, pvFrame))
        return 0;
     ...
     */
    bool fDropIt = false;
    unsigned cSegs = nemuNetFltDarwinMBufCalcSGSegs(pThis, pMBuf, pvFrame);
    if (cSegs < NEMUNETFLT_DARWIN_MAX_SEGS)
    {
        PINTNETSG pSG = (PINTNETSG)alloca(RT_OFFSETOF(INTNETSG, aSegs[cSegs]));
        nemuNetFltDarwinMBufToSG(pThis, pMBuf, pvFrame, pSG, cSegs, fSrc);
        ...
        if (fDropIt)
        {
            ...
            if (   fSrc == INTNETTRUNKDIR_WIRE
                && nemuNetFltDarwinIsPromiscuous(pThis))
            {
                fDropIt = false;
            }
 ...
 */
static errno_t nemuNetFltDarwinIffOutput(void *pvThis, ifnet_t pIfNet, protocol_family_t eProtocol, mbuf_t *ppMBuf)
{
    /** @todo there was some note about the ethernet header here or something like that... */

    NOREF(eProtocol);
    NOREF(pIfNet);
    return nemuNetFltDarwinIffInputOutputWorker((PNEMUNETFLTINS)pvThis, *ppMBuf, NULL, INTNETTRUNKDIR_HOST, eProtocol);
}
 ...
 */
static errno_t nemuNetFltDarwinIffInput(void *pvThis, ifnet_t pIfNet, protocol_family_t eProtocol, mbuf_t *ppMBuf, char **ppchFrame)
{
    NOREF(eProtocol);
    NOREF(pIfNet);
    return nemuNetFltDarwinIffInputOutputWorker((PNEMUNETFLTINS)pvThis, *ppMBuf, *ppchFrame, INTNETTRUNKDIR_WIRE, eProtocol);
}


/** A worker thread for nemuNetFltSendDummy(). */
static DECLCALLBACK(int) nemuNetFltSendDummyWorker(RTTHREAD hThreadSelf, void *pvUser)
{
    Assert(pvUser);
    ifnet_t pIfNet = (ifnet_t)pvUser;
    return NemuNetSendDummy(pIfNet);
}
 ...
 */
static void nemuNetFltSendDummy(ifnet_t pIfNet)
{
    RTTHREAD hThread;
    int rc = RTThreadCreate(&hThread, nemuNetFltSendDummyWorker, (void *)pIfNet, 0,
                            RTTHREADTYPE_DEFAULT, RTTHREADFLAGS_WAITABLE, "DummyThread");
    if (RT_SUCCESS(rc))
    {
        RTThreadWait(hThread, RT_INDEFINITE_WAIT, NULL);
        LogFlow(("nemuNetFltSendDummy: a dummy packet has been successfully sent in order to prevent stuck-in-dock issue\n"));
    }
    else
        LogFlow(("nemuNetFltSendDummy: unable to send dummy packet in order to prevent stuck-in-dock issue\n"));
}


/**
 * Internal worker for nemuNetFltOsInitInstance and nemuNetFltOsMaybeRediscovered.
 *
 ...
 */
static int nemuNetFltDarwinAttachToInterface(PNEMUNETFLTINS pThis, bool fRediscovery)
{
    LogFlow(("nemuNetFltDarwinAttachToInterface: pThis=%p (%s)\n", pThis, pThis->szName));
    ...
    if (err)
    {
        Assert(err == ENXIO);
        if (!fRediscovery)
            LogRel(("NemuFltDrv: failed to find ifnet '%s' (err=%d)\n", pThis->szName, err));
        else
            Log(("NemuFltDrv: failed to find ifnet '%s' (err=%d)\n", pThis->szName, err));
    ...
    if (!err)
    {
        ...
        RegRec.iff_name     = "NemuNetFlt";
        RegRec.iff_protocol = 0;
        RegRec.iff_input    = nemuNetFltDarwinIffInput;
        RegRec.iff_output   = nemuNetFltDarwinIffOutput;
        RegRec.iff_event    = nemuNetFltDarwinIffEvent;
        RegRec.iff_ioctl    = nemuNetFltDarwinIffIoCtl;
        RegRec.iff_detached = nemuNetFltDarwinIffDetached;
            ...
            && nemuNetFltTryRetainBusyNotDisconnected(pThis))
        {
             ...
             * traffic to the host.
             */
            pThis->pSwitchPort->pfnReportPromiscuousMode(pThis->pSwitchPort, nemuNetFltDarwinIsPromiscuous(pThis));
#endif
    ...
    if (RT_SUCCESS(rc))
        LogRel(("NemuFltDrv: attached to '%s' / %RTmac\n", pThis->szName, &pThis->u.s.MacAddr));
    else
        LogRel(("NemuFltDrv: failed to attach to ifnet '%s' (err=%d)\n", pThis->szName, err));
    IPRT_DARWIN_RESTORE_EFL_AC();
    return rc;
}
...
{
    nemuNetFltDarwinAttachToInterface(pThis, true /* fRediscovery */);
...
int  nemuNetFltPortOsXmit(PNEMUNETFLTINS pThis, void *pvIfData, PINTNETSG pSG, uint32_t fDst)
{
    ...
    if (pIfNet)
    {
        ...
        {
            mbuf_t pMBuf = nemuNetFltDarwinMBufFromSG(pThis, pSG);
            if (pMBuf)
            {
...
void nemuNetFltPortOsSetActive(PNEMUNETFLTINS pThis, bool fActive)
{
    ...
    if (pIfNet)
    {
        ...
        else
        {
             ...
             */
            unsigned const cPromiscBefore = NEMU_GET_PCOUNT(pIfNet);
            u_int16_t fIf;
            if (fActive)
            {
                    ...
                    if (!err)
                    {
                        ...
                        if (!(ifnet_flags(pIfNet) & IFF_PROMISC))
                        {
                            ...
                            if (!err)
                                err = ifnet_ioctl(pIfNet, 0, SIOCSIFFLAGS, NULL);
                            if (!err)
                                Log(("nemuNetFlt: fixed IFF_PROMISC on %s (%d->%d)\n", pThis->szName, cPromiscBefore, NEMU_GET_PCOUNT(pIfNet)));
                            else
                                Log(("NemuNetFlt: failed to fix IFF_PROMISC on %s, err=%d (%d->%d)\n",
                                     pThis->szName, err, cPromiscBefore, NEMU_GET_PCOUNT(pIfNet)));
                        }
                    }
                    else
                        Log(("NemuNetFlt: ifnet_set_promiscuous -> err=%d grr! (%d->%d)\n", err, cPromiscBefore, NEMU_GET_PCOUNT(pIfNet)));
                }
                else if (!err)
                    Log(("NemuNetFlt: Waiting for the link to come up... (%d->%d)\n", cPromiscBefore, NEMU_GET_PCOUNT(pIfNet)));
                if (err)
                    LogRel(("NemuNetFlt: Failed to put '%s' into promiscuous mode, err=%d (%d->%d)\n", pThis->szName, err, cPromiscBefore, NEMU_GET_PCOUNT(pIfNet)));
            }
            else
            {
                ASMAtomicWriteBool(&pThis->u.s.fNeedSetPromiscuous, false);
                ...
                fIf = ifnet_flags(pIfNet);
                Log(("NemuNetFlt: fIf=%#x; %d->%d\n", fIf, cPromiscBefore, NEMU_GET_PCOUNT(pIfNet)));
            }
        }
...
int  nemuNetFltOsInitInstance(PNEMUNETFLTINS pThis, void *pvContext)
{
    ...
    int rc = nemuNetFltDarwinAttachToInterface(pThis, false /* fRediscovery */);
    if (RT_FAILURE(rc))
        return rc;
    ...
    error = sock_socket(PF_SYSTEM, SOCK_RAW, SYSPROTO_EVENT,
                        nemuNetFltDarwinSysSockUpcall, pThis,
                        &pThis->u.s.pSysSock);
    if (error != 0)
    {
    ...
    for (ifaddr_t *pIfAddr = pIfAddrList; *pIfAddr != NULL; ++pIfAddr)
    {
        ...
        if (family == AF_INET)
        {
            ...
            if (NEMU_IN_LOOPBACK(u32Addr))
                continue;

            if (ifaddr_ifnet(ifa) != pIfNet && NEMU_IN_LINKLOCAL(u32Addr))
                continue;
     ...
     */
    nemuNetFltDarwinSysSockUpcall(pThis->u.s.pSysSock, pThis, MBUF_DONTWAIT);
...
static void nemuNetFltDarwinSysSockUpcall(socket_t pSysSock, void *pvData, int fWait)
{
    ...
    {
        Log(("nemuNetFltDarwinSysSockUpcall: %p != %p?\n",
    ...
    for (;;)
    {
        ...
        if (error != 0)
        {
            ...
            {
                Log(("nemuNetFltDarwinSysSockUpcall: EWOULDBLOCK - we are done\n"));
                error = 0;
            }
            else
        ...
        {
            Log(("nemuNetFltDarwinSysSockUpcall: %u bytes is too short\n", (unsigned int)len));
            mbuf_freem(m);
            return;
        }
        ...
        if (msg->kev_subclass == KEV_INET_SUBCLASS)
        {
            ...
            {
                Log(("nemuNetFltDarwinSysSockUpcall: %u bytes is too short for KEV_INET_SUBCLASS\n",
                     (unsigned int)len));
                mbuf_freem(m);
                return;
            }
        ...
        else if (msg->kev_subclass == KEV_INET6_SUBCLASS)
        {
            ...
            {
                Log(("nemuNetFltDarwinSysSockUpcall: %u bytes is too short for KEV_INET6_SUBCLASS\n",
                        (unsigned int)len));
                mbuf_freem(m);
                return;
            }
            ...
            Log(("nemuNetFltDarwinSysSockUpcall: subclass %u ignored\n", (unsigned)msg->kev_subclass));
