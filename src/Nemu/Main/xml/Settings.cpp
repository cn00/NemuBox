 ...
 *
 * The settings versions enum is defined in src/Nemu/Main/idl/VirtualBox.xidl. To introduce
 ...
 *
 * Certainly ConfigFileBase::ConfigFileBase() will. Change NEMU_XML_VERSION below as well.
 * NEMU_XML_VERSION does not have to be changed if the settings for a default VM do not
 ...
 * SettingsVersion enumeration in src/Nemu/Main/idl/VirtualBox.xidl for details about which
...
#define NEMU_XML_NAMESPACE      "http://www.innotek.de/VirtualBox-settings"

/** VirtualBox XML settings version number substring ("x.y")  */
#define NEMU_XML_VERSION        "1.12"

/** VirtualBox XML settings version platform substring */
#if defined (RT_OS_DARWIN)
#   define NEMU_XML_PLATFORM     "macosx"
#elif defined (RT_OS_FREEBSD)
#   define NEMU_XML_PLATFORM     "freebsd"
#elif defined (RT_OS_LINUX)
#   define NEMU_XML_PLATFORM     "linux"
#elif defined (RT_OS_NETBSD)
#   define NEMU_XML_PLATFORM     "netbsd"
#elif defined (RT_OS_OPENBSD)
#   define NEMU_XML_PLATFORM     "openbsd"
#elif defined (RT_OS_OS2)
#   define NEMU_XML_PLATFORM     "os2"
#elif defined (RT_OS_SOLARIS)
#   define NEMU_XML_PLATFORM     "solaris"
#elif defined (RT_OS_WINDOWS)
#   define NEMU_XML_PLATFORM     "windows"
#else
#   error Unsupported platform!
#endif

/** VirtualBox XML settings full version string ("x.y-platform") */
#define NEMU_XML_VERSION_FULL   NEMU_XML_VERSION "-" NEMU_XML_PLATFORM
    ...
    : m(new Data)
{
    ...
    else
    {
        ...
        m->strSettingsVersionFull = NEMU_XML_VERSION_FULL;
        m->sv = SettingsVersion_v1_12;
    }
}
...
void ConfigFileBase::createStubDocument()
{
    ...
    m->pelmRoot = m->pDoc->createRootElement("VirtualBox",
                                             ...
                                             "** Use NemuManage or the VirtualBox Manager GUI to make changes.\n"
);
    m->pelmRoot->setAttribute("xmlns", NEMU_XML_NAMESPACE);
       ...
       )
    {
        ...
        strFilenameNew.append(strExt);                          // .xml for main config, .nemu for machine config
...
#else
            "HostInterfaceNetworking-nemunet0";
#endif
...
void MainConfigFile::write(const com::Utf8Str strFilename)
{
    ...
    for (MachinesRegistry::const_iterator it = llMachines.begin();
         ...
         ++it)
    {
        // <MachineEntry uuid="{5f102a55-a51b-48e3-b45a-b28d33469488}" src="/mnt/innotek-unix/nemu-machines/Windows 5.1 XP 1 (Office 2003)/Windows 5.1 XP 1 (Office 2003).xml"/>
...
Hardware::Hardware()
        : strVersion("1"),
          ...
          graphicsControllerType(GraphicsControllerType_NemuVGA),
          ulVRAMSizeMB(8),
          cMonitors(1),
          fAccelerate3D(false),
 ...
 *
 * This is used by the OVF code if a <nemu:Machine> element has been encountered
    ...
    while ((pelmHwChild = nl1.forAllNodes()))
    {
        ...
        else if (pelmHwChild->nameEquals("Display"))
        {
            ...
            if (!pelmHwChild->getAttributeValue("controller", strGraphicsControllerType))
                hw.graphicsControllerType = GraphicsControllerType_NemuVGA;
            else
            {
                strGraphicsControllerType.toUpper();
                GraphicsControllerType_T type;
                if (strGraphicsControllerType == "NEMUVGA")
                    type = GraphicsControllerType_NemuVGA;
                                       ...
                                       MachineUserData *pUserData)
{
    ...
    if (   pUserData->strTeleporterPassword.isNotEmpty()
        && !NemuIsPasswordHashed(&pUserData->strTeleporterPassword))
        NemuHashPassword(&pUserData->strTeleporterPassword);
}
                                         ...
                                         const Storage &strg)
{
    ...
    if (hw.graphicsControllerType != GraphicsControllerType_NemuVGA)
    {
        const char *pcszGraphics;
        switch (hw.graphicsControllerType)
        {
            case GraphicsControllerType_NemuVGA:            pcszGraphics = "NemuVGA"; break;
    ...
    if (m->sv < SettingsVersion_v1_11)
    {
        /* In Nemu 4.0 these attributes are replaced with "Properties". */
        Utf8Str strPort;
 ...
 *
 *  --  BuildMachineXML_WriteNemuVersionAttribute: If set, add a settingsVersion
 *      attribute to the machine tag with the nemu settings version. This is for
...
void MachineConfigFile::buildMachineXML(xml::ElementNode &elmMachine,
                                        ...
                                        std::list<xml::ElementNode*> *pllElementsWithUuidAttributes)
{
    if (fl & BuildMachineXML_WriteNemuVersionAttribute)
...
# else /* NEMU_WITH_WINMM */
    return AudioDriverType_DirectSound;
# endif /* !NEMU_WITH_WINMM */
#elif defined(RT_OS_SOLARIS)
...
void MachineConfigFile::bumpSettingsVersionIfNeeded()
{
    ...
    if (m->sv < SettingsVersion_v1_14)
    {
        ...
        if (   !hardwareMachine.strDefaultFrontend.isEmpty()
            || hardwareMachine.graphicsControllerType != GraphicsControllerType_NemuVGA
    ...
    if (m->sv < SettingsVersion_v1_10)
    {
         ...
         * then increase the version to at least Nemu 3.2, which can have video channel properties.
         */
    ...
    if (m->sv < SettingsVersion_v1_11)
    {
         ...
         * "VideoChannel/Enabled" and "VideoChannel/Quality" then increase the version to Nemu 4.0.
         */
        unsigned cOldProperties = 0;
    ...
    try
    {
        ...
        buildMachineXML(*pelmMachine,
                        ...
                        | MachineConfigFile::BuildMachineXML_MediaRegistry,
                            // but not BuildMachineXML_WriteNemuVersionAttribute
                        NULL); /* pllElementsWithUuidAttributes */
