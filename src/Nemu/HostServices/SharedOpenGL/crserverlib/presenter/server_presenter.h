...
{
    NEMUVR_SCR_COMPOSITOR Compositor;
...
{
    NEMUVR_SCR_COMPOSITOR_ENTRY Entry;
    RTLISTNODE Node;
    uint32_t cRefs;
...
class CrFbWindow
{
    public:
        ...
        int SetCompositor(const struct NEMUVR_SCR_COMPOSITOR * pCompositor);
    ...
    private:
        ...
        GLint mSpuWindow;
        const struct NEMUVR_SCR_COMPOSITOR * mpCompositor;
        uint32_t mcUpdates;
        int32_t mxPos;
        int32_t myPos;
        uint32_t mWidth;
...
class CrFbDisplayWindowRootVr : public CrFbDisplayWindow
{
    ...
    protected:
        ...
        virtual int fbCleanup();
        virtual int fbSync();
        NEMUVR_SCR_COMPOSITOR_ENTRY* entryAlloc();
        void entryFree(NEMUVR_SCR_COMPOSITOR_ENTRY* pEntry);
        int synchCompositorRegions();
        virtual int synchCompositor();
        virtual int clearCompositor();
        void rootVrTranslateForPos();
        static DECLCALLBACK(NEMUVR_SCR_COMPOSITOR_ENTRY*) rootVrGetCEntry(const NEMUVR_SCR_COMPOSITOR_ENTRY*pEntry, void *pvContext);

    private:

        NEMUVR_SCR_COMPOSITOR mCompositor;
};
