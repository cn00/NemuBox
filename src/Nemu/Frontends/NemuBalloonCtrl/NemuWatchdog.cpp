/* $Id: NemuWatchdog.cpp $ */
/** @file
 * NemuWatchdog.cpp - VirtualBox Watchdog.
 */
...
static ComPtr<IEventListener>    g_pNemuEventListener = NULL;
        ...
        STDMETHOD(HandleEvent)(NemuEventType_T aType, IEvent *aEvent)
        {
            switch (aType)
            {
                ...
                case NemuEventType_OnNemuSVCAvailabilityChanged:
                {
                    ...
                    if (!fAvailable)
                    {
                        serviceLog("NemuSVC became unavailable\n");
                        watchdogShutdown();
                    }
                    else
                    {
                        serviceLog("NemuSVC became available\n");
...
NEMU_LISTENER_DECLARE(VirtualBoxEventListenerImpl)
...
#ifndef NEMU_WATCHDOG_GLOBAL_PERFCOL
...
static RTEXITCODE watchdogMain(HandlerArg *a)
{
    ...
    do
    {
         ...
         * - g_pEventSourceClient for NemuClient events (like NemuSVC handling)
         */
        ...
        ComObjPtr<VirtualBoxEventListenerImpl> nemuListenerImpl;
        nemuListenerImpl.createObject();
        nemuListenerImpl->init(new VirtualBoxEventListener());

        com::SafeArray<NemuEventType_T> eventTypes;
        eventTypes.push_back(NemuEventType_OnMachineRegistered);
        eventTypes.push_back(NemuEventType_OnMachineStateChanged);
        eventTypes.push_back(NemuEventType_OnNemuSVCAvailabilityChanged); /* Processed by g_pEventSourceClient. */

        g_pNemuEventListener = nemuListenerImpl;
        CHECK_ERROR_BREAK(g_pEventSource, RegisterListener(g_pNemuEventListener, ComSafeArrayAsInParam(eventTypes), true /* Active listener */));
        CHECK_ERROR_BREAK(g_pEventSourceClient, RegisterListener(g_pNemuEventListener, ComSafeArrayAsInParam(eventTypes), true /* Active listener */));
        ...
        if (g_pNemuEventListener)
        {
            if (!g_pEventSource.isNull())
                CHECK_ERROR(g_pEventSource, UnregisterListener(g_pNemuEventListener));
            g_pNemuEventListener.setNull();
        }
...
{
    RTStrmPrintf(g_pStdErr, NEMU_PRODUCT " Watchdog " NEMU_VERSION_STRING "\n"
...
static void displayHelp(const char *pszImage)
{
    ...
    /** @todo Change NEMUBALLOONCTRL_RELEASE_LOG to WATCHDOG*. */
    RTStrmPrintf(g_pStdErr, "\nUse environment variable NEMUBALLOONCTRL_RELEASE_LOG for logging options.\n");
}
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    rc = com::NemuLogRelCreate("Watchdog", g_fDaemonize ? NULL : pszLogFile,
                               RTLOGFLAGS_PREFIX_THREAD | RTLOGFLAGS_PREFIX_TIME_PROG,
                               "all", "NEMUBALLOONCTRL_RELEASE_LOG",
    ...
    if (g_fDaemonize)
    {
        ...
        if (!pszLogFile || !*pszLogFile)
        {
            ...
            rc = RTPathAppend(szLogFile, sizeof(szLogFile), "nemuballoonctrl.log");
            if (RT_FAILURE(rc))
        ...
        rc = com::NemuLogRelCreate("Watchdog", pszLogFile,
                                   RTLOGFLAGS_PREFIX_THREAD | RTLOGFLAGS_PREFIX_TIME_PROG,
                                   "all", "NEMUBALLOONCTRL_RELEASE_LOG",
