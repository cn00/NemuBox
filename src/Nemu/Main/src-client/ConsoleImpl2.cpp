/* $Id: ConsoleImpl2.cpp $ */
/** @file
 * Nemu Console COM Class implementation - VM Configuration Bits.
 *
...
#include <Nemu/vmm/pdmapi.h> /* For PDMR3DriverAttach/PDMR3DriverDetach. */
#include <Nemu/vmm/pdmusb.h> /* For PDMR3UsbCreateEmulatedDevice. */
...
static int findEfiRom(IVirtualBox* nemu, FirmwareType_T aFirmwareType, Utf8Str *pEfiRomFile)
{
    Bstr aFilePath, empty;
    BOOL fPresent = FALSE;
    HRESULT hrc = nemu->CheckFirmwarePresent(aFirmwareType, empty.raw(),
                                             empty.asOutParam(), aFilePath.asOutParam(), &fPresent);
    AssertComRCReturn(hrc, Global::nemuStatusCodeFromCOM(hrc));
...
static int getSmcDeviceKey(IVirtualBox *pVirtualBox, IMachine *pMachine, Utf8Str *pStrKey, bool *pfGetKeyFromRealSMC)
{
    *pfGetKeyFromRealSMC = false;
     ...
     */
    GetExtraDataBoth(pVirtualBox, pMachine, "NemuInternal2/SmcDeviceKey", pStrKey);
    ...
    if (!mptrExtPackManager->i_isExtPackUsable(s_pszPCIRawExtPackName))
        ...
        return VMR3SetError(pUVM, VERR_NOT_FOUND, RT_SRC_POS,
                N_("Implementation of the PCI passthrough framework not found!\n"
                   ...
                   "install the '%s' or disable PCI passthrough via NemuManage"),
...
void Console::i_reportDriverVersions()
{
    ...
    do
    {
        ...
        for (int i = 0; i < cDrivers; i++)
        {
            ...
            {
                if (_tcsnicmp(TEXT("nemu"), szDriver, 4))
                    continue;
            }
            else
                continue;
...
#if 0 /* enable to play with lots of memory. */
    if (RTEnvExist("NEMU_RAM_SIZE"))
        cRamMBs = RTStrToUInt64(RTEnvGet("NEMU_RAM_SIZE"));
#endif
...
# endif
...
#if ARCH_BITS == 32 /* The recompiler must use NemuREM64 (32-bit host only). */
            PCFGMNODE pREM;
            InsertConfigNode(pRoot, "REM", &pREM);
            InsertConfigInteger(pREM, "64bitEnabled", 1);
#endif

            /** @todo Not exactly pretty to check strings; NEMUOSTYPE would be better,
        ...
        InsertConfigNode(pNode,    "NemuC", &pMod);
#ifdef NEMU_WITH_XPCOM
        // NemuC is located in the components subdirectory
        char szPathNemuC[RTPATH_MAX];
        rc = RTPathAppPrivateArch(szPathNemuC, RTPATH_MAX - sizeof("/components/NemuC"));   AssertRC(rc);
        strcat(szPathNemuC, "/components/NemuC");
        InsertConfigString(pMod,   "Path",  szPathNemuC);
#else
        InsertConfigString(pMod,   "Path",  "NemuC");
#endif
        ...
        switch (enmGraphicsController)
        {
            case GraphicsControllerType_Null:
                break;
            case GraphicsControllerType_NemuVGA:
        ...
        else
        {
            ...
            /* Get boot args */
            Utf8Str bootArgs;
            GetExtraDataBoth(virtualBox, pMachine, "NemuInternal2/EfiBootArgs", &bootArgs);

            /* Get device props */
            Utf8Str deviceProps;
            GetExtraDataBoth(virtualBox, pMachine, "NemuInternal2/EfiDeviceProps", &deviceProps);

            /* Get GOP mode settings */
            uint32_t u32GopMode = UINT32_MAX;
            GetExtraDataBoth(virtualBox, pMachine, "NemuInternal2/EfiGopMode", &strTmp);
            if (!strTmp.isEmpty())
                u32GopMode = strTmp.toUInt32();

            /* UGA mode settings */
            uint32_t u32UgaHorizontal = 0;
            GetExtraDataBoth(virtualBox, pMachine, "NemuInternal2/EfiUgaHorizontalResolution", &strTmp);
            if (!strTmp.isEmpty())
                u32UgaHorizontal = strTmp.toUInt32();

            uint32_t u32UgaVertical = 0;
            GetExtraDataBoth(virtualBox, pMachine, "NemuInternal2/EfiUgaVerticalResolution", &strTmp);
            if (!strTmp.isEmpty())
...
# ifdef NEMU_WITH_EXTPACK
                    ...
                    {
                        /* Always fatal! Up to Nemu 4.0.4 we allowed to start the VM anyway
...
# endif
...
#ifdef NEMU_WITH_USB
            {
                ...
                //      NemuManage setextradata "myvm" "NemuInternal/USB/USBProxy/GlobalConfig/Force11PacketSize" 1
            ...
            if (aEmulatedUSBCardReaderEnabled)
            {
                ...
                InsertConfigNode(pInst,    "LUN#0", &pLunL0);
# ifdef NEMU_WITH_USB_CARDREADER_TEST
...
# endif
        ...
        for (ULONG ulInstance = 0; ulInstance < maxNetworkAdapters; ++ulInstance)
        {
            ...
            InsertConfigNode(pInst, "Config", &pCfg);
#ifdef NEMU_WITH_2X_4GB_ADDR_SPACE   /* not safe here yet. */ /** @todo Make PCNet ring-0 safe on 32-bit mac kernels! */
            if (pDev == pDevPCNet)
            {
...
# endif
...
#endif
#if defined(RT_OS_LINUX) || defined(RT_OS_FREEBSD) || defined(NEMU_WITH_SOLARIS_OSS)
         ...
         */
        {
            ...
            {
                /* Load the service */
                rc = pVMMDev->hgcmLoadService("NemuSharedClipboard", "NemuSharedClipboard");
                if (RT_FAILURE(rc))
                {
                ...
                else
                {
                    ...
                    pVMMDev->hgcmHostCall("NemuSharedClipboard",
                                          NEMU_SHARED_CLIPBOARD_HOST_FN_SET_HEADLESS, 1, &parm);
                }
            }
        }
         ...
         */
        {
            ...
            if (   hrc   == S_OK
                && value == "1")
            {
                rc = pVMMDev->hgcmLoadService("NemuHostChannel", "NemuHostChannel");
                if (RT_FAILURE(rc))
                {
                    LogRel(("NemuHostChannel is not available, rc=%Rrc\n", rc));
                    /* That is not a fatal failure. */
                    rc = VINF_SUCCESS;
                }
            }
         ...
         */
        {
            ...
            /* Load the service */
            rc = pVMMDev->hgcmLoadService("NemuDragAndDropSvc", "NemuDragAndDropSvc");
            if (RT_FAILURE(rc))
            {
            ...
            else
            {
                ...
                rc = HGCMHostRegisterServiceExtension(&hDummy, "NemuDragAndDropSvc",
                                                      &GuestDnD::notifyDnDDispatcher,
                                                      GuestDnDInst());
                if (RT_FAILURE(rc))
                    Log(("Cannot register NemuDragAndDropSvc extension, rc=%Rrc\n", rc));
                else
                {
...
# ifdef NEMU_WITH_VMSVGA3D
                && enmGraphicsController == GraphicsControllerType_NemuVGA
# endif
                )
            {
                BOOL fSupports3D = NemuOglIs3DAccelerationSupported();
                if (!fSupports3D)
                ...
                /* Load the service. */
                rc = pVMMDev->hgcmLoadService("NemuSharedCrOpenGL", "NemuSharedCrOpenGL");
                if (RT_FAILURE(rc))
                {
                ...
                else
                {
                    ...
                    rc = pVMMDev->hgcmHostCall("NemuSharedCrOpenGL", SHCRGL_HOST_FN_SET_CONSOLE,
                    ...
                    rc = pVMMDev->hgcmHostCall("NemuSharedCrOpenGL",
...
#endif /* NEMU_WITH_GUEST_PROPS defined */
...
#endif /* NEMU_WITH_GUEST_CONTROL defined */
...
/**
 * Applies the CFGM overlay as specified by NemuInternal/XXX extra data
 * values.
 *
...
int Console::i_configCfgmOverlay(PCFGMNODE pRoot, IVirtualBox *pVirtualBox, IMachine *pMachine)
{
    ...
    try
    {
            ...
            ++it, ++i2)
        {
            ...
            /*
             * We only care about keys starting with "NemuInternal/" (skip "G:" or "M:")
             */
            if (!strKey.startsWith("NemuInternal/"))
                continue;

            const char *pszExtraDataKey = strKey.c_str() + sizeof("NemuInternal/") - 1;
...
/**
 * Dumps the API settings tweaks as specified by NemuInternal2/XXX extra data
 * values.
 *
 * @returns Nemu status code.
 * @param   pVirtualBox     Pointer to the IVirtualBox interface.
...
{
    {
        ...
        for (size_t i = 0; i < aGlobalExtraDataKeys.size(); i++)
        {
            ...
            if (!strKey.startsWith("NemuInternal2/"))
                continue;
...
#ifdef NEMU_WITH_2X_4GB_ADDR_SPACE
...
#define H() \
    AssertMsgReturnStmt(SUCCEEDED(hrc), ("hrc=%Rhrc\n", hrc), if (phrc) *phrc = hrc, Global::nemuStatusCodeFromCOM(hrc))
...
{
    LogFlowFunc(("entry\n"));
    HANDLE hFile = CreateFile(L"\\\\.\\NemuNetLwf",
                              0,
...
#elif defined(NEMU_WITH_NETFLT)
                /*
                 * This is the new NemuNetFlt+IntNet stuff.
                 */
                Bstr BridgedIfName;
...
# elif defined(RT_OS_SOLARIS)
                 ...
                 * NemuManage and setting only the NIC name from there. If there is a space we
...
# elif defined(RT_OS_WINDOWS)
                ...
                hrc = NemuNetCfgWinQueryINetCfg(&pNc, FALSE, L"VirtualBox", 10, &pszApp);
                Assert(hrc == S_OK);
                if (hrc != S_OK)
                {
                ...
                hrc = NemuNetCfgWinGetComponentByGuid(pNc, &GUID_DEVCLASS_NET, (GUID*)hostIFGuid.raw(),
                                                      pAdaptorComponent.asOutParam());
                if (hrc != S_OK)
                {
                    NemuNetCfgWinReleaseINetCfg(pNc, FALSE /*fHasWriteLock*/);
                    LogRel(("NetworkAttachmentType_Bridged: NemuNetCfgWinGetComponentByGuid failed, hrc (0x%x)\n", hrc));
                    H();
                }
#define NEMU_WIN_BINDNAME_PREFIX "\\DEVICE\\"
                ...
                if (hrc == S_OK)
                {
                    ...
                    int cbFullBindNamePrefix = sizeof(NEMU_WIN_BINDNAME_PREFIX);
                    if (sizeof(szTrunkName) > cbFullBindNamePrefix + cwBindName)
                    {
                        strcpy(szTrunkName, NEMU_WIN_BINDNAME_PREFIX);
                ...
                {
                    NemuNetCfgWinReleaseINetCfg(pNc, FALSE /*fHasWriteLock*/);
                    AssertLogRelMsgFailed(("NetworkAttachmentType_Bridged: NemuNetCfgWinGetComponentByGuid failed, hrc (0x%x)",
                                           hrc));
                    H();
                }
...
#  if defined(RT_OS_FREEBSD)
                 ...
                 * interface via the current FreeBSD nemunetflt implementation.
                 */
                        ...
                        switch (hrc)
                        {
                            case VERR_ACCESS_DENIED:
                                return VMSetError(VMR3GetVM(mpUVM), VERR_HOSTIF_INIT_FAILED, RT_SRC_POS,  N_(
                                                ...
                                                "change the group of that node to nemuusers and make yourself "
...
# endif
                 ...
                 * for the same interface name, eg: "nge0 - ethernet" (GUI) vs "nge0" (NemuManage).
                 */
...
# else  /* defined NEMU_WITH_NETFLT*/
                ...
                {
                    NemuNetCfgWinReleaseINetCfg(pNc, FALSE /*fHasWriteLock*/);
                    LogRel(("NetworkAttachmentType_HostOnly: NemuNetCfgWinGetComponentByGuid failed, hrc=%Rhrc (0x%x)\n", hrc, hrc));
                    H();
                }
#  define NEMU_WIN_BINDNAME_PREFIX "\\DEVICE\\"
                char szTrunkName[INTNET_MAX_TRUNK_NAME];
                ...
                {
                    NemuNetCfgWinReleaseINetCfg(pNc, FALSE /*fHasWriteLock*/);
                    AssertLogRelMsgFailed(("NetworkAttachmentType_HostOnly: NemuNetCfgWinGetComponentByGuid failed, hrc=%Rhrc (0x%x)\n",
                                           hrc, hrc));
                    H();
                }
                ...
                networkName = Bstr(szNetwork);
                trunkName   = Bstr(pszTrunk);
# endif /* defined NEMU_WITH_NETFLT*/
...
#if !defined(RT_OS_WINDOWS) && defined(NEMU_WITH_NETFLT)
                ...
                if (SUCCEEDED(hrc) && !tmpAddr.isEmpty())
                {
                    ...
                    if (SUCCEEDED(hrc) && !tmpMask.isEmpty())
                        ...
                        hrc = hostInterface->EnableStaticIPConfig(tmpAddr.raw(),
                                                                  Bstr(NEMUNET_IPV4MASK_DEFAULT).raw());
                }
                else
                {
                    /* Grab the IP number from the 'nemunetX' instance number (see netif.h) */
            ...
            case NetworkAttachmentType_NATNetwork:
            {
                ...
                if (!bstr.isEmpty())
                {
                    /** @todo add intnet prefix to separate namespaces, and add trunk if dealing with nemunatX */
                    InsertConfigString(pLunL0, "Driver", "IntNet");
                    InsertConfigNode(pLunL0, "Config", &pCfg);
                    InsertConfigString(pCfg, "Network", bstr);
                    InsertConfigInteger(pCfg, "TrunkType", kIntNetTrunkType_WhateverNone);
                              ...
                              const char *pszFlags)
{
    ...
    pVMMDev->hgcmHostCall("NemuGuestPropSvc", guestProp::SET_PROP_HOST, 3,
                          &parms[0]);
}
...
{
    NEMUHGCMSVCPARM paParm;
    paParm.setUInt32(eFlags);
    int rc = pVMMDev->hgcmHostCall("NemuGuestPropSvc",
...
/* static */ int Console::i_configGuestProperties(void *pvConsole, PUVM pUVM)
{
    ...
    /* Load the service */
    int rc = pConsole->m_pVMMDev->hgcmLoadService("NemuGuestPropSvc", "NemuGuestPropSvc");

    if (RT_FAILURE(rc))
    {
        LogRel(("NemuGuestPropSvc is not available. rc = %Rrc\n", rc));
        /* That is not a fatal failure. */
        rc = VINF_SUCCESS;
    }
    else
    {
        ...
        {
            NEMUHGCMSVCPARM Params[2];
            int rc2 = pConsole->m_pVMMDev->hgcmHostCall("NemuGuestPropSvc", guestProp::GET_DBGF_INFO_FN, 2, &Params[0]);
            if (RT_SUCCESS(rc2))
            {
        ...
        /* Sysprep execution by NemuService. */
         ...
         * will override them.
         */
        /* Set the raw Nemu version string as a guest property. Used for host/guest
         * version comparison. */
        configSetProperty(pConsole->m_pVMMDev, "/VirtualBox/HostInfo/NemuVer",
                          NEMU_VERSION_STRING_RAW, "TRANSIENT, RDONLYGUEST");
        /* Set the full Nemu version string as a guest property. Can contain vendor-specific
         * information/branding and/or pre-release tags. */
        configSetProperty(pConsole->m_pVMMDev, "/VirtualBox/HostInfo/NemuVerExt",
                          NEMU_VERSION_STRING, "TRANSIENT, RDONLYGUEST");
        /* Set the Nemu SVN revision as a guest property */
        configSetProperty(pConsole->m_pVMMDev, "/VirtualBox/HostInfo/NemuRev",
         ...
         */
        HGCMSVCEXTHANDLE hDummy;
        HGCMHostRegisterServiceExtension(&hDummy, "NemuGuestPropSvc",
                                         Console::i_doGuestPropNotification,
                                         pvConsole);

#ifdef NEMU_WITH_GUEST_PROPS_RDONLY_GUEST
        rc = configSetGlobalPropertyFlags(pConsole->m_pVMMDev,
                                          guestProp::RDONLYGUEST);
        AssertRCReturn(rc, rc);
#endif

        Log(("Set NemuGuestPropSvc property store\n"));
    }
    return VINF_SUCCESS;
...
/* static */ int Console::i_configGuestControl(void *pvConsole)
{
    ...
    /* Load the service */
    int rc = pConsole->m_pVMMDev->hgcmLoadService("NemuGuestControlSvc", "NemuGuestControlSvc");

    if (RT_FAILURE(rc))
    {
        LogRel(("NemuGuestControlSvc is not available. rc = %Rrc\n", rc));
        /* That is not a fatal failure. */
        rc = VINF_SUCCESS;
    }
    else
    {
        HGCMSVCEXTHANDLE hDummy;
        rc = HGCMHostRegisterServiceExtension(&hDummy, "NemuGuestControlSvc",
                                              &Guest::i_notifyCtrlDispatcher,
                                              pConsole->i_getGuest());
        if (RT_FAILURE(rc))
            Log(("Cannot register NemuGuestControlSvc extension!\n"));
        else
            LogRel(("Guest Control service loaded\n"));
    }

    return rc;
#else /* !NEMU_WITH_GUEST_CONTROL */
    return VERR_NOT_SUPPORTED;
#endif /* !NEMU_WITH_GUEST_CONTROL */
}
