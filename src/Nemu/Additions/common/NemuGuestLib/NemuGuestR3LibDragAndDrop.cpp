/* $Id: NemuGuestR3LibDragAndDrop.cpp $ */
/** @file
 * NemuGuestR3Lib - Ring-3 Support Library for VirtualBox guest additions, Drag & Drop.
 */
...
#include <Nemu/GuestHost/DragAndDrop.h>
...
static int vbglR3DnDQueryNextHostMessageType(PVBGLR3GUESTDNDCMDCTX pCtx, uint32_t *puMsg, uint32_t *pcParms, bool fWait)
{
    ...
    DragAndDropSvc::NEMUDNDNEXTMSGMSG Msg;
    RT_ZERO(Msg);
    ...
    int rc = vbglR3DoIOCtl(NEMUGUEST_IOCTL_HGCM_CALL(sizeof(Msg)), &Msg, sizeof(Msg));
    if (RT_SUCCESS(rc))
    {
        rc = Msg.hdr.result;
        if (RT_SUCCESS(rc))
                                           ...
                                           uint32_t *pcbFormatsRecv)
{
    ...
    DragAndDropSvc::NEMUDNDHGACTIONMSG Msg;
    RT_ZERO(Msg);
    Msg.hdr.u32ClientID = pCtx->uClientID;
...
static int vbglR3DnDHGProcessLeaveMessage(PVBGLR3GUESTDNDCMDCTX pCtx)
{
    ...
    DragAndDropSvc::NEMUDNDHGLEAVEMSG Msg;
    RT_ZERO(Msg);
    Msg.hdr.u32ClientID = pCtx->uClientID;
...
static int vbglR3DnDHGProcessCancelMessage(PVBGLR3GUESTDNDCMDCTX pCtx)
{
    ...
    DragAndDropSvc::NEMUDNDHGCANCELMSG Msg;
    RT_ZERO(Msg);
    Msg.hdr.u32ClientID = pCtx->uClientID;
                                            ...
                                            uint32_t *pfMode)
{
    ...
    DragAndDropSvc::NEMUDNDHGSENDDIRMSG Msg;
    RT_ZERO(Msg);
    Msg.hdr.u32ClientID = pCtx->uClientID;
                                             ...
                                             uint32_t             *pfMode)
{
    ...
    DragAndDropSvc::NEMUDNDHGSENDFILEDATAMSG Msg;
    RT_ZERO(Msg);
    Msg.hdr.u32ClientID = pCtx->uClientID;
                                                ...
                                                uint64_t              *pcbTotal)
{
    ...
    DragAndDropSvc::NEMUDNDHGSENDFILEHDRMSG Msg;
    RT_ZERO(Msg);
    Msg.hdr.u32ClientID = pCtx->uClientID;
    ...
    if (RT_SUCCESS(rc))
        rc = vbglR3DoIOCtl(NEMUGUEST_IOCTL_HGCM_CALL(sizeof(Msg)), &Msg, sizeof(Msg));
    if (RT_SUCCESS(rc))
    {
        rc = Msg.hdr.result;
        if (RT_SUCCESS(rc))
                                                 ...
                                                 uint32_t *pcbDataTotal)
{
    ...
    DragAndDropSvc::NEMUDNDHGSENDDATAMSG Msg;
    RT_ZERO(Msg);
    Msg.hdr.u32ClientID = pCtx->uClientID;
                                                     ...
                                                     uint32_t *pcbDataTotal)
{
    ...
    DragAndDropSvc::NEMUDNDHGSENDMOREDATAMSG Msg;
    RT_ZERO(Msg);
    Msg.hdr.u32ClientID = pCtx->uClientID;
                                             ...
                                             size_t    *pcbDataRecv)
{
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         * the caller know.
         *
         * This keeps the actual (guest OS-)dependent client (like NemuClient /
         * NemuTray) small by not having too much redundant code. */
                                                   ...
                                                   uint32_t *puScreenId)
{
    ...
    DragAndDropSvc::NEMUDNDGHREQPENDINGMSG Msg;
    RT_ZERO(Msg);
    Msg.hdr.u32ClientID = pCtx->uClientID;
                                            ...
                                            uint32_t *puAction)
{
    ...
    DragAndDropSvc::NEMUDNDGHDROPPEDMSG Msg;
    RT_ZERO(Msg);
    Msg.hdr.u32ClientID = pCtx->uClientID;
...
VBGLR3DECL(int) VbglR3DnDConnect(PVBGLR3GUESTDNDCMDCTX pCtx)
{
    ...
    int rc = RTStrCopy(Info.Loc.u.host.achName, sizeof(Info.Loc.u.host.achName), "NemuDragAndDropSvc");
    if (RT_FAILURE(rc))
        return rc;

    rc = vbglR3DoIOCtl(NEMUGUEST_IOCTL_HGCM_CONNECT, &Info, sizeof(Info));
    if (RT_SUCCESS(rc))
    {
        rc = Info.result;
    ...
    /*
     * Check if the host is >= Nemu 5.0 which in case supports GUEST_DND_CONNECT.
     */
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        {
            char *pszHostVersion;
            rc2 = VbglR3GuestPropReadValueAlloc(uGuestPropSvcClientID, "/VirtualBox/HostInfo/NemuVer", &pszHostVersion);
            if (RT_SUCCESS(rc2))
            {
    ...
    if (fSupportsConnectReq)
    {
         ...
         * Note: This might fail when the Guest Additions run on an older Nemu host (< Nemu 5.0) which
         *       does not implement this command.
         */
        DragAndDropSvc::NEMUDNDCONNECTMSG Msg;
        RT_ZERO(Msg);
        Msg.hdr.result      = VERR_WRONG_ORDER;
        Msg.hdr.u32ClientID = pCtx->uClientID;
        ...
        int rc2 = vbglR3DoIOCtl(NEMUGUEST_IOCTL_HGCM_CALL(sizeof(Msg)), &Msg, sizeof(Msg));
        if (RT_SUCCESS(rc2))
    ...
    if (!fSupportsConnectReq)
        pCtx->uProtocol = 1; /* Fall back to protocol version 1 (< Nemu 5.0). */
...
VBGLR3DECL(int) VbglR3DnDDisconnect(PVBGLR3GUESTDNDCMDCTX pCtx)
{
    ...
    int rc = vbglR3DoIOCtl(NEMUGUEST_IOCTL_HGCM_DISCONNECT, &Info, sizeof(Info));
    if (RT_SUCCESS(rc))
        rc = Info.result;
...
VBGLR3DECL(int) VbglR3DnDHGAcknowledgeOperation(PVBGLR3GUESTDNDCMDCTX pCtx, uint32_t uAction)
{
    ...
    DragAndDropSvc::NEMUDNDHGACKOPMSG Msg;
    RT_ZERO(Msg);
    Msg.hdr.result      = VERR_WRONG_ORDER;
    Msg.hdr.u32ClientID = pCtx->uClientID;
...
VBGLR3DECL(int) VbglR3DnDHGRequestData(PVBGLR3GUESTDNDCMDCTX pCtx, const char* pcszFormat)
{
    ...
    DragAndDropSvc::NEMUDNDHGREQDATAMSG Msg;
    RT_ZERO(Msg);
    Msg.hdr.result      = VERR_WRONG_ORDER;
    Msg.hdr.u32ClientID = pCtx->uClientID;
...
VBGLR3DECL(int) VbglR3DnDHGSetProgress(PVBGLR3GUESTDNDCMDCTX pCtx, uint32_t uStatus, uint8_t uPercent, int rcErr)
{
    ...
    DragAndDropSvc::NEMUDNDHGEVTPROGRESSMSG Msg;
    RT_ZERO(Msg);
    Msg.hdr.result      = VERR_WRONG_ORDER;
    Msg.hdr.u32ClientID = pCtx->uClientID;
                                              ...
                                              uint32_t uDefAction, uint32_t uAllActions, const char* pcszFormats)
{
    ...
    DragAndDropSvc::NEMUDNDGHACKPENDINGMSG Msg;
    RT_ZERO(Msg);
    Msg.hdr.result      = VERR_WRONG_ORDER;
    Msg.hdr.u32ClientID = pCtx->uClientID;
                                       ...
                                       void *pvData, uint32_t cbData, uint32_t cbAdditionalData)
{
    ...
    DragAndDropSvc::NEMUDNDGHSENDDATAMSG Msg;
    RT_ZERO(Msg);
    Msg.hdr.result      = VERR_WRONG_ORDER;
    Msg.hdr.u32ClientID = pCtx->uClientID;
...
static int vbglR3DnDGHSendDir(PVBGLR3GUESTDNDCMDCTX pCtx, DnDURIObject *pObj)
{
    ...
    DragAndDropSvc::NEMUDNDGHSENDDIRMSG Msg;
    RT_ZERO(Msg);
    Msg.hdr.result      = VERR_WRONG_ORDER;
    Msg.hdr.u32ClientID = pCtx->uClientID;
...
static int vbglR3DnDGHSendFile(PVBGLR3GUESTDNDCMDCTX pCtx, DnDURIObject *pObj)
{
    ...
    {
        DragAndDropSvc::NEMUDNDGHSENDFILEHDRMSG MsgHdr;
        RT_ZERO(MsgHdr);
        ...
        rc = vbglR3DoIOCtl(NEMUGUEST_IOCTL_HGCM_CALL(sizeof(MsgHdr)), &MsgHdr, sizeof(MsgHdr));
        if (RT_SUCCESS(rc))
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         */
        DragAndDropSvc::NEMUDNDGHSENDFILEDATAMSG Msg;
        RT_ZERO(Msg);
        Msg.hdr.result      = VERR_WRONG_ORDER;
        Msg.hdr.u32ClientID = pCtx->uClientID;
...
VBGLR3DECL(int) VbglR3DnDGHSendError(PVBGLR3GUESTDNDCMDCTX pCtx, int rcErr)
{
    ...
    DragAndDropSvc::NEMUDNDGHEVTERRORMSG Msg;
    RT_ZERO(Msg);
    Msg.hdr.result      = VERR_WRONG_ORDER;
    Msg.hdr.u32ClientID = pCtx->uClientID;
