...
#define NEMUNET_IPV4ADDR_DEFAULT      0x0138A8C0  /* 192.168.56.1 */
#define NEMUNET_IPV4MASK_DEFAULT      "255.255.255.0"

#define NEMUNET_MAX_SHORT_NAME 50
...
typedef struct NETIFINFO
{
    ...
    RTUUID         Uuid;
    char           szShortName[NEMUNET_MAX_SHORT_NAME];
    char           szName[1];
} NETIFINFO;
...
int NetIfEnableStaticIpConfig(VirtualBox *pNemu, HostNetworkInterface * pIf, ULONG aOldIp, ULONG aNewIp, ULONG aMask);
int NetIfEnableStaticIpConfigV6(VirtualBox *pNemu, HostNetworkInterface * pIf, IN_BSTR aOldIPV6Address, IN_BSTR aIPV6Address, ULONG aIPV6MaskPrefixLength);
int NetIfEnableDynamicIpConfig(VirtualBox *pNemu, HostNetworkInterface * pIf);
int NetIfCreateHostOnlyNetworkInterface (VirtualBox *pNemu, IHostNetworkInterface **aHostNetworkInterface, IProgress **aProgress, const char *pcszName = NULL);
int NetIfRemoveHostOnlyNetworkInterface (VirtualBox *pNemu, IN_GUID aId, IProgress **aProgress);
...
int NetIfDhcpRediscover(VirtualBox *pNemu, HostNetworkInterface * pIf);
...
DECLINLINE(Bstr) getDefaultIPv4Address(Bstr bstrIfName)
{
    ...
    tmp.S_un.S_addr = NEMUNET_IPV4ADDR_DEFAULT + (iInstance << 16);
#else
    tmp.s_addr = NEMUNET_IPV4ADDR_DEFAULT + (iInstance << 16);
#endif
