/** $Id: NemuClientClipboardHostToGuest.cpp $ */
/** @file
 * NemuClient - Shared Clipboard Host -> Guest copying, Darwin.
 */
...
static int vbclClipboardGuestPasteData(PasteboardRef pPasteboard, UInt8 *pData, CFIndex cbDataSize, CFStringRef sFormat, bool fClear)
{
    ...
    if ((textData = CFDataCreate(kCFAllocatorDefault, pData, cbDataSize)))
    {
        ...
        if (rc != noErr)
        {
            NemuClientVerbose(3, "unable to put data into guest's clipboard: %d\n", rc);
...
static int vbclClipboardGuestPasteText(PasteboardRef pPasteboard, void *pData, uint32_t cbDataSize)
{
    ...
    /* Convert END-OF-LINE */
    rc = nemuClipboardUtf16GetLinSize((RTUTF16 *)pData, cbDataSize / 2, &cbActualLen);
    AssertReturn(RT_SUCCESS(rc), rc);
    pDataInternal = (RTUTF16 *)RTMemAlloc(cbActualLen * 2);
    AssertReturn(pDataInternal, VERR_NO_MEMORY);
    rc = nemuClipboardUtf16WinToLin((RTUTF16 *)pData, cbDataSize / 2, pDataInternal, cbActualLen);
...
static int vbclClipboardGuestPastePicture(PasteboardRef pPasteboard, void *pData, uint32_t cbDataSize)
{
    ...
    rc = nemuClipboardDibToBmp(pData, cbDataSize, &pBmp, &cbBmpSize);
    AssertReturn(RT_SUCCESS(rc), rc);
...
int vbclClipboardForwardToGuest(uint32_t u32ClientId, PasteboardRef pPasteboard, uint32_t fFormats)
{
    ...
    {
        if (fFormatsInternal & NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT)
        {
            NemuClientVerbose(3, "found NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT: %d\n", fFormatsInternal);

            rc = vbclClipboardReadHostData(u32ClientId, NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT, &pData, &cbDataSize, &cbMemSize);
            if (RT_SUCCESS(rc))
            {
            ...
            fFormatsInternal &= ~((uint32_t)NEMU_SHARED_CLIPBOARD_FMT_UNICODETEXT);
        }

        else if (fFormatsInternal & NEMU_SHARED_CLIPBOARD_FMT_BITMAP)
        {
            NemuClientVerbose(3, "found NEMU_SHARED_CLIPBOARD_FMT_BITMAP: %d\n", fFormatsInternal);

            rc = vbclClipboardReadHostData(u32ClientId, NEMU_SHARED_CLIPBOARD_FMT_BITMAP, &pData, &cbDataSize, &cbMemSize);
            if (RT_SUCCESS(rc))
            {
                /* Store data in guest buffer */
            ...
            fFormatsInternal &= ~((uint32_t)NEMU_SHARED_CLIPBOARD_FMT_BITMAP);
        }

        else if (fFormatsInternal & NEMU_SHARED_CLIPBOARD_FMT_HTML)
        {
            NemuClientVerbose(3, "found NEMU_SHARED_CLIPBOARD_FMT_HTML: %d\n", fFormatsInternal);

            rc = vbclClipboardReadHostData(u32ClientId, NEMU_SHARED_CLIPBOARD_FMT_HTML, &pData, &cbDataSize, &cbMemSize);
            if (RT_SUCCESS(rc))
            {
                /* Store data in guest buffer */
            ...
            fFormatsInternal &= ~((uint32_t)NEMU_SHARED_CLIPBOARD_FMT_HTML);
        }

        else
        {
            NemuClientVerbose(3, "received data in unsupported format: %d\n", fFormats);
            break;
        }
    }
