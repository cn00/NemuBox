...
/** @file
 * Nemu crOpenGL: Window to FBO redirect support.
 */
...
static int crServerRedirMuralDbSyncFb(CRMuralInfo *mural, HCR_FRAMEBUFFER hFb, CR_FBDATA **ppData)
{
    ...
    const struct NEMUVR_SCR_COMPOSITOR* pCompositor = CrFbGetCompositor(hFb);
    ...
    *ppData = NULL;
    ...
    NemuRectTranslate(&FbRect, pScreenInfo->i32OriginX, pScreenInfo->i32OriginY);

    NemuRectIntersect(&FbRect, &MuralRect);

    if (NemuRectIsZero(&FbRect))
        return VINF_SUCCESS;
    ...
    const struct NEMUVR_SCR_COMPOSITOR_ENTRY* pCEntry = CrFbEntryGetCompositorEntry(pData->hFbEntry);
...
static void crNemuServerMuralFbCleanCB(unsigned long key, void *data1, void *data2)
{
    CRMuralInfo *pMI = (CRMuralInfo*) data1;
...
static void crNemuServerMuralFbSetCB(unsigned long key, void *data1, void *data2)
{
    CRMuralInfo *pMI = (CRMuralInfo*) data1;
    HCR_FRAMEBUFFER hFb = (HCR_FRAMEBUFFER)data2;
    uint32_t i;
...
{
    crHashtableWalk(cr_server.muralTable, crNemuServerMuralFbSetCB, hFb);
}

void crNemuServerMuralFbResizeBegin(HCR_FRAMEBUFFER hFb)
{
    crHashtableWalk(cr_server.muralTable, crNemuServerMuralFbCleanCB, hFb);
}

DECLEXPORT(int) crNemuServerNotifyResize(const struct VBVAINFOSCREEN *pScreen, void *pvVRAM)
{
    if (cr_server.fCrCmdEnabled)
    {
        WARN(("crNemuServerNotifyResize for enabled CrCmd"));
...
DECLEXPORT(void) crServerNemuCompositionSetEnableStateGlobal(GLboolean fEnable)
{
}

DECLEXPORT(void) crServerNemuScreenshotRelease(CR_SCREENSHOT *pScreenshot)
{
...
DECLEXPORT(int) crServerNemuScreenshotGet(uint32_t u32Screen, uint32_t width, uint32_t height, uint32_t pitch, void *pvBuffer, CR_SCREENSHOT *pScreenshot)
{
            ...
            || pScreen->u16BitsPerPixel != 32)
    {
        ...
        if (!RT_SUCCESS(rc))
        {
            ...
            crServerNemuScreenshotRelease(pScreenshot);
            return rc;
        }
    }
    else
