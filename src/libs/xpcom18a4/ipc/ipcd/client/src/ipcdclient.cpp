...
# define NEMU_WITH_IPCCLIENT_RW_CS
#endif
...
public:
  ...
  ~ipcClientState()
  {
#ifndef NEMU_WITH_IPCCLIENT_RW_CS
    if (monitor)
      nsAutoMonitor::DestroyMonitor(monitor);
#else
  ...
  //
  PRMonitor    *monitor;
#else  /* NEMU_WITH_IPCCLIENT_RW_CS */
  RTCRITSECTRW  critSect;
#endif /* NEMU_WITH_IPCCLIENT_RW_CS */
...
private:
  ...
  ipcClientState()
    ...
    , selfID(0)
  {
#ifdef NEMU_WITH_IPCCLIENT_RW_CS
      ...
      case IPCM_MSG_PSH_CLIENT_STATE:
      {
                 ...
                 status->ClientState() == IPCM_CLIENT_STATE_UP)
        {
          ...
          if (data->senderID == IPC_SENDER_ANY)
          {
            ...
            /* NemuSVC/NemuXPCOMIPCD auto-start can cause that a client up
