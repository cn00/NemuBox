...
# ifdef NEMU_WITH_CRDUMPER_THUMBNAIL
...
#define NEMU_WITH_CONFIGURABLE_HIDPI_SCALING    1
 ...
 */
static NSOpenGLPixelFormat *nemuCreatePixelFormat(GLbitfield fVisParams)
{
    ...
    if (fVisParams & VMSVGA3D_NON_DEFAULT_PROFILE_BIT)
        attribs[1] = NEMU_VMSVGA3D_DEFAULT_OGL_PROFILE >= 3.2 ? NSOpenGLProfileVersionLegacy :  NSOpenGLProfileVersion3_2Core;
    else
        attribs[1] = NEMU_VMSVGA3D_DEFAULT_OGL_PROFILE >= 3.2 ? NSOpenGLProfileVersion3_2Core : NSOpenGLProfileVersionLegacy;
#endif
    ...
    if (NemuOglIsOfflineRenderingAppropriate())
    {
...
static NSOpenGLContext *nemuCtxGetCurrent(void)
{
#ifdef IN_VMSVGA3D
...
static bool nemuCtxSyncCurrentInfo(void)
{
#ifdef IN_VMSVGA3D
    return false;
#else
...
/** 
 * State carrying structure for use with nemuCtxEnter and nemuCtxLeave 
 */ 
typedef struct NEMU_CR_RENDER_CTX_INFO
{
    bool                fIsValid;
    NSOpenGLContext    *pCtx;
    NSView             *pView;
} NEMU_CR_RENDER_CTX_INFO;
/** Pointer to render context info for use with nemuCtxEnter/Leave. */
typedef NEMU_CR_RENDER_CTX_INFO *PNEMU_CR_RENDER_CTX_INFO;

static void nemuCtxEnter(NSOpenGLContext *pNewCtx, PNEMU_CR_RENDER_CTX_INFO pCtxInfo)
{
    NSOpenGLContext *pOldCtx  = nemuCtxGetCurrent(); 
        ...
        || pOldView != pNewView)
    {
        ...
        /** @todo r=bird: Why do we save the NEW VIEW here? nemuCtxLeave calls it 'pOldView'. Bug? */
...
static void nemuCtxLeave(PNEMU_CR_RENDER_CTX_INFO pCtxInfo)
{
    if (pCtxInfo->fIsValid)
    {
        ...
        if (pOldCtx != nil)
        {
            /* nemuCtxEnter saves the new view, not the old. So, what we actually
               do here is switching the view of the old context to that of the new
               one (wrt nemuCtxEnter) before making it current. */
 ...
 * Abstrack task class.
 */
@interface NemuTask : NSObject
{
}
- (void)run;
@end

@implementation NemuTask
 ...
 */
@interface NemuTaskPerformSelector : NemuTask
{
@private
    id  m_Object;
    SEL m_Selector;
...
@implementation NemuTaskPerformSelector
    
/**
 * Initializes a NemuTaskPerformSelector.
 *  
 ...
 * 
 */
@interface NemuTaskComposite : NemuTask
{
@private
...
- (id)init;
- (void)add:(NemuTask *)pTask;
- (void)run;
- (void)dealloc;
@end

@implementation NemuTaskComposite
- (id)init
{
    self = [super init];
...
- (void)run
{
    for (;;)
    {
        ...
        NemuTask *pTask = (NemuTask *)[m_pArray objectAtIndex:m_CurIndex];
        Assert(pTask != nil);
 ...
 * 
 */
@interface NemuMainThreadTaskRunner : NSObject
{
@private
    NemuTaskComposite *m_pTasks;
}
- (id)init;
- (void)add:(NemuTask *)pTask;
...
- (void)dealloc;
+ (NemuMainThreadTaskRunner *) globalInstance;
@end

@implementation NemuMainThreadTaskRunner
- (id)init
{
    self = [super init];
    if (self)
    {
        m_pTasks = [[NemuTaskComposite alloc] init];
    }
...
+ (NemuMainThreadTaskRunner *) globalInstance
{
    ...
    static NemuMainThreadTaskRunner *s_pRunner = nil;
    dispatch_once(&s_DispatchOnce, ^{
        s_pRunner = [[NemuMainThreadTaskRunner alloc] init];
    });
    return s_pRunner;
}
...
- (void)addObj:(id)aObject selector:(SEL)aSelector arg:(id)aArg
{
    NemuTaskPerformSelector *pSelTask = [[NemuTaskPerformSelector alloc] initWithObject:aObject selector:aSelector arg:aArg];
    [self add:pSelTask];
    [pSelTask release];
}
 ...
 * @param   pvUser      The NemuMainThreadTaskRunner singleton. 
 */ 
static DECLCALLBACK(void) NemuMainThreadTaskRunner_RcdRunCallback(void *pvUser)
{
    DEBUG_FUNC_ENTER();
    NemuMainThreadTaskRunner *pRunner = (NemuMainThreadTaskRunner *)pvUser;
    Assert(pRunner == [NemuMainThreadTaskRunner globalInstance]);
    [pRunner runTasks];
    DEBUG_FUNC_LEAVE();
}
...
- (bool)runTasksSyncIfPossible
{
    ...
    if (renderspuCalloutAvailable())
    {
        ...
        renderspuCalloutClient(NemuMainThreadTaskRunner_RcdRunCallback, self);
        return true;
    }
#endif
...
- (void)nemuSetPos:(NSPoint)pos;
- (void)nemuSetPosUI:(NSPoint)pos;
- (void)nemuSetPosUIObj:(NSValue *)pPos;
- (NSPoint)pos;
- (bool)isEverSized;
- (void)nemuDestroy;
- (void)nemuSetSizeUI:(NSSize)size;
- (void)nemuSetSizeUIObj:(NSValue *)pSize;
- (void)nemuSetSize:(NSSize)size;
- (NSSize)size;
- (void)updateViewportCS;
#ifdef NEMU_WITH_CONFIGURABLE_HIDPI_SCALING
- (NSRect)safeConvertRectToBacking:(NSRect *)pRect;
- (CGFloat)safeGetBackingScaleFactor;
#endif
- (NSRect)safeConvertToScreen:(NSRect *)pRect;
- (void)nemuReshapePerform;
- (void)nemuReshapeOnResizePerform;
- (void)nemuReshapeOnReparentPerform;
...
- (void)makeCurrentFBO;
- (void)swapFBO;
- (void)nemuSetVisible:(GLboolean)fVisible;
- (void)nemuSetVisibleUIObj:(NSNumber *)pVisible;
- (void)nemuSetVisibleUI:(GLboolean)fVisible;
- (void)nemuTryDraw;
- (void)nemuTryDrawUI;
- (void)nemuReparent:(NSView *)pParentView;
- (void)nemuReparentUI:(NSView *)pParentView;
- (void)nemuPresent:(const NEMUVR_SCR_COMPOSITOR *)pCompositor;
- (void)nemuPresentCS:(const NEMUVR_SCR_COMPOSITOR *)pCompositor;
#ifndef IN_VMSVGA3D
- (void)nemuPresentToDockTileCS:(const NEMUVR_SCR_COMPOSITOR *)pCompositor;
#endif
- (void)nemuPresentToViewCS:(const NEMUVR_SCR_COMPOSITOR *)pCompositor;
- (void)presentComposition:(const NEMUVR_SCR_COMPOSITOR_ENTRY *)pChangedEntry;
#ifndef IN_VMSVGA3D
- (void)nemuBlitterSyncWindow;
#endif

- (void)clearVisibleRegions;
- (void)setVisibleRegions:(GLint)cRects paRects:(const GLint *)paRects;
- (GLboolean)nemuNeedsEmptyPresent;
...
- (void)parentWindowFrameChanged:(NSNotification *)pNote
{
     ...
     */ 
    /*
    [m_pOverlayView performSelector:@selector(nemuReshapePerform) onThread:m_Thread withObject:nil waitUntilDone:YES];
    */

    if ([m_pOverlayView isEverSized])
    {    
        if ([NSThread isMainThread])
            [m_pOverlayView nemuReshapePerform];
        else
            [self performSelectorOnMainThread:@selector(nemuReshapePerform) withObject:nil waitUntilDone:NO];
    }
...
- (void)parentWindowChanged:(NSWindow *)pWindow
{
    ...
    if (pWindow != nil)
    {
         ...
         */
        /*
        [m_pOverlayView performSelector:@selector(nemuReshapePerform) withObject:nil afterDelay:0.2];
        [NSTimer scheduledTimerWithTimeInterval:0.2 target:m_pOverlayView selector:@selector(nemuReshapePerform) userInfo:nil repeats:NO];
        */
         ...
         fVisParams:(GLbitfield) fVisParams
{
    ...
    self = [super initWithFrame:frame];
#if defined(NEMU_WITH_CONFIGURABLE_HIDPI_SCALING) && !defined(IN_VMSVGA3D)
...
- (void)nemuSetPosUI:(NSPoint)pos
{
    ...
    DEBUG_MSG(("nemuSetPosUI: [%d, %d].\n", (int)pos.x, (int)pos.y));

    m_Pos = pos;

    if (m_fEverSized)
        [self nemuReshapePerform];
...
- (void)nemuSetPos:(NSPoint)pos
{
    ...
    NemuMainThreadTaskRunner *pRunner = [NemuMainThreadTaskRunner globalInstance];
    NSValue *pPos =  [NSValue valueWithPoint:pos];
    [pRunner addObj:self selector:@selector(nemuSetPosUIObj:) arg:pPos];
...
- (void)nemuSetSizeUI:(NSSize)size
{
    ...
    DEBUG_MSG(("OVIW(%p): nemuSetSize: new size: %dx%d\n", (void *)self, (int)m_Size.width, (int)m_Size.height));
    [self nemuReshapeOnResizePerform];
...
- (void)nemuSetSize:(NSSize)size
{
    ...
    [pRunner addObj:self selector:@selector(nemuSetSizeUIObj:) arg:pSize];
...
- (void)nemuReshapePerform
{
    ...
    DEBUG_MSG(("nemuReshapePerform: a new overlay frame [%d, %d, %dx%d] has been constructed from intersection of window frame "
    ...
    DEBUG_MSG(("nemuReshapePerform: [%#p]: m_RootRect pos[%d : %d] size[%d : %d]\n",
    ...
    if (m_pSharedGLCtx)
    {
        NEMU_CR_RENDER_CTX_INFO CtxInfo; 
        nemuCtxEnter(m_pSharedGLCtx, &CtxInfo);

        [self updateViewportCS];

        nemuCtxLeave(&CtxInfo);
    }
...
- (void)nemuTryDraw
{
    ...
    [self performSelectorOnMainThread:@selector(nemuTryDrawUI) withObject:nil waitUntilDone:NO];
...
- (void)nemuSetVisible:(GLboolean)fVisible
{
    ...
    [pRunner addObj:self selector:@selector(nemuSetVisibleUIObj:) arg:pVisObj];
...
- (void)nemuSetVisibleUIObj:(NSNumber *)pVisibleObj
{
    ...
    [self nemuSetVisibleUI:fVisible];
...
- (void)nemuReparent:(NSView *)pParentView
{
    ...
    NemuMainThreadTaskRunner *pRunner = [NemuMainThreadTaskRunner globalInstance];
    [pRunner addObj:self selector:@selector(nemuReparentUI:) arg:pParentView];
...
- (void)nemuReparentUI:(NSView *)pParentView
{
    ...
    if (pParentView != nil)
    {
        ...
        if ([self isEverSized])
            [self nemuReshapeOnReparentPerform];
    }
...
- (void)nemuTryDrawUI
{
    ...
    if (!m_pSharedGLCtx)
    {
        ...
        if (![self nemuSharedCtxCreate])
        {
            COCOA_LOG_FLOW(("%s: returns - nemuSharedCtxCreate failed\n", __PRETTY_FUNCTION__));
            return;
        }
    ...
    const NEMUVR_SCR_COMPOSITOR *pCompositor = NULL;
#ifndef IN_VMSVGA3D
    int rc = renderspuNemuCompositorLock(m_pWinInfo, &pCompositor);
    if (RT_FAILURE(rc))
    {
        COCOA_LOG_FLOW(("%s: returns - renderspuNemuCompositorLock failed (%Rrc)\n", __PRETTY_FUNCTION__, rc));
        return;
    }

    if (!pCompositor && !m_fCleanupNeeded)
    {
        renderspuNemuCompositorUnlock(m_pWinInfo);
        COCOA_LOG_FLOW(("%s: returns - noCompositorUI\n", __PRETTY_FUNCTION__));
        return;
    }

    NEMUVR_SCR_COMPOSITOR TmpCompositor;
    if (pCompositor)
    {
        if (!m_pSharedGLCtx)
        {
            Assert(!m_fDataVisible);
            Assert(!m_fCleanupNeeded);
            renderspuNemuCompositorRelease(m_pWinInfo);
            if (![self nemuSharedCtxCreate])
            {
                COCOA_LOG_FLOW(("%s: returns - nemuSharedCtxCreate failed\n", __PRETTY_FUNCTION__));
                return;
            ...
            pCompositor = renderspuNemuCompositorAcquire(m_pWinInfo);
            Assert(!m_fDataVisible);
            Assert(!m_fCleanupNeeded);
            if (!pCompositor)
            {
    ...
    {
        COCOA_LOG_FLOW(("%s: Calling nemuPresent\n", __PRETTY_FUNCTION__));
        [self nemuPresent:pCompositor];            
        [self unlockFocus];
    }
    ...
    else
    {
        ...
        [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(nemuTryDrawUI) userInfo:nil repeats:NO];
    }
...
- (void)nemuPresent:(PCNEMUVR_SCR_COMPOSITOR)pCompositor
{
    ...
    [self nemuPresentCS:pCompositor];
    
    nemuCtxLeave(&CtxInfo);
    COCOA_LOG_FLOW(("%s: returns\n", __PRETTY_FUNCTION__));
}

- (void)nemuPresentCS:(PCNEMUVR_SCR_COMPOSITOR)pCompositor
{
    COCOA_LOG_FLOW(("%s: self=%p pCompositor=%p\n", __PRETTY_FUNCTION__, (void *)self, (void *)pCompositor));
    if ([m_pSharedGLCtx view] != self)
    {
    ...
    [self nemuPresentToDockTileCS:pCompositor];
#endif

    /* change to #if 0 to see thumbnail image */            
#if 1
    [self nemuPresentToViewCS:pCompositor];
#else
...
DECLINLINE(void) nemuNSRectToRect(const NSRect *pR, RTRECT *pRect)
{
...
DECLINLINE(void) nemuNSRectToRectUnstretched(const NSRect *pR, RTRECT *pRect, float xStretch, float yStretch)
{
...
DECLINLINE(void) nemuNSRectToRectStretched(const NSRect *pR, RTRECT *pRect, float xStretch, float yStretch)
{
    pRect->xLeft   = (int)(pR->origin.x * xStretch);
    pRect->yTop    = (int)(pR->origin.y * yStretch);
    pRect->xRight  = (int)((pR->origin.x + pR->size.width) * xStretch);
...
- (void)nemuPresentToViewCS:(PCNEMUVR_SCR_COMPOSITOR)pCompositor
{
    NSRect r = [self frame];
...
#  if defined(NEMU_WITH_CONFIGURABLE_HIDPI_SCALING) && !defined(IN_VMSVGA3D)
    ...
    //crDebug("HiDPI: nemuPresentToViewCS: up-scaling is %s (backingStretchFactor=%d).",
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            {                   
                ...
                for (i = 0; i < cRegions; ++i)
                {
                    ...
                    nemuNSRectToRect(&m_RootRect, &RestrictDstRect);
                    NemuRectIntersected(&RestrictDstRect, pDstRect, &DstRect);
                    
                    if (NemuRectIsZero(&DstRect))
                        continue;

                    NemuRectTranslate(&DstRect, -RestrictDstRect.xLeft, -RestrictDstRect.yTop);

                    nemuNSRectToRectUnstretched(&m_RootRect, &RestrictSrcRect, xStretch / backingStretchFactor, yStretch / backingStretchFactor);
                    NemuRectTranslate(&RestrictSrcRect, 
                                      -CrVrScrCompositorEntryRectGet(pEntry)->xLeft, 
                                      -CrVrScrCompositorEntryRectGet(pEntry)->yTop);
                    NemuRectIntersected(&RestrictSrcRect, pSrcRect, &SrcRect);
                    
                    if (NemuRectIsZero(&SrcRect))
                        continue;
...
- (void)presentComposition:(PCNEMUVR_SCR_COMPOSITOR_ENTRY)pChangedEntry
{
...
#ifndef IN_VMSVGA3D
# ifdef NEMU_WITH_CRDUMPER_THUMBNAIL
static int g_cNemuTgaCtr = 0;
# endif
- (void)nemuPresentToDockTileCS:(PCNEMUVR_SCR_COMPOSITOR)pCompositor
{
    COCOA_LOG_FLOW(("%s: self=%p pCompositor=%p\n", __PRETTY_FUNCTION__, (void *)self, (void *)pCompositor));
    NSRect      r   = [self frame];
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    {                   
                        ...
                        for (i = 0; i < cRegions; ++i)
                        {
                            ...
                            NemuRectScale(&DstRect, m_FBOThumbScaleX, m_FBOThumbScaleY);
                    
                            if (NemuRectIsZero(&DstRect))
                                continue;
                        
                            nemuNSRectToRectUnstretched(&m_RootRect, &RestrictSrcRect, xStretch, yStretch);
                            NemuRectTranslate(&RestrictSrcRect, 
                                              -CrVrScrCompositorEntryRectGet(pEntry)->xLeft, 
                                              -CrVrScrCompositorEntryRectGet(pEntry)->yTop);
                            NemuRectIntersected(&RestrictSrcRect, pSrcRect, &SrcRect);
...
# ifdef NEMU_WITH_CRDUMPER_THUMBNAIL
            ...
            crDumpNamedTGAF((GLint)rr.size.width, (GLint)rr.size.height, 
                [[m_DockTileView thumbBitmap] bitmapData], "/Users/leo/nemudumps/dump%d.tga", g_cNemuTgaCtr);
# endif                
...
- (NSView *)dockTileScreen
{
    ...
    /* 
     * If it fails, fall back to the old variant (Nemu...).
     */
...
void cocoaGLCtxCreate(NativeNSOpenGLContextRef *ppCtx, GLbitfield fVisParams, NativeNSOpenGLContextRef pSharedCtx)
{
    ...
    NSOpenGLPixelFormat *pFmt = nemuCreatePixelFormat(fVisParams);
    if (pFmt)
    {
...
*
********************************************************************************/
static OverlayView *nemuViewCreate(WindowInfo *pWinInfo, NativeNSViewRef pParentView, GLbitfield fVisParams)
{
    COCOA_LOG_FLOW(("nemuViewCreate: pWinInfo=%p pParentView=%p fVisParams=%#x\n", pWinInfo, (void *)pParentView, fVisParams));
    ...
    if (pView)
    {
        ...
        COCOA_LOG_FLOW(("nemuViewCreate: returns %p\n", (void *)pView));
        return pView;
    }
    
    COCOA_LOG_FLOW(("nemuViewCreate: returns NULL\n"));
    return NULL;
}
...
static DECLCALLBACK(void) nemuRcdCreateView(void *pvCb)
{
    CR_RCD_CREATEVIEW *pCreateView = (CR_RCD_CREATEVIEW *)pvCb;
    pCreateView->pView = nemuViewCreate(pCreateView->pWinInfo, pCreateView->pParentView, pCreateView->fVisParams);
    COCOA_LOG_FLOW(("nemuRcdCreateView: returns pView=%p\n", (void *)pCreateView->pView));
}
...
void cocoaViewCreate(NativeNSViewRef *ppView, WindowInfo *pWinInfo, NativeNSViewRef pParentView, GLbitfield fVisParams)
{
    ...
    if (renderspuCalloutAvailable())
    {
        ...
        renderspuCalloutClient(nemuRcdCreateView, &CreateView);
...
#endif
            *ppView = nemuViewCreate(pWinInfo, pParentView, fVisParams);
#if 0
        });
#endif
    }
...
void cocoaViewReparent(NativeNSViewRef pView, NativeNSViewRef pParentView)
{
    ...
    if (pOView)
        [pOView nemuReparent:pParentView];
...
void cocoaViewDestroy(NativeNSViewRef pView)
{
    ...
    NemuMainThreadTaskRunner *pRunner = [NemuMainThreadTaskRunner globalInstance];
    [pRunner addObj:pView selector:@selector(nemuDestroy) arg:nil];
...
void cocoaViewShow(NativeNSViewRef pView, GLboolean fShowIt)
{
    ...
    [(OverlayView *)pView nemuSetVisible:fShowIt];
...
void cocoaViewSetPosition(NativeNSViewRef pView, NativeNSViewRef pParentView, int x, int y)
{
    ...
    [(OverlayView *)pView nemuSetPos:NSMakePoint(x, y)];
...
void cocoaViewSetSize(NativeNSViewRef pView, int cx, int cy)
{
    ...
    [pOverlayView nemuSetSize:NSMakeSize(cx, cy)];
...
void cocoaViewGetGeometry(NativeNSViewRef pView, int *px, int *py, int *pcx, int *pcy)
{
    ...
    if (renderspuCalloutAvailable())
    {
        ...
        renderspuCalloutClient(nemuRcdGetGeomerty, &GetGeometry);
...
void cocoaViewPresentComposition(NativeNSViewRef pView, PCNEMUVR_SCR_COMPOSITOR_ENTRY pChangedEntry)
{
...
GLboolean cocoaViewNeedsEmptyPresent(NativeNSViewRef pView)
{
    ...
    GLboolean fNeedsPresent = [(OverlayView *)pView nemuNeedsEmptyPresent];
