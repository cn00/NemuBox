...
#ifdef NEMU_WITH_SYS_V_IPC_SESSION_WATCHER
# include <errno.h>
# include <sys/types.h>
# include <sys/stat.h>
# include <sys/ipc.h>
...
#elif defined(NEMU_WITH_SYS_V_IPC_SESSION_WATCHER)
    if (mClientToken >= 0)
        ::semctl(mClientToken, 0, IPC_RMID);
# ifdef NEMU_WITH_NEW_SYS_V_KEYGEN
    mClientTokenId = "0";
# endif /* NEMU_WITH_NEW_SYS_V_KEYGEN */
#elif defined(NEMU_WITH_GENERIC_SESSION_WATCHER)
...
#elif defined(RT_OS_OS2)
    NOREF(pSessionMachine);
    Utf8Str ipcSem = Utf8StrFmt("\\SEM32\\NEMU\\VM\\{%RTuuid}",
...
# else /* !NEMU_WITH_NEW_SYS_V_KEYGEN */
...
# endif /* !NEMU_WITH_NEW_SYS_V_KEYGEN */
    ...
    /* ENOSPC can also be the result of NemuSVC crashes without properly freeing
     * the token */
...
{
#ifdef NEMU_WITH_GENERIC_SESSION_WATCHER
