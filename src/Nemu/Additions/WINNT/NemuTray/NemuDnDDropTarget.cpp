/* $Id: NemuDnDDropTarget.cpp $ */
/** @file
 * NemuDnDTarget.cpp - IDropTarget implementation.
 */
...
NemuDnDDropTarget::NemuDnDDropTarget(NemuDnDWnd *pParent)
    : mRefCount(1),
      mpWndParent(pParent),
      mdwCurEffect(0),
      mpvData(NULL),
...
NemuDnDDropTarget::~NemuDnDDropTarget(void)
{
    reset();
...
STDMETHODIMP_(ULONG) NemuDnDDropTarget::AddRef(void)
{
    return InterlockedIncrement(&mRefCount);
}

STDMETHODIMP_(ULONG) NemuDnDDropTarget::Release(void)
{
...
STDMETHODIMP NemuDnDDropTarget::QueryInterface(REFIID iid, void **ppvObject)
{
...
STDMETHODIMP NemuDnDDropTarget::DragEnter(IDataObject *pDataObject, DWORD grfKeyState,
                                          POINTL pt, DWORD *pdwEffect)
{
    ...
    if (fmtEtc.cfFormat)
    {
        LogFlowFunc(("Found supported format %RI16 (%s)\n",
                     fmtEtc.cfFormat, NemuDnDDataObject::ClipboardFormatToString(fmtEtc.cfFormat)));
         ...
         *       proxy window. */
        *pdwEffect = NemuDnDDropTarget::GetDropEffect(grfKeyState, *pdwEffect);
    }
    else
    {
        ...
        *pdwEffect = DROPEFFECT_NONE;
        ...
        switch (hr)
        {
            ...
            {
                LogRel(("DnD: Drag and drop format is not supported by NemuTray\n"));
                ...
                if (SUCCEEDED(hr2))
                {
                                              ...
                                              NULL /* pceltFetched */) == S_OK)
                    {
                        ...
                        LogRel(("\tcfFormat=%RI16 (%s), tyMed=%RI32, dwAspect=%RI32, strCustomName=%ls, hr=%Rhrc\n",
                                ...
                                NemuDnDDataObject::ClipboardFormatToString(curFormatEtc.cfFormat),
                                curFormatEtc.tymed,
...
STDMETHODIMP NemuDnDDropTarget::DragOver(DWORD grfKeyState, POINTL pt, DWORD *pdwEffect)
{
    AssertPtrReturn(pdwEffect, E_INVALIDARG);
...
STDMETHODIMP NemuDnDDropTarget::DragLeave(void)
{
#ifdef DEBUG_andy
...
STDMETHODIMP NemuDnDDropTarget::Drop(IDataObject *pDataObject,
                                     DWORD grfKeyState, POINTL pt, DWORD *pdwEffect)
{
    ...
    LogFlowFunc(("mFormatEtc.cfFormat=%RI16 (%s), pDataObject=0x%p, grfKeyState=0x%x, x=%ld, y=%ld\n",
                 mFormatEtc.cfFormat, NemuDnDDataObject::ClipboardFormatToString(mFormatEtc.cfFormat),
                ...
                switch (mFormatEtc.cfFormat)
                {
                    ...
                    default:
                        ...
                        AssertMsgFailed(("Format of type %RI16 (%s) not supported\n",
                                         mFormatEtc.cfFormat,
                                         NemuDnDDataObject::ClipboardFormatToString(mFormatEtc.cfFormat)));
    ...
    LogFlowFunc(("Returning with hr=%Rhrc (%Rrc), mFormatEtc.cfFormat=%RI16 (%s), *pdwEffect=%RI32\n",
                 hr, rc, mFormatEtc.cfFormat, NemuDnDDataObject::ClipboardFormatToString(mFormatEtc.cfFormat),
                 *pdwEffect));
...
/* static */
DWORD NemuDnDDropTarget::GetDropEffect(DWORD grfKeyState, DWORD dwAllowedEffects)
{
...
void NemuDnDDropTarget::reset(void)
{
    LogFlowFuncEnter();
...
RTCString NemuDnDDropTarget::Formats(void)
{
    return mFormats;
}

int NemuDnDDropTarget::WaitForDrop(RTMSINTERVAL msTimeout)
{
