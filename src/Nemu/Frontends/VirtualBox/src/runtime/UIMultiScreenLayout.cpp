...
/** @file
 * Nemu Qt GUI - UIMultiScreenLayout class implementation.
 */
...
void UIMultiScreenLayout::update()
{
    ...
    bool fShouldWeAutoMountGuestScreens = gEDataManager->autoMountGuestScreensEnabled(nemuGlobal().managedVMUuid());
    ...
    foreach (int iGuestScreen, m_guestScreens)
    {
        ...
        if (!fValid)
        {
            ...
            iHostScreen = gEDataManager->hostScreenForPassedGuestScreen(iGuestScreen, nemuGlobal().managedVMUuid());
            /* Revalidate: */
        ...
        if (!fValid)
        {
            ...
            QRect geo = gEDataManager->machineWindowGeometry(UIVisualStateType_Normal, iGuestScreen, nemuGlobal().managedVMUuid());
            ...
            if (!geo.isNull())
            {
                ...
                iHostScreen = nemuGlobal().screenNumber(topLeftPosition);
                /* Revalidate: */
                fValid =    iHostScreen >= 0 && iHostScreen < m_cHostScreens /* In the host screen bounds? */
                         && m_screenMap.key(iHostScreen, -1) == -1; /* Not taken already? */
            }
...
{
    m_cHostScreens = nemuGlobal().screenCount();
}
...
void UIMultiScreenLayout::saveScreenMapping()
{
    foreach (const int &iGuestScreen, m_guestScreens)
    {
        ...
        gEDataManager->setHostScreenForPassedGuestScreen(iGuestScreen, iHostScreen, nemuGlobal().managedVMUuid());
    }
}
...
quint64 UIMultiScreenLayout::memoryRequirements(const QMap<int, int> &screenLayout) const
{
    ...
    foreach (int iGuestScreen, m_guestScreens)
    {
        QRect screen;
        if (m_pMachineLogic->visualStateType() == UIVisualStateType_Seamless)
            screen = nemuGlobal().availableGeometry(screenLayout.value(iGuestScreen, 0));
        else
            screen = nemuGlobal().screenGeometry(screenLayout.value(iGuestScreen, 0));
