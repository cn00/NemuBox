...
{
    NemuVideoCmnPortWriteUlong(pCtx->port, offt);
}
 ...
 *                be completed
 */
RTDECL(void) NemuHGSMIHostCmdComplete(PHGSMIHOSTCOMMANDCONTEXT pCtx,
                                      void *pvMem)
{
...
{
    return NemuVideoCmnPortReadUlong(pCtx->port);
}
...
RTDECL(void) NemuHGSMIProcessHostQueue(PHGSMIHOSTCOMMANDCONTEXT pCtx)
{
...
RTDECL(bool) NemuHGSMIIsSupported(void)
{
    uint16_t DispiId;

    NemuVideoCmnPortWriteUshort(VBE_DISPI_IOPORT_INDEX, VBE_DISPI_INDEX_ID);
    NemuVideoCmnPortWriteUshort(VBE_DISPI_IOPORT_DATA, VBE_DISPI_ID_HGSMI);
 ...
 */
RTDECL(void *) NemuHGSMIBufferAlloc(PHGSMIGUESTCOMMANDCONTEXT pCtx,
                                    HGSMISIZE cbData,
                                    uint8_t u8Ch,
                                    uint16_t u16Op)
{
#ifdef NEMU_WDDM_MINIPORT
    return NemuSHGSMIHeapAlloc (&pCtx->heapCtx, cbData, u8Ch, u16Op);
#else
...
/**
 * Free a descriptor allocated by @a NemuHGSMIBufferAlloc.
 *
 * @param  pCtx      the context containing the heap used
 * @param  pvBuffer  the pointer returned by @a NemuHGSMIBufferAlloc
 */
RTDECL(void) NemuHGSMIBufferFree(PHGSMIGUESTCOMMANDCONTEXT pCtx,
                                 void *pvBuffer)
{
#ifdef NEMU_WDDM_MINIPORT
    NemuSHGSMIHeapFree (&pCtx->heapCtx, pvBuffer);
#else
...
/**
 * Submit a command descriptor allocated by @a NemuHGSMIBufferAlloc.
 *
 * @param  pCtx      the context containing the heap used
 * @param  pvBuffer  the pointer returned by @a NemuHGSMIBufferAlloc
 */
RTDECL(int) NemuHGSMIBufferSubmit(PHGSMIGUESTCOMMANDCONTEXT pCtx,
                                  void *pvBuffer)
{
    ...
    if (offBuffer != HGSMIOFFSET_VOID)
    {
        ...
        NemuVideoCmnPortWriteUlong(pCtx->port, offBuffer);
 ...
 * command. */
static int nemuHGSMIReportFlagsLocation(PHGSMIGUESTCOMMANDCONTEXT pCtx,
                                        HGSMIOFFSET offLocation)
{
    ...
    p = (HGSMIBUFFERLOCATION *)NemuHGSMIBufferAlloc(pCtx,
 ...
 */
static int nemuHGSMISendCapsInfo(PHGSMIGUESTCOMMANDCONTEXT pCtx,
                                 uint32_t fCaps)
{
    VBVACAPS *pCaps;
    int rc = VINF_SUCCESS;
    ...
    /* Allocate the IO buffer. */
    pCaps = (VBVACAPS *)NemuHGSMIBufferAlloc(pCtx,
    ...
    if (pCaps)
    {
        ...
        pCaps->fCaps = fCaps;
        rc = NemuHGSMIBufferSubmit(pCtx, pCaps);
        if (RT_SUCCESS(rc))
        {
            AssertRC(pCaps->rc);
            rc = pCaps->rc;
        ...
        /* Free the IO buffer. */
        NemuHGSMIBufferFree(pCtx, pCaps);
    }
    else
        rc = VERR_NO_MEMORY;
    return rc;
 ...
 */
RTDECL(int) NemuHGSMISendCapsInfo(PHGSMIGUESTCOMMANDCONTEXT pCtx,
                                  uint32_t fCaps)
{
    return nemuHGSMISendCapsInfo(pCtx, fCaps);
}


/** Tell the host about the location of the area of VRAM set aside for the host
 * heap. */
static int nemuHGSMIReportHostArea(PHGSMIGUESTCOMMANDCONTEXT pCtx,
                                   uint32_t u32AreaOffset, uint32_t u32AreaSize)
{
    ...
    /* Allocate the IO buffer. */
    p = (VBVAINFOHEAP *)NemuHGSMIBufferAlloc(pCtx,
 ...
 *                              of the host flags
 */
RTDECL(void) NemuHGSMIGetBaseMappingInfo(uint32_t cbVRAM,
 ...
 */
RTDECL(int) NemuHGSMISetupGuestContext(PHGSMIGUESTCOMMANDCONTEXT pCtx,
...
#ifdef NEMU_WDDM_MINIPORT
    return NemuSHGSMIInit(&pCtx->heapCtx, pvGuestHeapMemory,
 ...
 */
RTDECL(void) NemuHGSMIGetHostAreaMapping(PHGSMIGUESTCOMMANDCONTEXT pCtx,
                                         uint32_t cbVRAM,
                                         ...
                                         uint32_t *pcbHostArea)
{
    ...
    NemuQueryConfHGSMI(pCtx, NEMU_VBVA_CONF32_HOST_HEAP_SIZE, &cbHostArea);
    if (cbHostArea != 0)
    {
 ...
 */
RTDECL(void) NemuHGSMISetupHostContext(PHGSMIHOSTCOMMANDCONTEXT pCtx,
                                       void *pvBaseMapping,
 ...
 * @param  cbHostArea            size in bytes of the host heap area
 */
RTDECL(int) NemuHGSMISendHostCtxInfo(PHGSMIGUESTCOMMANDCONTEXT pCtx,
                                     HGSMIOFFSET offVRAMFlagsLocation,
                                     uint32_t fCaps,
                                     uint32_t offVRAMHostArea,
                                     uint32_t cbHostArea)
{
    Log(("NemuVideo::nemuSetupAdapterInfo\n"));

    /* setup the flags first to ensure they are initialized by the time the
     * host heap is ready */
    int rc = nemuHGSMIReportFlagsLocation(pCtx, offVRAMFlagsLocation);
    AssertRC(rc);
    if (RT_SUCCESS(rc) && fCaps)
    {
        /* Inform about caps */
        rc = nemuHGSMISendCapsInfo(pCtx, fCaps);
        AssertRC(rc);
    }
    if (RT_SUCCESS (rc))
    {
        /* Report the host heap location. */
        rc = nemuHGSMIReportHostArea(pCtx, offVRAMHostArea, cbHostArea);
        AssertRC(rc);
    }
    Log(("NemuVideo::nemuSetupAdapterInfo finished rc = %d\n", rc));
    return rc;
}
...
static int testQueryConf(PHGSMIGUESTCOMMANDCONTEXT pCtx)
{
    ...
    if (cOnce)
        return VINF_SUCCESS;
    cOnce = true;
    rc = NemuQueryConfHGSMI(pCtx, UINT32_MAX, &ulValue);
 ...
 */
RTDECL(int) NemuQueryConfHGSMIDef(PHGSMIGUESTCOMMANDCONTEXT pCtx,
                                  uint32_t u32Index, uint32_t u32DefValue, uint32_t *pulValue)
{
    ...
    if (RT_FAILURE(rc))
        return rc;
    /* Allocate the IO buffer. */
    p = (VBVACONF32 *)NemuHGSMIBufferAlloc(pCtx,
...
RTDECL(int) NemuQueryConfHGSMI(PHGSMIGUESTCOMMANDCONTEXT pCtx,
                               uint32_t u32Index, uint32_t *pulValue)
{
    return NemuQueryConfHGSMIDef(pCtx, u32Index, UINT32_MAX, pulValue);
}
 ...
 */
RTDECL(int)  NemuHGSMIUpdatePointerShape(PHGSMIGUESTCOMMANDCONTEXT pCtx,
                                         uint32_t fFlags,
                                         uint32_t cHotX,
                                         uint32_t cHotY,
                                         uint32_t cWidth,
                                         ...
                                         uint32_t cbLength)
{
    ...
    if (fFlags & NEMU_MOUSE_POINTER_SHAPE)
    {
        /* Size of the pointer data: sizeof (AND mask) + sizeof (XOR_MASK) */
        cbData = ((((cWidth + 7) / 8) * cHeight + 3) & ~3)
                 + cWidth * 4 * cHeight;
        /* If shape is supplied, then always create the pointer visible.
         * See comments in 'nemuUpdatePointerShape'
         */
        fFlags |= NEMU_MOUSE_POINTER_VISIBLE;
    }
    ...
    /* Allocate the IO buffer. */
    p = (VBVAMOUSEPOINTERSHAPE *)NemuHGSMIBufferAlloc(pCtx,
    ...
    if (p)
    {
        ...
        p->u32Width  = cWidth;
        p->u32Height = cHeight;
        if (p->fu32Flags & NEMU_MOUSE_POINTER_SHAPE)
 ...
 */
RTDECL(int) NemuHGSMICursorPosition(PHGSMIGUESTCOMMANDCONTEXT pCtx, bool fReportPosition, uint32_t x, uint32_t y,
                                    uint32_t *pxHost, uint32_t *pyHost)
{
    ...
    /* Allocate the IO buffer. */
    p = (VBVACURSORPOSITION *)NemuHGSMIBufferAlloc(pCtx, sizeof(VBVACURSORPOSITION), HGSMI_CH_VBVA, VBVA_CURSOR_POSITION);
    if (p)
    {
        /* Prepare data to be sent to the host. */
